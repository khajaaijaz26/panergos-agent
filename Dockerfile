...
# node:26 image instead (Panergos pins its toolchain to Node 26 everywhere).
...
# Do not write .pyc files at runtime: /opt/panergos is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/panergos/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when panergos
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# panergos process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
db.execute(\"INSERT INTO docs VALUES ('panergos')\"); \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main panergos process, the dashboard,
...
# 'Panergos WebUI' catalog, NAS compose projects that preserve an old
...
# Non-root user for runtime; UID can be overridden via PANERGOS_UID at runtime
RUN useradd -u 10000 -m -d /opt/data panergos
...
WORKDIR /opt/panergos

# ---------- Layer-cached dependency install ----------
# Copy only package manifests first so npm install + Playwright are cached
# unless the lockfiles themselves change.
#
# ui-tui/packages/panergos-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/panergos-ink/ ui-tui/packages/panergos-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as panergos-ink above).
...
# lock on the @panergos/ink entry, tripped the TUI launcher's
...
#
# The [otlp] extra contains the SDK/exporter imported by Panergos when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Panergos production image.
#
# The hindsight memory provider's client (hindsight-client) is baked in
# for the same reason: it lazy-installs into /opt/panergos/.venv at first
...
# PANERGOS_LAZY_INSTALL_TARGET when the extra is not present.
#
...
# gives the non-root panergos user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link panergos-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/panergos are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the panergos user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/panergos/bin && \
    cp /opt/panergos/docker/panergos-exec-shim.sh /opt/panergos/bin/panergos && \
    chmod 0755 /opt/panergos /opt/panergos/bin/panergos && \
    printf 'docker\n' > /opt/panergos/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $PANERGOS_HOME. $PANERGOS_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``panergos update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the panergos user via
# `s6-setuidgid panergos` in its run script. If PANERGOS_UID is unset, services
# run as the default panergos user (UID 10000).
...
# lives outside both /opt/panergos (which operators sometimes bind-mount as a
# checkout) and /opt/data (the mutable PANERGOS_HOME volume).
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `panergos dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the PANERGOS_GIT_SHA build-arg to
# /opt/panergos/.panergos_build_sha at build time, and have
# panergos_cli/build_info.py read it at runtime.  Both `panergos dump` and
...
ARG PANERGOS_GIT_SHA=
RUN set -eu; \
    if [ -n "${PANERGOS_GIT_SHA}" ]; then \
        printf '%s\n' "${PANERGOS_GIT_SHA}" > /opt/panergos/.panergos_build_sha; \
    fi; \
    mkdir -p /etc/panergos; \
    PANERGOS_GIT_SHA="${PANERGOS_GIT_SHA}" python3 -c 'import json, os, pathlib, runpy; distribution = runpy.run_path("/opt/panergos/panergos_cli/__init__.py"); marker = pathlib.Path("/etc/panergos/image-provenance.json"); marker.write_text(json.dumps({"schema": 1, "deployment_kind": "image", "manager": "docker", "image": distribution["__distribution_repo_slug__"], "version": distribution["__distribution_version__"], "revision": os.environ.get("PANERGOS_GIT_SHA") or None}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8"); marker.chmod(0o444)'

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-panergos + dashboard.
...
# `exec panergos`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $PANERGOS_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/panergos/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-panergos-setup && \
    chmod +x /etc/cont-init.d/01-panergos-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV PANERGOS_WEB_DIST=/opt/panergos/panergos_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/panergos/ui-tui/dist/entry.js`) and skip the
...
ENV PANERGOS_TUI_DIR=/opt/panergos/ui-tui
ENV PANERGOS_HOME=/opt/data
ENV PANERGOS_WRITE_SAFE_ROOT=/opt/data
ENV PANERGOS_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/panergos (root-owned, read-only) so a runtime
...
# is seeded + chowned to the panergos user by docker/stage2-hook.sh and lives
...
ENV PANERGOS_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> panergos ...` they default to root, and any file the
# command writes under $PANERGOS_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/panergos/bin/panergos, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid panergos`
# when invoked as root. Non-root callers (supervised processes,
# `--user panergos`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/panergos/.venv/bin/panergos). See the shim source for
# the opt-out env var (PANERGOS_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/panergos-exec-shim.sh /opt/panergos/bin/panergos
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/panergos/docker/entrypoint-dispatch.sh

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> panergos ...` and any
# subprocess that doesn't activate the venv first still find panergos.
#
# /opt/panergos/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/panergos/bin:/opt/panergos/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. panergos
# subcommand vs. no-args), drops to the panergos user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/panergos/docker/entrypoint-dispatch.sh" ]
CMD [ ]
