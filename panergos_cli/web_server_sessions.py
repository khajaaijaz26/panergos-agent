...
def _session_db_read_probe_statements() -> tuple:
    ...
    from panergos_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from panergos_state import SessionDB, is_malformed_schema_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    ``panergos serve`` runs neither CLI nor gateway startup hooks, so this
...
def _maybe_run_skill_maintenance(started_at: float) -> None:
    ...
    if _check_gateway_running(get_panergos_home()):
        return
