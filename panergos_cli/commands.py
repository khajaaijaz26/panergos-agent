"""Slash command registry for the Panergos CLI and gateway.
...
from panergos_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Key in ``panergos_cli.slash_exec.EXECUTORS`` (a string, not a callable: keeps this module
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of panergos config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Panergos works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .panergos/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey Panergos' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]", subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Panergos is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.panergos/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Panergos Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Panergos Agent version", "Info", aliases=("v",),
               busy_policy="dispatch", execute="version"),
    CommandDef("debug", "Create a local debug report (system info + logs)", "Info",
               args_hint="[local]"),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``panergos skills list`` and the agent's ``/skill-name`` dispatch but
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'discord_skill_commands_by_category': ('panergos_cli.commands_platforms', 'discord_skill_commands_by_category'),
    'slack_app_manifest': ('panergos_cli.commands_platforms', 'slack_app_manifest'),
    'slack_native_slashes': ('panergos_cli.commands_platforms', 'slack_native_slashes'),
    'slack_subcommand_map': ('panergos_cli.commands_platforms', 'slack_subcommand_map'),
    'telegram_bot_commands': ('panergos_cli.commands_platforms', 'telegram_bot_commands'),
    'telegram_menu_commands': ('panergos_cli.commands_platforms', 'telegram_menu_commands'),
    'telegram_menu_max_commands': ('panergos_cli.commands_platforms', 'telegram_menu_max_commands'),
}
