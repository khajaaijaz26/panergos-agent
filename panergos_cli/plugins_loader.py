...
dependency/config-schema warnings. Mixed into :class:`panergos_cli.plugins.PluginManager`.

Origin-internal names (``PluginContext``, ``LoadedPlugin``, ``_PLUGINS_DEBUG`` …) are imported lazily
through ``panergos_cli.plugins`` so tests that patch them on the origin keep working.
"""
...
from panergos_constants import get_panergos_home, reset_panergos_home_override, set_panergos_home_override
from registration_lifecycle import replacement_coordinator
from panergos_cli.plugins_discovery import ENTRY_POINTS_GROUP, _select_entry_point_group
from panergos_cli.plugins_manifest import PluginManifest, manifest_key, validate_config_schema
from panergos_cli.plugins_state import _plugin_settings_entry

if TYPE_CHECKING:  # pragma: no cover
    from panergos_cli.plugins import LoadedPlugin

logger = logging.getLogger("panergos_cli.plugins")

_NS_PARENT = "panergos_plugins"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Panergos home."""
...
class PluginLoaderMixin:
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``panergos plugins list`` until then."""
        from panergos_cli.plugins import LoadedPlugin
    ...
    def _register_deferred_platform_tools(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        would miss them in ``panergos tools`` / ``platform_toolsets``. Opt-in is explicit via ``provides_tools``;
        ...
        returns ``[]``, the toolset is missing from the ``panergos tools`` checklist, and even an explicit
        ...
        """
        from panergos_cli.plugins import PluginContext, _PLUGINS_DEBUG
        ...
        try:
            ...
            # Tools registered before the raise are live: credit them or `panergos plugins list` under-reports
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        See #64165.
        python_dependencies is a declaration seam ONLY: Panergos validates and prints the requirements with an
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Panergos does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from panergos_cli.plugins import LoadedPlugin, PluginContext, _PLUGINS_DEBUG
        ...
        if manifest.portable:
            ...
            return
        # requires_panergos gate: skip cleanly (no import, no traceback) on a version mismatch.
        from panergos_cli.plugins_manifest import requires_panergos_error
        reason = requires_panergos_error(manifest)
        if reason:
            loaded.error = reason
        ...
        # skipped with a clear reason instead of dying on ImportError mid-register (panergos_cli.plugin_compat).
        from panergos_cli.plugin_compat import disable_reason
        ...
        try:
            ...
            if register_fn is None:
                ...
                loaded.enabled = True
                from panergos_cli.plugins_ledger import _hook_source_of
    ...
    def _track_tool_override_policy(self, manifest: PluginManifest, module_name: str) -> None:
        ...
        from panergos_cli.plugins import PluginContext
    ...
    def _attribute_registrations(
        ...
        # Discovery-time tools predate registration_start; credit them back or `panergos plugins list`
    ...
    def _load_portable_plugin(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        lookup_key = manifest_key(manifest)
        try:
            from panergos_cli.agent_plugins import load_agent_plugin
            package = load_agent_plugin(
                Path(manifest.path), get_panergos_home() / "plugin-data" / manifest.skill_namespace)
    ...
    def _directory_module_name(self, manifest: PluginManifest) -> str:
        """Profile-safe import namespace for a directory plugin: the bare ``panergos_plugins.<slug>`` for the
    ...
    def _load_directory_module(
        ...
        """Import a directory plugin as ``panergos_plugins.<slug>`` (slug from ``manifest.key`` so
        ...
        # Evict stale entries for this slug (same slug cached from another Panergos home, or an earlier force
