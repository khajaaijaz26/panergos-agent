...
:mod:`panergos_cli.runtime_provider`; origin-internal collaborators
...
from panergos_cli.providers import custom_provider_aliases, custom_provider_slug
from agent.secret_scope import get_secret_str
from utils import base_url_hostname

logger = logging.getLogger("panergos_cli.runtime_provider")

_LLAMACPP_ALIASES = ("llamacpp", "llama.cpp", "llama-cpp")


def _rp():
    """Origin module, late-bound so test patches on ``panergos_cli.runtime_provider.*`` apply."""
    import panergos_cli.runtime_provider as origin
    return origin
...
def _shadowed_by_builtin(requested_norm: str) -> bool:
    ...
    canonical provider; an entry matching merely an alias (``kimi`` → ``kimi-coding``)
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(config.get("custom_providers"), dict):
        logger.warning("custom_providers in config.yaml is a dict, not a list. "
                       ...
                       "Run 'panergos doctor' for details.")
        return None
...
def canonical_custom_identity(*, base_url: Optional[str] = None, config_provider: Optional[str] = None,
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("PANERGOS_INFERENCE_PROVIDER", "").strip()
...
def _resolve_named_custom_runtime(*, requested_provider: str, explicit_api_key: Optional[str] = None,
    ...
    if family is not None and not custom_provider.get("api_mode"):
        from panergos_cli.models import normalize_opencode_base_url, opencode_model_api_mode
