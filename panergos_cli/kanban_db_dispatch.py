...
Split out of ``panergos_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from panergos_cli.kanban_db import Task
...
# ``PANERGOS_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a panergos profile. Expected steady-state
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``panergos kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``panergos_cli.profiles.profile_exists``, or ``None`` when it cannot be
    imported (local import avoids a cycle; callers fall back to trusting the
    assignee)."""
    try:
        from panergos_cli.profiles import profile_exists
    except Exception:
        return None
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Panergos profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Panergos ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``PANERGOS_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``panergos -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``panergos kanban tail`` shows why the task looks stuck.
...
def _module_panergos_argv() -> list[str]:
    """Interpreter-bound Panergos CLI invocation (``panergos_cli.main`` is the
    console-script target — there is no top-level ``panergos`` package)."""
    return [sys.executable, "-m", "panergos_cli.main"]


def _absolute_panergos_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Panergos shim."""
...
def _panergos_path_argv(path: str) -> list[str]:
    """argv for a resolved Panergos executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_panergos_argv()
    return [_absolute_panergos_path(path)]


def _resolve_panergos_argv() -> list[str]:
    """Resolve the ``panergos`` invocation as argv for ``Popen``: ``$PANERGOS_BIN``
    (path-like -> absolute; bare names keep PATH semantics, never a
    same-directory file), then ``which("panergos")`` (Windows: safe PATH search,
    batch shims fall back to the module form), then ``sys.executable -m
    panergos_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_panergos_bin``; local because
    ``panergos_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("PANERGOS_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _panergos_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _panergos_path_argv(resolved_env_bin)
        return _module_panergos_argv()

    which = _safe_which_no_cwd if _kb._IS_WINDOWS else shutil.which
    panergos_bin = which("panergos")
    if panergos_bin:
        return _panergos_path_argv(panergos_bin)
    return _module_panergos_argv()
...
def _resolve_worker_cli_toolsets(panergos_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``PANERGOS_KANBAN_TASK`` is set.
    """
    ...
    try:
        ...
        token = set_panergos_home_override(panergos_home)
        ...
        secret_token = (
            set_secret_scope(build_profile_secret_scope(Path(panergos_home)))
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for PANERGOS_HOME=%r (%s)",
            panergos_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, panergos_home: Optional[str]) -> list[str]:
    """Build the ``panergos -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_panergos_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped PANERGOS_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(panergos_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `panergos kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``panergos -p <profile> chat -q ...`` subprocess.
    ...
    ``PANERGOS_KANBAN_DB`` / ``PANERGOS_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from panergos_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    try:
        ...
        # PANERGOS_PROFILE (set below) instead.
        profile_home = None
    ...
    # Inject PANERGOS_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_panergos_home() falls back to the DEFAULT
    # profile root because `panergos -p` applies its override before
    # panergos_constants is imported.
    if profile_home:
        env["PANERGOS_HOME"] = profile_home
    ...
    if task.tenant:
        env["PANERGOS_TENANT"] = task.tenant
    env["PANERGOS_KANBAN_TASK"] = task.id
    env["PANERGOS_KANBAN_WORKSPACE"] = workspace
    ...
    env["PANERGOS_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["PANERGOS_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["PANERGOS_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["PANERGOS_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["PANERGOS_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["PANERGOS_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `panergos -p` rewrites PANERGOS_HOME (symlink / Docker layouts).
    env["PANERGOS_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["PANERGOS_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["PANERGOS_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["PANERGOS_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads PANERGOS_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["PANERGOS_PROFILE"] = profile_arg
    ...
    # `--cli` is the highest-precedence TUI override; dropping PANERGOS_TUI covers
    # older panergos builds on PATH that predate the flag's precedence.
    env.pop("PANERGOS_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("PANERGOS_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`panergos` executable not found on PATH. "
            "Install Panergos Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``panergos kanban dispatch`` — the standalone
...
from panergos_cli import kanban_db as _kb  # noqa: E402
from panergos_cli import kanban_db_connect as _kbc  # noqa: E402
from panergos_cli import kanban_db_workspace as _kbw  # noqa: E402
