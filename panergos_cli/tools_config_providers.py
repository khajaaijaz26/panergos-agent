"""Provider rows, active-provider detection, model pickers and provider-selection persistence for `panergos tools`."""
...
from panergos_cli.config import cfg_get, get_env_value, load_config, save_config, save_env_value
...
def _visible_providers(
    cat: dict, config: dict, *, force_fresh: bool = False, features=None,
) -> list[dict]:
    """Return direct/local provider entries for this category."""
...
def provider_readiness_status(provider: dict, config: dict, *, features=None, is_active: Optional[bool] = None) -> str:
    ...
    ``is_active`` is the completed-setup fallback for post-setup hooks with no installed check."""
    from panergos_cli.tools_config import (
        _POST_SETUP_READY, _provider_env_ready, _xai_credentials_present,
    )
...
def _toolset_needs_configuration_prompt(ts_key: str, config: dict, *, force_fresh: bool = False) -> bool:
    ...
    from panergos_cli.tools_config import TOOL_CATEGORIES, _post_setup_already_installed, _toolset_has_keys
...
def _configure_tool_category(ts_key: str, cat: dict, config: dict, *, force_fresh: bool = True, reconfigure: bool = False):
    ...
    ``reconfigure`` ("Reconfigure an existing tool"): no setup note or skip row, and the
    chosen provider goes through the key-update prompts instead of the new-enable prompts."""
    from panergos_cli.tools_config import _prompt_choice, _provider_env_ready
...
def _detect_active_provider_index(providers: list, config: dict, *, force_fresh: bool = False) -> int:
    ...
    from panergos_cli.tools_config import _provider_env_ready
...
def _pick_model_from_catalog(
    ...
    selection."""
    from panergos_cli.tools_config import _cfg_section, _prompt_choice
...
def _select_into(config: dict, section: str, key: str, vendor) -> dict:
    """Write a direct provider selection and discard the obsolete gateway flag."""
    from panergos_cli.tools_config import _cfg_section
...
def _select_plugin_gen_provider(section: str, plugin_name: str, config: dict) -> None:
    """Persist a plugin-backed image/video provider selection and run its model picker."""
...
def _write_provider_config(provider: dict, config: dict) -> None:
    ...
    Pure, non-interactive core shared by the CLI and GUI provider endpoints."""
    ...
    if "browser_provider" in provider:
        ...
        if bp:
            ...
            _select_into(config, "browser", "cloud_provider", bp)
        else:
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    # Plugin image/video gen backends record the provider name in their own section. FAL records an explicit
...
def _configure_vision_backend() -> None:
    ...
    Offers any authenticated provider + model (same surface as ``panergos model``) or a custom endpoint
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    Rows come from ``build_aux_picker_rows()`` so this lists exactly what the ``panergos model`` aux-task
    picker lists, including user-defined ``providers:`` / ``custom_providers:`` endpoints. Persists
    ``auxiliary.vision.provider`` + ``.model``."""
    from panergos_cli.tools_config import _prompt_choice
    ...
    if not providers:
        _print_warning("  No authenticated providers found. Configure a provider first "
                       "with `panergos model`, then re-run this.")
        return
...
def _configure_simple_requirements(ts_key: str, *, reconfigure: bool = False):
    ...
    from panergos_cli.tools_config import TOOLSET_ENV_REQUIREMENTS, _toolset_has_keys, _toolset_label
