"""``panergos approvals suggest`` — mine approval history into allowlist proposals.

Panergos has no dedicated approval-decision ledger: ``always`` answers land in ``command_allowlist``
...
def normalize_command(command: str) -> str:
    """Fold user/panergos home prefixes and collapse whitespace."""
    from tools.approval_detection import _rewrite_resolved_panergos_home, _rewrite_resolved_user_home
    return " ".join(_rewrite_resolved_user_home(_rewrite_resolved_panergos_home(command)).split())
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  panergos approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.panergos/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``panergos approvals suggest``."""
    ...
    if apply_spec:
        ...
        if as_json:
            ...
            for pattern in applied:
                print(f"  + {pattern}")
            print(f"\ncommand_allowlist now has {len(merged)} entries (~/.panergos/config.yaml).")
        return 0
...
def approvals_command(args) -> int:
    """Dispatch ``panergos approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from panergos_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: panergos approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `panergos approvals <subcommand> -h` for details."
    )
    return 1
