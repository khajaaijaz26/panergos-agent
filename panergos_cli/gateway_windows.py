...
_TASK_NAME_DEFAULT = "Panergos_Gateway"
_TASK_DESCRIPTION = "Panergos Agent Gateway - Messaging Platform Integration"
_TASK_LOGON_DELAY = "PT30S"
_TASK_RESTART_INTERVAL = "PT1M"
_TASK_RESTART_COUNT = 999

_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("PANERGOS_GATEWAY_DETACHED", "1"), ("PANERGOS_SUPERVISED_CHILD", "1"))
...
def _panergos_home() -> Path:
    from panergos_cli.config import get_panergos_home

    return Path(get_panergos_home())


def _preserve_panergos_home_path(path: str | Path) -> str:
    r"""Render Panergos-owned paths under the configured PANERGOS_HOME spelling.

    ``%LOCALAPPDATA%\panergos`` may be a symlink/junction to another drive; launcher files must not
    bake in the resolved target for paths under PANERGOS_HOME.
    """
    candidate = Path(path)
    try:
        home = _panergos_home()
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Panergos profile."""
    from panergos_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "panergos_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"PANERGOS_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "PANERGOS_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "PANERGOS_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from panergos_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<PANERGOS_HOME>/gateway-service/`` (per-profile
    installs stay self-contained); the VBS launcher lives beside it."""
    _assert_windows()
    script_dir = _panergos_home() / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at PANERGOS_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m panergos_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
    argv = [python_exe, "-m", "panergos_cli.main"]
    if profile_arg:
...
def _launcher_settings() -> tuple[str, str, str, str]:
    """Return (python_path, working_dir, panergos_home, profile_arg) for generated launchers."""
    from panergos_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init

    panergos_home = str(_panergos_home())
    return (
        _preserve_panergos_home_path(get_python_path()),
        _stable_gateway_working_dir(PROJECT_ROOT),
        panergos_home,
        _profile_arg(panergos_home),
    )


def _launcher_pythonpath_entries(extra_pythonpath: list[str]) -> list[str]:
    return [
        _preserve_panergos_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_panergos_home_path(entry) for entry in extra_pythonpath],
    ]


def _build_gateway_cmd_script(python_path: str, working_dir: str, panergos_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f'set "PANERGOS_HOME={panergos_home}"',
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        # VIRTUAL_ENV lets the gateway's own python detection find the venv.
        f'set "VIRTUAL_ENV={_preserve_panergos_home_path(venv_dir)}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, panergos_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('PANERGOS_HOME')}) = {q(panergos_home)}",
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"env.Item({q('VIRTUAL_ENV')}) = {q(_preserve_panergos_home_path(venv_dir))}",
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from panergos_cli.gateway import PROJECT_ROOT

    python_path, working_dir, panergos_home, profile_arg = _launcher_settings()
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    env_overlay = {"PANERGOS_HOME": panergos_home, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_panergos_home_path(venv_dir)}
    _prepend_pythonpath(env_overlay, [_preserve_panergos_home_path(p) for p in (PROJECT_ROOT, *extra_pythonpath)])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (PANERGOS_HOME,
    ...
    try:
        panergos_home = str(_panergos_home().resolve())
    except Exception:
        panergos_home = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "PANERGOS_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if panergos_home:
        env_overlay["PANERGOS_HOME"] = panergos_home
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("PANERGOS_GATEWAY_INSTALL_START_NOW")
    if start_on_login is None:
        start_on_login = _install_choice_from_env("PANERGOS_GATEWAY_INSTALL_START_ON_LOGIN")
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids or start_now:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"panergos {profile_arg} gateway start" if profile_arg else "panergos gateway start"
...
def _offer_elevated_install(headline: str, force: bool, start_now: bool, start_on_login: bool) -> bool:
    ...
    if prompt_yes_no("  Open the UAC prompt now?", False):
        if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
            print("✓ Launched elevated panergos gateway install prompt.")
            if start_now:
                print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
            else:
                print("  Approve the Windows UAC prompt, then run: panergos gateway status")
            return True
...
def install(
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: panergos gateway start")
        return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: panergos gateway start")
        _print_next_steps()
        return
...
def _start_attestation_path() -> Path:
    return _panergos_home().joinpath(*_START_ATTESTATION_RELATIVE)
...
def _write_start_attestation(pids: list[int], via: str) -> None:
    ...
    newer one written by a concurrent ``panergos gateway start`` (#110020 review)."""
    try:
        path = _start_attestation_path()
        path.parent.mkdir(parents=True, exist_ok=True)
        from panergos_cli.process_identity import _process_create_time
...
# meant to bridge the seconds between a ✓ and the next ``panergos gateway status``/``update``; a
...
def _attested_pid_exited_cleanly(pid: int, create_time: float | None = None) -> bool:
    ...
    try:
        ...
        data = json.loads(get_lifecycle_sentinel_path(_panergos_home()).read_text(encoding="utf-8"))
    except OSError:
        return False
    except Exception:
...
def attested_death_generation(current_pids: list[int]) -> str | None:
    ...
    one-shot marker — ``panergos update`` consults it to decide whether a Desktop-owned install
...
def _format_attestation_warning(attested: list[int], data: dict) -> str:
    ...
    lines = [
        ...
        "shutdown record.",
        "  This usually means the shell that ran `panergos gateway start` was inside "
...
def _print_next_steps() -> None:
    ...
    print(f"  type {_panergos_home()}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_gateway_command("uninstall"):
                    print("✓ Launched elevated panergos gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: panergos gateway status")
                    return
