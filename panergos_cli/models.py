...
``panergos_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from panergos_cli.urllib_security import open_credentialed_url
from panergos_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from panergos_cli.models_reasoning_caps import (
...
_PANERGOS_USER_AGENT = f"panergos-agent/{_PANERGOS_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from panergos_cli.config import get_custom_provider_tls_settings
...
def _openrouter_catalog_disk_ttl() -> float:
    ...
    from panergos_cli.model_catalog import refresh_interval_seconds
...
def _openrouter_catalog_disk_path() -> Path:
    ...
    return get_panergos_home() / "cache" / "openrouter_curated_catalog.json"
...
def _pricing_profile_key() -> str:
    ...
    return panergos_home_key()


def _extract_model_name(entry: Any) -> Optional[str]:
    """Pull a ``modelName`` field from a catalog entry, else None."""
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from panergos_cli.model_catalog import get_default_model_from_cache
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (panergos-agent is tool-calling-first).
    Permissive when the field is absent/malformed: some OpenRouter-compatible gateways don't
    populate it, and the picker must not silently empty for them.
...
# Reasoning-capability cache slots for OpenRouter. The logic
...
def fetch_openrouter_models(
    ...
    from panergos_cli.models_profile_cache import profile_slot_get, profile_slot_set
    ...
    try:
        from panergos_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — panergos-agent requires it and surfacing
...
def fetch_ai_gateway_models(
    ...
    from panergos_constants import AI_GATEWAY_BASE_URL
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from panergos_cli.auth import get_auth_status, has_usable_secret
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``panergos model`` and ``/model``)."""
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try the live API first when the provider supports /models.
...
def _resolve_static_model_alias(
    ...
    try:
        from panergos_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # Step 0: a bare provider name typed as the model (`/model openrouter`) is a provider switch to that
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if resolved_provider in _PROVIDER_LABELS and default_models and resolved_provider not in current_keys:
            ...
            # so [0] could silently select an unintended flagship.
...
def _resolve_provider_prefix(model_name: str) -> Optional[tuple[str, str]]:
    """Route an explicit ``vendor/model`` prefix (``openrouter/vendor-model``, ``ollama/qwen3.5:4b``) to
    a provider the user defined in ``providers:`` (by raw name or alias) instead of the default.

    an explicit provider/model id should route to the named provider instead of falling
...
def detect_provider_for_model(
    ...
    NAMED the provider (``/model openrouter``), or there is no current provider yet (``auto``) — then the
    first guess is returned so the credential step fails loudly instead of silently ignoring input."""
    from panergos_cli.models_detect import (
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``panergos_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Panergos should expose the /fast toggle for this model."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from panergos_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    def _pool_token() -> str:
        from panergos_cli.auth import read_credential_pool
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from panergos_cli.model_search import model_alias_canonical
...
def _codex_catalog(normalized: str, force_refresh: bool) -> list[str]:
    from panergos_cli.codex_models import get_codex_model_ids
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``panergos model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models Panergos actually ships. (#46850)
    ...
    from panergos_cli.providers import is_official_openai_host
...
# every authed provider's /v1/models). One JSON file at $PANERGOS_HOME/provider_models_cache.json;
...
def _spawn_swr_refresh(cache_key: str, refresh_fn=None) -> None:
    ...
    from panergos_constants import get_panergos_home_override, panergos_home_key
    inflight_key = cache_key if get_panergos_home_override() is None else (panergos_home_key(), cache_key)
...
def _provider_models_cache_path() -> Path:
    from panergos_constants import get_panergos_home
    return get_panergos_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    try:
        ...
        for rel in ("auth.json", "credentials.json"):
            _mtime_part(rel, get_panergos_home() / rel)
    except Exception:
        pass
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``panergos model --refresh``."""
    try:
...
def copilot_default_headers(*, is_agent_turn: bool = True) -> dict[str, str]:
    ...
    try:
        ...
        return {
            ...
            "User-Agent": "PanergosAgent/0.1.0",
...
def opencode_zen_free_headers() -> dict:
    ...
    try:
        from panergos_cli import __distribution_version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    headers: dict[str, str] = {"User-Agent": _PANERGOS_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"panergos-agent/{_PANERGOS_VERSION}"
    ...
    if isinstance(request_headers, dict):
        ...
        from panergos_cli.config import normalize_extra_headers
...
_DEEPINFRA_MODELS_QUERY = "filter=true"
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _PANERGOS_USER_AGENT}
    try:
...
def cached_fetch_api_models(
    ...
    from panergos_cli.model_switch_providers import _NativePickerModelList
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('panergos_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('panergos_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('panergos_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('panergos_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('panergos_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('panergos_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('panergos_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('panergos_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('panergos_cli.models_local', 'fetch_ollama_local_models'),
    'get_pricing_for_provider': ('panergos_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('panergos_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('panergos_cli.models_local', 'lmstudio_model_reasoning_options'),
    'ollama_model_supports_thinking': ('panergos_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('panergos_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('panergos_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('panergos_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('panergos_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('panergos_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('panergos_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('panergos_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('panergos_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'should_use_ollama_native_catalog': ('panergos_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('panergos_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('panergos_cli.models_validate', 'validate_requested_model'),
    'warm_openrouter_reasoning_caps_async': ('panergos_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
