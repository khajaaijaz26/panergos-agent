"""Top-level argparse construction for the panergos CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``panergos_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``PANERGOS_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/panergos_cli/
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``panergos --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``panergos_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    panergos                        Start interactive chat
    panergos chat -q "Hello"        Single query mode
    panergos --tui                  Launch the modern TUI (or set display.interface: tui)
    panergos --cli                  Force the classic REPL (overrides display.interface: tui)
    panergos -c                     Resume the most recent session
    panergos -c "my project"        Resume a session by name (latest in lineage)
    panergos --resume <session_id>  Resume a specific session by ID
    panergos --resume latest        Resume the most recent session (same as -c)
    panergos --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    panergos setup                  Run setup wizard
    panergos logout                 Clear stored authentication
    panergos auth add <provider>    Add a pooled credential
    panergos auth list              List pooled credentials
    panergos auth remove <p> <t>    Remove pooled credential by index, id, or label
    panergos auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    panergos auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    panergos auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
    panergos model                  Select default model
    panergos fallback [list]        Show fallback provider chain
    panergos fallback add           Add a fallback provider (same picker as `panergos model`)
    panergos fallback remove        Remove a fallback provider from the chain
    panergos config                 View configuration
    panergos config edit            Edit config in $EDITOR
    panergos config set model gpt-4 Set a config value
    panergos gateway                Run messaging gateway
    panergos -s panergos-agent-dev,github-auth
    panergos -w                     Start in isolated git worktree
    panergos gateway install        Install gateway background service
    panergos sessions list          List past sessions
    panergos sessions browse        Interactive session picker
    panergos sessions rename ID T   Rename/title a session
    panergos logs                   View agent.log (last 50 lines)
    panergos logs -f                Follow agent.log in real time
    panergos logs errors            View errors.log
    panergos logs --since 1h        Lines from the last hour
    panergos debug share             Upload debug report for support
    panergos console                Open the safe Panergos command console
    panergos update                 Update to latest version
    panergos dashboard              Start web UI dashboard (port 9119)
    panergos dashboard --stop       Stop running dashboard processes
    panergos dashboard --status     List running dashboard processes

For more help on a command:
    panergos <command> --help
"""
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via PANERGOS_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `panergos setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to PANERGOS_ACCEPT_HOOKS=1 or "
    ...
    inherited(parser, "--ignore-user-config", action="store_true", default=False,
              help="Ignore ~/.panergos/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)")
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``panergos -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/panergos_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Panergos Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also PANERGOS_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--ignore-user-config", action="store_true", default=SUPPRESS,
              help="Ignore ~/.panergos/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.")
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Panergos Agent itself.")
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="panergos", description="Panergos Agent - autonomous assistant with tool-calling capabilities",
