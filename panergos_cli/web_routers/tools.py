...
from panergos_cli.web_routers._common import (
    _CONFIG_MUTATION_LOCK, _profile_cli_args, _profile_scope, _spawn_panergos_action,
...
def _terminal_backend_rows() -> List[Dict[str, str]]:
    ...
    from panergos_cli.web_server_profiles import _TERMINAL_BACKENDS
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    legacy FAL rows carry ``imagegen_backend: "fal"``.
    """
    if ts_key == "image_gen":
...
def _toolset_model_catalog(ts_key: str, plugin_name: str):
    ...
    from panergos_cli.tools_config import _plugin_image_gen_catalog, _plugin_video_gen_catalog
...
def _category_providers(ts_key: str, config: dict) -> list:
    ...
    from panergos_cli.tools_config import TOOL_CATEGORIES, _visible_providers
...
async def get_toolsets(profile: Optional[str] = None):
    from panergos_cli.tools_config import (
        ...
        _toolset_configuration_platform, _toolset_has_keys,
        gui_toolset_label)
    from panergos_cli.platforms import platform_label
    ...
    for name, label, desc in toolset_rows:
        ...
        result.append({
            ...
            "configured": _toolset_has_keys(name, config), "tools": tools})
    return result
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # install `panergos tools` runs — otherwise the toggle "saves" but the tool
...
async def select_toolset_provider(
    ...
    ``panergos tools``, so both write identical keys).
    ...
    """
    from panergos_cli.tools_config import apply_provider_selection, web_provider_capabilities
...
async def run_toolset_post_setup(
    ...
    """Spawn ``panergos tools post-setup <key>`` (long-running installs) as a
    background action tailed via ``GET /api/actions/tools-post-setup/status``;
    ``profile`` is threaded so hooks see the drawer's PANERGOS_HOME."""
    from panergos_cli.tools_config import valid_post_setup_keys
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``panergos computer-use permissions grant`` (macOS-only: launches
...
_PLUGIN_COMPAT_LAZY = {
    'LateState': ('panergos_cli.web_deps', 'LateState'),
}
