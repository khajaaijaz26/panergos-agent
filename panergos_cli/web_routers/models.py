...
from panergos_cli.web_models import ModelAssignment, MoaConfigPayload, MoaModelSlot
...
def get_recommended_default_model(provider: str = ""):
    ...
    ``panergos model``'s curation so GUI onboarding lands on a sensible default.
    The provider gets the preferred silent default when its curated list carries it,
    else the first curated model —
    ...
    Response: {"provider", "model", "free_tier": bool | None}; ``model`` may be
    empty (caller degrades gracefully)."""
    slug = (provider or "").strip().lower()

    try:
        from panergos_cli.inventory import build_models_payload, load_picker_context
        from panergos_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    with http_failure("PUT /api/model/moa failed", 500, detail="Failed to save MoA config"):
        from panergos_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    ``~/.panergos/config.yaml`` — applies to **new** sessions only; a running chat
