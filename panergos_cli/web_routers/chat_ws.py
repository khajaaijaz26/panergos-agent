...
from panergos_cli.pty_session import RegistryFull
from panergos_cli.web_deps import LateState, late
from panergos_cli.web_server_chat import (
...
_active_session_file_for_channel = late("_active_session_file_for_channel", "panergos_cli.web_server_chat")
_profile_scope = late("_profile_scope", "panergos_cli.web_server_profiles")
_resolve_chat_argv_async = late("_resolve_chat_argv_async", "panergos_cli.web_server_chat")
_resolve_profile_dir = late("_resolve_profile_dir", "panergos_cli.web_server_profiles")
_ws_auth_reason = late("_ws_auth_reason", "panergos_cli.web_server_chat")
_ws_client_reason = late("_ws_client_reason", "panergos_cli.web_server_chat")
_ws_host_origin_reason = late("_ws_host_origin_reason", "panergos_cli.web_server_chat")
...
def _ws_auth_mode() -> str:
    ...
    from panergos_cli.web_server_chat import _LOOPBACK_HOSTS
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from panergos_cli.console_engine import PanergosConsoleEngine

        engine = PanergosConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
        ...
        try:
            ...
            if command_id == command_generation:
                ...
                await out.error_then_complete(
                    "Command timed out. Panergos Console returned to the prompt.", line, command_id, "timeout",
                )
        except Exception as exc:
            if command_id == command_generation:
                pending_confirmation = None
...
async def pty_ws(ws: WebSocket) -> None:
    from panergos_cli.web_server_chat import PTY_REGISTRY, PtyBridge, PtyUnavailableError, _PTY_BRIDGE_AVAILABLE, _RESIZE_RE
    ...
    # The selected platform bridge dependency is unavailable: say so and close cleanly.
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            "\r\n\x1b[31mChat unavailable: no platform PTY backend is installed.\x1b[0m\r\n"
            "\x1b[33mReinstall the current Panergos package to add pywinpty on Windows "
            "or ptyprocess on POSIX, then restart the dashboard.\x1b[0m\r\n"
        )
        await ws.close(code=1011)
        return
    ...
    if raw_resume and env:
        registry_resume = env.get("PANERGOS_TUI_RESUME") or raw_resume
...
async def gateway_ws(ws: WebSocket) -> None:
    ...
    await handle_ws(
        ws,
        auth_identity=getattr(ws, "_panergos_auth_identity", None),
        subprotocol=getattr(ws, "_panergos_ws_subprotocol", None),
    )


# --- /api/pub + /api/events: the PTY-side tui_gateway.entry opens /api/pub
# (PANERGOS_TUI_SIDECAR_URL from /api/pty's env) and writes every dispatcher emit
