"""Gateway restart/drain, Panergos update and background-action status dashboard routes.

Extracted from ``panergos_cli.web_server``; helpers/state that tests monkeypatch on
...
from panergos_cli.config import format_docker_update_message, recommended_update_command_for_method
from panergos_cli.web_deps import LateState, late
from panergos_cli.web_server_gateway import _ACTION_LOG_FILES
...
detect_install_method = late("detect_install_method", "panergos_cli.config")
get_panergos_home = late("get_panergos_home", "panergos_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "panergos_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "panergos_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "panergos_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "panergos_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import panergos_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== panergos-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Panergos updates are managed outside this dashboard in containerized environments."
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== panergos update started "):
            last_start = index
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``panergos gateway restart`` in the background."""
...
async def gateway_migrate_plan():
    ...
    from panergos_cli.gateway_migrate import build_migration_plan
...
async def gateway_migrate():
    """Run ``panergos gateway migrate --multiplex --yes`` detached; the CLI re-runs the preflight and
    refuses (exit 1 into the action log) when blocked, so the UI should gate on the plan first."""
    from panergos_cli.web_server_gateway import _spawn_panergos_action
    with http_failure("Failed to spawn gateway migrate", 500, "Failed to start gateway migration"):
        proc = _spawn_panergos_action(["gateway", "migrate", "--multiplex", "--yes"], "gateway-migrate")
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    _record_completed_action("panergos-update", message, exit_code=1)
    return {
        "ok": False, "pid": None, "name": "panergos-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/panergos/update")
async def update_panergos():
    """Kick off ``panergos update`` in the background."""
    ...
    existing = _ACTION_PROCS.get("panergos-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "panergos-update", "already_running": True}
        action_id = _ACTION_IDS.get("panergos-update")
        if action_id:
            response["action_id"] = action_id
        return response

    action_id = secrets.token_hex(16)
    with http_failure("Failed to spawn Panergos update", 500, "Failed to start update"):
        proc = _spawn_panergos_action(["update"], "panergos-update", env_overrides={"PANERGOS_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "panergos-update", "action_id": action_id}


_NON_APPLYABLE_MESSAGES = {
    "docker": format_docker_update_message,
    "apt": lambda: "Panergos is managed by Termux APT; run `pkg upgrade panergos-agent`.",
}


@router.get("/api/panergos/update/check")
async def check_panergos_update(force: bool = False):
    """Report whether a Panergos update is available, without applying it.
    ...
    try:
        from panergos_cli.banner import check_for_updates, upstream_commits_behind

        if force:
            with contextlib.suppress(OSError):
                (get_panergos_home() / ".update_check").unlink()
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "panergos-update":
        ...
        if durable_update_action_id:
            marker = f"=== panergos-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``panergos update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``panergos update`` run since #91283, including
...
@status_router.get("/api/panergos/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `panergos update` run recorded).")
