...
Shared helpers are reached via the late-binding seam in :mod:`panergos_cli.web_deps`
...
from panergos_cli.config import get_process_panergos_home
...
from panergos_cli.web_server_gateway import _strip_session_list_rows
...
from panergos_cli.web_models import (
    ...
    SessionPrScanBody)
from panergos_cli.web_server_profiles import _panergos_home_scope
...
_cron_profile_home = late("_cron_profile_home", "panergos_cli.web_server_cron")
...
def _profile_setup_command(name: str) -> str:
    ...
    return "panergos setup" if name == "default" else f"{name} setup"


def _write_profile_model(profile_dir: Path, provider: str, model: str, validate_in: Optional[Path] = None) -> None:
    """Write the main model assignment into ``profile_dir``'s config.yaml (PANERGOS_HOME-scoped)
    ...
    with _panergos_home_scope(validate_in or profile_dir):
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    from panergos_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_errors(log_msg: str, *args, not_found=(FileNotFoundError,),
    ...
    """Map panergos_cli.profiles exceptions to HTTP: ``not_found`` -> 404, ``bad_request`` -> 400
...
def get_profiles_projects_tree(preview_limit: int = 3, session_limit: int = 2000):
    ...
    for name, home in _profile_targets("GET /api/profiles/projects/tree", lightweight=False):
        def _read(db, name=name, home=home):
            with _panergos_home_scope(home):
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    def _set_model() -> bool:
        ...
        try:
            _write_profile_model(path, provider, model, validate_in=get_process_panergos_home())
    ...
    def _spawn_install(ident: str):
        return _spawn_panergos_action(["-p", body.name, "skills", "install", ident, "--yes"],
...
async def get_active_profile_endpoint():
    """``active`` is the sticky default written by ``panergos profile use`` (what new CLI
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``panergos profile use``); does not retarget the
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    own profile — ``POST /api/model/set`` (main scope) via the PANERGOS_HOME override."""
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    """Auto-generate a profile's description via the auxiliary LLM (mirrors ``panergos profile
...
# ── Export / Import ── wraps panergos_cli.profiles.export_profile / import_profile. Paths are
