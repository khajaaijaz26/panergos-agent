...
_channel_managed_env_keys = late("_channel_managed_env_keys", "panergos_cli.web_server_messaging")
...
async def update_config(
    ...
    def _run():
        ...
        # different PANERGOS_HOME than this process's gateway sessions.
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (the ``panergos model`` universe), so the desktop Keys tab renders a card even
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # by the SAME provider identity the CLI `panergos model` picker uses.
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    rather than the process-level PANERGOS_HOME (mirrors ``/api/config``).
    """
