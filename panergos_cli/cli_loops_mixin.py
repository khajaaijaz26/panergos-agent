...
Mixin on ``PanergosCLI``; cli.py symbols are imported lazily inside methods (import cycle)."""
...
class CLILoopsMixin:
    ...
    def _cmd_version(self, cmd_original: str):
        from panergos_cli.main import _print_version_info
        _print_version_info(check_updates=True)

    def _cmd_reload(self, cmd_original: str):
        from panergos_cli.config import reload_env
        count = reload_env()
    ...
    def _cmd_plugins(self, cmd_original: str):
        ...
        try:
            # Discover from disk (bundled + user) like `panergos plugins list`, so
            ...
            # on one line (full catalog behind `panergos plugins list`).
            user_entries = [e for e in entries if e[3] != "bundled"]
            bundled_count = len(entries) - len(user_entries)
            if not user_entries:
                print("No user plugins installed.")
                print("  Install one: panergos plugins install owner/repo")
                print(f"  Or drop a plugin directory into {display_panergos_home()}/plugins/")
                if bundled_count:
                    print(f"  ({bundled_count} bundled plugins available — see: panergos plugins list)")
                return
            ...
            if bundled_count:
                print(f"  (+{bundled_count} bundled — see: panergos plugins list)")
            print("  Enable/disable: panergos plugins enable/disable <name>")
    ...
    def _get_heartbeat_manager(self):
        ...
        def load():
            from panergos_cli.heartbeat import HeartbeatManager
    ...
    def _start_heartbeat_watchdog(self):
        ...
        from panergos_cli.heartbeat import POLL_SECONDS
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        _active_deleg = 0
        try:
            from panergos_cli.goals import count_active_delegations, gather_background_processes as _gather_bg
