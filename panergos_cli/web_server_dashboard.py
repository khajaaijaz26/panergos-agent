...
from panergos_cli.config import cfg_get, get_process_panergos_home
from utils import env_var_enabled

# Same logger the code used before extraction (record parity).
_log = logging.getLogger("panergos_cli.web_server")


def _normalise_prefix(raw: Optional[str]) -> str:
    """Normalise an X-Forwarded-Prefix header value (single source of truth lives in
    ``panergos_cli.dashboard_auth.prefix`` so gate, OAuth, cookies and SPA mount agree)."""
    from panergos_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    target palette instead of flashing the bundle's default Panergos Eclipse until
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="panergos-theme-bootstrap">'
                ":root{"
...
_HEADLESS_MSG = (
    "Headless backend (panergos serve): web UI disabled — use "
    "`panergos dashboard` for the browser UI."
)
...
def mount_spa(application: FastAPI):
    ...
    proxy (``X-Forwarded-Prefix: /panergos``) the served index.html is rewritten so absolute
    asset URLs and the runtime ``__PANERGOS_BASE_PATH__`` honour that prefix without a rebuild.
    ...
    """
    from panergos_cli.web_server import WEB_DIST, _DASHBOARD_EMBEDDED_CHAT_ENABLED, app
    from panergos_cli.web_deps import _server

    # `panergos serve` is the headless backend: it must NEVER serve the browser SPA, even if a
    # dist is lying around, so only the JSON-RPC/WS/API surface is reachable.
    if os.environ.get("PANERGOS_SERVE_HEADLESS") == "1":

        @application.get("/{full_path:path}")
        async def no_frontend(full_path: str):
            # Desktop token handshake: the Electron shell boots by fetching `/` and reading
            # ``window.__PANERGOS_SESSION_TOKEN__`` for /api/ws auth. When headless 404'd every
            # path, a renderer whose spawn token no longer matched (e.g. after `panergos update`)
            ...
            if full_path == "" and not gated:
                return HTMLResponse(
                    ...
                    f"window.__PANERGOS_SESSION_TOKEN__={json.dumps(_server()._SESSION_TOKEN)};"
                    "window.__PANERGOS_AUTH_REQUIRED__=false;"
    ...
    # A missing WEB_DIST is deliberately NOT a mount-time terminal state (#82614): a long-lived `panergos
    ...
    def _serve_index(prefix: str = ""):
        ...
        cookie auth; ``__PANERGOS_AUTH_REQUIRED__`` tells it which scheme to use for /api/pty
        ...
        token_js = "" if gated else f'window.__PANERGOS_SESSION_TOKEN__="{_server()._SESSION_TOKEN}";'
        bootstrap_script = (
            f"<script>{token_js}"
            f"window.__PANERGOS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__PANERGOS_BASE_PATH__="{prefix}";'
            f"window.__PANERGOS_AUTH_REQUIRED__={'true' if gated else 'false'};"
            f"</script>"
        )
        if prefix:
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    ~/.panergos/, the same trust level as config.yaml. Empty asset values are dropped so a
    ...
    try:
        ...
        noise = 1.0
    palette = {
        "background": _layer("background", "#120b1f", 1.0),
        "midground": _layer("midground", "#ff6b5e", 1.0),
        "foreground": _layer("foreground", "#ffffff", 0.0),
        "warmGlow": palette_src.get("warmGlow") or data.get("warmGlow") or "rgba(46, 230, 166, 0.2)",
        "noiseOpacity": noise,
    }
...
def _discover_user_themes() -> list:
    ...
    Uses the process launch home, not ``get_panergos_home()``, so a transient profile override
    from embedded chat does not hide themes under the server's own ``PANERGOS_HOME``.
    """
    themes_dir = get_process_panergos_home() / "dashboard-themes"
...
def _dashboard_plugin_search_dirs() -> List[tuple]:
    ...
    When the process itself is profile-scoped (``PANERGOS_HOME=<root>/profiles/<name>``) the
    launch home has no ``plugins/`` — user plugins live in the panergos root — so the default
    ...
    # context-local PANERGOS_HOME override (e.g. embedded /chat under --open-profile). #87197: when the
    # process itself is profile-scoped (``--profile <name>`` sets ``PANERGOS_HOME=<root>/profiles/<name>``),
    # the launch home is the profile directory, which has no ``plugins/`` — user plugins are installed in
    # the panergos root (``~/.panergos/plugins``). Scan the default root as well (``get_default_panergos_root()``
    # unwraps ``<root>/profiles/<name>`` → ``<root>`` and returns a custom ``PANERGOS_HOME`` unchanged when it
    # *is* the root), mirroring how ``panergos_cli.plugins`` resolves plugin install locations. The
    # ``seen_names`` dedupe below keeps profile-local plugins (if any) authoritative over same-named root
    # plugins.
    user_plugin_roots = [get_process_panergos_home() / "plugins"]
    root_plugins = get_default_panergos_root() / "plugins"
    ...
    # ``true`` / ``yes`` / ``on``) so the gate matches ``panergos_cli/plugins.py`` and the documented user
    # contract.
    if env_var_enabled("PANERGOS_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".panergos" / "plugins", "project"))
    return search_dirs
...
def _discover_dashboard_plugins() -> list:
    ...
    project plugin sources — same three sources as ``panergos_cli.plugins``."""
    plugins = []
    seen_names: set = set()
    ...
    try:
        ...
        for tname in provides_tools:
            ...
            if cached_result is None:
                ...
                return True, f"panergos auth {name}"
    except Exception:
        pass
    return False, ""
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    """
    from panergos_cli.web_server_memory import _discover_memory_provider_statuses, _normalize_memory_provider_name
    from panergos_cli.web_server import _get_dashboard_plugins
    from panergos_cli.config import get_panergos_home, load_config
    global _plugins_hub_cache, _plugins_hub_cache_expires_at
    now = time.monotonic()
    ...
    from panergos_cli.plugins_cmd import (
    ...
    from panergos_cli.plugins_cmd_catalog import removed_annotation
    ...
    plugins_root_resolved = (get_panergos_home() / "plugins").resolve()
...
def _plugin_api_mount_skip_reason(plugin: Dict[str, Any], enabled_set: set, disabled_set: set) -> Optional[str]:
    ...
    disable; project plugins (``./.panergos/plugins/``) ship with the CWD and are
...
def _mount_plugin_api_routes():
    ...
    (``./.panergos/plugins/``) ship with the CWD and are therefore attacker-controlled in any threat model
    ...
    """
    from panergos_cli.web_server import _get_dashboard_plugins, app
    try:
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.panergos/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"panergos_dashboard_plugin_{plugin['name']}"
