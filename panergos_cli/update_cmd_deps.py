"""Post-``panergos update`` dependency sync: venv preflight, editable reinstall, lazy refresh,
npm/Desktop rebuilds, self-lock deferral. Names are re-imported by ``update_cmd`` (so
``panergos_cli.update_cmd.<name>`` resolves/monkeypatches); origin helpers are imported lazily."""
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    -e .`` always rewrites console-script shims (Windows: ``panergos.exe`` quarantine, ``os error 32``
...
_UPDATE_CRITICAL_MODULES = "panergos_cli.main", "run_agent", "model_tools", "toolsets"
...
def _critical_module_import_failures(
    ...
    """
    from panergos_cli.update_cmd import _UPDATE_CRITICAL_MODULES, _m
    from panergos_constants import FIRST_PARTY_MODULE_ROOTS
    import secrets
    marker = f"__PANERGOS_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        ...
        # only our own packages count. Roots come from panergos_constants so the user hint can't drift.
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('panergos_') or %r:\n"
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``panergos tools``."""
    try:
        from panergos_cli import tools_config
        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active panergos Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    """Restore allowlisted ``panergos tools`` dependencies (from a pre-rebuild probe) into a rebuilt
    ...
    try:
        ...
        logger.debug("panergos Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} panergos Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `panergos update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `panergos update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``plugin.yaml`` (not Panergos extras / ``LAZY_DEPS``), so the core reinstall can strip them;
    ...
    try:
        from panergos_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    from panergos_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _npm_lockfile_changed(panergos_root: Path) -> bool:
    from panergos_cli.update_cmd import _m
    ...
    if (web_dir / "package.json").is_file() and not _web_build_toolchain_ready(
        *_web_toolchain_roots(web_dir)):
        return True
    try:
        cache_file = _npm_lock_cache_file(panergos_root)
...
def _npm_lock_cache_file(panergos_root: Path) -> Path:
...
def _record_npm_lockfile_hash(panergos_root: Path) -> None:
    digest = _npm_manifests_digest()
    if digest is None:
        return
    try:
        _npm_lock_cache_file(panergos_root).write_text(digest, encoding="utf-8")
    except OSError:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    """Repair Node deps on the ``commit_count == 0`` path: a failed npm install says "re-run panergos
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `panergos update`.")
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `panergos update`.")
            has_workspace = any(
    ...
    shared_panergos_root = get_default_panergos_root()
    ...
    # Runs before the lockfile-unchanged early return below since that's the common `panergos update` case.
    # See #43564.
    ...
    if not _m()._npm_lockfile_changed(shared_panergos_root):
    ...
    nixos_env = with_panergos_node_path(_m()._nixos_build_env())
    ...
    # The chatty npm-deprecation noise during `panergos update` comes from the *desktop* build, not this step;
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_panergos_root)
    ...
    print("    deps). Fix npm and re-run `panergos update`.")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (_m().PROJECT_ROOT / ".panergos-bootstrap-complete", _m()._update_marker_path())
...
# canonical case: ``panergos_cli.main`` used to import it at startup while resolving external secret sources;
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    a mapped ``.pyd`` -> exit 2, next launch's marker recovery finishes; the ``panergos.exe`` shim
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `panergos` launch will complete it")
...
def _rebuild_desktop_after_update(
    ...
    desktop_build_cmd = [sys.executable, "-m", "panergos_cli.main", "desktop", "--build-only"]
    # Capture the loud build output into update.log; retry once on failure (still-settling
    # rebuild window), then surface the tail. Put Panergos-managed Node on PATH: the desktop
    # updater chain loses shell PATH customizations, so a bare-PATH child hits `node: not found`.
    from panergos_constants import with_panergos_node_path
    build_env = with_panergos_node_path()
    for _attempt in range(2):
        ...
        if build_result.returncode == 0:
            break
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `panergos desktop` to retry)")
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv touched by ``sudo pip``/``sudo panergos`` dies mid-update with ``venv/bin/panergos`` already
    ...
    See #83529.
    A later normal ``panergos update`` then dies mid-mutation inside ``uv pip install -e .`` ("Permission
    denied (os error 13)") with ``venv/bin/panergos`` already deleted — the CLI is bricked. Same philosophy as
    the contended-venv gate (#87331): a venv we cannot safely mutate is never mutated at all.
    """
    from panergos_cli.update_cmd import _path_uid
    try:
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Panergos broken.")
    print("  This usually happens after running Panergos or pip with sudo. Offending paths:")
    for p, uid in foreign:
        print(f"    - {p} (owner uid {uid})")
    print("\n  Fix ownership, then re-run the update:")
    print(f"    sudo chown -R $(id -un): {project_root}")
    print("    panergos update")
...
def _sync_python_dependencies_after_pull(
    ...
    if _m()._refresh_active_lazy_features(install_prefix, env=lazy_env, features=active_lazy_features):
        ...
        print(
            "  ⚠ Lazy-refresh recovery incomplete — run `panergos` again "
    ...
    if not import_ok:
        ...
        print("    Run `panergos update` again — if it persists, reinstall:")
        print("    https://github.com/khajaaijaz26/panergos-agent")
