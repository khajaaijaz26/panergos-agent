...
# ``panergos auth add openrouter --api-key sk-or-...`` must keep working with no ``--type``.
...
def _anthropic_oauth_login(args) -> dict:
    ...
    creds = anthropic_mod.run_panergos_oauth_login_pure()
    if not creds:
...
def _qwen_oauth_login(args) -> dict:
    from panergos_cli.auth_qwen import _mark_qwen_oauth_active
...
class _OAuthAddSpec:
    """Per-provider parameters for the generic ``panergos auth add <provider> --type oauth`` path."""
...
_OAUTH_ADD_SPECS: dict[str, _OAuthAddSpec] = {
    "anthropic": _OAuthAddSpec(
        ...
        source=f"{SOURCE_MANUAL}:panergos_pkce",
...
def _add_credential(args, provider: str, pool, requested_type: str) -> PooledCredential:
    ...
    if spec is None:
        raise SystemExit(f"`panergos auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def _report_priority(provider: str, pool, moved, requested: int, verb: str, prep: str) -> None:
    ...
    print(f'{verb} {provider} credential "{moved.label}" {prep} priority {moved.priority} '
          f"(#{moved.priority + 1} in `panergos auth list {provider}`)")
...
def auth_priority_command(args) -> None:
    """`panergos auth priority <provider> <target> <priority>`: reorder one pooled credential."""
    provider = _normalize_provider(getattr(args, "provider", ""))
    pool = load_pool(provider)
...
def auth_remove_command(args) -> None:
    ...
    # Every credential source Panergos reads from (env vars, external OAuth files, auth.json blocks,
...
def auth_refresh_command(args) -> None:
    """`panergos auth refresh <provider> [target]`: force one pooled OAuth entry to refresh.
    ...
    if target is None or not str(target).strip():
        if len(entries) != 1:
            raise SystemExit(
                ...
                f"label (see `panergos auth list {provider}`).")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `panergos auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `panergos auth` is called bare."""
...
_AUTH_ACTIONS = {
    ...
    "status": auth_status_command, "logout": auth_logout_command, "spotify": auth_spotify_command,
}
