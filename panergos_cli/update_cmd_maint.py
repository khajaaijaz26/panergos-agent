"""Post-update maintenance for ``panergos update``: pre-update backup snapshot, state-db verify/restore, curator/FTS notices, FHS path guard, completion summary, stale-module purge.

Split out of ``update_cmd.py``, which re-imports every name so ``panergos_cli.update_cmd.<name>``
...
_UPDATE_RUNTIME_RELOAD_MODULES = "panergos_constants", "tools.environments.local", "tools.lazy_deps"
...
_STALE_PURGE_PREFIXES = "panergos_cli", "gateway", "tools", "tui_gateway", "agent"
...
_STALE_PURGE_PROTECTED = frozenset({"panergos_cli", "panergos_cli.main", "panergos_cli.panergos_logging"})
...
_STALE_PURGE_PROTECTED_PREFIX = "panergos_cli.update_"
...
def _purge_stale_panergos_modules() -> None:
    """Evict every cached Panergos module after the checkout changed in-place. Never raises.
    ...
    with _best_effort('Could not purge stale Panergos modules: %s'):
        importlib.invalidate_caches()
        ...
        if purged:
            logger.debug("Purged %d stale Panergos module(s) after checkout update", len(purged))
...
def _print_curator_first_run_notice() -> None:
    ...
    print("  Preview now:  panergos curator run --dry-run")
    print("  Pause it:     panergos curator pause")
    print("  Docs:         https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/features/curator")
...
def _print_fts_optimize_available_notice() -> None:
    ...
    if interrupted:
        ...
        print("    panergos sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  panergos sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    if "\n" in summary:
        ...
        for line in summary.splitlines():
            print(f"  {line}")
        print("  (This message shows once per curator run. View anytime: panergos curator status)")
...
def _reload_process_scan_modules() -> None:
    ...
    ``_scan_dashboard_processes`` does a function-level ``from panergos_cli._subprocess_compat import
    ...
    """
    _reload_modules(
        ("panergos_cli._subprocess_compat", "panergos_cli.dashboard_procs"),
        modules=sys.modules,
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    See #83595.
    """
    from panergos_cli.update_cmd import _m, _reload_process_scan_modules
    if node_failures:
        print()
    ...
    print("    panergos dashboard --port <port>")
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from panergos_cli.update_cmd import _branch_head_suffix
    print(f"{message}{_branch_head_suffix()}")
    action_id = os.environ.get("PANERGOS_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== panergos-update completed {action_id} ===")
...
def _post_update_sqlite_runtime_status():
    ...
    from panergos_cli.update_cmd import _m
    from panergos_constants import project_venv_dir
    from panergos_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Panergos runtime."""
    from panergos_cli.update_cmd import _post_update_sqlite_runtime_status
    ...
    print("  Rebuild the Panergos venv with a uv-managed Python, restart Panergos, then verify with `panergos doctor`.")
    return False
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    See #88251.
    """
    from panergos_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `panergos desktop` to retry the desktop rebuild.")
        if not sqlite_runtime_ok:
            print(
                "  The Python runtime remediation did not complete. Run `panergos "
                "update` again; if SQLite is unchanged, rebuild the Panergos venv "
                "with a uv-managed Python, restart Panergos, then verify with "
                "`panergos doctor`."
            )
    else:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from panergos_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from panergos_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (panergos gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Panergos) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    try:
        from panergos_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    with _best_effort('Sibling-profile state.db guard sweep failed: %s'):
        from panergos_cli.backup import _sibling_profile_homes
...
def _print_bundled_skills_sync_report() -> None:
    ...
    if result.get("user_modified"):
        ...
        print("    → see them: panergos skills list-modified  (diff/reset to resume updates)")
...
def _ensure_fhs_path_guard() -> None:
    ...
    no-op on non-Linux/non-root/non-FHS or when an installed Panergos command resolves."""
    from panergos_cli.update_cmd import _m
    ...
    try:
        ...
        return
    fhs_links = (Path("/usr/local/bin/panergos"),)
    ...
    try:
        probe = subprocess.run(
            ...
            # recovery path in `panergos gateway restart` (`systemd_restart()`) as of PR #20949.
            capture_output=True,
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal the ``panergos-acp`` launcher (mirrors install.sh).

    ACP hosts resolve the launcher on the login-shell PATH but the console script lives in
    the venv. Each shim delegates to the canonical sibling command when available.

    No-op on Windows (install.ps1 stages launchers into ``$PanergosHome\bin``, never
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        commands = (bin_dir / "panergos",)
        main_cmd = next((command for command in commands if command.is_file() or command.is_symlink()), None)
        if main_cmd is None:
            continue
        for launcher_name in ("panergos-acp",):
            acp_cmd = bin_dir / launcher_name
            try:
                # Already present — a console script, an earlier shim, or a symlink.
                if acp_cmd.exists() or acp_cmd.is_symlink():
                    continue
                shim = (
                    "#!/usr/bin/env bash\n"
                    "# Panergos Agent — ACP launcher (written by `panergos update`).\n"
                ...
                continue
            print(f"  ✓ Installed {launcher_name} launcher → {acp_cmd}")
...
def _verify_state_db_after_snapshot(snapshot_id: str) -> None:
    ...
    _src_path = get_panergos_home() / "state.db"
    ...
    _snap_state = _quick_snapshot_root(get_panergos_home()) / snapshot_id / "state.db"
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from panergos_cli.update_cmd import _record_update_step
    ...
    with _best_effort('Sibling profile snapshots failed: %s'):
        from panergos_cli.backup import create_pre_update_snapshots_all_profiles
        ...
        if _sibling_snaps:
            ...
            import panergos_cli.update_cmd_config as _cfg
...
def _run_full_backup() -> None:
    """Zip PANERGOS_HOME under ``backups/`` (restorable via ``panergos import``). Never raises."""
    try:
    ...
    from panergos_cli.sizefmt import format_bytes
    # display_panergos_home so the user sees ~/.panergos/...
    try:
        from panergos_constants import get_panergos_home, display_panergos_home
        display_path = f"{display_panergos_home()}/{out_path.relative_to(get_panergos_home())}"
    except Exception:
        display_path = str(out_path)

    print(f"  Saved:    {display_path} ({format_bytes(size_bytes)}, {elapsed:.1f}s)")
    print(f"  Restore:  panergos import {out_path}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    ``full`` — quick snapshot PLUS a zip of PANERGOS_HOME under ``backups/`` (``panergos import``).
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in panergos_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    # PANERGOS_HOME, so sync_skills()'s module-level PANERGOS_HOME cache can't skew it.
    with suppress(Exception):
        from panergos_cli.profiles import list_profiles, seed_profile_skills
    ...
    with suppress(Exception):
        # See #44792.
        from panergos_cli.profiles import backfill_profile_envs
...
def _refresh_cua_driver_after_update() -> None:
    ...
    if (
        ...
        from panergos_cli.tools_config import install_cua_driver
        print()
...
def _print_plugin_compat_notice() -> None:
    ...
    from panergos_cli.plugin_compat import compat_report, removal_in_effect, summary_lines
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    from panergos_cli.update_cmd import _m, _print_curator_first_run_notice, _print_curator_recent_run_notice
    ...
    def _migrate_windows_bin_path() -> None:
        ...
        from panergos_cli._install_repair import migrate_windows_bin_path
    ...
    for message, step in (
        ...
        ('panergos-acp launcher self-heal failed: %s', _ensure_acp_launcher),
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    from panergos_cli.update_cmd import _check_and_apply_config_migration, _m
    ...
    if sys.platform == "darwin" and had_desktop_app_before_update:
        print()
        print(
            "  ℹ macOS: if Panergos re-prompts for permissions you already "
            "granted (toggle shows ON), the stored grant is stale — run "
            "`tccutil reset ScreenCapture com.khajaaijaz26.panergos` (repeat "
    ...
    try:
        # See #95596.
        from panergos_cli.macos_tcc_anchor import ensure_tcc_anchor
        ensure_tcc_anchor()
    except Exception:
    ...
    with _best_effort('Model catalog seed during update failed: %s'):
        from panergos_cli.model_catalog import seed_cache_from_checkout
