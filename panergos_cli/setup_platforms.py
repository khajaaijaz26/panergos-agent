"""Messaging-platform setup wizards (Telegram, BlueBubbles, webhooks) and the ``panergos setup
gateway`` flow. setup.py re-exports the public names, and tests monkeypatch prompt/print/env
helpers on panergos_cli.setup, so those are imported lazily per function."""
...
def _profile_name_from_panergos_home(panergos_home) -> str | None:
    """Return the active profile name when PANERGOS_HOME is a profile dir."""
    return panergos_home.name if panergos_home.parent.name == "profiles" else None


def _setup_telegram_auto_result():
    """Attempt automatic Telegram bot creation via managed QR onboarding."""
    from panergos_cli.setup import get_panergos_home
    try:
        from panergos_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    profile_name: str | None = None
    with contextlib.suppress(Exception):
        profile_name = _profile_name_from_panergos_home(Path(get_panergos_home()))
...
def declines_reconfigure(label: str, question: str, *env_vars: str) -> bool:
    ...
    """
    from panergos_cli.setup import get_env_value, print_info, prompt_yes_no
...
def save_prompted(env_var: str, question: str, *, password: bool = False, success_msg: str | None = None,
    ...
    """
    from panergos_cli.setup import print_success, print_warning, prompt, save_env_value
...
def _save_allowlist(env_var: str, users: str, success_msg: str) -> None:
    ...
    from panergos_cli.setup import print_success, save_env_value
...
def _prompt_allowlist(env_var: str, question: str, success_msg: str, open_msg: str, preset: str | None = None) -> str:
    ...
    from panergos_cli.setup import print_info, prompt
...
def _save_port(env_var: str, value: str, default: str) -> None:
    ...
    from panergos_cli.setup import print_success, print_warning, save_env_value
    if not value:
        return
    try:
...
def _prompt_telegram_bot_token() -> str | None:
    from panergos_cli.setup import print_error, print_info, prompt
...
def _telegram_allowlist_nudge() -> None:
    ...
    from panergos_cli.setup import get_env_value, print_info, prompt, prompt_yes_no
...
def _obtain_telegram_token():
    """Return (token, setup_result); manual is default and QR is an explicit opt-in."""
    from panergos_cli.setup import _info, print_error, prompt
    _info("How would you like to create your Telegram bot?", None,
          "  [1] Manual (recommended)",
          "      Create a bot via @BotFather and keep the credential flow local.", None,
          "  [2] QR onboarding (configured self-hosted service)",
          "      Sends the bot display name to the service; after Telegram confirmation,",
          "      it returns the bot token and owner ID once for local storage.",
          "      Set TELEGRAM_ONBOARDING_URL to use a compatible self-hosted service.", None)
    ...
    if prompt("Choice [1/2]", default="1").strip() == "2":
        ...
        if setup_result:
            ...
            if not _is_valid_telegram_bot_token(token):
                print_error("External QR setup returned an invalid Telegram bot token.")
                token = setup_result = None
        if not token:
...
def _setup_telegram():
    ...
    from panergos_cli.setup import _info, print_info, print_header, print_success, prompt, prompt_yes_no, save_env_value
    ...
    _info(None, "📬 Home Channel: where Panergos delivers cron job results,",
...
def _setup_bluebubbles():
    ...
    from panergos_cli.setup import _info, print_header, print_success, prompt, prompt_yes_no
    print_header("BlueBubbles (iMessage)")
    if declines_reconfigure("BlueBubbles", "Reconfigure BlueBubbles?", "BLUEBUBBLES_SERVER_URL"):
        return
    _info("Connects Panergos to iMessage via BlueBubbles — a free, open-source",
...
def _setup_webhooks():
    ...
    from panergos_cli.setup import _info, print_header, print_success, print_warning, prompt, save_env_value
    ...
    print()
    _info("   Full guide: https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/messaging/webhooks/", None)
    ...
    _info(f"   1. Define webhook routes in {_dhh()}/config.yaml",
          ...
          "   https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/messaging/webhooks/#configuring-routes",
          None,
          # Printed twice upstream; kept verbatim for output parity.
          "   Open config in your editor:  panergos config edit",
          "   Open config in your editor:  panergos config edit")
...
def _warn_missing_home_channels() -> None:
    ...
    from panergos_cli.setup import get_env_value, _info, print_warning
    missing_home = [
    ...
    _info("   Without a home channel, cron jobs and cross-platform",
          ...
          *(f"     panergos config set {plat.upper()}_HOME_CHANNEL <channel_id>" for plat in missing_home))
...
def _restart_running_gateway(any_messaging: bool, supports_systemd: bool) -> None:
    ...
    from panergos_cli.setup import print_error, prompt_yes_no
...
def setup_gateway(config: dict):
    ...
    from panergos_cli.setup import _info, print_header, print_info, print_success, prompt_checklist
    from panergos_cli.gateway import _all_platforms, _platform_status, _configure_platform
    print_header("Messaging Platforms")
    _info("Connect to messaging platforms to chat with Panergos from anywhere.",
    ...
    if not selected:
        print_info("No platforms selected. Run 'panergos setup gateway' later to configure.")
    ...
    # mode (cron keeps running; adapters come up once tokens are added via `panergos import` /
    # `panergos setup gateway`). Gating it on messaging config left install-then-import machines
    # with cron jobs and bot tokens but no process to serve them.
    from panergos_cli.gateway import _is_service_running, supports_systemd_services, ensure_gateway_service
