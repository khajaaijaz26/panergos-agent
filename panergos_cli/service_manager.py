...
def detect_service_manager() -> ServiceManagerKind:
    ...
    backend-agnostic code (profile hooks, the s6 dispatch in ``panergos gateway``).
    """
    # Deferred so importing this module (Protocol type, validate_profile_name) doesn't drag in
    # the whole gateway dependency graph.
    from panergos_cli.gateway import is_macos, is_windows, supports_systemd_services
    ...
    # made the s6 dispatch inert on Fly, so `panergos gateway start` spawned a foreground gateway
...
def _s6_running() -> bool:
    ...
    Must work for the unprivileged panergos user too: ``/proc/1/exe`` is unreadable for other UIDs
    ...
    resolved name is the literal ``"exe"`` and detection always fails. Since every Panergos runtime call
    inside the container drops to panergos via ``s6-setuidgid``, that silent failure made the entire
...
# ---------------------------------------------------------------------------
# Host backends: thin facades over ``panergos_cli.gateway`` (systemd/launchd) and
# ``panergos_cli.gateway_windows``. The protocol's ``name`` parameter is unused here — host backends
# operate on the currently active profile (``panergos -p <profile>``); the shape exists for s6 where
# each profile maps to a distinct service directory.
# ---------------------------------------------------------------------------


class _HostServiceManager:
    """``start``/``stop``/``restart`` resolve to ``<_fn_prefix><op>`` on ``panergos_cli.<_backend>``
    ...
    def _backend_module(self):
        import importlib
        import panergos_cli
        importlib.import_module(f"panergos_cli.{self._backend}")
        return getattr(panergos_cli, self._backend)
...
class SystemdServiceManager(_HostServiceManager):
    """Wraps the ``systemd_*`` functions in panergos_cli.gateway (host call sites still use those
...
class LaunchdServiceManager(_HostServiceManager):
    """Wraps the ``launchd_*`` functions in panergos_cli.gateway."""
...
class WindowsServiceManager(_HostServiceManager):
    """Wraps ``panergos_cli.gateway_windows`` (Scheduled Task / Startup-folder fallback).
...
# `panergos profile create` inside the container. Static services (main-panergos, dashboard) live in
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    the caller's PANERGOS_HOME blindly: derive the shared profile root and map the suffix to the root
    default profile or ``<root>/profiles/<profile>``.
    """
    profile = _profile_from_service(name)
    validate_profile_name(profile)
    panergos_home = Path(os.environ.get("PANERGOS_HOME", "/opt/data"))
    root = panergos_home.parent.parent if panergos_home.parent.name == "profiles" else panergos_home
...
# UID/GID of the in-image ``panergos`` user; hardcoded to match what ``stage2-hook.sh`` enforces
# (tests/docker/test_uid_remap.py). s6-supervise starts as root and drops via ``s6-setuidgid``.
_PANERGOS_UID = 10000
_PANERGOS_GID = 10000


def _chown_panergos(path: Path) -> None:
    try:
        os.chown(path, _PANERGOS_UID, _PANERGOS_GID)
    except PermissionError:
        # Already running as panergos → the dir is panergos-owned by default; swallowing keeps root
        # and unprivileged callers on one code path.
        pass


def _seed_supervise_skeleton(svc_dir: Path) -> None:
    """Pre-create panergos-owned ``supervise/`` and top-level ``event/`` inside a service directory.

    s6-supervise (root) creates ``event/``/``supervise/`` 0700 and the control FIFO 0600, so the
    panergos user gets EACCES on every ``s6-svc``/``s6-svstat``. s6 treats EEXIST as success and skips
    ...
    dispatched as the panergos user.
...
class GatewayNotRegisteredError(S6Error):
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`panergos profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(profile: str, extra_env: dict[str, str]) -> str:
        ...
        Sources PANERGOS_HOME via with-contenv (run time, not baked in), resets ``HOME`` before the
        privilege drop so root's HOME does not leak, activates the venv, drops to panergos.
        ``profile == "default"`` emits NO ``-p`` flag: it is the sentinel for the root PANERGOS_HOME
        ...
        allocator: because each supervised profile gateway loads its own ``PANERGOS_HOME``, two profiles that
        ...
        deterministic SHA-256 allocator in ``panergos_cli.profiles._allocate_gateway_port``). PR #30136 review
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/panergos/.venv/bin/activate",
        ]
        ...
        lines.append("export PANERGOS_S6_SUPERVISED_CHILD=1")
        # Generalized supervisor marker — same meaning for the profile-redirect guard in
        # panergos_cli.main._apply_profile_override; kept alongside the s6 one for back-compat.
        lines.append("export PANERGOS_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its PANERGOS_HOME. Without it
        # a gateway started OUTSIDE s6 (stray ``panergos gateway run``, an agent action, the Open
        ...
        if profile == "default":
            gateway_cmd = "panergos gateway run --replace"
        else:
            gateway_cmd = f"panergos -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID → s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid panergos {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            f': "${{PANERGOS_HOME:=/opt/data}}"\n'
            f'log_dir="$PANERGOS_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock AS PANERGOS when starting as root. Never
            # chown/unlink panergos-writable volume paths from this restartable root-context script:
            # an unprivileged user can race a pathname op through a symlink swap (CWE-59/CWE-367).
            # Parent logs/gateways is seeded panergos-owned at stage2 boot (test_log_dir_seed.py).
            # See #45258.
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid panergos mkdir -p "$log_dir"\n'
            f'  s6-setuidgid panergos rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid panergos s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        try:
            ...
            # Seed panergos-owned supervise/ BEFORE publishing so the panergos-user s6-svc/s6-svstat/
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        root-owned but the parent ``supervise/`` is panergos-owned (see ``_seed_supervise_skeleton``)
