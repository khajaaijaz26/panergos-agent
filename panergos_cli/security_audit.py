"""On-demand supply-chain audit for Panergos Agent installs.
...
def _discover_plugins(panergos_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.panergos/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, panergos_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = panergos_home or Path(get_panergos_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `panergos security audit`."""
    ...
    components = _discover_components(panergos_home=home, **skips)
    ...
    try:
        findings = run_audit(panergos_home=home, components=components)
