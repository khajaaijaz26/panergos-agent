"""Git plumbing for ``panergos update``: fork/upstream sync, trampoline-git detection, lockfile/EOL churn cleanup, orphan rescue refs, parked-branch assessment, fetch-failure classification.
...
_BAR = "=" * 68
_UPSTREAM_ADD_CMD = (
    f"git remote add upstream https://github.com/{DISTRIBUTION_REPO_SLUG}.git")
...
def _git_stdout(git_cmd, args, cwd, **kw) -> Optional[str]:
    ...
    from panergos_cli.update_cmd import _git_run
...
def _prune_orphan_rescue_refs(
    ...
    """Expire old orphan rescue refs (``refs/panergos-update-backups/orphan-<branch>-<ts>-<sha>``).
    ...
    with suppress(OSError):
        prefix = f"refs/panergos-update-backups/orphan-{branch}-"
...
def _print_parked_branch_skip_warning(git_cmd: list[str], cwd: Path, current_branch: str, target_branch: str, reason: str) -> None:
    ...
    print(
        ...
        f"    git -C {cwd} status\n"
        f"    git -C {cwd} checkout {target_branch} && panergos update\n"
...
OFFICIAL_REPO_URLS = {
    f"https://github.com/{DISTRIBUTION_REPO_SLUG}.git",
    f"git@github.com:{DISTRIBUTION_REPO_SLUG}.git",
    f"https://github.com/{DISTRIBUTION_REPO_SLUG}",
    f"git@github.com:{DISTRIBUTION_REPO_SLUG}",
}
OFFICIAL_REPO_URL = f"https://github.com/{DISTRIBUTION_REPO_SLUG}.git"
...
def _canonical_github_slug(url: Optional[str]) -> Optional[str]:
    """Return a case-folded ``owner/repo`` for accepted GitHub remote forms."""
    if not url:
        return None
    ...
    for pattern in patterns:
        ...
        if match:
            return re.sub(r"\.git$", "", match.group(1), flags=re.IGNORECASE).casefold()
    return None


def _is_official_repo_url(url: Optional[str]) -> bool:
    return _canonical_github_slug(url) == DISTRIBUTION_REPO_SLUG.casefold()
...
def _has_upstream_remote(git_cmd: list[str], cwd: Path) -> bool:
    """True only when ``upstream`` exists and points to canonical Panergos."""
    url = _git_stdout(git_cmd, ["remote", "get-url", "upstream"], cwd)
    valid = _is_official_repo_url(url)
    if url and not valid:
        logger.warning("Ignoring non-canonical upstream remote: %s", url)
    return valid
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_panergos_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    with suppress(Exception):
        from panergos_constants import get_panergos_home
        (get_panergos_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()


def _sync_fork_with_upstream(git_cmd: list[str], cwd: Path) -> bool:
    """Push the updated distribution branch to origin; True on success."""
    return _git_ok(
        git_cmd, ["push", "origin", DISTRIBUTION_BRANCH, "--force-with-lease"],
        cwd, network=True)
...
def _offer_upstream_remote(git_cmd: list[str], cwd: Path, *, assume_yes: bool, input_fn) -> bool:
    ...
    from panergos_cli.update_cmd import _add_upstream_remote, _mark_skip_upstream_prompt
    print(
        f"\nℹ Your fork is not tracking the official {DISTRIBUTION_NAME} repository.\n"
        f"  This means you may miss updates from {DISTRIBUTION_REPO_SLUG}.\n"
    )
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    See #97052.
    """
    from panergos_cli.update_cmd import _count_commits_between, _has_upstream_remote, _no_prompt_git_kwargs, _should_skip_upstream_prompt
    ...
    try:
        subprocess.run(
            git_cmd + ["fetch", "upstream", DISTRIBUTION_BRANCH, "--quiet"],
            cwd=cwd, capture_output=True, check=True, **_no_prompt_git_kwargs())
    except subprocess.CalledProcessError:
        print("  ✗ Failed to fetch upstream. Skipping upstream sync.")
        return False
    upstream_ref = f"upstream/{DISTRIBUTION_BRANCH}"
    origin_ref = f"origin/{DISTRIBUTION_BRANCH}"
    ...
    if origin_ahead > 0:
        print(
            ...
            f"    git pull upstream {DISTRIBUTION_BRANCH}"
        )
        return True
    if upstream_ahead == 0:
    ...
    try:
        subprocess.run(
            git_cmd + ["pull", "--ff-only", "upstream", DISTRIBUTION_BRANCH],
...
def _portable_git_candidates() -> list:
    ...
    profile-scoped PANERGOS_HOME), then profile home as a fallback for custom layouts.

    The Panergos-managed PortableGit tree lives under the SHARED root (``<root>/git/...``), not the
    profile-scoped PANERGOS_HOME (``<root>/profiles/<name>``), so a profile-scoped ``panergos update`` must look
    there (monerostar review, #87876).
    """
    from panergos_cli.update_cmd import get_default_panergos_root, get_panergos_home
    candidates = []
    with suppress(Exception):
        candidates += [root / "git" / "mingw64" / "libexec" / "git-core" / "git.exe" for root in (get_default_panergos_root(), Path(get_panergos_home()))]
    return candidates
...
def _ensure_non_trampoline_git(git_cmd: list) -> list:
    ...
    from panergos_cli.update_cmd import _locate_real_git
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    on update; install.ps1 pins ``false`` but older checkouts never got it and only ``panergos update`` can
    ...
    reuses its build-pinned ``install.ps1`` forever — so ``panergos update``, which ships with the checkout
