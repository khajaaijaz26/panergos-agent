"""MCP picker — interactive `panergos mcp picker` (also the default `panergos mcp`)."""
...
from panergos_cli.colors import Colors
from panergos_cli.cli_output import prompt_yes_no
from panergos_cli.curses_ui import curses_single_select
from panergos_cli.mcp_catalog import (
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    _say(
        ...
        "Start a new Panergos session for changes to take effect."
    )
...
def _configure_tools(name: str) -> None:
    ...
    from panergos_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    """Plain-text catalog dump: `panergos mcp catalog` output and the non-curses fallback."""
    print()
    if not rows:
    ...
    print()
    _say("  Install: panergos mcp install <name>    Picker: panergos mcp", Colors.DIM)
    # Manifest-version warnings: the user's Panergos is too old to install everything listed.
    future = [d for d in catalog_diagnostics() if d[1] == "future_manifest"]
    if future:
        print()
        for name, _, _msg in future:
            _say(f"  ⚠ '{name}' requires a newer Panergos — run `panergos update` to install this entry.", Colors.YELLOW)
        print()
    print()


def show_catalog() -> None:
    """`panergos mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`panergos mcp picker` (and default `panergos mcp`) — interactive selector; re-renders after each
    action until ESC/q."""
    while True:
        rows = _build_rows()
...
def install_by_name(identifier: str) -> int:
    """`panergos mcp install <name>` — non-interactive entry-point."""
    from panergos_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        _say(
            f"  ✗ '{identifier}' is not in the catalog. Run `panergos mcp catalog` to see available entries.",
            Colors.RED,
        )
        return 1
...
_PLUGIN_COMPAT_LAZY = {
    'color': ('panergos_cli.colors', 'color'),
}
