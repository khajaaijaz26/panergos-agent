...
def _browser_row(config, feats):
    ...
    if provider == "local":
        try:
            ...
            available = _chromium_installed() or _using_lightpanda_engine()
        except Exception:
            available = False
        return (
            "Browser Automation",
            available,
            None if available else _BROWSER_MISSING_HINTS["Local browser"],
        )
    keys = ("BROWSER_USE_API_KEY", "BROWSERBASE_API_KEY", "CAMOFOX_URL")
    available = any(_setup.get_env_value(k) for k in keys)
