"""``panergos plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``PANERGOS_HOME``), so a crashing or
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "PANERGOS_VALIDATE_JSON:"
...
def _requires_panergos_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_panergos`` spec.

    Unlike :func:`panergos_cli.plugins_manifest.version_satisfies` (permissive at load
    ...
    """
    from panergos_cli.plugins_manifest import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_panergos(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_panergos") or "").strip()
    if not spec:
        report.add("requires_panergos", True, "not declared")
        return
    if _requires_panergos_spec_valid(spec):
        report.add("requires_panergos", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_panergos",
            False,
            f"requires_panergos spec {spec!r} does not parse "
...
# from panergos so a hostile plugin only sees a bare interpreter.
_PROBE_SCRIPT = r"""
...
try:
    spec = importlib.util.spec_from_file_location(
        "panergos_validate_probe_plugin",
...
def _run_capability_probe(plugin_dir: Path) -> Tuple[Optional[dict], str]:
    ...
    *error* is a human-readable failure description otherwise.
    """
    with tempfile.TemporaryDirectory(prefix="panergos-validate-") as scratch:
        env = dict(os.environ)
        env["PANERGOS_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_panergos(report, manifest)
