...
Pure leaf: imports nothing from ``panergos_cli.auth`` so the per-provider modules
(``auth_codex`` and other providers) can import it at module scope without cycles."""
...
# httpx is imported lazily (~30ms) because panergos_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("panergos_cli.auth.httpx.Client")
...
try:  # Distribution identity for the Codex token-endpoint User-Agent; fall back if unavailable.
...
CODEX_OAUTH_USER_AGENT = f"panergos-agent/{_PANERGOS_VERSION}"
...
SPOTIFY_DOCS_URL = "https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/features/spotify"
...
OAUTH_OVER_SSH_DOCS_URL = "https://khajaaijaz26.github.io/panergos-agent/docs/guides/oauth-over-ssh"
