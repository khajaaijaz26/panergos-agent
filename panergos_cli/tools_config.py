"""Unified tool configuration for Panergos Agent."""
...
from panergos_cli.cli_output import print_info as _print_info
from panergos_cli.colors import Colors, color
from panergos_cli.config import cfg_get, load_config, save_config, get_env_value
from panergos_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
from panergos_cli.toolset_scope import (
    _TOOLSET_PLATFORM_RESTRICTIONS, toolset_allowed_for_platform as _toolset_allowed_for_platform)
# Re-exports: keep ``panergos_cli.tools_config.X`` callers and test patch targets resolving.
from panergos_cli.tools_config_cua import (  # noqa: F401
...
from panergos_cli.tools_config_post_setup import (  # noqa: F401
...
from panergos_cli.tools_config_providers import (  # noqa: F401
...
from panergos_cli.tools_config_mcp import (  # noqa: F401
...
# OFF by default for new installs (still in _PANERGOS_CORE_TOOLS; the checklist won't pre-select them). x_search
...
# Config-only capabilities: provider setup in `panergos tools` (TOOL_CATEGORIES) but not model toolsets — zero
...
def _xai_credentials_present() -> bool:
    ...
    try:
        from panergos_cli.auth import _read_xai_oauth_tokens
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        ...
        from panergos_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Toolset keys the ``panergos tools`` checklist offers for ``platform`` (mirrors ``_prompt_toolset_checklist``);
...
def _platform_default_toolset(platform: str) -> str:
    """Composite toolset a platform falls back to (plugin platforms derive ``panergos-<platform>``)."""
    return PLATFORMS[platform]["default_toolset"] if platform in PLATFORMS else f"panergos-{platform}"
...
def _row(name: str, badge: str = "", tag: str = "", env_vars: list = (), **markers) -> dict:
    """One TOOL_CATEGORIES provider row; ``markers`` are provider and post-setup keys."""
    row = {"name": name}
    if badge:
        row["badge"] = badge
    if tag:
...
_LANGFUSE_PUBLIC = ("PANERGOS_LANGFUSE_PUBLIC_KEY", "Langfuse public key (pk-lf-...)")
_LANGFUSE_SECRET = ("PANERGOS_LANGFUSE_SECRET_KEY", "Langfuse secret key (sk-lf-...)")
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Panergos routes X searches through xAI's built-in x_search Responses tool for read-only public X "
    ...
    "browser": {
        ...
        # first so a fresh install's Enter lands on the free local backend.
        ...
        "providers": [
            ...
            _row("Lightpanda", "free · local · no Chromium", "Zig headless browser spawned by Panergos, text-only (no screenshots)",
    ...
    "computer_use": {
        ...
        # cua-driver reads HOME/TMPDIR from the process env; PANERGOS_CUA_DRIVER_CMD selects a specific
        ...
        "providers": [
            ...
            _row("Langfuse Self-Hosted", tag="Self-hosted Langfuse instance", post_setup="langfuse",
                 env_vars=[_key(*_LANGFUSE_PUBLIC), _key(*_LANGFUSE_SECRET),
                           _key("PANERGOS_LANGFUSE_BASE_URL", "Langfuse server URL (e.g. http://localhost:3000)", default="http://localhost:3000")]),
        ],
    },
}
...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    try:
        from panergos_cli.plugins import get_portable_mcp_server_names_nowait
...
#: "declined": saving ``panergos tools`` freezes a platform's composite into an explicit list nothing adds to, so
#: a later toolset stays off forever for picker users while ``[panergos-cli]`` users inherit it.
...
def _enable_recently_shipped_toolsets(enabled_toolsets: Set[str], config: dict, platform: str) -> None:
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # composites (panergos-acp, panergos-webhook) stay narrow.
...
def _explicit_toolsets(
    ...
    ``panergos-cli`` would re-enable disabled toolsets). A mixed list (``[panergos-cli, spotify]``) still expands the
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # An explicitly saved list (even a composite like ``panergos-discord``) is an opt-in to the platform's
    ...
    # Plugin toolsets are first-class on a saved list: ``[panergos-cli, a2a]`` must survive filtering.
    # Plugin-provided toolsets are first-class on a platform-toolsets list — explicit config like
    # ``[panergos-cli, a2a]`` must survive filtering just like a built-in configurable toolset would. See
    # issue #81163.
    ...
    # above. It may arrive as a JSON-array string ("['memory']") from `panergos config set` or a JSON-mode editor.
...
def _prune_toolsets_stripped_by_disabled(enabled_toolsets: Set[str], disabled_names: List[str]) -> Set[str]:
    ...
    never appear in the list. A name-only subtraction here left inspection surfaces (``panergos tools
...
def _recover_platform_native_toolsets(enabled_toolsets: Set[str], platform: str, *, skip: Set[str]) -> None:
    ...
    skip = skip | {k for k in TOOLSETS if k.startswith("panergos-")} | (set(_DEFAULT_OFF_TOOLSETS) - {platform})
...
def _warn_all_invalid_platform_toolsets(platform: str, explicit: list) -> None:
    """Warn once when an explicit platform list has only invalid names (``panergos`` for ``panergos-cli`` → no
    ...
    if named and not any(validate_toolset(t) for t in named) and platform not in _warned_invalid_platform_toolsets:
        ...
        logger.warning(
            ...
            "name(s): %s) - tools will be unavailable. Run `panergos tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # names): platform defaults (panergos-cli, ...) resolve to ALL tools and would silently override the user's
...
def _shared_metrics_menu_label(config: dict) -> str:
    ...
    state = "off" if not enabled else ("collecting + sending to the configured collector" if send else "collecting locally")
...
def _configure_shared_metrics_interactive(config: dict) -> None:
    ...
    from panergos_cli.setup import setup_telemetry
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `panergos tools` / `panergos setup tools`. ``first_install`` skips the menu (checklist + key
    ...
    print(color(f"  Tool configuration saved to {display_panergos_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'panergos' or gateway restart.", Colors.DIM))
    print()
