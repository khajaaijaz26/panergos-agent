...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None) -> list[int]:
    ...
    from panergos_cli.dashboard_procs import _scan_dashboard_processes
...
def _restart_managed_dashboard_service(reason: str, unit: str = _DASHBOARD_SYSTEMD_UNIT) -> bool:
    ...
    # User manager first (Panergos installs Linux services in the user scope by
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    """The systemd service unit name *pid* belongs to (``panergos-serve.service``), or None.
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Respawn manually-started dashboards after ``panergos update``, detached, logging to
    ...
    log_path = get_panergos_home() / "logs" / "dashboard-restart.log"
...
class _UpdateOutputStream:
    """stdout/stderr wrapper for ``panergos update``: mirrors to ``logs/update.log`` and, once the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        # panergos_cli.config.get_panergos_home to simulate setup failure.
        from panergos_cli.config import get_panergos_home as _get_panergos_home
        logs_dir = _get_panergos_home() / "logs"
        logs_dir.mkdir(parents=True, exist_ok=True)
        log_file = open(logs_dir / "update.log", "a", buffering=1, encoding="utf-8")

        import datetime as _dt

        log_file.write(f"\n=== panergos update started {_dt.datetime.now().isoformat(timespec='seconds')} ===\n")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No Panergos dashboard or serve processes running.")
        return 0

    print(f"{len(live)} Panergos dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from panergos_cli.web_server import should_require_dashboard_auth
    ...
    try:
        ...
        from panergos_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
        cfg = load_config()
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from panergos_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under the account's $HOME/.panergos/
    # desktop-ssh, independent of PANERGOS_HOME and the active profile. Anchor
    # validation there, NOT get_panergos_home(): a non-default profile or a Docker
    # /opt/data root re-homes get_panergos_home() and would reject every token.
    # See #69551.
    token_root = Path.home() / ".panergos" / "desktop-ssh"
    try:
    ...
    if ".." in relative.parts:
        raise SystemExit("--ssh-session-token-file must be under the desktop-ssh directory")
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    A standalone ``panergos dashboard`` inheriting that ``PANERGOS_WEB_DIST`` would
...
def _route_named_profile_dashboard(
    ...
    (PANERGOS_DESKTOP=1) stay per-profile. Returns normally when no routing applies.
    """
    try:
    ...
    if (
        ...
        or os.environ.get("PANERGOS_DESKTOP") == "1"
    ):
        return
    ...
    reexec_argv = [
        sys.executable, "-m", "panergos_cli.main",
        "-p", "default",
    ...
    # PANERGOS_HOME is pinned to the machine root below — the factory must not
    # re-inject a profile home.
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=False)
    # Pin the child to the machine ROOT, resolved explicitly rather than by
    # dropping PANERGOS_HOME: in the Docker layout the root is /opt/data, and an
    # unset PANERGOS_HOME would fall back to $HOME/.panergos = /opt/data/.panergos — an
    # empty auto-seeded home with only the default profile and no install stamp.
    # get_default_panergos_root() strips a trailing profiles/<name> for both layouts.
    try:
        from panergos_constants import get_default_panergos_root
        env["PANERGOS_HOME"] = str(get_default_panergos_root())
    except Exception:
        env.pop("PANERGOS_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    ``serve`` sets PANERGOS_SERVE_HEADLESS so mount_spa() stays off. Otherwise build
    unless PANERGOS_WEB_DIST / --skip-build promise a dist — then verify index.html
    (else the server serves 404s). --skip-build on the default location gets ONE
    recovery build; a caller-managed PANERGOS_WEB_DIST can't be populated.
    """
    from panergos_cli.main import PROJECT_ROOT
    from panergos_cli.main_web_build import _build_web_ui
    skip_build = getattr(args, "skip_build", False)
    if _headless_backend:
        os.environ["PANERGOS_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "PANERGOS_WEB_DIST" not in os.environ and not skip_build:
        ...
        _dist_root = (
            ...
            Path(os.environ["PANERGOS_WEB_DIST"])
            if "PANERGOS_WEB_DIST" in os.environ
            else PROJECT_ROOT / "panergos_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            # Only the default dist location is recoverable (desktop launches with
            # --build-mode skip after a wipe of web_dist); a custom PANERGOS_WEB_DIST
            ...
            _recoverable = "PANERGOS_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # PANERGOS_WEB_DIST without --skip-build: the env var points at a
        # caller-managed dist, so validate it like the --skip-build branch.
        # PANERGOS_WEB_DIST is set without --skip-build: the build is skipped (the env var points at a
        ...
        # path).
        _dist_root = Path(os.environ["PANERGOS_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ PANERGOS_WEB_DIST is set but no web dist found at: {_dist_root}")
            print(_PRE_BUILD_HINT)
            print("  Or unset PANERGOS_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # web_server reads PANERGOS_WEB_DIST raw at import (no expanduser), so a
        # validated "~/dist" would otherwise pass here and still 404 there.
        os.environ["PANERGOS_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from PANERGOS_WEB_DIST: {_dist_root}")
