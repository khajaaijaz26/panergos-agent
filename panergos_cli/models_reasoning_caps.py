...
Split out of ``panergos_cli.models``. Catalog sources share one implementation parametrized by
:class:`_CapsSource`; per-source globals stay defined on ``panergos_cli.models`` and are read or
written by attribute name.
...
logger = logging.getLogger("panergos_cli.models")

Caps = dict[str, Optional[dict[str, Any]]]


def _origin():
    from panergos_cli import models
    return models
...
# that must never block on HTTP — so without a disk copy, `panergos -p`, a cron job, or a freshly
# booted gateway answers "capability unknown" for its whole first turn. One file holds every
# catalog keyed by URL so distinct compatible endpoints cannot share capability data.
...
def _reasoning_caps_disk_path() -> Path:
    ...
    return get_panergos_home() / "cache" / "reasoning_caps.json"


def _read_reasoning_caps_disk() -> dict[str, Any]:
    from panergos_cli.models import _read_json_cache
...
def _save_reasoning_caps_disk(url: str, caps: Caps) -> None:
    ...
    from panergos_cli.models import _write_json_cache
    try:
...
def _warm_reasoning_caps_async(refresh) -> None:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        return
    # copy_context: the URL and disk mirror belong to the calling profile, not the launch home.
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    empty so callers remember the failure. Sends the standard application User-Agent."""
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._PANERGOS_USER_AGENT})
...
class _CapsSource:
    """One catalog's cache slots on ``panergos_cli.models`` plus how to name its URL.
    ...
    def get(self, slot: str):
        ...
        from panergos_cli.models_profile_cache import profile_slot_get
    ...
    def set(self, slot: str, value) -> None:
        if not self.per_profile:
            ...
            return
        from panergos_cli.models_profile_cache import profile_slot_set
...
def _caps_cached(src: _CapsSource) -> Optional[Caps]:
    ...
    One disk attempt per process; resolving a catalog URL can be too expensive for a per-turn caller.
    """
