"""CLI subcommand: ``panergos send`` — pipe text from shell scripts to any configured messaging platform
...
def _read_message_body(positional: Optional[str], file_path: Optional[str]) -> Optional[str]:
    ...
    if file_path:
        ...
        try:
            ...
            _fail(
                f"panergos send: {file_path} is not a text file. --file reads the "
                ...
                f'  panergos send --to telegram "MEDIA:{file_path}"\n'
                f'  panergos send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  panergos send --to telegram "[[as_document]] MEDIA:{file_path}"',
                _USAGE_EXIT)
        except OSError as exc:
            _fail(f"panergos send: cannot read {file_path}: {exc}", _USAGE_EXIT)
...
def _emit_result(result_json: str, *, json_mode: bool, quiet: bool) -> int:
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"panergos send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        return _fail(f"panergos send: failed to load channel directory: {exc}")
    try:
        raw = load_directory()
    except Exception as exc:
        return _fail(f"panergos send: failed to read channel directory: {exc}")
    ...
    if platform_filter:
        ...
        if not filtered:
            return _fail(
                f"panergos send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `panergos gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.panergos/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_panergos_env() -> None:
    """Populate ``os.environ`` from ``~/.panergos/.env`` AND bridge top-level ``config.yaml`` keys into
    ...
    if env_path.exists():
        try:
            from panergos_cli.env_loader import _load_dotenv_with_fallback
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    _load_panergos_env()  # the downstream gateway config loader reads credentials from os.environ
    if getattr(args, "list_targets", False):  # --list short-circuits everything else
        # `panergos send --list telegram` lands "telegram" in the `message` positional.
        exit_code = _list_targets(getattr(args, "message", None), json_mode=getattr(args, "json", False))
        sys.exit(exit_code)
    target = (getattr(args, "to", None) or "").strip()
    if not target:
        _fail(
            "panergos send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  panergos send --to telegram \"hello\"\n"
            "  panergos send --to discord:#ops --file report.md\n"
            "  panergos send --list      # list available targets",
            _USAGE_EXIT)
    message = _read_message_body(getattr(args, "message", None), getattr(args, "file", None))
    if message is None or not message.strip():
        _fail(
            "panergos send: no message provided. Pass text as a positional "
    ...
    # Lazy import keeps `panergos send --help` fast (no tool registry / gateway config stack).
...
_SEND_ARGUMENTS = (
    ...
    (("-l", "--list"), dict(dest="list_targets", action="store_true", default=False,
                            help="List available targets. Optional positional filter: `panergos send --list telegram`.")),
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Panergos "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.panergos/.env + ~/.panergos/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  panergos send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
