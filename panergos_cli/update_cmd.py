"""Panergos update pipeline: dispatchers (``_cmd_update_impl``/``_cmd_update_check``) + git plumbing.

Each concern lives in ``update_cmd_<concern>.py`` and is re-imported here so
``panergos_cli.update_cmd.<name>`` keeps resolving (and stays monkeypatchable). Imports are one-way:
main -> update_cmd -> update_cmd_*; ``_m()`` resolves ``panergos_cli.main`` at call time.
"""
...
from panergos_cli import (
    __distribution_default_branch__ as DISTRIBUTION_BRANCH,
    __distribution_repo_slug__ as DISTRIBUTION_REPO_SLUG,
)
from panergos_cli.config import get_panergos_home  # noqa: F401  (re-exported; patched via update_cmd)
from panergos_cli.update_cmd_common import _best_effort
from panergos_constants import get_default_panergos_root, venv_python_path

# Re-exports: every split-module name stays reachable (and monkeypatchable) as update_cmd.<name>.
from panergos_cli.update_abort_recovery import (  # noqa: F401
...
from panergos_cli.update_cmd_windows import (  # noqa: F401
    ...
    _handoff_reapable_backend_pids, _panergos_holder_subcommand, _holder_value_flags,
...
from panergos_cli.update_cmd_fleet import (  # noqa: F401
...
from panergos_cli.update_cmd_zip import (  # noqa: F401
...
from panergos_cli.update_cmd_stash import (  # noqa: F401
...
from panergos_cli.update_cmd_config import (  # noqa: F401
...
from panergos_cli.update_cmd_deps import (  # noqa: F401
...
from panergos_cli.update_cmd_git import (  # noqa: F401
...
from panergos_cli.update_cmd_maint import (  # noqa: F401
    ...
    _print_verified_update_completion, _purge_stale_panergos_modules, _read_project_version,
...
def _m():
    """Lazy ``panergos_cli.main`` handle: keeps main-side test patches effective, import one-way."""
    from panergos_cli import main
    return main
...
_UPDATE_CRITICAL_FILES = (
    "panergos_cli/main.py", "panergos_cli/config.py", "panergos_cli/__init__.py",
    "panergos_cli/web_server.py", "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
    "panergos_constants.py")
...
def _record_update_step(step: str, ok: bool, detail: str = "") -> None:
    ...
    with suppress(Exception):
        from panergos_cli.update_receipt import record_step
        record_step(step, ok, detail)


# A fetch whose transport dead-stalls (HTTP/2 to GitHub on some networks, a black-holed proxy)
# otherwise leaves `panergos update` on "Fetching updates..." forever (#93759, #95777). Five
...
def _validate_python_files_syntax(root, relpaths) -> tuple[bool, str | None, str | None]:
    ...
    import tempfile
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="panergos-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from panergos_constants import get_panergos_home  # noqa: F811  (deliberate: constants variant)
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    See #87331.
    """
    print("✗ Cannot continue the update: live Panergos launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["panergos.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Panergos")
    print("  Desktop, a gateway, or another panergos REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `panergos` command to finish it automatically.")
...
def _invalidate_update_cache():
    ...
    default_home = get_default_panergos_root()
...
def _format_concurrent_instances_message(matches: list[tuple[int, str]], scripts_dir: Path) -> str:
    """Explanation + remediation hint for the Windows concurrent-panergos.exe gate."""
    shim = scripts_dir / "panergos.exe"
    lines = [
        "✗ Another Panergos launcher is running:",
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from panergos_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _log_only_write(text: str) -> None:
    ...
    with suppress(Exception):
        if log_file is None:
            log_path = get_panergos_home() / "logs" / "update.log"
...
def _run_logged_subprocess(cmd, *, cwd=None, env=None):
    ...
    import codecs
    import io
    from panergos_cli._subprocess_compat import kill_process_tree, windows_hide_flags
...
def _cmd_update_check(branch: str = DISTRIBUTION_BRANCH, *, branch_explicit: bool = False):
    """``panergos update --check``: fetch and report without installing. ``branch_explicit`` is
    ...
    from panergos_cli.update_contract import evaluate_update_admission, record_refusal_receipt
    ...
    from panergos_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    # Only forks may consult an upstream remote, and the shared validator requires that
    # remote to resolve to the canonical Panergos repository. This prevents a stale or
    # hostile remote named ``upstream`` from becoming an update source.
    fetch_result = None
    origin_url = _get_origin_url(git_cmd, _m().PROJECT_ROOT)
    if (branch == DISTRIBUTION_BRANCH and _is_fork(origin_url)
    ...
    if is_shallow:
        ...
        from panergos_cli.gitlock import repair_broken_shallow_boundaries, prune_stale_shallow_grafts
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            return
        from panergos_cli.banner import _github_compare_behind
...
def _repair_venv_on_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id,
    ...
    _write_update_incomplete_marker()
    from panergos_cli.managed_uv import ensure_uv
    repair_uv = ensure_uv()
    ...
    if not healthy_after:
        ...
        print("  Close all Panergos windows/gateways and re-run: panergos update")
        return False
...
def _repair_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id,
    ...
    from panergos_cli.managed_uv import ensure_uv, update_managed_uv
    runtime_repairs = []
    ...
    if handed_off_sync:
        print("→ Finishing the dependency install handed off by the Panergos launcher...")
    elif not healthy:
    ...
    if runtime_repaired is not None and not _m()._is_windows():
        ...
        print(
            "  Any running Panergos gateways, Desktop backends, or other "
...
def _reconcile_diverged_checkout(git_cmd, branch: str, pre_pull_sha) -> None:
    ...
    if not has_common_ancestor and pre_pull_sha:
        ...
        rescue_ref = (
            f"refs/panergos-update-backups/orphan-{branch}-"
...
def _rollback_if_pulled_syntax_error(git_cmd, pre_pull_sha) -> None:
    ...
    if pre_pull_sha:
        ...
        if rollback_result.returncode == 0:
            ...
            print("  Try ``panergos update`` again later once a fix lands.")
        else:
...
def _pull_updates(
    ...
    # critical-path file (PR #28452 incident: orphan merge-conflict markers in panergos_cli/config.py bricked
    # every user who ran ``panergos update`` for the 7 minutes between the bad commit and the fix landing).
...
def _prepare_checkout_for_update(
    ...
    if commit_count == 0 and is_fork and branch == DISTRIBUTION_BRANCH:
...
class _UpdateOptions:
    """Resolved ``panergos update`` inputs (flags, config, pre-update snapshots)."""
...
def _begin_update_receipt_and_plan(args):
    ...
    pre-update plan (None if the probe failed); ``sys.exit(2)`` when a non-gateway panergos.exe
    holds the venv shim."""
    ...
    with _best_effort('Update receipt unavailable: %s'):
        ...
        from panergos_cli.update_receipt import begin_update_receipt
    ...
    # Plan phase (#91277 Phase 2): snapshot the pre-update fleet — every running Panergos runtime, its
    ...
    with _best_effort('Update plan phase failed: %s'):
        from panergos_cli.update_inventory import collect_runtime_inventory, record_plan_in_receipt
    ...
    # Windows: another panergos.exe holding the venv shim means WinError 32 spam and a
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        concurrent = _m()._detect_concurrent_panergos_instances(scripts_dir) if scripts_dir is not None else []
...
def _prepare_git_command() -> tuple[bool, list, bool]:
    ...
    if use_zip_update and sys.platform != "win32":
        ...
        print(
            ...
            f"https://github.com/{DISTRIBUTION_REPO_SLUG}#install-from-source")
        sys.exit(1)
...
def _verify_head_after_pull(
    ...
    # step re-detaches to the SHA). Before this guard, ``panergos update`` printed "✓ Code updated!" and
    # reinstalled deps + rebuilt the desktop app against the stale tree — no error, no warning, ``panergos
    ...
    if pre_pull_sha and post_pull_sha == pre_pull_sha:
        ...
        print(
            ...
            f"git -C {_m().PROJECT_ROOT} checkout {branch} && panergos update")
        _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
        sys.exit(1)
        ...
        if _called_process_error_is_python_dep_install(e):
            ...
            if _m()._is_windows():
                ...
                print(
                    ...
                    '"from panergos_cli.main import main; main()" update --yes')
...
def _apply_pulled_update(
    ...
    if is_fork and branch == DISTRIBUTION_BRANCH:
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Any venv python still running (typically the Desktop `panergos serve` backend) keeps .pyd
    ...
    # Self-lock deferral moved: the venv-holder sweep above excludes this process by design (a CLI `panergos
