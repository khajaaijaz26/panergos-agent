...
Mixin bound onto ``PanergosCLI`` via the MRO. cli.py-internal symbols are imported LAZILY
...
def _reset_model_to_config_default(cli, silent: bool) -> None:
    ...
    if not _config_model or _config_model == getattr(cli, "model", None):
        return
    try:
        from panergos_cli.model_switch import switch_model as _switch_model
...
class CLISessionMixin:
    ...
    def _show_status(self):
        ...
        try:
            ...
            separator_color = skin.get_color("banner_dim", "#B9A9C8")
            accent_color = skin.get_color("ui_accent", "#FF6B5E")
            label_color = skin.get_color("ui_label", "#F7C453")
        except Exception:
            separator_color, accent_color, label_color = "#B9A9C8", "#FF6B5E", "#F7C453"
    ...
    def _show_session_status(self):
        ...
        from panergos_cli.status_report import build_status_fields, status_lines
        session_meta = {}
        if self._session_db:
        ...
        lines = ["Panergos CLI Status", "", *status_lines(fields, "session_id", "path", "title", "model")]
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from panergos_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from panergos_cli.timefmt import relative_time as _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            _cli_visible_print(f"\n  [Panergos #{visible_index}]{_ts_suffix(msg)}")
    ...
    def _notify_session_boundary(self, event_type: str) -> None:
        ...
        with contextlib.suppress(Exception):
            from panergos_cli.lifecycle import finalize_session, invoke_hook
            ...
            if self._session_db:
                with contextlib.suppress(Exception):
                    ...
                    self._session_db.create_session(
                        ...
                        source=os.environ.get("PANERGOS_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self, cmd: str = "/save"):
        ...
        """
        from cli import datetime
        from panergos_cli.session_export import (
        ...
        saved_dir = get_panergos_home() / "sessions" / "saved"
        try:
        ...
        if filename:
            ...
            path = saved_dir / f"panergos_conversation_{timestamp}.{fmt}"
        ...
        try:
            ...
            # tools/subprocesses on this thread resolve PANERGOS_SESSION_ID to the child id after an
    ...
    def _write_terminal_breadcrumb(self) -> None:
        """Record this terminal's live session for bare ``panergos -c``. Called whenever
        ...
        with contextlib.suppress(Exception):
            from panergos_cli.terminal_breadcrumbs import write_breadcrumb
    ...
    def _toggle_yolo(self):
        ...
        ``PANERGOS_YOLO_MODE``: that env var is frozen into ``tools.approval._YOLO_MODE_FROZEN``
        ...
        if _YOLO_MODE_FROZEN:
            _cprint(
                ...
                " for this process (started with --yolo / PANERGOS_YOLO_MODE)."
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        # the hint ("default"/"custom" use the standard PANERGOS_HOME).
        try:
        ...
        print(f"  panergos --resume {self.session_id}{profile_flag}")
        if session_title:
            print(f"  panergos -c \"{session_title}\"{profile_flag}")
        print()
