...
__all__ = [
    ...
    "noninteractive_git_env_with_text_normalization",
...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_PANERGOS_GATEWAY_BREAKAWAY"
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Panergos's plumbing calls. ``core.sshCommand`` is pinned to ``ssh -o BatchMode=yes`` so the ssh
    ...
    Panergos shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def noninteractive_git_env_with_text_normalization(
    cwd: str | os.PathLike[str], base: "Mapping[str, str] | None" = None,
) -> dict[str, str]:
    """Hardened Git environment that preserves only safe line-ending semantics.

    Status/diff probes must interpret the worktree with the same ``core.autocrlf``
    and ``core.eol`` values used to populate the index.  The normal hardened
    environment intentionally blanks repository/global/system config; on Windows
    that otherwise reports every CRLF checkout as modified.  Read the two effective
    values first, allowlist them, then inject them into the hardened environment.
    """
    source = dict(base if base is not None else os.environ)
    for key in list(source):
        if key == "GIT_CONFIG_PARAMETERS" or key.startswith(_GIT_CONFIG_INJECT_PREFIXES):
            source.pop(key, None)
    source.pop("GIT_CONFIG_COUNT", None)
    source.update({"GIT_TERMINAL_PROMPT": "0", "GCM_INTERACTIVE": "Never", "GIT_PAGER": "cat"})
    ...
    for config_key, values in allowed.items():
        try:
            proc = subprocess.run(
                ["git", "-C", os.fspath(cwd), "config", "--get", config_key],
                stdin=subprocess.DEVNULL, capture_output=True, text=True,
                encoding="utf-8", errors="replace", timeout=3, env=source,
                creationflags=windows_hide_flags(),
            )
        except (OSError, subprocess.TimeoutExpired):
            continue
        value = proc.stdout.strip().casefold()
        if proc.returncode == 0 and value in values:
            env[f"GIT_CONFIG_KEY_{count}"] = config_key
...
def _text_names_panergos(text: str) -> bool:
    r"""True when *text* names Panergos at a path-segment / token boundary.

    A bare ``"panergos" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\spanergosa\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("panergos", ".panergos"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_panergos(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Panergos code."""
    try:
        import psutil
        ...
        return _text_names_panergos(f"{command} {executable}")
    except Exception:
        return False


def pid_is_panergos(pid: int, *, expected_start_time: int | None = None) -> bool:
    """Whether it is safe to use ``taskkill`` for *pid*.

    The PID must be valid, currently exist, and identify a Panergos process. When the caller captured
    ...
    if expected_start_time is not None and current_start_time != expected_start_time:
        return False
    try:
        return _process_command_is_panergos(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # be recycled while we hold it. The fail-closed ``pid_is_panergos`` guard is for BARE pids.
        try:
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``panergos update`` on slow-WMI
...
def bounded_git_probe(
    ...
    result = bounded_probe_run(
        argv, timeout=timeout, env=env if env is not None else noninteractive_git_env(),
    )
