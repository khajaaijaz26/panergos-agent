"""Post-setup install hooks and installed-state predicates for `panergos tools` provider rows."""
...
from panergos_cli.config import get_env_value, recommended_update_command_for_method
from panergos_cli.tools_config_cua import (
    _cua_driver_install_ready, _pip_install, _post_setup_no_window_flags, _run_text, install_cua_driver,
)

logger = logging.getLogger("panergos_cli.tools_config")
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Panergos-managed ``$PANERGOS_HOME/bin`` copy does."""
...
def _post_setup_lightpanda() -> None:
    ...
    if lightpanda_bin:
        ...
        if os.name == "nt":
            _print_info("    Lightpanda has no native Windows build; run Panergos under WSL2.")
...
def _post_setup_agent_browser(post_setup_key: str) -> None:
    ...
    global/Panergos-managed install), so there is no ``npm install`` step here."""
    ...
    try:
        ...
        # PATH/node_modules probes above. Mirror the rung panergos_cli.doctor uses so this probe can't diverge
        ...
        # agent-browser is no longer a root package.json dependency (#43564) — it resolves lazily via npx
        # (or a global/Panergos-managed install) instead of a local `npm install`, so there's no node_modules/
    ...
    # Reuse the runtime resolution cascade (PATH -> Homebrew/Panergos-managed node -> npx) rather than
    # a bare shutil.which — Panergos-managed-Node-only setups resolve agent-browser/npx only that way.
    try:
    ...
    if _running_in_docker():
        ...
        _info_lines("Rebuild the Panergos source image to get bundled Chromium:",
...
_PIP_POST_SETUP_HOOKS: dict = {
    "faster_whisper": _pip_hook(
        ...
        on_install=("Model sizes: tiny, base (default), small, medium, large-v3",
                    "Change via stt.local.model in ~/.panergos/config.yaml")),
    "kittentts": _pip_hook(
    ...
    "piper": _pip_hook(
        ...
        always=("Default voice: en_US-lessac-medium (downloaded on first TTS call)",
                ...
                "Switch voices by setting tts.piper.voice in ~/.panergos/config.yaml")),
    "ddgs": _pip_hook(
...
def _post_setup_spotify() -> None:
    # Full `panergos auth spotify` flow: no client_id yet → interactive wizard (persists to ~/.panergos/.env)
    # then PKCE; existing app → OAuth only.
    from types import SimpleNamespace
    try:
        from panergos_cli.auth import login_spotify_command
    except Exception as exc:
        _print_warning(f"    Could not load Spotify auth: {exc}")
        _info_lines("Run manually: panergos auth spotify")
        return
    ...
    try:
        ...
        _info_lines("Run later: panergos auth spotify")
    except Exception as exc:
...
def _post_setup_langfuse() -> None:
    ...
    try:
        from panergos_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
        ...
        _info_lines("Run manually: panergos plugins enable observability/langfuse")
    _info_lines("Restart Panergos for tracing to take effect.", "Verify: panergos plugins list")
...
def _post_setup_xai_grok() -> None:
    ...
    try:
        from panergos_cli.setup import prompt_choice, prompt as _setup_prompt
        from panergos_cli.setup_tts import _run_xai_oauth_login_from_setup
        from panergos_cli.config import save_env_value
    except Exception as exc:
        _print_warning(f"    Could not load setup helpers: {exc}")
        _info_lines("Run later: panergos auth add xai-oauth   (or set XAI_API_KEY)")
        return
    ...
    idx = prompt_choice(
        ...
        choices=["Sign in with xAI Grok OAuth (SuperGrok / Premium+) — browser login",
                 ...
                 "Skip — configure later via `panergos auth add xai-oauth`"])
    if idx == 0:
        if _run_xai_oauth_login_from_setup():
            _print_success("    Logged in — xAI will use these OAuth credentials")
        else:
            _print_warning("    xAI Grok OAuth login did not complete. Run later: panergos auth add xai-oauth")
    elif idx == 1:
        ...
        if api_key:
            ...
            _print_warning("    No API key provided. Run later: panergos auth add xai-oauth")
    else:
...
def run_post_setup_command(args) -> int:
    """``panergos tools post-setup <key>`` — non-interactive runner the dashboard spawns so the GUI can drive
    backend setup without re-implementing install logic. Exit code: 0 ok, 2 unknown key."""
    key = getattr(args, "post_setup_key", None)
    if not key:
        _print_error("Usage: panergos tools post-setup <key>")
        return 2
...
# Python deps installed via ``panergos tools`` aren't in the managed runtime's locked ``all`` sync, so a
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``panergos tools`` Python dependencies present in this runtime."""
    return [
...
def _agent_browser_installed() -> bool:
    ...
    *and* the Chromium build it drives (or the Lightpanda engine, which needs no Chromium), so "Run
    ...
    try:
        ...
        return bool(_find_agent_browser(validate=False)) and bool(_chromium_installed())
    except Exception:
        return False
