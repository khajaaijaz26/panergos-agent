"""Multi-provider authentication system for Panergos Agent.

- ``ProviderConfig`` / ``PROVIDER_REGISTRY`` describe every known inference provider.
- The auth store (``~/.panergos/auth.json``) holds per-provider state, the credential pool and
...
- ``OAUTH_PROVIDER_FLOWS`` maps each OAuth provider to its resolver/status builder; the flows live in
  ...
  re-imported here so ``panergos_cli.auth.<name>`` stays the public/patchable surface."""
...
from panergos_cli.config import (
    get_panergos_home, get_config_path, read_raw_config, require_readable_config_before_write)
from panergos_constants import OPENROUTER_BASE_URL, panergos_home_key, secure_parent_dir
...
from panergos_cli.auth_zai_kimi import (  # noqa: F401  re-exported
...
from panergos_cli.auth_model_picker import (  # noqa: F401  re-exported
    _prompt_model_selection, _save_model_choice)
from panergos_cli.auth_device_flow import (  # noqa: F401  re-exported
    _can_open_graphical_browser, _default_verify, _is_remote_session,
    _offer_existing_oauth_credentials,
    _poll_device_token_generic, _print_device_code_instructions,
    _print_login_success, _print_loopback_ssh_hint, _prompt_yes_no,
    _resolve_verify, _ssh_user_at_host)
from panergos_cli.auth_oauth_grants import (  # noqa: F401  re-exported
...
from panergos_cli.auth_minimax import (  # noqa: F401  re-exported
...
from panergos_cli.auth_xai import (  # noqa: F401  re-exported
...
from panergos_cli.auth_codex import (  # noqa: F401  re-exported
...
from panergos_cli.auth_spotify import (  # noqa: F401  re-exported
    _refresh_spotify_oauth_state, get_spotify_auth_status, login_spotify_command,
    resolve_spotify_runtime_credentials)
from panergos_cli.auth_openrouter import _openrouter_pkce_login  # noqa: F401  re-exported
from panergos_cli.auth_qwen import (  # noqa: F401  re-exported
...
from panergos_cli.auth_constants import (  # noqa: F401  re-exported
...
_REGISTRY_ROWS: Tuple[Any, ...] = (
    ...
    # is a 401), so there is no secret to configure. Select via `panergos model` / `/model free`.
...
def get_anthropic_key() -> str:
    ...
    Checks both the ``.env`` file and the process environment, preferring ``~/.panergos/.env`` so a deliberate
...
def _model_level_key_env(provider_id: str) -> str:
    ...
    (``model.key_env`` → ``$PANERGOS_HOME/.env``) instead of the registry's canonical env var,
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    if provider_id == "copilot":
        ...
        try:
            from panergos_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    # Prefer ~/.panergos/.env over os.environ so a deliberate key rotation in .env isn't shadowed by
    ...
    # model.key_env → $PANERGOS_HOME/.env (e.g. PANERGOS_CUSTOM_LMSTUDIO_API_KEY) while keeping
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    re-authenticating cannot fix it, so callers should say "retry later", not ``panergos auth``."""
    return (isinstance(error, AuthError) and not error.relogin_required
            and error.code == CODEX_RATE_LIMITED_CODE)


# Entitlement failures use provider-neutral guidance when a generic mapping exists.
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `panergos model` to re-authenticate."
...
# ── Auth Store — persistence layer for ~/.panergos/auth.json ──────────────────────────────────────────

def _auth_file_path() -> Path:
    path = get_panergos_home() / "auth.json"
    # Seat belt: under pytest, refuse to touch the real user's auth store (tests that forgot to
    # monkeypatch PANERGOS_HOME or escaped the hermetic conftest). In production: one dict lookup.
    if (os.environ.get("PYTEST_CURRENT_TEST")
            and _same_path(path, Path.home() / ".panergos" / "auth.json")):
        raise RuntimeError(
            f"Refusing to touch real user auth store during test run: {path}. "
            "Set PANERGOS_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_panergos_root()
    except Exception:
        return None
    return None if _same_path(get_panergos_home(), global_root) else global_root / "auth.json"
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST") and os.environ.get("HOME"):
        real_root = Path(os.environ["HOME"]) / ".panergos" / "auth.json"
        try:
...
def _file_lock(
    ...
    supply their own ``threading.local`` so independent locks track reentrancy separately."""
...
def _save_provider_state_to_source(
    ...
    A token refresh rewrites credentials, not the user's choice of provider, so
    ``active_provider`` is left unchanged."""
    if source_path is None or _same_path(source_path, _auth_file_path()):
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    """Set ``active_provider`` only when none is set yet: the first ``panergos auth add`` credential must
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    when the profile has zero entries for it (``panergos auth add`` in the profile shadows global)."""
...
def write_credential_pool(
    ...
    *status_cleared_ids* were cleared deliberately (``panergos auth reset``) and skip the
...
def _explicit_pool_entry_present(normalized: str) -> bool:
    """Pool rows from EXPLICIT Panergos flows (manual add / device-code / PKCE) or live env keys;
...
# Set by Claude Code itself, not by the user explicitly configuring anthropic in Panergos.
_IMPLICIT_ENV_VARS = frozenset({"CLAUDE_CODE_OAUTH_TOKEN"})
_EXPLICIT_POOL_SOURCES = frozenset({"device_code", "loopback_pkce", "panergos_pkce", "manual"})
...
def _explicit_env_credentials_present(normalized: str) -> bool:
    ...
    if pconfig is None:
        from panergos_cli.providers import get_provider
...
def _pool_entry_is_explicit(entry: Any) -> bool:
    """True for pool rows the user created via an explicit Panergos flow (or a still-live env key)."""
...
def _keyless_provider_has_explicit_config(normalized: str) -> bool:
    """Vertex / Bedrock count as explicit when Panergos-scoped routing config is present.
    ...
    Panergos-scoped signals (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH) count
    here."""
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    Panergos-initiated flow, or Panergos-scoped routing config for keyless cloud-SDK providers. Ambient
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    ...
    ``panergos logout``. Returns True if something was cleared."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        from panergos_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""
        lines = ["Config issue detected — run 'panergos doctor' for full diagnostics:"]
        for ci in issues:
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from panergos_cli.config import get_active_config_parse_failure
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run panergos setup.",
        code="corrupt_config")
...
def _openrouter_auto_detected(scoped_key_env: Callable[[str], str]) -> bool:
    ...
    `panergos auth add openrouter` has no env var; without the pool check it is invisible to
    ...
    if any(has_usable_secret(scoped_key_env(v)) for v in ("OPENAI_API_KEY", "OPENROUTER_API_KEY")):
        return True
    try:
        # Auto-detect an OpenRouter credential added via `panergos auth add openrouter` (manual pool entry, no
        # env var). Without this, a key that only lives in the credential pool is invisible to
        # auto-detection — the user sees `panergos auth list` showing the credential while requests go out
...
def _config_model_provider() -> Tuple[Any, Optional[str]]:
    ...
    previously read a llama.cpp/vLLM install as "nothing configured" and the
    dashboard's Ink chat parked every session on Setup Required while ``panergos chat`` worked
    (#108383)."""
    try:
        from panergos_cli.config import load_config
        model_cfg = (load_config() or {}).get("model")
        ...
        if base_url:
            from panergos_cli.runtime_provider import _config_base_url_trustworthy_for_bare_custom
...
def _env_key_auto_detected(
    ...
    preempts a logged-in OAuth provider so a stale key in ~/.panergos/.env never switches silently."""
            ...
            if has_usable_secret(scoped_key_env(env_var)):
                if oauth_active and oauth_active != pid:
                    logger.warning(
                        ...
                        # ~/.panergos/.env isn't silently switched without knowing why.
...
def resolve_provider(
    ...
    *,
    ...
    if normalized != "auto":
        ...
        tail = (f"\n\n{hint}" if hint else " Check 'panergos model' for available providers, "
                "or run 'panergos doctor' to diagnose config issues.")
    ...
    raise AuthError(
        "No inference provider configured. Run 'panergos model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.panergos/.env.",
...
def _quarantine_flat_oauth_state(state: Dict[str, Any], provider: str, exc: "AuthError") -> None:
    ...
    subsequent calls fail fast without a network retry."""
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("panergos_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
    terminal_refresh_codes: FrozenSet[str] = frozenset()  # retrying the same refresh token cannot succeed
    # ``panergos logout`` with no active provider falls back to config.yaml ``model.provider`` only
...
def _pool_first_oauth_status(
    ...
    *,
    ...
    """Return a store-backed OAuth status, preferring a usable pooled credential."""
    try:
        from agent.credential_pool import load_pool
...
def _provider_env_base_url(pconfig: ProviderConfig) -> str:
    if pconfig.id == "actual":
        from panergos_cli.providers import normalize_provider
...
def _provider_is_keyless(provider_id: str) -> bool:
    """PanergosOverlay keyless flag — the same source the provider catalog and GUI contract tests use."""
    try:
        from panergos_cli.providers import PANERGOS_OVERLAYS
        return bool(getattr(PANERGOS_OVERLAYS.get(provider_id), "keyless", False))
    except Exception:
        return False
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    Panergos can't read). Deliberately subprocess-free: spawning ``gh auth token`` from status
    ...
    try:
        from panergos_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def _external_process_spec(
    ...
    Launch details come from the provider's own profile (copilot-acp: PANERGOS_COPILOT_ACP_COMMAND /
    COPILOT_CLI_PATH / PANERGOS_COPILOT_ACP_ARGS), so out-of-tree providers describe their binary."""
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tests that patch ``panergos_cli.auth.get_*_auth_status`` still apply."""
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    CLI startup flat; ``panergos doctor`` runs the live probe). ``api_key`` (default): usable
    ``AZURE_FOUNDRY_API_KEY``."""
    info: Dict[str, Any] = {"provider": "azure-foundry"}
    try:
        from panergos_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            info.update(
                ...
                hint=(
                    ...
                    "is skipped here. Run `panergos doctor` to verify token acquisition."
                ) if installed else (
                    "azure-identity not installed. Install with: "
                    "pip install azure-identity  (or rely on Panergos' "
...
def login_command(args) -> None:
    """Deprecated: use 'panergos model' or 'panergos setup' instead."""
    print("The 'panergos login' command has been removed.\nUse 'panergos auth' to manage credentials,\n"
...
def logout_command(args) -> None:
    ...
    if not should_reset_config:
        ...
        print("Panergos will use OpenRouter for inference.")
    else:
        print("Run `panergos model` or configure an API key to use Panergos.")
...
_PLUGIN_COMPAT_LAZY = {
    'CODEX_OAUTH_USER_AGENT': ('panergos_cli.auth_constants', 'CODEX_OAUTH_USER_AGENT'),
    'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS': ('panergos_cli.auth_codex', 'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS'),
    'DEFAULT_SPOTIFY_REDIRECT_URI': ('panergos_cli.auth_constants', 'DEFAULT_SPOTIFY_REDIRECT_URI'),
    'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS': ('panergos_cli.auth_constants', 'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS'),
    'MINIMAX_OAUTH_GRANT_TYPE': ('panergos_cli.auth_constants', 'MINIMAX_OAUTH_GRANT_TYPE'),
    'OAUTH_OVER_SSH_DOCS_URL': ('panergos_cli.auth_constants', 'OAUTH_OVER_SSH_DOCS_URL'),
    'QWEN_OAUTH_CLIENT_ID': ('panergos_cli.auth_constants', 'QWEN_OAUTH_CLIENT_ID'),
    'QWEN_OAUTH_TOKEN_URL': ('panergos_cli.auth_constants', 'QWEN_OAUTH_TOKEN_URL'),
    'SINGLE_USE_OAUTH_SINGLETON_FILES': ('panergos_cli.auth_oauth_grants', 'SINGLE_USE_OAUTH_SINGLETON_FILES'),
    'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS': ('panergos_cli.auth_constants', 'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS'),
    'SPOTIFY_DASHBOARD_URL': ('panergos_cli.auth_constants', 'SPOTIFY_DASHBOARD_URL'),
    'XAI_OAUTH_DEVICE_CODE_URL': ('panergos_cli.auth_constants', 'XAI_OAUTH_DEVICE_CODE_URL'),
    'XAI_OAUTH_DISCOVERY_URL': ('panergos_cli.auth_constants', 'XAI_OAUTH_DISCOVERY_URL'),
    'XAI_OAUTH_ISSUER': ('panergos_cli.auth_constants', 'XAI_OAUTH_ISSUER'),
}
