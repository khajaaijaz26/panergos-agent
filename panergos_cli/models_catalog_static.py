...
maps. Split out of ``panergos_cli.models``.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    synthesis) so the gateway /model picker and the CLI ``panergos model`` flow share one source."""
    from panergos_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# Mirrors the xAI-direct IDs from $PANERGOS_HOME/models_dev_cache.json; the cache overrides it on the
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Panergos-curated xAI models missing from models.dev, right after the pinned headline."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    """Offline curated floor for xAI / xAI OAuth pickers: $PANERGOS_HOME/models_dev_cache.json
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # (see panergos_cli/model_setup_flows.py); validated live against a GCP project (global region).
    "vertex": [
...
# lists, displays, or iterates providers (panergos model, /model, list_authenticated_providers)
# derives from it. slug = internal ID (config.yaml, --provider); label = short display name;
# tui_desc = longer description for the `panergos model` picker.
# ---------------------------------------------------------------------------
...
# ...) fold under one top-level row in the INTERACTIVE PICKERS (``panergos model``, setup wizard,
...
# Offline/fresh-install fallback for the model Panergos silently lands on when the user never picked
...
# OpenRouter-style ids -> Copilot ids. Dash-notation Claude ids are accepted too: Panergos' default
