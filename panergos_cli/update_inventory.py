...
One read-only pass answering, BEFORE any mutation: which Panergos runtimes run on this machine, how
each is deployed, which ones this update touches, and how each restarts. Every collector is a
side-effect-free probe, so ``panergos update --plan`` is safe on a live fleet.
"""
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Panergos runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "panergos update"
...
def _detect_supervisor_for_pid(pid: int, service_pids: set, windows_service_pids: set | None = None) -> str:
    ...
    with suppress(Exception):
        from panergos_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    return _MECHANISM_DESCRIPTIONS.get(mechanism) or (
        f"panergos -p {profile} gateway restart" if profile != "default" else "panergos gateway restart"
    )
...
def _collect_install_shape(plan: UpdatePlan) -> None:
    with _probe("Install-method probe"):
        from panergos_cli.config import detect_install_method, get_managed_system, recommended_update_command_for_method
        ...
        with _probe("Image provenance probe"):
            # See #91277.
            from panergos_cli.image_provenance import read_image_provenance
...
def _supervisor_classifier() -> Callable[[int], str]:
    ...
    with _probe("Service-PID probe"):
        from panergos_cli.gateway import _get_service_pids
    ...
    with _probe("Windows SCM service-ownership probe"):
        from panergos_cli.gateway import find_windows_gateway_services
...
def _collect_gateway_runtimes(plan: UpdatePlan, profile_homes: list, seen: set[int]) -> None:
    ...
    with _probe("Gateway-state inventory"):
        ...
        from panergos_cli.update_receipt import _socket_identity
        ...
        for profile, home in profile_homes:
            ...
            if sock is not None:
                ...
                # touch, so `panergos update` exited partial (#109680).
    ...
    with _probe("PID-file gateway inventory"):
        from panergos_cli.gateway import find_profile_gateway_processes
...
def _collect_ledger_runtimes(plan: UpdatePlan, seen: set[int]) -> None:
    ...
    (a manual `panergos serve --host <ip>` for a remote Desktop, a long-lived `panergos dashboard`).
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Panergos services: none detected — code swap only.")
        return
...
def _serve_unit_matches_profile(profile: str, unit: object) -> bool:
    """Does *unit* name a ``panergos-serve*``/``panergos-dashboard*`` unit for *profile*? (OWN vocabulary;
    the gateway's ``panergos-gateway*`` names never cover serve/dashboard runtimes.)

    Exact names only — ``work`` must not claim ``panergos-serve-workbench`` — and a scope prefix
    (``user/panergos-serve``) is tolerated because the restart phase records scope-qualified identities in
    some lists. See #100479.
    """
    name = str(unit).removesuffix(".service").rsplit("/", 1)[-1]
    suffix = "" if profile == "default" else f"-{profile}"
    return name in {f"panergos-serve{suffix}", f"panergos-dashboard{suffix}"}
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    Never substring-match: ``foo`` must not claim ``panergos-gateway-foobar.service``.
    Launchd labels are ``ai.panergos.gateway`` / ``ai.panergos.gateway-<profile>`` — they do
    not contain the substring ``panergos-gateway``, so a successful macOS kickstart must
    still credit the planned default gateway. A scope prefix (``user/panergos-gateway``,
    ``gui/501/ai.panergos.gateway``) is stripped the same way serve units are.
    """
    name = str(service).removesuffix(".service").rsplit("/", 1)[-1]
    if profile == "default":
        return name in {"panergos-gateway", "ai.panergos.gateway", "gateway", "gateway-default"}
    return name in {f"panergos-gateway-{profile}", f"ai.panergos.gateway-{profile}", f"gateway-{profile}"}
...
def match_runtime_outcomes(
    ...
    See #91277.
    They never borrow the gateway's outcome: ``relaunched_profiles`` and ``panergos-gateway*`` name a
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    if any(o.get("kind") not in _SERVE_KINDS for o in missed):
        print("      panergos gateway restart                # active profile")
        print("      panergos -p <profile> gateway restart   # named profile")
    if any(o.get("kind") in _SERVE_KINDS for o in missed):
        ...
        # See #100479.
        print("      systemctl --user restart panergos-serve.service   # unit-managed serve")
        print("      relaunch `panergos serve` / `panergos dashboard` / the Desktop app")
    return True
...
def record_plan_in_receipt(plan: UpdatePlan) -> None:
    ...
    try:
        import panergos_cli.update_receipt as ur
