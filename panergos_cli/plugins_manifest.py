...
Split out of :mod:`panergos_cli.plugins`; validation warns and never fails a load.
"""
...
from panergos_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
...
# and continue loading. ``capabilities``/``emits``/``listens``/``panergos``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "panergos", "depends",
    "requires_panergos",
}

# Highest manifest schema version this Panergos understands.
...
def _plugins_debug() -> bool:
    from panergos_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Panergos "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `panergos plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `panergos plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # Panergos version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    # recorded with an error and skipped before import — see ``requires_panergos_error``.
    requires_panergos: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — Panergos never auto-installs these (isolation design for the install seam
...
# ── requires_panergos version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_panergos_version() -> str:
    """Installed ``panergos-agent`` distribution version, else ``panergos_cli.__version__`` (source checkout)."""
    try:
        return importlib.metadata.version("panergos-agent")
    except Exception:
...
def requires_panergos_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_panergos`` rejects the running version."""
    if not manifest.requires_panergos:
        return None
    current = running_panergos_version()
    if version_satisfies(manifest.requires_panergos, current):
        return None
    return f"requires panergos {manifest.requires_panergos}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_panergos=str(data.get("requires_panergos") or "").strip(),
