...
from panergos_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import panergos_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $PANERGOS_HOME/node is not on PATH, so
    ...
    "browser": lambda: (agent_browser_runnable(shutil.which("agent-browser")) or _has_system_browser()
                        or _has_panergos_agent_browser() or _has_npx_agent_browser()),
...
def _has_panergos_agent_browser() -> bool:
    from panergos_constants import get_panergos_home  # late import: tests patch panergos_constants.get_panergos_home
    ...
    # install.sh installs into $PANERGOS_HOME/node/bin/ via npm -g --prefix; legacy git clones used node_modules/.bin/.
...
def ensure_dependency(dep: str, interactive: bool = True) -> bool:
    ...
    if shell == "powershell":
        ...
        if not ps_bin:
            ...
            return False
        cmd = [ps_bin, "-ExecutionPolicy", "Bypass", "-File", str(script), "-Ensure", dep, "-PanergosHome", str(get_panergos_home())]
    else:
        cmd = ["bash", str(script), "--ensure", dep]
    run_env = {**panergos_subprocess_env(inherit_credentials=False), "IS_INTERACTIVE": "false"}
