"""On-demand worktree + branch reclaim (``panergos worktree`` / ``/worktree prune``).
...
def _archive_untracked(tree: Path, untracked: List[str]) -> Optional[Path]:
    ...
    dest = get_panergos_home() / "archive" / "worktree-prune" / f"{tree.name}-{stamp}"
    try:
...
def _classify_tree(_ops, repo_root: str, entry: Path, merge_cache, remote_heads) -> tuple[str, str, List[str]]:
    ...
    if _ops._worktree_lock_is_live(repo_root, path, timeout=5) == "live":
        return "keep", "in use by a running panergos session", []
...
def audit_external_trees(repo_root: str) -> List[ExternalTreeRecord]:
    ...
    ``panergos -w`` scratch trees all live under ``<repo>/.worktrees/``, but
    ...
    def _flush():
        ...
        if real == managed_root or real.startswith(managed_root + os.sep):
            return  # panergos-managed scratch tree — covered by audit_worktrees
...
def audit_worktrees(repo_root: str, *, with_sizes: bool = True,
    ...
    """
    from panergos_cli import worktree_ops as _ops
...
def audit_branches(repo_root: str) -> List[BranchRecord]:
    ...
    if workers > 1:
        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=workers, thread_name_prefix="panergos-branch-gc") as pool:
