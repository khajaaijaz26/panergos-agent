...
disk cache). Split out of ``panergos_cli.models``; helpers still defined there are looked up on
``panergos_cli.models`` at call time so ``patch("panergos_cli.models.<name>")`` mocks keep intercepting.
"""
...
from panergos_cli.urllib_security import url_origin
...
def _get_ollama_base_url() -> str:
    ...
    from panergos_cli.models import _get_model_config_dict
...
def probe_ollama_local_models(
    ...
    from panergos_cli.models import _PANERGOS_USER_AGENT, _get_ollama_base_url, _urlopen_model_catalog_request
    ...
    if failed_at is not None:
        ...
        _OLLAMA_LOCAL_PROBE_FAILURE_CACHE.pop(failure_key, None)
    try:
        request_headers = {"User-Agent": _PANERGOS_USER_AGENT, **(headers or {})}
...
def _ollama_local_catalog(force_refresh: bool) -> list[str]:
    ...
    Cloud)."""
    from panergos_cli.models import _get_provider_config_dict, fetch_api_models
    if force_refresh:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    return {"User-Agent": _PANERGOS_USER_AGENT, **({"Authorization": f"Bearer {token}"} if token else {})}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_panergos_home() / "ollama_cloud_models_cache.json"
