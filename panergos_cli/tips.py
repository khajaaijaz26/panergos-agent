...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or panergos -c.",
    ...
    "Ctrl+Z suspends Panergos to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "panergos -c resumes your most recent CLI session. panergos -c \"project name\" resumes by title.",
    "panergos -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "panergos -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "panergos chat -t web,terminal enables only specific toolsets for a focused session.",
    "panergos chat -s github-pr-workflow preloads a skill at launch.",
    "panergos chat -q \"query\" runs a single non-interactive query and exits.",
    "panergos chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "panergos chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "panergos --yolo bypasses all dangerous command approval prompts for the entire session.",
    "panergos chat --source telegram tags the session for filtering in panergos sessions list.",
    "panergos -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "panergos doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "panergos dump outputs a compact setup summary — great for bug reports.",
    "panergos config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "panergos config edit opens config.yaml in your default editor.",
    "panergos config check scans for missing or stale configuration options.",
    "panergos sessions browse opens an interactive session picker with search.",
    "panergos sessions stats shows session counts by platform and database size.",
    "panergos sessions prune --older-than 30 cleans up old sessions.",
    "panergos skills search react --source skills-sh searches the skills.sh public directory.",
    "panergos skills check scans installed hub skills for upstream updates.",
    "panergos skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "panergos skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "panergos mcp add github --command npx adds MCP servers from the command line.",
    "panergos mcp serve runs Panergos itself as an MCP server for other agents.",
    "panergos auth add lets you add multiple API keys for credential pool rotation.",
    "panergos completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "panergos logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "panergos backup creates a zip backup of your entire Panergos home directory.",
    "panergos profile create coder creates an isolated profile that becomes its own command.",
    "panergos profile create work --clone copies your current config and keys to a new profile.",
    "panergos update syncs new bundled skills to ALL profiles automatically.",
    "panergos gateway install sets up Panergos as a system service (systemd/launchd).",
    "panergos memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "panergos webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: panergos tools disables unused tools, panergos skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "panergos models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as panergos -w).",
    ...
    "PANERGOS_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Panergos tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'panergos profile create coder' creates the 'coder' command.",
    "panergos profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Panergos prints a resume command with session ID and stats.",
    "panergos sessions export backup.jsonl exports all sessions for backup or analysis.",
    "panergos -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "panergos skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .panergos/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: panergos cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Panergos runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "panergos gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Panergos via a local macOS server.",
    ...
    "Context files (.panergos.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Panergos loads project context from .panergos.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "panergos mcp opens an interactive picker of curated MCPs you can install in one keystroke.",
    "panergos mcp catalog lists curated MCP servers shipped with the repo.",
    "panergos mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — panergos tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.panergos/checkpoints/ — your project's .git is never touched.",
    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "panergos chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "panergos plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "panergos acp runs Panergos as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "PANERGOS_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "panergos auth add openai-codex sets up an OAuth-based provider.",
    ...
    "panergos status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.panergos/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Panergos your own.",
    ...
    "Before a gateway session reset, Panergos auto-flushes important facts to memory in the background.",
    ...
    "execute_code helpers require explicit imports: from panergos_tools import json_parse, shell_quote, retry.",
    ...
    "The skills audit log at ~/.panergos/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Panergos state via PANERGOS_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "PANERGOS_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in PANERGOS_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.panergos/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'panergos profile create ls' would be rejected.",
    "panergos profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "panergos claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "PANERGOS_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via panergos tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.panergos/.env.",
    "Official Panergos skills get a 'builtin' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.panergos/skills/.hub/quarantine/ holds skills pending security review.",
    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Panergos auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Panergos config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Panergos is working.',
    ...
    '/reload reloads ~/.panergos/.env into the running session — pick up new API keys without restarting.',
    ...
    "PANERGOS_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.panergos/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.panergos/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    # --- Curator ---
    'panergos curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "panergos curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'panergos curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'panergos auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through a configured gateway.',
    ...
    # --- TUI & Dashboard ---
    'PANERGOS_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "PANERGOS_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'panergos dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    ...
    'PANERGOS_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'PANERGOS_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'PANERGOS_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.panergos/cache/piper-voices/.',
    ...
    'panergos chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'panergos chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'panergos chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "panergos chat --source tool tags programmatic chats so they don't clutter panergos sessions list.",
    'panergos dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'panergos sessions rename <ID> "new title" renames any past session; panergos sessions delete <ID> removes one.',
    'panergos import restores a full Panergos backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'panergos fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'panergos pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'panergos setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'panergos status --deep runs the full health sweep across every component; plain panergos status is the quick view.',
    # --- Agent Behavior Env Vars ---
    'PANERGOS_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'PANERGOS_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.panergos/plugins/ — trust-gated by design.',
    "PANERGOS_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'PANERGOS_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'PANERGOS_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'PANERGOS_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'PANERGOS_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'PANERGOS_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'PANERGOS_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',
    # --- Gateway Behavior Env Vars ---
    'PANERGOS_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'PANERGOS_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'PANERGOS_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'PANERGOS_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Panergos block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from panergos sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.panergos/dashboard-plugins/.',
]
...
# Task-oriented example prompts for the empty composer. Kept generic — Panergos is
