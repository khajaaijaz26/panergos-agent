...
Callers: ``_early_recovery.recover_if_needed`` (stdlib-only, runs BEFORE ``panergos_cli.main``'s
third-party imports so a pending update completes while no native extension is mapped yet) and
``panergos_cli.main_install_repair._recover_core_update_marker_locked`` (the post-import recovery path).
...
from panergos_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (panergos_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``panergos acp`` speaks JSON-RPC on stdout; an inherited-fd install child writing there would
...
def _venv_scripts_dir(root: Path) -> Path | None:
    """Project venv Scripts/bin dir, when present (panergos_constants is stdlib-only)."""
    # panergos_constants is stdlib-only, so the canonical layout helpers are safe to use from this
...
#: Panergos root's ``bin``, next to uv.exe). Keep in lockstep with scripts/install.ps1.
_WINDOWS_BIN_LAUNCHERS = ("panergos", "panergos-acp")
...
def _default_panergos_root() -> Path | None:
    """The DEFAULT Panergos root (not ``get_panergos_home()``, which under ``panergos -p <name>`` is
    ...
    try:
        return Path(get_default_panergos_root())
    except Exception:
        return None
...
def ensure_windows_bin_launchers(
    ...
    r"""Re-stage the Windows ``panergos`` launchers when they vanish.

    On Windows, ``panergos`` resolves through launchers derived from the venv console scripts — never
    ...
    The canonical launcher home is the managed binary dir — the default Panergos root's ``bin``
    (``%LOCALAPPDATA%\\panergos\\bin``, next to the managed uv) — which lives OUTSIDE the git checkout so no
    git operation can ever touch it. It is a per-machine dir shared by every profile: ``get_panergos_home()``
    would point inside ``profiles\\<name>`` under ``panergos -p``, so the anchor here is
    :func:`panergos_constants.get_default_panergos_root`. See #83797.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
        return []
    root = Path(root)
    home = _default_panergos_root()
    if home is None:
        return []

    targets: list[Path] = []
    # Runs at every panergos_cli.main process start, so the healthy path must stay a few stat calls.
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print("  ✓ Restored Panergos launcher(s): " + ", ".join(restored), file=sys.stderr)
    return restored
...
def migrate_windows_bin_path(
    ...
    """One-time PATH migration to the ``PANERGOS_HOME\\bin`` launcher layout (``panergos update`` tail).
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    if kept != entries:
        try:
            ...
            return False
        with contextlib.suppress(OSError, ValueError):
            print(f"  ✓ Panergos launchers now resolve from {home_bin} "
...
def _quarantine_running_panergos_exe(
    ...
    """Rename live panergos*.exe shims aside so the installer can rewrite them.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    (#75584). Warnings land on stderr — this module runs in the early-recovery path and ``panergos acp``
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = _quarantine_running_panergos_exe(scripts_dir, failed_out=failed) if scripts_dir else []
    if failed:
    ...
    try:
        ...
        # the shims renamed aside and `panergos` gone from PATH. Restore only renames back when the
