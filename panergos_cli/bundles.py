"""Implementation of the ``panergos bundles`` CLI subcommand."""
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  panergos bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``panergos bundles`` argparse tree (called from panergos_cli/main.py, which owns the
    top-level subparser)."""
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `panergos bundles list`")
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``panergos bundles <subcommand>`` to the right handler."""
