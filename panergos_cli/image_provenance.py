"""Image-authored deployment provenance for immutable Panergos runtimes.

The image bakes ``/etc/panergos/image-provenance.json`` outside ``$PANERGOS_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/panergos/image-provenance.json")
