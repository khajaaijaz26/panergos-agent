"""Migrate Panergos MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
MIGRATION_END_MARKER = "# end panergos-agent managed section"
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in panergos config.")
...
# Panergos MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown Panergos keys.
_KNOWN_PANERGOS_KEYS = {
...
# (panergos key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, panergos_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one Panergos MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. Panergos' ``transport: sse`` hint is
    informational only — codex auto-negotiates. ``enabled`` is emitted only when explicitly false
    (codex defaults to true).
    """
    if not isinstance(panergos_cfg, dict):
        return None, []
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if panergos_cfg.get("command"):
        if panergos_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(panergos_cfg["command"])
        if panergos_cfg.get("args"):
            out["args"] = [str(a) for a in panergos_cfg["args"]]
        if panergos_cfg.get("env"):
            out["env"] = _str_map(panergos_cfg["env"])
        if panergos_cfg.get("cwd"):
            out["cwd"] = str(panergos_cfg["cwd"])
    elif panergos_cfg.get("url"):
        out["url"] = str(panergos_cfg["url"])
        if panergos_cfg.get("headers"):
            out["http_headers"] = _str_map(panergos_cfg["headers"])
        if panergos_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for panergos_key, codex_key, note in _TIMEOUT_KEYS:
        if panergos_key in panergos_cfg:
            try:
                out[codex_key] = float(panergos_cfg[panergos_key])
            except (TypeError, ValueError):
                skipped.append(note)
    if panergos_cfg.get("enabled") is False:
        out["enabled"] = False
    for key in panergos_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_PANERGOS_KEYS:
            skipped.append(f"{key} (unknown Panergos key)")
    return out, skipped
...
# env-var passthrough (PANERGOS_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by Panergos)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(codex_home=str(codex_home) if codex_home else None) as client:
            client.initialize(client_name="panergos-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a PANERGOS_HOME pointing there burned into
...
def _build_panergos_tools_mcp_entry() -> dict:
    """Codex stdio entry launching Panergos' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    PANERGOS_HOME passes through only IF SET, read from os.environ (not get_panergos_home()): when
    unset the codex subprocess must inherit its launcher's runtime PANERGOS_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched PANERGOS_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched panergos finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # PANERGOS_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_panergos_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever PANERGOS_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's PANERGOS_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("PANERGOS_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    # user's real ~/.codex/config.toml and silently brick codex once the tempdir is GC'd.
    panergos_home = os.environ.get("PANERGOS_HOME") or ""
    if panergos_home and not _looks_like_test_tempdir(panergos_home):
        env["PANERGOS_HOME"] = panergos_home
    if os.environ.get("PYTHONPATH"):
        env["PYTHONPATH"] = os.environ["PYTHONPATH"]
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["PANERGOS_QUIET"] = "1"
    env["PANERGOS_REDACT_SECRETS"] = env.get("PANERGOS_REDACT_SECRETS", "true")
    return {
...
def migrate(
    panergos_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_panergos_tools: bool = True) -> MigrationReport:
    """Translate Panergos mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_panergos_tools`` registers Panergos' own tool surface
    (agent/transports/panergos_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    panergos_servers = (panergos_config or {}).get("mcp_servers") or {}
    if not isinstance(panergos_servers, dict):
        report.errors.append("mcp_servers in panergos config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in panergos_servers.items():
        name = str(raw_name)
    ...
    if expose_panergos_tools:
        translated["panergos-tools"] = _build_panergos_tools_mcp_entry()
        if "panergos-tools" not in report.migrated:
            report.migrated.append("panergos-tools")
