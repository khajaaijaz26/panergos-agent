...
_MANAGED_FILES_ROOT_ENV = "PANERGOS_DASHBOARD_FILES_ROOT"
...
def _default_panergos_root_is_opt_data() -> bool:
    raw = os.environ.get("PANERGOS_HOME", "").strip()
    if not raw:
        return False
    try:
        from panergos_constants import get_default_panergos_root

        root = get_default_panergos_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """True when the dashboard should not offer ``panergos update``.

    Containerized dashboards are updated by the outer launcher/image — except a
    ``git`` install (bind-mounted checkout, e.g. the panergos-webui image), where
    ...
    if _default_panergos_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the Panergos
    # root actually IS /opt/data or PANERGOS_DASHBOARD_FILES_ROOT is set.
    if _default_panergos_root_is_opt_data():
