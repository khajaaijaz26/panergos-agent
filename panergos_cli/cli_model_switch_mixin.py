...
Mixin split out of ``cli.py``; bound onto ``PanergosCLI`` via the MRO. cli.py-internal
...
module-level functions taking ``cli`` and siblings are called as ``PanergosCLI.<name>(self, ...)``.
"""
...
def stored_session_route(session_meta, *, current_model, current_provider):
    ...
    if not stored_model:
        return None
    from panergos_state import SessionDB as _SessionDB
...
def _merge_preflight_warning(cli, result, custom_providers) -> None:
    ...
    if cli.agent is None:
        return
    try:
        from panergos_cli.context_switch_guard import merge_preflight_compression_warning
...
def _print_switch_summary(cli, result, old_model, *, one_turn: bool, strict_context: bool) -> None:
    ...
    from panergos_cli.model_switch import format_model_for_display, resolve_display_context_length
    ...
    # Provider-aware context chain: Codex OAuth and Copilot caps win over the raw
...
def _commit_model_switch(
    ...
    from cli import PanergosCLI, _cprint
    old_model = cli.model
    ...
    if persist_global:
        from panergos_cli.model_switch import persist_model_selection
    ...
    if not one_turn:
        PanergosCLI._persist_model_switch_to_session(cli, result)
...
def _show_model_picker(cli, ctx, force_refresh: bool) -> None:
    ...
    from cli import _cprint
    from panergos_cli.inventory import build_models_payload
...
class CLIModelSwitchMixin:
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            from panergos_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if opencode_provider_family(resolved_provider) is not None:
            from panergos_cli.models import normalize_opencode_model_id, opencode_model_api_mode
            return _adopt_with_mode(
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        if warning is None:
            return True
        choices = [
            ("once", "Switch anyway", "Use this model for the current panergos session."),
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from cli import _cprint
        from panergos_cli.model_switch import parse_model_switch_args, resolve_persist_behavior
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Panergos default (chat_completions)
            /codex-runtime codex_app_server      — hand turns to codex subprocess
            /codex-runtime on / off              — synonyms for the above
        """
        from cli import _cprint
        from panergos_cli import codex_runtime_switch as crs
    ...
    def _cmd_moa(self, cmd_original: str):
        ...
        from panergos_cli.moa_config import moa_usage, normalize_moa_config
