...
Mixin split out of ``cli.py``; bound onto ``PanergosCLI`` via the MRO. cli.py-internal symbols are
...
from panergos_constants import is_termux as _is_termux_environment
from rich.markup import escape as _escape
from utils import base_url_hostname

from panergos_cli.cli_modal_mixin import _gated_confirm
...
class CLIInfoMixin:
    ...
    def show_banner(self):
        ...
        if self.compact or shutil.get_terminal_size().columns < 80:
            ...
            from panergos_cli.banner import (
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Panergos needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # `panergos skills trust`. Never raises.
        try:
            ...
            if _proj_dirs:
                ...
                if _untrusted is not None:
                    ...
                    self._console_print(
                        ...
                        f"loaded — run `panergos skills trust` to enable them.[/]")
        except Exception:
    ...
    def show_help(self, arg: str = ""):
        ...
        from panergos_cli.commands import COMMANDS_BY_CATEGORY, HELP_SESSION_SUBGROUPS
    ...
    def handle_bang_shell(self, text: str) -> bool:
        ...
        (tests/panergos_cli/test_bang_shell_mode.py). Returns False when the text is not a bang command or
        bang mode is disabled for this context (gateway/cron), so the caller routes normally.
        """
        from cli import _rich_text_from_ansi
        from panergos_cli.bang_shell import (
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_panergos_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_panergos_home()}/config.yaml")
            print()

    def _print_random_tip(self) -> None:
        """Best-effort discovery tip (startup + /clear); never raises."""
        try:
            from panergos_cli.tips import get_random_tip
            _tip = get_random_tip()
            try:
                from panergos_cli.skin_engine import get_active_skin
                _tip_color = get_active_skin().get_color("banner_dim", "#B9A9C8")
            except Exception:
                _tip_color = "#B9A9C8"
    ...
    def _toggle_verbose(self):
        ...
        if getattr(self, "_focus_view_enabled", False):
            ...
            try:
                from panergos_cli.focus_view import FOCUS_CONFIG_KEY
    ...
    def _usage_reset(self, force: bool = False):
        ...
        if str(self._agent_or_self("provider") or "").strip().lower() != "openai-codex":
            ...
            print("  Switch with `/model` or `panergos auth` first.")
            return
    ...
    def _show_usage(self):
        """Rate limits and session token usage when a live agent exists."""
    ...
    def _show_insights(self, command: str = "/insights"):
        ...
        try:
            from panergos_state import SessionDB, _default_db_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.panergos/skills/ and queue a note for the next user turn.
