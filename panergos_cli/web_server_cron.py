"""Dashboard cron helpers: per-profile scheduler I/O, validation, and job firing."""
...
from panergos_cli.web_models import CronJobCreate
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from panergos_cli.web_server_profiles import _fallback_profile_dicts
...
def _cron_default_profile() -> str:
    ...
    profile's job into ~/.panergos. ``custom`` (PANERGOS_HOME outside the profiles tree) has no
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, PANERGOS_HOME)."""
    from panergos_cli import profiles as profiles_mod
...
def _cron_store_scope(home: Path):
    """Point PANERGOS_HOME and the cron.jobs store at one profile's home for the block.
