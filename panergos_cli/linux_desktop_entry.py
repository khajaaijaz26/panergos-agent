"""Install and remove the Linux desktop entry (``panergos.desktop``).
...
DESKTOP_ENTRY_NAME = "panergos.desktop"
...
def _can_import_panergos_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``panergos_cli.main`` unaided.
    ...
    ok = _run_quiet(
        [key, "-I", "-c", "import panergos_cli.main"],
...
def _running_interpreter_fallback() -> str:
    """The RUNNING interpreter — it has ``panergos_cli`` importable by definition."""
    return os.path.abspath(sys.executable)


def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``panergos desktop``.

    Prefer the installed Panergos launcher; fall back to ``<python> -m panergos_cli.main desktop``.
    """
    bin_path = _resolve_panergos_bin_for_desktop_entry(checkout_root=project_root)
    interpreter = _running_interpreter()
    if not _can_import_panergos_cli(Path(interpreter)):
        ...
        # The candidate interpreter cannot actually import panergos_cli.main (checked in isolated mode from a
    ...
    argv = [interpreter, "-m", "panergos_cli.main", "desktop"]
    if bin_path:
        resolved = Path(bin_path).resolve()
        # A Python launcher whose shebang points OUTSIDE the venv (e.g. the repo's `panergos` script
...
def _inside_checkout(candidate: str, checkout_root: Path, original_argv0: str) -> bool:
    ...
    try:
        ...
        return False
    # Anything shipped in the tree (e.g. the repo `panergos` script) is checkout-internal. Compare
    ...
    for root in {checkout_root, resolved_root}:
        if root is not None and (path == root or root in path.parents):
            return True
    # The `python -m panergos_cli.main` relaunch context surfaces the invoking interpreter as
...
def _resolve_panergos_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`panergos_cli.relaunch.resolve_panergos_bin` with one rule: an ``argv[0]`` inside this
    ...
    if resolve_fn is None:
        panergos_bin = shutil.which("panergos")
        from panergos_cli.relaunch import resolve_panergos_bin as resolve_fn
    ...
    # An external primary (another install's /opt/.../bin/panergos, a venv console script) wins
    ...
    primary = panergos_bin or resolve_fn()
    ...
    # /opt/.../bin/panergos from another install method, or a venv console script). It must be evaluated
    ...
    # argv[0] was checkout-internal AND PATH had no launcher — common in stripped systemd user
...
def _known_wrapper_candidates():
    ...
    (``/usr/local/bin``), and user (``~/.local/bin``).
    """
    candidates = []
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "panergos")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/panergos"))
    candidates.append(Path.home() / ".local" / "bin" / "panergos")
    return candidates
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Panergos\n"
        "GenericName=Panergos Desktop\n"
        "Comment=Launch Panergos Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Panergos\n"
    )
...
def _hicolor_icon_dest(subdir: str) -> Path:
    return _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "panergos.png"
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    """Install the app icon into the user's hicolor tree so ``Icon=panergos`` resolves without an
...
def _launcher_entry_management_enabled() -> bool:
    ...
    rewrite: a hand-edited ``panergos.desktop`` is then left alone instead
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    ...
    icon_value = str(icon) if icon.is_file() else "panergos"
