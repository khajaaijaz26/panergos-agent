"""Configuration management for Panergos Agent: config.yaml / .env loading, saving,
validation, migration, and the ``panergos config`` command."""
...
from panergos_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from panergos_cli.secret_prompt import masked_secret_prompt
# Re-export from panergos_constants — canonical definition lives there.
from panergos_constants import get_panergos_home, get_process_panergos_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    ...
    "refuse-write": (
        ...
        "Fix the YAML (panergos config edit) and retry.")}
...
def _warn_config_parse_failure(
    ...
    from panergos_cli.config_backups import backup_config
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  panergos config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# interpreter init (PYTHON*, NODE_*: Panergos restarts through them), PATH (fix tool lookup
...
# and Panergos runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``PANERGOS_*`` overall is NOT blocked — many integration credentials use that prefix
# (PANERGOS_LANGFUSE_PUBLIC_KEY, PANERGOS_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Panergos runtime location
    "PANERGOS_HOME", "PANERGOS_PROFILE", "PANERGOS_CONFIG", "PANERGOS_ENV",
    "PANERGOS_CONFIG_PATH", "PANERGOS_ENV_PATH",
    # MCP catalog trust root; package-manager wrappers may still set it in the process env.
    "PANERGOS_OPTIONAL_MCPS",
    # Local ACP subprocess selection (executable/argv authority).
    "PANERGOS_COPILOT_ACP_COMMAND", "PANERGOS_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "PANERGOS_YOLO_MODE", "PANERGOS_ACCEPT_HOOKS", "PANERGOS_REDACT_SECRETS",
    "PANERGOS_INTERACTIVE", "PANERGOS_EXEC_ASK", "PANERGOS_GATEWAY_SESSION",
    "PANERGOS_CRON_SESSION", "PANERGOS_SINGLE_QUERY_SESSION",
    "PANERGOS_SESSION_KEY", "PANERGOS_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Panergos runtime location and security policy (PANERGOS_HOME, PANERGOS_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.panergos/.env "
            "directly.")
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean PANERGOS_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "PANERGOS_TOOL_PROGRESS_MODE",
    ...
    "PANERGOS_LANGFUSE_ENV", "PANERGOS_LANGFUSE_RELEASE", "PANERGOS_LANGFUSE_SAMPLE_RATE",
    "PANERGOS_LANGFUSE_MAX_CHARS", "PANERGOS_LANGFUSE_CAPTURE", "PANERGOS_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "PANERGOS_ACP_AUTH_METHOD", "PANERGOS_ACP_AUTO_APPROVE", "PANERGOS_COPILOT_ACP_COMMAND",
    "PANERGOS_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# PANERGOS_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
# Explicit opt-out (``PANERGOS_MANAGED=false``): without this a bool-shaped value became a package
# manager literally named "false" and is_managed() blocked `panergos update` (#12864).
_MANAGED_FALSE_VALUES = frozenset({"false", "0", "no", "off"})


def get_managed_system() -> Optional[str]:
    """Return the package manager owning this install, if any.
    Signals: PANERGOS_MANAGED env var (systemd service) or a ``.managed`` marker file in
    PANERGOS_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("PANERGOS_MANAGED", "").strip().lower() or None
    managed_marker = get_panergos_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Panergos is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update Panergos through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Panergos was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$PANERGOS_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because PANERGOS_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``panergos update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``khajaaijaz26/panergos-agent`` image bakes a ``docker`` stamp into
    ``/opt/panergos`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of panergos_cli/), NOT of $PANERGOS_HOME,
    ...
    method = _install_method_stamp(get_panergos_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``panergos_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": (
        ...
        "https://github.com/khajaaijaz26/panergos-agent.git#main"
    ),
    "apt": "pkg upgrade panergos-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "panergos update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (panergos_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Panergos.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``panergos update`` doesn't update a running Docker container.

Panergos does not publish a prebuilt container image yet. Build the public source
branch, then recreate the container from that local image:

  docker build -t panergos-agent:local https://github.com/khajaaijaz26/panergos-agent.git#main
  # then restart whatever started the container using panergos-agent:local

Verify the new version after restart:
  docker run --rm panergos-agent:local --version

Notes:
  • Your config and session history live under ``$PANERGOS_HOME`` (``/opt/data``
    in the container, typically bind-mounted from the host) and persist
    across image upgrades when you keep that volume mounted."""


def format_docker_update_message() -> str:
    """Return the user-facing message for ``panergos update`` inside Docker."""
    return _DOCKER_UPDATE_MESSAGE


def format_managed_message(action: str = "modify this Panergos installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Panergos installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Panergos.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from PANERGOS_HOME/.container-mode.
    ...
    already inside the container, or when PANERGOS_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("PANERGOS_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_panergos_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "container_name": info.get("container_name", "panergos-agent"),
        "exec_user": info.get("exec_user", "panergos"),
        "panergos_bin": info.get("panergos_bin", "/data/current-package/bin/panergos")}


# ---- Config paths / PANERGOS_HOME skeleton ----

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_panergos_home() / "config.yaml"
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / PANERGOS_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("PANERGOS_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_panergos_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read PANERGOS_UID / PANERGOS_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns PANERGOS_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Panergos commonly set these to map the in-container user to a host user so
    ...
    return _env_int("PANERGOS_UID"), _env_int("PANERGOS_GID")


def _chown_to_panergos_uid(path) -> None:
    """Chown ``path`` to ``PANERGOS_UID:PANERGOS_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_panergos_home` on Docker deployments. See #34107.
    """
    uid, gid = _resolve_panergos_uid_gid()
...
def _secure_dir(path):
    ...
    Also applies ``PANERGOS_UID``/``PANERGOS_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    explicit_mode = os.environ.get("PANERGOS_HOME_MODE", "").strip()
    ...
    # start (#10757). An explicit PANERGOS_HOME_MODE is the operator's choice and is still applied.
...
def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or PANERGOS_CONTAINER / PANERGOS_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("PANERGOS_CONTAINER") or os.environ.get("PANERGOS_SKIP_CHMOD")
...
_PANERGOS_HOME_ENSURED: set = set()
_PANERGOS_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_panergos_home():
    """Ensure the ~/.panergos directory skeleton exists with secure permissions.
    ...
    from panergos_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _PANERGOS_HOME_ENSURED and home.is_dir():
        return
...
from panergos_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from panergos_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use panergos_cli.config.<name>)
...
# Back-compat re-exports — :mod:`panergos_cli.personality` owns personality/overlay semantics.
from panergos_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``panergos config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `panergos config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    return (
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `panergos update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (panergos_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # panergos_cli/tools_config.py toolset-save flow
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'panergos tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from panergos_cli.config_home import config_load_issue
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — Panergos won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'panergos doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        ...
        hint_path = display_panergos_home()
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from panergos_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ panergos config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from panergos_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from panergos_cli.toolset_validation import validate_platform_toolsets
        from panergos_cli.toolset_scope import toolset_allowed_for_platform
...
def _offer_list(heading: str, items: List[str], question: str) -> bool:
    ...
    if not _ask_yes_no(question):
        print("  Set later with: panergos config set <key> <value>")
        return False
    print()
    return True
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.panergos/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_panergos_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``panergos config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``panergos status``/``dump``) read ``name`` and
...
# the PANERGOS_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Panergos host
...
def _last_known_good_fallback(config_path: Path, path_key: str, cache_sig, exc: Exception) -> Optional[Dict[str, Any]]:
    ...
    if lkg is None:
        # Fresh process (CLI restart, `panergos config get`): nothing loaded yet in this process, so
        ...
        from panergos_cli.config_backups import load_newest_good_backup
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # load_panergos_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_panergos_dotenv() pins unexpanded literals
...
#   openai-codex (OAuth — panergos auth) — OpenAI Codex
...
def save_config(
    ...
    """Save configuration to ~/.panergos/config.yaml.
...
def load_env() -> Dict[str, str]:
    """Load ~/.panergos/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def sanitize_env_file() -> int:
    """Rewrite ~/.panergos/.env with normalized line formatting; returns the number of changed lines."""
    env_path = get_env_path()
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.panergos/.env (also matching ``export KEY=`` lines, so a save
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.panergos/.env and os.environ; True if it was found and removed."""
...
def reload_env() -> int:
    """Re-read ~/.panergos/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Panergos (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.panergos/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Panergos-managed credential preferring ``~/.panergos/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale PANERGOS_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("PANERGOS_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale PANERGOS_MAX_ITERATIONS={env_ghost} "
                    f"(run 'panergos doctor --fix' to remove)", Colors.YELLOW))
...
def show_config():
    ...
    print(color("  panergos config edit     # Edit config file", Colors.DIM))
    print(color("  panergos config set <key> <value>", Colors.DIM))
    print(color("  panergos setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``PANERGOS_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("PANERGOS_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "`panergos cron edit <job_id> --provider <provider> --model <model>` or set a fleet default "
        "with `panergos config set cron.model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``panergos config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``panergos config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    panergos config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    panergos config set --force {key} {value!r}"]
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_panergos_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Panergos may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `panergos config
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from panergos_cli.credential_lifecycle import remove_provider_env_credential
...
_USAGE_GET = ("Usage: panergos config get <key> [--json]", [
    "panergos config get model", "panergos config get terminal.backend",
    "panergos config get skills.config --json"], None)
_USAGE_SET = ("Usage: panergos config set [--force] <key> <value>", [
    "panergos config set model anthropic/claude-sonnet-4", "panergos config set terminal.backend docker",
...
_USAGE_UNSET = ("Usage: panergos config unset <key>", [
    "panergos config unset model", "panergos config unset terminal.backend",
    "panergos config unset OPENROUTER_API_KEY"], None)
...
def _cmd_config_check(args):
    ...
    if missing_config:
        ...
        print("    Run 'panergos config migrate' to add them")
...
_CONFIG_USAGE = """Available commands:
  panergos config           Show current configuration
  panergos config edit      Open config in editor
  panergos config get <key>          Print a resolved config value
  panergos config set <key> <value>   Set a config value
  panergos config unset <key>        Remove a config value
  panergos config check     Check for missing/outdated config
  panergos config migrate   Update config with new options
  panergos config path      Show config file path
  panergos config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``panergos config`` UI without the core knowing they exist.
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``panergos_cli/`` — i.e. the git checkout for source
    installs, ``/opt/panergos`` inside the published image. It is a property of
    the running interpreter, NOT of ``$PANERGOS_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$PANERGOS_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/panergos`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('panergos_cli.route_identity', 'normalize_route_base_url'),
}
