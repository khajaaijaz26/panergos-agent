"""``panergos gateway migrate --multiplex`` / ``--standalone``: move a per-profile-gateway install onto one
...
what the multiplexer would do at startup. ``panergos update`` calls :func:`maybe_auto_migrate_after_update`.
"""
...
MIGRATE_COMMAND = "panergos gateway migrate --multiplex"
...
class ProfileGateway:
    ...
    runtime_home: Optional[Path] = None  # PANERGOS_HOME the installed unit pins, when it differs from ``home``
...
class MigrationPlan:
    ...
    def eligible_for_migration(self) -> bool:
        ...
        This is the AUTO-migration (``panergos update``) bar; the explicit command also proceeds with
...
def _home_env(home: Path) -> Iterator[None]:
    """Run service-manager helpers as if ``home`` were the active PANERGOS_HOME. Both the contextvar
    override (``get_panergos_home``) and ``os.environ`` (``gateway.status`` identity files, unit
    ...
    previous = os.environ.get("PANERGOS_HOME")
    token = set_panergos_home_override(str(home))
    os.environ["PANERGOS_HOME"] = str(home)
    panergos_constants._default_panergos_root_memo = None
    try:
        yield
    finally:
        ...
        if previous is None:
            ...
            os.environ["PANERGOS_HOME"] = previous
        panergos_constants._default_panergos_root_memo = None
...
def _gateway_identity(home: Path, pid: Optional[int], service: Optional[tuple[str, bool]]) -> tuple[Optional[int], Path]:
    from panergos_cli.gateway_migrate_guards import gateway_identity
...
def _installed_service(home: Path) -> Optional[tuple[str, bool]]:
    ...
    from panergos_cli import gateway as gw
    with _home_env(home):
...
def _stop_gateway_process(home: Path) -> None:
    from panergos_cli.profiles import _stop_gateway_process
...
def _write_multiplex_flag(default_home: Path, value: bool) -> None:
    ...
    (same read-guard + nested-set + atomic write ``panergos config set`` uses; no raw YAML edits)."""
    from panergos_cli.config import _set_nested, _write_user_config, require_readable_config_before_write
    cfg_path = default_home / "config.yaml"
        ...
        for platform, platform_config in cfg.platforms.items():
            ...
            if platform_serves_profile_prefix(platform.value):
                ...
                plan.blockers.append(
                    ...
                    f"keep '{profile.name}' on a standalone gateway (panergos -p {profile.name} gateway start --force)."
                )
...
def build_migration_plan() -> MigrationPlan:
    ...
    for check in _PREFLIGHT_CHECKS:
        check(plan, configs)
    from panergos_cli.gateway_migrate_guards import auto_migration_blockers
    ...
    plan.notices.extend(f"Not migrated automatically by `panergos update`: {b}" for b in auto_migration_blockers(plan))
    plan.notices.append(
...
def format_plan(plan: MigrationPlan, *, dry_run: bool) -> list[str]:
    ...
    lines.append(f"  - record the previous state in {plan.default_home / MANIFEST_NAME} (rollback: panergos gateway migrate --standalone)")
    return lines + _plan_tail(plan)
...
def _no_manifest_lines(default_home: Path) -> list[str]:
    return [f"✗ No migration manifest at {_manifest_path(default_home)}; nothing to roll back.",
            "  To leave multiplex mode by hand: panergos config set gateway.multiplex_profiles false && panergos gateway restart"]
...
def format_update_warning(plan: MigrationPlan, auto_blockers: list[str]) -> list[str]:
    return [
        ...
        *[f"    • {b}" for b in (*plan.blockers, *auto_blockers)],
        ...
        "  (`panergos update` will migrate automatically once nothing blocks it.)",
    ]
...
def apply_migration(plan: MigrationPlan, *, served_wait: float = _SERVED_WAIT_SECONDS) -> bool:
    ...
    if served is not None and expected <= set(served):
        _print(["", f"✓ Migrated: the default gateway now serves {len(served)} profiles: {', '.join(served)}",
                f"  Rollback any time with: panergos gateway migrate --standalone",
    ...
    _print(["", f"⚠ Migration applied, but the default gateway has not confirmed serving: {', '.join(missing)}",
            "  Check `panergos gateway status` and the gateway log; the flag and manifest are in place.",
            "  Rollback: panergos gateway migrate --standalone"])
    return False
...
def rollback_migration(default_home: Optional[Path] = None) -> bool:
    ...
    if default_gw.has_gateway:
        try:
            ...
            print("    The default gateway is still multiplexing; stop it by hand (panergos gateway stop) and re-run.")
    if ok:
...
def _host_supports_migration() -> Optional[str]:
    ...
    if gw.is_windows():
        return "Windows Scheduled Tasks are not migrated automatically; set gateway.multiplex_profiles true, stop the per-profile tasks, and `panergos gateway restart`."
    return None


def cmd_migrate(args) -> None:
    """``panergos gateway migrate [--multiplex|--standalone] [--dry-run] [--yes]``."""
...
def maybe_auto_migrate_after_update() -> None:
    """``panergos update`` hook: with >= 2 profiles, per-profile gateways present and multiplex off,
    ...
    service-domain / UNIX-user / PANERGOS_HOME boundary blocks this path only (the explicit command decides)."""
    from panergos_cli.gateway_migrate_guards import auto_migration_blockers, auto_migration_opted_out
