...
class Session:
    ...
    mandatory; providers without orgs set ``org_id=""``. The tokens are opaque to Panergos."""
    user_id: str
    email: str
    display_name: str
    org_id: str
...
def classify_jwks_lookup_error(exc: BaseException) -> Exception:
    ...
    * ``jwt.DecodeError`` — the bearer is not a JWT at all (an opaque peer key, a legacy session token,
    garbage). Hosted agents previously answered every non-JWT bearer with a fast provider
