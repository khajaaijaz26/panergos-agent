...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``panergos_session_at``
(access token; Max-Age = token TTL), ``panergos_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``panergos_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``panergos_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``panergos_sso_attempt`` (auto-SSO loop guard, 60 s).
...
SESSION_AT_COOKIE = "panergos_session_at"
SESSION_RT_COOKIE = "panergos_session_rt"
SESSION_PROVIDER_COOKIE = "panergos_session_provider"
PKCE_COOKIE = "panergos_session_pkce"
SSO_ATTEMPT_COOKIE = "panergos_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    """``Path=/panergos`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    Some OIDC providers issue rotating refresh tokens; a provider that omits one returns
