...
again (a drift once 401'd ``/api/status`` and broke cookie-less liveness probes).
...
PUBLIC_API_PATHS: frozenset[str] = frozenset({
    ...
    "/api/health",
    # Public liveness summary: version, gateway state, session count, auth-gate shape. No secrets.
    "/api/status",
