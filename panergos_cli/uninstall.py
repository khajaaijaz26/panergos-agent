...
def _strip_panergos_path_lines(content: str) -> str:
    """Drop the ``# Panergos Agent`` marker (+ its PATH line) and any panergos PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if any(marker in line for marker in ('# Panergos Agent', '# panergos-agent')):
            skip_next = True
            continue
        if skip_next and ('panergos' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'panergos' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Panergos PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_panergos_path_lines(content)
...
def remove_wrapper_script():
    """Remove the panergos wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a panergos_cli / panergos-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('panergos_cli' in content or 'panergos-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("panergos", "panergos-acp", "panergos-agent"))
...
def remove_node_symlinks(panergos_home: Path) -> list:
    ...
    node_dir = (panergos_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from panergos_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_systemd_gateway() -> bool:
    ...
    from panergos_cli.gateway import (
...
def _remove_launchd_gateway() -> bool:
    ...
    from panergos_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from panergos_cli import gateway_windows as gw
...
# PANERGOS_HOME / PANERGOS_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\panergos\git\{cmd,bin,usr\bin}, ...\panergos\node), PortableGit + Node copies
...
def _panergos_path_markers(panergos_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Panergos-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(panergos_home).rstrip("\\/")
    subs = ("panergos-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(panergos_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Panergos-owned entries from User-scope PATH in the registry (see ``_panergos_path_markers``)."""
    markers = tuple(m.lower() for m in _panergos_path_markers(panergos_home, include_managed_bin=include_managed_bin))
...
def remove_panergos_env_vars_windows() -> list[str]:
    """Delete PANERGOS_HOME and PANERGOS_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("PANERGOS_HOME", "PANERGOS_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(panergos_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``panergos_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (panergos_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``panergos`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from panergos_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from panergos_constants import get_default_panergos_root
        bin_dir = get_default_panergos_root() / "bin"
    except Exception as e:
...
def _is_default_panergos_home(panergos_home: Path) -> bool:
    """Return True when ``panergos_home`` points at the default (non-profile) root."""
    try:
        from panergos_constants import get_default_panergos_root
        return panergos_home.resolve() == get_default_panergos_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``panergos -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current PANERGOS_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m panergos_cli.main` (the `panergos` wrapper may be half-gone).
    panergos_invocation = [sys.executable, "-m", "panergos_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                panergos_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's PANERGOS_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``panergos uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from panergos_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    panergos_home = get_panergos_home()
    summary = gui_install_summary(panergos_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ◆ Panergos Chat GUI Uninstaller                │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Panergos Chat GUI installation was found.")
        print(f"  Checked: {panergos_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Panergos agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(panergos_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Panergos agent at {panergos_home / 'panergos-agent'}")
        print(f"  • Your config, sessions, and secrets under {panergos_home}")
        print()
    ...
    print()
    uninstall_gui(panergos_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Panergos agent is still installed. Run 'panergos' to use the CLI,")
    print("or 'panergos uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.panergos/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, panergos_home=panergos_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie PANERGOS_HOMEs and systemd units behind.
    is_default_profile = _is_default_panergos_home(panergos_home)
    ...
    print()
    _print_box("│              Panergos Agent Uninstaller                │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {panergos_home / 'config.yaml'}")
    print(f"  Secrets: {panergos_home / '.env'}")
    print(f"  Data:    {panergos_home / 'cron/'}, {panergos_home / 'sessions/'}, {panergos_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, PANERGOS_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Panergos data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Panergos code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, panergos_home=panergos_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, panergos_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Panergos PATH entries from shell configs / Windows User PATH")
    print("  • Panergos wrapper scripts and Panergos-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Panergos config/data: {panergos_home}")
    else:
        print(f"  • Panergos config/data: {panergos_home}")
        profiles = _discover_named_profiles() if _is_default_panergos_home(panergos_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$PANERGOS_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: panergos_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; panergos\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_panergos_home(panergos_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(panergos_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Panergos PATH entries in User environment"),
        (windows, "Removing PANERGOS_HOME / PANERGOS_GIT_BASH_PATH User env vars...",
         remove_panergos_env_vars_windows, "Removed User env var: {}", "No Panergos compatibility env vars to remove"),
        (True, "Removing Panergos commands...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows Panergos launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows Panergos launchers found"),
        (True, "Removing Panergos-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(panergos_home), "Removed {}", "No Panergos-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE PANERGOS_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from panergos_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(panergos_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(panergos_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.panergos/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(panergos_home)
    else:
        log_info(f"Keeping configuration and data in {panergos_home}")
    ...
    print()
    print("Thank you for using Panergos Agent! ◆")
    print()


_REINSTALL_HINT = {
    True: (
        "  iex (irm https://raw.githubusercontent.com/khajaaijaz26/"
        "panergos-agent/main/scripts/install.ps1)"
    ),
    False: (
        "  curl -fsSL https://raw.githubusercontent.com/khajaaijaz26/"
        "panergos-agent/main/scripts/install.sh | bash"
    ),
}
...
def main(argv=None) -> int:
    """``python -m panergos_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``panergos_constants`` + ``panergos_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m panergos_cli.uninstall")
    parser.add_argument(
