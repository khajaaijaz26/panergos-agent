...
from panergos_cli._subprocess_compat import windows_detach_flags
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from panergos_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    """
    try:
        from panergos_cli.profiles import _check_gateway_running, profiles_to_serve
...
# ~/.panergos/logs/<action>.log which the dashboard tails.

_ACTION_LOG_DIR: Path = get_panergos_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "panergos-update": "panergos-update.log",
...
def _dashboard_spawn_executable() -> str:
    ...
    """
    from panergos_cli.web_server import PROJECT_ROOT
...
def _profile_action_environment(
    ...
    """Environment for a detached ``panergos <subcommand>`` action.

    The dashboard loads its own profile's ``.env`` into process-global ``os.environ``. Copying
    that mapping verbatim into ``panergos -p <other> ...`` lets the named child see the dashboard
    profile's platform credentials and ports *before* its own dotenv loads (``load_panergos_dotenv``
    ...
    Named-profile actions therefore start from Panergos' standard scrubbed subprocess env, then drop
    the profile-managed keys plus every key declared by the dashboard/default profile dotenv files
    and their hydrated secret sources, and pin ``PANERGOS_HOME`` to the target profile. The child's
    ...
    if profile is None:
        ...
        from panergos_cli.env_loader import (
            _PROFILE_MANAGED_ENV_KEYS, _env_keys_defined_in_dotenv, get_secret_source_values,
        )
        from panergos_cli.web_server_profiles import _resolve_profile_dir
        from panergos_constants import apply_subprocess_home_env, get_default_panergos_root
        ...
        try:
            source_homes = {str(get_default_panergos_root()), str(get_panergos_home())}
        except Exception:
            source_homes = set()
        ...
        action_env["PANERGOS_HOME"] = str(target_home)
        apply_subprocess_home_env(action_env)

    action_env["PANERGOS_NONINTERACTIVE"] = "1"
    # The dashboard runs inside the gateway process, so os.environ carries _PANERGOS_GATEWAY=1;
    ...
    action_env.pop("_PANERGOS_GATEWAY", None)
    if env_overrides:
        action_env.update(env_overrides)
    return action_env


def _spawn_panergos_action(
    subcommand: List[str], name: str, *, env_overrides: Optional[Dict[str, str]] = None
) -> subprocess.Popen:
    """Spawn ``panergos <subcommand>`` detached (via ``panergos_cli.main``) and record the handle."""
    from panergos_cli.web_server import PROJECT_ROOT
    _ACTION_LOG_DIR.mkdir(parents=True, exist_ok=True)
    log_file = open(_ACTION_LOG_DIR / _ACTION_LOG_FILES[name], "ab", buffering=0)
    log_file.write(f"\n=== {name} started {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n".encode())

    cmd = [_dashboard_spawn_executable(), "-m", "panergos_cli.main", *subcommand]
    # Named-profile actions get a scrubbed, pinned environment so the child cannot inherit the
    # dashboard profile's credentials; see _profile_action_environment (also drops _PANERGOS_GATEWAY).
    ...
    action_id = (env_overrides or {}).get("PANERGOS_ACTION_ID")
    if action_id:
...
def _own_profile_selector(profile: Optional[str]) -> Optional[str]:
    ...
    profile (a pooled Desktop ``panergos --profile X serve`` answers ``/api/gateway/*`` without
    ...
    own = profile_name_for_home(get_process_panergos_home())
    return own if own and own != "default" else None


def _gateway_subcommand(profile: Optional[str], verb: str) -> List[str]:
    """``panergos [-p X] gateway <verb>`` argv for a dashboard lifecycle action. A profile served by the
    ...
    pooled ``--profile X serve`` would inherit X's ``PANERGOS_HOME`` and hit the same exit-78 refusal."""
    from panergos_cli.web_server_profiles import _profile_cli_args
    ...
    if profile and verb == "restart" and multiplexed_profile_refusal(profile, verb) is not None:
        from panergos_constants import get_process_panergos_home, profile_name_for_home
        args = [] if profile_name_for_home(get_process_panergos_home()) == "default" else ["-p", "default"]
...
def multiplexed_profile_refusal(profile: Optional[str], verb: str) -> Optional[str]:
    ...
    The spawned ``panergos -p X gateway <verb>`` would only print exit-78 / "no gateway running for this
...
def _restart_gateway_after(profile: Optional[str], *, what: str, label: str) -> dict[str, Any]:
    ...
    from panergos_cli.web_server import _spawn_gateway_restart
    try:
