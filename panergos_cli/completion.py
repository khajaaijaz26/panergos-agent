"""Shell completion script generation for panergos CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_panergos_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Panergos Agent bash completion
# Add to ~/.bashrc:
#   eval "$(panergos completion bash)"

_panergos_profiles() {{
    local profiles_dir="$HOME/.panergos/profiles"
...
_panergos_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_panergos_profiles)" -- "$cur"))
        return
    fi
...
complete -F _panergos_completion panergos
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef panergos
# Panergos Agent zsh completion
# Add to ~/.zshrc:
...
_panergos_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.panergos/profiles" ]]; then
        profiles+=($HOME/.panergos/profiles/*(N/:t))
    fi
...
_panergos() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_panergos_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'panergos command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Panergos Agent fish completion",
        "# Add to your config:",
        "#   panergos completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __panergos_profiles",
        "    echo default",
        "    if test -d $HOME/.panergos/profiles",
        "        for d in $HOME/.panergos/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c panergos -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c panergos -f -s p -l profile"
        " -d 'Profile name' -xa '(__panergos_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c panergos -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__panergos_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
