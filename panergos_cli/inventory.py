...
def _strip_aggregator_overlaps(rows: list[dict]) -> None:
    ...
    try:
        from panergos_cli.providers import is_routing_aggregator
    except Exception:
        return
...
def _apply_capabilities(rows: list[dict]) -> None:
    ...
    from panergos_cli.models import model_supports_fast_mode
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`panergos_cli.providers.custom_provider_aliases` — lets the frontend do a membership check instead.
    """
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else f"Configured provider is not authenticated; run `panergos model` to reactivate. {tail}"
            )
...
def _anthropic_oauth_credentials_present() -> bool:
    """True when the user explicitly authenticated Anthropic via OAuth (Panergos device flow or Claude Code
    ...
    try:
        ...
        readers = (read_panergos_oauth_credentials, read_claude_code_credentials)
...
def _external_process_signed_in(slug: str) -> bool:
    ...
    try:
        from panergos_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's Panergos overlay declares it keyless."""
    try:
        from panergos_cli.providers import PANERGOS_OVERLAYS
        overlay = PANERGOS_OVERLAYS.get(slug)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (f"paste {key_env} to activate" if auth_type == "api_key" and key_env
                          else f"run `panergos model` to configure ({auth_type})")
...
def _apply_pricing(rows: list[dict], *, cached_only: bool = False) -> None:
    """Set ``row["pricing"] = {model_id: {input, output, cache | None, free}}``.
    ``cached_only`` never starts provider I/O."""
    from panergos_cli.models_pricing import (
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    try:
        from panergos_cli.local_runtime.bootstrap import staged_model_ids
        ...
        if not current:
            ...
            try:
                from panergos_cli.local_runtime.endpoint import _state_endpoint
...
def _prewarm_pricing_async(
    ...
    from panergos_cli.models_pricing import pricing_cache_scope
    ...
    endpoint_scope = tuple(sorted(
        ...
        for slug in slugs))
    prewarm_key = (panergos_home_key(), endpoint_scope)
    ...
    with _pricing_prewarm_lock:
        ...
        thread = Thread(target=copy_context().run, args=(_worker,),
                        name="panergos-picker-pricing-prewarm", daemon=True)
