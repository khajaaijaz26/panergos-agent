...
Custom-provider lookup lives in :mod:`panergos_cli.runtime_provider_custom`; Azure Foundry,
OpenRouter/bare-custom, Bedrock and external-process builders in
:mod:`panergos_cli.runtime_provider_backends` — both re-exported here so
``panergos_cli.runtime_provider.<name>`` imports and test patches keep working."""
...
from panergos_cli.auth import (  # resolve_external_process_provider_credentials is read via origin by runtime_provider_backends
...
from panergos_cli import config as _config_mod
from panergos_cli import models as _models  # attribute access keeps ``panergos_cli.models.<name>`` patches effective
from panergos_constants import OPENROUTER_BASE_URL
from panergos_cli.providers import determine_api_mode, is_actual_route, is_official_openai_host
from utils import base_url_host_matches, base_url_hostname


# Late-bound delegates, deliberately NOT module-level from-imports: this module is often imported
# lazily, so its first import can happen while a test has ``panergos_cli.config.load_config`` patched
...
# Hosts that only speak one wire protocol. Mirrors host_mandated_api_mode in panergos_cli/providers.py
...
def _api_key_provider_api_mode(provider: str, model_cfg: Dict[str, Any], api_key: str, base_url: str, effective_model: Any, *,
    ...
    if provider == "xai":
        ...
        # the host_mandated_api_mode clause in panergos_cli/providers.py so the runtime resolver stays in
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    """Provider request from explicit arg, then config, then ``PANERGOS_INFERENCE_PROVIDER``, else
    ...
    return get_secret_str("PANERGOS_INFERENCE_PROVIDER", "").strip().lower() or "auto"


# ── extracted collaborators (re-exported; see module docstring) ────────────────────────────

from panergos_cli.runtime_provider_custom import (  # noqa: E402,F401
...
from panergos_cli.runtime_provider_backends import (  # noqa: E402,F401
...
def _openrouter_should_use_pool(requested_provider, model_cfg, explicit_api_key, explicit_base_url) -> bool:
    ...
    # OpenRouter host — `panergos setup` persists https://openrouter.ai/api/v1 for plain installs,
...
_OAUTH_RUNTIME_PROVIDERS: Dict[str, _OAuthRuntimeSpec] = {
    "openai-codex": _OAuthRuntimeSpec(lambda: resolve_codex_runtime_credentials(), "codex_responses", "panergos-auth-store",
                                      "last_refresh", "Auto-detected Codex provider but credentials failed"),
    "xai-oauth": _OAuthRuntimeSpec(lambda: resolve_xai_oauth_runtime_credentials(), "codex_responses", "panergos-auth-store",
...
def _resolve_vertex_runtime(requested_provider: str) -> Dict[str, Any]:
    ...
    if not token or not base_url:
        raise AuthError("Vertex AI credentials could not be resolved. Vertex uses OAuth2 (not a static API key): provide a "
                        "service-account JSON via GOOGLE_APPLICATION_CREDENTIALS (or VERTEX_CREDENTIALS_PATH) in ~/.panergos/.env, "
                        "or run 'gcloud auth application-default login' for ADC. Set the GCP project/region under vertex: in "
                        "config.yaml if they aren't embedded in the credentials. Run `panergos setup` to install Vertex support.")
...
def resolve_runtime_provider(*, requested: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    rung returns or raises, else falls to the next):
      ...
      7. OAuth specs (codex/xai/qwen; "auto" swallows AuthError and logs) → minimax-oauth
...
_PLUGIN_COMPAT_LAZY = {
    'custom_provider_aliases': ('panergos_cli.providers', 'custom_provider_aliases'),
