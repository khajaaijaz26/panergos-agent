...
from panergos_cli.providers import (
...
# Re-exported: callers/tests patch panergos_cli.model_switch.<name>.
from panergos_cli.model_switch_providers import list_authenticated_providers
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, str):
        ...
        # Pre-fix Panergos wrote sentinel keys inside the user-facing ``models`` mapping.
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Panergos.
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    ``discover_models: false``. A catalog Panergos itself persisted (``discovered``) is never a pin."""
    if discovered:
        return False
    if isinstance(value, str):
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    try:
        from panergos_constants import get_config_path
...
def resolve_startup_model_route(
    ...
    ``PanergosCLI`` is constructed before the interactive ``/model`` pipeline runs; resolving here
    ...
    if current_provider:
        try:
            from panergos_cli.providers import is_routing_aggregator, normalize_provider as _norm_prov
            if is_routing_aggregator(_norm_prov(current_provider)):
                from panergos_cli.models import _find_openrouter_slug
    ...
    try:
        from panergos_cli.models import normalize_provider
...
def parse_model_flags_detailed(raw_args: str) -> ModelFlagParseResult:
    ...
    is the raw level word (validated by :func:`panergos_constants.parse_reasoning_effort` at apply
...
def _resolve_alias_fallback(
    ...
    """Resolve an alias on the user's authenticated providers (OpenRouter when none are given).
...
def _ollama_configured_base() -> tuple[dict, str]:
    from panergos_cli.models import _get_provider_config_dict
...
def _unknown_provider_message(explicit_provider: str) -> str:
    msg = (
        f"Unknown provider '{explicit_provider}'. Check 'panergos model' for available "
    ...
    try:  # Surface common config issues that cause provider resolution failures
        ...
        if issues:
            msg += "\n\nRun 'panergos doctor' — config issues detected:" + "".join(f"\n  • {ci.message}" for ci in issues[:3])
    except Exception:
        pass
    return msg
...
def _apply_direct_alias_endpoint(st: "_Switch", da: DirectAlias) -> None:
    ...
    from panergos_cli.models_local import _same_ollama_native_root
...
def _convert_vendor_colon_slug(st: _Switch) -> None:
    ...
    ``left`` names a provider Panergos knows, so ``/model alibaba:qwen3.6-plus`` routes like
...
def _route_from_model_input(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from panergos_cli.models import detect_provider_for_model
    raw_input, current_provider = st.raw_input, st.current_provider
    try:
        from panergos_cli.config import load_config
        from panergos_cli.moa_config import exact_moa_preset_name, normalize_moa_config
...
def _creds_for_switched_provider(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if st.explicit_provider and st.user_providers:
        from panergos_cli.providers import resolve_user_provider
...
def _creds_for_current_provider(st: _Switch) -> None:
    ...
    from panergos_cli.models_local import _get_ollama_request_headers, _same_ollama_native_root
    keep_current_ollama_endpoint = False
    ollama_headers: dict[str, str] = {}
    if st.current_provider == "custom" and st.current_base_url:
        try:
            from panergos_cli.models_local import should_use_ollama_native_catalog
...
def _validate_switch(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from panergos_cli.models_local import _get_ollama_request_headers
    from panergos_cli.models_validate import validate_requested_model
...
def _copilot_api_mode(provider: str, model: str, api_key: str) -> str:
    from panergos_cli.models import copilot_model_api_mode
...
def _opencode_api_mode(provider: str, model: str, api_key: str) -> str:
    ...
    # #16878.
    from panergos_cli.models import opencode_model_api_mode
...
def _build_switch_result(st: _Switch) -> ModelSwitchResult:
    ...
    from panergos_cli.models import normalize_opencode_base_url, opencode_provider_family
    ...
    try:
        from panergos_cli.runtime_provider import _get_named_custom_provider, _custom_provider_request_overrides
...
def persist_model_selection(result: ModelSwitchResult, config_path: Any = None) -> None:
    ...
    ``PANERGOS_HOME/config.yaml`` — the context override or ``PANERGOS_HOME`` at call time).
    ...
    from panergos_cli.config import get_config_path, read_user_config_raw, warn_unpinned_cron_jobs_after_model_config_change
...
def _scoped_key_env(name: str) -> str:
    ...
    ``$PANERGOS_HOME/.env`` authenticates the ``/model`` verification probe (#109315) and a rotated
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'list_picker_providers': ('panergos_cli.model_switch_providers', 'list_picker_providers'),
    'prewarm_picker_cache_async': ('panergos_cli.model_switch_providers', 'prewarm_picker_cache_async'),
}
