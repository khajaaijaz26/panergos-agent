...
Imported by ``panergos_cli/main.py`` BEFORE its heavy import wall (config, argparse tree, logging,
providers). Everything here must stay **stdlib-only and cheap** (os/sys file probes; no yaml, no
panergos_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
def active_profile_may_override_home(panergos_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect PANERGOS_HOME?"""
    active = (_read_text(os.path.join(panergos_root, "active_profile")) or "").strip()
    return bool(active and active != "default")


def _default_home() -> str:
    return os.path.join(os.path.expanduser("~"), ".panergos")


def _resolved_home() -> str:
    return os.environ.get("PANERGOS_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("PANERGOS_DEV") == "1" or is_container_startup_environment():
        return False
    panergos_home = os.environ.get("PANERGOS_HOME", "").strip()
    if panergos_home:
        if os.path.exists(os.path.join(panergos_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(panergos_home)))
        return parent_name != "profiles" and active_profile_may_override_home(panergos_home)
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``panergos --version`` output (also used by /version).
    ...
    try:
        ...
        from panergos_cli import (
            __distribution_name__, __distribution_version__,
            __release_date__, __version__,
        )

        print(
            f"{__distribution_name__} v{__distribution_version__} "
            f"· Panergos runtime v{__version__} ({__release_date__})")
    ...
    try:
        from panergos_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``panergos --version`` before the heavy import wall.
    ...
    PANERGOS_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("PANERGOS_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
