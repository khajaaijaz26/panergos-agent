"""``panergos slack manifest`` — generate the Slack app manifest JSON that registers every gateway
...
def _build_full_manifest(
    ...
    Other sections (OAuth scopes, socket mode) are sensible Panergos defaults, tweakable in the Slack
    UI after pasting.
    """
    from panergos_cli.commands_platforms import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Panergos in threads and DMs."}
        ...
        features["agent_view"] = {"agent_description": "Chat with Panergos in Slack Messages."}
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Panergos agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    """Print or write a Slack app manifest JSON (flags documented in ``panergos_cli/main.py``)."""
    name = getattr(args, "name", None) or "Panergos"
    description = getattr(args, "description", None) or "Your Panergos agent on Slack"
    ...
    def fail(msg: str) -> int:
        print(f"panergos slack manifest: {msg}", file=sys.stderr)
        return 2
    ...
    if isinstance(write_target, bool) and write_target:  # bare --write → default location
        ...
        target = Path(get_panergos_home()) / "slack-manifest.json"
    else:
    ...
    print(
        "\nNext steps:\n"
        "  1. Open https://api.slack.com/apps and pick your Panergos app\n"
