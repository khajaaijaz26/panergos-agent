"""Boundaries the AUTOMATIC multiplex migration (``panergos update``) must not cross, and the opt-out.

The multiplexer replaces a kernel-enforced boundary (separate UNIX users, separate service domains,
separate PANERGOS_HOME trees) with in-process isolation. An operator may choose that with
``panergos gateway migrate --multiplex``; an unattended update hook must not choose it for them.
...
if TYPE_CHECKING:
    from panergos_cli.gateway_migrate import MigrationPlan, ProfileGateway
...
def _system_unit_uid(unit_path: Path) -> Optional[int]:
    ...
    from panergos_cli.gateway import _read_systemd_user_from_unit
...
def gateway_identity(home: Path, pid: Optional[int], service: Optional[tuple[str, bool]]) -> tuple[Optional[int], Path]:
    ...
    None means unknown — never a different user. runtime_home: the PANERGOS_HOME the installed unit
    pins, which is where the gateway really runs; ``home`` when there is no unit or no pin.
    """
    from panergos_cli.gateway import _panergos_home_pinned_by_unit, get_systemd_unit_path
    from panergos_cli.gateway_migrate import _home_env
    ...
    if service is not None and service[0] == "systemd":
        ...
        pinned = _panergos_home_pinned_by_unit(unit_path)
        if pinned:
...
def _guard_home_tree(plan: MigrationPlan, profile: ProfileGateway) -> Optional[str]:
    ...
    if runtime_home.is_relative_to(profiles_root):
        return None
    return (f"Profile '{profile.name}' runs with PANERGOS_HOME={runtime_home}, outside {profiles_root}: "
...
def auto_migration_blockers(plan: MigrationPlan) -> list[str]:
    ...
    domain, one profiles/ tree — the only shape ``panergos update`` may fold on its own."""
    return [
        finding
