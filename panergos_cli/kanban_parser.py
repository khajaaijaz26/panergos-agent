"""Argparse tree for ``panergos kanban …`` (``build_parser``).
...
def _triage_sweep_args(verb: str, Verb: str, noun: str):
    ...
    return (
        ...
        _arg("--author",
             help=f"Author name recorded on the audit comment (default: $PANERGOS_PROFILE or '{noun}')"),
...
_BOARD_SPECS = [
    ...
    _cmd("import", [
        ...
        "Import a .tar.gz produced by `panergos kanban boards export`. The board always lands as a "
...
# Top-level ``panergos kanban <action>`` records, in ``--help`` order.
_SPECS = [
    ...
    _cmd("create", [
        ...
        _arg("--project",
             help="Link to a project (id or slug). Anchors the task's "
                  ...
                  "deterministic branch. See `panergos project list`."),
        _TENANT,
        _PRIORITY,
    ...
    _cmd("list", [
        _arg("--mine", action="store_true", help="Filter by $PANERGOS_PROFILE as assignee"),
        _arg("--assignee"),
    ...
    _cmd("comment", [
        ...
        _arg("--author", help="Author name (default: $PANERGOS_PROFILE or 'user')"),
    ...
    _cmd("attach", [
        ...
        _arg("--author", help="uploaded_by label (default: $PANERGOS_PROFILE or 'user')"),
    ...
    ], help="DEPRECATED — dispatcher now runs in the gateway. Use `panergos gateway start`."),
    _cmd("watch", [
    ...
    _cmd("assignees", [_json_flag()],
         help="List known profiles + per-profile task counts (union of ~/.panergos/profiles/ and current assignees on the board)"),
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description="Durable SQLite-backed task board shared across Panergos profiles. "
                    "Tasks are claimed atomically, can depend on other tasks, and "
                    "are executed by a named profile in an isolated workspace. "
                    "See https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/features/kanban.",
    )
    # --board scopes every subcommand to one board's DB; when omitted the
    # resolution is PANERGOS_KANBAN_BOARD, then the persisted current-board
    ...
    kanban_parser.add_argument("--board", default=None, metavar="<slug>",
                               help="Board slug to operate on. Defaults to the current board (set "
                                    "via `panergos kanban boards switch <slug>` or the "
                                    "PANERGOS_KANBAN_BOARD env var). Use `panergos kanban boards "
