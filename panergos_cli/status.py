"""Status command for panergos CLI."""
...
from panergos_cli.config import get_env_path, get_env_value, get_panergos_home, load_config
from panergos_cli.models import provider_label
from panergos_cli.runtime_provider import resolve_requested_provider
from panergos_cli.vercel_auth import describe_vercel_auth
from panergos_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers)
from panergos_constants import OPENROUTER_MODELS_URL
...
def _estop_status_line():
    """One-line pause banner for `panergos status`, or None when not paused."""
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `panergos resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'panergos-agent[vercel]')")
        ...
        # (fail-soft — never break `panergos status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        ...
        from panergos_cli.gateway_multiplex_served import multiplexer_served_secondaries
        ...
        if not snapshot.running and named_profile_served_by_running_multiplexer():
            ...
            _kv("Manage with:", "panergos gateway status   # from the default profile")
            return
        ...
        if snapshot.running and (served := multiplexer_served_secondaries()):
            ...
            from panergos_cli.gateway_multiplex_served import served_profile_ingress_urls
        ...
        if snapshot.has_process_service_mismatch:
            ...
            _kv("Start with:", "panergos gateway")
...
def _render_cron(ctx):
    ...
    jobs_file = get_panergos_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from panergos_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_panergos_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
    ...
    try:
        from panergos_cli.active_sessions import (
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'panergos doctor' for detailed diagnostics", "  Run 'panergos setup' to configure"),
            Colors.DIM)
    print()


# Print order of `panergos status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
...
def show_status(args):
    """Show status of all Panergos Agent components."""
    # Shared by section renderers.
