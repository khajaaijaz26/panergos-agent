"""Panergos Desktop (Chat GUI) uninstaller: removes only GUI state — built Electron artifacts, the packaged
...
def desktop_userdata_dir() -> Path:
    """Electron ``app.getPath('userData')`` for Panergos on each platform (GUI-only state)."""
    home = Path.home()
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Panergos"
    if sys.platform == "win32":
        return _env_dir("APPDATA", home / "AppData" / "Roaming") / "Panergos"
    return _env_dir("XDG_CONFIG_HOME", home / ".config") / "Panergos"


def _desktop_userdata_dirs() -> "list[Path]":
    """Desktop userData location."""
    return [desktop_userdata_dir()]


def source_built_gui_artifacts(panergos_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``panergos desktop`` inside the checkout (same ``panergos-agent/`` layout
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for Panergos."""
    home = Path.home()
    if sys.platform == "darwin":
        return [
            Path("/Applications/Panergos.app"), home / "Applications" / "Panergos.app",
        ]
    if sys.platform == "win32":
        ...
        paths = [
            local_base / "Programs" / "Panergos",
            local_base / "panergos-desktop",
        ]
        return paths + ([Path(program_files) / "Panergos"] if program_files else [])
    ...
    from panergos_cli.linux_desktop_entry import desktop_entry_path
    ...
    return [desktop_entry_path(), data_base / "applications" / "Panergos.desktop"] + [
        icons / size / "apps" / "panergos.png"
        for size in ("scalable", "24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024")]


def agent_is_installed(panergos_home: Path) -> bool:
    """True when a usable Python agent install exists under PANERGOS_HOME (gates the desktop UI's options).
    Package source or a venv alone is enough — a source checkout without a venv is still "the agent is here"."""
    return any((panergos_home / "panergos-agent" / sub).is_dir() for sub in ("panergos_cli", "venv", ".venv"))


def gui_is_installed(panergos_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    return any(p.exists() for p in (
        *source_built_gui_artifacts(panergos_home), *packaged_gui_app_paths(), *_desktop_userdata_dirs()))


def gui_install_summary(panergos_home: "Path | None" = None) -> dict:
    ...
    return {"panergos_home": str(home), "agent_installed": agent_is_installed(home),
            ...
            "userdata_exists": any(p.exists() for p in _desktop_userdata_dirs()), "platform": sys.platform}
...
def uninstall_gui(panergos_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if remove_userdata and any(path.exists() for path in _desktop_userdata_dirs()):
        log_info("Removing desktop app data (Electron userData)...")
        _remove_existing(_desktop_userdata_dirs())
    if not removed:
        log_info("No desktop GUI artifacts found to remove")
    if sys.platform.startswith("linux"):
        # The desktop entry was removed above but the menu caches still list it; reindex so Panergos
        # disappears from the launcher.
        try:
            from panergos_cli.linux_desktop_entry import desktop_entry_path, refresh_desktop_databases
        ...
        log_info("If you installed the desktop via a .deb / .rpm package, remove it with your package manager "
                 "(e.g. 'sudo apt remove panergos' or 'sudo dnf remove panergos'). "
                 "AppImage builds are a single "
