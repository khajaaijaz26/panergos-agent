"""Cross-process mutual exclusion for in-flight Panergos updates.
...
MARKER_NAME = ".panergos-update-in-progress"

# Set by an orchestrating updater (Tauri `panergos-setup --update`) to its own pid before
# spawning `panergos update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "PANERGOS_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Panergos is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Panergos home (never the context-local profile override): the Rust
    updater resolves ``$PANERGOS_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_panergos_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``panergos update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another panergos update is already running (PID {holder.pid}, "
