"""``panergos profile`` command — one handler per action, dispatched by ``PROFILE_ACTIONS``.

Imports from ``panergos_cli.profiles`` stay lazy (inside each handler) so tests can monkeypatch
...
def _render_distribution_plan(plan) -> None:
    ...
    from panergos_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.panergos_requires:
        print(f"  Requires: Panergos {mf.panergos_requires}")
...
def _profile_status(args):
    """Bare ``panergos profile`` — show current profile status."""
    from panergos_constants import display_panergos_home
    from panergos_cli.profiles import format_profile_label, get_active_profile_name, list_profiles
    profile_name = get_active_profile_name()
    dhh = display_panergos_home()
    ...
    if current is not None:
        ...
        if p.alias_path:
            print(f"Alias:          {p.alias_name or p.name} → panergos -p {p.name}")
    print()
...
def _shared_credential_warnings(profiles) -> list:
    ...
    from panergos_cli.profile_channels import shared_channel_credentials, shared_credential_warning
...
def _profile_use(args):
    from panergos_cli.profiles import set_active_profile
    name = args.profile_name
    try:
        set_active_profile(name)
        print("Switched to: default (~/.panergos)" if name == "default" else f"Switched to: {name}")
...
def _print_channel_clone_notice(name: str, source_label: str, clone_channels: bool, clone_flag: str) -> None:
    from panergos_cli.profile_channels import (
...
def _profile_create(args):
    ...
    if not no_alias:
        ...
        if collision:
            ...
            print(f"  Choose a custom alias:  panergos profile alias {name} --name <custom>")
            print(f"  Or access via flag:     panergos -p {name} chat")
        else:
    ...
    if served is not None and normalize_profile_name(name) in {normalize_profile_name(p) for p in served}:
        ...
        print("  panergos gateway restart    Serve this profile from the running multiplexed gateway")
    else:
...
def _profile_delete(args):
    from panergos_cli.profiles import delete_profile
    try:
...
def _describe_target_dir(name: str) -> Path:
    """Profile dir for ``describe``: ``default`` maps to the CURRENT home (get_panergos_home),
    everything else to its named directory."""
    from panergos_cli import profiles as _profiles_mod
    if _profiles_mod.normalize_profile_name(name) == "default":
        from panergos_constants import get_panergos_home as _hh
        return Path(_hh())
...
def _profile_describe(args):
    ...
    from panergos_cli import profile_describer as _pd
    if all_flag:
...
def _profile_show(args):
    ...
    if dist_name:
        ...
        print(f"  (run `panergos profile info {name}` for full manifest)")
    if alias_name:
        print(f"Alias:   {alias_name} → panergos -p {name}  ({_wrapper_path(alias_name)})")
    print()
...
def _profile_rename(args):
    from panergos_cli.profiles import normalize_profile_name, rename_profile
    try:
...
def _profile_export(args):
    from panergos_cli.profiles import export_profile, get_profile_export_path
    name = args.profile_name
    try:
...
def _profile_install(args):
    import tempfile
    from panergos_cli.profile_distribution import DistributionError, install_distribution, plan_install
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="panergos_dist_preview_") as tmp:
        ...
        if plan.has_cron:
            print(
                ...
                f"  Review them with:  panergos -p {plan.manifest.name} cron list"
            )
        print(f"\n  Use with:      panergos -p {plan.manifest.name} chat")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")


def _profile_update(args):
    from panergos_cli.profile_distribution import DistributionError, read_manifest, update_distribution
    from panergos_cli.profiles import get_profile_dir, normalize_profile_name
    try:
        ...
        if current is None:
            _die(
                ...
                "Only profiles installed via `panergos profile install` can be updated."
            )
        ...
        if plan.has_cron:
            print(f"  Cron files were refreshed.  Review with:  panergos -p {plan.manifest.name} cron list")
    except (DistributionError, ValueError) as e:
        _die(f"Error: {e}")
...
_INFO_FIELDS = (
    ...
    ("author", "Author:       "),
    ("license", "License:      "),
    ("panergos_requires", "Requires:     Panergos "),
    ("source", "Source:       "),
...
def _profile_info(args):
    from panergos_cli.profile_distribution import describe_distribution, DistributionError
    try:
...
# Order mirrors the original if/elif chain; None = bare ``panergos profile``.
PROFILE_ACTIONS = {
