"""MCP Server Management CLI — ``panergos mcp`` subcommand."""
...
from panergos_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_panergos_home,  # noqa: F401 — used by test mocks
)
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.panergos/.env`` into ``os.environ`` and
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return PanergosTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see panergos_cli/main.py for why the dest is renamed).
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  panergos mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  panergos mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  panergos mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: panergos mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_panergos_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def cmd_mcp_remove(args):
    ...
    # an earlier `panergos mcp test`) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  panergos mcp add <name> --url <endpoint>')
        _info('  panergos mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict, *, flow: str | None = None) -> bool:
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `panergos mcp remove` + `panergos mcp add` to reconfigure auth.")
        return False
    ...
    # re-auth path in web_server.py. force_interactive_oauth: `panergos mcp login` is explicitly
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `panergos mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    if not name:
        ...
        _info("Usage: panergos mcp reauth <name>   |   panergos mcp reauth --all")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'panergos mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    cfg = _lookup_server(name, _get_mcp_servers(), "Available")
    if cfg is None:
...
_MCP_USAGE = (
    "panergos mcp                                    Open the catalog picker (default)",
    "panergos mcp catalog                            List Panergos-curated MCPs",
    "panergos mcp install <name>                     Install a catalog MCP",
    "panergos mcp serve                              Run as MCP server",
    "panergos mcp add <name> --url <endpoint>        Add a custom MCP server",
    "panergos mcp add <name> --command <cmd>         Add a stdio server",
    "panergos mcp add <name> --preset <preset>       Add from a known preset",
    "panergos mcp remove <name>                      Remove a server",
    "panergos mcp list                               List configured servers",
    "panergos mcp test <name>                        Test connection",
    "panergos mcp configure <name>                   Toggle tools",
    "panergos mcp login <name>                       Re-authenticate OAuth",
    "panergos mcp reauth <name> | --all              Re-auth one or all OAuth servers",
)


def mcp_command(args):
    """Main dispatcher for ``panergos mcp`` subcommands."""
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from panergos_cli import mcp_picker
    ...
    if handler:
        handler(args)
        return
    # No subcommand — drop the user into the catalog picker (same UX as `panergos plugin`).
    from panergos_cli.mcp_picker import run_picker
    run_picker()
