"""Gateway fleet restart + post-update verification for ``panergos update``.

Split out of ``panergos_cli/update_cmd.py``; every name is re-imported there so
``panergos_cli.update_cmd.<name>`` keeps resolving/monkeypatching. Origin helpers are
...
from panergos_cli.update_cmd_common import _best_effort
from panergos_cli.update_inventory import _gateway_service_matches_profile

# Log-record parity with the origin module.
logger = logging.getLogger("panergos_cli.update_cmd")

# Under PANERGOS_HOME (not next to the venv): records the fleet-restart obligation
...
_LIST_GATEWAY_UNITS = ["list-units", "panergos-gateway*", "panergos-serve*", "--plain", "--no-legend", "--no-pager"]


def _write_gateway_update_exit_code(ok: bool) -> None:
    from panergos_cli.update_cmd import get_panergos_home
    path = get_panergos_home() / ".update_exit_code"
    with suppress(OSError):
        path.write_text("0" if ok else "1", encoding="utf-8")


def _fleet_restart_pending_marker_path() -> Path:
    """PANERGOS_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    from panergos_cli.update_cmd import get_panergos_home
    return get_panergos_home() / _FLEET_RESTART_PENDING_NAME


def _write_fleet_restart_pending_marker(*, expected_sha: str = "") -> None:
    """Drop the pull→restart obligation breadcrumb. Never raises."""
    from panergos_cli.update_cmd import _m
...
def _current_checkout_sha() -> str | None:
    ...
    from panergos_cli.update_cmd import _capture_head_sha, _m
    try:
        from panergos_cli.build_info import get_code_identity
...
def _receipt_looks_unfinished(receipt: dict) -> bool:
    ...
    successful receipt look unfinished, or the next ``panergos update`` retriggers
...
def _receipt_reports_stale_runtime(expected_sha: str | None = None) -> bool:
    ...
    *before* the pull, so a finished update's plan always looks stale and must not
    ...
    See #95294.
    """
    from panergos_cli.update_cmd import _current_checkout_sha
    try:
        from panergos_cli.update_receipt import read_latest_receipt
...
def _live_fleet_covers_receipt(expected_sha: str | None) -> bool:
    ...
    if not expected_sha:
        return False
    from panergos_cli.update_receipt import collect_fleet_versions, read_latest_receipt
...
def _restart_systemd_gateway_units_best_effort(failed: list, listings) -> None:
    """Best-effort ``systemctl restart`` of every panergos-gateway/serve unit."""
    answered = set()
...
def _run_pending_fleet_restart() -> bool:
    ...
    with suppress(Exception):
        _m()._purge_stale_panergos_modules()
    # Warn if legacy Panergos gateway unit files are still installed. When both panergos.service (from a
    # pre-rename install) and the current panergos-gateway.service are enabled, they SIGTERM-fight for the
    # same bot token (see PR #11909). Flagging here means every `panergos update` surfaces the issue until the
    # user migrates.
    try:
    ...
    try:
        ...
        # --- Systemd services (Linux) --- Discover all panergos-gateway* units (default + profiles) plus
        # panergos-serve* units (the Desktop app's backend, #83438).
        if systemd_listings is not None:
            _restart_systemd_gateway_units_best_effort(failed, systemd_listings)
        # --- Launchd services (macOS) --- Restart EVERY ai.panergos.gateway* LaunchAgent, not only the
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``panergos update``, finish a skipped restart.

    No-op when nothing is pending; exits 1 on incomplete catch-up so automation
    does not treat the fleet as healthy.
    """
    from panergos_cli.update_cmd import _run_pending_fleet_restart
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: panergos gateway restart")
    sys.exit(1)
...
def _is_panergos_gateway_unit(unit: str) -> bool:
    """Exact base unit or hyphenated profile family only: ``startswith("panergos-serve")``
    would accept ``panergos-server.service``."""
    return (
        ...
        unit == "panergos-gateway.service"
        or unit.startswith("panergos-gateway-")
        or unit == "panergos-serve.service"
        or unit.startswith("panergos-serve-")
    )


def _for_each_systemd_gateway_unit(list_units_stdout: str, *, process_unit, on_unit_timeout) -> None:
    """Process each panergos-gateway*/panergos-serve* unit from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not parts:
            continue
        unit = parts[0]
        if not unit.endswith(".service") or not _is_panergos_gateway_unit(unit):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``panergos-gateway*`` runs ``gateway/run.py`` (the handler); SIGUSR1 would just
    kill ``panergos-serve*`` and burn the drain budget, so those go straight to the blunt
    restart. Same exact/hyphenated shape as ``_for_each_systemd_gateway_unit`` so a
    near-prefix unit like ``panergos-gatewayd`` is never signalled.

    See #83438.
    """
    return svc_name == "panergos-gateway" or svc_name.startswith("panergos-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from panergos_cli.gateway import is_macos
    if not failed_units:
        return
    ...
    print("    panergos gateway status")
    if any(not name.startswith("ai.panergos.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.panergos.") for name in ordered):
...
def _restart_launchd_gateway_after_update(*, supervision_verify: bool = True) -> tuple[list, list]:
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: panergos gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: panergos gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    The pull is shared across profiles, so every ``ai.panergos.gateway*`` LaunchAgent
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    ``None`` when undeterminable (notably ``panergos_cli.gateway`` no longer importing
...
# NOT a claim that no supervisor exists: a systemd-launched serve sets neither PANERGOS_SPAWN
# nor PANERGOS_PARENT_PID ("manual-serve"). Unit-backed serves are recovered by the fresh
...
_SERVE_SKIP_REASON = (
    ...
    " systemd unit pass when it owns a panergos-serve* unit, else left running for explicit"
    " operator restart"
)
...
def _gateway_recovery_partition(plan, *, skip_profiles: set[str] | None = None) -> tuple[dict[str, str], list[dict]]:
    ...
    Uses only the pre-checkout inventory: re-importing ``panergos_cli.gateway`` in the
    ...
    child's ``panergos-serve*`` systemd pass enumerates units from systemd; leftovers are
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    logged at debug level, so an early failure (e.g. importing ``panergos_cli.gateway`` from the freshly
...
def _gateway_home_for_pid(pid: int):
    """PANERGOS_HOME of the gateway ``pid`` per the fleet inventory, else None (own profile's file)."""
    with suppress(Exception):
        from panergos_cli.update_receipt import _profile_homes
...
def _resolve_manage_cmd(cache: dict, scope_: str, scope_cmd_: list, svc_name_: str):
    ...
    so a least-privilege sudoers entry scoped to panergos-gateway* qualifies (idempotent
...
def _restart_one_systemd_gateway_unit(
    ...
    # → drain → exit, Restart=always respawns. panergos-serve has no handler → blunt restart below.
    _main_pid = 0
    ...
    if _manage_cmd is None:
        ...
        print(
            ...
            f"    To let `panergos update` restart it automatically, allow\n"
    ...
    # budget, restart-policy mismatch). Mirrors `panergos gateway restart` (`systemd_restart()`).
...
def _restart_systemd_gateway_units(restarted_services, failed_or_stale_units, restarted_scoped_units, drain_budget):
    """Restart every active panergos-gateway*/panergos-serve* systemd unit (user + system).
    ...
    """
    from panergos_cli.gateway import supports_systemd_services, _ensure_user_systemd_env
    ...
    def _on_list_timeout(scope: str, exc: subprocess.TimeoutExpired) -> None:
        ...
        print(
            ...
            f"Check the gateway with: panergos gateway status"
        )
...
class _GatewayRestartOutcome:
    ...
    def record_receipt(self, **extra) -> None:
        ...
        with suppress(Exception):
            from panergos_cli.update_receipt import record_gateway_restart
...
def _restart_manual_gateways(out: _GatewayRestartOutcome, _drain_budget) -> None:
    ...
    if out.restarted_services or out.killed_pids:
        ...
        if unmapped_count:
            ...
            print("    Restart manually: panergos gateway run")
            if unmapped_count > 1:
                print("    (or: panergos -p <profile> gateway run  for each profile)")
...
def _force_kill_stuck_gateways(killed_pids) -> None:
    ...
    with _best_effort('Post-restart survivor sweep failed: %s'):
        from panergos_cli.gateway import find_gateway_pids, _get_service_pids
...
def _recover_after_restart_phase_abort(
    ...
    from panergos_cli.update_cmd import (
...
def _restart_gateway_fleet_after_update(_pre_update_plan, gateway_mode: bool):
    ...
    """
    from panergos_cli.update_cmd import _m, _write_gateway_update_exit_code
    ...
    # Scope-qualified twin (``user/panergos-serve`` vs ``system/panergos-serve`` are different
    ...
    # Purge stale cached Panergos modules FIRST: the import below loads new gateway
    ...
    try:
        ...
        from panergos_cli.gateway import (  # noqa: F401
            is_macos,
            find_gateway_pids,
        ...
        try:
            from panergos_cli.gateway import _get_restart_exit_wait_budget
        ...
        # macOS: EVERY ai.panergos.gateway* LaunchAgent (systemd parity).
        if is_macos():
...
def _print_legacy_units_warning() -> None:
    """Legacy panergos.service fights panergos-gateway.service over the bot token; warn on
    every update until migrated."""
    from panergos_cli.gateway import (has_legacy_panergos_units, _find_legacy_panergos_units, supports_systemd_services)
    if not (supports_systemd_services() and has_legacy_panergos_units()):
        return
    print()
    print("⚠ Legacy panergos gateway unit(s) detected:")
    for name, path, is_sys in _find_legacy_panergos_units():
        scope = "system" if is_sys else "user"
        print(f"    {path}  ({scope} scope)")
    print()
    print("  These pre-rename units (panergos.service) fight the current")
    print("  panergos-gateway.service for the bot token and cause SIGTERM")
    print("  flap loops. Remove them with:")
    print()
    print("    panergos gateway migrate-legacy")
    print()
...
def _collect_fleet_snapshot(restart, rows_expected: bool) -> list:
    ...
    """
    from panergos_cli.update_receipt import collect_fleet_versions
...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    # units, so a unit-less `panergos serve` keeps stale sys.modules. Runs AFTER
    ...
    # units, so an sshd-spawned `serve --isolated` or a manual `panergos serve` (no unit) is left running its
    ...
    print("  panergos model              # Select provider and model")
    ...
    with _best_effort('Fleet version verification failed: %s'):
        from panergos_cli.update_receipt import print_fleet_version_matrix
    ...
    with _best_effort('Runtime-outcome reconciliation failed: %s'):
        ...
        if _pre_update_plan is not None and _pre_update_plan.runtimes:
            from panergos_cli.update_inventory import (match_runtime_outcomes, report_unaccounted_runtimes)
            ...
            if report_unaccounted_runtimes(_runtime_outcomes):
                restart.incomplete = True
            with suppress(Exception):
                import panergos_cli.update_receipt as _ur
    ...
    with _best_effort('Update receipt finalize failed: %s'):
        from panergos_cli.update_receipt import finalize_update_receipt
    ...
    with _best_effort('Multiplex auto-migration after update failed: %s'):
        from panergos_cli.gateway_migrate import maybe_auto_migrate_after_update
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state (typically the freshly-pulled
    ``panergos_cli.gateway`` no longer imports, one of the ways the phase aborts). That is proof-of-safety
