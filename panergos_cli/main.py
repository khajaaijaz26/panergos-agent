#!/usr/bin/env python3
"""Panergos CLI - Main entry point.

Usage:
    panergos                     # Interactive chat (default)
    panergos chat / gateway / setup / status / cron / doctor / update / ...
    panergos --version           # Show version and update status
    panergos <cmd> --help        # Per-command help
"""

# panergos_bootstrap must be the very first import — it sets up UTF-8 stdio on
# Windows (no-op on POSIX). Guarded: after a ``git pull`` / interrupted
# ``panergos update`` the editable install's ``.pth`` may not list it yet; crashing
# here would block ``panergos update``.
try:
    import panergos_bootstrap  # noqa: F401
...
from panergos_cli._subprocess_compat import suppress_platform_ver_console
...
# Inline path math so ``python panergos_cli/main.py`` (script mode: sys.path[0]
# is panergos_cli/, not the repo root) can import panergos_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from panergos_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below. A prior
# ``panergos update`` may have left a recovery marker with a core package wiped;
# the panergos_cli.config/env_loader imports further down would then crash before
...
# PID" class it exists for. ``panergos_startup_watchdog`` is stdlib-only so it
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from panergos_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from panergos_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Cosmetic: show 'panergos' instead of 'python3.xx' in ps/top/htop.

    Order: opt-in ``setproctitle`` dep; ctypes ``prctl(PR_SET_NAME)`` (Linux,
    15-char limit); ``pthread_setname_np`` (macOS — lldb/top only, not ``ps
    aux``); no-op on Windows (the .exe is already ``panergos.exe``). Never fatal.
    """
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("panergos")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"panergos", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"panergos")
    except Exception:
        pass
...
# panergos_cli.config is importable. Cached so early callers don't re-parse YAML.
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("PANERGOS_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".panergos", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    Precedence: ``--cli`` wins, then ``--tui``/``PANERGOS_TUI=1``, then a
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("PANERGOS_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `resetTerminalModes()` is the later cousin. ``PANERGOS_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("PANERGOS_TUI_NO_EARLY_DISABLE") == "1":
        return
...
# ``panergos --version`` is answered before config/logging imports.
...
from panergos_cli.subcommands.cron import build_cron_parser
from panergos_cli.subcommands.gateway import build_gateway_parser
from panergos_cli.subcommands.profile import build_profile_parser
from panergos_cli.subcommands.model import build_model_parser
from panergos_cli.subcommands.setup import build_setup_parser

from panergos_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
from panergos_cli.subcommands.slack import build_slack_parser
from panergos_cli.subcommands.login import build_login_parser
from panergos_cli.subcommands.logout import build_logout_parser
from panergos_cli.subcommands.auth import build_auth_parser
from panergos_cli.subcommands.status import build_status_parser
from panergos_cli.subcommands.pause import build_pause_parser
from panergos_cli.subcommands.webhook import build_webhook_parser
from panergos_cli.subcommands.hooks import build_hooks_parser
from panergos_cli.subcommands.doctor import build_doctor_parser
from panergos_cli.subcommands.verify import build_verify_parser
from panergos_cli.subcommands.security import build_security_parser
from panergos_cli.subcommands.approvals import build_approvals_parser
from panergos_cli.subcommands.dump import build_dump_parser
from panergos_cli.subcommands.debug import build_debug_parser
from panergos_cli.subcommands.backup import build_backup_parser
from panergos_cli.subcommands.import_cmd import build_import_cmd_parser
from panergos_cli.subcommands.import_agent import build_import_agent_parser
from panergos_cli.subcommands.config import build_config_parser
from panergos_cli.subcommands.skin import build_skin_parser
from panergos_cli.subcommands.console import build_console_parser
from panergos_cli.subcommands.update import build_update_parser
from panergos_cli.subcommands.uninstall import build_uninstall_parser
from panergos_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
from panergos_cli.subcommands.gui import build_gui_parser
from panergos_cli.subcommands.logs import build_logs_parser
from panergos_cli.subcommands.prompt_size import build_prompt_size_parser
from panergos_cli.subcommands.memory import build_memory_parser
from panergos_cli.subcommands.acp import build_acp_parser
from panergos_cli.subcommands.tools import build_tools_parser
from panergos_cli.subcommands.insights import build_insights_parser
from panergos_cli.subcommands.monitoring import build_monitoring_parser
from panergos_cli.subcommands.skills import build_skills_parser
from panergos_cli.subcommands.pairing import build_pairing_parser
from panergos_cli.subcommands.plugins import build_plugins_parser
from panergos_cli.subcommands.mcp import build_mcp_parser
from panergos_cli.subcommands.claw import build_claw_parser
from panergos_cli.subcommands.vault import build_vault_parser
from panergos_cli.subcommands.moa import build_moa_parser
...
from panergos_cli.subcommands.checkpoints import build_checkpoints_parser
from panergos_cli.subcommands.bundles import build_bundles_parser
from panergos_cli.subcommands.curator import build_curator_parser
from panergos_cli.subcommands.pets import build_pets_parser
from panergos_cli.subcommands.journey import build_journey_parser
from panergos_cli.subcommands.computer_use import build_computer_use_parser
from panergos_cli.subcommands.sessions import build_sessions_parser
from panergos_cli.subcommands.completion import build_completion_parser
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: 'panergos {command_name}' requires an interactive terminal.\n"
...
# Profile override — MUST happen before any panergos module import: many modules
# cache PANERGOS_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# PANERGOS_HOME set, and the flag stripped so argparse never sees it. Falls back
# to ~/.panergos/active_profile for the sticky default.
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors panergos_cli.profiles._PROFILE_ID_RE


def _inside_mcp_add_args(argv: list, index: int) -> bool:
    """True once argv reaches `panergos mcp add ... --args <command argv>`.
    ...
    ``--profile``), not to Panergos' own profile selector.
    """
    try:
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    Historically the flag worked even after the subcommand (`panergos chat -p
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    """Resolve `sudo panergos -p <name>` against the invoking user's home.
    ...
    if name == "default":
        return None
    from panergos_constants import sudo_invoker_default_home
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    ``-p <name>`` or pin PANERGOS_HOME to the profile dir; a bare invocation
    means "the root PANERGOS_HOME profile". If a supervised default-profile
    child read active_profile, switching the active profile (dashboard,
    ``panergos profile use``) would silently redirect the default gateway into
    ...
    PANERGOS_SUPERVISED_CHILD (systemd unit / launchd plist / Windows task),
    PANERGOS_S6_SUPERVISED_CHILD (legacy s6 container), INVOCATION_ID (systemd
    ...
    self-hosted CI runners), PANERGOS_GATEWAY_EXTERNAL_SUPERVISOR (explicit
    opt-in). XPC_SERVICE_NAME is deliberately NOT consulted: interactive macOS
    terminals set it too.
    """
    if os.environ.get("PANERGOS_SUPERVISED_CHILD") or os.environ.get("PANERGOS_S6_SUPERVISED_CHILD"):
        return True
    ...
    return os.environ.get(
        "PANERGOS_GATEWAY_EXTERNAL_SUPERVISOR", ""
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set PANERGOS_HOME before imports."""
    argv = sys.argv[1:]
    profile_name, consume, profile_index = _scan_profile_flag(argv)

    # PANERGOS_HOME already set with no explicit flag: trust it only when it
    # points at a specific profile dir ("profiles" as immediate parent). If it
    # points at the panergos root (systemd hardcodes PANERGOS_HOME=/root/.panergos)
    # we must still read active_profile — the user may have run
    # `panergos profile use` and the gateway should honour it (#22502).
    panergos_home_env = os.environ.get("PANERGOS_HOME", "")
    if profile_name is None and panergos_home_env and Path(panergos_home_env).parent.name == "profiles":
        return
    ...
    if profile_name is None and not _under_gateway_supervisor(argv) and not _desktop_ssh_backend(argv):
        try:
            ...
            active_path = get_default_panergos_root() / "active_profile"
    ...
    if profile_name is None:
        return
    try:
        from panergos_cli.profiles import resolve_profile_env

        panergos_home = resolve_profile_env(profile_name)
    except FileNotFoundError as exc:
        panergos_home = _resolve_sudo_user_profile_env(profile_name)
        if not panergos_home:
        ...
        # A bug in profiles.py must NEVER prevent panergos from starting
        print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
        return
    os.environ["PANERGOS_HOME"] = panergos_home
...
# Windows launcher self-heal — the ``panergos`` command is a COPY of the venv
# console script staged into the managed bin dir (outside the checkout, since
# ``panergos update``'s autostash once swept ``<checkout>\bin`` copies off disk;
...
# app's ``python -m panergos_cli.main`` spawn. Gates fail toward inaction. Sits
# AFTER the profile override on purpose — no panergos module may import before
...
# ``<checkout>\bin``, where ``panergos update``'s autostash (``git stash push --include-untracked``) swept
# them off disk; with the desktop updater's ``--keep-stash`` nothing restored them and ``panergos`` stopped
...
if sys.platform == "win32":
    try:
        from panergos_cli import _install_repair as _install_repair_mod

        _install_repair_mod.ensure_windows_bin_launchers(_bootstrap_root)
    except Exception:
        pass

# Load .env from ~/.panergos/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from panergos_cli.config import get_panergos_home
from panergos_cli.env_loader import load_panergos_dotenv
...
load_panergos_dotenv(
    project_env=PROJECT_ROOT / ".env",
    load_external_secrets=sys.argv[1:2] != ["update"],
)

# Bridge security.redact_secrets → PANERGOS_REDACT_SECRETS BEFORE panergos_logging
...
try:
    ...
    # serves panergos_logging, panergos_time and later raw reads: 3-4 config.yaml parses become one.
    # Managed overlay included: administrator-pinned redact_secrets / force_ipv4 win here too.
    from panergos_cli.config_effective import load_user_config_effective as _load_effective_early

    _cfg_path = get_panergos_home() / "config.yaml"
    if _cfg_path.exists():
        _early_cfg_raw = _load_effective_early(_cfg_path)
        if "PANERGOS_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["PANERGOS_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from panergos_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from panergos_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if panergos_constants not importable yet

import logging
import threading
from datetime import datetime

from panergos_cli import __version__, __release_date__

from panergos_cli.model_setup_flows import (
...
from panergos_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from panergos_cli.main_platform_setup import (
    cmd_slack,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from panergos_cli.main_dashboard import (
...
from panergos_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from panergos_cli.main_provider_setup import (
...
from panergos_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from panergos_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from panergos_cli.main_desktop import (
    cmd_gui,
)
from panergos_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _desktop_build_needed,
    _desktop_dist_exists,
    _desktop_macos_relaunchable_fixup,
    _desktop_packaged_executable,
)
from panergos_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from panergos_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from panergos_cli.main_tui_launch import (
    _launch_tui,
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_panergos_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("PANERGOS_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("PANERGOS_TERMUX_PREFETCH_UPDATES") == "1"


def _dotenv_has_provider_key(env_file: Path, provider_env_vars: set) -> bool:
    """True if ~/.panergos/.env assigns a non-empty value to any provider key."""
...
def _auth_store_logged_in(auth_file: Path, registry, strict_profile_scope: bool) -> bool:
    ...
    from panergos_cli.auth import get_auth_status
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    """Check if at least one inference provider is usable without creating one.
    ...
    """
    from panergos_cli.config import DEFAULT_CONFIG, get_env_path, get_panergos_home, load_config
    from panergos_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_model_name, dict):
            from panergos_cli.config import split_model_config_default
    ...
    _has_panergos_config = _model_name and _model_name != DEFAULT_CONFIG.get("model", "")
    ...
    if _auth_store_logged_in(get_panergos_home() / "auth.json", PROVIDER_REGISTRY, strict_profile_scope):
        return True
    ...
    # Claude Code OAuth credentials count only once Panergos is explicitly
    # configured — having Claude Code installed isn't consent to use its tokens.
    if _has_panergos_config and not strict_profile_scope:
        try:
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from panergos_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_db():
    """Yield a read-only ``SessionDB`` (lazy import, so test patches on ``panergos_state``
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``panergos -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, panergos_bin
        cli_args: the original CLI arguments (everything after 'panergos')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    panergos_bin = container_info["panergos_bin"]
    ...
    if _probe_container(inspect_cmd, backend).returncode != 0:
        ...
        if not sudo_path:
            print(
                ...
                f"The container may be running under root. Try: sudo panergos {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if _probe_container(cmd_prefix[:2] + inspect_cmd, backend, via_sudo=True).returncode != 0:
            print(
                ...
                f"  }}];\n"
                f"\n"
                f"Or run: sudo panergos {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _create_titled_session(title: str) -> Optional[str]:
    ...
    db = None
    try:
        from panergos_state_ids import new_session_id as mint_session_id
        from panergos_state_registry import acquire
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'panergos sessions list' to see available sessions, or "
            ...
            try:
                from panergos_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from panergos_cli.foreign_sessions import import_foreign_session, pick_foreign_session
    ...
    print(f"  (later: panergos --resume {_imported_id})")
...
def _resolve_chat_session_args(args, use_tui: bool) -> None:
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'panergos sessions list' to see available sessions.")
            sys.exit(1)
...
def _warn_retired_xai_models() -> None:
    ...
    try:
        from panergos_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from panergos_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'panergos doctor' for details.\033[0m\n\n")
    except Exception:
        pass
...
def _start_chat_background_prefetch() -> None:
    ...
    The ONE exception is an unseeded ~/.panergos/skills: there the banner
    ...
    if _termux_should_prefetch_update_check():
        try:
            from panergos_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from panergos_cli.config import get_panergos_home
            skills_dir = Path(get_panergos_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import panergos_cli.banner as _banner_mod
...
def _first_run_setup_guard(args) -> None:
    """No provider configured: offer `panergos setup` (TTY) or exit 1 with guidance."""
    print()
    print(
        "It looks like Panergos isn't configured yet -- no API keys or providers found."
    )
    print()
    print("  Run:  panergos setup")
    print()
    ...
    if reply in {"", "y", "yes"}:
        ...
        return
    print()
    print("You can run 'panergos setup' at any time to configure.")
    sys.exit(1)
...
def cmd_chat(args):
    ...
    if getattr(args, "yolo", False):
        os.environ["PANERGOS_YOLO_MODE"] = "1"
    ...
    if getattr(args, "ignore_rules", False):
        os.environ["PANERGOS_IGNORE_RULES"] = "1"
    # --source: tag session source for filtering (e.g. 'tool' for integrations)
    if getattr(args, "source", None):
        os.environ["PANERGOS_SESSION_SOURCE"] = args.source
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older panergos_cli.config) crash
        # here — e.g. missing resolve_turn_limit / split_model_config_default
        # (#96900). The agent-setup mixin prints this hint too late: PanergosCLI
        # construction already failed. Fast-chat launch also goes through
        # cmd_chat, so this one catch covers `panergos` / `panergos chat`.
        from panergos_constants import emit_partial_update_hint
...
def cmd_gateway(args):
    ...
    from panergos_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from panergos_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    """A ``panergos <cmd>`` handler that hands ``args`` to ``<module>.<attr>``.
...
cmd_setup = _forward_command("cmd_setup", "panergos_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "panergos_cli.auth", "login_command", doc='Authenticate Panergos CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "panergos_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "panergos_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "panergos_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "panergos_cli.cron", "cron_command", forward_return=True, doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "panergos_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "panergos_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "panergos_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "panergos_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "panergos_cli.doctor", "run_doctor", doc='Check configuration and dependencies.')
cmd_dump = _forward_command("cmd_dump", "panergos_cli.dump", "run_dump", doc='Dump setup summary for support/debugging.')
cmd_debug = _forward_command("cmd_debug", "panergos_cli.debug", "run_debug", doc='Debug tools (share report, etc.).')
cmd_skin = _forward_command("cmd_skin", "panergos_cli.skin_cmd", "skin_command", doc='Skin management (list / use / set).')
cmd_import = _forward_command("cmd_import", "panergos_cli.backup", "run_import", doc='Restore a Panergos backup from a zip file.')
cmd_gateway_enroll = _forward_command("cmd_gateway_enroll", "panergos_cli.gateway_enroll", "cmd_gateway_enroll", doc='Enroll a self-hosted gateway with a relay connector.')
cmd_prompt_size = _forward_command("cmd_prompt_size", "panergos_cli.prompt_size", "cmd_prompt_size", doc='Show a byte/char breakdown of the system prompt + tool schemas.')
cmd_pairing = _forward_command("cmd_pairing", "panergos_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "panergos_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "panergos_cli.mcp_config", "mcp_command")
cmd_claw = _forward_command("cmd_claw", "panergos_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "panergos_cli.agent_import", "import_agent_command")


def cmd_model(args):
    """Select default model — starts with provider selection, then model picker."""
    if getattr(args, "quick", False):
        from panergos_cli.model_quickstart import run_model_quickstart

        try:
            if run_model_quickstart(args):
                return
        except RuntimeError as exc:
            print(f"Model quick-start failed: {exc}", file=sys.stderr)
            raise SystemExit(2) from exc
    _require_tty("model")
    if getattr(args, "refresh", False):
        try:
            from panergos_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from panergos_cli.setup import run_setup_action_with_navigation
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from panergos_cli.auth import AuthError, format_auth_error, resolve_provider
    from panergos_cli.config import get_compatible_custom_providers, get_env_value
    from panergos_cli.providers import custom_provider_aliases, resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            print(
                f"Warning: Unknown provider '{effective_provider}'. Check 'panergos model' for "
                "available providers, or run 'panergos doctor' to diagnose config "
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``panergos model``) and the setup wizard
    ...
    effective_provider = config_provider or os.getenv("PANERGOS_INFERENCE_PROVIDER") or "auto"
    ...
    from panergos_cli.models import _PROVIDER_LABELS
    ...
    # panergos_cli.main keep intercepting.
    ...
    # "custom") leaves a stale OPENAI_BASE_URL in ~/.panergos/.env that poisons
...
# ``panergos_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on panergos_cli.main; importing update_cmd eagerly would cost every ``panergos``
...
_FROZEN_UPDATER_SURFACE: dict[str, tuple[str, ...]] = {
    "panergos_cli.update_cmd": (
        ...
        "_print_parked_branch_skip_warning", "_purge_stale_panergos_modules",
    ...
    "panergos_cli.dashboard_procs": (
        "_detect_concurrent_panergos_instances", "_kill_stale_dashboard_processes",
    ),
}
...
def cmd_verify(args):
    ...
    from panergos_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `panergos security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from panergos_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `panergos approvals <subcmd>`."""
    from panergos_cli.approvals_suggest import approvals_command
...
def cmd_backup(args):
    """Back up Panergos home directory to a zip file."""
...
def _print_version_info(*, check_updates: bool = True) -> None:
    # Shared with the `panergos --version` pre-import fast path.
...
def cmd_uninstall(args):
    """Uninstall Panergos Agent (or just the Chat GUI with --gui).
    ...
    if getattr(args, "gui_summary", False):
        from panergos_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from panergos_cli.uninstall import run_gui_uninstall
    ...
    from panergos_cli.uninstall import run_uninstall
...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    try:
        ...
        from panergos_cli.update_receipt import finalize_pending_update_receipt
...
def _update_preflight_handled(args) -> bool:
    ...
    from panergos_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Panergos Agent")
        return True
    ...
    if getattr(args, "plan", False):
        # Read-only plan phase (#91277 Phase 2): inventory every running Panergos runtime across profiles, its
        # supervisor, and its running code version — without mutating anything. Safe on a live fleet.
        from panergos_cli.update_inventory import (
    ...
    from panergos_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        from panergos_cli.update_cmd import _cmd_update_check
...
def cmd_update(args):
    """Update Panergos Agent: hangup protection + update lock around ``_cmd_update_impl``."""
    ...
    from panergos_cli.update_lock import (
    ...
    from panergos_cli.update_cmd import _cmd_update_impl
    ...
    try:
        ...
        from panergos_cli.update_receipt import COMMAND_BOUNDARY_STOP_REASON
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    ``panergos -c Pokemon Agent Dev`` → ``['-c', 'Pokemon Agent Dev']``; tokens
...
from panergos_cli.profile_cmd import cmd_profile
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        if not _find_stale_dashboard_pids():
            print("No Panergos dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `panergos update`;
        # it prints outcomes itself. Exit 1 only if every pid was unkillable.
        from panergos_cli.dashboard_procs import _kill_stale_dashboard_processes
...
def _dashboard_validate_serve_args(args, headless_backend, token_file):
    ...
    # `panergos serve` is headless/non-interactive: fail closed on a corrupt
    ...
    if token_file and not headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with panergos serve")
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    Desktop Electron spawns its backend with PANERGOS_DESKTOP=1 plus
    PANERGOS_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    PANERGOS_SERVE_HEADLESS=1). A shell inheriting those then running
    `panergos dashboard` would serve the desktop renderer ("Desktop IPC bridge
    ...
    (PANERGOS_DESKTOP=1) keeps its dist. Headless `serve` re-sets
    PANERGOS_SERVE_HEADLESS itself.
    """
    if os.environ.get("PANERGOS_DESKTOP") != "1":
        if _is_electron_packaged_web_dist(os.environ.get("PANERGOS_WEB_DIST", "")):
            os.environ.pop("PANERGOS_WEB_DIST", None)
    if not headless_backend:
        os.environ.pop("PANERGOS_SERVE_HEADLESS", None)
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    # the same logs directory as every other Panergos surface.
    try:
        from panergos_logging import setup_logging as _setup_logging_gui
    ...
    try:
        ...
        # #61115, #65696.
        from panergos_cli.config import apply_terminal_config_to_env
    ...
    mcp_discovery_after_bind = headless_backend and os.environ.get("PANERGOS_DESKTOP") == "1"
    if not mcp_discovery_after_bind:
        try:
            from panergos_cli.mcp_startup import start_background_mcp_discovery
...
def cmd_dashboard(args):
    ...
    from panergos_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from panergos_cli import completion
...
def cmd_logs(args):
    """View and filter Panergos log files."""
    from panergos_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Panergos command console."""
    from panergos_cli.console_engine import run_console_repl
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    on import; ``discover_plugins()`` alone leaves ``panergos photon`` failing
    ...
    not import it, so the CLI registration never happens and ``panergos photon`` fails with argparse ``invalid
    choice`` (issue #54678).
    """
    if not command_name:
        return
...
def _is_tui_chat_launch(args) -> bool:
    if getattr(args, "tui", False) or os.environ.get("PANERGOS_TUI") == "1":
        return True
    # The chat path decides TUI-vs-classic via _resolve_use_tui (--cli/--tui
    # flags, TTY gate, PANERGOS_TUI env, display.interface config). Bare
    # `panergos`/`panergos chat` with a TUI display config was previously missed
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo chokepoint: PANERGOS_YOLO_MODE must be set before any discovery
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from panergos_cli.plugins import start_background_plugin_discovery
    ...
    try:
        from panergos_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from panergos_cli.mcp_startup import get_mcp_server_filter
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["PANERGOS_SAFE_MODE"] = "1"
    os.environ["PANERGOS_IGNORE_USER_CONFIG"] = "1"
    os.environ["PANERGOS_IGNORE_RULES"] = "1"
...
def _light_chat_parser():
    ...
    from panergos_cli._parser import build_top_level_parser
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("PANERGOS_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
    ...
    try:
        from panergos_cli.config import get_container_exec_info
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("PANERGOS_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("PANERGOS_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["PANERGOS_DEFER_AGENT_STARTUP"] = "1"
            os.environ["PANERGOS_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["PANERGOS_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `panergos --tui` is the hot path on phones and the TUI immediately execs
...
def _advertise_agent_env() -> None:
    ...
    (``panergos-agent``) — matching is exact. ``PANERGOS_AGENT`` is the
    Panergos-specific marker. setdefault: never clobber an outer harness.
    ...
    harness that spawned them. Panergos running inside another agent's terminal).
    """
    os.environ.setdefault("AI_AGENT", "panergos-agent")
    os.environ.setdefault("PANERGOS_AGENT", "true")
...
def _register_plugin_cli_commands(subparsers) -> None:
    ...
    try:
        ...
        from panergos_cli.plugins import discover_plugins, get_plugin_manager
...
def _cmd_sessions_lazy(args, **kwargs):
    """``panergos sessions`` handler; sessions_cmd imports only when the subcommand runs."""
    from panergos_cli.sessions_cmd import cmd_sessions

    return cmd_sessions(args, **kwargs)


def _build_cli_parser():
    """Build the full ``panergos`` argparse tree -> ``(parser, subparsers)``.

    Registration ORDER is the ``panergos --help`` order; keep it stable. Groups
    live in ``panergos_cli/subcommands/<group>.py`` with handlers injected so
    ...
    from panergos_cli.send_cmd import register_send_subparser
    ...
    from panergos_cli.subcommands.peer import build_peer_parser
    build_peer_parser(subparsers)

    from panergos_cli.kanban import build_parser as _build_kanban_parser
    _build_kanban_parser(subparsers).set_defaults(func=cmd_kanban)

    from panergos_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # "desktop" is canonical (Panergos-Setup.exe tells users to run it, so it
...
def main():
    """Main entry point for panergos CLI."""
    _set_process_title()
    _advertise_agent_env()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from panergos_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``panergos.exe.old.*`` quarantine files from previous Windows
    # updates (see ``_quarantine_running_panergos_exe``). No-op elsewhere.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass
    ...
    # Self-heal a venv left half-built by an interrupted ``panergos update``, and
    ...
    # substring match is deliberately loose: over-matching (``panergos skills
    # install update``) only defers recovery one launch; under-matching
    # (``panergos -p work update``) would race. Never raises.
    # See #95294.
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from panergos_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('panergos_cli.cli_output', 'line_input'),
}
