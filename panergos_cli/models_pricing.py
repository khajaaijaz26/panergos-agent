...
OpenRouter-compatible ``/v1/models`` pricing fetch with a per-endpoint/per-credential cache and
Vercel AI Gateway / Novita / Fireworks / DeepInfra pricing adapters. Split out of
``panergos_cli.models``; helpers still defined there are
looked up on ``panergos_cli.models`` at call time so ``patch("panergos_cli.models.<name>")`` mocks keep
intercepting.
"""
...
from panergos_cli.models_reasoning_caps import _seed_reasoning_caps
...
def compute_sale_discount(prompt: str, completion: str, original: Any) -> tuple[int, str, str] | None:
    """Sale chrome from gateway ``pricing.original`` for callers that opt in via
    ``include_sale_original=True``: ``(discount_percent, was_prompt_raw,
...
def _get_json(url: str, headers: dict[str, str], timeout: float, opener=None) -> Optional[dict]:
    ...
    from panergos_cli.models import _urlopen_model_catalog_request
...
def _pricing_entry(pricing: dict, prompt_key: str = "prompt", completion_key: str = "completion") -> dict[str, Any]:
    ...
    ``pricing`` block whose cache fields already use the panergos names."""
    entry: dict[str, Any] = {
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    another's read. *include_sale_original* copies the gateway's pre-discount
    ``pricing.original`` rates through as a nested ``original`` dict for sale chrome."""
    from panergos_cli.models import _PANERGOS_USER_AGENT
    ...
    headers = {"Accept": "application/json", "User-Agent": _PANERGOS_USER_AGENT}
    if api_key:
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Copy optional pre-discount pricing only when the caller explicitly requests it.
...
def _remember_provider_cache_key(provider: str, cache_key: str) -> None:
    from panergos_cli.models import _pricing_profile_key
...
# Static endpoint identity per provider; dynamic ones are resolved in pricing_cache_scope.
...
def pricing_cache_scope(provider: str, *, current_provider: str = "", current_base_url: str = "") -> str:
    ...
    from panergos_cli.models import _deepinfra_catalog_url, _pricing_profile_key, normalize_provider
...
def _cached_only_pricing(normalized: str) -> dict[str, dict[str, str]]:
    ...
    from panergos_cli.models import _deepinfra_catalog_cache, _deepinfra_catalog_url, _pricing_profile_key
    if normalized == "deepinfra":
...
def _fetch_novita_pricing(timeout: float = 8.0, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "Accept": "application/json", "User-Agent": _PANERGOS_USER_AGENT}
