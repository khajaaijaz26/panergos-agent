...
``~/.panergos/cache/model_catalog.json`` → master URL fetch; any fetch failure keeps the stale copy
(or ``{}``). ``get_curated_openrouter_models()`` is a thin accessor whose callers fall back to the
in-repo list on ``None``.
"""
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/khajaaijaz26/panergos-agent/main/website/static/api/model-catalog.json",
)
...
def _cache_path() -> Path:
    ...
    return get_panergos_home() / "cache" / "model_catalog.json"
...
def _fetch_manifest(url: str, timeout: float) -> dict[str, Any] | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": _PANERGOS_USER_AGENT})
...
def refresh_catalogs() -> bool:
    """Force-refresh every remote catalog the picker reads, writing each disk cache so the next
    ``/model`` open in any process sees it.
...
def get_default_model_from_cache(provider: str) -> str | None:
    """The manifest's labeled default for ``provider`` (the model Panergos silently lands on when the
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    After ``panergos update`` that file IS the newest catalog, so the picker stays current even when
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``panergos model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime, _catalog_cache_source_path
    _catalog_cache = None
