"""PANERGOS_HOME state checks for panergos doctor: directories, memory files, state.db health, skills hub, memory provider, profiles.
...
from panergos_cli.sizefmt import format_bytes as _human_bytes
from panergos_state_common import FTS_STORAGE_VERSION
...
def _doctor_memory_config(panergos_home: Path | None = None) -> dict:
    ...
    try:
        ...
        config_path = (panergos_home if panergos_home is not None else PANERGOS_HOME) / "config.yaml"
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        if deferral.get("futile"):
            ...
            lines.append(("warn", f"state.db FTS repair is blocked after {deferral.get('attempts') or '?'} deferral(s) "
                          ...
                          "'panergos sessions optimize-storage' with every holder stopped)"))
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            detail += "; run 'panergos sessions optimize-storage' offline (with the gateway stopped) to compact FTS storage"
...
def _memory_store_flags(panergos_home: Path) -> tuple:
    from tools.memory_tool import get_builtin_memory_store_flags
    return get_builtin_memory_store_flags({"memory": _doctor_memory_config(panergos_home)})


@doctor_check()
def _check_directory_structure(should_fix: bool, f: Finding) -> None:
    """PANERGOS_HOME, expected subdirs, SOUL.md, and the enabled built-in memory files."""
    from panergos_cli.doctor import PANERGOS_HOME, _DHH
    panergos_home = PANERGOS_HOME
    ensure_dir(f, should_fix, panergos_home, f"{_DHH} directory exists", f"Created {_DHH} directory", f"{_DHH} not found")
    _memory_enabled, _user_profile_enabled = _memory_store_flags(panergos_home)
    ...
    for subdir_name in ["cron", "sessions", "logs", "skills"] + (["memories"] if memory_on else []):
        ensure_dir(f, should_fix, panergos_home / subdir_name, f"{_DHH}/{subdir_name}/ exists",
                   f"Created {_DHH}/{subdir_name}/", f"{_DHH}/{subdir_name}/ not found")
    # SOUL.md persona file
    soul_path = panergos_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Panergos a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text("# Panergos Agent Persona\n\n<!-- Edit this file to customize how Panergos communicates. -->\n\n"
                                 "You are Panergos, a helpful AI assistant.\n", encoding="utf-8")
    ...
    memories_dir = panergos_home / "memories"
    if not memory_on:
...
_STATE_DB_REPAIRS = {
    "fts": ("Repaired state.db FTS write health",
            ...
            "state.db FTS write corruption — run 'panergos doctor --fix' (or 'panergos sessions repair') to rebuild the FTS index"),
    "schema": ("Repaired state.db schema ({count} sessions recovered)",
               ...
               "state.db schema malformed — run 'panergos doctor --fix' (or 'panergos sessions repair') to recover hidden sessions"),
}
_STATE_DB_STRUCTURAL_ISSUE = (
    ...
    "cannot repair it. Stop the gateway, then run 'panergos {profile_arg}sessions recover --source {db_path} "
    "--inspect-only' and, if it reports recoverable, 'panergos {profile_arg}sessions recover --source {db_path} "
...
def _repair_state_db(f: Finding, should_fix: bool, state_db_path: Path, kind: str) -> None:
    ...
    if kind == "structural":
        from panergos_constants import profile_cli_selector
    ...
    from panergos_state_repair import repair_state_db_schema
...
def _state_db_health(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str) -> None:
    ...
    try:
        ...
        from panergos_state_repair import _db_opens_cleanly, state_db_has_structural_damage
        ...
        from panergos_state import is_malformed_db_error
...
def _state_db_stats(issues: list, state_db_path: Path) -> None:
    ...
    with warn_on_error("state.db stats unavailable ({e})", "", report=lambda t, _d: check_info(t)):
        from panergos_state_dbfile import collect_state_db_stats, count_db_holders
        ...
        for _kind, _text, _detail in rows:
            ...
            if "auto_prune" in _detail:
                issues.append("state.db is large — enable sessions.auto_prune in config.yaml"
                              + (" and run 'panergos sessions optimize-storage' offline (gateway stopped)" if "optimize-storage" in _detail else ""))
...
def _state_db_wal(f: Finding, should_fix: bool, state_db_path: Path) -> None:
    ...
    with warn_on_error(""):
        ...
        if size > 50 * 1024 * 1024:  # 50 MB
            ...
            if not should_fix:
                return f.issues.append("Large WAL file — run 'panergos doctor --fix' to checkpoint")
            ...
            from panergos_state_holders import live_writer_holds_db
            from panergos_state_repair import _connect_repair_durable, _exclusive_repair_db_guard
            _SKIP = ("Large WAL file — cannot prove state.db is quiet (stop the profile's gateway first, then "
                     "re-run 'panergos doctor --fix' to checkpoint)")
            if live_writer_holds_db(state_db_path, connect_repair_durable=_connect_repair_durable):
                ...
                check_warn("WAL checkpoint skipped: cannot prove state.db is quiet",
                           ...
                           "and re-run 'panergos doctor --fix')")
            ...
            with _exclusive_repair_db_guard(state_db_path) as (guard, guard_error):
                if guard is None:
                    check_warn("WAL checkpoint skipped: could not take exclusive ownership of state.db",
                               f"({guard_error}; stop the profile's gateway and re-run 'panergos doctor --fix')")
                    return f.issues.append(_SKIP)
...
def _check_state_db(should_fix: bool, f: Finding) -> None:
    ...
    from panergos_cli.doctor import PANERGOS_HOME, _DHH
    state_db_path = PANERGOS_HOME / "state.db"
...
def _check_skills_hub(should_fix: bool, f: Finding) -> None:
    from panergos_cli.doctor import PANERGOS_HOME, _DHH
    hub_dir = PANERGOS_HOME / "skills" / ".hub"
    if check_bool(hub_dir.exists(), "Skills Hub directory exists", ("Skills Hub directory not initialized", "(run: panergos skills list)")):
...
def _memory_provider_honcho(issues: list) -> None:
    ...
    if not cfg_path.exists():
        ...
        check_bool(hcfg.api_key or hcfg.base_url,
                   ...
                   ("Honcho config not found", "run: panergos memory setup"))
    elif not hcfg.enabled:
        check_info(f"Honcho disabled (set enabled: true in {cfg_path} to activate)")
    elif not (hcfg.api_key or hcfg.base_url):
        _fail_and_issue("Honcho API key or base URL not set", "run: panergos memory setup",
                        "No Honcho API key — run 'panergos memory setup'", issues)
    else:
...
def _memory_provider_mem0(issues: list) -> None:
    ...
    if mem0_cfg.get("api_key", ""):
        ...
        _fail_and_issue("Mem0 API key not set", "(set MEM0_API_KEY in .env or run panergos memory setup)",
...
def _memory_provider_generic(name: str) -> None:
    ...
    if _provider and _provider.is_available():
        ...
        check_warn(f"{name} configured but not available", "run: panergos memory status")
    else:
        check_warn(f"{name} plugin not found", "run: panergos memory setup")


@doctor_check()
def _check_memory_provider(should_fix: bool, f: Finding) -> None:
    from panergos_cli.doctor import PANERGOS_HOME
    name = _doctor_memory_config(PANERGOS_HOME).get("provider", "")
    if not name:
...
def _check_profiles(should_fix: bool, f: Finding) -> None:
    from panergos_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
    import re as _re
    ...
    if wrapper_dir.is_dir():
        for wrapper in wrapper_dir.iterdir():
            if not wrapper.is_file():
                continue
            with warn_on_error(""):
                _m = _re.search(r"panergos -p (\S+)", wrapper.read_text(encoding="utf-8"))
