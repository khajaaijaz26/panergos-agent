"""Provider-specific runtime builders for :mod:`panergos_cli.runtime_provider`: Azure Foundry, the
...
``panergos_cli.runtime_provider.*`` (``_get_model_config``, ``load_config``, ``has_usable_secret``,
...
def _azure_foundry_api_key(rp, explicit_api_key: str) -> str:
    ...
    if not api_key:
        raise rp.AuthError(
            ...
            "~/.panergos/.env or run 'panergos model' to configure. To use "
            "keyless Microsoft Entra ID auth instead, set "
            "model.auth_mode: entra_id in config.yaml (or pick "
            "'Microsoft Entra ID' in 'panergos model')."
        )
    return api_key
...
def _resolve_azure_foundry_runtime(*, requested_provider: str, model_cfg: Dict[str, Any],
    ...
    if not base_url:
        raise rp.AuthError(
            "Azure Foundry requires a base URL. Set it via 'panergos model' or "
...
def _resolve_bedrock_runtime(requested_provider: str, model_cfg: Dict[str, Any], target_model: Optional[str]) -> Dict[str, Any]:
    ...
    from panergos_cli.config import load_config  # direct (not the origin delegate), as before
    rp = _rp()
