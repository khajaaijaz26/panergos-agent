"""`panergos checkpoints` CLI subcommand.
...
from panergos_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: panergos checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``panergos checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `panergos checkpoints` → status
...
_PLUGIN_COMPAT_LAZY = {
    'cmd_list': ('panergos_cli.plugins_cmd', 'cmd_list'),
}
