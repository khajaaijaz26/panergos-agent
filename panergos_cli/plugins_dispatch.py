...
Mixed into :class:`panergos_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
from panergos_cli.middleware import OBSERVER_SCHEMA_VERSION
...
PLUGIN_SECTIONS_START = "<!-- panergos-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- panergos-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- panergos-plugin-section-chars:{len(content)} -->\n\n{content}")
...
# Reserved event namespace prefix — only core may publish ``panergos:<event>``.
PANERGOS_EVENT_NAMESPACE = "panergos"
...
class PluginDispatchMixin:
    ...
    def _invoke_hook_callback(callback: Callable, payload: Dict[str, Any]) -> Any:
        ...
        """
        from panergos_cli.plugins import resolve_plugin_command_result
        try:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from panergos_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"panergos-hook-{callback_name}"[:40], daemon=True)
        try:
            thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="panergos-plugin-events",
            daemon=True,
        )
