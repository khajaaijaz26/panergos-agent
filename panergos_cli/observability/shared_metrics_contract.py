"""Bounded product contract for the first Panergos shared-metrics slice."""
...
SCHEMA_KEY = "panergos.metrics.schema_version"
SCHEMA_VERSION = "panergos.metrics.event.v2"
MODEL_CALL_SCOPE = "panergos.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "panergos.task_run"
TOOL_CALL_SCOPE = "panergos.tool_call"
CLIENT_ACTIVE_MARK = "panergos.client.active"
TOOL_APPROVAL_MARK = "panergos.tool_approval"
SKILL_LIFECYCLE_MARK = "panergos.skill.lifecycle"
SKILL_LOAD_MARK = "panergos.skill.load"
SUBSCRIBER_NAME = "panergos.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "panergos.client.active"
LEGACY_MODEL_CALL_METRIC = "panergos.model_call.count"
MODEL_ROUTE_METRIC = "panergos.model_route.count"
TASK_STARTED_METRIC = "panergos.task_run.started"
TASK_FINISHED_METRIC = "panergos.task_run.finished"
TOOL_CALL_METRIC = "panergos.tool_call.count"
TOOL_APPROVAL_METRIC = "panergos.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "panergos.skill.lifecycle.count"
SKILL_LOAD_METRIC = "panergos.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "panergos_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Panergos installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    panergos_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(panergos_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "panergos_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("panergos_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Panergos logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "panergos.call_role"
    )
    call_role = (metadata or {}).get("panergos.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from panergos_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Panergos terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "panergos-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Panergos registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Panergos tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Panergos."""
