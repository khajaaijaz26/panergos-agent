"""``panergos debug`` debug tools for Panergos Agent."""
...
_REDACTION_BANNER = (
    "[panergos debug share: log content redacted at upload time. "
...
_USER_AGENT = "panergos-agent/debug-share"
...
# ``panergos debug`` sweeps on entry (CLI-only users). Replaced a fork-and-sleep subprocess that
# leaked ~20 MB per share.


def _pending_file() -> Path:
    return get_panergos_home() / "pastes" / "pending.json"
...
def _save_pending(entries: list[dict]) -> None:
    try:
        ...
        pass  # non-fatal — worst case the user runs ``panergos debug delete`` manually
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `panergos debug share` "
...
def _upload_dpaste_com(content: str, expiry_days: int = 1) -> str:
    boundary = "----PanergosDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from panergos_cli.logs import LOG_FILES
    filename = LOG_FILES.get(log_name)
    return (get_panergos_home() / "logs" / filename) if filename else None
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Panergos Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``panergos dump`` and return its stdout as a string."""
    from panergos_cli.dump import run_dump
    capture = io.StringIO()
...
def collect_debug_report(
    *, log_lines: int = 200, dump_text: str = "",
    ...
    ``dump_text`` is pre-captured dump output; when empty, ``panergos dump`` is run internally.
    """
    buf = io.StringIO()
...
class DebugShareResult:
    """Outcome of a debug-share upload for CLI and dashboard callers."""
...
def build_debug_share(
    ...
    Shared by ``panergos debug share`` and the dashboard ``POST /api/ops/debug-share``. Blocking
    ...
    if redact:
        logger.info(
            "panergos debug share: applied force-mode redaction to log snapshots before upload")
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `panergos debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    if dpaste_urls:
        ...
        print(f"⚠️  {len(dpaste_urls)} of {len(result.urls)} upload(s) fell back to "
              ...
              "deleted with `panergos debug delete`.\n"
              "\nShare these links with the Panergos team for support.")
    else:
        print(f"\n⏱  Pastes will auto-delete in {result.auto_delete_seconds // 3600} hours.\n"
              "To delete now:  panergos debug delete <url>\n"
              "\nShare these links with the Panergos team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: panergos debug delete <url> [<url> ...]\n"
              "  Deletes paste.rs pastes uploaded by 'panergos debug share'.")
        return
    for url in urls:
        try:
            if delete_paste(url):
