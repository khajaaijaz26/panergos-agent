...
def _anthropic_oauth_status() -> Dict[str, Any]:
    """Status for the "Anthropic API Key" card: Panergos-managed PKCE file first, then the
    ...
    """
    try:
        from agent.anthropic_credentials import read_panergos_oauth_credentials, _get_panergos_oauth_file
        panergos_creds = read_panergos_oauth_credentials()
    except Exception:
        panergos_creds = None
    if panergos_creds and panergos_creds.get("accessToken"):
        return _token_status("panergos_pkce", f"Panergos PKCE ({_get_panergos_oauth_file()})", panergos_creds)
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    store); the CLI may hold its session in an OS keychain Panergos can't read, so the unverified
    state reads "managed by the Copilot CLI" — never signed out."""
    try:
        from panergos_cli.auth import get_external_process_provider_status
...
def _external_process_cli_command(provider_id: str, default: str) -> str:
    ...
    (``PANERGOS_COPILOT_ACP_COMMAND`` / ``COPILOT_CLI_PATH``); others get ``default`` untouched."""
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    {"id": "openai-codex", "name": "ChatGPT or Codex Subscription", "flow": "device_code",
     "cli_command": "panergos auth add openai-codex", "docs_url": "https://platform.openai.com/docs",
     "status_fn": None},
    {"id": "qwen-oauth", "name": "Qwen (via Qwen CLI)", "flow": "external",
     "cli_command": "panergos auth add qwen-oauth", "docs_url": "https://github.com/QwenLM/qwen-code",
     "status_fn": None},
    ...
    {"id": "minimax-oauth", "name": "MiniMax (OAuth)", "flow": "device_code",
     "cli_command": "panergos auth add minimax-oauth", "docs_url": "https://www.minimax.io", "status_fn": None},
    ...
    {"id": "xai-oauth", "name": "xAI Grok OAuth (SuperGrok / Premium+)", "flow": "device_code",
     "cli_command": "panergos auth add xai-oauth",
     "docs_url": "https://khajaaijaz26.github.io/panergos-agent/docs/guides/xai-grok-oauth", "status_fn": None},
    ...
    # Login works via the terminal (`panergos auth add anthropic`) or a plain API key.
    {"id": "anthropic", "name": "Anthropic API Key", "flow": "external", "cli_command": "panergos auth add anthropic",
...
def _record_sign_in_state(sess: Dict[str, Any], state: Any) -> None:
    """Write one sign-in state onto the dashboard session, under the sessions lock.
...
def _minimax_poller(session_id: str, sess: Dict[str, Any]) -> None:
    """MiniMax poller using a PKCE-style ``code_verifier`` plus ``user_code``.
    ``device_code``. Builds the same auth_state as the CLI's ``_minimax_oauth_login`` and persists
    via ``_minimax_save_auth_state`` so the system ends up as after ``panergos auth add minimax-oauth``.
    Region is fixed to "global" here; cn-region operators use the CLI's ``--region cn``."""
    from panergos_cli.web_server_profiles import _profile_scope
...
def _xai_device_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    with _profile_scope(_oauth_session_profile(session_id)):
        ...
        # Mirror `panergos auth add xai-oauth`: first credential may become active; never overwrite.
