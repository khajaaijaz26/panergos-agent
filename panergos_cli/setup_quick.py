"""Streamlined setup flows: Blank Slate setup and the `--quick` missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``panergos_cli.setup`` take effect."""
...
def _blank_slate_done(config: dict, panergos_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from panergos_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         panergos skills opt-in --sync",
          "  Add MCP servers:     panergos mcp add", *extra, "  Tune agent settings: panergos setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from panergos_cli.setup import load_config
...
def _print_macos_fda_tip() -> None:
    ...
    """
    from panergos_cli.setup import _info
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Panergos Desktop if you use it), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Panergos update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``panergos-agent`` skill
    ...
    try:
        ...
        from panergos_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
        all_keys = {k for k, _, _ in CONFIGURABLE_TOOLSETS}
        all_keys.update(_get_plugin_toolset_keys())
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "panergos-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("panergos-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``panergos setup agent``."""
...
def _run_blank_slate_setup(config: dict, panergos_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `panergos-agent` skill is always kept so the agent",
          "can help you drive and configure Panergos Agent itself.", None)
    ...
    if path != 0:
        _blank_slate_walkthrough(config, panergos_home)
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `panergos update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, panergos_home, "  Enable tools:        panergos tools", "  Enable plugins:      panergos plugins",
                      intro="Enable anything later, on demand:")


def _blank_slate_walkthrough(config: dict, panergos_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `panergos-agent`",
              "skill). A .no-bundled-skills marker keeps future",
              "`panergos update` runs from re-injecting them. Opt back in any",
              "time with `panergos skills opt-in --sync`.")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `panergos tools`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `panergos plugins list` / `panergos plugins install`.",
         "No plugins enabled. Add later with `panergos plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `panergos mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `panergos mcp add`."),
    ):
    ...
    save_config(config)
    _blank_slate_done(config, panergos_home, "  Enable more tools:   panergos tools")


def _run_quick_setup(config: dict, panergos_home):
    ...
    from panergos_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'panergos setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Panergos to messaging apps to chat from anywhere.",
              "You can configure these later with 'panergos setup gateway'.")
