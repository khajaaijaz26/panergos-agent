...
class _ShallowLock:
    ...
    def __enter__(self) -> "_ShallowLock":
        ...
        try:
            os.write(fd, b"panergos shallow maintenance\n")
        finally:
            os.close(fd)
        return self
...
def prune_stale_shallow_grafts(repo_root: Path) -> int:
    ...
    break ``merge-base`` and push ``panergos update`` into the orphan-divergence reset path
