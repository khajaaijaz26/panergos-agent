"""Persistent session goals — the Ralph loop for Panergos.
...
# (schema DDL, FTS tables, first panergos_cli.config import) measures ~300ms warm and more on slow
...
def _get_session_db() -> Optional[Any]:
    """Cached SessionDB per PANERGOS_HOME (profile switches pick the right DB); None on any failure.
    ...
    if cached is not None and _registry_tore_down(cached):
        # ``panergos profile delete`` force-closes every handle under the profile home
        # (``panergos_state_registry.close_all_under``) before rmtree; a same-name recreate in this
