"""Agent construction + session-resume display for ``PanergosCLI``: credential resolution,
...
def _single_query_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    """Headless clarify answer for ``panergos chat -q``.
...
_RESUME_SKIN_COLORS = (
    ("banner_text", "#F7F2FF"), ("session_label", "#FF6B5E"), ("session_border", "#4A385A"),
    ("ui_ok", "#8FBC8F"))
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``PanergosCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not callable(api_key) and not (isinstance(api_key, str) and api_key):
            if _keyless_custom_base(base_url):
                ...
                print("   Run 'panergos model' to choose a provider, or "
                      "'panergos setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: panergos setup")
            return False
        ...
        # is just the provider slug/display name (`panergos chat --model <provider-name>`
        ...
        # Still empty (e.g. `panergos auth add` without `panergos model`): fall back to the
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        startup, TTY). Runs the same flow as ``panergos model`` so onboarding has a single
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'panergos model' or 'panergos setup' any time.")
            return False
        try:
            ...
            print()
            _cprint("  Setup cancelled. Run 'panergos model' any time.")
            return False
        except Exception as exc:
            logger.debug("first-run provider setup failed: %s", exc)
            _cprint(f"  ⚠️  Provider setup failed: {exc}")
            _cprint("  Run 'panergos model' to try again.")
            return False
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'panergos model' to retry.")
        return False
    ...
    def _load_resumed_history_late(self) -> bool:
        ...
        # stderr so stdout stays machine-readable for `$(panergos chat -Q --resume ...)`.
        ...
        if not session_meta:
            hint = "Use a session ID from a previous CLI run (panergos sessions list)."
            if _quiet_mode:
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        try:
            ...
            from panergos_constants import partial_update_hint
    ...
    def _preload_resumed_session(self) -> bool:
        ...
        if not session_meta:
            ...
            self._console_print("[dim]Use a session ID from a previous CLI run (panergos sessions list).[/]")
            return False
        session_meta = self._follow_compression_chain(
            session_meta,
