"""Opt-in Telegram QR client for a self-hosted onboarding service.

The service receives the requested bot name and, after Telegram confirmation, returns the raw bot
token and owner ID once so Panergos can save them locally. ``TELEGRAM_ONBOARDING_URL`` can point to
a compatible self-hosted service instead.
"""
...
# No hosted service is assumed. Operators can opt in with a compatible endpoint.
DEFAULT_API_URL = ""
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"
DEFAULT_BOT_NAME = "Panergos Agent"
...
def create_pairing(
    ...
    if not base_url:
        return None
    try:
        resp = httpx.post(f"{base_url}/v1/telegram/pairings", json={"bot_name": bot_name}, timeout=timeout)
...
def poll_pairing_result_once(
    ...
    if not base_url:
        return None
    resp = httpx.get(f"{base_url}/v1/telegram/pairings/{pairing.pairing_id}",
...
def auto_setup_telegram_bot_result(
    api_url: str | None = None, manager_bot: str = "PanergosSetupBot",
    ...
    if not resolved_api_url:
        print(f"\n  Telegram onboarding is not configured. Set {TELEGRAM_ONBOARDING_URL_ENV}, or use manual setup.")
        return None
    print(f"\n  Contacting Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the external Telegram onboarding service.\n"
...
DEFAULT_MANAGER_BOT = "PanergosSetupBot"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``panergos_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``panergos_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"panergos_{generate_username_slug()}_bot"
