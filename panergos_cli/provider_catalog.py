...
The provider list shown by ``panergos model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`panergos_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `panergos model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``panergos model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("panergos_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("panergos_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    PANERGOS_OVERLAYS = _safe_import("panergos_cli.providers", "PANERGOS_OVERLAYS", {})
    try:
