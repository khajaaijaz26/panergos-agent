...
01-panergos-setup (the stage2 hook) has chowned the volume and seeded $PANERGOS_HOME, but
...
def reconcile_profile_gateways(
    *, panergos_home: Path, scandir: Path, dry_run: bool = False,
    ...
    the top of ``$PANERGOS_HOME``): ``panergos_cli.gateway`` maps an empty profile suffix to it,
    so it is what ``panergos gateway start`` (no ``-p``) targets.

    Without it, bare ``panergos gateway start`` inside the container would land on ``s6-svc -u
    ...
    legacy_default_state = _maybe_migrate_legacy_gateway_run_state(
        panergos_home, container_argv=container_argv, dry_run=dry_run)
    default_prior_state = legacy_default_state or _read_desired_state(panergos_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(panergos_home)
        _register_service(scandir, "default", start=default_should_start)
    actions.append(_slot_action("default", panergos_home, default_prior_state, default_should_start))

    profiles_root = panergos_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            # SOUL.md (seeded by `panergos profile create`) is the "real profile" marker.
    ...
    if not dry_run:
        _write_reconcile_log(panergos_home, actions)
    return actions


def _maybe_migrate_legacy_gateway_run_state(
    panergos_home: Path, *, container_argv: Sequence[str] | None, dry_run: bool) -> str | None:
    ...
    state_file = panergos_home / "gateway_state.json"
    if state_file.exists():
        return None
    if os.environ.get("PANERGOS_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the panergos args.
    ...
    if args and args[0].endswith("entrypoint-dispatch.sh"):  # non-PID-1 dispatch shim
        args = args[1:]
    if args and Path(args[0]).name == "panergos":  # the wrapper re-execs `panergos <subcommand>`
        args = args[1:]
    return args
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        if not start:  # `panergos -p <profile> gateway start` brings it up later (s6-svc -u)
            (tmp_dir / "down").touch()
        # Pre-create supervise/ with panergos ownership BEFORE publishing so s6-supervise inherits
        # it and runtime s6-svc calls as the panergos user won't EACCES.
...
def _write_reconcile_log(panergos_home: Path, actions: list[ReconcileAction]) -> None:
    """Append one line per profile to $PANERGOS_HOME/logs/container-boot.log (rotated to ``.1``) —
...
def main() -> int:
    ...
    # A dashboard-only container must not reconcile: with a shared bind-mounted PANERGOS_HOME both
    ...
    actions = reconcile_profile_gateways(panergos_home=panergos_home, scandir=scandir)
    for a in actions:
