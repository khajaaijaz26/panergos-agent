...
Split out of ``panergos_cli.models``. Catalog fetchers defined in ``panergos_cli.models`` are looked up
there at call time (``_m.<name>``) so ``patch("panergos_cli.models.<name>")`` mocks keep intercepting;
local-server probes are looked up on ``panergos_cli.models_local`` (``_ml.<name>``) the same way.
...
def _validate_moa(req: _Request) -> dict[str, Any]:
    try:
        ...
        if req.requested in cfg["presets"]:
            return _accept()
        return _reject(f"MoA preset `{req.requested}` was not found. Run `panergos moa list`.")
...
def _validate_lmstudio(req: _Request) -> dict[str, Any]:
    from panergos_cli import models_local as _ml
...
def _ollama_probe_headers(req: _Request) -> dict[str, str]:
    ...
    from panergos_cli import models as _m
    from panergos_cli import models_local as _ml
    from panergos_cli.models_local import _configured_ollama_base_url, _drop_authorization
...
def _validate_ollama_native(req: _Request) -> Optional[dict[str, Any]]:
    ...
    if models is None:
        return _soft_accept(
            ...
            "Panergos will save the model name, but local Ollama model discovery could not verify it."
        )
...
def _validate_static_catalog(req: _Request) -> Optional[dict[str, Any]]:
    ...
    if req.normalized == "openai-codex":
        ...
        # the suffix is a Panergos picker convention, so an unknown `*-900k` can never be a real
...
def _validate_minimax(req: _Request) -> Optional[dict[str, Any]]:
    ...
    return match.verdict(req) or _soft_accept(
        ...
        f"{match.suggestion_text}"
        "\n  MiniMax does not expose a /models endpoint, so Panergos cannot verify the model name."
