"""Panergos skin/theme engine — the theme SDK for every surface."""
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces PANERGOS_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces the Panergos mark)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default", "description": "Panergos Eclipse — coral, jade, and gold on plum",
        # Dark-authored; values match the TUI fallback so both render one identity.
        "colors": {
            "banner_border": "#FF6B5E", "banner_title": "#FF6B5E", "banner_accent": "#2EE6A6",
            "banner_dim": "#B9A9C8", "banner_text": "#F7F2FF", "ui_accent": "#FF6B5E",
            "ui_label": "#F7C453", "ui_ok": "#2EE6A6", "ui_error": "#FF4773", "ui_warn": "#F7C453",
            "prompt": "#FF6B5E", "input_rule": "#FF6B5E", "response_border": "#FF6B5E",
            "status_bar_bg": "#120B1F", "status_bar_text": "#F7F2FF",
            "status_bar_strong": "#FF6B5E", "status_bar_dim": "#8F7E9E",
            "status_bar_good": "#2EE6A6", "status_bar_warn": "#F7C453", "status_bar_bad": "#FF8A4C",
            "status_bar_critical": "#FF4773", "session_label": "#FF6B5E",
            "session_border": "#6B557B", "completion_menu_bg": "#1A122A",
            "completion_menu_current_bg": "#332440", "selection_bg": "#4A385A",
            "shell_dollar": "#2EE6A6", "voice_status_bg": "#1A122A"},
        # Light overlay keeps the same hue families while using AA-readable
        # coral/jade/gold shades and light-polarity surfaces.
        "light_colors": {
            "banner_border": "#B03A32", "banner_title": "#B03A32", "banner_accent": "#14755D",
            "banner_dim": "#675873", "banner_text": "#20142B", "ui_accent": "#B03A32",
            "ui_label": "#8A6110", "ui_ok": "#14755D", "ui_error": "#B42342", "ui_warn": "#8A6110",
            "prompt": "#B03A32", "input_rule": "#B03A32", "response_border": "#B03A32",
            "session_label": "#B03A32", "status_bar_text": "#20142B",
            "status_bar_strong": "#B03A32", "status_bar_dim": "#675873",
            "status_bar_good": "#14755D", "status_bar_warn": "#8A6110", "status_bar_bad": "#A44820",
            "status_bar_critical": "#B42342", "shell_dollar": "#14755D",
            "completion_menu_bg": "#F7EFF8", "completion_menu_current_bg": "#E8CFC7",
            "selection_bg": "#F9E3DC", "status_bar_bg": "#F7EFF8", "voice_status_bg": "#F7EFF8"},
        "spinner": {},  # empty = hardcoded defaults in display.py
        "branding": _PANERGOS_BRANDING,
        "tool_prefix": "┊"},
    "crimson": {
        "name": "crimson", "description": "Crimson and bronze",
        "colors": {
    ...
    "mono": {
        ...
        "spinner": {},
        "branding": {**_PANERGOS_BRANDING, "help_header": "[?] Available Commands"},
        "tool_prefix": "┊"},
    "slate": {
        ...
        "spinner": {}, "branding": _PANERGOS_BRANDING, "tool_prefix": "┊"},
    "daylight": {
        "name": "daylight",
    ...
    "warm-lightmode": {
        ...
        "spinner": {}, "branding": _PANERGOS_BRANDING, "tool_prefix": "┊"},
    "tide": {
        "name": "tide", "description": "Deep blue and seafoam",
        "colors": {
        ...
        "spinner": {
            ...
            "thinking_verbs": [
                ...
                "steering the plan", "tracking changes", "plotting routes",
                "checking pressure", "preparing output"],
            "wings": _wings("≈", "∼", "∿", "◌")},
        "branding": _PANERGOS_BRANDING,
        "tool_prefix": "│",
    },
    "granite": {
        "name": "granite", "description": "Austere high-contrast grayscale",
        "colors": {
        ...
        "spinner": {
            ...
            "thinking_faces": ["(◉)", "(◬)", "(◌)", "(○)", "(●)"],
            "thinking_verbs": [
                "finding traction", "measuring progress", "resetting the plan",
                "counting steps", "testing leverage", "setting the baseline",
                "working steadily", "closing the loop"],
            "wings": _wings("◉", "◬", "◌", "⬤")},
        "branding": _PANERGOS_BRANDING,
        "tool_prefix": "│",
    },
    "ember": {
        "name": "ember", "description": "Burnt orange and ember",
        "colors": {
        ...
        "spinner": {
            ...
            "thinking_faces": ["(✦)", "(▲)", "(◇)", "(⌁)", "(🔥)"],
            "thinking_verbs": [
                "warming up", "measuring heat", "reading signals",
                "tracking progress", "setting parameters", "holding steady",
                "plotting next steps", "preparing output"],
            "wings": _wings("✦", "▲", "◌", "◇")},
        "branding": _PANERGOS_BRANDING,
        "tool_prefix": "│",
    }}

# Previous built-in identifiers remain accepted for saved config and scripts, but are deliberately
# absent from ``list_skins()`` so new config, help, and completions use the neutral canonical names.
_BUILTIN_SKIN_ALIASES = {
    "ares": "crimson",
    "poseidon": "tide",
    "sisyphus": "granite",
    "charizard": "ember",
...
def _routed_home_key() -> Optional[str]:
    ...
    return None if get_panergos_home_override() is None else panergos_home_key()
...
def _skins_dir() -> Path:
    return get_panergos_home() / "skins"


def canonical_skin_name(name: str) -> str:
    """Return a built-in's canonical name without overriding a same-named user skin."""
    canonical = _BUILTIN_SKIN_ALIASES.get(name)
    if canonical is None:
        return name
    user_file = _skins_dir() / f"{name}.yaml"
    return name if user_file.is_file() and _load_skin_from_yaml(user_file) else canonical
...
_STYLE_PALETTE = (
    ("prompt", "prompt", ""), ("input_rule", "input_rule", "#FF6B5E"),
    ("title", "banner_title", "#FF6B5E"), ("text", "banner_text", "#F7F2FF"),
    ("dim", "banner_dim", "#8F7E9E"), ("label", "ui_label", "@title"), ("warn", "ui_warn", "#F7C453"),
    ("error", "ui_error", "#FF4773"), ("status_bg", "status_bar_bg", "#120B1F"),
    ("status_text", "status_bar_text", "@text"), ("status_strong", "status_bar_strong", "@title"),
    ("status_dim", "status_bar_dim", "@dim"), ("ok", "ui_ok", "#2EE6A6"),
    ...
    ("menu_bg", "completion_menu_bg", "#1A122A"), ("menu_current_bg", "completion_menu_current_bg", "#332440"),
...
def get_prompt_toolkit_style_overrides() -> Dict[str, str]:
    ...
    palette["badge_bg"] = skin.colors.get(
        "status_bar_strong", skin.colors.get("banner_title", "#FF6B5E"))
    palette["badge_fg"] = skin.colors.get("status_bar_bg", "#120B1F")
