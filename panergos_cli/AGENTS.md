# panergos_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles
...
Skill slash commands (`agent/skill_commands.py`) scan `~/.panergos/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`panergos_cli/curses_ui.py`; example
`panergos_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`panergos_cli/commands.py`)
...
`panergos_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`panergos_cli/config.py`)
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`panergos tools/setup`, most subcommands, `panergos_cli/config.py`, merges `DEFAULT_CONFIG`);
  `panergos_cli/config_effective.py::load_user_config_effective()` (gateway runtime via
  `gateway/run.py::_load_gateway_config`, TUI gateway `_load_cfg`, cron, `panergos send`, doctor,
  `panergos_time`/`panergos_logging`: user file + managed overlay + `${VAR}` expansion + model-key
...
## Skin engine (`panergos_cli/skin_engine.py`)
...
## Update pipeline (`panergos update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `panergos update --plan`): read-only inventory — install kind, all
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `panergos-gateway*` unit /
  `ai.panergos.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.panergos/logs/update_receipts/`
...
`_apply_profile_override()` in `panergos_cli/main.py` sets `PANERGOS_HOME` before any module import, so
every `get_panergos_home()` scopes to the active profile (rules in root). Profiles are independent
...
cron all go through `mkdir_under_panergos_home` / `_ensure_cron_dir`, which refuse a deleted or
missing named profile, #94590). Process-global per-profile slots (MCP discovery in `mcp_startup.py`,
tool registry overlays) key on `panergos_constants.panergos_home_key()`, never a single flag.
Migration from per-profile gateways: `panergos_cli/gateway_migrate.py` (`panergos gateway migrate
...
other service domain / UNIX user / PANERGOS_HOME outside `profiles/` — notices for the explicit command,
