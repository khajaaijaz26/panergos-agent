"""Panergos Agent — Web UI server: FastAPI app assembly, auth/host middleware, ``start_server``.
...
Usage: ``python -m panergos_cli.main web [--port 8080]``.
"""
...
from panergos_cli.install_identity import get_install_id as _shared_get_install_id
from panergos_cli.pty_session import run_reaper
...
from panergos_cli import __distribution_version__
...
try:
    ...
    # running `panergos dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["PANERGOS_WEB_DIST"]) if "PANERGOS_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)


from panergos_cli.web_server_lifecycle import (  # noqa: E402
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The desktop spawns a ``panergos dashboard`` backend, not a gateway, so without
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            from panergos_cli.profiles import (
            ...
            if profile_homes:
                ...
                from panergos_logging import enable_profile_log_routing
...
async def _lifespan(app: "FastAPI"):
    ...
    # (#79531/#80037): a store left behind by `panergos update` otherwise 500s
    ...
    # Import panergos_cli.gateway *before* the yield: on Windows + 3.11 the
    ...
    # refuse with "restart required" after `panergos update` replaced the code
    ...
    # Desktop-spawned backends (PANERGOS_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `panergos
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("PANERGOS_DESKTOP") == "1":
        ...
        try:
            from panergos_cli.gateway import _reap_unsupervised_gateway_orphans
    ...
    def _boot_local_runtime():
        try:
            from panergos_cli.config import load_config
            from panergos_cli.local_runtime.bootstrap import ensure_local_runtime
    ...
    try:
        ...
        try:
            from panergos_cli.local_runtime.bootstrap import shutdown_local_runtime
...
app = FastAPI(title="Panergos Agent", version=__distribution_version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from panergos_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402

app.include_router(_memory_oauth_router)

# Session token for sensitive endpoints. The desktop shell mints it via
# PANERGOS_DASHBOARD_SESSION_TOKEN; otherwise fresh per server start. It dies with
# the process and is injected into the SPA HTML so only the web UI can use it.
def _resolve_session_token() -> str:
    return os.environ.get("PANERGOS_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Panergos-Session-Token"
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".panergos-ssh-runtime-{nonce}")
...
from panergos_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS as _PUBLIC_API_PATHS
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    """
    from panergos_cli.dashboard_auth.prefix import resolve_public_url
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    old launch scripts but IGNORED since the June 2026 panergos-0day campaign.
    """
...
def _desktop_loopback_auth_exempt(
    ...
    ``PANERGOS_DESKTOP=1``, and an operator-minted credential (env token, SSH
    session token, or owner nonce).
    """
    return (
        host in _LOOPBACK_HOST_VALUES
        and os.environ.get("PANERGOS_DESKTOP") == "1"
        and bool(os.environ.get("PANERGOS_DASHBOARD_SESSION_TOKEN") or ssh_session_token or ssh_owner_nonce)
    )
...
async def _plugin_api_runtime_gate(request: Request, call_next):
    ...
    if plugin_name and (
        ...
        try:
            ...
            # (#46435)
            from panergos_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
...
async def _dashboard_auth_gate(request: Request, call_next):
    ...
    """
    from panergos_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    """
    from panergos_cli.dashboard_auth.token_auth import token_auth_middleware
...
from panergos_cli import web_server_gateway as _gateway_mod  # noqa: E402
from panergos_cli.web_server_gateway import _ACTION_LOG_FILES, _terminate_desktop_managed_gateway  # noqa: E402
from panergos_cli.web_server_sessions import _auto_archive_ticker_loop  # noqa: E402
from panergos_cli.web_server_chat import PTY_REGISTRY  # noqa: E402
from panergos_cli.web_server_dashboard import (  # noqa: E402
...
# persisted under the ROOT Panergos home (not the profile PANERGOS_HOME) so every
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``panergos gateway restart``, reusing an in-flight or recent restart.
    ...
    proc = _gateway_mod._spawn_panergos_action(subcommand, "gateway-restart")
...
from panergos_cli.web_routers import (  # noqa: E402
...
from panergos_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _no_auth_provider_message(host: str) -> str:
    ...
    if host in _LOOPBACK_HOST_VALUES:
        ...
        fix_hint = (
            ...
            "(and unset PANERGOS_DASHBOARD_PUBLIC_URL) to restore the "
    ...
    fix_hint += (
        ...
        "  • OAuth: install and configure a DashboardAuthProvider plugin.\n"
    ...
    try:
        from panergos_cli.config import load_config as _load_cfg
        from panergos_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
        ...
        if has_creds and (set(disabled) & _BASIC_AUTH_PLUGIN_KEYS):
            fix_hint = (
                ...
                "`panergos plugins enable basic`), then restart the "
                "dashboard.\n\n"
            ) + fix_hint
    except Exception:
        pass
...
def _configure_auth_gate(
    ...
    # ``--insecure`` no longer disables the gate (June 2026 panergos-0day
    ...
    if app.state.auth_required:
        ...
        from panergos_cli.dashboard_auth import list_providers
...
def _build_uvicorn_server(host: str, port: int, *, ssh_isolated: bool = False):
    ...
    if ssh_isolated:
        from panergos_cli.web_server_idle_exit import (
...
def _on_server_started(
    ...
    *,
    ...
    def _reap_desktop_serves() -> None:
        from panergos_cli.dashboard_procs import _reap_orphaned_desktop_local_serves

        _reap_orphaned_desktop_local_serves()

    def _reap_mcp_helpers() -> None:
        from panergos_cli.process_identity import reap_orphaned_mcp_helpers
    ...
    # No-op for standalone `panergos serve` (no PANERGOS_PARENT_PID).
    _start_parent_death_watchdog()
    ...
    if getattr(app.state, "ssh_isolated_clients", None) is not None:
        from panergos_cli.web_server_idle_exit import DEFAULT_IDLE_GRACE_S, start_idle_watchdog
        try:
    ...
    # ACTUAL port — what lets `panergos update` relaunch a manually-started serve
    # on its real endpoint (#63206).
    def _register_identity() -> None:
        from panergos_cli.process_identity import attach_self_to_kill_on_close_job, register_self
    ...
    ready_token = "PANERGOS_BACKEND_READY" if headless else "PANERGOS_DASHBOARD_READY"
    ...
    if headless:
        ...
        print(f"  Panergos backend listening on {host}:{actual_port}", flush=True)
    else:
        print(f"  Panergos Web UI → http://{host}:{actual_port}")
    ...
    if start_mcp_discovery_after_bind:
        ...
        try:
            from panergos_cli.mcp_startup import defer_background_mcp_discovery
...
def start_server(
    ...
    JSON-RPC/WS backend, no UI build, no SPA mount (``PANERGOS_SERVE_HEADLESS``).
    ...
    from panergos_cli.resource_limits import apply_nofile_soft_limit
...
_PLUGIN_COMPAT_LAZY = {
    'AudioTranscriptionRequest': ('panergos_cli.web_models', 'AudioTranscriptionRequest'),
    'AutomationBlueprintInstantiate': ('panergos_cli.web_models', 'AutomationBlueprintInstantiate'),
    'BackupRequest': ('panergos_cli.web_models', 'BackupRequest'),
    'BulkDeleteSessions': ('panergos_cli.web_models', 'BulkDeleteSessions'),
    'CONFIG_SCHEMA': ('panergos_cli.web_server_config', 'CONFIG_SCHEMA'),
    'ChatImageUpload': ('panergos_cli.web_models', 'ChatImageUpload'),
    'ConfigUpdate': ('panergos_cli.web_models', 'ConfigUpdate'),
    'CredentialPoolAdd': ('panergos_cli.web_models', 'CredentialPoolAdd'),
    'CronJobCreate': ('panergos_cli.web_models', 'CronJobCreate'),
    'CronJobUpdate': ('panergos_cli.web_models', 'CronJobUpdate'),
    'CuratorPause': ('panergos_cli.web_models', 'CuratorPause'),
    'CustomEndpointUpdate': ('panergos_cli.web_models', 'CustomEndpointUpdate'),
    'DEFAULT_CONFIG': ('panergos_cli.config', 'DEFAULT_CONFIG'),
    'DebugShareRequest': ('panergos_cli.web_models', 'DebugShareRequest'),
    'EnvVarDelete': ('panergos_cli.web_models', 'EnvVarDelete'),
    'EnvVarReveal': ('panergos_cli.web_models', 'EnvVarReveal'),
    'EnvVarUpdate': ('panergos_cli.web_models', 'EnvVarUpdate'),
    'FontSetBody': ('panergos_cli.web_models', 'FontSetBody'),
    'FsWriteText': ('panergos_cli.web_models', 'FsWriteText'),
    'GitBranchSwitchBody': ('panergos_cli.web_models', 'GitBranchSwitchBody'),
    'GitCommitBody': ('panergos_cli.web_models', 'GitCommitBody'),
    'GitFileBody': ('panergos_cli.web_models', 'GitFileBody'),
    'GitPathBody': ('panergos_cli.web_models', 'GitPathBody'),
    'GitWorktreeAddBody': ('panergos_cli.web_models', 'GitWorktreeAddBody'),
    'GitWorktreeRemoveBody': ('panergos_cli.web_models', 'GitWorktreeRemoveBody'),
    'HookCreate': ('panergos_cli.web_models', 'HookCreate'),
    'HookDelete': ('panergos_cli.web_models', 'HookDelete'),
    'ImportRequest': ('panergos_cli.web_models', 'ImportRequest'),
    'LearningNodeEdit': ('panergos_cli.web_models', 'LearningNodeEdit'),
    'LearningNodeRef': ('panergos_cli.web_models', 'LearningNodeRef'),
    'MCPCatalogInstall': ('panergos_cli.web_models', 'MCPCatalogInstall'),
    'MCPEnabledToggle': ('panergos_cli.web_models', 'MCPEnabledToggle'),
    'MCPServerCreate': ('panergos_cli.web_models', 'MCPServerCreate'),
    'MCPServersReplace': ('panergos_cli.web_models', 'MCPServersReplace'),
    'ManagedDirectoryCreate': ('panergos_cli.web_models', 'ManagedDirectoryCreate'),
    'ManagedFileDelete': ('panergos_cli.web_models', 'ManagedFileDelete'),
    'ManagedFileUpload': ('panergos_cli.web_models', 'ManagedFileUpload'),
    'ManagedFilesPolicy': ('panergos_cli.web_server_files', 'ManagedFilesPolicy'),
    'MemoryProviderConfigUpdate': ('panergos_cli.web_models', 'MemoryProviderConfigUpdate'),
    'MemoryProviderSelect': ('panergos_cli.web_models', 'MemoryProviderSelect'),
    'MemoryProviderSetupRequest': ('panergos_cli.web_models', 'MemoryProviderSetupRequest'),
    'MemoryReset': ('panergos_cli.web_models', 'MemoryReset'),
    'MessagingPlatformUpdate': ('panergos_cli.web_models', 'MessagingPlatformUpdate'),
    'MoaConfigPayload': ('panergos_cli.web_models', 'MoaConfigPayload'),
    'MoaModelSlot': ('panergos_cli.web_models', 'MoaModelSlot'),
    'MoaPresetPayload': ('panergos_cli.web_models', 'MoaPresetPayload'),
    'ModelAssignment': ('panergos_cli.web_models', 'ModelAssignment'),
    'OAuthSubmitBody': ('panergos_cli.web_models', 'OAuthSubmitBody'),
    'OPTIONAL_ENV_VARS': ('panergos_cli.config', 'OPTIONAL_ENV_VARS'),
    'PairingApprove': ('panergos_cli.web_models', 'PairingApprove'),
    'PairingRevoke': ('panergos_cli.web_models', 'PairingRevoke'),
    'ProfileActiveUpdate': ('panergos_cli.web_models', 'ProfileActiveUpdate'),
    'ProfileCreate': ('panergos_cli.web_models', 'ProfileCreate'),
    'ProfileDescribeAuto': ('panergos_cli.web_models', 'ProfileDescribeAuto'),
    'ProfileDescriptionUpdate': ('panergos_cli.web_models', 'ProfileDescriptionUpdate'),
    'ProfileModelUpdate': ('panergos_cli.web_models', 'ProfileModelUpdate'),
    'ProfileRename': ('panergos_cli.web_models', 'ProfileRename'),
    'ProfileSoulUpdate': ('panergos_cli.web_models', 'ProfileSoulUpdate'),
    ...
    'PtyBridge': ('panergos_cli.pty_bridge', 'PtyBridge'),
    'PtySessionRegistry': ('panergos_cli.pty_session', 'PtySessionRegistry'),
    'PtyUnavailableError': ('panergos_cli.pty_bridge', 'PtyUnavailableError'),
    'RawConfigUpdate': ('panergos_cli.web_models', 'RawConfigUpdate'),
    'RegistryFull': ('panergos_cli.pty_session', 'RegistryFull'),
    'STORAGE_HONCHO_HOST_BLOCK': ('plugins.memory.config_schema', 'STORAGE_HONCHO_HOST_BLOCK'),
    'SessionImport': ('panergos_cli.web_models', 'SessionImport'),
    'SessionPrune': ('panergos_cli.web_models', 'SessionPrune'),
    'SessionRename': ('panergos_cli.web_models', 'SessionRename'),
    'SkillContentUpdate': ('panergos_cli.web_models', 'SkillContentUpdate'),
    'SkillCreate': ('panergos_cli.web_models', 'SkillCreate'),
    'SkillInstallRequest': ('panergos_cli.web_models', 'SkillInstallRequest'),
    'SkillToggle': ('panergos_cli.web_models', 'SkillToggle'),
    'SkillUninstallRequest': ('panergos_cli.web_models', 'SkillUninstallRequest'),
    'SkillsUpdateRequest': ('panergos_cli.web_models', 'SkillsUpdateRequest'),
    'TTSLeaseRequest': ('panergos_cli.web_models', 'TTSLeaseRequest'),
    'TTSSpeakRequest': ('panergos_cli.web_models', 'TTSSpeakRequest'),
    'TelegramOnboardingApply': ('panergos_cli.web_models', 'TelegramOnboardingApply'),
    'TelegramOnboardingStart': ('panergos_cli.web_models', 'TelegramOnboardingStart'),
    'TerminalBackendSelect': ('panergos_cli.web_models', 'TerminalBackendSelect'),
    'ThemeSetBody': ('panergos_cli.web_models', 'ThemeSetBody'),
    'ToolsetEnvUpdate': ('panergos_cli.web_models', 'ToolsetEnvUpdate'),
    'ToolsetModelSelect': ('panergos_cli.web_models', 'ToolsetModelSelect'),
    'ToolsetPostSetup': ('panergos_cli.web_models', 'ToolsetPostSetup'),
    'ToolsetProviderSelect': ('panergos_cli.web_models', 'ToolsetProviderSelect'),
    'ToolsetToggle': ('panergos_cli.web_models', 'ToolsetToggle'),
    'WebhookCreate': ('panergos_cli.web_models', 'WebhookCreate'),
    'WebhookEnabledToggle': ('panergos_cli.web_models', 'WebhookEnabledToggle'),
    'WhatsAppOnboardingApply': ('panergos_cli.web_models', 'WhatsAppOnboardingApply'),
    'WhatsAppOnboardingStart': ('panergos_cli.web_models', 'WhatsAppOnboardingStart'),
    'activate_custom_endpoint': ('panergos_cli.web_routers.config_env', 'activate_custom_endpoint'),
    'add_credential_pool_entry': ('panergos_cli.web_routers.ops', 'add_credential_pool_entry'),
    'add_mcp_server': ('panergos_cli.web_routers.mcp', 'add_mcp_server'),
    'apply_telegram_onboarding': ('panergos_cli.web_routers.messaging', 'apply_telegram_onboarding'),
    'apply_whatsapp_onboarding': ('panergos_cli.web_routers.messaging', 'apply_whatsapp_onboarding'),
    'approve_pairing': ('panergos_cli.web_routers.ops', 'approve_pairing'),
    'auth_mcp_server': ('panergos_cli.web_routers.mcp', 'auth_mcp_server'),
    'build_cron_model_impact': ('panergos_cli.config', 'build_cron_model_impact'),
    'bulk_delete_sessions_endpoint': ('panergos_cli.web_routers.sessions', 'bulk_delete_sessions_endpoint'),
    'cancel_oauth_session': ('panergos_cli.web_routers.oauth', 'cancel_oauth_session'),
    'cancel_telegram_onboarding': ('panergos_cli.web_routers.messaging', 'cancel_telegram_onboarding'),
    'cancel_whatsapp_onboarding': ('panergos_cli.web_routers.messaging', 'cancel_whatsapp_onboarding'),
    'cfg_get': ('panergos_cli.config', 'cfg_get'),
    'check_config_version': ('panergos_cli.config', 'check_config_version'),
    'check_panergos_update': ('panergos_cli.web_routers.actions', 'check_panergos_update'),
    'clear_model_endpoint_credentials': ('panergos_cli.config', 'clear_model_endpoint_credentials'),
    'clear_pending_pairing': ('panergos_cli.web_routers.ops', 'clear_pending_pairing'),
    'coerce_provider_id': ('panergos_cli.config', 'coerce_provider_id'),
    'console_ws': ('panergos_cli.web_routers.chat_ws', 'console_ws'),
    'count_empty_sessions_endpoint': ('panergos_cli.web_routers.sessions', 'count_empty_sessions_endpoint'),
    'create_cron_job': ('panergos_cli.web_routers.cron', 'create_cron_job'),
    'create_hook': ('panergos_cli.web_routers.ops', 'create_hook'),
    'create_managed_directory': ('panergos_cli.web_routers.files', 'create_managed_directory'),
    'create_profile_endpoint': ('panergos_cli.web_routers.profiles', 'create_profile_endpoint'),
    'create_skill': ('panergos_cli.web_routers.skills', 'create_skill'),
    'create_webhook': ('panergos_cli.web_routers.ops', 'create_webhook'),
    'custom_endpoint_key_env': ('panergos_cli.config', 'custom_endpoint_key_env'),
    'delete_agent_plugin': ('panergos_cli.web_routers.dashboard_ui', 'delete_agent_plugin'),
    'delete_cron_job': ('panergos_cli.web_routers.cron', 'delete_cron_job'),
    'delete_custom_endpoint': ('panergos_cli.web_routers.config_env', 'delete_custom_endpoint'),
    'delete_empty_sessions_endpoint': ('panergos_cli.web_routers.sessions', 'delete_empty_sessions_endpoint'),
    'delete_hook': ('panergos_cli.web_routers.ops', 'delete_hook'),
    'delete_learning_node': ('panergos_cli.web_routers.status', 'delete_learning_node'),
    'delete_managed_file': ('panergos_cli.web_routers.files', 'delete_managed_file'),
    'delete_profile_endpoint': ('panergos_cli.web_routers.profiles', 'delete_profile_endpoint'),
    'delete_session_endpoint': ('panergos_cli.web_routers.sessions', 'delete_session_endpoint'),
    'delete_webhook': ('panergos_cli.web_routers.ops', 'delete_webhook'),
    'derive_gateway_busy': ('gateway.status', 'derive_gateway_busy'),
    'derive_gateway_drainable': ('gateway.status', 'derive_gateway_drainable'),
    'describe_profile_auto_endpoint': ('panergos_cli.web_routers.profiles', 'describe_profile_auto_endpoint'),
    'detect_install_method': ('panergos_cli.config', 'detect_install_method'),
    'disconnect_oauth_provider': ('panergos_cli.web_routers.oauth', 'disconnect_oauth_provider'),
    'download_dashboard_backup': ('panergos_cli.web_routers.ops', 'download_dashboard_backup'),
    'download_managed_file': ('panergos_cli.web_routers.files', 'download_managed_file'),
    'enable_webhooks': ('panergos_cli.web_routers.ops', 'enable_webhooks'),
    'env_var_enabled': ('utils', 'env_var_enabled'),
    'events_ws': ('panergos_cli.web_routers.chat_ws', 'events_ws'),
    'export_session_endpoint': ('panergos_cli.web_routers.sessions', 'export_session_endpoint'),
    'find_provider_entry': ('panergos_cli.config', 'find_provider_entry'),
    'format_docker_update_message': ('panergos_cli.config', 'format_docker_update_message'),
    'fs_default_cwd': ('panergos_cli.web_routers.files', 'fs_default_cwd'),
    'fs_download': ('panergos_cli.web_routers.files', 'fs_download'),
    'fs_git_root': ('panergos_cli.web_routers.files', 'fs_git_root'),
    'fs_list': ('panergos_cli.web_routers.files', 'fs_list'),
    'fs_read_data_url': ('panergos_cli.web_routers.files', 'fs_read_data_url'),
    'fs_read_text': ('panergos_cli.web_routers.files', 'fs_read_text'),
    'fs_write_text': ('panergos_cli.web_routers.files', 'fs_write_text'),
    'gateway_drain': ('panergos_cli.web_routers.actions', 'gateway_drain'),
    'gateway_ws': ('panergos_cli.web_routers.chat_ws', 'gateway_ws'),
    'get_action_status': ('panergos_cli.web_routers.actions', 'get_action_status'),
    'get_active_profile_endpoint': ('panergos_cli.web_routers.profiles', 'get_active_profile_endpoint'),
    'get_auxiliary_models': ('panergos_cli.web_routers.models', 'get_auxiliary_models'),
    'get_client_voice_config': ('panergos_cli.web_routers.audio', 'get_client_voice_config'),
    'get_computer_use_status': ('panergos_cli.web_routers.tools', 'get_computer_use_status'),
    'get_config': ('panergos_cli.web_routers.config_env', 'get_config'),
    'get_config_path': ('panergos_cli.config', 'get_config_path'),
    'get_config_raw': ('panergos_cli.web_routers.analytics', 'get_config_raw'),
    'get_cron_delivery_targets': ('panergos_cli.web_routers.cron', 'get_cron_delivery_targets'),
    'get_cron_job': ('panergos_cli.web_routers.cron', 'get_cron_job'),
    'get_curator_status': ('panergos_cli.web_routers.status', 'get_curator_status'),
    'get_dashboard_font': ('panergos_cli.web_routers.dashboard_ui', 'get_dashboard_font'),
    'get_dashboard_plugins': ('panergos_cli.web_routers.dashboard_ui', 'get_dashboard_plugins'),
    'get_dashboard_themes': ('panergos_cli.web_routers.dashboard_ui', 'get_dashboard_themes'),
    'get_defaults': ('panergos_cli.web_routers.config_env', 'get_defaults'),
    'get_egress_status': ('panergos_cli.web_routers.config_env', 'get_egress_status'),
    'get_elevenlabs_voices': ('panergos_cli.web_routers.audio', 'get_elevenlabs_voices'),
    'get_env_path': ('panergos_cli.config', 'get_env_path'),
    'get_env_vars': ('panergos_cli.web_routers.config_env', 'get_env_vars'),
    'get_health': ('panergos_cli.web_routers.status', 'get_health'),
    'get_panergos_home': ('panergos_cli.config', 'get_panergos_home'),
    'get_learning_graph': ('panergos_cli.web_routers.status', 'get_learning_graph'),
    'get_learning_node': ('panergos_cli.web_routers.status', 'get_learning_node'),
    'get_logs': ('panergos_cli.web_routers.status', 'get_logs'),
    'get_media': ('panergos_cli.web_routers.files', 'get_media'),
    'get_memory_provider_config': ('panergos_cli.web_routers.memory_providers', 'get_memory_provider_config'),
    'get_memory_status': ('panergos_cli.web_routers.ops', 'get_memory_status'),
    'get_messaging_platforms': ('panergos_cli.web_routers.messaging', 'get_messaging_platforms'),
    'get_moa_models': ('panergos_cli.web_routers.models', 'get_moa_models'),
    'get_model_info': ('panergos_cli.web_routers.models', 'get_model_info'),
    'get_model_options': ('panergos_cli.web_routers.models', 'get_model_options'),
    'get_models_analytics': ('panergos_cli.web_routers.analytics', 'get_models_analytics'),
    'get_plugins_hub': ('panergos_cli.web_routers.dashboard_ui', 'get_plugins_hub'),
    'get_portal_status': ('panergos_cli.web_routers.status', 'get_portal_status'),
    'get_process_panergos_home': ('panergos_cli.config', 'get_process_panergos_home'),
    'get_profile_setup_command': ('panergos_cli.web_routers.profiles', 'get_profile_setup_command'),
    'get_profile_soul': ('panergos_cli.web_routers.profiles', 'get_profile_soul'),
    'get_profiles_sessions': ('panergos_cli.web_routers.profiles', 'get_profiles_sessions'),
    'get_profiles_sessions_sidebar': ('panergos_cli.web_routers.profiles', 'get_profiles_sessions_sidebar'),
    'get_provider_config_schema': ('plugins.memory.config_schema', 'get_provider_config_schema'),
    'get_recommended_default_model': ('panergos_cli.web_routers.models', 'get_recommended_default_model'),
    ...
    'get_schema': ('panergos_cli.web_routers.config_env', 'get_schema'),
    'get_session_detail': ('panergos_cli.web_routers.sessions', 'get_session_detail'),
    'get_session_latest_descendant': ('panergos_cli.web_routers.sessions', 'get_session_latest_descendant'),
    'get_session_messages': ('panergos_cli.web_routers.sessions', 'get_session_messages'),
    'get_session_stats': ('panergos_cli.web_routers.sessions', 'get_session_stats'),
    'get_sessions': ('panergos_cli.web_routers.sessions', 'get_sessions'),
    'get_skill_content': ('panergos_cli.web_routers.skills', 'get_skill_content'),
    'get_skills': ('panergos_cli.web_routers.skills', 'get_skills'),
    'get_ssh_ownership': ('panergos_cli.web_routers.status', 'get_ssh_ownership'),
    'get_status': ('panergos_cli.web_routers.status', 'get_status'),
    'get_system_stats': ('panergos_cli.web_routers.status', 'get_system_stats'),
    'get_telegram_onboarding_status': ('panergos_cli.web_routers.messaging', 'get_telegram_onboarding_status'),
    'get_terminal_backends': ('panergos_cli.web_routers.tools', 'get_terminal_backends'),
    'get_toolset_config': ('panergos_cli.web_routers.tools', 'get_toolset_config'),
    'get_toolset_models': ('panergos_cli.web_routers.tools', 'get_toolset_models'),
    'get_toolsets': ('panergos_cli.web_routers.tools', 'get_toolsets'),
    'get_update_receipt': ('panergos_cli.web_routers.actions', 'get_update_receipt'),
    'get_usage_analytics': ('panergos_cli.web_routers.analytics', 'get_usage_analytics'),
    'get_whatsapp_onboarding_status': ('panergos_cli.web_routers.messaging', 'get_whatsapp_onboarding_status'),
    'git_base_branches_route': ('panergos_cli.web_routers.git', 'git_base_branches_route'),
    'git_branch_switch_route': ('panergos_cli.web_routers.git', 'git_branch_switch_route'),
    'git_branches_route': ('panergos_cli.web_routers.git', 'git_branches_route'),
    'git_commit_context_route': ('panergos_cli.web_routers.git', 'git_commit_context_route'),
    'git_commit_route': ('panergos_cli.web_routers.git', 'git_commit_route'),
    'git_create_pr_route': ('panergos_cli.web_routers.git', 'git_create_pr_route'),
    'git_file_diff_route': ('panergos_cli.web_routers.git', 'git_file_diff_route'),
    'git_push_route': ('panergos_cli.web_routers.git', 'git_push_route'),
    'git_rev_parse_route': ('panergos_cli.web_routers.git', 'git_rev_parse_route'),
    'git_revert_route': ('panergos_cli.web_routers.git', 'git_revert_route'),
    'git_review_diff_route': ('panergos_cli.web_routers.git', 'git_review_diff_route'),
    'git_review_list_route': ('panergos_cli.web_routers.git', 'git_review_list_route'),
    'git_ship_info_route': ('panergos_cli.web_routers.git', 'git_ship_info_route'),
    'git_stage_route': ('panergos_cli.web_routers.git', 'git_stage_route'),
    'git_status_route': ('panergos_cli.web_routers.git', 'git_status_route'),
    'git_unstage_route': ('panergos_cli.web_routers.git', 'git_unstage_route'),
    'git_worktree_add_route': ('panergos_cli.web_routers.git', 'git_worktree_add_route'),
    'git_worktree_remove_route': ('panergos_cli.web_routers.git', 'git_worktree_remove_route'),
    'git_worktrees_route': ('panergos_cli.web_routers.git', 'git_worktrees_route'),
    'grant_computer_use_permissions': ('panergos_cli.web_routers.tools', 'grant_computer_use_permissions'),
    'import_sessions_endpoint': ('panergos_cli.web_routers.sessions', 'import_sessions_endpoint'),
    'install_mcp_catalog_entry': ('panergos_cli.web_routers.mcp', 'install_mcp_catalog_entry'),
    'install_skill_hub': ('panergos_cli.web_routers.skills', 'install_skill_hub'),
    'instantiate_blueprint': ('panergos_cli.web_routers.cron', 'instantiate_blueprint'),
    'is_nix_install_method': ('panergos_cli.config', 'is_nix_install_method'),
    'list_checkpoints': ('panergos_cli.web_routers.ops', 'list_checkpoints'),
    'list_credential_pool': ('panergos_cli.web_routers.ops', 'list_credential_pool'),
    'list_cron_blueprints': ('panergos_cli.web_routers.cron', 'list_cron_blueprints'),
    'list_cron_job_runs': ('panergos_cli.web_routers.cron', 'list_cron_job_runs'),
    'list_cron_jobs': ('panergos_cli.web_routers.cron', 'list_cron_jobs'),
    'list_custom_endpoints': ('panergos_cli.web_routers.config_env', 'list_custom_endpoints'),
    'list_hooks': ('panergos_cli.web_routers.ops', 'list_hooks'),
    'list_managed_files': ('panergos_cli.web_routers.files', 'list_managed_files'),
    'list_mcp_catalog': ('panergos_cli.web_routers.mcp', 'list_mcp_catalog'),
    'list_mcp_servers': ('panergos_cli.web_routers.mcp', 'list_mcp_servers'),
    'list_oauth_providers': ('panergos_cli.web_routers.oauth', 'list_oauth_providers'),
    'list_pairing': ('panergos_cli.web_routers.ops', 'list_pairing'),
    'list_profiles_endpoint': ('panergos_cli.web_routers.profiles', 'list_profiles_endpoint'),
    'list_skills_hub_sources': ('panergos_cli.web_routers.skills', 'list_skills_hub_sources'),
    'list_webhooks': ('panergos_cli.web_routers.ops', 'list_webhooks'),
    'load_env': ('panergos_cli.config', 'load_env'),
    'mcp_oauth_callback': ('panergos_cli.web_routers.mcp', 'mcp_oauth_callback'),
    'mcp_oauth_flow_status': ('panergos_cli.web_routers.mcp', 'mcp_oauth_flow_status'),
    'normalize_updated_at': ('gateway.status', 'normalize_updated_at'),
    'open_profile_terminal_endpoint': ('panergos_cli.web_routers.profiles', 'open_profile_terminal_endpoint'),
    'parse_active_agents': ('gateway.status', 'parse_active_agents'),
    'pause_cron_job': ('panergos_cli.web_routers.cron', 'pause_cron_job'),
    'poll_oauth_session': ('panergos_cli.web_routers.oauth', 'poll_oauth_session'),
    'post_agent_plugin_disable': ('panergos_cli.web_routers.dashboard_ui', 'post_agent_plugin_disable'),
    'post_agent_plugin_enable': ('panergos_cli.web_routers.dashboard_ui', 'post_agent_plugin_enable'),
    'post_agent_plugin_install': ('panergos_cli.web_routers.dashboard_ui', 'post_agent_plugin_install'),
    'post_agent_plugin_update': ('panergos_cli.web_routers.dashboard_ui', 'post_agent_plugin_update'),
    'post_plugin_visibility': ('panergos_cli.web_routers.dashboard_ui', 'post_plugin_visibility'),
    'preview_skill_hub': ('panergos_cli.web_routers.skills', 'preview_skill_hub'),
    'prune_checkpoints': ('panergos_cli.web_routers.ops', 'prune_checkpoints'),
    'prune_sessions_endpoint': ('panergos_cli.web_routers.sessions', 'prune_sessions_endpoint'),
    'pty_ws': ('panergos_cli.web_routers.chat_ws', 'pty_ws'),
    'pub_ws': ('panergos_cli.web_routers.chat_ws', 'pub_ws'),
    'put_plugin_providers': ('panergos_cli.web_routers.dashboard_ui', 'put_plugin_providers'),
    'read_managed_file': ('panergos_cli.web_routers.files', 'read_managed_file'),
    'read_raw_config': ('panergos_cli.config', 'read_raw_config'),
    'read_runtime_status': ('gateway.status', 'read_runtime_status'),
    'recommended_update_command_for_method': ('panergos_cli.config', 'recommended_update_command_for_method'),
    'redact_key': ('panergos_cli.config', 'redact_key'),
    'remove_credential_pool_entry': ('panergos_cli.web_routers.ops', 'remove_credential_pool_entry'),
    'remove_env_value': ('panergos_cli.config', 'remove_env_value'),
    'remove_env_var': ('panergos_cli.web_routers.config_env', 'remove_env_var'),
    'remove_mcp_server': ('panergos_cli.web_routers.mcp', 'remove_mcp_server'),
    'rename_profile_endpoint': ('panergos_cli.web_routers.profiles', 'rename_profile_endpoint'),
    'rename_session_endpoint': ('panergos_cli.web_routers.sessions', 'rename_session_endpoint'),
    'replace_mcp_servers': ('panergos_cli.web_routers.mcp', 'replace_mcp_servers'),
    'rescan_dashboard_plugins': ('panergos_cli.web_routers.dashboard_ui', 'rescan_dashboard_plugins'),
    'reset_memory': ('panergos_cli.web_routers.ops', 'reset_memory'),
    'resolve_cron_model_drift_defaults': ('panergos_cli.config', 'resolve_cron_model_drift_defaults'),
    'resolve_gateway_liveness': ('gateway.status', 'resolve_gateway_liveness'),
    'restart_gateway': ('panergos_cli.web_routers.actions', 'restart_gateway'),
    'resume_cron_job': ('panergos_cli.web_routers.cron', 'resume_cron_job'),
    'reveal_env_var': ('panergos_cli.web_routers.config_env', 'reveal_env_var'),
    'revoke_pairing': ('panergos_cli.web_routers.ops', 'revoke_pairing'),
    'run_backup': ('panergos_cli.web_routers.ops', 'run_backup'),
    'run_config_migrate': ('panergos_cli.web_routers.status', 'run_config_migrate'),
    'run_curator': ('panergos_cli.web_routers.status', 'run_curator'),
    'run_debug_share_endpoint': ('panergos_cli.web_routers.status', 'run_debug_share_endpoint'),
    'run_doctor': ('panergos_cli.doctor', 'run_doctor'),
    'run_dump': ('panergos_cli.dump', 'run_dump'),
    'run_import': ('panergos_cli.web_routers.ops', 'run_import'),
    'run_import_upload': ('panergos_cli.web_routers.ops', 'run_import_upload'),
    'run_prompt_size': ('panergos_cli.web_routers.status', 'run_prompt_size'),
    'run_security_audit': ('panergos_cli.web_routers.ops', 'run_security_audit'),
    'run_toolset_post_setup': ('panergos_cli.web_routers.tools', 'run_toolset_post_setup'),
    'save_config': ('panergos_cli.config', 'save_config'),
    'save_env_value': ('panergos_cli.config', 'save_env_value'),
    'save_toolset_env': ('panergos_cli.web_routers.tools', 'save_toolset_env'),
    'scan_skill_hub': ('panergos_cli.web_routers.skills', 'scan_skill_hub'),
    'search_sessions': ('panergos_cli.web_routers.sessions', 'search_sessions'),
    'search_skills_hub': ('panergos_cli.web_routers.skills', 'search_skills_hub'),
    'select_terminal_backend': ('panergos_cli.web_routers.tools', 'select_terminal_backend'),
    'select_toolset_model': ('panergos_cli.web_routers.tools', 'select_toolset_model'),
    'select_toolset_provider': ('panergos_cli.web_routers.tools', 'select_toolset_provider'),
    'serve_plugin_asset': ('panergos_cli.web_routers.dashboard_ui', 'serve_plugin_asset'),
    'set_active_profile_endpoint': ('panergos_cli.web_routers.profiles', 'set_active_profile_endpoint'),
    'set_curator_paused': ('panergos_cli.web_routers.status', 'set_curator_paused'),
    'set_dashboard_font': ('panergos_cli.web_routers.dashboard_ui', 'set_dashboard_font'),
    'set_dashboard_theme': ('panergos_cli.web_routers.dashboard_ui', 'set_dashboard_theme'),
    'set_env_var': ('panergos_cli.web_routers.config_env', 'set_env_var'),
    'set_mcp_server_enabled': ('panergos_cli.web_routers.mcp', 'set_mcp_server_enabled'),
    'set_memory_provider': ('panergos_cli.web_routers.ops', 'set_memory_provider'),
    'set_moa_models': ('panergos_cli.web_routers.models', 'set_moa_models'),
    'set_model_assignment': ('panergos_cli.web_routers.models', 'set_model_assignment'),
    'set_webhook_enabled': ('panergos_cli.web_routers.ops', 'set_webhook_enabled'),
    'setup_memory_provider': ('panergos_cli.web_routers.memory_providers', 'setup_memory_provider'),
    'speak_stream_ws': ('panergos_cli.web_routers.audio', 'speak_stream_ws'),
    'speak_text': ('panergos_cli.web_routers.audio', 'speak_text'),
    'start_gateway': ('panergos_cli.web_routers.ops', 'start_gateway'),
    'start_oauth_login': ('panergos_cli.web_routers.oauth', 'start_oauth_login'),
    'start_telegram_onboarding': ('panergos_cli.web_routers.messaging', 'start_telegram_onboarding'),
    'start_whatsapp_onboarding': ('panergos_cli.web_routers.messaging', 'start_whatsapp_onboarding'),
    'stop_gateway': ('panergos_cli.web_routers.ops', 'stop_gateway'),
    'stream_managed_file': ('panergos_cli.web_routers.files', 'stream_managed_file'),
    'submit_oauth_code': ('panergos_cli.web_routers.oauth', 'submit_oauth_code'),
    'test_mcp_server': ('panergos_cli.web_routers.mcp', 'test_mcp_server'),
    'test_messaging_platform': ('panergos_cli.web_routers.messaging', 'test_messaging_platform'),
    'toggle_skill': ('panergos_cli.web_routers.skills', 'toggle_skill'),
    'toggle_toolset': ('panergos_cli.web_routers.tools', 'toggle_toolset'),
    'transcribe_audio_upload': ('panergos_cli.web_routers.audio', 'transcribe_audio_upload'),
    'trigger_cron_job': ('panergos_cli.web_routers.cron', 'trigger_cron_job'),
    'tts_lease': ('panergos_cli.web_routers.audio', 'tts_lease'),
    'uninstall_skill_hub': ('panergos_cli.web_routers.skills', 'uninstall_skill_hub'),
    'update_config': ('panergos_cli.web_routers.config_env', 'update_config'),
    'update_config_raw': ('panergos_cli.web_routers.analytics', 'update_config_raw'),
    'update_cron_job': ('panergos_cli.web_routers.cron', 'update_cron_job'),
    'update_panergos': ('panergos_cli.web_routers.actions', 'update_panergos'),
    'update_learning_node': ('panergos_cli.web_routers.status', 'update_learning_node'),
    'update_memory_provider_config': ('panergos_cli.web_routers.memory_providers', 'update_memory_provider_config'),
    'update_messaging_platform': ('panergos_cli.web_routers.messaging', 'update_messaging_platform'),
    'update_profile_description_endpoint': ('panergos_cli.web_routers.profiles', 'update_profile_description_endpoint'),
    'update_profile_model_endpoint': ('panergos_cli.web_routers.profiles', 'update_profile_model_endpoint'),
    'update_profile_soul': ('panergos_cli.web_routers.profiles', 'update_profile_soul'),
    'update_skill_content': ('panergos_cli.web_routers.skills', 'update_skill_content'),
    'update_skills_hub': ('panergos_cli.web_routers.skills', 'update_skills_hub'),
    'upload_chat_image': ('panergos_cli.web_routers.files', 'upload_chat_image'),
    'upload_managed_file': ('panergos_cli.web_routers.files', 'upload_managed_file'),
    'upload_managed_file_stream': ('panergos_cli.web_routers.files', 'upload_managed_file_stream'),
    'upsert_custom_endpoint': ('panergos_cli.web_routers.config_env', 'upsert_custom_endpoint'),
    'validate_custom_endpoint': ('panergos_cli.web_routers.config_env', 'validate_custom_endpoint'),
    'validate_provider_credential': ('panergos_cli.web_routers.config_env', 'validate_provider_credential'),
    'windows_detach_flags': ('panergos_cli._subprocess_compat', 'windows_detach_flags'),
    'windows_hide_flags': ('panergos_cli._subprocess_compat', 'windows_hide_flags'),
    'write_platform_config_field': ('panergos_cli.config', 'write_platform_config_field'),
}
