"""Dashboard process-hygiene helpers — extracted from ``panergos_cli/main.py``.

Helpers defined in ``panergos_cli.main_dashboard`` / ``panergos_cli.main_install_repair`` are imported at
...
def _iter_process_table() -> list[tuple[int, str]]:
    ...
    if sys.platform == "win32":
        ...
        return rows
    # ps, not `pgrep -f "panergos.*dashboard"` (greedy regex; consistent with gateway pid scan).
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    ``panergos update`` (every API call 401s). *exclude_pids* (Desktop's PANERGOS_DESKTOP_CHILD_PID
    backends) are never returned.

    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the Panergos
    Desktop Electron app to protect its own backend child process: when the desktop spawns ``panergos serve``
    as a backend and triggers an auto-update, the update must not kill the backend that the desktop itself
    manages. The desktop sets the environment variable ``PANERGOS_DESKTOP_CHILD_PID`` on the spawned backend
    ...
    with contextlib.suppress(Exception):
        ...
        from panergos_cli.process_identity import ledger_entries
...
def _panergos_home_for_pid(pid: int) -> str | None:
    """Best-effort ``PANERGOS_HOME`` from *pid*'s environment (psutil, then /proc)."""
    with contextlib.suppress(Exception):
        import psutil
        if home := psutil.Process(pid).environ().get("PANERGOS_HOME"):
            return home
    try:
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"PANERGOS_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    ``panergos update`` multiplies listening backends because ``--port 0`` binds a fresh port.
...
def _profile_key_for_respawn(argv: list[str], panergos_home: str | None = None) -> str:
    """Stable owner key: ``PANERGOS_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    """
    if panergos_home:
        parts = _resolved_home(panergos_home).parts
        if len(parts) >= 2 and parts[-2] == "profiles" and parts[-1]:
            return f"profile:{parts[-1]}"
        return f"home:{_normalized_home_for_compare(panergos_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends ``(pid, argv, panergos_home)`` to respawn after update.

    Rules: never resurrect Desktop ``--port 0`` backends; never replay a backend from a
    **foreign** ``PANERGOS_HOME`` (the argv-only respawn would come back on this install's home
    ...
    (``PANERGOS_DESKTOP_CHILD_PID``) owns their lifecycle. These are also the PPID-1 orphans that previously
    ...
    Intentionally does **not** blanket-skip every PPID-1 process: a prior ``panergos update`` respawn detaches
    ...
    if own_home is None:
        try:
            ...
            own_home = str(get_panergos_home())
        except Exception:
            own_home = ""
    ...
    for _pid, argv, panergos_home in candidates:
        if not argv or _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and panergos_home and _normalized_home_for_compare(panergos_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        profile_key = _profile_key_for_respawn(argv, panergos_home)
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (``PANERGOS_DESKTOP_CHILD_PID``, comma-separated)."""
    out: set[int] = set()
    for part in os.environ.get("PANERGOS_DESKTOP_CHILD_PID", "").split(","):
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``panergos dashboard`` / ``panergos serve`` processes (update end, ``--stop``).
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    e.g. ``panergos update``'s systemd fleet-restart loop, which restarts ``panergos-serve*`` units before this
    ...
    """
    from panergos_cli import main_dashboard as _dash

    if restart_managed and _dash._restart_managed_dashboard_service(reason):
        # The dashboard unit is handled but other backends (e.g. panergos-serve.service) are not:
        # mark the unit handled so the filter below drops its PIDs, and keep going.
        _dash_unit = getattr(_dash, "_DASHBOARD_SYSTEMD_UNIT", "panergos-dashboard.service")
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid] and (cmdline := _dash._dashboard_cmdline_for_pid(pid)):
                # Manual process: exact argv + PANERGOS_HOME for the respawn and its profile cap.
                # Manually-started process: preserve its exact argv so we can respawn it after the update
                # (#40449, #68934). Snapshot PANERGOS_HOME before the kill so per-profile caps still work
                # after the process is gone (#78821).
                pid_cmdline[pid] = cmdline
                pid_home[pid] = _panergos_home_for_pid(pid)
...
def _restart_killed_backends(
    ...
    # Two categories: Without this, a remote backend (panergos serve) under Restart=on-failure never comes
...
def _detect_concurrent_panergos_instances(
    ...
    Windows blocks DELETE/REPLACE on a running .exe, so a Desktop-spawned ``panergos.EXE`` makes
    ...
    *shim* ancestor (the setuptools launcher is a separate native process from its
    ...
    """
    from panergos_cli.main_install_repair import _panergos_exe_shims, _is_windows
    ...
    try:
        ...
        return []
    shim_paths = {_norm_exe(shim) for shim in _panergos_exe_shims(scripts_dir)}
    if not shim_paths:
        return []
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    """True for the Desktop-local shape ``panergos serve [--isolated] --host 127.0.0.1 --port 0``.
    ...
    # ``vim notes about panergos serve`` contains both markers — this predicate decides a kill.
    if _panergos_holder_subcommand(command) != "serve":
        return False
...
# (apps/desktop/electron/remote-lifecycle.ts) for every ``panergos serve`` it spawns. Such a backend
...
def _panergos_home_dir() -> Path:
    """The process's Panergos home: remote-backend locks are a process-level asset, so a request scoped
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    if any(not isinstance(parsed.get(f), str) or len(parsed[f]) > 1024
           for f in ("profile", "panergosPath", "panergosHome", "logPath", "startedAt")):
        return False
    # Suffix-only check of logPath so a relocated PANERGOS_HOME can't reject a legitimate backend.
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    """PIDs claimed by valid ``{panergos_home}/desktop-ssh/<ownershipId>/backend.lock.json`` records
    (best-effort: a bad record contributes no PID; never raises)."""
    import json
    root = base_dir if base_dir is not None else _panergos_home_dir() / _REMOTE_LOCK_SUBDIR
    owned: set[int] = set()
    try:
...
def _reap_orphaned_desktop_local_serves(
    *, reason: str = "orphaned desktop-local panergos serve", signal_term=None, signal_kill=None,
    sleep_fn=None, lock_owned_pids_fn=None, process_age_seconds_fn=None) -> dict[str, list]:
    """Kill leftover Desktop-local ``panergos serve`` backends with no parent. Never raises.
    ...
    0/1; not self / parent / PANERGOS_DESKTOP_CHILD_PID; not claimed by a valid
    ...
    after PANERGOS_BACKEND_READY, so a live sibling mid-startup is briefly unowned).
    """
    import signal as _signal
    import time as _time
