"""``panergos dashboard`` / ``panergos serve`` subcommand parsers.
...
def _add_server_runtime_args(parser) -> None:
    ...
    # manager / PID file: they scan the process table for `panergos dashboard|serve`
    # cmdlines and SIGTERM them — the same path `panergos update` uses.
    parser.add_argument(
        "--stop", action="store_true", help="Stop all running Panergos web server processes and exit")
    parser.add_argument(
        "--status", action="store_true", help="List running Panergos web server processes and exit")
...
def build_serve_parser(
    *, cmd_dashboard: Callable, add_help: bool = True, exit_on_error: bool = True,
    ...
    parser = argparse.ArgumentParser(
        prog="panergos serve",
        description="Run the Panergos backend server - the JSON-RPC/WebSocket gateway the "
...
def build_dashboard_parser(subparsers, *, cmd_dashboard: Callable) -> None:
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Panergos Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically")
    # Compat shim: desktop shells <= 0.15.x spawn `panergos dashboard --no-open --tui ...`;
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Panergos backend server (headless; powers the desktop app and remote backends)",
        description="Run the Panergos backend server — the JSON-RPC/WebSocket gateway the "
            "desktop app and remote clients connect to. Headless: it never opens "
            "a browser UI.")
