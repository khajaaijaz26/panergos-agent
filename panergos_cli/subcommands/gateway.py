"""``panergos gateway``, ``connect``, and ``proxy`` subcommand parsers."""
...
from panergos_cli.subcommands._shared import add_accept_hooks_flag
...
def _cmd_connect(args) -> None:
    from panergos_cli.connectors import connect_command
...
def build_gateway_parser(
    ...
    """Attach the ``gateway``, ``connect``, and ``proxy`` subcommands to ``subparsers``."""
    ...
    _flag(gateway_run, "--no-supervise",
        help="Inside the s6-overlay Docker image, normally `gateway run` is "
            ...
            "if PANERGOS_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy", help="Remove legacy gateway services from pre-rename installs",
        description="Stop, disable, and remove legacy gateway unit files left over from older installs. "
            "Profile units and unrelated third-party services are never touched.")
    ...
    gateway_enroll = gateway_subparsers.add_parser("enroll",
        ...
        description="Redeem a single-use enrollment token with a relay connector. "
            "Authenticates through the configured OIDC provider, mints this gateway's per-gateway secret "
            "and per-tenant delivery key, and writes GATEWAY_RELAY_ID / "
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.panergos/.env. "
            "Requires being logged in (panergos setup). Not available in managed installs.")
    ...
    gateway_enroll.add_argument("--wake-url", dest="wake_url", default=None,
        help="Phase 5 §5.2 wake URL: a reachable URL the connector pokes "
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.panergos/.env and forwarded at provision. "
    ...
    connect_parser = subparsers.add_parser(
        "connect", help="List, configure, and check messaging connectors and provider accounts",
        description="One entry point for platform connectors; existing gateway and auth setup flows do the work.")
    connect_parser.add_argument(
        "connect_action", nargs="?", default="list",
        help="list, status, setup, auth, or a connector name (for example: telegram)")
    connect_parser.add_argument(
        "connector", nargs="?", help="Connector name, or provider id after 'auth'")
    _flag(connect_parser, "--json", help="Print a secret-free JSON status document")
    connect_parser.add_argument(
        "--type", dest="auth_type", choices=["oauth", "api-key", "api_key"],
        help="Account credential type (with 'connect auth PROVIDER')")
    connect_parser.add_argument("--label", help="Optional account label")
    _flag(connect_parser, "--no-browser", help="Do not open a browser during account OAuth")
    ...
    proxy_start.add_argument("--provider", default="xai",
        help="Upstream OAuth provider (default: xai). See `panergos proxy providers`.")
