"""``panergos debug`` subcommand parser."""
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Panergos Agent. Use 'panergos debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    panergos debug share              Upload debug report (asks for confirmation)
    panergos debug share --yes        Skip confirmation (for scripts/CI)
    panergos debug share --lines 500  Include more log lines
    panergos debug share --expire 30  Keep dpaste.com fallback pastes for 30 days
    panergos debug share --local      Print report locally (no upload)
    panergos debug share --no-redact  Disable upload-time secret redaction
    panergos debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'panergos debug share'")
