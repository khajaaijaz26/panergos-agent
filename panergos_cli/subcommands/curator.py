"""``panergos curator`` subcommand parser."""
...
def build_curator_parser(subparsers) -> None:
    ...
    try:
        from panergos_cli.curator import register_cli as _register_curator_cli

        _register_curator_cli(curator_parser)
    except Exception as _exc:
        logging.getLogger("panergos_cli.main").debug("curator CLI wiring failed: %s", _exc)
