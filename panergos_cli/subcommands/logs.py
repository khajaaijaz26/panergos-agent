"""``panergos logs`` subcommand parser."""
...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter Panergos log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    panergos logs                    Show last 50 lines of agent.log
    panergos logs -f                 Follow agent.log in real time
    panergos logs errors             Show last 50 lines of errors.log
    panergos logs gateway -n 100     Show last 100 lines of gateway.log
    panergos logs gui -f             Follow gui.log in real time
    panergos logs desktop -f         Follow desktop.log (Electron app boot/backend)
    panergos logs --level WARNING    Only show WARNING and above
    panergos logs --session abc123   Filter by session ID
    panergos logs --component tools  Only show tool-related lines
    panergos logs --since 1h         Lines from the last hour
    panergos logs --since 30m -f     Follow, starting from 30 min ago
    panergos logs list               List available log files with sizes
""")
