"""``panergos pause`` / ``panergos resume`` — the global emergency stop.

``pause`` writes the ESTOP sentinel at ``$PANERGOS_HOME/ESTOP``; cron, kanban and new gateway
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Panergos paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `panergos resume` to lift the pause.")
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Panergos resumed — dispatch picks up on the next tick.")
    else:
        print(f"Panergos is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description="Engage the global emergency stop. Halts NEW work only — cron "
            ...
            "`panergos resume`. In-flight work is never killed.")
    pause_parser.add_argument(
        "--reason", default=None, help="Optional reason stored in the sentinel and shown to users")
    pause_parser.set_defaults(func=cmd_pause)

    resume_parser = subparsers.add_parser(
        "resume", help="Lift the emergency stop set by `panergos pause`",
