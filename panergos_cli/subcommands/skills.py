"""``panergos skills`` subcommand parser."""

from __future__ import annotations

from typing import Callable

from panergos_cli.subcommands._shared import add_json_flag, add_yes_flag
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_trust = skills_subparsers.add_parser("trust",
        help="Trust a project so its repo-local skills (./.panergos/skills, ./.agents/skills) load")
    ...
    skills_reset = skills_subparsers.add_parser("reset",
        ...
        description="Clear a bundled skill's entry from the sync manifest (~/.panergos/skills/.bundled_manifest) "
            "so future 'panergos update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified", help="List bundled skills you've edited (which `panergos update` keeps)",
        description="Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `panergos update` reports as user-modified and skips. "
            "Use `panergos skills diff <name>` to see changes and `panergos skills reset "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description="Write the .no-bundled-skills marker so the installer, "
            "`panergos update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description="Remove the .no-bundled-skills marker so bundled skills are seeded "
            "again on the next `panergos update`. Pass --sync to re-seed now.")
