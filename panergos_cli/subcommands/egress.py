"""``panergos egress`` subcommand parser."""
...
def build_egress_parser(subparsers) -> None:
    ...
    # OUTBOUND egress firewall (iron-proxy); `panergos proxy` is the separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description="Manage iron-proxy, the optional TLS-intercepting egress firewall "
            ...
            "https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/egress/iron-proxy")

    from panergos_cli import proxy_cli as _proxy_cli
    _proxy_cli.register_cli(egress_parser)

    def _dispatch_egress(args):  # noqa: ANN001
        # dest='egress_command' stays disjoint from ``panergos proxy`` (dest='proxy_command').
