"""Single source of truth for provider identity in Panergos Agent."""
...
# -- Panergos overlay: metadata models.dev doesn't provide ----------------------

@dataclass(frozen=True)
class PanergosOverlay:
    """Panergos-specific provider metadata layered on top of models.dev."""
...
PANERGOS_OVERLAYS: Dict[str, PanergosOverlay] = {
    "moa": PanergosOverlay(auth_type="virtual", base_url_override="moa://local"),
    "openrouter": PanergosOverlay(is_aggregator=True, base_url_env_var="OPENROUTER_BASE_URL"),
    "openai-codex": PanergosOverlay(transport="codex_responses", auth_type="oauth_external",
                                  base_url_override="https://chatgpt.com/backend-api/codex"),
    "openai-api": PanergosOverlay(transport="codex_responses", base_url_override="https://api.openai.com/v1",
                                base_url_env_var="OPENAI_BASE_URL"),
    "xai-oauth": PanergosOverlay(transport="codex_responses", auth_type="oauth_external",
                               base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "qwen-oauth": PanergosOverlay(auth_type="oauth_external", base_url_override="https://portal.qwen.ai/v1",
                                base_url_env_var="PANERGOS_QWEN_BASE_URL"),
    "lmstudio": PanergosOverlay(extra_env_vars=("LM_API_KEY",), base_url_override="http://127.0.0.1:1234/v1",
                              base_url_env_var="LM_BASE_URL"),
    "copilot-acp": PanergosOverlay(transport="codex_responses", auth_type="external_process",
                                 base_url_override="acp://copilot", base_url_env_var="COPILOT_ACP_BASE_URL"),
    "github-copilot": PanergosOverlay(extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN")),
    "anthropic": PanergosOverlay(transport="anthropic_messages", extra_env_vars=("ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN")),
    "zai": PanergosOverlay(extra_env_vars=("GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY"), base_url_env_var="GLM_BASE_URL"),
    "kimi-for-coding": PanergosOverlay(base_url_env_var="KIMI_BASE_URL"),
    "stepfun": PanergosOverlay(extra_env_vars=("STEPFUN_API_KEY",),
                             base_url_override="https://api.stepfun.ai/step_plan/v1",
                             base_url_env_var="STEPFUN_BASE_URL"),
    "minimax": PanergosOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_BASE_URL"),
    "minimax-oauth": PanergosOverlay(transport="anthropic_messages", auth_type="oauth_external",
                                   base_url_override="https://api.minimax.io/anthropic"),
    "minimax-cn": PanergosOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_CN_BASE_URL"),
    "deepseek": PanergosOverlay(base_url_env_var="DEEPSEEK_BASE_URL"),
    "alibaba": PanergosOverlay(base_url_env_var="DASHSCOPE_BASE_URL"),
    "alibaba-coding-plan": PanergosOverlay(base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL"),
    "vercel": PanergosOverlay(is_aggregator=True),
    "opencode": PanergosOverlay(is_aggregator=True, base_url_env_var="OPENCODE_ZEN_BASE_URL"),
    "opencode-go": PanergosOverlay(is_aggregator=True, base_url_env_var="OPENCODE_GO_BASE_URL"),
    "opencode-free": PanergosOverlay(is_aggregator=True, base_url_override="https://opencode.ai/zen/v1", keyless=True),
    "kilo": PanergosOverlay(is_aggregator=True, base_url_env_var="KILOCODE_BASE_URL"),
    "huggingface": PanergosOverlay(is_aggregator=True, base_url_env_var="HF_BASE_URL"),
    "novita": PanergosOverlay(is_aggregator=True, base_url_env_var="NOVITA_BASE_URL"),
    "xai": PanergosOverlay(transport="codex_responses", base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "nvidia": PanergosOverlay(base_url_override="https://integrate.api.nvidia.com/v1", base_url_env_var="NVIDIA_BASE_URL"),
    "xiaomi": PanergosOverlay(base_url_env_var="XIAOMI_BASE_URL"),
    "tencent-tokenhub": PanergosOverlay(base_url_env_var="TOKENHUB_BASE_URL"),
    "tencent-tokenplan": PanergosOverlay(transport="anthropic_messages",
                                       base_url_override="https://api.lkeap.cloud.tencent.com/plan/anthropic",
                                       base_url_env_var="TOKENPLAN_BASE_URL"),
    "arcee": PanergosOverlay(base_url_override="https://api.arcee.ai/api/v1", base_url_env_var="ARCEE_BASE_URL"),
    "gmi": PanergosOverlay(extra_env_vars=("GMI_API_KEY",), base_url_override="https://api.gmi-serving.com/v1",
                         base_url_env_var="GMI_BASE_URL"),
    "fireworks": PanergosOverlay(extra_env_vars=("FIREWORKS_API_KEY",),
                               base_url_override="https://api.fireworks.ai/inference/v1"),
    "actual": PanergosOverlay(transport="chat_completions", extra_env_vars=("ACTUAL_API_KEY",),
                            base_url_override="https://api.actual.inc/v1", base_url_env_var="ACTUAL_BASE_URL"),
    "upstage": PanergosOverlay(extra_env_vars=("UPSTAGE_API_KEY",), base_url_override="https://api.upstage.ai/v1",
                             base_url_env_var="UPSTAGE_BASE_URL"),
    "nebius-token-factory": PanergosOverlay(extra_env_vars=("NEBIUS_API_KEY", "NEBIUS_TOKEN_FACTORY_API_KEY"),
                                          base_url_override="https://api.tokenfactory.nebius.com/v1",
                                          base_url_env_var="NEBIUS_BASE_URL"),
    "ollama-cloud": PanergosOverlay(base_url_override="https://ollama.com/v1", base_url_env_var="OLLAMA_BASE_URL"),
    # Azure Foundry serves OpenAI- and Anthropic-style endpoints; transport comes from model.api_mode.
    "azure-foundry": PanergosOverlay(base_url_env_var="AZURE_FOUNDRY_BASE_URL"),
    "bedrock": PanergosOverlay(transport="bedrock_converse", auth_type="aws_sdk"),
    ...
    "vertex": PanergosOverlay(auth_type="vertex"),
}
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "panergos", "user-config"
...
def _overlay_pdef(canonical, ov: PanergosOverlay, name, env_vars, base_url, doc, source) -> ProviderDef:
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    """Look up a built-in provider by id or alias: models.dev catalog merged with the Panergos overlay;
    Panergos-only overlays (openai-codex, etc.); plugin provider profiles with a concrete endpoint."""
    canonical = normalize_provider(name)
    mdev_info = _models_dev_info(canonical, allow_network)
    overlay = PANERGOS_OVERLAYS.get(canonical)
    if mdev_info is not None:
        ov = overlay or PanergosOverlay()
    ...
    if overlay is not None:
        return _overlay_pdef(canonical, overlay, _LABEL_OVERRIDES.get(canonical, canonical), overlay.extra_env_vars,
                             overlay.base_url_override, "", "panergos")
    # Plugin-registered profiles (plugins/model-providers/<name>/) absent from models.dev and
    # PANERGOS_OVERLAYS would otherwise be "Unknown provider" in /model, --provider and model-switch
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    """API mode for a provider/endpoint: host-mandated mode, known provider transport,
    then bedrock, else ``chat_completions``."""
...
def _lossy_alias_registry_pdef(raw: str, canonical: str) -> Optional[ProviderDef]:
    """Exact Panergos registry ids win over LOSSY alias collapsing (kimi-coding-cn must stay distinct
    ...
    try:
        from panergos_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
...
def _llamacpp_pdef() -> Optional[ProviderDef]:
    ...
    try:
        from panergos_cli.local_runtime.endpoint import resolve_llamacpp_endpoint
