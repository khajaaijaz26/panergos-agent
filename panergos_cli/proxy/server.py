...
from panergos_cli.proxy.sse_done import DONE_SSE_FRAME, SseDoneTracker, content_type_is_sse
...
def _require_aiohttp() -> None:
    if not AIOHTTP_AVAILABLE:
        raise RuntimeError("aiohttp is required for `panergos proxy`. Run `panergos setup` to install it.")
...
def create_app(adapter: UpstreamAdapter) -> "web.Application":
    ...
    Every adapter method is synchronous and blocking (adapters may take a cross-process
    ...
    async def handle_proxy(request: "web.Request") -> "web.StreamResponse":
        ...
        if upstream_resp.status in {401, 429}:
            # One-shot retry with a refreshed or rotated credential (some providers refresh
