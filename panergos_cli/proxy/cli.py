"""CLI handlers for the ``panergos proxy`` subcommand."""
...
from panergos_cli.proxy.adapters import ADAPTERS, get_adapter
from panergos_cli.proxy.server import (
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not AIOHTTP_AVAILABLE:
        _err("panergos proxy requires aiohttp. Run `panergos setup` to install it.")
        return 1
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"panergos auth add {adapter.name}")
        _err(f"Not logged into {adapter.display_name}. Run `{auth_hint}` first.")
        return 2
    host = getattr(args, "host", None) or DEFAULT_HOST
    port = getattr(args, "port", None) or DEFAULT_PORT
    _err(
        f"Starting panergos proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("panergos proxy upstream adapters\n")
    ...
    print("\nStart the proxy with: panergos proxy start [--provider <name>]")
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``panergos proxy <subcommand>``; no/unknown subcommand prints the short help."""
    handler = _SUBCOMMANDS.get(getattr(args, "proxy_command", None))
    if handler is not None:
        return handler(args)
    _err(
        "panergos proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  panergos proxy start [--provider xai] [--host 127.0.0.1] [--port 8645]\n"
