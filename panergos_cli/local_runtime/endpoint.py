...
def _state_endpoint() -> dict | None:
    ...
    if not base_url:
        return None
    # Ownership proof: on the stable port a SECOND install (different PANERGOS_HOME) can own
...
def _boot_in_flight(config: dict | None) -> bool:
    ...
    with suppress(Exception):
        ...
        if not ((config or {}).get("local_runtime") or {}).get("enabled"):
            return False
        from panergos_cli.local_runtime.binaries import manifest_verified, runtimes_root
