...
guess (``panergos -p coder ...`` loads coder's ``.env``, so an env-only opt-in on the default profile
...
def live_default_gateway_pid() -> Optional[int]:
    ...
    return live_gateway_pid_for_home(get_default_panergos_root())
...
def recorded_served_profiles(default_root: Optional[Path] = None) -> Optional[list[str]]:
    ...
    if live_default_gateway_pid() is None:
        return None
    runtime = read_runtime_status((default_root or get_default_panergos_root()) / "gateway_state.json")
...
def served_profile_ingress_urls(profile: Optional[str] = None) -> dict[str, dict[str, str]]:
    ...
    if live_default_gateway_pid() is None:
        return {}
    runtime = read_runtime_status(get_default_panergos_root() / "gateway_state.json") or {}
...
def notify_multiplexer_profiles_changed(profile_name: str, *, timeout: float = 8.0) -> Optional[list[str]]:
    ...
    try:
        ...
        if live_default_gateway_pid() is None:
            return None
        answer = rescan_gateway_profiles(get_default_panergos_root(), timeout=timeout)
    except Exception:
