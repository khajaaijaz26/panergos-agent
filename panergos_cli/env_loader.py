"""Helpers for loading Panergos .env files consistently across entrypoints."""
...
# Once-per-process guards: load_panergos_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `panergos
...
# PANERGOS_HOME paths already pulled external secrets for: load_panergos_dotenv() runs at import time from
...
# Behavioral routing keys a parent Panergos process injects into child env that silently redirect a profile
...
def get_secret_source_values(panergos_home: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``panergos_home``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(panergos_home).resolve()), {}))


def hydrate_profile_secret_sources(panergos_home: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(panergos_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_panergos_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
        ...
        local_env["PANERGOS_HOME"] = str(home)
...
def reset_secret_source_cache(panergos_home: str | os.PathLike | None = None) -> None:
    """Forget applied homes so the next load re-pulls (tests, long-running processes after config edits).

    ``panergos_home`` limits the reset to ONE home: a multiplex gateway keeps every profile's snapshot in
    ...
    (#102041)."""
    if panergos_home is None:
        _APPLIED_HOMES.clear()
        _SECRET_SOURCES.clear()
        _SECRET_SOURCE_VALUES_BY_HOME.clear()
        return
    home_key = str(Path(panergos_home).resolve())
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `panergos setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from panergos_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_panergos_dotenv(
    *,
    panergos_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load Panergos env files: ``~/.panergos/.env`` overrides stale shell exports; project ``.env`` is a dev
    ...
    home_path = Path(panergos_home) if panergos_home else get_process_panergos_home()
    ...
    if is_multiplex_active() and get_panergos_home_override() is not None:
        ...
        if load_external_secrets:
            from panergos_cli import _early_recovery
    ...
    # Windows (#73381, #86735). 2. A fresh ``panergos update`` retry just completed a deferred dependency
    ...
    # ~/.panergos/.env win on every reload and flip the backend mid-session in long-lived processes.
    ...
    # with override=True — so a stale TERMINAL_ENV=docker left in ~/.panergos/.env (e.g. written by an older
    # `panergos setup` before the user switched terminal.backend in config.yaml) silently wins again on every
    # reload. Startup launchers bridge config→env once, but long-lived processes (gateway per-turn reload,
    # cron standalone runs) call load_panergos_dotenv() repeatedly and used to flip the effective backend back
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_panergos_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE Panergos reads credentials; failures never block startup. Precedence/conflicts/provenance
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and panergos_logging reuse one parse instead of 3-4.
    if home_path == _process_panergos_home():
        try:
...
def _process_panergos_home() -> Path:
    """The PANERGOS_HOME the running process was launched under.

    Must be the *true* process home, ignoring any context-local
    ``set_panergos_home_override`` a per-request task has installed. Both
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``panergos_constants.get_process_panergos_home()`` is the override-immune
