"""Per-profile view of ``panergos_cli.models``' module-level catalog slots.
...
files. In a multiplexed gateway every turn runs under a PANERGOS_HOME override, so a single module slot
...
def profile_slot_get(module: Any, attr: str, default: Any = None) -> Any:
    ...
    return _SLOTS_BY_HOME.get((panergos_home_key(), attr), default)
...
def profile_slot_set(module: Any, attr: str, value: Any) -> None:
    if get_panergos_home_override() is None:
        ...
        _SLOTS_BY_HOME[(panergos_home_key(), attr)] = value
