"""panergos fallback — manage the fallback provider chain (tried in order when the primary fails).

Subcommands: ``list`` (default), ``add`` (same picker as `panergos model`), ``remove``, ``clear``.
"""
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if chain is None:
        print("  Add one with:  panergos fallback add\n")
        return
    print()
    ...
    print("  Docs: https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/features/fallback-providers\n")


def cmd_fallback_add(args) -> None:
    """Launch the same picker as `panergos model`, then append the selection to the chain."""
    from panergos_cli.main import _require_tty, select_provider_and_model
    from panergos_cli.config import load_config, save_config
    ...
    print("\n  Adding a fallback provider.  The picker below is the same one used by\n"
          "  `panergos model` — select the provider + model you want as a fallback.\n")
    ...
    print("  Run `panergos fallback list` to view, or `panergos fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``panergos fallback [subcommand]``."""
