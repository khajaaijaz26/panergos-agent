"""PTY bridge for `panergos dashboard` chat tab.
...
``panergos_cli.web_server``. This module is POSIX-only and depends on ``fcntl``, ``termios`` and
``ptyprocess``; native Windows selects the separate ``win_pty_bridge`` ConPTY implementation.
"""
...
# ui-tui/packages/panergos-ink/src/ink/termio/host.ts — keep the two in sync.
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                           "Use the pywinpty-backed win_pty_bridge on Windows.")
