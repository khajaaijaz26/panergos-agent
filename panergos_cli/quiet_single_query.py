"""Quiet ``panergos chat -Q`` helpers: bind this session's key and resume nested notifies.

Bot Mode delivers a local DM as ``panergos -p <bot> chat -Q --query-file``. Interactive
...
``PANERGOS_SESSION_KEY`` and never woke the recipient. Quiet also printed and exited
