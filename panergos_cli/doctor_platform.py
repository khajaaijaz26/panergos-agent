"""Host-platform checks for panergos doctor: interpreter, SQLite, certificates, macOS TCC, gateway supervision, command install.
Split out of ``panergos_cli/doctor.py``, which re-exports every name so ``panergos_cli.doctor.<name>`` keeps resolving (and monkeypatching)."""
...
from panergos_cli.config import is_nix_install_method, recommended_update_command_for_method
from panergos_cli.doctor_report import (
    Finding, _fail_and_issue, _section, check_bool, check_fail, check_info, check_ok, check_warn, doctor_check,
    warn_on_error,
)
from panergos_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    from panergos_cli.doctor import PROJECT_ROOT
    from panergos_cli.config import detect_install_method
    ...
    action = cmd if is_nix_install_method(method) else {  # nix: prose guidance, not a shell command
        "docker": f"run `{cmd}`, then recreate all Panergos containers", "apt": f"run `{cmd}`"}.get(method, "run `panergos update`")
    return f"({action}; fixed versions: 3.51.3+ / 3.50.7 / 3.44.6 — see https://sqlite.org/wal.html#walresetbug)"


def _panergos_database_paths(panergos_home: Path) -> list[tuple[str, Path]]:
    """(display name, path) pairs for Panergos-managed SQLite databases: backup.py's per-profile store list + per-board kanban.db."""
    from panergos_cli.backup import _QUICK_STATE_FILES
    entries = [(name, panergos_home / name) for name in _QUICK_STATE_FILES if name.endswith(".db")]
    for board_db in sorted((panergos_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(panergos_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    process's POSIX advisory locks (see ``panergos_cli.sqlite_safe_read``), and the dashboard console runs
    ``run_doctor`` in-process with live ``SessionDB`` connections — the helper refuses then (unreadable).
    """
    from panergos_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
...
def _report_database_journal_modes(panergos_home: Path | None = None, version_info: tuple[int, ...] | None = None) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from panergos_cli.doctor import PANERGOS_HOME
    from panergos_state_wal import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable
    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    try:
        databases = _panergos_database_paths(panergos_home if panergos_home is not None else PANERGOS_HOME)
    except Exception as exc:
        check_warn(f"Could not list Panergos databases: {exc}")
        return
    exposed = []
...
def _check_version_consistency(issues: list[str]) -> None:
    """Detect pyproject.toml vs panergos_cli.__version__ drift (a conflict resolution can revert one but not the
    other). Silent no-op for installed wheels (no pyproject)."""
    try:
        from panergos_cli import __version__ as init_version
    except Exception:
        return
    ...
    _fail_and_issue("Version mismatch between source files", f"(pyproject.toml {pyproject_version} != panergos_cli/__init__.py {init_version})",
                    "Re-sync version files (e.g. run 'panergos update', or set panergos_cli/__init__.py __version__ to match pyproject.toml)", issues)
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    try:
        from panergos_cli.service_manager import S6ServiceManager, detect_service_manager
    except Exception:
        return
    ...
    for static in ("main-panergos", "dashboard"):  # s6-rc symlinks under /run/service/, same s6-svstat probe
    ...
    if not profiles:
        return check_info("No per-profile gateways registered yet — create one with `panergos profile create <name>`")
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        issues.append(f"Repair the CA bundle: run `panergos doctor --fix`, or `{pip_cmd}`")
        return
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        from panergos_cli.gateway import get_systemd_linger_status, get_systemd_unit_path, is_linux
...
_TCC_CDHASH_DETAIL = (
    ...
    "all permission grants. Run `panergos update` to get the stable identifier-pinned signing identity, "
    "then re-grant permissions once.")
_TCC_STABLE_DETAIL = {
    True: "(certificate-anchored DR; grants survive rebuilds)",
    False: "(identifier-pinned DR; grants survive rebuilds — for the strongest anchor, see `panergos desktop --setup-tcc-identity`)",
}
...
def check_macos_tcc_grants() -> None:
    ...
    check_info("If macOS still re-prompts for permissions (toggle shows ON): the stored grant is stale — run "
               "`tccutil reset ScreenCapture com.khajaaijaz26.panergos` (repeat per affected service), toggle it ON in "
               "System Settings, then fully quit & relaunch Panergos once.")


def _desktop_app_bundle() -> Path | None:
    """Locate the locally-built desktop bundle (``apps/desktop/release/mac-<arch>/Panergos.app``), newest first.

    The only layout whose ad-hoc re-signed bundle can invalidate TCC grants. ``/Applications/Panergos.app`` is
    deliberately not probed: it is the separately signed, certificate-anchored installed app.
    """
    release_dir = Path(__file__).resolve().parents[1] / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/Panergos.app") if p.is_dir()]
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    with warn_on_error("macOS TCC anchor check failed"):
        from panergos_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    try:
        ...
        check_info("One switch silences all macOS folder prompts: grant your terminal app Full Disk Access and Panergos "
                   ...
                   "    then enable your terminal (and Panergos.app if you use Desktop), and restart them once. "
                   "With Panergos' stable signing identities the grant survives every update.")
    except OSError:
...
def _check_security_advisories(should_fix: bool, f: Finding) -> None:
    ...
    from panergos_cli.security_advisories import detect_compromised, filter_unacked, full_remediation_text, get_acked_ids
    ...
    for hit in fresh_hits:
        ...
        _fail_and_issue(f"{hit.advisory.title}", f"({hit.package}=={hit.installed_version})",
                        ...
                        f"and rotate credentials, then run `panergos doctor --ack {hit.advisory.id}`.", f.manual_issues)
...
def _check_python_environment(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("SQLite version probe failed: {e}", ""):
        import sqlite3
        from panergos_state_wal import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        src = sqlite_source_id()
        # Warn-only: Panergos already refuses WAL on fresh DBs and runtime repair is best-effort.
...
def _check_command_installation(should_fix: bool, f: Finding) -> None:
    ...
    venv_bin = next(
        (
            ...
            if (candidate := PROJECT_ROOT / venv_name / "bin" / "panergos").exists()
        ),
        None,
    )
    if venv_bin is None:
        check_warn("Venv entry point not found", "(panergos not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')")
    ...
    link = link_dir / "panergos"
    if link.is_symlink():
        target, expected = link.resolve(), venv_bin.resolve()
        if target == expected:
            return check_ok(f"{display}/panergos → correct target")
        check_warn(f"{display}/panergos points to wrong target", f"(→ {target}, expected → {expected})")
        if not should_fix:
            return f.issues.append(f"Broken symlink at {display}/panergos — run 'panergos doctor --fix'")
        link.unlink()
        verb = "Fixed"
    elif link.exists():  # regular file (wrapper script), not a symlink
        return check_ok(f"{display}/panergos exists (non-symlink)")
    else:
        check_fail(f"{display}/panergos not found", "(panergos command may not work outside the venv)")
        if not should_fix:
            return f.issues.append(f"Missing {display}/panergos symlink — run 'panergos doctor --fix'")
        link_dir.mkdir(parents=True, exist_ok=True)
        verb = "Created"
    link.symlink_to(venv_bin)
    check_ok(f"{verb} symlink: {display}/panergos → {venv_bin}")
    f.fixed += 1
