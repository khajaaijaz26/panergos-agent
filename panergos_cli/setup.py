"""Interactive setup wizard for Panergos Agent (config lives in ~/.panergos/).
...
``panergos_cli.setup.<name>`` keep working.
"""
...
from panergos_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
# Config helpers are re-exported (tests patch them on this module). display_panergos_home is
# imported lazily at call sites (stale-module safety during panergos update).
from panergos_cli.config import (
    cfg_get, DEFAULT_CONFIG, get_panergos_home, get_config_path, get_env_path, load_config, save_config,
    save_env_value, remove_env_value, get_env_value, ensure_panergos_home,
)
from panergos_cli.colors import Colors, color
from panergos_cli.cli_output import print_error, print_info, print_success, print_warning
...
_DOCS_BASE = "https://khajaaijaz26.github.io/panergos-agent/docs"
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    _info("The interactive wizard cannot be used here.", None,
          "Configure Panergos using environment variables or config commands:",
          "  panergos config set model.provider custom",
          "  panergos config set model.base_url http://localhost:8080/v1",
          "  panergos config set model.default your-model-name", None,
          "Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.",
          "Run 'panergos setup' in an interactive terminal to use the full wizard.", None)
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar("panergos_setup_navigation", default=None)
...
def _setup_navigation_scope():
    ...
    from panergos_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def run_setup_action_with_navigation(
    ...
    ``panergos model`` that use the wizard's pickers outside ``run_setup_wizard``."""
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from panergos_cli.curses_ui import curses_radiolist
...
def is_noninteractive() -> bool:
    ...
    with ``stdin=DEVNULL`` and ``PANERGOS_NONINTERACTIVE=1`` (``panergos_cli/web_server.py``), where a
    prompt that aborts on EOF would kill the spawned action — callers fall back to their default."""
    return os.environ.get("PANERGOS_NONINTERACTIVE", "").strip().lower() in {"1", "true", "yes", "on"}


def prompt_yes_no(question: str, default: bool = True) -> bool:
    """Prompt for yes/no. Ctrl+C exits; empty input, ``PANERGOS_NONINTERACTIVE=1`` or a
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from panergos_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    print()
    _prompt_and_save_env_var(var, "  ✓ Saved", "  Skipped (configure later with 'panergos setup')")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    """Configure the inference provider and default model via the ``panergos model`` flow (one code
    ...
    from panergos_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: panergos model")
    ...
    # keep safe defaults (configure via `panergos auth add` / `panergos setup tts`).
    config.clear()
...
def _apply_default_agent_settings(config: dict):
    ...
    # config.yaml is authoritative for max_turns (the gateway bridges it into PANERGOS_MAX_ITERATIONS);
    # a stale .env entry silently shadowing it caused the 60-vs-500 bug, so drop it.
    remove_env_value("PANERGOS_MAX_ITERATIONS")
    ...
    _info("  Max iterations: 150", "  Tool progress: all", "  Compression threshold: 0.50",
          "  Run `panergos setup agent` later to customize.")
...
_TOOL_PROGRESS_HELP = (
    ...
    "  log     — Silent in chat; write every tool call to ~/.panergos/logs/tool_calls.log (gateway only)",
)
def setup_agent_settings(config: dict):
    ...
    if max_iter is None:
        ...
        # config.yaml only; gateway/run.py derives PANERGOS_MAX_ITERATIONS from agent.max_turns.
...
def setup_tools(config: dict, first_install: bool = False):
    """`panergos setup tools` == `panergos tools`: platform selection → toolset toggles → provider keys.
    ``first_install`` selects the simplified flow (no platform menu, prompts for all missing keys)."""
    from panergos_cli.tools_config import tools_command
...
_SEND_CONSENT_EXPLAINER = (
    "", "Sending uploads each daily package to the configured metrics collector.",
    "Packages carry your profile-scoped install ID, a",
...
def setup_telemetry(config: dict):
    ...
    _info("Shared metrics contain only bounded counters and histograms.",
          "Collection is local. Sending them to a configured collector is a separate opt-in.")
    ...
    shared_metrics["send"] = prompt_yes_no("Send shared metrics to the configured collector?", default=shared_metrics.get("send") is True)
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    try:
        from panergos_cli.observability.shared_metrics import SharedMetricsStore
        from panergos_cli.observability.shared_metrics_sender import reconcile_send_consent
        from panergos_cli.sqlite_util import write_txn
...
# panergos_cli.setup. They import this module lazily inside bodies, so this is cycle-free.

from panergos_cli.setup_tts import setup_tts  # noqa: E402
from panergos_cli.setup_terminal import setup_terminal_backend  # noqa: E402
from panergos_cli.setup_platforms import setup_gateway  # noqa: E402
from panergos_cli.setup_summary import _print_setup_summary  # noqa: E402,F401
from panergos_cli.setup_migration import _offer_openclaw_migration, _skip_configured_section  # noqa: E402
from panergos_cli.setup_quick import _run_quick_setup  # noqa: E402
...
def _run_setup_section(config: dict, section: str) -> None:
    """``panergos setup <section>``: run one SETUP_SECTIONS entry under the banner."""
...
def _run_full_setup(config: dict, panergos_home, *, is_existing: bool, migration_ran: bool) -> None:
    ...
    _info(f"Config file:  {get_config_path()}", f"Secrets file: {get_env_path()}",
          f"Data folder:  {panergos_home}", f"Install dir:  {PROJECT_ROOT}", None,
          "You can edit these files directly or use 'panergos config edit'")
    if migration_ran:
    ...
    def _gateway_step() -> None:
        ...
        from panergos_cli.gateway import ensure_gateway_service
...
_FIRST_TIME_MODES = (
    ("Full setup — configure every provider, tool & option yourself", None),
...
def _run_setup_wizard_impl(args):
    ...
    ``panergos setup <section>`` from SETUP_SECTIONS."""
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps([("Quick Setup", lambda: _run_quick_setup(config, panergos_home))])
            return
        print_header("Reconfigure", gap=True)
        print_success("You already have Panergos configured.")
        _info("Running the full wizard — each prompt shows your current value.",
              ...
              "Tip: jump straight to a section with 'panergos setup model|terminal|",
        ...
        migration_ran = _offer_openclaw_migration(panergos_home)  # before configuration begins
        if migration_ran:
            config = load_config()
        setup_mode = prompt_choice("How would you like to set up Panergos Agent?", [label for label, _ in _FIRST_TIME_MODES], 0)
        label, runner = _FIRST_TIME_MODES[setup_mode]
        if runner is not None:
            from panergos_cli import setup_quick
            _run_setup_steps([(label, lambda: getattr(setup_quick, runner)(config, panergos_home, is_existing))])
            return
    _run_full_setup(config, panergos_home, is_existing=is_existing, migration_ran=migration_ran)
    ...
    _print_setup_summary(config, panergos_home)
...
_PLUGIN_COMPAT_LAZY = {
    'get_optional_skills_dir': ('panergos_constants', 'get_optional_skills_dir'),
}
