"""Windows gateway lifecycle for ``panergos update``: pause/resume/cold-start the service, sweep venv holders, reap orphaned backends.

Split out of ``update_cmd.py``; names are re-imported there so ``panergos_cli.update_cmd.<name>`` still resolves/monkeypatches.
...
from panergos_cli import __distribution_default_branch__ as DISTRIBUTION_BRANCH
from panergos_cli.update_cmd_common import _best_effort

logger = logging.getLogger("panergos_cli.update_cmd")  # log-record parity with the origin module
...
def _self_and_non_gateway_ancestor_pids(psutil) -> set[int]:
    ...
    with suppress(Exception):
        # Never return ourselves or our own ancestry: a CLI ``panergos update`` runs from the venv python and
...
def _detect_venv_python_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str, str]]:
    ...
    The panergos.exe shim guard misses the Desktop backend and anything off ``venv\\Scripts\\python(w).exe``;
    ...
    for proc in proc_iter:
        ...
        name_low = name.lower()
        if not (in_venv or name_low.startswith(("python", "pypy")) or name_low in {"uv.exe", "uvx.exe", "panergos.exe"}):
            continue
        ...
        # its .pyd files — match cmdline (venv path, or `-m panergos_cli.main` + root/cwd).
        if in_venv or venv_prefix in cmdline_low or (
            "panergos_cli.main" in cmdline_low and (root_prefix in cmdline_low or _cwd_prefix(proc).startswith(root_prefix))
        ):
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    Labels come from the parsed SUBCOMMAND, never substring: a standalone ``panergos dashboard`` must not be
    ...
    for pid, name, cmdline in matches[:6]:
        hint = hint_by_subcommand.get(_panergos_holder_subcommand(cmdline) or "", "")
    ...
    lines.append(
        ...
        "  Close the panergos desktop app / other Panergos terminals, then re-run:\n    panergos update\n"
        "  (or use `panergos update --force-venv` to proceed anyway at your own risk)"
    )
    return "\n".join(lines)
...
def _leftover_pausable_gateway_pids(matches: list[tuple[int, str, str]]) -> list[int] | None:
    ...
    from panergos_cli._scan_venv_blockers import _is_pausable_gateway
    psutil = _psutil()
    pids: list[int] = []
...
def _refuse_gateway_ancestor_tree_kill(pids: list[int], *, gateway_mode: bool) -> bool:
    """Refuse a plain Windows update that would tree-kill its own ancestry (a chat agent's ``panergos update`` is
    ...
    if gateway_mode or not pids:
        return False
    def _ancestors():
        from panergos_cli.gateway import _is_pid_ancestor_of_current_process
    ...
    print(
        ...
        "  From a chat platform, use `/update` instead.\n  Otherwise, run `panergos update` from a separate terminal."
    )
    return True
...
def _ledger_manual_serve_holders(matches: list[tuple[int, str, str]]) -> list[dict]:
    ...
    try:
        from panergos_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _serve_relaunch_commands(entries: list[dict]) -> list[list[str]]:
    ...
    with suppress(Exception):
        ...
        if scripts_dir is not None:
            panergos = next((str(scripts_dir / n) for n in ("panergos.exe", "panergos") if (scripts_dir / n).is_file()), panergos)
    ...
    for entry in entries:
        ...
        commands.append(
            [panergos] + (["--profile", profile] if profile and profile != "default" else [])
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    from panergos_cli.update_cmd import _m, _record_update_step
    ...
    if skipped or failed:
        print("  ⚠ Some stopped backends could not be relaunched automatically; restart them manually (panergos serve --host <ip> --port <port>).")
    _record_update_step(
...
def _is_backend_argv(argv_low: str) -> bool:
    ...
    Same predicate as ``_looks_like_desktop_control_plane``: ``-m panergos_cli.main`` entry shape (the
    Desktop's only spawn shape, ``apps/desktop/electron/main.ts``) AND the canonical holder classifier says
    ``serve``/``dashboard``. A user-launched ``panergos.exe serve`` / ``panergos dashboard`` is NOT the
...
def _orphaned_desktop_backend_pids(matches: list[tuple[int, str, str]]) -> list[tuple[int, int]] | None:
    ...
    would dead-end the update with "Panergos is still running" and zero open windows. Qualifies only if cmdline
    is a Panergos backend AND the parent is demonstrably gone (PID missing or reused). Tree-aware: holders inside
    ...
    it tree-kills its backends and waits for the venv shim before spawning panergos-setup, and the
    ...
    dead-ends the update with "Panergos is still running" while the user stares at zero open windows (ryanc's
    ...
    for pid, _name, cmdline in matches:
        ...
        try:
            ...
            # gateway.status.get_process_start_time so pid_is_panergos round-trips at kill time.
...
def _ledger_reapable_backend_pids(matches: list[tuple[int, str, str]]) -> list[int]:
    ...
    try:
        from panergos_cli.process_identity import REAPABLE_PURPOSES, ledger_entries, spawner_is_dead
...
def _handoff_reapable_backend_pids(matches: list[tuple[int, str, str]]) -> list[int] | None:
    ...
    hand-off. Inside the hand-off gate (marker + ``--gateway`` + no live ``panergos.exe`` shim) nothing legitimate
...
def _stop_process_trees(pids: list[int] | list[tuple[int, int]]) -> None:
    ...
    ``taskkill /T /F``: stopping only the parent can leave a ``.panergos-runtime`` child holding the install open.
    ...
    for entry in pids:
        ...
        try:
            if expected_start_time is None:
                ...
                continue
            if not pid_is_panergos(pid, expected_start_time=expected_start_time):
                logger.debug("Skipping taskkill of non-Panergos or changed PID %s", pid)
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``panergos serve`` / ``panergos dashboard`` argv (Desktop control plane).
    ...
    """
    return "panergos_cli.main" in (cmdline or "").lower() and _panergos_holder_subcommand(cmdline) in _BACKEND_PURPOSES
...
def _windows_cold_start_plan() -> dict | None:
    ...
    *generation* that authorized it on the token: the marker is a mutable one-shot that any concurrent
    ``panergos gateway status``/``start`` consumes, so execution authorizes the spawn from the token and
    consumes only that generation (#110020 review)."""
    from panergos_cli.update_cmd import _desktop_owns_gateway_lifecycle
...
def _pause_windows_gateway_services(service_gateways, token: dict, profiles: dict, unmapped: list) -> dict:
    ...
    from panergos_cli.update_cmd import _restore_windows_gateway_service, _stop_windows_gateway_service
    paused_services = []
...
def _discover_windows_gateways():
    ...
    from panergos_cli.gateway import find_gateway_pids, find_profile_gateway_processes, find_windows_gateway_services
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    Scheduled/startup gateways run via pythonw.exe, invisible to the panergos.exe instance guard, yet keep files
    ...
    with _abort_on_error("Could not prepare Windows gateway pause for update"):
        ...
        from panergos_cli.gateway import _capture_gateway_argv
    ...
    print("→ Stopping Windows gateway process(es) before updating Panergos...")
    ...
    if unmapped_pids:
        ...
        if any(not u.get("argv") for u in unmapped):  # no recoverable cmdline (psutil missing, denied, gone)
            print("    Restart manually after update: panergos gateway run")
...
def _cold_start_windows_gateway_after_update(token: dict | None = None) -> bool:
    ...
    ``panergos gateway status``/``start`` consumes, which would otherwise skip this spawn and clear the
    token (#110020 review). Only that generation is consumed afterwards — never a newer marker.
    """
    from panergos_cli.update_cmd import _desktop_owns_gateway_lifecycle, _m
    if not _m()._is_windows():
        return True
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    artifacts written once at install time — ``panergos update`` never touched them, so installs created
...
def _refresh_bootstrap_cache_scripts(branch: str = DISTRIBUTION_BRANCH) -> None:
    """Overwrite ``$PANERGOS_HOME/bootstrap-cache/install-<ref>.{ps1,sh}`` for *branch* from the fresh checkout.

    Old ``panergos-setup.exe`` builds NEVER re-download a cached branch-ref script, so a stale one runs
    ...
    so the poisoned cache outlives every ``panergos update``.
    ...
    with _best_effort('Could not refresh bootstrap-cache scripts after update: %s'):
        cache_dir = Path(_m().get_panergos_home()) / "bootstrap-cache"
        if not cache_dir.is_dir():
            return
        safe_ref = re.sub(
            r"[^A-Za-z0-9._-]", "_", str(branch or DISTRIBUTION_BRANCH)
        )  # install_script.rs::sanitize_ref()
...
def _relaunch_paused_gateways(token: dict, profiles: dict, unmapped: list) -> tuple[list[str], int]:
    ...
    with _abort_on_error("Could not load Windows gateway restart helper"):
        from panergos_cli.gateway import launch_detached_gateway_restart_by_cmdline, launch_detached_profile_gateway_restart
    ...
    *,
    ...
    if not ready_pids:
        ...
        print(
            ...
            "    Recover with: panergos gateway restart"
        )
...
def _in_handoff_without_live_shim(args) -> bool:
    """GUI hand-off gate: ``--gateway`` + update-incomplete marker AND no live ``panergos.exe`` shim.
    ...
    try:
        if not (bool(getattr(args, "gateway", False)) and _m()._update_marker_path().exists()):
            return False
        scripts_dir = _m()._venv_scripts_dir()
        return scripts_dir is not None and not _m()._detect_concurrent_panergos_instances(scripts_dir)
    except Exception:
        return False
...
def _clear_windows_venv_holders_or_exit(args, gateway_mode: bool, _windows_gateway_resume):
    ...
    from panergos_cli.update_cmd import _m, _record_update_step, _refuse_gateway_ancestor_tree_kill
    ...
    for classifier, message in (
        (_m()._ledger_reapable_backend_pids, "ledger-identified orphaned Panergos backend process(es) hold the venv"),
    ...
    # Manual serve/dashboard rung (e.g. `panergos serve --host <ip>` for a REMOTE Desktop): ledger identity
    ...
    if holders and _in_handoff_without_live_shim(args) and (handoff_backends := _m()._handoff_reapable_backend_pids(holders)):
        holders = _reap_and_rescan(
            f"  ⚠ {len(handoff_backends)} Panergos backend process(es) "
