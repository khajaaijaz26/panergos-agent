"""Per-provider model-selection wizard flows for ``panergos setup`` / ``panergos model``.

main / config / auth / models helpers are imported lazily inside bodies: avoids the main.py import
cycle and lets tests patch ``panergos_cli.config.load_config`` etc. at call time. The shared skeleton
lives in :mod:`panergos_cli.model_setup_flows_common`; the custom / Azure / Bedrock flows live in their
...
from panergos_cli.model_setup_flows_custom import _model_flow_custom, _model_flow_named_custom
from panergos_cli.model_setup_flows_azure import _model_flow_azure_foundry
from panergos_cli.model_setup_flows_bedrock import _model_flow_bedrock
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from panergos_constants import OPENROUTER_BASE_URL
    from panergos_cli.auth import ProviderConfig, _prompt_model_selection
    ...
    from panergos_cli.models import model_ids
    from panergos_cli.models_pricing import get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from panergos_cli.main_provider_setup import _prompt_api_key
    from panergos_cli.auth import PROVIDER_REGISTRY, _prompt_model_selection
    from panergos_cli.config import get_env_value
    ...
    from panergos_cli.models import ai_gateway_model_ids
    from panergos_cli.models_pricing import get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    from panergos_cli.auth import _save_model_choice
    from panergos_cli.moa_config import normalize_moa_config
    moa = normalize_moa_config(config.get("moa") if isinstance(config, dict) else {})
    presets = moa.get("presets") or {}
    if not presets:
        print("No MoA presets configured. Run `panergos moa configure <name>` first.")
        return
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    # Prefer the credential pool (where `panergos auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from panergos_cli.models import provider_model_ids
    ...
    # credentials may live only in the pool (``panergos auth add xai-oauth``) — fall back to
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from panergos_cli.main_provider_setup import _DEFAULT_QWEN_PORTAL_MODELS
    from panergos_cli.auth import (
...
def _copilot_catalog(api_key: str):
    ...
    from panergos_cli.models import fetch_github_model_catalog, normalize_copilot_model_id
...
def _copilot_obtain_token() -> bool:
    ...
    if choice == "1":
        try:
            from panergos_cli.copilot_auth import copilot_device_code_login
    ...
    if choice == "2":
        ...
        with contextlib.suppress(ImportError):
            from panergos_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot(config, current_model=""):
    ...
    from panergos_cli.models import fetch_api_models, copilot_model_api_mode
    provider_id = "copilot"
    ...
    if not api_key:
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from panergos_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    _say("  GitHub Copilot ACP delegates Panergos turns to `copilot --acp`.",
         "  Panergos currently starts its own ACP subprocess for each request.",
         "  Panergos uses your selected model as a hint for the Copilot ACP session.",
         f"  Command: {resolved_command}", f"  Backend marker: {effective_base}", "")
    try:
        creds = resolve_external_process_provider_credentials(provider_id)
    except Exception as exc:
        _say(f"  ⚠ {exc}", "  Set PANERGOS_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from panergos_cli.auth import PROVIDER_REGISTRY, KIMI_CODE_BASE_URL
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from panergos_cli.main_provider_setup import _infer_stepfun_region, _prompt_provider_choice, _stepfun_base_url_for_region
    from panergos_cli.auth import PROVIDER_REGISTRY
    from panergos_cli.config import save_env_value
    from panergos_cli.models import _PROVIDER_MODELS, fetch_api_models
    provider_id = "stepfun"
    pconfig = PROVIDER_REGISTRY[provider_id]
    base_url_env = pconfig.base_url_env_var or ""
...
def _model_flow_vertex(config, current_model=""):
    ...
    from panergos_cli.auth import _prompt_model_selection
    from panergos_cli.config import load_config, get_env_value
    from panergos_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        _say("  Vertex credentials: Application Default Credentials (ADC)",
             ...
             "      • set VERTEX_CREDENTIALS_PATH in ~/.panergos/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    """Picker for the official Z.AI endpoints (``ZAI_ENDPOINTS`` in ``panergos_cli.auth``, kept in
    sync with the probe list) plus a custom-proxy option. Returns the selected base URL;
    *current_base* on cancel/error."""
    from panergos_cli.main_provider_setup import _prompt_provider_choice
    from panergos_cli.auth import ZAI_ENDPOINTS
...
_GEMINI_FREE_TIER_NOTICE = (
    ...
    "   Panergos typically makes 3-10 API calls per user turn (tool iterations + auxiliary tasks),",
    "   so the free tier is exhausted after a handful of messages and cannot sustain",
    "   an agent session.", "",
    "   To use Gemini with Panergos, enable billing on your Google Cloud project and regenerate",
    "   the key in a billing-enabled project: https://aistudio.google.com/apikey", "",
    "   Alternatives with workable free usage: DeepSeek, OpenRouter free models, and Groq.", "",
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from panergos_cli.auth import PROVIDER_REGISTRY
    from panergos_cli.config import save_env_value, load_config
    from panergos_cli.models import opencode_model_api_mode, normalize_opencode_model_id
    pconfig = PROVIDER_REGISTRY[provider_id]
...
def _anthropic_authenticate() -> bool:
    ...
    from panergos_cli.main_provider_setup import _run_anthropic_oauth_flow
    from panergos_cli.config import save_env_value, save_anthropic_api_key
...
_PLUGIN_COMPAT_LAZY = {
    'BEDROCK_GEO_PREFIXES': ('panergos_cli.model_setup_flows_bedrock', 'BEDROCK_GEO_PREFIXES'),
    'bedrock_model_routable_from_region': ('panergos_cli.model_setup_flows_bedrock', 'bedrock_model_routable_from_region'),
    'bedrock_region_geo_prefix': ('panergos_cli.model_setup_flows_bedrock', 'bedrock_region_geo_prefix'),
    'custom_provider_slug': ('panergos_cli.providers', 'custom_provider_slug'),
