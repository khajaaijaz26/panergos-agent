...
def _confirm_selection_guards(
    ...
    """
    try:
        from panergos_cli.model_selection_guards import combined_message, selection_warnings
...
class _ModelPickerRows:
    """Column-aligned picker rows (name + $/Mtok + optional sale chrome).
    ...
    def __init__(
        ...
        from panergos_cli.models_pricing import _format_price_per_mtok, compute_sale_discount
...
def _prompt_model_selection(
    ...
    *unavailable_models* render grayed out and unselectable with an upgrade link to *portal_url*.
    ...
    sale_chrome = bool(pricing and any((pricing.get(mid) or {}).get("original") for mid in model_ids))
    ...
    try:
        ...
        if not unavailable_footer and _unavailable:
            unavailable_footer = f"See {_upgrade_url} for access details" if _upgrade_url else "Unavailable for this account"
        ...
        from panergos_cli.model_search import model_search_text
        model_search_labels = []
        for mid in ordered:
    ...
    from panergos_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    from panergos_cli.config import save_config, load_config
    config = load_config()
