"""ZIP-download fallback for ``panergos update`` (Windows with broken git): two-phase stage/commit swap, dirty-tree guard.

Split out of ``update_cmd.py``; every name is re-imported there so ``panergos_cli.update_cmd.<name>`` keeps
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = ".panergos-update-staging", ".panergos-update-old"
...
_STASH_HINT = "  Stash or commit your changes, then rerun `panergos update`."
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    Thin alias over the two-phase helpers; retained for the ``panergos_cli.main`` re-export surface.
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    staging = f"{dst}.panergos-update-staging"
    backup = f"{dst}.panergos-update-old"
...
def _zip_overlay_block_reason(root: Path, *, ignore_staging_artifacts: bool = False) -> Optional[str]:
    ...
    is for the pre-swap re-check: phase 1 leaves our own ``*.panergos-update-staging`` siblings that git
...
def _extracted_root(tmp_dir: str, branch: str) -> str:
    """GitHub ZIPs extract to ``panergos-agent-<branch>/``; fall back to the first non-``__MACOSX`` dir."""
    extracted = os.path.join(tmp_dir, f"panergos-agent-{branch}")
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str) -> None:
    ...
    need = sum(
        ...
        # root holds 20 first-party modules (run_agent.py, cli.py, panergos_constants.py, ...). Check up front
...
def _stage_entries(extracted: str, entries: list[str], project_root: str) -> list[tuple[str, str]]:
    ...
    try:
        for item in entries:
            ...
            # desktop app, win-unpacked/ Panergos.exe) exists only in the LIVE tree. Graft the live release
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    tmp_dir = tempfile.mkdtemp(prefix="panergos-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"panergos-agent-{branch}.zip")
        ...
        print(
            "  Re-run `panergos update` to retry; if the agent won't start, "
            f"reinstall from https://github.com/{DISTRIBUTION_REPO_SLUG}"
            "#install-from-source")
        _m().sys.exit(1)
    finally:
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    if branch != DISTRIBUTION_BRANCH:
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `panergos update "
            f"--branch {branch}`, or update against {DISTRIBUTION_BRANCH} "
            "with `panergos update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    _download_and_swap_zip(
        branch,
        f"https://github.com/{DISTRIBUTION_REPO_SLUG}/archive/refs/heads/{branch}.zip")
    ...
    if not import_ok:
        ...
        print("  Re-run `panergos update` to complete it.")
        _m().sys.exit(1)
