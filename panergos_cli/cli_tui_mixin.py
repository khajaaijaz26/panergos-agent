...
interactive CLI. Mixin bound onto ``PanergosCLI`` via the MRO; cli.py-internal symbols are imported
...
from panergos_cli.commands_completion import SlashCommandAutoSuggest, SlashCommandCompleter
...
class CLITuiMixin:
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from panergos_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from panergos_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def _render_scroll_list_panel(self, state, title, hint, labels, *, min_width, max_width, indent):
        ...
        """
        from cli import PanergosCLI, _panel_box_width, _wrap_panel_text
        ...
        scroll_offset, visible = PanergosCLI._compute_model_picker_viewport(
        ...
        if code := self._sudo_state.get("vault_code"):
            return self._render_sudo_style_panel(
                ...
                [f'{code["site"]} is asking for a one-time code (text message, email or authenticator app).',
                 'Type the code and press Enter; Panergos enters it into the page for you.',
    ...
    def _tui_handle_ignored_terminal_sequence(self, event):
        ...
        panergos_cli.pt_input_extras registers focus reports (CSI I / CSI O) as Keys.Ignore at the
    ...
    def _tui_handle_ctrl_z(self, event):
        ...
        agent_name = get_active_skin().get_branding("agent_name", "Panergos Agent")
    ...
    def _tui_enter_while_busy(self, text: str, images: list, payload) -> None:
        ...
        """
        from cli import CLI_CONFIG, _ACCENT, _DIM, _RST, _cprint, _panergos_home
        ...
        if _effective_mode == "queue":
            ...
            if redirected:
                ...
                try:
                    with open(_panergos_home / "interrupt_debug.log", "a", encoding="utf-8") as _f:
                        _f.write(
        ...
        try:
            ...
            if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                ...
                mark_seen(_panergos_home / "config.yaml", BUSY_INPUT_FLAG)
    ...
    def _tui_enter_overlay(self, event) -> bool:
        ...
        if self._model_picker_state:
            try:
                ...
                from panergos_cli.model_switch import resolve_persist_behavior
    ...
    def _tui_collapse_paste(self, text: str, line_count: int, *, fallback: bool) -> str:
        """Save a large paste under ~/.panergos/pastes and return the placeholder for the buffer."""
        from cli import _panergos_home, datetime, logger
        self._tui_paste_counter += 1
        paste_dir = _panergos_home / "pastes"
    ...
    def _tui_handle_prompt_stash(self, event):
        ...
        """
        from panergos_cli.prompt_stash import ACTION_RESTORED, ACTION_STASHED, resolve_ctrl_s
        buf = event.app.current_buffer
    ...
    def _tui_init_run_state(self):
        ...
        from panergos_cli.config import get_config_path as _get_config_path
    ...
    def _tui_build_key_bindings(self):
        ...
        from panergos_cli.cli_subagent_monitor import modal_prompt_active, open_monitor, toggle_dock
        for key in ('c-t', 'f6'):
    ...
    def _tui_voice_record_key_sequence(self) -> tuple:
        ...
        try:
            ...
            from panergos_cli.voice import (
    ...
    def _tui_build_layout(self, kb):
        ...
        cli_ref = self
        from panergos_cli.cli_subagent_monitor import install_dock
        install_dock(self)
    ...
    def _tui_set_base_style(self):
        ...
        self._tui_style_base = {
            ...
            'hint': '#888888 italic',
            'status-bar': 'bg:#120B1F #F7F2FF',
            'status-bar-strong': 'bg:#120B1F #FF6B5E bold',
            'status-bar-dim': 'bg:#120B1F #8F7E9E',
            'status-bar-good': 'bg:#120B1F #2EE6A6 bold',
            'status-bar-warn': 'bg:#120B1F #F7C453 bold',
            'status-bar-bad': 'bg:#120B1F #FF8A4C bold',
            'status-bar-critical': 'bg:#120B1F #FF4773 bold',
            'status-bar-yolo': 'bg:#120B1F #FF4773 bold',
            'status-bar-session-title': 'bg:#FF6B5E #200B13 bold',
            'input-rule': '#FF6B5E',
            'image-badge': '#67D8FF bold',
            'completion-menu': 'bg:#1A122A #F7F2FF',
            'completion-menu.completion': 'bg:#1A122A #F7F2FF',
            'completion-menu.completion.current': 'bg:#332440 #FF6B5E',
            'completion-menu.meta.completion': 'bg:#1A122A #8F7E9E',
            'completion-menu.meta.completion.current': 'bg:#332440 #2EE6A6',
            'clarify-border': '#FF6B5E',
            'clarify-title': '#FF6B5E bold',
            'clarify-question': '#F7F2FF bold',
            'clarify-choice': '#B9A9C8',
            'clarify-selected': '#FF6B5E bold',
            'clarify-active-other': '#FF6B5E italic',
            'clarify-answer': '#2EE6A6',
            'clarify-countdown': '#FF6B5E',
            'sudo-prompt': '#FF4773 bold',
            'sudo-border': '#FF6B5E',
            'sudo-title': '#FF4773 bold',
            'sudo-text': '#F7F2FF',
            'approval-border': '#FF6B5E',
            'approval-title': '#F7C453 bold',
            'approval-desc': '#F7F2FF bold',
            'approval-cmd': '#B9A9C8 italic',
            'approval-choice': '#B9A9C8',
            'approval-selected': '#FF6B5E bold',
            'voice-prompt': '#67D8FF',
            'voice-recording': '#FF4773 bold',
            'voice-processing': '#F7C453 italic',
            'voice-status': 'bg:#1A122A #67D8FF',
            'voice-status-recording': 'bg:#1A122A #FF4773 bold'}
