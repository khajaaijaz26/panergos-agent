"""``panergos skin`` — list, switch, and tweak skins from the CLI.
...
def _skin_set(key: str, value: str, skin: str | None) -> int:
    ...
    if target != name:
        _use(target)
    print(f"✓ {key} = {value} in {display_panergos_home()}/skins/{target}.yaml (live within ~1s)")
    return 0
