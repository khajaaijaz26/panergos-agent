...
cycle) and tests patch ``panergos_cli.config.load_config`` etc. at call time.
"""
...
def _ask(prompt: str, *, secret: bool = False, raw: bool = False, cancel_msg: str | None = "\nCancelled.",
    ...
    if secret:
        from panergos_cli.secret_prompt import masked_secret_prompt as fn
    else:
...
def _commit_model_config(cfg: dict) -> None:
    ...
    from panergos_cli.auth import deactivate_provider
...
def _activate_provider_model(selected, provider_id: str, base_url: str, done: str,
    ...
    from panergos_cli.auth import _save_model_choice, _update_config_for_provider
    if not selected:
