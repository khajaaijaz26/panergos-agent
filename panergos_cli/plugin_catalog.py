"""Plugin catalog — curated Panergos plugins shipped with the repo.
...
an installed Panergos sees new entries and removals without updating. Any fetch failure falls back to the
...
LIVE_CATALOG_URL = "https://khajaaijaz26.github.io/panergos-agent/docs/api/plugin-catalog.json"
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "maintainer": self.maintainer, "tier": self.tier, "requires_panergos": self.requires_panergos,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_panergos=str(data.get("requires_panergos") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_panergos_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``PANERGOS_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_panergos:
        bits.append(f"Requires Panergos {entry.requires_panergos}.")
    return " ".join(bits)
