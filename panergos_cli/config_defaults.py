"""Default configuration data for Panergos Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from panergos_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["panergos-cli"],
    # journal_mode: SQLite journal mode for every Panergos DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `panergos -c`: each CLI session writes a breadcrumb under
        # $PANERGOS_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `panergos
        ...
        # Panergos-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Panergos (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env PANERGOS_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `panergos doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`panergos status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # PANERGOS_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else PANERGOS_HOME/cache/terminal (auto-pruned
        ...
        # PANERGOS_HOME/home; "real" = force real HOME; "profile" = force PANERGOS_HOME/home when it
        ...
        # ["/home/user/.panergos/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Panergos image,
        # which drops to `panergos` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `panergos tools`, not a ring
        # member). Set by the `panergos tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Panergos-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Panergos' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.panergos/browser-profile/ so credentials
        ...
        # runs `panergos browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `panergos chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.panergos/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `panergos checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .panergos.md,
    ...
    # Same bound for single-query mode (``panergos -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Panergos' summarizer cannot shrink it. native = codex decides; panergos = Panergos' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `panergos model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `panergos`/`panergos chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / PANERGOS_TUI=1 forces the TUI.
        "interface": "cli",
        # `panergos --tui` auto-resumes the most recent human-facing session (like `panergos -c`).
        # PANERGOS_TUI_RESUME=<id> always wins.
        ...
        # `path`/`paths` = PANERGOS_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/panergos`.
        ...
        # CLI/TUI/desktop, managed with `panergos pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_panergos_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: PANERGOS_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than PANERGOS_TUI_SESSION_TTL_S, default 6h) with
        ...
        # OAuth gate (engaged when --host is set and --insecure is not). Generic environment overrides win when
        ...
        # no-op. Env PANERGOS_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # PANERGOS_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env PANERGOS_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/panergos) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.panergos/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    "voice": {
        ...
        #   chained  — STT → Panergos turn → TTS (the stt.* / tts.* providers below)
        #   gpt-live — one full-duplex voice model (OpenAI GPT-Live) owns the mic and speaker and
        #              DELEGATES every real request to Panergos (any model / provider you have
    ...
    # "Hey Panergos" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_panergos" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.panergos/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.panergos/cache/delegation/ with a head+tail window + read_file offset footer, nothing
    ...
    "moa": {
        ...
        # <panergos_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.panergos/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # PANERGOS_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.panergos/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `panergos skills trust` / `untrust`.
        "trusted_project_dirs": [],
        # Substitute ${PANERGOS_SKILL_DIR} / ${PANERGOS_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `panergos skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.panergos/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.panergos/.curator_backups/blobs/); powers `panergos
    ...
    # daemon. `panergos curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `panergos curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `panergos update` restoring them);
        ...
        "prune_builtins": True,
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `panergos curator purge`
        # delete older archived skills (never automatic; logged in the ledger).
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.panergos/skills/ to
        # ~/.panergos/skills/.curator_backups/<utc-iso>/skills.tar.gz (`panergos curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); panergos-specific overrides only here.
    "honcho": {},
    ...
    #   cron job, a -q session (PANERGOS_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export PANERGOS_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. PANERGOS_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `panergos plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `panergos plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.panergos/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # PANERGOS_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .panergos config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `panergos doctor --ack <id>`; remove by editing the list. Catalog:
        # panergos_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $PANERGOS_HOME/plugins/ <name>/),
        # An unknown or unavailable
        ...
        # Env override: PANERGOS_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: PANERGOS_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: PANERGOS_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: PANERGOS_CRON_MEDIA_SEND_TIMEOUT.
    ...
    # promotes dependency-satisfied todos to ready, and fires `panergos -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `panergos kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with panergos-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core Panergos tools
    ...
    "tools": {
        ...
        # Remote connector discovery/lifecycle through a configured gateway.
        "connectors": {"enabled": True},
    },
    "logging": {  # File logging to ~/.panergos/logs/: agent.log captures INFO+, errors.log WARNING+.
        "level": "INFO",       # minimum level for agent.log: DEBUG, INFO, WARNING
        "max_size_mb": 5,      # max size per log file before rotation
        "backup_count": 3,     # rotated backups to keep
    },
    # Remote model-catalog manifest: curated model lists fetched from this
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://khajaaijaz26.github.io/panergos-agent/docs/api/model-catalog.json",
        # Disk cache TTL in minutes. The gateway refreshes in the background on this cadence; the
        # CLI refetches on the next /model or `panergos model` once the cache is older. Network
    ...
    # defaults (200K context, tools on, vision/reasoning off) and get patched. Provider keys: Panergos
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "panergos-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to PANERGOS_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to PANERGOS_STARTUP_WATCHDOG / PANERGOS_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.panergos/sessions/sessions.json mirror of the routing index
        ...
        # stamps the profile into session keys. Flip with `panergos gateway migrate --multiplex`
        # (records a rollback manifest; `--standalone` undoes it) or `panergos config set
        # gateway.multiplex_profiles true` + `panergos gateway restart`. GATEWAY_MULTIPLEX_PROFILES
        # in the environment overrides. Two profiles configuring the same bot token cannot be
        # served together — the duplicate adapter is parked; `panergos profile create --clone`
        # therefore leaves messaging channels behind unless --clone-channels is passed.
        "multiplex_profiles": False,
        # May `panergos update` fold this install onto a multiplexed default gateway by itself?
        ...
        # `panergos gateway migrate --multiplex` is an explicit request and always proceeds.
        "auto_migrate": True,
        ...
        # (PANERGOS_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # PANERGOS_GATEWAY_MAX_STARTS / PANERGOS_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.panergos/.env, auth.json). True:
        # files must be under the Panergos cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to PANERGOS_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Panergos cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to PANERGOS_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to PANERGOS_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # PANERGOS_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.panergos/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `panergos sessions prune`).
        ...
        # stay until `panergos sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `panergos update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.panergos/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to PANERGOS_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`panergos sessions export`); checked
    ...
    # and transmission (`send`) are separate opt-ins; see
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `panergos doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Passive version/banner checks only; explicit `panergos update --check` remains enabled.
        "check": True,
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <PANERGOS_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``panergos backup`` zip in
        # <PANERGOS_HOME>/backups/ (``panergos import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `panergos update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `panergos update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <PANERGOS_HOME>/lsp/bin/ on
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `panergos tools`.
    "x_search": {
    ...
    # them in ~/.panergos/.env.
    # Browser credential vault: which login sources browser_vault_list/fill may draw from. The local
    # encrypted vault (`panergos vault add`, Desktop → Settings → Credential Vault) is always on.
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.panergos/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.panergos/cache/,
            ...
            # Auto-download bws into ~/.panergos/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Panergos sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Panergos also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.panergos/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Panergos Desktop (Electron) launch options; only affect `panergos desktop`.
        ...
        # Bridged to PANERGOS_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to PANERGOS_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "local_runtime": {
        # Off = detection-only (Panergos still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Panergos releases after validation.
        "tag": "b10679",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Panergos mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'panergos model' for endpoint-specific config)",
    ...
    "PANERGOS_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "PANERGOS_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "PANERGOS_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Panergos, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @panergos:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. PANERGOS_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #panergos)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: panergos-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'panergos-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Panergos API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Panergos API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Panergos API server (proxy mode). Must "
    ...
    # PANERGOS_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. PANERGOS_TOOL_PROGRESS is unsupported.
    "PANERGOS_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "PANERGOS_EPHEMERAL_SYSTEM_PROMPT": _setting(
