"""``panergos gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            message = (
                "Connector rejected the caller identity (401). Verify the configured "
                "OIDC credentials and retry."
            )
        elif exc.code == 403:
...
def cmd_gateway_enroll(args) -> None:
    ...
    from panergos_cli.config import is_managed, save_env_value

    # Managed installs get GATEWAY_RELAY_* stamped in by the orchestrator; save_env_value refuses
    # to write there anyway.
    if is_managed():
        _fail(
            "✗ `panergos gateway enroll` is not available in a managed/hosted install.\n"
    ...
    # is set (air-gapped / self-hosted IdP).
    try:
        ...
        if getattr(exc, "relogin_required", False):
            _fail(
                "✗ No relay identity token is available.",
                "  Configure gateway.idp and retry.",
            )
        _fail(f"✗ Could not resolve a relay identity token: {exc}")
    ...
    # Persist idempotently; save_env_value writes the sensitive values to ~/.panergos/.env and never
    ...
    from panergos_cli.config import get_env_path
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    try:
        ...
        default_root = Path(get_default_panergos_root()).resolve()
