"""Dispatcher and maintenance verbs for ``panergos kanban``: ``dispatch``,
...
from panergos_cli.kanban_output import _err, _fmt_ts, _print_json
...
_DAEMON_DEPRECATED = (
    "panergos kanban daemon: DEPRECATED — the dispatcher now runs\ninside the gateway. To use "
    "kanban:\n\n    panergos gateway start       # starts the gateway + embedded dispatcher\n\nReady "
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"successfully. Check profile health (venv, PATH, credentials) and `panergos "
                    f"kanban list --status ready` / `panergos kanban list --status blocked` for "
