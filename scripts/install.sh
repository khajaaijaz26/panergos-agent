...
# Python-driven tool session (e.g. Panergos terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:khajaaijaz26/panergos-agent.git"
REPO_URL_HTTPS="https://github.com/khajaaijaz26/panergos-agent.git"
PANERGOS_HOME="${PANERGOS_HOME:-$HOME/.panergos}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${PANERGOS_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$PANERGOS_INSTALL_DIR"
...
#   code at /usr/local/lib/panergos-agent, command at /usr/local/bin/panergos,
#   data still at /root/.panergos (PANERGOS_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$PANERGOS_HOME/.no-bundled-skills so future"
            echo "                   'panergos update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Panergos.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.panergos/panergos-agent"
            echo "                   default (root, Linux): /usr/local/lib/panergos-agent"
            echo "  --panergos-home PATH  Data directory (default: ~/.panergos, or \$PANERGOS_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Panergos installs the code under"
            echo "  /usr/local/lib/panergos-agent and links the command into"
            echo "  /usr/local/bin/panergos, with panergos-agent and panergos-acp launchers."
            echo "  Data, config, sessions, and logs still live in \$PANERGOS_HOME"
            echo "  (default /root/.panergos).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$PANERGOS_HOME/panergos-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `panergos update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Panergos-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Panergos Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install panergos command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `panergos` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $PANERGOS_HOME/panergos-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $PANERGOS_HOME/panergos-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/panergos-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $PANERGOS_HOME/panergos-agent — then preserve it)
#
# Always no-op when the user set --dir or $PANERGOS_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in PANERGOS_HOME.
    if is_termux; then
        INSTALL_DIR="$PANERGOS_HOME/panergos-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$PANERGOS_HOME/panergos-agent/.git" ]; then
            INSTALL_DIR="$PANERGOS_HOME/panergos-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/panergos-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/panergos-agent"
        ...
        # /usr/local/bin/panergos wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/panergos"
        log_info "  Data:    $PANERGOS_HOME (unchanged)"
...
# Point a Panergos-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $PANERGOS_HOME/node/bin — which is NOT on
...
# Panergos's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Panergos-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$PANERGOS_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$PANERGOS_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$PANERGOS_HOME/node/etc/npmrc"
}

get_panergos_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/panergos" ]; then
        echo "$link_dir/panergos"
    elif [ -x "$link_dir/panergos" ]; then
        echo "$link_dir/panergos"
    else
        echo "panergos"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/khajaaijaz26/panergos-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Panergos owns its own uv at $PANERGOS_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (panergos_cli/managed_uv.py) looks in the same
    # place, so install.sh and `panergos update` stay in sync.
    local _managed_uv="$PANERGOS_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $PANERGOS_HOME/bin ..."
    mkdir -p "$PANERGOS_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/panergos-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/panergos-uv-installer.$$.sh")"
    ...
    # directly into $PANERGOS_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$PANERGOS_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Panergos currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Panergos requires Python >=3.11,<3.14"
...
# replaced with the Panergos-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Panergos-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Panergos-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Panergos-managed Node from a previous run over a too-old system one.
    if [ -x "$PANERGOS_HOME/node/bin/node" ] && [ -x "$PANERGOS_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$PANERGOS_HOME/node/bin/node" --version)"; then
        export PATH="$PANERGOS_HOME/node/bin:$PATH"
        log_success "Node.js $("$PANERGOS_HOME/node/bin/node" --version) found (Panergos-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Panergos-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Panergos requires Node 22.22+, 24.11+, or 26+) — installing Panergos-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.panergos/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.panergos/node/..."
    ...
    # Place into ~/.panergos/node/ and symlink binaries into the same bin dir
    # the panergos command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$PANERGOS_HOME/node"
    mkdir -p "$PANERGOS_HOME"
    mv "$extracted_dir" "$PANERGOS_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$PANERGOS_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$PANERGOS_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$PANERGOS_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$PANERGOS_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.panergos/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Panergos install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Panergos Agent itself does not require or retain root access."
...
clone_repo() {
    ...
    # Never fetch main through an unrelated origin: preserve that checkout
    # beside the install and clone Panergos cleanly.
    if [ -d "$INSTALL_DIR/.git" ]; then
        local origin_url
        origin_url="$(git -C "$INSTALL_DIR" remote get-url origin 2>/dev/null || true)"
        origin_url="$(printf '%s' "$origin_url" | tr '[:upper:]' '[:lower:]')"
        case "$origin_url" in
            https://github.com/khajaaijaz26/panergos-agent|\
            https://github.com/khajaaijaz26/panergos-agent.git|\
            git@github.com:khajaaijaz26/panergos-agent|\
            git@github.com:khajaaijaz26/panergos-agent.git|\
            ssh://git@github.com/khajaaijaz26/panergos-agent|\
            ssh://git@github.com/khajaaijaz26/panergos-agent.git)
                ;;
            *)
                backup_dir="${INSTALL_DIR}.pre-panergos-$(date -u +%Y%m%d-%H%M%S)-$$"
                log_warn "Existing checkout belongs to a different origin: ${origin_url:-unknown}"
                log_warn "Moving it aside to $backup_dir before installing Panergos."
                mv "$INSTALL_DIR" "$backup_dir"
                ;;
        esac
    fi
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `panergos update` path
                # (#4735).
                ...
                local stash_name
                stash_name="panergos-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``panergos update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Panergos behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up panergos command..."

    if [ "$USE_VENV" = true ]; then
        PANERGOS_BIN="$INSTALL_DIR/venv/bin/python"
        PANERGOS_ENTRYPOINT="$INSTALL_DIR/panergos"
    else
        PANERGOS_BIN="$(which panergos 2>/dev/null || echo "")"
        if [ -z "$PANERGOS_BIN" ]; then
            log_warn "panergos not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$PANERGOS_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$PANERGOS_ENTRYPOINT" ]; }; then
        log_warn "Panergos launcher prerequisites not found"
    ...
    # Create a user-facing shim for the panergos command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $PANERGOS_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$PANERGOS_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/panergos"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/panergos" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$PANERGOS_BIN" "$PANERGOS_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/panergos" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/panergos"
    log_success "Installed panergos launcher → $command_link_display_dir/panergos"

    # Also expose `panergos-agent`. The `panergos-agent` console script declared in
    ...
    rm -f "$command_link_dir/panergos-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/panergos-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$PANERGOS_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/panergos-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$PANERGOS_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/panergos-agent"
    log_success "Installed panergos-agent launcher → $command_link_display_dir/panergos-agent"

    # Also expose the ACP launcher. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the console script
    # lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Panergos as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/panergos-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/panergos-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$PANERGOS_BIN" "$PANERGOS_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/panergos-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$PANERGOS_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/panergos-acp"
    log_success "Installed panergos-acp launcher → $command_link_display_dir/panergos-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "panergos command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v panergos' \
                >/dev/null 2>&1; then
        ...
        log_info "panergos not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Panergos Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Panergos Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Panergos Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for the current session so Panergos works immediately.
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.panergos directory structure (config at top level, code in subdir)
    mkdir -p "$PANERGOS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.panergos/.env (top level, easy to find)
    if [ ! -f "$PANERGOS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$PANERGOS_HOME/.env"
            log_success "Created ~/.panergos/.env from template"
        else
            touch "$PANERGOS_HOME/.env"
            log_success "Created ~/.panergos/.env"
        fi
    else
        log_info "~/.panergos/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$PANERGOS_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.panergos/config.yaml (top level, easy to find)
    if [ ! -f "$PANERGOS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$PANERGOS_HOME/config.yaml"
            log_success "Created ~/.panergos/config.yaml from template"
        fi
    else
        log_info "~/.panergos/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in panergos_cli/default_soul.py — the
    ...
    if [ ! -f "$PANERGOS_HOME/SOUL.md" ]; then
        cat > "$PANERGOS_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.panergos/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.panergos/"

    # Seed bundled skills into ~/.panergos/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `panergos update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'panergos update'." \
            > "$PANERGOS_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $PANERGOS_HOME/.no-bundled-skills"
        log_info "  Future 'panergos update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.panergos/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.panergos/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$PANERGOS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$PANERGOS_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.panergos/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$PANERGOS_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Panergos Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Panergos will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Panergos Agent browser tools — explicit browser override."
...
# install — the same closure `panergos update` installs
# (panergos_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_node_deps() {
    ...
    # Keep the checkout clean so `panergos update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `panergos tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$PANERGOS_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Panergos' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$PANERGOS_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'panergos tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `panergos tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `panergos computer-use install` on their own).
    ...
    # Same upstream installer `panergos computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Panergos install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Panergos'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'panergos tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: panergos computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'panergos setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m panergos_cli.main setup < /dev/tty
    else
        python -m panergos_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$PANERGOS_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Panergos to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$PANERGOS_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'panergos whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                PANERGOS_CMD="$(get_panergos_command_path)"
                $PANERGOS_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'panergos whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'panergos gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $PANERGOS_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $PANERGOS_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: panergos gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: panergos gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $PANERGOS_CMD gateway > "$PANERGOS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.panergos/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: panergos gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: panergos gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.panergos-bootstrap-complete, which tells the Panergos
    ...
    local marker_path="$INSTALL_DIR/.panergos-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $PANERGOS_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $PANERGOS_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $PANERGOS_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}panergos${NC}              Start chatting"
    echo -e "   ${GREEN}panergos setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}panergos config${NC}       View/edit configuration"
    echo -e "   ${GREEN}panergos config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}panergos gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}panergos update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'panergos' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'panergos' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'panergos' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$PANERGOS_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$PANERGOS_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$PANERGOS_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$PANERGOS_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Panergos` rename
...
# _purge_electron_build_cache() used by `panergos desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Panergos-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `panergos desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Panergos" ]; then
            app="$desktop_dir/release/linux-unpacked/Panergos"
        elif [ -x "$desktop_dir/release/linux-unpacked/panergos" ]; then
            app="$desktop_dir/release/linux-unpacked/panergos"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Panergos.app" \
            "$desktop_dir/release/mac/Panergos.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `panergos desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading PANERGOS_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if PANERGOS_HOME="$PANERGOS_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from panergos_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `panergos update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Panergos-managed Node
            # provisioned during prerequisites/node-deps (at $PANERGOS_HOME/node/bin)
        ...
        complete)
            ...
            # $PANERGOS_HOME. $PANERGOS_HOME is a shared data dir (it can be
            # bind-mounted into a Docker gateway too), so a stamp there gets
            # clobbered by the container's 'docker' stamp and wrongly blocks
            # 'panergos update' on this host install. See detect_install_method().
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $PANERGOS_HOME.
    # $PANERGOS_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'panergos update' on this host install.
