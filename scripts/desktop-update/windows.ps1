...
# (%PANERGOS_HOME%\panergos-setup.exe). That binary has no self-update path --
# copy_self_to_panergos_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `panergos update` refreshes
...
#     -InstallRoot <path>   repo checkout (PANERGOS_HOME\panergos-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Panergos.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .panergos-update-in-progress in place (tests)
#
...
# .panergos-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim PANERGOS_HOME\.panergos-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining PANERGOS_UPDATE_STARTED_AT from the Desktop hand-off.
# panergos_cli/update_lock.py's ancestry rule lets our
# `panergos update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Panergos.exe pid.
try {
    Add-Type -Namespace PanergosHandoff -Name Win32 -MemberDefinition @'
...
$PanergosHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $PanergosHome ".panergos-update-in-progress"
$LogDir = Join-Path $PanergosHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $PanergosHome ".panergos-update-result.json"
$script:Ui = $null
$script:UiStage = "Panergos will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale panergos-update-ui-* leftovers from interrupted
    ...
    # update marker (.panergos-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "panergos-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-PanergosStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("panergos-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [PanergosHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `panergos debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"panergos debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [PanergosHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen Panergos themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Panergos.exe before writing the replacement,
    ...
    # DO NOT spawn Panergos.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [PanergosHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [PanergosHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [PanergosHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Panergos process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [PanergosHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `panergos update` deliberately runs its build steps with stdout
# inherited (panergos_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:PANERGOS_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:PANERGOS_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:PANERGOS_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:PANERGOS_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `panergos update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (panergos_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:PANERGOS_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:PANERGOS_UPDATE_PROGRESS_LOG
}
...
if (-not ("PanergosUpdateJob" -as [type])) {
...
public static class PanergosUpdateJob {
...
function Invoke-PanergosStep([string]$Exe, [string[]]$PanergosArgs, [string]$Tag) {
    ...
    # afterwards, where `panergos debug share` picks it up.
    #
    ...
    # .panergos-update-result.json, clearing .panergos-update-in-progress,
    ...
    # Panergos" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($PanergosArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [PanergosUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `panergos update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [PanergosUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
# the page can be QA'd in any browser; PANERGOS_SELFTEST_FAIL=1 exercises the
# error state, PANERGOS_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:PANERGOS_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:PANERGOS_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:PANERGOS_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-PanergosStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Panergos install to
...
#            update log, the shape of every real `panergos update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:PANERGOS_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:PANERGOS_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:PANERGOS_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:PANERGOS_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "panergos-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "panergos-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "panergos-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "panergos-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "panergos-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "panergos-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "panergos-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "panergos-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `panergos update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-PanergosStep $powershell @(
    ...
    $flood = Invoke-PanergosStep $powershell @(
    ...
    $stall = Invoke-PanergosStep $powershell @(
    ...
    try {
        $logstall = Invoke-PanergosStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-PanergosStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-PanergosStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:PANERGOS_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    # Desktop launches us from PANERGOS_HOME; pin the process directory to the
    ...
    if ($SelfTestWorkingDirectory) {
        ...
        $probe = Invoke-PanergosStep $probeExe @("-NoProfile", "-Command", "[Environment]::CurrentDirectory") "cwd"
    ...
    Publish-UiProgress "Waiting for Panergos to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Panergos window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Panergos fully and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    Publish-UiProgress "Preparing Panergos files"
    $shim = Join-Path $InstallRoot "venv\Scripts\panergos.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Panergos install open (venv\Scripts\panergos.exe locked after 20s). Nothing was changed. Close other Panergos windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the panergos.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\panergos.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_panergos_exe must first rename the running panergos.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that panergos.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Panergos window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Panergos.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m panergos_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Panergos installer or `panergos doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "panergos_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Panergos windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m panergos_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed panergos predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-PanergosStep $pythonExe $updateArgs "update"
    Write-HandoffLog "panergos update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-PanergosUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `panergos update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "panergos update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-PanergosStep $pythonExe @("-m", "panergos_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        $verifyCode = "import panergos_cli.main; from panergos_cli.desktop_update_verify import verify_windows_desktop_update; verify_windows_desktop_update()"
        $verify = Invoke-PanergosStep $pythonExe @("-c", $verifyCode) "verify"
        if ($verify.Code -ne 0) {
            $finalCode = 8
            $finalMsg = "The updated Panergos runtime or Desktop build failed verification. Repair the installation and review antivirus quarantine before retrying."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    }
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `panergos desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `panergos debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Panergos is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Panergos was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Panergos."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Panergos"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Panergos to finish (it could not restart itself)."
