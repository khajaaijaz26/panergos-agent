#!/usr/bin/env bash
# Canonical test runner for panergos-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.panergos/...)
#
# Usage:
...
# (PANERGOS_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.panergos/panergos-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.panergos/panergos-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${PANERGOS_PYTHON:-}" ] && [ -x "$PANERGOS_PYTHON" ] \
    && "$PANERGOS_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: PANERGOS_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `panergos` binary rather
  # than the devShell hook.
  PYTHON="$PANERGOS_PYTHON"
  echo "▶ no local venv — using Nix dev venv via PANERGOS_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and PANERGOS_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.panergos/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.panergos"
...
#
#   * PANERGOS_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * PANERGOS_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     PANERGOS_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# PANERGOS_RUN_SLOW_PET_TESTS / PANERGOS_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no PANERGOS_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in PANERGOS_TEST_IMAGE PANERGOS_TEST_WORKERS PANERGOS_TEST_PATHS \
  PANERGOS_TEST_FILE_TIMEOUT PANERGOS_TEST_FILE_RETRIES PANERGOS_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${PANERGOS_RUN_SLOW_PET_TESTS:+PANERGOS_RUN_SLOW_PET_TESTS="$PANERGOS_RUN_SLOW_PET_TESTS"} \
  ${PANERGOS_E2E_BROWSER:+PANERGOS_E2E_BROWSER="$PANERGOS_E2E_BROWSER"} \
