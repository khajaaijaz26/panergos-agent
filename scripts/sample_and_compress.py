...
# No vendor dataset is selected implicitly. Pass ``--datasets`` explicitly.
...
def load_dataset_from_hf(dataset_name: str) -> List[Dict[str, Any]]:
    ...
    Args:
        dataset_name: Hugging Face dataset name (e.g., "organization/dataset-name")
...
def main(
    ...
    Args:
        ...
        datasets: Comma-separated dataset names (required unless using existing downloads)
    ...
    if not skip_download and not dataset_list:
        print("❌ Pass --datasets=organization/dataset[,organization/dataset].")
        return
    ...
    print(f"   huggingface-cli upload <organization>/{output_name} {final_output}")
