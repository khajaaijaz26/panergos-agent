...
# tree generates llms.txt — the index every LLM (Panergos included, via the
# panergos-agent skill) reads to learn what Panergos can do — and
...
_PY_RELEVANT_CONTRACT_FILES = {
    ...
    # tests/panergos_cli/test_desktop_slash_registry.py
...
_MCP_CATALOG_FILES = {"panergos_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Panergos-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``panergos serve`` backend, the Docker image)
