...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[panergos] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[panergos`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[panergos`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        PanergosHome = $(if ($paths) { $paths.panergos_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\panergos\panergos-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-panergos'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-panergos') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
