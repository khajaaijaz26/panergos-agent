...
# changing PATH, installing software, or touching the user's Panergos home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("panergos-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$PanergosHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -PanergosHome $PanergosHome -InstallDir $InstallDir
