#!/usr/bin/env python3
"""Build the Panergos Skills Index — a bounded multi-source JSON catalog.

This script crawls every skill source (skills.sh, GitHub taps, official,
clawhub, lobehub) and writes a JSON index with source metadata. The index is
served as a static file on the docs site so that
`panergos skills search/install` can use it without hitting the GitHub API.
...
# Ensure PANERGOS_HOME is set (needed by tools/skills_hub*.py imports)
os.environ.setdefault("PANERGOS_HOME", os.path.join(os.path.expanduser("~"), ".panergos"))
...
_LEGACY_IDENTITY = re.compile(
    rf"{_LEGACY_PRODUCT}|{_LEGACY_VENDOR}research|"
    rf"(?<![a-z]){_LEGACY_VENDOR}(?![a-z])",
    re.IGNORECASE,
)
...
def main():
    print("Building Panergos Skills Index...", flush=True)
    ...
    # Bulk GitHub path resolution costs two API calls for each of thousands of
    # skills.sh repositories, and ClawHub owner enrichment needs one detail call
    # per skill. Both are optional: identifiers remain installable and source
    # adapters resolve details on demand, so the static bootstrap omits them.
    ...
    before_identity_filter = len(deduped)
    deduped = [skill for skill in deduped if not _contains_legacy_identity(skill)]
    removed_identity_entries = before_identity_filter - len(deduped)
    if removed_identity_entries:
        print(f"  Excluded {removed_identity_entries} entries tied to retired identities")
