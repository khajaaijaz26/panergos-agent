...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^Panergos\s+(Agent|Audit)$", re.IGNORECASE),
    ...
    re.compile(r".*\[bot\]$", re.IGNORECASE),             # any GitHub [bot] suffix (panergos-seaeye[bot] etc.)
...
IGNORED_EMAILS = {
    ...
    "panergos-audit@example.com",
    "panergos@habibilabs.dev",
    "omx@oh-my-codex.dev",
    "codex@openai.com",
...
def gh_pr_list():
    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "list",
                "--repo", "khajaaijaz26/panergos-agent",
                "--state", "merged",
