"""Run a real Panergos CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_panergos_executable(panergos_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "panergos",
        Path(".venv") / "Scripts" / "panergos.exe",
    ):
        candidate = panergos_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("panergos")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Panergos executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Panergos source checkout containing .venv/bin/panergos",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated PANERGOS_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["panergos.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['panergos.client.active']}"
        )
    [model] = by_name["panergos.model_route.count"]
    expected_model = {
        "name": "panergos.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['panergos.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["panergos.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['panergos.task_run.started']}"
        )
    [terminal] = by_name["panergos.task_run.finished"]
    ...
    [tool] = by_name["panergos.tool_call.count"]
    ...
    lifecycle = by_name["panergos.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["panergos.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Panergos development environment requires jsonschema"
        ) from exc
    ...
    if metrics["panergos.client.active"] != [
        {
            "name": "panergos.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['panergos.client.active']}"
        )
    [model] = metrics["panergos.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['panergos.model_route.count']}"
        )
    [terminal] = metrics["panergos.task_run.finished"]
    ...
    [tool] = metrics["panergos.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["panergos.skill.lifecycle.count"]
    ...
    loads = metrics["panergos.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    panergos_repo = args.panergos_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    panergos = _resolve_panergos_executable(panergos_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="panergos-relay-shared-metrics-"))
    home = root / "panergos-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["PANERGOS_HOME"] = str(home)
        python_paths = [str(panergos_repo)]
    ...
    (root / "panergos.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "panergos.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Panergos exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Panergos model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Panergos did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Panergos did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "panergos.shared_metrics.v2.schema.json",
    )

    print("Panergos -> NeMo Relay shared-metrics smoke test passed")
