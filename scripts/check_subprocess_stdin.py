...
When Panergos runs in TUI mode, the gateway child process communicates with
...
# ``get_panergos_home() / "plugins"`` (user) and ``./.panergos/plugins/`` (project,
# gated behind ``PANERGOS_ENABLE_PROJECT_PLUGINS``) — see
# ``panergos_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``panergos_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import panergos_constants (this script
    ...
    # plugins in get_panergos_home()/plugins/ and project plugins in
    # ./.panergos/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_panergos_home() / "plugins"]
    if os.environ.get("PANERGOS_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".panergos" / "plugins")
