 ...
 *   The artifact published on the website right now -- Panergos-Setup.exe has
 ...
 * @typedef {InstallMethod | 'panergos-update' | 'open-app-update' | 'panergos-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; panergos-desktop-app-update starts the app
 *   via `panergos desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      // irm https://raw.githubusercontent.com/khajaaijaz26/panergos-agent/main/scripts/install.ps1 | iex
      { method: 'installer-script' },
      // The same one-liner with -IncludeDesktop: builds Panergos.exe AND
      ...
      // Website Panergos-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via the legacy `panergos desktop` shim so
      // sampled pre-Panergos releases remain upgradable.
      { method: 'panergos-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published Panergos-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `panergos desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'panergos-desktop-app-update';
}
