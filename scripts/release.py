...
VERSION_FILE = REPO_ROOT / "panergos_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@panergos.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "75556242+webtecnica@users.noreply.github.com": "webtecnica",  # PR #63360 salvage
    ...
    "kavi@local.panergos": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level PANERGOS_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (panergos journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "panergos.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.panergos/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "brett@personalfinancelab.com": "brett539",  # PR #49369 salvage (cap Telegram initialize() with asyncio.wait_for(PANERGOS_TELEGRAM_INIT_TIMEOUT, default 30s) per attempt so an unreachable fallback-IP connect chain can't block gateway startup indefinitely; add WARNING progress logs before DoH discovery and each connect attempt)
    ...
    "hodlclone@gmail.com": "HODLCLONE",  # PR #49351 salvage
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows panergos-shim resolution + prefer --update on recovery; #52378)
    ...
    "panergos@marian.local": "Schrotti77",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on panergos mcp add)
    ...
    "panergos@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@panergos-agent": "teknium1",
    ...
    "github@nadyapanergos.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (PANERGOS_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (PANERGOS_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: panergos-eval community link)
    ...
    "vigo@panergos": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.panergos@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Panergos runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level PANERGOS_HOME state files)
    ...
    "zhchl@panergos-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Panergos
    ...
    # Keep the bootstrap installer (Panergos-Setup.dmg CFBundleShortVersionString)
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/khajaaijaz26/panergos-agent",
    ...
    lines.append(f"# Panergos Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Panergos Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Legacy upstream release tooling (publish disabled)")
    ...
    if args.publish:
        raise SystemExit(
            "Legacy CalVer publishing is disabled for Panergos; "
            "use scripts/release_distribution.py --publish"
        )
    ...
    print(f"{'='*60}")
    print("  Panergos Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Panergos Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Panergos Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Panergos Agent v{new_version} ({calver_date})' "
