# ============================================================================
# Panergos Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$PanergosHome = $(if ($env:PANERGOS_HOME) { $env:PANERGOS_HOME } else { "$env:LOCALAPPDATA\panergos" }),
    [string]$InstallDir = $(if ($env:PANERGOS_HOME) { "$env:PANERGOS_HOME\panergos-agent" } else { "$env:LOCALAPPDATA\panergos\panergos-agent" }),
    ...
    # builds apps/desktop into a launchable Panergos.exe.
    #
    # Why opt-in:
    #   * Panergos-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Panergos.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Panergos.exe
    ...
    #     `panergos desktop` already builds on demand.
...
# PANERGOS_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[panergos] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'PanergosInstall.LongPath').Type) {
            Add-Type -Namespace 'PanergosInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [PanergosInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -PanergosHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('PanergosHome')) {
    $PanergosHome = ConvertTo-LongPath $PanergosHome
} else {
    $PanergosHome = ConvertTo-LongPath $(
        if ($env:PANERGOS_HOME) { $env:PANERGOS_HOME } else { "$env:LOCALAPPDATA\panergos" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:PANERGOS_HOME) { "$env:PANERGOS_HOME\panergos-agent" } else { "$env:LOCALAPPDATA\panergos\panergos-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: PanergosHome=$PanergosHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:khajaaijaz26/panergos-agent.git"
$RepoUrlHttps = "https://github.com/khajaaijaz26/panergos-agent.git"
$PythonVersion = "3.11"
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $PanergosHome)) {
        New-Item -ItemType Directory -Force -Path $PanergosHome | Out-Null
    }
    $envFile = Join-Path $PanergosHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `panergos update`'s npx
    ...
    $prefixDir = Join-Path $PanergosHome "node"
...
function Install-Uv {
    # Panergos owns its own uv at $PanergosHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (panergos_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `panergos update` stay in sync.
    $managedUv = Join-Path $PanergosHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $PanergosHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $PanergosHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $PanergosHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $PanergosHome "bin"
        ...
        # (panergos_cli/managed_uv.py looks only at $PanergosHome\bin\uv.exe).
...
# Put the Panergos-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone panergos-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Panergos-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Panergos-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("panergos-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and PANERGOS_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Panergos belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # panergos_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".panergos-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Panergos-managed interpreter
    ...
    #
    # Under Panergos-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Panergos-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Panergos setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Panergos uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\panergos\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Panergos needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Panergos copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\panergos\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``PANERGOS_GIT_BASH_PATH`` (User scope) so Panergos can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Panergos-managed PortableGit install instead..."
    }

    # Download PortableGit into $PanergosHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $PanergosHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Panergos features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Panergos needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``PANERGOS_GIT_BASH_PATH`` (User env scope) so Panergos can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $PanergosHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $PanergosHome\git\usr\bin\bash.exe
    $candidates += "$PanergosHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$PanergosHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("PANERGOS_GIT_BASH_PATH", $candidate, "User")
            $env:PANERGOS_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set PANERGOS_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Panergos may not find Git Bash."
    Write-Info "If needed, set PANERGOS_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Panergos-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Panergos requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Panergos requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Panergos-managed Node.js installation instead..."

    # Prefer a Panergos-managed Node from a previous run over a too-old system one.
    $managedNode = "$PanergosHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$PanergosHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$PanergosHome\node"
        Write-Success "Node.js $version found (Panergos-managed)"
        ...
        Update-ManagedNpm "$PanergosHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Panergos-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $PanergosHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $PanergosHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\panergos-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$PanergosHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$PanergosHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$PanergosHome\node") {
                    try {
                        Rename-Item "$PanergosHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Panergos-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$PanergosHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $PanergosHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\panergos-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if (Test-Path "$InstallDir\.git") {
            ...
            try {
                ...
                if ($revParseOk -and $statusOk -and $hasCommit) {
                    $repoValid = $true
                    $originRaw = & git -c windows.appendAtomically=false remote get-url origin 2>$null
                    $originUrl = if ($null -eq $originRaw) { "" } else { ($originRaw | Select-Object -First 1).ToString().Trim() }
                    $isPanergosOrigin = $originUrl -match '^(https://github\.com/khajaaijaz26/panergos-agent(?:\.git)?|git@github\.com:khajaaijaz26/panergos-agent(?:\.git)?|ssh://git@github\.com/khajaaijaz26/panergos-agent(?:\.git)?)$'
                    if (-not $isPanergosOrigin) {
                        $foreignOrigin = if ($originUrl) { $originUrl } else { "unknown" }
                        $repoValid = $false
                    }
                }
            } catch {}
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `panergos
                    # update` path (#4735).
                    ...
                    $stashName = "panergos-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. panergos-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``panergos update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Panergos behaves unexpectedly."
                        } else {
            ...
            $backupKind = if ($foreignOrigin) { "pre-panergos" } else { "broken" }
            $backupDir = "$InstallDir.$backupKind-" + (Get-Date -Format "yyyyMMdd-HHmmss") + "-$PID"
            if ($foreignOrigin) {
                Write-Warn "Existing checkout belongs to a different origin: $foreignOrigin"
            } else {
            ...
            try {
                ...
                Write-Info "terminals, running panergos processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/khajaaijaz26/panergos-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/khajaaijaz26/panergos-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/khajaaijaz26/panergos-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\panergos-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\panergos-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `panergos update` (see the notes at the shared clone-path
    ...
    # next `panergos update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Panergos-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Panergos-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running panergos process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running panergos processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Panergos_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (panergos.exe) plus its child tree.
            & taskkill /F /T /IM panergos.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM panergos.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m panergos_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not panergos.exe.
            ...
            # Panergos children re-exec through .panergos-runtime, so killing only
            ...
            # user (see panergos_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Panergos " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `panergos update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Panergos (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `panergos.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'panergos-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install panergos-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and panergos.exe broken with
    ...
    # here, not 30 seconds later when the user runs `panergos`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Panergos processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Panergos processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Panergos and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register panergos.exe in dist-info/RECORD but fail to
        ...
        # `panergos` command while panergos-agent.exe / panergos-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m panergos_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `panergos dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\panergos_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `panergos dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `panergos dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: panergos_cli/web_server.py"
        }
    }
...
function Install-PanergosCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the Panergos launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring panergos.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "panergos.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the panergos command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # panergos_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("panergos", "panergos-acp")) {
    ...
    $requiredExe = Join-Path $Destination "panergos.exe"
    $requiredCmd = Join-Path $Destination "panergos.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the panergos command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up panergos command..."
    
    if ($NoVenv) {
        $panergosBin = "$InstallDir"
    } else {
        # $PanergosHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `panergos update`'s autostash
        ...
        # installer staged under panergos-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-PanergosCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $panergosBin = "$PanergosHome\bin"
        Install-PanergosCommandLaunchers -Root $InstallDir -Destination $panergosBin | Out-Null
    }
    ...
    #   panergos-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The panergos-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept panergos via $panergosBin)"
        }
    }
    
    if ($currentPath -notlike "*$panergosBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$panergosBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $panergosBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set PANERGOS_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\panergos instead
    # of the Unix default ~/.panergos
    $currentPanergosHome = [Environment]::GetEnvironmentVariable("PANERGOS_HOME", "User")
    if (-not $currentPanergosHome -or $currentPanergosHome -ne $PanergosHome) {
        [Environment]::SetEnvironmentVariable("PANERGOS_HOME", $PanergosHome, "User")
        Write-Success "Set PANERGOS_HOME=$PanergosHome"
    }
    $env:PANERGOS_HOME = $PanergosHome
    
    # Update current session
    $env:Path = "$panergosBin;$env:Path"
    
    Write-Success "panergos command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.panergos-bootstrap-complete which tells the Panergos
    ...
    # Panergos-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `panergos update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".panergos-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the PANERGOS_HOME directory structure ($PanergosHome, default %LOCALAPPDATA%\panergos)
    New-Item -ItemType Directory -Force -Path "$PanergosHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PanergosHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PanergosHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PanergosHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PanergosHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PanergosHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PanergosHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PanergosHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PanergosHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$PanergosHome\.env"
    ...
    # Create config.yaml
    $configPath = "$PanergosHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Panergos's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$PanergosHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in panergos_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $PanergosHome"
    
    # Seed bundled skills into $PanergosHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $PanergosHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $PanergosHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$PanergosHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $PanergosHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Panergos-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'panergos setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\panergos-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\panergos-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\panergos-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `panergos tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $PanergosHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Panergos' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Panergos' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'panergos tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `panergos
...
# paths still lazy-install via install_cua_driver() (panergos_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `panergos computer-use install` runs. Bounded
        ...
        # Panergos' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'panergos tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: panergos computer-use install"
            }
        } else {
...
# the final `electron` -> `Panergos` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Panergos.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_panergos_desktop command
    # resolves apps/desktop/release/win-unpacked/Panergos.exe directly,
    ...
    # enforces the supported Node lines and prepends the Panergos-managed Node to
    ...
    # The --dir mode produces an unpacked Panergos.exe in
    ...
    # are NOT trying to work around). The Panergos icon + product name are
    # stamped onto Panergos.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\panergos-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Panergos.exe",
        "$desktopDir\release\win-arm64-unpacked\Panergos.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Panergos.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Panergos icon + identity are stamped onto Panergos.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, panergos-agent#38216).
    ...
    #    Panergos.exe. We deliberately do NOT point them at `panergos desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Panergos.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Panergos.lnk')
        )
        ...
        # with the Panergos icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.panergos/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'panergos setup'."
        return
    }
    ...
    # Run panergos setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m panergos_cli.main setup
    } else {
        python -m panergos_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $panergosCmd = "$InstallDir\venv\Scripts\panergos.exe"
    if (-not (Test-Path $panergosCmd)) {
        $panergosCmd = "$InstallDir\venv\Scripts\panergos.exe"
    }
    if (-not (Test-Path $panergosCmd)) {
        $panergosCmd = "panergos"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$PanergosHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'panergos whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: panergos gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$PanergosHome\logs\gateway.log"
            Start-Process -FilePath $panergosCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$PanergosHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: panergos gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: panergos gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$PanergosHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$PanergosHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$PanergosHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$PanergosHome\panergos-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   panergos            " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   panergos setup      " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   panergos config     " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   panergos config edit" -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   panergos gateway    " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   panergos update     " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Panergos repository";          Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.panergos-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (Panergos-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Panergos to PATH";              Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/khajaaijaz26/panergos-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
