...
Run after changing any ``desktop=`` value or alias in ``panergos_cli/commands.py``;
``tests/panergos_cli/test_desktop_slash_registry.py`` fails until the committed
...
def render() -> str:
    ...
    from panergos_cli.commands import desktop_surface_registry
