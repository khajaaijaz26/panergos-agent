...
from panergos_constants import OPENROUTER_BASE_URL, get_panergos_home
from agent.retry_utils import jittered_backoff
from panergos_cli.env_loader import load_panergos_dotenv

# Load .env from PANERGOS_HOME first, then project root as a dev fallback.
load_panergos_dotenv(panergos_home=get_panergos_home(), project_env=Path(__file__).parent / ".env")
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: panergos setup")
