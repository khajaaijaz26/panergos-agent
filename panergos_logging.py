"""Centralized logging setup for Panergos Agent.
...
# in Panergos (TUI, gateway, hy_memory, MCP servers, CLI commands all log) —
...
from panergos_constants import get_config_path, get_panergos_home, mkdir_under_panergos_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_panergos_session_injector", False):
        return
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.panergos_home = str(get_panergos_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.panergos_home = ""  # type: ignore[attr-defined]
        return record
    _session_record_factory._panergos_session_injector = True  # type: ignore[attr-defined]
...
# Logger name prefixes per component; used by _ComponentFilter and ``panergos logs --component``.
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "panergos_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("panergos_cli", "cli"),
    "cron": ("cron",),
    "gui": ("panergos_cli.web_server", "panergos_cli.pty_bridge", "tui_gateway", "uvicorn"),
}


def setup_logging(
    *,
    panergos_home: Optional[Path] = None,
    ...
    """Configure the Panergos logging subsystem; returns the ``logs/`` directory.
    ...
    home = panergos_home or get_panergos_home()
    log_dir = mkdir_under_panergos_home(home / "logs")
...
def setup_verbose_logging() -> None:
    ...
    if any(getattr(h, "_panergos_verbose", False) for h in root.handlers):
        return
    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._panergos_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from panergos_cli.config import is_managed
...
def _new_file_handler(
    ...
    mkdir_under_panergos_home(path.parent)
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their Panergos home.
    ...
    def __init__(self, existing: RotatingFileHandler, profile_homes: Sequence[Path]) -> None:
        ...
        self._panergos_routed_log_path = resolved
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "panergos_home", "")
        try:
...
def _register_queued_handler(handler: logging.Handler) -> None:
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._panergos_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    if listener is None:
        return
    t = threading.Thread(target=lambda: _quietly(listener.stop), name="panergos-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_panergos_queue", False):
...
def _add_rotating_handler(
    ...
    *,
    ...
    for existing in _queued_file_handlers:
        ...
        if getattr(existing, "_panergos_routed_log_path", None) == resolved or (
...
def _read_logging_config():
    ...
    try:
        ...
        # can pin logging.*) so this reuses panergos_cli.main's early parse (one config.yaml parse
        # per process); fall back to a direct parse for bare panergos_logging consumers.
        try:
