...
When any profile carries ``ui_meta['panergos-bots']`` in profile.yaml (Bot-Mode-managed),
...
def _default_home() -> str:
    """Ambient process PANERGOS_HOME (env, else the platform default) as a string."""
...
def _panergos_root(home: Path) -> Path:
    """Root ~/.panergos for both the default profile and named profiles."""
...
DEFAULT_BOT_HANDLE = "panergos"
DEFAULT_BOT_HANDLE_ALIASES = frozenset({DEFAULT_BOT_HANDLE, "panergos"})


def _handle(name: str) -> str:
    # Keep the default profile callable through a stable product-facing handle.
    return DEFAULT_BOT_HANDLE if name == "default" else name
...
def _bots_meta(data: dict | None) -> dict | None:
    """The ``ui_meta['panergos-bots']`` block of a parsed profile.yaml, if a dict."""
    ui_meta = data.get("ui_meta") if data else None
    bots = ui_meta.get("panergos-bots") if isinstance(ui_meta, dict) else None
    return bots if isinstance(bots, dict) else None


def _is_bot_managed(profile_dir: Path) -> bool:
    return _bots_meta(_read_yaml_dict(profile_dir / "profile.yaml", "panergos-bots")) is not None
...
def is_bot_mode_managed(home: str | os.PathLike | None = None) -> bool:
    ...
    return _swallow(lambda: _any_managed(_panergos_root(_resolve_home(home))), False)
...
def _peers(root: Path) -> list[str]:
    """Registered peer gateway names (``panergos peer``) from config.yaml, read
...
def _peer_paragraph(root: Path) -> str:
    ...
    return (
        ...
        "for the peer's main agent). Run `panergos peer list` for the live "
        "peer list."
    )


def _build_section(home: Path) -> str:
    root = _panergos_root(home)
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each panergos profile is an agent teammate with "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    the AGENT'S OWN resolved home (session-db derived), not ambient PANERGOS_HOME — build threads
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    try:
        ...
        token = set_panergos_home_override(str(resolved))
        try:
