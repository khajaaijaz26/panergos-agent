...
local server > environment). Facade-owned state and helpers
...
def _transcribe_openai(
    ...
    def _run(client):
        ...
        with tempfile.TemporaryDirectory(prefix="panergos-stt-") as work_dir:
            try:
...
def _transcribe_xai(
    ...
    if not api_key:
        return _error_result("No xAI credentials found. Configure xAI OAuth in `panergos model` or set XAI_API_KEY")
    ...
    def _post() -> Any:
        from tools.xai_http import panergos_xai_user_agent
        ...
        def _post_transcription(bearer: str, endpoint_base_url: str):
            headers = {"Authorization": f"Bearer {bearer}", "User-Agent": panergos_xai_user_agent()}
...
def _transcribe_deepinfra(
    ...
    """Resolve DeepInfra credentials/model (shared ``panergos_cli.models`` helpers), then delegate to :func:`_transcribe_openai`."""
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    """Resolve direct or local OpenAI-compatible credentials for STT."""
