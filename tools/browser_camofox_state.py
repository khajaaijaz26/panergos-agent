"""Panergos-managed Camofox state helpers.

With managed persistence enabled, Panergos sends a deterministic userId derived from the
...
def get_camofox_state_dir() -> Path:
    ...
    return get_panergos_home() / "browser_auth" / "camofox"


def get_camofox_identity(task_id: Optional[str] = None) -> Dict[str, str]:
    """Stable Panergos-managed Camofox identity: userId is profile-scoped, session key is
    ...
    return {"user_id": f"panergos_{user_digest}", "session_key": f"task_{session_digest}"}
