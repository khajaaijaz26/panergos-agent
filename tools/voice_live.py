"""GPT-Live voice chat mode: the full-duplex voice frontend that delegates to Panergos.
...
delegates every real request to Panergos as its *client-delegation* backend. Panergos stays the
...
* the renderer turns each ``session.delegation.created`` into a normal ``prompt.submit`` on the
  ...
  ``session.commentary.append`` — Panergos' answer is what the voice speaks.
...
# backend (Panergos) carries the real instructions, tools and memory.
LIVE_PERSONA = (
    "You are Panergos, a calm and friendly voice assistant. Speak naturally at an unhurried pace. "
    ...
    "Delegation policy:\n"
    "Backend tools:\n"
    "- Panergos agent: a full AI agent with tools — it can run commands, read and edit files, "
...
def _resolve_credentials(live: Dict[str, Any]) -> tuple[str, str]:
    ...
    This mode uses a direct provider key.
    """
