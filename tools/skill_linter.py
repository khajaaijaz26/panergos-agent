...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    panergos_meta = meta.get("panergos") if isinstance(meta, dict) else None
    if not isinstance(panergos_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.panergos.{tags, related_skills}.")
    elif "tags" not in panergos_meta:
        yield _warn("missing-metadata", "metadata.panergos.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("panergos", "agent", "panergos agent") and (
        author != "Panergos Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Panergos Agent' (proper caps) "
