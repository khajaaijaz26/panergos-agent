...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    from panergos_cli.env_loader import secret_source_names
    env = {
    ...
    for key in secret_source_names():
        ...
        if value is not None:
            env[key] = value
    for key in ("PANERGOS_KANBAN_DB", "PANERGOS_KANBAN_BOARD"):
...
def _node_fallback(command: str) -> str:
    ...
    panergos_home = os.path.expanduser(os.getenv("PANERGOS_HOME", os.path.join(home, ".panergos")))
    # /usr/local/bin: canonical Node location (from-source Linux, Panergos Docker image, Intel Homebrew),
    # needed when a hand-authored env.PATH omits it — npx's shebang re-execs /usr/bin/env node.
    candidates = (os.path.join(panergos_home, "node", "bin", command), os.path.join(home, ".local", "bin", command),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``panergos_constants._candidate_node_command_names``). ``windows``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    for nothing (~48 MB private memory per MCP server, measured); Panergos already supervises the
