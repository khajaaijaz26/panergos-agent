...
refresh; this module supplies ``PanergosTokenStorage`` (on-disk persistence), the localhost callback
listener and ``build_oauth_auth()`` (legacy entry point). client_id is Panergos' Client ID Metadata
...
from panergos_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__panergos_user_skipped__"


def _get_token_dir(panergos_home: str | Path | None = None) -> Path:
    """``PANERGOS_HOME/mcp-tokens/`` — per-profile token directory."""
    from panergos_constants import get_panergos_home

    return Path(panergos_home if panergos_home is not None else get_panergos_home()) / "mcp-tokens"
...
def _cached_client_info(storage: "PanergosTokenStorage | None") -> dict | None:
...
def _cached_redirect(storage: "PanergosTokenStorage | None") -> "tuple[str | None, int | None]":
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    ``lead`` is the boundary-specific first sentence; this helper appends the shared, actionable ``panergos
    mcp login`` next-step so the guidance wording lives in one place across every non-interactive OAuth
    boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `panergos mcp login <server>` interactively to (re)authorize, then restart or reload the gateway."
        )
...
class PanergosTokenStorage:
    """Persist OAuth state as ``PANERGOS_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ``.client.json`` (client info), ``.meta.json`` (server metadata), ``.cimd-off`` (CIMD refused)."""

    def __init__(self, server_name: str, *, panergos_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._panergos_home = Path(panergos_home) if panergos_home is not None else None
    ...
    def _path(self, suffix: str) -> Path:
        return _get_token_dir(self._panergos_home) / f"{self._server_name}{suffix}"
    ...
    def _fixup_loaded_tokens(self, data: dict) -> None:
        # ``panergos_issuer`` is Panergos bookkeeping, not an SDK OAuthToken field: pop before validation.
        self.loaded_issuer = data.pop("panergos_issuer", None)
    ...
    async def set_tokens(self, tokens: "OAuthToken") -> None:
        ...
        if self._bound_issuer:  # which authorization server granted these tokens (never sent on the wire)
            payload["panergos_issuer"] = self._bound_issuer
    ...
    def stamp_issuer(self, issuer: str) -> None:
        """Backfill ``panergos_issuer`` onto a pre-binding token file: adopt the currently discovered
        issuer once instead of forcing a re-login, so the *next* read is protected."""
        data = _read_json(self._tokens_path())
        if data is None or data.get("panergos_issuer"):
            return
        data["panergos_issuer"] = str(issuer)
        try:
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._panergos_home)
...
def _make_callback_handler() -> tuple[type, dict]:
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = ("<h2>Authorization Successful</h2><p>You can close this tab and return to Panergos.</p>" if parsed["code"]
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `panergos mcp login <server>` later to authenticate, "
...
_SSH_HINT_LOOPBACK = (
    ...
    "\n"
    "  See: https://khajaaijaz26.github.io/panergos-agent/docs/guides/oauth-over-ssh\n")
...
def _callback_outcome(result: dict, cimd_url: str | None):
    ...
    if result["auth_code"] is None:
        hint = (
            ...
            f"server rejected Panergos' Client ID Metadata Document ({cimd_url}); set ``cimd: false`` under that "
...
PanergosOAuthClientProvider: Any = None


def remove_oauth_tokens(server_name: str, *, panergos_home: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    PanergosTokenStorage(server_name, panergos_home=panergos_home).remove()
    logger.info("OAuth tokens removed for '%s'", server_name)


# CIMD (OAuth Client ID Metadata Documents): the client_id IS an HTTPS URL the server fetches for our
# name/logo/redirect URIs, replacing per-install DCR. The SDK does the protocol; Panergos only decides
# eligibility. Published from ``website/static/oauth/client-metadata.json``; the github.io origin is
# deliberate because authorization servers may refuse redirects while fetching a client document.
_CIMD_CLIENT_METADATA_URL = "https://khajaaijaz26.github.io/panergos-agent/docs/oauth/client-metadata.json"
...
def _server_declined_cimd(storage: "PanergosTokenStorage | None") -> bool:
    """True when cached metadata shows this server doesn't advertise CIMD. The SDK decides CIMD vs DCR
    in its 401 branch — after Panergos must fix the redirect URI — so cached metadata closes the gap;
...
def _maybe_use_cimd(cfg: dict, storage: "PanergosTokenStorage | None" = None) -> "tuple[str, int] | None":
...
def _configure_callback_port(cfg: dict, storage: "PanergosTokenStorage | None" = None) -> int:
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    metadata_kwargs: dict[str, Any] = {
        "client_name": cfg.get("client_name", "Panergos Agent"),
...
def _invalidate_tokens_on_client_change(
    storage: "PanergosTokenStorage", new_client_id: str, new_client_secret: str | None) -> None:
    ...
    if removed:
        logger.warning(
            ...
            "the previous client. Re-authorize with: panergos mcp login %s",
            storage._server_name, old_client_id, new_client_id, storage._server_name)


def _maybe_preregister_client(storage: "PanergosTokenStorage", cfg: dict, client_metadata: "OAuthClientMetadata") -> None:
...
def humanize_oauth_registration_error(
    ...
    when the user overrode it or an older Panergos is running."""
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None = None) -> "OAuthClientProvider | None":
    ...
    global PanergosOAuthClientProvider
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"requires browser authorization. Run `panergos mcp login {server_name}` interactively first to complete "
            "initial authorization, then cached tokens will be reused.")
    kwargs = build_provider_kwargs(cfg, storage, ssh_proxy_hint=True)
    if PanergosOAuthClientProvider is None:
        from tools.mcp_oauth_provider import PanergosProviderMixin

        PanergosOAuthClientProvider = type("PanergosOAuthClientProvider", (PanergosProviderMixin, _sdk_class("OAuthClientProvider")), {
            "__doc__": "SDK provider plus Panergos' token-endpoint fixes (see ``PanergosProviderMixin``).",
            "__module__": __name__, "_panergos_logger": logger})
    return PanergosOAuthClientProvider(server_url=server_url, **kwargs)
