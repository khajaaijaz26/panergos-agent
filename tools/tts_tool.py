...
def _get_default_output_dir() -> str:
    ...
    return str(get_panergos_dir("cache/audio", "audio_cache"))
...
def _default_output_dir() -> str:
    ...
    PANERGOS_HOME and later scope requests to a different profile via
    ``panergos_constants.set_panergos_home_override()`` — a frozen module constant keeps writing synthesized
    ...
    re-resolve from the live profile-scoped PANERGOS_HOME on every call.
    """
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("panergos_cli.config not available, using default TTS config")
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    """Configured provider or the free default."""
...
_BUILTIN_DISPATCH: Dict[str, tuple] = {
    ...
    "mistral": (lambda: _importable(_import_mistral_client), "Mistral Voxtral TTS", "_generate_mistral_tts",
                ...
                "Run `panergos setup` to install Mistral support."),
    ...
    "neutts": (lambda: _check_neutts_available(), "NeuTTS (local)", "_generate_neutts",
               ...
               "Run panergos setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."),
    "kittentts": (lambda: _importable(_import_kittentts), "KittenTTS (local, ~25MB)", "_generate_kittentts",
                  ...
                  "Run 'panergos setup tts' and choose KittenTTS, or install manually: "
    ...
    "piper": (lambda: _importable(_import_piper), "Piper (local)", "_generate_piper_tts",
              ...
              "Run 'panergos tools' and select Piper under TTS, or install manually: "
...
def _session_platform() -> tuple:
    ...
    platform = get_session_env("PANERGOS_SESSION_PLATFORM", "").lower()
...
def _tts_schema_overrides() -> dict:
    ...
    params["properties"]["output_path"]["description"] = _output_path_description(display_panergos_home())
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": _output_path_description("the profile PANERGOS_HOME")
            },
            "speed": {
                "type": "number",
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'panergos_xai_user_agent': ('tools.xai_http', 'panergos_xai_user_agent'),
