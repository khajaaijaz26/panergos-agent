...
elsewhere the agent falls back to ``panergos mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(server: str = "", action: str = "install", reason: str = "", callback: Optional[Callable] = None) -> str:
    ...
    if callback is None:
        ...
        return tool_error(
            "setup_mcp is only available in the panergos desktop app. Use the "
            "terminal instead: `panergos mcp install <name>` for catalog entries, "
            "`panergos mcp login <name>` for OAuth.")
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "unanswered, continue without it. Catalog names: `panergos mcp "
