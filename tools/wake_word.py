"""Wake-word ("Hey Panergos") detection — hands-free session trigger.
...
# Minimum gap between two wake fires, so one wake phrase can't retrigger across
...
_DEFAULTS: Dict[str, Any] = {
    ...
    "provider": "sherpa", "phrase": "hey panergos", "sensitivity": 0.6,
    "confirmation_frames": _DEFAULT_CONFIRMATION_FRAMES, "start_new_session": True,
}

# Bundled legacy "hey panergos" model (tools/wakewords/); selecting openWakeWord without a
# custom model keeps existing installations working while sherpa handles the Panergos default.
_BUNDLED_MODEL_NAME = "hey_panergos"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_panergos", "hey panergos", "panergos"})


def _bundled_wakeword_path(framework: str = "onnx") -> str:
    """Path to the shipped hey_panergos model (.onnx/.tflite) for ``framework``."""
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey panergos"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    with suppress(Exception):
        ...
        from panergos_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return ("Microphone delivers only silence. Grant the Panergos backend "
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    ladder = (
        ...
        (bool(missing), lambda: (f"Wake word needs {missing} configured — run `panergos tools` "
...
# ── Process-wide singleton (mirrors panergos_cli.voice's continuous API) ──
...
def _lock_path() -> Path:
    ...
    return get_default_panergos_root() / "runtime" / "wake-word.lock"
