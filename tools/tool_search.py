...
tool_search / tool_describe / tool_call. Invariants: core tools (``toolsets._PANERGOS_CORE_TOOLS``)
...
def _config_from_loader(loader_name: str) -> ToolSearchConfig:
    """Tool-search config via ``panergos_cli.config.<loader_name>`` (defaults on any failure)."""
    try:
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        from toolsets import _PANERGOS_CORE_TOOLS
        return frozenset(_PANERGOS_CORE_TOOLS)
    except Exception:
        return frozenset()


# Session-gated GUI toolsets: off ``_PANERGOS_CORE_TOOLS`` so non-GUI clients never pay
...
_DEFAULT_DEFERRED_TOOLS = frozenset({
    "computer_use", "session_search", "project_memory", "image_generate",
...
def _search_description(deferred_count: int, listing: Optional[str], listing_form: str,
    ...
    """tool_search bridge description with the listing embedded."""
    desc = (
        ...
        _bridge_schema(
            ...
            "Policy, hooks, and approvals run as for directly-listed tools.",
            {
                "calls": {
                    ...
                    "description": "Exactly one deferred tool invocation.",
                },
            },
            ["calls"],
        ),
...
def dispatch_tool_search(args: Dict[str, Any], *, current_tool_defs: List[Dict[str, Any]],
    ...
    required}}}``. ``limit`` is the total per query across local tools. Empty groups get
    ``available_sources`` + ``hint`` so
...
def resolve_underlying_call(args: Dict[str, Any]) -> Tuple[Optional[str], Dict[str, Any], Optional[str]]:
    ...
    * the trajectory recorder.
    ...
    if len(entries) > 1:
        return None, {}, (
            "tool_call requires exactly one invocation."
        )
