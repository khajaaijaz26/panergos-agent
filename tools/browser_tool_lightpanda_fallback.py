...
def lightpanda_engine_status() -> Tuple[bool, str]:
    ...
    precedence with config-only gates (no network I/O) for ``/browser status`` / ``panergos doctor``.
    """
    _bt = _origin()
    ...
    return True, "Browser Use mode: Panergos spawns `lightpanda serve` per session"
    ...
    if not _install._chromium_installed():
        if _install._running_in_docker():
            hint = ("Chrome fallback requires Chromium, but it is missing. You're running in Docker — "
                    f"rebuild the Panergos source image: {_session._DOCKER_REBUILD}")
        else:
