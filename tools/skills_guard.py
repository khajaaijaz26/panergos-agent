...
_PANERGOS_CONFIG_FILES = r'\.panergos/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.panergos/\.env|\~/\.panergos/\.env',
     "panergos_env_access", "critical", "exfiltration", "directly references panergos secrets file"),
    ...
    # quarantine only acts on "dangerous") but high for Panergos/other config (setup docs routinely say
    ...
    (_prose_modify_re(_PANERGOS_CONFIG_FILES),
     "panergos_config_mod", "high", "persistence", "modification language aimed at Panergos configuration files (verify intent)"),
    (_shell_write_re(_PANERGOS_CONFIG_FILES),
     "panergos_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Panergos configuration files"),
    (r'\.panergos/config\.yaml|\.panergos/SOUL\.md',
     "panergos_config_ref", "low", "persistence", "references Panergos configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Panergos-native; `.clawhubignore` is honored for skills published through ClawHub.
