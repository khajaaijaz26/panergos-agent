...
# BU_NAME + daemon pid).
_OWN_TAB_PREAMBLE = """\
# panergos: pin this named session to its own tab (once per daemon process)
def _panergos_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "panergos-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_panergos_ensure_own_tab()
del _panergos_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # (Panergos's venv) wins over its site-packages → wrong-ABI C-extensions and a crash.
    # PYTHONPATH/PYTHONHOME inherited from the agent process point at Panergos's venv site-packages, and a
    ...
    # #84841, #86006, #86104). Strip both — the CLI manages its own environment and never needs Panergos's
    # import path.
...
def _use_gateway(browser_cfg: dict) -> bool:
    """True when the browser section explicitly selects a remote gateway."""
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_panergos_home()) / "cache" / ".browser_use_default_notice"
        ...
        with contextlib.suppress(OSError):
            ...
            stamp.touch()
        return ("Browser Use CLI not found — using the built-in browser tools. Run `panergos tools` "
...
def _managed_bin_dir() -> str:
    """$PANERGOS_HOME/bin — where install.sh puts uv/uvx and install_cli() links browser-use."""
    return str(Path(get_panergos_home()) / "bin")


def _find_cli() -> Optional[List[str]]:
    """Locate the browser-use CLI, or None when it can't be run. MANAGED-FIRST: Panergos' own ``$PANERGOS_HOME/bin``
    copy always wins so every session drives one Panergos-controlled binary; PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    the binary into ``$PANERGOS_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()`` resolves it for every profile.
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_panergos_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _served_profile_tag() -> str:
    ...
    return "" if get_panergos_home_override() is None else panergos_home_key()
...
def _resolve_lightpanda_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    """Point the harness at a Panergos-spawned ``lightpanda serve`` (``browser.engine: lightpanda`` and
    ...
    err = _export_session_cdp(
        ...
        lambda e: (f"Lightpanda could not be started: {e} Set browser.engine to auto "
                   "to use local Chrome, or switch backends via `panergos tools` → Browser Automation."),
...
def _resolve_managed_chromium_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    """Point the harness at Panergos' packaged Chromium, launched through agent-browser for this cache key —
...
def _resolve_backend_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    ...
    (4) the local engine — ``browser.engine: lightpanda`` or Panergos' packaged Chromium via agent-browser
    ...
    # legacy provider would create a second, redundant session. Explicit gateway configs resolve through
    # the provider: the gateway
    ...
    err = _export_session_cdp(
        ...
        lambda e: (f"Cloud browser provider {provider_name} failed to provide a session: {e}. "
                   "Fix the provider configuration or switch backends via `panergos tools` → Browser Automation."),
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            ...
            "description": ("Drive the user's own local browser (a Panergos-managed copy of their real "
