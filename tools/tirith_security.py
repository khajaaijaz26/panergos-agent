...
GitHub releases to $PANERGOS_HOME/bin/tirith in a background thread -- SHA-256 always verified,
...
from panergos_constants import get_panergos_home, get_panergos_home_override, panergos_home_key
...
def _cached_path() -> str | None:
    ...
    if get_panergos_home_override() is not None:
        return _resolved_path_by_home.get(panergos_home_key())
...
def _store_resolved(path: str) -> None:
    ...
    if get_panergos_home_override() is not None:
        _resolved_path_by_home[panergos_home_key()] = path
    else:
        _resolved_path = path
...
def _failure_marker_path() -> str:
    return os.path.join(str(get_panergos_home()), ".tirith-install-failed")
...
# --- Auto-install ---
def _panergos_bin_dir() -> str:
    """$PANERGOS_HOME/bin, created if needed."""
    os.makedirs(d := os.path.join(str(get_panergos_home()), "bin"), exist_ok=True)
    return d
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $PANERGOS_HOME/bin/tirith -> ``(installed_path,
    ...
    try:
        ...
        if src is None:
            return None, reason
        dest = os.path.join(_panergos_bin_dir(), "tirith")
        try:
...
def _find_local_tirith() -> str | None:
    """Cheap local lookup for the default "tirith": PATH, then $PANERGOS_HOME/bin."""
    panergos_bin = os.path.join(_panergos_bin_dir(), "tirith")
    return shutil.which("tirith") or (panergos_bin if _is_executable(panergos_bin) else None)
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    $PANERGOS_HOME/bin/tirith → install; failures cached in-process and on disk for 24h). On a miss
