...
~/.panergos/skills/, tracking each synced skill's origin hash in .bundled_manifest (v2 "name:hash"
...
from panergos_constants import get_bundled_skills_dir, get_panergos_home, get_optional_skills_dir
...
# retarget PANERGOS_HOME after import, and frozen constants would resolve (and for
...
# once under the launch PANERGOS_HOME and later scope requests to a different profile via
# set_panergos_home_override(). See #65828.
_PANERGOS_HOME_AT_IMPORT = PANERGOS_HOME
...
def _panergos_home() -> Path:
    return _live(PANERGOS_HOME, _PANERGOS_HOME_AT_IMPORT, get_panergos_home)


def _skills_dir() -> Path:
    return _live(SKILLS_DIR, _SKILLS_DIR_AT_IMPORT, lambda: _panergos_home() / "skills")
...
# Written by `panergos profile create --no-skills` / installer `--no-skills`: sync seeds only
# essential skills. Mirrors panergos_cli.profiles.NO_BUNDLED_SKILLS_MARKER (no CLI import here).
NO_BUNDLED_SKILLS_MARKER = ".no-bundled-skills"


def _get_bundled_dir() -> Path:  # PANERGOS_BUNDLED_SKILLS env first, then repo-relative
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        if not _matches_origin_hash(candidate, origin_hash):  # moving a customized copy would edit user work
            st.say(
                ...
                f"Run `panergos skills reset {skill_name} --restore` to move to the new location.")
            continue
        try:
        ...
        if dest.exists():
            ...
            if _dir_hash(dest) == bundled_hash:
                ...
                st.say(
                    ...
                    f"by this name — yours was kept. Run `panergos skills reset {skill_name}` to "
...
def sync_skills(quiet: bool = False) -> dict:
    """Sync bundled skills into ~/.panergos/skills/ using the manifest; returns the per-category
    result dict. Opted-out profiles seed ONLY ESSENTIAL_SKILLS (the system prompt always
    points at ``panergos-agent``)."""
    essential_only = (_panergos_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
def _rmtree_writable(path: Path) -> None:
    ...
    child of the active skills root (bad join / missing PANERGOS_HOME / malicious manifest entry).
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.panergos/skills/ ...")
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_panergos_home() / NO_BUNDLED_SKILLS_MARKER).exists()
