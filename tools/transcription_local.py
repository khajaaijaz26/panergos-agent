...
def _try_lazy_install_stt() -> bool:
    ...
    try:
        ...
        logger.warning(
            ...
            "This is often a permission issue: the Panergos process user cannot "
            "write to the virtual environment. Try running manually as the "
            "venv owner: `stat -c '%%u' '$(dirname $(dirname $(which python3)))'` "
            "then `su - <owner> -c 'VIRTUAL_ENV=/opt/panergos/.venv "
...
def _transcribe_local_command(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="panergos-local-stt-") as output_dir:
            ...
            # Scrub Panergos secrets from the child env (same policy as _run_command_stt).
            # Scrub Panergos secrets from the child env (sibling path to #56332 / _run_command_stt — this
            ...
            _run_quiet(shlex.split(command), timeout=300, env=panergos_subprocess_env(inherit_credentials=False))
