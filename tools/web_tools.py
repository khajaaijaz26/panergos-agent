...
Backend is selected during ``panergos tools`` (``web.backend`` in config.yaml; per
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via the config-aware env layer (``panergos config set`` values), then process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set through Panergos' config/.env layer
    (``panergos config set``, ``panergos tools``) are honored here too — not just raw process-env exports.
...
def _get_backend() -> str:
    ...
    # Never-configured install: use the first available direct or local provider.
    backend_candidates = (
...
# firecrawl client, lazy SDK proxy, config resolution, response normalizers, and
# check_firecrawl_api_key() all live in plugins.web.firecrawl.provider.
def _web_requires_env() -> list[str]:
    """Tool-registry metadata environment variables for direct web backends."""
    return [
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            fallback = "No web search provider configured. Run `panergos tools` to set one up."
...
def _provider_is_ready(provider) -> bool:
    ...
    False (so dispatch can emit a precise error), so readiness gates (tool check_fn, ``panergos doctor``)
