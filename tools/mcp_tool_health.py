...
class MCPServerHealthMixin:
    ...
    def _make_logging_callback(self):
        """``logging_callback`` forwarding server ``notifications/message`` into Panergos logging (SDK default drops them).

        Routes MCP ``notifications/message`` log notifications from the server into Panergos' logging
        (agent.log via panergos_logging), tagged with the server name. Without this, the SDK's default
