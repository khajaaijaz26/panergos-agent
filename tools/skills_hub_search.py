"""Skills Hub discovery: the centralized Panergos index fetch (cached, stale-
...
from tools.skills_hub_official import PanergosIndexSource, OptionalSkillSource
...
PANERGOS_INDEX_URL = "https://khajaaijaz26.github.io/panergos-agent/docs/api/skills-index.json"
PANERGOS_INDEX_TTL = 6 * 3600  # 6 hours


def _panergos_index_cache_file() -> Path:
    from tools.skills_hub import _index_cache_dir
    return _index_cache_dir() / "panergos-index.json"


def _load_panergos_index() -> Optional[dict]:
    """Fetch the centralized skills index (docs site, rebuilt daily), cached
    locally for PANERGOS_INDEX_TTL; on any failure serve the stale cache.
    ...
    cache_file = _panergos_index_cache_file()
    cached = _read_json_if_fresh(cache_file, PANERGOS_INDEX_TTL)
    if cached is not None:
        return cached
    data = None
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            resp = httpx.get(PANERGOS_INDEX_URL, timeout=15, follow_redirects=True,
                             headers={"Accept-Encoding": accept_encoding})
            if resp.status_code != 200:
                logger.debug("Panergos index fetch returned %d", resp.status_code)
                return _load_stale_index_cache()
            data = resp.json()
            break
        except httpx.DecodingError as e:
            logger.debug("Panergos index decode failed (Accept-Encoding=%s): %s", accept_encoding, e)
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Panergos index fetch failed: %s", e)
            return _load_stale_index_cache()
...
def _load_stale_index_cache() -> Optional[dict]:
    ...
    from tools.skills_hub import _read_json_if_fresh
    return _read_json_if_fresh(_panergos_index_cache_file(), float("inf"))
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    return [
        ...
        PanergosIndexSource(auth=auth),     # centralized index (search + resolved install paths)
...
def _select_active_sources(sources: List[SkillSource], source_filter: str) -> List[SkillSource]:
    ...
    index_available = effective == "all" and any(
        src.source_id() == "panergos-index" and getattr(src, "is_available", False) for src in sources
    )
