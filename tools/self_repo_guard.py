...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(panergos_home).expanduser() if panergos_home else Path.home() / ".panergos") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite Panergos' live source checkout "
        ...
        "is pushed. To change this checkout, stop Panergos, run the command externally, then restart "
        "Panergos.")
