...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_panergos_home() / "auth.json"
...
def panergos_xai_user_agent() -> str:
    """Return a stable Panergos-specific User-Agent for xAI HTTP calls."""
    try:
        from panergos_cli import __distribution_version__
    except Exception:
        __distribution_version__ = "unknown"
    return f"PanergosAgent/{__distribution_version__}"


def panergos_xai_default_headers() -> Dict[str, str]:
    """Default headers for OpenAI-SDK and raw HTTP clients talking to xAI (replaces the SDK User-Agent)."""
    return {"User-Agent": panergos_xai_user_agent()}
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Panergos home, then mark it seen."""
    notice = xai_storage_notice_text(section_name)
    if not notice:
        return None
    try:
        from panergos_constants import get_panergos_home
        marker_dir = get_panergos_home() / "state"
...
def _xai_base_url_override() -> str:
    """``PANERGOS_XAI_BASE_URL`` then ``XAI_BASE_URL``, stripped; '' when unset."""
    from panergos_cli.config import get_env_value
    return str(get_env_value("PANERGOS_XAI_BASE_URL") or get_env_value("XAI_BASE_URL") or "").strip().rstrip("/")
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    Default order: Panergos-managed xAI OAuth, then ``XAI_API_KEY`` (via ``get_env_value`` so
    ``~/.panergos/.env`` keys count). ``prefer_api_key=True`` inverts that for API-metered
    endpoints where the subscription OAuth bearer authorizes but misbehaves (x_search answers
    without citations, TTS 403s). Both branches honor ``PANERGOS_XAI_BASE_URL``/``XAI_BASE_URL``
    ...
    Prefers Panergos-managed xAI OAuth credentials when available, then falls back to ``XAI_API_KEY`` resolved
    via ``panergos_cli.config.get_env_value`` so keys stored in ``~/.panergos/.env`` (the standard Panergos
    ...
    scoping is identical to the fallback branch, and the base URL honors ``PANERGOS_XAI_BASE_URL`` /
