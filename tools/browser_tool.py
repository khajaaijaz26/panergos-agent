...
from panergos_cli.config import DEFAULT_CONFIG, cfg_get
...
# dependency could read every Panergos secret from process.env.
...
def _build_browser_env() -> dict:
    ...
    env = panergos_subprocess_env(inherit_credentials=False)
...
# than a concrete path (also compared in panergos_cli/tools_config.py and doctor.py).
...
# The config-derived ones are keyed by profile home (``panergos_home_key()``): the multiplexed
...
_REAL_PROFILE_SESSION = "panergos-real-profile"
...
def _socket_safe_tmpdir() -> str:
    """Short temp dir for Unix sockets: macOS ``TMPDIR`` + ``agent-browser-panergos_…``
...
def _url_policy_error(url: str, *, auto_local: bool = False) -> Optional[dict]:
    ...
    URL protects nothing. Panergos' own secrets leaking into a URL are caught by ``_secret_url_error``."""
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        if not cfg_get(read_raw_config(), "browser", "record_sessions", default=False):
            return
        recordings_dir = panergos_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    screenshots_dir = get_panergos_dir("cache/screenshots", "browser_screenshots")
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_panergos_home_override': ('panergos_constants', 'get_panergos_home_override'),
    'panergos_home_key': ('panergos_constants', 'panergos_home_key'),
    ...
    'node_tool_runnable': ('panergos_constants', 'node_tool_runnable'),
