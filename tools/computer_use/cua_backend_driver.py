...
# var would only LOOK like it pinned. Point PANERGOS_CUA_DRIVER_CMD at a specific binary instead.
_CUA_DRIVER_CMD_ENV = "PANERGOS_CUA_DRIVER_CMD"
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command to its DrvFS ``/mnt/<drive>/...`` form when Panergos runs in WSL
    (a Windows cua-driver manifest can report ``C:\\...`` while Panergos spawns via POSIX). Non-Windows paths and
    ...
    return str(PurePosixPath("/mnt", drive, *win.parts[1:])) if wsl and drive else path

def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    """Candidate commands in resolution order. ``override`` / a non-empty ``PANERGOS_CUA_DRIVER_CMD`` is authoritative
...
def cua_driver_install_hint() -> str:
    ...
    return ("cua-driver is not installed. Install with one of:\n  panergos computer-use install\n"
            ...
            "Or run `panergos tools` and enable the Computer Use toolset to install it automatically.")
...
def _resolve_mcp_invocation(driver_cmd: str, *, timeout: float = 6.0) -> Tuple[str, List[str]]:
    ...
    Surface 8 of khajaaijaz26/panergos-agent#47072: instead of hardcoding ``["mcp"]`` we ask the driver itself
    ...
    keeps working without a Panergos patch.
...
def _manifest_contract_reason(manifest: Optional[Dict[str, Any]]) -> str:
    ...
    if tuple(int(part) for part in match.groups()) < _CUA_DRIVER_RUNTIME_CONTRACT_MIN:
        return "Panergos computer use requires cua-driver 0.20.0 or newer"
...
def cua_driver_runtime_contract_status(binary: Optional[str] = None) -> Dict[str, Any]:
    """Report whether a local driver can host Panergos' 0.20 integration."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (f"cua-driver {state.get('latest_version') or '?'} is available "
            f"(you have {state.get('current_version') or '?'}); update with `panergos computer-use install --upgrade`.")
