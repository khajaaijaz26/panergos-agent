...
def _panergos_tags(fm: dict) -> Any:
    """``metadata.panergos.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    panergos_meta = metadata.get("panergos", {}) if isinstance(metadata, dict) else None
    return panergos_meta.get("tags", []) if isinstance(panergos_meta, dict) else []
