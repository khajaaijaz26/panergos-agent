...
class ClawHubSource(GuardedFetchMixin, SkillSource):
    ...
    # Wall-clock budget for catalog walks. Bulk callers may raise this on their
    # instance, but no path is allowed to disable the deadline entirely.
    CATALOG_WALK_BUDGET_SECONDS = 12
    catalog_walk_complete: Optional[bool] = None
    catalog_walk_stop_reason: Optional[str] = None
    ...
    def search(self, query: str, limit: int = 10) -> List[SkillMeta]:
        ...
        if query:
            ...
            # 50k+ skills. A zero limit removes the item cap, not the deadline.
            catalog = self._load_catalog_index(max_items=max(limit, 0))
            if catalog:
                deduped = _dedupe_results(catalog)
                if limit > 0 and len(deduped) > limit:
                    self.catalog_walk_complete = False
                    self.catalog_walk_stop_reason = "item_limit"
    ...
    def _load_catalog_index(self, max_items: int = 0) -> List[SkillMeta]:
        ...
        gathered (browse's cold-start fallback renders one page); ``0`` removes
        only the item cap. Every walk retains a wall-clock deadline. Only a
        cursor-exhausted walk is written to the shared ``clawhub_catalog_v1``
        cache — a capped, timed-out, or failed walk would poison it with a
        partial slice.
        """
        cache_key = "clawhub_catalog_v1"
        cached = _cached_metas(cache_key)
        if cached is not None:
            self.catalog_walk_complete = True
            self.catalog_walk_stop_reason = None
            return cached
        ...
        self.catalog_walk_complete = stop_reason is None
        self.catalog_walk_stop_reason = stop_reason
        if stop_reason is None and results:
            _cache_metas(cache_key, results)
        elif stop_reason:
            log = logger.info if stop_reason == "item_limit" else logger.warning
            log(
                "ClawHub catalog walk stopped at %d skills (%s); partial results were not cached",
                len(results), stop_reason,
            )
        return results
