...
The LLM writes a Python script that calls Panergos tools via RPC, collapsing
...
# Loopback TCP replaces AF_UNIX on Windows, so execute_code runs on every platform Panergos does.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_panergos_dir("cache/exec", "exec_spill")
...
# ---- panergos_tools.py code generator ----
...
def _missing_panergos_tools_import_hint(m, enabled_tools) -> str:
    missing = m.group(1)
    if missing in {"json_parse", "shell_quote", "retry"}:
        return (f"Import helpers with `from panergos_tools import {missing}`. "
                "If that import failed, the generated module may be stale or another "
                "panergos_tools may be first on sys.path. Check panergos_tools.__file__ "
...
# these as the top execute_code failure classes: panergos_tools import misuse, missing helper
# imports, treating tool results as strings, importing third-party packages absent from the sandbox.
_FAILURE_HINT_RULES = (
    (r"cannot import name '(\w+)' from 'panergos_tools'", _missing_panergos_tools_import_hint),
    (r"NameError: name '(json_parse|shell_quote|retry)' is not defined",
     lambda m, _: f"Import {m.group(1)} before calling it: "
                  f"from panergos_tools import {m.group(1)}"),
...
def generate_panergos_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """Source of the panergos_tools.py stub module for SANDBOX_ALLOWED_TOOLS ∩ *enabled_tools*.
...
"""Auto-generated panergos tools RPC stubs."""
...
def _connect():
    ...
    PANERGOS_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["PANERGOS_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("PANERGOS_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (PANERGOS_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("PANERGOS_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated panergos tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("PANERGOS_RPC_DIR") or os.path.join(tempfile.gettempdir(), "panergos_rpc")
_seq = 0
...
def _run_remote_per_call(env, env_type: str, code: str, effective_task_id: str,
    ...
    """Per-call script ship: stage panergos_tools.py + script.py in a fresh remote sandbox dir,
    serve file-RPC from a polling thread, run, clean up."""
    sandbox_dir = f"{_env_temp_dir(env)}/panergos_exec_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{sandbox_dir}/panergos_tools.py",
                             generate_panergos_tools_module(list(sandbox_tools), transport="file"))
        ...
        rpc_thread.start()
        env_prefix = (f"PANERGOS_RPC_DIR={quoted_rpc_dir} PANERGOS_RPC_TOKEN={shlex.quote(rpc_token)} "
...
def _execute_remote(code: str, task_id: Optional[str], enabled_tools: Optional[List[str]],
    ...
    try:
        ...
        try:
            # --- Session-kernel path (panergos-agent#96873) ------------------- Same always-on model as
...
def execute_code(
    ...
    with RPC access to a subset of Panergos tools; returns the JSON result string. "Sandbox" means
    ...
    # execute_code is a straight bypass — the terminal() path refuses `launchctl bootout ai.panergos.gateway`,
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.panergos/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
            ...
            "Panergos' own python (the common case — stdlib plus Panergos' "
    ...
    description = (
        "Run Python that calls panergos tools programmatically. Use when you "
        ...
        f"Available via `from panergos_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        f"{cwd_note}\n\n"
        "Helpers require imports: `from panergos_tools import json_parse, shell_quote, retry`. "
        ...
        "parameters": {
            ...
            "properties": {
                "code": {"type": "string", "description": (
                    ...
                    f"`from panergos_tools import {import_str}` "
