...
_FOREGROUND_GUIDANCE = (
    (
        ...
        "notify_on_complete=true) so Panergos tracks the process, then run readiness "
...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``panergos gateway restart|stop|uninstall``
    targeting panergos-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_PANERGOS_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (panergos
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``panergos gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "or determine whether it is independent of Panergos. Perform authorized "
    ...
    if contains_gateway_lifecycle_command_or_referenced_script(
        ...
        return _blocked_json(
            ...
            "to child processes). Run `panergos gateway restart` from a "
