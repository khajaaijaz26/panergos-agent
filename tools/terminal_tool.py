...
processes survive cleanup, idle reaping, or Panergos exit.
...
# display_panergos_home imported lazily at call site (stale-module safety during panergos update)
...
def _current_session_key() -> str:
    ...
    return get_session_env("PANERGOS_SESSION_KEY", "")


def _current_session_profile() -> str:
    """Active session's Panergos profile name, or "" (same lookup discipline as
    :func:`_current_session_key`)."""
    from gateway.session_context import get_session_env

    return get_session_env("PANERGOS_SESSION_PROFILE", "")
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps Exited containers labeled ``panergos-agent=1`` for the current
    profile — leftovers of Panergos processes that died without firing
    ...
    Panergos processes sharing a profile) and the once-per-interpreter flag so
...
def _ensure_terminal_env_bridged() -> None:
    ...
    at startup; processes that skip those paths (``panergos serve``, Desktop
    ...
    env values (possibly stale from ``panergos setup``); env values for omitted keys
    ...
    context-local ``PANERGOS_HOME`` override (multiplexed dashboard / gateway
    ...
    with _quiet("terminal config → env fallback bridge failed"):
        from panergos_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _resolve_config_cwd(env_type: str, mount_docker_cwd: bool) -> tuple:
    ...
    from panergos_cli.config import _is_ssh_remote_tilde_cwd
...
def terminal_tool(
    ...
    ``_host_local`` forces the local backend for Panergos-owned control-plane
