...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    try:
        ...
        if name in ESSENTIAL_SKILLS:
            return (
                f"Skill '{name}' is essential to Panergos (the agent's own "
    ...
    if _is_pinned(name, "pinned-guard"):
        return (
            ...
            f"run `panergos curator unpin {name}` if they want to delete it. Patches and edits "
...
def _background_review_write_guard(
    ...
    if _is_pinned(name, "pinned skill guard"):
        return _refusal(
            ...
            f"are off-limits to autonomous maintenance. Ask the user to run `panergos curator "
    ...
    try:
        ...
        # closed for both shapes; `panergos curator adopt <name>` is the supported way in. See #67140.
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return _refusal(
                ...
                f"curation. Run `panergos curator adopt {name}` to opt it in.")
    except Exception:
