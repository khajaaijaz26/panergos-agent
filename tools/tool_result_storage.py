...
is ALWAYS host-side ``$PANERGOS_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
STORAGE_DIR = "/tmp/panergos-results"
...
def get_spillover_dir():
    """Return $PANERGOS_HOME/cache/spillover as a Path (not created)."""
    from panergos_constants import get_panergos_home
    return get_panergos_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $PANERGOS_HOME/cache/spillover; returns path str or None."""
    try:
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/panergos-results" if temp_dir else STORAGE_DIR
...
HEREDOC_MARKER = "PANERGOS_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
