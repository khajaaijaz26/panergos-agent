...
from tools.environments.local_env_policy import _PANERGOS_PROVIDER_ENV_BLOCKLIST, _is_panergos_internal_secret


def load_panergos_env_vars() -> dict[str, str]:
    """``~/.panergos/.env`` values, or ``{}`` — a broken .env must not fail command execution."""
    try:
...
def resolve_passthrough_env(explicit_forward: Iterable[str] = (),
                            panergos_env_loader: Callable[[], dict[str, str]] = load_panergos_env_vars,
    ...
    is filtered through the Panergos provider-credential blocklist and the dynamic internal-secret
    ...
    try:
        ...
        pass
    implicit_forward = {k for k in passthrough_keys if not _is_panergos_internal_secret(k)}
    forward_keys = set(explicit_forward) | (implicit_forward - _PANERGOS_PROVIDER_ENV_BLOCKLIST)
    panergos_env = panergos_env_loader() if forward_keys else {}
    exec_env: dict[str, str] = {}
    unset_names: set[str] = set()
    for key in sorted(forward_keys):
        value = os.getenv(key) or panergos_env.get(key)
