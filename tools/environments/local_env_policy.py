"""Secret-scrub policy for Panergos child processes: pure data + predicates for which env
names are Panergos-managed credentials. The env *builders* applying it (``_make_run_env``,
``_sanitize_subprocess_env``, ``panergos_subprocess_env``) live in ``tools.environments.local``."""

import os

# Prefix a caller uses in ``extra_env`` to force a blocklisted var through.
_PANERGOS_PROVIDER_ENV_FORCE_PREFIX = "_PANERGOS_FORCE_"

# Panergos-managed AWS *inference* credentials for ``auth_type="aws_sdk"`` (Bedrock):
...
_STATIC_PROVIDER_ENV_BLOCKLIST = frozenset({
    ...
    "EMAIL_HOME_ADDRESS", "EMAIL_HOME_ADDRESS_NAME", "PANERGOS_DASHBOARD_SESSION_TOKEN",
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Claude Code install, not Panergos: stripping it made agent-spawned ``claude`` CLIs
    ...
    # It is set and owned by the user's Claude Code install (subscription OAuth), not a Panergos-managed
    # inference credential — Claude subscription auth is not a working Panergos provider path. It arrives via
...
_PANERGOS_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# ``_sanitize_subprocess_env``); execute_code, panergos_subprocess_env, docker and
...
# ``buzz`` (the gateway's ``PANERGOS_SESSION_PLATFORM`` ContextVar; concurrency safe under a multi-session
...
# scope. Every other surface stays sealed — execute_code scrubbing, :func:`panergos_subprocess_env` (browser /
...
# the blocklist entirely because it is NOT a Panergos credential; these ARE Panergos-managed first-party
...
def _buzz_terminal_context_active() -> bool:
    ...
    try:
        ...
        return get_session_env("PANERGOS_SESSION_PLATFORM", "").strip().lower() == "buzz"
    except Exception:
        return False
...
# ANOTHER project's deps into the Panergos venv (still reachable via PATH, so stripping
# is safe); PYTHONHOME redirects a child interpreter's stdlib to the Panergos venv
# (version-mismatch crashes). PYTHONPATH is handled separately (Panergos-owned entries only).
...
# dependencies into the Panergos venv path instead of the project's own ``.venv`` — silently clobbering the
# Panergos environment (e.g. a project pinned to a different Python version overwrites it and breaks the
# gateway). PYTHONHOME is included because a gateway-inherited value redirects the standard-library search
# of ANY child interpreter — including unrelated system/venv Pythons — to the Panergos venv's stdlib, which
# crashes with version-mismatch errors before a child script even imports a package (#75018). Panergos itself
...
# _strip_panergos_owned_pythonpath() which removes only Panergos-owned entries, preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_panergos_internal_secret(key: str) -> bool:
    """True for Panergos-internal secrets injected under *dynamic* names the static
...
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_panergos_internal_secret, but _ID has no secret suffix, so it must be
    # enumerated here to stay stripped on the inherit_credentials=True path.
    "GATEWAY_RELAY_ID", "GATEWAY_RELAY_SECRET", "GATEWAY_RELAY_DELIVERY_KEY",
    "HASS_TOKEN", "EMAIL_PASSWORD", "PANERGOS_DASHBOARD_SESSION_TOKEN",
