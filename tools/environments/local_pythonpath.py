"""Panergos-owned PYTHONPATH stripping for child processes. Launchers prepend the repo
root and the Panergos venv's site-packages so the backend can ``import tools``; leaked
into a child Python of a DIFFERENT version they load the backend's C extensions and
crash. Only entries proven Panergos-owned by *path provenance* are removed — never by a
cross-version heuristic. Module state (``_panergos_repo_root_aliases``, ``_in_venv``,
``_panergos_site_packages``) lives in ``tools.environments.local`` (via :func:`_state`)
...
def _build_panergos_repo_root_aliases(
    resolved_root: Path, lexical_root: Path, configured_home: Path,
) -> tuple[Path, ...]:
    """Exact repo-root spellings emitted by Panergos launchers. Mirrors
    ``gateway_windows._preserve_panergos_home_path`` (physical path under the resolved
    PANERGOS_HOME -> configured spelling) so a junction-backed install matches without
    treating arbitrary PANERGOS_HOME descendants as Panergos-owned. A repo-level junction
    ...
    # and the repo lives beside the profiles dir (as get_default_panergos_root() does).
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    if not env.get("VIRTUAL_ENV") or not any(
            _same_path(candidate, root / "venv") for root in _state()._panergos_repo_root_aliases):
        return None
    try:
...
def _get_panergos_site_packages(env: dict) -> list[Path]:
    """Exact site-packages dirs owned by the Panergos runtime (cached):
    ...
    local = _state()
    if local._panergos_site_packages is None:
        result: list[Path] = []
        if local._in_venv:
            try:
                import site
        ...
        local._panergos_site_packages = list(result)
    result = list(local._panergos_site_packages)
...
def _strip_panergos_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Panergos-owned PYTHONPATH entries, then the runtime marker vars. Order is
...
def _strip_panergos_owned_pythonpath(env: dict) -> None:
    """Remove Panergos-owned PYTHONPATH entries: only exact matches of the repo root
    ...
    if not pp:
        return
    owned_paths = [*_get_panergos_site_packages(env), *_state()._panergos_repo_root_aliases]
    ...
    if stripped:
        logger.debug("Stripped Panergos-owned entries from PYTHONPATH: %s", stripped)
