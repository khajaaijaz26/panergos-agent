...
_FORCE_SYNC_ENV = "PANERGOS_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.panergos") -> list[tuple[str, str]]:
    """Enumerate all (host_path, remote_path) pairs to sync to a remote. Credential paths are
    remapped from the hardcoded /root/.panergos to *container_base* (remote home may differ)."""
    ...
    files = [
        (entry["host_path"], entry["container_path"].replace("/root/.panergos", container_base, 1))
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        per ``sync_interval`` unless *force* or ``PANERGOS_FORCE_FILE_SYNC=1``. Transactional:
    ...
    def _sync_transaction(self, *, force: bool = False) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="panergos-sync-push-") as staging:
                staged_files = []
                pushed_hashes = {}
    ...
    def sync_back(self, panergos_home: Path | None = None) -> None:
        """Pull remote changes back to the host: download the remote ``.panergos/`` as a tar and
        ...
        with self._transaction_lock:
            self._sync_back_transaction(panergos_home=panergos_home)

    def _sync_back_transaction(self, panergos_home: Path | None = None) -> None:
        ...
        # to avoid retry storms against an uninitialized remote .panergos/.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (panergos_home or get_panergos_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="panergos-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str, file_mapping: list[tuple[str, str]] | None = None, *,
        ...
        substitution (``/root/.panergos/skills/b.md`` -> ``~/.panergos/skills/b.md``)."""
