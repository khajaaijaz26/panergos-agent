...
# Module-level binding: tests patch ``ssh._load_panergos_env_vars`` to fake the .env file.
...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "panergos-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.panergos"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.panergos directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.panergos"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="panergos-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .panergos/ as a tar archive."""
        # Tar from / with the full path so archive entries keep absolute paths
        # (home/user/.panergos/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.panergos".lstrip("/")
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False, timeout: int = 120,
        ...
        values, unset_names = resolve_passthrough_env(panergos_env_loader=_load_panergos_env_vars)
