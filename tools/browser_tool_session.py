...
_DOCKER_REBUILD = recommended_update_command_for_method("docker")
_CHROMIUM_INSTALL = "npx agent-browser install --with-deps (or: npx playwright install --with-deps chromium)"
_CHROMIUM_MISSING_DOCKER_HINT = ("Chromium browser is missing. You're running in Docker — rebuild the "
                                 f"Panergos source image to get bundled Chromium: {_DOCKER_REBUILD}")
    ...
    if "sandbox" in f"{stderr}\n{stdout}".lower():
        ...
        if _install._running_in_docker():
            parts.append("The browser daemon may still be starting or Chromium may be "
                         f"missing. Rebuild the Panergos source image: {_DOCKER_REBUILD}")
        else:
            parts.append("The browser daemon may still be starting, or Chromium may be "
...
def _prepare_session_socket_dir(session_name: str) -> str:
    ...
    with our PID BEFORE first use — another panergos process's orphan reaper rmtree's any
...
def _create_local_session(task_id: str, allow_real_profile: bool = True) -> Dict[str, str]:
    ...
    # Browser Use mode + ``browser.engine: lightpanda`` drives a Panergos-spawned
