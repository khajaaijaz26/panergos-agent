...
_ACTIONS = frozenset({"sync", "search", "status", "remember"})
_ACTION_ARGS = {
...
def _read_stable(path: Path) -> tuple[bytes, os.stat_result]:
    ...
    try:
        ...
        data = b"".join(chunks)
        after = os.fstat(fd)
        if len(data) > _MAX_FILE_BYTES or (before.st_size, before.st_mtime_ns) != (after.st_size, after.st_mtime_ns):
...
def project_memory(action: object, root: object = None, query: object = None,
                   limit: object = None, note: object = None) -> str:
    ...
    except (OSError, RuntimeError, sqlite3.Error, subprocess.TimeoutExpired, ValueError) as exc:
