"""Skills Hub official sources: repo-shipped optional skills and the centralized Panergos index."""
...
from tools.skills_hub_models import (
    SkillBundle, SkillMeta, SkillSource, _panergos_tags, _matches_query, _memo_json, _parse_frontmatter, hub,
)
...
class OptionalSkillSource(SkillSource):
    """Skills from the repo's ``optional-skills/`` directory: official (Panergos-maintained) but not
    activated by default — absent from the system prompt and not copied to ~/.panergos/skills/ at
    setup. Discoverable via the Skills Hub as source "official" with "builtin" trust."""

    SOURCE_ID = "official"
    TRUST_LEVEL = "builtin"
    OFFICIAL_REPO = "khajaaijaz26/panergos-agent"
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # (metadata.panergos.upstream); install pulls the live content from there.
        try:
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``main``; rather than demanding ``panergos update`` first, resolve against the live repo.
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.panergos.upstream: {repo: owner/name, path: ...}`` out of SKILL.md content
        ...
        panergos_meta = meta_block.get("panergos") if isinstance(meta_block, dict) else None
        upstream = panergos_meta.get("upstream") if isinstance(panergos_meta, dict) else None
...
class PanergosIndexSource(SkillSource):
    """Skill source backed by the centralized Panergos Skills Index: a JSON catalog on the docs site,
    rebuilt by CI, with metadata + source identifiers for every skill — search and path
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            from tools.skills_hub_search import _load_panergos_index
            self._index, self._loaded = _load_panergos_index(), True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        for github_id in filter(None, candidates):
            ...
            if bundle:
                bundle.source = entry.get("source", "panergos-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            ...
            source=entry.get("source", "panergos-index"), identifier=entry.get("identifier", ""),
