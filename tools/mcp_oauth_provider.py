"""Shared ``OAuthClientProvider`` customizations for Panergos MCP OAuth.
...
class PanergosProviderMixin:
    """Token-endpoint fixes layered over the SDK's ``OAuthClientProvider`` (must precede it in
    the MRO; subclasses set ``_panergos_logger`` to keep their own logger name).
    ...
    def _prepare_token_request(self, request):
        ...
        ua = getattr(self, "_panergos_token_user_agent", None)  # tests build via __new__
        if ua:
            request.headers["User-Agent"] = ua
        return request

    def _coerce_client_secret_post(self) -> None:
        """Same rule as ``PanergosTokenStorage._coerce_secret_auth_method``, applied to the
        ...
        if PanergosTokenStorage._coerce_secret_auth_method(data):
    ...
    async def _initialize(self) -> None:
        ...
        from tools.mcp_oauth import PanergosTokenStorage
        if isinstance(storage, PanergosTokenStorage) and self.context.oauth_metadata is None:
    ...
    async def _handle_refresh_response(self, response) -> bool:
        ...
        if not (200 <= response.status_code < 300):
            self._panergos_logger.warning("Token refresh failed: %s", response.status_code)
        ...
        try:
            ...
            self._panergos_logger.warning("Invalid refresh response: %s", response.status_code)
            self.context.clear_tokens()
            return False
...
def bind_issuer_from_context(context: Any) -> None:
    ...
    it. No-op when metadata is not discovered yet or storage is not Panergos'."""
    from tools.mcp_oauth import PanergosTokenStorage
    storage = getattr(context, "storage", None)
    issuer = _metadata_issuer(context)
    if isinstance(storage, PanergosTokenStorage) and issuer:
...
def enforce_refresh_token_issuer(context: Any) -> None:
    ...
    if not isinstance(storage, PanergosTokenStorage) or tokens is None or not getattr(tokens, "refresh_token", None):
        return
...
def prepare_oauth_config(server_name: str, server_url: str, oauth_config: dict | None) -> tuple[dict, "PanergosTokenStorage"]:
    ...
    return cfg, mo.PanergosTokenStorage(server_name)


def build_provider_kwargs(cfg: dict, storage: "PanergosTokenStorage", *, ssh_proxy_hint: bool) -> dict[str, Any]:
