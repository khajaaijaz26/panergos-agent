...
# ladders, parked-server self-probes and repeated `panergos mcp test` runs re-run the preflight
...
# The cache is also persisted under the Panergos home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``panergos_constants.get_panergos_home()`` so the cache follows the
    active profile and is isolated across Panergos homes. The cache directory
    is created on demand. Returns ``None`` when Panergos home cannot be
...
def _query_osv(package: str, ecosystem: str, version: Optional[str] = None) -> list:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "panergos-agent-osv-check/1.0"},
        method="POST")
