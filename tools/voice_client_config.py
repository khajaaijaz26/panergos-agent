...
def _deepinfra_model(section: Dict[str, Any], kind: str) -> Optional[str]:
    ...
    from panergos_cli.models import deepinfra_model_ids
...
def _resolve_stt_client_config() -> Dict[str, Any]:
    ...
    if provider == "openai":
        # Resolve the configured direct endpoint and credentials.
        try:
    ...
    if provider == "deepinfra":
        ...
        if not api_key:
            return _relay("no credentials")
        from panergos_cli.models import deepinfra_base_url
...
def _resolve_tts_client_config() -> Dict[str, Any]:
    ...
    if provider == "openai":
        # Covers direct-key and custom-base-url selections.
        try:
            api_key, base_url, _ = tts_tool_openai._resolve_openai_audio_client_config()
        except ValueError as exc:
            return _relay(f"openai resolution failed: {exc}")
...
def resolve_client_voice_config() -> Dict[str, Any]:
    ...
    Callers scope the profile via ``panergos_constants.set_panergos_home_override``
