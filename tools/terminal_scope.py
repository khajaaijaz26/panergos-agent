...
_terminal_scope_var: ContextVar = ContextVar("panergos_terminal_scope", default=None)
...
def build_profile_terminal_scope(
    panergos_home: "Any", *, env_overlay: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    *env_overlay* is a TRUSTED ``TERMINAL_*`` mapping captured from the launch process before
    ...
    """
    from panergos_cli.config import TERMINAL_CONFIG_ENV_MAP, _terminal_env_value
    from panergos_cli.config_defaults import DEFAULT_CONFIG

    home = Path(panergos_home)
    scope: Dict[str, str] = {}
    ...
    if config_exists:
        from panergos_cli.config import fast_safe_load
...
def _resolve_scope_cwd_placeholder(scope: Dict[str, str]) -> None:
    ...
    ``resolve_agent_cwd()`` falls back to the multiplexer PROCESS cwd (wherever ``panergos gateway``
...
def install_profile_terminal_scope(
    panergos_home: "Any", *, env_overlay: Optional[Dict[str, str]] = None) -> Token:
    """Build AND install a profile's policy; on failure install the refusal scope. Never raises."""
    try:
        return set_terminal_scope(build_profile_terminal_scope(panergos_home, env_overlay=env_overlay))
...
@contextmanager
def install_and_reset_profile_terminal_scope(panergos_home: "Any") -> Iterator[None]:
    """Install the profile's terminal policy for a bounded turn/fire. Never raises."""
    token = install_profile_terminal_scope(panergos_home)
    try:
        yield
    finally:
