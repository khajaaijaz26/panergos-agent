...
# prefix, the exact-name PANERGOS_ allowlist, or (Windows) an OS-essential name.
# The broad "PANERGOS_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (PANERGOS_BASE_URL, PANERGOS_KANBAN_DB, *_WEBHOOK).
# PANERGOS_RPC_SOCKET / PANERGOS_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. PANERGOS_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports Panergos code loses the Kanban mutation guard
# while still inheriting PANERGOS_HOME.
_PANERGOS_CHILD_ALLOWED = frozenset({
    "PANERGOS_HOME", "PANERGOS_PROFILE", "PANERGOS_CONFIG", "PANERGOS_ENV", "PANERGOS_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational PANERGOS_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret PANERGOS_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    for k, v in source_env.items():
        ...
        if (any(k.startswith(p) for p in _SAFE_ENV_PREFIXES)
                or k in _PANERGOS_CHILD_ALLOWED
                or (is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS)):
            scrubbed[k] = v
        elif k.startswith("PANERGOS_"):
            _dropped_panergos.append(k)
    if _dropped_panergos:
        logger.debug(
            "execute_code: dropped %d non-allowlisted PANERGOS_* var(s) from the "
            ...
            len(_dropped_panergos), ", ".join(sorted(_dropped_panergos)),
        )
    ...
    # non-allowlisted PANERGOS_* values in otherwise ordinary execution environments.
    if scoped.get(DELEGATED_CHILD_ENV_MARKER):
        for key in (DELEGATED_CHILD_ENV_MARKER, "PANERGOS_KANBAN_DB", "PANERGOS_KANBAN_BOARD"):
            if key in scoped:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    from panergos_constants import apply_subprocess_home_env
    child_env = _scrub_child_env(os.environ)
    child_env["PANERGOS_RPC_SOCKET"] = rpc_endpoint
    child_env["PANERGOS_RPC_TOKEN"] = rpc_token
    ...
    # Only TZ reaches the child; PANERGOS_TIMEZONE is an internal setting (and under the multiplexed
    # gateway holds only the default profile's value — panergos_time resolves the routed profile's).
    from panergos_time import get_timezone_name

    _tz_name = get_timezone_name()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("PANERGOS_TIMEZONE", None)
    apply_subprocess_home_env(child_env)
    # PYTHONPATH: the staging dir (panergos_tools.py) must always be importable even when project
    # mode changes CWD. Panergos's root is added ONLY when the child runs in Panergos's Python env —
    # exposing Panergos's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited Panergos-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip Panergos-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-Panergos-
    ...
    _strip_panergos_owned_pythonpath(child_env)
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_panergos_python_environment(child_python):
        _pp_parts.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    elif child_python not in _external_env_logged:
        # Surface once per interpreter so "import panergos_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the Panergos "
                    "environment; Panergos root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
# Interpreter paths already reported as outside the Panergos environment.
...
def _uses_panergos_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Panergos's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the panergos root."""
