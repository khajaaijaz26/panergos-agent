...
from tools.bot_mode_probe import DEFAULT_BOT_HANDLE, DEFAULT_BOT_HANDLE_ALIASES, _default_home, _panergos_root
...
# Mirrors RELAY_DELIVER_TIMEOUT_MS in apps/desktop/src/plugins/panergos-bots/relay.ts; both test suites pin it.
...
# One turn in a profile's canonical Bot Chat: ``panergos -p <profile> *BOT_CHAT_TURN_ARGS``.
...
def _bot_mode_cfg(key: str, *, loader: str) -> Any:
    ...
    try:
        import panergos_cli.config as cfgmod
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    handle = str(row.get("handle") or "").strip().lstrip("@") or (DEFAULT_BOT_HANDLE if profile == "default" else profile)
...
def resolve_remote_target(raw_target: str, roster: list[dict]) -> Any:
    ...
    matches = [
        ...
        if (
            want_lower in (row["handle"].lower(), row["profile"].lower())
            or (
                want_lower in DEFAULT_BOT_HANDLE_ALIASES
                and (
                    row["profile"].lower() == "default"
                    or row["handle"].lower() in DEFAULT_BOT_HANDLE_ALIASES
                )
            )
        )
...
def cleanup_bot_relay_artifacts(max_age_hours: float | None = None) -> int:
    ...
    del max_age_hours
    try:
        base = relay_root(_panergos_root(Path(_default_home())))
...
def _panergos_cli() -> str:
    """panergos CLI beside this interpreter, then ``shutil.which``, then the bare name
    (service contexts lack PATH, so a bare "panergos" died with ENOENT).

    The deliver RPC runs on the target gateway, whose process is the venv python — its bin/Scripts directory
    holds the matching ``panergos`` entrypoint. A bare ``"panergos"`` relies on PATH, which is exactly what
    ...
    bin_dir = Path(sys.executable or "").parent
    sibling = bin_dir / f"panergos{suffix}"
    if sibling.is_file():
        return str(sibling)
    return shutil.which("panergos") or "panergos"
...
def local_delivery_command(profile: str, query_file: str) -> list[str]:
    ...
    return [_panergos_cli(), "-p", profile, *BOT_CHAT_TURN_ARGS, "--query-file", query_file]
...
def _delivery_child_session_env_names() -> "tuple[str, ...]":
    ...
    ``PANERGOS_SESSION_*`` prefix match, which would also strip non-identity knobs
    (e.g. ``PANERGOS_SESSION_STALL_TIMEOUT``)."""
...
def delivery_env(author: Optional[dict]) -> dict[str, str]:
    """Environment for one delivery turn's ``panergos`` child. The dispatcher's own PANERGOS_TURN_AUTHOR is
...
# Deliveries are separate ``panergos`` subprocesses, so the lock is a per-profile
...
# target profile must never run their Bot Chat turns concurrently: deliveries spawn separate ``panergos``
