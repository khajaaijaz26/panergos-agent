...
local → ``panergos -p <name> chat --in ~ -c "Bot Chat" --create-if-missing -Q
--query-file <tmp>``; peer → ``panergos peer dm <peer>[/<name>] < <tmp>``; both via
...
# delivery runner (``python bot_mode_dm.py --run-delivery …``); Panergos helpers import lazily.
...
_DM_DIR_NAME = "panergos-dm"
_DM_STALE_SECONDS = 24 * 60 * 60
_LIVE_WAIT_SECONDS = 300

# '<peer>/<agent>' — peer names are lowercase (``panergos peer`` normalizes them).
...
def _default_home() -> str:
    ...
    return str(get_process_panergos_home())
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'panergos' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map the current or legacy default handle to the default profile."""
    from tools.bot_mode_probe import DEFAULT_BOT_HANDLE_ALIASES

    want = target.strip().lower()
    if want in DEFAULT_BOT_HANDLE_ALIASES:
...
def message_agent_tool(target: str = "", message: str = "", task_id: Optional[str] = None, agent: Any = None) -> str:
    ...
    try:
        from tools.bot_mode_probe import (
            BOT_CHAT_TITLE, _handle, _panergos_root, _peers, _profile_name as _self_profile_name, _roster,
            is_bot_mode_managed,
        )
        from tools.bot_relay import BOT_CHAT_TURN_ARGS, _panergos_cli
    ...
    root, me = _panergos_root(Path(home)), _self_profile_name(Path(home))
    ...
    if peer_match or raw_target.lower() in peers:
        ...
        # Pin the registry-owning profile: `panergos peer` resolves bot_peers via the profile-scoped
        ...
        return _start_delivery([_panergos_cli(), "-p", _self_profile_name(root), "peer", "dm", dm_target], content,
    ...
    return _start_delivery([_panergos_cli(), "-p", resolved, *BOT_CHAT_TURN_ARGS], content, f"@{_handle(resolved)}",
...
def cleanup_bot_dm_cache(max_age_hours: float = _DM_STALE_SECONDS / 3600, *, now: float | None = None) -> int:
    ...
    locations = [(temp_root, "panergos-dm-*.txt"), (temp_root, "panergos-relay-dm-*.txt")]
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    if stdin_file or len(argv) < 3 or cli not in ("panergos", "panergos.exe") or argv[1] != "-p":
        return contextlib.nullcontext()
    from tools.bot_mode_probe import _panergos_root
    from tools.bot_relay import acquire_turn_lock

    return acquire_turn_lock(_panergos_root(Path(_default_home())), argv[2])
...
def _run_local_turn(argv: list[str], dm_file: str, *, env: Optional[dict[str, str]] = None) -> int:
    ...
    reason = next((line.removeprefix("panergos-refusal-reason: ").strip()
                   for line in stderr_text.splitlines()
                   if line.startswith("panergos-refusal-reason: ")), None)
...
def _local_delivery_home(argv: list[str]) -> Path | None:
    cli = (argv[0] if argv else "").rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
    if len(argv) < 3 or cli not in ("panergos", "panergos.exe") or argv[1] != "-p":
        return None
    from tools.bot_mode_probe import _panergos_root, _roster

    return dict(_roster(_panergos_root(Path(_default_home())))).get(argv[2])
...
def _run_delivery(argv: list[str], dm_file: str, *, stdin_file: bool,
    ...
    ``author`` rides to the child as PANERGOS_TURN_AUTHOR; ``panergos peer dm`` forwards it in the request body.
