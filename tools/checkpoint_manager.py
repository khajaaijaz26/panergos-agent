...
config / ``--checkpoints`` flag.  One store under ``~/.panergos/checkpoints/`` so git dedupes
blobs across projects (pre-v2 one-repo-per-workdir re-stored ~40 MB each): ``store/`` bare
repo with per-project ``refs/panergos/<hash16>``, ``indexes/<hash16>``, ``projects/<hash16>.json``
...
CHECKPOINT_BASE = get_panergos_home() / "checkpoints"
...
def _resolve_checkpoint_base() -> Path:
    ...
    changed it, else live profile-scoped PANERGOS_HOME — under the multiplexed gateway one process
    ...
    return CHECKPOINT_BASE if CHECKPOINT_BASE != _CHECKPOINT_BASE_AT_IMPORT else get_panergos_home() / "checkpoints"

_STORE_DIRNAME, _INDEXES_DIRNAME, _PROJECTS_DIRNAME, _LEDGERS_DIRNAME = "store", "indexes", "projects", "ledgers"
_REFS_PREFIX, _LEGACY_PREFIX, _PRUNE_MARKER_NAME = "refs/panergos", "legacy-", ".last_prune"
...
DEFAULT_EXCLUDES = [
    ...
    ".git/", ".hg/", ".svn/", ".worktrees/",  # VCS + worktrees (Panergos convention — don't snapshot siblings)
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("PANERGOS_CHECKPOINT_TIMEOUT", 30)))
...
_STORE_GIT_CONFIG = (("user.email", "panergos@local"), ("user.name", "Panergos Checkpoint"),
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file``/``patch`` produced, so restores can tell Panergos' writes from later user edits."""
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    users may want to recover; the archive falls under retention and ``panergos checkpoints
    ...
    logger.info("Migrated pre-v2 checkpoint repos to %s. "
                "Clear with `panergos checkpoints clear-legacy` when safe.", legacy_root)
    return legacy_root
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir() if base.exists() else ():
        ...
        try:
            if (child / "PANERGOS_WORKDIR").exists():
                workdir = (child / "PANERGOS_WORKDIR").read_text(encoding="utf-8").strip()
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Panergos just wrote (agent-write ledger), so safe-mode
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        """Classify files changed since ``commit_hash``: ``restore`` = still matching what Panergos
        last wrote (or deleted since); ``skipped`` = user-edited afterwards or never written by
        Panergos.  ``ledger_empty`` => no ledger, callers fall back to a full restore."""
        ...
        for rel in filter(None, names_out.split("\x00")):
            ...
            panergos_authored = recorded is not None and _hash_file(abs_path) in (None, recorded)
            out["restore" if panergos_authored else "skipped"].append(rel)
    ...
    def restore(self, working_dir: str, commit_hash: str, file_path: str = None,
        ...
        the user hand-edited after Panergos' last write untouched (agent-write ledger); the result
    ...
    def _apply_safe_restore_deletes(self, p: _ProjectRefs, commit_hash: str,
        ...
        from the checkpoint is Panergos-created (delete to restore) — unless ``max_file_size_mb`` kept
