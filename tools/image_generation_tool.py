#!/usr/bin/env python3
"""Image generation via FAL.ai (model picked in ``panergos tools``, persisted to ``image_gen.model``).
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    return _load_fal_client().submit(model, arguments=arguments)
...
_CACHE_BASE_BY_BACKEND = {"docker": "/root/.panergos", "singularity": "/root/.panergos",
                          "modal": "/root/.panergos", "ssh": "~/.panergos"}
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.panergos"
...
def _prepare_fal_request(model_id, meta, prompt, aspect_ratio, seed, overrides, source_images):
    ...
    if source_images and not edit_endpoint:
        raise ValueError(
            ...
            f"via `panergos tools` → Image Generation.")
...
def check_fal_api_key() -> bool:
    """True when direct FAL credentials are available."""
    return fal_key_is_configured()


def _build_no_backend_setup_message() -> str:
    """Actionable no-backend error for direct FAL or another plugin provider."""
    ...
    lines.append("  2. Configure a different image_gen provider via `panergos tools` → Image Generation "
                 "(run `panergos plugins list` to see installed backends)")
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    Providers without ``upscale`` ignore it via ``**kwargs``.
    """
    configured = _plugin_provider_name()
    if configured is None:
        return None
    ...
    if provider is None:
        return _provider_error(
            ...
            f"Run `panergos plugins list` to see available image gen backends.", "provider_not_registered")
    ...
    try:
        ...
        if is_type_error and ("image_url" in kwargs or "reference_image_urls" in kwargs):
            ...
            return _provider_error(
                ...
                f"text-to-image, or pick a backend that supports editing via `panergos tools` → "
...
def _handle_image_generate(args, **kw):
    ...
    # Explicit plugin providers take precedence over the direct FAL fallback.
