...
# resolve per call in production (below) because ``get_panergos_home()`` /
...
_panergos_config_resolved: str | None = None
_panergos_config_resolved_loaded = False
_real_panergos_home_cached: str | None = None
_real_panergos_home_loaded = False
...
def _panergos_home_real() -> str:
    from panergos_constants import get_panergos_home
    return os.path.realpath(str(get_panergos_home()))


def _get_panergos_config_resolved() -> str | None:
    """Resolved absolute path of the Panergos config file for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``PANERGOS_HOME`` scope (#107327);
    a test may pin it via ``_panergos_config_resolved`` + ``_panergos_config_resolved_loaded``."""
    if _panergos_config_resolved_loaded:
        return _panergos_config_resolved
    try:
        ...
        # home rather than the active multiplex ``PANERGOS_HOME`` — comparing
        ...
        # ``get_panergos_home()`` key the happy path uses, and substitute no
        # unrelated home if even that is gone (#107327 follow-up; PR #107335).
        try:
            from panergos_constants import get_panergos_home
            return str((Path(str(get_panergos_home())) / "config.yaml").resolve())
        except Exception:
            return None


def _get_real_panergos_home() -> str | None:
    """Realpath of the authoritative Panergos home for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``PANERGOS_HOME`` scope (#107327);
    a test may pin it via ``_real_panergos_home_cached`` + ``_real_panergos_home_loaded``."""
    if _real_panergos_home_loaded:
        return _real_panergos_home_cached
    try:
        return _panergos_home_real()
    except Exception:
        # Same active-profile binding on the exception path (see
        # ``_get_panergos_config_resolved``): re-derive from ``get_panergos_home()``
        # rather than ``_expand_tilde("~/.panergos")`` so the protected-instruction
        ...
        # the consumer: the ``~/.panergos`` exemption is skipped, so the gate runs.
        try:
            from panergos_constants import get_panergos_home
            return os.path.realpath(str(get_panergos_home()))
        except Exception:
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    panergos_config = _get_panergos_config_resolved()
    if panergos_config and panergos_config in candidates:
        return (
            f"Refusing to write to Panergos config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.panergos/config.yaml directly or use 'panergos config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .panergos tree).
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    try:
        from panergos_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.panergos itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.panergos`` component rule, which would match the home.
    real_home = _get_real_panergos_home()
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .panergos config dirs (<repo>/.panergos/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.panergos.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".panergos":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Panergos mirror prefix for persistent Docker file tools."""
    try:
