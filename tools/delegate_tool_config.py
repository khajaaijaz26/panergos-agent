...
# match panergos_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _runtime_provider_credentials(v: dict, explicit_request_overrides) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers include openrouter, zai, kimi-coding, and minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'panergos auth'."
        )
...
def _load_config() -> dict:
    ...
    (follows PANERGOS_HOME/profile; no deepcopy, since this runs on every get_definitions() rebuild) over the legacy
    ``cli.CLI_CONFIG``, which can hide user-set keys — except that ``PANERGOS_IGNORE_USER_CONFIG=1`` is only honored
    by the legacy loader, so it stays authoritative when that flag is set."""
    if os.environ.get("PANERGOS_IGNORE_USER_CONFIG") != "1":
        try:
...
def _resolve_child_runtime(
    ...
    # would pin a child subagent onto the parent's incompatible wire.
