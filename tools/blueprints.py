...
``metadata.panergos.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.panergos.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.panergos.blueprint`` is a mapping with a
    ...
    panergos = meta.get("panergos") if isinstance(meta, dict) else None
    blueprint = panergos.get("blueprint") if isinstance(panergos, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.panergos.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.panergos.blueprint`` block) ready for ``panergos skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"panergos": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
