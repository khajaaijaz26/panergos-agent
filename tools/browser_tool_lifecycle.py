...
def _emergency_cleanup_all_sessions():
    ...
    panergos processes — every clean exit reaps accumulated orphans, not only
    ...
    # owned by other live panergos processes.
    _best_effort("Orphan reap on exit", _reap_orphaned_browser_sessions)


@contextlib.contextmanager
def _session_owner_scope(task_id: str):
    """Run under the Panergos home + secret scope owning ``task_id``'s session (no-op if unrecorded).
    ...
    home_token = set_panergos_home_override(owner_home)
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record this panergos PID in ``<socket_dir>/<session>.owner_pid`` so the orphan
...
def _reap_socket_dir(socket_dir: str, session_name: str, tracked_names: set) -> bool:
    ...
    A live ``owner_pid`` means another panergos process owns it — leave it UNLESS untracked
...
def _reap_orphaned_browser_sessions():
    """Kill agent-browser daemons whose owning panergos process is gone (an unclean exit loses
    ...
    for prefix in ("agent-browser-h_*", "agent-browser-cdp_*", "agent-browser-panergos_*",
...
def _update_session_activity(task_id: str):
    """Touch the activity timestamp and record the owning Panergos home on first sight (the
...
def _cleanup_old_recordings(max_age_hours=72):
    ...
    try:
        recordings_dir = get_panergos_home() / "browser_recordings"
    except Exception as e:
