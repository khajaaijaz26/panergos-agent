...
# Panergos WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: PANERGOS_TINI_SHIM_TARGET overrides the `/init` path (and
# PANERGOS_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${PANERGOS_TINI_SHIM_TARGET:-/init}"
WRAPPER="${PANERGOS_TINI_SHIM_WRAPPER:-/opt/panergos/docker/main-wrapper.sh}"
