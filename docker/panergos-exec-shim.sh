#!/bin/sh
# shellcheck shell=sh
# /opt/panergos/bin/panergos — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `panergos` user (UID 10000). When an operator runs `docker exec <c> panergos ...`
# the default UID is root (0), and any file the command writes under
# $PANERGOS_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> panergos login`, this writes
...
# Fix
# ---
# This shim sits at /opt/panergos/bin/panergos and is placed earliest on PATH.
# When invoked as root, it drops to the panergos user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $PANERGOS_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user panergos`, kanban subagents, etc. — it short-circuits
...
# (/opt/panergos/.venv/bin/panergos), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set PANERGOS_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the panergos CLI. Default is to drop.

set -e

REAL=/opt/panergos/.venv/bin/panergos

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "panergos-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user panergos`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${PANERGOS_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the panergos user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "panergos-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "panergos-shim: re-run with --user panergos or set PANERGOS_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the panergos user's home before dropping privileges. Without
