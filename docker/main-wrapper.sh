#!/bin/sh
# shellcheck shell=sh
# /opt/panergos/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching PANERGOS_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `panergos` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `panergos <args>` (subcommand passthrough)
#
# Drop to panergos via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${PANERGOS_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${PANERGOS_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export PANERGOS_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset PANERGOS_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid panergos "$@"; exec "$@"; }
...
# an arbitrary non-root, non-panergos UID, the bootstrap was skipped and the
# baked image dirs (owned by the panergos build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u panergos)" ]; then
    cat >&2 <<EOF
[panergos] ERROR: container started with --user $cur_uid (an arbitrary, non-panergos UID) — not supported.
...
# to the panergos user's home before dropping privileges so libraries that
...
_panergos_orig_cwd="${PANERGOS_ORIG_CWD:-$PWD}"
...
# the user's command so `panergos chat` starts in the Docker -w
# directory, not /opt/data.
cd "$_panergos_orig_cwd"
...
# Panergos subcommand pass-through.
drop panergos "$@"
