# Contributing to Panergos Agent

Thank you for contributing to Panergos Agent. New work targets `main`; see
[PANERGOS.md](PANERGOS.md) for the implemented system and
[.agents-cli-spec.md](.agents-cli-spec.md) for the roadmap.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Panergos to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo khajaaijaz26/panergos-agent "<your terms>"
  gh search prs --repo khajaaijaz26/panergos-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/khajaaijaz26/panergos-agent/issues?q=) · [PRs (all states)](https://github.com/khajaaijaz26/panergos-agent/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Panergos itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Panergos install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `panergos skills browse` (labeled "official") and install it with `panergos skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub**. Users can install it with `panergos skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.panergos/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(panergos_home, config)` for setup-wizard integration
...
- Integrate with `panergos memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Panergos ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.panergos/plugins/`, project `.panergos/plugins/`, or a pip entry point) — see [Build a plugin](website/docs/developer-guide/plugins/index.md)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Publish clear installation instructions so users can find and install it
...
The installer creates the Panergos virtual environment, wires the `panergos` command, stamps the
install method for `panergos update`, and clones the full git project into
`$PANERGOS_HOME/panergos-agent` (usually `~/.panergos/panergos-agent`). That keeps your
development environment on the same layout the CLI, updater, lazy dependency
installer, gateway, and docs assume.

```bash
curl -fsSL https://raw.githubusercontent.com/khajaaijaz26/panergos-agent/main/scripts/install.sh | bash
cd "${PANERGOS_HOME:-$HOME/.panergos}/panergos-agent"
...
Use this only if you intentionally do not want Panergos' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `panergos` entrypoint from this venv; running the
system `python3 -m panergos_cli.main` can pick up unrelated system Python
packages.
...
uv venv ~/.panergos/venvs/panergos-dev --python 3.11
...
```bash
mkdir -p ~/.panergos/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.panergos/config.yaml
touch ~/.panergos/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.panergos/.env
```

### Run

```bash
# The standard installer already put `panergos` on PATH.
panergos doctor
panergos chat -q "Hello"
```

If you used the manual clone fallback, invoke the venv entrypoint or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$VIRTUAL_ENV/bin/panergos" ~/.local/bin/panergos
```
...
├── cli.py                    # PanergosCLI class — interactive CLI orchestrator (~4.6k LOC + panergos_cli/cli_*_mixin.py)
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (panergos-cli, panergos-telegram, etc.)
├── panergos_state.py           # SessionDB facade (~1.4k LOC); implementation in panergos_state_*.py (21 siblings) — FTS5 search, session titles
...
│   ├── display.py                # RelaySpinner, tool progress formatting
│   ├── model_metadata.py         # Model context lengths, token estimation
│   └── trajectory.py             # Trajectory saving helpers
│
├── panergos_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── cli_*_mixin.py            # PanergosCLI mixins (slash commands, display, session, ...)
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider and credential resolution
...
│
├── skills/                   # Bundled skills (copied to ~/.panergos/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Panergos documentation site (inherited Docusaurus content)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.panergos/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.panergos/`)

| Path | Purpose |
|------|---------|
| `~/.panergos/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.panergos/.env` | API keys and secrets |
| `~/.panergos/auth.json` | Compatible provider credentials |
| `~/.panergos/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.panergos/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.panergos/state.db` | SQLite session database |
| `~/.panergos/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and explicit `/save` exports. Automatic per-session JSON snapshots are no longer written; state.db is canonical. |
| `~/.panergos/cron/` | Scheduled job data |
| `~/.panergos/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`panergos_state.py`) with full-text search and unique session titles. Automatic per-session JSON snapshots have been removed. Existing files are left untouched; use `/save json` or `panergos sessions export` for an explicit export.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with direct providers, local runtimes, and OpenAI-compatible custom endpoints. Provider resolution happens at initialization.
...
(for example `_PANERGOS_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.panergos`:
...
The user may skip setup and keep loading the skill. Panergos only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `panergos setup` or update `~/.panergos/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Panergos tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Panergos Agent" is the secondary collaborator. If the contributor's commit shows "Panergos Agent" as author because they used Panergos to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and the Panergos tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `panergos --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Panergos uses the inherited data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.panergos/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `panergos_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `panergos_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Panergos runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the inherited runtime wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `panergos_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `panergos_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `panergos_cli/gateway_windows.py::
...
Panergos has terminal access. Security matters.
...
2. **Test manually**: Run `panergos` and exercise the code path you changed
...
- Use [Panergos GitHub Issues](https://github.com/khajaaijaz26/panergos-agent/issues)
- Include: OS, Python version, Panergos version (`panergos --version`), full error traceback
