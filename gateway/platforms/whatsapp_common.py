...
class WhatsAppBehaviorMixin(OwnAccessPolicyMixin):
    ...
    DEFAULT_REPLY_PREFIX: str = "◇ *Panergos Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    /opt/panergos) is mirrored to PANERGOS_HOME so npm install works."""
    import shutil
    from panergos_constants import get_panergos_home
    install_bridge = Path(__file__).resolve().parents[2] / "scripts" / "whatsapp-bridge"
    panergos_home_bridge = get_panergos_home() / "scripts" / "whatsapp-bridge"
    try:
        ...
        pass
    if panergos_home_bridge.exists():
        return panergos_home_bridge
    try:
        panergos_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(install_bridge, panergos_home_bridge, dirs_exist_ok=False)
        return panergos_home_bridge
    except Exception:
        return install_bridge
