...
There are two ways to add a platform to the Panergos gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.panergos/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Panergos code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `panergos gateway status` or `get_connected_platforms()` until the SDK
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `panergos_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Panergos core.
...
```python
"panergos-your-platform": {
    "description": "Your Platform bot toolset",
    "tools": _PANERGOS_CORE_TOOLS,
    "includes": []
},
```

And add it to the `panergos-gateway` composite:

```python
"panergos-gateway": {
    "includes": [..., "panergos-your-platform"]
}
```
...
## 12. Status Display (`panergos_cli/status.py`)
...
## 13. Gateway Setup Wizard (`panergos_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ panergos_cli/ toolsets.py \
  --include="*.py" -l | sort -u
