...
def _get_panergos_version() -> str:
    """Return the panergos-agent package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    """``QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Panergos/<panergos_version>)``."""
    v = sys.version_info
    return (f"QQBotAdapter/{QQBOT_VERSION} (Python/{v.major}.{v.minor}.{v.micro}; "
            f"{platform.system().lower()}; Panergos/{_get_panergos_version()})")
