...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.panergos to the active profile home."""
    ...
    if raw == "~/.panergos" or raw.startswith("~/.panergos/"):
        return panergos_home / raw[len("~/.panergos/"):]
    path = Path(raw).expanduser()
    return path if path.is_absolute() else panergos_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script; must live under PANERGOS_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from panergos_constants import get_panergos_home
    scripts_root = (get_panergos_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.panergos" or raw_text.startswith("~/.panergos/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        Non-zero exit, empty/``[SILENT]`` stdout, or a ``[SILENT]``/``__panergos_ignore__`` flag drops the
        ...
        if not isinstance(transformed, dict):
            ...
            return False, None
        silenced = transformed.get("[SILENT]") is True or transformed.get("__panergos_ignore__") is True
