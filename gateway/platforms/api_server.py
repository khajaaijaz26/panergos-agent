...
def _prefix_names_served_profile(profile: str) -> bool:
    ...
    try:
        from panergos_cli.profiles import profile_matches_home
...
_STATIC_FEATURE_FLAGS = {
    ...
    "session_continuity_header": "X-Panergos-Session-Id",
    "session_key_header": "X-Panergos-Session-Key"}
...
_BROWSER_CONTROL_WS_PROTOCOL = "panergos-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "panergos-browser-control-ticket."
...
def _panergos_version() -> str:
    """Canonical Panergos version: ``panergos_cli.__version__`` (dist-info can be stale on
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    from panergos_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    try:
...
def _request_agent_overrides(
    ...
    The virtual model (``panergos-agent``) means "gateway default". A bare ``model`` without
    ``provider`` is honored only when ``allow_bare_model`` (generic clients hardcode "gpt-4o";
    OpenAI-compatible handlers pass the ``direct_model_requests`` opt-in, Panergos-native
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            ...
            with suppress(Exception):
                ...
                db_path = str(get_panergos_home() / "response_store.db")
        ...
        from panergos_state_wal import apply_wal_with_fallback
...
def _derive_chat_session_id(system_prompt: Optional[str], first_user_message: str) -> str:
    ...
    turns of an Open WebUI-style conversation), so one Panergos session/sandbox is reused."""
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    """aiohttp server routing OpenAI-format requests through panergos-agent's AIAgent."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # executing model. Requests that send an explicit ``provider`` — and the Panergos-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        """Advertised /v1/models name: explicit override > active profile name > "panergos-agent"
        (precedence owned by ``panergos_cli.model_switch.resolve_effective_model``)."""
        from panergos_cli.model_switch import resolve_effective_model
        profile_name = ""
        with suppress(Exception):
        ...
        return resolve_effective_model(explicit, profile_name, "panergos-agent")
        ...
        if not profile:
            with suppress(Exception):
                ...
                if is_multiplex_active():
                    ...
                    from panergos_constants import get_panergos_home
                    return _profile_runtime_scope(get_panergos_home())
            return nullcontext()
        from gateway.run import _profile_runtime_scope
    ...
    def _declared_conversation_session(self, gateway_session_key: Optional[str]) -> Optional[str]:
        """Resolve the live session a client declared with ``X-Panergos-Session-Key`` (the key
    ...
    def _parse_session_key_header(
        ...
        """Validate ``X-Panergos-Session-Key`` (per-channel memory scope) -> ``(key_or_None, None)``
        or ``(None, error)``. Requires API-key auth so a client can't guess another scope."""
        raw = request.headers.get("X-Panergos-Session-Key", "").strip()
        if not raw:
            return None, None
        if not self._api_key:
            logger.warning(
                "X-Panergos-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping.")
            return None, _error_response(
                "X-Panergos-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        """SessionDB for the active profile home (the runtime scope redirects ``get_panergos_home()``
        per profile). Sync, for ``_create_agent``; handlers use ``_ensure_session_db_async``."""
        if self._session_db is not None:
            return self._session_db
        try:
            from panergos_constants import get_panergos_home
            return self._open_and_cache_session_db(get_panergos_home())
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        "panergos-agent" upstream as a provider model id 400s)."""
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "panergos_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text not in allowed:
            return "api_server"
        return "panergos_browser" if text == "browser" else text
    ...
    def _recover_or_record_model(self, model: str, runtime_kwargs: Dict[str, Any], gateway_session_key) -> str:
        ...
        # No model.default but a provider resolved (e.g. `panergos auth add` without `panergos model`).
    ...
    def _select_agent_runtime(
        ...
        # panergos_cli.model_switch.resolve_effective_model.
        from panergos_cli.model_switch import resolve_effective_model
        if session_override:
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "panergos-agent", "version": _panergos_version()})
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "status": readiness["status"], "readiness": readiness, "platform": "panergos-agent",
            "version": _panergos_version(), "gateway_state": gw_state,
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — panergos-agent plus configured model_routes aliases (alias + resolved
        ...
        def _model(mid: str, root: str, parent) -> Dict[str, Any]:
            return {"id": mid, "object": "model", "created": now, "owned_by": "panergos", "permission": [],
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        ...
        try:
            from panergos_cli.inventory import build_model_options_payload, load_picker_context
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "description": (
                    "The API server creates a server-side Panergos AIAgent; "
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<PANERGOS_HOME>/plugin-data/.../artifacts``-style controlled root), so artifacts never escape the
        ...
        try:
            ...
            # Unscoped fallback (tests/manual wiring): controlled root under the Panergos home.
            try:
                from panergos_state import get_panergos_home
                root = Path(get_panergos_home()) / "artifacts" / "browser-control"
            except Exception:
    ...
    async def _handle_toolsets(self, request: "web.Request") -> "web.Response":
        ...
        try:
            ...
            from panergos_cli.tools_config import (
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Panergos sessions."""
        db = await self._ensure_session_db_async()
        if db is None:
        ...
        # Exact-title lookup (`panergos peer dm` -> canonical "Bot Chat"). include_hidden is honored
        ...
        if title_filter and not sessions:
            # A canonical Bot Chat auto-archived by the orphan reaper would make `panergos peer dm`
            ...
            try:
                ...
                # which would fail `panergos peer dm` resolution and mint transient sessions — same accident
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Panergos session row. Existence check, insert and
        ...
        # would persist "panergos-agent" and later send it to the provider literally.
        ...
        return web.json_response({"object": "panergos.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "panergos.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "panergos.session", "session": self._session_response(fork)}, status=201)
    ...
    def _session_headers(session_id: str, gateway_session_key: Optional[str]) -> Dict[str, str]:
        """``X-Panergos-Session-Id`` (+ ``X-Panergos-Session-Key`` when declared) response headers."""
        headers = {"X-Panergos-Session-Id": session_id}
        if gateway_session_key:
            headers["X-Panergos-Session-Key"] = gateway_session_key
        return headers
    ...
    def _effective_turn_runtime(self, runtime_request: Dict[str, Any], result: Any, usage: Any) -> Dict[str, Any]:
        ...
        return self._sanitize_runtime_metadata(
            ...
            # gateway/status readers resolve process-level paths and do NOT follow the PANERGOS_HOME
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"object": "panergos.session.chat.completion",
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        runtime = self._sanitize_runtime_metadata(
            ...
            model_lock="accepted")
        return web.json_response(
            {"object": "panergos.session.model_lock", "session_id": session_id, "runtime": runtime})
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        # Optional transient per-run context (standalone `panergos cron run` /
    ...
    def _bind_api_server_session(
        *, chat_id: str = "", session_key: str = "", session_id: str = "", profile: str = "",
        ...
        reach ``PANERGOS_SESSION_PROFILE``: the persistent-Docker container key is derived from it, so an
    ...
    def _turn_runtime_metadata(
        ...
        runtime = dict(getattr(agent, "_panergos_api_runtime", {}) or {})
