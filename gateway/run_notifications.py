...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``panergos update --gateway`` and its watcher exchange under PANERGOS_HOME."""
    ...
    async def _deliver_media_from_response(
        ...
        with _log_suppressed(logging.WARNING, "Post-stream media extraction failed: %s"):
            ...
            if image_paths:
                try:
                    images = [(Path(p).as_uri(), "") for p in image_paths]
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_panergos_home / ".update_pending.json",
            claimed=_panergos_home / ".update_pending.claimed.json", output=_panergos_home / ".update_output.txt",
            exit_code=_panergos_home / ".update_exit_code",
            prompt=_panergos_home / ".update_prompt.json", response=_panergos_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        """Watch ``panergos update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ Panergos update finished." if exit_code == 0
                        else "❌ Panergos update failed (exit code {}).".format(exit_code)
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ Panergos update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
        ...
        try:
            ...
            if chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    status = "✅ Panergos update finished." if exit_code == 0 else "❌ Panergos update failed."
                    msg = f"{status}\n\n```\n{output}\n```"
                else:
                    msg = (
                        "✅ Panergos update finished successfully." if exit_code == 0 else
                        "❌ Panergos update failed. Check the gateway logs or run `panergos update` manually for details."
                    )
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _panergos_home, _non_conversational_metadata
        notify_path = _panergos_home / ".restart_notify.json"
    ...
    async def _send_home_channel_startup_notifications(
        ...
        message = "♻️ Gateway online — Panergos is back and ready."
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if getattr(self, "_session_db_handle_cache", None) is not None:
            ...
            if not error:
                ...
                return
        from panergos_constants import get_default_panergos_root, profile_cli_selector
        from panergos_state import _default_db_path, classify_persistence_error, format_session_db_unavailable
        cause = classify_persistence_error(error)
        # Copy-pasteable, so name the real store and pin the profile: a bare `panergos` follows
        # active_profile, which may be a different database (#105887).
        profile_arg = profile_cli_selector()
        if cause == "corrupt":
            db_path = _default_db_path()
            backups_dir = get_default_panergos_root() / "backups"
            message = (
                "⚠️ Session database corruption detected. Messages may not be "
                "persisted. Recovery options:\n"
                f"1. Run `panergos {profile_arg}doctor --fix`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   panergos {profile_arg}sessions recover --source {db_path} "
                "--inspect-only\n"
                f"   (if it reports recoverable) panergos {profile_arg}sessions recover "
                ...
                f"Run `panergos {profile_arg}doctor` for sanitized diagnostics."
            )
            ...
            message = (
                ...
                f"it is repaired: run `panergos {profile_arg}doctor --fix`, then restart the gateway. Do not run "
                "recovery tools or restore a backup unless `panergos doctor` confirms damage."
            )
        else:
            message = (
                f"⚠️ Session database unavailable — messages may not be persisted. "
                f"{format_session_db_unavailable()}\nRun `panergos doctor` for diagnostics."
            )
    ...
    async def _inject_watch_notification(
        ...
        if not source:
            # API-server sessions bind the RAW X-Panergos-Session-Id key, not a structured ``agent:...`` key.
    ...
    def _completion_event_scope(self, evt: dict):
        ...
        (``tools.async_delegation`` → ``get_panergos_home()/state.db``) resolve from the ambient scope.
        ...
        if get_panergos_home_override() == str(profile_home):
            return contextlib.nullcontext()
