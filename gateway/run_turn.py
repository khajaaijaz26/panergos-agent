...
class GatewayTurnMixin:
    ...
    def _resolve_session_agent_runtime(
        ...
        # Provider resolved but no model.default (`panergos auth add` without `panergos model`): use the
    ...
    def _resolve_turn_agent_config(self, user_message: str, model: str, runtime_kwargs: dict) -> dict:
        ...
        from panergos_cli.models import resolve_fast_mode_overrides
    ...
    async def _hmwa_acquire_turn_lease(self, _quick_key, run_generation, session_entry, _session_env_tokens):
        ...
        try:
            _lease_token = await _lease_registry.acquire(
                ...
                timeout=_float_env("PANERGOS_TURN_LEASE_TIMEOUT", DEFAULT_LEASE_WAIT),
            )
    ...
    async def _hmwa_hygiene_settings(self, source, session_key):
        ...
        try:
            ...
            if hs.config_context_length is not None:
                try:
                    from panergos_cli.route_identity import should_clear_context_pin_async
    ...
    async def _hmwa_hygiene_detached_attempt(
        ...
        try:
            ...
            attempt.future = loop.run_in_executor(
                ...
                # secret scope / PANERGOS_HOME override live in ContextVars, and a bare run_in_executor worker
    ...
    async def _hmwa_first_contact_notes(self, source, history, turn_sidecar_notes):
        ...
        from gateway.run import _panergos_home, _home_target_env_var, _load_gateway_config
        if history:
            return
        if not await self.async_session_store.has_any_sessions():
            ...
            try:
                ...
                if profile_build_mode(_onb_cfg) == "ask" and not is_seen(_onb_cfg, PROFILE_BUILD_FLAG):
                    ...
                    mark_seen(_panergos_home / "config.yaml", PROFILE_BUILD_FLAG)
                else:
        ...
        if not home_env:
            with suppress(Exception):
                ...
                if prof and prof != "default" and _lgc().get_home_channel(source.platform):
                    home_env = "set"
        if not home_env:
            # Slack routes every command through the parent `/panergos`; bare `/sethome` would fail.
            sethome_cmd = "/panergos sethome" if source.platform == Platform.SLACK else "/sethome"
            await self._deliver_platform_notice(
                source, f"📬 No home channel is set for {platform_name.title()}. "
                f"A home channel is where Panergos delivers cron job results and cross-platform "
    ...
    def _hmwa_apply_message_timestamp(self, event, message_text):
        ...
        try:
            from panergos_time import get_timezone as _get_evt_tz
    ...
    async def _execute_mcp_reload(self, event: MessageEvent) -> str:
        ...
        if multiplex and not get_panergos_home_override():
        ...
        try:
            ...
            # a browser OAuth flow either: an expired token parks with a `panergos mcp login` hint.
    ...
    async def _run_agent_via_proxy(
        ...
        """Forward the message to a remote Panergos API server instead of running a local AIAgent.
        ...
        # OpenAI chat format. The remote keeps continuity via X-Panergos-Session-Id; send the current
        ...
        body = {"model": "panergos-agent", "messages": api_messages, "stream": True}
    ...
    def _run_agent_display_settings(self, source: SessionSource) -> "GatewayRunner._RunAgentDisplay":
        ...
        progress_mode, _tool_progress_explicit = resolve_tool_progress(
            user_config, platform_key, os.getenv("PANERGOS_TOOL_PROGRESS_MODE"),
        )
        ...
        # "log" mode: tool calls go to ~/.panergos/logs/tool_calls.log instead of the chat. Gateway-only.
    ...
    async def _run_agent_write_tool_log(self, log_queue: Any) -> None:
        ...
        tool_logger = logging.getLogger(f"panergos.tool_calls.{id(log_queue)}")
    ...
    def _run_agent_start_turn_worker(self, turn_ctx: TurnContext, run_sync: Callable[[], Any]) -> "GatewayRunner._RunAgentWorker":
        ...
        *Inactivity* timeout (agent.gateway_timeout / PANERGOS_AGENT_TIMEOUT, env wins; 0 = unlimited),
        ...
        _agent_timeout, _agent_warning = (
            v if v > 0 else None
            for v in (_float_env("PANERGOS_AGENT_TIMEOUT", 1800), _float_env("PANERGOS_AGENT_TIMEOUT_WARNING", 900))
        )
    ...
    def _run_agent_evict_on_fallback(self, turn_ctx: TurnContext) -> None:
        ...
        with suppress(Exception):
            from panergos_cli.model_normalize import _AGGREGATOR_PROVIDERS, normalize_model_for_provider
    ...
    async def _run_agent_drain_pending(
        ...
        if pending and pending.strip().startswith("/"):
            ...
            if _pending_cmd_word:
                with suppress(Exception):
                    from panergos_cli.commands import resolve_command as _rc_pending
    ...
    async def _run_agent_notify_long_running(
        ...
        Interval: agent.gateway_notify_interval / PANERGOS_AGENT_NOTIFY_INTERVAL (default 180s; 0 or
        ...
        _NOTIFY_INTERVAL = _float_env("PANERGOS_AGENT_NOTIFY_INTERVAL", 180)
