...
def _kbc():
    from panergos_cli import kanban_db_connect
    return kanban_db_connect


def _kbd():
    from panergos_cli import kanban_db_dispatch
...
class _KanbanDispatcher:
        ...
        try:
            ...
            if self.is_corrupt_board_db_error(exc):
                ...
                logger.error(
                    ...
                    "then run `panergos kanban init` if you need a fresh board.",
                    slug, fingerprint[0],
                )
                return None
    ...
    def auto_decompose_tick(self, auto_decompose_per_tick: int) -> int:
        ...
        """
        try:
            from panergos_cli import kanban_decompose as _decomp
        ...
        with _default_profile_secret_scope():
            for slug in self._board_slugs():
                ...
                prev_env = os.environ.get("PANERGOS_KANBAN_BOARD")
                try:
                    os.environ["PANERGOS_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        ...
                        os.environ["PANERGOS_KANBAN_BOARD"] = prev_env
        return successes
...
def _default_profile_secret_scope():
    ...
    LLM reads ``auxiliary.*`` from ``get_panergos_home()``, so its credentials come
    ...
    if not is_multiplex_active():
        yield
        return
    token = set_secret_scope(build_profile_secret_scope(Path(get_panergos_home())))
    try:
        yield
    finally:
