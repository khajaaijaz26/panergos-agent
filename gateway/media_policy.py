...
``validate_media_delivery_path`` reads ``PANERGOS_MEDIA_DELIVERY_STRICT`` (gateway.strict),
``PANERGOS_MEDIA_ALLOW_DIRS`` (gateway.media_delivery_allow_dirs) and
``PANERGOS_MEDIA_TRUST_RECENT_FILES`` (gateway.trust_recent_files).  Every delivery
entrypoint (gateway startup, ``panergos cron run``, ``panergos send``) calls
...
_FLAG_ENVS = (("strict", "PANERGOS_MEDIA_DELIVERY_STRICT"), ("trust_recent_files", "PANERGOS_MEDIA_TRUST_RECENT_FILES"))
_ALLOW_DIRS_ENV = "PANERGOS_MEDIA_ALLOW_DIRS"
_TRUST_RECENT_SECONDS_ENV = "PANERGOS_MEDIA_TRUST_RECENT_SECONDS"
_TRUTHY = frozenset({"1", "true", "yes", "on"})


def _routed_gateway_cfg() -> Optional[Dict[str, Any]]:
    """``gateway`` section of the ROUTED profile's config when a PANERGOS_HOME override is active
    ...
    if not get_panergos_home_override():
        return None
    try:
