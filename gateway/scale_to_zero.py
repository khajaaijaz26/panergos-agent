...
SCALE_TO_ZERO_ENV = "PANERGOS_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(panergos_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under PANERGOS_HOME."""
    if panergos_home is None:
        from panergos_constants import get_panergos_home
        panergos_home = get_panergos_home()
    return Path(panergos_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
