...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``panergos setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``panergos gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved PANERGOS_HOME and overlay the
        ...
        # `panergos` invocation, so the import happens only when a lookup asks for it.
