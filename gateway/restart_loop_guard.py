...
Defenses 1-2 (``_PANERGOS_GATEWAY`` guard on ``panergos gateway stop|restart`` /
...
persisted to ``<PANERGOS_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_panergos_home() / "gateway" / "restart_loop.json"
