...
All hooks write atomic JSON payloads under ``<panergos_home>/pending_messages/``:
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active PANERGOS_HOME."""
    from panergos_constants import get_panergos_home
    flush_dir = get_panergos_home() / "pending_messages"
...
def spool_dropped_transcript_message(session_id: str, message: Dict[str, Any]) -> Optional[Path]:
    ...
    message under ``<panergos_home>/pending_messages/``), so a runtime cap rotation no longer silently
