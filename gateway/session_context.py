"""Session-scoped context variables for the Panergos gateway.

Replaces the old ``os.environ``-based ``PANERGOS_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    _BROWSER_CONTROL_SESSION_ID, _BROWSER_CONTROL_RUN_ID, _CRON_SESSION, _SESSION_PARENT_CHAT_ID,
) = tuple(ContextVar(name, default=_UNSET) for name in (
    "PANERGOS_SESSION_PLATFORM", "PANERGOS_SESSION_SOURCE", "PANERGOS_SESSION_CHAT_ID",
    "PANERGOS_SESSION_CHAT_TYPE", "PANERGOS_SESSION_CHAT_NAME", "PANERGOS_SESSION_THREAD_ID",
    "PANERGOS_SESSION_USER_ID", "PANERGOS_SESSION_USER_ID_ALT", "PANERGOS_SESSION_USER_NAME",
    "PANERGOS_SESSION_SCOPE_ID", "PANERGOS_SESSION_KEY", "PANERGOS_SESSION_ID",
    "PANERGOS_UI_SESSION_ID", "PANERGOS_SESSION_MESSAGE_ID", "PANERGOS_SESSION_PROFILE",
    "PANERGOS_BROWSER_CONTROL_PRINCIPAL", "PANERGOS_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "PANERGOS_BROWSER_CONTROL_SESSION_ID", "PANERGOS_BROWSER_CONTROL_RUN_ID",
    "PANERGOS_CRON_SESSION", "PANERGOS_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("PANERGOS_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("PANERGOS_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("PANERGOS_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("PANERGOS_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("PANERGOS_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``PANERGOS_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def set_session_vars(
    ...
    browser_control_transport_family: str = "", browser_control_session_id: str = "",
    browser_control_run_id: str = "", cwd: str = "", async_delivery: bool = True,
    ...
    values = (
        ...
        browser_control_session_id, browser_control_run_id, cron_session, parent_chat_id,
    )
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``PANERGOS_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds PANERGOS_SESSION_PLATFORM, CLI/TUI/
# desktop bind PANERGOS_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``PANERGOS_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("PANERGOS_PLATFORM") or get_session_env("PANERGOS_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("PANERGOS_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See khajaaijaz26/panergos-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``PANERGOS_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
