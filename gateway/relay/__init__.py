"""Relay/connector support package for the Panergos gateway.
...
def relay_explicitly_disabled() -> bool:
    ...
    try:
        cfg = read_yaml_layers(get_panergos_home())
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from panergos_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # prefix every reply with the stock brand and shadow the connector's
        # linked-owner fallback, which actually disambiguates. Only a customized
        # name is forwarded.
        if value in {"Panergos Agent", "Panergos Agent"}:
            value = ""
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``panergos gateway enroll``.
    ...
    if not token_url:
        raise RuntimeError("gateway relay identity requires gateway.idp.token_url")
...
def self_provision_relay() -> bool:
    ...
    ``~/.panergos/.env``), so a hosted container re-provisions every boot; the
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'panergos'}"
