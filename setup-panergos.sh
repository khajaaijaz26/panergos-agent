...
# 5. Symlinks the 'panergos' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}▶ Panergos Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/panergos-uv-install.$$.log")"
...
if is_termux; then
    # Panergos currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Panergos requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink panergos into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up panergos command..."

PANERGOS_BIN="$SCRIPT_DIR/venv/bin/panergos"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$PANERGOS_BIN" "$COMMAND_LINK_DIR/panergos"
echo -e "${GREEN}✓${NC} Symlinked panergos → $COMMAND_LINK_DISPLAY_DIR/panergos"
...
# ============================================================================
# Seed bundled skills into ~/.panergos/skills/
# ============================================================================

PANERGOS_SKILLS_DIR="${PANERGOS_HOME:-$HOME/.panergos}/skills"
mkdir -p "$PANERGOS_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.panergos/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$PANERGOS_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  panergos status        # Check configuration"
if is_termux; then
    echo "  panergos gateway       # Run gateway in foreground"
else
    echo "  panergos gateway install # Install gateway service (messaging + cron)"
fi
echo "  panergos cron list     # View scheduled jobs"
echo "  panergos doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m panergos_cli.main setup
fi
