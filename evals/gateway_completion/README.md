...
Run with a fresh HOME and PANERGOS_HOME using the repository virtualenv:

```sh
probe=$(mktemp -d)
HOME="$probe" PANERGOS_HOME="$probe/state" .venv/bin/python evals/gateway_completion/delivery_lifecycle.py
```
...
and unavailable raw API-owner retention. `PANERGOS_EVAL_REPO` selects another checkout for A/B.
