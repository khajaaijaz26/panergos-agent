#!/usr/bin/env python3
"""Local A/B harness for `panergos auth add openrouter --type oauth` against a FAKE OpenRouter.

Runs the REAL entry point (``panergos_cli.auth_commands.auth_add_command``) with a temp PANERGOS_HOME.
...
Scenarios (each prints PASS/FAIL, exit code = number of failures):
  ...
  api_key_path   `panergos auth add openrouter --api-key` still works with no --type (regression)

Usage: PANERGOS_PYTHON=<venv python> python3 evals/openrouter_pkce_ab/harness.py [--json OUT]
...
def run(scenario: str, fake: FakeOpenRouter, home: str) -> dict:
    ...
    for m in [m for m in sys.modules if m.startswith(("panergos_cli", "agent", "panergos_constants"))]:
        del sys.modules[m]
    ...
    try:  # BEFORE (origin/main) has no auth_openrouter sibling; the flow itself must then fail.
        ...
        import panergos_cli.auth_device_flow as dfl
