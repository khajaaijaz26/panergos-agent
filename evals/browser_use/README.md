...
The A/B battery behind PR [#81958](https://github.com/khajaaijaz26/panergos-agent/pull/81958)
...
- **Arms differ only by tree + config.** `base` runs the built-in twelve
  ...
  digest's value). Each cell gets a throwaway `PANERGOS_HOME`; web-fetch
...
- **Backend matrix.** `orchestrate.py` drives a local headless-Chrome CDP;
  `orchestrate_cloud.py --backend browserbase` provisions a cloud browser per cell.
