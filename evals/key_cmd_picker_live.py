...
Run with the Panergos venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by Panergos children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def rows_worker():
    ...
    from panergos_cli.model_switch_providers import (
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "panergos_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="panergos-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".panergos"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "PANERGOS_HOME": str(state),
