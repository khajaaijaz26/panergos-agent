...
Run from the repo with a clean environment and a temporary PANERGOS_HOME:
...
from panergos_cli import heartbeat  # noqa: E402
...
async def main(base_poller):
    assert os.environ.get("PANERGOS_HOME"), "Use a temporary PANERGOS_HOME"
    ...
    with patch.object(heartbeat, "time", SimpleNamespace(time=lambda: clock.now)):
        ...
        if not base_poller:
            ...
            runner.session_store = SessionStore(
                sessions_dir=Path(os.environ["PANERGOS_HOME"]) / "sessions", config=runner.config)
