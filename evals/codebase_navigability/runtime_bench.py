...
Each probe runs in a FRESH subprocess with an isolated PANERGOS_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "PANERGOS_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "PANERGOS_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("PANERGOS_SESSION", "PANERGOS_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "panergos_cli.main", "gateway.run", "tools.registry", "panergos_state", "tui_gateway.server", "panergos_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "panergos-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `panergos --version`, `panergos --help`, `panergos doctor --help`, `panergos config get model` (no network)
CLI = {"version": ["panergos_cli/main.py", "--version"], "help": ["panergos_cli/main.py", "--help"], "config_get": ["panergos_cli/main.py", "config", "get", "model"], "tools_list": ["panergos_cli/main.py", "tools", "--help"], "skills_help": ["panergos_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_panergos_default_ms", lambda: resolve_toolset("panergos-default"), 200)
...
try:
    import panergos_state, tempfile, uuid
    from pathlib import Path
    db=panergos_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(panergos_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
