...
with tempfile.TemporaryDirectory(prefix="delegate-schema-") as home:
    os.environ.clear()
    os.environ.update(HOME=home, PANERGOS_HOME=home, PATH="/usr/bin:/bin", PANERGOS_PLATFORM="cli")
    ...
    definitions = get_tool_definitions(enabled_toolsets=["panergos-cli"], quiet_mode=True)
    serialized = json.dumps(definitions, separators=(",", ":"))
    repeated = get_tool_definitions(enabled_toolsets=["panergos-cli"], quiet_mode=True)
