...
async def setup(url):
    ...
    runner.session_store = SessionStore(Path(os.environ['PANERGOS_HOME']) / 'sessions', GatewayConfig())
...
def rows():
    with sqlite3.connect(Path(os.environ['PANERGOS_HOME']) / 'state.db') as conn:
