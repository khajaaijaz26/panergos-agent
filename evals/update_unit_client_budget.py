...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), PANERGOS_HOME=str(home / "panergos"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("panergos-gateway*", "panergos-serve*")] + [unit + ".service"]
...
unit = f"panergos-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
