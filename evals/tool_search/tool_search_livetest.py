#!/usr/bin/env python3
"""Live test harness for Panergos Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any panergos imports.
ORIGINAL_HOME = os.environ.get("PANERGOS_HOME")
ORIGINAL_AUTH = Path.home() / ".panergos" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.panergos/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.panergos/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="panergos_ts_live_"))
    panergos_home = home_dir / ".panergos"
    panergos_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, panergos_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".panergos" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, panergos_home / ".env")
        ...
        load_panergos_dotenv(panergos_home=str(Path.home() / ".panergos"))
    ...
    (panergos_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new PANERGOS_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "panergos_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original PANERGOS_HOME
    if ORIGINAL_HOME is not None:
        os.environ["PANERGOS_HOME"] = ORIGINAL_HOME
    else:
