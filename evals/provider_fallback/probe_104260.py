...
sandbox = tempfile.TemporaryDirectory(prefix="panergos-104260-")
os.environ.clear()
os.environ.update(
    HOME=sandbox.name,
    PANERGOS_HOME=sandbox.name + "/panergos",
    PATH="/usr/bin:/bin",
    PYTHONDONTWRITEBYTECODE="1",
    PANERGOS_DISABLE_MODEL_METADATA_FETCH="1",
)
Path(os.environ["PANERGOS_HOME"]).mkdir()
...
# JSON is valid YAML; only the temporary Panergos home is written.
Path(os.environ["PANERGOS_HOME"], "config.yaml").write_text(
