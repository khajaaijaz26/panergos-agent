...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='panergos-acp-empty-'))
    panergos = home / '.panergos'
    panergos.mkdir()
    (panergos / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), PANERGOS_HOME=str(panergos), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               PANERGOS_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(panergos / 'state.db') as db:
