"""Drive the real ``panergos fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from panergos_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="panergos_test_fallback_") as home:
        hh = Path(home) / ".panergos"
        hh.mkdir()
        (hh / "config.yaml").write_text(CONFIG, encoding="utf-8")
        (hh / ".env").write_text("OPENROUTER_API_KEY=local-not-used\n", encoding="utf-8")
        auth = hh / "auth.json"
        auth.write_text(json.dumps({"version": 1, "providers": {}, "active_provider": "openrouter"}))
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "PANERGOS_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "panergos_cli.main", "fallback", "add"],
