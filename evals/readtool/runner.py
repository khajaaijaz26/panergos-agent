"""Run the read-tool eval through the REAL Panergos AIAgent.

For each task: fresh temp PANERGOS_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    panergos_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".panergos"
    ...
    try:
        ...
        shutil.rmtree(panergos_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.panergos/.env; set +a  — then relaunch."
        )
