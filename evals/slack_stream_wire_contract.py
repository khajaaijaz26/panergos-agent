...
async def run(repo: str, out: str):
    ...
    for m in list(sys.modules):
        if m.startswith(("gateway", "plugins", "tools", "panergos", "agent")):
            del sys.modules[m]
    ...
    r1 = await adapter.send_native_task_card_progress(
        ...
        metadata=metadata, fallback_text="Panergos is working\n- terminal - ls - running")
...
def main():
    ...
    a = ap.parse_args()
    os.environ.setdefault("PANERGOS_HOME", os.path.join(os.path.dirname(a.out), "panergos-home-probe"))
    os.makedirs(os.environ["PANERGOS_HOME"], exist_ok=True)
