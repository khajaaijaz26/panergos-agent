...
ctx keys: workspace, panergos_home, events (desktop emit records),
...
def _grep_home(ctx, needle):
    """Binary-safe grep of the entire PANERGOS_HOME tree (sqlite included)."""
    nb = needle.encode()
    for root, _dirs, files in os.walk(ctx["panergos_home"]):
        for fn in files:
            try:
