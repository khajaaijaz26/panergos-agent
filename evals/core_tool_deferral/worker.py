...
# --- hermetic env BEFORE any panergos import -------------------------------
for var in list(os.environ):
    if var.endswith(("_API_KEY", "_TOKEN")) and var != "OPENROUTER_API_KEY":
        os.environ.pop(var, None)
os.environ.pop("FAL_KEY", None)
os.environ.pop("PANERGOS_PROFILE", None)

tmp_root = tempfile.mkdtemp(prefix=f"ab-{ARM}-{TASK_ID}-")
panergos_home = os.path.join(tmp_root, ".panergos")
workspace = os.path.join(tmp_root, "ws")
os.makedirs(panergos_home)
os.makedirs(workspace)
with open(os.path.join(panergos_home, "config.yaml"), "w", encoding="utf-8") as f:
    f.write("model:\n  provider: openrouter\n  model: %s\n" % MODEL)
...
PREAMBLE = ("You are running inside the Panergos desktop app on the user's machine. "
...
ctx = {
    "workspace": workspace, "panergos_home": panergos_home,
