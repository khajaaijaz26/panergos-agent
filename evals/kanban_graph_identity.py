...
def main():
    ...
    if not args.child:
        with tempfile.TemporaryDirectory(prefix="kanban-identity-") as home:
            env = {"HOME": home, "PANERGOS_HOME": home + "/panergos", "PATH": os.defpath,
    ...
    from panergos_cli import kanban_db as kb, kanban_db_connect as kbc
    from panergos_cli.kanban_db_dispatch import dispatch_once
    from tools.kanban_tools import _handle_create
    from panergos_cli.kanban_decompose import _apply_fanout, _Routing
    graph = importlib.import_module("panergos_cli.kanban_db_graph") if (args.repo / "panergos_cli/kanban_db_graph.py").exists() else kb
