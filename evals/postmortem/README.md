...
- **`forensics/`** reads a *copy* of a Panergos `state.db` (plus rotated `agent.log*` and git) and
...
- A Panergos checkout with its venv (`.venv/bin/python`). NeMo Relay is not required for the forensics;
  it is what the run itself used for the wire captures in `live_ab/cache_prefix_wire.py`.
- For forensics: a **copy** of `~/.panergos/state.db` (never point at the live file; `sqlite3 state.db ".backup copy.db"`
  or `cp` while Panergos is idle) and, optionally, the rotated `~/.panergos/logs/agent.log*`.
- For `--live` probes: real credentials in `PANERGOS_HOME` (they spend cents per run).

## Forensics: recompute the observed numbers for YOUR run

```bash
cd <panergos-checkout>
P=.venv/bin/python
...
$P -m evals.postmortem.forensics.logcalls   --db state_copy.db --logs "$HOME/.panergos/logs/agent.log*"
...
Panergos recorded (an estimator, not an invoice).
...
```bash
# offline probes (fake providers, temp PANERGOS_HOME), main vs a branch or integration checkout:
...
This instrument runs N concurrent real `AIAgent`
...
python -m evals.postmortem.live_ab.cache_concurrency_probe --repo . --provider anthropic --workers 20 --calls 6 --out /tmp/a.jsonl
```

Reference results (2026-09-05/06): OpenRouter pinned 0/161; OpenRouter unpinned
9.8% collapse. `--ttl 1h` reproduces the 2× write price (#104168). The
`.summary.json` carries bad pairs with both response ids for provider logs.
...
Forensic lanes: five parallel Panergos subagents (2026-09-04), rewritten here to take `--db`/`--root`
