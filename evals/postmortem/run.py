...
temp PANERGOS_HOME (probes that need real credentials say so and are only run with --live). A probe
...
def run_probe(script: str, args: list[str], repo: str, timeout: int = 240) -> tuple[int, str]:
    env = dict(os.environ)
    env.setdefault("PANERGOS_HOME", tempfile.mkdtemp(prefix="pm-probe-"))
