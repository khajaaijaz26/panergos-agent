...
Runs N independent Panergos ``AIAgent`` sessions concurrently, each the same ~8-step tool loop whose
...
Reference results (2026-09-05/06, 20 sessions x 6 calls unless noted):
...
Usage:
  ...
  providers: openrouter (OPENROUTER_API_KEY or --api-key), anthropic
             (ANTHROPIC_API_KEY or --api-key)
Cost: ~$50 per 20x6 arm on Fable 5.1 at the 5m tier. Every run starts from the real Panergos request
...
def _parse():
    ...
    ap.add_argument("--repo", required=True, help="panergos-agent checkout to import from")
    ...
    ap.add_argument("--wire", choices=["chat", "native"], default=None,
                    help="force the wire (default: what Panergos would pick for the provider/model)")
...
os.environ.setdefault("PANERGOS_HOME", os.path.expanduser("~/.panergos"))
import anthropic
...
class _Ctx:
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.rec["provider_headers"] = {k: v for k, v in hdrs.items() if k.lower() in ("cf-ray", "x-request-id", "request-id")}
...
json.dump(dict(provider=PROVIDER, wire=API_MODE, model=MODEL, workers=N, calls_per_session=CALLS, ttl=ARGS.ttl, pin=ARGS.pin, settle_s=SETTLE_S,
               ...
               bad_pairs=[dict(kind=k, session=f"cacheprobe-{PROVIDER}-{ARGS.wire or API_MODE}-{N}-{w}", ok_id=p.get("message_id"), ok_cf_ray=(p.get("provider_headers") or {}).get("cf-ray"),
                              bad_id=c.get("message_id"), bad_cf_ray=(c.get("provider_headers") or {}).get("cf-ray"), read_before=p["cache_read"], read_after=c["cache_read"],
