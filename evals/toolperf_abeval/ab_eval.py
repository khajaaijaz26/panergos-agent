...
measured in the production session DB) through `panergos chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    PANERGOS_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Panergos home with credentials for the
...
(tracker: khajaaijaz26/panergos-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "PANERGOS_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "panergos_cli.main", "chat", "--query", q,
