...
env.update(HOME=str(home), PANERGOS_HOME=str(home), PANERGOS_DASHBOARD_SESSION_TOKEN='history-fixture-token', PYTHONPATH=repo, PANERGOS_NONINTERACTIVE='1')
cmd=[sys.executable, '-m', 'panergos_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(args.port), '--isolated']
...
try:
    for _ in range(120):
        try:
            req=urllib.request.Request(f'http://127.0.0.1:{args.port}/api/status',headers={'X-Panergos-Token':'history-fixture-token'})
