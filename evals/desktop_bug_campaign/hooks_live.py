...
env = {k: v for k, v in os.environ.items() if not (k.startswith('PANERGOS_') or k.endswith(('_API_KEY', '_TOKEN')))}
env.update(HOME=str(out / 'os-home'), PANERGOS_HOME=str(home), PANERGOS_DASHBOARD_SESSION_TOKEN='hooks-fixture-token', PANERGOS_IGNORE_RULES='1', PYTHONPATH=str(a.repo))
cmd = [sys.executable, '-m', 'panergos_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(a.port), '--skip-build']
