...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "PANERGOS_HOME": str(root / "home/.panergos"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "PANERGOS_NONINTERACTIVE": "1"}
    Path(env["PANERGOS_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from panergos_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'panergos')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,panergos_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,panergos_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=Panergos\n", b"Name=Panergos custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["PANERGOS_HOME"]) / "config.yaml"
