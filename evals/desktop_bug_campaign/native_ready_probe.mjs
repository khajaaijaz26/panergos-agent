...
const home = mkdtempSync(join(tmpdir(), 'panergos-native-ready-'))
const token = randomUUID()
const env = {
  PATH: process.env.PATH, HOME: home, PANERGOS_HOME: home,
  LANG: 'C.UTF-8', PYTHONUNBUFFERED: '1', PANERGOS_NONINTERACTIVE: '1',
  PANERGOS_DASHBOARD_SESSION_TOKEN: token, PANERGOS_SERVE_HEADLESS: '1',
  PANERGOS_PARENT_PID: String(process.pid),
}
const child = spawn(process.argv[2] || join(repo, '.venv/bin/python'),
  ['-m', 'panergos_cli.main', 'serve', '--host', '127.0.0.1', '--port', '0', '--isolated'],
...
try {
  ...
  for (let i = 0; i < 20; i++) {
    for (const path of ['/api/status', '/api/profiles']) {
      const response = await fetch(`http://127.0.0.1:${port}${path}`, { headers: { 'X-Panergos-Session-Token': token } })
