...
def main():
    ...
    env = {k: v for k, v in os.environ.items() if not (k.startswith('PANERGOS_') or 'API_KEY' in k or 'TOKEN' in k or k in ('PYTHONPATH', 'PYTEST_PLUGINS'))}
    env.update(PANERGOS_HOME=str(home), HOME=str(home / 'os-home'), PANERGOS_DASHBOARD_SESSION_TOKEN='persistence-fixture-token', PANERGOS_IGNORE_RULES='1', PYTHONPATH=str(args.repo), OPENAI_API_KEY='local-fixture')
    log = (args.out / 'serve.log').open('w')
    cmd = [sys.executable, '-m', 'panergos_cli.main', 'serve', '--isolated', '--port', str(args.port)]
    if observe:
