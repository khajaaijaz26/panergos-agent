...
if len(sys.argv) == 2:
    with tempfile.TemporaryDirectory(prefix="kanban-transport-") as home:
        env = {"HOME": home, "PANERGOS_HOME": home + "/panergos", "PATH": os.environ["PATH"], "PYTHONDONTWRITEBYTECODE": "1"}
...
hh = Path(os.environ["PANERGOS_HOME"])
...
os.environ.update({"PANERGOS_KANBAN_DB": str(db), "PANERGOS_KANBAN_BOARD": "default", "PANERGOS_KANBAN_TASK": own, "PANERGOS_KANBAN_RUN_ID": str(task.current_run_id), "PANERGOS_KANBAN_CLAIM_LOCK": task.claim_lock})
ch = home / "codex"
ch.mkdir()
(ch / "config.toml").write_text(
    'model="fixture"\nmodel_provider="fixture"\n'
    '[model_providers.fixture]\nname="fixture"\nbase_url="http://127.0.0.1:9/v1"\nwire_api="responses"\n'
    '[mcp_servers.panergos-mcp]\ncommand=' + json.dumps(sys.executable) + '\nargs=["-m","agent.transports.panergos_tools_mcp_server"]\nstartup_timeout_sec=40\n'
    '[mcp_servers.panergos-mcp.env]\nPYTHONPATH=' + json.dumps(str(repo)) + '\nPANERGOS_HOME=' + json.dumps(str(hh)) + '\n'
)
report = {}
with CodexAppServerClient(codex_bin=shutil.which("codex") or "codex", codex_home=str(ch)) as c:
    try:
        ...
        raise
    script = "import os,sys,json;sys.path.insert(0," + repr(str(repo)) + ");from tools import kanban_tools as kt;print(json.dumps({'marker':os.getenv('PANERGOS_DELEGATED_CHILD_CONTEXT'),'db':os.getenv('PANERGOS_KANBAN_DB'),'mutation':json.loads(kt._handle_complete({'task_id':" + repr(own) + ",'summary':'must refuse'}))}))"
    ...
    for name, tid in (("foreign", foreign), ("own", own)):
        report[name] = c.request("mcpServer/tool/call", {"threadId": thread_id, "server": "panergos-mcp", "tool": "kanban_complete", "arguments": {"task_id": tid, "summary": "supervised parent handoff"}}, timeout=50)
