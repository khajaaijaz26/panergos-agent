...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["PANERGOS_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(PanergosTokenStorage("positive", panergos_home=owner).get_tokens())
        ...
        override = set_panergos_home_override(other)
        try:
...
def main():
    ...
    if args.cold_probe:
        sys.path.insert(0, str(repo))
        logging.disable(logging.CRITICAL)
        from panergos_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="panergos-oauth-http-") as temp:
            home = Path(temp)
            (home / ".panergos").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "PANERGOS_HOME": str(home / ".panergos"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}
