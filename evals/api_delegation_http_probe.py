...
async def probe():
    ...
    db = SessionDB(db_path=Path(os.environ["PANERGOS_HOME"]) / "state.db")
    ...
    def create_agent(**kwargs):
        ...
        def run(**turn):
            sid = get_session_env("PANERGOS_SESSION_CHAT_ID", "")
            args = [sid]
    ...
    async with TestClient(TestServer(app)) as client:
        for stream in (False, True):
            for explicit in (False, True):
                ...
                if explicit:
                    headers["X-Panergos-Session-Id"] = "parent"
                ...
                records.append({"stream": stream, "explicit": explicit, "status": response.status, "header": response.headers.get("X-Panergos-Session-Id"), "runtime": captured[-1] if captured else None, "body": body[:120]})
        ...
        response = await client.post("/v1/chat/completions", headers={"Authorization": "Bearer fixture-api-key", "X-Panergos-Session-Id": "parent"}, json={"messages": [{"role": "user", "content": "read result"}]})
        await response.read()
        ...
        for name, body, extra_headers in (
            ...
            ("declared_key", {}, {"X-Panergos-Session-Key": "fixture-key"}),
        ):
            count = len(captured)
