...
os.environ.clear()
os.environ.update(
    HOME=home, PANERGOS_HOME=home + "/.panergos", PATH="/usr/bin:/bin", NO_PROXY="*"
)
os.chdir(home)
...
assert Path(os.environ["PANERGOS_HOME"]).is_relative_to(Path(os.environ["HOME"]))
...
Path(os.environ["PANERGOS_HOME"]).mkdir(parents=True, exist_ok=True)
# JSON is valid YAML, allowing the real config loader to read the seeded file.
(Path(os.environ["PANERGOS_HOME"]) / "config.yaml").write_text(json.dumps(config))
from run_agent import AIAgent
from providers import get_provider_profile
from panergos_cli.config import get_compatible_custom_providers, load_config
from panergos_cli.config_providers import get_custom_provider_context_length
    ...
    json.dumps(
        {
            ...
            "panergos_home": os.environ["PANERGOS_HOME"],
            "captures": captures,
            "rows": rows,
