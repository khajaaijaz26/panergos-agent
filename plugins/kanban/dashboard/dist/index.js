/**
 * Panergos Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.panergos/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__PANERGOS_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__PANERGOS_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "panergos-kanban-dot-triage",
    todo: "panergos-kanban-dot-todo",
    ready: "panergos-kanban-dot-ready",
    running: "panergos-kanban-dot-running",
    blocked: "panergos-kanban-dot-blocked",
    review: "panergos-kanban-dot-review",
    done: "panergos-kanban-dot-done",
    archived: "panergos-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-panergos-task";
  ...
  const DOCS_URL = "https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "panergos.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="panergos-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])panergos-kanban-md-code\1/i.test(attrs)) {
      return ' class="panergos-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "panergos-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "panergos-kanban-md",
  ...
  // standard `drop` event and our `panergos-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("panergos-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("panergos-kanban-column--drop");
          if (target) target.classList.add("panergos-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("panergos-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("panergos-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("panergos-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__PANERGOS_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "panergos-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "panergos-kanban-attention",
        "panergos-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "panergos-kanban-attention-bar" },
        h("span", { className: "panergos-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "panergos-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "panergos-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "panergos-kanban-attention-row",
                  "panergos-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "panergos-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "panergos-kanban-attention-row-id" }, task.id),
                h("span", { className: "panergos-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "panergos-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "panergos-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "panergos-kanban-diag-action-btn",
      action.suggested ? "panergos-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " panergos-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".panergos-kanban-drawer-comment-row input, .panergos-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "panergos-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("panergos-kanban-diag", sevClass) },
      h("div", { className: "panergos-kanban-diag-header" },
        h("span", { className: "panergos-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "panergos-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "panergos-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "panergos-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "panergos-kanban-diag-data-row" },
                  h("span", { className: "panergos-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "panergos-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "panergos-kanban-diag-data-row" },
                h("span", { className: "panergos-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "panergos-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "panergos-kanban-diag-reassign-row" },
            h("span", { className: "panergos-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "panergos-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "panergos-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "panergos-kanban-diag-msg",
              msg.ok ? "panergos-kanban-diag-msg--ok" : "panergos-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "panergos-kanban-section" },
      h("div", { className: "panergos-kanban-section-head-row" },
        h("span", { className: "panergos-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "panergos-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "panergos-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "panergos-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "panergos-kanban-docs-link",
      title: "Open Panergos Kanban docs in a new tab",
      "aria-label": "Panergos Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `panergos kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "panergos-kanban-msg-ok" : "panergos-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "panergos-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "panergos-kanban-boardswitcher" },
      h("div", { className: "panergos-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "panergos-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "panergos-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "panergos-kanban-dialog-title" },
        ...
        h("div", { className: "panergos-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Panergos profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "panergos-kanban-bulk" },
      h("span", { className: "panergos-kanban-bulk-count" },
      ...
      h("div", { className: "panergos-kanban-bulk-priority",
      ...
      h("div", { className: "panergos-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Panergos profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "panergos-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("panergos-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("panergos-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "panergos-kanban-trash",
        dragOver ? "panergos-kanban-trash--drop" : "",
        props.draggingTaskId ? "panergos-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "panergos-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "panergos-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".panergos-kanban-card")) return true;
      if (target.closest && target.closest(".panergos-kanban-column-add")) return true;
      if (target.closest && target.closest(".panergos-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("panergos-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("panergos-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".panergos-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "panergos-kanban-columns",
        isScrollable ? "panergos-kanban-columns--scrollable" : "",
        isPanning ? "panergos-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("panergos-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("panergos-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "panergos-kanban-column",
        dragOver ? "panergos-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "panergos-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "panergos-kanban-col-check",
        ...
        h("span", { className: cn("panergos-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "panergos-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "panergos-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "panergos-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "panergos-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "panergos-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "panergos-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "panergos-kanban-lane" },
                  h("div", { className: "panergos-kanban-lane-head" },
                    h("span", { className: "panergos-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "panergos-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "panergos-kanban-card--stale-red";
    if (age >= tier.amber) return "panergos-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".panergos-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "panergos-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "panergos-kanban-card",
        props.selected ? "panergos-kanban-card--selected" : "",
        props.failed ? "panergos-kanban-card--failed" : "",
        props.draggingSource ? "panergos-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "panergos-kanban-card-content" },
          h("div", { className: "panergos-kanban-card-row" },
            h("label", {
              className: "panergos-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "panergos-kanban-card-check",
            ...
            h("span", { className: "panergos-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or panergos kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "panergos-kanban-warning-badge",
                    "panergos-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "panergos-kanban-needs-assignee",
          ...
          h("div", { className: "panergos-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "panergos-kanban-card-row panergos-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "panergos-kanban-assignee",
                            title: `Assigned to Panergos profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "panergos-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "panergos-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "panergos-kanban-ago",
  ...
  // centered modal reusing the panergos-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "panergos-kanban-dialog panergos-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Panergos profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Panergos profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "panergos-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "panergos-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "panergos-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "panergos-kanban-drawer-close",
        ...
        data ? h("div", { className: "panergos-kanban-drawer-comment-foot" },
          h("div", {
            className: "panergos-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "panergos-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "panergos-kanban-section" },
      h("div", { className: "panergos-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "panergos-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "panergos-kanban-drawer-body" },
      h("div", { className: "panergos-kanban-drawer-title" },
        h("span", { className: cn("panergos-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "panergos-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "panergos-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "panergos-kanban-section" },
            h("div", { className: "panergos-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "panergos-kanban-section" },
            h("div", { className: "panergos-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "panergos-kanban-done-no-result panergos-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "panergos-kanban-section" },
            h("div", { className: "panergos-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "panergos-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "panergos-kanban-section" },
        h("div", { className: "panergos-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "panergos-kanban-comment" },
            h("div", { className: "panergos-kanban-comment-head" },
              h("span", { className: "panergos-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "panergos-kanban-diag-action-btn",
      ...
      h("div", { className: "panergos-kanban-section" },
        h("div", { className: "panergos-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "panergos-kanban-comment" },
                h("div", { className: "panergos-kanban-comment-head" },
                  h("span", { className: "panergos-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "panergos-kanban-comment-ago" },
      ...
      h("div", { className: "panergos-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "panergos-kanban-event",
              isDiag ? "panergos-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "panergos-kanban-event-header" },
                  h("span", { className: "panergos-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "panergos-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "panergos-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "panergos-kanban-event-header-plain" },
                  h("span", { className: "panergos-kanban-event-kind" }, e.kind),
                  h("span", { className: "panergos-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "panergos-kanban-event-phantom-row" },
                  h("span", { className: "panergos-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "panergos-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "panergos-kanban-event-payload" },
    ...
    return h("div", { className: "panergos-kanban-section" },
      h("div", { className: "panergos-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "panergos-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `panergos-kanban-run--${r.outcome || r.status || "ended"}`
          : "panergos-kanban-run--active";
        return h("div", { key: r.id, className: cn("panergos-kanban-run", outcomeClass) },
          h("div", { className: "panergos-kanban-run-head" },
            h("span", { className: "panergos-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "panergos-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "panergos-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "panergos-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "panergos-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "panergos-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "panergos-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "panergos-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "panergos-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "panergos-kanban-pre panergos-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "panergos-kanban-meta-row" },
      h("span", { className: "panergos-kanban-meta-label" }, props.label),
      h("span", { className: "panergos-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "panergos-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "panergos-kanban-meta-row" },
        h("span", { className: "panergos-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "panergos-kanban-meta-value panergos-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "panergos-kanban-meta-row" },
        h("span", { className: "panergos-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "panergos-kanban-meta-value panergos-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "panergos-kanban-meta-row" },
        h("span", { className: "panergos-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "panergos-kanban-meta-value panergos-kanban-editable",
    ...
    return h("div", { className: "panergos-kanban-meta-row" },
      h("span", { className: "panergos-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "panergos-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "panergos-kanban-section" },
      h("div", { className: "panergos-kanban-section-head-row" },
        h("span", { className: "panergos-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "panergos-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "panergos-kanban-section" },
      h("div", { className: "panergos-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "panergos-kanban-deps-row" },
        h("span", { className: "panergos-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "panergos-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "panergos-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "panergos-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "panergos-kanban-dep-chip-x",
      ...
      h("div", { className: "panergos-kanban-deps-row" },
        h("span", { className: "panergos-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "panergos-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "panergos-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "panergos-kanban-msg-ok"
          : "panergos-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "panergos-kanban-section" },
      ...
      h("div", { className: "panergos-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "panergos-kanban-home-sub panergos-kanban-home-sub--on"
              : "panergos-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__PANERGOS_PLUGINS__ && typeof window.__PANERGOS_PLUGINS__.register === "function") {
    window.__PANERGOS_PLUGINS__.register("kanban", KanbanPage);
  }
})();
