...
# OpenAI speed tiers. Some gateways serve them as distinct slugs (``-fast``/``-flex``); OpenRouter
# serves them as ENDPOINTS of the base model (tags ``openai/fast``, ``openai/flex``) and silently
# routes an unknown suffix to the standard tier at standard price. Here the wire model becomes
# the base slug with ``provider.only``
...
class OpenRouterProfile(ProviderProfile):
    ...
    def _clamp_reasoning_to_catalog(cfg: dict[str, Any], model: str | None) -> dict[str, Any] | None:
        ...
        if not effort and not disabled:
            return cfg
        try:
            from panergos_cli.models import clamp_reasoning_effort_to_supported
            from panergos_cli.models_reasoning_caps import openrouter_model_reasoning_capabilities
    ...
    def fetch_models(
        ...
        filtering happens in panergos_cli/models.py, which the picker reaches first."""
    ...
    def build_api_kwargs_extras(
        ...
        if supports_reasoning:
            ...
            # turn after the first tool call. See panergos-agent#42991 (disable case) and the tool-replay
            ...
            # panergos-agent#43432). Route the existing ``reasoning_config["effort"]`` (sourced from
