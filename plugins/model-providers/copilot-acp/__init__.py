...
An out-of-tree ACP provider (``~/.panergos/plugins/model-providers/`` or a pip entry point)
...
class CopilotACPProfile(ProviderProfile):
    ...
    def fetch_models(
        ...
        The CLI may keep its login in an OS credential store with no token Panergos can reuse, so
        ...
        """
        from panergos_cli.auth import resolve_external_process_provider_credentials
...
copilot_acp = CopilotACPProfile(
    ...
    # panergos_cli/auth.py), so existing setups keep working.
    process_command="copilot",
    process_args=("--acp", "--stdio"),
    process_command_env_vars=("PANERGOS_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH"),
    process_args_env_var="PANERGOS_COPILOT_ACP_ARGS",
)
