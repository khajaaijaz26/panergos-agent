...
This plugin ships bundled with Panergos but is **opt-in** — it only loads when
...
panergos tools  # → Langfuse Observability
...
Set these in `~/.panergos/.env` (or via `panergos tools`):
...
panergos chat -q "hello"              # then check Langfuse for a "Panergos turn" trace
```

Generation observations include the Panergos system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `PANERGOS_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
PANERGOS_LANGFUSE_ENV=production       # environment tag
PANERGOS_LANGFUSE_RELEASE=v1.0.0       # release tag
PANERGOS_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
PANERGOS_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
PANERGOS_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
PANERGOS_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`PANERGOS_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
panergos plugins disable observability/langfuse
```
