...
# JWKS Cache-Control max-age shared by bundled OIDC providers.
...
def pkce_login_start(authorize_url: str, *, client_id: str, scope: str, redirect_uri: str) -> LoginStart:
    ...
    ``cookie_payload["panergos_session_pkce"]`` as a flat ``state=…;verifier=…`` string
    ...
    return LoginStart(
        ...
        cookie_payload={"panergos_session_pkce": f"state={state};verifier={code_verifier}"})
...
class JwtOAuthProvider(DashboardAuthProvider):
    ...
    (for example an OIDC ID token). Subclasses set ``_client_id`` and
