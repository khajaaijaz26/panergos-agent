...
Let the Panergos agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where Panergos runs) ──────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `panergos meet node run`)                         │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `panergos meet` CLI |
...
| `cli.py` | `panergos meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$PANERGOS_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `panergos meet node {run,list,approve,remove,status,ping}` |
...
panergos meet install                                      # pip + Chromium
panergos meet setup                                        # preflight
panergos meet auth                                         # optional
panergos meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
panergos meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.panergos/.env
panergos meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
panergos meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
panergos meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.panergos/.env
panergos meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, Panergos will **not** switch your system audio input automatically — the
...
panergos plugins enable google_meet
panergos meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
panergos meet node ping my-mac
