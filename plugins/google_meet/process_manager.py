...
One active meeting at a time, recorded in ``$PANERGOS_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_panergos_home()) / "workspace" / "meetings"
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Panergos Agent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "PANERGOS_MEET_URL": url, "PANERGOS_MEET_OUT_DIR": str(out),
           "PANERGOS_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "PANERGOS_MEET_HEADED"),
        (auth_state, "PANERGOS_MEET_AUTH_STATE"),
        (duration, "PANERGOS_MEET_DURATION"),
        (mode, "PANERGOS_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "PANERGOS_MEET_REALTIME_MODEL"),
        (realtime_voice, "PANERGOS_MEET_REALTIME_VOICE"),
        (realtime_instructions, "PANERGOS_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("PANERGOS_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["PANERGOS_MEET_REALTIME_KEY"] = realtime_api_key
