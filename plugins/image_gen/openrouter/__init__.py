...
OpenRouter speaks the OpenAI-style ``/chat/completions`` image protocol (``modalities:
["image","text"]``, references as ``image_url`` parts, output in
``choices[0].message.images[].image_url.url``); only ``(base_url, api_key)`` differs,
via :func:`panergos_cli.runtime_provider.resolve_runtime_provider`. OpenRouter alone also
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    """OpenRouter image provider using runtime credentials and the configured image API surface."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Live catalog combines ``GET /images/models`` and chat-completions image models.
        Offline discovery uses the default chain plus the bundled snapshot."""
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return fail(
                ...
                f"Configure {self._display} in `panergos tools` → Image Generation.",
                "missing_api_key")
...
def register(ctx: Any) -> None:
    """Register the OpenRouter image generation provider."""
