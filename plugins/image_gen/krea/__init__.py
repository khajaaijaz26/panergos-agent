...
# Panergos' 3 abstract ratios → Krea's enum (1:1, 4:3, 3:2, 16:9, 2.35:1, 4:5, 2:3, 9:16).
...
_USER_AGENT = f"{_PANERGOS_USER_AGENT} (krea-image-gen)"
...
def _enhance_image(base_url: str, auth_token: str, image_url: str, prompt: str) -> Optional[str]:
...
def _submit_job(
    base_url: str, auth_token: str, model_path: str, payload: Dict[str, Any], model_id: str,
    fail: ErrorFn,
...
class KreaImageGenProvider(StaticImageGenProvider):
    ...
    setup = dict(
        ...
        tag="Krea 2 foundation model — Medium, Large, and Medium Turbo with style and reference guidance.",
        ...
        **kwargs: Any,
        ...
        if not auth_token:
            return error_factory("krea", aspect)(
                "KREA_API_KEY not set. Configure Krea via `panergos tools` or create a key at "
                "https://www.krea.ai/settings/api-tokens.", "auth_required")
        ...
        job_id, err = _submit_job(
            base_url, auth_token, meta["path"], payload, model_id, fail)
        if err is not None:
            return err

        # 2. Poll with the same direct credential used for submission.
        ...
        if _upscale_requested(kwargs.get("upscale"), meta):
            enhanced_url = _enhance_image(
                base_url, auth_token, result_image_url, prompt)
            if enhanced_url:
