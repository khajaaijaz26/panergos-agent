...
after 14 days; empty dirs under PANERGOS_HOME always. Prompt-only: research
(keep 10 newest, > 30 days), chrome-profile > 14 days, any file > 500 MB.
Scope: strictly PANERGOS_HOME and /tmp/panergos-*; never ~/.panergos/logs/ or system dirs.
"""
...
def _state_file(name: str) -> Path:
    """``$PANERGOS_HOME/disk-cleanup/<name>`` — deliberately outside ``$PANERGOS_HOME/logs/``."""
    return get_panergos_home() / "disk-cleanup" / name


def is_safe_path(path: Path) -> bool:
    """Accept only paths under PANERGOS_HOME or ``/tmp/panergos-*`` (rejects /mnt/c etc.)."""
    with contextlib.suppress(ValueError, OSError):
        path.resolve().relative_to(get_panergos_home())
        return True
    parts = path.parts
    return len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("panergos-")
...
# Top-level PANERGOS_HOME dirs whose empty subdirs are never swept (last row: user project trees).
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    "logs", "memories", "sessions", "cron", "cronjobs",
    "cache", "skills", "plugins", "disk-cleanup", "optional-skills",
    "panergos-agent", "backups", "profiles", ".worktrees",
    "patches", "projects", "skins", "themes", "contributors"})

_EMPTY_DIR_SWEEP_PRUNE_DIRS = frozenset({
    ".git", "node_modules", "venv", ".venv", "site-packages", "__pycache__"})

# Top-level PANERGOS_HOME entries guess_category() never auto-tracks: state, logs, memory,
...
# Paths under $PANERGOS_HOME that must NEVER be deleted by quick(), regardless of what the stored category
...
def _is_protected_cron_path(p: Path) -> bool:
    return str(p.resolve()) in _protected_cron_paths(get_panergos_home())
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside PANERGOS_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    for item, p, age in _live_items(load_tracked(), datetime.now(timezone.utc), log_stale=True):
        ...
        if err is None:
            ...
            errors.append(err)
            new_tracked.append(item)
    empty_removed = _sweep_empty_dirs(get_panergos_home())
...
def _sweep_empty_dirs(panergos_home: Path) -> int:
    """Remove empty dirs under PANERGOS_HOME without recursing into durable/heavy trees (a full
    rglob over a checkout+venv under PANERGOS_HOME can stall the gateway loop for minutes).
    Iterative post-order so parents emptied by child removal are caught."""
    removed = 0
    stack: List[Tuple[Path, bool]] = [
        (top, False) for top in _subdirs(panergos_home, _EMPTY_DIR_PROTECTED_TOP_LEVEL | _EMPTY_DIR_SWEEP_PRUNE_DIRS)]
    while stack:
...
def guess_category(path: Path) -> Optional[str]:
    ...
    if not is_safe_path(path):
        return None
    with contextlib.suppress(ValueError):  # not under PANERGOS_HOME (/tmp/panergos-*) — name rules only
        rel = path.resolve().relative_to(get_panergos_home())
