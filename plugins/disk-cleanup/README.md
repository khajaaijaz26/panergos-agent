...
Auto-tracks and cleans up ephemeral files created during Panergos Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$PANERGOS_HOME` and `/tmp/panergos-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `PANERGOS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `PANERGOS_HOME` or `/tmp/panergos-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$PANERGOS_HOME/disk-cleanup/` is itself excluded
- `$PANERGOS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
