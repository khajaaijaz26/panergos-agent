...
```bash
panergos plugins enable security-guidance
# or edit ~/.panergos/config.yaml manually:
plugins:
  enabled:
    - security-guidance
```
...
* **No LLM diff review.** Anthropic's layer 2 spawns an auxiliary LLM call
  on every agent turn that touched files. On Panergos that would route
...
* **No project-local rules file.** Anthropic's `.claude/claude-security-guidance.md`
  ...
  add an analogous `.panergos/security-guidance.md` once layer 2 lands.
