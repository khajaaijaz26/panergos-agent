...
path containing one) → ``DEFAULT_MODEL``. Authentication uses ``FAL_KEY``; output is an HTTPS URL.
"""
...
# Enums default to None (endpoint decides), flags to False. ``durations`` is an
...
def _build_payload(family: Dict[str, Any], *, prompt: str, image_url: Optional[str], duration: Optional[int], aspect_ratio: str,
    ...
    payload: Dict[str, Any] = {key: value for ok, key, value in (
        ...
        # Newer endpoints declare no `seed`; gate it on the family.
...
def _fal_video_available() -> bool:
    """True when a direct FAL credential is configured."""
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    """Submit with direct FAL credentials; ``.get()`` blocks."""
...
_NO_BACKEND_MSG = "No FAL backend available. Set FAL_KEY via `panergos tools` → Video Generation → FAL."
_MODALITY_MISSING_MSG = {
    "image": "FAL family {fid} has no image-to-video endpoint. Pick a family with image-to-video support via `panergos tools` → Video Generation.",
