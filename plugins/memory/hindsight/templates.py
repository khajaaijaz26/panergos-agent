...
catalog, filter to the ``panergos`` integration, apply a manifest via
...
def fetch_panergos_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``panergos`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "panergos" in (e.get("integrations") or [])]
...
def run_template_step(*, api_url: str, bank_id: str, api_key: str | None, select, cancelled, log=print) -> str | None:
    ...
    try:
        entries = fetch_panergos_templates()
