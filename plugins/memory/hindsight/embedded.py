...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    at import, so Panergos degrades instead of retrying a broken backend).
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    khajaaijaz26/panergos-agent#7718.
    """
    ...
    if "no module named" in text and any(m in text for m in ("hindsight'", 'hindsight"', "hindsight_embed")):
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'panergos memory setup'. "
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    on the Panergos .env during post_setup, where a Notepad BOM would stick to the first key."""
    if not path.exists():
        return {}
...
def _embedded_profile_env_path(config: dict[str, Any]) -> Path:
    profile = str(config.get("profile", "panergos") or "panergos")
