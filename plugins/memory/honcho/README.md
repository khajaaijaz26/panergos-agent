...
```bash
panergos memory setup honcho   # configure Honcho directly (works on a fresh install)
panergos memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
panergos config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.panergos/.env
```

> `panergos honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `panergos memory setup honcho`.
...
Set `"recallSync": true` in `$PANERGOS_HOME/honcho.json` (at the root or under
`hosts.panergos`), or enable **Current-query recall** in the memory settings or
`panergos honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
cycle (not per pass). Select a fast, inexpensive model under `panergos model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$PANERGOS_HOME/honcho.json` | Profile-local (isolated Panergos instances) |
| 2 | `~/.panergos/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Panergos profile: `panergos` (default) or `panergos_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `panergos honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Panergos session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`panergos-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`panergos`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the user peer name is prepended: `alice-panergos-agent`.
...
- **`per-session`** — Panergos session ID (timestamp + hex). Every `panergos` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Panergos profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`panergos`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Panergos profile: `panergos` (default) or `panergos_<profile>` (e.g. `panergos -p coder` -> host key `panergos_coder`). Older `panergos.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `PANERGOS_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `panergos-agent`) |
...
|---------|-------------|
| `panergos memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `panergos honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `panergos memory setup`) |
| `panergos honcho status` | Show resolved config for active profile |
| `panergos honcho enable` / `disable` | Toggle Honcho for active profile |
| `panergos honcho mode <mode>` | Change recall or observation mode |
| `panergos honcho peer --user <name>` | Update user peer name |
| `panergos honcho peer --ai <name>` | Update AI peer name |
| `panergos honcho tokens --context <N>` | Set context token budget |
| `panergos honcho tokens --dialectic <N>` | Set dialectic max chars |
| `panergos honcho map <name>` | Map current directory to a session name |
| `panergos honcho sync` | Create host blocks for all Panergos profiles |
