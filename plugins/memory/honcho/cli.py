"""``panergos honcho`` subcommands: setup wizard, status, peers, sessions, identity, migrate."""
...
from panergos_cli.config import cfg_get
from utils import read_json_or_empty

RULE = "─" * 40
REASONING_LEVELS = ("minimal", "low", "medium", "high", "max")
_RETRY_HINT = "  Re-run 'panergos honcho setup' to retry, or choose an API key instead.\n"
...
def _local_config_path() -> Path:
    ...
    return get_panergos_home() / "honcho.json"
...
def _label(host: str) -> str:
    return f"[{host}] " if host != "panergos" else ""
...
def _sync_profiles(verbose: bool) -> int:
    ...
    cfg = _read_config()
    if not cfg:
        return say("  No Honcho config found. Run 'panergos honcho setup' first.\n") or 0
    default_block, has_key = _default_block_and_key(cfg)
    if not default_block and not has_key:
        return say("  Honcho not configured on default profile. Run 'panergos honcho setup' first.\n") or 0
...
def sync_honcho_profiles_quiet() -> int:
    """Sync host blocks for all profiles from `panergos update`; no output, no exceptions."""
...
def cmd_enable(args) -> None:
    ...
    if not _resolve_api_key(cfg, block, env=False):
        ...
        setup = "panergos honcho setup" + (f" --target-profile {profile}" if profile != "default" else "")
...
def _resolve_effective_identity_mapping(cfg: dict, panergos_host: dict) -> tuple[bool, dict, str, bool, bool]:
    ...
    pin_sources = (panergos_host.get("pinUserPeer"), panergos_host.get("pinPeerName"),
                   cfg.get("pinUserPeer"), cfg.get("pinPeerName"))
    pin = bool(next((v for v in pin_sources if v is not None), False))

    def _inherit(key):
        if key in panergos_host:
            return panergos_host.get(key), False
        val = cfg.get(key)
...
def _apply_runtime_prefix(panergos_host: dict, current_prefix: str, prefix_from_root: bool, label: str) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        panergos_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(panergos_host: dict) -> None:
    print(f"  resolved →\n    pinUserPeer       = {bool(panergos_host.get('pinUserPeer'))}\n"
          f"    userPeerAliases   = {panergos_host.get('userPeerAliases') or '{}'}\n"
          f"    runtimePeerPrefix = {panergos_host.get('runtimePeerPrefix') or '(none)'}")


def _configure_raw_identity_mapping(panergos_host, current_pin, current_aliases, current_prefix,
    ...
    _scrub_identity_mapping(panergos_host)
    panergos_host["pinUserPeer"] = pin
    if pin:
        return
    ...
    while entry := _prompt("    alias", default="").strip():
        ...
        if rid and peer:
            aliases[rid] = peer
    if aliases:
        panergos_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(panergos_host, current_prefix, prefix_from_root,
                          "runtimePeerPrefix — namespace for unknown IDs (blank for none)")


def _setup_identity_mapping(cfg: dict, panergos_host: dict, current_peer: str, new_host: bool) -> None:
    ...
    current_pin, current_aliases, current_prefix, aliases_from_root, prefix_from_root = (
        _resolve_effective_identity_mapping(cfg, panergos_host))
    ...
    if gw_platforms:
        ...
        notice, question = (
            ("\n  Each gateway account (a Telegram user, a Discord user, ...)\n"
             ...
             "Running the Panergos gateway (Telegram/Discord/etc.)? (y/N)") if gw_platforms is None else
            ("\n  No gateway platforms connected — nothing to map.", "Configure anyway? (y/N)"))
        print(notice)
        if not _yes(_prompt(question, default="n")):
            return

    peer_target = panergos_host.get("peerName") or current_peer or "user"
    ai_peer_label = panergos_host.get("aiPeer") or cfg.get("aiPeer") or "panergos"
    ...
    print("\n  This step covers the HUMAN mapping only. Each account using the\n"
          ...
          "  Panergos profile brings its own AI peer to the gateway.\n"
    ...
    if shape == "raw":
        _configure_raw_identity_mapping(panergos_host, current_pin, current_aliases, current_prefix,
        ...
        if shape == "multi" and aliases_from_root:
            prior_aliases = {}
        _scrub_identity_mapping(panergos_host)  # each shape starts from a clean slate
        panergos_host["pinUserPeer"] = shape == "single"
        if shape == "single":
    ...
    _echo_identity_mapping(panergos_host)
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if panergos_cli internals move."""
    try:
        from panergos_cli.auth import _can_open_graphical_browser, _is_remote_session
        return _is_remote_session(), _can_open_graphical_browser()
    except Exception:
        return False, True


def _apply_grant_to_host(cfg: dict, panergos_host: dict, cred) -> None:
    ...
    panergos_host["apiKey"] = cred.access_token
    panergos_host["oauth"] = cred.oauth_block()
    ...
    if cred.consent_peer_name:  # default the peer prompt to the consent name
        panergos_host["peerName"] = cred.consent_peer_name
    print("  Authorized — token saved. Let's finish configuring.\n")


def _setup_local_auth(cfg: dict, panergos_host: dict) -> None:
    ...
    if new_url := _prompt("Base URL", default=cfg.get("baseUrl") or "http://localhost:8000"):
        cfg["baseUrl"] = new_url
    current_host_key = panergos_host.get("apiKey", "")
    ...
    if new_local_key:
        panergos_host["apiKey"] = new_local_key
...
def _setup_device_login(cfg: dict, panergos_host: dict, write_path: Path, *, open_browser: bool) -> bool:
    ...
    try:
        cred = authorize_via_device_code(
            config_path=write_path, source="panergos-cli", apply_config=False, display=_show,
            open_url=webbrowser.open if open_browser else None, on_poll=lambda: print(".", end="", flush=True),
        )
    except KeyboardInterrupt:
        print("\n  Cancelled. Re-run 'panergos honcho setup' to try again.\n")
    except (AuthorizationTimeout, DeviceCodeExpired):
        print("\n  Device code expired before approval.\n  Re-run 'panergos honcho setup' to get a new code.\n")
    except AccessDenied:
        ...
        print(" approved")
        _apply_grant_to_host(cfg, panergos_host, cred)
        return True
    return False


def _setup_browser_login(cfg: dict, panergos_host: dict, write_path: Path) -> bool:
    ...
    try:
        cred = authorize_via_loopback(config_path=write_path, source="panergos-cli", apply_config=False, open_url=_open)
    except Exception as e:
...
def _setup_cloud_auth(cfg: dict, panergos_host: dict, write_path: Path) -> bool:
    ...
    existing_oauth = OAuthCredential.from_host_block(panergos_host)
    ...
    if device_available and method in {"device", "d"}:
        return _setup_device_login(cfg, panergos_host, write_path, open_browser=can_browse and not is_remote)
    if method in {"oauth", "o"}:
        return _setup_browser_login(cfg, panergos_host, write_path)
    # A leftover grant on the host block would shadow the pasted key.
    stale_grant = existing_oauth is not None or is_oauth_access_token(panergos_host.get("apiKey"))
    current = ("" if stale_grant else panergos_host.get("apiKey", "")) or cfg.get("apiKey", "")
    ...
    if not key:
        print("\n  No API key configured. Get yours at https://app.honcho.dev\n"
              "  Run 'panergos honcho setup' again once you have a key.\n")
        return False
    panergos_host.pop("oauth", None)
    panergos_host["apiKey"] = key
    return True
...
def _choice_step(panergos_host, key, current, label, valid, fallback=None) -> None:
    """Prompt for one of ``valid``; an invalid answer writes ``fallback`` (None = keep current)."""
    new = _prompt(label, default=current)
    if new in valid:
        panergos_host[key] = new
    elif fallback is not None:
        panergos_host[key] = fallback


def _setup_tuning(cfg: dict, panergos_host: dict) -> None:
    ...
    _choice_step(panergos_host, "observationMode", _pref(panergos_host, cfg, "observationMode", "directional"),
    ...
    new_wf = _prompt("Write frequency", default=str(_pref(panergos_host, cfg, "writeFrequency", "async")))
    panergos_host["writeFrequency"] = _first_parsed([new_wf], int, new_wf if new_wf in {"async", "turn", "session"} else "async")

    _menu("Recall mode", *(f"{m:<7} -- {desc}" for m, desc in _MODES.items()))
    raw_recall = _pref(panergos_host, cfg, "recallMode", "hybrid")
    _choice_step(panergos_host, "recallMode", raw_recall if raw_recall in _MODES else "hybrid", "Recall mode", _MODES)

    panergos_host["recallSync"] = _yes(_prompt(
        "Wait for current-query recall (bounded by request timeout, default 5s)? (y/N)",
        default="y" if panergos_host.get("recallSync", cfg.get("recallSync", False)) else "n"))

    current_ctx_tokens = _pref(panergos_host, cfg, "contextTokens")
    ...
    _menu("Dialectic cadence",
          ...
          "Recommended: 1-5.")
    new_dialectic = _prompt("Dialectic cadence", default=str(_pref(panergos_host, cfg, "dialecticCadence") or "2"))
    if (val := _first_parsed([new_dialectic], int, None)) is None:
        panergos_host["dialecticCadence"] = 2
    elif val >= 1:
        panergos_host["dialecticCadence"] = val
    ...
    _choice_step(panergos_host, "dialecticReasoningLevel", _pref(panergos_host, cfg, "dialecticReasoningLevel") or "low",
                 "Reasoning level", REASONING_LEVELS, "low")

    _menu("Session strategy", *(f"{s:<13} -- {desc}" for s, desc in _STRATEGIES.items()))
    _choice_step(panergos_host, "sessionStrategy", _pref(panergos_host, cfg, "sessionStrategy", "per-session"),
...
def _setup_wizard(args) -> None:
    ...
    print(f"\nHoncho memory setup\n{RULE}\n  Honcho gives Panergos persistent cross-session memory.\n  Config: {write_path}")
    ...
    panergos_host = cfg.setdefault("hosts", {}).setdefault(_host_key(), {})
    _migrate_pin_key(cfg)  # canonicalize legacy pinPeerName before detection/writes
    _migrate_pin_key(panergos_host)
    # Taken before the prompts populate the block: an existing install must not default to pinning every account.
    new_host = not any(k in panergos_host or k in cfg for k in (*_IDENTITY_MAPPING_KEYS, "peerName", "workspace", "enabled"))
    ...
    if is_local:
        _setup_local_auth(cfg, panergos_host)
    elif not _setup_cloud_auth(cfg, panergos_host, write_path):
        return

    # --- 3. Identity ---
    current_peer = panergos_host.get("peerName") or cfg.get("peerName", "")
    for key, label, default in (
        ("peerName", "Your name (user peer)", current_peer or os.getenv("USER", "user")),
        ("aiPeer", "AI peer name", _pref(panergos_host, cfg, "aiPeer", "panergos")),
        ("workspace", "Workspace ID", _pref(panergos_host, cfg, "workspace", "panergos")),
    ):
        if new := _prompt(label, default=default):
            panergos_host[key] = new

    _setup_identity_mapping(cfg, panergos_host, current_peer, new_host)
    print("\n  For a gateway with many users and agents, run\n"
          "  'panergos honcho peers map' to map accounts interactively.")

    _setup_tuning(cfg, panergos_host)
    panergos_host["enabled"] = True
    panergos_host.setdefault("saveMessages", True)
    _write_config(cfg)
    ...
    try:  # auto-enable Honcho as memory provider in config.yaml
        ...
        panergos_config = load_config()
        panergos_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(panergos_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}\n  Run: panergos config set memory.provider honcho")
  ...
  Other commands:
    panergos honcho status     -- show full config
    panergos honcho mode       -- change recall/observation mode
    panergos honcho tokens     -- tune context and dialectic budgets
    panergos honcho peer       -- update peer names
    panergos honcho map <name> -- map this directory to a session name
""")
...
def _active_profile_name() -> str:
    """Active Panergos profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    cfg = _read_config()
    # _host_block (not hosts.get) keeps legacy dot-form keys ("panergos.work") readable.
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: panergos honcho setup\n")
        return
...
def _state_db_path() -> Path:
    ...
    if _profile_override and _profile_override not in {"default", "custom"}:
        try:
            from panergos_cli.profiles import get_profile_dir
            return get_profile_dir(_profile_override) / "state.db"
        except Exception:
            pass
    return get_panergos_home() / "state.db"
...
def _classify_workspace_peers(
    ...
    panergos_hosts = {hostk for _, hostk, _ in profile_rows}
    ...
    # Host blocks that are not Panergos profiles: other apps sharing the config.
    for hostk, block in (cfg.get("hosts") or {}).items():
        if hostk in panergos_hosts or not isinstance(block, dict):
            continue
...
def cmd_peers_map(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    panergos_host = _host_block(cfg, host)
    pin, aliases, prefix, aliases_from_root, _ = _resolve_effective_identity_mapping(cfg, panergos_host)
    peer_name = panergos_host.get("peerName") or cfg.get("peerName") or ""

    if pin:
        print("\n  pinUserPeer is on: every gateway account resolves to peer")
        print(f"  '{peer_name or '(peerName not set)'}' and aliases have no effect.")
        print("  Turn the pin off with 'panergos honcho setup' to use per-account peers.")
    ...
    workspace = (
        ...
        or panergos_host.get("workspace") or cfg.get("workspace") or host
    )
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        return print(f"  No session mappings configured.\n\n  Add one with: panergos honcho map <session-name>\n"
...
def cmd_peer(args) -> None:
    ...
    def show(panergos, cfg):
        print(f"""
Honcho peers
{RULE}
  User peer:   {_pref(panergos, cfg, 'peerName') or '(not set)'}
    Your identity in Honcho. Messages you send build this peer's card.
  AI peer:     {_pref(panergos, cfg, 'aiPeer') or _host_key()}
    Panergos' identity in Honcho. Seed with 'panergos honcho identity <file>'.
    Dialectic calls ask this peer questions to warm session context.

  Dialectic reasoning:  {_pref(panergos, cfg, 'dialecticReasoningLevel') or 'low'}  ({', '.join(REASONING_LEVELS)})
  Dialectic cap:        {_pref(panergos, cfg, 'dialecticMaxChars') or 600} chars
""")
...
def _show_or_set_choice(args, *, attr: str, key: str, noun: str, title: str, choices: dict,
    ...
    if value is None:
        ...
        return print(f"\n  Set with: panergos honcho {attr} [{'|'.join(choices)}]\n")
...
{RULE}
  ...
  Context     {_pref(panergos, cfg, 'contextTokens') or '(Honcho default)'} tokens
  ...
  Dialectic   {_pref(panergos, cfg, 'dialecticMaxChars') or 600} chars, reasoning: {_pref(panergos, cfg, 'dialecticReasoningLevel') or 'low'}
    AI-to-AI inference. Panergos asks Honcho's AI peer a question
  ...
  Set with: panergos honcho tokens [--context N] [--dialectic N]
""")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        return print("  No API key configured. Run 'panergos honcho setup' first.\n")
    ...
    if getattr(args, "show", False):
        ...
        print(ai_rep.get("representation") or ai_rep.get("card")
              or "  No representation built yet.\n  Run 'panergos honcho identity <file>' to seed one.")
        print()
        return
...
{RULE}
  ...
  AI peer:   {hcfg.ai_peer}
    ...
    panergos honcho identity --show        — show both peer representations
    panergos honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt
""")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Panergos + Honcho."""
    ...
    print("\nHoncho migration: OpenClaw native memory → Panergos\n" + "─" * 50)
    print("""
...
""")
    if has_key:
        ...
        print("""  Honcho is a cloud memory service that gives Panergos persistent memory
  across sessions. You need an API key to use it.

  1. Get your API key at https://app.honcho.dev
  2. Run:  panergos honcho setup
     Paste the key when prompted.
""")
        if _yes(_prompt("  Run 'panergos honcho setup' now?", default="y")):
            cmd_setup(args)
            cfg = _read_config()
            has_key = bool(cfg.get("apiKey", ""))
        else:
            print("\n  Run 'panergos honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  No OpenClaw native memory files found in cwd or ~/.openclaw/.\n"
              ...
              "  or seed them manually:  panergos honcho identity <path/to/file>")
...
""")
    if user_files:
        ...
        print("""
  These are picked up automatically the first time you run 'panergos'
  with Honcho configured and no prior session history.
  (Panergos calls migrate_memory_files() on first session init.)

  If you want to migrate them now without starting a session:""")
        print("    panergos honcho migrate  — this step handles it interactively\n" * len(user_files), end="")
        if has_key:
            _offer("  Upload user memory files to Honcho now?", _migrate_upload, user_files)
        else:
            print("  Run 'panergos honcho setup' first, then re-run this step.")
    else:
...
Step 4  Seed AI identity files → Honcho AI peer
  ...
  In Panergos, they are seeded once into Honcho's AI peer through the
...
""")
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'panergos honcho setup' first, then seed manually:")
            print("\n".join(f"    panergos honcho identity {f}" for f in agent_files))
    else:
        print("  No agent identity files detected.\n  To seed manually:  panergos honcho identity <path/to/SOUL.md>")
...
Step 5  What changes vs. OpenClaw native memory
  ...
  Session naming
    ...
    Panergos: per-session by default — each run gets its own session
              Map a custom name:  panergos honcho map <session-name>

Step 6  Next steps
""")
    if not has_key:
        print("  1. panergos honcho setup              — configure API key (required)\n"
              "  2. panergos honcho migrate            — re-run this walkthrough")
    else:
        print("""  1. panergos honcho status             — verify Honcho connection
  2. panergos                           — start a session
     (user memory files auto-uploaded on first turn if not done above)
  3. panergos honcho identity --show    — verify AI peer representation
  4. panergos honcho tokens             — tune context and dialectic budgets
  5. panergos honcho mode               — view or change memory mode""")
    print()
...
_SUBCOMMANDS = (
    ("setup", "Initial Honcho setup (redirects to panergos memory setup)", None, ()),
    ...
    ("migrate", "Step-by-step migration guide from openclaw-honcho to Panergos Honcho", cmd_migrate, ()),
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":  # honcho setup goes through the unified memory-provider path
        print("\n  Honcho is configured via the memory provider system.\n  Running 'panergos memory setup'...\n")
        from panergos_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``panergos honcho`` argparse subcommand tree on the ``panergos honcho`` parser."""
