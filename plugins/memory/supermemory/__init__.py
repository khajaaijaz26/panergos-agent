...
_DEFAULT_CONTAINER_TAG = "panergos"
...
def _load_supermemory_config(panergos_home: Optional[str] = None) -> dict:
    """Defaults overlaid with $panergos_home/supermemory.json (None = defaults only), every key normalized."""
    config = {k: (list(d) if isinstance(d, list) else d) for k, (d, _) in _CONFIG_SPEC.items()}
    if panergos_home is not None:
        config.update({k: v for k, v in _read_json_dict(Path(panergos_home) / "supermemory.json").items() if v is not None})
    for key, (_, normalize) in _CONFIG_SPEC.items():
        config[key] = normalize(config[key])
    return config


def _save_supermemory_config(values: dict, panergos_home: str) -> None:
    from utils import atomic_json_write
    config_path = Path(panergos_home) / "supermemory.json"
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(api_key=api_key, base_url=self._base_url, timeout=timeout, max_retries=0,
                                   default_headers={"x-sm-source": "panergos"})

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Panergos writes into the "Panergos" Space in the Supermemory app so the user
        # can filter / bulk-manage them per source agent (a routing key for the user, not telemetry).
        merged = {"sm_source": "panergos", **(metadata or {})}
...
def _probe_supermemory_connection(api_key: str, panergos_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(panergos_home)
...
class SupermemoryMemoryProvider(MemoryProvider):
    def __init__(self):
        self._api_key = self._session_id = self._panergos_home = ""
    ...
    def get_config_schema(self):
        # Only the API key is prompted during `panergos memory setup`; other options live in supermemory.json / env.
    ...
    def save_config(self, values, panergos_home):
        ...
        _save_supermemory_config(sanitized, panergos_home)

    def get_status_config(self, provider_config: dict) -> dict:
        from panergos_constants import get_panergos_home
        return {"summary": _format_connection_summary(_probe_supermemory_connection(get_secret("SUPERMEMORY_API_KEY", "") or "", str(get_panergos_home())))}

    def post_setup(self, panergos_home: str, config: dict) -> None:
        from panergos_cli.config import save_config
        from panergos_cli.memory_setup import _prompt, _write_env_vars
        ...
        save_config(config)
        if val:
            _write_env_vars({"SUPERMEMORY_API_KEY": val}, panergos_home=panergos_home)
        ...
        status = _probe_supermemory_connection(api_key, panergos_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from panergos_constants import get_panergos_home
        self._panergos_home = kwargs.get("panergos_home") or str(get_panergos_home())
        self._session_id, self._turn_count, self._session_turns = session_id, 0, []
        config = _load_supermemory_config(self._panergos_home)
