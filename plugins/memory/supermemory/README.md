...
Before running `panergos memory setup`, add the local endpoint to
...
Then run `panergos memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$PANERGOS_HOME/supermemory.json`
...
| `container_tag` | `panergos` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `panergos-{identity}` → `panergos-coder`). |
...
`https://api.supermemory.ai`. Panergos resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: panergos`, and document writes stamp
`metadata.sm_source: panergos`. This is a **functional routing key, not telemetry**:
it groups Panergos-written memories into the existing "Panergos" compatibility Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Panergos profile:

```json
{
  "container_tag": "panergos-{identity}"
}
```

For a profile named `coder`, this resolves to `panergos-coder`. The default profile resolves to `panergos-default`. Without `{identity}`, all profiles share the same container.
