"""panergos-memory-store — holographic memory plugin (MemoryProvider): structured fact storage with entity
resolution, trust scoring, and HRR-based compositional retrieval. Original plugin by dusterbloom (PR #2351).
Config in $PANERGOS_HOME/config.yaml under plugins.panergos-memory-store: db_path ($PANERGOS_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from panergos_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
        return cfg_get(load_config_readonly(), "plugins", "panergos-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, panergos_home):
        """Write config to config.yaml under plugins.panergos-memory-store."""
        # The canonical writer: config lock, managed-mode refusal, default stripping, atomic replace.
        # ``merge_existing`` keeps every other section; *panergos_home* is the active profile already.
        from panergos_cli.config import save_config
        save_config({"plugins": {"panergos-memory-store": dict(values)}}, merge_existing=True)

    def get_config_schema(self):
        from panergos_constants import display_panergos_home
        return [
            {"key": "db_path", "description": "SQLite database path", "default": f"{display_panergos_home()}/memory_store.db"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _panergos_home = str(get_panergos_home())
        db_path = self._config.get("db_path", _panergos_home + "/memory_store.db")
        if isinstance(db_path, str):  # expand $PANERGOS_HOME so paths resolve to the active profile
            db_path = db_path.replace("$PANERGOS_HOME", _panergos_home).replace("${PANERGOS_HOME}", _panergos_home)
