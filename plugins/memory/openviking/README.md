...
- OpenViking server running and reachable from Panergos

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Panergos can identify older servers that expose the legacy status-only health
...
connection values into Panergos, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.panergos/.env`; for a named profile use
...
OpenViking's server config is separate from Panergos:
...
Panergos-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Panergos lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Panergos sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Panergos also sends the compatibility identifier `User-Agent: openviking-memory-panergos/<version>` on
OpenViking requests. This stable harness identifier contains the legacy Panergos
...
does not ask for a peer ID. Without a configured peer, Panergos sends neither
...
order remains environment, linked OpenViking config, then Panergos YAML. To use
...
on the old implicit `panergos` peer now use user memory for new writes. Without
...
Set `agent: panergos` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `panergos-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Panergos
...
Use the same OpenViking profile and credentials as Panergos for manual recovery.
...
Panergos built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Panergos action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Panergos
...
file URIs, such as
`viking://user/default/peers/panergos/memories/preferences/mem_abc123.md` (any
