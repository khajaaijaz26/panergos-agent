...
```bash
panergos memory setup    # select "mem0"
```
...
Behavioral settings live in `$PANERGOS_HOME/mem0.json` (set them via `panergos memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.panergos/.env`.
...
| `user_id` | `panergos-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Panergos session and call `mem0_search` — it connects to your server.
...
```bash
panergos memory setup mem0 --mode oss \
...
```bash
panergos memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$PANERGOS_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.panergos/mem0_qdrant"}}
  }
}
```
...
```bash
panergos memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$PANERGOS_HOME/mem0.json`).
