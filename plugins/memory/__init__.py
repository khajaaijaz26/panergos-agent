...
``$PANERGOS_HOME/plugins/<name>/``, project ``./.panergos/plugins/<name>/`` (opt-in via
PANERGOS_ENABLE_PROJECT_PLUGINS), then ``panergos_agent.memory_providers`` entry points.
...
ENTRY_POINTS_GROUP = "panergos_agent.memory_providers"
# Per Panergos home (plugin managers are per home too): pruning under one multiplexed profile must
...
def _registered_skills_for_active_home() -> dict[str, Path]:
    ...
    return _REGISTERED_MEMORY_PROVIDER_SKILLS.setdefault(panergos_home_key(), {})

# Synthetic parent package so user-installed providers don't collide with bundled ones.
_USER_NAMESPACE = "_panergos_user_memory"
...
def _get_project_plugins_dir() -> Optional[Path]:
    """``./.panergos/plugins/`` or None. Gated on PANERGOS_ENABLE_PROJECT_PLUGINS like the
    PluginManager scan: a repo you merely ``cd`` into must not offer a memory backend."""
    try:
        from panergos_cli.plugins import _env_enabled

        if not _env_enabled("PANERGOS_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".panergos" / "plugins"
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    provider silently loses its dashboard panel and ``panergos <provider>`` commands."""
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from panergos_cli.plugins import resolve_module_origin
...
class _ProviderCollector:
    ...
    def _plugin_context(self):
        ...
        if self._context is None:
            from panergos_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
    ...
    try:
        ...
        if cli_mod is None:
            if not _is_bundled(plugin_dir):
                # cli.py imports as _panergos_user_memory.<name>.cli, usually before the
