...
``$PANERGOS_HOME/photon/sidecar``. The mirror is refreshed by content compare; ``node_modules``
...
# Written by `panergos photon install-sidecar` on npm failure so check_requirements() can
...
def dir_writable(path: Path) -> bool:
    ...
    probe = path / ".panergos-write-probe"
    try:
        probe.touch()
        probe.unlink()
    except OSError:
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_panergos_home() / "photon" / "sidecar"
    try:
