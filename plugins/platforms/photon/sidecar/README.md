...
Small Node helper that bridges Panergos Agent to Photon's Spectrum SDK
(`spectrum-ts`). Panergos is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Panergos-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Panergos can receive inbound messages
...
The Panergos plugin's `panergos photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Panergos is Python, so replies go through this sidecar
