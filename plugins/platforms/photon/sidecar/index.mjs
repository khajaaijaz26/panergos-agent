// Panergos Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Panergos): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Panergos -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Panergos-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `panergos photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Panergos can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "panergos-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Panergos'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Panergos allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Panergos, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-panergos-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Panergos supplied them so a known-good
