...
// `message.content.text` before Panergos can see it. We rewrite the two inbound
...
const MARKER = "Panergos patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Panergos to patch; keep the legacy v8 path below for older pinned installs.
    if (
