...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, PANERGOS_HOME).
        ...
        try:
            ...
            panergos_home = Path(get_panergos_home()).resolve()
        except Exception:
            panergos_home = Path.home().joinpath(".panergos").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), panergos_home)):
...
def is_connected(config) -> bool:
    """Surface in ``panergos status`` even before the adapter is instantiated."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """``env_enablement_fn``: seed ``PlatformConfig.extra`` from env-only setups so ``panergos status`` sees them."""
    if not _env_credentials_present():
        return None
...
def interactive_setup() -> None:
    """``panergos setup line`` wizard (writes ``~/.panergos/.env``); CLI helpers are lazy-imported."""
    from panergos_cli.config import get_env_value, save_env_value
    from panergos_cli.cli_output import print_header, print_info, prompt
