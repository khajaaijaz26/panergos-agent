"""IRC Platform Adapter for Panergos Agent — stdlib asyncio only, zero external dependencies.

config.yaml ``gateway.platforms.irc.extra`` keys: server, port (6697), nickname (panergos-bot), channel,
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = _env_or_extra(extra, "IRC_NICKNAME", "nickname", "panergos-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Panergos Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            with contextlib.suppress(Exception):
                await self._send_raw("QUIT :Panergos Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        elif command == "433":  # ERR_NICKNAMEINUSE — retry: panergos_, panergos_1, panergos_2...
...
def interactive_setup() -> None:
    """`panergos gateway setup` flow (lazy panergos_cli imports keep the plugin importable outside the CLI)."""
    ...
    if declines_reconfigure("IRC", "Reconfigure IRC?", "IRC_SERVER"):
        return
    info("Connect Panergos to an IRC network. Uses Python stdlib — no extra packages needed.",
    ...
    for label, env, what in (("Bot nickname (e.g. panergos-bot)", "IRC_NICKNAME", "Nickname"),
                             ("Channel to join (e.g. #panergos — comma-separate for multiple)", "IRC_CHANNEL", "Channel")):
    ...
    print()
    print_success("IRC configuration saved to ~/.panergos/.env")
    print_info("Restart the gateway for changes to take effect: panergos gateway restart")
...
async def _sa_register(conn: _StandaloneConn, nick_base: str, server_password: str) -> Optional[Dict[str, Any]]:
    ...
    await conn.raw(f"NICK {standalone_nick}")
    await conn.raw(f"USER {standalone_nick} 0 * :Panergos Agent (cron)")
...
async def _standalone_send(pconfig, chat_id: str, message: str, *, thread_id: Optional[str] = None,
    ...
    nick_base = _env_or_extra(extra, "IRC_NICKNAME", "nickname", "panergos-bot").rstrip("_0123456789-")[:24] or "panergos-bot"
