# A2A — Agent-to-Agent protocol for Panergos
...
peer (another Panergos, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
panergos gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Panergos serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.panergos/a2a_audit.jsonl`.
- Conversations persist to `~/.panergos/a2a_conversations/` — they survive context
