...
_NATIVE_SLASH_COMMANDS: tuple = (
    ...
    ("reset", "Reset your Panergos session", (), "/reset", "Session reset~"),
    ...
    ("status", "Show Panergos session status", (), "/status", "Status sent~"),
    ("sethome", "Set this chat as the home channel", (), "/sethome", None),
    ("stop", "Stop the running Panergos agent", (), "/stop", "Stop requested~"),
    ...
    ("reload-skills", "Re-scan ~/.panergos/skills/ for new or removed skills", (), "/reload-skills", None),
    ("voice", "Toggle voice reply mode",
     ...
     "/voice {mode}", None),
    ("update", "Update Panergos Agent to the latest version", (), "/update", "Update initiated~"),
    ("restart", "Gracefully restart the Panergos gateway", (), "/restart", "Restart requested~"),
    ...
    ("thread", "Create a new thread and start a Panergos session in it", (), None, None),
...
def _format_privileged_intents_guidance(*, needs_members: bool) -> str:
    ...
    lines.extend(
        [
            ...
            "Docs: https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/messaging/discord",
        ]
    )
    return "\n".join(lines)
...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("PANERGOS_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning("Ignoring invalid PANERGOS_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r", raw)
    return 30.0
...
class DiscordAdapter(DiscordMediaMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("PANERGOS_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("PANERGOS_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            env_key="PANERGOS_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold", 2,
            env_key="PANERGOS_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
        ...
        self._discord_recovery_store = DiscordRecoveryStore(get_panergos_home())
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("PANERGOS_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        from panergos_constants import get_panergos_home
        directory = get_panergos_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Panergos work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Panergos/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:panergos|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Panergos manages."""
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "panergos_voice", f"ack_{_uuid.uuid4().hex[:12]}.mp3",
        )
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Panergos pairing grant."""
    ...
    def _register_thread_slash(self, tree, name: str, description: str) -> None:
        ...
        @discord.app_commands.describe(
            name="Thread name", message="Optional first message to send to Panergos in the thread",
    ...
    def _register_slash_commands(self) -> None:
        ...
        def _auto_register(name: str, description: str, args_hint: str) -> None:
            ...
            try:
                ...
                pass
        try:
            from panergos_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
            try:
        ...
        try:
            from panergos_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill", description="Run a Panergos skill", callback=_skill_handler,
            )
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        from panergos_cli.commands_platforms import discord_skill_commands_by_category
    ...
    async def _create_thread(
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Panergos: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Real semantic naming is done after the first agent turn, when Panergos has an LLM-generated session
        ...
        thread_name = content[:80] if content else "Panergos"
        if len(content) > 80:
    ...
    def _stamp_auto_thread_name(thread: Any, thread_name: str) -> Any:
        ...
        try:
            setattr(thread, "_panergos_auto_thread_initial_name", thread_name)
        except Exception:
            pass
        return thread
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Panergos: **{thread_name}**"
                    )
    ...
    async def rename_thread(
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Panergos semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(self, parent_chat_id: str, name: str) -> Optional[str]:
        ...
        reason = "Panergos session handoff"
        try:
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Panergos handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
    ...
    _EA_HEADER = ("⚠️ **Command Approval Required**\n\n"
                  "Do you want Panergos to run this command?\n\n"
                  "**Requested command:**\n")
    _EA_CODE_OPEN = "```bash\n"
    _EA_CODE_CLOSE = "\n```\n"
    ...
    async def send_clarify(
        ...
        def _build(_channel):
            embed = discord.Embed(
                title="❓ Panergos needs your input",
            ...
            content = self._self_contained_prompt_content(
                "❓ **Panergos needs your input**", str(question or "").strip(), tail=f"\n\n{hint}",
            )
    ...
    async def send_update_prompt(
        ...
        """Yes/No prompt for the gateway ``/update`` watcher when ``panergos update --gateway`` needs input."""
        def _build(_channel):
    ...
    async def _handle_message(
                ...
                if thread:
                    ...
                    try:
                        ...
                        await message.channel.send(
                            "⚠️ Panergos could not create a Discord thread for "
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_panergos_auto_thread_initial_name", None)
...
def _define_discord_view_classes() -> None:
    ...
    class _PanergosView(discord.ui.View):
        """Shared plumbing for Panergos component views: allowlist auth, single-use
    ...
    class ExecApprovalView(_PanergosView):
    ...
    class SlashConfirmView(_PanergosView):
    ...
    class UpdatePromptView(_PanergosView):
        """Yes/No buttons for ``panergos update`` prompts; the answer is written to
        ...
        async def _respond(self, interaction: discord.Interaction, answer: str, color: discord.Color, label: str):
            ...
            try:
                from panergos_constants import get_panergos_home
                response_path = get_panergos_home() / ".update_response"
    ...
    class ModelPickerView(_PanergosView):
        ...
        async def _expensive_warning_for(self, model_id: str):
            try:
                from panergos_cli.model_selection_guards import combined_selection_warning
    ...
    class ChoicePickerView(_PanergosView):
    ...
    class ClarifyChoiceView(_PanergosView):
...
# process (e.g. standalone ``panergos cron``); same forum/thread/multipart logic via Discord REST.
...
def _prompt_discord_bot_token(prompt) -> str:
    ...
    from panergos_cli.cli_output import print_error
    token = ""
...
def interactive_setup() -> None:
    ...
    from panergos_cli.config import get_env_value, remove_env_value, save_env_value
...
_YAML_WEBSOCKET_LIVENESS_KEYS = (
    ("websocket_liveness_interval_seconds", "liveness_interval_seconds", "PANERGOS_DISCORD_LIVENESS_INTERVAL_SECONDS"),
    ("websocket_liveness_failure_threshold", "liveness_failure_threshold", "PANERGOS_DISCORD_LIVENESS_FAILURE_THRESHOLD"),
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `panergos setup` to install Discord support.",
