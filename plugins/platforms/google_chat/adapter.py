...
def ensure_google_chat_deps() -> bool:
    ...
    ``PANERGOS_LAZY_INSTALL_TARGET`` instead of the read-only venv. Resets the
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        from panergos_constants import get_panergos_home as _get_panergos_home
        self._thread_count_store = _ThreadCountStore(_get_panergos_home() / "google_chat_thread_counts.json")
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        from panergos_constants import get_panergos_home as _get_panergos_home
        return _get_panergos_home() / "google_chat_bot_id.json"
    ...
    async def _build_message_event(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> Optional[MessageEvent]:
        """Parse a Chat API message into a panergos MessageEvent."""
    ...
    async def send_clarify(
        ...
        def _button(text: str, choice: str) -> Dict[str, Any]:
            return {"text": text, "action": "panergos_clarify", "parameters": {"clarify_id": clarify_id, "choice": choice}}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Panergos is thinking…' marker (Chat has no typing API); ``send()``
        ...
        body = _thread_body(getattr(self.config, "typing_status_text", None) or "Panergos is thinking…", thread_id)
...
Walkthrough:
  ...
  3. Create a Pub/Sub topic (e.g. panergos-chat-events) and a Pull subscription.
...
def interactive_setup() -> None:
    """``panergos setup`` wizard: print GCP instructions, prompt for env vars, persist to ``~/.panergos/.env``."""
    from panergos_cli.cli_output import print_info, print_success, print_warning, prompt, prompt_yes_no
    ...
    print()
    print_success("Google Chat configuration saved to ~/.panergos/.env")
    print_info("Restart the gateway: panergos gateway restart")
...
async def _standalone_send(
    ...
    gateway runner is not in-process, e.g. ``panergos cron``). Needs SA credentials and a
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `panergos setup` to install Google Chat support.",
        ...
        platform_hint=(
            ...
            "confirmations instead. While you are generating a response, a 'Panergos is thinking…' marker message "
