...
PANERGOS_SIMPLEX_TEXT_BATCH_DELAY (quiet seconds, default 0.8, merging rapid-fire inbound text).
...
_CORR_PREFIX = "panergos-"  # marks requests we sent so our own echoes can be ignored
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay_seconds = float(os.getenv("PANERGOS_SIMPLEX_TEXT_BATCH_DELAY", "0.8"))
...
async def _standalone_send(
    ...
    this process (``panergos cron``). ``thread_id``/``force_document`` are signature parity only;
...
def interactive_setup() -> None:
    """``panergos setup gateway`` → SimpleX wizard (writes ``~/.panergos/.env``); CLI helpers are lazy-imported."""
