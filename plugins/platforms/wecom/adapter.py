...
class WeComAdapter(WeComStreamMixin, WeComMediaMixin, ChatSendQueueMixin, OwnAccessPolicyMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("PANERGOS_WECOM_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("PANERGOS_WECOM_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
...
def qr_scan_for_bot_info(*, timeout_seconds: int = _QR_POLL_TIMEOUT) -> Optional[Dict[str, str]]:
    ...
    def _get_json(url: str, timeout: int) -> Dict[str, Any]:
        req = urllib.request.Request(url, headers={"User-Agent": _PANERGOS_USER_AGENT})
    ...
    try:
        raw = _get_json(_QR_GENERATE_URL, 15)
    except Exception as exc:
...
_ACCESS_CHOICES = (
    ...
    ("Use DM pairing (unknown users request access, you approve with 'panergos pairing approve')", (("WECOM_DM_POLICY", "pairing"),),
     (("success", "DM pairing mode — users will receive a code to request access."), ("info", "Approve with: panergos pairing approve <platform> <code>"))),
    ("Disable direct messages", (("WECOM_DM_POLICY", "disabled"),), (("warning", "Direct messages disabled."),)),
    ("Skip for now (bot will deny all users until configured)", (), (("info", "Skipped — configure later with 'panergos gateway setup'"),)),
)
...
def register(ctx) -> None:
    common = dict(install_hint="Run `panergos setup` to install WeCom support.", emoji="💼", allow_update_command=True)
