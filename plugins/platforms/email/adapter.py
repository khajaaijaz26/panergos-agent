"""Email platform adapter for the Panergos gateway: users talk to Panergos by sending email; IMAP (polled)
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _panergos_version = "0"
        imap.xatom("ID", f'("name" "panergos-agent" "version" "{_panergos_version}" '
                         '"vendor" "Panergos")')
...
class EmailAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing := [name for name, value in required if not value]:
            message = f"Not configured — missing {', '.join(missing)}. Set it via `panergos gateway setup` (env) or platforms.email in config.yaml."
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    try:
        ...
        for key, value in (("From", address), ("To", chat_id), ("Subject", "Panergos Agent"), ("Date", formatdate(localtime=True))):
            msg[key] = value
