...
A plugin MUST NOT modify `run_agent.py`, `cli.py`, `gateway/run.py`, `panergos_cli/main.py`, etc.
...
`panergos memory setup` → `provider.post_setup(panergos_home, config)`, never a parallel top-level
...
- **No new in-tree memory providers (May 2026).** `plugins/memory/` is closed (honcho, mem0,
  ...
  the same path, integrated via `panergos memory setup` / `post_setup()`.
- **No new third-party-product plugins (June 2026).** Observability/metrics backends, vendor SaaS
  ...
  (`~/.panergos/plugins/` or pip entry point) promoted in Discord `#plugins-skills-and-skins`. Reason:
...
- Reference and documentation-only plugins live in standalone repositories, not here.
...
each changed entry at its pin and runs `panergos plugins validate`). `removed.yaml` is the kill list —
every install path (CLI, dashboard, TUI) refuses matches; only the CLI has a loud `--allow-removed`.
Code: `panergos_cli/plugin_catalog.py` (loader, live refresh from
`/docs/api/plugin-catalog.json` published by the docs build, in-tree fallback),
`panergos_cli/plugins_cmd_catalog.py` (resolution, `.panergos-catalog.json` provenance sidecar,
...
until `panergos_cli.plugin_compat.COMPAT_REMOVAL_DATE`, when the commit that added them is reverted.
`panergos_cli/plugin_compat.py` is the single source: `scan_plugin` (AST scan), `compat_report`
(hits across enabled external plugins, cached to `PANERGOS_HOME/.plugin-compat-report.json`,
refreshed by discovery), `removal_in_effect`, `warn_once`. Surfaces: CLI banner notice,
`panergos plugins compat` (shows affected user plugins), `panergos doctor`, post-update notices, the
...
(`scripts/check_compat_pointers.py` in CI; `-W error::panergos_cli.plugin_compat.PanergosPluginCompatWarning`).
External-plugin compat is handled ONCE here — never add per-PR re-export shims.

## Tests

`tests/plugins/`. Load through real discovery with a temp `PANERGOS_HOME`; assert behaviour (tool
