"""Per-plugin persistent storage: ``<panergos home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<panergos home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `panergos plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<panergos home>/plugin-data/<name>/``; resolves ``get_panergos_home()`` on
    ...
    root = get_panergos_home() / "plugin-data" / _validate_name(name)
