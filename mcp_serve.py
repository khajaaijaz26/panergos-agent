"""
Panergos MCP Server — expose messaging conversations as MCP tools (`panergos mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Panergos-specific
channels_list. Client config: {"mcpServers": {"panergos": {"command": "panergos", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("panergos.mcp_serve")
...
def _panergos_home() -> Path:
    try:
        ...
        return Path(os.environ.get("PANERGOS_HOME", Path.home() / ".panergos"))


def _get_sessions_dir() -> Path:
    return _panergos_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_panergos_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_panergos_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Panergos analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Panergos MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("panergos", instructions=(
        "Panergos Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Panergos MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
