...
an unlinked SQLite DB/WAL/SHM generation.  ``panergos_state`` supplies only the
...
_PANERGOS_EXECUTABLES = frozenset({"panergos", "panergos-agent", "panergos-acp"})
_PANERGOS_PYTHON_MODULES = frozenset({"acp_adapter", "panergos_cli.main"})
_PANERGOS_PYTHON_SCRIPTS = frozenset({"panergos_cli/main.py", "run_agent.py"})
...
def _looks_like_panergos(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Panergos execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _PANERGOS_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _PANERGOS_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _PANERGOS_PYTHON_SCRIPTS
    )
...
def _argv_scoped_to_other_home(argv: Sequence[str], db_path: Path) -> bool:
    ...
    ``state.db`` lives at the PANERGOS_HOME root, so an absolute-path token
    containing a ``/.panergos`` segment (or naming a ``state.db``/WAL/SHM under
    some other parent) identifies that token's own Panergos home.  When at least
    ...
    of ours (issue #92401: a second gateway under /home/demo/.panergos deferred
    ...
    for token in argv:
        ...
        if path_token is not None:
            ...
            if normalized in ours or normalized.startswith(this_home + os.sep):
                return False
            if "/.panergos" in normalized or normalized.endswith("/.panergos"):
...
def foreign_state_db_holders(db_path: Path) -> List[Tuple[int, str]]:
    ...
    # PANERGOS_HOME would otherwise make every holder invisible and let maintenance proceed.
