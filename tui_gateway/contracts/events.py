...
class SkinPayload(OpenPayload):
    """``tui_gateway/change_watcher.py::resolve_skin`` — the resolved active skin (``PanergosSkin``).
...
class BillingBlock(Payload):
    """``agent.conversation_loop._billing_block_dict`` with an optional unverified verdict."""
...
class NotificationShowPayload(Payload):
    """Structured notices via ``notice_callback`` and ``server._await_agent_ready``'s slow-build notice.

    ``level``: info | warn | error | success; ``kind``: sticky | ttl | agent.
    """
