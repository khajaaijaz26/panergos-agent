"""Contracts for config, setup readiness, model inventory, connectors, diagnostics, image generation, and session control."""
...
class ConfigProviderRef(OpenModel):
    """``panergos_cli/models.py::list_available_providers`` row."""
...
class SetupStatusResult(Result):
    ...
    ``other_providers`` and ``inference_provider`` ride along on the launch profile.
...
class SetupRuntimeCheckResult(Result):
    """``ok=False`` + ``error`` when the resolved model cannot be served."""
...
class ModelPricing(Result):
    """``panergos_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown,
    ``"free"``); sale fields are optional provider metadata."""
...
class ModelCapabilities(Result):
    """``panergos_cli/inventory.py::_apply_capabilities``."""
...
class ModelOptionProvider(OpenModel):
    """One ``panergos_cli/inventory.py::build_models_payload`` provider row (the union of every field
...
class ConnectorRow(OpenModel):
    """Secret-free projection of ``panergos_cli.connectors.connector_snapshot``."""
...
method("connectors.list", params=ConnectorsListParams, result=ConnectorsListResult,
       doc="Local messaging connector inventory for one transport-owned session.")
...
class ConnectorSetupStatus(WireEnum):
    active = "active"
...
method("connectors.connect", params=ConnectorsConnectParams, result=ConnectorsConnectResult,
       doc="Return local connector state or an official Panergos setup guide; never starts OAuth.")
...
class GoalContractSnapshot(Result):
    """``panergos_cli/goals.py::GoalContract.to_dict``."""
