...
_WORKER_UNAVAILABLE = "Group Chat worker is unavailable. Restart the Panergos gateway and try again."
...
def _profile_name() -> str:
    return (os.getenv("PANERGOS_PROFILE") or "default").strip() or "default"
...
def _profile_execution_policy(profile: str) -> dict:
    ...
    if _bound_server is not None and profile not in {_current_profile(), _profile_name()}:
        token = set_panergos_home_override(str(_foreign_profile_home(profile)))
    try:
