...
def _relay_root() -> Path:
    ...
    every PANERGOS_HOME, including non-``profiles/`` subdirs of ``~/.panergos``."""
    from tools.bot_mode_probe import _default_home, _panergos_root
    return _panergos_root(Path(_default_home()))
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery) -> dict:
    ...
    GATEWAY via the one-turn ``panergos -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "panergos" else profile
        ...
        fd, tmp = tempfile.mkstemp(prefix="panergos-relay-dm-", suffix=".txt", text=True)
        try:
            ...
            # ``fdopen`` normally owns and closes the descriptor, but a failed/custom
            # writer must not leave a Windows handle that prevents unlinking the payload.
