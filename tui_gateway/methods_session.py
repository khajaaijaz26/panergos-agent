"""Session, delegation, spawn-tree, usage, and pet JSON-RPC handlers.
...
def _profile_build_scope(profile_home):
    """Bind PANERGOS_HOME + secret + terminal scope for an agent build: the same composition a turn
...
def _persist_branch(db, new_key: str, parent_key: str, title: str, history: list, *, source, cwd, profile_name,
    ...
    try:
        ...
        from panergos_state_errors import is_disk_full_error
...
def _(rid, params: dict) -> dict:
    ...
    # ``profile`` (app-global remote mode): stored so the build and every turn re-bind PANERGOS_HOME.
...
def _resume_guard(ctx: _Resume) -> dict | None:
    ...
    from panergos_state import SessionResumeTooLargeError, resolved_max_resume_messages
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if (session := _sessions.get(sid)) is not None:
                ...
                # Each turn re-binds PANERGOS_HOME (mid-turn memory/skills reads); lease claimed lazily on turn 1.
...
def _(rid, params: dict) -> dict:
    ...
    if not (raw := _str_param(params, "cwd")):
        return _err(rid, 4016, "cwd required")
    from panergos_constants import translate_cwd_for_wsl_backend
...
def _(rid, params: dict) -> dict:
    ...
    # the WS-detached sentinel: detached is attachable until grace-reap, and ``panergos --tui`` rides stdio.
...
def _(rid, params: dict) -> dict:
    ...
    with _profile_db(params, writer=True) as db:
        ...
        try:
            home = Path(profile_home) if profile_home is not None else get_panergos_home()
...
def _(rid, params: dict, slug: str) -> dict:
    ...
    from agent.pet import store
    from panergos_cli.pets import _clear_active_if
...
def _pet_config_followup(what: str, fn, *args) -> None:
    """Best-effort ``panergos_cli.pets`` active-slug update after a store op that already succeeded."""
    try:
        fn(*args)
    except Exception as exc:  # noqa: BLE001
...
def _(rid, params: dict, slug: str) -> dict:
    ...
    if new_slug != slug:
        from panergos_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    from panergos_cli.pets import _set_enabled
    _set_enabled(False)
...
def _(rid, params: dict) -> dict:
    ...
    from panergos_cli.pets import set_pet_scale
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    lines = [
        "Panergos TUI Status", "", *status_lines(fields, "session_id", "path"),
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    path = saved_dir / f"panergos_conversation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with session["history_lock"]:
        messages = list(session.get("history", []))
