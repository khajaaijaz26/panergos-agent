...
activate with `panergos --tui` or `PANERGOS_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
(`4000` + key path) and, under `PANERGOS_TEST_ISOLATION=1`, raises `ContractViolation` when a handler's
...
themselves come from `panergos_cli/commands.py` (`panergos_cli/AGENTS.md`).
...
npm run dev       # watch mode (rebuilds panergos-ink + tsx --watch)
npm start         # production
npm run build     # full build (panergos-ink + tsc)
