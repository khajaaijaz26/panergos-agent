...
class HostSupervisor:
    ...
    def __init__(
        ...
        expected_panergos_home: str | None = None, autostart: bool = True) -> None:
        self.registry_path = (
            Path(registry_path) if registry_path is not None
            else get_panergos_home() / "state" / _REGISTRY_NAME)
        ...
        self.expected_panergos_home = (
            str(get_panergos_home()) if expected_panergos_home is None else expected_panergos_home)
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        self._hello = {}
        env = {**panergos_subprocess_env(inherit_credentials=True), **os.environ, **(self.env or {})}
        env["PANERGOS_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
        root = str(_repo_root())
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("panergos_home") or "")
        if got_home and got_home != self.expected_panergos_home:
            raise RuntimeError(
                f"compute host PANERGOS_HOME mismatch: {got_home} != {self.expected_panergos_home}")
