...
def resolve_skin() -> dict:
    try:
        from panergos_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _watcher_home() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _panergos_home)
...
def _ensure_skin_watcher() -> None:
    ...
    threading.Thread(target=_loop, name="panergos-change-watcher", daemon=True).start()
