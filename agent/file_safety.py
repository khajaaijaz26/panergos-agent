...
def _constants_path(getter_name: str) -> Path:
    """Call ``panergos_constants.<getter_name>()`` (local import avoids cycles); ``~/.panergos`` on any failure."""
    try:
        import panergos_constants

        return getattr(panergos_constants, getter_name)()
    except Exception:
        return Path(os.path.expanduser("~/.panergos"))


def _panergos_home_path() -> Path:
    """Active PANERGOS_HOME (profile-aware). Tests monkeypatch this name."""
    return _constants_path("get_panergos_home")


def _panergos_root_path() -> Path:
    """Panergos root dir (parent of any profile, never per-profile)."""
    return _constants_path("get_default_panergos_root")


def _panergos_dirs() -> list[Path]:
    """Resolved active PANERGOS_HOME and global root, deduplicated.

    Both are checked so credential stores at <root>/... stay guarded when
    running under a profile (PANERGOS_HOME = <root>/profiles/<name>).
    """
    return list(dict.fromkeys(_resolve_each((_panergos_home_path(), _panergos_root_path()))))
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    panergos_files = (".env", ".anthropic_oauth.json", os.path.join("cache", "bws_cache.enc.json"))
    paths = [
        *(os.path.join(home, *f) for f in home_files),
        *(str(base / f) for f in panergos_files for base in (_panergos_home_path(), _panergos_root_path())),
...
def get_safe_write_roots() -> set[str]:
    """Resolved PANERGOS_WRITE_SAFE_ROOT paths (``os.pathsep``-separated list)."""
    roots: set[str] = set()
    for path in filter(None, os.getenv("PANERGOS_WRITE_SAFE_ROOT", "").split(os.pathsep)):
...
# PANERGOS_HOME / root subpaths that the agent's generic file tools must not
...
_PANERGOS_PROTECTED_SUBPATHS = ("state.db", "sessions", "mcp-tokens", "pairing")
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    for base in _panergos_dirs():
        for sub in _PANERGOS_PROTECTED_SUBPATHS:
            with suppress(Exception):
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside PANERGOS_WRITE_SAFE_ROOT "
...
# Exact-file credential stores under PANERGOS_HOME / <root>. The agent never
...
# Directory-prefix read denies under PANERGOS_HOME / <root>: (subdir, message for
...
_READ_DENIED_DIRS = (
    ("mcp-tokens",
     "is the Panergos MCP token directory and cannot be read directly.",
     "is a Panergos MCP token file and cannot be read directly."),
    ("browser-profile",
     "is the Panergos real-profile browser snapshot directory (copied cookies/logins) and cannot be read directly.",
     "is inside the Panergos real-profile browser snapshot (copied cookies/logins) and cannot be read directly."),
    # vault.key + vault.json.enc sit side by side; key + ciphertext = plaintext, so the whole dir is one credential.
    ("vault",
     "is the Panergos credential vault directory and cannot be read directly (secrets are filled server-side by browser_vault_fill).",
     "is inside the Panergos credential vault (encrypted secrets + local key) and cannot be read directly (browser_vault_fill resolves them server-side)."),
)


def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Panergos path.

    Blocked: internal skill-hub caches (prompt-injection carriers), credential
    stores under PANERGOS_HOME and the global root (exact files, plus anything
    ...
    panergos_dirs = _panergos_dirs()
    reason = None
    if any(_is_under(resolved, hd / "skills" / ".hub") for hd in panergos_dirs):
        reason = (
            "is an internal Panergos cache file and cannot be read directly to prevent "
            "prompt injection. Use the skills_list or skill_view tools instead."
        )
    elif any(resolved in _resolve_each(hd / name for hd in panergos_dirs) for name in _CREDENTIAL_FILE_NAMES):
        reason = (
            "is a Panergos credential store and cannot be read directly. Provider tools "
            "consume these credentials through internal channels." + _DID_SUFFIX
        )
    else:
        for subdir, dir_msg, file_msg in _READ_DENIED_DIRS:
            for blocked_dir in _resolve_each(hd / subdir for hd in panergos_dirs):
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Panergos read (see ``get_read_block_error``).
...
def _resolve_active_profile_name() -> str:
    """Active profile name from PANERGOS_HOME: ``~/.panergos`` -> ``"default"``,
    ``~/.panergos/profiles/X`` -> ``"X"``; ``"default"`` on any resolution failure."""
    try:
        parts = _panergos_home_path().resolve().relative_to(_panergos_root_path().resolve() / "profiles").parts
...
#   <PANERGOS_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.panergos/...
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Panergos state: ``None``
    for non-mirror paths, else ``target_path`` (resolved), ``mirror_root`` (the
    ``…/home/.panergos`` prefix) and ``inner_path`` (what the agent meant on the host)."""
    target = _resolve_target(path)
    parts = target.parts if target is not None else ()
    # Need at least: sandboxes / <backend> / <task> / home / .panergos / <thing>; inner_idx = the .panergos part.
    inner_idx = next(
        (i + 4 for i, part in enumerate(parts)
         if part == "sandboxes" and i + 5 < len(parts) and parts[i + 3] == "home" and parts[i + 4] == ".panergos"),
        None,
    )
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "Writes here land on a copy that the host Panergos process never reads — the "
        "authoritative file is likely {inner_path!r} under the real PANERGOS_HOME.",
...
def classify_container_mirror_target(path: str, mirror_prefix: str | None = None) -> Optional[dict]:
    ...
    the bind mount strips the ``sandboxes/`` prefix (the agent sees plain ``/root/.panergos/…``),
...
def get_container_mirror_warning(path: str, mirror_prefix: str | None = None) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "the container's bind-mounted home — a per-task mirror that the host Panergos "
        "process never reads. The authoritative file is {inner_path!r} under "
        "the real PANERGOS_HOME.",
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Panergos scope, or is inside
    ...
    try:
        ...
        root_real = _panergos_root_path().resolve()
