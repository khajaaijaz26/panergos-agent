...
from panergos_constants import OPENROUTER_MODELS_URL, openrouter_variant_base
...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    for env_var in ("PANERGOS_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _cache_file(name: str) -> Path:
    ...
    return get_panergos_home() / "cache" / name
...
def _endpoint_disk_cache_get(normalized: str) -> Optional[Dict[str, Dict[str, Any]]]:
    ...
    runs (``panergos -q``, cron) start cold, so
...
def _strip_openrouter_routing_variant(
    ...
    :func:`panergos_cli.models.validate_requested_model` applies. Sharing the
...
def _get_context_cache_path() -> Path:
    ...
    from panergos_constants import get_panergos_home
    return get_panergos_home() / "context_length_cache.yaml"
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    # Common relay format: "maximum context length is N …" with text/tool/output parts.
...
# DashScope, Anthropic, OpenRouter, LM Studio/llama.cpp, and OpenAI-compatible relays.
_OUTPUT_CAP_SIGNALS = (
...
def _normalize_model_version(model: str) -> str:
    """Normalize dotted and dashed model version spellings for catalog comparison."""
...
def _resolve_codex_oauth_context_length_with_source(model: str, access_token: str = "") -> Tuple[Optional[int], str]:
    ...
    # ``-900k`` variants are Panergos picker aliases — the Codex catalog only knows the base slug, so resolve
...
def _resolve_provider_aware_context_length(model: str, base_url: str, api_key: str, provider: str, effective_provider: str) -> Optional[int]:
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        with contextlib.suppress(Exception):  # fall through to models.dev
            from panergos_cli.models import get_copilot_model_context
            ctx = get_copilot_model_context(model, api_key=api_key)
            if ctx:
                return ctx
    # Codex OAuth has lower limits than the direct API for the same slug; its live catalog is authoritative.
    sourced = {
...
def get_model_context_length(
    ...
    # in panergos_cli.models.validate_requested_model.
    ...
    if not effective_provider:
        ...
        if model in metadata:
            ...
            if or_ctx == 32768 and _model_name_suggests_stale_32k_underreport(model):
                ...
                return or_ctx
    # 7. Local server before hardcoded defaults — ``Example-Llama-3.1-70B`` matches ``llama``
