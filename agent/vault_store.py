...
- Follows the repo's "default frictionless, 0600 files OK" policy: the key
  file and vault file are created 0600 under ``<PANERGOS_HOME>/vault/``.
...
# browser fill targets (agent/vault_login_classifier.py); the Desktop Add dialog and `panergos vault add`
...
class VaultStore:
    """Encrypted, profile-scoped vault under ``<PANERGOS_HOME>/vault/``."""

    def __init__(self, base_dir: Optional[Path] = None):
        self._base = Path(base_dir) if base_dir is not None else (
            Path(get_panergos_home()) / "vault"
        )
    ...
    def _ensure_dir(self) -> None:
        ...
        # group-share mode and PANERGOS_UID/GID ownership) rather than a
    ...
    def _locked(self):
        ...
        `panergos vault add` and a TUI slash worker all write the same ``vault.json.enc``; two unlocked
