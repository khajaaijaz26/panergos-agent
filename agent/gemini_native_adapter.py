...
try:
    import panergos_cli as _panergos_cli

    _PANERGOS_VERSION = str(_panergos_cli.__version__)
except Exception:
    _PANERGOS_VERSION = "0.0.0"
_API_CLIENT = f"panergos-agent/{_PANERGOS_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Panergos typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.panergos/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.panergos/.env (get one at https://aistudio.google.com/app/apikey), or run `panergos setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Panergos retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No PANERGOS_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
