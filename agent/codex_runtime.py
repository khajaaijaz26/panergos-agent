...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    """Translate Codex app-server token usage into Panergos accounting. Prompt bucket = uncached + cached
    ...
    estimate grows monotonically and panergos-mode fires thread compaction on tiny threads (#100381)."""
...
def _record_codex_app_server_compaction(agent, turn, *, approx_tokens: int | None = None, force: bool = False) -> bool:
    ...
    if compressor is not None:
        ...
        # Codex owns this summary: a prior Panergos deterministic-fallback flag must not leak into it.
...
# --- Codex app-server → Panergos UI bridge -------------------------------------
# The app-server bypasses the Panergos tool loop, so the bridge translates JSON-RPC notifications
# into the callbacks the standard runtime fires (tool_progress_callback, _fire_stream_delta, ...).

# Item types that project to a Panergos tool_call (keep in sync with agent/transports/codex_event_projector.py
...
# Internal MCP server wrapping Panergos' native tools: its inner dispatch has no tool_progress_callback, so the
# codex-level mcpToolCall IS the display event and the mcp.panergos-tools.* prefix is stripped (users see Panergos tools).
_INTERNAL_MCP_SERVER = "panergos-tools"
...
def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Panergos tool name for a codex item (mirrors CodexEventProjector)."""
...
def _ensure_codex_session(agent) -> None:
    ...
    # Approval callback: Panergos' standard prompt flow when a CLI thread installed one.
    ...
    # explicit approval bypass (approvals.mode: off, /yolo, --yolo, PANERGOS_YOLO_MODE) hands policy to codex's sandbox.
    ...
    # Panergos' gateway UI callbacks (tool_progress_callback, _fire_stream_delta,
...
def _bypass_sdk_request_transform(stream_kwargs: dict) -> dict:
    ...
    request without the walk. PANERGOS_CODEX_SDK_TRANSFORM=1 disables."""
    if os.environ.get("PANERGOS_CODEX_SDK_TRANSFORM", "").strip().lower() in {"1", "true", "yes", "on"}:
