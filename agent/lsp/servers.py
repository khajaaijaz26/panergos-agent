...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    ``PSES_BUNDLE_PATH`` env, then ``<PANERGOS_HOME>/lsp/PowerShellEditorServices``."""
    ...
    candidates = [
        ...
        os.path.join(str(get_panergos_home()), "lsp", "PowerShellEditorServices"),
    ]
...
_PSES_MISSING_MSG = (
    ...
    "lsp.servers.powershell.command to the bundle path or unzip it to <PANERGOS_HOME>/lsp/PowerShellEditorServices."
)
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    session_dir = panergos_lsp_session_dir()
    inner = (
        ...
        f"-HostName Panergos -HostProfileId panergos -HostVersion 1.0.0 -Stdio -LogLevel Normal"
    )
    return SpawnSpec(
...
def panergos_lsp_session_dir() -> str:
    """Return (and create) the dir for PSES session/log scratch files."""
    from panergos_constants import get_panergos_home
    d = os.path.join(str(get_panergos_home()), "lsp", "pses")
