...
Installs go to a Panergos-owned staging dir, ``<PANERGOS_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``panergos lsp status`` reports it).
...
# ``bin`` in ``<PANERGOS_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `panergos lsp status` reports its presence.
...
def panergos_lsp_bin_dir() -> Path:
    """Return the Panergos-owned bin staging dir for LSP servers."""
    from panergos_constants import get_panergos_home

    p = get_panergos_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(panergos_lsp_bin_dir() / name):
...
def _link_into_bin(target: Path) -> str:
    ...
    link = panergos_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $PANERGOS_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that Panergos installed.
    npm = find_node_executable("npm")
    if npm is None:
        logger.info("[install] cannot install %s: no usable npm found", pkg)
        return None
    staging = panergos_lsp_bin_dir().parent  # <PANERGOS_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = panergos_lsp_bin_dir().parent / "python-packages"
    pip_target.mkdir(parents=True, exist_ok=True)
    try:
        logger.info("[install] pip install --target %s %s", pip_target, pkg)
        from panergos_cli.tools_config import _pip_install
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``panergos lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "panergos_lsp_bin_dir"]
