"""Language Server Protocol (LSP) integration for Panergos Agent.
...
# Routed multiplex profiles (PANERGOS_HOME override) each get their own service: ``lsp.*`` config
...
def get_service() -> Optional[LSPService]:
    ...
    without it every ``panergos chat`` exit leaks pyright processes for a few seconds
