...
# `export PANERGOS_REDACT_SECRETS=false`) cannot disable redaction mid-session.
...
# to this env var in panergos_cli/main.py, gateway/run.py, and cli.py) or `PANERGOS_REDACT_SECRETS=false` in
# ~/.panergos/.env. An opt-out warning is logged at gateway and CLI startup so operators see the downgrade —
# see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("PANERGOS_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}

# Routed multiplex profiles: the import-time snapshot above is the LAUNCH profile's policy. A profile
# served under a PANERGOS_HOME override resolves its own ``security.redact_secrets`` (its ``.env``
# value first, like the standalone bridge in panergos_cli/main.py), cached per home so the hot path
...
def _redact_enabled() -> bool:
    ...
    try:
        ...
        raw = scope.get("PANERGOS_REDACT_SECRETS") if scope else None
        if raw is None:
...
def mask_secret(value: str, *, head: int = 4, tail: int = 4, floor: int = 12,
    ...
    """Mask a secret for display (``panergos config`` / ``status`` / ``dump``):
...
# a shell rc/profile, Panergos' own ``config.yaml`` where ``panergos mcp add --env`` writes
...
_PANERGOS_HOME_PREFIXES = ("$PANERGOS_HOME/", "${PANERGOS_HOME}/")
# ``$HOME/.panergos/config.yaml`` keeps the ``.panergos`` segment, so stripping the prefix is
...
def _is_secret_file_arg(arg: str) -> bool:
    ...
    ``.panergos`` directory or ``$PANERGOS_HOME`` (never arbitrary YAML)."""
    path = arg.strip("\"'").replace("\\", "/")
    panergos_home = False
    for prefix in _PANERGOS_HOME_PREFIXES:
    ...
    if parts[-1] in _ENV_FILE_BASENAMES or parts[-1] in _SHELL_RC_BASENAMES:
        return True
    return parts[-1] == "config.yaml" and (panergos_home or ".panergos" in parts[:-1])
...
def redact_terminal_output(output: str, command: str | None = None, *, force: bool = False) -> str:
    ...
    shell rc, Panergos ``config.yaml``); otherwise code_file=True avoids false positives on
    source/config dumps."""
    if not output:
        return output
