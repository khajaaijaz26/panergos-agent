...
from panergos_constants import (
    get_panergos_home, get_skills_dir, is_wsl, reset_panergos_home_override, set_panergos_home_override,
)
...
def _find_panergos_md(cwd: Path) -> Optional[Path]:
    """Nearest ``.panergos.md`` / ``PANERGOS.md`` from *cwd* up to the git root, else None."""
    ...
    for directory in [current, *current.parents] if stop_at else [current]:
        found = next((directory / n for n in (".panergos.md", "PANERGOS.md") if (directory / n).is_file()), None)
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Panergos Agent, a capable open-source autonomous agent. Be direct: "
...
PANERGOS_AGENT_HELP_GUIDANCE = (
    # Injected only when skill_view exists AND the panergos-agent skill is installed (system_prompt.py slot
    # resolution). No "when the two differ" clause: docs-are-authoritative already carries the precedence.
    "You run on Panergos Agent. When the user needs help with "
    "Panergos itself — configuring, setting up, using, extending, or troubleshooting it — use "
    "https://github.com/khajaaijaz26/panergos-agent/blob/main/PANERGOS.md as the authoritative Panergos reference; "
    "The `panergos-agent` skill has the actual commands and proven workflows — load it with "
    "skill_view(name='panergos-agent') "
    "before configuring, modifying, or troubleshooting Panergos so you don't guess or invent workarounds."
)

# Variant for sessions without the skills toolset (e.g. Blank Slate): naming skill_view() there would dangle.
PANERGOS_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on Panergos Agent. When the user needs help with "
    "Panergos itself, use https://github.com/khajaaijaz26/panergos-agent/blob/main/PANERGOS.md as the authoritative "
    "Panergos reference."
)
...
KANBAN_GUIDANCE = (
    ...
    "You have been assigned ONE task from the shared board at `~/.panergos/kanban.db`. Your task id is in "
    "`$PANERGOS_KANBAN_TASK`; your workspace is `$PANERGOS_KANBAN_WORKSPACE`. The `kanban_*` tools in your schema are your "
    ...
    "2. **Work inside the workspace.** `cd $PANERGOS_KANBAN_WORKSPACE` before any file operations. The workspace is "
    ...
    "- **Workspace.** `cd $PANERGOS_KANBAN_WORKSPACE` first. For a `worktree` kind with no `.git`, `git worktree add "
    "<path> ${PANERGOS_KANBAN_BRANCH:-wt/$PANERGOS_KANBAN_TASK}` from the main repo, then cd there. For a project-linked "
    "task the workspace is a fresh `<repo>/.worktrees/<task-id>` and `$PANERGOS_KANBAN_BRANCH` a deterministic "
    ...
    "sits in `ready` forever). Ground every assignee in a real profile (`panergos profile list`, or ask the user), and "
    "express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n\n"
    "- Do not shell out to `panergos kanban <verb>` for board operations. Use the `kanban_*` tools — they work across "
...
# cost that compounds over a long conversation. The panergos-agent runtime already executes a batch of tool
...
# to panergos-agent's Python prompt-assembly architecture.
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Panergos appends their message to the end of a tool result, wrapped exactly as:\n"
    f"{STEER_MARKER_OPEN}\n<their message>\n{STEER_MARKER_CLOSE}\n"
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         "[Note: this message came from HUD mode — a small floating Panergos "
         ...
         "rather than anything inside Panergos. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Panergos terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
    ...
    "desktop": (
        ...
        "You are chatting inside the Panergos desktop app, a graphical chat surface. Markdown renders with full GitHub "
        ...
        "or it measures full-bleed. Widgets talk back: data-panergos-send=\"prompt\" on any clickable element (or "
        "window.panergos.send(\"prompt\")) sends that prompt as a hidden user turn — answer it by updating the widget's "
        "file, not with prose."
    ),
    "sms": (
...
def _run_backend_probe(env_type: str, terminal_tool) -> str:
    ...
    env = _create_environment(
        ...
        # snapshot. A normal SSHEnvironment would upload the whole ~/.panergos tree just to run `uname`,
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    cache_key = (panergos_home_key(), env_type, _tenv_read("TERMINAL_CWD", ""))
...
def _remote_backend_hint(backend: str) -> str:
    ...
    if probe:
        return lead + (
            f"this {backend} environment — NOT on the machine where Panergos itself is running. The host OS, "
            f"home, and cwd of the Panergos process are irrelevant; only the following backend state matters:\n{probe}"
        )
    description = (
    ...
    return lead + (
        f"{description} — NOT on the machine where Panergos itself runs. The backend probe didn't respond at "
...
def _embedder_environment_hint() -> str:
    """Embedder-supplied environment description: PANERGOS_ENVIRONMENT_HINT (container ENV)
    wins over config.yaml ``agent.environment_hint``. Read once at prompt-build time."""
    return (os.getenv("PANERGOS_ENVIRONMENT_HINT") or "").strip() or str(
...
RUNTIME_ENVIRONMENT_HEADING = "# Panergos runtime environment"
RUNTIME_ENVIRONMENT_END = "<!-- End Panergos runtime environment -->"
...
def _skills_prompt_snapshot_path() -> Path:
    return get_panergos_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("PANERGOS_PLATFORM") or os.environ.get("PANERGOS_SESSION_PLATFORM")
    if platform:
        return platform
    get_session_env = getattr(sys.modules.get("gateway.session_context"), "get_session_env", None)
    try:
        return (get_session_env("PANERGOS_SESSION_PLATFORM") if get_session_env else "") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    ``skills_dir_override`` makes home resolution EXPLICIT: a build thread that never bound the PANERGOS_HOME
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_panergos_home_override(str(skills_dir.parent))
    else:
    ...
    try:
        ...
        if _home_token is not None:
            reset_panergos_home_override(_home_token)
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    """SOUL.md from PANERGOS_HOME (identity slot #1), or None.

    Callers must pass ``skip_soul=True`` to ``build_context_files_prompt`` so it isn't injected twice.
    ``home_override`` pins the profile home (a thread that lost the PANERGOS_HOME ContextVar reads the wrong one).

    ``home_override`` scopes the read to an explicit profile home (the agent knows its own home from its
    session_db path). Without it, resolution is ambient — which on a thread that lost the PANERGOS_HOME
    ...
    """
    try:
        from panergos_cli.config import ensure_panergos_home
        ensure_panergos_home()
    except Exception as e:
        logger.debug("Could not ensure PANERGOS_HOME before loading SOUL.md: %s", e)
    soul_path = (Path(home_override) if home_override is not None else get_panergos_home()) / "SOUL.md"
...
def _load_panergos_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.panergos.md / PANERGOS.md — nearest match walking up to the git root."""
    panergos_md_path = _find_panergos_md(cwd_path)
    content = _read_context_file(panergos_md_path) if panergos_md_path else ""
    if not content:
        return ""
    label = str(panergos_md_path.relative_to(cwd_path)) if panergos_md_path.is_relative_to(cwd_path) else panergos_md_path.name
    return _context_section(_strip_yaml_frontmatter(content), label, ".panergos.md", panergos_md_path, context_length)
...
def build_context_files_prompt(
    ...
    Only ONE project context type loads, first found wins: .panergos.md/PANERGOS.md (walk to git root) →
    AGENTS.md chain (git root → cwd) → CLAUDE.md (cwd) → .cursorrules + .cursor/rules/*.mdc (cwd). SOUL.md
    from PANERGOS_HOME is independent and always included unless *skip_soul* (already the identity slot).
    """
    cwd_path = Path(cwd if cwd is not None else os.getcwd()).resolve()
    # A FALLBACK-picked cwd inside the Panergos install tree must not gain system-prompt authority (the desktop
    # default would load this repo's contributor AGENTS.md). An explicit cwd is honored verbatim.
    # An explicitly configured cwd is honored verbatim — the Panergos tree is a legitimate workspace when the
    # user deliberately points a session at it — and CLI-style surfaces pass
    # allow_install_tree_fallback=True because their launch dir IS the user's shell cwd (developing Panergos
    # in-tree). See #64590.
    from agent.runtime_cwd import _is_install_tree
    if cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the Panergos "
            "install tree (%s) — set terminal.cwd to your project directory", cwd_path,
        )
        sections = []
    else:
        sections = [_load_panergos_md(cwd_path, context_length) or _load_agents_md(cwd_path, context_length)
