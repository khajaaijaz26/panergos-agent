"""OpenAI-shape bridge shared by Panergos' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Panergos' tool schemas travel INTO the
...
read/edit/execute tools forwards only Panergos' agent-level tools, since re-offering overlapping ones
makes Panergos redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``panergos_projected_messages``)
    Panergos reads off the ``create`` result; a plain list would drop them on the stream path."""
