...
_OBSERVABLE_METRIC_NAMES = (
    "panergos.gateway.up", "panergos.gateway.state", "panergos.gateway.active_agents", "panergos.gateway.busy",
    "panergos.gateway.drainable", "panergos.gateway.restart_requested", "panergos.gateway.background_work",
    "panergos.gateway.background_delegations", "panergos.platform.up", "panergos.platform.degraded",
    "panergos.cron.scheduler.heartbeat_age_seconds", "panergos.cron.scheduler.last_success_age_seconds",
    "panergos.cron.scheduler.catch_up_occurrences", "panergos.cron.jobs.enabled", "panergos.cron.jobs.running",
    "panergos.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="panergos-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("panergos.gateway.background_work", _read_background_work_count),
            ("panergos.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("panergos.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("panergos.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'panergos-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="panergos-gateway-health-export", daemon=True)
            thread.start()
