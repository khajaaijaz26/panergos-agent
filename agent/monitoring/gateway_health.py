...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "panergos.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("panergos.gateway.up", int(bool(gateway_running))),
        metric("panergos.gateway.active_agents", active_agents),
        metric("panergos.gateway.busy", int(bool(busy))),
        metric("panergos.gateway.drainable", int(bool(drainable))),
        metric("panergos.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("panergos.gateway.state", 1, **{"panergos.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"panergos.platform": str(platform), "panergos.platform.state": state}
        metrics.append(metric("panergos.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("panergos.platform.degraded", int(is_degraded), **pattrs, **{"panergos.error_code": error_code}))
        if is_degraded:
