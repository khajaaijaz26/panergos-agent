"""``$PANERGOS_HOME/cache/<kind>/`` materialisation helpers for the image/video
...
def cache_dir(kind: str) -> Path:
    """Return ``$PANERGOS_HOME/cache/<kind>/``, creating parents as needed."""
    from panergos_constants import get_panergos_home
    path = get_panergos_home() / "cache" / kind
