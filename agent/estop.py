...
``panergos pause`` writes a sentinel at ``$PANERGOS_HOME/ESTOP``; ``panergos resume``
...
(fail safe, e.g. ``touch ~/.panergos/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.panergos).
from agent.file_safety import _panergos_home_path as _panergos_home, _panergos_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `panergos pause`."""
    return _panergos_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (PANERGOS_HOME=~/.panergos/profiles/<n>) must still honor an operator's ~/.panergos/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Panergos is paused{tag}. New work is on hold; run `panergos resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `panergos resume` (%s)",
