...
``skill_manage`` per the Panergos authoring standards; large prose sources get the knowledge-base layout (lean SKILL.md
...
Follow the Panergos skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Panergos`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Panergos.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.panergos.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through Panergos tools.
...
Panergos-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Panergos tools by name in backticks: `terminal`, `read_file`,
