...
Owns one Codex thread per Panergos session: drives ``turn/start``, consumes
...
# Panergos' tools.terminal.security_mode -> Codex permissions profile id.
# Missing config -> workspace-write (Codex's own default).
_PANERGOS_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Panergos session, lifetime owned by AIAgent. Not thread-safe: one caller at a time."""
    ...
    def __init__(
        ...
        self._permission_profile = permission_profile or _PANERGOS_TO_CODEX_PERMISSION_PROFILE.get(
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(client_name="panergos", client_title="Panergos Agent", client_version=_get_panergos_version())
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Answer a codex server request (approval / elicitation) via Panergos' approval flow.
    ...
    def _respond_elicitation(self, params: dict) -> dict:
        """MCP elicitation: auto-accept our own panergos-tools server (opted in by enabling the runtime;
        exposes nothing codex's shell can't do); decline others so the user opts in via codex's own flow."""
        action = "accept" if (params.get("serverName") or "") == "panergos-tools" else "decline"
...
# Panergos approval choice -> codex decision (app-server-protocol v2). "deny" and
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map a Panergos approval choice onto codex's approval decision wire value."""
...
def _get_panergos_version() -> str:
    """Best-effort Panergos version string for codex's userAgent line."""
    try:
