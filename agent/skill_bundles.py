...
YAML files in ``<PANERGOS_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``PANERGOS_BUNDLES_DIR`` override (tests) or ``<PANERGOS_HOME>/skill-bundles``."""
    override = os.environ.get("PANERGOS_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_panergos_home() / "skill-bundles"
...
# File-level CRUD — used by `panergos bundles`.
