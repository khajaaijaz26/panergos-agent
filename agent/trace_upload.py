"""Upload a Panergos session transcript to Hugging Face as an agent trace, re-emitted in the **Claude Code
...
DEFAULT_DATASET_NAME = "panergos-traces"
_PANERGOS_VERSION = "panergos-agent"
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.panergos/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `panergos trace upload`)."
)
...
# --- Conversion: Panergos OpenAI-format messages -> Claude Code JSONL ---
...
def build_trace_jsonl(messages: List[Dict[str, Any]], *, session_id: str, model: str = "", cwd: str = "", redact: bool = True) -> str:
    ...
    for msg in messages:
        ...
        entry = {  # key order is the wire order
            ...
            "sessionId": session_id, "version": _PANERGOS_VERSION, "gitBranch": git_branch, "uuid": turn_uuid,
...
def upload_session_trace(
    ...
    """CLI/gateway entry point: load, convert, upload to ``{user}/panergos-traces``. Status string, never raises."""
    if not session_id:
