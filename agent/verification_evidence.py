...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("panergos-verify-", "panergos-ad-hoc-")
...
def _db_path() -> Path:
    return get_panergos_home() / "verification_evidence.db"
...
def record_verify_run(
    *, root: str | Path, session_id: str | None = None, ok: bool, command: str = "panergos verify",
    scope: str = "full", output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``panergos verify`` run as verification evidence.
    ...
    return _insert_evidence(VerificationEvidence(
        command=command, canonical_command="panergos verify", kind="verify",
