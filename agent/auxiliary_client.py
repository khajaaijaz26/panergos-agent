...
from panergos_constants import OPENROUTER_BASE_URL, panergos_home_key
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_verify``, ``PANERGOS_CA_BUNDLE`` / ``SSL_CERT_FILE``); any failure → httpx default (``True``)."""
    try:
        from agent.ssl_verify import resolve_httpx_verify
        from panergos_cli.config import get_custom_provider_tls_settings, load_config_readonly
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `panergos update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    with contextlib.suppress(Exception):
        from panergos_cli.models import OPENCODE_ZEN_FREE_KEYLESS_PLACEHOLDER, opencode_zen_free_headers
    ...
    # Panergos owns aux retry/fallback policy; the SDK default (max_retries=2) would triple
    # wall time on a hung endpoint before Panergos sees one failure.
    # Panergos owns auxiliary retry + provider/model fallback policy (the same-provider transient retry in
    ...
    # so a 120s timeout can stall ~360s before Panergos sees a single failure (issue #54465). Disable
    # SDK-internal retries by default and let Panergos control the budget; explicit callers can still override
    # via kwargs.
...
def _run_protected_sync_provider_call(callback: Callable[[dict[str, Any]], Any], kwargs: dict[str, Any]) -> Any:
    ...
    threading.Thread(
        target=provider_context.run, args=(_provider_worker,), name="panergos-protected-aux-provider",
        daemon=True).start()
    while True:
...
def _client_declares(client_obj: Any, flag: str) -> bool:
    ...
    not isinstance, so out-of-tree clients can opt out of wrappers unimported (cf. SUPPORTS_PANERGOS_TOOL_CALLS)."""
    try:
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    try:
        from panergos_cli.auth import resolve_api_key_provider_credentials
        from panergos_cli.models_pricing import fetch_models_with_pricing
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://github.com/khajaaijaz26/panergos-agent",
    "X-Title": "Panergos Agent",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    try:
        from panergos_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Precedence env > config > default: ``PANERGOS_OPENROUTER_CACHE`` overrides
    ``openrouter.response_cache``; ``PANERGOS_OPENROUTER_CACHE_TTL`` (1-86400 s) overrides
    ...
    if or_config is None:
        try:
            ...
            or_config = {}
    env_cache = os.environ.get("PANERGOS_OPENROUTER_CACHE", "").strip().lower()
    if not (env_cache in {"1", "true", "yes", "on"} if env_cache else or_config.get("response_cache", False)):
        return headers
    headers["X-OpenRouter-Cache"] = "true"
    env_ttl = os.environ.get("PANERGOS_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
...
_NVIDIA_NIM_CLOUD_HEADERS = {"X-BILLING-INVOKE-ORIGIN": "PanergosAgent"}
...
_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://github.com/khajaaijaz26/panergos-agent",
    "X-Title": "Panergos Agent",
    "User-Agent": f"PanergosAgent/{_PANERGOS_VERSION}",
}

# OpenRouter's silent fallback must remain free; configured models are honored unchanged.
...
def _to_openai_base_url(base_url: str) -> str:
    ...
    if base_url_hostname(url) == "api.actual.inc":
        from panergos_cli.auth import normalize_actual_base_url
...
def _pool_runtime_api_key(entry: Any) -> str:
    # runtime_api_key handles provider-specific fallback; None entry returns an empty key.
...
class _CodexCompletionsAdapter:
    ...
    def _build_responses_kwargs(self, kwargs: Dict[str, Any]) -> Tuple[Dict[str, Any], str, Any]:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Codex only knows the base slug; strip the Panergos ``-900k`` picker suffix.
    ...
    def create(self, **kwargs) -> Any:
        from panergos_cli.providers import is_actual_route

        if is_actual_route(
            getattr(self._client, "_panergos_aux_effective_provider", ""),
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        # (already TRANSLATED to native fields — raw would 400 on strict gateways) and ``_`` Panergos plumbing.
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``panergos_cli.runtime_provider._detect_api_mode_for_url`` so aux and main agree: any
...
def _maybe_wrap_anthropic(
    ...
    """
    # Anthropic/Bedrock/Codex wrappers, plus any client declaring PANERGOS_SKIP_TRANSPORT_WRAP
    ...
    if (
        ...
        or _client_declares(client_obj, "PANERGOS_SKIP_TRANSPORT_WRAP")
    ):
        return client_obj
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    """
    try:
        from panergos_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL, _xai_validate_inference_base_url
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    _url(_scoped_key_env("PANERGOS_XAI_BASE_URL"))
    ...
    try:
        from panergos_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    if pool_present:
        ...
        if token:
            return token
    try:
        from panergos_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from panergos_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            with contextlib.suppress(ImportError):
                from panergos_cli.auth import is_provider_explicitly_configured
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            from panergos_cli.models import copilot_default_headers
...
def _endpoint_default_headers(
    ...
    if base_url_host_matches(base_url, "api.kimi.com"):
        ...
        from panergos_cli.copilot_auth import copilot_request_headers
        ...
        from tools.xai_http import panergos_xai_default_headers
        headers = dict(panergos_xai_default_headers())
    else:
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    try:
        from panergos_cli.config import cfg_get, load_config_readonly
...
def _read_main_field(field: str, *, readonly: bool, lower: bool = False) -> str:
    ...
    with contextlib.suppress(Exception):
        from panergos_cli import config as _cfg_mod
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `panergos setup` or `panergos model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key, base_url=base_url, default_headers=panergos_xai_default_headers()
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *, model: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    try:
        from panergos_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _refresh_copilot_credentials() -> bool:
    from panergos_cli.copilot_auth import _jwt_cache, _token_fingerprint, exchange_copilot_token, resolve_copilot_token
...
def _refresh_codex_credentials() -> bool:
    from panergos_cli.auth import resolve_codex_runtime_credentials
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_panergos_fallback_destination", None)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    from panergos_cli.fallback_config import resolve_entry_api_key
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        with contextlib.suppress(Exception):
            client._panergos_fallback_destination = _fallback_destination_from_entry(entry, client, resolved_model)
...
def _warn_stale_openai_base_url(runtime_provider: str) -> None:
    ...
    ~/.panergos/.env value after `panergos model` poisons routing)."""
    ...
    if (_env_base and _cfg_provider and _cfg_provider != "custom" and not _cfg_provider.startswith("custom:")):
        logger.warning(
            ...
            "Run: panergos model to reconfigure, or remove "
            "OPENAI_BASE_URL from ~/.panergos/.env",
...
def _discovery_chain_allowed(main_provider: str, task: Optional[str] = None) -> bool:
    ...
    compression silently charged to an unrelated provider account)."""
    ...
    logger.warning(
        ...
        "configured — refusing to guess another logged-in provider. Re-authenticate (`panergos model`) "
...
def _resolve_auto_route(
    ...
    top-level chain; (3) OpenRouter → custom → API-key providers, only with no policy
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_panergos_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if _client_declares(sync_client, "PANERGOS_SKIP_ASYNC_WRAP"):
        return sync_client, model
    ...
    # Panergos owns the auxiliary retry/timeout budget; disable SDK-internal retries.
    # See #54465.
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from panergos_cli.model_normalize import normalize_model_for_provider
...
def _is_actual_auxiliary_route(req: _ResolveRequest, base_url: str) -> bool:
    from panergos_cli.auth import normalize_actual_base_url
    from panergos_cli.providers import is_actual_route
    from panergos_cli.route_identity import normalize_route_base_url
...
def _wrap_transport(req: _ResolveRequest, client_obj: Any, final_model_str: str,
    ...
    if _is_actual_auxiliary_route(req, base_url_str):
        ...
        client._panergos_aux_effective_provider = "actual"
        return client
    needs_codex = not (
...
def _resolve_auto_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if routed_client is not None and effective_provider:
        try:
            setattr(routed_client, "_panergos_aux_effective_provider", effective_provider)
...
def _resolve_openai_codex_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if not model:
        ...
        return None, None
    no_token_msg = "resolve_provider_client: openai-codex requested but no Codex OAuth token found (run: panergos model)"
    if req.raw_codex:
...
def _resolve_xai_oauth_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default,
                          ...
                          "OAuth token found (run: panergos model -> xAI Grok OAuth — SuperGrok / Premium+)")
...
def _resolve_named_custom_branch(req: _ResolveRequest) -> Optional[_ResolveResult]:
    ...
    # canonical names still defer to their built-in definitions.
    custom_entry = None
...
def _resolve_azure_foundry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default_model,
                          ...
                          "runtime resolution failed (run: panergos doctor for diagnostics)")
...
def _resolve_api_key_branch(req: _ResolveRequest, pconfig: Any, resolve_creds: Callable) -> _ResolveResult:
    ...
    try:
        from panergos_cli.models import opencode_zen_free_runtime as _oc_free_rt
    ...
    if provider == "copilot" and final_model and not req.raw_codex:
        with contextlib.suppress(ImportError):
            from panergos_cli.models import _should_use_copilot_responses_api
...
def _resolve_registry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    try:
        ...
        logger.debug("panergos_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if auth_type == "vertex":
        ...
        # Supported OAuth providers return from their explicit branches.
...
def resolve_provider_client(
    ...
    # Exclude ``auto`` because a stale main slug could pair with a different selected provider.
    if not model and provider != "auto":
        ...
        # Each provider branch below sees a non-empty model whenever the user has one configured.
...
# openai-codex has no safe default model; callers set auxiliary.<task>.model.
...
def _vision_auto_route(
    ...
    """Auto-detect order: main provider + model, OpenRouter, then DeepInfra."""
...
def get_auxiliary_extra_body() -> dict:
    """Return shared auxiliary-request body fields."""
    return {}
...
def _client_cache_key(
    ...
    # Profile home leads the key: callers that omit api_key (for example pooled credentials) would
    # otherwise share one client across multiplex profiles holding different credentials.
    return (panergos_home_key(), provider, async_mode, base_url or "", api_key_key, api_mode or "", runtime_key, is_vision, task_key, pool_hint, model_key)
...
def _acquire_sync_aux_semaphore(task: Optional[str]) -> Optional[threading.BoundedSemaphore]:
    ...
    if limit is None:
        return None
    from panergos_constants import panergos_home_key
    return _cached_semaphore(_aux_sync_semaphores, (panergos_home_key(), task), limit, threading.BoundedSemaphore)
...
def _acquire_async_aux_semaphore(task: Optional[str]):
    ...
    try:
        ...
        return None
    from panergos_constants import panergos_home_key
    return _cached_semaphore(_aux_async_semaphores, (panergos_home_key(), task, id(loop)), limit, asyncio.Semaphore)
...
def _merge_aux_extra_body(
    ...
    """Merge caller, profile, and generic reasoning request-body fields."""
...
def _managed_local_netloc() -> str:
    ...
    if now - ts < _MANAGED_LOCAL_STATE_TTL_S:
        return cached
    try:
        from panergos_cli.local_runtime.supervisor import state_path
...
def _is_managed_local_endpoint(base_url: Optional[str]) -> bool:
    """True when *base_url* targets the llama-server this Panergos manages."""
    if not base_url:
        return False
...
def _resolve_call_client(
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is None:
                    raise RuntimeError(
                        ...
                        f"a different provider with `panergos model`.")
    ...
    if client is None:
        raise RuntimeError(f"No LLM provider configured for task={task} "
                           f"provider={resolved_provider}. Run: panergos setup")
