...
from panergos_cli.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'panergos memory status'. Note: "
        "systemd/gateway services do not inherit ~/.panergos/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    with suppress(Exception):
        from panergos_cli.providers import PANERGOS_OVERLAYS, get_provider
        overlay = PANERGOS_OVERLAYS.get(provider)
    ...
    with suppress(Exception):
        ...
        from panergos_cli.models import normalize_provider as normalize_model_provider
        from panergos_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: panergos config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    """Per-profile marker path (``$PANERGOS_HOME`` is profile-scoped; not a config key)."""
    return get_panergos_home() / ".codex_gpt55_autoraise_notice"
...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    ...
    if is_actual_route(agent.provider, base_url):
        ...
        try:
            ...
            from panergos_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    with suppress(Exception):
        from panergos_cli.model_normalize import (
...
def _init_prompt_cache_config(agent):
    ...
    agent._cache_ttl = "5m"
    with suppress(Exception):
        from panergos_cli.config import load_config_readonly as _load_pc_cfg
...
def _setup_logging(agent):
    ...
    from panergos_logging import setup_logging, setup_verbose_logging
    setup_logging(panergos_home=_ra()._panergos_home)
    ...
    # would starve the root file handlers. Noise reduction belongs in panergos_logging.
...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
        try:
            from panergos_cli.auth import build_minimax_oauth_token_provider
...
def _routed_client_kwargs(agent, fallback_model, _provider_timeout) -> Dict[str, Any]:
    ...
    if _routed_client is not None:
        from panergos_cli.providers import is_actual_route, normalize_provider
        effective_provider = getattr(_routed_client, "_panergos_aux_effective_provider", "")
    ...
    if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
        ...
        raise RuntimeError(
            ...
            f"variable, or switch to a different provider with `panergos model`."
        )
    raise RuntimeError(
        "No LLM provider configured. Run `panergos model` to "
        "select a provider, or run `panergos setup` for first-time "
        "configuration."
    )
...
_HOST_DEFAULT_HEADERS: List[tuple[str, Callable[[Any, str], Dict[str, str]]]] = [
    ...
    ("githubcopilot.com", _lazy_headers("panergos_cli.models", "copilot_default_headers")),
    ...
    ("x.ai", _lazy_headers("tools.xai_http", "panergos_xai_default_headers")),
]
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    # A multiplexed gateway may have switched PANERGOS_HOME since model_tools was imported;
    ...
    # Kanban guidance is session-static (kanban_show iff PANERGOS_KANBAN_TASK); resolve once.
...
def _publish_session_id(session_id: str) -> None:
    ...
    try:
        ...
        if not delegated_child:
            os.environ["PANERGOS_SESSION_ID"] = session_id
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    # ~/.panergos/sessions/ — kept unconditionally for request_dump_*.json debug breadcrumbs.
    agent.logs_dir = get_panergos_home() / "sessions"
    ...
    # Process-scoped --yolo is persisted so `panergos --resume` restores the bypass
...
def _memory_provider_init_kwargs(agent, platform) -> Dict[str, Any]:
    ...
    with suppress(Exception):
        ...
        kwargs["agent_workspace"] = "panergos"
    return kwargs
...
def _compression_codex_settings(cfg: Dict[str, Any]) -> tuple[str, bool, Optional[int]]:
    ...
    if app_server_auto not in {"native", "panergos", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, panergos, off.",
            app_server_auto,
        )
...
def _configured_default_base_url(_agent_cfg, _model_cfg, _custom_providers) -> str:
    ...
    if _norm in _RUNTIME_FIRST_PROVIDER_IDS:
        ...
        with suppress(Exception):
            from panergos_cli.auth import resolve_provider as resolve_auth_provider
...
def _select_context_engine(_agent_cfg):
    ...
    if _selected_engine is None:
        try:
            from panergos_cli.plugins import get_plugin_context_engine
...
def _enforce_minimum_context(agent):
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Panergos Agent.  Choose a model with at least "
    ...
    if agent.context_compressor:
        try:
            agent.context_compressor.on_session_start(
                agent.session_id, panergos_home=str(get_panergos_home()),
...
def init_agent(
    ...
    Non-obvious parameters:
      ...
      skip_context_files: skip SOUL.md/.panergos.md/AGENTS.md/CLAUDE.md/.cursorrules injection;
        load_soul_identity keeps ~/.panergos/SOUL.md as identity regardless.
    """
    _install_safe_stdio()
    ...
    try:
        from panergos_cli.config import load_config_readonly as _load_agent_config
