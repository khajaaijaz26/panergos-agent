...
# Parameters Panergos sends on SOME routes only → hosts where that is deliberate.
...
# Some APIs surface credit depletion as a model vanishing; keep billing before model_not_found.
_404_RULES = (
...
def _plugin_verdict(c: _Ctx) -> Optional[Verdict]:
    ...
    try:
        from panergos_cli.plugins import get_plugin_error_classification
...
def _classify_400(c: _Ctx) -> Verdict:
    ...
    # Reasoning-mandatory route rejecting a disable (for example GLM-5.3). Deterministic
