...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from panergos_cli.moa_config import coerce_privacy_filter
...
def _slot_runtime(slot: dict[str, Any]) -> dict[str, Any]:
    ...
    # panergos_home_key() in the key: the resolved api_key/base_url are per-profile, and under a
    ...
    cache_key = (panergos_home_key(), provider, model)
    now = time.monotonic()
...
def _run_references_parallel(
    ...
    *agent*, the wait polls every ``_REFERENCE_POLL_INTERVAL_S`` so a user interrupt
    ...
    # Propagate the turn's contextvars (approval callbacks and request metadata).
...
def aggregate_moa_context(
    *, user_prompt: str, api_messages: list[dict[str, Any]], reference_models: list[dict[str, Any]],
    ...
    synth_prompt = (
        ...
        "Panergos agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Panergos agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
