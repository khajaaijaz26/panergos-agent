...
def ctx_bound(fn: Callable[..., Any]) -> Callable[..., Any]:
    ...
    is a ContextVar-scoped PANERGOS_HOME override plus the per-turn secret scope; a worker started
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include ``panergos_home`` (profile-scoped storage; never hardcode
        ``~/.panergos``) and ``platform``; may include ``agent_context`` ("primary" |
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        """Setup fields for ``panergos memory setup`` ([] if none): ``key``, ``description``,
    ...
    def backup_paths(self) -> List[str]:
        """Absolute paths of provider state OUTSIDE PANERGOS_HOME for ``panergos backup``/``import``
