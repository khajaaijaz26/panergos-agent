"""Fold an agent-as-provider's own activity back into Panergos' turn state.

Agent providers (ACP CLI shims, the codex app-server) run their own tools, so that
work must never come back as pending ``tool_calls`` (Panergos would re-run it) — but
...
``reasoning``. The client hands back ``panergos_projected_messages`` (completed
assistant/tool rows) and ``panergos_provider_tool_iterations`` on the completion
...
def splice_provider_projection(agent: Any, response: Any, messages: list[dict[str, Any]]) -> int:
    ...
    """
    projected = getattr(response, "panergos_projected_messages", None)
    ...
    try:
        iterations = int(getattr(response, "panergos_provider_tool_iterations", 0) or 0)
