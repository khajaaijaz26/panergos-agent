...
_GLOBAL_ENV_EXACT = frozenset({
    # Panergos runtime / deployment
    "PANERGOS_HOME", "PANERGOS_PROFILE", "PANERGOS_GATEWAY_LOCK_DIR",
    "PANERGOS_MAX_ITERATIONS", "PANERGOS_API_TIMEOUT",
    ...
    "PANERGOS_KANBAN_DB", "PANERGOS_KANBAN_WORKSPACES_ROOT", "PANERGOS_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "PANERGOS_KANBAN_",
    "PANERGOS_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Panergos writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    """THE ``.env`` tokenizer: every reader (profile scope, ``panergos_cli.config.load_env``, the dashboard
...
def build_profile_secret_scope(panergos_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(panergos_home) / ".env")
    try:
        from panergos_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(panergos_home))
    except Exception:
        external_secrets = {}
