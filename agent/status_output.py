...
class StatusOutputMixin:
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        the main response; ``force=True`` bypasses both. ``suppress_status_output`` (``panergos chat -q``) wins."""
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``suppress_status_output`` (the strict machine-readable mode used by ``panergos chat -Q``) always
    ...
    def _emit_notice(self, notice) -> None:
        """Fire a structured notice to the active driver (TUI / CLI)."""
