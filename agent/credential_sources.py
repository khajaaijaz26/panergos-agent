"""Unified removal contract for every credential source Panergos reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``panergos auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.panergos/.env; hint when the shell exports it."""
    from panergos_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like panergos_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            f"(not in ~/.panergos/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Panergos.",
            f"  The pool entry is now suppressed — Panergos will ignore "
            f"{env_var} until you run `panergos auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_panergos_pkce(provider: str, removed) -> RemovalResult:
    """~/.panergos/.anthropic_oauth.json is ours — delete it outright."""
    from panergos_constants import get_panergos_home

    result = RemovalResult()
    oauth_file = get_panergos_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Panergos Anthropic OAuth credentials")
...
def _remove_auth_store_oauth(provider: str, removed) -> RemovalResult:
    """Clear auth.json ``providers.<provider>`` for supported OAuth providers.
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `panergos model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `panergos auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `panergos auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `panergos auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="panergos_pkce",
        remove_fn=_remove_panergos_pkce,
        description="~/.panergos/.anthropic_oauth.json",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `panergos auth add qwen-oauth` to re-enable if needed.",
        ),
