...
_PERSISTENCE_CAUSE_EXPLANATIONS: Dict[str, str] = {
    ...
    "turn_lease": (
        "the turn was stopped because another Panergos process "
    ...
    "locked": (
        ...
        "(another Panergos process was writing to the state "
    ...
    "replaced": (
        ...
        "`panergos doctor --fix` or in-place FTS repair — stop "
    ...
    "deleted_wal": (
        "the turn was stopped because a live Panergos process held a retired "
        ...
        "Check the logs for whether Panergos captured the retired generation, "
        "then read the adjacent state.db.retired-wal-*/manifest.json. If "
        "manifest.main.mode is `copied`, inspect that artifact with `panergos "
    ...
    "corrupt": (
        ...
        "1. Run `panergos {profile_arg}doctor --fix`\n"
        "2. Stop the gateway, then recover with:\n"
        "   panergos {profile_arg}sessions recover --source {db_path} --inspect-only\n"
        "   (if it reports recoverable) panergos {profile_arg}sessions recover "
    ...
    "fts_index": (
        ...
        "recovery tools or restore a backup. Run `panergos {profile_arg}doctor --fix` "
        "(or restart Panergos, which repairs the index on open), then "
...
_PERSISTENCE_DEFAULT_EXPLANATION = (
    ...
    "Check the state database health (`panergos doctor`), then "
    "send your message again."
)
...
def _display_flag_enabled(agent, *, env_var: str, config_key: str, cache_attr: str) -> bool:
    ...
    and tests patch it at ``panergos_cli.config``). Any failure → True (safe default: on)."""
    try:
...
class TurnExplainersMixin:
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # Panergos-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        """``display.file_mutation_verifier`` / ``PANERGOS_FILE_MUTATION_VERIFIER`` (a patchable seam)."""
        return _display_flag_enabled(
            self, env_var="PANERGOS_FILE_MUTATION_VERIFIER", config_key="file_mutation_verifier",
            cache_attr="_file_mutation_verifier_enabled_cache",
        )

    def _turn_completion_explainer_enabled(self) -> bool:
        """``display.turn_completion_explainer`` / ``PANERGOS_TURN_COMPLETION_EXPLAINER``."""
        return _display_flag_enabled(
            self, env_var="PANERGOS_TURN_COMPLETION_EXPLAINER", config_key="turn_completion_explainer",
    ...
    def _format_turn_completion_explanation(
        ...
        if body is None and reason == "session_persistence_failed":
            ...
            if persistence_cause in ("corrupt", "fts_index"):
                ...
                # the process default, and a bare `panergos` follows active_profile (#105887).
                ...
                body = body.replace(
                    "{backups_dir}", str(get_default_panergos_root() / "backups")
                )
