...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    if is_token_provider(key):
        ...
        _plines(
            ...
            "   Run `panergos doctor` for credential-chain diagnostics, or",
    ...
    _dhh = display_panergos_home()
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Panergos-managed OAuth/setup tokens",
        ...
        "     • Legacy cleanup: panergos config set ANTHROPIC_TOKEN \"\"",
        "     • Clear stale keys: panergos config set ANTHROPIC_API_KEY \"\"",
    )
...
def _refresh_credentials_after_401(
    ...
    Anthropic), printing user-facing diagnostics when a refresh fails.
...
def recover_after_classification(
    ...
    Credential-pool rotation → image shrink →
    ...
    # Some reasoning-mandatory routes (for example OpenRouter GLM-5.3) 400 on
    ...
    if (
        ...
        try:
            from panergos_cli.models_reasoning_caps import refresh_reasoning_caps_async
    ...
    if provider in {"openai-codex", "xai-oauth"} and status_code == 401:
        if provider == "openai-codex":
            _vlines(
                ...
                "      2. Then run `panergos auth` to re-authenticate.",
            )
        elif provider == "xai-oauth":
            _vlines(
                agent,
                "   💡 xAI OAuth token was rejected (HTTP 401). To fix:",
                "      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `panergos model`.",
            )
        return
    _vlines(
        agent,
        "   💡 Your API key was rejected by the provider. Check:",
        "      • Is the key valid? Run: panergos setup",
...
def nonretryable_client_error_result(
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            "        panergos fallback add   (interactive picker — same as `panergos model`)",
        )
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _policy_response = (
            ...
            "(not a Panergos/gateway failure).\n\n"
...
def max_retries_exhausted_result(
    ...
    if _is_thinking_timeout:
        _vlines(
            ...
            "in `~/.panergos/config.yaml` to extend the per-call timeout. (Panergos's built-in floor is 600s for "
    ...
    result.update({
        ...
        # Present only for billing walls.
...
def log_api_error_attempt(
    ...
    if getattr(api_error, "status_code", None) == 404:
        try:
            from panergos_cli.model_normalize import suggest_prefixed_model_id
        ...
        if _suggestion:
            _blines(
                ...
                f"      Did you mean '{_suggestion}'?  Re-pick it with `panergos model`.",
            )
...
class ClassifiedErrorVerdict:
    ...
    ``"break"`` (restart armed on ``_retry``), ``"continue"`` (re-enter the retry loop),
    or ``"fallthrough"`` (proceed to overflow / client-error / backoff
