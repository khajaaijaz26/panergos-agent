...
def _fm_field(fm: dict[str, Any], key: str) -> Any:
    """Top-level ``key`` or ``metadata.panergos.<key>``; tolerant of the string-valued
    ...
    return panergos.get(key) if isinstance(panergos, dict) else None
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        try:
            return json.loads((get_panergos_home() / "skills" / ".usage.json").read_text(encoding="utf-8"))
        except Exception:
            return {}
...
def _memory_cards() -> list[dict[str, Any]]:
    ...
    base = get_panergos_home() / "memories"
...
def build_learning_graph() -> dict[str, Any]:
    ...
    roots = [("base", Path(__file__).resolve().parent.parent / "skills"), ("profile", get_panergos_home() / "skills")]
    learned_skills = {
