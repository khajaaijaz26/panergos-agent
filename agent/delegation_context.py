...
A Panergos process may itself be a Kanban dispatcher worker with PANERGOS_KANBAN_* in
...
_DELEGATED_CHILD_CONTEXT: ContextVar[bool] = ContextVar("panergos_delegated_child_context", default=False)
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar("panergos_non_dispatcher_owned_context", default=False)

DELEGATED_CHILD_ENV_MARKER = "PANERGOS_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "PANERGOS_KANBAN_TASK", "PANERGOS_KANBAN_RUN_ID", "PANERGOS_KANBAN_CLAIM_LOCK",
    "PANERGOS_KANBAN_GOAL_MODE", "PANERGOS_KANBAN_GOAL_MAX_TURNS",
)
...
def is_dispatcher_owned_worker_context() -> bool:
    """The single predicate every ``PANERGOS_KANBAN_*`` identity gate should use."""
...
def delegated_child_subprocess_env(
    ...
    """
    if not (is_delegated_child_process_context() or os.environ.get("PANERGOS_KANBAN_TASK")
            or (env and (env.get("PANERGOS_KANBAN_TASK") or env.get(DELEGATED_CHILD_ENV_MARKER)))):
