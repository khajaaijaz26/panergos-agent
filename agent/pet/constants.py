...
inferred from the atlas shape so Panergos renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (Panergos names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy Panergos/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical Panergos names -> accepted row-name aliases in descending preference.
