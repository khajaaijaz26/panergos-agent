...
def resolve_provider(*, require_references: bool = True, prefer: str | None = None) -> SpriteProvider:
    ...
    Preference: ``PANERGOS_PET_IMAGE_PROVIDER`` (QA override, unknown values ignored),
    ...
    forced = os.environ.get("PANERGOS_PET_IMAGE_PROVIDER", "").strip().lower()
    ...
    raise GenerationError(
        ...
        "Open `panergos tools` → Image Generation and configure OpenRouter "
...
def generate(
    ...
    *aspect_ratio* ``"landscape"`` (row strips) gives every frame horizontal room so
    ...
    # Providers disagree on the ref kwarg name: the OpenRouter backend reads
