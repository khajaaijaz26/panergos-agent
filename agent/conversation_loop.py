...
from panergos_logging import set_session_context
from tools.skill_provenance import set_current_write_origin
from utils import base_url_host_matches

logger = logging.getLogger(__name__)

# Must mirror _STALE_TOOL_CALL_MARKER_RE in panergos_state.py; kept local so importing
# panergos_state (module-level DEFAULT_DB_PATH) is not forced at load time.
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Panergos tool use: model=%s provider=%s base_url=%s "
    ...
    return (
        f"Ollama loaded `{model}` with only {runtime_ctx:,} tokens of runtime context, but Panergos "
        ...
        "again. A known-good starting point is 65,536 tokens. In Panergos config, set "
...
def _maybe_grow_local_window(agent: Any, compressor: Any,
    ...
    if provider not in ("llamacpp", "llama.cpp", "llama-cpp", "custom") or not (
        ...
        return None
    try:
        from panergos_cli.local_runtime.growth import maybe_grow_window
...
_BILLING_PROVIDERS = (
    (("openai",), ("api.openai.com",), "OpenAI", "https://platform.openai.com/settings/organization/billing"),
    (("anthropic",), ("api.anthropic.com",), "Anthropic", "https://console.anthropic.com/settings/billing"),
    (("openrouter",), ("openrouter.ai",), "OpenRouter", "https://openrouter.ai/settings/credits"),
    (("xai", "xai-oauth"), ("api.x.ai",), "xAI", "https://console.x.ai/team/default/billing"),
    (("deepseek",), ("api.deepseek.com",), "DeepSeek", "https://platform.deepseek.com/top_up"),
    (("groq",), ("api.groq.com",), "Groq", "https://console.groq.com/settings/billing"),
    (("mistral",), ("api.mistral.ai",), "Mistral", "https://console.mistral.ai/billing"),
    (("together",), ("api.together.ai", "api.together.xyz"), "Together AI", "https://api.together.ai/settings/billing"),
    (("fireworks",), ("fireworks.ai",), "Fireworks AI", "https://fireworks.ai/account/billing"),
    (("perplexity",), ("perplexity.ai",), "Perplexity", "https://www.perplexity.ai/settings/api"),
    (("google", "gemini"), ("generativelanguage.googleapis.com",), "Google AI", "https://aistudio.google.com/app/billing"),
...
def _billing_provider_details(provider: str, base_url: str) -> tuple[str, Optional[str]]:
    """Resolve a generic provider label and billing page from its slug or API host."""
    slug = (provider or "").strip().lower()
    base = str(base_url or "")
    for slugs, hosts, label, url in _BILLING_PROVIDERS:
        if slug in slugs or any(base_url_host_matches(base, host) for host in hosts):
            return label, url
...
def _billing_or_entitlement_message(
    *, capability: str, provider: str, base_url: str, model: str, unverified: bool = False
    ...
    provider_label, billing_url = _billing_provider_details(provider, base_url)
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            return "\n".join([
                ...
                "Retry with a fresh credential state: `panergos auth reset anthropic`. Until that "
    ...
    billing_action = (
        f"Add credits or update billing at {billing_url}, then retry."
        if billing_url
        else "Add credits or update billing with that provider, then retry."
    )
    return "\n".join([
...
def _billing_block_dict(provider, base_url, model, message="", *, unverified: bool = False) -> Optional[dict]:
    """Return a provider-neutral structured billing descriptor."""
    provider_label, billing_url = _billing_provider_details(provider, base_url)
    block = {
...
def _decode_inline_moa_turn(user_message, persist_user_message):
    ...
    try:
        from panergos_cli.moa_config import decode_moa_turn
...
def _run_conversation_turn(
    ...
    # Adopt ~/.panergos/.env credential/base-url edits made since the last turn — a
