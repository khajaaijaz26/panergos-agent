...
INSPECTION_STAMP_ATTR = "data-panergos-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-panergos-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-panergos-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-panergos-vault-slot]").forEach((n) => n.removeAttribute("data-panergos-vault-slot"));
