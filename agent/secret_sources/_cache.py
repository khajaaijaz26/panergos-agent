...
a cache problem must never block Panergos startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_panergos_dotenv()``, else ``$PANERGOS_HOME``/``~/.panergos``."""
    if home_path is None:
        from panergos_constants import get_panergos_home

        home_path = get_panergos_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<panergos_home>/cache/<basename>``::
