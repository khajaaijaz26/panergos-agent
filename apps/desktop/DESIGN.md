...
2. **Borderless elevation for floating panels.** Overlays float on
   `shadow-eclipse` + a `--stroke-eclipse` hairline, not thick framed boxes. In-panel
...
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-eclipse`,
...
```
shadow-eclipse           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-eclipse) /* currentColor hairline, theme-adaptive */
```
...
| `--stroke-eclipse` | the overlay hairline (pairs with `shadow-eclipse`) |
...
- Pick the vocabulary by semantic context and reuse the existing icon for an
  ...
  group.
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the Panergos icon from
  `assets/icon.png`, softly rounded and identical in light/dark.
...
- [ ] Reuse a primitive (`Button`, `SearchField`, `SegmentedControl`,
      ...
      forking one?
- [ ] Tokens (`--ui-*`, `shadow-eclipse`, `--stroke-eclipse`) — zero raw colors /
      one-off shadows?
...
- [ ] Overlay uses `shadow-eclipse` + `border-(--stroke-eclipse)`, no hard border?
