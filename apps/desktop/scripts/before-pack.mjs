 ...
 * product name (`Panergos`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Panergos'
 *
 * This is a hard failure with no obvious cause for the user — `panergos desktop`
 ...
 * (the mac-unpacked Panergos.app bundle) and Windows (win-unpacked), so we
 ...
 * Panergos.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `panergos desktop --build-only` (panergos_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Panergos.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Panergos'}.exe`
