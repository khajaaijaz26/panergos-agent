...
// no longer bundles the Panergos Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Panergos.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Panergos'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Panergos.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Panergos'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default PANERGOS_HOME for non-sandboxed runs -- matches main.ts's
// resolvePanergosHome(). On Windows it's %LOCALAPPDATA%\panergos; elsewhere
// it's ~/.panergos. The fresh-install sandbox launchFresh() sets its own
// PANERGOS_HOME and never touches this.
const DEFAULT_PANERGOS_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'panergos')
  }
  return path.join(os.homedir(), '.panergos')
})()
const VENV_ROOT = path.join(DEFAULT_PANERGOS_HOME, 'panergos-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'panergos-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.PANERGOS_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Panergos-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Panergos-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Panergos-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Panergos-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.PANERGOS_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.PANERGOS_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const panergosHome = path.join(sandbox, 'panergos-home')
  ...
  env.PANERGOS_DESKTOP_CWD = cwd
  env.PANERGOS_DESKTOP_IGNORE_EXISTING = '1'
  env.PANERGOS_DESKTOP_TEST_MODE = 'fresh-install'
  env.PANERGOS_DESKTOP_USER_DATA_DIR = userDataDir
  env.PANERGOS_HOME = panergosHome
  delete env.PANERGOS_DESKTOP_PANERGOS
  delete env.PANERGOS_DESKTOP_PANERGOS_ROOT
  ...
  console.log(`  PANERGOS_HOME: ${panergosHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(panergosHome, 'panergos-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Panergos Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of panergos_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'panergos-agent', 'panergos_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Panergos
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + PANERGOS_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  PANERGOS_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
