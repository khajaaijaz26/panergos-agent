...
//   D=$(mktemp -d) && PANERGOS_GUIDED_ONBOARDING=1 PANERGOS_SKIP_INTRO=1 \
//     PANERGOS_HOME="$D/home" PANERGOS_DESKTOP_USER_DATA_DIR="$D/userdata" panergos desktop
...
const sandbox = process.argv[2] ? path.resolve(process.argv[2]) : fs.mkdtempSync(path.join(os.tmpdir(), 'panergos-connectors-'))
const port = 5194
const url = `http://127.0.0.1:${port}`
const python = process.env.PANERGOS_DESKTOP_PYTHON
if (!python || !fs.existsSync(python)) throw new Error('Set PANERGOS_DESKTOP_PYTHON to your installed Panergos Python executable.')
...
for (const dir of ['electron', 'work']) fs.mkdirSync(path.join(sandbox, dir), { recursive: true })
const env = Object.fromEntries(['PATH', 'TMPDIR', 'LANG', 'SHELL'].filter(key => process.env[key]).map(key => [key, process.env[key]]))
Object.assign(env, {
  HOME: sandbox, PANERGOS_HOME: path.join(sandbox, '.panergos'),
  PANERGOS_DESKTOP_USER_DATA_DIR: path.join(sandbox, 'electron'), PANERGOS_DESKTOP_CWD: path.join(sandbox, 'work'),
  PANERGOS_DESKTOP_PANERGOS_ROOT: root, PANERGOS_DESKTOP_PYTHON: python,
  PANERGOS_DESKTOP_IGNORE_EXISTING: '1', PANERGOS_DESKTOP_APP_NAME: 'Panergos Connector Rehearsal',
  PANERGOS_GUIDED_ONBOARDING: '1', PANERGOS_SKIP_INTRO: '1', PANERGOS_DESKTOP_DEV_SERVER: url, PANERGOS_DESKTOP_CDP_PORT: '9344'
})
