#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Panergos icon + version metadata onto the
// built Panergos.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Panergos.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `panergos desktop`, the installer's
...
// path (which rebuilds via `panergos desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Panergos.exe>
//
...
// Stamp the Panergos icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Panergos Agent',
      FileDescription: 'Panergos Agent',
      CompanyName: 'Shaik Khaja Aijaz Ahmed',
      LegalCopyright: 'Copyright (c) 2026 Shaik Khaja Aijaz Ahmed'
    }
  })

  console.log('[set-exe-identity] done — Panergos icon + identity stamped')
}
