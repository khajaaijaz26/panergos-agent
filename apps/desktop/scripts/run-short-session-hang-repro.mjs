...
const UPSTREAM_URL = 'https://github.com/khajaaijaz26/panergos-agent.git'
...
const LEGACY_STORAGE_KEY = 'panergos.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'panergos.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.panergosDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `panergos-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const panergosHome = join(sandbox, 'panergos-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(panergosHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(panergosHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        PANERGOS_DESKTOP_APP_NAME: `PanergosShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        PANERGOS_DESKTOP_PANERGOS_ROOT: target.targetRoot,
        PANERGOS_DESKTOP_IGNORE_EXISTING: '1',
        PANERGOS_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'panergos-short-session-ab-'))
  const prepared = []
