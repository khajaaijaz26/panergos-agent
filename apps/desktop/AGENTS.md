...
How to build Panergos Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Panergos surface can also
...
- A **connection/mode apply** (local ↔ remote) is the soft re-home:
...
- A **runtime home change** (switching the underlying `PANERGOS_HOME` profile) is
...
Desktop lives at the seams: versions, profiles, local vs remote,
...
its shape. "Plugin" means several unrelated things across Panergos — do not assume
...
`env -u NODE_ENV PANERGOS_GUIDED_ONBOARDING=1 PANERGOS_HOME=<tmp>/.panergos PANERGOS_DESKTOP_USER_DATA_DIR=<tmp>/electron-user-data npm run dev`
(replace `<tmp>` with that directory). Stop Electron and its dev server after
the run.
