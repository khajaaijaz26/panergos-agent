  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-muted-foreground">
        <strong className="font-medium text-foreground">Nothing connects yet.</strong> Panergos will offer to link these
...
export function LookCard({ locked }: CardProps) {
  ...
  const accent = answers.accent ?? PANERGOS_ACCENT
  const picked = accents.find(swatch => swatch.hex === accent.toLowerCase())

  const pickAccent = (hex: string) => {
    const seed = hex === PANERGOS_ACCENT ? null : hex
