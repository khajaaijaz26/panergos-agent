 ...
 *
 * The chat belongs to a persistent `panergos-setup` profile, so it survives onboarding and can be found again. `setup`
 * is the internal name throughout this module (the profile key, the atoms, the hidden `[setup]` notes); the user sees
 * only Panergos and the title `Welcome to Panergos`.
 *
...
export const SETUP_PROFILE = 'panergos-setup'
...
export const SETUP_CHAT_TITLE = 'Welcome to Panergos'
...
export function buildFirstTaskRunbook(
  ...
  return [
    `You are Panergos. The user's welcome chat just opened this session so one task can have room to run: ${task.trim()}.`,
...
const pluginRunbook = (root: string) => [
  'THIS IS A PLUGIN JOB: the thing you are building is a piece of the Panergos app itself, and it will appear in the window the user is looking at right now. That is the whole point — do not let it become a script in a folder.',
  `A plugin is ONE file: \`${root}/<name>/plugin.js\`. Plain ESM, no build step, no package.json, no install. It imports from \`@panergos/plugin-sdk\` and calls \`jsx()\` from \`react/jsx-runtime\` directly (there is no JSX compiler in this path — writing \`<div>\` will not work). It default-exports \`{ id, name, register(ctx) }\` and \`register\` calls \`ctx.register({ id, area, order, render })\`. The runtime loads it the moment you save, and reloads it on every later save, so there is no restart to ask them for.`,
  'LOOK BEFORE YOU WRITE. Read the desktop plugin-building skill first — it has the SDK surface, the areas you can render into, and the traps. If a Panergos source checkout is available, inspect a reviewed plugin under `apps/desktop/src/plugins` close to what you are making; those built-ins show the real shapes (a statusbar chip, a composer action, a full pane).',
  ...
  'Never ask them to restart the app, never edit anything outside their plugin folder, and never touch the Panergos install itself. If the plugin errors on load, the app toasts it and keeps running — read the error, fix the file, save again.'
]
...
export async function buildFirstTaskSeedMessages(
  ...
  const root = plan === 'plugin' ? await window.panergosDesktop?.desktopPluginsRoot?.() : undefined
...
export async function ensureSetupProfile(request: GatewayRequest): Promise<void> {
  try {
    await request('profiles.create', {
      description: 'Where Panergos met you — walks your first run, then checks in as you find your feet.',
      name: SETUP_PROFILE,
      clone_from: 'default',
      share_auth: true,
      no_alias: true,
