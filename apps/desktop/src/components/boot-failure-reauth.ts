...
// True when the app is pointed at a remote backend (it resolves to a
 ...
 * post-boot startPanergos()/ticket mint fails (liveness reset → rebuild, wake
...
// the local Retry/Repair buttons can't. A lapsed remote OAuth session is the
// same failure.
