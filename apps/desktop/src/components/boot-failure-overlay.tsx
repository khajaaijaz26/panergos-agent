...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.panergosDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.panergosDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.panergosDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }

  // Clear this gateway's stale auth first, then re-establish it through the
  // connection's owning login flow. Hosted OAuth connections reuse their session
  ...
  const openLogs = () => void window.panergosDesktop?.revealLogs().catch(() => undefined)
  ...
  if (view === 'connect') {
    return (
      <div
        ...
        data-glass-opaque=""
      >
        <div className="flex max-h-[86vh] w-full max-w-[46rem] flex-col overflow-hidden rounded-xl border border-(--stroke-eclipse) bg-(--ui-chat-bubble-background) shadow-eclipse">
  ...
  return (
    <div
      ...
      data-glass-opaque=""
    >
      <div className="w-full max-w-[40rem] overflow-hidden rounded-xl border border-(--stroke-eclipse) bg-(--ui-chat-bubble-background) shadow-eclipse">
