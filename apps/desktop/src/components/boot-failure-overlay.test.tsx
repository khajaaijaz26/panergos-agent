...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Panergos gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.panergosDesktop
  Object.defineProperty(window, 'panergosDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'panergosDesktop', { configurable: true, value: original })
}
