...
const PANERGOS_REF_TYPES = WIRE_REFERENCE_KINDS
type PanergosRefType = (typeof PANERGOS_REF_TYPES)[number]
...
const PANERGOS_DIRECTIVE_RE = referenceRe()
...
export const panergosDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(PANERGOS_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...panergosDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Panergos directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.panergosDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.panergosDesktop?.readFileDataUrl(id)
