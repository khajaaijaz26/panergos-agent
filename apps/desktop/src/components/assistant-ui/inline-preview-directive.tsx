 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.panergos.send(prompt)` (or declarative
 * `data-panergos-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'panergos-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'panergos-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `panergos.send(prompt)`.
 ...
 *  wires `data-panergos-send` so declarative HTML works with zero script:
 *  `<button data-panergos-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.panergos={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-panergos-send]"):null;' +
    'if(el)send(el.getAttribute("data-panergos-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `panergos serve` — renders live.)
