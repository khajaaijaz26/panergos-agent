...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's PANERGOS_HOME/logs. On a
  ...
  // An expired or revoked OAuth grant (HTTP 401):
  ...
  const showSwitchProvider = surface != null && ['auth', 'endpoint', 'provider'].includes(surface.layer)

  const openLogs = useCallback(async () => {
    try {
      const root = await window.panergosDesktop?.logsRoot?.()
      ...
      const result = await window.panergosDesktop?.openDir?.(root)
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {window.panergosDesktop?.logsRoot && (
