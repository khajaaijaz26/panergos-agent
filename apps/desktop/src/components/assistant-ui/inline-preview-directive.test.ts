...
describe('frameSizeFromMessage', () => {
  const msg = (over: Record<string, unknown> = {}) => ({
    type: 'panergos-inline-preview-size',
    token: 'tok',
    height: 500,
    width: 300,
    ...over
...
describe('intentFromMessage', () => {
  const msg = (over: Record<string, unknown> = {}) => ({
    type: 'panergos-inline-preview-intent',
    token: 'tok',
  ...
  it('rejects wrong token, wrong type, empty, and hostile shapes', () => {
    ...
    expect(intentFromMessage(msg({ type: 'panergos-inline-preview-size' }), 'tok')).toBeNull()
...
describe('withInlineChrome intent wiring', () => {
  it('injects panergos.send and the data-panergos-send click bridge', () => {
    const framed = withInlineChrome('<html><body><h1>w</h1></body></html>', 'tok', '')

    expect(framed).toContain('window.panergos={send:send}')
    expect(framed).toContain('data-panergos-send')
    expect(framed).toContain('panergos-inline-preview-intent')
  })
})
