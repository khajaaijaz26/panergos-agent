...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Panergos Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Panergos docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Panergos Agent Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Panergos docs', url: 'https://example.com/docs' }
    ])
  })
})
...
describe('buildToolView browser_navigate title', () => {
  it('shows failed title when navigate returns success=false', () => {
    ...
    expect(view.status).toBe('error')
    expect(view.title).toBe('Failed to open docs.example.com/guide')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://docs.example.com/guide' },
        result: { success: true, url: 'https://docs.example.com/guide', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened docs.example.com/guide')
  })
})
