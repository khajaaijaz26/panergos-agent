          ...
          <span
            className={cn(
              ...
              copied
                ...
                : 'border-(--stroke-eclipse) text-foreground group-hover:border-(--ui-stroke-secondary)'
            )}
            key={i}
          >
            {ch}
...
function CodeBlock({ copied, onCopy, text }: { copied: boolean; onCopy: () => void; text: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-md border border-(--stroke-eclipse) px-3 py-2">
...
function ConfirmingModelPanel({
  ...
  // Pull pricing for the just-picked default so the confirm card matches the
  // model picker.
