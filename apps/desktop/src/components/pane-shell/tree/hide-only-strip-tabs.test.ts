...
function sessionsBotsTree() {
  ...
  registerPane('panergos-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'panergos-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('panergos-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('panergos-bots:pane')
    expect($hiddenStripTabs.get()).toContain('panergos-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('panergos-bots:pane')

    expect(setStripTabHidden('panergos-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('panergos-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('panergos-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('panergos-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('panergos.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('panergos-bots:pane')
    ...
    revealTreePane('panergos-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('panergos-bots:pane')
    expect(window.localStorage.getItem('panergos.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'panergos-bots:pane', title: 'panergos-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'panergos-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
