...
const USER_PLACED_KEY = 'panergos.desktop.userPlacedPanes.v1'
const LEGACY_HEAL_KEY = 'panergos.desktop.paneDockHeals.v1'
  ...
  children: [
    {
      ...
      children: [
        ...
        { type: 'group', id: 'g-bots', panes: ['panergos-bots:pane'], active: 'panergos-bots:pane' }
      ]
    },
...
describe('enforced dock (stacked Bots pane → sessions-zone tab, every boot)', () => {
  ...
  it('re-homes a stacked bots pane into the sessions tab strip, keeping sessions active', async () => {
    ...
    const group = model.findGroupOfPane(tree.$layoutTree.get()!, 'panergos-bots:pane')!

    expect(group.panes).toEqual(['sessions', 'panergos-bots:pane'])
    ...
    expect(JSON.stringify(persisted)).toContain('"panes":["sessions","panergos-bots:pane"]')
  })

  it('re-homes even a USER-PLACED pane — the owner invariant beats the drag record', async () => {
    window.localStorage.setItem(USER_PLACED_KEY, JSON.stringify(['panergos-bots:pane']))
  ...
  it('re-homes even when the retired heal token was already burned, and clears the stale ledger', async () => {
    window.localStorage.setItem(LEGACY_HEAL_KEY, JSON.stringify(['panergos-bots:pane:sessions-tab-v1']))
  ...
  it('is idempotent within a boot and does not fight an intra-session drag', async () => {
    ...
    expect(model.findGroupOfPane(tree.$layoutTree.get()!, 'panergos-bots:pane')!.panes).toContain('sessions')
    ...
    expect(group.panes).toEqual(['panergos-bots:pane'])
  })
  ...
  it('re-homes again on the NEXT boot after a drag persisted the stacked shape', async () => {
    ...
    const group = second.model.findGroupOfPane(second.tree.$layoutTree.get()!, 'panergos-bots:pane')!
      ...
      children: [
        {
          type: 'group',
          id: 'g-left',
          panes: ['sessions', 'panergos-bots:pane'],
          active: 'panergos-bots:pane',
          headerHidden: true
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
  ...
  it('re-homes an edge-enforced pane stranded in the sessions tab strip', async () => {
      ...
      children: [
        {
          type: 'group',
          id: 'g-sessions',
          panes: ['sessions', 'panergos-bots:pane', 'panergos-bots:routines'],
          active: 'panergos-bots:pane'
        },
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' }
      ]
    ...
    const botsGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'panergos-bots:pane')!
    const routinesGroup = model.findGroupOfPane(tree.$layoutTree.get()!, 'panergos-bots:routines')!

    expect(botsGroup.panes).toEqual(['sessions', 'panergos-bots:pane'])
    expect(botsGroup.active).toBe('panergos-bots:pane')
    expect(routinesGroup.panes).toEqual(['panergos-bots:routines'])
    ...
    const dockedRoutinesTree = {
      ...
      children: [
        ...
        { type: 'group', id: 'g-main', panes: ['workspace'], active: 'workspace' },
        {
          type: 'group',
          id: 'g-routines',
          panes: ['panergos-bots:routines'],
          active: 'panergos-bots:routines'
        }
      ]
    }
