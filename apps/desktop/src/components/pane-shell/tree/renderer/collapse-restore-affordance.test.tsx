...
describe('Sessions/Bots strip — #91223', () => {
  beforeEach(() => {
    ...
    registerPane('panergos-bots:pane', { hideOnly: true, placement: 'left' }, 'Bots')
  ...
  it('double-clicking the Sessions tab leaves the strip and both chips', () => {
    ...
    expect(tabEl('panergos-bots:pane')).toBeTruthy()
  ...
  it('double-clicking the Bots tab leaves the strip too', () => {
    ...
    tap(tabEl('panergos-bots:pane')!)
    doubleTap(tabEl('panergos-bots:pane')!)
...
describe('docked tool tile — collapsing keeps the restore chip', () => {
  beforeEach(() => {
    registerPane('workspace', { placement: 'main', uncloseable: true }, 'Chat')
    registerPane('panergos-bots:routines', { placement: 'main', width: '250px' }, 'Cronjobs')
    $layoutTree.set(
      split('row', [
        group(['workspace'], { active: 'workspace', id: 'g-main' }),
        group(['panergos-bots:routines'], { active: 'panergos-bots:routines', id: 'g-routines' })
      ])
    )
  })
  ...
  it('clicking the active tab does not collapse the tile or drop its label', () => {
    ...
    expect(tabEl('panergos-bots:routines')).toBeTruthy()
    tap(tabEl('panergos-bots:routines')!)

    expect(zoneAt(1).minimized).toBeFalsy()
    expect(tabEl('panergos-bots:routines')).toBeTruthy()
    expect(tabEl('panergos-bots:routines')?.textContent).toMatch(/cronjobs/i)
  })
  ...
  it('collapsing via the store does not dismiss the pane from the tree', () => {
    collapseTreePane('panergos-bots:routines')

    expect(zoneAt(1).minimized).toBe(true)
    expect($dismissedPanes.get().has('panergos-bots:routines')).toBe(false)
    expect(zoneAt(1).panes).toContain('panergos-bots:routines')
