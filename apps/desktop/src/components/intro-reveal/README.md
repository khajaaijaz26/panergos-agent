...
preference. Fonts use the system sans stack and bundled JetBrains Mono face.

Eligibility is `guidedOnboardingEnabled && !firstRunSkipped && !hasSeenIntroReveal()`.
Electron sets the flag from `PANERGOS_GUIDED_ONBOARDING=1` or `--guided-onboarding`.
The gate queues the guided chat on completion; the chat gate acknowledges the
provider setup notice as the cinematic starts. With the flag off, neither gate starts.
