...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Panergos doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in panergos_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `panergos model` uses — instead of
...
// A configured pairing service can mint a bot on the user's behalf; the desktop shows
...
export interface PanergosConfig {
  agent?: {
...
export type PanergosConfigRecord = Record<string, unknown>
...
// non-desktop) Panergos simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (panergos.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors panergos_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Panergos, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Panergos but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `panergos tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side. Optional for older backends. */
 ...
 *  not Panergos); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** panergos-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** panergos-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/panergos/update/check` — the backend's own update state.
