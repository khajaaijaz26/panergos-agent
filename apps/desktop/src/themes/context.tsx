...
import { ensureContrast, mix, parseColor } from '@panergos/shared/color'
...
import { BUILTIN_THEME_LIST, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, eclipseTheme } from './presets'
...
const SKIN_KEY = 'panergos-desktop-theme-v2'
const MODE_KEY = 'panergos-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'panergos-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'panergos-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'panergos-desktop-active-profile-v1'
...
// mix / ensureContrast live in @panergos/shared/color (shared with the TUI);
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? eclipseTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...eclipseTheme.typography, ...theme.typography }
  ...
  root.dataset.panergosTheme = skinName
  root.dataset.panergosMode = rendered
  ...
  window.panergosDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('panergos-boot-background', chromeBg)
    window.localStorage.setItem('panergos-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.panergosThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.panergosDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Panergos authoring/activating a skin from a
