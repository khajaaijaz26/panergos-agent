/**
 * Live skin sync from the Panergos backend.
 *
 * The backend resolves the active skin (built-in or `$PANERGOS_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Panergos
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { PanergosSkin } from '@panergos/shared/skin'
...
const BACKEND_THEMES_KEY = 'panergos-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: PanergosSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  // (eclipse), so we never register a converted theme under `default`. It is still a
  // valid apply TARGET though: a runtime switch back to `default` must repaint the
  // desktop to its own default (setTheme normalizes `default` → eclipse). So we only
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as PanergosSkin)
