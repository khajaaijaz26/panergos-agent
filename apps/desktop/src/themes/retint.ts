 ...
 * surfaces mixed from it. In `eclipse` that's 7 slots per mode —
 ...
 * the family with stable ratios. Those ratios live in ACCENT_MIX below and
 * keep soft surfaces coherent while the original seed remains an explicit
 * no-op.
 *
 * ONE color in, both modes out. A single hex can't serve both appearances
 * literally — a color can pass on a light sidebar but fail on a dark one.
 * The second mode therefore gets
 ...
 *
 * The neutral canvas is deliberately untouched: rotating it with the accent
 * turns every theme into a monochrome wash.
 */
...
function retintColors(colors: DesktopThemeColors, seed: string, isDark: boolean): DesktopThemeColors {
  ...
  // OKLab, not sRGB: gamma-space blending visibly drifts saturated hues.
 ...
 * this accent get in dark mode". Reapply that delta within a bounded range:
 * first-party palettes may use intentionally extreme light/dark seeds that
 * would push an unrelated custom hue out of sRGB and wash it nearly gray.
 *
...
function carryToDark(theme: DesktopTheme, seed: string): string {
  ...
  // ponytail: keep one global cap; add per-theme tuning only if a shipped skin
  // demonstrates that ±0.16 cannot preserve both contrast and chroma.
  const delta = Math.max(-0.16, Math.min(0.16, dark.l - light.l))
  const shifted = oklchToHex({ ...picked, l: Math.min(1, Math.max(0, picked.l + delta)) })
