...
describe('ingestBackendSkin', () => {
  ...
  it('seed does not paint, but a later same-name skin.changed applies (missed-activation recovery)', () => {
    ...
    // restarted). Panergos re-affirms it — `panergos config set display.skin neon`
    // or a `panergos skin set` recolor. That explicit event must repaint even
    ...
    ingestBackendSkin(skin('forest'), { apply: true }) // Panergos authored a new skin
    expect($pendingSkinApply.get()).toBe('forest')
  })
  ...
  it('applies a runtime switch back to default (repaints the desktop to its own default)', () => {
    ...
    ingestBackendSkin(skin('default'), { apply: true }) // Panergos switched back to default
  ...
  it('hydrates the registry from storage on load, so a persisted pick resolves before the gateway connects', async () => {
    ingestBackendSkin(skin('neon'), { apply: false })
    expect(JSON.parse(window.localStorage.getItem('panergos-desktop-backend-themes-v1') ?? '{}').neon?.name).toBe('neon')
  ...
  it('drops junk and built-in names from the cached registry', async () => {
    ...
    window.localStorage.setItem('panergos-desktop-backend-themes-v1', JSON.stringify({ mono, bad: { name: 'bad' } }))
