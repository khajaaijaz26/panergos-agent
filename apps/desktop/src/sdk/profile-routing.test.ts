...
const { deleteProfile, panergosApi } = await import('@/panergos')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its PANERGOS_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { panergosDesktop: unknown }).panergosDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(panergosApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(panergosApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(panergosApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
