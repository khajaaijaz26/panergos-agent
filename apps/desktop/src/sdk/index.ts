/**
 * @panergos/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__PANERGOS_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, panergosApi, type PanergosGateway } from '@/panergos'
import {
  $gateway,
...
import type { PaginatedSessions, UsageStats } from '@/types/panergos'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Panergos profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.panergosDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.panergosDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and PanergosGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (panergos-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.panergosDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Panergos Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.panergosDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Panergos Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Panergos Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (panergos-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Panergos Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return panergosApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return panergosApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Panergos gateway unavailable')
    }
...
/** One glyph per gateway kind — device, terminal, network. The statusbar
...
/** The oversized display lettering an empty chat is titled with — core writes
 *  "PANERGOS AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { PanergosOpenTarget } from '@/lib/panergos-open-target'
...
/** Compact labels for the reasoning levels exported from @panergos/shared, so a
...
export type { PanergosGateway } from '@/panergos'
...
export type { StatusResponse } from '@/types/panergos'
...
export type { GatewayEvent as RpcEvent } from '@panergos/shared'
/** THE compact-number formatter — every user-facing count/token figure goes
 *  through here (1230 → "1.2k", 1_500_000 → "1.5M"). Don't hand-roll `/1000`. */
export { compactNumber } from '@panergos/shared'
/** Panergos' reasoning levels, so a plugin surfacing a thinking depth uses the
