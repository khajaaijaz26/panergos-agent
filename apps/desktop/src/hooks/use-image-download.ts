...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'panergos:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.panergosDesktop?.saveImageFromUrl) {
        if (await window.panergosDesktop.saveImageFromUrl(src)) {
