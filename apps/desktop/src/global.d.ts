import type { GatewayWsUrlResult } from '@panergos/shared'
import type { TranslucencyState } from '@panergos/shared/translucency'

import type { PanergosNotification } from '../electron/notification-types'
...
declare global {
  interface Window {
    panergosDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`panergos --tui
      ...
      profile: {
        ...
        // backend under the new PANERGOS_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: PanergosApiRequest) => Promise<T>
      notify: (payload: PanergosNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<PanergosReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<PanergosReadFileTextResult>
      selectPaths: (options?: PanergosSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<PanergosPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<PanergosPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<PanergosPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: PanergosActiveWork) => void
      setTitleBarTheme?: (payload: PanergosTitleBarTheme) => void
      ...
      /** Launch flag: skip the first-run film (PANERGOS_SKIP_INTRO=1 or
       *  --skip-intro) so a fresh PANERGOS_HOME lands on the guided chat. */
      skipIntro?: boolean
      ...
      readDir: (path: string) => Promise<PanergosReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<PANERGOS_HOME>/desktop-plugins),
      ...
      /** LOCAL `<PANERGOS_HOME>/logs` (profile-aware) — error card "Open Logs". */
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<PanergosGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<PanergosGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<PanergosGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<PanergosRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: PanergosReviewScope, baseRef?: null | string) => Promise<PanergosReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: PanergosReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<PanergosReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<PanergosRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<PanergosPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: PanergosTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<PanergosTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: PanergosWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: PanergosPreviewFileChanged) => void) => () => void
...
export interface PanergosTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface PanergosTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `panergos update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  panergosRoot?: string
...
export interface PanergosConnection {
  baseUrl: string
  darwinMajor?: number
  isFullscreen: boolean
  // The live, resolved connection mode.
  mode?: 'local' | 'remote'
  authMode?: 'oauth' | 'token'
  remoteHost?: string
  remoteIdentity?: string
  remoteKind?: 'ssh' | 'url'
  remotePanergosVersion?: string
...
export interface PanergosTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface PanergosActiveWork {
  count: number
  titles: string[]
}

export interface PanergosWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopRegistryConnection {
  ...
  // Names of the stored extra gateway headers (access proxies such as Cloudflare Access);
...
export interface PanergosApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface PanergosPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface PanergosReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface PanergosPreviewWatch {
  id: string
  path: string
}
...
export interface PanergosGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface PanergosGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface PanergosGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface PanergosRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface PanergosRepoStatus {
  branch: null | string
...
// turn began.
export type PanergosReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface PanergosReviewFile {
  path: string
  added: number
  removed: number
...
export interface PanergosReviewList {
  files: PanergosReviewFile[]
...
export interface PanergosReviewPr {
  url: string
  state: string
  number: number
}
...
export interface PanergosBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface PanergosRepoPullRequests {
  ghReady: boolean
  prs: PanergosBranchPullRequest[]
}
...
export interface PanergosPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface PanergosReviewShipInfo {
  ghReady: boolean
  pr: PanergosReviewPr | null
}

export interface PanergosReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface PanergosReadDirResult {
  entries: PanergosReadDirEntry[]
  error?: string
}

export interface PanergosPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface PanergosSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
