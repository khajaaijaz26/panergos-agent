import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@panergos/shared'
...
import { getProfiles, panergosApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/panergos'
...
import type { ProfileInfo } from '@/types/panergos'
...
// /api/profiles/active `current`). "default" is the root ~/.panergos. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'panergos.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'panergos.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await panergosApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new PANERGOS_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.panergosDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<PanergosConnection | null> {
  const getConnection = window.panergosDesktop?.getConnection
...
export async function ensureGatewayProfile(profile: string | null | undefined): Promise<void> {
  ...
  // #89206 split-brain — observed live as atom 'default' over a panergos-setup
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<PanergosConnection | null> {
  const getConnectionFor = window.panergosDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'panergos.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.panergosDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.panergosDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.panergos) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.panergosDesktop?.touchBackend?.(target).catch(() => undefined)
}
