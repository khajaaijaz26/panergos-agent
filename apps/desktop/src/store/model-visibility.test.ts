...
describe('model visibility', () => {
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    ...
    const visible = effectiveVisibleKeys(stored, [
      provider('acme', ['example-model-a', 'example-model-b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('acme', 'example-model-a'))).toBe(false)
    expect(visible.has(modelVisibilityKey('acme', 'example-model-b'))).toBe(false)
  ...
  it('restores model when toggling on after hiding all', () => {
    ...
    afterToggle.add(modelVisibilityKey('acme', 'example-model-a'))
    ...
    expect(visible.has(modelVisibilityKey('acme', 'example-model-a'))).toBe(true)
    expect(visible.has(modelVisibilityKey('acme', 'example-model-b'))).toBe(false)
  })
  ...
  it('resolveVisibleKeys preserves sentinels that effectiveVisibleKeys strips', () => {
    const stored = new Set([emptyProviderSentinelKey('acme')])
    const providers = [provider('acme', ['model-x', 'model-y']), provider('ollama', ['qwen3:latest'])]

    const resolved = resolveVisibleKeys(stored, providers)
    expect(resolved.has(emptyProviderSentinelKey('acme'))).toBe(true)
    expect(resolved.has(modelVisibilityKey('acme', 'model-x'))).toBe(false)
...
describe('toggleModelVisibility', () => {
  const providers = [provider('openai', ['gpt-a', 'gpt-b']), provider('acme', ['model-x', 'model-y'])]
  ...
  it('keeps a hidden provider hidden when a different provider is toggled (regression for #43485)', () => {
    ...
    let stored: Set<string> | null = null
    stored = apply(stored, 'acme', 'model-x')
    stored = apply(stored, 'acme', 'model-y')
    ...
    expect(visible.has(modelVisibilityKey('acme', 'model-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('acme', 'model-y'))).toBe(false)
  })
  ...
  it('toggling from an empty (non-null) stored set adds the model without expanding defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('acme', 'model-x'))).toBe(false)
  })
  ...
  it('toggling off one default model from null stored keeps the rest of the curated defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('acme', 'model-x'))).toBe(true)
