...
const checkPanergosUpdateSpy = vi.fn()
const updatePanergosSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/panergos', () => ({
  checkPanergosUpdate: (...args: unknown[]) => checkPanergosUpdateSpy(...args),
  updatePanergos: (...args: unknown[]) => updatePanergosSpy(...args),
...
describe('checkBackendUpdates', () => {
  beforeEach(() => {
    storage.clear()
    notifySpy.mockClear()
    checkPanergosUpdateSpy.mockReset()
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkPanergosUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'panergos update',
      message: null,
    ...
    expect(checkPanergosUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkPanergosUpdateSpy).not.toHaveBeenCalled()
  })
})

// The ⌘K "Update Panergos" row. It used to call applyBackendUpdate() flat, which
...
describe('requestActiveUpdate', () => {
  ...
  beforeEach(() => {
    ...
    updatePanergosSpy.mockReset().mockResolvedValue({ ok: true, name: 'update' })
    checkPanergosUpdateSpy.mockReset().mockResolvedValue({
      install_method: 'git',
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      panergosDesktop: { updates: { apply: applyClientMock, check: checkClientMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('applies the CLIENT update in local mode, never the backend', async () => {
    ...
    expect(updatePanergosSpy).not.toHaveBeenCalled()
  ...
  it('applies the BACKEND update in remote mode', async () => {
    ...
    requestActiveUpdate()
    await vi.waitFor(() => expect(updatePanergosSpy).toHaveBeenCalled())
...
describe('explicit update targets', () => {
  ...
  // for updating THIS app — routes here via `panergos:open-updates`.
  ...
  it('still defaults to the connected machine when no target is named', async () => {
    ...
    expect($updateOverlayTarget.get()).toBe('backend')
    await vi.waitFor(() => expect(checkPanergosUpdateSpy).toHaveBeenCalled())
  ...
  it('keeps the everything-flow for the generic, target-less apply', async () => {
    ...
    await vi.waitFor(() => expect(updatePanergosSpy).toHaveBeenCalled(), { timeout: 5000 })
  })
...
describe('applyEverythingUpdate', () => {
  ...
  it('remote mode: updates the backend, then the still-behind client — the stale-GUI gap', async () => {
    ...
    expect(updatePanergosSpy).toHaveBeenCalledTimes(1)
  ...
  it('a failed backend leg does not strand the fan-out or the client', async () => {
    setRemote(true)
    $mockConnectionsRegistry.set(registryOf(['local', 'vps']))
    updatePanergosSpy.mockRejectedValue(new Error('backend gone'))
...
describe('applyUpdates terminal state', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      panergosDesktop: { updates: { apply: applyMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('keeps the manual command state for CLI installs with no staged updater', async () => {
    applyMock.mockResolvedValue({ ok: true, manual: true, command: 'panergos update' })

    await applyUpdates()

    expect($updateApply.get().stage).toBe('manual')
    expect($updateApply.get().command).toBe('panergos update')
    expect($updateOverlayOpen.get()).toBe(true)
    expect(notifySpy).not.toHaveBeenCalled()
  })
  ...
  it('lands on a closeable manual-restart state when the rebuilt sandbox blocks auto-relaunch', async () => {
    ...
    applyMock.mockResolvedValue({
      ...
      manualRestart: true,
      sandboxBlocked: true,
      message: 'Backend updated. Quit and reopen Panergos to finish.'
    })
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    ...
    updatePanergosSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: [`=== panergos-update completed ${actionId} ===`],
      name: 'update',
      pid: null,
      running: false
    })
  ...
  it('keeps waiting past the old 45-second cutoff while the update action is running', async () => {
    ...
    updatePanergosSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'panergos-update', pid: 1 })

    for (let attempt = 0; attempt < 31; attempt += 1) {
      getActionStatusSpy.mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== panergos-update started now ===', `step ${attempt}`],
  ...
  it('treats a successful no-op as complete without waiting for a restart', async () => {
    updatePanergosSpy.mockResolvedValue({ ok: true, name: 'panergos-update', pid: 1 })
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['stale output from another run', '=== panergos-update started now ===', '✓ Already up to date!'],
      name: 'panergos-update',
      pid: 1,
      running: false
    })
  ...
  it('treats a successful dependency repair as complete without waiting for a restart', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== panergos-update started now ===', '✓ Dependencies repaired!', '✓ Update complete!'],
      name: 'panergos-update',
      pid: 1,
      running: false
    })
  ...
  it('waits for current-action completion proof after the backend restarts', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== panergos-update completed ${'c'.repeat(32)} ===`],
        name: 'panergos-update',
        pid: null,
        running: false
      })
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== panergos-update completed ${actionId} ===`],
        name: 'panergos-update',
        pid: null,
        running: false
      })
  ...
  it('accepts its terminal receipt when a verbose update pushes the start marker out of the log tail', async () => {
    ...
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: ['final build output', 'Update complete!', `=== panergos-update completed ${actionId} ===`],
      name: 'panergos-update',
      pid: null,
      running: false
    })
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(getActionStatusSpy).toHaveBeenCalledWith('panergos-update', 2000)
  })

  it('proves a pre-action-ID backend reached its requested commit after restart', async () => {
    $backendUpdateStatus.set({
      behind: 2,
      commits: [{ at: 1, author: 'Acme', sha: 'requested-target', summary: 'target' }],
      fetchedAt: 1,
      supported: true,
    ...
    checkPanergosUpdateSpy
      ...
      .mockResolvedValueOnce({
        behind: 1,
        can_apply: true,
        commits: [{ at: 2, author: 'Acme', sha: 'newer-commit', summary: 'newer' }],
        current_version: '0.18.2',
        install_method: 'git',
        message: null,
        update_available: true,
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkPanergosUpdateSpy).toHaveBeenCalledTimes(2)
  })
  ...
  it('proves a fast pre-action-ID packaged update by its changed version', async () => {
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkPanergosUpdateSpy).toHaveBeenCalledWith(true)
  })
  ...
  it('resumes action polling after a transient status failure', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== panergos-update started now ===', 'still running'],
        name: 'panergos-update',
        pid: 1,
        running: true
      })
      .mockResolvedValueOnce({
        exit_code: 0,
        lines: ['=== panergos-update started now ===', 'Update complete!'],
        name: 'panergos-update',
        pid: 1,
        running: false
      })
  ...
  it('restores the fixed action deadline after reconnecting', async () => {
    updatePanergosSpy.mockResolvedValue({ action_id: 'a'.repeat(32), ok: true, name: 'panergos-update', pid: 1 })
  ...
  it('shares one in-flight update between concurrent apply requests', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== panergos-update started now ===', '✓ Already up to date!'],
      name: 'panergos-update',
      pid: 1,
      running: false
    })
  ...
  it('fails immediately when the update action exits nonzero', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 1,
      lines: ['=== panergos-update started now ===', 'update failed'],
      name: 'panergos-update',
      pid: 1,
      running: false
    })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updatePanergosSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkPanergosUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
...
describe('startUpdatePoller', () => {
  ...
  beforeEach(() => {
    ...
    $updateStatus.set(null)
    ;(globalThis as unknown as { window: unknown }).window = {
      panergosDesktop: { updates: { check: checkMock, onProgress: onProgressMock } },
