...
import { checkPanergosUpdate, getActionStatus, updatePanergos } from '@/panergos'
import { $connectionsRegistry, refreshConnectionsRegistry } from '@/store/connections'
import { reconnectGateway } from '@/store/gateway-reconnect'
import { dismissNotification, notify } from '@/store/notifications'
import { onboardingSurfaceActive } from '@/store/onboarding-presence'
import { $connection } from '@/store/session'
import type { BackendUpdateCheckResponse } from '@/types/panergos'
...
const UPDATE_TOAST_SNOOZE_KEY = 'panergos:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'panergos:backend-skew-toast-snooze-until'
...
const INSTALL_METHOD_TOAST_SNOOZE_KEY = 'panergos:install-method-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: {
      label: translateNow('notifications.updatePanergos'),
      onClick: () => {
        snoozeSkewToast()
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.panergosDesktop?.getVersion?.()
...
export async function checkBackendUpdates({
  ...
  try {
    const status = mapBackendCheck(await checkPanergosUpdate(force))
...
export async function checkUpdates({ force = false }: UpdateCheckOptions = {}): Promise<DesktopUpdateStatus | null> {
  const bridge = window.panergosDesktop?.updates
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `panergos update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'panergos update',
        command: result.command ?? 'panergos update'
      })
...
function completedAfterRestart(
  ...
  return !!actionId && status.lines.some(line => line === `=== panergos-update completed ${actionId} ===`)
}
...
async function runBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    const started = await updatePanergos()
    ...
    if (!started.ok) {
      ...
      const command = (started as { update_command?: string }).update_command || 'panergos update'
    ...
    while (Date.now() < deadline) {
      ...
      if (!started.action_id && last.exit_code === null) {
        try {
          const status = await checkPanergosUpdate(true)
...
const FOCUS_RECHECK_KEY = 'panergos.updates.last-passive-check'
