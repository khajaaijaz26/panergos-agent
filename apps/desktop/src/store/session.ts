import type { ConnectionState } from '@panergos/shared'
...
import type { SessionInfo, UsageStats } from '@/types/panergos'
...
const COMPOSER_MODEL_KEY = 'panergos.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'panergos.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'panergos.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'panergos.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'panergos.desktop.composer.fast'
...
function composerScopeForConnection(connection: PanergosConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'panergos.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'panergos.desktop.lastRoute'
...
function workspaceCwdKey(connection: PanergosConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.panergosDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.panergosDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<PanergosConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'panergos.desktop.sessionOwnerHints.v1'
 ...
 * PanergosConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<PanergosConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by usePanergosConfig.
