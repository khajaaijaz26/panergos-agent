...
describe('collidesWithWorkspace', () => {
  it('suppresses a skill named exactly like the cwd folder', () => {
    expect(collidesWithWorkspace('panergos-agent', '/Users/b/www/panergos-agent')).toBe(true)
  })

  it('suppresses inside worktree-suffixed folders too', () => {
    expect(collidesWithWorkspace('panergos-agent', '/Users/b/www/panergos-agent-suggest')).toBe(true)
  })
  ...
  it('does not suppress on substring-only overlap', () => {
    ...
    expect(collidesWithWorkspace('clean', '/Users/b/www/panergos-agent')).toBe(false)
  })

  it('never collides when detached (empty cwd)', () => {
    expect(collidesWithWorkspace('panergos-agent', '')).toBe(false)
  })
})
...
describe('skillTouchedInMessages', () => {
  ...
  it('matches qualified skill names (category/name, plugin:name)', () => {
    expect(
      skillTouchedInMessages('panergos-agent-dev', [toolCall('skill_view', { name: 'github/panergos-agent-dev' })])
    ).toBe(true)
    expect(
