...
import { getSkills } from '@/panergos'
 ...
 *  Working in `~/www/panergos-agent`, the draft says "panergos-agent" constantly
 *  — a "Use skill: panergos-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`panergos-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/panergos-agent-dev`, `plugin:skill`). */
