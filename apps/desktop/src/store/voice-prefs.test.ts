...
vi.mock('@/panergos', () => ({
  getPanergosConfigRecord: vi.fn(async () => ({})),
  savePanergosConfig: vi.fn(async () => undefined)
}))
...
it('keeps the desktop toggle local across config refreshes', async () => {
  for (const fails of [false, true]) {
    for (const enabled of [false, true]) {
      ...
      vi.mocked(savePanergosConfig).mockClear()
      ...
      try {
        ...
        expect(savePanergosConfig).not.toHaveBeenCalled()
        expect(localStorage.getItem('panergos.desktop.autoSpeakReplies')).toBe(fails ? null : String(enabled))
      } finally {
        write.mockRestore()
      }
    }
  ...
  for (const fails of [false, true]) {
    for (const enabled of [false, true]) {
      ...
      try {
        ...
        expect(localStorage.getItem('panergos.desktop.autoSpeakReplies')).toBeNull()
        prefs.applyAutoSpeakFromConfig({ voice: { auto_tts: enabled } })
        prefs.applyAutoSpeakFromConfig({ voice: { auto_tts: !enabled } })
        expect(prefs.$autoSpeakReplies.get()).toBe(enabled)
        expect(localStorage.getItem('panergos.desktop.autoSpeakReplies')).toBe(fails ? null : String(enabled))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye panergos', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye panergos')
  })
