...
const { getProfiles } = await import('@/panergos')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/panergos/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<PanergosConnection> = {}): PanergosConnection =>
  ({ baseUrl: 'https://agent.example', mode: 'remote', profile: 'vps-remote', ...over }) as PanergosConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { panergosDesktop: { getConnection } })
