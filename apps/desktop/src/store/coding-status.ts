...
import type { PanergosGitWorktree, PanergosRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: PanergosGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, PanergosRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, PanergosGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<PanergosRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<PanergosGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<PanergosRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<PanergosGitWorktree[]>>()
const $noRepoStatus = atom<PanergosRepoStatus | null>(null)
const $noWorktrees = atom<PanergosGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<PanergosRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<PanergosGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: PanergosRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: PanergosGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<PanergosRepoStatus | null>
  seq: number
  target: string
}
