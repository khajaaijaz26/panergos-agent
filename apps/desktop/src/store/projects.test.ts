...
const panergos = await import('@/panergos')
const getPanergosConfig = vi.mocked(panergos.getPanergosConfig)
...
describe('project scope', () => {
  ...
  it('persists the scope to localStorage', () => {
    enterProject('p_abc')
    expect(window.localStorage.getItem('panergos.desktop.projectScope')).toBe('p_abc')
  })
})
  ...
  it('names the stale-backend remedy when a remote gateway lacks the worktree route', async () => {
    ...
    desktopGit.mockReturnValue({
      worktreeAdd: vi.fn(async () => {
        throw new Error(
          'Expected JSON from https://vps/api/git/worktree/add but got HTML (status 404). The endpoint is likely missing on the Panergos backend.'
        )
      })
    } as never)
...
describe('createProject', () => {
  ...
  it.each(['default', 'coder'])('creates in the active %s profile without leaving All profiles', async profile => {
    const created = { folders: [], id: 'p_new', name: 'Panergos Agent', primary_path: '/srv/panergos' }
    ...
    vi.mocked(panergos.panergosApi).mockResolvedValue({ projects: [tree], active_id: created.id })
    $activeGatewayProfile.set(profile)
    setShowAllProfiles(true)

    await expect(createProject({ folders: ['/srv/panergos'], name: created.name, use: true })).resolves.toEqual(created)
    ...
    expect(panergos.panergosApi).toHaveBeenCalledWith(
  ...
  it('does not retarget a project create when the profile changes during reconnect', async () => {
    ...
    const pending = createProject({ folders: ['/srv/panergos'], name: 'Panergos Agent' })
    const rejection = expect(pending).rejects.toThrow('Active Panergos profile changed while connecting')
...
describe('repository discovery policy', () => {
  ...
  it('records disabled policy without invoking the filesystem scanner', async () => {
    ...
    getPanergosConfig.mockResolvedValue({
      desktop: {
  ...
  it('passes custom roots and exclusions to Electron and records on the origin gateway', async () => {
    ...
    expect(getPanergosConfig).toHaveBeenCalledWith('default')
  ...
  it('does not scan the local filesystem for remote connections but still refreshes the project tree', async () => {
    ...
    expect(scanRepos).not.toHaveBeenCalled()
    expect(getPanergosConfig).not.toHaveBeenCalled()
    ...
    // `scan: true`) — repos with zero Panergos sessions must still surface —
