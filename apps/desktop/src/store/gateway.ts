...
import type { PanergosConnection } from '@/global'
import { isTimeoutError, RECONNECT_ATTEMPT_TIMEOUT_MS, withTimeout } from '@/lib/with-timeout'
import { PanergosGateway, setApiRequestConnection } from '@/panergos'
...
function dialProfile(
  desktop: NonNullable<typeof window.panergosDesktop>,
  profile: string,
...
const isOpen = (gateway: PanergosGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: PanergosConnection) => void
  /**
...
interface Secondary {
  ...
  connection: PanergosConnection | null
  gateway: PanergosGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: PanergosGateway | null
  ...
  $gateway: ReturnType<typeof atom<PanergosGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('panergos.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<PanergosGateway | null>(null),
...
export function setPrimaryGateway(gateway: PanergosGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<PanergosConnection, 'connectionId'> | null): void {
...
export function activeGateway(): PanergosGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the panergos module (null
...
function publishActiveConnection(connection: PanergosConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new PanergosGateway()
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: PanergosGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Panergos gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.panergosDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Panergos Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<PanergosGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Panergos gateway is not connected" on the Sessions `+` action (#88880).
