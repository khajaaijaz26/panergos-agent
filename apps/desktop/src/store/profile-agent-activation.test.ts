...
const agentConn = (over: Partial<PanergosConnection> = {}): PanergosConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as PanergosConnection

const localConn = (over: Partial<PanergosConnection> = {}): PanergosConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as PanergosConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<PanergosConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<PanergosConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { panergosDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<PanergosConnection>(() => undefined))
