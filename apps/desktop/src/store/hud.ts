 ...
 * composer with the reply scrolling above it, so Panergos can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.panergosDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.panergosDesktop?.hud
...
export function resetHudLayout(): void {
  void window.panergosDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.panergosDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.panergosDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
