...
export async function openFirstBuildLinks(storedId: string, part: FirstBuildConnectorPart, deps: OpenLinksDeps) {
  ...
  const rows = connectionRows(part.args, part.result).map((seed): FirstBuildConnectorRow => {
    ...
    const connectUrl = ['initiated', 'setup_required'].includes(String(entry?.status))
      ? connectorAuthorizationUrl(entry?.setup_url ?? entry?.connect_url)
      : null
  ...
  $firstBuildConnections.setKey(storedId, {
    ...
    rows,
    started: previous?.started ?? readKey(`panergos.onboarding.started.v1.${storedId}`) === '1'
  })

  const links = entries.flatMap(entry => {
    const url = ['initiated', 'setup_required'].includes(String(entry.status))
      ? connectorAuthorizationUrl(entry.setup_url ?? entry.connect_url)
      : null
  ...
  const key = `panergos.onboarding.links-opened.v1.${part.toolCallId}`
...
export function startFirstBuild(storedId: string, submit: (text: string) => boolean): void {
  const state = $firstBuildConnections.get()[storedId]
  const key = `panergos.onboarding.started.v1.${storedId}`
