...
vi.mock('@/panergos', () => ({
  getPanergosConfigRecord: mocks.getPanergosConfigRecord,
...
afterEach(() => {
  ...
  mocks.getPanergosConfigRecord.mockReset()
...
it('shows the toast with Sign in + Disable, then stays quiet for a day and re-nudges after it', async () => {
  ...
  mocks.getPanergosConfigRecord.mockResolvedValue(servers)
...
it('coalesces reconnects during a sweep into one fresh follow-up sweep', async () => {
  ...
  mocks.getPanergosConfigRecord.mockReturnValueOnce(first).mockResolvedValue({ mcp_servers: {} })

  startMcpHealthChecker()
  mocks.gatewayState.set('open')
  await flush()
  expect(mocks.getPanergosConfigRecord).toHaveBeenCalledTimes(1)
  ...
  await flush()
  await flush()
  expect(mocks.getPanergosConfigRecord).toHaveBeenCalledTimes(2)
})
