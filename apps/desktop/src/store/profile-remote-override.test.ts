...
describe('remoteHostLabel', () => {
  it('keeps a non-default port and drops default ones', () => {
    expect(remoteHostLabel('https://panergos.example.com:8443/x')).toBe('panergos.example.com:8443')
    expect(remoteHostLabel('https://panergos.example.com:443')).toBe('panergos.example.com')
    expect(remoteHostLabel('http://panergos.example.com:80')).toBe('panergos.example.com')
  })
...
describe('refreshProfileRemoteOverrides', () => {
  it('publishes only profiles whose scope resolves to a remote override', async () => {
    getConnectionConfig.mockImplementation(async (profile: string) => {
      ...
      if (profile === 'hosted') {
        return { mode: 'remote', remoteUrl: 'https://hosted.example.com:8443' }
      }

      return { mode: 'local', remoteUrl: '' }
    })

    await refreshProfileRemoteOverrides(['work', 'hosted', 'home'])

    expect($profileRemoteOverrides.get()).toEqual({
      work: { host: 'work.example.com', url: 'https://work.example.com' },
      hosted: { host: 'hosted.example.com:8443', url: 'https://hosted.example.com:8443' }
    })
  })
