 ...
 *
 * This store is the only writer of panergos-intro-reveal-seen-v1. First-run eligibility is guest onboarding
...
const SEEN_KEY = 'panergos-intro-reveal-seen-v1'
...
export function isIntroRevealEnabled(): boolean {
  return isOnboardingEnabled() && window.panergosDesktop?.skipIntro !== true
}
...
export function startIntroReveal(): void {
  ...
  void window.panergosDesktop?.introReveal?.open({ hideMain: true }).catch(finishIntroReveal)
}
...
export function finishIntroReveal(): void {
  ...
  void window.panergosDesktop?.introReveal?.close({ showMain: true }).catch(() => undefined)
}

export function installIntroRevealBridgeListeners(): () => void {
  const bridge = window.panergosDesktop?.introReveal
