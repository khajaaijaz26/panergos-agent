 ...
 *
 * The archive is the CLI's own `panergos profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/panergos'
...
import type { ProfileDesktopOverlay } from '@/types/panergos'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Panergos profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.panergosDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.panergosDesktop?.selectPaths?.({
