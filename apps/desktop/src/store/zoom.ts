...
// Mirror DEFAULT_ZOOM_LEVEL (110%) so Appearance doesn't flash 100% before
...
export function setZoomPercent(percent: number): void {
  window.panergosDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.panergosDesktop?.zoom) {
  void window.panergosDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.panergosDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
