...
import type { PanergosReviewFile, PanergosReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Panergos' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'panergos.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'panergos.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'panergos.desktop.reviewTreeMode'
const SELECTED_KEY = 'panergos.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<PanergosReviewFile[]>([])
...
export const $reviewShipInfo = atom<PanergosReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['panergosDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: PanergosReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly PanergosReviewFile[], path: string): PanergosReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.panergosDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.panergosDesktop?.openExternal?.(url)
    }
