 ...
 *
 * Local models ship on main behind `--local` (either `panergos desktop --local`
 * or the flag on Panergos.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.panergosDesktop?.localModelsEnabled === true
)
