 ...
 * app's bounds and stays visible while Panergos is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'panergos.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'panergos.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.panergosDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.panergosDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.panergosDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.panergosDesktop?.petOverlay?.close()
}
