...
import { type PanergosOpenTarget, resolvePanergosOpenPath } from '@/lib/panergos-open-target'
...
export type { PanergosOpenTarget }
...
const STORAGE_KEY = 'panergos:native-notifications'
...
// "Backgrounded" = the user isn't on Panergos. `document.hidden` only flips when
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `panergos://index-network/intent/1`.
   */
  activate?: string
...
export function dispatchNativeNotification(input: NativeNotificationInput): boolean {
  ...
  void window.panergosDesktop?.notify({
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `panergos://index-network/intent/1`). */
  activate?: PanergosOpenTarget
 ...
 *  user is away from Panergos — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolvePanergosOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolvePanergosOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.panergosDesktop
