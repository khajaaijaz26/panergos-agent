 ...
 * `@panergos/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-panergos-glass]`
 * block in styles.css).
 */
...
} from '@panergos/shared/translucency'
 ...
 * sets `panergosDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.panergosDesktop?.glassSupported === 'boolean'
    ? window.panergosDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.panergosDesktop?.translucencySupported === 'boolean'
    ? window.panergosDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-panergos-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-panergos-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-panergos-glass-on', glassLive)
  root.toggleAttribute('data-panergos-glass', glassOn)
  root.toggleAttribute('data-panergos-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-panergos-glass-scope', scope)
  } else {
    root.removeAttribute('data-panergos-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.panergosDesktop?.setTranslucency?.(state)
  })
