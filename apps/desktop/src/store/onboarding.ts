...
import type { OAuthProvider, OAuthStartResponse } from '@/types/panergos'
...
const CONFIGURED_CACHE_KEY = 'panergos-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'panergos-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Panergos is ready', message: `${provider} connected.` })
}
...
async function fetchProviderDefaultModel(
  ...
  // Prefer the backend's recommended default and fall back to the first
  // curated model if the endpoint cannot resolve one.
  ...
  try {
    ...
    if (recommended.model && models.map(String).includes(recommended.model)) {
      ...
      // Recommended model is not in the curated options list; trust it anyway.
      defaultModel = recommended.model
    }
  } catch {
...
async function completeWithModelConfirm(
  ...
  if (defaults) {
    ...
    try {
      ...
      onFail(error instanceof Error ? error.message : 'Panergos could not save the selected model.')
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Panergos still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Panergos still cannot resolve a usable provider.'
}

/** Re-read the OAuth provider list into the onboarding cache. Exported so a
 *  flow that changes a provider's auth state outside onboarding (an account
...
export function requestDesktopOnboarding(reason = DEFAULT_ONBOARDING_REASON) {
  // Do not cover the guided first-launch conversation with the provider picker.
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Panergos Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
async function openSignInUrl(url: string) {
  if (window.panergosDesktop?.openExternal) {
    try {
      await window.panergosDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Panergos still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Panergos still cannot reach ${url}.` }
    }
