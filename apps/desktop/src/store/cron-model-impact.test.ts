...
import type { ModelAssignmentResponse } from '@/types/panergos'
...
describe('setMainModelAssignment', () => {
  it('shows one consumer warning and routes via a read-only review action', async () => {
    ...
    await setMainModelAssignment({ provider: 'acme', model: 'new/model' })
  ...
  it('keeps an existing warning for an older backend but clears it on explicit zero impact', async () => {
    ...
    await setMainModelAssignment({ provider: 'acme', model: 'one' })
    expect($notifications.get()).toHaveLength(1)

    setModelAssignment.mockResolvedValueOnce(response(undefined))
    await setMainModelAssignment({ provider: 'acme', model: 'two' })
    expect($notifications.get()).toHaveLength(1)
    ...
    await setMainModelAssignment({ provider: 'acme', model: 'three' })
    expect($notifications.get()).toEqual([])
  })
  ...
  it('publishes only the latest same-profile assignment when responses reverse', async () => {
    ...
    const firstCall = setMainModelAssignment({ provider: 'acme', model: 'first' })
    const secondCall = setMainModelAssignment({ provider: 'acme', model: 'second' })
  ...
  it('invalidates pending responses and action closures on profile or connection changes', async () => {
    ...
    const call = setMainModelAssignment({ provider: 'acme', model: 'pending' })
    ...
    await setMainModelAssignment({ provider: 'acme', model: 'current' })
