...
import type { PanergosGitBaseBranch, PanergosGitBranch } from '@/global'
...
import { getPanergosConfig, panergosApi, type PanergosGateway } from '@/panergos'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/panergos'
...
const PROJECT_SCOPE_KEY = 'panergos.desktop.projectScope'
...
async function activeProjectsContext(profile = projectProfile()): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== normalizeProfileKey($activeGatewayProfile.get())) {
    throw new Error('Active Panergos profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await panergosApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<PanergosGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<PanergosGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Panergos
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getPanergosConfig(context.profile))
...
// repo, returning where Panergos should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<PanergosGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<PanergosGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.panergosDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.panergosDesktop?.writeClipboard?.(path)
  }
}
