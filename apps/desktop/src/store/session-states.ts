...
import { type GatewayEvent, LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@panergos/shared'
...
const TILES_KEY = 'panergos.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'panergos.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'panergos.desktop.botChatSessions.v1'
 ...
 *  (panergos-agent#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_panergos_home() re-creates
 * the profile directory the delete just removed (panergos-agent#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (panergos-agent#94235). Tiles persisted
...
// Dev hook for automation (mirrors __PANERGOS_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__PANERGOS_SESSION_TILES__ = {
