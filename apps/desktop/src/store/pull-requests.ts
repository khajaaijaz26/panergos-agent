...
import type { PanergosBranchPullRequest } from '@/global'
import { desktopGit } from '@/lib/desktop-git'
import { Codecs, persistentAtom } from '@/lib/persisted'
import { scanSessionPullRequests, type SessionInfo } from '@/panergos'
...
export const $pullRequestsByBranch = atom<Record<string, PanergosBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'panergos.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('panergos.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: PanergosBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
