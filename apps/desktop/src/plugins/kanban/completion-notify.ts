 ...
 *    desktop shell fires only while the user is AWAY from Panergos. This is the
...
import { host, type PluginOs, type PluginRestOptions, type PluginTranslate } from '@panergos/plugin-sdk'
...
function notifyOne(kind: string, spec: { titleKey: string; toast: ToastKind }, ev: CompletionEvent): void {
  ...
  // is away from Panergos (the toast above covers the foreground case). Isolated:
