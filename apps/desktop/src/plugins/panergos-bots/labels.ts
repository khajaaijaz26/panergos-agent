...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // activation and remote-only rosters (#89131).
  ...
  // "Panergos". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `panergos profile rename
  ...
  // never generic "Panergos" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Panergos (the agent it is)
