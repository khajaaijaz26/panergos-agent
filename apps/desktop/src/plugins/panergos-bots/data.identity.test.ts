...
vi.mock('@panergos/plugin-sdk', async () => {
...
vi.mock('./shared', () => ({ getPluginCtx: () => null, ID: 'panergos-bots' }))
...
describe('the @handle a bot answers to', () => {
  it('presents the primary profile as @panergos — "default" never surfaces in the UI', () => {
    expect(botHandle('default')).toBe('panergos')
    expect(botHandle('ops')).toBe('ops')
  })

  it('prefers a precomputed multi-source handle over the bare name', () => {
    expect(botHandle('default', row({ handle: 'default-vera', name: 'default' }))).toBe('panergos-vera')
  })
})

describe('renamed bots stay taggable', () => {
  // Discord report, Aug 2026: renaming a bot — Bot Mode title or `panergos
  ...
  it('drops reserved tokens so a rename cannot hijack a built-in tag', () => {
    expect(mentionNameForms('Panergos')).toEqual([])
  ...
  it('inserts the renamed slug, falling back to the profile handle', () => {
    ...
    expect(botMentionTag(row({ name: 'default' }))).toBe('panergos')
    // display_name (`panergos profile rename`) drives the tag too.
...
describe('resolving @mentions against the roster', () => {
  ...
  it('resolves the primary handle without self-handoffs', () => {
    expect(resolveRosterMentions('@panergos do it', roster, { connectionId: 'local', name: 'default' })).toEqual([])

    // From another bot's chat the tag is a real handoff.
    expect(
      resolveRosterMentions('@panergos do it', roster, { connectionId: 'mac-mini', name: 'dixie' }).map(
        bot => bot.name
      )
    ).toEqual(['default'])
  })
...
describe('roster search narrows without re-ranking', () => {
  ...
  it('matches profile handles and preserves roster order', () => {
    ...
    expect(filterBots(roster, meta, '@panergos').map(bot => bot.name)).toEqual(['default'])
    expect(filterBots(roster, meta, '@panergos').map(bot => bot.name)).toEqual(['default'])
...
describe('needs-attention badge (#93091 item 3)', () => {
  ...
  it('classifies the raw gateway error strings current backends emit', () => {
    ...
    expect(reasonFor('No LLM provider configured. Run panergos model to pick one.')).toBe('missing_config')
...
describe('alias identity reaches the mention layer (#89131)', () => {
  it('keeps @moxie resolving against the hosted row after handoff', () => {
    indexAliasRoutes([{ connectionId: 'remote-abc', mode: 'remote', profile: 'moxie', targetProfile: 'default' }])
    $botMeta.set({ 'remote-abc::moxie': { title: 'Moxie' } })

    const hostedRow = row({
      connectionId: 'remote-abc',
      name: 'default',
      remoteSource: true,
      sourceScoped: true,
