/**
 * Routines: the Panergos cron jobs scoped to the bot you're chatting with — the
...
// panergos -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `panergos -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Panergos-native schedule string; Advanced exposes it raw.
...
/** Compose the Panergos schedule string from picker state. */
