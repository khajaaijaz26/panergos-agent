...
import { forgetSessionUnread, host, queryClient } from '@panergos/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['panergos-bots']
...
/** Permanently delete a bot's Panergos profile, then remove plugin-local state
 ...
 * directory (panergos-agent#52279). That is the "can't delete a bot" error. */
