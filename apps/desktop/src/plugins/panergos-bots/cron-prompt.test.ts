 ...
 * job for a different profile keeps the `panergos -p <bot> chat` delegation
...
/** Run the delegation command under a `panergos` stub that prints its argv, so
 *  the assertion is what the SHELL passed — not what the string looks like. */
function argvOf(prompt: string): string[] {
  const command = prompt.slice(prompt.indexOf('panergos '), prompt.lastIndexOf('\n\nIf the command'))

  const shell = process.platform === 'win32' && existsSync('C:\\Program Files\\Git\\bin\\sh.exe')
    ? 'C:\\Program Files\\Git\\bin\\sh.exe'
    : 'sh'

  const result = spawnSync(shell, ['-c', `panergos() { printf '%s\\037' "$@"; }\n${command}`], {
    encoding: 'utf8'
  })
...
describe('direct vs delegated execution', () => {
  ...
  it('keeps the delegation wrapper for a different active profile', () => {
    ...
    expect(prompt).toMatch(/panergos -p 'research' chat/)
