...
async function contributions({
  ...
  cache.set(JSON.stringify(['panergos-bots', 'roster', cacheKeyConnection]), {
    key: ['panergos-bots', 'roster', cacheKeyConnection],
    value: { profiles }
  })
...
describe('@-mention completions', () => {
  it('offers the remote @name-device handle from the suffixed cache entry', async () => {
    ...
    expect(provide('').map(item => item.insert)).toContain('@panergos-vera')
  })
  ...
  it('surfaces default as @panergos and prefix-filters on the handle', async () => {
    const { provide } = await contributions({
      focused: 'researcher',
      profiles: [
        { name: 'default' },
    ...
    expect(provide('').map(item => item.insert)).toEqual(expect.arrayContaining(['@panergos', '@writer-homelab']))
  ...
  it('never offers the bot whose chat you are already in', async () => {
    ...
    expect(provide('').map(item => item.insert)).toEqual(['@panergos-vera'])
  })

  it('narrows on the typed prefix and labels the source device', async () => {
    const { provide } = await contributions()

    expect(provide('panergos-v')[0]).toMatchObject({ insert: '@panergos-vera', meta: expect.stringContaining('Vera') })
...
describe('the mention middleware', () => {
  it('identifies a remote @name-device mention without delivering anything', async () => {
    ...
    expect(result.text).not.toMatch(/panergos -p '?default/)
  ...
  it('annotates the resolvable handle for a local row whose UI alias differs', async () => {
    ...
    // The local resolver accepts bare profile names plus the Panergos alias.
    const { handler } = await contributions({
      focused: 'ops',
      profiles: [
    ...
    expect(result.text).toMatch(/@panergos-this-device = agent profile "default"/)
    expect(result.text).toMatch(/message_agent target: "panergos"/)
  ...
  it('teaches no shellout and forbids forwarding the user’s text verbatim', async () => {
    ...
    // a `panergos -p …` handoff, giving the model a second send path and a way
    // to relay the raw draft.
    const { handler } = await contributions({ focused: 'research', profiles: [{ name: 'research' }, { name: 'ops' }] })
    const result = await handler({ text: 'ask @ops to summarize' })

    expect(result.text).not.toMatch(/panergos -p/)
  ...
  it('keeps a poisoned bot title inert prose', async () => {
    ...
    // The invariant that matters is that no Panergos command is ever emitted.
    const { handler } = await contributions({
      focused: 'ops',
      profiles: [
        { name: 'ops' },
    ...
    expect(result.text).not.toMatch(/`panergos/)
  })
