...
import { Button, host, Input } from '@panergos/plugin-sdk'
import { useEffect, useRef, useState } from 'react'

import { useBots } from './i18n'

// ── skills hub section: the REAL hub page (docs) embedded as a picker ──────
// https://khajaaijaz26.github.io/panergos-agent/docs/skills?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'panergos-skill-pick', ...} to us (panergos-agent#86243). We validate
...
const HUB_PICKER_URL = HUB_ORIGIN + '/panergos-agent/docs/skills?embed=picker'
...
export function HubSkillsSection({ forProfile, onInstalled }: HubSkillsSectionProps) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.type !== 'panergos-skill-pick' || !data.name) {
        return
      }
