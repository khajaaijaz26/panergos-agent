...
// turn in its OWN persistent per-group Panergos session and is fed only the
...
export function parseGroupChatMentions(text: unknown, members: GroupMember[]) {
  ...
  for (const member of members) {
    ...
    const legacyHandle = String(member.handle || '').trim()
    ...
    const forms = new Set([
      ...
      ...(legacyHandle
        ? [legacyHandle.toLowerCase(), legacyHandle.toLowerCase().replace(/[\s_-]+/g, '')]
        : []),
      ...(title
    ...
    if (member.name.toLowerCase() === 'default') {
      forms.add('panergos')
    }
