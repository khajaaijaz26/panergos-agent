 ...
 *
 * #89131 is the alias half: a per-profile remote alias ("moxie" → exact remote
 ...
 * (remote-abc, default) — a different identity than the alias meta. It read as
 * the raw remote hostname, or as generic "Panergos" when the remote was the only
 * source.
 *
...
vi.mock('@panergos/plugin-sdk', () => ({ host: hostMock }))
...
/** The remote backend's own roster row after alias handoff: the remote
 *  connection answers as its root profile, so the row identity is
 *  (remote-abc, default) — NOT the alias key. */
const hostedRow = {
  connectionId: 'remote-abc',
  connectionLabel: 'gateway.example.com',
  name: 'default',
  remoteSource: true,
  route: { connectionId: 'remote-abc', mode: 'remote', profile: 'default', targetProfile: 'default' },
  sourceScoped: true,
  targetProfile: 'default'
} as RosterRow
...
describe('alias identity survives the hosted handoff (#89131)', () => {
  it('names the backend row after the alias, through every meta generation', () => {
    ...
    const metaV2 = { 'remote-abc::moxie': { title: 'Moxie' } }

    expect(botRosterMeta(hostedRow, metaV2)).toBe(metaV2['remote-abc::moxie'])
  ...
  it('renders the sole remote-only default as the alias, not "Panergos"', () => {
    // Global route is remote: the active gateway IS the remote connection and
    // profiles.list returns one unannotated rich `default` row.
    hostMock.state.connectionId.get.mockReturnValue('remote-abc')
  ...
  it('never leaks the alias onto a same-named default on another connection', () => {
    ...
    // Local default while the ACTIVE gateway is local: untouched "Panergos".
    expect(displayName({ name: 'default' }, null)).toBe('Panergos')
  })

  it('fails closed when two aliases claim one backend row', () => {
    indexAliasRoutes([
      MOXIE_ROUTE,
      { connectionId: 'remote-abc', mode: 'remote', profile: 'roxie', targetProfile: 'default' }
    ])

    expect(aliasIdentityFor(hostedRow)).toBeNull()
    // Ambiguous: fall back to the source label, not a guessed alias.
    expect(displayName(hostedRow, null)).toBe('gateway.example.com')
  })
