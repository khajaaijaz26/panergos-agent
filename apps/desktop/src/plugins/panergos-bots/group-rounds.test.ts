...
describe('routing', () => {
  ...
  it('resolves the primary handle to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@panergos take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @panergos', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @panergos,/)
    ...
    expect(peer).toMatch(/group chat with @panergos/)
  })
