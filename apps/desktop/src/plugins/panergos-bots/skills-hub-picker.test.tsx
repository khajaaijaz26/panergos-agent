 ...
 * `{type:'panergos-skill-pick'}` back and the plugin installs via skills.manage.
 *
...
const HUB_ORIGIN = 'https://khajaaijaz26.github.io'
...
vi.mock('@panergos/plugin-sdk', async importOriginal => {
  const original = await importOriginal<typeof PanergosSdk>()
...
function openHubBrowser() {
  ...
  expect(frame?.getAttribute('src')).toBe(
    'https://khajaaijaz26.github.io/panergos-agent/docs/skills?embed=picker'
  )
...
describe('hub pick messages', () => {
  it('installs the picked skill when it comes from our own frame', () => {
    ...
    postPick(
      { identifier: 'acme/web-research', name: 'Web Research', type: 'panergos-skill-pick' },
      {
  ...
  it('regression: refuses identifiers outside the slug charset', () => {
    ...
    for (const identifier of ['../../etc/passwd', 'skill; rm -rf /', '-flag', 'name with spaces', '']) {
      postPick({ identifier, name: 'Web Research', type: 'panergos-skill-pick' }, { source: frame.contentWindow })
    }
  ...
  it('ignores messages that are not a skill pick', () => {
    ...
    postPick({ identifier: 'acme/web-research', type: 'panergos-skill-pick' }, { source: frame.contentWindow })
