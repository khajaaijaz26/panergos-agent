...
import { atom, host, queryClient, useQuery, useValue } from '@panergos/plugin-sdk'
...
export const BOT_ATTENTION_HINTS: Record<string, string> = {
  ...
  missing_config: 'Provider not configured — run panergos model',
...
function attentionReasonFromError(errorTextOrReason: unknown) {
  ...
  if (/quota|out of funds|insufficient (credits?|funds|balance)|payment required|\b402\b/.test(text)) {
 ...
 *  protocol into the system prompt itself (panergos-agent bot_mode_probe).
...
export function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (panergos-agent #86875) also expose the union
 ...
 *  callable alias is 'panergos' — the mention middleware resolves it back to
...
export function botHandle(name: string, bot?: Partial<RosterRow> | null): string {
  ...
  if (bot?.handle && bot.handle !== name) {
    return profile === 'default' ? bot.handle.replace(/^default(?=-|$)/i, 'panergos') : bot.handle
  }

  return profile === 'default' ? 'panergos' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`panergos profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Panergos"
 *  can never hijack the primary profile's @panergos alias. */
export function mentionNameForms(value: null | string | undefined): string[] {
  ...
  return [...new Set([slug, collapsed])].filter(
    form =>
      ...
      !['all', 'everyone', 'user', 'default', 'panergos'].includes(form)
  )
}
...
export function botFriendlyNames(bot: Partial<RosterRow> | null | undefined): Array<null | string | undefined> {
  ...
  return [bot?.ui_meta?.['panergos-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update Panergos Desktop to open another Bot chat.'
    })
...
export function resolveRosterMentions(
  ...
  for (const bot of members) {
    ...
    // Saved prompts may still use the former public handle.
    if (name === 'default') {
      forms.add('panergos')
    }
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    const bot = byForm.get(match[2].toLowerCase())
...
export function filterBots(roster: RosterRow[], metaByName: Record<string, BotMeta>, query: string) {
  ...
  return roster.filter(bot => {
    ...
    const legacyHandle = profile === 'default' ? 'panergos' : ''
