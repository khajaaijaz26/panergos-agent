...
import { atom, host } from '@panergos/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'panergos-bots-groups'
...
/** The v3 envelope stored under the default profile's `panergos-bots-groups`
 ...
 *  clients (Panergos Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Panergos"
 *  (community report, Aug 21 2026: renamed default still read "Panergos is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Panergos (matching displayName and the
 *  @panergos handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`panergos profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Panergos' : trimmed
}
