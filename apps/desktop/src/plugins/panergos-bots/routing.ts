...
// A Desktop per-profile alias (profile `moxie` with a URL/SSH override)
...
// raw remote hostname, or to generic 'Panergos' in remote-only mode.
//
...
export function aliasIdentityFor(bot: Partial<RosterRow> | null | undefined): AliasIdentity | null {
  ...
  const connectionId = String(
    bot?.connectionId ||
      ...
      // the ACTIVE gateway — remote-only mode must resolve the alias too.
