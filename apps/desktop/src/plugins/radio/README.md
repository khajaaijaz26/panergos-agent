...
Bundled Panergos Desktop plugin, **off by default**. Enable **Radio** in
...
`plugin.js` imports only `@panergos/plugin-sdk`, `react`, and
...
`$PANERGOS_HOME/desktop-plugins/radio/plugin.js` for development. Do not install
that duplicate alongside the bundled version. Layout styles use Panergos theme
...
Stream availability, regional restrictions, and metadata vary. Panergos is not
