...
const CSS = `
.panergos-radio-bar{display:flex;align-items:center;gap:2px;height:100%;color:var(--ui-text-tertiary)}
.panergos-radio-waveform{position:relative;height:24px;width:100%;font-family:var(--font-mono,monospace);font-size:12px;line-height:12px;color:var(--ui-text-tertiary);opacity:.45;overflow:hidden;flex-shrink:0}
.panergos-radio-waveform-layer{position:absolute;inset:0;display:grid;grid-template-columns:repeat(var(--radio-columns),minmax(0,1fr));grid-auto-rows:12px}
.panergos-radio-waveform-layer>span{text-align:center;min-width:0}
.panergos-radio-waveform-layer:nth-child(1){opacity:.12}
.panergos-radio-waveform-layer:nth-child(2){opacity:.28}
.panergos-radio-waveform[data-compact=true]{width:40px;height:12px;margin-inline:2px}
.panergos-radio-next{display:flex;align-items:center;width:12px;height:12px}
.panergos-radio-waveform[data-active=true]{color:var(--ui-accent);opacity:1}
.panergos-radio-signal{height:48px;padding:5px 6px 0;min-width:0}
.panergos-radio-track{display:flex;align-items:center;gap:5px;height:19px;min-width:0;font-size:11px;line-height:16px;color:var(--ui-text-secondary)}
.panergos-radio-track-text{overflow:hidden;white-space:nowrap;text-overflow:ellipsis;min-width:0}
.panergos-radio-track[data-track=true]{color:var(--ui-text-primary)}
.panergos-radio-artist{min-width:0;max-width:55%;flex-shrink:1;font-size:inherit;font-weight:inherit}
.panergos-radio-artist>span{overflow:hidden;text-overflow:ellipsis}
.panergos-radio-track-separator{flex-shrink:0;color:var(--ui-text-quaternary)}
.panergos-radio-bar .radio-name{max-width:112px;min-width:0;flex:1;overflow:hidden;text-overflow:ellipsis;text-align:left}
.panergos-radio-bar .panergos-radio-action{width:16px;height:16px;padding:0;flex-shrink:0}
.panergos-radio-action-icon{display:flex;align-items:center;justify-content:center;width:12px;height:12px;flex-shrink:0;overflow:hidden}
.panergos-radio-website{display:flex;width:24px;height:24px;flex-shrink:0}
.panergos-radio-panel{width:292px;max-width:calc(100vw - 24px)}
.panergos-radio-search{display:flex;align-items:center;height:32px;padding:0 4px 4px}
.panergos-radio-search-field{flex:1;min-width:0}
.panergos-radio-search-field input{flex:1;width:0;min-width:0}
.panergos-radio-search .panergos-radio-search-field{border:0}
.panergos-radio-search-loader{display:flex;align-items:center;justify-content:center;width:16px;flex-shrink:0;font-size:12px;color:var(--ui-text-tertiary)}
.panergos-radio-list{height:168px;max-height:calc(100dvh - 180px);min-height:80px;overflow-y:auto;overscroll-behavior:contain;scrollbar-gutter:stable}
.panergos-radio-row{display:flex;align-items:center;gap:4px;border-radius:4px}
.panergos-radio-row[data-current=true]{background:var(--chrome-action-hover)}
.panergos-radio-row[data-current=true] .panergos-radio-row-name{color:var(--ui-accent)}
.panergos-radio-row:hover{background:var(--chrome-action-hover)}
.panergos-radio-row [aria-pressed=true]{color:var(--ui-accent)}
.panergos-radio-pin{opacity:0;pointer-events:none}
.panergos-radio-row:hover .panergos-radio-pin,.panergos-radio-row:focus-within .panergos-radio-pin{opacity:1;pointer-events:auto}
@media(hover:none){.panergos-radio-pin{opacity:1;pointer-events:auto}}
.panergos-radio-row-main{display:flex;align-items:center;gap:8px;flex:1;min-width:0;text-align:left;height:24px;padding:2px 6px;cursor:pointer;border-radius:4px}
.panergos-radio-row-main:focus-visible{outline:1px solid var(--ui-accent);outline-offset:-1px}
.panergos-radio-row-icon{display:flex;align-items:center;justify-content:center;width:14px;flex-shrink:0;color:var(--ui-text-tertiary);opacity:0}
.panergos-radio-row:hover .panergos-radio-row-icon,.panergos-radio-row:focus-within .panergos-radio-row-icon,.panergos-radio-row[data-current=true] .panergos-radio-row-icon{opacity:1}
.panergos-radio-row[data-current=true] .panergos-radio-row-icon{color:var(--ui-accent)}
.panergos-radio-row-copy{min-width:0;flex:1}
.panergos-radio-row-name{display:block;font-size:12px;line-height:17px;color:var(--ui-text-primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.panergos-radio-volume{display:flex;align-items:center;gap:4px;height:28px;padding:2px 2px 0}
.panergos-radio-volume-space{flex:1;min-width:6px}
.panergos-radio-volume input{width:76px;min-width:0;height:3px;accent-color:var(--ui-accent);cursor:pointer}
.panergos-radio-error{font-size:11px;line-height:17px;color:var(--ui-text-secondary);padding:4px 4px 12px}
`
...
function createPlayer(ctx) {
  ...
  const bus = new BroadcastChannel('panergos:radio:playback')
  ...
  async function play(next = station.get(), analyse = true) {
    ...
    audio = element
    element.dataset.panergosRadioAudio = 'true'
    element.hidden = true
...
function Waveform({ player, compact = false }) {
  ...
  return jsx('div', { ref, className: 'panergos-radio-waveform', style: { '--radio-columns': width }, 'data-radio-meter': width, 'data-compact': compact, 'data-meter-mode': mode, 'data-active': active, 'aria-hidden': true,
    children: [0, 1, 2].map(layer => jsx('span', { className: 'panergos-radio-waveform-layer', children: baseline.map((cell, i) => jsx('span', { children: layer === 2 ? cell : '⠀' }, i)) }, layer)) })
}

function NextArrow() {
  return jsxs('span', { className: 'panergos-radio-next', 'aria-hidden': true, children: [
...
function SmallAction({ label, icon, onClick, size = 'micro', pressed, className, busy = false }) {
  return jsx(Tip, { label, children: jsx(Button, {
    variant: 'ghost', size, className: `panergos-radio-action ${className ?? ''}`, 'aria-label': label, 'aria-pressed': pressed, onClick,
    children: jsx('span', { className: 'panergos-radio-action-icon', children: busy ? jsx(GlyphSpinner, { ariaLabel: label }) : jsx(icon, {}) })
  }) })
}
...
async function artistRequest(path, signal, ctx) {
  ...
  return navigator.locks.request('panergos:radio:artist-lookup', { signal }, async () => {
    ...
    const response = await fetch(`https://musicbrainz.org/ws/2/${path}`, {
      ...
      headers: { 'User-Agent': 'PanergosRadio/1.0 (https://github.com/khajaaijaz26/panergos-agent)' }
    })
...
function Signal({ player, ctx }) {
  ...
  return jsxs('div', { className: 'panergos-radio-signal', children: [
    jsx(Waveform, { player }),
    jsxs('div', { className: 'panergos-radio-track', 'data-track': Boolean(track), role: 'status', children: [
      ...
      credit.artist && (artistUrl ? jsx(Tip, { label: `${t('artistProfile')}: ${credit.artist}`, children: jsx(Button, {
        variant: 'text', size: 'inline', asChild: true, className: 'panergos-radio-artist',
      ...
      }) }) : jsx('span', { className: 'panergos-radio-track-text', children: credit.artist })),
      credit.artist && jsx('span', { className: 'panergos-radio-track-separator', 'aria-hidden': true, children: '—' }),
      jsx(Tip, { label: text || undefined, children: jsx('span', { className: 'panergos-radio-track-text', children: credit.artist ? credit.title : text }) })
    ] })
  ] })
}
...
function StationRow({ station, player }) {
  ...
  return jsxs('div', { className: 'panergos-radio-row', 'data-current': current, 'data-pinned': saved, children: [
    jsxs(RowButton, { className: 'panergos-radio-row-main', 'aria-label': `${t(failed ? 'retry' : current && active ? 'pause' : 'play')}: ${station.name}`, title: failed ? t('streamError') : undefined, 'aria-current': current ? 'true' : undefined,
      onClick: () => current ? player.toggle() : void player.play(station), children: [
        jsx('span', { className: 'panergos-radio-row-icon', role: failed ? 'status' : undefined, 'aria-label': failed ? t('error') : undefined, children: jsx(failed ? icons.AlertCircle : current && active ? icons.Pause : icons.Play, { size: 12 }) }),
        jsx('span', { className: 'panergos-radio-row-copy', children: jsx('span', { className: 'panergos-radio-row-name', children: station.name }) })
      ] }),
    jsx(SmallAction, { label: `${t(saved ? 'unsave' : 'save')}: ${station.name}`, icon: icons.Pin, onClick: () => player.favorite(station), size: 'icon-xs', pressed: saved, className: 'panergos-radio-pin' })
  ] })
}
...
function Stations({ player }) {
  ...
  return jsxs('div', { children: [
    jsx('div', { className: 'panergos-radio-search', 'data-filled': Boolean(search), children: jsx(SearchField, {
      value: search, onChange: value => { setBrowseStation(player.station.get()); setSearch(value) }, placeholder: t('search'),
      containerClassName: 'panergos-radio-search-field',
      trailingAction: pending ? jsx('span', { className: 'panergos-radio-search-loader', children: jsx(GlyphSpinner, { ariaLabel: t('searching') }) }) : null
    }) }),
    jsxs('div', { ref: listRef, className: 'panergos-radio-list', 'data-radio-pending': pending, children: [
      ...items.map(station => jsx(StationRow, { station, player }, station.id)),
      searchFailed && jsx('div', { className: 'panergos-radio-error', role: 'status', children: t('searchError') }),
      !pending && !searchFailed && text && results.length === 0 && jsx('div', { className: 'panergos-radio-error', role: 'status', children: t('noResults') })
    ] })
  ] })
}
...
function Transport({ player, ctx }) {
  ...
  const status = useValue(player.status)
  const active = status === 'live' || status === 'connecting'
  return jsxs('div', { className: 'panergos-radio-volume', children: [
    ...
    jsx('span', { className: 'panergos-radio-volume-space' }),
    ...
    jsx('span', { className: 'panergos-radio-website', children: httpsUrl(station.homepage) && jsx(SmallAction, { label: t('visit'), icon: icons.ExternalLink, size: 'icon-xs', onClick: () => void ctx.os.openExternal(station.homepage) }) })
  ] })
}
...
function RadioBar({ player, ctx }) {
  ...
  return jsxs('div', { className: 'panergos-radio-bar', 'data-tour': 'radio-player', 'data-radio-status': status, children: [
    ...
    jsxs(Popover, { open, onOpenChange, children: [
      ...
      jsx(PopoverContent, { side: 'top', align: 'end', className: 'panergos-radio-panel', 'aria-label': t('radio'), 'data-tour': 'radio-panel', children:
        jsxs('div', { children: [
