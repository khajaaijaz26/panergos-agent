...
## The desktop is its own chat surface on a `panergos serve` backend
...
(`@panergos/shared`: `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard also consumes.
The desktop has **no build/runtime dependency on the dashboard frontend**: it spawns a headless
`panergos serve` (`headless_backend=True` → `cmd_dashboard` skips `_build_web_ui` and exports
`PANERGOS_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists).
`dashboard` and `serve` share `cmd_dashboard`/`start_server` but neither launches the other. It does
NOT embed `panergos --tui` — own composer, transcript, slash pipeline.
...
and ONLY when it does not (older managed install / PATH `panergos` not yet updated) rewrites argv to
...
- `src/lib/desktop-slash-commands.ts` is the load-bearing file: `DESKTOP_COMMAND_SPECS` (built-ins
  ...
  `CommandDef` in `panergos_cli/commands.py`; the live `commands.catalog` carries it, and
  `src/lib/desktop-slash-registry.json` (regenerate with `scripts/dump_desktop_slash_registry.py`;
  `tests/panergos_cli/test_desktop_slash_registry.py` + the vitest file fail on drift) is the offline
...
## Bot Mode (`src/plugins/panergos-bots/`) — one bot = ONE canonical forever-chat, identified by NAME

Each bot is a Panergos **profile** with a persistent identity. This invariant regressed repeatedly,
...
**There is NO session-id pin.** The old design stored a pointer in `ui_meta['panergos-bots'].chat`;
...
in `src/plugins/panergos-bots/`: `canonical-chat-registry.test.ts` (tripwire: the open path never
