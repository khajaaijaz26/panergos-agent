...
import { getPanergosConfigRecord, listAllProfileSessions } from '@/panergos'
...
// Panergos session ids: <YYYYMMDD>_<HHMMSS>_<6 hex>. Used to offer a direct
...
function CommandPaletteBody({ onExited }: { onExited: () => void }) {
  ...
  const configQuery = useQuery({
    ...
    queryFn: () => getPanergosConfigRecord()
  })
        ...
        : []),
      {
        ...
        items: [
          ...
          },
          {
            ...
            icon: Download,
            id: 'cc-update-panergos',
            keywords: ['update', 'upgrade', 'panergos', 'version', 'system', 'restart'],
            label: cc.updatePanergos,
  ...
  const searchGroups = useMemo<PaletteGroup[]>(() => {
    ...
    // Apply a theme directly from the root search (e.g. "eclipse" → Eclipse). Live
