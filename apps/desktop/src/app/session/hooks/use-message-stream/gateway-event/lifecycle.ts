import type { GatewayEvent } from '@panergos/shared'
...
export function handleLifecycleEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'skin.changed') {
    // A runtime skin switch (Panergos activating an authored skin, or `/skin`
    // on another surface). Only the active source+profile's change repaints.
    if (fromActiveSource()) {
      ingestBackendSkin(payload as PanergosSkin | undefined, { apply: true })
    }
