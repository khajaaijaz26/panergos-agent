...
export function handleStatusEvent(ctx: GatewayEventContext): boolean {
  ...
  if (event.type === 'notification.show') {
    ...
    // native OS notification when Panergos is backgrounded; dispatch is gated
  ...
  if (event.type === 'error') {
    const errorMessage = payload?.message || 'Panergos reported an error'
