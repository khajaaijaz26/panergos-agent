...
let refreshPanergosConfig: ReturnType<typeof vi.fn<() => Promise<void>>>
...
beforeEach(() => {
  sessionStates = null
  refreshPanergosConfig = vi.fn<() => Promise<void>>(async () => undefined)
...
describe('session.info config refetch gating', () => {
  it('coalesces active-session bursts into one trailing config fetch', async () => {
    ...
    expect(refreshPanergosConfig).not.toHaveBeenCalled()
