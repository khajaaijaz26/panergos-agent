...
import { getPanergosConfig, getPanergosConfigDefaults } from '@/panergos'
...
interface PanergosConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function usePanergosConfig({ activeSessionIdRef }: PanergosConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshPanergosConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getPanergosConfig(), getPanergosConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Panergos' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshPanergosConfig, sttEnabled, voiceMaxRecordingSeconds }
}
