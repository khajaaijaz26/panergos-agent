...
import { getPanergosConfig } from '@/panergos'
import {
  $currentCwd,
  $currentFastMode,
...
import { usePanergosConfig } from './use-panergos-config'

vi.mock('@/panergos', () => ({
  getPanergosConfig: vi.fn(),
  getPanergosConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'panergos.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getPanergosConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getPanergosConfig>>)

describe('usePanergosConfig refreshPanergosConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Panergos' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => usePanergosConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshPanergosConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => usePanergosConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getPanergosConfig>>>()
    vi.mocked(getPanergosConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => usePanergosConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshPanergosConfig(true)
    })
    expect(getPanergosConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getPanergosConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getPanergosConfig>>>()
    vi.mocked(getPanergosConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => usePanergosConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshPanergosConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getPanergosConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getPanergosConfig>>>()
    vi.mocked(getPanergosConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshPanergosConfig(true)
      refreshC = result.current.refreshPanergosConfig(true)
    })
