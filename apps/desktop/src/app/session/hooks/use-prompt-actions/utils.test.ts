...
describe('renderRpcResult', () => {
  ...
  describe('session.save', () => {
    it('echoes the saved file path', () => {
      expect(renderRpcResult({ file: '/home/user/.panergos/sessions/saved/x.json' }, 'save')).toBe(
        'Saved transcript to /home/user/.panergos/sessions/saved/x.json'
      )
    })
  })
  ...
  describe('session.status', () => {
    it('passes through the multi-line plain-text output verbatim', () => {
      const output = 'Panergos TUI Status\n\nSession ID: s-1\nModel: example-model (unknown)'
