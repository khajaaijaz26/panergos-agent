import { skillInvocationText } from '@panergos/shared'
import { parseCommandDispatch, parseSlashCommand } from '@panergos/shared'
...
import { getProfiles } from '@/panergos'
...
const renderWakeStatus = (status: WakeStatusResponse): string => {
  const lines = [
    ...
    `Phrase: "${status.phrase?.trim() || 'hey panergos'}"`,
  ...
  return useCallback(
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      ...
      const actionHandlers: Record<DesktopActionId, (ctx: SlashActionCtx) => Promise<void>> = {
        ...
        // lease. Sending it through slash.exec creates a separate PanergosCLI in
