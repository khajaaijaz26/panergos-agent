...
function setWorkspaceTransfer(transferring: boolean): void {
  window.panergosDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.panergosDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.panergosDesktop?.hud?.beginMove?.()
}
 ...
 * (see panergos:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.panergosDesktop?.hud?.endMove?.()
      }
