...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { panergosDesktop: { readDir: typeof readDir } }).panergosDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): PanergosReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { panergosDesktop: unknown }).panergosDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: PanergosReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<PanergosReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: PanergosReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: PanergosReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: PanergosReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<PanergosReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.panergosDesktop is missing', async () => {
    delete (window as unknown as { panergosDesktop?: unknown }).panergosDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: PanergosReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: PanergosReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: PanergosReadDirResult) => void) | undefined
