...
import type { PanergosReadDirEntry, PanergosReadDirResult } from '@/global'
...
export type ProjectTreeEntry = PanergosReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: PanergosReadDirEntry) {
...
async function filterIgnored(entries: PanergosReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<PanergosReadDirResult> {
