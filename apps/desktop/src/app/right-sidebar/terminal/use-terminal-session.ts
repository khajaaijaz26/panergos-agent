...
const PANERGOS_PATHS_MIME = 'application/x-panergos-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(PANERGOS_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(PANERGOS_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.panergosDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (panergos --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.panergosDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[panergos-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.panergosDesktop?.terminal?.write(sessionId, `${command}\r`)
