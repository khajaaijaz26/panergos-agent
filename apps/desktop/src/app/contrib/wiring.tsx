...
import { getLatestSessionMessages } from '@/panergos'
...
import { usePanergosConfigRecord } from '../hooks/use-config-record'
...
import { usePanergosConfig } from '../session/hooks/use-panergos-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  // Without this pin, the owner ladder sends session.create to panergos-setup
  ...
  const { refreshPanergosConfig, sttEnabled, voiceMaxRecordingSeconds } = usePanergosConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('panergos:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('panergos:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshPanergosConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshPanergosConfig])
  ...
  const configRecord = usePanergosConfigRecord()
