...
import { getPanergosConfigRecord, saveMcpServers } from '@/panergos'
import { $mcpInstallRequest } from '@/store/mcp-deeplink-install'
import { notify, readableError } from '@/store/notifications'

import { setPanergosConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `panergos://mcp/install` deep links. The payload is
 ...
 * confirming lets Panergos spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getPanergosConfigRecord())
      ...
      setPanergosConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
