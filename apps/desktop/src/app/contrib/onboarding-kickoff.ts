...
async function adoptGuideSession(
  ...
  _guideRequest: AmbientGatewayRequest
): Promise<void> {
...
/** Seeds the runbook and a pre-written greeting on panergos-setup before the phase advances.
...
export function useOnboardingKickoff({
  ...
  return useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      if (canonical?.id) {
        await adoptGuideSession(canonical, resumeSession, guideRequest)
