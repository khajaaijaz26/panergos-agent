...
export function markFirstBuildSession(storedId: string): void {
  writeKey('panergos.onboarding.first-build.v1', storedId)
}

export function endFirstBuildConnect(storedId: string): void {
  writeKey('panergos.onboarding.first-build.done.v1', storedId)
}

export function isFirstBuildSession(storedId: string | null | undefined): boolean {
  return (
    !!storedId &&
    readKey('panergos.onboarding.first-build.v1') === storedId &&
    readKey('panergos.onboarding.first-build.done.v1') !== storedId
  )
}

export function handoffReceiptKey(connection: null | string, guideStoredId: string): string {
  return `panergos.onboarding.handoff.v1.connection.${encodeURIComponent(connection ?? 'ambient')}.profile.default.guide.${encodeURIComponent(guideStoredId)}`
}
