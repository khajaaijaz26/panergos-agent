...
import { pathFromPanergosDeepLink, resolvePanergosOpenPath } from '@/lib/panergos-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Panergos" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.panergosDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.panergosDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.panergosDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.panergosDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // panergos://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.panergosDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future panergosDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolvePanergosOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.panergosDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (panergos://index-network/intent/1, panergos://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromPanergosDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.panergosDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.panergosDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.panergosDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.panergosDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
