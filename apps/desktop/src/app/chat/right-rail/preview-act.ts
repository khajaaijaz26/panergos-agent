...
const preamble = () => `  var w = window;
  ...
  w.__panergosActHolder = w.__panergosActHolder || {};
  w.__panergosAct = ${actEngineSource()};
  w.__panergosWatch_fn = (${watchInPage.toString()});
  w.__panergosWatchTag = ${WATCH_TAG};
  var holder = w.__panergosActHolder;
  var act = function (a) { return w.__panergosAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__panergosWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  w.__panergosHit = null;
  document.addEventListener('pointerdown', function (e) {
    w.__panergosHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'PANERGOS-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
