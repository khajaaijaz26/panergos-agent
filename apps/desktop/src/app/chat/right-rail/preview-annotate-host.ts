...
export function overlayInstallScript(source: string): string {
  return '(function(){var api=' + source + ';window.__panergosAnnotate=api;api.install();})()'
}
...
export async function teardownAnnotateOverlay(guest: PreviewAnnotateGuest): Promise<void> {
  await guest.executeJavaScript(`
    (function () {
      if (window.__panergosAnnotate && window.__panergosAnnotate.teardown) {
        window.__panergosAnnotate.teardown();
      }
      window.__panergosAnnotate = null;
    })()
  `)
}
...
export async function waitAnnotateEvent(guest: PreviewAnnotateGuest): Promise<AnnotatePageEvent> {
  const event = await guest.executeJavaScript(`
    window.__panergosAnnotate ? window.__panergosAnnotate.wait() : Promise.resolve({ type: 'end' })
  `)
...
export async function syncAnnotatePins(guest: PreviewAnnotateGuest, pins: AnnotatePinChrome[]): Promise<void> {
  await guest.executeJavaScript(`
    window.__panergosAnnotate && window.__panergosAnnotate.showPins(${JSON.stringify(pins)})
  `)
}
...
export async function showAnnotateDraft(
  ...
  await guest.executeJavaScript(`
    window.__panergosAnnotate && window.__panergosAnnotate.showDraft(${JSON.stringify(rect)}, ${number})
  `)
}

export async function hideAnnotateDraft(guest: PreviewAnnotateGuest): Promise<void> {
  await guest.executeJavaScript(`
    window.__panergosAnnotate && window.__panergosAnnotate.hideDraft()
  `)
}
...
export async function captureAnnotateCrop(
  ...
  await tryGuest(guest, 'window.__panergosAnnotate ? window.__panergosAnnotate.beginCapture() : null')

  try {
    return await guest.capture(padRect(rect))
  } finally {
    await tryGuest(guest, 'window.__panergosAnnotate && window.__panergosAnnotate.endCapture()')
  }
}
