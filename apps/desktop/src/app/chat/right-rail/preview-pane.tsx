        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.panergosDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.panergosDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Panergos to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.panergosDesktop?.watchPreviewFile ||
      !window.panergosDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.panergosDesktop.onPreviewFileChanged(payload => {
    ...
    void window.panergosDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.panergosDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.panergosDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:panergos-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.panergosDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.panergosDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.panergosDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.panergosDesktop?.openExternal(currentUrl)
                : undefined
            }
