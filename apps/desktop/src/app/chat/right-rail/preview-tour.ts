...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__panergosTourEngine) {
    ${driverIife}
    w.__panergosTourHolder = {};
    w.__panergosTourCollect = (${collectTourTargets.toString()});
    w.__panergosTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__panergos-tour-style')) {
    var style = document.createElement('style');
    style.id = '__panergos-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__panergosTourEngine(
    w.driver.js.driver,
    w.__panergosTourHolder,
    ${JSON.stringify(action)},
    w.__panergosTourCollect,
    document
  ));
})()`
}
