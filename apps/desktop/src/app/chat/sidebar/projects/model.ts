...
import type { SessionInfo } from '@/panergos'
...
// zero Panergos sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, PanergosGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, PanergosGitWorktree[]>>({})
