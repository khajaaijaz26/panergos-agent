...
export function ProfileRemoteOverrideDialog({ profileNames }: { profileNames: string[] }) {
  ...
  useEffect(() => {
    ...
    window.panergosDesktop?.connections
      ?.list()
      .then(registry => {
        if (cancelled) {
          return
  ...
  const performSave = async () => {
    ...
    try {
      await window.panergosDesktop.applyConnectionConfig({
        mode: 'remote',
        profile,
  ...
  const removeOverride = async () => {
    ...
    try {
      await window.panergosDesktop.applyConnectionConfig({ mode: 'local', profile })
