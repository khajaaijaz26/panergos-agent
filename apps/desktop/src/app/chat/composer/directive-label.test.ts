...
import { panergosDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
function sentLabel(draft: string) {
  return panergosDirectiveFormatter
    .parse(draft)
...
describe('one label per reference, on every surface', () => {
  ...
  it('an agent mention inserts as plain @name text, never an @simple: chip', () => {
    ...
    expect(panergosDirectiveFormatter.serialize(item)).toBe('@mr-tester')
  ...
  it('a url still reads host + path on every surface', () => {
    const item = backendRow('@url:https://github.com/khajaaijaz26/panergos-agent/pull/74533', '', '')
    const { editor, result } = typed('@gith')

    act(() => result.current.replaceTriggerWithChip(item))

    const expected = 'github.com/khajaaijaz26/panergos-agent/pull/74533'
