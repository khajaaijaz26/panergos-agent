...
import { extractDroppedFiles, PANERGOS_PATHS_MIME, partitionDroppedFiles } from '../../hooks/use-composer-actions'
...
export function useComposerDrop({
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, PANERGOS_PATHS_MIME)) {
      return
    }
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, PANERGOS_PATHS_MIME)) {
      return
    }
