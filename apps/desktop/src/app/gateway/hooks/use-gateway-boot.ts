...
import type { DesktopBootProgress, PanergosConnection } from '@/global'
...
import { PanergosGateway } from '@/panergos'
import {
  $desktopBoot,
...
export function primaryRuntimeConnectionId(connection: Pick<PanergosConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.panergosDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: PanergosGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: PanergosConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Panergos...". Once the
    ...
    const attemptReconnect = async (manual?: { profile: string; activationEpoch: number }) => {
      ...
      try {
        ...
        // "Starting Panergos…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Panergos backend'
        )
        ...
        // as an opaque "Could not connect to Panergos gateway". resolveGatewayWsUrl
        ...
        if (!manual || (isActivePrimary() && gatewayActivationEpoch() === manual.activationEpoch)) {
          reconcileBusyStatesOnReconnect()
          await callbacksRef.current.refreshPanergosConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshPanergosConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const onBootProgress = (payload: DesktopBootProgress) => {
      ...
      // Soft switch / post-boot startPanergos re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new PanergosGateway()
    ...
    configureGatewayRegistry({
      onServerRequest: request => {
        if (!callbacksRef.current.handleServerRequest(request)) {
          request.fail(JSON_RPC_METHOD_NOT_FOUND, `Panergos Desktop cannot answer ${request.method}`)
        }
      },
    ...
    const offGatewayReconnect = registerGatewayReconnect(async () => {
      ...
      if (!isActivePrimary()) {
        ...
        if (!(await ensureActiveGatewayOpen())) {
          throw new Error('Panergos gateway is not connected')
        }
    ...
    // (remotes have no local process whose death would drop the socket —
    ...
    async function boot() {
      ...
      try {
        ...
        // reaches the recovery affordance instead of hanging "Starting Panergos…".
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshPanergosConfig(),
          ...
          // retry it), not brick boot behind the "Panergos couldn't start"
