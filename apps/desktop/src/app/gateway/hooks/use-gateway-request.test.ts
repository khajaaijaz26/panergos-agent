...
vi.mock('@/panergos', async importOriginal => {
  const actual = await importOriginal<typeof PanergosModule>()

  class FakePanergosGateway {
  ...
  return { ...actual, PanergosGateway: FakePanergosGateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as PanergosGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as PanergosGateway, 'default')
  $gateway.set(primary as unknown as PanergosGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.panergosDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as PanergosGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { panergosDesktop: unknown }).panergosDesktop = { getConnection }
    $gateway.set(dropped)
