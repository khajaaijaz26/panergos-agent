...
// bug that drives the REAL useGatewayBoot hook + REAL PanergosGateway through a
...
function Harness({
  ...
  refreshPanergosConfig = async () => undefined,
  refreshSessions
}: {
  beforeConnectionSwitch?: () => void
  refreshPanergosConfig?: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
beforeEach(() => {
  ...
  ;(window as { panergosDesktop?: unknown }).panergosDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { panergosDesktop?: unknown }).panergosDesktop
  window.localStorage.removeItem('panergos.desktop.workspace-cwd')
  $currentCwd.set('')
  $busy.set(false)
  $awaitingResponse.set(false)
})
...
describe('primary failure foreground isolation', () => {
  it('ignores a boot snapshot superseded by a successful connection', async () => {
    ...
    ;(window as { panergosDesktop?: unknown }).panergosDesktop = desktop
    render(<Harness />)
    await flushAsync()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForPanergos) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startPanergos()'s remote branch awaits waitForPanergos() for 45s before it
    ...
    // After ~45s waitForPanergos gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Panergos backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('a superseded Settings switch cannot publish delayed cwd or config work after the winner', async () => {
    ...
    const refreshPanergosConfig = async (_force = false, shouldPublish?: () => boolean) => {
      if (!shouldPublish) {
        return
      }
    ...
    render(<Harness refreshPanergosConfig={refreshPanergosConfig} />)
    await flushAsync()
    expect($gatewayState.get()).toBe('open')
  ...
  it('keeps registered source sockets alive during a legacy mode apply', async () => {
    ...
    let opening!: Promise<boolean>
    act(() => {
      opening = ensureGatewayForAgent('remote-source', 'default')
    })
    await flushAsync()
    await opening
  ...
  it('onActiveConnectionInvalidated: a fallback getConnection() that hangs rejects on its own instead of latching $connection forever (#93454 sibling)', async () => {
    ...
    act(() => {
      disposeSecondariesForConnection('remote-source')
    })
    await flushAsync()
  ...
  it('a getConnection() that hangs on INITIAL boot rejects on its own after the reconnect-attempt timeout, not only when main eventually gives up (#93454)', async () => {
    ...
    // eventual timeout (e.g. waitForPanergos, ~45s) ever settled it. A wedge
    // that main never resolves (not even a rejection) must not hang
    // "Starting Panergos…" forever; the renderer needs to own its own bound
  ...
  it('manual reconnect replaces only the active secondary route', async () => {
    ;(window as { panergosDesktop?: unknown }).panergosDesktop = {
      ...fakeDesktop(),
      getConnectionFor: vi.fn(async () => coderConn),
      getGatewayWsUrlFor: vi.fn(async () => coderConn.wsUrl)
    }
  ...
  it('FIX: post-boot ticket-mint boot-progress errors do not lock the UI', async () => {
    ...
    act(() => {
      desktop.emitBootProgress({
        error: 'Could not reach the remote Panergos gateway while refreshing its WebSocket ticket. Try reconnecting.',
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Panergos couldn't start" overlay — the socket is open
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\Panergos')
    expect($currentCwd.get()).toBe('C:\\Panergos')
  })
  ...
  it('FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection', async () => {
    ...
    // SSH/HTTP remote whose transport dropped. startPanergos() rejects with a
  ...
  it('RETRY CONTRACT: a post-connect failure stays terminal even when its socket closes before boot catches it — a closed socket after a good dial is not a dial failure', async () => {
    ...
    const refreshPanergosConfig = vi.fn(async () => {
    ...
    expect(refreshPanergosConfig).toHaveBeenCalledTimes(1)
    expect($desktopBoot.get().error).toBeTruthy()
    expect(desktop.getConnection).toHaveBeenCalledTimes(1)
    await advanceBackoff()
    expect(desktop.getConnection).toHaveBeenCalledTimes(1)
