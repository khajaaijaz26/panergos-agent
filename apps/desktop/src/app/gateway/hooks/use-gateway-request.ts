...
import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@panergos/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as PanergosGateway | null
  const gatewayRef = useRef<PanergosGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.panergosDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<PanergosGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as PanergosGateway | null
      }),
    []
  )
