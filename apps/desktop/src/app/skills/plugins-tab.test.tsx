...
describe('PluginsTab', () => {
  ...
  it('renders a unified package as ONE row with a Desktop switch and an Agent switch', () => {
    $pluginRecords.set({
      media: { id: 'media', name: 'Media Studio', kind: 'disk', status: 'loaded', packageName: 'media-studio-plugin' }
    })
    $agentPlugins.set([
      {
        description: '',
        key: 'media-studio-plugin',
  ...
  it('offers "Install here" for a desktop half whose agent half is not in the selected profile', async () => {
    $pluginRecords.set({
      media: {
        ...
        kind: 'disk',
        status: 'loaded',
        packageName: 'media-studio-plugin',
        packageOrigin: { repo: 'https://github.com/example-org/media-studio-plugin.git', sha: 'abc' }
      }
    })
    ...
    await waitFor(() => {
      expect($pluginInstallRequest.get()).toMatchObject({
        legacyHint: 'agent',
        profile: 'workbot',
        repo: 'https://github.com/example-org/media-studio-plugin.git',
        sha: 'abc'
      })
    })
  })
