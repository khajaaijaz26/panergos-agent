...
describe('mergePluginPackages', () => {
  it('shows a unified package as ONE row with both halves, never two rows', () => {
    const rows = mergePluginPackages(
      [desktop({ id: 'media', name: 'Media Studio', packageName: 'media-studio-plugin' })],
      [agent({ name: 'media-studio-plugin', has_desktop_half: true, description: 'Generate media.' })]
    )
    ...
    expect(rows[0].agent?.name).toBe('media-studio-plugin')
  })

  it('a desktop half whose agent half is absent from THIS profile offers the install-here affordance', () => {
    const rows = mergePluginPackages([desktop({ id: 'media', packageName: 'media-studio-plugin' })], [])
  ...
  it('standalone desktop plugins and agent-only packages keep one empty side; unified rows sort first', () => {
    const rows = mergePluginPackages(
      ...
      [agent({ name: 'agent-only' }), agent({ name: 'unified', has_desktop_half: true })]
    )
