...
import type { OfficialSkillInfo, SkillInfo, ToolsetInfo } from '@/types/panergos'
...
export function SkillsView({
  ...
  const registryBridge = window.panergosDesktop?.connections
  const rosterBridge = window.panergosDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Panergos (default)' : p.name,
      value: p.name
    }))
                ...
                <MasterDetail pane={skillEditorPane} resizeId="capabilities-split" split="wide">
                  ...
                  >
                    ...
                    {/* The built-in optional-skills catalog, below the
                        installed list: every official skill Panergos ships but
