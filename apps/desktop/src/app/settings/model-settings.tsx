...
import { DEFAULT_REASONING_EFFORT, REASONING_EFFORT_VALUES } from '@panergos/shared'
...
import { invalidatePanergosConfig, panergosConfigCacheWriter, usePanergosConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `panergos model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in panergos_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile }: ModelSettingsProps) {
  ...
  const { data: config } = usePanergosConfigRecord(scopeProfile)
  const setConfig = useMemo(() => panergosConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidatePanergosConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await savePanergosConfig(setNested({}, key, value), scopeProfile)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `panergos model` curation). Best-effort — fall through to the refreshed
  ...
  const recycleStaleBackend = useCallback(async () => {
    ...
    try {
      await window.panergosDesktop?.recycleBackend?.(scopeProfile)
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Panergos runs the flow for you.`}
          </p>
        )}
