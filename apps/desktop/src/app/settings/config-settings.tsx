...
import { getElevenLabsVoices, getPanergosConfigSchema, savePanergosConfig } from '@/panergos'
...
import type { ConfigFieldSchema, PanergosConfigRecord } from '@/types/panergos'

import { panergosConfigCacheWriter, usePanergosConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<PanergosConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = usePanergosConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => panergosConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['panergos-config-schema'] : ['panergos-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getPanergosConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `panergos config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<PanergosConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await savePanergosConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: PanergosConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: PanergosConfigRecord) => {
