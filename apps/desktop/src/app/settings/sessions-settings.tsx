...
import type { PanergosConfigRecord, SessionInfo } from '@/types/panergos'
...
function AutoArchiveSetting() {
  ...
  useEffect(() => {
    ...
    if (!window.panergosDesktop) {
      return
    }

    let alive = true

    void getPanergosConfigRecord()
      .then(record => {
        if (!alive) {
          return
        }
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        ...
        await savePanergosConfig({ sessions: { auto_archive: autoArchive, auto_archive_days: archiveDays } })
      } catch (err) {
...
// / Program Files), which buried any files Panergos wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.panergosDesktop` is
    ...
    const settings = window.panergosDesktop?.settings
