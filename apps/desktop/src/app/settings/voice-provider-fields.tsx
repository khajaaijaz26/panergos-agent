...
export function VoiceProviderFields({ section, providerKey }: { section: 'tts' | 'stt'; providerKey: string }) {
  ...
  const { data: schemaResponse } = useQuery({
    queryKey: ['panergos-config-schema'],
    queryFn: () => getPanergosConfigSchema(),
    staleTime: 5 * 60 * 1000
  })
  ...
  const [baseline, setBaseline] = useState<PanergosConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      void savePanergosConfig(diffConfig(baseline ?? {}, config))
        .then(() => {
          setBaseline(config)
          setPanergosConfigCache(config)
        })
