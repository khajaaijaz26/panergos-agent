...
import { savePanergosConfig } from '@/panergos'
import { notifyError } from '@/store/notifications'
import type { PanergosConfigRecord } from '@/types/panergos'

import { setPanergosConfigCache, usePanergosConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: PanergosConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = usePanergosConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<PanergosConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setPanergosConfigCache, so
    ...
    const timeout = window.setTimeout(() => {
      ...
      void savePanergosConfig(setNested({}, 'terminal.font_family', value))
        .then(result => {
          if (!result.ok) {
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="panergos-terminal-font-families"
          ...
          <datalist id="panergos-terminal-font-families">
