 ...
 * agent sources (local runtime plus any number of remote gateways
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.panergosDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.panergosDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `panergos update` to every eligible source; per-connection results
