...
import type { ToolsetConfig } from '@/types/panergos'
...
const getPanergosConfigSchema = vi.fn()
...
vi.mock('@/panergos', () => ({
  ...
  getPanergosConfigSchema: () => getPanergosConfigSchema(),
...
beforeEach(() => {
  ...
  getPanergosConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(savePanergosConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = savePanergosConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
  ...
  describe('readiness pills', () => {
    it('renders the server status instead of assuming keyless rows are Ready', async () => {
      // A remote row requiring sign-in and a never-installed local TTS both
      // have zero env vars — the old client heuristic pilled every such row
      // "Ready". The server status must win.
      getToolsetConfig.mockResolvedValue(
        config({
          providers: [
            {
