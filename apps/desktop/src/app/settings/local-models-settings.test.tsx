...
import * as panergos from '@/panergos'

const mocked = vi.mocked(panergos)
...
describe('BrowseSection', () => {
  ...
  it('searches HF after a pause and shows fit-priced files on demand', async () => {
    ...
    try {
      vi.mocked(panergos.searchHFModels).mockResolvedValue({
        hits: [{ downloads: 872724, gated: false, likes: 47, repo: 'unsloth/Qwen3.8-27B-GGUF', updated: '2026-08-18' }]
      })
      vi.mocked(panergos.listHFRepoFiles).mockResolvedValue({
        files: [
      ...
      expect(panergos.searchHFModels).not.toHaveBeenCalled()
      await act(async () => {
        await vi.advanceTimersByTimeAsync(400)
      })
      expect(panergos.searchHFModels).toHaveBeenCalledWith('qwen')
      ...
      vi.mocked(panergos.downloadBrowsedModel).mockResolvedValue({ job_id: 'j1', model_id: 'Qwen3.8-27B-Q4_K_M' })
      fireEvent.click(q4Btn)
      await act(async () => {
        await vi.runOnlyPendingTimersAsync()
      })
      expect(panergos.downloadBrowsedModel).toHaveBeenCalledWith('unsloth/Qwen3.8-27B-GGUF', ['Qwen3.8-27B-Q4_K_M.gguf'])
    } finally {
      vi.useRealTimers()
    }
  })
...
describe('added-by-you rows', () => {
  it('staged models outside the catalog get the full action set', async () => {
    vi.mocked(panergos.getLocalModelsStatus).mockResolvedValue({
      ...BASE_STATUS,
      loaded_models: { 'Qwen3-Coder-30B-Q5_K_M': 'loaded' },
      models: [{ id: 'Qwen3-Coder-30B-Q5_K_M', size_bytes: 25 * 2 ** 30, size_label: '25.0 GB' }],
      placement: {
    ...
    vi.mocked(panergos.getLocalCatalog).mockResolvedValue({ models: [] })

    renderPane()
    await screen.findByText('Qwen3-Coder-30B-Q5_K_M')
