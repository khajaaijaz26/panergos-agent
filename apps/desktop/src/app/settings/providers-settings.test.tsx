...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `panergos auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'acme' ? 'Acme ID' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
describe('ProvidersSettings', () => {
  ...
  it('uses the settings target for account reads, removal and sign-in', async () => {
    ...
    try {
      ...
      fireEvent.click(await screen.findByText('Acme ID'))
      expect(startManualProviderOAuth).toHaveBeenCalledWith('acme', 'beta')
      fireEvent.click(await screen.findByRole('button', { name: 'Remove Acme ID' }))
  ...
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove Acme ID' })
    await act(async () => {
    ...
    await waitFor(() => expect(disconnectOAuthProvider).toHaveBeenCalledWith('acme', undefined))
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          cli_command: 'panergos auth add qwen-oauth',
          disconnect_hint: "Use `panergos auth add qwen-oauth` or that provider's CLI to remove it.",
