...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: PanergosConfigRecord = { display: { theme: 'mono' } }
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // PANERGOS_QWEN_ shares the PANERGOS_ stem with other integrations.
      expect(providerGroup('PANERGOS_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
  ...
  describe('enumOptionsFor — backend selector dropdowns', () => {
    ...
    it('surfaces user-defined command-type TTS providers (canonical providers nesting + legacy)', () => {
      const withCustom: PanergosConfigRecord = {
        tts: {
          provider: 'neutts',
    ...
    it('never offers a built-in name as a command provider, even one absent from the dropdown list', () => {
      const shadowing: PanergosConfigRecord = {
        tts: {
          provider: 'edge',
          providers: {
  ...
  describe('sectionFieldEntries', () => {
    ...
    it('infers the field type from the config value when the schema omits the key', () => {
      const config: PanergosConfigRecord = { memory: { provider: '', memory_enabled: true, memory_char_limit: 2200 } }
    ...
    it('prefers the backend schema entry over inference when both exist', () => {
      ...
      const config: PanergosConfigRecord = { memory: { provider: 'honcho' } }
  ...
  describe('clearsEnabledToolsets', () => {
    it('flags a non-empty → empty transition', () => {
      const prev: PanergosConfigRecord = { toolsets: ['memory', 'terminal', 'web_search'] }
      const next: PanergosConfigRecord = { toolsets: [] }
    ...
    it('does not flag a non-empty → missing transition (deep-merge preserves the key)', () => {
      ...
      const prev: PanergosConfigRecord = { toolsets: ['memory'] }
      const next: PanergosConfigRecord = {}

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when at least one toolset remains', () => {
      const prev: PanergosConfigRecord = { toolsets: ['memory', 'terminal'] }
      const next: PanergosConfigRecord = { toolsets: ['memory'] }

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when the list was already empty', () => {
      const prev: PanergosConfigRecord = { toolsets: [] }
      const next: PanergosConfigRecord = { toolsets: [] }
    ...
    it('does not flag an unrelated edit that never touched toolsets', () => {
      const prev: PanergosConfigRecord = { model: 'a', toolsets: ['memory'] }
      const next: PanergosConfigRecord = { model: 'b', toolsets: ['memory'] }
  ...
  describe('diffConfig', () => {
    it('omits a top-level key the draft never touched', () => {
      ...
      // an agent set via `panergos config set` while the page sat open must not
      // come back in the patch just because it's still present in the draft.
      const baseline: PanergosConfigRecord = { fallback_providers: ['nara1'], timezone: 'UTC' }
      const draft: PanergosConfigRecord = { fallback_providers: ['nara1'], timezone: 'America/New_York' }
    ...
    it('includes a nested key only when it actually changed, leaving siblings out', () => {
      const baseline: PanergosConfigRecord = { display: { personality: 'default', show_reasoning: true } }
      const draft: PanergosConfigRecord = { display: { personality: 'default', show_reasoning: false } }
    ...
    it('sends a new key that was absent from the baseline', () => {
      const baseline: PanergosConfigRecord = {}
      const draft: PanergosConfigRecord = { timezone: 'UTC' }
    ...
    it('returns an empty object when the draft matches the baseline exactly', () => {
      const baseline: PanergosConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }
      const draft: PanergosConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }

      expect(diffConfig(baseline, draft)).toEqual({})
    })

    it('treats an array as a whole value, not diffed element by element', () => {
      const baseline: PanergosConfigRecord = { toolsets: ['memory', 'terminal'] }
      const draft: PanergosConfigRecord = { toolsets: ['memory'] }
