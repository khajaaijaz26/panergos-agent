...
const getPanergosConfigRecord = vi.fn()
const savePanergosConfig = vi.fn()
...
vi.mock('@/panergos', () => ({
  ...
  getPanergosConfigRecord: () => getPanergosConfigRecord(),
  savePanergosConfig: (config: unknown) => savePanergosConfig(config),
...
beforeEach(() => {
    ...
    providers: [
      {
        name: 'Acme',
        slug: 'acme',
        models: ['example-model', 'example-mini'],
        authenticated: true,
  ...
  setModelAssignment.mockResolvedValue({ ok: true, provider: 'acme', model: 'example-model' })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'acme', model: 'example-model' })
  setEnvVar.mockResolvedValue({ ok: true })
  getPanergosConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  savePanergosConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('writes the profile default speed (service_tier) as a sparse patch, never the cached snapshot', async () => {
    ...
    getPanergosConfigRecord.mockResolvedValue({
      agent: { reasoning_effort: 'medium', service_tier: 'normal' },
      auxiliary: { curator: { provider: 'auto', model: '', reasoning_effort: 'high' } }
    })
    await renderModelSettings()
    await waitFor(() => expect(getPanergosConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() => expect(savePanergosConfig).toHaveBeenCalledWith({ agent: { service_tier: 'fast' } }))
  })
...
describe('ModelSettings code-skew 503', () => {
  const skewError = new Error(
    'Error invoking remote method \'panergos:api\': Error: 503: {"detail":"Restart required: This process is running code from 08b4875f4a but the checkout on disk is now 48d2528066. The model picker would risk a stale-module crash — restart the Desktop-owned backend to load the new code (use Restart backend in Panergos Desktop, or quit and reopen the app)"}'
  )
  ...
  it('unwraps the stale-backend 503 instead of dumping IPC JSON', async () => {
    ...
    expect(screen.queryByText(/panergos:api/)).toBeNull()
  ...
  it('recycles the Desktop-owned backend and reloads the catalog', async () => {
    ...
    ;(window as unknown as { panergosDesktop: { recycleBackend: typeof recycleBackend } }).panergosDesktop = {
      recycleBackend
    }
