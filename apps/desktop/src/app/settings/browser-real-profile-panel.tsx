...
import { type ProfileScope, savePanergosConfigRecord } from '@/panergos'
 ...
 * the default browser's profile (cookies/logins) into a Panergos-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = usePanergosConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        ...
        await savePanergosConfigRecord({ browser: { use_real_profile: on } }, profile)
        notify({
          kind: 'info',
