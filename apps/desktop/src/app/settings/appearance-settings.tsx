...
function ResumeLastSessionSetting() {
  ...
  const configQuery = usePanergosConfigRecord()
  ...
  const update = (on: boolean) => {
    ...
    void savePanergosConfig(setNested({}, 'display.resume_last_session', on))
      .then(result => {
        if (!result.ok) {
...
// baseline; the shipped default is the 110% preset. Ids double as the percent
...
function MarketplaceThemeResults({
  ...
  const search = useQuery({
    ...
    queryFn: () => window.panergosDesktop?.themes?.searchMarketplace(debounced) ?? Promise.resolve([]),
