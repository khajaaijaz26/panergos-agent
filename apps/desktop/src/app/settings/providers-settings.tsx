...
import { disconnectOAuthProvider, listOAuthProviders } from '@/panergos'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.panergosDesktop?.terminal)
...
//      in panergos_cli/provider_catalog.py) — the SAME provider identity
//      `panergos model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Panergos can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Panergos can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Panergos never deletes creds another tool owns behind a silent API call.
