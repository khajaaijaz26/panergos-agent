...
import type { MessagingPlatformInfo } from '@/types/panergos'
...
describe('MessagingView setup-guide link', () => {
  ...
  it('opens a real docs URL through the validated external opener', async () => {
    const docsUrl = 'https://docs.example.com/messaging/teams'
...
describe('MessagingView Telegram quick setup', () => {
  it('runs the QR pairing to apply on the page scope and watches the backend restart', async () => {
    ...
    startTelegramOnboarding.mockResolvedValue({
      ...
      pairing_id: 'pair-1',
      qr_payload: 'tg://pair',
      suggested_username: 'panergos_bot'
    })
    ...
    try {
      ...
      expect(screen.getByText(/operator-chosen compatible onboarding service/)).toBeTruthy()
      expect(screen.getByText(/TELEGRAM_ONBOARDING_URL/)).toBeTruthy()

      await act(async () => {
        fireEvent.click(await screen.findByRole('button', { name: /Start external QR setup/ }))
      })
