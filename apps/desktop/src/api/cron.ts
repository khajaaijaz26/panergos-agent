...
import { connectionScoped, panergosApi, profileScoped, STARTUP_REQUEST_TIMEOUT_MS } from './client'
...
// Cron jobs are stored per-profile (<PANERGOS_HOME>/cron/jobs.json), and the
...
export async function getCronJobRuns(jobId: string, limit = 20): Promise<SessionInfo[]> {
  const { runs } = await panergosApi<{ runs: SessionInfo[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export async function getCronDeliveryTargets(): Promise<CronDeliveryTarget[]> {
  const { targets } = await panergosApi<{ targets: CronDeliveryTarget[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export function getAutomationBlueprints(): Promise<{ blueprints: AutomationBlueprint[] }> {
  return panergosApi<{ blueprints: AutomationBlueprint[] }>({
    ...profileScoped(),
    ...connectionScoped(),
