import { reconnectBackoffDelayMs } from '@panergos/shared'

import type { PanergosConnection } from '@/global'
import { RECONNECT_ATTEMPT_TIMEOUT_MS, withTimeout } from '@/lib/with-timeout'

import { getApiRequestConnection, getApiRequestProfile, panergosApi, profileScoped } from './client'
 ...
 *
 *  Exported for tests. */
export async function activeConnection(): Promise<PanergosConnection> {
  ...
  return withTimeout(
    window.panergosDesktop.getConnection(profile),
    RECONNECT_ATTEMPT_TIMEOUT_MS,
    `Timed out connecting to profile "${profile}"`
  )
}

/** Options for a plugin REST call — mirrors the app's own `panergosDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see PanergosApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.panergosDesktop?.api) {
    throw new Error('Panergos desktop bridge unavailable')
  }
