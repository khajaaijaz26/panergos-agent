...
export function getMemoryProviderConfig(provider: string, profile?: null | string): Promise<MemoryProviderConfig> {
  return panergosApi<MemoryProviderConfig>({
...
export function startMemoryProviderOAuth(
  ...
  return panergosApi<MemoryProviderOAuthStatus>({
    ...profileScoped(profile),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `panergos memory` / `panergos curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return panergosApi<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return panergosApi<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return panergosApi<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return panergosApi<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
export function updatePanergos(): Promise<ActionResponse> {
  return panergosApi<ActionResponse>({
    ...profileScoped(),
    path: '/api/panergos/update',
    method: 'POST'
  })
}
...
export function checkPanergosUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return panergosApi<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/panergos/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200, profile?: ProfileScope): Promise<ActionStatusResponse> {
  return window.panergosDesktop.api<ActionStatusResponse>({
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return panergosApi<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return panergosApi<AudioSpeakResponse>({
    ...profileScoped(),
...
export function setTtsLease(lease: string, active: boolean): Promise<AudioTtsLeaseResponse> {
  return panergosApi<AudioTtsLeaseResponse>({
    ...profileScoped(),
...
export function getElevenLabsVoices(profile?: null | string): Promise<ElevenLabsVoicesResponse> {
  return panergosApi<ElevenLabsVoicesResponse>({
...
export function getGhAuthStatus(refresh = false): Promise<{ available: boolean; authenticated: boolean }> {
  return panergosApi<{ available: boolean; authenticated: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `panergos doctor` / `panergos security
// audit` / `panergos backup` / `panergos debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return panergosApi<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return panergosApi<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return panergosApi<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
