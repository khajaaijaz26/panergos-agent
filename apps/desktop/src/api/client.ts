import { JsonRpcGatewayClient } from '@panergos/shared'

import type { PanergosApiRequest } from '@/global'
...
// "Timed out connecting to Panergos backend" that hangs the UI (#48504).
//
...
export class PanergosGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Panergos gateway connection closed',
      connectErrorMessage: 'Could not connect to Panergos gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Panergos gateway is not connected',
...
// process to route them to that host (panergos:api's registry branch). Null
...
export function panergosApi<T>(request: PanergosApiRequest): Promise<T> {
  return window.panergosDesktop.api<T>({ ...connectionScoped(), ...request })
}
