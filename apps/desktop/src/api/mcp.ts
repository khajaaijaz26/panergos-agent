import type { McpCatalogResponse, McpServerSummary } from '@/types/panergos'
...
export function testMcpServer(name: string, profile?: ProfileScope): Promise<McpTestResult> {
  return window.panergosDesktop.api<McpTestResult>({
 ...
 *  `savePanergosConfig`, this REPLACES rather than deep-merges, so deletes,
...
// `panergos mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via savePanergosConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return panergosApi<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(
  ...
  return window.panergosDesktop.api<McpServerSummary>({
    ...capabilityScoped(profile),
    path: '/api/mcp/servers',
    method: 'POST',
    body
...
export function getMcpCatalog(profile?: ProfileScope): Promise<McpCatalogResponse> {
  return window.panergosDesktop.api<McpCatalogResponse>({
    ...capabilityScoped(profile),
...
export function installMcpCatalogEntry(
  ...
  return window.panergosDesktop.api<{ ok: boolean; name?: string; pid?: number; action?: string; background?: boolean }>({
    ...capabilityScoped(profile),
