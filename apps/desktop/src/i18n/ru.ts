...
export const ru = defineLocale({
  sessionImport: {
    ...
    subtitle: 'Перенесите разговор в Panergos и продолжите с того места, где остановились.',
    ...
    chooseHelp: 'Выберите сессию, чтобы прочитать историю перед импортом в Panergos.',
    ...
    you: 'Вы',
    snapshot: 'Этот разговор уже есть в Panergos. Откройте существующую копию, чтобы продолжить.',
    copyNotice:
      'Копируется текст разговора. Исходные файлы не меняются. Вывод инструментов и рассуждения не переносятся.',
    importing: 'Импорт…',
    open: 'Открыть в Panergos',
    continue: 'Продолжить в Panergos',
  ...
  boot: {
    ready: 'Panergos Desktop готов',
    desktopBootFailedWithMessage: message => `Не удалось запустить приложение: ${message}`,
    steps: {
      connectingGateway: 'Подключение к шлюзу',
      loadingSettings: 'Загрузка настроек Panergos',
      loadingSessions: 'Загрузка последних сеансов',
      retryingRemoteBackend: 'Переподключение к удалённому бэкенду Panergos…',
      startingDesktopConnection: 'Запуск подключения приложения',
      startingPanergosDesktop: 'Запуск Panergos Desktop…'
    },
    errors: {
      backgroundExited: 'Фоновый процесс Panergos завершён.',
      backgroundExitedDuringStartup: 'Фоновый процесс Panergos завершился при запуске.',
    ...
    failure: {
      title: 'Panergos не удалось запустить',
      description:
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Ваш бэкенд Panergos старше этой сборки приложения и может работать некорректно. Обновите их, чтобы они совпали.',
    installMethodUnsupportedTitle: 'Неподдерживаемый способ установки',
    updatePanergos: 'Обновить Panergos',
    ...
    errors: {
      ...
      methodNotAllowed:
        'Бэкенд приложения отклонил запрос (405 Method Not Allowed). Попробуйте перезапустить Panergos Desktop.',
    ...
    native: {
      ...
      inputBody: 'Panergos ожидает ваш ответ.',
      turnDoneTitle: 'Panergos завершил',
      turnDoneBody: '',
  ...
  settings: {
    ...
    resetConfirm: 'Сбросить все настройки к значениям Panergos по умолчанию?',
    ...
    notifications: {
      ...
      focusedHint: 'Оповещения о завершении срабатывают только пока Panergos в фоне.',
      kinds: {
        approval: {
        ...
        input: {
          label: 'Нужен ввод',
          description: 'Panergos задал вопрос или требует пароль/секрет.'
        },
        turnDone: {
          label: 'Ответ готов',
          description: 'Ход завершился, пока Panergos был в фоне.'
        },
        turnError: {
          label: 'Ход не удался',
        ...
        plugin: {
          ...
          description: 'Плагин приложения отправил уведомление, пока Panergos был в фоне.'
        }
      },
    ...
    appearance: {
      ...
      colorModeDesc: 'Выберите фиксированный режим или позвольте Panergos следовать настройкам системы.',
      ...
      reactionsDesc: 'Эмодзи-тапбеки в стиле iMessage — реагируйте на сообщения, и Panergos сможет реагировать на ваши.',
      ...
      pet: {
        title: 'Питомец',
        intro:
          'Заберите анимированного питомца из petdex, который парит над приложением и реагирует на действия Panergos — «бегает», пока выполняются инструменты, радуется успеху и хмурится при ошибках.',
        restartHint:
          'Питомцам нужен быстрый перезапуск — текущее приложение запустилось до появления этой функции. Выйдите из Panergos и откройте снова, затем вернитесь сюда.',
        on: 'Вкл',
        off: 'Выкл',
        scaleTitle: 'Размер',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      agent: {
        ...
        maxTurns: 'Верхний предел ходов с вызовами инструментов, после которого Panergos останавливает запуск.'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Максимальное число символов, которые Panergos может прочитать из одного запроса к файлу.',
      approvals: {
        mode: 'Как Panergos обрабатывает команды, требующие явного подтверждения.',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Когда Panergos обновляет себя из приложения (без запроса в терминале), сохранять локальные правки исходников (stash) или выбрасывать (discard). Обновления из терминала всегда спрашивают.'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Рантайм Panergos обновлён, но само приложение — ещё старая сборка: новые функции интерфейса (например, Bot Mode) не появятся до обновления. Запустите обновление ниже, чтобы пересобрать приложение. Если предупреждение не исчезнет, переустановите с последнего установщика.',
      ...
      automaticUpdatesDesc: 'Panergos автоматически проверяет обновления в фоне и сообщает, когда они готовы.',
    ...
    config: {
      ...
      loading: 'Загрузка конфигурации Panergos…',
    ...
    quickEntry: {
      ...
      enabledDesc:
        'Глобальным горячим ключом вызывайте маленький композер откуда угодно и отправляйте запрос, не открывая Panergos.',
    ...
    connections: {
      ...
      intro:
        'Управляйте этим устройством и всеми шлюзами Panergos, до которых можно дотянуться через удалённые или SSH-соединения.',
      stagedNote:
      ...
      kindSsh: 'SSH',
      kindLocalDesc: 'Рантайм Panergos, которым управляет это приложение.',
      kindRemoteDesc: 'Шлюз Panergos, доступный по HTTP(S) — LAN, Tailscale или интернет.',
      kindSshDesc: 'Установка Panergos, доступная по SSH.',
      labelTitle: 'Имя',
      labelDesc:
      ...
      headersDesc:
        'Отправляются с каждым HTTP- и WebSocket-запросом к этому шлюзу — например, для прокси доступа вроде Cloudflare Access (CF-Access-Client-Id / CF-Access-Client-Secret). Значения следуют настройке шифрования сохранённых секретов. Заголовки, которыми управляет Panergos (Authorization, Cookie, Host…), игнорируются.',
    ...
    gateway: {
      ...
      intro:
        'По умолчанию — локальный. Используйте удалённый, когда приложение должно управлять бэкендом Panergos в другом месте. Соединения шлюзов — на уровне машины; профили обнаруживаются из подключённых шлюзов.',
      envOverrideTitle: 'Переменные окружения управляют этой сессией приложения.',
      envOverrideDesc:
        'Сбросьте PANERGOS_DESKTOP_REMOTE_URL и PANERGOS_DESKTOP_REMOTE_TOKEN, чтобы использовать сохранённую настройку ниже.',
      modeTitle: 'Режим подключения',
      localTitle: 'Локальный шлюз',
      localDesc: 'Запускает приватный бэкенд Panergos на localhost. Это значение по умолчанию, работает офлайн.',
      remoteTitle: 'Удалённый шлюз',
      remoteDesc: 'Подключает это приложение к удалённому бэкенду Panergos.',
      remoteAuthHint:
      ...
      remoteUrlDesc: 'Базовый URL удалённого бэкенда дашборда. Поддерживаются префиксы пути, например /panergos.',
      ...
      restartingMessage: 'Panergos Desktop переподключится с сохранёнными настройками — оболочка останется открытой.',
      savedMessage: 'Сохранено для следующего перезапуска.',
      connectedTo: (baseUrl, version) => `Подключено к ${baseUrl}${version ? ` · Panergos ${version}` : ''}`,
      ...
      sshDesc:
        'Panergos запускается на удалённой машине по SSH и туннелируется в это приложение — ничего не нужно запускать или открывать самим. Требуется рабочая SSH-аутентификация по ключу на хост.',
      sshTrustHint:
      ...
      sshPanergosPathTitle: 'Путь к Panergos (необязательно)',
      sshPanergosPathDesc: 'Полный путь к бинарнику Panergos на удалённой машине. Пусто = автоопределение.',
      sshPanergosPathPlaceholder: 'автоопределение',
      ...
      sshReachable: (host, platform) => `Доступен: ${host} (${platform}) — Panergos найден`,
      ...
      sshErrAuth:
        'SSH-аутентификация не удалась. Загрузите ключ в ssh-agent (ssh-add) или задайте IdentityFile в ~/.ssh/config — Panergos запускает ssh в неинтерактивном режиме.',
      sshErrHostKey:
      ...
      sshErrNotInstalled:
        'Panergos не установлен на удалённой машине. Установите его там (curl -fsSL https://raw.githubusercontent.com/khajaaijaz26/panergos-agent/main/scripts/install.sh | sh) или задайте путь к Panergos.',
      sshErrPlatform:
        'Неподдерживаемая удалённая платформа. SSH-режим Panergos Desktop поддерживает удалённые хосты Linux, macOS и Windows.',
      sshErrTimeout: 'SSH-соединение истекло. Хост может быть недоступен или «спит».',
      sshErrUpdateRequired: 'Перед подключением через SSH обновите Panergos на удалённой машине.',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'Ссылка запросила добавить этот MCP-сервер в Panergos. Проверьте конфигурацию ниже — она пришла из ссылки, а не из Panergos.',
    ...
    providers: {
      ...
      intro:
        'Войдите по подписке — копировать API-ключ не нужно. Panergos проведёт вход в браузере прямо здесь, в приложении.',
      ...
      localEndpoint: {
        ...
        description: 'Направьте Panergos на любой OpenAI-совместимый эндпоинт (Zyphra, vLLM, llama.cpp, Ollama и т. д.).'
      },
  ...
  skills: {
    ...
    skillArchivedMessage: 'Восстановить через panergos curator restore.',
    hub: {
  ...
  starmap: {
    ...
    emptyDesc: 'По мере того как Panergos создаёт навыки и память для вашей работы, они появятся здесь.',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'Перезапустите Panergos, чтобы использовать питомцев — бэкенд старше этой функции.',
    ...
    generatePet: {
      ...
      staleBackend: 'Обновите Panergos, чтобы генерировать питомцев.',
      backgroundHint: 'Можно закрыть — Panergos уведомит, когда будет готово.',
    ...
    nav: {
      ...
      settings: { title: 'Настройки', detail: 'Настройка Panergos desktop' },
    ...
    panergosActiveSessions: (version, count) => `Panergos ${version} · Активные сеансы: ${count}`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle:
        'Для локальной настройки рекомендуется BotFather. QR-настройка требует внешнего сервиса, заданного оператором.',
      quickSetup: 'Необязательная внешняя QR-настройка',
      recommended: 'Внешний сервис',
      quickHelp:
        'Доступно только если TELEGRAM_ONBOARDING_URL указывает на выбранный оператором совместимый сервис подключения.',
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ID пользователя бота', placeholder: '@panergos:example.org' },
  ...
  profiles: {
    ...
    deleting: 'Удаление...',
    createDesc: 'Профили — это независимые среды Panergos: отдельные настройки, навыки и SOUL.md.',
    nameLabel: 'Имя',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      saveFailed: 'Panergos не сохранил это изменение модели.'
    },
    ...
    newCron: 'Новый cron',
    emptyDescNew:
      'Запланируйте промпт, который будет выполняться по cron-выражению. Panergos выполнит его и доставит результаты в выбранное вами место.',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'Обновите бэкенд Panergos, чтобы создавать проекты — ваш бэкенд старше этого desktop-приложения (Настройки → Обновления → Бэкенд).',
      deleteConfirm: 'Это удалит сохранённый проект из Panergos. Файлы, git-репозитории и worktrees не пострадают.',
      ...
      worktreeStaleBackend:
        'Обновите бэкенд Panergos, чтобы создавать worktrees по этому удалённому соединению — он старше git worktree API.',
  ...
  composer: {
    ...
    placeholderStarting: 'Запуск Panergos...',
    placeholderReconnecting: 'Переподключение к Panergos…',
    placeholderFollowUp: 'Отправить продолжение',
    newSessionPlaceholders: [
      'Что будем делать?',
      'Дайте Panergos задачу',
      'О чём вы думаете?',
      'Опишите, что нужно',
      'Что обсудим?',
    ...
    attachUrlDesc: 'Panergos загрузит страницу и добавит её как контекст для этого хода.',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Попросить Panergos открыть PR',
  ...
  updates: {
    stages: {
      ...
      pull: 'Почти готово…',
      pydeps: 'Завершаем…',
      update: 'Обновляем Panergos…',
      rebuild: 'Пересобираем desktop-приложение…',
      restart: 'Перезапускаем Panergos…',
    ...
    unsupportedMessage: 'Эта версия Panergos не может обновлять себя из приложения.',
    ...
    availableBody: 'Новая версия Panergos готова к установке.',
    availableTitleBackend: 'Доступно обновление бэкенда',
    availableBodyBackend: 'Новая версия подключённого бэкенда Panergos готова к установке.',
    ...
    manualBody:
      'Panergos установлен из командной строки, поэтому обновления тоже выполняются там. Вставьте это в терминал:',
    manualPickedUp: 'Panergos подхватит новую версию при следующем запуске.',
    guiSkewTitle: 'Обновите desktop-приложение',
    guiSkewBody:
      'Бэкенд обновлён, но пакет этого desktop-приложения не изменился. Обновите или переустановите desktop-приложение Panergos (ваш AppImage / .deb / .rpm), чтобы версии совпали.',
    copy: 'Копировать',
    copied: 'Скопировано',
    done: 'Готово',
    applyingBody:
      'Обновлятор Panergos возьмёт процесс на себя в своём окне и автоматически перезапустит Panergos, когда закончит. Пожалуйста, не открывайте Panergos вручную во время обновления.',
    applyingBodyBackend:
      'Удалённый бэкенд применяет обновление и перезапустится. Panergos переподключится автоматически, когда он снова заработает.',
    applyingClose: 'Это окно закроется во время обновления, затем Panergos откроется сам.',
    errorTitle: 'Обновление не завершилось',
    errorBody: 'Не переживайте — ничего не потеряно. Можно попробовать снова прямо сейчас.',
    blockerTitle: 'Закрыть локальные предпросмотры, чтобы обновить Panergos?',
    blockerBody:
      'Panergos нужно остановить эти локальные предпросмотры перед обновлением. Ваши файлы не будут изменены или удалены.',
    foreignBlockerTitle: 'Закрыть другие процессы, чтобы обновить Panergos',
    foreignBlockerBody:
      'Panergos не может безопасно закрыть эти процессы автоматически. Закройте приложение, терминал или службу, которой принадлежит каждый из них, и повторите обновление.',
    mixedBlockerBody:
      'Panergos может закрыть локальные предпросмотры, перечисленные ниже. Остальные процессы нужно закрыть вручную, прежде чем обновление сможет продолжиться.',
  ...
  guidedGreeting: {
    line: 'Заходите. Я Panergos. Дайте мне пару минут — обустрою тут всё под вас, а потом займёмся тем, что вам правда нужно.\n\nДля начала: как к вам обращаться?',
  ...
  install: {
    ...
    oneTimeTitle: 'Panergos требует одноразовой установки',
    ...
    setupChoiceTitle: 'Настройка Panergos Desktop',
    setupChoiceDesc:
      'Подключите это приложение к уже работающему шлюзу Panergos или установите Panergos локально на этот компьютер.',
    connectExistingTitle: 'Подключиться к существующему Panergos',
    ...
    installLocalTitle: 'Установить Panergos локально',
    installLocalDesc: 'Скачайте Panergos, создайте его Python-окружение и запустите бэкенд на этом компьютере.',
    localStartUnavailable: 'Не удалось начать локальную установку. Перезапустите Panergos Desktop и попробуйте снова.',
    remoteSetupTitle: 'Подключиться к существующему Panergos',
    remoteSetupDesc: 'Введите URL вашего шлюза. Panergos Desktop определит, нужен токен или вход через браузер.',
    remoteUrlTitle: 'URL шлюза',
    remoteUrlDesc: 'Используйте базовый URL шлюза Panergos, включая https:// для удалённых.',
    remoteUrlPlaceholder: 'https://gateway.example.com/panergos',
    probing: 'Определяем аутентификацию шлюза...',
    probeError: 'Не удалось подключиться к этому шлюзу Panergos.',
    ...
    settingUpTitle: 'Настройка Panergos Agent',
    finishingTitle: 'Завершаем',
    failedDesc:
      'Один из шагов установки завершился ошибкой. На Windows это может произойти, если запущена другая инстанция Panergos CLI или desktop. Остановите все работающие инстанции Panergos и повторите. Подробности — ниже или в журнале desktop.',
    activeDesc:
      'Это одноразовая настройка. Установщик Panergos скачивает зависимости и настраивает вашу машину. При следующих запусках этот шаг будет пропущен.',
  ...
  onboarding: {
    headerTitle: 'Настроим для вас Panergos Agent',
    headerDesc: 'Подключите провайдера модели, чтобы начать общение. Большинство вариантов — в один клик.',
    preparingInstall: 'Panergos завершает установку. Обычно это занимает меньше минуты при первом запуске.',
    starting: 'Запускаем Panergos…',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'self-hosted',
        description:
          'Укажите Panergos локальный или self-hosted OpenAI-совместимый endpoint (vLLM, llama.cpp, Ollama и т.д.).'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'Откроет страницу подтверждения в браузере — Panergos подключится автоматически',
    ...
    authorizeThere: 'Авторизуйте Panergos там.',
    ...
    autoBrowser: provider =>
      `Мы открыли ${provider} в вашем браузере. Авторизуйте Panergos там, и подключение произойдёт автоматически — ничего копировать и вставлять не нужно.`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} — ${size}. Panergos покажет только первые 512 КБ.`,
    ...
    web: {
      ...
      tryAgain: 'Попробовать снова',
      restarting: 'Panergos перезапускается...',
      askRestart: 'Попросить Panergos перезапустить сервер',
      lookingRestart: taskId => `Panergos ищет сервер предпросмотра для перезапуска (${taskId})`,
      restartingTitle: 'Перезапуск сервера предпросмотра',
      restartingMessage: 'Panergos работает в фоне. Следите за прогрессом в консоли предпросмотра.',
      ...
      blankPageBody: 'Введите адрес выше, чтобы просматривать, или попросите Panergos открыть страницу.',
      finishedRestarting: message => `Panergos завершил перезапуск сервера предпросмотра${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Panergos не смог перезапустить сервер.',
      stillWorking:
        'Panergos всё ещё работает, но результата перезапуска пока нет. Команда сервера может выполняться в foreground.',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Panergos загружает ответ',
    ...
    approval: {
      gatewayDisconnected: 'Шлюз Panergos не подключён',
      ...
      alwaysDescription: pattern =>
        `Это добавит паттерн «${pattern}» в ваш постоянный список разрешений (~/.panergos/config.yaml). Panergos больше не будет спрашивать о подобных командах — ни в этом сеансе, ни в будущем.`,
    ...
    mcpSetup: {
      ...
      catalogSource: 'Из отобранного каталога',
  ...
  prompts: {
    ...
    sudoDesc:
      'Panergos нужен ваш пароль sudo, чтобы выполнить команду с повышенными правами. Он отправляется только вашему локальному агенту.',
    sudoPlaceholder: 'пароль sudo',
    secretTitle: 'Требуется секрет',
    secretDesc: 'Panergos нужны учётные данные, чтобы продолжить.',
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Перезапустите Panergos Desktop, чтобы использовать «Сохранить изображение».',
    restartToSaveImages: 'Перезапустите Panergos Desktop, чтобы сохранять изображения',
    ...
    handoff: {
      ...
      timedOut: 'Превышено время ожидания шлюза. Выполняется ли `panergos gateway`?'
    }
  },
  errors: {
