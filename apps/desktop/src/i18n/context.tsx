import { applyDocumentLocale, isRecord } from '@panergos/shared/i18n'
import { createContext, type ReactNode, useCallback, useContext, useEffect, useMemo, useRef, useState } from 'react'

import { getPanergosConfigRecord, type PanergosConfigRecord, savePanergosConfig } from '@/panergos'
...
export interface I18nConfigClient {
  getConfig: () => Promise<PanergosConfigRecord>
  saveConfig: (config: PanergosConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.panergosDesktop?.api) {
    ...
    return getPanergosConfigRecord(undefined, { includeDefaults: false })
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.panergosDesktop?.api) {
    ...
    return savePanergosConfig(config, undefined, { preserveLanguage: true })
  }
}

export function getConfigDisplayLanguage(config: PanergosConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: PanergosConfigRecord, locale: Locale): PanergosConfigRecord {
    ...
    const loadLocale = () => {
      ...
      return configClient
        ...
        .then(async config => {
          ...
          const machineProfile = await window.panergosDesktop?.getMachineProfile?.().catch(() => null)
