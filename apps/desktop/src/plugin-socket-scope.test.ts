...
// (#73044). Exercises the REAL panergos + store/gateway + store/profile chain:
...
const { pluginSocket, setApiRequestConnection, setApiRequestProfile } = await import('@/panergos')
...
const conn = (over: Partial<PanergosConnection> = {}): PanergosConnection =>
  ({
    authMode: 'oauth',
