...
// Panergos is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.panergosMode
 ...
 * the only carriers of Panergos' palette — are dropped so the paste target's
