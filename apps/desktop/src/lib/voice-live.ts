...
/**
 * GPT-Live voice chat: the full-duplex voice frontend that DELEGATES to Panergos.
 *
 ...
 * that into an ordinary Panergos turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * Panergos keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (Panergos) for help. `context` is the recent
...
export async function fetchVoiceLiveStatus(): Promise<null | VoiceLiveStatus> {
  try {
    const response = await panergosApi<{ ok: boolean } & VoiceLiveStatus>({
      ...profileScoped(),
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by Panergos; late results for an
  ...
  async start(history: LiveHistoryMessage[]): Promise<void> {
    ...
    const response = await panergosApi<{
      ok: boolean
      session?: { id: string }
  ...
  /** Quiet progress for the live model ("Panergos is running the tests…"). */
