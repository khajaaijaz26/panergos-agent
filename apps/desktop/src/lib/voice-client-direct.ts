...
import { getApiRequestConnection, getApiRequestProfile, panergosApi } from '@/panergos'
...
export async function fetchVoiceClientConfig(): Promise<null | VoiceClientConfig> {
  ...
  const promise = (async () => {
    try {
      // panergosApi carries connectionScoped(); profileScoped() adds the
      // profile — the same routing every relay audio call uses, so the
      // config comes from the backend the user is actually talking to.
      const response = await panergosApi<{ ok: boolean } & VoiceClientConfig>({
        ...profileScoped(),
