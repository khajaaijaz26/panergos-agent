/**
 * Shared resolver for in-app `panergos://` deep links and `host.navigate('/path?…')` targets.
 *
 ...
 *  - `panergos://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `panergos://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type PanergosOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const SUPPORTED_PROTOCOLS = ['panergos:'] as const
...
export function normalizePanergosOpenString(raw: string): string | null {
  ...
  if (SUPPORTED_PROTOCOLS.some(protocol => trimmed.startsWith(`${protocol}//`))) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // panergos://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // panergos://index-network/intent/1 → /index-network/intent/1
...
export function resolvePanergosOpenPath(target: PanergosOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizePanergosOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizePanergosOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizePanergosOpenString(target.path)
 ...
 * (`panergos://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromPanergosDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolvePanergosOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolvePanergosOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `panergos://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromPanergosDeepLink('open', name, params)
}
