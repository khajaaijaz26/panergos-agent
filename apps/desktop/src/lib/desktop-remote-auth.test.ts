...
describe('deriveRemoteAuthProviderShape', () => {
  ...
  it('keeps OAuth copy for redirect providers and mixed deployments', () => {
    expect(
      deriveRemoteAuthProviderShape([{ name: 'acme', displayName: 'Acme ID', supportsPassword: false }])
    ).toEqual({
      isPassword: false,
    ...
    expect(
      deriveRemoteAuthProviderShape([
        ...
        { name: 'acme', displayName: 'Acme ID', supportsPassword: false }
      ])
    ).toEqual({
      isPassword: false,
      providerLabel: 'Username & Password / Acme ID'
    })
  })
