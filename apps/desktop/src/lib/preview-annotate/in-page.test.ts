...
afterEach(() => {
  ...
  document.querySelectorAll('panergos-annotate').forEach(node => node.remove())
})
...
describe('annotateInPage overlay', () => {
  it('installs a host and tears it down completely', () => {
    ...
    expect(document.querySelector('panergos-annotate')).toBeTruthy()

    api.teardown()

    expect(api.isInstalled()).toBe(false)
    expect(document.querySelector('panergos-annotate')).toBeNull()
  })
  ...
  it('keeps a pin glued to its element after the element moves', () => {
    ...
    const pin = () =>
      document.querySelector('panergos-annotate')!.shadowRoot!.querySelector('[data-annotate-pin="1"]') as HTMLElement
  ...
  it('keeps the picked element when the draft becomes a pin, even if the selector is wrong', () => {
    ...
    const api = annotateInPage(document)
    api.install()
    const host = document.querySelector('panergos-annotate') as HTMLElement
  ...
  it('shows the draft marker before a capture can photograph it', async () => {
    ...
    const shadow = document.querySelector('panergos-annotate')!.shadowRoot!
