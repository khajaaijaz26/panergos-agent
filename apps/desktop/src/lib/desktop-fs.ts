import { panergosApi } from '@/api/client'
import type {
  PanergosConnection,
  PanergosReadDirResult,
  PanergosReadFileTextResult,
  PanergosSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: PanergosConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: PanergosConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<PanergosReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<PanergosReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<PanergosReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<PanergosReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.panergosDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: PanergosSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
