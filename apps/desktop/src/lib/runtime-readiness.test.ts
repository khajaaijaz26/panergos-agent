...
describe('fetchRuntimeReadinessSignals', () => {
  it('scopes setup.runtime_check to the requested provider', async () => {
    ...
    await fetchRuntimeReadinessSignals(requestGateway, 'acme')

    expect(calls).toEqual([{ method: 'setup.status' }, { method: 'setup.runtime_check', params: { provider: 'acme' } }])
  })
})
...
describe('evaluateRuntimeReadiness', () => {
  it('forwards requestedProvider to setup.runtime_check', async () => {
    ...
    const result = await evaluateRuntimeReadiness(requestGateway, { requestedProvider: 'acme' })
