...
const UPDATE_BACKEND = 'Update the Panergos backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.panergosDesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Panergos Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.panergosDesktop.openExternal(authUrl)
    let pollFailures = 0
