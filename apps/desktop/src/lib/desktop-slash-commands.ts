...
/** Why a known Panergos command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Panergos profile', surface: action('profile') },
  {
    name: '/skin',
 ...
 * `panergos_cli/commands.py::desktop_surface_registry` by
...
function isKnownPanergosSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Panergos' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownPanergosSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Panergos and were never asked for. It is
