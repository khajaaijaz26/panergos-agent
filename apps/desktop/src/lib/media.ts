...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.panergosDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.panergosDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.panergosDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.panergosDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaGatewayStreamUrl(path: string): string {
  ...
  if (isRemoteGateway()) {
    ...
    return `panergos-media://remote/${file}${scope ? `?${scope}` : ''}`
  }
...
export function mediaStreamUrl(path: string): string {
  return `panergos-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.panergos/cache, etc.); /api/media is intentionally
...
export async function downloadGatewayMediaFile(
  ...
  if (!window.panergosDesktop?.saveGatewayFile) {
    throw new Error('Desktop file download bridge is unavailable')
  }

  return window.panergosDesktop.saveGatewayFile({
