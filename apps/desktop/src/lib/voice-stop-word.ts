...
//
// When someone is in a hands-free "Hey Panergos" voice chat, the natural way to
...
// phrase (optionally addressed to Panergos), so a real turn that merely contains
...
// Optional address prefixes so "panergos stop" / "ok stop" / "hey panergos, stop" still count.
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // A bare address ("panergos") is not a stop command on its own.
      continue
    }
 ...
 * to Panergos). Returns false for anything that merely contains "stop" as part of
