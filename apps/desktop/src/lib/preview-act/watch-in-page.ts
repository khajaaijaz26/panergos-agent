...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __panergosWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__panergosWatch || (win.__panergosWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('panergos-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('panergos-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __panergosWatchTag?: number }).__panergosWatchTag
