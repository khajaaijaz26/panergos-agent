 ...
 *    a `PanergosPlugin` registers automatically (vite glob — drop a folder in).
 *    `panergos-bots` (Bot Mode) ships in-tree and is ON by default. `.js`
 *    entries are SDK-consumer plugins adopted from standalone
 *    repos — they keep the plain-ESM plugin.js form so the file stays
 *    loadable by older desktops' runtime door too.
 *  - RUNTIME: the on-disk doors (`<panergos home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<panergos home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: PanergosPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default PanergosPlugin export — skipped`)
