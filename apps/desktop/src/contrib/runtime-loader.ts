 ...
 *   (`@panergos/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default PanergosPlugin -> register(ctx)
 *
 ...
 * and the two on-disk doors — `<panergos home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<panergos home>/plugins/<name>/desktop/
...
import { createPluginContext, type PanergosPlugin } from './plugin'
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.panergos/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@panergos/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @panergos/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: PanergosPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default PanergosPlugin export`)
    }

    // A disk/runtime copy of a plugin that now ships BUNDLED (e.g. a
    // standalone install of panergos-bots predating its adoption in-tree) must
...
// ---------------------------------------------------------------------------
// The on-disk plugin door — ONE app-level root, `<panergos home>/desktop-plugins/`:
//  - `<id>/plugin.js` — a standalone desktop plugin (agent- or user-written);
//  - `<package>/plugin.js` + `.panergos-package.json` — the desktop HALF of a
 ...
 *  backend's panergos_home — #66899). Resolving it also runs Electron's
 *  reconcile, so unified packages' desktop halves are current before we scan. */
async function diskRoots(): Promise<DiskRoot[]> {
  const root = await window.panergosDesktop?.desktopPluginsRoot?.()
 ...
 *  agent row. */
const PACKAGE_MARKER = '.panergos-package.json'
...
async function readPackageMarker(desktop: Window['panergosDesktop'], folder: string): Promise<null | PackageMarker> {
  try {
...
async function readPluginSourceText(file: string): Promise<string> {
  ...
  if (result.truncated) {
    throw new PluginSourceOversizeError(
      "plugin.js exceeds this shell's 512 KiB read limit — update Panergos Desktop to load larger plugins"
    )
  }
...
async function resolveDiskPluginEntry(
  desktop: Window['panergosDesktop'],
  folderPath: string,
