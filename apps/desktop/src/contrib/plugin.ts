 ...
 * `PanergosPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/panergos'
...
export type { PluginRestOptions } from '@/panergos'
 ...
 *  under `panergos.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.panergosDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Panergos — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `panergos://index-network/intent/1`),
...
export interface PanergosPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `panergos.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.panergosDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.panergosDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.panergosDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.panergosDesktop
