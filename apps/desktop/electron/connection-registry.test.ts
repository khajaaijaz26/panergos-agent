...
test('primary SSH reuse rejects a descriptor with a different remote Panergos path', async () => {
  const registry = migrateV1ToRegistry({
    mode: 'ssh',
    remote: { mode: 'ssh', host: 'build-host', remotePanergosPath: '/srv/panergos', user: 'alice' },
    profiles: {}
  })
    ...
    await reuseMatchingPrimarySshBackend({
      ...
      ensurePrimary: async () => ({
        ...
        ssh: {
          effectiveConfigFingerprint: 'same-effective-config',
          host: 'build-host',
          remotePanergosPath: '/opt/panergos',
          remoteProfile: '',
          user: 'alice'
        }
      }),
...
test('registry primary reuses a matching primary backend descriptor', () => {
  const registry = normalizeRegistry({
    ...
    primary: 'panergos-vps',
    launchMode: 'primary',
    lastUsed: 'panergos-vps',
    connections: [
      { id: LOCAL_CONNECTION_ID, kind: 'local', label: 'This device' },
      { id: 'panergos-vps', kind: 'ssh', label: 'Panergos VPS', host: 'panergos-vps' }
    ]
  })

  const descriptor = {
    connectionId: 'panergos-vps',
    mode: 'remote' as const,
    remoteKind: 'ssh' as const,
    ssh: { host: 'panergos-vps' }
  }

  assert.equal(registrySourceOwnsPrimaryBackend(registry, 'panergos-vps', descriptor), true)
...
test('resolvedConnectionId reuses the exact URL envelope and rejects weak or duplicate matches', () => {
  ...
  assert.equal(
    resolvedConnectionId(registry, { baseUrl: sharedUrl, mode: 'remote', remoteKind: 'retired' } as any),
    null
  )
...
test('resolvedConnectionId keeps same-host SSH routes distinct by port, key, path, and profile', () => {
  const base = {
    ...
    keyPath: '/keys/a',
    kind: 'ssh' as const,
    remotePanergosPath: '/srv/panergos',
    remoteProfile: 'alpha',
    user: 'root'
  }
  ...
  const registry: ConnectionRegistry = {
    ...
    connections: [
      ...
      { ...base, id: 'ssh-path', label: 'SSH path', remotePanergosPath: '/opt/panergos' },
  ...
  assert.equal(resolve({ ...base, remotePanergosPath: '/opt/panergos' }), 'ssh-path')
...
// The electron and @panergos/shared implementations MUST stay byte-identical —
...
test('backendScopeKey: electron and shared implementations agree everywhere', async () => {
  ...
  const shared = (await import(String('@panergos/shared'))) as {
...
test('roster: source profile metadata follows the connection-qualified row', () => {
  ...
  const vpsMeta = {
    display_name: 'Emma',
    ui_meta: { 'panergos-bots': { title: 'Emma', shape: 'blobatar::sun', color: '#8b5cf6' } },
    has_avatar: true
  }
...
test('update fan-out accepts every supported connection kind', () => {
...
test('save rejects the retired hosted connection kind', () => {
  assert.throws(
    () =>
      normalizeConnectionInput(
        { kind: 'retired', label: 'Retired route', url: 'https://retired.invalid' } as any,
        emptyRegistry()
      ),
...
test('merge preserves SSH fields the editor does not carry', () => {
  const ssh = {
    host: 'homelab.lan',
    id: 's',
    keyPath: '/k/id',
  ...
  assert.equal(labelOnly.remotePanergosPath, '/opt/panergos')
...
test('duplicate remote gateway URLs are rejected', () => {
  let registry = emptyRegistry()
  registry = upsertConnection(
    registry,
    normalizeConnectionInput({ kind: 'remote', label: 'Homelab', url: 'http://10.0.0.5:9119' }, registry)
  ...
  // Same URL modulo trailing slash is a duplicate.
  assert.throws(
...
test('normalizeRegistry round-trips supported rows and quarantines retired hosted rows', () => {
  const input = {
    version: 2,
    primary: 'homelab',
  ...
  assert.equal(registry.quarantined?.[0].reason, 'entry-unrecognized-kind')
})
...
test('migrate: retired hosted mode is ignored', () => {
  const registry = migrateV1ToRegistry({
    mode: 'retired',
    remote: { url: 'https://retired.invalid', authMode: 'oauth' }
  })

  assert.equal(registry.primary, LOCAL_CONNECTION_ID)
  assert.deepEqual(registry.connections.map(connection => connection.kind), ['local'])
})
...
test('Apply remote preserves an existing URL identity and label without duplicates', () => {
  ...
  assert.equal(matches[0].id, 'panergos-alex')
  assert.equal(matches[0].label, 'Existing gateway')
  assert.equal(matches[0].authMode, 'oauth')
  assert.equal(applied.primary, 'panergos-alex')
  assert.equal(applied.lastUsed, 'panergos-alex')
})
...
test('normalizeConnectionInput keeps filtered headers on remote and drops them elsewhere', () => {
  const registry = emptyRegistry()
