...
async function fixture(beforeRefreshResponse?: () => Promise<void>) {
  const home = mkdtempSync(join(tmpdir(), 'panergos-auth-recovery-'))
  ...
  const server = createServer(async (req, res) => {
    ...
    if (req.url === '/auth/native/refresh') {
      state.refreshes++
      expect(JSON.parse(body)).toMatchObject({ refresh_token: 'old-rt', provider: 'acme' })
...
test('HTTP outage, dead-refresh and cookie coexistence reach the correct ticket/readiness/media verdicts', async () => {
  ...
  try {
    ...
    try {
      await waitForPanergosReady(f.baseUrl, {
    ...
    const mediaRequest = { url: 'panergos-media://remote/%2Ftmp%2Fclip.mp4', headers: new Headers(), method: 'GET' }
