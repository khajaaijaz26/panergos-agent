 ...
 *
 * The staged Tauri `panergos-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `panergos update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `panergos update`, swaps/relaunches
 * the app, and writes .panergos-update-result.json. With the app gone before
 * the update starts, the PANERGOS_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `panergos update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into PANERGOS_HOME on *every* platform
 * (`panergos-setup.exe` on Windows, `panergos-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_panergos_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `panergos update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(panergosHome, 'panergos-setup.exe')
 ...
 *
 * `copy_self_to_panergos_home` deliberately no-ops during `--update`
 ...
 * "Another Panergos update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `panergos update`, which
 ...
 * the installer is written to PANERGOS_HOME at install/repair time, making mtime
