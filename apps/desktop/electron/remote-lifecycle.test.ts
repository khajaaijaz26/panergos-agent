...
function ownedLock(over: any = {}) {
  return {
    ...
    port: 40000,
    profile: '',
    panergosPath: '~/.local/bin/panergos',
    panergosHome: '~/.panergos',
...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.panergos-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing Panergos', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('PANERGOS_HOME')) {
          return '/home/alice/.panergos\n'
        }

        if (command.includes('.panergos-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.panergos/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.panergos-update-in-progress/)
})
...
test('listRemotePanergosProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/PANERGOS_HOME/, '/Users/zillajr/.panergos\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemotePanergosProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemotePanergosProfiles rejects a hostile PANERGOS_HOME', async () => {
  const ssh = fakeSsh([[/PANERGOS_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemotePanergosProfiles(ssh),
    (err: any) => {
...
test('locatePanergos prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/panergos/, 'OK']])
  assert.equal(await locatePanergos(ssh, '/opt/panergos'), '/opt/panergos')
})

test('locatePanergos throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong panergos" bug.
  const ssh = fakeSsh([
    [/command -v panergos/, '/home/u/.local/bin/panergos\n'],
    [/\[ -x .*\.local\/bin\/panergos/, 'OK']
  ])

  await assert.rejects(
    () => locatePanergos(ssh, '/bad/path/panergos'),
    (err: any) => {
      assert.equal(err.kind, 'panergos-not-found')
      assert.match(err.message, /\/bad\/path\/panergos/)
...
test('locatePanergos falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v panergos/, '/home/u/.local/bin/panergos\n'],
    [/\[ -x .*\.local\/bin\/panergos/, 'OK']
  ])

  assert.equal(await locatePanergos(ssh, ''), '/home/u/.local/bin/panergos')
})

test('locatePanergos preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$PANERGOS_BIN" "$PANERGOS_ENTRYPOINT" "$@",
  // where $PANERGOS_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.panergos/panergos-agent/venv/bin/python\n']
  ])

  assert.equal(await locatePanergos(ssh, ''), '/home/u/.local/bin/panergos')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locatePanergos must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locatePanergos returns an explicit remotePanergosPath unchanged', async () => {
  // The override half of #74411: an explicit remotePanergosPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/panergos changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/panergos/, 'OK'],
    [/python3 -c/, '/home/u/.panergos/panergos-agent/venv/bin/python\n']
  ])

  assert.equal(await locatePanergos(ssh, '~/.local/bin/panergos'), '~/.local/bin/panergos')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remotePanergosPath must never be rewritten')
})

test('locatePanergos falls back to ~/.local/bin/panergos when the login-shell probe misses', async () => {
  ...
  assert.equal(await locatePanergos(ssh, ''), '~/.local/bin/panergos')
})

test('locatePanergos tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/panergos/, 'OK']])
  assert.equal(await locatePanergos(ssh, ''), '~/.panergos/panergos-agent/venv/bin/panergos')
})

test('locatePanergos throws a panergos-not-found error with an install hint', async () => {
...
test('locatePanergos uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v panergos/, '/x/panergos'],
    [/\[ -x/, 'OK']
  ])

  await locatePanergos(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.panergos/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.panergos/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.panergos/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/panergos'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/panergos serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/panergos'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/panergos', '/Users/cd9c/.panergos', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /panergos-agent.*venv.*bin.*panergos/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'panergos wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'panergos')
    const launcher = path.join(temp, 'panergos launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/panergos-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/panergos', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('PANERGOS_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/panergos', 'work', {
    panergosHome: '~/.panergos',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.ok(cmd.includes('.panergos-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"panergos-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /panergos-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'panergos-update-mutex-'))
  const panergosPath = path.join(directory, 'panergos')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *panergos-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(panergosPath, '', {
      panergosHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { panergosPath: '/x/panergos', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('PANERGOS_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('PANERGOS_DASHBOARD_READY port=8765')?.[1], '8765')
  // The remote log is `>> log 2>&1`, so a stderr chunk without a newline can be spliced onto the sentinel.
  assert.equal(READY_RE.exec('INFO  Started server process [4711]PANERGOS_BACKEND_READY port=65238')?.[1], '65238')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { panergosPath: '/x/panergos', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nPANERGOS_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'PANERGOS_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'PANERGOS_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /\.panergos\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'Panergos Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'PANERGOS_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile panergosPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ panergosPath: '/old/stale/panergos', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remotePanergosPath: '/new/panergos', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'PANERGOS_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes panergosHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/PANERGOS_HOME/, '/home/alice/.panergos\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'PANERGOS_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"panergosHome":"\/home\/alice\/\.panergos"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'PANERGOS_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'PANERGOS_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/panergos'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.panergos/panergos-agent/venv/bin/panergos'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/panergos'))
  assert.doesNotThrow(() => validateRemotePath('~/.panergos/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/panergos'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('panergos'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/panergos'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/panergos\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/panergos\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/panergos\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/panergos', '/usr/`id`/panergos', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.panergos/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/panergos')
  assert.ok(result.includes('/usr/local/bin/panergos'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/panergos')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('PANERGOS_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/panergos', 'work', {
    tokenFilePath: `~/.panergos/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.panergos\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/panergos', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing Panergos', () => {
  const cmd = buildSpawnCommand('/x/panergos', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env PANERGOS_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { panergosPath: '/x/panergos', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { panergosPath: '/x/panergos', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.panergos/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.panergos/unrelated.log' }))
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/panergos'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/panergos'), false)
})
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), 'panergos-argv-shim-'))
    ...
    try {
      ...
      assert.equal(
        argv[2],
        `${fakeHome}/.panergos/.panergos-update-in-progress.mutex`,
      ...
      const base = `${fakeHome}/.panergos/desktop-ssh/${OWNERSHIP_ID}`
