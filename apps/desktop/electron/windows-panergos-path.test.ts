// Unit tests for the pure Windows `panergos` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvPanergosCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `panergos` shim shadows
//      the real panergos.cmd/panergos.exe.
...
//   3. resolveVenvPanergosCommand() — must probe the venv python via
//      canImportPanergosCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvPanergos: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvPanergos: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvPanergos: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvPanergos: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvPanergos: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvPanergos: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvPanergosCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportPanergosCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    panergosHome: '/fake/panergos-home',
...
test('resolveVenvPanergosCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvPanergosCommand('/root/venv/Scripts/panergos.exe', [], deps), null)
})

test('resolveVenvPanergosCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvPanergosCommand('/root/venv/Scripts/panergos.cmd', [], deps), null)
})

test('resolveVenvPanergosCommand: returns null when the basename is not a Panergos CLI alias', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvPanergosCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvPanergosCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvPanergosCommand('/root/venv/bin/panergos.exe', [], deps), null)
})

test('resolveVenvPanergosCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvPanergosCommand('/root/venv/Scripts/panergos.exe', [], deps), null)
})

test('resolveVenvPanergosCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportPanergosCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvPanergosCommand('/root/venv/Scripts/panergos.exe', ['serve'], deps)
...
test('resolveVenvPanergosCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvPanergosCommand('/root/venv/Scripts/panergos.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'panergos_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvPanergosCommand: accepts the canonical command case-insensitively', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvPanergosCommand('/root/venv/Scripts/PANERGOS.EXE', [], deps))
  assert.ok(resolveVenvPanergosCommand('/root/venv/SCRIPTS/panergos.exe', [], deps))
})
