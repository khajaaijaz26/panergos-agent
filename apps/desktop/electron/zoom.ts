 ...
 * but forgot to emit 'panergos:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('panergos:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `panergos:zoom:changed`, which never fired —
