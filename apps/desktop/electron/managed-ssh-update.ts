 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `panergos update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.panergosHome, 'Panergos home')
  const panergosPath = validateRemoteValue(target.panergosPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(panergosPath)

  const updateCommand =
    `env PANERGOS_HOME=${homeWord} ` +
    `PANERGOS_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'PANERGOS_UPDATE_ORIGIN_PROFILE=default ' +
    `PANERGOS_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `PANERGOS_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:PANERGOS_HOME=${psLiteral(home)}`,
    `$env:PANERGOS_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:PANERGOS_UPDATE_ORIGIN_PROFILE="default"',
    `$env:PANERGOS_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:PANERGOS_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:PANERGOS_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:PANERGOS_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:PANERGOS_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(panergosPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.panergos-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Panergos updated and every managed SSH profile is ready.'
        : restoreOk
