...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    panergosHome: 'C:\\Users\\andre\\.panergos',
    python: 'C:\\Users\\andre\\.panergos\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.panergos-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      panergosPath: 'C:\\Panergos\\panergos.exe',
      panergosHome: 'C:\\Users\\andre\\.panergos',
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command panergos.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          panergosHome: 'C:\\Users\\alice\\.panergos',
          panergosPath: 'C:\\Panergos\\panergos.exe',
          python: 'C:\\Panergos\\python.exe'
        })
      }

      if (script.includes('.panergos-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('panergos_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.panergos\\profiles\\research')
  assert.match(script, /\.panergos-update-in-progress/)
...
test('Windows probe validates Panergos and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $panergosHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Panergos is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Panergos is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Panergos's\\python.exe" }, 'inspect', [
    'C:\\x y\\panergos.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'panergos_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Panergos''s/)
  assert.match(script, /C:\\x y\\panergos\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { panergosPath: lock.panergosPath, panergosHome: lock.panergosHome }
