...
test('environment URL without its token keeps the existing error', () => {
  assert.throws(
    ...
    /PANERGOS_DESKTOP_REMOTE_TOKEN is not/
  )
})
...
test('profile SSH route fails closed when any dial field differs', () => {
  ...
  const variants = [
    { ...ssh, port: 2200 },
    { ...ssh, keyPath: '/keys/b' },
    { ...ssh, remotePanergosPath: '/opt/panergos' },
...
test('global SSH treats an omitted port as 22 and checks the primary route', () => {
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'ssh', remote: { mode: 'ssh', host: 'box.test', user: 'panergos' } },
    registry: registry('ssh-primary', [
      { id: 'ssh-primary', kind: 'ssh', label: 'SSH primary', host: 'box.test', user: 'panergos', port: 22 }
    ])
  })
...
test('retired hosted mode is ignored instead of promoted to generic remote', () => {
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'retired', remote: { url: 'https://hosted.example', authMode: 'oauth' } },
    registry: registry('local', [])
  })
...
test('profile remote wins over a registry-backed global SSH route', () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      mode: 'ssh',
      remote: { mode: 'ssh', host: 'global-box.test', user: 'panergos' },
      profiles: {
        worker: { mode: 'remote', url: 'https://worker.test', authMode: 'token', token: tokenA }
      }
    },
    ...
    registry: registry('global-ssh', [
      { id: 'global-ssh', kind: 'ssh', label: 'Global SSH', host: 'global-box.test', user: 'panergos' },
...
test('profile SSH wins over a different registry primary SSH route', () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      ...
      remote: { mode: 'ssh', host: 'global-box.test', user: 'panergos' },
      profiles: {
        worker: { mode: 'ssh', host: 'worker-box.test', user: 'panergos' }
      }
    },
    profile: 'worker',
    registry: registry('global-ssh', [
      { id: 'global-ssh', kind: 'ssh', label: 'Global SSH', host: 'global-box.test', user: 'panergos' },
      { id: 'worker-ssh', kind: 'ssh', label: 'Worker SSH', host: 'worker-box.test', user: 'panergos' }
    ])
  })
...
test('local route does not inherit an unrelated registry SSH connection', () => {
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'local' },
    registry: registry('local', [
      { id: 'unused-ssh', kind: 'ssh', label: 'Unused SSH', host: 'box.test', user: 'panergos' }
    ])
  })
...
// remote transport, or startPanergos() spawns a loopback `panergos serve` the
...
test('ignores a retired hosted registry primary when the v1 mode is local', () => {
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'local' },
    profile: null,
    registry: registry('retired-1', [
      {
        id: 'retired-1',
        kind: 'retired',
        label: 'Retired route',
