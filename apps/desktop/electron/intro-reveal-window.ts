...
export function createIntroRevealWindowController({
  ...
  function spawnIntroRevealWindow() {
    ...
    win.on('closed', () => {
      ...
      if (main && !main.isDestroyed()) {
        main.webContents.send('panergos:intro-reveal:closed')
      }
    })
  ...
  ipcMain.handle('panergos:intro-reveal:open', (_event, payload?: IntroRevealOpenPayload) => openIntroReveal(payload))
  ipcMain.handle('panergos:intro-reveal:close', (_event, payload?: IntroRevealClosePayload) => closeIntroReveal(payload))
  ipcMain.on('panergos:intro-reveal:ready', event => {
    if (event.sender === introRevealWindow?.webContents) {
      introRevealShow?.reveal()
    }
  })
  ipcMain.on('panergos:intro-reveal:skip', event => {
    const main = mainWindow()

    if (event.sender === introRevealWindow?.webContents && main && !main.isDestroyed()) {
      main.webContents.send('panergos:intro-reveal:skip')
    }
  })
