...
// registry, and the panergos:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // PANERGOS_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.PANERGOS_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Panergos' TUI into
    ...
    env.TERM_PROGRAM = 'Panergos'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a panergos/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from PANERGOS_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.PANERGOS_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `panergos:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('panergos:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('panergos:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('panergos:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('panergos:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('panergos:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('panergos:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
