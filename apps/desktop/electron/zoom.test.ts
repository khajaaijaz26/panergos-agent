...
test('storage key stays stable so persisted zoom survives upgrades', () => {
  assert.equal(ZOOM_STORAGE_KEY, 'panergos:desktop:zoomLevel')
})

test('default zoom matches the Appearance 110% preset', () => {
  assert.equal(ZOOM_STEP, 0.1)
  assert.equal(zoomLevelToPercent(DEFAULT_ZOOM_LEVEL), 110)
  assert.equal(DEFAULT_ZOOM_LEVEL, percentToZoomLevel(110))
})
...
// 'panergos:zoom:changed' events. applyZoomLevel is the single funnel every zoom
...
test('applyZoomLevel applies the level then notifies the renderer', () => {
  ...
  assert.deepEqual(wc.calls, [
    ['setZoomLevel', 3],
    ['send', 'panergos:zoom:changed', { level: 3, percent: zoomLevelToPercent(3) }]
  ])
})
...
test('applyZoomLevel clamps garbage before applying and notifying', () => {
  ...
  assert.deepEqual(wc.calls, [
    ...
    ['send', 'panergos:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) }]
  ])
})
