 ...
 * startPanergos() re-throw the cached error without re-attempting the connect —
...
export interface BackendStartFailureContext {
  /**
   * True when the boot that just failed was resolving/dialing a REMOTE primary
   * backend rather than spawning a local child.
   */
  attemptedRemote: boolean
}

/**
 * Whether a startPanergos() failure should latch into `backendStartFailure`.
 ...
 * Every subsequent `getConnection`/`api` call re-runs `startPanergos`, re-emits
