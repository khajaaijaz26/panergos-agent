...
//   PANERGOS_WIN_SSH_HOST   ssh alias/host of the Windows box
//   PANERGOS_WIN_SSH_USER   remote user
//   PANERGOS_WIN_SSH_PANERGOS absolute path to the remote panergos.exe under test
const liveHost = process.env.PANERGOS_WIN_SSH_HOST || ''
const liveUser = process.env.PANERGOS_WIN_SSH_USER || ''
const configuredPanergos = process.env.PANERGOS_WIN_SSH_PANERGOS || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Panergos-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredPanergos)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredPanergos.replace('panergos.exe', 'python.exe')}' -m panergos_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredPanergos.replace('panergos.exe', 'python.exe')
        const terminate = `& '${python}' -m panergos_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.panergosPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m panergos_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m panergos_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
