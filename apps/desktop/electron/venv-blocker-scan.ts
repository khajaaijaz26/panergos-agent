...
const SCAN_MODULE = 'panergos_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Panergos processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'panergos_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Panergos process is using this installation.',
    '',
...
export function formatProbeFailedMessage(error?: string): string {
  ...
  return (
    'Update aborted: Desktop could not verify the Panergos installation is free.' +
    timeoutDetail +
    '\n\n' +
    'Close other Panergos windows and terminals, then retry.  If the problem\n' +
    'persists, run `panergos update` in a terminal for detailed diagnostics.'
  )
}
