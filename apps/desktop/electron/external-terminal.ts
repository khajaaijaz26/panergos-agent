// Launching the Panergos TUI in the user's OWN terminal emulator.
//
...
// `panergos --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Panergos runtime is often a venv Python invoked
//     as `python -m panergos_cli.main`, not a `panergos` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Panergos
...
export function terminalScriptEnv(
  ...
  if (panergosHome) {
    out.PANERGOS_HOME = panergosHome
  }
