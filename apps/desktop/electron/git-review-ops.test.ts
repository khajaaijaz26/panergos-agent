...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'panergos-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'panergos-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Panergos Test'], { cwd: dir })
