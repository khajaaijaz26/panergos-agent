 ...
 *   1. PANERGOS_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\panergos\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect PANERGOS_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.PANERGOS_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'panergos', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'panergos', 'git', 'usr', 'bin', 'bash.exe'))
  }
