 ...
 *
 * The pre-update handoff kills Panergos-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `panergos`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Panergos-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Panergos and holds venv shims mapped).
 */
export function isPanergosOwnedVenvDaemon(
