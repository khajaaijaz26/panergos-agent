 ...
 *   - the browser sees only a minimal "return to Panergos" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Panergos.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('panergos:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('panergos:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('panergos:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
