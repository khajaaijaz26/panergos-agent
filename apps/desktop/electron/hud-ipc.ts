...
export function registerHudIpc({
  ...
  ipcMain.on('panergos:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('panergos:hud:windowing', event => {
  ...
  ipcMain.on('panergos:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('panergos:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('panergos:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('panergos:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('panergos:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('panergos:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('panergos:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('panergos:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('panergos:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('panergos:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('panergos:hud:close', async () => {
    closeHudWindow()
