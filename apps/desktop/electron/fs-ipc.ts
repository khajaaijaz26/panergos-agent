...
// hardening, PANERGOS_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('panergos:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('panergos:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('panergos:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('panergos:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<PANERGOS_HOME>/desktop-plugins`,
  // resolved from the main-process PANERGOS_HOME (see resolvePanergosHome) — NOT from
  // the connected backend. A remote backend reports its own `panergos_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    const base = profile && profile !== 'default' ? path.join(panergosHome, 'profiles', profile) : panergosHome
  ...
  async function desktopPluginsRoot(): Promise<string> {
    const root = await ensureDir(path.join(panergosHome, DESKTOP_PLUGINS_DIR))
    await migrateProfileScopedDesktopPlugins(panergosHome, root)
    await reconcileUnifiedDesktopHalves(panergosHome, root)

    return root
  }

  ipcMain.handle('panergos:fs:desktopPluginsRoot', async () => desktopPluginsRoot())
  ...
  ipcMain.handle('panergos:fs:reconcileDesktopPlugins', async () => {
    const root = await ensureDir(path.join(panergosHome, DESKTOP_PLUGINS_DIR))

    return reconcileUnifiedDesktopHalves(panergosHome, root)
  })

  // The LOCAL logs root (`<PANERGOS_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where PANERGOS_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('panergos:fs:logsRoot', async () => localPluginsRoot('logs'))

  ipcMain.handle('panergos:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('panergos:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('panergos:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('panergos:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('panergos:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
