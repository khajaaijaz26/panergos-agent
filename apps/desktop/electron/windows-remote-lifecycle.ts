...
const READY_RE = /^PANERGOS_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitPanergosPath = '') {
  const explicit = psLiteral(explicitPanergosPath)
  ...
  const script = [
    ...
    '$panergosHome=$env:PANERGOS_HOME',
    'if(-not $panergosHome){$panergosHome=Join-Path $env:LOCALAPPDATA "panergos"}',
    'Assert-NoReparse $panergosHome $true',
    '$candidate=[IO.Path]::Combine($panergosHome, "panergos-agent\\venv\\Scripts\\panergos.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "panergos-agent\\.venv\\Scripts\\panergos.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $panergosHome "panergos-agent\\venv\\Scripts\\panergos.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "panergos-agent\\.venv\\Scripts\\panergos.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command panergos.exe -ErrorAction SilentlyContinue',
    ...
    '$panergos=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$panergos=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $panergos){throw "Panergos is not installed on the remote Windows host."}',
    'Assert-NoReparse $panergos $false',
    'if($explicit -and $panergos -ne $explicit){throw "The configured Panergos path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($panergos), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;panergosHome=$panergosHome;panergosPath=$panergos;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(panergosHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class PanergosMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(panergosHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".panergos-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[PanergosMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, panergosHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(panergosHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitPanergosPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitPanergosPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'panergos_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'panergos_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'panergos_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.panergosHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".panergos-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};panergosPath=${psLiteral(reservation.panergosPath)};panergosHome=${psLiteral(reservation.panergosHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.panergosPath === 'string' &&
    typeof lock.panergosHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.panergosPath === runtime.panergosPath &&
    lock.panergosHome === runtime.panergosHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remotePanergosPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.panergosHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.panergosPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Panergos on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.panergosPath = inspection.path
  const panergosVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located panergos at ${runtime.panergosPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, panergosPath: runtime.panergosPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForPanergos(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Panergos SSH"', 'powershell.exe -NoLogo')
