// The standalone desktop-plugin root (`<PANERGOS_HOME>/desktop-plugins`) and the
...
//      is COPIED out as `<root>/<name>/` with a `.panergos-package.json` marker
...
export const PACKAGE_MARKER = '.panergos-package.json'
...
async function packageOrigin(packageDir: string): Promise<Pick<DesktopHalfMarker, 'catalogName' | 'repo' | 'sha'>> {
  try {
    const sidecar = JSON.parse(await fs.promises.readFile(path.join(packageDir, '.panergos-catalog.json'), 'utf8')) as {
      catalog_name?: string
      repo?: string
      sha?: string
    }
...
/** Every panergos home the app knows about locally: the default plus each profile. */
export async function localHomes(panergosHome: string): Promise<string[]> {
  const profiles = await listDirs(path.join(panergosHome, 'profiles'))

  return [panergosHome, ...profiles.map(name => path.join(panergosHome, 'profiles', name))]
}
...
export async function migrateProfileScopedDesktopPlugins(panergosHome: string, appRoot: string): Promise<string[]> {
  const moved: string[] = []

  for (const profile of await listDirs(path.join(panergosHome, 'profiles'))) {
    const scopedRoot = path.join(panergosHome, 'profiles', profile, DESKTOP_PLUGINS_DIR)
...
export async function reconcileUnifiedDesktopHalves(panergosHome: string, appRoot: string): Promise<string[]> {
  const touched: string[] = []
  const seen = new Set<string>()

  for (const home of await localHomes(panergosHome)) {
