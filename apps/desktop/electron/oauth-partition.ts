 ...
 * (`persist:panergos-remote-oauth`) — the jar was keyed on the auth *mode*, not
 ...
 * setup: one box, two dashboards) fought over the same `panergos_session*`
...
export const LEGACY_OAUTH_PARTITION = 'persist:panergos-remote-oauth'
...
export function resolveOauthPartition(requestUrl: unknown, opts: ResolveOauthPartitionOptions = {}): string {
  ...
  for (const entry of registry.connections) {
    ...
    // jar. Token entries never use cookies; the primary (and the v1 remote it
    // migrated from) keeps the
