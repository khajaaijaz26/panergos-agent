...
test('terminalScriptEnv drops PATH in any casing and keeps the rest', () => {
  ...
  assert.deepEqual(env, { PYTHONPATH: '/repo', PYTHONUTF8: '1', PANERGOS_HOME: '/home/b/.panergos' })
})
...
test('buildTerminalScript execs the resolved runtime with its env', () => {
  const script = buildTerminalScript({
    args: ['-m', 'panergos_cli.main', '--tui', '--resume', 'sess'],
    command: '/home/b/.panergos/panergos-agent/venv/bin/python',
    cwd: "/home/b/o'brien",
    env: { PYTHONPATH: '/home/b/.panergos/panergos-agent' },
    platform: 'darwin'
  })
  ...
  assert.equal(
    ...
    [
      ...
      `export PYTHONPATH='/home/b/.panergos/panergos-agent'`,
      `exec '/home/b/.panergos/panergos-agent/venv/bin/python' '-m' 'panergos_cli.main' '--tui' '--resume' 'sess'`,
      ''
    ].join('\n')
  )
})
...
test('buildTerminalScript emits a cmd script on Windows', () => {
  const script = buildTerminalScript({
    ...
    command: 'C:\\panergos\\venv\\Scripts\\panergos.exe',
    cwd: 'C:\\Users\\b',
    env: { PYTHONUTF8: '1' },
    platform: 'win32'
  })
  ...
  assert.deepEqual(script.split('\r\n'), [
    ...
    'cd /d "C:\\Users\\b"',
    'set "PYTHONUTF8=1"',
    '"C:\\panergos\\venv\\Scripts\\panergos.exe" "--tui" "--resume" "sess"',
    ''
  ])
})
