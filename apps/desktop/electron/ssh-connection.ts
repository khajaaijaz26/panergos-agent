...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(PANERGOS_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Panergos-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.panergos/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'panergos-desktop-ssh')
  }

  return path.join(os.homedir(), '.panergos', 'desktop-ssh')
}
