 ...
 * the three user-facing uninstall modes to the `panergos uninstall` CLI flags,
 ...
 *              `panergos uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `panergos uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `panergos uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `panergos` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m panergos_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `panergos_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'panergos_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Panergos.app/Contents/MacOS/Panergos  → …/Panergos.app
 *   Windows: …\Panergos\Panergos.exe                 → …\Panergos  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Panergos.app/Contents/MacOS/Panergos → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Panergos.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Panergos.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Panergos$/i.test(dir) || /[\\/]panergos-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/Panergos
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import panergos_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, panergosHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export PANERGOS_HOME=${q(panergosHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import panergos_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Panergos install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "PANERGOS_HOME=${String(panergosHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
