...
test('stagedUpdaterSupportsPrewrittenMarker rejects installers predating the self-adopt fix', () => {
  ...
  // refreshed because copy_self_to_panergos_home no-ops during --update.
  assert.equal(
    stagedUpdaterSupportsPrewrittenMarker('C:\\Panergos\\panergos-setup.exe', {
...
test('resolveStagedUpdaterBinary still returns a stale staged updater on Windows', () => {
  ...
  assert.equal(
    resolveStagedUpdaterBinary('C:\\Panergos', {
      fileExists: () => true,
      isWindows: true,
      stagedMtimeMs: () => MARKER_SELF_ADOPT_EPOCH_MS - 60 * DAY_MS
    }),
    path.join('C:\\Panergos', 'panergos-setup.exe')
  )
})
...
test('spawnUpdaterProcess hides the updater console and detaches the child on Windows', () => {
  ...
  const result = spawnUpdaterProcess(
    'panergos-setup.exe',
    ['--update', '--branch', 'main'],
    { cwd: 'C:\\Panergos', detached: true, stdio: 'ignore' },
    {
      isWindows: true,
  ...
  assert.deepEqual(calls, [
    {
      ...
      command: 'panergos-setup.exe',
      options: { cwd: 'C:\\Panergos', detached: true, stdio: 'ignore', windowsHide: true }
    }
  ])
})
...
test('resolveStagedUpdaterBinary hands Windows the staged installer it finds', () => {
  const home = 'C:\\Users\\panergos\\AppData\\Local\\panergos'
  const staged = path.join(home, 'panergos-setup.exe')
  const probed: string[] = []
...
test('resolveStagedUpdaterBinary returns null off Windows even when panergos-setup is staged (#74836)', () => {
  const home = '/Users/panergos/.panergos'
  let probes = 0

  const resolved = resolveStagedUpdaterBinary(home, {
    // The installer stages panergos-setup on macOS/Linux too, so "it exists" is
...
test('resolveStagedUpdaterBinary returns null on Windows when nothing is staged', () => {
  const resolved = resolveStagedUpdaterBinary('C:\\Users\\panergos\\AppData\\Local\\panergos', {
    fileExists: () => false,
    isWindows: true
  })
...
test('resolveUpdateScriptHandoff prefers the repo script on Windows when present', () => {
  const root = String.raw`C:\Users\panergos\AppData\Local\panergos\panergos-agent`
...
test('resolveUpdateScriptHandoff returns null when the checkout predates the script', () => {
  const handoff = resolveUpdateScriptHandoff(String.raw`C:\Users\panergos\AppData\Local\panergos\panergos-agent`, {
    isWindows: true,
    fileExists: () => false
  })
...
test('resolveUpdateScriptHandoff is Windows-only (POSIX updates in place)', () => {
  const handoff = resolveUpdateScriptHandoff('/home/panergos/.panergos/panergos-agent', {
    isWindows: false,
    fileExists: () => true
  })
...
test('resolvePosixScriptHandoff returns the bash recipe when the script exists', () => {
  const root = '/home/panergos/.panergos/panergos-agent'
...
test('resolvePosixScriptHandoff is null when the checkout predates the script', () => {
  const handoff = resolvePosixScriptHandoff('/home/panergos/.panergos/panergos-agent', {
    isWindows: false,
    fileExists: () => false
  })
...
test('resolvePosixScriptHandoff is null on Windows', () => {
  const handoff = resolvePosixScriptHandoff(String.raw`C:\Users\panergos\AppData\Local\panergos\panergos-agent`, {
    isWindows: true,
    fileExists: () => true
  })
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--no-sandbox',
    'panergos://open/session/abc',
    '--profile=work'
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'panergos://open/session/abc', '--profile=work'])
