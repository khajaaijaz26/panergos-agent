...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'panergos serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/panergos serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m panergos_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/panergos --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Panergos Runtime\\panergos.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('panergos chat --query serve'), false)
