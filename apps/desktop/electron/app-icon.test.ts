...
// Regression: a packaged app.asar can contain a TRUNCATED assets/icon.png
...
test('resolveAppIcon skips an existing but undecodable candidate', () => {
  ...
  const probe = (p: string) => {
    ...
    return p !== '/packaged/app.asar/assets/icon.png'
  }

  const picked = resolveAppIcon(
    ['/packaged/app.asar/assets/icon.png', '/packaged/app.asar.unpacked/assets/icon.png'],
    probe
  )

  assert.equal(picked, '/packaged/app.asar.unpacked/assets/icon.png')
  assert.deepEqual(probeCalls, [
    '/packaged/app.asar/assets/icon.png',
    '/packaged/app.asar.unpacked/assets/icon.png'
  ])
})
...
test('decodingFileProbe rejects a missing file', () => {
  const missing = path.join(os.tmpdir(), `panergos-icon-missing-${process.pid}.png`)
  assert.equal(decodingFileProbe(missing), false)
})

test('decodingFileProbe rejects an existing but empty (0-byte) file', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'panergos-icon-'))
  const empty = path.join(dir, 'icon.png')
...
test('decodingFileProbe rejects a directory', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'panergos-icon-dir-'))
...
test('appIconCandidates keeps the documented precedence ladder', () => {
  const mac = appIconCandidates({
    isWindows: false,
    appRoot: '/Applications/Panergos.app/Contents/Resources',
    unpackedPathFor: p => `${p}.unpacked`
  })

  assert.deepEqual(mac, [
    path.join('/Applications/Panergos.app/Contents/Resources', 'assets', 'icon.png'),
    path.join('/Applications/Panergos.app/Contents/Resources.unpacked', 'assets', 'icon.png')
  ])
  ...
  assert.equal(
    win.filter(c => c.endsWith('icon.png')).length,
    2,
    'both PNG rungs remain after the ico rungs'
  )
})
