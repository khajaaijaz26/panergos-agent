// Profile-delete routing logic for the `panergos:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_panergos_home() and
...
/** Parse a `panergos:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its PANERGOS_HOME.
 *
...
/**
 * Route the next `panergos:api` request away from the primary/window backend
 ...
 * ensure_panergos_home() recreates the directory the delete just removed.
 */
