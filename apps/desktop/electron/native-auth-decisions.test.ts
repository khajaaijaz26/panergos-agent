...
test('oauthGuardMayHardFail keeps the strict guard for oauth and mixed deployments', () => {
  assert.equal(oauthGuardMayHardFail([{ name: 'acme', supportsPassword: false }]), true)
  assert.equal(
    oauthGuardMayHardFail([
...
test('oauthGuardMayHardFail keeps the strict guard for string oauth providers', () => {
  assert.equal(oauthGuardMayHardFail(['acme'] as any), true)
  assert.equal(oauthGuardMayHardFail(['acme', 'basic'] as any), true)
})
