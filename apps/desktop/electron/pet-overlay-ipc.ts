...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('panergos:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('panergos:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('panergos:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('panergos:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('panergos:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('panergos:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('panergos:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('panergos:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('panergos:pet-overlay:control', payload)
  })
}
