 ...
 *     `X-Panergos-Session-Token` header; WS uses `?token=`.
...
// panergos_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - panergos_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - panergos_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     rotating and reuse-detected). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-panergos_session_at', '__Secure-panergos_session_at', 'panergos_session_at']
const RT_COOKIE_VARIANTS = ['__Host-panergos_session_rt', '__Secure-panergos_session_rt', 'panergos_session_rt']

// Keep this aligned with panergos_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['panergos', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  if (needsOauthLogin) {
    ...
    // change until the user signs in, so tag it the way startPanergos latches
  ...
  // statusCode (the fetch layer attaches err.statusCode). Auth semantics are
  // unchanged: 401/403 route to reauth, 5xx stays a transport failure, and
...
/** Which Panergos profile the remote SSH dashboard should actually run as.
...
function normalizeSshConfig(entry) {
  ...
  const remotePanergosPath = String(entry.remotePanergosPath || '').trim()

  if (remotePanergosPath) {
    out.remotePanergosPath = remotePanergosPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Panergos installation. Preserve an explicit mapping
  // when it is a valid Panergos profile identifier; otherwise fall back to the
 ...
 * override only with `mode: 'remote'` and a non-empty
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_panergos_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `PANERGOS_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process PANERGOS_HOME can belong to a different
 ...
 * `panergos:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://panergos.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `panergos:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a panergos session ACCESS-token cookie
