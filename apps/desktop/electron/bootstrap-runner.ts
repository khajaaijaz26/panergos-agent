 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Panergos Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.panergos-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(panergosHome) {
  return path.join(panergosHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.panergos/panergos-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(panergosHome) {
  if (!panergosHome) {
    return null
  }

  const candidate = path.join(panergosHome, 'panergos-agent', 'scripts', installScriptName())
...
function cachedScriptPath(panergosHome, commit) {
  return path.join(bootstrapCacheDir(panergosHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/khajaaijaz26/panergos-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(panergosHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(panergosHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, panergosHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass PANERGOS_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          PANERGOS_HOME: panergosHome || process.env.PANERGOS_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, panergosHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, panergosHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--panergos-home', panergosHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, panergosHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, panergosHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(panergosHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, panergosHome, emit })
    abortSignal?.throwIfAborted()
