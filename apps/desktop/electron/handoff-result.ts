 ...
 * console. It writes PANERGOS_HOME/.panergos-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Panergos within 30 minutes.
...
export function handoffResultPath(panergosHome: string): string {
  return path.join(panergosHome, '.panergos-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(panergosHome)
  let raw: string
