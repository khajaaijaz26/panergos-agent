  ...
  return {
    ...
    async reapOrphans(): Promise<number[]> {
      ...
      for (let i = 0; i < entries.length; i += 1) {
        ...
        // if it reaches reapOrphans (see main.ts startPanergos + #87295).
...
export function backendCommandMatches(command: unknown): boolean {
  return /(?:^|[\s/\\"])(?:panergos(?:\.exe)?|panergos_cli\.main|panergos_cli[/\\]main\.py)"?(?:\s+(?:--profile|-p)\s+\S+)?\s+(?:serve|dashboard)(?:\s|$)/i.test(
    String(command ?? '')
  )
}
