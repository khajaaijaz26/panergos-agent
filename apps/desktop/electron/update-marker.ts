...
export function markerPath(panergosHome) {
  return path.join(panergosHome, '.panergos-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(panergosHome)
  let raw
 ...
 *
 * The Tauri-based panergos-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(panergosHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(panergosHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Panergos to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(panergosHome, opts)
