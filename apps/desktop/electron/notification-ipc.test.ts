...
import type { PanergosNotification } from './notification-types'
...
it('returns native clicks and approval actions to the emitting window, not the primary', () => {
  ...
  const notify = host.handle.mock.calls[0][1] as (event: IpcMainInvokeEvent, payload: PanergosNotification) => boolean
  ...
  expect(source.webContents.send).toHaveBeenCalledWith('panergos:focus-session', payload.focusSessionId)
  host.shown[0].emit('action', { actionIndex: 1 }, undefined)
  expect(source.webContents.send).toHaveBeenCalledWith('panergos:notification-action', {
  ...
  expect(primary.webContents.send).not.toHaveBeenCalled()
  host.shown[0].emit('click')
  expect(primary.webContents.send).toHaveBeenCalledWith('panergos:focus-session', payload.focusSessionId)
})
...
it('delivers plugin callbacks to their source and falls back only for navigation after it closes', () => {
  ...
  expect(source.webContents.send).toHaveBeenCalledWith(
    'panergos:notification-activate',
