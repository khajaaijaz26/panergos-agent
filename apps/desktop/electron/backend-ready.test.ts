 ...
 * PANERGOS_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS, clamped to a 45s floor.
 */
...
test('honors a valid PANERGOS_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS override', () => {
  const env = { PANERGOS_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '120000' }
  assert.equal(resolvePortAnnounceTimeoutMs(env), 120_000)
})

test('clamps an override below the floor up to the 45s minimum', () => {
  const env = { PANERGOS_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '1000' }
  assert.equal(resolvePortAnnounceTimeoutMs(env), MIN_PORT_ANNOUNCE_TIMEOUT_MS)
})

test('rounds a fractional override', () => {
  const env = { PANERGOS_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '60000.7' }
...
test('falls back to the default for malformed / non-positive overrides', () => {
  for (const bad of ['', 'abc', '0', '-5', 'NaN', undefined]) {
    const env = bad === undefined ? {} : { PANERGOS_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: bad }
    assert.equal(
...
test('resolves with the announced port', async () => {
  ...
  child.stdout.emit('data', 'noise before\nPANERGOS_DASHBOARD_READY port=54321\n')
  assert.equal(await p, 54321)
})

test('resolves with a PANERGOS_BACKEND_READY port (headless `serve`)', async () => {
  const child = makeFakeChild()
  const p = waitForDashboardPort(child, 1000)
  child.stdout.emit('data', 'PANERGOS_BACKEND_READY port=43210\n')
...
test('parses the port even when the line arrives split across chunks', async () => {
  const child = makeFakeChild()
  const p = waitForDashboardPort(child, 1000)
  child.stdout.emit('data', 'PANERGOS_DASHBOARD_READY po')
...
test('rejects with the timeout message after the deadline', async () => {
  ...
  await assert.rejects(
    ...
    /Timed out waiting for Panergos backend port announcement \(20ms\)/
  )
})
...
test('a late announcement after timeout does not throw (listeners torn down)', async () => {
  ...
  assert.doesNotThrow(() => {
    child.stdout.emit('data', 'PANERGOS_DASHBOARD_READY port=9999\n')
  })
})
...
function mkTmpReadyFile() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'panergos-ready-test-'))
...
test('exit-before-announcement error carries the buffered output tail (stdout path)', async () => {
  ...
  const wait = waitForDashboardPortAnnouncement(child, {
    describeOutputTail: () => '\nRecent backend output:\nModuleNotFoundError: panergos_cli'
  })

  child.emit('exit', 1, null)

  await assert.rejects(wait, /exited before port announcement \(1\)[\s\S]*ModuleNotFoundError: panergos_cli/)
})

test('exit-before-announcement error carries the buffered output tail (ready-file path)', async () => {
  const child = makeFakeChild()
  const readyFile = path.join(os.tmpdir(), `panergos-ready-${process.pid}-${Date.now()}.json`)
...
test('resolves from bufferedOutput when the sentinel was consumed before the wait attached (#60323)', async () => {
  ...
  const alreadyConsumed = 'boot noise\nPANERGOS_BACKEND_READY port=43211\n'
...
test('bufferedOutput accepts the legacy PANERGOS_DASHBOARD_READY sentinel too', async () => {
  const child = makeFakeChild()

  const port = await waitForDashboardPortAnnouncement(child, {
    bufferedOutput: () => 'PANERGOS_DASHBOARD_READY port=43212\n',
    timeoutMs: 500
  })
...
test('bufferedOutput without a sentinel still resolves from later live stdout', async () => {
  ...
  child.stdout.emit('data', Buffer.from('PANERGOS_BACKEND_READY port=43213\n'))
...
test('the merged-tail seed recovers a sentinel spliced onto a partial stderr line (#103792)', async () => {
  ...
  const port = await waitForDashboardPortAnnouncement(child, {
    bufferedOutput: () => 'INFO  Started server process [4711]PANERGOS_BACKEND_READY port=65238',
    timeoutMs: 500
  })
...
test('the merged-tail seed does not match prose that merely names the sentinel', async () => {
  ...
  const wait = waitForDashboardPort(
    ...
    50,
    () => '',
    () => 'still waiting for PANERGOS_BACKEND_READY from the backend\n'
  )
