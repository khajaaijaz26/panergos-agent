 ...
 *
 * The Python side (panergos_cli/plugin_compat.py) statically scans the user's enabled external plugins on
 * every CLI/gateway/TUI start and writes PANERGOS_HOME/.plugin-compat-report.json when any plugin imports a
...
export function readReport(panergosHome: string): PluginCompatReport | null {
  try {
    const raw = fs.readFileSync(path.join(panergosHome, REPORT_FILE), 'utf8')
...
export function pendingNotice(panergosHome: string, userData: string): PendingNotice | null {
  const report = readReport(panergosHome)
