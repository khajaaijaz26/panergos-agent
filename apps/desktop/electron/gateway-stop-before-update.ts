 ...
 *
 * So we delegate to `panergos gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `panergos update`'s
...
export function stopGatewayBeforeUpdate(
  panergosCliPath: string,
  panergosHome: string,
  deps: StopGatewayBeforeUpdateDeps = {}
): boolean {
  return runGatewayLifecycleCommand(panergosCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(panergosCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(panergosCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(panergosCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(panergosCliPath, args)
  }

  if (!existsSync(panergosCliPath)) {
    return false
  }

  try {
    exec(panergosCliPath, args, {
