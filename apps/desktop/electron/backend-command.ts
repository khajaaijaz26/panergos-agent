// Backend subcommand routing for the desktop-managed Panergos process.
//
// The desktop app launches its own headless backend via `panergos serve` — it
...
// app hasn't updated yet, or an older `panergos` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Panergos profile to pin via `--profile`.
 */
 ...
 * `-m panergos_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `panergos_cli/subcommands/dashboard.py` source registers
