...
describe('formatRendererConsoleLine', () => {
  it('formats Electron console event details at error level', () => {
  ...
  it('drops non-error levels', () => {
    expect(formatRendererConsoleLine('main', { level: 'warning', message: 'x', sourceId: 's', lineNumber: 1 })).toBeNull()
  })
})

describe('attachRendererConsoleCapture', () => {
  it('logs error-level messages and skips the rest', () => {
    type Details = { level: string; message: string; sourceId: string; lineNumber: number }
    const log = vi.fn()
    let handler: ((details: Details) => void) | undefined

    const win = {
      webContents: {
        on: (_event: 'console-message', listener: (details: Details) => void) => {
          handler = listener
        }
      }
    }
