...
function tmp() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'panergos-compat-'))
}
...
const REPORT = {
  ...
  plugins: {
    ...
    beta: [
      { file: 'a.py', line: 1, old: 'panergos_cli.kanban_db.connect', new: 'panergos_cli.kanban_db_connect.connect' },
      {
        file: 'a.py',
        line: 9,
        old: 'panergos_cli.kanban_db.connect_closing',
        new: 'panergos_cli.kanban_db_connect.connect_closing'
      }
    ]
  },
  lines: [
    '2 plugins use import paths that stop working on 2026-09-14 (10 days): alpha (1), beta (2)',
    'Details: panergos plugins compat'
  ]
}
...
test('report → one notice naming plugins, date and the CLI command', () => {
  ...
  assert.match(n.detail, /panergos plugins compat/)
})
