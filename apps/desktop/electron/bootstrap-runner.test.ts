...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'panergos-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/panergos-runner-test',
    sourceRepoRoot: null,
    panergosHome: '/tmp/panergos-runner-test',
    logRoot: '/tmp/panergos-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'panergos-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'panergos-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/panergos-agent',
      panergosHome: '/tmp/panergos'
    }),
    ['--dir', '/tmp/panergos-agent', '--panergos-home', '/tmp/panergos', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/panergos-agent', '--panergos-home', '/tmp/panergos', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/panergos',
      panergosHome: '/tmp/home'
    }),
    ['--dir', '/tmp/panergos', '--panergos-home', '/tmp/home', '--branch', 'main']
  )
})

test('fallback install stamps without a branch use main', () => {
  assert.deepEqual(installRefForStamp({ commit: ZERO_COMMIT }), {
    ref: 'main',
    cacheKey: 'fallback-main',
    pinned: false
  })
