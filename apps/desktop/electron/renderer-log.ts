...
/** Format Electron's `Event<WebContentsConsoleMessageEventParams>` object, or
 *  return null for non-error levels. */
export function formatRendererConsoleLine(label: string, details: ConsoleMessageDetails): string | null {
  if (details.level !== 'error') {
    return null
  }

  return `[renderer console:${label}] ${String(details.message)} (${String(details.sourceId)}:${String(details.lineNumber)})`
}
...
export function attachRendererConsoleCapture(win: WindowLike, label: string, log: (line: string) => void): void {
  win.webContents.on('console-message', details => {
    const formatted = formatRendererConsoleLine(label, details)
...
/** Format a renderer error-boundary report (panergos:logs:renderer-error IPC)
