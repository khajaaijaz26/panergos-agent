...
/**
 * Panergos-managed Node.js directories, in preferred lookup order.
 *
 * There are two on-disk layouts. `scripts/install.ps1` unpacks portable Node
 * straight into `%LOCALAPPDATA%\panergos\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$PANERGOS_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_panergos_node_dirs()` in panergos_constants.py, which the Electron main
 * process cannot import.
 */
function panergosManagedNodePathEntries(
  panergosHome,
  ...
  const root = pathModule.join(panergosHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const panergosNodeDirs = panergosManagedNodePathEntries(panergosHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([panergosNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizePanergosHomeRoot(panergosHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!panergosHome) {
    return panergosHome
  }

  const resolved = pathModule.resolve(String(panergosHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). panergos_bootstrap sets this inside the child too, but only
