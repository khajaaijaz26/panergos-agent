...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.panergos`). Default's state.db and gateway.pid live here. */
  panergosHome: string
  /** Named-profile root (`~/.panergos/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isPanergosProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `panergosHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, panergosHome: string, profilesRoot: string): string {
  return name === 'default' ? `${panergosHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, panergosHome: string, profilesRoot: string): string {
  return name === 'default' ? `${panergosHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolvePanergosHome(profilesRoot: string, panergosHome?: string): string {
  if (panergosHome) {
    return panergosHome
  }

  // Tests that predate panergosHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live panergos process.
 ...
 *
 * `panergosHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isPanergosProcess'> & { panergosHome?: string }
): string[] {
  const panergosHome = resolvePanergosHome(profilesRoot, deps.panergosHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, panergosHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.panergosHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$PANERGOS_HOME`, not `$PANERGOS_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `panergos --profile default` and is worse than writing nothing
