...
export function registerNativeNotifications({ getMainWindow, focusWindow }: NotificationHost): void {
  ...
  ipcMain.handle('panergos:notify', (event, payload: PanergosNotification) => {
    ...
    notification.on('click', () => {
      ...
      if (focusSessionId) {
        window.webContents.send('panergos:focus-session', focusSessionId)
      }

      if (payload?.activate || payload?.notifyId) {
        window.webContents.send('panergos:notification-activate', {
    ...
    notification.on('action', (actionEvent, index) => {
      ...
      if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
        ...
        window.webContents.send('panergos:notification-action', { sessionId: payload.sessionId, actionId: action.id })
