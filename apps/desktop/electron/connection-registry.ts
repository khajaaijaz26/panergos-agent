 ...
 * runtime, remote gateways, and SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@panergos/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remotePanergosPath || '').trim() !== String(activeSsh.remotePanergosPath || '').trim() ||
...
/** Turn `ls ~/.panergos/profiles` output into roster names. Always includes
...
/** Every supported connection kind is user-managed and update-eligible. */
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remotePanergosPath: input.remotePanergosPath,
  ...
  if (kind === 'remote') {
    ...
    // Duplicate prevention: remote entries collide on normalized URL.
    ...
    // transport-/Panergos-managed names; an empty result stores nothing.
 ...
 * `remotePanergosPath`/`remoteProfile`. Only fields the payload explicitly
 ...
 * user-chosen label win. Switching to
