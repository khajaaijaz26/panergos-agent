 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Panergos dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Panergos install on the remote (login-shell probe),
 ...
 *     `PANERGOS_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForPanergos in.
 *
 * The minted PANERGOS_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const REMOTE_LOCK_DIR = '~/.panergos/desktop-ssh'
...
// Resolve the remote panergos executable. An EXPLICIT path is honored strictly
...
async function locatePanergos(ssh, remotePanergosPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The panergos binary or wrapper script
    ...
    //     the Panergos version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/khajaaijaz26/panergos-agent/issues/74411
    return candidate
  }
  ...
  if (remotePanergosPath) {
    if (await isExecutable(remotePanergosPath)) {
      return resolveLauncher(remotePanergosPath)
    }

    const err: any = new Error(
      `The Panergos path you set is not an executable on the remote host: "${remotePanergosPath}". ` +
        'Check the path (it must be the full path to the `panergos` or compatible `panergos` binary), ' +
        'or clear it to auto-detect.'
    )

    err.kind = 'panergos-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v panergos')}`)).trim()
  ...
  candidates.push('~/.local/bin/panergos')
  candidates.push('/usr/local/bin/panergos')
  candidates.push('~/.panergos/panergos-agent/venv/bin/panergos')
  ...
  const err: any = new Error(
    'Panergos is not installed on the remote host (could not find `panergos` or compatible `panergos`). ' +
      'Install it on the remote with:  curl -fsSL https://raw.githubusercontent.com/' +
      'khajaaijaz26/panergos-agent/main/scripts/install.sh | sh  ' +
      '— or set the Panergos path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<panergos> --version`,
// e.g. "Panergos Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH panergos a
// connection uses, so a stale/unexpected install is visible.
async function probePanergosVersion(ssh, panergosPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(panergosPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Panergos Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The PANERGOS_HOME the remote dashboard will use (explicit env wins, else
// ~/.panergos). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemotePanergosHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${PANERGOS_HOME:-$HOME/.panergos}"')).trim().split('\n').pop()

    return out || '~/.panergos'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Panergos home.')
...
marker=home/'.panergos-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Panergos checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, panergosHome) {
  const home = assertSafeRemoteHome(panergosHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Panergos install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Panergos update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Panergos update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemotePanergosProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemotePanergosHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Panergos profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Panergos home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'panergosPath', 'panergosHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !panergosPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(panergosPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/panergos`, leaving neither
      // launcher nor PANERGOS_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `panergos_home=os.path.expanduser(${shq(panergosHome)}) if ${shq(panergosHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if panergos_home:\n' +
      ' expected_entries.add(os.path.join(panergos_home,"panergos-agent","venv","bin","panergos"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.panergosPath)
  const expectedHome = lock.panergosHome ? expandRemotePath(lock.panergosHome) : "''"
  ...
  const executableMatch = lock.panergosHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.panergosPath === expected.panergosPath &&
    lock.panergosHome === expected.panergosHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.panergosPath)})
panergos_home=os.path.expanduser(${py(lock.panergosHome)})
expected_entries={expected_path,os.path.join(panergos_home,"panergos-agent","venv","bin","panergos")}
...
// execing Panergos. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"panergos-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(panergosPath, profile, opts: any = {}) {
  const panergos = expandRemotePath(panergosPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.panergosHome || '~/.panergos')}/.panergos-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.panergosHome || '~/.panergos')}/.panergos-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env PANERGOS_DESKTOP=1 ${panergos} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} panergos-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, panergosPath) {
  const panergos = expandRemotePath(panergosPath)

  const out = await ssh.exec(
    `help="$(${panergos} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, panergosPath))) {
    const err: any = new Error(
      'The remote Panergos install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Panergos on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(panergosPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        panergosHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForPanergos, a token-gated
...
async function connect(deps) {
  ...
  const panergosHome = await probeRemotePanergosHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, panergosHome)
  const panergosPath = await locatePanergos(ssh, remotePanergosPath)
  log(`located panergos at ${panergosPath}`)
  const panergosVersion = await probePanergosVersion(ssh, panergosPath)

  if (panergosVersion) {
    log(`remote panergos version: ${panergosVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Panergos Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.panergosPath === panergosPath &&
      lock.panergosHome === panergosHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    panergosHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, panergosHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForPanergos(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
...
export {
  ...
  ownershipDirectory,
  pidIsOurDashboard,
  probePanergosVersion,
  probeRemotePanergosHome,
  probeRemotePlatform,
  PROTOCOL_VERSION,
  readLockfile,
  READY_RE,
