...
export function resolveDesktopRemoteRoute({
  ...
  if (override) {
    ...
    const route = { ...profileConfig, kind: 'remote' as const } as StoredRoute
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'PANERGOS_DESKTOP_REMOTE_URL is set but PANERGOS_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Panergos backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `panergos serve` the desktop never
  ...
  const route = { ...config.remote, kind: 'remote' as const } as StoredRoute
