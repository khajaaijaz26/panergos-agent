...
test('compare payload maps to the behind count and a newest-first commit list; malformed = null', () => {
  ...
  assert.equal(githubRepoSlug('git@github.com:Someone/panergos-agent.git'), 'someone/panergos-agent')
  assert.equal(githubRepoSlug('https://gitlab.example/x/y.git'), null)
  assert.equal(
    branchTipApiUrl('khajaaijaz26/panergos-agent', 'bb/gui'),
    'https://api.github.com/repos/khajaaijaz26/panergos-agent/commits/bb%2Fgui'
  )
})
