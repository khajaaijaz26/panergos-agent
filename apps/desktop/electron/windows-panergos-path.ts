/**
 * windows-panergos-path.ts
 *
 * Pure, dependency-injected pieces of Windows `panergos` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvPanergosCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `panergos` shim shadowed the real
 *      panergos.cmd/panergos.exe; the shim then failed the --version probe and
 ...
 *      and Panergos launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvPanergosCommand() — unwrapWindowsVenvPanergosCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `panergos`) must
 * not shadow `panergos.cmd`/`panergos.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Panergos launcher that drives `panergos update`.
...
export interface ResolveVenvPanergosCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportPanergosCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `panergos`/`panergos.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/panergos(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m panergos_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportPanergosCli(). A
 * venv whose update died mid-`pip install` still has python.exe + panergos.exe
 ...
 *
 * Returns null when `command` is not a venv panergos shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvPanergosCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvPanergosCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^panergos(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Panergos at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Panergos Python at ${python}`,
    command: python,
