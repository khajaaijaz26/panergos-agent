 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `panergos update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `panergos desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Panergos couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>panergos desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>panergos desktop --force-build</code>, then restart the app.</p>
