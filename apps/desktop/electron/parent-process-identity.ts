export type ParentWatchdogEnv = {
  PANERGOS_PARENT_PID: string
  PANERGOS_PARENT_START_MARKER?: string
  PANERGOS_PARENT_NONCE?: string
  /** Spawn tag consumed by panergos_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { PANERGOS_PARENT_PID: String(pid) }
  env.PANERGOS_SPAWN = spawnTag(pid, startMarker)
  ...
  env.PANERGOS_PARENT_START_MARKER = startMarker
  env.PANERGOS_PARENT_NONCE = nonce

  return env
}

/** Build the `PANERGOS_SPAWN` tag mirrored by panergos_cli/process_identity.py.
