...
test('a live primary keeps answering for its booted profile after the preference moves', () => {
  ...
  // startPanergos() boots the primary as the preference at that moment.
  ...
  // panergos:profile:remember rewrites active-profile.json without re-homing.
  preference = 'claude'
  ...
  // spawned for the same PANERGOS_HOME.
  assert.equal(
    pin.resolve(() => preference),
    'default'
  )
