...
import { buildDesktopBackendEnv, normalizePanergosHomeRoot, panergosManagedNodePathEntries } from './backend-env'
import { isReauthRequiredError, waitForPanergosReady } from './backend-health'
...
import { isPanergosOwnedVenvDaemon } from './venv-holder-select'
...
import {
  ...
  resolveVenvPanergosCommand
} from './windows-panergos-path'
import {
...
const USER_DATA_OVERRIDE = process.env.PANERGOS_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.PANERGOS_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.PANERGOS_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with PANERGOS_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[panergos] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[panergos] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. PANERGOS_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[panergos] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[panergos] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via PANERGOS_DESKTOP_PASSWORD_STORE, bridged by the
// `panergos desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[panergos] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[panergos] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `panergos desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Panergos sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[panergos] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[panergos] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[panergos] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[panergos] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[panergos] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('panergos:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Panergos burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[panergos] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[panergos] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[panergos] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[panergos] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// PANERGOS_HOME — the user-facing root for everything Panergos-related. Mirrors
// scripts/install.ps1's $PanergosHome and scripts/install.sh's $PANERGOS_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\panergos (matches install.ps1)
//   macOS / Linux: ~/.panergos (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.panergos directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\panergos yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// PANERGOS_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// PANERGOS_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.panergos / %LOCALAPPDATA%\panergos.
function resolvePanergosHome() {
  if (process.env.PANERGOS_HOME) {
    return normalizePanergosHomeRoot(process.env.PANERGOS_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'panergos-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a PANERGOS_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\panergos and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('PANERGOS_HOME')

    if (fromRegistry) {
      return normalizePanergosHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'panergos')
    const legacy = path.join(app.getPath('home'), '.panergos')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.panergos setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.panergos')
}

const PANERGOS_HOME = resolvePanergosHome()

function pathWithPanergosManagedNode(...entries) {
  const managed = panergosManagedNodePathEntries(PANERGOS_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_PANERGOS_ROOT — the canonical mutable Panergos install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_PANERGOS_ROOT = path.join(PANERGOS_HOME, 'panergos-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_PANERGOS_ROOT, 'venv')
...
// boots, going straight to `resolvePanergosBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_PANERGOS_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_PANERGOS_ROOT, '.panergos-bootstrap-complete')
...
// active-profile.json records which Panergos profile the desktop launches its
// local backend as. When set, startPanergos() passes `panergos --profile <name>
// dashboard …`, which deterministically pins PANERGOS_HOME (see
// _apply_profile_override in panergos_cli/main.py) and bypasses the sticky
// ~/.panergos/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors panergos_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Canonical Panergos release branch. Users can still override it at runtime.
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under PANERGOS_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by panergos_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(PANERGOS_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (panergos_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.PANERGOS_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.PANERGOS_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.PANERGOS_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const GUIDED_ONBOARDING =
  process.env.PANERGOS_GUIDED_ONBOARDING === '1' || process.argv.includes('--guided-onboarding')

const SKIP_INTRO = process.env.PANERGOS_SKIP_INTRO === '1' || process.argv.includes('--skip-intro')

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.PANERGOS_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.PANERGOS_DESKTOP_APP_NAME || 'Panergos'
...
// Windows prefers assets/icon.ico (shipped to resources/ via extraResources)
// and falls back to the bundled Panergos PNG if the ico is missing.
...
// 'panergos:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.khajaaijaz26.panergos) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.khajaaijaz26.panergos')
}

// Seed the native About panel with the live Panergos version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `panergos update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolvePanergosVersion(),
  copyright: 'Panergos modifications © 2026 Shaik Khaja Aijaz Ahmed'
})
...
// startPanergos(); this pool only holds EXTRA profile
...
// The legacy PANERGOS_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.PANERGOS_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.PANERGOS_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Panergos backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
//   * WSL2 IPC stall  = the renderer's `panergos:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.PANERGOS_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startPanergos() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning panergos serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Panergos backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  panergosLog.push(...lines)

  if (panergosLog.length > 300) {
    panergosLog.splice(0, panergosLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-panergos-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('panergos:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the panergos:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('panergos:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_PANERGOS_ROOT
  })
}
...
// The Tauri updater writes PANERGOS_HOME/.panergos-update-in-progress for the whole
...
// (`force_kill_other_panergos`, taskkill /IM panergos.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(PANERGOS_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--panergos-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `panergos update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    ...
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Panergos will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(PANERGOS_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Panergos update did not finish',
        `${result.message}\n\nDetails: ${path.join(PANERGOS_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Panergos to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `panergos` — must not shadow `panergos.cmd`/`panergos.exe`.
...
function unwrapWindowsVenvPanergosCommand(command, backendArgs) {
  return resolveVenvPanergosCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `panergos serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `panergos` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'panergos_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `panergos --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, PANERGOS_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolvePanergosBackend step 4); execFileSync of a .cmd without
...
function isPanergosSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'panergos_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.PANERGOS_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Panergos deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the panergos-import
...
// findGitBash — locate bash.exe on Windows. Resolves PANERGOS_GIT_BASH_PATH
...
// normal PANERGOS_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\panergos\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'panergos', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'panergos', 'git', 'bin', 'git.exe'))
  }
...
function recentPanergosLog() {
  return panergosLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's panergos root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_PANERGOS_ROOT.
// PANERGOS_DESKTOP_PANERGOS_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.PANERGOS_DESKTOP_PANERGOS_ROOT && path.resolve(process.env.PANERGOS_DESKTOP_PANERGOS_ROOT),
    !IS_PACKAGED && isPanergosSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isPanergosSourceRoot(ACTIVE_PANERGOS_ROOT) ? ACTIVE_PANERGOS_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_PANERGOS_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('panergos:updates:progress', merged)
  }
}
...
// #105855 was a run of GitHub outages that read as a Panergos bug because the
// UI hid the cause.
    ...
    const req = https.get(
      url,
      {
        headers: {
          ...
          'User-Agent': 'panergos-desktop-update-check'
        },
        timeout: 10_000
      },
      res => {
...
// Windows that binary owns ALL repo mutation — running `panergos update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(PANERGOS_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `panergos update` can write
// fresh entry points. On Windows this is the file the running backend
// `panergos.exe` holds open; on POSIX it's never mandatory-locked.
function venvPanergosShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'panergos.exe')
    : path.join(updateRoot, 'venv', 'bin', 'panergos')
}
...
// Kill only Panergos-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `panergos`,
...
function killPanergosOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isPanergosOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Panergos-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `panergos.exe` that
// spawned its own grandchildren (a `panergos` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Panergos backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Panergos backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.panergosBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Panergos backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.panergosBackendIdentity
...
// `panergosProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `panergos.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a panergos
// REPL, a pty terminal, the gateway) keep `panergos.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const panergosProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\panergos.exe, but the
  // backend is `python.exe -m panergos_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (panergosProcess && Number.isInteger(panergosProcess.pid)) {
    initialPids.push(panergosProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(panergosProcess, {
  ...
  // `panergos gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvPanergosShimPath(updateRoot), PANERGOS_HOME)

  // Reap Panergos-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killPanergosOwnedVenvDaemons(updateRoot)

  const shim = venvPanergosShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentPanergosProcess = backendConnectionState.getProcess()

        if (currentPanergosProcess && Number.isInteger(currentPanergosProcess.pid)) {
          stragglers.push(currentPanergosProcess.pid)
        }
...
// `panergos update`). Instead we spawn the staged Panergos-Setup binary with
// --update and quit, so it can run `panergos update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `panergos update` → relaunch),
      ...
      // in: the PANERGOS_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `panergos update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `panergos desktop`, never the Tauri installer that self-copies
      // panergos-setup.exe into PANERGOS_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `panergos` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== DEFAULT_UPDATE_BRANCH) {
              command = `panergos update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `panergos update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, panergosRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(PANERGOS_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Panergos — this window will close and the updater will open. Don’t reopen Panergos yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(PANERGOS_HOME, rememberLog)
    ...
    // panergos.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running panergos, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Panergos install open ' +
        '(a second Panergos window or a terminal running panergos?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvPanergosShimPath(updateRoot))
      }
    ...
    // Panergos processes running from this venv.  The updater normally refuses
    ...
    // `panergos update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `panergos update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          PANERGOS_HOME,
          PANERGOS_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithPanergosManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `panergos update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(PANERGOS_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Panergos update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(PANERGOS_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Panergos will keep running — try again, or run \`panergos update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvPanergos = path.join(venvBin, IS_WINDOWS ? 'panergos.exe' : 'panergos')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Panergos launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the Panergos
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.panergos-bootstrap-complete')),
      hasVenvPanergos: fileExists(venvPanergos),
...
// Python-level pre-update snapshot inside `panergos update`.
function preflightStateDb(panergosHome, rememberLog) {
  const stateDbPath = path.join(panergosHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(panergosHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(panergosHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(panergosHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `panergos update`, swaps/relaunches the app bundle, and writes
// .panergos-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'panergos update', percent: null })

    return { ok: true, manual: true, command: 'panergos update', panergosRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      PANERGOS_HOME,
      PANERGOS_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithPanergosManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Panergos — this window will close. Don’t reopen Panergos yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_PANERGOS_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isPanergosSourceRoot(ACTIVE_PANERGOS_ROOT) &&
    fileExists(venvPython) &&
    canImportPanergosCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_PANERGOS_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `panergos
...
function resolveWebDist() {
  const override = process.env.PANERGOS_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set PANERGOS_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: panergos desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: panergos desktop --force-build`
  )
...
function resolvePanergosCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Panergos.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.PANERGOS_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolvePanergosCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'panergos_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_PANERGOS_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Panergos at ${ACTIVE_PANERGOS_ROOT}`,
    command,
    args: ['-m', 'panergos_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      panergosHome: PANERGOS_HOME,
      pythonPathEntries: [ACTIVE_PANERGOS_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_PANERGOS_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolvePanergosBackend(backendArgs) {
  // 1. Explicit override -- PANERGOS_DESKTOP_PANERGOS_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.PANERGOS_DESKTOP_PANERGOS_ROOT && path.resolve(process.env.PANERGOS_DESKTOP_PANERGOS_ROOT)

  if (overrideRoot && isPanergosSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Panergos source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `panergos` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isPanergosSourceRoot.)
  if (!IS_PACKAGED && isPanergosSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Panergos source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_PANERGOS_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\panergos\\panergos-agent (Windows) or ~/.panergos/panergos-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Panergos runtime at ${ACTIVE_PANERGOS_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `panergos` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    PANERGOS_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.PANERGOS_DESKTOP_IGNORE_EXISTING !== '1') {
    let panergosCommand = null
    const panergosOverride = process.env.PANERGOS_DESKTOP_PANERGOS

    if (panergosOverride) {
      const resolvedOverride = findOnPath(panergosOverride)

      if (resolvedOverride) {
        panergosCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(panergosOverride, { isWsl: IS_WSL })) {
        panergosCommand = panergosOverride
      } else {
        rememberLog(`Ignoring Windows Panergos override under WSL: ${panergosOverride}`)
      }
    } else {
      panergosCommand = findOnPath('panergos')
    }

    if (panergosCommand) {
      if (looksLikeDesktopAppBinary(panergosCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Panergos CLI: ${panergosCommand}`)
        panergosCommand = null
      }
    }

    if (panergosCommand) {
      const unwrapped = unwrapWindowsVenvPanergosCommand(panergosCommand, backendArgs)
      ...
      const shellForProbe = isCommandScript(panergosCommand)

      // PANERGOS_DESKTOP_PANERGOS is an explicit deployment override (used by
      ...
      if (shouldTrustPanergosOverride(panergosOverride) || verifyPanergosCli(panergosCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvPanergosCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Panergos CLI at ${panergosCommand}`,
          command: panergosCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Panergos CLI at ${panergosCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed panergos_cli module via system Python.
  ...
  if (python) {
    ...
    // having panergos_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\panergos\panergos-agent\venv.
    if (canImportPanergosCli(python)) {
      return {
        kind: 'python',
        label: `installed panergos_cli module via ${python}`,
        command: python,
        args: ['-m', 'panergos_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: panergos_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolvePanergosBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Panergos Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_PANERGOS_ROOT,
...
async function runEnsureRuntime(backend: any): Promise<any> {
  ...
  // backend.kind === 'bootstrap-needed' means resolvePanergosBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Panergos install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Panergos recovery was handed off to Panergos Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      panergosHome: PANERGOS_HOME,
      logRoot: path.join(PANERGOS_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Panergos install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Panergos bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(PANERGOS_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startPanergos() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // panergos:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolvePanergosBackend(backend.args))
  }
  ...
  if (!isPanergosSourceRoot(ACTIVE_PANERGOS_ROOT)) {
    throw new Error(
      `Panergos install at ${ACTIVE_PANERGOS_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Panergos' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\panergos\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `panergos` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Panergos on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Panergos.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isPanergosSourceRoot()
    // plus an importable panergos_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Panergos venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Panergos at ${ACTIVE_PANERGOS_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Panergos runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Panergos backend URL protocol: ${parsed.protocol}`))
        ...
        const req = client.request(
          parsed,
          {
            ...
            headers: {
              ...
              'X-Panergos-Session-Token': token,
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Panergos backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Panergos backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Panergos-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Panergos-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('panergos:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolvePanergosCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('panergos:preview-file-changed', payload)
}
...
// waitForPanergosReady how to read a 401 — rejected session vs gated route.
...
async function waitForPanergos(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForPanergosReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('panergos:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('panergos:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('panergos:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('panergos:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Panergos...".
function sendPowerResume() {
  ...
  webContents.send('panergos:power-resume')
}
...
// 'panergos:power-battery' push below.
ipcMain.handle('panergos:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('panergos:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(PANERGOS_HOME, app.getPath('userData'))
  } catch (err) {
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('panergos:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('panergos:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('panergos:context-menu-spellcheck', {
...
//
// Remote gateways can gate the dashboard behind an OAuth provider
// instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``panergos_session_at``),
...
//   * Deployments may issue a rotating refresh token alongside the short-lived
//     access token.
//     Both are set as HttpOnly cookies (``panergos_session_at`` ~15 min,
//     ``panergos_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// jar above). The primary / v1 remote flow keeps the legacy shared partition;
// see oauth-partition.ts for the full rules.
...
// "not signed in", which on the initial boot path (startPanergos → the renderer's
// single-shot boot() with no retry) surfaced as the "Panergos couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Panergos
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Panergos gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Panergos backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every panergos:connections:* call.
...
// a valid stored value (pins the root PANERGOS_HOME explicitly); null means "no
...
// contains "panergos", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isPanergosProcess(pid) {
  try {
  ...
  try {
    ...
    return cmdline.includes('panergos')
  } catch {
...
//   1. Legacy ~/.panergos/active_profile (explicit CLI choice via panergos profile use)
//   2. Running gateway (gateway.pid with verified liveness + panergos identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(PANERGOS_HOME, 'active_profile'),
    panergosHome: PANERGOS_HOME,
    profilesRoot: path.join(PANERGOS_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.PANERGOS_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.PANERGOS_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    sshRemotePanergosPath: (ssh || savedSsh)?.remotePanergosPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by PANERGOS_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  const mode = input.mode === 'ssh' ? 'ssh' : input.mode === 'remote' ? 'remote' : 'local'
  ...
  if (key) {
    // Per-profile scope: a remote entry pins this profile to its own
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remotePanergosPath: input.sshRemotePanergosPath ?? existingBlock.remotePanergosPath,
...
async function buildRemoteConnection(
  ...
  if (!token) {
    throw new Error(
      'Remote Panergos gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.PANERGOS_DESKTOP_REMOTE_TOKEN,
      url: process.env.PANERGOS_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      panergosPath: result.panergosPath,
      panergosHome: result.panergosHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { panergosPath: result.panergosPath, panergosHome: result.panergosHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remotePanergosPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      platform,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remotePanergosPath: sshConfig.remotePanergosPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForPanergos: (baseUrl, token) => waitForPanergos(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        panergosVersion: result.panergosVersion || '',
        ...
        // per-profile pools also use bare keys. Only startPanergos' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.panergosVersion || 'panergos (version unknown)'} at ${result.panergosPath || '?'}`
  )
  ...
  return {
    ...connection,
    remotePanergosVersion: result.panergosVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (PANERGOS_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
// Remote, or the env override): a SINGLE remote backend serves every profile
// via ?profile=.
// Distinct from per-profile overrides — here there's one host for all.
function globalRemoteActive() {
  if (process.env.PANERGOS_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startPanergos() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Panergos
  // gateway and reports:
  ...
  if (authRequired) {
    ...
    // button can use the provider's own display name, and so a
    // username/password provider can be distinguished from
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remotePanergosPath: input.sshRemotePanergosPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remotePanergosPath || '')
          let panergosPath
          let panergosVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            panergosPath = runtime.panergosPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.panergosPath])
            panergosVersion = inspection.version
            supported = inspection.supported
          } else {
            panergosPath = await remoteLifecycle.locatePanergos(ssh, sshConfig.remotePanergosPath || '')
            panergosVersion = await remoteLifecycle.probePanergosVersion(ssh, panergosPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, panergosPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Panergos on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remotePanergosPath: panergosPath,
            remotePanergosVersion: panergosVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startPanergos())
  ...
  // connect to Panergos gateway". Mirror the renderer's connect here so the test
...
function resetPanergosConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  // The next startPanergos() re-reads active-profile.json for its launch profile.
  primaryProfilePin.clear()
  const panergosProcess = backendConnectionState.invalidate()
  stopBackendChild(panergosProcess)
...
// startPanergos() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetPanergosConnection() invalidates it.
  const panergosProcess = backendConnectionState.getProcess()
  const dying = panergosProcess && !panergosProcess.killed ? panergosProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('panergos:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('panergos:connections:changed', payload)
    }
  }
}
...
// startPanergos() and cleared when the primary is torn down; while a primary is
...
async function ensureBackend(profile, opts: { passive?: boolean; spawnPriority?: LocalBackendSpawnPriority } = {}) {
  ...
  if (route.backend === 'primary') {
    const connection = await startPanergos()
  ...
  // spawning, so two children never share one profile's PANERGOS_HOME.
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remotePanergosPath: source.remotePanergosPath,
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. panergos:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: panergosLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForPanergos(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.panergosPath || !state?.panergosHome) {
  ...
  return {
    ...
    panergosPath: state.panergosPath,
    panergosHome: state.panergosHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remotePanergosPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.panergosPath ? platform : await probeWindowsRemote(ssh, config.remotePanergosPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          panergosPath: runtime.panergosPath,
          panergosHome: runtime.panergosHome,
          pythonPath: runtime.python
        }
      }
    }

    const panergosPath = await remoteLifecycle.locatePanergos(ssh, config.remotePanergosPath || '')
    const panergosHome = await remoteLifecycle.probeRemotePanergosHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, panergosPath, panergosHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { panergosPath: state.panergosPath, panergosHome: state.panergosHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForPanergos(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startPanergos() but without the boot-progress UI,
...
async function runPoolBackendStart(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForPanergos(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited PANERGOS_HOME env (see _apply_profile_override
  // step 3 in panergos_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolvePanergosBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const panergosCwd = resolvePanergosCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(PANERGOS_HOME, 'profiles', key))
  )
  rememberLog(`Starting Panergos backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawnOwnedBackend(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
          ...
          TERMINAL_CWD: panergosCwd,
          PANERGOS_DASHBOARD_SESSION_TOKEN: token,
          ...
          ...parentIdentityEnv,
          PANERGOS_WEB_DIST: webDist,
          ...(readyFile ? { PANERGOS_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Panergos backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Panergos backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Panergos backend for profile "${profile}" exited (${signal || code})`)
    releaseLocalBackendSlot(entry)
    releaseBackendChild(child)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Panergos backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForPanergos(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Panergos backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Panergos backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_panergos_home() recreates the deleted profile directory.
//
...
function startPanergos() {
  return localBackendLifecycle.start(runPanergosStart)
}

async function runPanergosStart() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Panergos Desktop is already running in another window.')
  }
  ...
  // process, every subsequent startPanergos() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Panergos backend', 8)
  ...
  // later panergos:profile:remember must not retarget requests mid-life.
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Panergos backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Panergos backend at ${remote.baseUrl}`, 24)
      await waitForPanergos(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Panergos backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, panergosLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.panergos/active_profile file) and
    // resolves PANERGOS_HOME the same way `panergos -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Panergos runtime', 28)

        return resolvePanergosBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Panergos backend via ${backend.label}`, 84)
    rememberLog(`Starting Panergos backend via ${backend.label}`)
    ...
    const panergosProcess = spawnOwnedBackend(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: panergosCwd,
        env: {
            ...process.env,
            // Explicitly pin PANERGOS_HOME for the child so Python's get_panergos_home()
            // resolves to the SAME location our resolvePanergosHome() picked. Without
            // this pin, Python falls back to ~/.panergos on every platform — fine on
            // mac/linux (where our default matches), but on Windows our default is
            // %LOCALAPPDATA%\panergos, which differs from C:\Users\<u>\.panergos.
            // Mismatch would split config / sessions / .env / logs across two
            // directories. install.ps1 sets PANERGOS_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(panergosProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(panergosProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, panergosProcess)

    if (!processOwner) {
      stopBackendChild(panergosProcess)
      await waitForBackendExit(panergosProcess)
      releaseBackendChild(panergosProcess)
      throw new Error('Panergos backend start was superseded by a newer connection attempt.')
    }

    panergosProcess.stdout.on('data', rememberLog)
    panergosProcess.stderr.on('data', rememberLog)
    ...
    panergosProcess.once('error', error => {
      releaseBackendChild(panergosProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Panergos backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Panergos backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Panergos backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Panergos backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    panergosProcess.once('exit', (code, signal) => {
      releaseBackendChild(panergosProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Panergos backend exit (${signal || code})`)
      ...
      rememberLog(`Panergos backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Panergos backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Panergos backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentPanergosLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Panergos backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Panergos backend to become ready', 90)
    await Promise.race([waitForPanergos(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => panergosProcess.exitCode === null && !panergosProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Panergos backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Panergos backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed PANERGOS_HOME/.plugin-compat-report.json.
    ...
    // getConnection()/startPanergos() caller arriving during the exit wait would
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `panergos:openExternal`, not here. See window-open-policy.ts.
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('panergos:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Panergos is
...
// pushes pet state over IPC (panergos:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via panergos:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Panergos app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Panergos icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see panergos:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Panergos in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('panergos:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Panergos can be driven
...
// left the user with NO Panergos window after the second toggle (#88513).
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('panergos:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('panergos:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[panergos] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('panergos:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `panergos:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, panergos:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('panergos:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('panergos:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('panergos:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'panergos desktop --force-build',
  ...
  startPanergos().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('panergos:connection', async (_event, profile, extra) => {
...
ipcMain.handle('panergos:connection:for', async (_event, payload) => {
  ...
  // Same single-owner claim as 'panergos:connection', keyed by the composite
...
ipcMain.on('panergos:connection:active-route', (event, route) => {
...
// "Starting Panergos…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('panergos:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetPanergosConnection({ soft: true }),
...
ipcMain.handle('panergos:backend:touch', async (_event, profile) => {
...
ipcMain.handle('panergos:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('panergos:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('panergos:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('panergos:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('panergos:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('panergos:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('panergos:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolvePanergosBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Panergos is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `panergos-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, PANERGOS_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('panergos:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('panergos:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('panergos:zoom:get', event => {
...
ipcMain.on('panergos:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('panergos:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('panergos:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startPanergos() call restarts the
...
ipcMain.handle('panergos:bootstrap:repair', async () => {
  // Forceful repair: force the next startPanergos() through the full installer
  ...
  // "bypass active runtime" path inside startPanergos() and fall through
  ...
  resetPanergosConnection()

  return { ok: true }
})
ipcMain.handle('panergos:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('panergos:bootstrap:cancel', async () => {
...
ipcMain.handle('panergos:boot-progress:get', async () => bootProgressState)
ipcMain.handle('panergos:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('panergos:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('panergos:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('panergos:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('panergos:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('panergos:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('panergos:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('panergos:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('panergos:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('panergos:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('panergos:connections:remove', async (_event, id) => {
  const key = String(id || '')
  ...
  // removed connection keep their WebSocket open (remotes have no local
...
ipcMain.handle('panergos:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('panergos:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('panergos:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('panergos:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemotePanergosPath: entry.remotePanergosPath
    })
  ...
  if (entry.kind === 'local') {
    const local = await startPanergos()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remotePanergosPath: connection.remotePanergosPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemotePanergosProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root PANERGOS_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('panergos:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// panergos:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('panergos:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('panergos:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `panergos update` to every eligible registered connection at once.
// Each dispatch reports independently so one dead LAN box can't wedge the
// batch. Local reuses the
...
ipcMain.handle('panergos:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/panergos/update', {}, { timeoutMs: 15_000 })
...
// analogue of the panergos:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('panergos:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('panergos:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startPanergos() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('panergos:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
ipcMain.handle('panergos:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('panergos:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('panergos:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of panergos:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('panergos:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('panergos:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new PANERGOS_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('panergos:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('panergos:requestMicrophoneAccess', async () => {
...
ipcMain.handle('panergos:window:readBelow', async event => {
...
async function handlePanergosApiRequest(request) {
  ...
  // backend calls ensure_panergos_home() which recreates the profile directory,
...
ipcMain.handle('panergos:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its PANERGOS_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handlePanergosApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handlePanergosApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('panergos:ambient:claim', (_event, key) => ownsAmbientCue(String(key ?? '')))
...
ipcMain.handle('panergos:data-url-read-max:get', () => ({
...
ipcMain.handle('panergos:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('panergos:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('panergos:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('panergos:readFileText', async (_event, filePath) => {
...
// `panergos:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('panergos:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('panergos:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('panergos:writeClipboard', (_event, text) => {
...
ipcMain.handle('panergos:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('panergos:readClipboard', () => clipboard.readText())

ipcMain.handle('panergos:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('panergos:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('panergos:context-menu:edit', (event, command) => {
...
ipcMain.handle('panergos:context-menu:copy-image', event => {
...
ipcMain.handle('panergos:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('panergos:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('panergos:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('panergos:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('panergos:savePastedText', async (_event, payload) => {
...
ipcMain.handle('panergos:saveClipboardImage', async () => {
  let image = null

  try {
    image = await readClipboardImage(await clipboard.read())
  } catch {
    // Some platform clipboards can fail transiently; the WSL host fallback below
    // still gets a chance before we report that no image was available.
  }

  if (image) {
    return writeComposerImage(image.buffer, image.extension)
  }
...
ipcMain.handle('panergos:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('panergos:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('panergos:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('panergos:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('panergos:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('panergos:titlebar-theme', (_event, payload) => {
...
ipcMain.on('panergos:native-theme', (_event, mode) => {
...
ipcMain.on('panergos:translucency:support', event => {
...
// it arrives from `panergos desktop --local` or directly on Panergos.exe (a
// shortcut edit), and survives self-relaunches because collectRelaunchArgs
// only strips internal flags.
ipcMain.on('panergos:launch-flags', event => {
  event.returnValue = {
...
ipcMain.on('panergos:translucency', (_event, payload) => {
...
ipcMain.on('panergos:keep-awake', (_event, on) => {
...
ipcMain.handle('panergos:quick-entry:settings:get', async () => {
...
ipcMain.handle('panergos:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('panergos:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('panergos:quick-entry:submit', {
...
ipcMain.on('panergos:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('panergos:quick-entry:state', payload)
  }
})

ipcMain.on('panergos:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('panergos:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('panergos:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `panergos:window:openSession`).
...
ipcMain.handle('panergos:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('panergos:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('panergos:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('panergos:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolvePanergosCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('panergos:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolvePanergosCwd()
}))

ipcMain.handle('panergos:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('panergos:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('panergos:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('panergos:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('panergos:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('panergos:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('panergos:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: panergosLog.slice(-200) }))
...
ipcMain.on('panergos:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (panergos:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('panergos:updates:check', async (_event, opts) =>
...
ipcMain.handle('panergos:updates:apply', async (_event, payload) =>
...
ipcMain.handle('panergos:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('panergos:updates:branch:set', async (_event, name) => {
...
// Resolve the Panergos distribution version from the shared Python metadata so
// the desktop About panel shows the public release version instead of the
// Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolvePanergosVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'panergos_cli', '__init__.py')

    if (fileExists(initPath)) {
      const raw = fs.readFileSync(initPath, 'utf8')
      const match = raw.match(/__distribution_version__\s*=\s*["']([^"']+)["']/)
...
// Renderer-bundle skew: `panergos update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Panergos version and push it into the native About panel
// just before showing it, so an in-place `panergos update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolvePanergosVersion()} — app build out of date, update the desktop app`
        : resolvePanergosVersion(),
      copyright: 'Copyright © 2026 Shaik Khaja Aijaz Ahmed'
    })
    app.showAboutPanel()
  })
}

ipcMain.handle('panergos:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolvePanergosVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    panergosRoot: resolveUpdateRoot(),
...
// The About page's "Restart Panergos" button (shown when bundleSwapPending):
...
ipcMain.handle('panergos:app:relaunch', async () => {
...
ipcMain.handle('panergos:machine:profile', async () => {
...
// the actual removal via `panergos uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (panergos_cli/uninstall.py + panergos_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_PANERGOS_ROOT
  ...
  const fallback = () => ({
    panergos_home: PANERGOS_HOME,
    agent_installed: isPanergosSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'panergos_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, PANERGOS_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Panergos agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import panergos_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_PANERGOS_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_PANERGOS_ROOT, 'uninstall')
  } catch (error) {
  ...
  const scriptArgs = {
    ...
    pythonExe: py,
    pythonPath,
    agentRoot: ACTIVE_PANERGOS_ROOT,
    uninstallArgs,
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `panergos-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `panergos-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('panergos:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('panergos:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('panergos:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('panergos:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// panergos:// deep links (e.g. panergos://blueprint/morning-brief?time=08:00,
// panergos://mcp/install?name=NAME&config=B64, or
// panergos://plugin/install?repo=owner/repo). Dev registers panergos-dev://.
...
// ---------------------------------------------------------------------------
const REGISTERED_PROTOCOL = DEV_SERVER ? 'panergos-dev' : 'panergos'

/** Schemes accepted when parsing inbound URLs. */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['panergos-dev', 'panergos'] : ['panergos']
...
function handleDeepLink(url) {
  ...
  // panergos://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('panergos:deep-link', payload)
...
ipcMain.handle('panergos:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${REGISTERED_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(REGISTERED_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(REGISTERED_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${REGISTERED_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `panergos://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startPanergos), whose
...
app.whenReady().then(() => {
  ...
  // startPanergos() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching Panergos URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Panergos never keeps another app's chord hostage.
