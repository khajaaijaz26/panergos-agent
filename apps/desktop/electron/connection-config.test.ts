...
test('normalizeRemoteHeaders keeps safe proxy headers and drops transport/auth headers', () => {
  assert.deepEqual(
    normalizeRemoteHeaders({
      ...
      'X-Panergos-Session-Token': { encoding: 'plain', value: 'token' },
...
test('remoteRequestMatchesBaseUrl treats HTTPS and WSS as the same gateway origin', () => {
  assert.equal(
    remoteRequestMatchesBaseUrl(
      'wss://panergos.example.com/gateway/api/ws?ticket=abc',
      'https://panergos.example.com/gateway'
    ),
    true
  )
  assert.equal(remoteRequestMatchesBaseUrl('ws://panergos.example.com/api/ws', 'http://panergos.example.com'), true)
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://panergos.example.com/other/api/ws', 'https://panergos.example.com/gateway'),
    false
  )
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://other.example.com/gateway/api/ws', 'https://panergos.example.com/gateway'),
    false
  )
})
...
test('modeIsRemoteLike accepts only the generic remote mode', () => {
  assert.equal(modeIsRemoteLike('remote'), true)
  assert.equal(modeIsRemoteLike('retired'), false)
...
test('profileRemoteOverride returns the per-profile remote with defaulted auth mode', () => {
  const config = {
    profiles: {
      coder: { mode: 'remote', url: '  https://coder.example.com/panergos  ', token: { value: 'sek' } }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://coder.example.com/panergos',
    authMode: 'token',
    token: { value: 'sek' }
  })
})
...
test('profileRemoteOverride ignores a retired hosted entry', () => {
  // Retired proprietary modes are not promoted into generic remotes.
  const config = {
    profiles: {
      coder: { mode: 'retired', url: 'https://hosted-agent.example.com', authMode: 'oauth' }
    }
  }
...
test('localProfileEntry preserves inactive SSH drafts but drops unsupported route state', () => {
  const ssh = { mode: 'ssh', host: 'box', user: 'alice', remotePanergosPath: '/panergos' }
  assert.deepEqual(localProfileEntry(ssh), { mode: 'local', savedSsh: ssh })
  assert.deepEqual(localProfileEntry({ mode: 'local', savedSsh: ssh }), {
    mode: 'local',
    savedSsh: ssh
  })
  assert.equal(localProfileEntry({ mode: 'retired', url: 'https://agent' }), null)
})
...
// --- translateSelfProfileQuery (registry SSH-scoped panergos:api contract) ---
...
test('resolveProfileApiRequest uses exact method and path eligibility for mixed families', () => {
  ...
  assert.deepEqual(
    resolveProfileApiRequest('iris', '/api/model/recommended-default?provider=acme', {
      requestMethod: 'GET'
    }),
    {
      backendProfile: 'iris',
      requestPath: '/api/model/recommended-default?provider=acme'
    }
  )
})
...
test('resolveProfileApiRequest routes action-status polls with the action-spawning routes', () => {
  ...
  // install): _spawn_panergos_action registers the dynamic action name only in
...
test('normalizeRemoteBaseUrl strips trailing slashes, hash, and query', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/panergos/'), 'https://gw.example.com/panergos')
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/panergos?x=1#frag'), 'https://gw.example.com/panergos')
})

test('normalizeRemoteBaseUrl preserves a path prefix', () => {
  assert.equal(normalizeRemoteBaseUrl('https://host/panergos'), 'https://host/panergos')
})
...
test('normalizeRemoteBaseUrl auto-prepends http:// for scheme-less host:port input', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('gw.example.com/panergos/'), 'http://gw.example.com/panergos')
})
...
test('buildGatewayWsUrl honors a path prefix', () => {
  assert.equal(buildGatewayWsUrl('https://host/panergos', 't'), 'wss://host/panergos/api/ws?token=t')
})
...
test('buildGatewayWsUrlWithTicket uses ?ticket= not ?token=', () => {
  const url = buildGatewayWsUrlWithTicket('https://gw.example.com/panergos', 'tkt-9')
  assert.equal(url, 'wss://gw.example.com/panergos/api/ws?ticket=tkt-9')
...
test('cookiesHaveSession detects the bare access-token cookie', () => {
  assert.equal(cookiesHaveSession([{ name: 'panergos_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession detects the __Host- and __Secure- prefixed variants', () => {
  assert.equal(cookiesHaveSession([{ name: '__Host-panergos_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveSession([{ name: '__Secure-panergos_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession is false for an empty value', () => {
  assert.equal(cookiesHaveSession([{ name: 'panergos_session_at', value: '' }]), false)
})
...
test('cookiesHaveSession ignores unrelated cookies (AT-only by design)', () => {
  ...
  assert.equal(cookiesHaveSession([{ name: 'panergos_session_rt', value: 'x' }]), false)
...
test('AT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(AT_COOKIE_VARIANTS, ['__Host-panergos_session_at', '__Secure-panergos_session_at', 'panergos_session_at'])
})

test('RT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(RT_COOKIE_VARIANTS, ['__Host-panergos_session_rt', '__Secure-panergos_session_rt', 'panergos_session_rt'])
})

// --- cookiesHaveLiveSession (AT or RT — the connectivity check) ---

test('cookiesHaveLiveSession is true for a live access-token cookie', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'panergos_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-panergos_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-panergos_session_at', value: 'x' }]), true)
})
...
test('cookiesHaveLiveSession is true for an RT cookie even with NO access-token cookie', () => {
  ...
  assert.equal(cookiesHaveLiveSession([{ name: 'panergos_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-panergos_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-panergos_session_rt', value: 'x' }]), true)
})

test('cookiesHaveLiveSession is true when both AT and RT are present', () => {
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'panergos_session_at', value: 'a' },
      { name: 'panergos_session_rt', value: 'r' }
    ]),
    true
  )
})
...
test('cookiesHaveLiveSession is false for empty values', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'panergos_session_at', value: '' }]), false)
  assert.equal(cookiesHaveLiveSession([{ name: 'panergos_session_rt', value: '' }]), false)
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'panergos_session_at', value: '' },
      { name: 'panergos_session_rt', value: '' }
    ]),
    false
  )
})
...
test('gateway WS URL IPC result serializes success and the auth-vs-transport matrix', async () => {
  ...
  for (const error of [
    ...
    new Error('Timed out connecting to Panergos backend after 8000ms'),
...
test('FIX #95701: a confirmed 401/403 ticket rejection is tagged isReauthRequired so startPanergos latches it', () => {
  for (const statusCode of [401, 403]) {
