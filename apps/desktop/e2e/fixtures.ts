/**
 * Shared E2E fixtures for the Panergos desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → panergos serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `panergos desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `panergos-e2e-${prefix}-${Math.random()}`))
  const panergosHome = path.join(root, 'panergos-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(panergosHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(panergosHome, 'config.yaml')
 ...
 */
export function writeEnvFile(panergosHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(panergosHome, '.env')
 ...
 */
function writeEmptyConfig(panergosHome: string): void {
  const configPath = path.join(panergosHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - PANERGOS_HOME → sandbox panergos-home (isolated config/sessions)
 *  - PANERGOS_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - PANERGOS_DESKTOP_IGNORE_EXISTING=1 → don't pick up `panergos` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - PANERGOS_DESKTOP_PANERGOS_ROOT → REPO_ROOT (dev checkout resolution)
 *  - PANERGOS_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    PANERGOS_HOME: sandbox.panergosHome,
    PANERGOS_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    PANERGOS_DESKTOP_IGNORE_EXISTING: '1',
    PANERGOS_DESKTOP_PANERGOS_ROOT: REPO_ROOT,
    PANERGOS_DESKTOP_APP_NAME: `PanergosE2E-${Date.now()}`,
    ...
    PANERGOS_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated PANERGOS_HOME + userData
 * @param env      - the process environment (already has PANERGOS_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(
    sandbox.panergosHome,
    mock.url,
    options.extraDisplayConfig,
    options.extraConfig,
    options.modelContextLength,
  )
  writeEnvFile(sandbox.panergosHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.panergosHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via PANERGOS_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`panergos serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.panergosHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { PANERGOS_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Panergos backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Panergos.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Panergos.app', 'Contents', 'MacOS', 'Panergos')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'Panergos')
}
 ...
 * progress without spawning a real Panergos backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * PANERGOS_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    PANERGOS_DESKTOP_BOOT_FAKE: '1',
    PANERGOS_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + panergos-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).PANERGOS_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).PANERGOS_DESKTOP_PANERGOS
  delete (env as Record<string, string | undefined>).PANERGOS_DESKTOP_PANERGOS_ROOT
