...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Panergos E2E'], { cwd: repo })
...
function configureRepoCwd(panergosHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(panergosHome, mockUrl)
  fs.appendFileSync(path.join(panergosHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(panergosHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.panergosHome, mock.url, repo)
