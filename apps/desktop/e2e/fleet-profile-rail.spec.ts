 ...
 * second gateway, "Homelab", is a REAL second `panergos serve` this spec spawns
 * with its own PANERGOS_HOME, profiles and session token, registered in the v2
...
function findPanergosBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'panergos')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['panergos'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('panergos binary not found: create the repo venv (uv sync) or put panergos on PATH')
}
...
/**
 * Spawn a second, fully real `panergos serve` as the remote gateway. Its
 * session token is pinned through PANERGOS_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        PANERGOS_HOME: home,
        PANERGOS_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote panergos serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-Panergos-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote panergos serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    writeMockProviderConfig(sandbox.panergosHome, mock.url)
    ...
    seedProfiles(sandbox.panergosHome, ['research'])
