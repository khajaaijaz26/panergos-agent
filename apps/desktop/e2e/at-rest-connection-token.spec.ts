/**
 * E2E contract for remote-gateway tokens at rest.
 *
 * Keychain encryption is optional and off by default. Under either policy the
 * connection file must be owner-only, a saved token must survive a restart,
 * and the app must send the exact token to the gateway. When encryption is
 * enabled, plaintext token bytes must not appear anywhere under userData or
 * PANERGOS_HOME. Existing 0644 files are tightened on first read even when the
 * JSON is corrupt.
 *
 ...
 */
const SENTINEL_TOKEN = 'panergos-e2e-at-rest-sentinel-Zq7Z4hV9nX2pL8sK3tB6wR1yM5jD0fG'
 ...
 */
const STABLE_APP_NAME = 'PanergosE2EAtRestStorage'

// ─── Fake gateway ───────────────────────────────────────────────────────

interface FakeGateway {
  url: string
  /** Every `X-Panergos-Session-Token` value the app has sent us. */
  sessionTokens: string[]
  close: () => Promise<void>
}

/**
 * A minimal stand-in for a remote Panergos gateway. It serves the public
...
async function startFakeGateway(): Promise<FakeGateway> {
  ...
  const server = http.createServer((req, res) => {
    const token = req.headers['x-panergos-session-token']
 ...
 * backend, so we skip spawning `panergos serve` (no Python needed, ~3s launch,
 ...
 * this state. BOOT_FAKE_ERROR short-circuits startPanergos() *before* remote
...
async function launchAgainst(sandbox: Sandbox): Promise<{ app: ElectronApplication; page: Page }> {
  const env = buildAppEnv(sandbox, {
    PANERGOS_DESKTOP_APP_NAME: STABLE_APP_NAME,
    PANERGOS_DESKTOP_BOOT_FAKE_ERROR: 'E2E at-rest storage spec: local backend intentionally not started',
  })
  ...
  await page.waitForFunction(
    () => Boolean((window as unknown as { panergosDesktop?: Record<string, unknown> }).panergosDesktop?.saveConnectionConfig),
    undefined,
    { timeout: 60_000 },
  )
 ...
 * posture Panergos should take there (refuse to save vs. accept basic_text) is
...
async function saveRemoteToken(page: Page, remoteUrl: string, remoteToken?: string): Promise<SaveOutcome> {
  return page.evaluate(
    async ([url, token]) => {
      const desktop = (window as unknown as { panergosDesktop: any }).panergosDesktop
...
test.describe('remote gateway session token at rest', () => {
  test('with keychain encryption opted IN, a newly configured token is never written to userData in plaintext, and still works after restart', async () => {
    ...
    // PANERGOS_HOME (desktop.log lives there).
    expect(
      scanTreeForSecret(userDataDir, needles),
      'the gateway session token leaked into a userData file',
    ).toEqual([])
    expect(
      scanTreeForSecret(sandbox.panergosHome, needles),
      'the gateway session token leaked into a PANERGOS_HOME file (logs included)',
    ).toEqual([])
  ...
  test('an install whose connection.json predates owner-only mode is tightened on read', async () => {
    ...
    // about it — including the token envelope — is exactly what the app wrote.
