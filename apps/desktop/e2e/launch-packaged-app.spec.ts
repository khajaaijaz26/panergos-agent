...
/**
 * E2E smoke tests for the packaged Panergos desktop app.
 *
 ...
 * (credential stripping, isolated PANERGOS_HOME + userData, unique app name).
 *
...
test('window opens with the Panergos title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      panergosDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).panergosDesktop?.hud?.open({ sessionId: null })
  )
