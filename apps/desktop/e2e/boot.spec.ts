 ...
 * packaged binary) with a real `panergos serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → panergos serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Panergos title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Panergos')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { panergosDesktop?: Record<string, unknown> }).panergosDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → panergos serve is spawned → WS connects → config
