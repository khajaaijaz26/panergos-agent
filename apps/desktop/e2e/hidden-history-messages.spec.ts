...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.panergosHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.panergosHome)
  const builder = await RealSessionBuilder.start(sandbox.panergosHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.panergosHome, mock.url)
  fs.appendFileSync(path.join(sandbox.panergosHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
