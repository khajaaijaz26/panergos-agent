 ...
 * AIAgent loop, using the E2E mock provider configured in `panergosHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(panergosHome: string) {
  ...
  static async start(panergosHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(panergosHome)
