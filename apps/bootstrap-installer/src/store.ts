...
export const $panergosHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.panergos/logs/bootstrap-installer.log')
    $panergosHome.set('~/.panergos')
  ...
  try {
    const [logPath, panergosHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_panergos_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $panergosHome.set(panergosHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Panergos" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Panergos-Setup.exe --update);
...
export async function launchPanergosDesktop(): Promise<void> {
  ...
  await invoke('launch_panergos_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Panergos repository' },
