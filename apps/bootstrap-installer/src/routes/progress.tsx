...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Panergos' : 'Setting up Panergos Agent'

  const description = isUpdate
    ? 'Panergos is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The Panergos installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="panergos-fade-in flex h-full flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {showLogs && (
          <div className="flex w-1/2 flex-col border-l border-(--stroke-eclipse)">
            <div className="flex shrink-0 items-center justify-between border-b border-(--stroke-eclipse) px-3 py-2 text-xs">
      ...
      <div className="flex shrink-0 items-center justify-between border-t border-(--stroke-eclipse) px-6 py-3">
        <button
