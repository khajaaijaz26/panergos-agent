...
//!
//! Driven when the installer is launched as `Panergos-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Panergos desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `panergos update`
//!      or repair bootstrap can race locked files),
//!   2. run `panergos update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in panergos_cli/main.py),
//!   3. run `panergos desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `panergos update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_panergos) and
...
/// `panergos update` exit code meaning "another panergos process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// panergos_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;
const DEFAULT_UPDATE_BRANCH: &str = "main";

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `panergos update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `panergos update` claims
/// the same file (see `panergos_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `panergos update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in panergos_cli/update_lock.py — all three read
...
/// a dashboard-spawned `panergos update`) still blocks.
...
/// the marker name our PID — because a `panergos update` child that
...
async fn run_update(app: AppHandle) -> Result<()> {
    let panergos_home = crate::paths::panergos_home();
    let install_root = panergos_home.join("panergos-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_panergos`
    ...
    //
    // The same marker is the cross-process update lock (panergos_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `panergos update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Panergos update is already running (PID {}, started {} ago). \
    ...
    let update_branch = resolve_update_branch(
        std::env::args().skip(1),
    ...
    let panergos = resolve_panergos(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the Panergos CLI under {}. Is Panergos installed? \
    ...
    // async on Windows. If it still holds the venv shim, `panergos update`
    ...
    // ---- stage 2: panergos update -----------------------------------------
    // Pass --branch so `panergos update` targets the branch this installer was
    ...
    // without --branch, `panergos update` switches the checkout to `main` (a
    ...
    // --force skips `panergos update`'s Windows running-exe guard (which would
    ...
    // time `panergos update` runs there is no legitimate panergos.exe to protect,
    // and the guard would only produce a false "Panergos is still running" stop.
    //
    ...
    // probe only checks the panergos.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Panergos windows" message.
    ...
    // Retry-once for the update-boundary crash. `panergos update` lazily imports
    ...
    // second `panergos update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Panergos" state
    // a retry can't fix.
    ...
    // checkout predating the PANERGOS_UPDATE_HANDOFF_PID env fix (8c76fe19f)
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Panergos is still running. Close all Panergos windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "panergos update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::panergos_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: panergos desktop --build-only ----------------------------
    // `panergos update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `panergos update` already does above, and `panergos update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `panergos desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Panergos manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_panergos_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Panergos to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // Panergos.exe, so an image-name kill would tear down the app itself.
            emit_log(
                app,
                Some(stage),
                LogStream::Stdout,
                &format!(
                    "[handoff] Panergos still holding install files ({}); locating backend shims…",
                    format_locked_paths(&locked)
                ),
            );
            let shim = venv_panergos(install_root);
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_panergos(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Panergos.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Panergos.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Find processes running the exact `venv\Scripts\panergos.exe` shim for this
/// installation. Windows image names are case-insensitive and the desktop is
/// also Panergos.exe, so matching by image name alone is unsafe.
#[cfg(windows)]
...
/// Spawn `panergos <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `panergos update` is precisely the shape that
...
/// Path to the venv Panergos shim under an install root, regardless of existence.
fn venv_panergos(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("panergos.exe")
    } else {
        install_root.join("venv").join("bin").join("panergos")
    }
}

/// Resolve the Panergos CLI to drive, preferring the updated venv shim.
fn resolve_panergos(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_panergos(install_root);
    if shim.exists() {
        return Some(shim);
    }
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let panergos_home = crate::paths::panergos_home();
    let mut envs = vec![(
        "PANERGOS_HOME".to_string(),
        panergos_home.as_os_str().to_os_string(),
    )];
    // `panergos update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `panergos update` child claims that SAME lock (panergos_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Panergos is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in panergos_cli/update_lock.py.
    envs.push((
        "PANERGOS_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        panergos_home.join("node").join("bin"),
...
fn resolve_update_branch<I, S>(args: I, build_pin: Option<String>) -> String
where
    I: IntoIterator<Item = S>,
    S: AsRef<str>,
{
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_panergos_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Panergos.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.panergos-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.panergos-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::panergos_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_panergos_is_under_install_root() {
        let root = Path::new("/x/panergos-agent");
        let shim = venv_panergos(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/panergos-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/panergos-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "PANERGOS_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the panergos update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_panergos(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/panergos-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn same_windows_path_accepts_case_only_difference() {
        assert!(same_windows_path(
            Path::new(r"C:\Users\tester\.panergos\panergos-agent\venv\scripts\PANERGOS.EXE"),
            Path::new(r"c:\users\tester\.panergos\panergos-agent\venv\Scripts\panergos.exe"),
        ));
    }
    ...
    fn same_windows_path_rejects_desktop_binary() {
        assert!(!same_windows_path(
            Path::new(r"C:\Users\tester\.panergos\panergos-agent\apps\desktop\Panergos.exe"),
            Path::new(r"C:\Users\tester\.panergos\panergos-agent\venv\Scripts\panergos.exe"),
        ));
    }
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".panergos-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `panergos update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(panergosHome, child.pid) races ahead
    ...
    // checkout's `panergos update` reads it as a live foreign update and exits
    ...
    fn missing_or_garbage_marker_never_heals() {
        ...
        let garbage = dir.join(".panergos-update-in-progress");
    ...
    fn heal_end_to_end_marker_lifecycle() {
        ...
        // own exit 2, e.g. a genuinely still-running Panergos, stays terminal).
        assert!(!should_heal_self_marker_refusal(
            Some(UPDATE_EXIT_CONCURRENT),
            &marker
        ));
    ...
    #[test]
    fn update_branch_prefers_runtime_then_build_pin_then_panergos_default() {
        assert_eq!(
            resolve_update_branch(["--update", "--branch", "release"], Some("build".into())),
            "release"
        );
        assert_eq!(resolve_update_branch(["--update"], Some("build".into())), "build");
        assert_eq!(resolve_update_branch(["--update"], None), "main");
    }
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Panergos.app"]),
            Some(PathBuf::from("/Applications/Panergos.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Panergos.app");
        let tmp = base.join("Panergos.app.panergos-update-new");
        let old = base.join("Panergos.app.panergos-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Panergos.app");
        let tmp = base.join("Panergos.app.panergos-update-new"); // intentionally absent
        let old = base.join("Panergos.app.panergos-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Panergos.app");
        let tmp = base.join("Panergos.app.panergos-update-new"); // absent
        let old = base.join("Panergos.app.panergos-update-old");
        write_marker(&target, "OLD");
