...
/// read side does not see EOF until the last of them closes it. `panergos update`
...
/// stranded with it.
///
/// Same bound `Invoke-PanergosStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `panergos_home_override` propagates to the child as $PANERGOS_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, panergos_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = panergos_home_override {
        cmd.env("PANERGOS_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, panergos_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = panergos_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_panergos_home() {
    ...
    /// child. `panergos update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
