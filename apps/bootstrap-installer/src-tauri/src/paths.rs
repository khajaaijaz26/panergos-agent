...
//!
//! Mirrors `panergos_constants.get_panergos_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\panergos
//!   macOS:   ~/.panergos
//!   Linux:   ~/.panergos  (override via $PANERGOS_HOME)
//!
//! NOTE (macOS): Python's get_panergos_home(), scripts/install.sh, and the
//! Electron desktop's resolvePanergosHome() ALL use ~/.panergos on macOS — there
...
/// Returns the canonical Panergos home directory, respecting $PANERGOS_HOME if set.
pub fn panergos_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("PANERGOS_HOME") {
    ...
    {
        // %LOCALAPPDATA%\panergos — matches scripts/install.ps1's $PanergosHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("panergos");
        }
    }

    // macOS + Linux + fallback: ~/.panergos (matches Python get_panergos_home(),
    // install.sh, and the Electron desktop's resolvePanergosHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".panergos");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".panergos")
}

pub fn log_dir() -> PathBuf {
    panergos_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    panergos_home().join("bootstrap-cache")
}
...
/// PANERGOS_HOME so it survives repo checkout deletion (unlike anything under
/// panergos-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\panergos\panergos-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_panergos`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under PANERGOS_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves PANERGOS_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    panergos_home().join(".panergos-update-in-progress")
}
...
pub fn copy_self_to_panergos_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to PANERGOS_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_PANERGOS_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".panergos-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under PANERGOS_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[panergos-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("PANERGOS_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_panergos_home() -> String {
    panergos_home().to_string_lossy().into_owned()
}
