...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $PANERGOS_SETUP_DEV_REPO_ROOT
...
//! Electron app's APP_ROOT/../.. trick, because Panergos-Setup.exe is meant
...
const INSTALL_SCRIPT_RAW_BASE: &str =
...
///
/// `pin` is the commit-or-branch from either Panergos-Setup's build-time
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("PANERGOS_SETUP_DEV_REPO_ROOT") {
...
fn download_url(kind: ScriptKind, commit_or_ref: &str) -> String {
    format!(
        "{INSTALL_SCRIPT_RAW_BASE}/{commit_or_ref}/scripts/{}",
        kind.filename()
    )
}

async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = download_url(kind, commit_or_ref);
    ...
    let response = reqwest::Client::builder()
        ...
        .get(&url)
        .header("User-Agent", "panergos-setup/0.0.1")
        .send()
        .await
...
mod tests {
    ...
    #[test]
    fn install_script_url_targets_panergos_distribution() {
        assert_eq!(
            download_url(ScriptKind::Ps1, "main"),
            "https://raw.githubusercontent.com/khajaaijaz26/panergos-agent/main/scripts/install.ps1"
        );
    }
    ...
    fn upgrade_cached_script_adds_bom_to_legacy_ps1() {
        ...
        let dir = std::env::temp_dir().join(format!("panergos-bom-test-{}", std::process::id()));
    ...
    fn upgrade_cached_script_leaves_sh_untouched() {
        let dir = std::env::temp_dir().join(format!("panergos-bom-sh-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
