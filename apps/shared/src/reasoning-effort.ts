/** Panergos' reasoning levels, in ascending order — mirrors the backend's
 *  VALID_REASONING_EFFORTS (panergos_constants.py). `none` is not a level: it's
 *  thinking disabled. */
...
/** Panergos' built-in level when neither the surface nor the profile config
