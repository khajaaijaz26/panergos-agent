...
/** One persisted reaction row (``panergos_state_messages.set_message_reaction``); ``seen`` is stamped once announced. */
...
/** ``provider_configured`` is the loose answer; the boot record's fields (``ready``, ``other_providers`` and ``inference_provider`` ride along on the launch profile. An unknown ``profile`` answers ``ok=False`` + ``error``. */
...
/** One ``panergos_cli/inventory.py::build_models_payload`` provider row (the union of every field the builder sets; ``pricing_pending`` marks the cached-only path). */
...
/** ``panergos_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown, ``"free"``); sale fields are optional provider metadata. */
...
export type ConnectorSetupStatus = 'active' | 'setup_required' | 'unavailable'
...
/** ``panergos_cli/foreign_sessions_browser.py::list_foreign_sessions`` — ``id`` is an opaque handle, never a path. */
...
/** ``list`` → ``plugins`` + counts; ``toggle`` → ``ok``/``unchanged``/``name``/``plugin``; ``install`` → ``panergos_cli.plugins_cmd.dashboard_install_plugin``'s ok payload; ``update`` → ``ok``/``unchanged``/``sha``. */
...
/** ``tui_gateway/change_watcher.py::resolve_skin`` — the resolved active skin (``PanergosSkin``). ``{}`` when the skin engine failed to load. Colour maps are token → colour string. */
...
/** Structured notices via ``notice_callback`` and ``server._await_agent_ready``'s slow-build notice. ``level``: info | warn | error | success; ``kind``: sticky | ttl | agent. */
...
export interface RpcMethods {
  ...
  /** Run ``panergos <argv>`` non-interactively and capture its output; ``blocked`` explains a refusal. */
  ...
  /** Return local connector state or an official Panergos setup guide; never starts OAuth. */
  'connectors.connect': { params: ConnectorsConnectParams; result: ConnectorsConnectResult }
  /** Local messaging connector inventory for one transport-owned session. */
  ...
  /** Re-read ~/.panergos/.env (CLI /reload parity); built agents keep their pool until /new. */
  ...
  /** Export the transcript to ~/.panergos/sessions/saved (classic /save). */
  ...
  /** Token, context, and cost counters for the session. */
