# Panergos Agent Security Policy

This document describes Panergos Agent's trust model, names the one security
boundary the project treats as load-bearing, and defines the scope for
vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [Panergos GitHub Security Advisories](https://github.com/khajaaijaz26/panergos-agent/security/advisories/new).
Do not send Panergos vulnerabilities to unrelated upstream organizations or open
public issues for security vulnerabilities. **Panergos Agent does not operate a bug
bounty program.**
...
- Environment details (`panergos --version`, commit SHA, OS, Python
  version).
- A reproduction against `main` or the latest Panergos release.
...
Panergos Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Panergos Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Panergos Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Panergos Agent's documentation
...
Panergos Agent supports two OS-level isolation postures. They address
...
Panergos supports whole-process wrapping in two ways:

- **A locally built image from this repository's Dockerfile and Compose setup.**
  Panergos 0.1 does not publish a prebuilt image. The agent runs in a standard
  container with operator-configured mounts and network policy.
...
Under a whole-process wrapper, Panergos Agent's in-process heuristics
...
Panergos Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Panergos Agent
itself. Bugs in Panergos Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Panergos Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Panergos Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Panergos Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  panergos-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Panergos Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Panergos Agent. Bugs in Panergos
...
- Run the agent as a non-root user and verify that property in any locally
  built or third-party container image.
...
- Panergos Agent includes supply-chain guards for MCP server
...
- **Channel:** the Panergos GitHub Security Advisory thread.
