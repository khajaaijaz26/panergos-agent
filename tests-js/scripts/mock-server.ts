 ...
 * dev:mock dev flow.
 *
 * Implements just enough of the /v1/* surface for `panergos serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. Panergos should add
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `panergos-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
...
/** Create an isolated PANERGOS_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `panergos-dev-mock-${Date.now()}`))
  const panergosHome = nodePath.join(root, 'panergos-home')
...
function writeMockConfig(panergosHome: string, mockUrl: string): void {
  fs.writeFileSync(
    nodePath.join(panergosHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(nodePath.join(panergosHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function runDevLaunch(): Promise<void> {
  ...
  writeMockConfig(sandbox.panergosHome, mock.url)
  console.log(`  PANERGOS_HOME: ${sandbox.panergosHome}`)
  ...
  const env: Record<string, string> = {
    ...
    PANERGOS_DESKTOP_PANERGOS_ROOT: repoRoot,
    PANERGOS_DESKTOP_APP_NAME: `PanergosDevMock-${Date.now()}`,
  }
