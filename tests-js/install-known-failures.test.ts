...
const lockedLog = [
  'error: failed to remove file `C:/install/venv/Lib/site-packages/../../Scripts/panergos.exe`: Access is denied. (os error 5)',
  'File "C:/install/venv/Scripts/panergos.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'panergos-update',
  error: 'E2E ASSERTION FAILED: panergos update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('panergos.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'panergos-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured panergos desktop spec; update completed',
      logs: { desktop: '[panergos] [updates] no staged updater; surfacing manual `panergos update` for CLI install at C:/install\n[panergos] [updates] manual: panergos update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: panergos update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> panergos-update (v2026.3.12 -> HEAD)'
