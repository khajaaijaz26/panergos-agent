...
try:
    from panergos_cli import (
        ...
        __version__ as PANERGOS_VERSION,
    )
except Exception:
    DISTRIBUTION_NAME = "Panergos Agent"
    DISTRIBUTION_VERSION = "0.0.0"
    PANERGOS_VERSION = "0.0.0"
...
class SlashCommandsMixin:
    """Slash-command surface for ``PanergosACPAgent``; relies on ``_conn``, ``_send``, ``_schedule_soon``,
    ...
    _COMMANDS: dict[str, tuple[str, str, str | None]] = {
        ...
        "version": ("Show Panergos version", "Show Panergos version", None),
    }
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # and ``/model`` REBUILD the system prompt, so unpinned they'd bake the Panergos install tree
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            ...
            toolsets = _expand_acp_enabled_toolsets(getattr(state.agent, "enabled_toolsets", None) or ["panergos-acp"])
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"{DISTRIBUTION_NAME} v{DISTRIBUTION_VERSION} · Panergos runtime v{PANERGOS_VERSION}"
