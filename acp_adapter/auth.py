"""ACP auth helpers — detect and advertise Panergos authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "panergos-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Panergos runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Panergos.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Panergos credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Panergos using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Panergos provider", type="terminal", args=["--setup"],
        description=("Open Panergos' interactive model/provider setup in a terminal. "
                     "Use this when Panergos has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Panergos can resolve any runtime provider credentials."""
