"""CLI entry point for the panergos-agent ACP adapter.

Loads ``~/.panergos/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: panergos acp / panergos-acp
"""

# IMPORTANT: panergos_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See panergos_bootstrap.py for full rationale.
try:
    import panergos_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Partial ``panergos update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Panergos modules.
...
def _load_env() -> None:
    """Load .env from PANERGOS_HOME (default ``~/.panergos``)."""
    from panergos_cli.env_loader import load_panergos_dotenv

    panergos_home = get_panergos_home()
    loaded = load_panergos_dotenv(panergos_home=panergos_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", panergos_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="panergos-acp", description="Run Panergos Agent as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print Panergos version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive Panergos provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.panergos/node/ "
...
def _print_version() -> None:
    from panergos_cli import __version__ as panergos_version

    print(panergos_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import PanergosACPAgent  # noqa: F401

    print("Panergos ACP check OK")


def _run_setup() -> None:
    from panergos_cli.main import main as panergos_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "panergos", "model"]
        panergos_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting Panergos ACP adapter")
    ...
    import acp
    from .server import PanergosACPAgent
    ...
    if os.environ.get("PANERGOS_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
