...
React + Ink terminal UI for Panergos. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `PANERGOS_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `panergos setup`, suspends Ink while it runs, opens a new session on success
...
- `registry.ts` — assembles `SLASH_COMMANDS` from all command files in registration order and exposes `findSlashCommand(name)` for case-insensitive lookup
...
- Input history is stored in `~/.panergos/.panergos_history` or under `PANERGOS_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `panergos setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/panergos-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external panergos setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.panergos/tui-parent.log
    ...
    types/
      panergos-ink.d.ts            type declarations for @panergos/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent PanergosCLI subprocess for slash commands
```
