import { contrastRatio, ensureContrast, mix, parseColor, relativeLuminance, toHex } from '@panergos/shared/color'
import type { SkinBranding, SkinColors } from '@panergos/shared/skin'
...
// ── Color math ───────────────────────────────────────────────────────
//
// Generic color computation lives in @panergos/shared/color (the primitives,
...
// Mirrors @panergos/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @panergos/ink is bundled separately from packages/.
...
// The light variant keeps the Eclipse hue families with readable ink tones.
...
//     background — so a wrong-polarity fill (dark menu on a light terminal)
...
//     (near-white authored text gets rescued).
...
// authored look is the palette rendered essentially raw. The light floor is
// therefore a near-invisible rescue rather than a full recoloring pass.
...
//
// Mix knobs are the single source of truth for custom-skin tone hierarchy.
...
/** Derive coherent secondary tones when a custom skin omits them. */
...
// Terminal. Explicit PANERGOS_TUI_THEME / COLORFGBG signals above still win,
...
// `PANERGOS_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `PANERGOS_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `PANERGOS_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `PANERGOS_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Panergos palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.PANERGOS_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.PANERGOS_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.PANERGOS_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into PANERGOS_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into PANERGOS_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base identity fills only carry over
  //    for the skinless default — a
