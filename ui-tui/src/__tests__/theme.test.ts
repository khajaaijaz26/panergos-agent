...
// PANERGOS_TUI_THEME=light (or PANERGOS_TUI_BACKGROUND set to something
...
describe('DEFAULT_THEME', () => {
  ...
  it('has color palette', async () => {
    ...
    expect(DEFAULT_THEME.color.primary).toBe('#FF6B5E')
    expect(DEFAULT_THEME.color.error).toBe('#FF4773')
  })
})

describe('LIGHT_THEME', () => {
  it('uses readable Eclipse ink tones on light backgrounds', async () => {
    const { LIGHT_THEME } = await importThemeWithCleanEnv()

    expect(LIGHT_THEME.color.primary).toBe('#B03A32')
    expect(LIGHT_THEME.color.accent).toBe('#B03A32')
    expect(LIGHT_THEME.color.muted).toBe('#675873')
    expect(LIGHT_THEME.color.statusWarn).toBe('#8A6110')
  })
...
describe('detectLightMode', () => {
  ...
  it('honors PANERGOS_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ PANERGOS_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ PANERGOS_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ PANERGOS_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ PANERGOS_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ PANERGOS_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets PANERGOS_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', PANERGOS_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors PANERGOS_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ PANERGOS_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ PANERGOS_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', PANERGOS_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', PANERGOS_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses PANERGOS_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ PANERGOS_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ PANERGOS_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ PANERGOS_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ PANERGOS_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ PANERGOS_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ PANERGOS_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ PANERGOS_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ PANERGOS_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ PANERGOS_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ PANERGOS_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // PANERGOS_TUI_THEME=light would flip the base palette and make these
  ...
  it('derives completion current background from resolved completion background (polarity-compatible)', async () => {
    ...
    const { fromSkin } = await importThemeWithEnv({ PANERGOS_TUI_BACKGROUND: '#ffffff' })
  ...
  it('normalizes non-banner foregrounds on light Apple Terminal', async () => {
    ...
    const theme = fromSkin(
      {
        banner_accent: '#2EE6A6',
        banner_border: '#FF6B5E',
        banner_dim: '#B9A9C8',
        banner_text: '#F7F2FF',
        banner_title: '#FF6B5E',
        prompt: '#FF6B5E'
    ...
    expect(theme.color.primary).toBe('#FF6B5E')
    expect(theme.color.accent).toBe('#2EE6A6')
    expect(theme.color.border).toBe('#FF6B5E')
  ...
  it('skinIsLight: the authored background decides; host detection only when absent', async () => {
    const { skinIsLight } = await importThemeWithEnv({ PANERGOS_TUI_BACKGROUND: '#ffffff' })
  ...
  it('keeps truecolor light Apple Terminal in truecolor (adapting, not ansi256-bucketing)', async () => {
    ...
    const theme = fromSkin({ banner_text: '#FFFDF8' }, {})

    // No ansi256 bucketing on truecolor terminals — near-white authored text
...
describe('derived tone ladder', () => {
  it('keeps the Eclipse identity tokens consistent across polarities', async () => {
    const dark = await importThemeWithCleanEnv()
    const light = await importThemeWithEnv({ PANERGOS_TUI_BACKGROUND: '#ffffff' })

    const cases: Array<[string, string, string]> = [
      [dark.DARK_THEME.color.muted, '#B9A9C8', 'dark muted'],
      [dark.DARK_THEME.color.completionBg, '#1A122A', 'dark surface'],
      [dark.DARK_THEME.color.completionCurrentBg, '#332440', 'dark chip'],
      [dark.DARK_THEME.color.selectionBg, '#4A385A', 'dark selection'],
      [light.LIGHT_THEME.color.muted, '#675873', 'light muted'],
      [light.LIGHT_THEME.color.completionBg, '#F7EFF8', 'light surface'],
      [light.LIGHT_THEME.color.completionCurrentBg, '#F7E6E1', 'light chip'],
      [light.LIGHT_THEME.color.selectionBg, '#F9E3DC', 'light selection']
    ]

    for (const [got, expected, label] of cases) {
  ...
  it('derives dim/secondary tones from the skin identity, not another palette', async () => {
    ...
    // accent tint — a red-family derivative, never another skin's palette.
...
describe('background-aware adaptation (OSC-11 light terminals)', () => {
  it('renders a dark-authored skin on light like minimumContrastRatio hosts do (the standardized look)', async () => {
    const { contrastRatio, fromSkin } = await importThemeWithEnv({ PANERGOS_TUI_BACKGROUND: '#ffffff' })
    ...
    // while foregrounds only clear the near-invisible floor (1.18).
  ...
  it('rescues near-invisible colors with a hue-preserving multiplicative lift', async () => {
    const { contrastRatio, fromSkin } = await importThemeWithEnv({ PANERGOS_TUI_BACKGROUND: '#ffffff' })
    // Near-white authored text is genuinely invisible on a light canvas.
    const { color } = fromSkin({ banner_text: '#FFFDF8' }, {})

    expect(color.text.toLowerCase()).not.toBe('#fffdf8')
  ...
  it('leaves the same skin untouched on a dark background', async () => {
    const { fromSkin } = await importThemeWithEnv({ PANERGOS_TUI_BACKGROUND: '#1e1e2e' })
    const { color } = fromSkin(SLATE_COLORS, {})
  ...
  it('empty skin on a light background resolves to the light base palette', async () => {
    const { fromSkin, LIGHT_THEME } = await importThemeWithEnv({ PANERGOS_TUI_BACKGROUND: '#FFF9F6' })
  ...
  it('base palettes are fixed points of the adaptation', async () => {
    ...
    const light = await importThemeWithEnv({ PANERGOS_TUI_BACKGROUND: '#FFF9F6' })

    expect(light.fromSkin({}, {}).color).toEqual(light.LIGHT_THEME.color)
  })

  it('defaultThemeForCurrentBackground follows a late PANERGOS_TUI_BACKGROUND write', async () => {
    ...
    expect(defaultThemeForCurrentBackground({ PANERGOS_TUI_BACKGROUND: '#ffffff' }).color).toEqual(LIGHT_THEME.color)
  })
