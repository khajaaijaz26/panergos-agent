import type { ModelOptionProvider } from '@panergos/shared/gateway-events'
...
describe('ModelPicker reasoning step', () => {
  it('emits one /model request carrying provider, effort and scope', () => {
    expect(modelPickerCommand('gpt-5.6', 'openai', false, 'high')).toBe(
      'gpt-5.6 --provider openai --reasoning high --tui-session'
    )
    expect(modelPickerCommand('gpt-5.6', 'openai', true, 'none')).toBe(
      'gpt-5.6 --provider openai --reasoning none --global'
    )
    // "Keep current effort" (empty value) adds no flag at all.
    expect(modelPickerCommand('gpt-5.6', 'openai', false, '')).toBe('gpt-5.6 --provider openai --tui-session')
    expect(REASONING_PICKER_ROWS.at(-1)?.value).toBe('')
    // The new-session draft label strips the effort flag like it strips --provider.
    expect(draftModelNameFromArg(modelPickerCommand('gpt-5.6', 'openai', false, 'low'))).toBe('gpt-5.6')
  })
