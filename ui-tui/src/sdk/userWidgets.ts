...
import { Box, Text } from '@panergos/ink'
...
/**
 * User widget apps — Panergos authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$PANERGOS_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.panergos/plugins/`: files under PANERGOS_HOME execute
...
const widgetsDir = () => join(process.env.PANERGOS_HOME?.trim() || join(homedir(), '.panergos'), 'tui-widgets')
 ...
 *  every change, so a widget Panergos writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Panergos) ever writes must hot-load too — a 10s poll here read
