import type { InputEvent, Key } from '@panergos/ink'
import * as Ink from '@panergos/ink'
 ...
 * but that accepts `key.meta` on macOS — and panergos-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.PANERGOS_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which panergos-ink decodes as meta+'d'; without this branch it
