...
import type { GatewayEvent } from '@panergos/shared/gateway-events'
import {
  ...
  type ServerRequest,
  wireFrameText
} from '@panergos/shared/json-rpc-channel'
import { reconnectBackoffDelayMs } from '@panergos/shared/reconnect-backoff'
...
const STARTUP_TIMEOUT_MS = Math.max(5000, parseInt(process.env.PANERGOS_TUI_STARTUP_TIMEOUT_MS ?? '15000', 10) || 15000)
const REQUEST_TIMEOUT_MS = Math.max(30000, parseInt(process.env.PANERGOS_TUI_RPC_TIMEOUT_MS ?? '120000', 10) || 120000)
...
// @panergos/shared's JsonRpcRequestChannel; these re-exports keep the TUI's
...
const resolveGatewayAttachUrl = () => {
  const raw = process.env.PANERGOS_TUI_GATEWAY_URL?.trim()

  return raw ? raw : null
}

const resolveSidecarUrl = () => {
  const raw = process.env.PANERGOS_TUI_SIDECAR_URL?.trim()

  return raw ? raw : null
}

const resolvePython = (root: string) => {
  const configured = process.env.PANERGOS_PYTHON?.trim() || process.env.PYTHON?.trim()
...
export class GatewayClient extends EventEmitter {
  ...
  private startSpawnedGateway(root: string) {
    ...
    const cwd = process.env.PANERGOS_CWD || root
    const env = { ...process.env }
    const pyPath = env.PYTHONPATH?.trim()

    env.PYTHONPATH = pyPath ? `${root}${delimiter}${pyPath}` : root
    // Tell the gateway child where the Panergos source root is so its import
    // guard can force it ahead of any same-named package in the launch cwd.
    env.PANERGOS_PYTHON_SRC_ROOT = root
  ...
  start() {
    ...
    const root = process.env.PANERGOS_PYTHON_SRC_ROOT ?? resolve(import.meta.dirname, '../../')
