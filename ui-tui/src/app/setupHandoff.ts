import type { RunExternalProcess } from '@panergos/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`panergos ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching panergos: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`panergos ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
