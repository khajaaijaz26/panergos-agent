...
import { terminalBackgroundHex } from '@panergos/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $PANERGOS_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see PANERGOS_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['PANERGOS_TUI_BACKGROUND', process.env.PANERGOS_TUI_BACKGROUND ?? '(unset)'],
            ['PANERGOS_TUI_THEME', process.env.PANERGOS_TUI_THEME ?? '(unset)'],
