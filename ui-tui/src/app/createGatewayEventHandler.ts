...
import { forceRedraw, onTerminalBackground, onTerminalForeground } from '@panergos/ink'
import { stripAnsi } from '@panergos/shared/ansi'
import { relativeLuminance } from '@panergos/shared/color'
import type { StreamDeltaPayload, SubagentStatus, Usage } from '@panergos/shared/gateway-events'
...
const commitTheme = (theme: Theme) => {
  ...
  const pin = configPinnedTheme ? process.env.PANERGOS_TUI_THEME : undefined

  writeBootTheme(theme, process.env.PANERGOS_TUI_BACKGROUND, pin === 'light' || pin === 'dark' ? pin : undefined)
 ...
 * to PANERGOS_TUI_THEME — the priority-2 signal `detectLightMode` already
 * honors (only an explicit PANERGOS_TUI_LIGHT env var outranks it); 'auto'
 ...
 */
// True once CONFIG (via light/dark) owns the PANERGOS_TUI_THEME env pin, so an
...
export function applyConfiguredTuiTheme(raw: unknown): void {
  ...
  const current = process.env.PANERGOS_TUI_THEME ?? ''
  ...
  if (mode === 'light' || mode === 'dark') {
    ...
    process.env.PANERGOS_TUI_THEME = mode
  } else {
    // 'auto' clears only a pin CONFIG set — never a PANERGOS_TUI_THEME the user
    ...
    delete process.env.PANERGOS_TUI_THEME
  }
 ...
 * is cached into PANERGOS_TUI_BACKGROUND — the slot `detectLightMode` already
 ...
 * PANERGOS_TUI_LIGHT / PANERGOS_TUI_THEME overrides still win inside
...
export function syncThemeToTerminalBackground(): void {
  ...
  onTerminalBackground(hex => {
    ...
    resolved = true
    process.env.PANERGOS_TUI_BACKGROUND = hex
    reapplyTheme()
  })
  ...
  onTerminalForeground(hex => {
    if (resolved || process.env.PANERGOS_TUI_THEME || process.env.PANERGOS_TUI_LIGHT) {
      return
    }
    ...
    resolved = true
    process.env.PANERGOS_TUI_BACKGROUND = inferred
    reapplyTheme()
  })
  ...
  setTimeout(() => {
    if (
      ...
      process.env.PANERGOS_TUI_BACKGROUND ||
      process.env.PANERGOS_TUI_THEME ||
      process.env.PANERGOS_TUI_LIGHT ||
    ...
    execFile('defaults', ['read', '-g', 'AppleInterfaceStyle'], (error, stdout) => {
      if (resolved || process.env.PANERGOS_TUI_BACKGROUND || process.env.PANERGOS_TUI_THEME) {
        return
      }
      ...
      // inference).
      resolved = true
      process.env.PANERGOS_TUI_BACKGROUND = dark ? '#1e1e1e' : '#ffffff'
      reapplyTheme()
    })
  }, 1500).unref?.()
}
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: AnyGatewayEvent) => void {
  ...
  const scheduleStartupPrompt = () => {
    ...
    setTimeout(async () => {
      ...
      // launches, `panergos --tui -q "…"`) — submit LITERALLY, bypassing the
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // Arm "Hey Panergos" if this surface owns it (server gates on config).
  ...
  return (ev: AnyGatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'wake.detected': {
        // "Hey Panergos": optionally open a fresh session (start_new_session),
        ...
        void (async () => {
          ...
          if (wakeProfile && wakeProfile !== ownProfile) {
            sys(`wake phrase for profile '${wakeProfile}' — run: panergos -p ${wakeProfile} --tui`)
