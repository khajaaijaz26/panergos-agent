 ...
 * `panergos-boot-background` / `panergos-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports PANERGOS_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.panergos matches get_panergos_home()'s default.
const bootFilePath = () => join(process.env.PANERGOS_HOME ?? join(homedir(), '.panergos'), 'tui-theme-boot.json')

// Never touch the user's real ~/.panergos from test runs (the TS suite has no
// PANERGOS_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into PANERGOS_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.PANERGOS_TUI_THEME || env.PANERGOS_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.PANERGOS_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.PANERGOS_TUI_BACKGROUND
  ) {
    env.PANERGOS_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.PANERGOS_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.PANERGOS_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into PANERGOS_TUI_THEME.
