...
//
// Both gate on PANERGOS_DEV_PERF=1 and dump JSON-lines (default ~/.panergos/perf.log,
// override PANERGOS_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// PANERGOS_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
import { scrollFastPathStats } from '@panergos/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.PANERGOS_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.PANERGOS_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.PANERGOS_DEV_PERF_LOG?.trim() || join(homedir(), '.panergos', 'perf.log')
