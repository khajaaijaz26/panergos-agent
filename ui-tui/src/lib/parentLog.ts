...
const logDir = join(process.env.PANERGOS_HOME?.trim() || join(homedir(), '.panergos'), 'logs')
...
// a real ~/.panergos (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('panergos-tui: parent lifecycle log unavailable\n')
    }
  }
}
