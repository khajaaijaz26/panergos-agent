...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with panergos-agent)
2. User plugins: ``$PANERGOS_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``panergos_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$PANERGOS_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$PANERGOS_HOME/plugins/model-providers/`` if it exists."""
    try:
        from panergos_constants import get_panergos_home

        d = get_panergos_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$PANERGOS_HOME/plugins/`` if it exists.

    This is where ``panergos plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple PANERGOS_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_panergos_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``panergos_agent.plugins``
    ...
    * **Provider targets only.** The ``panergos_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$PANERGOS_HOME``) keep their
    ...
    try:
        from panergos_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$PANERGOS_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``panergos plugins install`` at
          ``$PANERGOS_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``panergos_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$PANERGOS_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with panergos-agent.
    ...
    # 2. User plugins — under $PANERGOS_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``panergos plugins install`` / the plugin index.
    #     Those clone into $PANERGOS_HOME/plugins/<name>/ — flat, NOT under
