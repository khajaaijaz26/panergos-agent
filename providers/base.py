...
def _profile_user_agent() -> str:
    """Return a ``panergos-agent/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from panergos_cli import __distribution_version__ as _ver  # lazy: avoid layer cycle at import time
        return f"panergos-agent/{_ver}"
    except Exception:
...
class ProviderProfile:
    ...
    # describe how to launch it; panergos_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.panergos/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # panergos-cli UA so the catalog endpoint is reachable.
