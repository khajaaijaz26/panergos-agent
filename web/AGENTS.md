# web/ + panergos_cli/web_routers/ — the dashboard (`panergos dashboard` → `/chat`)

Applies on top of the root `AGENTS.md`. Backend routers: `panergos_cli/web_routers/*.py`, one file per
dashboard surface, mounted by `panergos_cli/web_server.py` (+ `web_server_*.py` siblings). Frontend:
`web/src/`. Shared JSON-RPC/WS client: `apps/shared` (`@panergos/shared`), also used by the desktop.

## The dashboard embeds the REAL `panergos --tui` — not a rewrite

`panergos_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `web_server.py`:
...
- The server spawns exactly what `panergos --tui` would spawn, through `ptyprocess` on POSIX or
  `pywinpty`/ConPTY on native Windows.
...
`cmd_dashboard` skips `_build_web_ui` and exports `PANERGOS_SERVE_HEADLESS=1` so `mount_spa()`
...
- Routers are one-file-per-surface; a new surface is a new `web_routers/<surface>.py`, not a growing
  `web_server.py`.
- Tests: Python in `tests/panergos_cli/` (routers, pty bridge); JS in the `web/` vitest suite. Python
