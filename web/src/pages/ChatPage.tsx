/**
 * ChatPage — embeds `panergos --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (panergos_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "panergos.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__PANERGOS_SESSION_TOKEN__ &&
    !window.__PANERGOS_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `panergos dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__PANERGOS_AUTH_REQUIRED__;
    ...
    //      payloads upload to PANERGOS_HOME/images then drive `/image`.
    //
    ...
    // WebSocket. In gated mode (``window.__PANERGOS_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets PANERGOS_HOME pointed at the
