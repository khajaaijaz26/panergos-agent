...
export default function SystemPage() {
  ...
  // spawned `panergos import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_panergos === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkPanergosUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_panergos],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_panergos === false) {
      showToast(
        "Panergos Agent updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updatePanergos();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "panergos-update");
  ...
  const canUpdatePanergos = status?.can_update_panergos !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdatePanergos && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Panergos Agent?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'panergos update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'panergos update' (${updateInfo?.update_command ?? "panergos update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Panergos Agent</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.panergos_version}</span>
                  {canUpdatePanergos &&
                    updateInfo &&
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$PANERGOS_HOME/backups/panergos-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Panergos Agent backup?"
              description={`This will overwrite your current Panergos Agent configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Panergos maintainers. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
