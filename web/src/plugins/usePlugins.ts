...
export const MANIFEST_CACHE_KEY = "panergos:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${PANERGOS_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${PANERGOS_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?panergos_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-panergos-plugin", manifest.name);
