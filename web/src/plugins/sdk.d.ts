/**
 * Panergos Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__PANERGOS_PLUGIN_SDK__`` and
 * ``window.__PANERGOS_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__PANERGOS_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@panergos/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``PanergosPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@panergos/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @panergos/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__PANERGOS_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__PANERGOS_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__PANERGOS_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface PanergosPluginSDK {
  ...
  /**
   * Shared UI primitives (Panergos UI / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __PANERGOS_PLUGIN_SDK__?: PanergosPluginSDK;
    __PANERGOS_PLUGINS__?: PluginRegistry;
  }
}
