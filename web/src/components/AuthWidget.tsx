 ...
 * OAuth gate (Phase 7 of .panergos/plans/2026-05-21-dashboard-oauth-auth.md).
 *
 * Renders nothing in loopback / --insecure mode. In gated mode, fetches
 * /api/auth/me on mount and surfaces:
 *
 *   - the user_id (truncated to 14 chars + ellipsis) when the provider does
 *     not emit email/display_name claims
...
export function AuthWidget({ className }: AuthWidgetProps) {
  ...
  const gated =
    typeof window !== "undefined" && !!window.__PANERGOS_AUTH_REQUIRED__;
