...
interface PanergosConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function PanergosConsoleModal({ open, onClose }: PanergosConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("panergos> ");
  const inputPromptRef = useRef("panergos> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "panergos> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Panergos Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="panergos-console-title"
    >
      <div
        className={cn(
          themedBody,
