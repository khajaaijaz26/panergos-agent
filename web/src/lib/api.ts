...
// (e.g. https://mission-control.tilos.com/panergos/). The Python backend
// injects ``window.__PANERGOS_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__PANERGOS_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const PANERGOS_BASE_PATH = readBasePath();
const BASE = PANERGOS_BASE_PATH;
...
declare global {
  interface Window {
    __PANERGOS_SESSION_TOKEN__?: string;
    __PANERGOS_BASE_PATH__?: string;
    ...
    __PANERGOS_AUTH_REQUIRED__?: boolean;
  }
}
...
export async function fetchJSON<T>(
  ...
  const token = window.__PANERGOS_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``panergos update``, ``panergos gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__PANERGOS_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__PANERGOS_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__PANERGOS_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``panergos_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__PANERGOS_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Panergos-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``panergos_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__PANERGOS_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildPanergosWebSocketUrl({
...
export const api = {
  ...
  updatePanergos: () =>
    fetchJSON<ActionResponse>("/api/panergos/update", { method: "POST" }),
  checkPanergosUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/panergos/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * some provider contracts (when the access token has no email/name claims).
 * The AuthWidget surfaces a truncated ``user_id`` instead.
 */
...
/** Preflight for `panergos gateway migrate --multiplex` (mirrors the CLI plan JSON). */
...
export interface SkillHubSource {
  ...
  /** panergos-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
  ...
  /** Registered ``DashboardAuthProvider`` names.
   ...
   * updates are handled by the outer launcher instead of ``panergos update``. */
  can_update_panergos?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the PANERGOS_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the PANERGOS_HOME volume — no staleness dimension, so no
 * sampled_at. */
