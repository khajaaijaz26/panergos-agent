...
const STORAGE_KEY = "panergos-dashboard-theme";
...
const FONT_STORAGE_KEY = "panergos-dashboard-font";
...
const THEME_NAME_ALIASES: Record<string, string> = {
  // Early preview name retained for local preference migration.
  "lens-5i": "porcelain",
};
 ...
 *  `_THEME_NAMED_ASSET_KEYS` in `panergos_cli/web_server.py`. */
...
const CUSTOM_CSS_STYLE_ID = "panergos-theme-custom-css";
...
function applyCustomCSS(css: string | undefined) {
  ...
  if (!el) {
    ...
    el.setAttribute("data-panergos-theme-css", "true");
...
function injectFontStylesheet(url: string | undefined) {
  ...
  link.rel = "stylesheet";
  link.href = url;
  link.setAttribute("data-panergos-theme-font", "true");
...
function applyTheme(theme: DashboardTheme) {
  ...
  // (e.g. Panergos Mist) doesn't leave its values behind when the user
