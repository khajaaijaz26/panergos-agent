# Panergos Agent — Web UI

Browser-based dashboard for managing Panergos Agent configuration, API keys, and monitoring active sessions.
...
`panergos dashboard` on port 9119 serves the **built** bundle from `panergos_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `dashboard --no-open` + Vite as above).
...
This outputs to `../panergos_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
- Use the **semantic text tokens** from `@panergos/ui`'s `globals.css`:
...
- The dashboard preserves the Panergos brand display aesthetic, but it is **opt-in per element, not global**.
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `panergos-agent/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Panergos Agent” — mixed case, no Mondwest, no `text-display` |
