...
const BACKEND = process.env.PANERGOS_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `panergos dashboard` server injects a one-shot
 * session token into `index.html` (see `panergos_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__PANERGOS_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function panergosDevToken(): Plugin {
  const TOKEN_RE = /window\.__PANERGOS_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__PANERGOS_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "panergos:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[panergos] Could not find session token in ${BACKEND} — ` +
              `is \`panergos dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__PANERGOS_SESSION_TOKEN__="${match[1]}";` +
              `window.__PANERGOS_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[panergos] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`panergos dashboard\` or set PANERGOS_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@panergos/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
    // When @panergos/ui is symlinked via `file:../../design-language`,
  ...
  server: {
    proxy: {
      ...
      // Same host as `panergos dashboard` must serve these; Vite has no
