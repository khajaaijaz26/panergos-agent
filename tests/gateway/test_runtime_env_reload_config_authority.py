...
PANERGOS_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env PANERGOS_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``panergos config get terminal.backend`` still says local.
    """
    panergos_home = tmp_path / ".panergos"
    panergos_home.mkdir()
    (panergos_home / "config.yaml").write_text(
    ...
    (panergos_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
