...
``panergos -p X`` dispatcher would build, and X's notifications are rendered/filtered under X's
config.
"""
...
def served(tmp_path, monkeypatch):
    ...
    (root / ".env").write_text("PANERGOS_MODEL=default-model\nTERMINAL_ENV=docker\n")
    ...
    monkeypatch.setenv("PANERGOS_MODEL", "default-model")
    monkeypatch.setenv("TERMINAL_ENV", "docker")
    monkeypatch.setenv("PANERGOS_KANBAN_DB", str(tmp_path / "board.db"))
...
def test_worker_for_served_profile_gets_its_own_env_and_toolset_pin(served, monkeypatch):
    ...
    env = spawned["env"]
    assert env["PANERGOS_HOME"] == str(served.alpha)
    assert "PANERGOS_MODEL" not in env and "TERMINAL_ENV" not in env
...
class RecordingAdapter:
    ...
    async def send(self, chat_id, text, metadata=None):
        ...
        self.sent.append({"text": text, "home": str(get_panergos_home()), "strict": media_delivery_strict()})
