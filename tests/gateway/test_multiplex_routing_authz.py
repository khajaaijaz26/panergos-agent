...
Every test builds a bare ``GatewayRunner`` with stub adapters against a temp ``PANERGOS_HOME`` and
...
def mux(tmp_path, monkeypatch):
    ...
    with patch("panergos_cli.profiles.profiles_to_serve", return_value=served), \
            patch("panergos_cli.profiles.get_profile_dir", side_effect=lambda n: home / "profiles" / n), \
            patch("panergos_cli.profiles.profile_exists", return_value=True):
