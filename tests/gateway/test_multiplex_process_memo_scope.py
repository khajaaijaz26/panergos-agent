...
(PANERGOS_HOME override), not the launch home the module constants were frozen from."""
...
def two_homes(tmp_path, monkeypatch):
    """Launch home A (PANERGOS_HOME) and a routed profile B with different config everywhere."""
    a = tmp_path / ".panergos"
    b = a / "profiles" / "b"
...
def test_max_turns_and_fallback_chain_follow_routed_profile(two_homes, monkeypatch):
    ...
    monkeypatch.setattr(gateway_run, "_panergos_home", a)
...
async def test_gateway_hooks_fire_per_routed_profile(two_homes):
    ...
    token = set_panergos_home_override(str(b))
    try:
        ctx_b: dict = {}
        ...
        reset_panergos_home_override(token)
    assert ctx_a.get("seen") == ["panergos_hook_hook-a"]
    assert ctx_b.get("seen") == ["panergos_hook_hook-b"]
