...
class LongPreviewAgent:
    ...
    LONG_CMD = "cd /home/teknium/.panergos/panergos-agent/.worktrees/panergos-d8860339 && source .venv/bin/activate && python -m pytest tests/gateway/test_run_progress_topics.py -n0 -q"
...
class UrlPreviewAgent:
    URL = "https://docs.example.com/gateway/discord/tool-progress"
...
async def test_run_agent_progress_uses_event_message_id_for_slack_dm(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("PANERGOS_TOOL_PROGRESS_MODE", "all")
...
async def test_slack_null_tool_progress_is_inheritance_not_explicit_off(monkeypatch, tmp_path, display_cfg, env_mode):
    ...
    monkeypatch.delenv("PANERGOS_TOOL_PROGRESS_MODE", raising=False)
    if env_mode is not None:
        monkeypatch.setenv("PANERGOS_TOOL_PROGRESS_MODE", env_mode)
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        DuplicateNativeToolsAgent,
...
async def test_terminal_progress_verbose_shows_full_command(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("PANERGOS_TOOL_PROGRESS_MODE", "verbose")
