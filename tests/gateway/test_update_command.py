...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _panergos_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._panergos_home", tmp_path), \
        ...
        with patch("gateway.run._panergos_home", tmp_path):
            ...
            # project_root will be the real panergos-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_resolve_panergos_bin_fallback(self):
        """_resolve_panergos_bin falls back to sys.executable argv when which fails."""
        import sys
        ...
        with patch("shutil.which", return_value=None), \
            ...
            result = _resolve_panergos_bin()

        assert result == [sys.executable, "-m", "panergos_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_panergos_bin_prefers_panergos(self):
        """The canonical binary is resolved from PATH."""
        from gateway.run import _resolve_panergos_bin

        with patch("shutil.which", side_effect=lambda name: f"/usr/bin/{name}"):
            result = _resolve_panergos_bin()

        assert result == ["/usr/bin/panergos"]
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._panergos_home", panergos_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/panergos" if x == "panergos" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = panergos_home / ".update_pending.json"
    ...
    async def test_fallback_when_no_setsid(self, tmp_path):
        ...
        assert "Starting Panergos update" in result
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_MANAGED", "")
        ...
        # Later steps may legitimately return success ("Starting Panergos
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (panergos_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = panergos_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (panergos_home / ".update_output.txt").write_text("done")
        (panergos_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (panergos_home / ".update_pending.json").write_text(json.dumps(pending))
        (panergos_home / ".update_output.txt").write_text(
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert not (panergos_home / ".update_pending.claimed.json").exists()
    ...
    async def test_completion_notification_tolerates_invalid_utf8_output(self, tmp_path):
        ...
        assert "Panergos update finished" in sent_text
        assert not pending_path.exists()
        assert not output_path.exists()
        assert not exit_code_path.exists()
...
class TestWatchUpdateProgress:
    ...
    async def test_invalid_utf8_update_output_does_not_crash_watcher(self, tmp_path):
        ...
        (panergos_home / ".update_pending.json").write_text(json.dumps({
            "platform": "telegram",
            "chat_id": "67890",
            "user_id": "12345",
        }))
        (panergos_home / ".update_output.txt").write_bytes(
        ...
        assert "Panergos update finished" in sent
        assert not (panergos_home / ".update_pending.json").exists()
