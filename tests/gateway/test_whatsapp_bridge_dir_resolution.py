...
(/opt/panergos/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable PANERGOS_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_panergos_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable PANERGOS_HOME."""
    ...
    monkeypatch.setattr(
        "panergos_constants.get_panergos_home", lambda: panergos_home
    )
    ...
    expected = panergos_home / "scripts" / "whatsapp-bridge"
