...
class TestApiServerModelsUnderProfile:
    def test_resolve_model_name_follows_active_profile(self, monkeypatch):
        ...
        adapter = _make_adapter(multiplex=True)
        adapter._model_name = "panergos-agent"
        monkeypatch.setattr(
...
class TestApiServerSessionProfileBinding:
    """PANERGOS_SESSION_PROFILE must be bound per /p/<profile>/ request.
    ...
    so every API-server turn bound PANERGOS_SESSION_PROFILE="" and the
    ...
    def test_profile_is_bound_into_session_vars(self):
        ...
        try:
            assert get_session_env("PANERGOS_SESSION_PROFILE") == "nm-media"
        finally:
