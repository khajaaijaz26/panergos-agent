...
async def test_run_agent_binds_api_session_context_for_tool_env(adapter, monkeypatch):
    ...
    monkeypatch.setenv("PANERGOS_SESSION_ID", "stale-session")
    observed = {}
    ...
    class FakeAgent:
        ...
        def run_conversation(self, user_message, conversation_history, task_id):
            ...
            observed["context_session_id"] = get_session_env("PANERGOS_SESSION_ID")
            observed["context_platform"] = get_session_env("PANERGOS_SESSION_PLATFORM")
            observed["context_session_key"] = get_session_env("PANERGOS_SESSION_KEY")
            observed["child_session_id"] = _make_run_env({}).get("PANERGOS_SESSION_ID")
...
async def test_session_chat_treats_pre_existing_poisoned_row_as_no_model(session_db):
    ...
    virtual model alias (e.g. "panergos-agent") persisted literally as its
...
async def test_session_model_lock_endpoint_then_chat_reuses_persisted_lock_and_provider_credentials(
    ...
    assert captured["provider"] == "example"
    assert captured["model"] == "x-ai/grok-4.5"
    assert captured["api_key"] == "sk-example"
    assert captured["base_url"] == "https://example.example/v1"
    assert payload["runtime"]["provider"] == "example"
...
async def test_session_model_lock_endpoint_then_chat_stream_reuses_persisted_lock(
        ...
        return (
            {
                ...
                "runtime": {
                    "provider": "example",
                    "model": "x-ai/grok-4.5",
                    "requested": {"provider": "example", "model": "x-ai/grok-4.5"},
    ...
    assert captured["route"] == {"provider": "example", "model": "x-ai/grok-4.5"}
    assert captured["requested_runtime"]["provider"] == "example"
...
async def test_confirmed_runtime_lock_rejects_actual_runtime_mismatch(adapter, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="confirmed model lock runtime mismatch"):
        await adapter._run_agent(
            ...
            conversation_history=[],
            session_id="mismatch-session",
            route={"provider": "example", "model": "x-ai/grok-4.5"},
            requested_runtime={"provider": "example", "model": "x-ai/grok-4.5"},
            route_source="session_model_lock",
...
async def test_unconfirmed_request_does_not_replace_confirmed_session_lock(adapter, session_db):
    ...
    assert config["browser_model_lock"]["provider"] == "example"
