...
class TestResolutionOrder:
    ...
    def test_source_profile_wins_over_routing(self, mock_runner, discord_source):
        ...
        with patch("panergos_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("panergos_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("panergos_cli.profiles.profile_exists", return_value=True):
                    mock_get_dir.return_value = Path("/panergos/profiles/from-source")
                    result = mock_runner._resolve_profile_home_for_source(discord_source)
                    
                    assert result == Path("/panergos/profiles/from-source")
...
class TestMissingProfileWarning:
    ...
    def test_nonexistent_profile_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("panergos_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("panergos_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/panergos/profiles/nonexistent")
                with patch("panergos_cli.profiles.profile_exists", return_value=False):
                    with patch("panergos_constants.get_panergos_home", return_value=Path("/panergos")):
                        with caplog.at_level(logging.WARNING):
                            result = mock_runner._resolve_profile_home_for_source(discord_source)
                            
                            # Should fall back to global PANERGOS_HOME
                            assert result == Path("/panergos")
...
class TestExceptionHandling:
    ...
    def test_get_profile_dir_exception_logs_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("panergos_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("panergos_cli.profiles.get_profile_dir", side_effect=ValueError("Invalid profile name")):
                with patch("panergos_constants.get_panergos_home", return_value=Path("/panergos")):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
...
class TestRoutingConsultation:
    ...
    def test_routing_consulted_when_source_profile_empty(self, mock_runner, discord_source):
        ...
        with patch("panergos_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("panergos_cli.profiles.get_profile_dir") as mock_get_dir:
                mock_get_dir.return_value = Path("/panergos/profiles/routed")
...
class TestNonDiscordProfileRouting:
    ...
    def test_telegram_route_resolves(self, mock_runner, telegram_source):
        ...
        with patch(
            "panergos_cli.profiles.profiles_to_serve",
