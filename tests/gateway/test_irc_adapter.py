...
class TestIRCAdapterInit:
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        assert adapter.nickname == "panergos"
        assert adapter.channel == "#panergos-dev"
...
class TestIRCAdapterLockConflict:
    ...
    async def test_connect_fails_when_identity_lock_held(self, monkeypatch):
        ...
        adapter = IRCAdapter(PlatformConfig(enabled=True, extra={
            "server": "irc.example", "nickname": "panergos", "channel": "#x"}))
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_addressed_channel_message(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG #test :panergos: hello there")
    ...
    async def test_ctcp_action_converted(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG panergos :\x01ACTION waves\x01")
        assert len(dispatched) == 1
    ...
    async def test_unauthorized_user_blocked(self, monkeypatch):
        ...
        adapter = IRCAdapter(cfg)
        adapter._current_nick = "panergos"
        ...
        await adapter._handle_line(":eve!u@host PRIVMSG #test :panergos: hello")
        assert len(dispatched) == 0
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_completes_handshake_and_sends_privmsg(self, monkeypatch):
        ...
        monkeypatch.setenv("IRC_NICKNAME", "panergosbot")
        monkeypatch.setenv("IRC_USE_TLS", "false")

        # Server greets us with 001 RPL_WELCOME, then nothing for QUIT drain.
        conn = _FakeIRCConnection([b":server 001 panergosbot-cron :Welcome"])
        ...
        assert any(line.startswith("NICK panergosbot-cron") for line in sent_lines)
        assert any(line.startswith("USER panergosbot-cron 0 * :Panergos Agent (cron)")
...
class TestMultiplexProfileScope:
    ...
    def test_secondary_missing_keys_fail_closed(
        ...
        assert adapter.nickname == "panergos-bot"
