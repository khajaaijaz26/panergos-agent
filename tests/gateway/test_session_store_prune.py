...
def test_session_store_default_db_uses_runtime_panergos_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime PANERGOS_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    panergos_state before a fixture redirected PANERGOS_HOME used to pin every
    default SessionDB() at the developer's real ~/.panergos/state.db.
    """
    config = GatewayConfig()
    fake_home = tmp_path / "alt_panergos_home"
    fake_home.mkdir()
    monkeypatch.setenv("PANERGOS_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "panergos sessions list" in raw["_README"]
