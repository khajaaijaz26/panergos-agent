...
class TestGetDefaultModelForProvider:
    """Unit tests for panergos_cli.models.get_default_model_for_provider."""
    ...
    def test_catalog_label_overrides_constant(self):
        ...
        with patch(
            "panergos_cli.model_catalog.get_default_model_from_cache",
            return_value="qwen/qwen3.8-max-0902",
        ):
            assert (
                models_mod.get_preferred_silent_default_model("example")
                == "qwen/qwen3.8-max-0902"
            )
            # example catalog carries qwen3.8-max-0902, so the full resolver follows.
            assert (
                models_mod.get_default_model_for_provider("example")
                == "qwen/qwen3.8-max-0902"
            )
