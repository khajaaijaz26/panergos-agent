...
_FAKE_TOKEN = "panergos-test-bearer-credential-ABCDEFGHIJKLMNOPQRSTUVWX"
_FAKE_URL_SECRET = "https://hooks.example/send?access_token=sk_live_ABCDEF0123456789"


@pytest.fixture(autouse=True)
def _redaction_on(monkeypatch):
    # The switch is snapshotted at import; a developer shell with PANERGOS_REDACT_SECRETS=false must not
...
async def test_ntfy_standalone_failure_is_redacted(monkeypatch):
    ...
    result = await ntfy._standalone_send(PlatformConfig(enabled=True, extra={}), "panergos-test", "hi")
