...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.panergos/.update_response."""
        adapter = self._make_adapter()
        panergos_home = tmp_path / "panergos_home"
        panergos_home.mkdir()
        monkeypatch.setattr(
            "panergos_constants.get_panergos_home",
            lambda: panergos_home,
        )
        ...
        response = panergos_home / ".update_response"
