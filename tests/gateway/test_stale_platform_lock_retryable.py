...
``gateway run --replace`` only kills same-PANERGOS_HOME PID-file holders.
...
def test_lock_conflict_names_owning_profile(adapter):
    ...
    existing = {
        ...
        "panergos_home": "/opt/data/profiles/lead-gen-outreach",
    }
    ...
    assert adapter._fatal_error_message == (
        ...
        "(panergos --profile lead-gen-outreach gateway stop)."
    )
    assert adapter._fatal_error_retryable is True
    assert adapter._fatal_error_code == "telegram-bot-token_lock"


def test_lock_conflict_infers_profile_from_legacy_panergos_home(adapter):
    """Locks written before the profile field existed still attribute via panergos_home."""
    existing = {
        "pid": 559,
        "start_time": 123,
        "panergos_home": "/opt/data/profiles/lead-gen-outreach",
