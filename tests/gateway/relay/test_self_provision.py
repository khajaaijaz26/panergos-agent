...
The trigger is deliberately independent of package-manager supervision. The
real gate is "relay_url set + no pinned secret + a resolvable identity token".
"""
...
def _arm(monkeypatch, *, url="wss://connector.example/relay", token="identity-token"):
...
def test_provisions_and_sets_env_in_process(monkeypatch):
    ...
    assert captured["access_token"] == "identity-token"
...
def test_no_identity_token_is_non_fatal(monkeypatch):
    """A self-hosted box with a relay URL but no resolvable identity skips quietly."""
...
def test_relay_display_name_suppresses_stock_brand(monkeypatch):
    """The default 'Panergos Agent' brand is identical on every install — forwarding
    ...
    class _Skin:
        def get_branding(self, key, fallback=""):
            return "Panergos Agent" if key == "agent_name" else fallback

    monkeypatch.setattr("panergos_cli.skin_engine.get_active_skin", lambda: _Skin())
