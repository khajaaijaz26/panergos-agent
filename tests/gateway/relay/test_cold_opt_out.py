...
def test_cold_fronted_platforms_is_read_only(tmp_path, case):
    ...
    # pytest's imported modules. HOME and cwd are temporary as well as PANERGOS_HOME.
    ...
    env.update({"HOME": str(tmp_path), "USERPROFILE": str(tmp_path),
                "PANERGOS_HOME": str(home), "PANERGOS_MANAGED_DIR": str(managed),
    ...
    code = textwrap.dedent("""
        ...
        if sys.argv[1] == "scoped":
            set_panergos_home_override(sys.argv[2])
