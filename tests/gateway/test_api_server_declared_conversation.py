...
``X-Panergos-Session-Key``.  Every conversation-affinity hint Panergos sends is
derived from that physical id — ``prompt_cache_key`` on both OpenAI-wire
transports, the OpenRouter/example sticky ``session_id``, and xAI's
...
def _headers(session_key=None):
    ...
    if session_key:
        h["X-Panergos-Session-Key"] = session_key
    return h
