...
class TestTelegramYamlConfigLoading:
    ...
    def _write_config(self, tmp_path, content: str):
        ...
        panergos_home.mkdir()
        (panergos_home / "config.yaml").write_text(content, encoding="utf-8")
    ...
    def test_extra_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        panergos_home = self._write_config(
            tmp_path, "telegram:\n  extra:\n    reply_to_mode: \"off\"\n"
        )
...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/khajaaijaz26/panergos-agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via Panergos-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/khajaaijaz26/panergos-agent/issues/87051:
    ...
    response in a different chat lane than the Panergos topic the session runs
    in. The no-anchor branch must prefer the Panergos topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_panergos_topic_thread_id(self):
