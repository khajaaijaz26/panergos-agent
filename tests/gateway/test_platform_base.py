...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    panergos_home.mkdir()
    monkeypatch.setattr(base, "_PANERGOS_HOME", panergos_home)
    monkeypatch.setattr(base, "_PANERGOS_ROOT", panergos_home)
    path = panergos_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestSecretCaptureGuidance:
    def test_gateway_secret_capture_message_points_to_local_setup(self):
        ...
        assert "~/.panergos/.env" in message
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.panergos/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("PANERGOS_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("PANERGOS_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("PANERGOS_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("PANERGOS_KANBAN_HOME", str(tmp_path / "panergos"))
        monkeypatch.setenv("PANERGOS_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "panergos" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("PANERGOS_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("PANERGOS_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("PANERGOS_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.panergos/mcp-tokens/ must
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._PANERGOS_ROOT",
            panergos_dir,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the PANERGOS_HOME root (google_token.json)
    ...
    def test_denylist_blocks_non_cache_file_under_panergos_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.panergos
    ...
    def test_denylist_blocks_session_stores_but_not_neighbouring_artifacts(self, tmp_path, monkeypatch):
        ...
        pasted into a chat; ``MEDIA:~/.panergos/state.db`` must not exfiltrate them (#41071). Named
        ...
        for rel in denied + allowed:
            path = panergos_dir / rel
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_bytes(b"SQLite format 3\x00")
        assert [rel for rel in denied if BasePlatformAdapter.validate_media_delivery_path(str(panergos_dir / rel))] == []
        assert [rel for rel in allowed if not BasePlatformAdapter.validate_media_delivery_path(str(panergos_dir / rel))] == []
    ...
    def test_denylist_covers_every_profile_home_not_just_the_launch_home(self, tmp_path, monkeypatch):
        ...
        turn is the launch (default) profile's or the secondary's own (PANERGOS_HOME override), while
        ...
        for scope in (None, profile_b):  # default profile's turn, then beta's own turn
            token = set_panergos_home_override(scope)
            try:
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting PANERGOS_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose PANERGOS_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.panergos) emits
        ...
        panergos_root = denied_root / ".panergos"
        prof_cache = panergos_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._PANERGOS_ROOT", panergos_root
        )
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        """MEDIA:/root/.panergos/cache/images/... (the agent_visible_image path
        under docker) must translate to the HOST cache file, not the sandbox
        home copy."""
        panergos_home = tmp_path / ".panergos"
        cache = panergos_home / "cache" / "images"
    ...
    def test_container_credential_path_never_translates_through_home(self, tmp_path, monkeypatch):
        """/root/.panergos/* outside a cache mount (the sandbox's credential
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "PANERGOS_HUMAN_DELAY_MODE": "natural",
            "PANERGOS_HUMAN_DELAY_MIN_MS": "oops",
            "PANERGOS_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_tolerates_malformed_env_vars(self):
        env = {
            "PANERGOS_HUMAN_DELAY_MODE": "custom",
            "PANERGOS_HUMAN_DELAY_MIN_MS": "oops",
            "PANERGOS_HUMAN_DELAY_MAX_MS": "still-bad",
        }
        with patch.dict(os.environ, env):
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"PANERGOS_MEDIA_DELIVERY_STRICT": "1",
                                     "PANERGOS_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Panergos
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../panergos/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.panergos/cache/media/sensitive_host_path_abc123.bin"
...
class TestDockerProfileSandboxMediaTranslation:
    ...
    def test_home_credential_surface_still_refused(self, monkeypatch):
        """The /root/.panergos exclusion survives profile scoping: translating
        ...
        for task in ("default", f"session:{self.SESSION_KEY}"):
            secrets = self._sandbox_dir(task) / "home" / ".panergos"
            secrets.mkdir(parents=True, exist_ok=True)
            (secrets / "auth.json").write_text("{}")

        assert (
            BasePlatformAdapter.validate_media_delivery_path(
                "/root/.panergos/auth.json", session_key=self.SESSION_KEY
            )
            is None
        )
