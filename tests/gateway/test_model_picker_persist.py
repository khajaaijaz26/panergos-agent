...
closure the PR changed, against a real temp ``PANERGOS_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "panergos_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_panergos_home() / config.yaml`` — point it here.
    monkeypatch.setattr("panergos_constants.get_panergos_home", lambda: panergos_home)
    monkeypatch.setattr("panergos_cli.config.get_panergos_home", lambda: panergos_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_panergos_home", default_home)
    _stub_picker_dependencies(monkeypatch)
