...
def test_routed_profile_flows_into_prune_via_send_metadata(tmp_path: Path):
    ...
    assert meta["panergos_profile"] == "coder"
    assert "panergos_profile" not in runner._thread_metadata_for_source(_source(None, "99"))
    ...
    adapter._panergos_profile_name = "default"  # transport = primary bot
