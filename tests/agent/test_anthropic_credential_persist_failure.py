...
``~/.panergos/.anthropic_oauth.json`` for ``panergos_pkce`` ones.  Those singletons
...
def panergos_home(tmp_path, monkeypatch):
    """Real on-disk PANERGOS_HOME so ``load_pool()`` re-reads what we persisted."""
    ...
    monkeypatch.setattr(
        "panergos_cli.auth.is_provider_explicitly_configured", lambda pid: True
    )
    return home
...
def test_panergos_oauth_writer_raises_instead_of_swallowing(panergos_home, monkeypatch):
    ...
    with pytest.raises(CredentialPersistError):
        AA._write_panergos_oauth_credentials(
            _ROTATED_ACCESS, _ROTATED_REFRESH, _EXPIRED_MS + 3_600_000
        )
...
def test_pool_claude_code_fails_closed_and_reload_cannot_resurrect(
    panergos_home, claude_credentials, monkeypatch
):
    monkeypatch.setattr(AA, "refresh_anthropic_oauth_pure", _rotating_refresh)
    _break_durable_write(monkeypatch)
...
# ---------------------------------------------------------------------------
# Pool path: panergos_pkce
# ---------------------------------------------------------------------------


def test_pool_panergos_pkce_fails_closed_and_reload_cannot_resurrect(
    ...
    reloaded = [
        e for e in load_pool("anthropic").entries() if e.source == "panergos_pkce"
    ]
    assert reloaded
    assert reloaded[0].refresh_token == _STALE_REFRESH
    assert reloaded[0].last_status == STATUS_DEAD
