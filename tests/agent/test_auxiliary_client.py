...
class TestResolveTaskProviderModel:
    ...
    def test_explicit_provider_moa_unwraps_to_aggregator(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "panergos_cli.moa_config.resolve_moa_preset",
            lambda cfg, name: preset,
        )
        monkeypatch.setattr("panergos_cli.config.load_config", lambda: {"moa": {}})
        monkeypatch.setattr("panergos_cli.config.load_config_readonly", lambda: {"moa": {}})
...
class TestMoaAggregatorSharedResolution:
    ...
    Real-config tests: write an actual config.yaml under a temp PANERGOS_HOME
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``panergos auth status`` already reports these as logged in; compression
        ...
        from panergos_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))
        monkeypatch.setenv("PANERGOS_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``panergos model``, no per-task override is
    ...
    fallback chain (OpenRouter / custom / API-key providers), so aux tasks billed
...
class TestOpenRouterPaidLaneGuard:
    ...
    def test_free_only_allows_builtin_default_model(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("panergos_cli.config.load_config_readonly", return_value={"auxiliary": {"free_only": True}}), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
            client, model = _try_openrouter()
    ...
    def test_free_only_skips_paid_configured_model(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("panergos_cli.config.load_config_readonly",
    ...
    def test_paid_lane_warns_once(self, monkeypatch, caplog):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("panergos_cli.config.load_config_readonly", return_value=_paid_cfg), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("panergos_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
...
class TestTryPaymentFallback:
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        When OpenRouter/custom/API-key providers all fail, payment-fallback returns None —
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Panergos auth / credential pool."""
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from panergos_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("panergos_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Panergos owns the retry/timeout budget (issue #54465). The SDK
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("panergos_cli.config.load_config", return_value=config), patch("panergos_cli.config.load_config_readonly", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
    ...
    def test_moa_tasks_reject_task_level_reasoning_effort(self, moa_task, caplog):
        ...
        with patch("panergos_cli.config.load_config", return_value=config), patch("panergos_cli.config.load_config_readonly", return_value=config), \
             caplog.at_level(logging.WARNING, logger="agent.auxiliary_client"):
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Panergos reasoning.

    MoA slot reasoning is carried through call_llm as a Panergos
    ...
    def test_anthropic_messages_profile_resolves_to_messages_adapter(self, monkeypatch):
        ...
        with patch("agent.anthropic_adapter.build_anthropic_client", return_value=MagicMock()):
            client, _ = resolve_provider_client(
                "commandcode-anthropic", model="claude-haiku-4-5-20251001"
            )
        assert isinstance(client, AnthropicAuxiliaryClient)
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/khajaaijaz26/panergos-agent/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/khajaaijaz26/panergos-agent/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "PanergosAgent"
...
class TestAuxUnhealthyCache:
    ...
    def test_payment_fallback_skips_unhealthy(self):
        ...
        # unhealthy. The chain should still find the generic API-key provider.
        ...
        with patch("agent.auxiliary_client._read_main_provider", return_value="auto"), \
             ...
             patch("agent.auxiliary_client._resolve_api_key_provider", return_value=(api_key_client, "n-model")):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with patch("panergos_cli.config.load_config", return_value=fake_config), patch("panergos_cli.config.load_config_readonly", return_value=fake_config), \
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"), \
             ...
             patch("panergos_cli.config.load_config", return_value={"model": {}}), patch("panergos_cli.config.load_config_readonly", return_value={"model": {}}), \
             patch.object(ac, "_create_openai_client", side_effect=_capture_create):
            client, model = resolve_provider_client(
                "custom",
                model="test-model",
...
class TestFastModelTier:
    ...
    def test_catalog_match_prefers_rolling_alias_over_pinned_id(self):
        ...
        with patch("panergos_cli.models_pricing.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("zai") == "~openai/gpt-mini-latest"
    ...
    def test_catalog_match_skips_reasoning_batch_and_embedding_lookalikes(self):
        ...
        with patch("panergos_cli.models_pricing.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("zai") == "google/gemini-3.6-flash"
    ...
    def test_catalog_match_skips_the_non_chat_siblings_of_a_chat_model(self):
        ...
        with patch("panergos_cli.models_pricing.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("zai") == "openai/gpt-4o-mini"
    ...
    def test_catalog_match_takes_the_newest_of_a_family(self):
        ...
        with patch("panergos_cli.models_pricing.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("zai") == "openai/gpt-10-mini"
    ...
    def test_catalog_fetch_is_authenticated(self):
        ...
        with patch(
            "panergos_cli.auth.resolve_api_key_provider_credentials",
            return_value={"api_key": "sk-test", "base_url": "https://api.example.com/v1"},
        ), patch(
            "panergos_cli.models_pricing.fetch_models_with_pricing", return_value={}
        ) as fetch:
    ...
    def test_fast_tier_is_opt_in(self):
        ...
        with patch.object(ac, "_fast_model_from_catalog") as spy:
            ac._get_aux_model_for_provider("zai")
