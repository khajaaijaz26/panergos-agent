...
def test_client_cache_key_is_scoped_per_profile_home(tmp_path):
    """Callers that omit api_key (pool / example auth.json paths) must not share a client across
    multiplex profiles: the per-turn PANERGOS_HOME override has to participate in the key."""
    ...
    keys = []
    for home in (a, b):
        tok = panergos_constants.set_panergos_home_override(str(home))
        try:
            keys.append(aux._client_cache_key("example", async_mode=False, base_url="https://inf.example", model="m"))
        finally:
