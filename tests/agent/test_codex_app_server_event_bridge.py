"""Regression tests for the codex_app_server → Panergos UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_panergos_tools_mcp_server_emits_bare_tool_name(self):
        """The panergos-tools MCP server wraps Panergos' own tools for codex;
        ...
        mcp.panergos-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "panergos-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "panergos-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "panergos agent docs"
        assert calls[0].args[3] == {"query": "panergos agent docs"}
