...
class TestSurrogateVsAsciiSanitization:
    ...
    def test_ascii_codec_strips_all_non_ascii(self):
        ...
        messages = [{"role": "user", "content": "test λ🤖你好 end"}]
    ...
    def test_no_surrogates_returns_false(self):
        ...
        messages = [{"role": "user", "content": "hello λ world"}]
    ...
    def test_sanitizes_nested_dict_structure(self):
        payload = {
            "default_headers": {
                ...
                "User-Agent": "Panergos/1.0 🤖",
            }
        }
        assert _sanitize_structure_non_ascii(payload) is True
        assert payload["default_headers"]["X-Title"] == "Panergos  Agent"
        assert payload["default_headers"]["User-Agent"] == "Panergos/1.0 "
