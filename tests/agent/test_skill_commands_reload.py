...
def panergos_home(monkeypatch):
    """Isolate PANERGOS_HOME for ``reload_skills`` tests.

    Rather than popping cache-bearing modules from ``sys.modules``,
    we monkeypatch the module-level ``PANERGOS_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="panergos-reload-skills-")
    monkeypatch.setenv("PANERGOS_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "PANERGOS_HOME", home, raising=False)
...
class TestReloadSkillsHelper:
    ...
    def test_detects_removed_skill_carries_description(self, panergos_home):
        from agent.skill_commands import reload_skills

        skill_dir = _write_skill(panergos_home / "skills", "demo", "soon to be gone")
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, panergos_home):
