...
class TestCacheFileReadBlocking:
    """Internal Panergos cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        panergos_home = tmp_path / ".panergos"
        cache = panergos_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._panergos_home_path", return_value=panergos_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Panergos cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        panergos_home = tmp_path / ".panergos"
        hub = panergos_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_panergos_home(self, tmp_path):
        """The env basename guard does not depend on PANERGOS_HOME resolution."""
        panergos_home = tmp_path / ".panergos"
        panergos_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        panergos_home = tmp_path / ".panergos"
        cache = panergos_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
