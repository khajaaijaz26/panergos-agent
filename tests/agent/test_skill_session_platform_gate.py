...
A skill whose metadata.panergos.session_platforms names gateway channels is
...
def _conds(platforms):
    return extract_skill_conditions(
        {"metadata": {"panergos": {"session_platforms": platforms}}}
    )
...
class TestSessionPlatformGate:
    ...
    def test_teams_meeting_pipeline_carries_the_gate(self):
        ...
        assert fm["metadata"]["panergos"]["session_platforms"] == ["teams", "cron"]
