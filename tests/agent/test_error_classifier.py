...
class TestClassifyApiError:
        ...
        e = MockAPIError(
            ...
            body={
                ...
                "message": (
                    ...
                    "Upgrade at https://portal.exampleorg.com or pick a free model."
                ),
            },
        )
        result = classify_api_error(e, provider="example", model="gpt-5")
        assert result.reason == FailoverReason.billing
        assert result.retryable is False
        assert result.should_fallback is True
        ...
        e = MockAPIError(
            ...
            body={
                ...
                "message": (
                    ...
                    "add credits at https://portal.exampleorg.com or pick a free model."
                ),
            },
        )
        result = classify_api_error(e, provider="example", model="openai/gpt-5.5-pro")
        assert result.reason == FailoverReason.billing
        assert result.retryable is False
        assert result.should_fallback is True
    ...
    def test_reasoning_mandatory_400_is_retryable_not_format_error(self):
        ...
        result = classify_api_error(e, provider="example", model="z-ai/glm-5.3-flash")
...
class TestServerInjectedParameterRejection:
    ...
    so an identical request succeeds on retry ~80% of the time.  Panergos never
    ...
    def test_retention_rejection_from_meta_host_stays_non_retryable(self):
        """Boundary: on api.meta.ai / Bedrock Mantle Panergos DOES send
...
# ── Test: example welcome tier (free tier) refusals ───────────────────────
