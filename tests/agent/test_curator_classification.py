...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"panergos-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "panergos-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "panergos-agent-dev"
...
# `panergos curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `panergos curator pin <umbrella>` in the rename block so
