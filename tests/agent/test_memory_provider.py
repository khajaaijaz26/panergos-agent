...
class TestUserInstalledProviderDiscovery:
    """Memory providers installed to $PANERGOS_HOME/plugins/ should be found.
    ...
    def test_load_user_plugin(self, tmp_path, monkeypatch):
        """load_memory_provider() can load from $PANERGOS_HOME/plugins/."""
        from plugins.memory import load_memory_provider
...
class TestUserInstalledProviderCli:
    ...
    ``_panergos_user_memory.<name>.cli`` without registering the parent
...
class TestEntryPointMemoryProviderDiscovery:
    ...
    class FakeEntryPoint:
        def __init__(self, name, module, exposure="register"):
            ...
            self.group = "panergos_agent.memory_providers"
            self._module = module
...
class TestSetupFieldFiltering:
    ...
    memory setup wizard in panergos_cli/memory_setup.py.
...
class TestOnMemoryWriteBridge:
    ...
    def test_memory_manager_tool_injection_deduplicates(self):
        ...
        on providers that enforce unique names (e.g. Xiaomi MiMo via example Portal).

        Regression test for: duplicate mnemosyne_recall / mnemosyne_remember /
        mnemosyne_stats in tools array → 400 from example Portal.
        """
        mgr = MemoryManager()
...
class TestMemoryToolToolsetGate:
    ...
    def test_composite_toolset_with_memory_injects(self):
        ...
        tools, names = self._run_memory_injection(["panergos-acp"], mgr)
        assert "hindsight_recall" in names
        assert any(t["function"]["name"] == "hindsight_recall" for t in tools)

    @pytest.mark.parametrize("enabled_toolsets", [None, ["memory"], ["all"], ["panergos-acp"]])
