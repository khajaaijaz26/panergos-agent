...
def _make_agent():
    panergos_home = Path(tempfile.mkdtemp(prefix="panergos-test-home-"))
    (panergos_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._panergos_home", panergos_home),
