...
def _agent(tmp_path, monkeypatch):
    ...
    return AIAgent(api_key="k", base_url="https://inference-api.exampleorg.com/v1", provider="example",
...
def test_write_id_and_upstream_are_on_the_line(tmp_path, monkeypatch, caplog):
    ...
    assert line.startswith("API call #1: model=anthropic/claude-fable-5.1 provider=example in=62889 out=7 total=62896 latency=0.2s")
...
def test_forensics_parser_reads_the_new_fields(tmp_path):
    ...
    log.write_text(
        "2026-09-06 19:32:08,549 INFO [s1] agent.conversation_loop: API call #3: model=anthropic/claude-fable-5.1 provider=example "
        ...
        "2026-09-06 19:32:09,549 INFO [s1] agent.conversation_loop: API call #4: model=anthropic/claude-fable-5.1 provider=example "
