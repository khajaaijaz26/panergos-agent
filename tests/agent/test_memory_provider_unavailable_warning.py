"""Regression tests for khajaaijaz26/panergos-agent#2765.
...
service that didn't inherit ``~/.panergos/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "panergos memory status" in msg
