...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate PANERGOS_HOME under tmp_path."""
    monkeypatch.setenv("PANERGOS_HOME", str(tmp_path / ".panergos"))
    (tmp_path / ".panergos").mkdir()

    # Force the constants/image cache helpers to re-read PANERGOS_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("panergos_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_panergos_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under PANERGOS_HOME — gateway cleanup
