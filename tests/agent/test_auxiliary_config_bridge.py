...
class TestAuxiliaryConfigBridge:
    ...
    def test_approval_bridged(self, monkeypatch):
        config = {
            "auxiliary": {
                "approval": {"provider": "example", "model": "gemini-2.5-flash"},
            }
        }
        _run_auxiliary_bridge(config, monkeypatch)
        assert os.environ.get("AUXILIARY_APPROVAL_PROVIDER") == "example"
...
class TestDefaultConfigShape:
    """Verify the DEFAULT_CONFIG in panergos_cli/config.py has correct auxiliary structure."""
