...
conversation-affinity hint Panergos sends — ``prompt_cache_key`` on both
OpenAI-wire transports, the OpenRouter/example sticky ``session_id``, and xAI's
``x-grok-conv-id`` — so the conversation never lands back on the routing
bucket it warmed. Panergos cannot infer the logical conversation from the id's
syntax (#79017's failure class), but it does not have to: the host declares
it through ``gateway_session_key`` (the ``X-Panergos-Session-Key`` /
...
class TestDeclaredConversationScope:
    ...
    def test_scope_never_carries_the_raw_key(self, db):
        ...
        A session id is a Panergos-internal token; a session KEY embeds the
...
class TestProviderStickyKeys:
    """OpenRouter / example sticky ids and x-grok-conv-id read the same scope."""
...
class TestPeerIdentityIsSourceQualified:
    ...
    ``X-Panergos-Session-Key`` accepts any authenticated caller-supplied string,
...
class TestSourceOverrideDomain:
    ...
    ``PANERGOS_SESSION_SOURCE``. Under an override both sides of a ``/new``
    ...
    def test_the_pre_row_source_matches_persistence(self, db, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_SESSION_SOURCE", "override-src")
