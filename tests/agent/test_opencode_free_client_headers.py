...
@patch("agent.process_bootstrap.OpenAI")
def test_opencode_free_sends_panergos_attribution(mock_openai):
    """Keyless requests identify as Panergos (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Panergos Agent"
    assert str(headers.get("User-Agent", "")).startswith("PanergosAgent/")
