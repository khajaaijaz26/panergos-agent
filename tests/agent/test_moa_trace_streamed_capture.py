...
path via ``panergos chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp PANERGOS_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point PANERGOS_HOME at a temp dir and turn moa.save_traces on."""
    panergos_home = tmp_path / ".panergos"
    panergos_home.mkdir()
    monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))

    # save_moa_turn reads config via panergos_cli.config.load_config; stub it to
    ...
    return panergos_home / "moa-traces"
