...
def backup_env(monkeypatch, tmp_path):
    """Isolate PANERGOS_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_panergos_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd panergos_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under PANERGOS_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level PANERGOS_DIR picks up the tmp HOME."""
    import panergos_constants
    importlib.reload(panergos_constants)
