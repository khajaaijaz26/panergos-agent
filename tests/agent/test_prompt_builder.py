...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("panergos_cli.config.load_config", default_load_config)
        monkeypatch.setattr("panergos_cli.config.load_config_readonly", default_load_config)
    ...
    def test_truncation_warning_points_to_config_key(self, monkeypatch):
        ...
        monkeypatch.setattr("panergos_cli.config.load_config", fake_load_config)
        monkeypatch.setattr("panergos_cli.config.load_config_readonly", fake_load_config)
...
class TestBuildContextFilesPrompt:
    def test_empty_dir_loads_seeded_global_soul(self, tmp_path):
        ...
        assert "Panergos Agent" in result
    ...
    def test_agents_md_no_git_root_stays_cwd_only(self, tmp_path, monkeypatch):
        ...
        from agent.prompt_builder import _load_agents_md
        monkeypatch.setattr(
            "agent.prompt_builder._agents_md_directory_chain",
    ...
    def test_panergos_md_still_wins_over_agents_override(self, tmp_path):
        (tmp_path / ".panergos.md").write_text("Panergos-first context.")
        (tmp_path / "AGENTS.override.md").write_text("Override context.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Panergos-first context" in result
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("PANERGOS_HOME", str(tmp_path / "panergos_home"))
        panergos_home = tmp_path / "panergos_home"
        panergos_home.mkdir()
        (panergos_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
    ...
    # --- .panergos.md / PANERGOS.md discovery ---
...
# =========================================================================
# .panergos.md helper functions
# =========================================================================


class TestFindPanergosMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".panergos.md").write_text("rules")
        assert _find_panergos_md(tmp_path) == tmp_path / ".panergos.md"
    ...
    def test_unreadable_parent_is_treated_as_no_git_root(self, tmp_path, monkeypatch):
        ...
        def _exists(self):
            if self.name == ".git":
                if self.parent == tmp_path / "locked":
    ...
    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".panergos.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_panergos_md(sub) == tmp_path / ".panergos.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .panergos.md planted in /tmp, /home, or /
...
class TestEnvironmentHints:
    ...
    def test_probe_remote_backend_ssh_is_probe_only_and_torn_down(self, monkeypatch):
        ...
        ~/.panergos and snapshot a session just to run `uname`, and its __del__ would
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """PANERGOS_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("PANERGOS_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._BACKEND_PROBE_CACHE.clear()
        result = _pb.build_environment_hints()
...
class TestContextFileReadTimeout:
    def test_slow_panergos_md_is_skipped_and_agents_md_still_loads(self, tmp_path, monkeypatch, caplog):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".panergos.md").write_text("Panergos project rules.")
        ...
        assert "Panergos project rules" not in result
