...
def isolated_home(monkeypatch):
    """Temp PANERGOS_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="panergos_test_31179_")
    panergos_home = os.path.join(test_home, ".panergos")
...
def _fresh_modules():
    """Drop cached panergos modules so each test reloads against current env."""
    _drop_reload_targets()
