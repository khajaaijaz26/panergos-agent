...
def _make_agent(
    *tool_names: str,
    ...
    with (
        ...
        patch("panergos_cli.config.load_config_readonly", return_value=config or {}),
...
def test_relay_rewrite_precedes_sequential_policy_approval_checkpoint_and_dispatch(tmp_path):
    agent = _make_agent("write_file")
    original_args = {"path": str(tmp_path / "original"), "content": "old"}
    final_args = {"path": str(tmp_path / "approved"), "content": "new"}
    ...
    assert observed["checkpoint"] == [
        (final_args["path"], "before write_file")
    ]
