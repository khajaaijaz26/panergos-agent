...
class TestParseConfigStringList:
    """#86661: `panergos config set` and JSON-mode editor saves store lists as
    ...
    def test_python_literal_array_string_parses(self):
        # `panergos config set` can persist single-quoted Python-literal forms.
...
def test_skill_config_home_vars_use_subprocess_home(tmp_path, monkeypatch):
    ...
    panergos_home = tmp_path / "da\\ta"
    subprocess_home = panergos_home / "home"
    subprocess_home.mkdir(parents=True)
    (panergos_home / "config.yaml").write_text("", encoding="utf-8")
    monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))
    monkeypatch.setenv("HOME", str(panergos_home))
