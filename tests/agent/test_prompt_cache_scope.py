...
class TestPerResponseRunNonceIsolation:
    ...
    Panergos Studio group chat builds ``gc_run_<room>_<profile>_<name>_<uuid4hex>``
    for every reply and destroys it when the reply completes
    (``groupRuntimeSessionId``), so every conversation-affinity hint Panergos
    ...
    is an identity, and Panergos' public session API lets a client choose one
    ...
    def test_parentless_rows_resolve_to_their_own_scope(self, db):
        ...
        an owner Panergos was never told about must never be guessed. #96811
    ...
    def test_distinct_ids_keep_distinct_openrouter_sticky_keys(self):
