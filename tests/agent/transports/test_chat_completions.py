...
class TestChatCompletionsBasic:
    ...
    @pytest.mark.parametrize("provider", ["ai-gateway", "openrouter"])
    ...
    def test_convert_messages_strips_internal_scaffolding_markers(self, transport):
        """Panergos-internal ``_``-prefixed markers must never reach the wire.
...
class TestChatCompletionsKimi:
    ...
    def test_moonshot_tool_schemas_are_sanitized_by_model_name(self, transport):
        """Aggregator routes (example, OpenRouter) hit Moonshot by model name, not base URL."""
        tools = [
            {
                "type": "function",
                "function": {
...
class TestChatCompletionsGeminiNativeExtraBodyStrip:
    ...
    def _tagged_profile():
        ...
        profile = ProviderProfile(name="tagged")
        profile.build_extra_body = lambda **_kwargs: {"tags": ["test"]}
        return profile

    """Profile extra_body (e.g. example portal tags) must not reach a native
    ...
    def test_tags_stripped_when_endpoint_is_native_gemini(self, transport):
        kw = transport.build_kwargs(
            ...
            None,
            provider_profile=self._tagged_profile(),
