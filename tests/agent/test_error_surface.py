...
def test_auth_surface_names_oauth_vs_api_key_recovery():
    ...
    assert oauth["provider_label"] == "ChatGPT or Codex Subscription"
    ...
    assert "auth_kind" not in build_error_surface_from_result(
        _failed_result("rate_limit"), provider="openai-codex"
    )


def test_result_billing_block_wins():
    surface = build_error_surface_from_result(
        _failed_result("rate_limit", billing_block={"provider": "example"})
    )
