...
def _ensure_redaction_enabled(monkeypatch):
    """Ensure PANERGOS_REDACT_SECRETS is not disabled by prior test imports."""
...
class TestTerminalOutputRedaction:
    ...
    def test_command_reads_secret_file_detection(self):
        ...
        # With paths
        assert _command_reads_secret_file("cat ~/.panergos/.env")
    ...
    def test_cat_env_file_masks_opaque_token(self):
        ...
        out = (
            ...
            "EXAMPLE_API_KEY=xyz789opaqueKey\n"
            "DEBUG=true\n"
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("cat ~/.panergos/config.yaml", "api_key: panergosConfigSecret123", "panergosConfigSecret123"),
            (
                "head ~/.panergos/profiles/work/config.yaml",
            ...
            (
                "grep 'foo|bar' ~/.panergos/config.yaml",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "cat ~/.panergos/config.example.yaml",
            "cat ~/.panergos/config.template.yaml",
            "cat ~/.bashrc.example",
