...
def test_deepseek_v4_pro_pricing_entry_exists():
    ...
    in panergos insights because the _OFFICIAL_DOCS_PRICING table had no
...
def test_curated_google_flash_models_resolve_official_snapshot_pricing(monkeypatch):
    """Every ``google/gemini-*-flash`` model curated for the OpenRouter and example
    ...
    """
    from panergos_cli.models_catalog_static import OPENROUTER_MODELS, _PROVIDER_MODELS

    monkeypatch.setattr(
        "agent.usage_pricing.fetch_endpoint_model_metadata",
        lambda *_args, **_kwargs: {},
    )
    curated = {m for m, _desc in OPENROUTER_MODELS} | set(_PROVIDER_MODELS["example"])
