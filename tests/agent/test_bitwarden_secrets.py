...
binary download are exercised manually by `panergos secrets bitwarden
...
def panergos_home(tmp_path, monkeypatch):
    """Point Panergos at an isolated home directory."""
    home = tmp_path / ".panergos"
    home.mkdir()
    monkeypatch.setenv("PANERGOS_HOME", str(home))
    # Some modules cache get_panergos_home; clear if needed.
    import panergos_constants
    if hasattr(panergos_constants, "_PANERGOS_HOME_CACHE"):
        panergos_constants._PANERGOS_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(panergos_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from panergos_cli.env_loader import _apply_external_secret_sources
