...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated PANERGOS_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="panergos_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".panergos"))
    prev_home = os.environ.get("PANERGOS_HOME")
    os.environ["PANERGOS_HOME"] = os.path.join(test_home, ".panergos")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("panergos_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into panergos_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["PANERGOS_HOME"] = prev_home
