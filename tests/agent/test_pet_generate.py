...
pytestmark = pytest.mark.skipif(
    os.environ.get("PANERGOS_RUN_SLOW_PET_TESTS") != "1",
    reason=(
        "pet generation image-processing suite is opt-in; run with "
        "PANERGOS_RUN_SLOW_PET_TESTS=1 scripts/run_tests.sh tests/agent/test_pet_generate.py"
    ),
)
...
def test_list_sprite_providers_marks_default(monkeypatch):
    ...
    registry = {"openai": _FakeImgProvider("openai"), "example": _FakeImgProvider("example")}
    ...
    # Listed in preference order: example Portal before OpenAI.
    assert [p["name"] for p in listed] == ["example", "openai"]
