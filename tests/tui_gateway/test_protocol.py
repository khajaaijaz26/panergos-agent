...
def server():
    ...
    # e.g. panergos_cli.active_sessions would bind the mocked get_panergos_home
    ...
    with patch.dict("sys.modules", {
        "panergos_constants": MagicMock(get_panergos_home=MagicMock(return_value="/tmp/panergos_test")),
        "panergos_cli.env_loader": MagicMock(),
        "panergos_cli.banner": MagicMock(),
        "panergos_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `PANERGOS_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("PANERGOS_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("PANERGOS_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_idle_reaper_rearms_missing_ws_orphan_timer(server, monkeypatch, tmp_path, closed_transport):
    ...
    successor = subprocess.run(
        [
            ...
            "-c",
            (
                "from panergos_cli.active_sessions import try_acquire_active_session; "
...
def test_config_roundtrip(server, tmp_path):
    server._panergos_home = tmp_path
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/panergos-agent-dev": {"name": "panergos-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "panergos-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient PANERGOS_HOME
    ...
    # The gateway's own PANERGOS_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `panergos config set` does)
    ...
    import panergos_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_panergos_home", lambda: tmp_path)
    ...
    # Activate midnight, as `panergos config set display.skin midnight` would.
