...
def test_default_home_aliases_are_reported_as_default(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_panergos_home", launch_home)

    for alias in (default_home.name, "panergos"):
    ...
    # "panergos" is a legal profile id: a REAL named profile of that name is never swallowed by the alias.
    (default_home / "profiles" / "panergos").mkdir(parents=True)
    assert server._response_profile_name("panergos") == "panergos"
