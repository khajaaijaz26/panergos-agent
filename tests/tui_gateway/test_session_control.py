...
@pytest.fixture()
def server(panergos_home, monkeypatch):
    with patch.dict(
        "sys.modules",
        {
    ...
    monkeypatch.setattr(mod, "_panergos_home", panergos_home)
...
def _save_goal(key, **overrides):
    from panergos_cli.goals import GoalState, save_goal
...
def _save_loop(key, **overrides):
    from panergos_cli.loops import LoopState, save_loop
...
def _save_heartbeat(key, **overrides):
    from panergos_cli.heartbeat import HeartbeatState, save_heartbeat
...
class TestStructuredRead:
    ...
    def test_goal_contract_subgoals_and_gates_are_structured_and_sanitized(self, server, session):
        from panergos_cli.goals import GoalContract, GoalGate
...
class TestManagerOnlyMutations:
    def test_subgoal_add_remove_clear_are_real_one_based_mutations_without_dispatch(self, server, session, monkeypatch):
        from panergos_cli.goals import load_goal
...
class TestUpdatePublication:
    ...
    def test_real_turn_publishes_after_the_goal_judge_ran(self, server, session, monkeypatch, tmp_path):
        ...
        for name, value in {
            ...
            "_get_usage": lambda agent: {}, "_panergos_home": tmp_path,
        }.items():
        ...
        def judge(self, raw, **kwargs):
            from panergos_cli.goals import save_goal
