...
The slash worker that parses ``/heartbeat`` runs a PanergosCLI whose watchdog queues the due prompt
...
@pytest.fixture()
def server(panergos_home):
    with patch.dict("sys.modules", {"panergos_cli.env_loader": MagicMock(), "panergos_cli.banner": MagicMock()}):
...
def _arm_due(key: str):
    from panergos_cli.heartbeat import HeartbeatManager, save_heartbeat
...
def test_notification_poller_fires_due_heartbeat_when_idle(server, session):
    ...
    from panergos_cli.heartbeat import load_heartbeat
...
def test_heartbeat_tick_defers_when_busy_or_not_due(server, session, running, due):
    ...
    if not due:
        ...
        from panergos_cli.heartbeat import save_heartbeat
...
def test_abandon_fire_never_overwrites_a_concurrent_pause(panergos_home):
    from panergos_cli.heartbeat import HeartbeatManager, load_heartbeat
