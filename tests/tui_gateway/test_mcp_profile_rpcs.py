...
``PANERGOS_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
@pytest.fixture
def panergos_root(tmp_path, monkeypatch):
    """A temp PANERGOS_HOME root with two named profiles: 'work' and 'other'.

    Pointing PANERGOS_HOME at a dir outside ~/.panergos makes it the profile ROOT
    (get_default_panergos_root's Docker/custom branch), so named profiles live at
    ``<root>/profiles/<name>/`` and the launch/default profile is ``<root>``.
    """
    root = tmp_path / "panergos_home"
...
def test_add_lands_in_named_profile_only(panergos_root):
    root = panergos_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(panergos_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_status_is_profile_scoped_and_credential_safe(panergos_root):
    _result(
        _call(
            "mcp.servers.add",
            {"profile": "work", "name": "svc-a", "config": {"command": "svc-a-bin"}},
...
def test_status_does_not_mix_launch_runtime_into_another_profile(panergos_root):
...
def test_status_includes_named_profile_runtime_in_multiplex(panergos_root):
    ...
    work_token = set_panergos_home_override(panergos_root / "profiles" / "work")
    try:
        work_scope = panergos_home_key()
    finally:
        reset_panergos_home_override(work_token)
...
def test_set_api_key_writes_env_and_header_to_right_profile(panergos_root):
    root = panergos_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(panergos_root):
    root = panergos_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(panergos_root):
    root = panergos_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(panergos_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(panergos_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(panergos_root):
    root = panergos_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == PANERGOS_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
...
def test_test_resolves_env_refs_from_requested_profile_secret_scope(panergos_root, monkeypatch):
    ...
    work = panergos_root / "profiles" / "work"
