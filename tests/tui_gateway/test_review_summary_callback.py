    ...
    with patch.dict(
        "sys.modules",
        {
            "panergos_constants": MagicMock(
                get_panergos_home=MagicMock(return_value="/tmp/panergos_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'panergos-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'panergos-release' patched"
    }
