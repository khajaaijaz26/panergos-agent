...
"No LLM provider configured. Run `panergos model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real PANERGOS home.
...
# provider (e.g. example) but who switched THIS session to a self-hosted model
...
# found" (the b200/panergos-ultra-sft report). The stored MODEL NAME is the one
...
ULTRA_CONFIG = {
    ...
    "model": {"default": "some-example-model", "provider": "example"},
    "providers": {
        "panergos-ultra": {
            "api": ULTRA_URL,
            "api_key": "sk-ultra",
            "models": ["panergos-ultra-sft"],
        }
    },
}
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("panergos-ultra-sft")
            == "custom:panergos-ultra"
        )
...
# what left room bots stuck on a stale provider (e.g. "out of example credits"
...
#
# Regression: GH #89497 (room bots hang then report "out of example credits").
...
class TestRoomPlumbingRuntimeOverrides:
    def test_marked_row_returns_no_overrides(self):
        ...
        row = {
            ...
            "model_config": json.dumps(
                {"model": "openai/gpt-5.6-luna-pro", "provider": "example", "room_plumbing": True}
            ),
        }
    ...
    def test_marked_row_dict_model_config(self):
        ...
        row = {
            "model": "openai/gpt-5.6-luna-pro",
            "model_config": {"model": "openai/gpt-5.6-luna-pro", "provider": "example", "room_plumbing": True},
        }
        assert _stored_session_runtime_overrides(row) == {}
    ...
    def test_legacy_group_title_shape_still_skipped(self):
        ...
        row = {
            ...
            "model": "openai/gpt-5.6-luna-pro",
            "billing_provider": "example",
            "model_config": json.dumps({"model": "openai/gpt-5.6-luna-pro", "provider": "example"}),
        }
        assert _stored_session_runtime_overrides(row) == {}
...
# "out of example credits" after the profile was switched to ollama-cloud) while
...
class TestFollowProfileConfigRuntimeOverrides:
    ...
    def test_unmarked_row_still_restores_stored_runtime(self):
        ...
        row = {
            ...
            "billing_provider": "example",
            "model_config": json.dumps(
                {"model": "openai/gpt-5.6-luna-pro", "provider": "example"}
            ),
        }
        overrides = _stored_session_runtime_overrides(row)
        assert overrides["model_override"]["model"] == "openai/gpt-5.6-luna-pro"
        assert overrides["model_override"]["provider"] == "example"
...
# VeniceAI/empero endpoint under a model that should run on example). The sibling
...
class TestRuntimeModelConfigDropsStaleKeys:
    ...
    def test_falsy_model_drops_stale_existing_model(self):
        ...
        agent = _agent_like(model="", provider="example")
        existing = {"model": "meituan/longcat-2.0:free", "provider": "example"}
    ...
    def test_truthy_provider_overwrites_stale_existing(self):
        ...
        config = _runtime_model_config(_agent_like(provider="example"), existing)
    ...
    def test_resume_overrides_get_no_stale_provider(self):
        ...
        assert overrides["model_override"]["provider"] == "example"
        assert overrides["provider_override"] == "example"
    ...
    def test_existing_none_returns_only_agent_identity(self):
        ...
        config = _runtime_model_config(_agent_like(provider="example"), None)

        assert config == {"model": "deepseek/deepseek-v4-flash-0731", "provider": "example"}
