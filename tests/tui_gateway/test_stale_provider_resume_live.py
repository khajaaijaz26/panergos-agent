...
def live_home(monkeypatch):
    """A REAL isolated PANERGOS_HOME with a config.yaml + state.db on disk."""
    tmp = Path(tempfile.mkdtemp(prefix="panergos-live-staleprov-"))
    home = tmp / ".panergos"
    ...
    # panergos_constants caches the resolved home at first read — the env var
    ...
    for var in list(os.environ):
        if var.endswith("_API_KEY") or var in ("OPENROUTER_KEY", "EXAMPLE_KEY"):
            monkeypatch.delenv(var, raising=False)

    import panergos_cli.config as hconfig
    ...
    # conftest also re-pins panergos_state.DEFAULT_DB_PATH per test; pin it to
    # THIS home so server._get_db() opens the same real state.db we seed.
    monkeypatch.setattr(panergos_state, "DEFAULT_DB_PATH", home / "state.db")
    monkeypatch.setattr(server, "_db", None, raising=False)
    monkeypatch.setattr(server, "_db_error", None, raising=False)
    monkeypatch.setattr(server, "_panergos_home", str(home), raising=False)
    yield home, server
