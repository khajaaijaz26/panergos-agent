...
guard is covered in ``tests/panergos_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``panergos_cli.model_switch_providers.prewarm_picker_cache_async``
...
import panergos_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from panergos_cli.model_switch,
