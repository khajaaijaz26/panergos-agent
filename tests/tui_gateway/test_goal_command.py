...
def panergos_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves PANERGOS_HOME.
...
def server(panergos_home, monkeypatch):
    ...
    # Pin config resolution to the isolated PANERGOS_HOME. Sibling test
    ...
    # module-level ``_panergos_home = get_panergos_home()`` snapshot freezes
...
def test_moa_bare_returns_usage(server, session, panergos_home):
    _write_moa_config(panergos_home, """
moa:
...
def test_goal_draft_uses_session_profile_without_blocking_rpc_reader(
    ...
    def draft(objective):
        observed.append(get_panergos_home())
        started.set()
