...
class _Agent:
    def __init__(self):
        self.model, self.provider, self.base_url, self.api_key, self.api_mode = "old", "example", "", "", ""
...
def _quiet_switch(monkeypatch):
    result = SimpleNamespace(
        success=True, new_model="new/model", target_provider="example", base_url="", api_key="key",
        api_mode="chat_completions", warning_message="", model_info=None, error_message="",
        runtime_capabilities=None)
    monkeypatch.setattr("panergos_cli.model_switch.switch_model", lambda **_kw: result)
    monkeypatch.setattr("panergos_cli.model_switch.persist_model_selection", lambda _r: None)
    monkeypatch.setattr("panergos_cli.model_cost_guard.expensive_model_warning", lambda *a, **k: None)
...
def test_reasoning_flag_survives_the_swap_and_pins_the_session(_quiet_switch):
    ...
    out = server._apply_model_switch("sid", session, "new/model --provider example --reasoning high --session")
...
def test_reasoning_flag_with_global_writes_config_and_drops_the_pin(_quiet_switch):
    ...
    server._apply_model_switch("sid", session, "new/model --provider example --reasoning none --global")
