...
def test_create_send_drive_publish_and_replay_without_client_transport(tmp_path: Path):
    ...
    room = service.create_room(
        ...
        name="Release room",
        members=[
            {"member_id": "default", "profile": "default", "handle": "panergos"},
...
def test_same_thread_followup_migrates_and_delivers_committed_peer_reply(
    ...
    service.send(
        room_id="room-1",
        event_id="user-2",
        payload={"text": "@panergos continue", "thread_id": "thread-1"},
    )
    ...
    assert "User (user): @panergos continue" in prompt
...
def test_active_same_thread_followup_waits_for_current_task(tmp_path: Path):
    ...
    service.send(
        room_id="room-1",
        event_id="user-2",
        payload={"text": "@panergos follow up", "thread_id": "thread-1"},
    )
    ...
    assert service.stop(timeout=5.0)
    assert "User (user): @panergos follow up" in service.rpc.prompts[1][1]
...
def test_service_derives_room_deadline_from_agent_timeout(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("PANERGOS_AGENT_TIMEOUT", "90")
...
def test_local_profiles_skips_delete_tombstones_and_dot_dirs(tmp_path: Path):
    """`panergos profile delete` leaves ``profiles/.deleted/<name>``; neither the tombstone dir nor a
