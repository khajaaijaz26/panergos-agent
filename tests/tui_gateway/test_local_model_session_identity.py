...
from panergos_cli.local_runtime import endpoint
...
def local_route(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("panergos_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("panergos_cli.banner.get_available_skills", lambda: {})
...
def test_session_info_recovers_identity_from_the_owning_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PANERGOS_HOME", str(launch))
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setattr(server, "_panergos_home", launch)
    monkeypatch.setattr(server, "_probe_credentials", lambda agent: None)
    monkeypatch.setattr("panergos_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("panergos_cli.banner.get_available_skills", lambda: {})
    ...
    assert get_panergos_home() == launch
    ...
    with server._profile_build_scope(secondary):
        ...
        assert get_panergos_home() == secondary
    assert get_panergos_home() == launch
