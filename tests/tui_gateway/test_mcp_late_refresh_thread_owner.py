...
by surface:
  ...
  * ``tui_gateway.entry`` — the stdio ``panergos --tui`` path.
  * ``panergos_cli.mcp_startup`` — the desktop app + dashboard WebSocket sidecar
    (``tui_gateway/ws.py``) and ``panergos dashboard``.
...
``panergos_cli.mcp_startup``), so the scheduler bailed immediately and a slow MCP
...
import panergos_cli.mcp_startup as startup
...
def test_entry_in_flight_sees_startup_thread(clean_discovery_globals):
    """Desktop/dashboard surface: discovery thread lives on panergos_cli.mcp_startup.

    The entry-level in-flight check must report True so the late-refresh
    scheduler does not bail (the #51587 bug).
    """
    stop = threading.Event()
    startup._mcp_discovery_thread[panergos_home_key()] = _alive_thread(stop)
    try:
...
def test_startup_module_exposes_in_flight_helpers(clean_discovery_globals):
    """panergos_cli.mcp_startup gains the in-flight/join helpers entry delegates to."""
    assert startup.mcp_discovery_in_flight() is False
    assert startup.join_mcp_discovery(timeout=0.1) is True

    stop = threading.Event()
    t = _alive_thread(stop)
    startup._mcp_discovery_thread[panergos_home_key()] = t
    try:
