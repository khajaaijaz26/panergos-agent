...
``[diff "x"] command=/textconv=`` driver. Panergos gathers workspace context by
...
context-gathering git path Panergos runs neutralizes every sink. They use a real
...
def test_goals_fingerprint_is_safe(malicious_repo):
    from panergos_cli.goals import workspace_fingerprint
    repo, marker = malicious_repo
...
def test_web_git_diff_is_safe(malicious_repo):
    from panergos_cli import web_git
    repo, marker = malicious_repo
