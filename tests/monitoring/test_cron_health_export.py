...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("panergos.gateway.up"), _M("panergos.gateway.active_agents"),
        _M("panergos.gateway.busy"), _M("panergos.gateway.drainable"),
        _M("panergos.gateway.restart_requested"),
        _M("panergos.platform.up"), _M("panergos.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("panergos.cron.scheduler.heartbeat_age_seconds"),
        _M("panergos.cron.scheduler.last_success_age_seconds"),
        _M("panergos.cron.scheduler.catch_up_occurrences"),
        _M("panergos.cron.jobs.enabled"), _M("panergos.cron.jobs.running"),
        _M("panergos.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_local_health_scope_and_terminal_flush():
    from pathlib import Path

    text = Path("website/docs/developer-guide/gateway-monitoring.md").read_text(encoding="utf-8")

    assert "Locally owned bridge" in text
    assert "platform connector health" in text
