"""Tests for panergos_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Panergos entry point
(panergos, panergos-agent, panergos-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Panergos entry point imports panergos_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported panergos_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("panergos_bootstrap", None)
    import panergos_bootstrap  # noqa: WPS433
    return panergos_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Panergos CLI
...
class TestEntryPointsImportBootstrap:
    """Every Panergos entry point must import panergos_bootstrap as its
    ...
    # Entry points that invoke Panergos as a process.  Each one must
    # import panergos_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "panergos_cli/main.py",   # panergos CLI (console_script)
        "run_agent.py",          # panergos-agent (console_script)
        "acp_adapter/entry.py",  # panergos-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import panergos_bootstrap' and that
        ...
        half-finished ``panergos update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``panergos_bootstrap`` as a top-level module) leaves panergos
        ...
        """
        # Resolve relative to the panergos-agent repo root.  Tests live
        # at tests/test_panergos_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # panergos start even when panergos_bootstrap hasn't been
        ...
        assert first_import_name == "panergos_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'panergos_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import panergos_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Panergos's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("PANERGOS_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["PANERGOS_PYTHON_SRC_ROOT"] = env
            elif "PANERGOS_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["PANERGOS_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/panergos")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("PANERGOS_PYTHON_SRC_ROOT", None)
            else:
                os.environ["PANERGOS_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/panergos", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Panergos root.  The guard must relocate Panergos to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/panergos"])
        assert result[0] == "/opt/panergos"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Panergos root.
        assert result.index("/opt/panergos") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["PANERGOS_PYTHON_SRC_ROOT"] = "/env/panergos"
            hb.harden_import_path()
            assert sys.path[0] == "/env/panergos"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("PANERGOS_PYTHON_SRC_ROOT", None)
