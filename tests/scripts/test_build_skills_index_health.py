...
class _FakeSource:
    ...
    def search(self, query, limit=10):
        ...
        if self._src == "clawhub":
            self.catalog_walk_complete = count == self._n
            self.catalog_walk_stop_reason = None if self.catalog_walk_complete else "item_limit"
...
def test_healthy_crawl_writes_index_with_all_sources(tmp_path, monkeypatch):
    ...
    assert clawhub_status["catalog_complete"] is False
    assert clawhub_status["coverage"] == "bounded_snapshot"
    assert clawhub_status["stop_reason"] == "item_limit"
    assert clawhub_status["returned"] == clawhub_status["item_limit"]
