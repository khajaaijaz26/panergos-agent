"""Windows installer regression for Panergos children outside the venv.

The venv sweep deliberately selects process roots by executable path so it
does not kill unrelated Python processes. A selected Panergos process can spawn
...
def test_venv_sweep_stops_managed_runtime_children_but_not_unrelated_processes(
    ...
    runtime_dir = install_dir / ".panergos-runtime" / "python" / "generation-test"
    ...
    # Keep the test away from real gateway tasks and real Panergos launchers
    ...
    _write_cmd(
        ...
        '/C:"/IM panergos.exe" >nul\n'
        "if not errorlevel 1 exit /b 0\n"
        '"%SystemRoot%\\System32\\taskkill.exe" %*\n',
    )
    uv = panergos_home / "bin" / "uv.exe"
    ...
    wrapper.write_text(
        ...
        f"-InstallDir '{install_dir}' -PanergosHome '{panergos_home}'\n"
        "exit $LASTEXITCODE\n",
        encoding="utf-8",
    )
