...
CANONICAL_ORIGIN = "https://github.com/khajaaijaz26/panergos-agent.git"
...
def _installer_env(tmp_path: Path, remote: Path, **extra: str) -> dict[str, str]:
    """Keep the stored origin canonical while routing fixture fetches locally."""
    ...
    wrapper.write_text(
        "#!/usr/bin/env bash\n"
        "case \" $* \" in\n"
        f"  *\" remote get-url origin \"*) exec \"{REAL_GIT}\" \"$@\" ;;\n"
        "esac\n"
        f"exec \"{REAL_GIT}\" -c \"url.{remote.as_uri()}.insteadOf={CANONICAL_ORIGIN}\" \"$@\"\n",
        encoding="utf-8",
    )
    wrapper.chmod(0o755)
    return os.environ | {"PATH": str(wrapper_dir) + os.pathsep + os.environ["PATH"], **extra}
...
def _make_conflicted_managed_checkout(tmp_path: Path) -> tuple[Path, Path]:
    ...
    managed = tmp_path / "panergos-agent"
    ...
    _git(managed, "remote", "set-url", "origin", CANONICAL_ORIGIN)
...
def test_install_sh_repository_stage_recovers_from_autostash_conflict(
    ...
    env = _installer_env(
        ...
        PANERGOS_INSTALL_DIR=str(managed),
    )
...
def test_install_ps1_repository_stage_recovers_from_autostash_conflict(
    ...
    result = subprocess.run(
        [
            ...
            str(managed),
            "-PanergosHome",
            str(tmp_path / "panergos-home"),
        ],
        cwd=tmp_path,
        env=_installer_env(tmp_path, remote),
        capture_output=True,
        text=True,
    )
