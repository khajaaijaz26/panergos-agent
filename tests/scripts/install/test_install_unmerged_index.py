...
The ``panergos update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m panergos_cli.main gateway run`` — image name
    ``pythonw``, not ``panergos.exe`` — so the ``taskkill /IM panergos.exe`` guard
    ...
    # The panergos.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM panergos.exe' in text
