...
under contributors/emails/ (additions never merge-conflict). Release tooling
loads only mappings that are explicitly present in this repository.
"""
...
def test_effective_map_uses_only_repository_mappings():
    assert release.AUTHOR_MAP == release._load_contributor_dir()
...
def test_add_refuses_conflicting_existing_mapping(emails_dir):
    emails_dir.mkdir(parents=True)
    path = emails_dir / "person@example.com"
    path.write_text("original\n", encoding="utf-8")

    assert add_contributor(path.name, "different") == 1
    assert read_mapping_file(path) == "original"
