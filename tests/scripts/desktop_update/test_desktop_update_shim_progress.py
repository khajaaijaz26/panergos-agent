...
def progress(tmp_path):
    ...
    status = tmp_path / "panergos-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Panergos to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `panergos update`, and reports the stage that was on screen
...
n="$(cat "$PANERGOS_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$PANERGOS_TEST_CALLS"
for f in "$TMPDIR"/panergos-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$PANERGOS_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$PANERGOS_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "panergos-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    panergos = install_root / "venv" / "bin" / "panergos"
    panergos.write_text(FAKE_PANERGOS)
    panergos.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "PANERGOS_TEST_CAPTURE": str(capture),
        "PANERGOS_TEST_CALLS": str(calls),
        "PANERGOS_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".panergos-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`panergos update` is the longest wait in the hand-off and the one the
