...
from panergos_cli.profiles import _get_default_panergos_home
...
def _without_honcho_env(**values):
    env = {
        key: value for key, value in os.environ.items()
        if not key.startswith("HONCHO_") and key != "PANERGOS_HONCHO_HOST"
    }
    return patch.dict(os.environ, env | values, clear=True)


class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        config = HonchoClientConfig()
        assert config.host == "panergos"
        assert config.workspace_id == "panergos"
...
class TestFromGlobalConfig:
    ...
    def test_missing_config_still_reads_honcho_url(self, tmp_path):
        ...
        """
        with _without_honcho_env(HONCHO_URL="http://localhost:8000"):
            config = HonchoClientConfig.from_global_config(
                config_path=tmp_path / "nonexistent.json"
            )
        assert config.base_url == "http://localhost:8000"
    ...
    def test_endpoint_base_url_wins_over_top_level_and_env(self, tmp_path):
        ...
        with _without_honcho_env(HONCHO_BASE_URL="http://localhost:9003"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.base_url == "http://localhost:8000"
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"panergos": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"panergos": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"panergos": {"baseUrl": "http://host:1"}},
        ...
        for i, want in enumerate(expected):
            ...
            with _without_honcho_env(**env_dict):
                config = HonchoClientConfig.from_global_config(config_path=config_file)
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="panergos-agent"
        ):
            result = config.resolve_session_name("/home/user/panergos-agent/subdir")
        assert result == "panergos-agent"


class TestResolveConfigPath:
    def test_prefers_panergos_home_when_exists(self, tmp_path):
        panergos_home = tmp_path / "panergos"
        panergos_home.mkdir()
        local_cfg = panergos_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: PANERGOS_HOME points at ~/.panergos/profiles/<name>, so
        # _get_default_panergos_home() must resolve back to ~/.panergos — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".panergos"
        ...
        assert _get_default_panergos_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "panergos_coder"
        assert profile_host_key("default") == "panergos"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"PANERGOS_HONCHO_HOST": "panergos.coder"}):
            assert resolve_active_host() == "panergos.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("PANERGOS_HONCHO_HOST", None)
            # Temporarily remove panergos_cli.profiles to simulate import failure
            saved = sys.modules.get("panergos_cli.profiles")
            sys.modules["panergos_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "panergos"
            finally:
                if saved is not None:
                    sys.modules["panergos_cli.profiles"] = saved
                else:
                    sys.modules.pop("panergos_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="panergos_coder")
        assert config.host == "panergos_coder"
        assert config.workspace_id == "panergos"  # shared workspace
        assert config.ai_peer == "panergos_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"panergos": {"enabled": True, "aiPeer": "panergos"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (panergos.work)
        ...
        cfg = HonchoClientConfig(
            ...
            raw={"hosts": {"panergos.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_panergos_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with _without_honcho_env(), \
             patch("panergos_cli.profiles.get_active_profile_name", return_value="default"), \
