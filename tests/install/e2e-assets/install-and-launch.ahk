...
; Drive the REAL Panergos-Setup.exe (Tauri bootstrap installer) window:
; click Install, wait for the install to finish, click Launch, and wait for
; the real Panergos.exe (Electron desktop) window to appear.
;
; Adapted from @ethernet8023's e2e/windows/install-panergos-desktop.ahk
...
setupExe := A_Args.Length >= 2 ? A_Args[2] : "Panergos-Setup.exe"
...
Log("Launch clicked; waiting for the Panergos desktop app window")
...
try {
    ...
    throw Error("Panergos.exe real-sized window did not appear within 300s of clicking Launch")
}
