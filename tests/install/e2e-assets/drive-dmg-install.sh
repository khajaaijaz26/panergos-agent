#!/usr/bin/env bash
# Drive the Panergos-Setup dmg bootstrap through its first-run GUI.
#
...
# the redirect env), click "Install Panergos ->" with native input, then watch
...
tell application "System Events"
  set procs to (every process whose name contains "Panergos")
...
PANERGOS_BIN="$INSTALL_DIR/venv/bin/panergos"
...
install_complete() {
  [ -d "$INSTALL_DIR/.git" ] && [ -x "$PANERGOS_BIN" ] && installed_app
}
...
BOOTSTRAP_LOG="$HOME/.panergos/logs/bootstrap-installer.log"
bootstrap_error() {
...
while :; do
  if install_complete; then
    log "install landed: checkout + venv console script + Panergos.app present"
  ...
  if ! kill -0 "$SETUP_PID" 2>/dev/null; then
    ...
    log "Panergos-Setup exited (pid $SETUP_PID) before the install landed"
    exit 1
  fi
  err="$(bootstrap_error)"
  if [ -n "$err" ]; then
