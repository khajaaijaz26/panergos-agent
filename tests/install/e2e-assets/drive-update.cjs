// drive-update.cjs — launch the INSTALLED Panergos.exe (real Electron desktop
...
//
//   node <this file> <path-to-Panergos.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <Panergos.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: PANERGOS_HOME (isolated install) and
  ...
  // The real backend (`panergos serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting Panergos... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.panergosDesktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating Panergos — this window will close" overlay should appear,
  ...
  // the staged binary), which claims PANERGOS_HOME/.panergos-update-in-progress
  ...
  // proved the desktop had exited and `panergos update` was already running).
  //
  ...
  const panergosHome = process.env.PANERGOS_HOME
  const markerPath = panergosHome ? path.join(panergosHome, '.panergos-update-in-progress') : null
  const resultPath = panergosHome ? path.join(panergosHome, '.panergos-update-result.json') : null
