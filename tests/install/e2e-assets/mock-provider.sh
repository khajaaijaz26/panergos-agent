...
#   mock_start <workroot>      start, write config into $PANERGOS_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, PANERGOS_HOME, ok/fail helpers.
...
mock_start() {
  ...
  cat > "$PANERGOS_HOME/config.yaml" <<EOF
model:
  default: mock-model
  provider: mock
providers:
...
EOF
  ...
  ok "provider 'mock' configured in $PANERGOS_HOME (api $url/v1)"
}
