"""Driver-side spawn interception for panergos desktop E2E legs.

The installed ``panergos`` is a venv console script, so its interpreter
imports ``sitecustomize`` at startup when this directory is on
``PYTHONPATH``. Behind an explicit env-var opt-in the module wraps
``subprocess.run`` so the FINAL electron launch call of ``panergos
...
Opt-in: ``PANERGOS_E2E_CAPTURE_LAUNCH=<path>`` -- the spec is written
there, and the marker file ``<path>.captured`` distinguishes "panergos
...
tag's own panergos_cli/main.py):
...
_SPEC_PATH = os.environ.get("PANERGOS_E2E_CAPTURE_LAUNCH")
...
if _SPEC_PATH:
    ...
    def _match_shape(argv: "list[str]") -> str:
        ...
        # apps/desktop/release/ (win-unpacked/Panergos.exe, linux-unpacked/...,
        # mac*/Panergos.app/Contents/MacOS/...).
