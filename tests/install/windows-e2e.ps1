...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs Panergos the way
...
# actually touches:
#
#   INSTALL   - downloads the production Panergos-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron Panergos.exe must appear.
...
#   installer's `git clone` and `panergos update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and panergos-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "panergos-desktop-app-update", "panergos-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published Panergos-Setup.exe carries
    ...
    [string]$WorkRoot = $(if ($env:PANERGOS_E2E_WORKROOT) { $env:PANERGOS_E2E_WORKROOT } else { Join-Path $env:TEMP "panergos-desktop-gui-e2e" }),

    [string]$SetupExeUrl = "https://github.com/khajaaijaz26/panergos-agent/releases/latest/download/Panergos-Setup.exe",
...
$PanergosHome  = Join-Path $WorkRoot "panergos-home"
$InstallDir  = Join-Path $PanergosHome "panergos-agent"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if panergos ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit panergos thinks we're on.
...
"@ | Set-Content -LiteralPath $gitCfg -Encoding ASCII
    ...
    # Shim git and make 'git remote get-url origin' report the canonical Panergos remote.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\Panergos.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\Panergos.exe")
    )) {
...
# Install-side state snapshot, taken BEFORE Test-PanergosRuns can throw: on
...
function Save-InstallSideState([string]$Label) {
    ...
    $logsDir = Join-Path $PanergosHome "logs"
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "panergos-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $PanergosHome ".panergos-update-result.json"
    ...
    Get-ChildItem -LiteralPath $PanergosHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "panergos-home-ls.txt")
}

function Test-PanergosRuns([string]$Label) {
    Save-InstallSideState $Label
    $panergosExe = Join-Path $InstallDir "venv\Scripts\panergos.exe"
    Assert-True (Test-Path -LiteralPath $panergosExe) "$Label -- venv\Scripts\panergos.exe exists"
    & $panergosExe --version 2>&1 | ForEach-Object { Write-Host "    panergos --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- panergos --version exits 0"
}
...
function Invoke-RefInstaller {
    ...
    $flags = @("-SkipSetup", "-PanergosHome", $PanergosHome, "-InstallDir", $InstallDir)
...
function Invoke-PanergosUpdate {
    # The venv updater. --yes reaches the update subcommand only in later
    # releases; ask the installed binary, never parse its source.
    $panergosExe = Join-Path $InstallDir "venv\Scripts\panergos.exe"
    $updateArgs = @("update")
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    $helpText = & $panergosExe update --help 2>&1 | Out-String
    if ($helpText -match '--yes') { $updateArgs += "--yes" }
    New-Item -ItemType Directory -Path (Join-Path $WorkRoot "logs") -Force | Out-Null
    $log = Join-Path $WorkRoot "logs\update.log"
    Push-Location $InstallDir
    try {
        & $panergosExe @updateArgs 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $updateExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
    }
    Write-LogGroup "panergos update transcript" $log
    Assert-True ($updateExit -eq 0) "panergos update exited $updateExit (expected 0)"
}

function Invoke-PanergosDesktopAppUpdate([string]$TargetSha) {
    # The panergos-desktop launch surface: `panergos desktop` runs its whole
    ...
    $prevCap = $env:PANERGOS_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:PANERGOS_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        & $panergosExe desktop 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $capExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
        $env:PYTHONPATH = $prevPy
        $env:PANERGOS_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "panergos desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "panergos desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $PanergosHome ".panergos-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured panergos desktop spec; update completed"
}
...
function Stop-PanergosAppProcesses([string]$Label) {
    # Close the desktop app the blunt way between phases (a user quitting).
    # Only Panergos.exe (Electron) -- never panergos.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "Panergos" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) Panergos.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `panergos update`/desktop builds use the Panergos-managed Node; use the same
    # one to run the Playwright driver so no system Node is required.
    $candidates = @(
        (Join-Path $PanergosHome "node\node.exe"),
        (Join-Path $PanergosHome "bin\node\node.exe"),
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website Panergos-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): Panergos-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "Panergos-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "Panergos-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:PANERGOS_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $PanergosHome -Force | Out-Null
    ...
    try {
        ...
        Write-Host "  Panergos-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> Panergos.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "Panergos-Setup.exe", (Join-Path $PanergosHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "Panergos" -ErrorAction SilentlyContinue)) "Panergos.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "Panergos-Setup.exe exited after Launch"
    }
    finally {
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        # Surface the installer's own log win or lose, full and folded.
        $bootLog = Join-Path $PanergosHome "logs\bootstrap-installer.log"
    ...
    Stop-PanergosAppProcesses "post-install"
    ...
    Test-PanergosRuns "post-$Mode-gui"
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop Panergos.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged Panergos.exe present before update"
    ...
    try {
        ...
        # `panergos update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .panergos-update-result.json on every exit;
        #   * older checkouts -> the staged panergos-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/panergos/relaunch
        ...
        $updateLog = Join-Path $PanergosHome "logs\update.log"
        $updateLogPos = 0
        ...
        Test-PanergosRuns "post-update"
        Assert-True ($null -ne (Get-DesktopExe)) "Panergos.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched Panergos.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "Panergos" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched Panergos window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "Panergos" -ErrorAction SilentlyContinue |
...
'@ -ErrorAction SilentlyContinue
        ...
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        $handoffLog = Join-Path $PanergosHome "logs\desktop-update-handoff.log"
        ...
        Stop-PanergosAppProcesses "post-update"
    }
}

function Invoke-PhaseInstall {
    # Dispatch on the install axis. Each arm ends with the same contract:
    # checkout at OLD, panergos runs, and state carries how OLD landed so any
    ...
    switch ($InstallMethod) {
        ...
        "installer-script" {
            ...
            Test-PanergosRuns "post-install-script"
        }
        "installer-script+desktop" {
            ...
            Assert-True ((Get-InstalledHead) -eq $state.old) "installed checkout is at OLD"
            Test-PanergosRuns "post-install-script-desktop"
...
function Invoke-PhaseUpdate {
    ...
    New-Item -ItemType File -Path (Join-Path $PanergosHome ".skip_upstream_prompt") -Force | Out-Null
    ...
    switch ($Route) {
        ...
        "panergos-desktop-app-update" {
            Invoke-PanergosDesktopAppUpdate $state.current
        }
        "panergos-update" {
            Invoke-PanergosUpdate
        }
        "installer-script" {
        ...
        "desktop-installer@latest" {
            # A user re-downloading Panergos-Setup.exe and clicking Install over
...
function Invoke-CheckedPhaseUpdate {
    ...
    foreach ($oldLog in @((Join-Path $WorkRoot "logs\update.log"), (Join-Path $PanergosHome "logs\desktop.log"))) {
