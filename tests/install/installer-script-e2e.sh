...
#              install landed on OLD with a working `panergos`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `panergos`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method panergos-update|installer-script|installer-script+desktop
...
#   --update-method  panergos-update      `panergos update`
#                    installer-script   re-run install.sh (HEAD's copy)
#                    installer-script+desktop  re-run with --include-desktop
#                    panergos-desktop-app-update  launch the app via `panergos
...
case "$UPDATE_METHOD" in
  panergos-update|installer-script|installer-script+desktop|panergos-desktop-app-update) ;;
  *) echo "error: --update-method must be panergos-update, installer-script, installer-script+desktop or panergos-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/panergos-installer-script-e2e"
LOG_DIR="${PANERGOS_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled panergos or a
# developer config, and old installer scripts hardcode $HOME/.panergos (the
# PANERGOS_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export PANERGOS_HOME="$HOME/.panergos"
...
assert_desktop_artifact() {
  ...
  local cand
  for cand in \
    "$release_dir/linux-unpacked/Panergos" \
    "$release_dir/mac-arm64/Panergos.app" \
    "$release_dir/mac/Panergos.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local panergos="$INSTALL_DIR/venv/bin/panergos"
  [ -x "$panergos" ] || fail "no panergos console script at $panergos"
  "$panergos" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "panergos --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "panergos --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `panergos desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `panergos update` itself makes. Probe the
  # INSTALLED panergos for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `panergos desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local panergos="$INSTALL_DIR/venv/bin/panergos"
  if ! "$panergos" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "panergos desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$panergos" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "panergos desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "panergos desktop --build-only ($1) exited $rc; transcript above"
  ok "panergos desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  panergos-update)
    ...
    # installed panergos; older ones read the prompt from stdin, so close it.
    PANERGOS="$INSTALL_DIR/venv/bin/panergos"
    if "$PANERGOS" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$PANERGOS" update --yes)
    else
      update_cmd=("$PANERGOS" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "panergos update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "panergos update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  panergos-desktop-app-update)
    # The real user surface: `panergos desktop` launches the app, the user
    ...
    # suite's own) is configured into PANERGOS_HOME exactly like the dev:mock
    ...
    step "capturing the panergos desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      PANERGOS_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$PANERGOS" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "panergos desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "panergos desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "panergos desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$PANERGOS_HOME/logs" "$ildest/panergos-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/panergos/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$PANERGOS_HOME/.panergos-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$PANERGOS_HOME" > "$ildest/panergos-home-ls.txt" 2>/dev/null || true
