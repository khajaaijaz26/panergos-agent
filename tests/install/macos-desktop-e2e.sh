...
# (Panergos-Setup.dmg from the website) can reach HEAD.
#
...
#              panergos-desktop-app-update  capture `panergos desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              panergos-update              CLI update from the installed venv
...
#     --update-method open-app-update|panergos-desktop-app-update
...
UPDATE_METHOD=""
INSTALL_REF=""
DMG_URL="https://github.com/khajaaijaz26/panergos-agent/releases/latest/download/Panergos-Setup.dmg"
...
case "$UPDATE_METHOD" in
  open-app-update|panergos-desktop-app-update|panergos-update|installer-script|installer-script+desktop) ;;
...
WORK_ROOT="${PANERGOS_E2E_WORKROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}/panergos-macos-desktop-e2e}"
...
EOF
  ...
  export INSTALL_DIR="$PANERGOS_HOME/panergos-agent"
}
...
phase_stage() {
  ...
  touch "$PANERGOS_HOME/.skip_upstream_prompt"
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/Panergos.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/Panergos.app" \
    "/Applications/Panergos.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published Panergos-Setup.dmg"

  local dmg="$WORK_ROOT/Panergos-Setup.dmg"
  ...
  # "Install Panergos" button with native input.
  local rc=0
  ...
  log_group "Panergos-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$panergos" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "panergos --version failed after install"
  ok "panergos --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed Panergos.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    panergos-update)
      ...
      # update subcommand only in later releases; ask the installed panergos.
      local panergos="$INSTALL_DIR/venv/bin/panergos"
      local update_cmd=("$panergos" update)
      ...
      [ "$rc" -eq 0 ] || fail "panergos update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/Panergos.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/Panergos.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built Panergos.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, panergos update) need.
      local app app_bin
...
PYEOF
    ...
    panergos-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "panergos desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "panergos desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.panergos/logs" "$ildest/panergos-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/Panergos"
  ...
  "$INSTALL_DIR/venv/bin/panergos" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "panergos --version failed after update"
  ok "panergos --version works post-update"
