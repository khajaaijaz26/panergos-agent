"""Regression test: `panergos dashboard --tui` must not hard-crash.

Older Panergos desktop app shells (<= 0.15.x) spawn the backend as::

    panergos dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Panergos couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real panergos_cli.main parser in a subprocess.
