...
from panergos_cli import gateway, main, update_cmd_fleet as fleet
...
def test_pending_marker_requires_complete_systemd_recovery(monkeypatch, tmp_path, failure):
    ...
    def systemctl(cmd, **kw):
        if "list-units" in cmd:
            ...
            return SimpleNamespace(returncode=int(failure == "listing"), stdout=(
                "panergos-gateway-one.service loaded active running\n"
                "panergos-gateway-two.service loaded failed failed\n"), stderr="")
        bad = cmd[-1] == "panergos-gateway-two"
        if "restart" in cmd:
    ...
    if failure not in (None, "running"):
        ...
        assert set(recovered) == {"panergos-gateway-one", "panergos-gateway-two"}
...
def test_pending_launchd_requires_complete_supervision(monkeypatch, tmp_path, failure):
    ...
    current, sibling = "ai.panergos.gateway", "ai.panergos.gateway-two"
