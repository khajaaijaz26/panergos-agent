...
Bug class: the checkout's ``.py`` files change (``panergos update``, manual
...
def _make_pycache(repo: Path, subdir: str = "panergos_cli") -> Path:
...
def test_sweep_clears_pycache_when_checkout_changed(monkeypatch, tmp_path):
    ...
    panergos_main._sweep_stale_bytecode_if_checkout_changed()
...
# ---------------------------------------------------------------------------
# Plugin-update sibling site: __pycache__ under ~/.panergos/plugins/<name>
# ---------------------------------------------------------------------------
