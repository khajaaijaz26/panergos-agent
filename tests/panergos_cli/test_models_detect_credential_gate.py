...
class TestNoCredentialsNoSwitch:
    ...
    def test_explicitly_named_provider_is_not_gated(self, no_live_catalog, authed, monkeypatch):
        """``/model example`` names the provider: hand it back so the credential step can prompt/fail
        loudly instead of silently ignoring the request."""
        monkeypatch.setattr(models, "detect_static_provider_for_model", lambda n, c: ("example", "example-model"))
        assert models.detect_provider_for_model("example", "deepseek") == ("example", "example-model")
