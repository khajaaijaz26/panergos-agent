...
def _local_git(head_sha):
    def fake_run(cmd, **kwargs):
        if cmd[:4] == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="https://github.com/khajaaijaz26/panergos-agent.git\n")
...
def test_local_checkout_recovers_exact_count(tmp_path):
    ...
    with patch("panergos_cli.banner.subprocess.run", side_effect=_local_git(SHA_A)), \
