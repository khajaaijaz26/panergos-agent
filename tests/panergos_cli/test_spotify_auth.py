...
import panergos_cli.auth_spotify as auth_spotify
from panergos_cli.auth import AuthError, resolve_spotify_runtime_credentials
...
def test_resolve_spotify_runtime_credentials_refreshes_without_changing_active_provider(
    ...
    assert auth_mod.get_active_provider() == "example"
...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from panergos_cli.auth_commands import auth_status_command
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    Mirrors example / xAI-OAuth / Codex-OAuth / MiniMax quarantine pattern.
    """
    monkeypatch.setenv("PANERGOS_HOME", str(tmp_path))
    ...
    def _terminal_refresh(_state, **_kw):
        raise AuthError(
            "Spotify token refresh failed. Run `panergos auth spotify` again.",
            provider="spotify",
