...
1. The venv-repair import probes (early recovery + `panergos update`) must
...
2. ``panergos doctor`` must fail the certificate check in that state, and
   ``panergos doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `panergos update`'s venv repair must apply
...
# =========================================================================
# 2. panergos doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "panergos doctor --fix" in message
