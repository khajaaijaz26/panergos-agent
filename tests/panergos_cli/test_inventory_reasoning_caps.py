...
from panergos_cli import models_reasoning_caps


def _patch_catalog(monkeypatch, caps_by_model, *, provider="example"):
    """Point the example/OpenRouter catalog readers at a fixed capability map."""
...
def test_optional_reasoning_route_can_disable(monkeypatch):
    ...
    rows = [{"slug": "example", "models": ["deepseek/deepseek-v4-pro"]}]
...
def test_non_reasoning_route_offers_no_reasoning_controls(monkeypatch):
    ...
    rows = [{"slug": "example", "models": ["moonshotai/kimi-k3-instruct"]}]
    inv._apply_capabilities(rows)
...
def test_reasoning_mandatory_route_cannot_disable(monkeypatch):
    ...
    rows = [{"slug": "example", "models": ["z-ai/glm-5.3"]}]
    inv._apply_capabilities(rows)
...
def test_unlisted_model_states_no_restriction(monkeypatch):
    ...
    """
    _patch_catalog(monkeypatch, {})
    rows = [{"slug": "example", "models": ["mystery/model"]}]
    inv._apply_capabilities(rows)
...
def test_catalog_failure_never_breaks_the_picker(monkeypatch):
    ...
    monkeypatch.setattr(models_reasoning_caps, "EXAMPLE_model_reasoning_capabilities", _boom)
    rows = [{"slug": "example", "models": ["deepseek/deepseek-v4-pro"]}]
    inv._apply_capabilities(rows)
