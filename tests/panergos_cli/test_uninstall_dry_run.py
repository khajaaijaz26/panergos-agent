...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_panergos_home", lambda: panergos_home)
    monkeypatch.setattr(uninstall, "_is_default_panergos_home", lambda home: False)
    ...
    assert str(panergos_home) in output
    assert project_root.exists()
    assert panergos_home.exists()
