"""Invariants for ``panergos_cli.config_effective.load_user_config_effective`` — the one loader every
defaults-free config reader (gateway runtime, TUI gateway, cron, ``panergos send`` bridge, doctor,
...
def _reset_caches():
    ...
    from panergos_cli import config_effective, managed_scope
...
def _reset_caches_keep_last_good():
    import panergos_cli.config as cfg
    from panergos_cli import config_effective
