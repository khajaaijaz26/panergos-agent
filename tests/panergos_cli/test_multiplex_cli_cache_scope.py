"""Multiplexed gateway: panergos_cli's process-wide caches must not hand profile A's value to profile B.

Every test builds two real profile homes (config.yaml / .env / cache files that differ), warms a
cache under ``set_panergos_home_override(A)`` and reads under B. Only the HTTP transport is canned —
...
def homes(tmp_path, monkeypatch):
    ...
    for var in ("DEEPINFRA_API_KEY", "DEEPINFRA_BASE_URL", "EXAMPLE_INFERENCE_BASE_URL"):
...
def test_model_catalog_in_process_copy_is_bound_to_its_cache_file(homes, monkeypatch):
    a, b = homes
    import panergos_cli.model_catalog as mc
