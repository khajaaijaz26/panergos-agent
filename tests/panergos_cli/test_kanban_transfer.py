"""Tests for kanban board export / import (``panergos_cli.kanban_transfer``).
...
from panergos_cli import kanban_transfer as kt
from panergos_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("PANERGOS_KANBAN_HOME", str(root))
        for var in ("PANERGOS_KANBAN_DB", "PANERGOS_KANBAN_WORKSPACES_ROOT",
                    "PANERGOS_KANBAN_ATTACHMENTS_ROOT", "PANERGOS_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Panergos"):
