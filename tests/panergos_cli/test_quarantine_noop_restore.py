...
``panergos*.exe`` shims aside (``panergos.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``panergos`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("panergos", "panergos-agent", "panergos-acp", "panergos-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "panergos.exe" in names, "panergos.exe must be restored after a no-op install"
    assert "panergos-acp.exe" in names
    assert "panergos-gateway.exe" in names
    assert (scripts / "panergos.exe").read_bytes() == b"MZ-old-panergos"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "panergos.exe").read_bytes() == b"MZ-new-panergos"
...
# ---------------------------------------------------------------------------
# panergos_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "panergos.exe" in names, "panergos.exe must be restored after a no-op recovery install"
    assert (scripts / "panergos.exe").read_bytes() == b"MZ-old-panergos"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "panergos.exe").write_bytes(b"MZ-new-panergos")
