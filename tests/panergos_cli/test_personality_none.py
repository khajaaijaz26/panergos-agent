...
Updated for the single-owner unification (panergos_cli.personality): built-ins
...
class TestCLIPersonalityNone:
    ...
    def test_set_persists_display_personality_not_system_prompt(self):
        ...
        with patch("panergos_cli.personality.persist_personality", side_effect=_persist):
    ...
    def test_neutral_restores_manual_system_prompt_without_wiping_config(self):
        ...
        with (
            patch("panergos_cli.personality.persist_personality", side_effect=_persist),
            patch(
...
class TestGatewayPersonalityNone:
    ...
    def _gateway_env(self, tmp_path):
        ...
        # gateway.run._panergos_home) and persists via persist_personality
        # (rooted at PANERGOS_HOME) — point both at the same tmp dir.
        return (
            patch("gateway.run._panergos_home", tmp_path),
            patch.dict(os.environ, {"PANERGOS_HOME": str(tmp_path)}),
        )
