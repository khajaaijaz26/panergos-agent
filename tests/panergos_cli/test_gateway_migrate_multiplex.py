"""``panergos gateway migrate``: preflight verdicts, apply/rollback bookkeeping, and the update hook.

Service layer is faked through the module's ``_installed_service`` / ``_service_op`` seams (the same
shape ``panergos gateway install`` tests use); the default gateway boot is faked by writing the
...
from panergos_cli import gateway_migrate as gm
...
def fleet(tmp_path, monkeypatch):
    ...
    def _service_op(kind, system, verb, home):
        ...
        if verb == "start" and name != "default":
            # What the real `panergos -p <name> gateway run` checks first: is a live multiplexer
        ...
        if verb == "uninstall":
            ...
            runtime.update({"pid": os.getpid(), "panergos_home": str(root), "gateway_state": "running"})
    ...
    monkeypatch.setattr(status, "_read_process_cmdline", lambda pid: "panergos gateway run")
...
def _name(home: Path) -> str:
    return panergos_constants.profile_name_for_home(home) or "default"
...
def test_update_hook_refuses_to_cross_service_user_or_home_boundary(
    ...
    boundary (other service domain, other UNIX user, PANERGOS_HOME outside profiles/). It prints the
...
def test_update_hook_still_migrates_same_user_same_scope_profiles_under_the_default_tree(fleet, capsys, monkeypatch):
    ...
    profiles/ (the shape `panergos profile create` produces) still auto-migrates."""
    fleet.services["default"] = ("systemd", False)
...
def test_auto_multiplex_migration_false_opts_out_of_the_update_hook_but_not_the_explicit_command(fleet, capsys):
    ...
    left alone by ``panergos update`` (no output, no ops, no flag flip), while the operator typing
    ...
    from panergos_cli.gateway_migrate_guards import auto_migration_opted_out
    (fleet.root / "config.yaml").write_text(
        "model:\n  default: x\nauto_multiplex_migration: false\n", encoding="utf-8")
...
def test_explicit_migrate_with_no_standalone_secondaries_still_flips_flag_and_restarts_default(fleet, capsys, monkeypatch):
    ...
    def _detached(home):  # no service manager anywhere -> detached start writes the served record
        ...
        (fleet.root / "gateway.pid").write_text(json.dumps({"pid": os.getpid(), "panergos_home": str(fleet.root)}))
        (fleet.root / "gateway_state.json").write_text(json.dumps({
            "pid": os.getpid(), "panergos_home": str(fleet.root), "gateway_state": "running",
