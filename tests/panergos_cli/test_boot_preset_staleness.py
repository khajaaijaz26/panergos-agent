...
def test_presets_stale_when_a_staged_model_has_no_section(panergos_home):
    from panergos_cli.local_runtime.bootstrap import _presets_stale

    _stage(panergos_home, "model-a")
    _stage(panergos_home, "model-b")
    _write_presets(panergos_home, "model-a")
    assert _presets_stale() is True


def test_presets_current_when_every_staged_model_is_covered(panergos_home):
    from panergos_cli.local_runtime.bootstrap import _presets_stale
...
def test_legacy_presets_without_model_paths_are_regenerated(panergos_home):
    from panergos_cli.local_runtime.bootstrap import _presets_stale

    _stage(panergos_home, "model-a")
    _write_presets(panergos_home, "model-a")
    ini = panergos_home / "runtimes/llamacpp/presets.ini"
    ini.write_text("[model-a]\nctx-size = 65536\n")
    assert _presets_stale()


def test_no_models_is_never_stale(panergos_home):
    from panergos_cli.local_runtime.bootstrap import _presets_stale
...
def test_boot_replaces_incumbent_with_stale_presets(panergos_home, monkeypatch):
    ...
    import panergos_cli.local_runtime.bootstrap as boot
    ...
    stopped = {}
    monkeypatch.setattr(
        "panergos_cli.local_runtime.endpoint._state_endpoint",
    ...
    monkeypatch.setattr(
        "panergos_cli.local_runtime.binaries.ensure_runtime_installed", fake_boot)
...
def test_refresh_bounces_an_adopted_server(panergos_home, monkeypatch):
...
def test_refresh_no_server_anywhere_is_a_noop(panergos_home, monkeypatch):
    import panergos_cli.local_runtime.bootstrap as boot

    monkeypatch.setattr(boot, "_SUPERVISOR", None)
    monkeypatch.setattr(
        "panergos_cli.local_runtime.endpoint._state_endpoint", lambda: None)
