...
SessionDB.session_gateway_runtime (panergos_state.py) — the round trip that
makes `panergos --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare PanergosCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.PanergosCLI)
...
def test_session_gateway_runtime_falls_back_to_top_level_keys():
    ...
    meta = _row(model_config={"provider": "example", "api_mode": "chat_completions"})
    runtime = SessionDB.session_gateway_runtime(meta)
    assert runtime == {"provider": "example", "api_mode": "chat_completions"}
...
def test_session_gateway_runtime_explicit_provider_wins_over_billing():
    ...
    meta = _row(model_config={"provider": "example"})
