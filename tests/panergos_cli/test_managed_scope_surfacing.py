"""Surfacing tests — managed scope shown in `config show` and `panergos doctor`."""
import pytest
...
def test_config_show_no_managed_scope_silent(tmp_path, monkeypatch, capsys):
    ...
    home.mkdir()
    monkeypatch.setenv("PANERGOS_HOME", str(home))
    monkeypatch.setenv("PANERGOS_MANAGED_DIR", str(tmp_path / "nope"))
    (home / "config.yaml").write_text("model:\n  default: user/model\n", encoding="utf-8")
    import panergos_cli.config as cfg
    from panergos_cli import managed_scope
    ...
    cfg._RAW_CONFIG_CACHE.clear()
    managed_scope.invalidate_managed_cache()
    from panergos_cli.config import show_config
...
def test_doctor_silent_with_no_managed_scope(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("PANERGOS_MANAGED_DIR", str(tmp_path / "nope"))
    from panergos_cli import managed_scope, doctor
