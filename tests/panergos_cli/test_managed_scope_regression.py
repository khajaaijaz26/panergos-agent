...
def panergos_home(tmp_path, monkeypatch):
    ...
    # /etc/panergos on the dev/CI box can't influence the test.
    monkeypatch.setenv("PANERGOS_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(panergos_home, monkeypatch):
...
def test_env_expansion_in_user_config(panergos_home, monkeypatch):
    from panergos_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_panergos_dotenv(panergos_home=str(home))
