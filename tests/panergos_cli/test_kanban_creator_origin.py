...
def test_creator_origin_survives_without_dependency_parent(tmp_path, monkeypatch, capsys, surface):
    from panergos_cli import kanban_db as kb, kanban_db_connect as kbc, kanban_db_notify as kn
    ...
    with kbc.connect_closing() as conn:
        ...
        if surface == "builtin":
            ...
            import json
            import argparse
            from panergos_cli.kanban import kanban_command
            from panergos_cli.kanban_parser import build_parser
            parser = argparse.ArgumentParser()
            build_parser(parser.add_subparsers())
            monkeypatch.setenv("PANERGOS_KANBAN_TASK", owner)
