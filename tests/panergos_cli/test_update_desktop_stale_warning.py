...
#88251: ``panergos update`` treated a failed desktop pack as non-fatal, printed
...
import panergos_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "panergos_constants.with_panergos_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "panergos_constants.display_panergos_home", lambda: str(tmp_path), raising=False
    )
...
def test_summary_omits_success_banner_when_desktop_rebuild_failed(capsys):
    ...
    assert "panergos desktop" in out
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_panergos_home", lambda: tmp_path)
