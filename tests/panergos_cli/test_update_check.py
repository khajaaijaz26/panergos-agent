"""Tests for the update check mechanism in panergos_cli.banner.
...
from panergos_cli import (
    __distribution_default_branch__,
    __distribution_repo_slug__,
)
...
def git_repo(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "panergos-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    monkeypatch.setenv("PANERGOS_HOME", str(tmp_path))
    monkeypatch.delenv("PANERGOS_REVISION", raising=False)
    monkeypatch.setattr(banner, "_resolve_repo_dir", lambda: repo_dir)
    monkeypatch.setattr("panergos_cli.config.detect_install_method", lambda root: "git")
    monkeypatch.setattr("panergos_cli.config.get_project_root", lambda: repo_dir)
    return repo_dir


def _stub_git(
    monkeypatch, *, head=SHA_A,
    origin=f"https://github.com/{__distribution_repo_slug__}.git",
):
    calls = []
...
def test_passive_check_uses_the_api_and_never_fetches(git_repo, monkeypatch):
    ...
    tip.assert_called_once_with(
        __distribution_repo_slug__.lower(), __distribution_default_branch__)
...
def test_cache_is_daily_but_invalidated_when_head_moves(git_repo, monkeypatch):
    """A fresh cache answers without any network; ``panergos update`` moving HEAD busts it at once;
    ...
    def write_cache(*, ts, head, behind):
        cache_file.write_text(json.dumps(
            {"ts": ts, "behind": behind, "rev": None, "ver": __version__,
             "distribution": __distribution_repo_slug__, "head": head}))
