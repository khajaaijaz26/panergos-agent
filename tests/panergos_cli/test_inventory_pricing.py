...
columns + Free/Pro badges and gate paid models on free example accounts, the
same way the `panergos model` CLI picker does.
"""

from threading import Event
from time import monotonic

import panergos_cli.inventory as inv
...
def test_apply_pricing_free_models_get_flat_100_percent_sale(monkeypatch):
    ...
    rows = [{"slug": "example", "models": ["a/free", "b/natively-free"]}]
    inv._apply_pricing(rows)
...
def test_prewarm_preserves_context_and_runs_once_per_profile(tmp_path, monkeypatch):
    ...
    def capture_context(_rows):
        ...
        observed[label] = (panergos_home_key(), dict(scope))
        started[label].set()
    ...
    try:
        for label in ("a", "b"):
            home = tmp_path / label
            home_token = set_panergos_home_override(str(home))
        ...
        assert observed["a"] == (
            panergos_home_key(tmp_path / "a"),
            {"PROFILE_MARKER": "a"},
        )
        assert observed["b"] == (
            panergos_home_key(tmp_path / "b"),
            {"PROFILE_MARKER": "b"},
        )
    finally:
        release.set()
...
def test_prewarm_endpoint_rotation_starts_a_new_worker(tmp_path, monkeypatch):
    ...
    def fetch_pricing(*, force_refresh=False):
        base_url = models_pricing._novita_pricing_scope()
        started[base_url].set()
        if base_url == endpoint_a:
            release_a.wait(timeout=5)
        return models_pricing._cache_catalog(base_url, expected[base_url])

    monkeypatch.setenv("NOVITA_API_KEY", "fixture-key")
    monkeypatch.setenv("NOVITA_BASE_URL", endpoint_a)
    monkeypatch.setattr(models_pricing, "_fetch_novita_pricing", fetch_pricing)
    monkeypatch.setattr(
        inv,
        "_apply_pricing",
        lambda _rows: models_pricing.get_pricing_for_provider("novita"),
    )

    token = set_panergos_home_override(str(tmp_path / "profile"))
    threads = []
    try:
        threads.append(
            inv._prewarm_pricing_async(
                [{"slug": "novita", "models": ["a/model"]}],
                current_provider="novita",
                current_base_url=endpoint_a,
            )
        )
        assert started[endpoint_a].wait(timeout=1)

        monkeypatch.setenv("NOVITA_BASE_URL", endpoint_b)
        threads.append(
            inv._prewarm_pricing_async(
                [{"slug": "novita", "models": ["b/model"]}],
                current_provider="novita",
...
def test_cached_only_dynamic_pricing_is_profile_scoped(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: models_pricing._pricing_cache[models_pricing._novita_pricing_scope()],
    )

    def in_profile(home, endpoint, *, cached_only):
        token = set_panergos_home_override(str(home))
        monkeypatch.setenv("NOVITA_BASE_URL", endpoint)
        try:
            return models_pricing.get_pricing_for_provider(
                "novita", cached_only=cached_only
            )
        finally:
            reset_panergos_home_override(token)
