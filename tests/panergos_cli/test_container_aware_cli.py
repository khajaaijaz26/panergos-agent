...
def container_env(tmp_path, monkeypatch):
    """Set up a fake PANERGOS_HOME with .container-mode file."""
    panergos_home = tmp_path / ".panergos"
    panergos_home.mkdir()
    monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))
    monkeypatch.delenv("PANERGOS_DEV", raising=False)

    container_mode = panergos_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=panergos-agent\n"
        "exec_user=panergos\n"
        "panergos_bin=/data/current-package/bin/panergos\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("panergos_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "panergos-agent"
    assert info["exec_user"] == "panergos"
    assert info["panergos_bin"] == "/data/current-package/bin/panergos"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from panergos_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "panergos"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "panergos-agent" in cmd
    assert "/data/current-package/bin/panergos" in cmd
    assert "chat" in cmd
