...
from panergos_cli.subcommands.dashboard import build_dashboard_parser
...
from panergos_cli.subcommands.whatsapp import build_whatsapp_parser
...
def test_config_get_unset_subcommands_parse():
    """`panergos config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `panergos login` fails gracefully, not with argparse error ────
#
# `panergos login` is a removed command; its handler (`login_command` in
# `panergos_cli/auth.py`) prints a deprecation notice pointing at `panergos auth` /
# `panergos model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `panergos --help`.
