...
def test_setup_telemetry_enables_shared_metrics(monkeypatch):
    config = {}
    monkeypatch.setattr(
        "panergos_cli.setup.prompt_yes_no",
...
def test_disabling_collection_closes_the_send_consent_window(monkeypatch, tmp_path):
    """`panergos tools` -> disable shared metrics must withdraw send consent.
    ...
    monkeypatch.setattr(
        "panergos_cli.setup.prompt_yes_no", lambda _question, default: False
    )
