"""Tests for `panergos -z --resume <session>` (#105892).
...
from panergos_cli.oneshot import (
...
class TestLoadResumeTarget:
    ...
    def test_empty_session_keeps_resolved_id(self, tmp_path):
        ...
        # re-minted a session for `panergos -z "hello" -c <title> --create-if-missing`,
...
class TestRunAgentResumeRuntime:
    ...
    def test_run_agent_uses_stored_runtime_and_reopens_row(self, tmp_path, monkeypatch):
        import panergos_cli.oneshot as oneshot_mod
        ...
        monkeypatch.setattr("panergos_cli.config.load_config", lambda: {"model": {"default": "ambient-model", "provider": "openrouter"}})
        monkeypatch.setattr(
        ...
        monkeypatch.setattr("panergos_cli.tools_config._get_platform_tools", lambda _cfg, _p: [])
        monkeypatch.setattr("panergos_cli.mcp_startup.ensure_mcp_discovery_before_agent_build", lambda **_kw: None)
...
class TestRunOneshotForwardsResume:
    def test_resume_kwarg_reaches_run_agent(self, monkeypatch):
        ...
        monkeypatch.setattr("panergos_cli.oneshot._run_agent", _fake_run_agent)
