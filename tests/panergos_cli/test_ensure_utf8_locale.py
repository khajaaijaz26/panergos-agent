"""Regression tests for panergos_cli._ensure_utf8().

Covers the crash class where the setup wizard (and other banner-printing
commands) emit box-drawing characters and the Relay sigil, which raise
...
in ``panergos setup`` because the repair returned early. See the Raspberry Pi
