...
1. The standalone daemon path (``panergos kanban daemon --force`` /
   :func:`panergos_cli.kanban_db_dispatch.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``panergos kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
...
def test_nonspawnable_review_does_not_tax_ready_budget(
    ...
    import panergos_cli.config as cfgmod
    import panergos_cli.profiles as profmod
