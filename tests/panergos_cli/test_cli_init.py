"""Tests for PanergosCLI initialization -- catches configuration bugs
...
def _make_cli(env_overrides=None, config_overrides=None, **kwargs):
    """Create a PanergosCLI instance with minimal mocking."""
    import importlib
    ...
    clean_env = {"LLM_MODEL": "", "PANERGOS_MAX_ITERATIONS": ""}
    if env_overrides:
    ...
    try:
        with patch.dict(sys.modules, prompt_toolkit_stubs), \
            ...
            with patch.object(_cli_mod, "get_tool_definitions", return_value=[]), \
                ...
                return _cli_mod.PanergosCLI(**kwargs)
    finally:
...
class TestFallbackChainInit:
    def test_merges_new_and_legacy_fallback_config(self):
        cli = _make_cli(config_overrides={
            ...
            "fallback_model": {"provider": "example", "model": "example-model"},
        })
...
class TestPromptToolkitTerminalCompatibility:
    ...
    def test_cpr_gating_posix_suppresses_without_ssh(self, monkeypatch):
        ...
        ``tests/panergos_cli/test_cpr_local_leak.py`` under ``windows_only``, where it
...
class TestHistoryDisplay:
    ...
    def test_resume_without_target_lists_recent_sessions(self, capsys):
        ...
        cli._session_db.list_sessions_rich.return_value = [
            ...
            },
            {
                ...
                "title": "Checking Running Panergos Agent",
                "preview": "check running gateways for panergos agent",
                "last_active": 0,
            },
        ]
        ...
        assert "Checking Running Panergos Agent" in output
    ...
    def test_sessions_with_target_delegates_to_resume(self):
        ...
        with patch.object(cli, "_handle_resume_command") as mock_resume:
            cli.process_command("/sessions Checking Running Panergos Agent")

        mock_resume.assert_called_once_with(
            "/resume Checking Running Panergos Agent"
        )
...
class TestNestedDictModelDefaultPairing:
    ...
    model AND the nested provider, so ``PanergosCLI`` routes the model through
    ...
    def test_nested_dict_default_keeps_provider_paired(self):
        cli = _make_cli(config_overrides={
            "model": {
                "default": {"provider": "example", "model": "nested-default-model"},
                "provider": "auto",
            },
        })
        assert cli.model == "nested-default-model"
        assert cli.requested_provider == "example"
        assert cli.provider == "example"
    ...
    def test_provider_prefixed_startup_model_overrides_stale_provider(self):
        cli = _make_cli(
            config_overrides={
                ...
                "providers": {
                    "example": {
                        "base_url": "https://inference-api.exampleorg.com/v1",
                    },
                },
            },
...
class TestRootLevelProviderOverride:
    ...
    def test_model_provider_wins_over_root_provider(self, tmp_path, monkeypatch):
        ...
        import cli
        monkeypatch.setattr(cli, "_panergos_home", panergos_home)
    ...
    def test_nested_dict_default_flattens_model_and_provider(self):
        ...
        assert result["model"]["provider"] == "example"
