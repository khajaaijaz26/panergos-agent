...
Covers:
 - Normal path (tokens already in Panergos auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Panergos
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `panergos auth openai-codex`. The old
...
@pytest.fixture()
def panergos_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Panergos auth store (no Codex CLI needed)."""
    ...
    for var in [
        ...
        "EXAMPLE_API_KEY", "DEEPSEEK_API_KEY",
    ]:
...
def test_normal_path_still_works(panergos_auth_only_env):
    """openai-codex appears when tokens are already in Panergos auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Panergos
    auth store."""
    panergos_home = tmp_path / ".panergos"
    panergos_home.mkdir()
    ...
    (panergos_home / "auth.json").write_text(
...
def test_no_codex_when_no_credentials(tmp_path, monkeypatch):
    ...
    for var in [
        "OPENROUTER_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY",
        "EXAMPLE_API_KEY", "DEEPSEEK_API_KEY", "COPILOT_GITHUB_TOKEN",
