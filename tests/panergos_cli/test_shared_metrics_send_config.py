...
from panergos_cli.observability.shared_metrics_send_config import (
    DEFAULT_ENDPOINT,
    resolve_send_config,
...
class TestDefaults:
    ...
    def test_no_collector_is_configured_by_default(self):
        shared = DEFAULT_CONFIG["telemetry"]["shared_metrics"]
...
class TestSendRequiresCollection:
    ...
    def test_send_with_collection_sends(self):
        resolved = resolve_send_config(
            _config(enabled=True, send=True, endpoint="https://collector.example.test/v1")
        )
...
class TestEndpointPrecedence:
    ...
    def test_no_environment_variable_can_redirect_telemetry(self, monkeypatch):
        ...
        AGENTS.md also reserves PANERGOS_* for secrets, not behaviour.
        """
        for name in (
            "PANERGOS_TELEMETRY_ENDPOINT",
            "TELEMETRY_ENDPOINT",
            "PANERGOS_SHARED_METRICS_ENDPOINT",
        ):
