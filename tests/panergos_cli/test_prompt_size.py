"""Tests for the ``panergos prompt-size`` diagnostic (issue #34667)."""
...
from panergos_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(panergos_home, memory_text="", user_text=""):
    mem_dir = panergos_home / "memories"
...
def _seed_skill(panergos_home, name, description):
    skill_dir = panergos_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    ...
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "EXAMPLE_API_KEY",
                "ANTHROPIC_API_KEY"):
