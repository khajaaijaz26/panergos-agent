...
from panergos_cli import gateway as panergos_gateway
...
from panergos_cli import process_identity
...
def test_control_plane_argv_is_not_a_gateway():
    ...
    serve = "C:\\Panergos\\.venv\\Scripts\\python.exe -m panergos_cli.main serve --host 127.0.0.1"
    run = "C:\\Panergos\\.venv\\Scripts\\python.exe -m panergos_cli.main gateway run"
...
def test_control_plane_classifier_is_token_based_not_substring():
    ...
    py = "C:\\Panergos\\.venv\\Scripts\\python.exe -m panergos_cli.main"
...
def test_pause_skips_cold_start_plan_when_desktop_owns_lifecycle(monkeypatch):
    ...
    monkeypatch.setattr(panergos_gateway, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(
        panergos_gateway, "find_windows_gateway_services", lambda **_k: []
    )
...
def test_attested_dead_gateway_survives_desktop_ownership_and_marker_is_consumed_on_spawn(
    ...
    monkeypatch.setattr("panergos_cli.config.get_panergos_home", lambda: str(tmp_path))
    monkeypatch.setattr(cli_main, "_is_windows", lambda: True)
    monkeypatch.setattr(main_install_repair, "_is_windows", lambda: True)
    monkeypatch.setattr(panergos_gateway, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(panergos_gateway, "find_windows_gateway_services", lambda **_k: [])
    monkeypatch.setattr(gateway_windows, "is_installed", lambda: True)
    monkeypatch.setattr(update_cmd, "_desktop_owns_gateway_lifecycle", lambda: True)
    monkeypatch.setattr(update_cmd_windows, "_desktop_owns_gateway_lifecycle", lambda: True)
...
def test_cold_start_is_authorized_by_the_token_generation_not_the_mutable_marker(
    ...
    """#110020 review (a): the marker is a one-shot that a concurrent ``panergos gateway status``
    consumes between plan and execution. The spawn must still happen (authority lives on the
    token), and a *newer* marker written by a concurrent ``panergos gateway start`` must not be
    ...
    # Concurrent ``panergos gateway status`` consumed the marker...
    assert gateway_windows.check_start_attestation(current_pids=[]) is not None
    assert not marker.exists()
    # ...and a concurrent ``panergos gateway start`` wrote a fresh one for its own PID.
