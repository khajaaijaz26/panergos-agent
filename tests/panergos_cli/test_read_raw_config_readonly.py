...
@pytest.fixture()
def isolated_panergos_home():
    """Per-test PANERGOS_HOME dir (already redirected by the autouse conftest
...
def test_freshness_after_config_edit(isolated_panergos_home):
    from panergos_cli.config import read_raw_config_readonly

    cfg = _write_config(isolated_panergos_home, {"display": {"ephemeral_system_ttl": 1}})
    first = read_raw_config_readonly()
    assert first["display"]["ephemeral_system_ttl"] == 1

    _write_config(isolated_panergos_home, {"display": {"ephemeral_system_ttl": 7}})
...
def test_missing_config_returns_empty(isolated_panergos_home):
    from panergos_cli.config import read_raw_config_readonly

    cfg = isolated_panergos_home / "config.yaml"
    if cfg.exists():
        cfg.unlink()
