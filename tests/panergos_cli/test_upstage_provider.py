...
Regression guard for the bug where `panergos model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`panergos_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in PANERGOS_OVERLAYS
        overlay = PANERGOS_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `panergos_cli/main.py` (cmd_model): read
