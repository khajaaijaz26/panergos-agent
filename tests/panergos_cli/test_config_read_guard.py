...
Canonical owners:
  ...
  * ``panergos_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._panergos_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/panergos/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "panergos_cli/managed_scope.py",
