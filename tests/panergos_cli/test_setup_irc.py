"""Tests for IRC gateway configuration via `panergos setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Panergos install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Panergos install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#panergos")
            monkeypatch.setenv("IRC_NICKNAME", "panergos-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `panergos setup gateway` experience with IRC present."""
