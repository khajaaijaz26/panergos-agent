...
``--model example/deepseek-v4-pro`` / ``--model ollama/qwen3.5:4b`` used to fall
...
class TestVendorPrefixRouting:
    ...
    def test_configured_provider_prefix_routes_to_provider(self, monkeypatch):
        ...
        monkeypatch.setattr(models, "_configured_provider_ids", lambda: {"example"})
        detected = models.detect_provider_for_model("example/deepseek-v4-pro", "anthropic")
        assert detected == ("example", "deepseek-v4-pro")
    ...
    def test_openrouter_slug_still_wins_over_prefix_routing(self, monkeypatch):
        ...
        from panergos_cli import models_detect
