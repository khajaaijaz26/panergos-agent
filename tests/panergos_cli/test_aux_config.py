"""Tests for the auxiliary-model configuration UI in ``panergos model``.
...
def test_save_aux_choice_persists_to_config_yaml(tmp_path, monkeypatch):
    ...
    (tmp_path / ".panergos").mkdir(exist_ok=True)
...
def test_reset_aux_clears_delegation_routing_preserves_settings(tmp_path, monkeypatch):
    ...
    from panergos_cli.config import load_config as _lc, save_config
...
def test_delegation_cfg_as_task_projection():
    ...
    shaped = _delegation_cfg_as_task(
        {"delegation": {"provider": "example", "model": "example-reasoning-model"}}
    )
    assert _format_aux_current(shaped) == "example · example-reasoning-model"
...
def test_leave_unchanged_replaces_cancel_label(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(panergos_cli_main_provider_setup, "_prompt_provider_choice", fake_prompt)
