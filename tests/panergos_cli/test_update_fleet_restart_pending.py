...
A ``panergos update`` killed after git pull advanced HEAD but before the
...
- next ``panergos update`` with git already up to date still runs the
...
import panergos_cli.main_web_build as main_web_build
import panergos_cli.main_install_repair as main_install_repair
from panergos_cli import update_cmd
import panergos_cli.update_cmd_fleet as update_cmd_fleet
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    monkeypatch.setattr(panergos_main.subprocess, "run", run_side_effect)
    monkeypatch.setattr(panergos_main, "PROJECT_ROOT", tmp_path)
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(panergos_main, "_resolve_update_branch", lambda args: "main")
    monkeypatch.setattr(panergos_main, "_is_windows", lambda: False)
    ...
    monkeypatch.setattr(
        panergos_main,
        "_get_origin_url",
        lambda *a, **k: "https://github.com/khajaaijaz26/panergos-agent.git",
    )
    monkeypatch.setattr(update_cmd, "_is_fork", lambda *a, **k: False)
    monkeypatch.setattr(
        panergos_main, "_stash_local_changes_if_needed", lambda *a, **k: None
    )
    monkeypatch.setattr(panergos_main, "_clear_bytecode_cache", lambda *a, **k: 0)
    monkeypatch.setattr(
        panergos_main, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(
        main_web_build, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(panergos_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(
        panergos_main, "_pause_windows_gateways_for_update", lambda: None
    )
    monkeypatch.setattr(
        panergos_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(panergos_main, "_write_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(panergos_main, "_clear_update_incomplete_marker", lambda: None)
    ...
    monkeypatch.setattr(panergos_main, "_build_web_ui", lambda *a, **k: None)
    ...
    monkeypatch.setattr(update_cmd, "_purge_stale_panergos_modules", lambda: None)
    monkeypatch.setattr(panergos_main, "_purge_stale_panergos_modules", lambda: None)

    import panergos_cli.gateway as panergos_gateway

    monkeypatch.setattr(
        panergos_gateway, "find_gateway_pids", lambda **_kwargs: []
    )
    monkeypatch.setattr(panergos_gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(
        panergos_gateway, "find_profile_gateway_processes", lambda *a, **k: []
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "panergos_cli.update_inventory.collect_runtime_inventory",
...
def test_marker_round_trip_under_panergos_home():
    path = update_cmd._fleet_restart_pending_marker_path()
    assert path.parent == get_panergos_home()
...
def test_pending_needed_when_unfinished_receipt_runtime_sha_skews(monkeypatch):
    ...
    receipt_dir = get_panergos_home() / "logs" / "update_receipts"
...
def test_run_pending_restart_true_when_no_gateways(monkeypatch, capsys):
    monkeypatch.setattr(
        "panergos_cli.gateway.find_gateway_pids", lambda **k: []
    )
    monkeypatch.setattr(panergos_main, "_purge_stale_panergos_modules", lambda: None)
...
def test_clean_update_escalates_surviving_serve_as_unaccounted(
    ...
    ``panergos-gateway.service``) and an unmanaged ``serve`` on the same
    ...
    import panergos_cli.update_inventory as ui
    ...
    import panergos_cli.process_identity as pi
    ...
    latest = get_panergos_home() / "logs" / "update_receipts" / "latest.json"
...
def test_interrupt_between_pull_and_restart_leaves_marker(
    ...
    monkeypatch.setattr(panergos_main, "_clear_bytecode_cache", _interrupt)
