...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            panergos_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(panergos_constants, "get_panergos_home", lambda: tmp_path)
    monkeypatch.setattr(panergos_state, "SessionDB", FakeSessionDB)
    ...
    assert "panergos sessions optimize-storage" in capsys.readouterr().out
    conn.close()
