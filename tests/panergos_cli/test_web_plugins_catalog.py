...
``.panergos-catalog.json`` sidecar) and the install endpoint has NO kill-list bypass."""
...
from panergos_cli import plugin_catalog as pc_cat
...
@pytest.fixture
def client(monkeypatch, tmp_path, _isolate_panergos_home):
    try:
...
def _install(name: str, sidecar: dict | None):
    from panergos_constants import get_panergos_home
    d = get_panergos_home() / "plugins" / name
    d.mkdir(parents=True)
    (d / "plugin.yaml").write_text(yaml.safe_dump({"name": name, "version": "1.0", "description": "x"}))
    if sidecar:
        (d / ".panergos-catalog.json").write_text(json.dumps(sidecar))
...
def test_install_endpoint_refuses_removed_plugins_with_no_bypass(client, monkeypatch):
    from panergos_cli import plugins_cmd
    ...
    for body in ({"identifier": "https://github.com/evil/bad-plugin.git"},
        ...
        assert "removed" in resp.json()["detail"] or "not in the Panergos plugin catalog" in resp.json()["detail"]
