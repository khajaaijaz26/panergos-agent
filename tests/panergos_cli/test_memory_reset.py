"""Tests for the `panergos memory reset` CLI command.
...
- Profile-scoped reset (uses PANERGOS_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake PANERGOS_HOME with memory files."""
    panergos_home = tmp_path / ".panergos"
    memories = panergos_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nPanergos repo is at ~/.panergos/panergos-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `panergos memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        panergos_home = tmp_path / ".panergos"
        (panergos_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))
