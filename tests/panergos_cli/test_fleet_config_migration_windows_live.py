...
fresh-process semantics via PANERGOS_HOME env — mirrors the Linux live E2E.
"""
import sys
...
def test_fleet_config_migration_live_windows(tmp_path, monkeypatch):
    active = tmp_path / "panergos-home"
    profiles = active / "profiles"
    for name, ver in [("research", 12), ("work", 25)]:
        home = profiles / name
        home.mkdir(parents=True)
        (home / "config.yaml").write_text(
            yaml.safe_dump({"_config_version": ver, "model": {"provider": "example"}}),
            encoding="utf-8",
        )
    ...
    monkeypatch.setenv("PANERGOS_HOME", str(active))
