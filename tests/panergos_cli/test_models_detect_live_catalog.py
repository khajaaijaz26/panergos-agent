...
class TestCurrentProviderCatalogWins:
    @pytest.mark.parametrize("provider,model", [
        ...
        ("example", "some-portal-only-model"),    # Portal serves it, static snapshot doesn't
    ...
    def test_bare_name_resolves_to_current_providers_full_slug(self, live_catalog):
        live_catalog["example"] = ["zai/glm-5.3-flash"]
        assert models.detect_provider_for_model("glm-5.3-flash", "example") == ("example", "zai/glm-5.3-flash")
    ...
    def test_unserved_model_still_walks_the_ladder(self, live_catalog, monkeypatch):
        ...
        live_catalog["example"] = ["example-model"]
        assert models.detect_provider_for_model("no-such-model", "example") == ("openrouter", "vendor/no-such-model")
