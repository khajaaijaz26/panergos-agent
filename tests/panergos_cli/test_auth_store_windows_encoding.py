...
``_load_auth_store`` and the Codex/example shared-store readers previously called
...
def panergos_home(tmp_path, monkeypatch):
    """Point PANERGOS_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.panergos/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, panergos_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, panergos_home, windows_default_encoding):
        ...
        assert not (panergos_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, panergos_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, panergos_home):
        auth_path = panergos_home / "auth.json"
...
# --- sibling readers of the same ~/.panergos/auth.json in other modules -------
#
# _load_auth_store lives in panergos_cli/auth.py, but several other modules read
# the same ~/.panergos/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, panergos_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(panergos_home / "auth.json", store)
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, panergos_home, monkeypatch, windows_default_encoding
    ):
        """panergos_cli.main._has_any_provider_configured reads auth.json.
        ...
        # panergos_cli.auth; patch it on the source module so the local import
        # sees the fake.
