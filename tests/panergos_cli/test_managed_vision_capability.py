...
def _stage(home_root, name):
    # Machine-scoped models dir (the shared root — tmp PANERGOS_HOME IS the root here).
    from panergos_cli.local_runtime.bootstrap import models_dir
...
def test_not_ours_returns_none(panergos_home):
    from panergos_cli.local_runtime.capabilities import managed_model_supports_vision

    assert managed_model_supports_vision("gpt-4o") is None


def test_catalog_vision_model_with_projector_on_disk(panergos_home):
    """Staged catalog model with an mmproj present: True (server down —
    the catalog + on-disk projector answer)."""
    from panergos_cli.local_runtime.bootstrap import assets_dir
    from panergos_cli.local_runtime.capabilities import managed_model_supports_vision
    ...
    _stage(panergos_home, variant.model_id)
    adir = assets_dir()
...
def test_catalog_vision_model_missing_projector_is_blind(panergos_home):
...
def test_live_props_beats_catalog(panergos_home, monkeypatch):
    """A running child's modalities report wins over the catalog: the
    server that will receive the image is the authority."""
    import panergos_cli.local_runtime.capabilities as caps
...
def test_lookup_chain_consults_managed_runtime(panergos_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "panergos_cli.local_runtime.capabilities.managed_model_supports_vision",
        lambda mid: True)
...
def test_webp_transcodes_to_png_for_managed_provider(panergos_home, monkeypatch, tmp_path):
...
def test_webp_passes_through_for_cloud_providers(panergos_home, monkeypatch, tmp_path):
...
def test_vision_analyze_normalization_narrows_for_managed(panergos_home, monkeypatch, tmp_path):
