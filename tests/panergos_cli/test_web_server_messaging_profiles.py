...
profile's PANERGOS_HOME, and the dashboard's own profile stays untouched.
"""
import pytest
import yaml
...
class TestProfileScopedMessagingReads:
    ...
    def test_scoped_read_returns_profile_path_command_and_startup_failure(
        ...
        monkeypatch.setattr(
            ...
            # relying on process-level PANERGOS_HOME resolution (#71211).
            lambda *a, **k: {
        ...
        assert payload["gateway_start_command"] == (
            "panergos -p worker_alpha gateway start"
        )
...
def test_named_current_home_matches_unscoped(client, isolated_profiles, monkeypatch):
    from panergos_cli.web_server_profiles import _config_profile_scope, _panergos_home_scope
    from panergos_constants import get_panergos_home
    ...
    with _panergos_home_scope(isolated_profiles["worker_alpha"]):
        with _config_profile_scope("default") as scoped:
            assert scoped is None
            assert get_panergos_home() == isolated_profiles["default"]
...
def test_credential_write_hot_serves_a_multiplexed_profile(client, isolated_profiles, monkeypatch, topology):
    ...
    ``?profile=`` and a pooled ``panergos --profile X serve`` that receives the PUT unscoped (#109088)."""
    import panergos_cli.gateway as gateway_cli
    import panergos_cli.gateway_multiplex_served as served_mod
    notified = []
