...
minimal ``PanergosCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def panergos_home(tmp_path, monkeypatch):
    """Isolated PANERGOS_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".panergos"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("PANERGOS_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves PANERGOS_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal PanergosCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, panergos_home):
        ...
        with patch("panergos_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, panergos_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, panergos_home):
        ...
        src = inspect.getsource(PanergosCLI.chat)
