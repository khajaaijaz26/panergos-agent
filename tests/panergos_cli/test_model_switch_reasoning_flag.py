...
The parser is the single owner (panergos_cli.model_switch.parse_model_switch_args); the CLI and
...
def test_cli_commit_applies_effort_after_the_agent_swap(monkeypatch):
    ...
    agent = _Agent()
    cli = SimpleNamespace(
        model="old", provider="example", requested_provider="example", _explicit_api_key="", _explicit_base_url="",
    ...
    cli._stage_and_swap_model = lambda result, old: cli_mod.PanergosCLI._stage_and_swap_model(cli, result, old)
    monkeypatch.setattr(mixin, "_print_switch_summary", lambda *_a, **_k: None)
    monkeypatch.setattr(cli_mod.PanergosCLI, "_persist_model_switch_to_session", lambda *_a: None)
    saved = {}
    monkeypatch.setattr(cli_mod, "save_config_value", lambda k, v: saved.setdefault(k, v) or True)

    result = ModelSwitchResult(success=True, new_model="new", target_provider="example")
