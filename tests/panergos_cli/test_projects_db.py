"""Tests for the per-profile Projects store (panergos_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Panergos Agent", folders=["/tmp/panergos"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "panergos-agent"
    assert proj.name == "Panergos Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/panergos"
    assert [f.path for f in proj.folders] == ["/tmp/panergos"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "panergos-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' PANERGOS_HOME.
