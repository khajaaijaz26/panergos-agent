"""Tests for the top-level `./panergos` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./panergos` should use `panergos_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "panergos"
    called = []

    fake_main_module = types.ModuleType("panergos_cli.main")

    def fake_main():
        called.append("panergos_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "panergos_cli.main", fake_main_module)
    ...
    assert called == ["panergos_cli.main"]
