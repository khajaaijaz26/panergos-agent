...
def _rows_with_env(monkeypatch, env_name: str, provider: str) -> list[dict]:
    ...
    with (
        ...
        patch("panergos_cli.models.cached_provider_model_ids", return_value=["model-a"]),
        patch("panergos_cli.providers.PANERGOS_OVERLAYS", {}),
    ):
        return list_authenticated_providers(max_models=5)


def test_models_dev_only_provider_is_not_selectable(monkeypatch):
    rows = _rows_with_env(monkeypatch, "COHERE_API_KEY", "cohere")

    assert all(row["slug"] != "cohere" for row in rows)
    assert not is_runtime_provider_routable("cohere")
