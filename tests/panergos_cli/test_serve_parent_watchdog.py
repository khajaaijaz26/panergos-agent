"""Regression tests for Desktop-owned ``panergos serve`` lifecycle tracking."""

import pytest

from panergos_cli.web_server_lifecycle import (
    _is_serve_orphaned,
...
def test_parent_watchdog_treats_empty_marker_env_as_absent(monkeypatch):
    """Blank inherited PANERGOS_PARENT_START_MARKER/NONCE degrade to PID-only tracking."""
    from panergos_cli import web_server_lifecycle

    monkeypatch.setenv("PANERGOS_PARENT_PID", "4242")
    monkeypatch.setenv("PANERGOS_PARENT_START_MARKER", "")
    monkeypatch.setenv("PANERGOS_PARENT_NONCE", "")
    seen = {}
...
def test_parent_watchdog_warns_when_disarmed_by_unusable_marker(monkeypatch, caplog):
    ...
    monkeypatch.setenv("PANERGOS_PARENT_PID", "4242")
    monkeypatch.setenv("PANERGOS_PARENT_START_MARKER", "ps:Sat")
    monkeypatch.setenv("PANERGOS_PARENT_NONCE", "n")
    started = []
    monkeypatch.setattr(
        web_server_lifecycle.threading, "Thread", lambda *a, **k: started.append(1) or _NoThread()
    )
    with caplog.at_level(logging.WARNING, logger="panergos_cli.web_server"):
        web_server_lifecycle._start_parent_death_watchdog()
