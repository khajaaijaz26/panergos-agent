"""Tests for the panergos_cli models module."""
...
import panergos_cli.models as _models_mod
from panergos_cli import models_local
from panergos_cli import models_validate
...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("panergos_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        ...
        with patch("panergos_cli.model_catalog.get_curated_openrouter_models", return_value=None), \
             patch("panergos_cli.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        panergos-agent is tool-calling-first — surfacing a non-tool model leads to
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with (
            patch("panergos_cli.model_catalog.get_curated_openrouter_models", return_value=[]),
            patch("panergos_cli.models._urlopen_model_catalog_request", return_value=_Resp()),
        ):
            models = fetch_openrouter_models(force_refresh=True)
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from panergos_cli.models import _openrouter_model_supports_tools
...
class TestDetectProviderForModel:
    ...
    def test_short_alias_resolves_to_static_model(self):
        ...
        ), patch("panergos_cli.models_detect.provider_has_credentials", return_value=True):
...
class TestFormatPricePerMtok:
    ...
    def test_standard_prices_keep_two_decimals(self):
        from panergos_cli.models_pricing import _format_price_per_mtok
...
class TestLocalOllamaModelDiscovery:
    ...
    def test_native_tags_cache_expires(self, monkeypatch):
        from panergos_cli.models_local import fetch_ollama_local_models
        ...
        try:
            ...
            monkeypatch.setattr("panergos_cli.models.time.monotonic", lambda: 301.0)
    ...
    def test_runtime_error_from_config_load_does_not_escape_ollama_helpers(self):
        ...
        from panergos_cli.models import _get_ollama_base_url
        from panergos_cli.models_local import should_use_ollama_native_catalog

        with patch("panergos_cli.config.load_config", side_effect=RuntimeError("bad home")), patch(
            "panergos_cli.models_local.probe_ollama_local_models",
            return_value=None,
        ):
    ...
    def test_probe_ollama_models_malformed_base_url_returns_none(self):
        ...
        from panergos_cli.models_local import probe_ollama_local_models
    ...
    def test_fetch_ollama_models_preserves_probe_failure(self):
        ...
        with patch("panergos_cli.models_local.probe_ollama_local_models", return_value=None):
    ...
    def test_ollama_port_detection_requires_working_api_tags(self):
        ...
        with patch("panergos_cli.models_local.probe_ollama_local_models", return_value=["qwen3:1.7b"]):
    ...
    def test_provider_model_ids_ollama_cloud_config_uses_generic_catalog(self):
        ...
        ), patch("panergos_cli.models_local.fetch_ollama_local_models") as fetch_local, patch(
    ...
    def test_ollama_cache_fingerprint_does_not_probe_custom_endpoint(self):
        from panergos_cli.models import _credential_fingerprint
        ...
        ), patch("panergos_cli.models_local.probe_ollama_local_models") as probe_ollama:
    ...
    def test_picker_user_provider_verifies_ambiguous_ollama_port(self):
        ...
        with patch("panergos_cli.config.load_config", return_value={"providers": {}}), patch(
            "panergos_cli.models_local.should_use_ollama_native_catalog",
            return_value=True,
        ), patch(
            "panergos_cli.models_local.fetch_ollama_local_models",
            return_value=["qwen3:1.7b"],
        ), patch("panergos_cli.models.fetch_api_models", return_value=[]) as fetch_api:
            rows = list_authenticated_providers(
    ...
    def test_named_custom_model_flow_discovers_ollama_api_tags(self):
        ...
        from panergos_cli.model_setup_flows import _model_flow_named_custom
        ...
        try:
            with patch("panergos_cli.config.load_config", return_value=config), patch(
                "panergos_cli.config.save_config"
            ), patch("panergos_cli.auth._save_model_choice"), patch(
                "panergos_cli.auth.deactivate_provider"
            ), patch("panergos_cli.main_provider_setup._save_custom_provider"), patch(
                "panergos_cli.curses_ui.curses_radiolist",
    ...
    def test_model_validation_ollama_cloud_config_does_not_use_local_tags(self):
        ...
        ), patch("panergos_cli.models_local.probe_ollama_local_models") as probe_ollama, patch(
    ...
    def test_switch_model_on_current_ollama_custom_endpoint_keeps_base_url(self):
        ...
        from panergos_cli.model_switch import switch_model
        ...
        try:
            with patch(
                ...
                "panergos_cli.model_switch.get_model_info",
                return_value=None,
            ):
                result = switch_model(
    ...
    def test_ollama_host_environment_forms_are_normalized(self, monkeypatch):
        from panergos_cli.models import _get_ollama_base_url, _root_for_ollama_native_api
    ...
    def test_ollama_failed_probe_is_cached_briefly(self):
        ...
        with patch(
            "panergos_cli.models._urlopen_model_catalog_request",
    ...
    def test_switch_model_direct_ollama_alias_preserves_matching_origin_api_key(self):
        import panergos_cli.model_switch as model_switch
...
class TestOpenRouterCatalogDiskCache:
    ...
    def _install_network(monkeypatch, calls):
        ...
        monkeypatch.setattr("panergos_cli.model_catalog.get_curated_openrouter_models",
                            lambda: [("a/one", "")])
