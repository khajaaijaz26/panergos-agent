"""Tests for the Ctrl+S prompt stash wiring inside PanergosCLI.

The state machine itself is covered by tests/panergos_cli/test_prompt_stash.py. These
tests verify the cli.py side:
  - PanergosCLI.__init__ creates a PromptStash
...
tests/panergos_cli/test_cli_extension_hooks.py.
"""
...
def _make_cli(**kwargs):
    """Create a PanergosCLI with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
...
class TestStashStateInit:
    ...
    def test_stash_starts_empty(self, cli):
        ...
        # module, which re-imports panergos_cli.prompt_toolkit stubs and can
...
class TestKeybindingRegistration:
    ...
    ``PanergosCLI.run()``'s local ``KeyBindings`` instance, so they are not
