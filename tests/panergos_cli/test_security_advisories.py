"""Tests for panergos_cli.security_advisories.
...
import panergos_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect PANERGOS_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full panergos_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "panergos_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
