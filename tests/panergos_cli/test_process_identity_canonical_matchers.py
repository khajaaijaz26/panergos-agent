...
from panergos_cli.dashboard_procs import _is_desktop_local_serve_cmdline
from panergos_cli.update_cmd_windows import _panergos_holder_subcommand, _is_backend_argv
...
# value. The updater's kill set additionally requires the Desktop's `-m panergos_cli.main` spawn shape —
...
CMDLINES = [
    ...
    ("panergos --profile ops serve " + LOOPBACK, "serve", True, False),
    ("panergos -m serve kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("python -m panergos_cli.main kanban --preserve-cache " + LOOPBACK, "kanban", False, False),
    ("panergos --reasoning high dashboard " + LOOPBACK, "dashboard", False, False),
...
def test_kill_and_relaunch_predicates_agree_with_the_canonical_holder_matcher(
    ...
    assert _panergos_holder_subcommand(cmdline) == subcommand
...
def test_desktop_local_serve_spares_fixed_port_and_remote_hosts():
    assert not _is_desktop_local_serve_cmdline("panergos serve --host 100.106.105.2 --port 9119 --skip-build")
    assert not _is_desktop_local_serve_cmdline("panergos serve --host 127.0.0.1 --port 9119")
    assert _is_desktop_local_serve_cmdline("panergos serve --host localhost --port 0")
