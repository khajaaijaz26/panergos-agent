"""Regression coverage for stale ``panergos serve`` generations after an abort.

Issue #92145: ``panergos update`` moves the checkout from generation N to N+1
...
``panergos serve`` is not a gateway profile.  It hosts ``tui_gateway.server``,
systemd lists ``panergos-gateway.service`` before ``panergos-serve.service`` (so
...
from panergos_cli import update_abort_recovery as abort_recovery
from panergos_cli import update_cmd
from panergos_cli import update_restart_recovery as recovery
...
def _identity_module():
    return __import__("panergos_cli.process_identity", fromlist=["ledger_entries"])
...
def test_failed_serve_unit_blocks_completion():
    ...
    result["serve_units"] = {"verified": [], "failed": ["panergos-serve"]}
    assert (
        update_cmd._abort_recovery_is_complete(
            planned_gateway_profiles={"default"},
            covered_gateway_profiles={"default"},
...
class _Systemctl:
    ...
    def __call__(self, argv, **kwargs):
        if "list-units" in argv:
            ...
            body = "\n".join(
                f"{unit} loaded active running Panergos" for unit in self.listed
            )
...
def test_serve_unit_verified_when_main_pid_changes(linux_systemctl):
    fake = _Systemctl(
        listed=["panergos-serve.service"],
        active={"panergos-serve.service": True},
        main_pids={"panergos-serve.service": 4242},
    )
    ...
    assert out == {"verified": ["user/panergos-serve"], "failed": []}
    assert fake.restarted == ["panergos-serve.service"]
...
def test_unchanged_main_pid_is_not_verified(linux_systemctl):
    ...
    assert out == {"verified": [], "failed": ["user/panergos-serve"]}
...
def test_unit_that_does_not_come_back_active_is_failed(linux_systemctl):
    ...
    def dies(argv, **kwargs):
        if "restart" in argv:
            fake.restarted.append(argv[-1])
            fake.active["panergos-serve.service"] = False
            return _Completed(0)
        return original_call(argv, **kwargs)
...
def test_inactive_serve_unit_is_left_alone(linux_systemctl):
    ...
    fake = _Systemctl(
        listed=["panergos-serve.service"],
        active={"panergos-serve.service": False},
        main_pids={"panergos-serve.service": 0},
    )
    out = recovery.restart_serve_units(run=fake, sleep=lambda _: None)
...
def test_units_already_restarted_by_the_phase_are_skipped(linux_systemctl):
    fake = _Systemctl(
        listed=["panergos-serve.service", "panergos-serve-work.service"],
        active={"panergos-serve.service": True, "panergos-serve-work.service": True},
        main_pids={"panergos-serve.service": 1, "panergos-serve-work.service": 2},
    )
    out = recovery.restart_serve_units(
        skip_units=[{"scope": "user", "unit": "panergos-serve"}],
        run=fake,
        sleep=lambda _: None,
    )
    assert fake.restarted == ["panergos-serve-work.service"]
    assert out == {"verified": ["user/panergos-serve-work"], "failed": []}


def test_unrelated_panergos_server_service_is_never_touched(linux_systemctl):
    """``panergos-serve*`` is a systemd glob, not a name gate (review on #83595)."""
    fake = _Systemctl(
        listed=["panergos-server.service", "panergos-serve.service"],
        active={"panergos-server.service": True, "panergos-serve.service": True},
        main_pids={"panergos-server.service": 7, "panergos-serve.service": 8},
    )
    out = recovery.restart_serve_units(run=fake, sleep=lambda _: None)
    assert fake.restarted == ["panergos-serve.service"]
    assert out["verified"] == ["user/panergos-serve"]
...
def test_gateway_units_are_not_restarted_by_the_serve_pass(linux_systemctl):
    ...
    fake = _Systemctl(
        listed=["panergos-gateway.service"],
        active={"panergos-gateway.service": True},
        main_pids={"panergos-gateway.service": 5},
    )
    out = recovery.restart_serve_units(run=fake, sleep=lambda _: None)
    assert fake.restarted == []
    assert out == {"verified": [], "failed": []}
...
def test_same_unit_name_in_both_scopes_is_proven_in_both(linux_systemctl):
    ...
    def dual_scope(argv, **kwargs):
        ...
        if "list-units" in argv:
            return _Completed(0, stdout="panergos-serve.service loaded active running x")
        if "is-active" in argv:
    ...
    assert out == {
        "verified": ["user/panergos-serve"],
        "failed": ["system/panergos-serve"],
    }
...
def test_skip_unit_names_are_filtered_to_the_serve_family():
            ...
            json.dumps(
                {
                    ...
                    "serve_units": {
                        ...
                        "skip": [
                            "panergos-serve",
                            "panergos-serve-work.service",
                            {"scope": "system", "unit": "panergos-serve"},
                            {"scope": "user", "unit": "panergos-serve-work.service"},
                            "panergos-server",
                            "../../etc/systemd/evil",
                            "panergos-serve; rm -rf /",
                            "panergos-gateway",
                        ],
                    },
                }
    ...
    assert skip == [
        "panergos-serve",
        "panergos-serve-work",
        "system/panergos-serve",
        "user/panergos-serve-work",
    ]
...
def test_serve_only_fleet_still_spawns_the_recovery_child(monkeypatch):
            ...
            stdout=json.dumps(
                {
                    ...
                    "relaunch_attempted": [],
                    "failed": [],
                    "serve_units": {"verified": ["panergos-serve"], "failed": []},
                }
            ),
        )
    ...
    assert result["serve_units"] == {"verified": ["panergos-serve"], "failed": []}
...
def test_verified_serve_units_do_not_enter_gateway_restart_vocabulary(monkeypatch):
    ...
    assert "panergos-serve" not in result["verified"]
...
def test_already_restarted_units_are_forwarded_as_skips(monkeypatch):
    ...
    update_cmd._recover_gateway_restart_after_abort(
        _plan(_runtime("gateway", "default", "systemd", 111)),
        gateway_mode=False,
        skip_units={"user/panergos-serve"},
    )
    # The scope travels with the unit: settling `user/panergos-serve` must not
    # suppress recovery of a stale system-scope unit of the same name.
    assert captured["payload"]["serve_units"]["skip"] == [
        {"scope": "user", "unit": "panergos-serve"}
    ]
...
def test_serve_coverage_reaches_the_persisted_receipt(tmp_path, monkeypatch):
    from panergos_cli import update_receipt
    ...
    update_receipt.record_gateway_restart(
        restarted_services=[],
        incomplete=True,
        phase_error="cannot import name 'line_input' from 'panergos_cli.cli_output'",
        fresh_recovery={
            "requested": ["default"],
            "verified": ["default"],
            "relaunch_attempted": [],
            ...
            "serve_units": {
                "verified": ["user/panergos-serve"],
                "failed": ["system/panergos-serve-work"],
            },
            "stale_runtimes": [
                {
                    "pid": 4242,
...
def test_stale_serve_warning_names_the_process_and_the_fix(capsys):
    ...
    assert "4242" in out
    assert "serve" in out
    assert "systemctl --user restart panergos-serve.service" in out
...
def test_recovery_module_reports_serve_units_in_a_real_process():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "panergos_cli.update_restart_recovery", "--stdin"],
        input=json.dumps(
...
def _stub_dashboard_helpers(monkeypatch, **helpers):
    """Stub the ``panergos_cli.main_dashboard`` helpers the dashboard-cleanup path reads at call time."""
...
def _dashboard_main_stub(scan_calls, *, restart_result=True):
    return dict(
        _DASHBOARD_SYSTEMD_UNIT="panergos-dashboard.service",
...
def test_managed_dashboard_restart_still_scans_for_serve_backends(monkeypatch):
    ...
    The reporter's host runs ``panergos-dashboard.service`` AND
    ``panergos-serve.service``.  Returning as soon as the dashboard unit was
...
def test_restarted_dashboard_unit_is_not_killed_by_the_continued_scan(monkeypatch):
    ...
    _stub_dashboard_helpers(
        ...
        _get_systemd_service_for_pid=lambda pid: "panergos-dashboard.service",
...
def test_serve_backend_survives_selection_when_the_dashboard_unit_restarts(monkeypatch):
    ...
    _stub_dashboard_helpers(
        ...
        _get_pid_cgroup_path=lambda pid: "/user.slice/panergos-serve.service",
        _get_systemd_service_for_pid=lambda pid: "panergos-serve.service",
        _dashboard_cmdline_for_pid=lambda pid: None,
    ...
    assert restarted == ["panergos-serve.service"]
...
def _dual_scope_systemctl(*, user_pid=10, system_pid=20, settle=("user", "system")):
    """Both managers own a ``panergos-serve.service``; each is its own process."""
...
def test_settled_user_scope_does_not_suppress_the_stale_system_scope(
    ...
    ``user/panergos-serve`` was restarted before the phase aborted;
    ``system/panergos-serve`` is a different process still on the pre-update
    generation. A bare ``panergos-serve`` skip token suppressed BOTH, leaving
    ...
    assert out == {"verified": ["system/panergos-serve"], "failed": []}
...
def test_settled_system_scope_does_not_suppress_the_stale_user_scope(
    ...
    run, calls = _dual_scope_systemctl()
    out = recovery.restart_serve_units(
        skip_units=["system/panergos-serve"],
        run=run,
        sleep=lambda _: None,
    )
    restarted_scopes = [scope for scope, argv in calls if "restart" in argv]
...
def test_one_authorized_scope_never_mutates_the_same_name_in_the_other(
    ...
    recovery.restart_serve_units(
        skip_units=[{"scope": "user", "unit": "panergos-serve.service"}],
        run=run,
        sleep=lambda _: None,
    )
    user_verbs = [
...
def test_legacy_unqualified_skip_is_honoured_in_both_scopes(linux_systemctl):
    ...
    run, calls = _dual_scope_systemctl()
    out = recovery.restart_serve_units(
        skip_units=["panergos-serve"], run=run, sleep=lambda _: None
    )
...
def test_qualified_skip_payload_shape():
    ...
    assert update_cmd._qualified_serve_skips(
        {"user/panergos-serve", "system/panergos-serve-work"}
    ) == [
        {"scope": "system", "unit": "panergos-serve-work"},
        {"scope": "user", "unit": "panergos-serve"},
    ]
...
def test_unscoped_bookkeeping_entries_are_forwarded_without_a_scope():
    ...
    assert update_cmd._qualified_serve_skips({"panergos-serve"}) == [
        {"unit": "panergos-serve"}
    ]
