...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    from panergos_cli import status as status_mod
    ...
    monkeypatch.setattr(status_mod, "get_panergos_home", lambda: tmp_path, raising=False)
    ...
    assert "Start with:   panergos gateway" in output
...
class TestShowStatusXaiOAuth:
    """xAI OAuth row in panergos status."""
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.panergos/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.panergos/auth.json" in out
    ...
    def test_import_failure_does_not_break_other_oauth_providers(self, monkeypatch, capsys, tmp_path):
        """example/Codex/MiniMax rows must still appear when xAI import fails."""
        import panergos_cli.auth as auth_mod
        status_mod = _base_xai_mocks(monkeypatch, tmp_path)
        monkeypatch.delattr(auth_mod, "get_xai_oauth_auth_status", raising=False)
    ...
    def test_status_function_returns_none_does_not_crash(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: panergos auth add xai-oauth)" in out


def test_show_status_reports_gateway_session_last_activity(monkeypatch, capsys, tmp_path):
    """panergos status should surface freshest gateway last_active (#72016)."""
    from panergos_cli import status as status_mod
    import panergos_cli.auth as auth_mod
    import panergos_cli.gateway as gateway_mod
    ...
    monkeypatch.setattr(panergos_state, "SessionDB", _FakeDB)
