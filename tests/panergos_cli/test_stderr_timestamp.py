"""Tests for panergos_cli.stderr_timestamp."""
...
from panergos_cli import stderr_timestamp
...
_LAUNCHD_ENV = {"PATH": "/usr/bin", "XPC_SERVICE_NAME": "ai.panergos.gateway-butler"}
...
def test_main_injects_flag_into_stale_gateway_child(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "ai.panergos.gateway-butler")
    ...
    stale = [sys.executable, "-c", code, "-m", "panergos_cli.main", "gateway", "run", "--replace"]
