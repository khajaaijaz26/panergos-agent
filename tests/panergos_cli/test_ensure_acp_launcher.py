"""`panergos update` must self-heal the ACP launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) resolve the agent by the
``panergos-acp`` command name on the login-shell PATH. Fresh installs get it from
``scripts/install.sh``; existing installs get it from ``_ensure_acp_launcher()``
during ``panergos update``.
"""
...
from panergos_cli.update_cmd import _ensure_acp_launcher
...
def test_does_not_follow_symlink_into_venv(fake_home, tmp_path):
    """#21454 failure mode: never write through a symlinked panergos-acp."""
    (fake_home / "panergos").write_text("#!/bin/sh\n", encoding="utf-8")
    ...
    (fake_home / "panergos-acp").symlink_to(console_script)

    _ensure_acp_launcher()

    assert console_script.read_text(encoding="utf-8") == marker
    assert (fake_home / "panergos-acp").is_symlink()
...
def test_unwritable_bin_dir_is_skipped(fake_home):
    ...
    if not hasattr(os, "geteuid"):
        ...
        _ensure_acp_launcher()
        assert not (fake_home / "panergos-acp").exists()
        return
    if os.geteuid() == 0:
