...
from panergos_cli.observability import relay_shared_metrics as mod
...
def _config(**shared):
    if shared.get("send") is True and "endpoint" not in shared:
        shared["endpoint"] = "https://collector.example.test/v1"
...
def capture_sender(monkeypatch):
    ...
    monkeypatch.setattr(
        "panergos_cli.observability.shared_metrics_sender.SharedMetricsSender",
        FakeSender,
    )
    return record
...
def _set_config(monkeypatch, config):
    monkeypatch.setattr(
        "panergos_cli.config.read_raw_config_readonly", lambda: config, raising=False
    )
...
class TestConsentWindows:
    ...
    def test_reconcile_runs_even_when_collection_is_disabled(
        ...
        """
        from panergos_cli.observability.shared_metrics import SharedMetricsStore
        from panergos_cli.observability.shared_metrics_sender import (
        ...
        # PANERGOS_HOME: the boot reconciler probes the default path (without
        ...
        home = tmp_path / "home"
        monkeypatch.setattr(
            "panergos_constants.get_panergos_home", lambda: home
        )
        ...
        monkeypatch.setattr(
            "panergos_cli.observability.shared_metrics.SharedMetricsStore",
            lambda *a, **k: store,
        )
...
class TestFailureIsolation:
    ...
    def test_an_unreadable_config_does_not_break_export(self, runtime, monkeypatch, capture_sender):
        ...
        monkeypatch.setattr(
            "panergos_cli.config.read_raw_config_readonly", explode, raising=False
        )
        runtime._export()
        assert runtime.subscriber.store.exported == 1
        assert capture_sender["passes"] == []
