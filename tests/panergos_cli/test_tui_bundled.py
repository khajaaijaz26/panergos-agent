...
def test_tui_finds_bundled_entry_js(tmp_path):
    ...
    tui_dist = tmp_path / "panergos_cli" / "tui_dist"
    tui_dist.mkdir(parents=True)
    entry = tui_dist / "entry.js"
    entry.write_text("// bundled TUI", encoding="utf-8")

    from panergos_cli.main_tui_launch import _find_bundled_tui
    result = _find_bundled_tui(panergos_cli_dir=tmp_path / "panergos_cli")
