...
The notice prints the most recent curator run summary on `panergos update`,
...
session start) so users normally never see the rename map. `panergos update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from panergos_cli import update_cmd_maint
