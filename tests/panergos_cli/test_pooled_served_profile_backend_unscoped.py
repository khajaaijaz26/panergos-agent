"""A pooled Desktop backend (``panergos --profile X serve``, PANERGOS_HOME=<root>/profiles/X) answers its
REST without ``?profile=``. Those UNSCOPED reads/verbs are about X — a profile the live default
multiplexer serves — and must agree with the scoped ``?profile=X`` answer and with ``panergos -p X status``:
...
restart`` under X's PANERGOS_HOME that exited 78 while the UI reported success.
"""
...
def pooled_served_process(tmp_path, monkeypatch):
    """Process whose PANERGOS_HOME is a served named profile; the default home records a live multiplexer."""
    ...
    monkeypatch.setenv("PANERGOS_HOME", str(root / "profiles" / "alpha"))
...
def test_unscoped_lifecycle_verbs_in_a_served_profile_process_address_the_multiplexer(pooled_served_process):
    ...
    # The child must run under the DEFAULT home (the multiplexer's), not inherit alpha's PANERGOS_HOME.
    assert _profile_action_environment(restart)["PANERGOS_HOME"] == str(pooled_served_process)
