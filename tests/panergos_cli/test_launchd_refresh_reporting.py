...
def _stale_plist(tmp_path, monkeypatch, *, registered: bool):
    plist_path = tmp_path / "com.panergos.plist"
    ...
    monkeypatch.setattr(gw, "get_launchd_label", lambda: "com.panergos.agent")
...
def test_install_repair_warns_instead_of_claiming_success(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("panergos_constants.display_panergos_home", lambda: "~/.panergos-work")
    ...
    assert "~/.panergos-work/logs/launchd-reload.log" in out
