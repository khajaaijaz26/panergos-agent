...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "panergos_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # PANERGOS_HOME.  For a standard install (PANERGOS_HOME unset) that root is
        # the platform-native default (~/.panergos), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from panergos_constants import get_default_panergos_root
        assert env.get("PANERGOS_HOME") == str(get_default_panergos_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (PANERGOS_DESKTOP=1) must NOT
