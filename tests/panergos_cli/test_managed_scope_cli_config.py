...
panergos_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `panergos config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_panergos_home = get_panergos_home()`` at import time (module
    singleton), so monkeypatching PANERGOS_HOME after import doesn't move it.
    ...
    this only matters for tests that swap PANERGOS_HOME.)
    """
    import cli

    cli._panergos_home = home
