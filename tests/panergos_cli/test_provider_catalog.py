"""Tests for the unified provider catalog (panergos_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``panergos model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from panergos_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
