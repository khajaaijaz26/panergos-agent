"""Behavior tests for panergos_cli.inventory.
...
from panergos_cli.inventory import (
    ConfigContext,
...
def test_explicit_only_filters_ambient_credentials_but_keeps_current_and_custom_rows():
    rows = [
        ...
        {"slug": "copilot", "name": "Copilot", "models": ["gpt-5.4"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "panergos"},
        {"slug": "example", "name": "example", "models": ["anthropic/claude-sonnet-5"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "panergos"},
...
def test_explicit_only_keeps_anthropic_row_with_oauth_credentials():
    ...
    Claude Code (~/.claude/.credentials.json) and Panergos' own device flow
    ...
    with (
        ...
        patch(
            "panergos_cli.inventory._anthropic_oauth_credentials_present",
            return_value=True,
        ),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_anthropic_oauth_presence_accepts_pool_only_oauth_entry():
    ...
    """
    from panergos_cli.inventory import _anthropic_oauth_credentials_present
    ...
    with (
        ...
        patch(
            "panergos_cli.auth.read_credential_pool",
            return_value=[
...
def test_user_defined_rows_carry_alias_set_for_gui_current_match():
    ...
    rows = [
        ...
        _aggregator_row("openrouter", ["or-model"]),
    ]
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("panergos_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from panergos_cli import model_switch

    with patch("panergos_cli.models.clear_provider_models_cache") as clear:
