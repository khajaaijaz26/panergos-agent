"""Tests for the post-pull HEAD-movement gate in ``panergos update``.
...
Before this guard ``panergos update`` printed "✓ Code updated!" and
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the panergos_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns panergos_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/panergos_cli/test_cmd_update.py).
    """
    ...
    monkeypatch.setattr(
        panergos_main, "_resolve_update_branch", lambda args: "main"
    )
    ...
    monkeypatch.setattr(
        panergos_main, "_run_pre_update_backup", lambda *a, **k: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        panergos_gateway, "find_gateway_pids", lambda all_profiles=False: []
    )
    monkeypatch.setattr(
        panergos_gateway, "supports_systemd_services", lambda: False
    )
    monkeypatch.setattr(
...
def test_update_success_when_head_moves(monkeypatch, tmp_path, capsys):
    ...
    panergos_main.cmd_update(args)  # completes normally (no SystemExit)
