...
def client(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PANERGOS_DASHBOARD_SESSION_TOKEN", "soul-test-token")
...
def profile_dir(tmp_path, monkeypatch) -> Path:
    """Create a real profile directory under the test PANERGOS_HOME."""
    monkeypatch.setenv("PANERGOS_HOME", str(tmp_path))
...
class TestSoulWriteDurability:
    ...
    def test_existing_soul_survives_an_interrupted_save(
        ...
        # PANERGOS_HOME patch the client/profile_dir fixtures installed.
...
class TestSoulIoIsOffTheEventLoop:
    ...
    ``tests/panergos_cli/test_web_profiles_off_loop.py``.
    """
