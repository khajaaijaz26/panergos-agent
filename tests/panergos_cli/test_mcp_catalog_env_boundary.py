...
@pytest.fixture
def catalog_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, _isolate_panergos_home):
    ...
    monkeypatch.setenv("PANERGOS_OPTIONAL_MCPS", str(catalog))
...
def test_catalog_rejects_undeclared_key_before_any_write_or_install(
    ...
    import panergos_cli.mcp_catalog as mcp_catalog
...
def test_catalog_cannot_declare_reserved_control_key(
    ...
    response = client.post(
        "/api/mcp/catalog/install",
        headers=HEADERS,
        json={"name": "demo", "env": {"PANERGOS_YOLO_MODE": "1"}},
    )
    ...
    assert installs == []
    env_path = catalog_env / ".env"
    assert not env_path.exists() or "PANERGOS_YOLO_MODE" not in env_path.read_text(
        encoding="utf-8"
    )
...
def test_rejected_copilot_controls_do_not_change_live_resolvers(
    ...
    monkeypatch.setenv("PANERGOS_COPILOT_ACP_COMMAND", "/opt/trusted/copilot")
    monkeypatch.setenv("PANERGOS_COPILOT_ACP_ARGS", "--acp --stdio")
    expected_command = _resolve_command()
    expected_args = _resolve_args()

    attempts = {
        "PANERGOS_COPILOT_ACP_COMMAND": "/tmp/attacker-command",
        "PANERGOS_COPILOT_ACP_ARGS": "--malicious-transport",
    }
...
def test_preexisting_copilot_controls_remain_usable(
    ...
    monkeypatch.setenv("PANERGOS_COPILOT_ACP_COMMAND", "parent-placeholder")
    monkeypatch.setenv("PANERGOS_COPILOT_ACP_ARGS", "--parent-placeholder")
    (catalog_env / ".env").write_text(
        "PANERGOS_COPILOT_ACP_COMMAND=/opt/operator/copilot\n"
        "PANERGOS_COPILOT_ACP_ARGS=--acp --stdio --operator-mode\n",
        encoding="utf-8",
    )

    load_panergos_dotenv(
        panergos_home=catalog_env,
