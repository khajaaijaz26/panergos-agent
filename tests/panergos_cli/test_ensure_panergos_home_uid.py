"""Regression tests for #34107 — Docker UID/GID handling in ensure_panergos_home.

When Panergos runs in Docker with ``PANERGOS_UID=1000`` / ``PANERGOS_GID=911``,
the entrypoint chowns the top-level ``PANERGOS_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_panergos_home()`` — especially
...
The fix is a ``_chown_to_panergos_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_panergos_uid_gid
# ---------------------------------------------------------------------------


class TestResolvePanergosUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("PANERGOS_UID", "1000")
        monkeypatch.setenv("PANERGOS_GID", "911")
        from panergos_cli.config import _resolve_panergos_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_panergos_uid
# ---------------------------------------------------------------------------


class TestChownToPanergosUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_panergos_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("PANERGOS_UID", raising=False)
        monkeypatch.delenv("PANERGOS_GID", raising=False)
        from panergos_cli import config as cfg
