...
non-OpenAI provider (Anthropic, xAI, ...) the constructor value is a
...
class TestShowConfigCredentialSource:
    def test_prefers_live_agent_key(self, capsys):
        out = _run_show_config(
            _make_stand_in(cli_key="sk-proj-WRONGVENDORKEY1234", agent_key="live-REALKEY-abcdef9876"),
            capsys,
        )
        assert "live-REA" in out
