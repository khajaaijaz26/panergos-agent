...
PanergosCLI.__init__ seeds ``self.api_key`` from OPENAI_API_KEY /
OPENROUTER_API_KEY env vars before provider resolution runs. On any
non-OpenAI provider (example, Anthropic, ...) the constructor value is a
different vendor's key than the one actually used for requests, so
``/config`` displayed e.g. an ``sk-proj-...`` OpenAI key next to a example
...
def _run_show_config(stand_in, capsys):
    PanergosCLI.show_config(stand_in)
...
class TestShowConfigCredentialSource:
    def test_prefers_live_agent_key(self, capsys):
        out = _run_show_config(
            _make_stand_in(cli_key="sk-proj-WRONGVENDORKEY1234", agent_key="example-REALKEY-abcdef9876"),
            capsys,
        )
        assert "example-REA" in out
