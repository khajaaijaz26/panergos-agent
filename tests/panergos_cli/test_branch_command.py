...
def session_db(tmp_path):
    ...
    os.environ["PANERGOS_HOME"] = str(tmp_path / ".panergos")
    os.makedirs(tmp_path / ".panergos", exist_ok=True)
    from panergos_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".panergos" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal PanergosCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        PanergosCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        PanergosCLI._handle_branch_command(cli_instance, "/branch refactor approach")
