"""Tests that provider selection via `panergos model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated PANERGOS_HOME with a minimal string-format config."""
...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_yaml_write(self, config_home):
        ...
        from panergos_cli.auth import _update_config_for_provider
        ...
        def _boom(path, data, **kwargs):
            ...
            assert data["model"]["provider"] == "example"
            assert data["model"]["base_url"] == "https://inference.example.com/v1"
            assert data["model"]["default"] == "some-old-model"
            assert kwargs["sort_keys"] is False
            raise OSError("simulated atomic write failure")

        with patch("panergos_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
    ...
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        with patch("panergos_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("panergos_cli.auth.deactivate_provider"), \
...
class TestBaseUrlValidation:
    ...
    def test_empty_base_url_keeps_default(self, config_home, monkeypatch):
        ...
        with patch("panergos_cli.auth._prompt_model_selection", return_value="MiniMax-M2"), \
             patch("panergos_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestZaiEndpointPicker:
    ...
    def test_custom_proxy_rejects_invalid_url(self, config_home, monkeypatch, capsys):
        ...
        with patch("panergos_cli.main_provider_setup._prompt_provider_choice", return_value=custom_idx), \
             patch("panergos_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("panergos_cli.auth.deactivate_provider"), \
    ...
    def test_current_endpoint_is_default_choice(self, config_home, monkeypatch):
        ...
        from panergos_cli.auth import ZAI_ENDPOINTS
        from panergos_cli.model_setup_flows import _select_zai_endpoint
        ...
        with patch("panergos_cli.main_provider_setup._prompt_provider_choice", side_effect=fake_choice):
