...
Covers:
- Defense 1: gateway stop/restart refuse when _PANERGOS_GATEWAY=1
...
from panergos_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "panergos gateway uninstall",
        "panergos  gateway  restart",         # double spaces
        "PANERGOS GATEWAY RESTART",           # uppercase
    ])
    def test_panergos_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.panergos.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.panergos/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l panergos-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.panergos.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.panergos.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.panergos.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.panergos/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.panergos.gateway-apollo:/a.plist' "
            "'ai.panergos.gateway-cronus:/c.plist' 'ai.panergos.gateway-plutus:/p.plist' "
            "'ai.panergos.gateway:/Users/botuser/Library/LaunchAgents/ai.panergos.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated panergos note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.panergos.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.panergos.gateway',
        "launchctl kick'start' -k gui/501/ai.panergos.gateway",
        "launchctl kick\\start -k gui/501/ai.panergos.gateway",
        'launchctl "kickstart" -k gui/501/ai.panergos.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.panergos.gateway',
        "launchctl dis\\able gui/501/ai.panergos.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.panergos."gateway"',
        # Same class on the systemctl and panergos-CLI branches.
        'systemctl re"start" panergos-gateway',
        'panergos gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.panergos.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.panergos.update-checker',
        'systemctl re"start" panergos-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "panergos cron list",
        "panergos update",
        "panergos config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `panergos gateway start` is benign — starting a gateway from inside a
        ...
        "panergos gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway panergos
        # services must not be falsely blocked (the old `.*panergos` matched any
        # panergos token).
        "launchctl unload ai.panergos.update-checker.plist",
        "launchctl restart ai.panergos.daemon",
        ...
        "systemctl restart panergos-meta.service",
        "systemctl restart panergos-cron-helper",
        ...
        "echo after the panergos gateway restarted cleanly",
        "the panergos gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "panergos skill view gateway-notes && echo panergos gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `panergos` is a path component
        # there, not a command.
        "cat '/docs/panergos gateway restart-notes.md'",
        "less /home/user/notes/panergos gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "panergos gateway restart",
        "panergos gateway restart; echo done",
        "panergos gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;panergos gateway restart",
        "true && panergos gateway stop",
        "echo $(panergos gateway restart)",
        "echo `panergos gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.panergos.gateway"])',
        'subprocess.run(["panergos", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.panergos.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["panergos", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: panergos gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `panergos -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry PANERGOS_HOME/PANERGOS_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("PANERGOS_PROFILE", "zeus")
        monkeypatch.delenv("PANERGOS_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "panergos -p zeus gateway stop",
        "panergos -p zeus gateway restart",
        "panergos --profile zeus gateway restart",
        "panergos --profile zeus gateway stop",
        "panergos --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "panergos -v -p zeus gateway restart",
        "panergos -p zeus -v gateway restart",
        "panergos --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "panergos -p venus gateway stop",
        "panergos -p venus gateway restart",
        "panergos --profile venus gateway restart",
        "panergos --profile=venus gateway stop",
        "panergos -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "panergos -p zeus gateway start",
        "panergos -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("panergos gateway restart")
        assert _contains_gateway_lifecycle_command("panergos gateway stop")

    def test_panergos_home_derived_profile(self, monkeypatch):
        # Without PANERGOS_PROFILE the guard falls back to the PANERGOS_HOME-
        ...
        assert _contains_gateway_lifecycle_command("panergos -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("panergos -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("panergos -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("panergos gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_panergos_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade panergos then run panergos gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_panergos_gateway_once.sh). The script must live under
        # PANERGOS_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("PANERGOS_HOME", str(tmp_path / ".panergos"))
        scripts_dir = tmp_path / ".panergos" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_PANERGOS_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _PANERGOS_GATEWAY=1.

    Issue #37453: systemctl --user restart panergos-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart panergos-gateway",
        "systemctl --user restart panergos-gateway",
        "systemctl stop panergos-gateway.service",
        "panergos gateway restart",
        "panergos gateway uninstall",
        "launchctl kickstart gui/501/ai.panergos.gateway",
        "launchctl bootout gui/501/ai.panergos.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.panergos.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.panergos/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.panergos.gateway.restart-once.plist",
        "pkill -f panergos.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart panergos-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.panergos.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-panergos label: label-independent detection is the point
        # (#62891 second reproduction used `ai.panergos.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.panergos.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _PANERGOS_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _PANERGOS_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_PANERGOS_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="panergos gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["panergos gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.panergos.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.panergos.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-panergos systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run panergos gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("panergos gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfepanergos gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under PANERGOS_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("panergos gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/panergos\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 panergos gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_panergos_home()` falls back to Path.home(), which raises
        RuntimeError when neither PANERGOS_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart panergos-gateway' .",
        "grep -rn 'panergos gateway restart' ./logs",
        "rg 'panergos gateway restart' ../archive",
        "grep -c 'systemctl stop panergos-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM panergos_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell panergos gateway restart"',
        'sqlite3 ./db ".system systemctl restart panergos-gateway"',
        'psql ./x -c "\\! systemctl restart panergos-gateway"',
        "grep -r 'panergos gateway restart' . | sh",
        "grep -r 'panergos gateway restart' ./logs | bash",
        "grep -r 'panergos gateway restart' . | sudo sh",
        "grep -r 'x' . ; panergos gateway restart",
        "grep -r 'x' . && systemctl restart panergos-gateway",
        'grep -r "$(panergos gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart panergos-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell panergos gateway restart"',
        'sqlite3 db ".system systemctl restart panergos-gateway"',
        'psql -c "\\! systemctl restart panergos-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    panergos_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run panergos gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_HOME", str(tmp_path / ".panergos"))
        (tmp_path / ".panergos").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run panergos gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop panergos-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart panergos-gateway' /var/log/syslog",
        "rg 'panergos gateway restart' /home/user/.panergos/logs/",
        "journalctl -u panergos-gateway --grep 'systemctl restart panergos-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'panergos_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell panergos gateway restart"',
        'psql -c "\\! systemctl restart panergos-gateway"',
        "grep 'systemctl restart panergos-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart panergos-gateway",
        'grep "$(systemctl restart panergos-gateway)" f',
        "grep 'restart' log; systemctl restart panergos-gateway",
        'sqlite3 db "SELECT 1"; panergos gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "panergos gateway restart",
        "sudo systemctl stop panergos-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart panergos-gateway%'\""
        )
