"""``panergos update`` skips the editable reinstall when the pull can't affect it.
...
that rewrite is the only reason the running ``panergos.exe`` gets quarantined,
...
from panergos_cli.update_cmd import _editable_install_is_current
