"""Tests for ``panergos update`` / ``--check`` inside the Docker container.
...
fix, ``panergos update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Panergos, not an update to the running container, so the message
...
@patch("panergos_cli.config.is_managed", return_value=False)
@patch("panergos_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``panergos update`` inside Docker → friendly message + exit 2, no git calls.
    ...
    assert "doesn't update a running Docker container" in out
    assert "docker build -t panergos-agent:local" in out
...
def test_format_docker_update_message_contents():
    ...
    """
    from panergos_cli.config import format_docker_update_message

    msg = format_docker_update_message()

    # Primary command — the entire reason this message exists.
    assert "docker build -t panergos-agent:local" in msg
    ...
    assert "main" in msg, "must identify the Panergos source branch"
    assert "PANERGOS_HOME" in msg or "/opt/data" in msg, (
        "must address config persistence across upgrades"
    )

    # Acknowledges that forks exist (build-your-own-image escape hatch).
    assert "does not publish a prebuilt container image" in msg
