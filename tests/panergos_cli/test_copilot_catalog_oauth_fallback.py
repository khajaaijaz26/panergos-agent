...
``panergos auth add copilot`` or by ``_seed_from_env`` when the env var
is set in ``~/.panergos/.env`` — the picker was silently dropping back to
...
from panergos_cli.models import _resolve_copilot_catalog_api_key
