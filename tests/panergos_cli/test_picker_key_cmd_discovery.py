...
from panergos_cli import model_switch_providers as picker
...
def test_picker_command_auth_is_lazy_and_credential_scoped(monkeypatch, route, static):
    ...
    monkeypatch.setattr("panergos_cli.models_local.should_use_ollama_native_catalog", lambda *a, **k: False)
    ...
    def rows(entries, *, probe=True):
        b = picker._PickerBuild(
            current_provider="", current_base_url="", current_model="", max_models=None,
            for_picker=False, probe_custom_providers=probe,
...
def test_setup_probe_credentials_never_become_saved_credentials(monkeypatch, route, discover, static):
    from agent import command_token_source
    from panergos_cli import config as config_module, models
    from panergos_cli import model_setup_flows_custom as setup
    ...
    from panergos_cli.main_provider_setup import _named_custom_provider_map
