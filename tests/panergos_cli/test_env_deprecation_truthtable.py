...
session restoration / cron / the gateway):
    ...
    * There is NO quiet/non-interactive gating in the scanner itself; the
      gateway calls it before setting PANERGOS_QUIET (gateway/run.py), so the
      warning fires regardless of PANERGOS_QUIET.

Exercises the real scanner against a real temp PANERGOS_HOME .env file.
"""
...
TRUTH_TABLE = [
    ...
    pytest.param(
        "TERMINAL_CWD=.\n", {"PANERGOS_QUIET": "1"}, None, ("TERMINAL_CWD",),
        id="quiet-mode-no-gating-dotenv-warns",
    ),
    pytest.param(
        "API_KEY=x\n",
        {"PANERGOS_QUIET": "1", "TERMINAL_CWD": "."},
        None,
        (),
...
def test_deprecated_env_warning_truth_table(
    ...
    if env_file is not None:
        (panergos_home / ".env").write_bytes(env_file.encode("utf-8"))
    ...
    for key in ("TERMINAL_CWD", "MESSAGING_CWD", "PANERGOS_QUIET"):
        monkeypatch.delenv(key, raising=False)
    for key, value in proc_env.items():
        monkeypatch.setenv(key, value)

    import panergos_cli.config as config_module
