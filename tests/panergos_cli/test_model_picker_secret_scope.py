...
class TestSwitchModelKeyEnvScope:
    ...
    def _run_switch(self, monkeypatch, user_cfg):
        ...
        monkeypatch.setattr(
            "panergos_cli.runtime_provider.resolve_runtime_provider", _fake_runtime
        )
...
class TestPickerKeyEnvDotenv:
    ...
    that lives only in ``$PANERGOS_HOME/.env`` authenticates the ``/model`` verification probe, and
    ...
    def test_switch_probe_uses_dotenv_key_over_stale_process_env(self, monkeypatch, tmp_path):
        ...
        import panergos_cli.model_switch as ms
        import panergos_cli.models_validate as mv
        ...
        monkeypatch.setattr("panergos_cli.runtime_provider.resolve_runtime_provider", _fake_runtime)
        monkeypatch.setattr(ms, "resolve_alias", lambda *a, **k: None)
