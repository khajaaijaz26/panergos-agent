"""Tests for ``panergos plugins validate`` (panergos_cli/plugin_validate.py).
...
def test_requires_panergos_spec_is_validated(tmp_path):
    manifest = dict(BASE_MANIFEST, requires_panergos=">=0.21")
    d = _make_plugin(tmp_path, manifest=manifest)

    report = validate_plugin_dir(d)

    assert report.ok, report.failures
    assert ("requires_panergos", True, "spec '>=0.21' parses") in report.checks
...
class TestRequiresPanergosSpec:
    """A typo'd ``requires_panergos`` clause must fail admission, not silently gate nothing."""

    def test_typoed_clause_fails_admission(self, tmp_path):
        d = _make_plugin(
            tmp_path, manifest={**BASE_MANIFEST, "requires_panergos": ">=0.21.1,<0.x"}
        )
        report = validate_plugin_dir(d)
        assert not report.ok
        assert any(
            "requires_panergos" in f and "does not parse" in f for f in report.failures
        ), report.failures
