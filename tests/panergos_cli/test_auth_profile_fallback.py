...
When ``PANERGOS_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.panergos/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.panergos            (has its own auth.json fixture)
    * Profile     -> tmp_path/.panergos/profiles/coder   (active, PANERGOS_HOME points here)
...
def test_provider_auth_state_falls_back_to_global_when_profile_has_none(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "example": {"access_token": "example-global", "refresh_token": "rt-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))

    state = get_provider_auth_state("example")
    assert state is not None
    assert state["access_token"] == "example-global"
...
def test_provider_auth_state_returns_none_when_neither_has_it(profile_env):
    ...
    assert get_provider_auth_state("example") is None


# ---------------------------------------------------------------------------
# _load_provider_state — internal global fallback (issue #18594 follow-up)
#
# Runtime credential helpers call ``_load_provider_state`` directly with
# a profile-loaded auth store rather than going through
# ``get_provider_auth_state``. Without the fallback wired into
# ``_load_provider_state`` itself, those helpers raise ``"Panergos is not
# logged into example Portal"`` even though the user has a valid global example
...
def test_codex_cooldown_clear_writes_to_the_store_that_owns_the_borrowed_pool(profile_env):
    ...
    from panergos_cli.auth_codex import clear_codex_pool_quota_cooldowns
...
def test_root_write_through_is_visible_to_the_next_fallback_read(profile_env):
    ...
    import os
    from panergos_cli.auth import _save_auth_store, read_credential_pool
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_panergos_home_override(profile_b)
        try:
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (PANERGOS_HOME != ~/.panergos, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    panergos_home = tmp_path / "classic"
    panergos_home.mkdir()
