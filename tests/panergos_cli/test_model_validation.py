...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("panergos_cli.models.fetch_api_models", return_value=api_models), \
         patch("panergos_cli.models.probe_api_models", return_value=probe_payload):
...
class TestCuratedModelsForProvider:
    ...
    def test_live_catalog_projected_to_tuples_else_static_fallback(self):
        with patch("panergos_cli.models.provider_model_ids", return_value=["m-live"]):
            assert curated_models_for_provider("example") == [("m-live", "")]
        with patch("panergos_cli.models.provider_model_ids", return_value=[]), patch.dict(
            "panergos_cli.models._PROVIDER_MODELS", {"example": ["m-static"]}
        ):
            assert curated_models_for_provider("example") == [("m-static", "")]
...
class TestFetchApiModels:
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("panergos_cli.models._urlopen_model_catalog_request", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("panergos_cli.models._urlopen_model_catalog_request", return_value=_Resp()) as mock_urlopen:
...
class TestNormalizeOpencodeBaseUrl:
    ...
    def test_strips_v1_for_anthropic_messages(self):
        from panergos_cli.models import normalize_opencode_base_url
...
class TestValidateApiFallback:
    ...
    def test_fetch_lmstudio_models_filters_embedding_type(self):
        ...
        with patch("panergos_cli.models._urlopen_model_catalog_request", return_value=mock_resp):
    ...
    def test_validate_lmstudio_distinguishes_auth_failure(self):
        ...
        with patch("panergos_cli.models._urlopen_model_catalog_request", side_effect=http_error):
...
class TestRequestedIdIsNeverRewritten:
    ...
    def test_static_catalog_near_miss_keeps_requested_id(self):
        ...
        with patch("panergos_cli.models.provider_model_ids", return_value=codex_models):
...
class TestValidateCodex900kVariants:
    """`-900k` is a Panergos picker convention: valid variants come from the
    ...
    def test_catalog_listed_variant_accepted(self):
        with patch("panergos_cli.models.provider_model_ids", return_value=self._CATALOG):
    ...
    def test_valid_variant_missing_from_catalog_still_accepted(self):
        ...
        with patch("panergos_cli.models.provider_model_ids", return_value=["gpt-5.6-sol"]):
            result = validate_requested_model("gpt-5.6-sol-900k", "openai-codex")
        assert result["accepted"] is True
...
class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a Panergos User-Agent.
    ...
    def test_probe_sends_panergos_user_agent(self):
        ...
        assert ua.startswith("panergos-agent/"), (
            f"User-Agent must advertise panergos-agent, got {ua!r}"
        )
    ...
    def test_probe_user_agent_sent_without_api_key(self):
        ...
        assert ua and ua.startswith("panergos-agent/")
...
class TestValidateOpenRouterVariantSuffixes:
    ...
    def test_static_catalog_fallback_accepts_variant(self):
        ...
        with patch("panergos_cli.models.fetch_api_models", return_value=None), \
             patch(
...
class TestValidateCustomUnreachableFallback:
    ...
    def _validate(self, model, provider, models, **kw):
        ...
        with patch("panergos_cli.models.probe_api_models", return_value=probe):
