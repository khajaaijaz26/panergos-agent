...
def test_add_priority_places_reauthenticated_row_in_multi_entry_pool(monkeypatch):
    ...
    assert [e["label"] for e in entries] == ["reauthenticated", "account0", "account1"]
...
def test_refresh_rejects_ambiguous_and_non_oauth_targets():
    ...
    # Qwen's resolver refreshes only its singleton, never an independent pool grant.
    write_credential_pool("qwen-oauth", _rows())
    before = read_credential_pool("qwen-oauth")
    with pytest.raises(SystemExit, match="not a refreshable"):
        auth_commands.auth_refresh_command(SimpleNamespace(provider="qwen-oauth", target="row0"))
    assert read_credential_pool("qwen-oauth") == before
