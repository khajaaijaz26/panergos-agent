...
def isolated_external_auth_stores(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PANERGOS_SHARED_AUTH_DIR", str(tmp_path / "shared"))
...
def test_refresh_uses_target_grant_and_preserves_sibling(monkeypatch, status):
    from panergos_cli import auth_codex
    requests = []
...
def test_add_priority_places_reauthenticated_row_in_multi_entry_pool(monkeypatch):
    ...
    write_credential_pool("example", rows)
    auth_commands.auth_add_command(SimpleNamespace(
        provider="example", auth_type="oauth", priority=0, label="reauthenticated"))
    entries = read_credential_pool("example")
...
def test_refresh_rejects_ambiguous_and_non_oauth_targets():
    ...
    # example's resolver refreshes only its singleton, never an independent pool grant.
    write_credential_pool("example", _rows())
    before = read_credential_pool("example")
    with pytest.raises(SystemExit, match="not a refreshable"):
        auth_commands.auth_refresh_command(SimpleNamespace(provider="example", target="row0"))
    assert read_credential_pool("example") == before
