"""Tests for panergos_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate PANERGOS_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from panergos_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "example": {
                "metadata": {"display_name": "example Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from panergos_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://khajaaijaz26.github.io/panergos-agent/docs/api/model-catalog.json"
    FALLBACK = (
...
class TestDefaultModelFromCache:
    ...
    def _manifest_with_default(self) -> dict:
        ...
        m["providers"]["example"]["models"][1]["default"] = True  # kimi-k2.6
        return m
    ...
    def test_reads_label_from_disk_cache(self, isolated_home):
        ...
        with patch.object(model_catalog, "_fetch_manifest") as fetch:
            ...
            assert (
                model_catalog.get_default_model_from_cache("example")
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        """The in-repo manifest labels the same default entry as the code."""
        import panergos_cli.model_catalog as model_catalog
        from panergos_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestRefreshCadence:
    def test_default_ttl_is_twenty_minutes_and_legacy_hours_honoured(self):
        ...
        with patch("panergos_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20}}):
        ...
        with patch("panergos_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20, "ttl_hours": 3}}):
    ...
    def test_refresh_catalogs_forces_every_source(self):
        ...
        with patch.object(model_catalog, "_load_catalog_config", return_value={
            "enabled": True, "url": "http://master", "ttl_hours": 1.0, "providers": {},
             ...
             patch("panergos_cli.models.fetch_openrouter_models") as orm:
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["example"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['example'] / OPENROUTER_MODELS. "
