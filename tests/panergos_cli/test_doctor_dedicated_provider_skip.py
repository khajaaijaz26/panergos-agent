"""Regression: panergos doctor must not run a generic Bearer-auth health
...
with HTTP 404. The dedicated check at panergos_cli/doctor.py already covers
Anthropic with the right headers, so the pluggable profile must be
skipped by `_build_apikey_providers_list()`.

See: khajaaijaz26/panergos-agent#22346
"""

from __future__ import annotations
from panergos_cli import doctor_connectivity


def test_build_apikey_providers_list_skips_dedicated_check_providers():
    from panergos_cli import doctor
