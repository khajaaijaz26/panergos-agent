...
from panergos_cli.main, these tests mock the delegation point and verify
...
from panergos_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside panergos setup must survive the wizard's final save."""
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("panergos_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("panergos_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("panergos_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    This drives the local browser readiness checks used by the summary.
    """
    monkeypatch.setenv("PANERGOS_HOME", str(tmp_path))
    _clear_provider_env(monkeypatch)
