"""`panergos sessions repair --check-only` must fail (non-zero) when the store is unhealthy.
...
from panergos_cli import sessions_cmd
...
def test_check_only_exit_status_tracks_probe_verdict(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(panergos_state, "DEFAULT_DB_PATH", db_path)
