...
def test_overrides_roundtrip_and_clear(panergos_home):
    from panergos_cli.local_runtime.growth import (
...
def test_corrupt_overrides_read_as_empty(panergos_home):
    from panergos_cli.local_runtime.growth import (
        load_window_overrides,
...
def test_growth_declines_foreign_endpoints(panergos_home):
...
def test_occupancy_confirmed_skips_gate_one():
    ...
    from panergos_cli.local_runtime.context_policy import growth_decision
    from panergos_cli.local_runtime.estimator import (
        HardwareBudget,
        LayerKind,
        ModelProfile,
    )
    ...
    # Panergos' threshold (e.g. 80% of window) can sit BELOW the ladder's 85%
...
def _tiny_profile(model_id: str):
    from panergos_cli.local_runtime.estimator import LayerKind, ModelProfile
...
def test_preset_generation_for_catalog_model_with_mmproj(panergos_home, tmp_path, monkeypatch):
    ...
    import panergos_cli.local_runtime.presets as presets_mod

    from panergos_cli.local_runtime.catalog import CATALOG
    from panergos_cli.local_runtime.estimator import HardwareBudget
...
def test_preset_restores_grown_window_capped_at_native(panergos_home, tmp_path, monkeypatch):
    ...
    from panergos_cli.local_runtime.estimator import HardwareBudget
    from panergos_cli.local_runtime.growth import save_window_override
...
def test_preset_ignores_override_below_launch_window(panergos_home, tmp_path, monkeypatch):
...
def test_preset_restores_grown_window_midladder(panergos_home, tmp_path, monkeypatch):
    """The real growth shape: launch at a lower rung, override to a middle
    rung -> the preset window follows the override."""
    import panergos_cli.local_runtime.presets as presets_mod

    from panergos_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile
    from panergos_cli.local_runtime.growth import save_window_override
...
def test_mtp_plan_matches_cost_at_initial_and_restored_windows(panergos_home, tmp_path, monkeypatch):
    from dataclasses import replace
    from types import SimpleNamespace

    from panergos_cli.local_runtime import presets
    from panergos_cli.local_runtime.context_policy import FLOOR, RUNTIME_OVERHEAD_BYTES, ub_logits_bytes
    from panergos_cli.local_runtime.estimator import HardwareBudget, LayerKind, ModelProfile, ctx_bytes
    from panergos_cli.local_runtime.growth import save_window_override
...
def test_growth_requires_an_admissible_materialized_preset(panergos_home, tmp_path, monkeypatch):
    from dataclasses import replace
    from types import SimpleNamespace

    from panergos_cli.local_runtime import bootstrap, catalog, growth, hardware, presets
    from panergos_cli.local_runtime.context_policy import FLOOR, RUNTIME_OVERHEAD_BYTES, ub_logits_bytes
    from panergos_cli.local_runtime.estimator import HardwareBudget, ctx_bytes
    ...
    from panergos_cli.local_runtime import gguf, estimator
    ...
    from panergos_cli.local_runtime.binaries import runtimes_root
...
def test_sampling_ladder_file_beats_catalog_beats_nothing(panergos_home, tmp_path, monkeypatch):
