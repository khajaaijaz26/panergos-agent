"""Tests for panergos_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $PANERGOS_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Panergos-managed node dir.
"""
...
def _make_panergos_node(panergos_home: Path) -> Path:
    """Create a fake $PANERGOS_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = panergos_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    panergos_home = fake_home / ".panergos"
    _make_panergos_node(panergos_home)
    ...
    removed = uninstall.remove_node_symlinks(panergos_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    panergos_home = fake_home / ".panergos"
    node_bin = _make_panergos_node(panergos_home)
