"""Tests for the stale-dashboard handling run at the end of ``panergos update``.

``panergos update`` detects ``panergos dashboard`` processes left over from the
...
from panergos_cli.main_dashboard import _find_stale_dashboard_pids
from panergos_cli.dashboard_procs import _kill_stale_dashboard_processes
from panergos_cli import dashboard_procs
from panergos_cli import main_dashboard
from panergos_cli import update_cmd
from panergos_cli.update_cmd import _finish_dashboard_update_cleanup
from panergos_cli.main_dashboard import _restart_managed_dashboard_service
from panergos_cli.dashboard_procs import _kill_stale_dashboard_processes as _warn_stale_dashboard_processes
...
def _write_valid_ssh_backend_lock(tmp_path, monkeypatch) -> int:
    ...
    (lock_dir / "backend.lock.json").write_text(json.dumps({
        ...
        "port": 46369,
        "profile": "default",
        "panergosPath": "/opt/panergos/bin/panergos",
        "panergosHome": str(tmp_path),
    ...
    monkeypatch.delenv("PANERGOS_DESKTOP_CHILD_PID", raising=False)
    return pid
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "panergos_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m panergos_cli.main dashboard"),
                    _ps_line(12345, "panergos dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "panergos-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="panergos-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "panergos-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "panergos-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "panergos-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("panergos_cli.main_dashboard._find_stale_dashboard_pids", return_value=[]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "panergos-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "panergos-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "panergos-dashboard.service"],
            ["systemctl", "--user", "restart", "panergos-dashboard.service"],
        ]
        ...
        assert "✓ restarted panergos-dashboard.service" in capsys.readouterr().out
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "panergos_cli.main._kill_stale_dashboard_processes",
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
            ...
            mock_probe.return_value = subprocess.CompletedProcess(
                ...
                returncode=0,
                stdout=(
                    "CommandLine=python -m panergos_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
...
class TestSupervisedBackendRestart:
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(main_dashboard, "_get_pid_cgroup_path",
                          return_value="/system.slice/panergos-serve.service"), \
             patch.object(main_dashboard, "_get_systemd_service_for_pid",
                          return_value="panergos-serve.service"), \
        ...
        restart.assert_called_once_with(
            "panergos-serve.service", "/system.slice/panergos-serve.service"
        )
        out = capsys.readouterr().out
        assert "✓ restarted systemd service panergos-serve.service" in out
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: panergos update's systemd fleet-restart loop may
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"panergos-serve"}
            )
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        live = self._live()
        argv = ["panergos", "dashboard", "--port", "8300"]
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("panergos_cli.dashboard_procs._panergos_home_for_pid", return_value=None), \
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["panergos", "dashboard", "--port", "8300"],
                ["panergos", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["panergos", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["panergos", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["panergos", "serve"]])

        assert failed == [["panergos", "serve"]]
        out = capsys.readouterr().out
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from panergos_cli.dashboard_procs import _filter_dashboard_respawn_candidates
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.panergos/profiles/mini-cat"),
        ]) == []
    ...
    def test_keeps_ppid1_fixed_port_for_repeat_update(self):
        ...
        argv = ["panergos", "dashboard", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([(10, argv, None)]) == [argv]

    def test_dedupes_identical_normalized_cmdlines(self):
        from panergos_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        a = ["/usr/bin/python3", "-m", "panergos_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "panergos_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["panergos", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["panergos", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["panergos", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_panergos_home(self):
        from panergos_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        home = "/tmp/panergos-home-a"
        a = ["panergos", "dashboard", "--port", "8300"]
        b = ["panergos", "dashboard", "--port", "8301"]
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, home),
                (2, b, home),
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.panergos/profiles/coder"),
        ])
        assert out == [a]
    ...
    def test_default_profile_same_root_home_caps(self):
        ...
        a = ["panergos", "--profile", "default", "dashboard", "--port", "8300"]
        b = ["panergos", "dashboard", "--port", "8301"]
        home = "/home/u/.panergos"
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, home),
                (2, b, home),
    ...
    def test_foreign_home_backend_is_not_replayed(self):
        """A backend from another PANERGOS_HOME is never respawned (#94030).
        ...
        out = _filter_dashboard_respawn_candidates(
            [
                (1, a, "/home/u/.panergos"),
                (2, b, "/work/project/.panergos"),
            ],
            own_home="/home/u/.panergos",
        )
        assert out == [a]
    ...
    def test_skips_sidecar_fixed_port_serve_on_foreign_home(self):
        ...
        argv = [
            "/Users/u/.panergos-sidecar/panergos-agent/venv/bin/python",
            "-m", "panergos_cli.main",
            "serve", "--host", "127.0.0.1", "--port", "9118", "--skip-build",
        ]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.panergos-lifeos")],
            own_home="/Users/u/.panergos",
        )
        assert out == []

    def test_matching_panergos_home_is_kept(self):
        from panergos_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        argv = ["panergos", "serve", "--host", "127.0.0.1", "--port", "9118"]
        out = _filter_dashboard_respawn_candidates(
            [(15364, argv, "/Users/u/.panergos")],
            own_home="/Users/u/.panergos",
        )
        assert out == [argv]

    def test_symlinked_panergos_home_compares_equal(self, tmp_path):
        from panergos_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_unknown_home_stays_eligible(self):
        """Unreadable PANERGOS_HOME (env probe failed) keeps pre-#94030 behaviour."""
        from panergos_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_own_home_defaults_to_get_panergos_home(self, monkeypatch):
        ...
        monkeypatch.setattr(
            panergos_constants, "get_panergos_home", lambda: Path("/home/u/.panergos")
        )
        argv = ["panergos", "serve", "--port", "9118"]
        foreign = _filter_dashboard_respawn_candidates([
            (1, argv, "/Users/u/.panergos-lifeos"),
        ])
        assert foreign == []
        own = _filter_dashboard_respawn_candidates([
            (1, argv, "/home/u/.panergos"),
        ])
        assert own == [argv]
    ...
    def test_keeps_fixed_port_serve(self):
        ...
        argv = ["panergos", "serve", "--host", "0.0.0.0", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([
            (9, argv, None),
        ]) == [argv]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00panergos_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "panergos_cli.main", "serve"]
...
class TestPostUpdateStaleModuleReload:
    ...
    ``panergos update`` runs in the PRE-pull Python process. When the update
    adds a new symbol to ``panergos_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from panergos_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_node_failures_skip_reload_and_kill(self):
        ...
        with patch.object(update_cmd, "_reload_process_scan_modules") as mock_reload, \
             patch("panergos_cli.main._kill_stale_dashboard_processes") as mock_kill:
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import panergos_cli._subprocess_compat as compat
        from panergos_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["panergos_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["panergos_cli._subprocess_compat"])
            importlib.reload(sys.modules["panergos_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "panergos_cli._subprocess_compat" in reloaded
        assert "panergos_cli.dashboard_procs" in reloaded
