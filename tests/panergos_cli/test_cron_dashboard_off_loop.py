"""Regression test: cron dashboard handlers must not run profile I/O on the event loop.

Guards POST /api/cron/blueprints/instantiate
(``_call_cron_for_profile`` → ``create_job``).
