...
A Windows git install runs `panergos update` from `panergos.exe`, which reexecs a
...
def test_current_checkout_repair_rebuilds_desktop_under_project_root():
    ...
    with (
        ...
        m.return_value.PROJECT_ROOT = update_cmd.Path("/fake/panergos")
    ...
    assert rebuild.call_args[0][0] == update_cmd.Path("/fake/panergos/apps/desktop")
...
def test_handoff_venv_repair_finishes_node_and_web_phase(tmp_path):
    ...
    project_root = tmp_path / "panergos"
