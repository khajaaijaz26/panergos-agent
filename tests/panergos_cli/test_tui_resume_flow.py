...
def main_mod(monkeypatch):
    import panergos_cli.main as mod
...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_panergos_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import panergos_cli.oneshot as oneshot
        from panergos_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """panergos -z bypasses PanergosCLI, but recall still needs SessionDB."""
    from panergos_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "panergos_state_registry", mod("panergos_state_registry", acquire=fake_acquire))
    monkeypatch.setitem(
        sys.modules,
        "panergos_cli.config",
        mod("panergos_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "panergos_cli.models",
        mod("panergos_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        ...
        mod("panergos_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["PANERGOS_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="example/panergos-test", provider="example", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["PANERGOS_MODEL"] == "example/panergos-test"
    assert env["PANERGOS_INFERENCE_MODEL"] == "example/panergos-test"
    assert env["PANERGOS_TUI_PROVIDER"] == "example"
    assert env["PANERGOS_INFERENCE_PROVIDER"] == "example"
    assert env["PANERGOS_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["PANERGOS_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("panergos-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_panergos_ink(monkeypatch, main_mod, tmp_path):
    tui_dir = tmp_path / "ui-tui"
    tsx = tui_dir / "node_modules" / ".bin" / "tsx"
    ink_dir = tui_dir / "packages" / "panergos-ink"
    ...
    monkeypatch.delenv("PANERGOS_TUI_DIR", raising=False)
