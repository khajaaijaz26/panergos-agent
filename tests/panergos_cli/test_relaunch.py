"""Tests for panergos_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from panergos_cli import relaunch as relaunch_mod


class TestResolvePanergosBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/panergos"
        ...
        assert relaunch_mod.resolve_panergos_bin() == fake

    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        fake = tmp_path / "panergos"
        fake.write_text("#!/bin/sh\n")
        fake.chmod(0o755)
        monkeypatch.setattr(sys, "argv", [str(fake.name)])
        monkeypatch.chdir(tmp_path)
        # Ensure we don't accidentally match a real 'panergos' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_panergos_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/panergos" if name == "panergos" else None
        )
        assert relaunch_mod.resolve_panergos_bin() == "/usr/bin/panergos"
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_panergos_bin", lambda: "/usr/bin/panergos")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/panergos"
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/panergos", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/panergos", ["/usr/bin/panergos", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        panergos).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_panergos_bin", lambda: r"C:\Users\test\panergos.exe")
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\panergos.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\panergos.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_panergos_bin", lambda: r"C:\panergos.exe")
...
class TestResolvePanergosBinWindowsPyGuard:
    """On Windows, resolve_panergos_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a panergos.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\panergos.exe" if name == "panergos" else None,
        )

        bin_path = relaunch_mod.resolve_panergos_bin()
        # Must NOT be the .py — must be the panergos.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\panergos.exe"
    ...
    def test_posix_python_launcher_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        The git installer invokes ``panergos`` with the venv interpreter while the
        ...
        script = tmp_path / "panergos"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        wrapper = tmp_path / "bin" / "panergos"
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: str(wrapper) if name == "panergos" else None,
        )
        assert relaunch_mod.resolve_panergos_bin() == str(wrapper)
    ...
    def test_posix_python_launcher_on_path_falls_back_to_current_python(
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil,
            "which",
            lambda name: str(script) if name == "panergos" else None,
        )

        assert relaunch_mod.resolve_panergos_bin() is None

        # A console script pinned to the running interpreter keeps the venv: still exec-able.
        pinned = tmp_path / "pinned" / "panergos"
        pinned.parent.mkdir()
        pinned.write_text(f"#!{sys.executable}\n")
        pinned.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(pinned), "chat"])
        assert relaunch_mod.resolve_panergos_bin() == str(pinned)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_panergos_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND panergos.exe
