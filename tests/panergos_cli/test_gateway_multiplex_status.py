...
``panergos gateway status`` / ``gateway list`` / ``profile list`` keyed liveness
...
def _fake_multiplexer(monkeypatch, tmp_path, *, multiplex: bool, pid_file: bool = True):
    ...
    (tmp_path / "gateway_state.json").write_text(json.dumps({
        "pid": os.getpid(), "kind": "panergos-gateway", "gateway_state": "running",
        "start_time": status._get_process_start_time(os.getpid()), "panergos_home": str(tmp_path),
    }))
    ...
    monkeypatch.setattr(
        status, "_read_process_cmdline", lambda pid: "python -m panergos_cli.main gateway run --replace"
    )
