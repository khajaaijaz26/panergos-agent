...
1. `panergos secrets bitwarden setup --help` works (dispatch path)
2. `panergos update --check` works (update path)
3. `panergos secrets bitwarden disable` works (handler execution)
4. `panergos secrets onepassword status` works (lazy backend loads on demand)
...
def _run_panergos(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run panergos CLI as a subprocess from repo root.
    ...
    connectivity — but ``panergos update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-panergos-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-panergos-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_1": "git@",
        }
    )
    return subprocess.run(
        [sys.executable, "-m", "panergos_cli.main"] + args,
        capture_output=True,
        text=True,
        cwd=str(repo_root),
        env=env,
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`panergos secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_panergos(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`panergos secrets bitwarden disable` must exit 0."""
        result = _run_panergos(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`panergos secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_panergos(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`panergos secrets onepassword setup --help` must exit 0."""
        result = _run_panergos(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `panergos update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`panergos update --check` must not load cryptography._rust."""
        result = _run_panergos(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['panergos','update','--check']
...
sys.argv = ['panergos', 'update', '--check']

import panergos_cli.main as m
...
# resolves the panergos_cli.main module's lazily re-exported attribute —
# so the patch must land on update_cmd._cmd_update_check.
with patch('panergos_cli.update_cmd._cmd_update_check', capture_update_check):
    try:
        m.main()
