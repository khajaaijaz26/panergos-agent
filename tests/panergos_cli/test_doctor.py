...
import panergos_cli.doctor as doctor
import panergos_constants
from panergos_cli import config as config_mod
import panergos_cli.gateway as gateway_cli
from panergos_cli import doctor as doctor_mod
from panergos_cli.doctor_config import _has_provider_env_config
import shutil
from panergos_cli import doctor_tools
from panergos_cli import doctor_state
from panergos_cli import doctor_platform
from panergos_cli import doctor_config
...
class TestDoctorPlatformHints:
    def test_termux_package_hint(self, monkeypatch):
        ...
        assert panergos_constants.is_termux() is True
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker build -t panergos-agent:local" in hint
        assert "recreate all Panergos containers" in hint
        assert "panergos update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor_platform._sqlite_upgrade_hint("git")

        assert "run `panergos update`" in hint

    def test_sqlite_upgrade_hint_uses_pkg_for_apt_managed_install(self):
        hint = doctor_platform._sqlite_upgrade_hint("apt")

        assert "run `pkg upgrade panergos-agent`" in hint
        assert "panergos update" not in hint

    def test_sqlite_upgrade_hint_preserves_nix_guidance_as_prose(self):
        from panergos_cli.config import recommended_update_command_for_method
...
class TestDoctorToolAvailabilitySummary:
    ...
    def test_image_gen_without_provider_reports_setup_hint_not_system_dependency(self, monkeypatch):
        """image_gen declares no single env var (FAL / managed example / plugin providers); an
        unconfigured backend is a setup problem and must say so, and it counts toward the
        'run panergos setup' summary like any missing key (#9516)."""
        ...
        assert "panergos tools" in image_line and "system dependency" not in image_line and "unavailable" in image_line
        assert "system dependency not met" in next(line for line in out.splitlines() if "homeassistant" in line)
        assert any("panergos setup" in issue for issue in f.issues)
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `panergos doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "PANERGOS_HOME", panergos_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("PANERGOS_KANBAN_TASK", "probe")
...
def test_doctor_reports_vercel_backend_diagnostics(monkeypatch, tmp_path):
    monkeypatch.setattr(
        doctor_mod,
        "DOCTOR_CHECKS",
        ((None, doctor_mod._check_terminal_backend),),
    )
...
class TestDoctorMemoryProviderSection:
    ...
    def _run_doctor_and_capture(
        ...
        *,
        ...
        home = self._make_panergos_home(tmp_path, provider, memory_config)
        ...
        monkeypatch.setattr(doctor_mod, "PANERGOS_HOME", home)
        ...
        try:
            from panergos_cli import auth as _auth_mod
...
def _limit_doctor_to_config_check(monkeypatch):
    """Keep provider-validation tests isolated from external doctor probes."""
    monkeypatch.setattr(
        doctor_mod,
        "DOCTOR_CHECKS",
        ((None, doctor_mod._check_config_file),),
    )


def test_run_doctor_accepts_named_provider_from_providers_section(monkeypatch, tmp_path):
    _limit_doctor_to_config_check(monkeypatch)
    home = tmp_path / ".panergos"
    home.mkdir(parents=True, exist_ok=True)
...
def test_run_doctor_accepts_panergos_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (panergos_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".panergos"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`panergos doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp PANERGOS_HOME.

        ``run_doctor`` reads the module-level ``PANERGOS_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("PANERGOS_HOME")`` alone
        leaves doctor probing the REAL ~/.panergos. On a dev machine with a
    ...
    def test_no_token_and_not_gh_authenticated_shows_warn(self, monkeypatch, tmp_path):
        ...
        from panergos_cli.doctor import run_doctor
        import io, contextlib
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from panergos_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("panergos_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "panergos_cli.doctor", raising=False)
        from panergos_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorXaiOAuthStatus:
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        monkeypatch.setattr(
            doctor_mod,
            "DOCTOR_CHECKS",
            (("Auth Providers", doctor_mod._check_auth_providers),),
        )
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale PANERGOS_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"PANERGOS_MAX_ITERATIONS={ghost}\n")
        (panergos_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "PANERGOS_HOME", panergos_home)
        monkeypatch.setattr(doctor_mod, "get_panergos_home", lambda: panergos_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("PANERGOS_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("PANERGOS_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale PANERGOS_MAX_ITERATIONS" in out
        env_after = (panergos_home / ".env").read_text(encoding="utf-8")
        assert "PANERGOS_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_panergos_tool_progress_warning_says_unsupported_since_floor(self):
        """PANERGOS_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_config.collect_deprecated_env_vars({"PANERGOS_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["PANERGOS_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_config.collect_deprecated_env_vars({"PANERGOS_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["PANERGOS_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        panergos_home = tmp_path / ".panergos"
        panergos_home.mkdir(parents=True)
        (panergos_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (panergos_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_config.report_deprecated_config_and_env(
            ...
            {"PANERGOS_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: PANERGOS_TOOL_PROGRESS_MODE" in out
...
class TestMacOSTCCGrants:
    ...
    def test_warns_on_cdhash_pinned_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.khajaaijaz26.panergos" and cdhash H"97e692f3890f781fa0ad5ad6cb9d769cfaf42628"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC grants will reset after every update" in out
        assert "cdhash-pinned" in out
        assert "panergos update" in out
    ...
    def test_ok_and_repair_info_on_identifier_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.khajaaijaz26.panergos"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        ...
        assert "tccutil reset ScreenCapture com.khajaaijaz26.panergos" in out
        assert "toggle" in out
        assert "relaunch" in out

    def test_ok_on_certificate_anchored_dr(self, monkeypatch, capsys, tmp_path):
        """A cert-anchored DR (panergos desktop --setup-tcc-identity, or a
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.khajaaijaz26.panergos" and certificate root = H"aabbcc"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC signing identity is stable" in out
