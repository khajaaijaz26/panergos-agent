...
Profile-aware location: ``$PANERGOS_HOME/logs/dashboard-auth.log``.
...
from panergos_cli.dashboard_auth.audit import audit_log, AuditEvent


@pytest.fixture
def profile_home(tmp_path, monkeypatch):
    """Redirect $PANERGOS_HOME and ~ to a tmp dir for the duration of the test."""
...
def test_audit_writes_jsonlines(profile_home):
    audit_log(AuditEvent.LOGIN_START, provider="example", ip="1.2.3.4")
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="example", user_id="u1",
...
def test_audit_redacts_token_like_fields(profile_home):
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="example", access_token="should-not-appear",
