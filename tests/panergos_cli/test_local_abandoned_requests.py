...
def managed_state(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("panergos_cli.local_runtime.supervisor.state_path",
                        lambda: state)
...
def test_remote_providers_unaffected(managed_state):
    assert aux._provider_requires_stream("example",
                                         "https://inference-api.exampleorg.com/v1/") is False
...
def test_reap_orphans_kills_only_our_parentless_binaries(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(__import__("sys").modules, "psutil", fake_psutil)
    monkeypatch.setattr("panergos_cli.local_runtime.supervisor.server_binary",
