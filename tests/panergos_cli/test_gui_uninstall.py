"""Tests for panergos_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import panergos_cli.gui_uninstall as gu


def _make_agent(panergos_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = panergos_home / "panergos-agent"
    (agent_root / "panergos_cli").mkdir(parents=True)
    (agent_root / "panergos_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(panergos_home: Path) -> None:
    """Create the source-built GUI artifacts a `panergos desktop` run produces."""
    desktop = panergos_home / "panergos-agent" / "apps" / "desktop"
    ...
    (panergos_home / "panergos-agent" / "node_modules").mkdir(parents=True)
    (panergos_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(panergos_home: Path) -> None:
    (panergos_home / "config.yaml").write_text("x: 1\n")
    (panergos_home / ".env").write_text("KEY=secret\n")
    (panergos_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(panergos_home)
    _make_gui_build(panergos_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(panergos_home)
    ...
    assert summary["panergos_home"] == str(panergos_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `panergos desktop` installs is removable."""
...
def test_windows_discovery_uses_current_name(tmp_path, monkeypatch):
    monkeypatch.setattr(gu.sys, "platform", "win32")
    monkeypatch.setenv("APPDATA", str(tmp_path / "roaming"))
    monkeypatch.setenv("LOCALAPPDATA", str(tmp_path / "local"))
    monkeypatch.delenv("ProgramFiles", raising=False)

    assert [path.name for path in gu._desktop_userdata_dirs()] == ["Panergos"]
    assert {path.name for path in gu.packaged_gui_app_paths()} >= {"Panergos"}
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    panergos_home = tmp_path / ".panergos"
    _make_agent(panergos_home)
    icon = lde.icon_path(panergos_home / "panergos-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(panergos_home)
    ...
    assert lde.icon_path(panergos_home / "panergos-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (panergos_home / "panergos-agent" / "panergos_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".panergos")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import panergos_cli.uninstall as uninstall
