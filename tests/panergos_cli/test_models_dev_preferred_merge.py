...
These guard the contract:
  ...
  * OpenRouter and example Portal are NEVER merged — they keep their curated
    (OpenRouter) or live-Portal (example) semantics.
...
appear in ``/model`` without a Panergos release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_k3_live_discovery_is_scoped_to_kimi_coding_endpoint(self):
        ...
        with patch("panergos_cli.urllib_security.open_credentialed_url", side_effect=fake_open):
            with patch(
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from panergos_cli.model_setup_flows import _model_flow_kimi
        ...
        with (
            patch("panergos_cli.main_provider_setup._prompt_api_key", return_value=("sk-kimi-test", False)),
            patch("panergos_cli.auth._prompt_model_selection", side_effect=fake_select),
            patch("panergos_cli.config.get_env_value", return_value=""),
