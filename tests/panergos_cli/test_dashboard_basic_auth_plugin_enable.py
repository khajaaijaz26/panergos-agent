...
from panergos_cli.dashboard_auth import clear_providers, list_providers
from panergos_cli.plugins import PluginManager, discover_plugins
...
class TestBasicProviderLoadsAfterUnblock:
    def test_disabled_basic_blocks_registration(self, panergos_home, monkeypatch):
    ...
    def test_unblock_then_rediscover_registers_provider(
        self, panergos_home, monkeypatch,
    ):
        password_hash = basic_plugin.hash_password("hunter2")
        cfg = {
            "dashboard": {
        ...
        _write_config(panergos_home, cfg)
