"""Regression for #41403 — ``panergos update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.panergos.gateway-<profile>`` services kept running pre-update
...
class TestLaunchdGatewayLabelsForInstall:
    def test_labels_derive_from_this_installs_profiles(self, monkeypatch):
        ...
        the shared per-user LaunchAgents dir. A sandboxed PANERGOS_HOME (tests,
        ...
        assert launchd_gateway_labels_for_install() == [
            "ai.panergos.gateway",
            "ai.panergos.gateway-merit-ops",
            "ai.panergos.gateway-tfl-wiki",
        ]
...
class TestLocateLaunchdGatewayService:
    def test_domains_resolve_per_label_not_from_cache(self, monkeypatch):
        ...
        gui_loaded = {"ai.panergos.gateway-a"}
        ...
        assert _locate_launchd_gateway_service("ai.panergos.gateway-a") == (
            f"gui/{UID}",
            4242,
        )
        assert _locate_launchd_gateway_service("ai.panergos.gateway-b") == (
            f"user/{UID}",
            4242,
        )
    ...
    def test_loaded_without_live_process(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.panergos.gateway-x") == (
            f"gui/{UID}",
            None,
        )
    ...
    def test_not_loaded_in_either_domain(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.panergos.gateway-x") == (None, None)
    ...
    def test_timeout_propagates_to_caller(self, monkeypatch):
        ...
        with pytest.raises(subprocess.TimeoutExpired):
            _locate_launchd_gateway_service("ai.panergos.gateway-x")
...
class TestProbeLaunchdDomainForLabel:
    def test_unloaded_label_falls_back_to_managername(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.panergos.gateway-x") == f"gui/{UID}"
    ...
    def test_unloaded_label_defaults_to_user_domain(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.panergos.gateway-x") == f"user/{UID}"
...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        monkeypatch.setattr(gw, "get_launchd_label", lambda: "ai.panergos.gateway")
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
            lambda: ["ai.panergos.gateway", "ai.panergos.gateway-a", "ai.panergos.gateway-b"],
        )
        located = {
            "ai.panergos.gateway": (f"gui/{UID}", 100),
            "ai.panergos.gateway-a": (f"gui/{UID}", 200),
            "ai.panergos.gateway-b": (None, None),  # not bootstrapped
        }
        monkeypatch.setattr(
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through panergos_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.panergos.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.panergos.gateway", current, "ai.panergos.gateway-user-scoped"],
            located={
                "ai.panergos.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.panergos.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.panergos.gateway",
            f"user/{UID}/ai.panergos.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.panergos.gateway",
            f"user/{UID}/ai.panergos.gateway-user-scoped",
        ...
        assert restarted == [
            current,
            "ai.panergos.gateway",
            "ai.panergos.gateway-user-scoped",
        ]
        assert failed == []
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.panergos.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.panergos.gateway-a"],
            located={"ai.panergos.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.panergos.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.panergos.gateway",
            labels=["ai.panergos.gateway", "ai.panergos.gateway-idle"],
            located={
                "ai.panergos.gateway": (f"gui/{UID}", 100),
                "ai.panergos.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.panergos.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.panergos.gateway",
            labels=["ai.panergos.gateway", "ai.panergos.gateway-dormant"],
            located={
                "ai.panergos.gateway": (f"gui/{UID}", 100),
                "ai.panergos.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.panergos.gateway-dormant"]
        assert restarted == ["ai.panergos.gateway", "ai.panergos.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.panergos.gateway",
            labels=["ai.panergos.gateway", "ai.panergos.gateway-a"],
            located={
                "ai.panergos.gateway": (f"gui/{UID}", 100),
                "ai.panergos.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.panergos.gateway-a"]
        assert restarted == ["ai.panergos.gateway", "ai.panergos.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.panergos.gateway",
                "ai.panergos.gateway-bad",
                "ai.panergos.gateway-good",
            ],
            located={
                "ai.panergos.gateway": (f"gui/{UID}", 100),
                "ai.panergos.gateway-bad": (f"gui/{UID}", 200),
                "ai.panergos.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.panergos.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.panergos.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.panergos.gateway-good"]
        assert restarted == ["ai.panergos.gateway", "ai.panergos.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.panergos.gateway",
                "ai.panergos.gateway-wedged",
                "ai.panergos.gateway-after",
            ],
            located={
                "ai.panergos.gateway": (f"gui/{UID}", 100),
                "ai.panergos.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.panergos.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.panergos.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.panergos.gateway-after"]
        assert restarted == ["ai.panergos.gateway", "ai.panergos.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.panergos.gateway": (f"gui/{UID}", 100),
                "ai.panergos.gateway-wedged": (f"gui/{UID}", 200),
                "ai.panergos.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.panergos.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.panergos.gateway",
            labels=["ai.panergos.gateway", "ai.panergos.gateway-zombie"],
            located={
                "ai.panergos.gateway": (f"gui/{UID}", 100),
                "ai.panergos.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.panergos.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.panergos.gateway"]
        assert failed == ["ai.panergos.gateway-zombie"]
...
class TestWaitForLaunchdServicePid:
    def test_returns_true_once_pid_changes(self, monkeypatch):
        ...
        assert gw._wait_for_launchd_service_pid(
            "ai.panergos.gateway-x", old_pid=200, timeout=5.0, domain=f"gui/{UID}"
        )
