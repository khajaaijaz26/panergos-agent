"""Tests for `panergos fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate PANERGOS_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".panergos" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from panergos_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_returns_copies_not_aliases(self):
        from panergos_cli.fallback_cmd import _read_chain
        cfg = {"fallback_providers": [{"provider": "example", "model": "foo"}]}
        result = _read_chain(cfg)
        result[0]["provider"] = "mutated"
        assert cfg["fallback_providers"][0]["provider"] == "example"
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from panergos_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        from panergos_cli.fallback_cmd import cmd_fallback_list
        cmd_fallback_list(types.SimpleNamespace())
        out = capsys.readouterr().out
        assert "No fallback providers configured" in out
        assert "panergos fallback add" in out
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("panergos_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("panergos_cli.main._require_tty"):
            from panergos_cli.fallback_cmd import cmd_fallback_add
    ...
    def test_restore_preserves_absent_active_provider(self):
        ...
        from panergos_cli import auth, fallback_cmd
    ...
    def test_picker_failure_restores_persisted_primary_without_masking_error(
        ...
        from panergos_cli import fallback_cmd
        ...
        with patch(
            "panergos_cli.main.select_provider_and_model",
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "example / example-model")
        with patch("panergos_cli.setup._curses_prompt_choice", return_value=1):
            from panergos_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from panergos_cli.fallback_cmd import cmd_fallback_clear
...
class TestDispatcher:
    ...
    def test_unknown_subcommand_exits(self, isolated_home):
        _write_config(isolated_home, {})
        from panergos_cli.fallback_cmd import cmd_fallback
...
class TestArgparseWiring:
    """Verify `panergos fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "panergos_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
