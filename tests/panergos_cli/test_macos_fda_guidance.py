...
reports the state and prints the one-switch setup; `panergos setup` surfaces
...
import pathlib
from panergos_cli.setup_quick import _print_macos_fda_tip
...
class TestSetupFdaTip:
    def test_silent_on_non_macos(self, monkeypatch):
        import panergos_cli.setup as setup_mod
    ...
    def test_tip_printed_when_denied(self, monkeypatch, tmp_path):
        ...
        assert "Full Disk Access" in out
        assert "Privacy_AllFiles" in out
        assert "survives every Panergos update" in out
