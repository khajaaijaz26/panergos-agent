"""Parity guard for the slash-command dispatch table in cli.PanergosCLI.
...
def test_every_old_branch_resolves_to_a_handler():
    for name in OLD_CHAIN_COMMANDS:
        entry = PanergosCLI._slash_handler(name)
        assert entry is not None, name
        method_name, pass_arg = entry
        assert callable(getattr(PanergosCLI, method_name)), name
        assert isinstance(pass_arg, bool)
    # explicit table entries are only the ones the naming convention can't cover
    for name, (method_name, pass_arg) in PanergosCLI._SLASH_DISPATCH.items():
...
def test_registry_names_resolve_into_the_table():
    from panergos_cli.commands import COMMAND_REGISTRY, resolve_command

    for name in PanergosCLI._SLASH_DISPATCH:
        cmd = resolve_command(name)
        assert cmd is not None and PanergosCLI._slash_handler(cmd.name) is not None, name
    # registry commands the CLI never handled inline must still fall through
    dispatched = {c.name for c in COMMAND_REGISTRY if PanergosCLI._slash_handler(c.name)}
    assert dispatched == set(OLD_CHAIN_COMMANDS) - {"exit"}


def _cli():
    c = PanergosCLI.__new__(PanergosCLI)
...
def test_dispatch_return_semantics_and_side_effects():
    c = _cli()
    with patch.object(PanergosCLI, "_toggle_yolo", return_value=None) as m, \
            patch("panergos_cli.plugins.fire_pre_command_hook") as hook:
    ...
    c = _cli()
    with patch.object(PanergosCLI, "_handle_resume_command") as m:
    ...
    c = _cli()
    with patch.object(PanergosCLI, "_handle_handoff_command", return_value=False):
        assert c.process_command("/handoff telegram") is False
    with patch.object(PanergosCLI, "_handle_handoff_command", return_value=True):
        assert c.process_command("/handoff telegram") is True
    with patch.object(PanergosCLI, "_handle_update_command", return_value=True):
        assert c.process_command("/update") is False
    with patch.object(PanergosCLI, "_handle_update_command", return_value=False):
...
def test_unknown_command_falls_through():
    ...
    with patch.object(PanergosCLI, "_process_unregistered_slash", return_value=True) as m:
