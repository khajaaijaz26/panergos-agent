"""Tests for ``panergos doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from panergos_cli import doctor_live
from panergos_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `panergos doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("panergos_cli.doctor.PANERGOS_HOME", tmp_path / "home")
        monkeypatch.setattr("panergos_cli.doctor.PROJECT_ROOT", tmp_path / "root")
    ...
    def test_false_on_termux_local_bare_npx(self, monkeypatch, tmp_path):
        ...
        advertise as ready — must not diverge from dep_ensure/EXAMPLE_subscription's
        same carve-out."""
        self._block_path_and_node_modules_checks(monkeypatch, tmp_path)
