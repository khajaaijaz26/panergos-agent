"""Tests for panergos_cli.cron command handling."""
...
from panergos_cli import cron as cron_cli
...
class TestCronCommandLifecycle:
    ...
    def test_edit_persists_user_owned_inference_pins(self, tmp_cron_dir, capsys):
        ...
        assert updated["provider"] == "example"
...
class TestUnverifiedDeliveryVisibility:
    ...
    state must be visible in ``panergos cron list`` and ``panergos cron doctor``,
...
class TestCronListStatusRendering:
    ...
    def test_delivery_failed_is_not_green_ok(self, tmp_cron_dir, capsys, monkeypatch):
        monkeypatch.setattr("panergos_cli.gateway.find_gateway_pids", lambda: [1])
        # capsys is not a tty, so force colors on to check the paint itself.
        monkeypatch.setattr("panergos_cli.colors.should_use_color", lambda: True)
        create_job(prompt="Daily digest", schedule="every 1h")
        jobs = load_jobs()
...
class TestGatewayNotRunningWarning:
    ...
    def test_list_warns_when_gateway_absent(self, tmp_cron_dir, capsys, monkeypatch):
        ...
        monkeypatch.setattr("panergos_cli.gateway.find_gateway_pids", lambda: [])
        cron_command(Namespace(cron_command="list", all=True))
        out = capsys.readouterr().out
...
class TestExternalCronProviderStatus:
    """With an external cron provider, jobs fire outside the in-process
    ticker. The ticker-heartbeat /
    ...
    external scheduler (the heartbeat is intentionally never written).
    """

    def test_status_reports_provider_not_ticker_for_external_scheduler(
        self, tmp_cron_dir, capsys, monkeypatch
    ):
        create_job(prompt="Ping", schedule="every 2m")
        monkeypatch.setattr(
            "panergos_cli.cron._active_cron_provider_name", lambda: "external"
        )
        # Even with NO gateway process and NO ticker heartbeat, external status
    ...
    def test_create_silent_for_external_scheduler_even_without_gateway(
        self, tmp_cron_dir, capsys, monkeypatch
    ):
...
class TestCronRunBackgroundDispatch:
    """`panergos cron run` must not report 'failed' when the run was dispatched
