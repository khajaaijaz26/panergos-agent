...
the SAME provider universe as ``panergos model`` (the CLI/TUI picker).
...
def test_every_panergos_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `panergos model` universe."""
    ...
    assert not missing, (
        "providers shown in `panergos model` but not configurable in the desktop "
