...
`panergos update` migrated only the active profile's config.yaml; sibling
...
context-local PANERGOS_HOME override.
...
import panergos_cli.update_cmd_config as update_cmd_config
...
def _write_profile(root: Path, name: str, version: int) -> Path:
    ...
    (home / "config.yaml").write_text(
        yaml.safe_dump({"_config_version": version, "model": {"provider": "example"}}),
        encoding="utf-8",
    )
    return home
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    ...
    monkeypatch.setattr(
        panergos_constants, "get_process_panergos_home", lambda: active_home
    )
    monkeypatch.setattr(
...
def test_sibling_behind_is_migrated_on_disk(monkeypatch, tmp_path):
    ...
    assert on_disk["model"]["provider"] == "example"
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_panergos_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_panergos_home_override() == before
