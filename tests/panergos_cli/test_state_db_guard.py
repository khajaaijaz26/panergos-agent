...
from panergos_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    PANERGOS_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def panergos_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("panergos_cli.config") or mod == "panergos_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, panergos_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, panergos_home, capsys, monkeypatch):
