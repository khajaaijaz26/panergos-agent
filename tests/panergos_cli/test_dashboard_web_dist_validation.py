"""Regression tests: `panergos dashboard` validates PANERGOS_WEB_DIST before serving.

A custom PANERGOS_WEB_DIST without --skip-build previously skipped BOTH the
...
@pytest.fixture()
def main_mod():
    import panergos_cli.main as main
    return main
...
def _wire_common(main_mod, monkeypatch):
    monkeypatch.setattr(
        "panergos_cli.profiles.get_active_profile_name", lambda: "default"
    )
    ...
    monkeypatch.setattr(
        "panergos_cli.mcp_startup.start_background_mcp_discovery",
        lambda **_k: None,
    )


def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """PANERGOS_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("PANERGOS_WEB_DIST", str(empty_dist))
    ...
    builds = []
    monkeypatch.setattr(
        "panergos_cli.main_web_build._build_web_ui", lambda *a, **k: builds.append(a) or True
    )
    ...
    assert "PANERGOS_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("PANERGOS_WEB_DIST", raising=False)
    project_root = tmp_path / "proj"
    dist = project_root / "panergos_cli" / "web_dist"
    ...
    monkeypatch.setattr("panergos_cli.main_web_build._build_web_ui", fake_build)
