"""Tests for _purge_stale_panergos_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `panergos update`.

Field failure (2026-08-20, Teknium's Linux box): `panergos update` pulled a
checkout where panergos_cli/gateway.py newly imports `line_input` from
panergos_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from panergos_cli.gateway
...
the Panergos package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Panergos modules from the cache; later
...
def test_purge_evicts_panergos_prefixed_modules():
    victims = [
        "panergos_cli.cli_output",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_panergos_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_panergos_modules()
    assert sys.modules.get("panergos_cli.update_cmd") is update_cmd
    assert sys.modules.get("panergos_cli.main") is cli_main
    assert "panergos_cli" in sys.modules
...
def test_purge_preserves_active_update_receipt(tmp_path, monkeypatch):
    ...
    import panergos_cli.update_receipt as receipt
    ...
    try:
        ...
        cli_main._purge_stale_panergos_modules()
        post_purge_receipt = importlib.import_module("panergos_cli.update_receipt")
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["panergos_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_panergos_modules()
    finally:
        sys.modules.pop("panergos_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from panergos_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
...
def test_purge_keeps_plan_record_class_identity():
    ...
    from panergos_cli.update_inventory import RuntimeRecord as before

    cli_main._purge_stale_panergos_modules()
    from panergos_cli.update_inventory import RuntimeRecord as after
