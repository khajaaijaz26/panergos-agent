...
managers acquire the same path through ``panergos_state_registry`` from the REPL thread a
...
import panergos_cli.goals as goals
...
def test_cli_session_store_is_the_registry_handle_goals_reuse(monkeypatch):
    ...
    constructed = []
    real_open = panergos_state_registry._open_session_db

    def recording_open(path):
        constructed.append(path)
        return real_open(path)

    monkeypatch.setattr(panergos_state_registry, "_open_session_db", recording_open)

    cli = SimpleNamespace()
    try:
        PanergosCLI._init_session_store(cli)
        ...
        panergos_state_registry.close_all()
