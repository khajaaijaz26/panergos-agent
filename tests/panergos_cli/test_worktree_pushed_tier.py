...
class TestFetchRemoteBranchHeads:
    def test_lists_pushed_branches(self, repo_with_bare_origin):
        ...
        _mk_worktree(repo, "panergos-a", "panergos/panergos-a", push=True)
        heads = worktree_ops._fetch_remote_branch_heads(str(repo))
        assert heads is not None
        assert "main" in heads
        assert "panergos/panergos-a" in heads
...
class TestBranchPushedExact:
    def test_exact_match_true(self, repo_with_bare_origin):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "panergos-x", "panergos/panergos-x", push=True)
        heads = worktree_ops._fetch_remote_branch_heads(str(repo))
    ...
    def test_local_ahead_of_push_false(self, repo_with_bare_origin):
        ...
        repo = repo_with_bare_origin
        wt = _mk_worktree(
            repo, "panergos-y", "panergos/panergos-y",
    ...
    def test_never_pushed_false(self, repo_with_bare_origin):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "panergos-z", "panergos/panergos-z", push=False)
        heads = worktree_ops._fetch_remote_branch_heads(str(repo))
        assert worktree_ops._worktree_branch_pushed_exact(str(wt), heads) is False
    ...
    def test_none_heads_false(self, repo_with_bare_origin):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "panergos-n", "panergos/panergos-n", push=True)
...
class TestStartupPrunerPushedTier:
    ...
    def test_pushed_lane_tree_reaped_branch_kept(self, repo_with_bare_origin):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "panergos-pushed", "panergos/panergos-pushed", push=True)
        ...
        assert _branch_exists(repo, "panergos/panergos-pushed"), (
    ...
    def test_unpushed_lane_still_preserved(self, repo_with_bare_origin):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "panergos-unpushed", "panergos/panergos-unp", push=False)
    ...
    def test_ahead_of_pushed_lane_preserved(self, repo_with_bare_origin):
        ...
        repo = repo_with_bare_origin
        wt = _mk_worktree(
            repo, "panergos-ahead", "panergos/panergos-ahead",
            push=True, extra_commit_after_push=True,
        )
    ...
    def test_pushed_but_dirty_preserved(self, repo_with_bare_origin):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "panergos-pdirty", "panergos/panergos-pdirty", push=True)
    ...
    def test_offline_remote_preserves(self, repo_with_bare_origin, tmp_path):
        import cli
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "panergos-off", "panergos/panergos-off", push=True)
...
class TestCronWorktreeMaintenance:
    ...
    def test_maintenance_prunes_via_real_pruner(self, repo_with_bare_origin, monkeypatch):
        ...
        repo = repo_with_bare_origin
        wt = _mk_worktree(repo, "panergos-cronreap", "panergos/panergos-cronreap", push=True)
        ...
        assert _branch_exists(repo, "panergos/panergos-cronreap")
