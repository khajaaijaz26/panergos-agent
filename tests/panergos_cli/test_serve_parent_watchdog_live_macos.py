...
calls ``os._exit(0)`` ~5ms after ``PANERGOS_BACKEND_READY``, which no in-process
...
def test_live_backend_survives_timezone_drifted_parent_marker(tmp_path):
    ...
    try:
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1",
            PANERGOS_PARENT_PID=str(parent.pid),
            PANERGOS_PARENT_START_MARKER=drifted_marker,
            PANERGOS_PARENT_NONCE="nonce-95693",
            PANERGOS_SERVE_WATCHDOG_POLL_S="0.5",
        )
        env.pop("PANERGOS_DESKTOP", None)
        code = (
        ...
        assert _read_until(serve, "PANERGOS_BACKEND_READY"), "backend never announced READY"
