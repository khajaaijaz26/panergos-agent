...
def _uncached_sidebar_endpoints(monkeypatch):
    ...
    from panergos_cli.web_routers import profiles as profiles_routes

    monkeypatch.setattr(profiles_routes, "_SIDEBAR_CACHE_TTL_SECONDS", 0.0)


@pytest.fixture
def profiles_on_disk(tmp_path, monkeypatch, _isolate_panergos_home):
...
def client(monkeypatch, profiles_on_disk):
    ...
    from panergos_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
    from panergos_constants import get_panergos_home
...
def _seed_project(home, name, folder):
    from panergos_cli import projects_db
...
class TestCrossProfileProjectTree:
    ...
    def test_each_profile_contributes_its_own_projects_db(self, client, profiles_on_disk, tmp_path):
        ...
        filters through ``get_panergos_home()``. If the fan-out failed to rebind
    ...
    def test_a_profile_that_cannot_be_read_does_not_sink_the_rest(
        ...
        def explode_for_worker(db, **kwargs):
            ...
            if get_panergos_home().name == "worker":
