...
def test_format_banner_version_label_on_upstream_main():
    from panergos_cli import banner
...
def test_get_git_banner_state_reads_origin_and_head(tmp_path):
    ...
    results = {
        ("git", "rev-parse", "--short=8", f"origin/{__distribution_default_branch__}"): MagicMock(
            returncode=0, stdout="b2f477a3\n"),
        ("git", "rev-parse", "--short=8", "HEAD"): MagicMock(returncode=0, stdout="af8aad31\n"),
        ("git", "rev-list", "--count", f"origin/{__distribution_default_branch__}..HEAD"): MagicMock(
    ...
    with patch("panergos_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_via_local_git_ssh_fastpath_ahead_not_behind(tmp_path):
    ...
    nudges the user into `panergos update`, which can wipe the carried work.
    """
    ...
    def fake_git_stdout(args, *, cwd, timeout=5, network=False):
        if args == ["remote", "get-url", "origin"]:
            return "git@github.com:khajaaijaz26/panergos-agent.git"
...
def test_check_via_local_git_insteadof_rewrite_routes_to_ssh_fastpath(tmp_path, monkeypatch):
    ...
    setup_cmds = [
        ...
        ["git", "remote", "add", "origin", "git@github.com:khajaaijaz26/panergos-agent.git"],
        ["git", "rev-parse", "HEAD"],
    ]
    head_sha = None
