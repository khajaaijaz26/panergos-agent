"""The conftest WAL gate must agree with panergos_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``panergos_state``. That is deliberate: importing
``panergos_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.panergos`` before the per-test ``PANERGOS_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_panergos_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and panergos_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_panergos_state_at_collection():
    """The gate must stay import-free of panergos_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.panergos, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``panergos_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "panergos_state" or name.startswith("panergos_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported panergos_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.panergos during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("panergos_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["panergos_state"] = saved
    assert not blocked
