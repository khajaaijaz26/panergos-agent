...
The defect: panergos_state already knows macOS `fsync()` does not guarantee
...
from panergos_state import repair_state_db_schema
from panergos_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in panergos_state_repair; panergos_state only
    # re-imports them.
    import panergos_state_repair

    source = Path(panergos_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(panergos_state_repair.__file__))
