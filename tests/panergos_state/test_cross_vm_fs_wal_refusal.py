...
from panergos_state_wal import WalUnsupportedError, _detect_cross_vm_fs, apply_wal_with_fallback
...
class TestDetectCrossVmFs:
    @pytest.mark.parametrize("path,expected", [
        ...
        ("/home/user/.panergos", False),  # ext4 root
    ...
    def test_ordinary_filesystems_never_flagged(self, tmp_path, fstype):
        ...
        assert _detect_cross_vm_fs("/home/user/.panergos", mountinfo_path=mi) is False
...
class TestWalRefusalOnCrossVmFs:
    ...
    def _isolate(self, monkeypatch):
        ...
        monkeypatch.setattr(panergos_state_wal, "is_sqlite_wal_reset_vulnerable", lambda *a, **k: False)
        monkeypatch.setattr(panergos_state_wal, "resolve_journal_mode", lambda: "wal")
        panergos_state_wal._cross_vm_warned_paths.clear()

    def test_fresh_db_on_cross_vm_fs_gets_delete_and_without_detection_gets_wal(self, tmp_path, monkeypatch):
        monkeypatch.setattr(panergos_state_wal, "_path_on_cross_vm_fs", lambda p: True)
        ...
        monkeypatch.setattr(panergos_state_wal, "_path_on_cross_vm_fs", lambda p: False)
