...
Panergos instances: a main gateway (user ``ubuntu``,
PANERGOS_HOME=/home/ubuntu/.panergos) and a demo gateway (user ``demo``,
PANERGOS_HOME=/home/demo/.panergos).  The demo gateway runs as another user, so
its ``/proc/<pid>/fd`` table is unreadable from the main instance and the
holder scan falls back to ``/proc/<pid>/cmdline`` + ``_looks_like_panergos``.
The demo process's argv matches the Panergos patterns exactly, so the fallback
...
rebuild in ``panergos_state_schema._recover_stale_fts`` was deferred 42 times
...
panergos); a genuine second instance with a DIFFERENT PANERGOS_HOME is still
misjudged on current main (issue #92401).

Behavior contract: an uninspectable holder identified only by argv must be
counted unless its own argv proves it is scoped to a *different* Panergos
...
# token lives under /home/demo/.panergos, the binary name matches the Panergos
# patterns, and nothing references the main instance's home or state.db.
DEMO_HOME_ARGV = [
    "/home/demo/.panergos/panergos-agent/panergos",
    "gateway",
    "run",
]

# Same, spelled through the venv interpreter + panergos launcher script.
DEMO_VENV_ARGV = [
    "/home/demo/.panergos/panergos-agent/venv/bin/python",
    "/home/demo/.panergos/panergos-agent/panergos_cli/main.py",
    "gateway",
]

# A Panergos-shaped argv with no absolute paths: cannot disprove that this
# process touches our state.db, so it must stay fail-closed.
AMBIGUOUS_ARGV = ["panergos", "gateway", "run"]
...
class TestUninspectableHolderInstanceScope:
    def test_other_instance_argv_is_not_a_holder_of_our_db(self, tmp_path, monkeypatch):
        ...
        DIFFERENT Panergos home → not a holder of our state.db."""
    ...
    def test_argv_referencing_our_db_stays_flagged(self, tmp_path, monkeypatch):
        ...
        for argv in (
            ["panergos", f"--db={db_path}", "gateway"],
            ["panergos", "checkpoint", f"{db_path}-wal"],
            ["panergos", "--home", our_home, "gateway"],
        ):
            assert panergos_state_holders._looks_like_panergos(argv) or argv[0] == "panergos"
            _install_fake_proc(monkeypatch, tmp_path, unreadable_pids=(222,))
