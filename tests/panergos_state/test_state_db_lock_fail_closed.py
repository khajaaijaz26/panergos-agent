...
work on a file several Panergos processes share (gateway service, the Desktop
app's `panergos serve` backend, CLI sessions, the TUI slash worker):

* `panergos_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `panergos_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
