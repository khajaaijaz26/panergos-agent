"""Tests for state.db health/stats collection (panergos doctor section).

Covers:
- ``panergos_state_dbfile.collect_state_db_stats``: read-only, best-effort stats
  (page_count, freelist, WAL size, journal mode, row counts, FTS presence,
  pending v23 FTS-rebuild bookkeeping).
- ``panergos_state_dbfile.count_db_holders``: /proc-based best-effort probe for how
  many processes hold the DB file open (Linux only; None elsewhere/on error).
- ``panergos_cli.doctor_state._render_state_db_stats``: formatting/threshold helper
...
import panergos_cli.doctor_state
...
def test_render_warns_on_large_db():
    from panergos_cli.doctor_state import STATE_DB_SIZE_WARN_BYTES, _render_state_db_stats
