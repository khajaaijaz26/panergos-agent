...
def test_publish_marks_evidence_upload_failure_in_pr_comment(tmp_path, monkeypatch):
    comment = {
        "id": 123,
        "body": "before\n<!-- panergos-e2e-evidence:start -->\npending\n<!-- panergos-e2e-evidence:end -->\nafter",
    }
    updates = []
    ...
    assert updates == [
        (
            "https://api.github.com/repos/khajaaijaz26/panergos-agent/issues/comments/123",
            "github-token",
            "PATCH",
            {
...
def test_find_review_comment_requires_the_evidence_marker():
    pending = "<!-- panergos-ci-review-bot -->\n<!-- panergos-e2e-evidence:start -->\npending\n<!-- panergos-e2e-evidence:end -->"

    assert _mod._find_review_comment([{"body": "<!-- panergos-ci-review-bot --> no evidence"}]) is None
