...
Uses a throwaway PANERGOS_HOME so it never touches ~/.panergos. No real names.
"""
...
def _fresh_home():
    """Point PANERGOS_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_panergos_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["PANERGOS_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER PANERGOS_HOME is set.
...
# a different get_panergos_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"PANERGOS_HOME (throwaway): {HOME}")
