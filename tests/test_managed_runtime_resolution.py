"""Guard: Panergos-owned subprocesses must not resolve managed runtimes by bare PATH.

Panergos installs runtimes for itself — ``uv`` at ``$PANERGOS_HOME/bin/uv``, Node at
``$PANERGOS_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Panergos's own
...
* when a system copy also exists, the one Panergos does not own wins — which is
...
``iter_panergos_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Panergos provisions into PANERGOS_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Panergos-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("panergos_cli/update_cmd_deps.py", "uv"): (
    ...
    ("panergos_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (panergos_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Panergos-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$PANERGOS_HOME/bin (uv) and $PANERGOS_HOME/node are not on an "
        ...
        "  node/npm -> panergos_constants.find_node_executable()\n"
        "  PATH env -> panergos_constants.iter_panergos_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(panergos_constants, helper))


def test_managed_uv_helpers_exist():
    from panergos_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
