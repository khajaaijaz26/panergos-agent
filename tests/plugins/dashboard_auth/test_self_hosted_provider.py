...
Covers the self-hosted provider contract:
...
_ISSUER = "https://auth.example.com/application/o/panergos"
_CLIENT_ID = "panergos-dashboard"
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://panergos.example/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://panergos.example/auth/callback"
    ...
    def test_state_in_cookie_matches_url(self, provider):
        ...
        parsed = urllib.parse.urlparse(result.redirect_url)
        params = dict(urllib.parse.parse_qsl(parsed.query))
        pkce = result.cookie_payload["panergos_session_pkce"]
...
class TestVerifySession:
    ...
    def test_jwks_client_sends_explicit_http_headers(self):
        from panergos_cli import __distribution_user_agent__
        ...
        client_cls.assert_called_once_with(
            ...
            headers={
                ...
                "User-Agent": __distribution_user_agent__,
            },
        )
...
class TestPluginRegister:
    ...
    def clear_env(self, monkeypatch):
        for var in (
            "PANERGOS_DASHBOARD_OIDC_ISSUER",
            "PANERGOS_DASHBOARD_OIDC_CLIENT_ID",
            "PANERGOS_DASHBOARD_OIDC_SCOPES",
            "PANERGOS_DASHBOARD_OIDC_CLIENT_SECRET",
        ):
    ...
    def test_skips_when_unconfigured(self, patch_config):
        ...
        assert "PANERGOS_DASHBOARD_OIDC_ISSUER" in oidc_plugin.LAST_SKIP_REASON
        assert "self_hosted" in oidc_plugin.LAST_SKIP_REASON


    def test_registers_from_env(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv("PANERGOS_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("PANERGOS_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
    ...
    def test_config_load_failure_falls_through(self, monkeypatch):
        ...
        monkeypatch.setattr("panergos_cli.config.load_config", _broken)
        ctx = MagicMock()
    ...
    def test_secret_from_env(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("PANERGOS_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        monkeypatch.setenv("PANERGOS_DASHBOARD_OIDC_CLIENT_SECRET", "env-secret")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_empty_env_secret_does_not_shadow_config(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_DASHBOARD_OIDC_CLIENT_SECRET", "")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
