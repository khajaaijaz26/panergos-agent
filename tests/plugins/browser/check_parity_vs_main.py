...
Run from the PR worktree:
    ...
    cd ~/.panergos/panergos-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.panergos/panergos-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.panergos/panergos-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a panergos-agent checkout"
)
...
# Isolated PANERGOS_HOME for the config write.
