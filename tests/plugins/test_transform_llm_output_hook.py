...
from panergos_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(panergos_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <panergos_home>/plugins/<name> and opt it in."""
    plugin_dir = panergos_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PANERGOS_HOME", str(tmp_path / "panergos_empty"))
