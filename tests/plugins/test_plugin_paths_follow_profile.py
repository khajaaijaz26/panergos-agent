"""Plugin data paths follow the active profile's PANERGOS_HOME, including the ContextVar override.

Several plugins carried a ``~/.panergos`` fallback (guarding an ImportError of ``panergos_constants``
...
def _openviking_log(home):
    ...
    return ov.get_panergos_home() / ov._OPENVIKING_SERVER_LOG_RELATIVE_PATH
