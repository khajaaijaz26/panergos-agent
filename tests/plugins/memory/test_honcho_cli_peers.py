"""Regression tests for #76414: `panergos honcho peers` showed "(not set)"
for every non-default profile.

_all_profile_host_configs() built the per-profile host key inline as
f"{HOST}.{profile}" ("panergos.work") while every other reader/writer —
profile_host_key(), resolve_active_host(), honcho status/enable/sync and
the runtime plugin — uses the underscore form ("panergos_work"). The lookup
...
a real honcho.json (temp PANERGOS_HOME, no network).
"""
import io
import json
import sys
...
def honcho_home(tmp_path, monkeypatch):
    cfg = {
        "peerName": "alice",
        "hosts": {
            "panergos": {"peerName": "alice", "aiPeer": "panergos"},
            "panergos_work": {"peerName": "alice", "aiPeer": "panergos"},
            "panergos_my_profile": {"peerName": "bob", "aiPeer": "panergos"},
        },
    }
...
class TestAllProfileHostConfigs:
    def test_profile_host_keys_match_writer_form(self, honcho_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "panergos_cli.profiles.list_profiles",
        ...
        assert host == "panergos_work"  # not "panergos.work"
    ...
    def test_sanitized_profile_names_resolve(self, honcho_home, monkeypatch):
        ...
        resolve — profile_host_key maps 'my.profile' -> 'panergos_my_profile';
    ...
    def test_legacy_dot_form_host_key_still_readable(self, honcho_home, monkeypatch):
        ...
        ("panergos.work") must keep working — the README promises those keys
        ...
        del cfg["hosts"]["panergos_work"]
        cfg["hosts"]["panergos.work"] = {"peerName": "carol", "aiPeer": "panergos"}
        path.write_text(json.dumps(cfg))
        monkeypatch.setattr(
            "panergos_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
...
class TestCmdPeers:
    def test_peers_shows_populated_identity_not_host_key_leak(
        ...
        raw malformed key 'panergos.work' (or '(not set)')."""
        monkeypatch.setattr(
            "panergos_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
        )
        out = _peers_output(SimpleNamespace())
        assert "panergos.work" not in out
        ...
        assert "alice" in work_line and "panergos" in work_line
    ...
    def test_peers_falls_back_cleanly_when_block_missing(
        ...
        out = _peers_output(SimpleNamespace())
        assert "panergos.new" not in out  # well-formed key, no dot-form leak
