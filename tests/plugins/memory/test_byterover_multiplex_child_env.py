...
def _served_turn(prof_home: Path, scope: dict):
    ...
    home_tok = set_panergos_home_override(str(prof_home))
...
def test_secondary_profile_child_uses_its_own_key_not_defaults(two_profiles):
    ...
    assert env["PANERGOS_HOME"] == str(prof_b)
    assert "PANERGOS_MODEL" not in env  # launch profile's .env residue is stripped too
