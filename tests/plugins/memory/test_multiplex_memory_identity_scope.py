...
OPENVIKING_* / HINDSIGHT_BANK_ID / PANERGOS_HONCHO_HOST, the provider must fall back to its own default
...
_DEFAULT_ENV = {
    ...
    "PANERGOS_HONCHO_HOST": "host-default", "HONCHO_BASE_URL": "https://honcho.default",
...
def test_secondary_profile_memory_identity_never_inherits_default_environ(secondary_profile):
    ...
    assert (cfg["agent_id"], cfg["host"], cfg["mode"]) == ("panergos", "", "platform")
    ...
    provider.initialize("s1", panergos_home=str(secondary_profile))
    assert provider._client.project == "panergos-b"
    ...
    assert (hcfg["banks"]["panergos"]["bankId"], hcfg["mode"]) == ("panergos", "cloud")
