#!/usr/bin/env python3
"""Tests for the OpenRouter-compatible image gen provider (OpenRouter + example)."""
...
_RUNTIME = "panergos_cli.runtime_provider.resolve_runtime_provider"
...
class TestProviderClass:
    def test_names(self):
        ...
        assert names == {"openrouter", "example"}
    ...
    def test_display_names(self):
        ...
        assert by_name["example"].display_name == "example Portal"
...
class TestGenerate:
    ...
    def test_posts_to_resolved_base_url(self):
        """example routes to its own base URL — proves the same code serves both."""
        EXAMPLE_runtime = _runtime_ok(
            provider="example", base_url="https://inference.exampleorg.com/v1", api_key="example-tok"
        )
        with patch(_RUNTIME, return_value=EXAMPLE_runtime), \
             patch("requests.post", return_value=_mock_chat_response([_PNG_DATA_URI])) as mock_post, \
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            from plugins.image_gen.openrouter import _build_providers

            example = {p.name: p for p in _build_providers()}["example"]
            result = example.generate(prompt="a pet")

        assert result["success"] is True
        assert result["provider"] == "example"
        url = mock_post.call_args[0][0]
        assert url == "https://inference.exampleorg.com/v1/chat/completions"
...
class TestImageApiSurface:
    ...
    def test_catalog_models_are_offered_only_by_openrouter(self):
        ...
        EXAMPLE_ids = {m["id"] for m in by_name["example"].list_models()}
        assert "openai/gpt-image-2" in openrouter_ids
        assert set(_IMAGE_API_MODELS) <= openrouter_ids
        assert not (set(_IMAGE_API_MODELS) & EXAMPLE_ids)
...
class TestRegistration:
    def test_register_both(self):
        ...
        assert set(registered) == {"openrouter", "example"}
    ...
    def test_both_are_reference_capable_for_pets(self):
        ...
        assert "example" in _REF_CAPABLE
