...
(``tests/cli`` beside ``tests/panergos_cli``, ``tests/run_agent`` beside
``tests/agent``, ``tests/state`` beside ``tests/panergos_state``) and ~250
...
_NON_MIRROR_DIRS = {
    ...
    "security", "skills", "verify", "website", "computer_use", "panergos_state",
}
...
def test_every_test_directory_mirrors_a_source_directory_or_is_declared():
    ...
    assert not offenders, (
        ...
        "tests/panergos_cli, ...) or, for a genuinely cross-cutting suite, add the "
