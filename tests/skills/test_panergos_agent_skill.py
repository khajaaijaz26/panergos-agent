...
def test_the_index_is_published_where_the_skill_says_it_is(skill_text):
    ...
    assert "github.com/khajaaijaz26/panergos-agent" not in index
...
def test_public_identity_and_routes_are_panergos_first(skill_text):
    ...
    assert "github.com/khajaaijaz26/panergos-agent" not in skill_text
    assert not re.search(r'terminal\(command="panergos\b', skill_text)


def test_legacy_skill_and_storage_names_remain_compatible(skill_text):
...
def test_routed_public_instructions_are_panergos_first():
    legacy_cli_instruction = re.compile(
        r"(?m)(?:^|[\s`$>])panergos\s+(?=(?:--?[\w-]+|[a-z][\w-]*))"
    )
    ...
    for path in PANERGOS_ROUTED_FILES:
        ...
        assert "github.com/khajaaijaz26/panergos-agent" not in text, path
        assert not re.search(r"\bPanergos(?:['’]s)?\b", text), path
        assert not legacy_cli_instruction.search(text), path

    skin = (SKILL_DIR / "templates" / "skin.yaml").read_text(encoding="utf-8")
    assert "agent_name: Panergos Agent" in skin


def test_routed_legacy_identifiers_remain_compatible():
    routed_text = "\n".join(
    ...
    assert "panergos://" in routed_text
    assert "api.example.test" in routed_text
