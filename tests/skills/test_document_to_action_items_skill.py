...
def test_frontmatter_required_fields():
    ...
    panergos = fm["metadata"]["panergos"]
    assert panergos["tags"]
    assert "related_skills" in panergos
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Panergos Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["panergos"]["related_skills"]:
