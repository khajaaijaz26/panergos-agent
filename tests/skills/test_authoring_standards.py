...
def test_required_frontmatter_fields(p):
    ...
    panergos = (fm.get("metadata") or {}).get("panergos") or {}
    if not (panergos.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.panergos.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    panergos = (fm.get("metadata") or {}).get("panergos") or {}
    dangling = [
        rs for rs in (panergos.get("related_skills") or []) if rs not in _all_names()
    ]
