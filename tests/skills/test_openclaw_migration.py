...
"""
    ...
    assert "Tyler Williams > Active Projects: Panergos Agent" in entries
...
def test_unreadable_config_is_refused_not_overwritten(tmp_path: Path):
    ...
    ``panergos_cli/agent_import.py``.
    """
    mod = load_module()
    migrator, config_path = _allowlist_migrator(
        mod, tmp_path, MALFORMED_PANERGOS_CONFIG)
...
def test_unreadable_config_blocks_later_config_steps_instead_of_partial_writes(
    ...
    assert config_path.read_text(encoding="utf-8") == MALFORMED_PANERGOS_CONFIG
...
def test_symlinked_config_stays_a_symlink(tmp_path: Path):
    """Managed deployments symlink ~/.panergos/config.yaml into a dotfiles repo.
    ...
    real.write_text("model: example-model\ncommand_allowlist:\n  - /usr/bin/*\n",
                    encoding="utf-8")
    ...
    assert "example-model" in real.read_text(encoding="utf-8")
...
def test_unreadable_config_refused_by_model_config_too(tmp_path: Path):
    ...
    config_path = target / "config.yaml"
    config_path.write_text(MALFORMED_PANERGOS_CONFIG, encoding="utf-8")
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # user at ~/.panergos/config.yaml in its post-migration summary — it never
    # writes to either. Under skills-guard-v2 (#92021) these score as
    # informational _ref findings (the old critical agent_config_mod /
    # panergos_config_mod findings no longer fire for bare mentions), so the
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_ref", "panergos_config_ref"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Panergos prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Panergos to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Panergos config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Panergos uses tools well"
    assert mod.rebrand_text("openclaw should always respond concisely") == "panergos should always respond concisely"
    assert mod.rebrand_text("Edit ~/.openclaw/config.yaml") == "Edit ~/.panergos/config.yaml"
