...
--------------------
When a non-default profile is active (``PANERGOS_HOME`` points at
...
class ProfileTripwire:
    ...
    Attributes:
        root:     the default profile's home (``<tmp>/panergos``).
...
def profile_tripwire(tmp_path, monkeypatch):
    ...
    1. A temp panergos root acting as the *default* profile, seeded with the
       ...
       detectable.
    2. ``PANERGOS_HOME`` (env var AND the context-local override) pointed at
       the testprof home — the exact activation shape ``--profile`` uses.
    3. ``panergos_state.DEFAULT_DB_PATH`` restored to its import-time snapshot.
       ...
       makes resolution flow through ``get_panergos_home()`` — the production
