"""Tests for panergos_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import panergos_logging
# Use whatever RotatingFileHandler class panergos_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. panergos_logging
...
from panergos_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    panergos_logging._logging_initialized = False
    ...
    panergos_logging._reset_queued_handlers()
    ...
    panergos_logging._install_session_record_factory()
    yield
    ...
    panergos_logging._logging_initialized = False
    panergos_logging.clear_session_context()


@pytest.fixture
def panergos_home(tmp_path, monkeypatch):
    """Provide an isolated PANERGOS_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_panergos_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["PANERGOS_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, panergos_home):
        log_dir = panergos_logging.setup_logging(panergos_home=panergos_home)
        assert log_dir == panergos_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, panergos_home):
        panergos_logging.setup_logging(panergos_home=panergos_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in panergos_logging._queued_file_handlers
    ...
    def test_idempotent_no_duplicate_handlers(self, panergos_home):
        panergos_logging.setup_logging(panergos_home=panergos_home)
        panergos_logging.setup_logging(panergos_home=panergos_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, panergos_home):
        panergos_logging.setup_logging(panergos_home=panergos_home)

        test_logger = logging.getLogger("test_panergos_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        panergos_logging.flush_log_queue()

        agent_log = panergos_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_profile_routing_follows_context_home(self, panergos_home, tmp_path):
        ...
        profile_home.mkdir()
        panergos_logging.setup_logging(panergos_home=panergos_home)
        assert panergos_logging.enable_profile_log_routing(
            [panergos_home, profile_home]
        ) is True
        ...
        assert "profile-routed cron record" not in (
            panergos_home / "logs" / "agent.log"
        ).read_text()




    def test_explicit_params_override_config(self, panergos_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (panergos_home / "config.yaml").write_text(yaml.dump(config))

        panergos_logging.setup_logging(panergos_home=panergos_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, panergos_home):
        panergos_logging.setup_logging(panergos_home=panergos_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, panergos_home):
        panergos_logging.setup_logging(panergos_home=panergos_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, panergos_home):
        ...
        gw_log = panergos_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, panergos_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, panergos_home):
        panergos_logging.setup_logging(panergos_home=panergos_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, panergos_home):
        panergos_logging.setup_logging(panergos_home=panergos_home, mode="gui")

        logging.getLogger("panergos_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        panergos_logging.flush_log_queue()

        gui_log = panergos_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, panergos_home):
        """When session context is set, log lines include [session_id]."""
        panergos_logging.setup_logging(panergos_home=panergos_home)
        panergos_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = panergos_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, panergos_home):
        panergos_logging.setup_logging(panergos_home=panergos_home)
        panergos_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_panergos_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        panergos_logging._add_rotating_handler(
            log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in panergos_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)]
        ...
        panergos_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("panergos_cli.config.is_managed", return_value=True):
                panergos_logging._add_rotating_handler(
                    log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = panergos_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert panergos_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not panergos_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, panergos_home):
        level, max_size, backup = panergos_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, panergos_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> panergos_logging._ManagedRotatingFileHandler:
        handler = panergos_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that panergos_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        panergos_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        panergos_logging.setup_logging(panergos_home=panergos_home, mode="gateway")
        gw_path = panergos_home / "logs" / "gateway.log"
        rotated = panergos_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = panergos_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, panergos_home):
        panergos_logging.setup_logging(panergos_home=panergos_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_panergos_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in panergos_logging._queued_file_handlers
        )
