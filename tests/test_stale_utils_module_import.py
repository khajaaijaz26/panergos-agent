...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.panergos/panergos-agent/utils.py)
...
2. ``panergos update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``panergos_cli/main.py`` (the ``panergos update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``panergos_constants`` after the
