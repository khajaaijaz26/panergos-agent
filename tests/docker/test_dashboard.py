"""Harness: dashboard opt-in via PANERGOS_DASHBOARD.

Today (tini): dashboard starts once when PANERGOS_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``panergos`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without PANERGOS_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "panergos dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without PANERGOS_DASHBOARD"
    )
...
# whenever `PANERGOS_DASHBOARD_HOST` was non-loopback, silently disabling
...
"""
    ...
    probe = (
        "/opt/panergos/.venv/bin/python - <<'PY'\n"
        f"{py_program}"
        "PY"
    )
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    when a ``DashboardAuthProvider`` is registered (the bundled example
    provider activates whenever ``PANERGOS_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    1. ``/api/auth/providers`` (publicly reachable through the gate so
       the login page can bootstrap) returns 200 with ``example`` in the
    ...
    start_container(
        ...
        "PANERGOS_DASHBOARD_HOST=0.0.0.0",
        "PANERGOS_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "example" in provider_names, (
        "Bundled dashboard_auth/example provider should register when "
        f"PANERGOS_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and PANERGOS_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``PANERGOS_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
