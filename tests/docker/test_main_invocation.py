...
def test_no_args_starts_panergos(built_image: str) -> None:
    """``docker run <image>`` should start panergos cleanly.
...
def test_chat_subcommand_passthrough(built_image: str) -> None:
    """``docker run <image> chat --help`` should exec ``panergos chat --help``.
...
def test_container_exit_code_matches_inner_exit(built_image: str) -> None:
    ...
    Critical for CI: ``docker run <image> panergos batch ...`` returns a
