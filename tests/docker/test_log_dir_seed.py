"""Runtime smoke test for Docker $PANERGOS_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the panergos user after container boot.
...
registered later runs its log service as the dropped panergos user and
...
def test_logs_gateways_seeded_and_panergos_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by panergos after boot."""
    ...
    # Both must be owned by panergos
    r = docker_exec_sh(
        container_name,
    ...
    assert "logs=panergos" in r.stdout, (
        f"logs/ not owned by panergos: {r.stdout}"
    )
    assert "gateways=panergos" in r.stdout, (
        f"logs/gateways/ not owned by panergos: {r.stdout}"
    )
