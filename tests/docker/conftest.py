...
Override the image with ``PANERGOS_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("PANERGOS_TEST_IMAGE", "panergos-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``PANERGOS_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("PANERGOS_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged panergos user
# ---------------------------------------------------------------------------
#
# Background: every Panergos runtime path inside the container drops to UID
# 10000 (the ``panergos`` user) via ``s6-setuidgid panergos``. ``docker exec``
...
# fails (PermissionError swallowed) for panergos, so a test that ran as root
...
# helpers below run every probe under ``-u panergos`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: panergos).
