...
The shim (docker/panergos-exec-shim.sh, installed at /opt/panergos/bin/panergos)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> panergos login` would write /opt/data/auth.json as
...
failed: Panergos is not logged into example Portal" on every message.

These tests verify:

1. ``docker exec <c> panergos …`` (defaulting to root) gets dropped to the
   panergos user before the real binary runs.
2. ``docker exec --user panergos <c> panergos …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $PANERGOS_HOME from a ``docker exec`` session land
   as panergos:panergos — the actual user-visible invariant.
4. The PANERGOS_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-panergos-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown panergos:panergos`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to panergos:panergos,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$PANERGOS_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (panergos_cli.container_boot), which s6 runs
    *strictly after* ``01-panergos-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_panergos_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `panergos` with a Python-style `-c` shim equivalent — there's no
    pure-panergos "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `panergos` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `panergos`
    do anything that writes to disk, then check the file's owner.

    Use `panergos config set` which writes config.yaml under PANERGOS_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "panergos", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by panergos, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "panergos:panergos", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected panergos:panergos. "
        "The shim did not drop privileges before invoking panergos."
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/panergos/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid panergos panergos <args>` which resolves `panergos`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    # `panergos auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `panergos config set` first to
    # provoke a write into PANERGOS_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "panergos", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # PANERGOS_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "panergos", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the panergos user "
