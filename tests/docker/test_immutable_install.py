...
Build the real image and verify at runtime:
  ...
  1. /opt/panergos is not writable by the panergos user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and PANERGOS_DISABLE_LAZY_INSTALLS are set
  3. /opt/panergos/.install_method contains "docker" (code-scoped stamp)
  4. $PANERGOS_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $PANERGOS_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_panergos(
    built_image: str, container_name: str,
) -> None:
    """The panergos user must not be able to modify /opt/panergos.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/panergos as the panergos user
        "touch /opt/panergos/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"panergos user can write to /opt/panergos (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/panergos/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"panergos user can write to /opt/panergos/.venv: {r.stdout}"
    )


def test_panergos_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    PANERGOS_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$PANERGOS_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"PANERGOS_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $PANERGOS_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $PANERGOS_HOME was not healed on boot: "
        f"{r.stdout}"
    )
