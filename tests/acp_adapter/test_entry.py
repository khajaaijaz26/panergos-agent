...
def test_help_uses_canonical_acp_command_and_brand(capsys):
    with pytest.raises(SystemExit) as excinfo:
        entry._parse_args(["--help"])

    assert excinfo.value.code == 0
    output = capsys.readouterr().out
    assert output.startswith("usage: panergos-acp")
    assert "Run Panergos Agent as an ACP stdio server." in output
...
def test_main_skips_configured_mcp_discovery_when_requested(monkeypatch):
    ...
    monkeypatch.setattr(entry, "_setup_logging", lambda: None)
    monkeypatch.setattr(entry, "_load_env", lambda: None)
    monkeypatch.setenv("PANERGOS_ACP_SKIP_CONFIGURED_MCP", "1")
    monkeypatch.setattr(
...
def test_main_setup_offers_browser_install_when_tty(monkeypatch):
    ...
    monkeypatch.setattr("panergos_cli.main.main", lambda: None)
...
def test_main_setup_browser_propagates_browser_failure(monkeypatch):
    ...
    monkeypatch.setattr("panergos_cli.dep_ensure.ensure_dependency", fake_ensure)
