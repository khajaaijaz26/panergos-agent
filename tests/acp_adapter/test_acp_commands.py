...
class FakeAgent:
    def __init__(self):
        ...
        self.enabled_toolsets = ["panergos-acp"]
...
def test_version_command_uses_distribution_brand_and_preserves_core_attribution():
    acp_agent, state, _fake, _conn = make_agent_and_state()

    output = acp_agent._cmd_version("", state)

    assert output.startswith("Panergos Agent v")
    assert "Panergos runtime v" in output
...
def test_acp_real_agent_gets_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "panergos_cli.config",
        mod("panergos_cli.config", load_config=lambda: {"model": {"default": "m", "provider": "p"}}),
    )
    monkeypatch.setitem(
        sys.modules,
