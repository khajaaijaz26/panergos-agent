...
def test_root_session_no_compression(db):
    ...
    assert prov["currentPanergosSessionId"] == "root1"
    assert prov["rootPanergosSessionId"] == "root1"
    assert prov["parentPanergosSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_panergos_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentPanergosSessionId"] == "old"
    assert prov["rootPanergosSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousPanergosSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootPanergosSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"panergos"}
    assert "sessionProvenance" in meta["panergos"]
    assert meta["panergos"]["sessionProvenance"]["currentPanergosSessionId"] == "root1"
