...
Keeps the ACP server tests offline: ``PanergosACPAgent._build_model_state``
calls ``panergos_cli.inventory.build_models_payload``, which (without this
...
def _offline_model_inventory(monkeypatch):
    ...
    import panergos_cli.inventory as inventory
