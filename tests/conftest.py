"""Shared fixtures for the panergos-agent test suite.
...
2. **Isolated PANERGOS_HOME.** PANERGOS_HOME points to a per-test tempdir so
   code reading ``~/.panergos/*`` via ``get_panergos_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".panergos"`` instead of the canonical
   ``get_panergos_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No PANERGOS_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox PANERGOS_HOME before ANY test module is imported ──────────────────
# `panergos_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_panergos_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.panergos/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes PANERGOS_HOME, but fixtures run
...
# the REAL Panergos root — capture it BEFORE the sandbox rewires PANERGOS_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.panergos (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("PANERGOS_KANBAN_HOME", "").strip()
_PRE_SANDBOX_PANERGOS_HOME = os.environ.get("PANERGOS_HOME", "")


def _panergos_home_points_at_production(value: str) -> bool:
    """True when a pre-set PANERGOS_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    PANERGOS_HOME=~/.panergos``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``panergos_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.panergos`` — the escape vector that landed pytest
    ...
    PANERGOS_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".panergos").resolve()
    except Exception:
        return True
...
if _panergos_home_points_at_production(os.environ.get("PANERGOS_HOME", "")):
    _SESSION_PANERGOS_HOME = tempfile.mkdtemp(prefix="panergos-test-home-")
    os.environ["PANERGOS_HOME"] = _SESSION_PANERGOS_HOME
    atexit.register(shutil.rmtree, _SESSION_PANERGOS_HOME, True)
...
# like a real CLI") — which used to disarm panergos_state's live-DB guard in
# the child at the same moment the child lost the PANERGOS_HOME redirect.
# PANERGOS_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# panergos_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export PANERGOS_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["PANERGOS_TEST_ISOLATION"] = os.environ.get("PANERGOS_HOME", "") or "1"

#: PANERGOS_HOME as it stood when conftest was imported - i.e. before any test
...
PANERGOS_HOME_AT_CONFTEST_IMPORT = os.environ.get("PANERGOS_HOME", "")
...
# PANERGOS_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_PANERGOS_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``panergos_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Panergos agent session inherits them and
    # panergos_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "PANERGOS_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "PANERGOS_INTERACTIVE",
    "PANERGOS_QUIET",
    "PANERGOS_TOOL_PROGRESS",
    "PANERGOS_TOOL_PROGRESS_MODE",
    "PANERGOS_MAX_ITERATIONS",
    "PANERGOS_SESSION_PLATFORM",
    "PANERGOS_SESSION_CHAT_ID",
    "PANERGOS_SESSION_CHAT_NAME",
    "PANERGOS_SESSION_CHAT_TYPE",
    "PANERGOS_SESSION_THREAD_ID",
    "PANERGOS_SESSION_SOURCE",
    "PANERGOS_SESSION_KEY",
    "PANERGOS_GATEWAY_SESSION",
    "PANERGOS_CRON_SESSION",
    "_PANERGOS_GATEWAY",
    "PANERGOS_PLATFORM",
    "PANERGOS_MODEL",
    "PANERGOS_INFERENCE_MODEL",
    "PANERGOS_INFERENCE_PROVIDER",
    "PANERGOS_TUI_PROVIDER",
    "PANERGOS_MANAGED",
    "PANERGOS_MANAGED_DIR",
    "PANERGOS_DEV",
    "PANERGOS_CONTAINER",
    "PANERGOS_EPHEMERAL_SYSTEM_PROMPT",
    "PANERGOS_TIMEZONE",
    "PANERGOS_REDACT_SECRETS",
    "PANERGOS_BACKGROUND_NOTIFICATIONS",
    "PANERGOS_EXEC_ASK",
    "PANERGOS_HOME_MODE",
    "PANERGOS_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.panergos/kanban.db instead of the per-test PANERGOS_HOME.
    "PANERGOS_KANBAN_DB",
    "PANERGOS_KANBAN_BOARD",
    "PANERGOS_KANBAN_HOME",
    "PANERGOS_KANBAN_WORKSPACES_ROOT",
    "PANERGOS_KANBAN_LOGS_ROOT",
    "PANERGOS_KANBAN_TASK",
    "PANERGOS_KANBAN_WORKSPACE",
    "PANERGOS_KANBAN_RUN_ID",
    "PANERGOS_KANBAN_CLAIM_LOCK",
    "PANERGOS_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "PANERGOS_DELEGATED_CHILD_CONTEXT",
    "PANERGOS_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "panergos" host.
    "PANERGOS_HONCHO_HOST",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `example` plugin auto-registers itself on plugin discovery,
    ...
    "PANERGOS_DASHBOARD_OAUTH_CLIENT_ID",
    "PANERGOS_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and PANERGOS_HOME to per-test tempdirs so code that
    reads ``~/.panergos/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral PANERGOS_* vars that could change test semantics.
    for name in _PANERGOS_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("PANERGOS_HONCHO_HOST", "panergos")

    # 3. Redirect PANERGOS_HOME to a per-test tempdir. Code that reads
    #    ``~/.panergos/*`` via ``get_panergos_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.panergos/*`` via
    #    ``Path.home() / ".panergos"`` instead of ``get_panergos_home()``
    #    is a bug to fix at the callsite.
    fake_panergos_home = tmp_path / "panergos_test"
    fake_panergos_home.mkdir()
    (fake_panergos_home / "sessions").mkdir()
    (fake_panergos_home / "cron").mkdir()
    (fake_panergos_home / "memories").mkdir()
    (fake_panergos_home / "skills").mkdir()
    monkeypatch.setenv("PANERGOS_HOME", str(fake_panergos_home))
    ...
    # panergos_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("PANERGOS_TEST_ISOLATION", str(fake_panergos_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("PANERGOS_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. panergos_state computes ``DEFAULT_DB_PATH = get_panergos_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from panergos_state import ...``) that
    ...
    panergos_state_mod = sys.modules.get("panergos_state")
    if panergos_state_mod is not None and hasattr(panergos_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            panergos_state_mod, "DEFAULT_DB_PATH", fake_panergos_home / "state.db"
        )
    ...
    monkeypatch.setenv("PANERGOS_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.panergos/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import panergos_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path PANERGOS_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_panergos_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from panergos_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
def _neutralize_git_safe_directory_read(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_safe_directory"):
        return
    try:
        from panergos_cli import _subprocess_compat
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.panergos. This autouse
...
# allow-list because test-level fixtures legitimately move PANERGOS_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session PANERGOS_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``PANERGOS_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Panergos root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_PANERGOS_HOME and not _panergos_home_points_at_production(
        _PRE_SANDBOX_PANERGOS_HOME
    ):
        # PANERGOS_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_panergos_root().resolve()
    # No pre-existing PANERGOS_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".panergos").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.panergos`` captured at import time. Hermetic tests that legitimately
    move PANERGOS_HOME to sibling tempdirs are unaffected.

    Only patches when ``panergos_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("panergos_cli.kanban_db")
    _kdbc = sys.modules.get("panergos_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import panergos_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.panergos. See #69283."
        )
...
# ``panergos_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Panergos root. This fixture wires the
# test-side knobs:
...
#     installs where PANERGOS_HOME is not ~/.panergos) into the guard's
...
# so subprocess children that import panergos_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("panergos_state")
    ...
    if _PRE_SANDBOX_PANERGOS_HOME and not _panergos_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_PANERGOS_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Panergos home override redirects every later
    # ``get_panergos_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from panergos_constants import get_panergos_home_override, set_panergos_home_override

        if get_panergos_home_override() is not None:
            set_panergos_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal panergos config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``panergos-gateway`` process
...
# correlated with a ``tests/panergos_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> panergos-gateway``
...
def _wal_is_usable() -> bool:
    """True when Panergos will actually put a database into WAL mode here.

    Panergos refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Panergos can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Panergos managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``panergos_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_panergos_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_panergos_home`` fixture
    redirects ``PANERGOS_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.panergos/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``panergos_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["PANERGOS_VOICE_TTS"] = "1"``. The
...
#      ``panergos_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``panergos_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``panergos_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``panergos_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Panergos runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.panergos``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Panergos falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Panergos uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart panergos-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting panergos/python patterns are also blocked.
    """
    ...
    def _matches_panergos_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _PANERGOS_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_panergos_gateway(cmd_str):
            return False
        try:
            tokens = _shlex.split(cmd_str)
        except ValueError:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch panergos-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m panergos_cli.main".
                if (
                    "panergos" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live panergos-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting panergos/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `panergos update` (or the
        # equivalent `python -m panergos_cli.main update`).  These commands
        ...
        if "update" in low and (
            # panergos update / panergos update --gateway / setsid bash -c ... panergos update
            ("panergos" in low and "update" in low.split())
            or
            # python -m panergos_cli.main update --gateway
            ("panergos_cli" in low and "update" in low.split())
            or
            # venv/bin/panergos update  (absolute path variant used in tests)
            (".venv/bin/panergos" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`panergos update` against the real checkout, fetching "
        ...
        # Block spawning a REAL gateway runtime (``python -m panergos_cli.main
        # gateway run|start|restart``). ``_spawn_panergos_action`` launches it
        # with start_new_session=True, so it outlives the pytest worker; the
        # child inherits the pytest-tmp PANERGOS_HOME, resolves the DEVELOPER's
        # ``panergos-gateway`` systemd unit (a tmp home hashes to no profile
        ...
        # A gateway launched INSIDE a container (`docker exec … panergos gateway start`) cannot
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "panergos gateway runtime that outlives the test (it is "
                "detached), restarts the developer's live gateway, and "
                "holds the webhook port. Patch the spawn seam where "
                "production reads it (panergos_cli.web_server_gateway."
                "_spawn_panergos_action), or mark with "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_PANERGOS_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import panergos_cli.voice as _voice
    except Exception:
