...
Drives the REAL ``tick()`` against a throwaway PANERGOS_HOME with a ``no_agent`` script job that
...
def slot_env(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("PANERGOS_MACHINE_ID", raising=False)

    import cron.executions as E
    import cron.jobs as J
    import cron.scheduler as S

    monkeypatch.setattr(J, "PANERGOS_DIR", home)
    ...
    monkeypatch.setattr(S, "_panergos_home", home)
    ...
    slot = (J._panergos_now() - timedelta(minutes=1)).replace(microsecond=0).isoformat()
    stored = J.load_jobs()
    ...
    def crash_before_dispatch() -> None:
        ...
        env = dict(os.environ, PANERGOS_HOME=str(home))
...
class TestMissedWindowCatchUp:
    def test_slot_lost_before_dispatch_fires_once_after_restart(self, slot_env):
        ...
        assert J._ensure_aware(J.datetime.fromisoformat(after_crash["next_run_at"])) > J._panergos_now()
        ...
        assert J._ensure_aware(J.datetime.fromisoformat(rec["next_run_at"])) > J._panergos_now()
