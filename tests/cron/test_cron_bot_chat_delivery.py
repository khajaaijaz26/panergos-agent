...
def test_named_profile_resolves_when_exists():
    with mock.patch("panergos_cli.profiles.profile_exists", return_value=True):
...
def test_unknown_profile_resolves_to_none():
    with mock.patch("panergos_cli.profiles.profile_exists", return_value=False):
...
def test_deliver_runs_canonical_bot_chat_lane():
    ...
    with mock.patch.object(sched.subprocess, "run", side_effect=fake_run), \
         mock.patch.object(sched_delivery.shutil, "which", return_value="/usr/bin/panergos") as which:
        err = _deliver_to_bot_chat({"id": "j1", "name": "Daily digest"}, "the output", "")

    assert err is None
    argv = calls["argv"]
    assert argv[0] == "/usr/bin/panergos"
    which.assert_called_once_with("panergos")
    assert "-p" not in argv  # own profile: subprocess inherits PANERGOS_HOME
    assert "chat" in argv
...
def test_deliver_named_profile_uses_p_flag_and_clears_home():
    ...
    with mock.patch.object(sched.subprocess, "run", side_effect=fake_run), \
         mock.patch.object(
             sched_delivery.shutil, "which",
             side_effect=lambda name: "/usr/bin/panergos" if name == "panergos" else None,
         ) as which, \
         mock.patch.dict(sched.os.environ, {"PANERGOS_HOME": "/tmp/other-profile"}):
        err = _deliver_to_bot_chat({"id": "j1", "name": "n"}, "out", "research")

    assert err is None
    argv = calls["argv"]
    assert argv[0] == "/usr/bin/panergos"
    assert [call.args for call in which.call_args_list] == [("panergos",)]
    assert argv[1:3] == ["-p", "research"]
    # -p owns resolution; the scheduler's own PANERGOS_HOME must not leak in.
    assert "PANERGOS_HOME" not in calls["kwargs"]["env"]


def test_deliver_failure_returns_error_string():
    with mock.patch.object(
        sched.subprocess, "run", return_value=_completed(returncode=1, stderr="boom")
    ), mock.patch.object(sched_delivery.shutil, "which", return_value="/usr/bin/panergos"):
...
def test_deliver_timeout_returns_error_string():
    with mock.patch.object(
        ...
        side_effect=subprocess.TimeoutExpired(cmd="panergos", timeout=600),
    ), mock.patch.object(sched_delivery.shutil, "which", return_value="/usr/bin/panergos"):
        err = _deliver_to_bot_chat({"id": "j1", "name": "n"}, "out", "")
    assert err is not None
...
def test_delivery_targets_include_local_profiles():
    with mock.patch("panergos_cli.profiles.list_profile_names",
