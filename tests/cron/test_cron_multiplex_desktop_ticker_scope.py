...
def test_standalone_fallback_pool_keeps_profile_scope(tmp_path, monkeypatch):
    ...
    from panergos_constants import get_panergos_home, set_panergos_home_override
    ...
    async def _inside_running_loop():
        ...
        set_panergos_home_override(str(sec_home))
...
def test_multiplex_ticker_profile_gate_skips_rejected_profile(tmp_path):
    ...
    def _tick(*args, **kwargs):
        ticked.append(str(get_panergos_home()))
        if len(ticked) >= 3:
            stop.set()
        return 0
...
def test_desktop_ticker_gates_on_profile_gateway_running(tmp_path, monkeypatch, profile_count):
    ...
    running = {homes[-1][1]}
    monkeypatch.setattr(
        "panergos_cli.profiles.profiles_to_serve", lambda multiplex=False: list(homes)
    )
    monkeypatch.setattr(
        "panergos_cli.profiles._check_gateway_running", lambda home: home in running
    )
    monkeypatch.setattr("panergos_cli.profiles._served_by_running_multiplexer", lambda name: False)
    captured = {}
    ...
    monkeypatch.setattr("panergos_logging.enable_profile_log_routing", lambda homes: None)
