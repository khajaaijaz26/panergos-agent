...
scheduler plugin fires a job: across N gateway replicas, exactly ONE wins the
claim for a given fire. Single-machine deployments always win (unaffected).

These exercise the real store against a temp PANERGOS_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    ...
    # cron.jobs caches no home at import; get_panergos_home() reads the env live.
    yield tmp_path
...
def test_fresh_claim_from_a_dead_same_host_owner_is_reclaimable(temp_home):
    ...
    ``panergos cron run`` killed mid-flight must not block the next manual run for the whole TTL
