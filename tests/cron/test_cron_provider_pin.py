...
resolve_runtime_provider against a temp PANERGOS_HOME) and the create_job snapshot capture.
"""
...
def _run(job, tmp_path, *, current_provider="openrouter", current_model=None, cron_model=None,
    ...
    with patch("cron.scheduler._panergos_home", tmp_path), \
         patch("cron.scheduler._get_panergos_home", return_value=tmp_path), \
         ...
         patch("panergos_cli.runtime_provider.resolve_runtime_provider", side_effect=_resolve), \
...
class TestSnapshotIsTheEffectivePin:
    ...
    def test_missing_model_guides_to_user_owned_cli(self, tmp_path, monkeypatch):
        ...
        assert "panergos cron edit pin-test --model <name>" in error
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("panergos_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
                prompt="do a thing", schedule="every 1 hour", provider="example"
            )
