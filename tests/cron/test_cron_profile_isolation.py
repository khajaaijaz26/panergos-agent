...
profile's PANERGOS_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.panergos/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active PANERGOS_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever PANERGOS_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    PANERGOS_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import panergos_constants

    monkeypatch.setattr(
        panergos_constants, "_get_platform_default_panergos_home", lambda: root
    )
    monkeypatch.setenv("PANERGOS_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile PANERGOS_HOME (<root>/profiles/<name>), the cron store
    ...
    assert panergos_constants.get_panergos_home().resolve() == profile_home.resolve()
    assert panergos_constants.get_default_panergos_root().resolve() == root.resolve()

    # cron/jobs.py computes PANERGOS_DIR from get_panergos_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.PANERGOS_DIR.resolve() == profile_home.resolve()
        assert (
