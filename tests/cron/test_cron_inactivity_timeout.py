...
- Unlimited timeout (PANERGOS_CRON_TIMEOUT=0)
- Backward compat: PANERGOS_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """PANERGOS_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """PANERGOS_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("PANERGOS_CRON_TIMEOUT", "1200")
        raw = os.getenv("PANERGOS_CRON_TIMEOUT", "").strip()
...
class TestSysPathOrdering:
    ...
    def test_panergos_time_importable(self):
        """panergos_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _panergos_now
        assert callable(_panergos_now)
