...
An external provider's `fire_due` can reuse the IDENTICAL body — no duplicated
correctness.
...
def test_escaped_failure_delivery_carries_the_streak_nudge(monkeypatch):
    ...
    assert "panergos cron pause scout" in delivered[0]
...
def test_run_one_job_installs_secret_scope_under_multiplex(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(s, "_get_panergos_home", lambda: tmp_path)
