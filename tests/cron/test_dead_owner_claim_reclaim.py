...
A one-shot ``panergos cron run <job_id>`` used to background-dispatch the run
...
1. ``panergos_cli.cron._job_action("run", ...)`` declares the channel stateless
...
def _orphan_claimed_row(executions, job_id: str) -> str:
    ...
    Mirrors what a one-shot ``panergos cron run`` leaves behind: a row stuck in
