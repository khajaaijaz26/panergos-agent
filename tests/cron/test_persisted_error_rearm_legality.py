...
def cron_store(tmp_path, monkeypatch):
    ...
    (panergos_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))
    monkeypatch.setattr(J, "PANERGOS_DIR", panergos_home)
    monkeypatch.setattr(J, "CRON_DIR", panergos_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", panergos_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", panergos_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_panergos_now", lambda: now):
            due = J.get_due_jobs()
