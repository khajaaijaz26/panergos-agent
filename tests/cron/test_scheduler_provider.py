...
CronScheduler plan, .panergos/plans/cron-scheduler-provider-interface.md). They
...
No production code is exercised beyond the two ticker entry points:
  ...
  - panergos_cli/web_server.py::_start_desktop_cron_ticker  (desktop fallback)
...
def test_desktop_ticker_calls_tick_then_stops():
    ...
    no adapters/loop."""
    from panergos_cli.web_server import _start_desktop_cron_ticker
...
def test_discover_cron_schedulers_returns_list():
    ...
    assert all(name != "builtin" for name, _desc, _available in result)
...
def test_multiplex_recovery_isolates_profile_failures(tmp_path):
    ...
    def _tick(*args, **kwargs):
        tick_homes.append(str(get_panergos_home()))
        if len(tick_homes) >= 2:
            stop.set()
        return 0
...
def test_multiplex_ticker_reenumerates_profiles_each_cycle(tmp_path):
    ...
    def _tick(*args, **kwargs):
        ...
        if len(ticked) == 1:  # "panergos profile create gamma" happens between two cycles
