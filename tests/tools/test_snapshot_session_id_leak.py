"""Cross-session PANERGOS_SESSION_ID leak via the shared bash snapshot.
...
``PANERGOS_SESSION_ID`` into the snapshot, so every LATER session ``source``d that
stale value and its ``echo $PANERGOS_SESSION_ID`` reported a FOREIGN session's id
— overriding the correct per-command Popen env injected by
``_inject_session_context_env``.

The fix strips the per-session bridged vars (PANERGOS_SESSION_* / UI /
...
def test_export_snippet_shape():
    snippet = _export_dump_excluding_session_vars('"$__panergos_snap_tmp"')
    ...
    assert "${!PANERGOS_SESSION_*}" in snippet
    assert "${!PANERGOS_CRON_AUTO_DELIVER_*}" in snippet
    assert "${!PANERGOS_BROWSER_CONTROL_*}" in snippet
    assert "PANERGOS_UI_SESSION_ID" in snippet
    assert "grep -vE" not in snippet
    assert '"$__panergos_snap_tmp"' in snippet
    ...
    assert snippet.rstrip().endswith('> "$__panergos_snap_tmp"')
    ...
    try:
        def run_as(sid):
            ...
            def worker():
                ...
                out["r"] = env.execute('echo "[$PANERGOS_SESSION_ID]"')
        ...
        if os.path.exists(snap):
            with open(snap) as f:
                assert "PANERGOS_SESSION_ID" not in f.read()
    finally:
        env.cleanup()
