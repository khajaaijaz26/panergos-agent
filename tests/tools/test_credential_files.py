...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        panergos_home.mkdir()
        (panergos_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"PANERGOS_HOME": str(panergos_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(panergos_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.panergos/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        panergos_home = tmp_path / ".panergos"
        panergos_home.mkdir()
        (panergos_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        panergos_home = tmp_path / ".panergos"
        skills_dir = panergos_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.panergos/skills"


    def test_custom_container_base(self, tmp_path):
        panergos_home = tmp_path / ".panergos"
        (panergos_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"PANERGOS_HOME": str(panergos_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.panergos")

        assert mounts[0]["container_path"] == "/home/user/.panergos/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.panergos/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.panergos/skills/cat/myskill/scripts/run.sh" in paths
    ...
    def test_skips_excluded_bookkeeping_dirs(self, tmp_path):
        ...
        paths = {f["container_path"] for f in files}
        assert "/root/.panergos/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.panergos/skills/cat/myskill/references/api.md" in paths
        for excluded in (
            ".hub",
            ".archive",
            ".curator_backups",
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape PANERGOS_HOME."""
        monkeypatch.setenv("PANERGOS_HOME", str(tmp_path / ".panergos"))
        (tmp_path / ".panergos").mkdir()

        # Create a sensitive file one level above panergos_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside panergos_home
    ...
    def test_nested_subdir_inside_panergos_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of PANERGOS_HOME must be allowed."""
        panergos_home = tmp_path / ".panergos"
        panergos_home.mkdir()
        subdir = panergos_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside PANERGOS_HOME pointing outside must be rejected."""
        panergos_home = tmp_path / ".panergos"
        panergos_home.mkdir()
        monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))

        # Create a sensitive file outside panergos_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside panergos_home pointing outside
        symlink = panergos_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes PANERGOS_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, panergos_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape PANERGOS_HOME."""
        panergos_home = tmp_path / ".panergos"
        panergos_home.mkdir()
        monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))
        ...
        sensitive.write_text("{}")
        self._write_config(panergos_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(panergos_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside PANERGOS_HOME via config must still mount."""
        panergos_home = tmp_path / ".panergos"
        panergos_home.mkdir()
        monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))

        (panergos_home / "oauth.json").write_text("{}")
        self._write_config(panergos_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        panergos_home = tmp_path / ".panergos"
        panergos_home.mkdir()
        (panergos_home / "cache" / "documents").mkdir(parents=True)
        (panergos_home / "cache" / "audio").mkdir(parents=True)
        (panergos_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.panergos/cache/documents" in paths
        assert "/root/.panergos/cache/audio" in paths
        assert "/root/.panergos/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_panergos_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        panergos_home = tmp_path / ".panergos"
        panergos_home.mkdir()
        # Use legacy dir name with content — get_panergos_dir prefers
        # populated old over new.
        legacy_doc = panergos_home / "document_cache"
        legacy_img = panergos_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(panergos_home / "document_cache") in host_paths
        assert str(panergos_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.panergos/cache/documents" in container_paths
        assert "/root/.panergos/cache/images" in container_paths
    ...
    def test_empty_panergos_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.panergos/attachments" in container_paths
        assert "/root/.panergos/images" in container_paths
        assert "/root/.panergos/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``PANERGOS_HOME/images``, not
        ...
        """
        panergos_home = tmp_path / ".panergos"
        (panergos_home / "images").mkdir(parents=True)
        monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.panergos/images" in by_container
        assert by_container["/root/.panergos/images"] == str(panergos_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        panergos_home = tmp_path / ".panergos"
        (panergos_home / "images").mkdir(parents=True)
        upload = panergos_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.panergos/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        panergos_home = tmp_path / ".panergos"
        img_dir = panergos_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.panergos/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(panergos_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.panergos/cache/images/x.png"
        assert (panergos_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    Panergos cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        panergos_home = tmp_path / ".panergos"
        (panergos_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("PANERGOS_HOME", str(panergos_home))
        return str(panergos_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_panergos(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.panergos/attachments/drop.zip"

    def test_ssh_maps_to_tilde_panergos(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.panergos/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        panergos_home = tmp_path / ".panergos"
        doc_dir = panergos_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — PANERGOS_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping PANERGOS_HOME, but every master
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.panergos/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.panergos/google_token.json" in paths
        assert "/root/.panergos/.env" not in paths
