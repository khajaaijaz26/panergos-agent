...
class TestClawHubCatalogWalkBounded(unittest.TestCase):
    ...
    @patch("tools.skills_hub._read_index_cache", return_value=None)
    @patch("tools.skills_hub.httpx.get")
    def test_max_items_zero_still_obeys_wall_clock_budget(
        self, mock_get, _mock_read_cache, mock_write_cache
    ):
        """Removing the item cap must never remove the wall-clock deadline."""
        page_calls = {"n": 0}
        mock_get.side_effect = self._infinite_pages(page_calls)
        ...
        self.assertEqual(page_calls["n"], 0)
        self.assertEqual(results, [])
        self.assertFalse(self.src.catalog_walk_complete)
        self.assertEqual(self.src.catalog_walk_stop_reason, "time_budget")
        mock_write_cache.assert_not_called()
