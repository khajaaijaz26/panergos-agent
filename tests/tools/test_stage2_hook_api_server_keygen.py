...
was gated on ``[ -f "$PANERGOS_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        "set -eu\n"
        f"{env_setup}"
        f'PANERGOS_HOME="{home}"\n'
        # In tests we run unprivileged; as_panergos is a passthrough then.
        'as_panergos() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Panergos loads $PANERGOS_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/panergos/.env.example -> $PANERGOS_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $PANERGOS_HOME/.env from it on first boot"
    )
