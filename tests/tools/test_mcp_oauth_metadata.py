...
Covers:
- :class:`PanergosTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.PanergosMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _PANERGOS_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# PanergosTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = PanergosTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("PANERGOS_HOME", str(tmp_path))
        storage = PanergosTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (PanergosMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: PanergosTokenStorage, **context_attrs):
    ...
    provider = _PANERGOS_PROVIDER_CLS.__new__(_PANERGOS_PROVIDER_CLS)
    provider._panergos_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_HOME", str(tmp_path))
        storage = PanergosTokenStorage("mgr-srv")
        ...
        with patch.object(
            _PANERGOS_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_HOME", str(tmp_path))
        storage = PanergosTokenStorage("flow-srv")
        ...
        with patch.object(
            _PANERGOS_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
