...
class TestModalBulkUpload:
    ...
    def test_tar_archive_contains_all_files(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src_a), "/root/.panergos/credentials/a.json"),
            (str(src_b), "/root/.panergos/skills/b.py"),
        ]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            ...
            assert "root/.panergos/credentials/a.json" in names
            assert "root/.panergos/skills/b.py" in names

            # Verify content
            a_content = tar.extractfile("root/.panergos/credentials/a.json").read()
            assert a_content == b"cred_content"
            b_content = tar.extractfile("root/.panergos/skills/b.py").read()
    ...
    def test_stdin_chunked_for_large_payloads(self, monkeypatch, tmp_path):
        ...
        files = [(str(src), "/root/.panergos/large.bin")]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            names = tar.getnames()
            assert "root/.panergos/large.bin" in names
