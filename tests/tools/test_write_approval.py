...
and the shared slash-command handlers (panergos_cli/write_approval_commands.py).
...
@pytest.fixture
def panergos_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="panergos_wa_test_")
    home = os.path.join(d, ".panergos")
    os.makedirs(home)
    monkeypatch.setenv("PANERGOS_HOME", home)
    yield home
    shutil.rmtree(d, ignore_errors=True)


def _set_approval(subsystem, enabled):
    import panergos_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(panergos_home):
...
def test_invalid_subsystem_is_off(panergos_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(panergos_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(panergos_home, capsys):
...
def test_load_on_disk_store_honors_configured_limits_and_permissions(panergos_home, monkeypatch):
    ...
    monkeypatch.setattr("panergos_cli.config.load_config", _boom)
...
def test_handle_approve_all(panergos_home):
...
def test_handle_approval_on(panergos_home):
    from panergos_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(panergos_home):
    from panergos_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(panergos_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(panergos_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(panergos_home):
