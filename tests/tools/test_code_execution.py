...
UDS socket lifecycle, panergos_tools generation, timeout enforcement,
...
from tools.code_execution_tool import (
    ...
    execute_code,
    generate_panergos_tools_module,
...
class TestPanergosToolsGeneration(unittest.TestCase):
    def test_generates_all_allowed_tools(self):
        src = generate_panergos_tools_module(list(SANDBOX_ALLOWED_TOOLS))
        for tool in SANDBOX_ALLOWED_TOOLS:
            self.assertIn(f"def {tool}(", src)


    def test_empty_list_generates_nothing(self):
        src = generate_panergos_tools_module([])
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_panergos_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"panergos_rpc\")", src)
        self.assertNotIn('os.environ.get("PANERGOS_RPC_DIR", "/tmp/panergos_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_panergos_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        mkdir_cmd = next(cmd for cmd, _, _ in env.commands
                         if "mkdir -p" in cmd and "panergos_exec_" in cmd)
        run_cmd = next(cmd for cmd, _, _ in env.commands if "python3 script.py" in cmd)
        cleanup_cmd = next(cmd for cmd, _, _ in env.commands
                           if "rm -rf" in cmd and "panergos_exec_" in cmd)
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/panergos_exec_", mkdir_cmd)
        self.assertIn("PANERGOS_RPC_DIR=/data/data/com.termux/files/usr/tmp/panergos_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/panergos_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/panergos_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """PANERGOS_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"PANERGOS_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import panergos_constants; print(panergos_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("panergos_constants.py", result["output"])
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from panergos_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from panergos_tools import json_parse
...
"""
    ...
    def test_retry_helper_all_fail(self):
        ...
        code = """
from panergos_tools import retry
def always_fail():
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_panergos_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("PANERGOS_RPC_SOCKET", child_env)
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_does_not_import_interactive_cli(self):
        ...
        with patch.dict("sys.modules", {"cli": mock_cli}), \
             patch("panergos_cli.config.load_config_readonly", return_value={}):
            result = _load_config()
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching PANERGOS_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated panergos_tools module reads PANERGOS_RPC_TOKEN and sends it."""
        src = generate_panergos_tools_module(["terminal"], transport="uds")
        self.assertIn("PANERGOS_RPC_TOKEN", src)
