...
class TestAugust2026Catalog:
    ...
    def test_recraft_v41_omits_keys_its_schema_lacks(self, image_tool):
        """Recraft V4.1 exposes no num_images/output_format/seed â€” the
...
class TestMetaMuseImage:
    """Meta Muse Image (meta/muse-image/*) â€” Aug 2026 addition."""
    ...
    def test_text_payload_matches_vendor_schema(self, image_tool):
        ...
        output_format/sync_mode â€” no seed, no resolution/quality knobs."""
...
class TestGptQualityPinnedToMedium:
    ...
    and cannot be overridden via config. Pinning keeps example Portal billing
...
class TestRegistryIntegration:
    ...
    def test_schema_exposes_expected_agent_params(self, image_tool):
        """The static registration schema stays minimal â€” prompt (required)
...
class TestUpscaleOptIn:
    ...
    def test_explicit_true_upscales_native_hi_res_model(self, image_tool, monkeypatch):
        """Seedream Lite has upscale=False in the catalog (native 4K) â€”
    ...
    def test_omitted_is_off_for_previously_default_on_model(self, image_tool, monkeypatch):
        """flux-2-pro was the old default-on model â€” now off like the rest."""
        self._run(image_tool, monkeypatch,
...
class TestUpscaleDispatchForwarding:
    ...
    def test_dispatch_forwards_upscale(self, image_tool, monkeypatch):
        ...
        monkeypatch.setattr(
            "panergos_cli.plugins._ensure_plugins_discovered", lambda *a, **k: None
        )
