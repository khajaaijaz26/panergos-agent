"""Tests for file write safety and PANERGOS_WRITE_SAFE_ROOT sandboxing.
...
class TestSafeWriteRoot:
    """PANERGOS_WRITE_SAFE_ROOT should sandbox writes to a specific subtree."""
    ...
    def test_safe_root_does_not_override_static_deny(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_WRITE_SAFE_ROOT", os.path.expanduser("~"))
        assert _is_write_denied(os.path.expanduser("~/.ssh/id_rsa")) is True


class TestMultipleSafeWriteRoots:
    """PANERGOS_WRITE_SAFE_ROOT with multiple colon-separated directories."""
    ...
    def test_write_inside_first_root_allowed(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_WRITE_SAFE_ROOT", f"{root_a}{os.pathsep}{root_b}")
        assert _is_write_denied(str(child)) is False
    ...
    def test_trailing_separator_ignored(self, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setenv("PANERGOS_WRITE_SAFE_ROOT", f"{root}{os.pathsep}")
        assert _is_write_denied(str(inside)) is False
...
class TestGetWriteDeniedError:
    ...
    def test_credential_path_message(self):
        ...
        assert "PANERGOS_WRITE_SAFE_ROOT" not in err
    ...
    def test_safe_root_message(self, tmp_path: Path, monkeypatch):
        ...
        assert err is not None
        assert "outside PANERGOS_WRITE_SAFE_ROOT" in err
...
class TestSafeRootDenialMessageIntegration:
    ...
    message when PANERGOS_WRITE_SAFE_ROOT blocks a path.
    ...
    def test_write_file_safe_root_outside_shows_safe_root_message(
        ...
        assert "outside PANERGOS_WRITE_SAFE_ROOT" in res.error
...
class TestAtomicWrite:
    ...
    def test_no_temp_file_leaked_on_success(self, ops, tmp_path: Path):
        ...
        assert [p for p in os.listdir(tmp_path) if ".panergos-tmp" in p] == []
...
class TestProtectedInstructionFiles:
    ...
    AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project-local .panergos
    ...
    def test_project_local_panergos_dir_is_gated(self, tmp_path, approvals):
        proj = tmp_path / "proj" / ".panergos"
        proj.mkdir(parents=True)
        approvals["answer"] = "deny"
        res = self._write(proj / "config.yaml")
        assert res.get("error") and "BLOCKED" in res["error"]

    def test_checkout_nested_under_panergos_dir_not_gated(self, tmp_path, approvals):
        """A repo living UNDER a .panergos dir (e.g. ~/.panergos/panergos-agent)
        must not have every write gated — only files directly inside a
        .panergos dir count as project config."""
        repo = tmp_path / ".panergos" / "some-repo" / "src"
        repo.mkdir(parents=True)
        res = self._write(repo / "module.py", "x = 1\n")
        assert not res.get("error"), res
        assert approvals["calls"] == []

    def test_real_panergos_home_not_gated_by_this_check(
        self, tmp_path, approvals, monkeypatch
    ):
        """~/.panergos itself is governed by existing guards, not this gate."""
        import tools.file_tools_write_guards as ft
        fake_home = tmp_path / ".panergos"
        (fake_home / "notes").mkdir(parents=True)
        monkeypatch.setattr(
            ft, "_get_real_panergos_home", lambda: str(fake_home.resolve())
        )
...
class TestMultiplexProfileWriteGuardsAreProfileScoped:
    """#107327: a multiplexed gateway scopes ``PANERGOS_HOME`` per turn via a
    ...
    def test_home_getter_tracks_active_profile_after_a_prior_scope(self, tmp_path):
        ...
        tok = set_panergos_home_override(str(a))
        try:
            assert ft._get_real_panergos_home() == os.path.realpath(str(a))
        finally:
            reset_panergos_home_override(tok)
        # The next turn is beta — the getter must now return beta's home, not alpha's.
        tok = set_panergos_home_override(str(b))
        try:
            assert ft._get_real_panergos_home() == os.path.realpath(str(b))
        finally:
            reset_panergos_home_override(tok)
    ...
    def test_config_hard_block_refuses_beta_config_even_after_alpha_turn(self, tmp_path):
        ...
        tok = set_panergos_home_override(str(a))
        try:
            ft._get_panergos_config_resolved()  # warm the (formerly poisoning) alpha lookup
        finally:
            reset_panergos_home_override(tok)
        ...
        try:
            ...
            reset_panergos_home_override(tok)
        assert err is not None
        assert "Refusing to write to Panergos config file" in err
