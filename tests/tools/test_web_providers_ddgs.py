...
Covers:
- DDGSWebSearchProvider.is_available() â€” reflects package importability
- DDGSWebSearchProvider.search() â€” happy path, missing package, runtime error
...
def _install_fake_ddgs(monkeypatch, *, text_results=None, text_raises=None, text_sleep=None):
    ...
    ``text_sleep``: if set, DDGS().text blocks for this many seconds before
        yielding â€” simulates a hung/slow search for the timeout test.
    """
    ...
    class _FakeDDGS:
        def __init__(self, **kwargs):
            # Accept timeout= (and any other constructor kwargs) â€” the provider
...
class TestDDGSProviderSearch:
    ...
    def test_hung_search_times_out_and_returns_failure(self, monkeypatch):
        ...
        timeout and reaped â€” even when the child never returns to Python."""
        _install_fake_ddgs(monkeypatch)
        import plugins.web.ddgs.provider as prov
    ...
    def test_fast_search_not_affected_by_timeout_wrapper(self, monkeypatch):
        ...
        fast search â€” results flow through unchanged."""
        _install_fake_ddgs(
            monkeypatch,
