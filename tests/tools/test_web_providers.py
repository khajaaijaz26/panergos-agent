...
class TestUnconfiguredErrorEnvelopeParity:
    ...
    def test_explicit_firecrawl_unconfigured_uses_firecrawl_keyless(self, monkeypatch):
        ...
        keyless cloud client (PR #50659 salvage) â€” a keyless ring peer must not
        ...
        # Developer machines may carry FIRECRAWL_* in ~/.panergos/.env â€” the
        # config-aware lookup must see a truly keyless environment here.
        monkeypatch.setattr(
            "panergos_cli.config.get_env_value", lambda name: None, raising=True
        )
...
class TestDispatchersTriggerPluginDiscovery:
    ...
    Mirrors :func:`tools.browser_tool_cloud._ensure_browser_plugins_loaded` â€”
    ...
    def test_web_extract_tool_runs_discovery_before_registry_lookup(self, monkeypatch):
        ...
        try:
            ...
            # underlying ``panergos_cli.plugins._ensure_plugins_discovered`` so
...
class TestDisabledPluginDiagnostic:
    ...
    (they already set it correctly â€” the provider just isn't loaded).
    """
