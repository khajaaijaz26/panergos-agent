...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad PANERGOS_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: PANERGOS_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_panergos_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "PANERGOS_HOME": "/h", "PANERGOS_PROFILE": "p",
        "PANERGOS_CONFIG": "/c.yaml", "PANERGOS_ENV": "/e",
        "PANERGOS_DELEGATED_CHILD_CONTEXT": "1",
        # other PANERGOS_* → dropped (broad prefix removed)
        "PANERGOS_BASE_URL": "https://x", "PANERGOS_INTERACTIVE": "1",
        "PANERGOS_KANBAN_TASK": "t_parent",
    ...
    for kept in (
        ...
        "PANERGOS_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "PANERGOS_BASE_URL", "PANERGOS_INTERACTIVE", "PANERGOS_KANBAN_TASK",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped PANERGOS_* vars."""
    import logging
