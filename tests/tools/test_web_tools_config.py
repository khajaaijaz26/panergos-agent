...
Coverage:
  ...
  plugins.web.parallel.provider._get_sync_client() â€” Parallel client configuration, singleton caching.
...
class TestFirecrawlClientConfig:
    ...
    def test_no_config_raises_with_helpful_message(self):
        """Neither key nor URL raises with direct-credential guidance."""
    ...
    def test_explicit_firecrawl_config_without_creds_uses_keyless_client(self):
        ...
        with patch("tools.web_tools._load_web_config", return_value={"backend": "firecrawl"}), \
             patch("plugins.web.firecrawl.provider.Firecrawl", side_effect=AssertionError("SDK path should not run")):
            from plugins.web.firecrawl.provider import _get_firecrawl_client
            result = _get_firecrawl_client()
...
class TestBackendSelection:
    ...
    ``panergos tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def test_fallback_keenable_only_key(self):
        """Only KEENABLE_API_KEY set â†’ 'keenable'."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={}), \
    ...
    def test_fallback_no_keys_defaults_to_firecrawl(self):
        """No keys, no config, keyless tier off â†’ 'firecrawl' sentinel.

        With the keyless tier on (default), zero credentials resolves to
        the Parallel/Exa free tier instead â€” covered in
    ...
    def test_invalid_config_is_returned_verbatim(self):
        ...
        dispatch path raises the honest selection-naming error â€” never
    ...
    def test_stored_backend_wins_over_other_credentials(self):
        ...
        vendors â€” no availability probe, no credential override."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={"backend": "firecrawl"}), \
             patch.dict(os.environ, {"EXA_API_KEY": "exa-test"}):
            assert _get_backend() == "firecrawl"
...
class TestCheckWebApiKey:
    ...
    def setup_method(self):
        ...
        self._managed_patchers = [
            ...
            # fallback surfaces so this class only exercises direct env-key
            # resolution â€” otherwise these tests flip on machines where the
    ...
    def test_null_backend_value_does_not_crash(self):
        ...
        # must not raise AttributeError on None.lower() â€” mirrors _get_backend.
...
class TestNonBuiltinProviderAvailability:
    ...
    def test_capability_backend_honors_custom_extract_provider(self):
        ...
        custom provider when configured, instead of dead-ending â€” issue #32698."""
        with patch("tools.web_tools._ddgs_package_importable", return_value=False), \
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from panergos_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.panergos/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.panergos/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_keenable_search_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Keenable-Title"] == "panergos-agent"
