...
``panergos_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``PANERGOS_`` prefix was dropped
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            out[k] = v
            continue
        if k in _PANERGOS_CHILD_ALLOWED:
            out[k] = v
    return out
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # PANERGOS_* handling (#27303): only the operational allowlist passes;
        # every other PANERGOS_* is dropped (the broad prefix was removed).
        "PANERGOS_HOME": "/home/alice/.panergos",        # allowlisted → kept
        "PANERGOS_PROFILE": "default",                 # allowlisted → kept
        "PANERGOS_INTERACTIVE": "1",                   # not allowlisted → dropped
        "PANERGOS_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "PANERGOS_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``panergos_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``panergos_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        import tempfile, ast
        stub = generate_panergos_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_panergos_tools_module(["terminal"], transport="uds")
...
# let the child import panergos_tools, a user script that printed non-ASCII
