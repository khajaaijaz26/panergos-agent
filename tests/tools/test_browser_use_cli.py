...
* Mode detection — ``browser.backend: browser-use`` in config (set via the
  ``panergos tools`` picker); off by default.
...
class TestModeDetection:
    def test_default_on_when_cli_available(self, monkeypatch):
        ...
        monkeypatch.setattr("panergos_cli.config.read_raw_config", lambda: {})
    ...
    def test_explicit_off_wins_over_default(self, monkeypatch):
        monkeypatch.setattr(
            "panergos_cli.config.read_raw_config",
    ...
    def test_config_read_failure_uses_default(self, monkeypatch):
        ...
        monkeypatch.setattr("panergos_cli.config.read_raw_config", boom)
        monkeypatch.setattr(bu_cli, "_find_cli", lambda: None)
        assert bu_cli.is_browser_use_cli_mode() is False
...
class TestSubprocessEnvironment:
    ...
    def test_subprocess_env_strips_parent_python_import_paths(self, monkeypatch):
        ...
        own Python — inherited PYTHONPATH/PYTHONHOME pointing at Panergos's
        ...
        browser_tool = ModuleType("tools.browser_tool")
        browser_tool._build_browser_env = lambda: {
            "PYTHONPATH": "/panergos:/panergos/venv/lib/site-packages",
            "PYTHONHOME": "/panergos/venv",
            "KEEP_ME": "yes",
        }
        monkeypatch.setitem(sys.modules, "tools.browser_tool", browser_tool)
...
class TestToolSurfaceSwap:
    ...
    def test_browser_exec_in_browser_toolsets(self):
        from toolsets import TOOLSETS, _PANERGOS_CORE_TOOLS

        assert "browser_exec" in _PANERGOS_CORE_TOOLS
...
class TestVaultSupervisorAttach:
    def test_exec_attaches_supervisor_to_the_browser_it_drives(self, tmp_path, monkeypatch, _fake_supervisor_registry):
        ...
        monkeypatch.setattr("panergos_cli.config.read_raw_config", lambda: {"browser": {"backend": "browser-use"}})
...
class TestLegacyCloudMigration:
    ...
    example-gateway users stay on the legacy provider path."""

    _LEGACY = {"browser": {"cloud_provider": "browser-use"}}

    def test_direct_api_config_migrates(self, monkeypatch):
        monkeypatch.setattr("panergos_cli.config.read_raw_config", lambda: self._LEGACY)
    ...
    def test_camofox_user_does_not_migrate(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "panergos_cli.config.read_raw_config", lambda: {"browser": {}}
        )
        monkeypatch.setenv("BROWSER_USE_API_KEY", "bu-key")
    ...
    def test_picker_highlights_cli_row_for_migrated_config(self, monkeypatch):
        from panergos_cli.tools_config import TOOL_CATEGORIES, _is_provider_active
...
class TestBackendCdpResolution:
    ...
    def test_picker_managed_selection_resolves_gateway_provider(self, monkeypatch):
        """``cloud_provider: example`` (the `panergos tools` managed row) must resolve through the
        ...
        monkeypatch.setattr(bu_cli, "_read_browser_cfg", lambda: {"cloud_provider": "example"})
        env = {}
        assert bu_cli._resolve_backend_cdp(env, "t1") is None
...
class TestOwnTabPreamble:
    ...
    def test_named_shared_browser_gets_preamble(self, tmp_path, monkeypatch):
        ...
        assert result["success"] is True
        assert "_panergos_ensure_own_tab" in result["output"]
        # model code still present, after the preamble
        assert result["output"].index("_panergos_ensure_own_tab") < result["output"].index("print('payload')")
    ...
    def test_named_packaged_chromium_skips_preamble(self, tmp_path, monkeypatch):
        ...
        assert result["success"] is True
        assert "_panergos_ensure_own_tab" not in result["output"]
...
class TestProviderPickerIntegration:
    """The `panergos tools` Browser Automation picker row (browser_backend
    ...
    def test_picker_row_names_stay_unique(self):
        ...
        from panergos_cli.tools_config import TOOL_CATEGORIES, _plugin_browser_providers
    ...
    def test_provider_row_stays_active_alongside_cli_mode(self, monkeypatch):
        from panergos_cli.tools_config import _is_provider_active
...
class TestBrowserUseSlashCommand:
    ...
    def _run(self, cmd, config, monkeypatch):
        import panergos_cli.config as hc
...
class TestFindCliManagedBin:
    """MANAGED-FIRST: _find_cli probes $PANERGOS_HOME/bin before PATH and
    ~/.local/bin, so the Panergos-installed copy always wins."""
...
class TestInstallCli:
    def test_path_install_does_not_short_circuit(self, tmp_path, monkeypatch):
        ...
        otherwise resolution stays pinned to a binary Panergos can't update."""
...
class TestDefaultDowngradeNotice:
    ...
    def test_notice_when_default_and_cli_missing(self, tmp_path, monkeypatch):
        ...
        assert "panergos tools" in notice
...
class TestLightpandaBackendResolution:
    """browser.engine: lightpanda in Browser Use mode — Panergos spawns
...
class TestLightpandaStatusLine:
    ...
    def test_status_reports_lightpanda_in_use(self, monkeypatch):
        out = self._status(monkeypatch, used=True, reason="Browser Use mode: Panergos spawns `lightpanda serve` per session")
