...
class TestToolsetConsistency:
    ...
    def test_panergos_platforms_share_core_tools(self):
        """All panergos-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on panergos-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["panergos-cli", "panergos-telegram", "panergos-discord", "panergos-whatsapp", "panergos-slack", "panergos-signal", "panergos-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_panergos_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("panergos-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("panergos-cli")
        second = resolve_toolset("panergos-cli")
        ...
        assert (
            "panergos-cli", True, registry_id, generation, registry.current_scope_key()
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("panergos-cli", include_registry=False)
        second = resolve_toolset("panergos-cli", include_registry=False)
        assert first == second
