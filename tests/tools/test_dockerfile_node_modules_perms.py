...
Older images made /opt/panergos/ui-tui and /opt/panergos/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV PANERGOS_TUI_DIR=/opt/panergos/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R panergos:panergos /opt/panergos/ui-tui" not in text
    assert "chown -R panergos:panergos /opt/panergos/node_modules" not in text
