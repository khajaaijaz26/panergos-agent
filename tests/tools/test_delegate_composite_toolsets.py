...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_panergos_cli_expands_web(self):
        """panergos-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"panergos-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("panergos-cli", expanded)


    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['panergos-cli'] yields ['web']."""
        parent_toolsets = {"panergos-cli"}
