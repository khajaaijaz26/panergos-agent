...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("PANERGOS_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("PANERGOS_LOCAL_STT_LANGUAGE", raising=False)
...
class TestTranscribeLocalCommand:
    def test_command_provider_uses_sanitized_child_env(self, monkeypatch):
        """Salvage of #56332: command STT must not inherit Panergos secrets."""
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("PANERGOS_LOCAL_STT_LANGUAGE", "en")
...
def mock_xai_http_module():
    ...
    fake_module = MagicMock()
    fake_module.panergos_xai_user_agent = MagicMock(return_value="panergos-xai/test")
...
class TestTranscribeXAI:
    ...
    def test_sends_language_and_format(self, monkeypatch, sample_ogg, mock_xai_http_module):
        ...
        monkeypatch.setenv("PANERGOS_LOCAL_STT_LANGUAGE", "fr")
...
class TestExtractTranscriptText:
    def test_strips_qwen3_asr_language_envelope(self):
        ...
        result = _extract_transcript_text(
            "language zh\n<audio_language>zh</audio_language>\n<asr_text>ä½ å¥½ï¼Œä¸–ç•Œ",
        )

        assert result == "ä½ å¥½ï¼Œä¸–ç•Œ"
...
class TestLocalModelLock:
    """#24767 â€” concurrent first-use must not double-load the whisper model."""
...
class TestLocalBaseUrlNoApiKey:
    """#25193 â€” empty api_key with a local base_url should not raise."""
...
class TestTranscribeCredentialReadGuard:
    ...
    def test_transcribe_audio_blocks_credential_read(self, tmp_path):
        ...
        plaintext is never shipped to an external STT provider â€” mirroring the
        ...
        # or provider error â€” proving the guard fired before dispatch.
