...
class TestWriteDenyExactPaths:
    ...
    def test_panergos_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.panergos/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "panergos_root"
        ...
        # Sanity check: PANERGOS_HOME does point to the profile dir, not the root.
        from panergos_constants import get_panergos_home, get_default_panergos_root
...
class TestWriteAllowed:
    ...
    def test_panergos_control_files_requested_writable(self):
