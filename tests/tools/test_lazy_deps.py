...
class TestSecurityGating:
    ...
    def test_config_failure_fails_open(self, monkeypatch):
        ...
        monkeypatch.delenv("PANERGOS_DISABLE_LAZY_INSTALLS", raising=False)
        monkeypatch.setattr(
...
class TestIsSatisfiedVersionAware:
    ...
    def test_trace_upload_hub_at_core_locked_version_is_current(self, monkeypatch):
        ...
        core installs satisfies the trace-upload spec, so the `panergos update`
...
# ---------------------------------------------------------------------------
# active_features + refresh_active_features (Piece A — panergos update wiring)
# ---------------------------------------------------------------------------
...
class TestActiveFeatures:
    ...
    def test_shared_dependency_does_not_activate_feature(self, monkeypatch):
        ...
        # reasons. It must not make panergos update try to refresh Matrix unless
...
class TestRefreshActiveFeatures:
    ...
    def test_windows_matrix_refresh_is_skipped_before_pip(self, monkeypatch):
        ...
        # path. `panergos update` must not retry that doomed install every run.
        #
...
class TestInstallSpecs:
    ...
    def test_never_raises_on_unexpected_error(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "panergos_cli.config.load_config", lambda: {}, raising=False
        )
...
class TestInstallWarmsBytecode:
    ...
    def _install(monkeypatch, returncode):
        ...
        monkeypatch.setattr(
            "panergos_cli.managed_uv.resolve_uv", lambda *a, **kw: "uv", raising=False
        )
