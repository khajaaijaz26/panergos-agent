...
from tools.mcp_oauth import PanergosTokenStorage  # noqa: E402
...
def test_legacy_file_without_issuer_adopts_current_issuer_once(tmp_path):
    ...
    assert json.loads(_token_file(tmp_path).read_text())["panergos_issuer"] == "https://as.example.com"
    # Now bound: a later issuer change is rejected.
    storage2 = PanergosTokenStorage("srv", panergos_home=tmp_path)
...
def test_set_tokens_stamps_bound_issuer_and_get_tokens_keeps_it_out_of_the_sdk_model(tmp_path):
    ...
    assert json.loads(_token_file(tmp_path).read_text())["panergos_issuer"] == "https://as.example.com"
    fresh = PanergosTokenStorage("srv", panergos_home=tmp_path)
    tokens = asyncio.run(fresh.get_tokens())
    assert fresh.loaded_issuer == "https://as.example.com"
    assert not hasattr(tokens, "panergos_issuer")  # never leaks into the wire model
