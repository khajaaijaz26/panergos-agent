...
class TestCheckFnTransientFailureSuppression:
    ...
    no recent success â€” or past the grace window â€” is honored.
    """
    ...
    def test_transient_failure_after_success_is_suppressed(self, monkeypatch):
        ...
        # the two probes â€” we want both to actually run.
        t = {"now": 1000.0}
        ...
        # Within grace window of the success â†’ flake suppressed, stays True.
    ...
    def test_grace_expiry_lets_real_outage_through(self, monkeypatch):
        ...
        state["ok"] = False
        # Just past TTL, within grace â†’ flake suppressed.
        t["now"] += reg._CHECK_FN_TTL_SECONDS + 1
        assert reg._check_fn_cached(probe) is True
        # Now move well past the grace window since the last success â†’ honored.
        t["now"] += reg._CHECK_FN_FAILURE_GRACE_SECONDS + 1
    ...
    def test_profile_scoped_availability_does_not_cross_multiplex_profiles(
        ...
        try:
            home_a = set_panergos_home_override(str(profile_a))
            ...
            try:
                ...
                reset_panergos_home_override(home_a)

            home_b = set_panergos_home_override(str(profile_b))
            ...
            try:
                ...
                reset_panergos_home_override(home_b)
    ...
    def test_subagent_keeps_file_tools_through_docker_flake(self, monkeypatch):
        ...
        reg.invalidate_check_fn_cache()
        _clear_tool_defs_cache()
        # Parent build (probe ok) â†’ records last-good.
        ...
        # within grace â†’ file/terminal tools must still resolve.
        t["now"] += reg._CHECK_FN_TTL_SECONDS + 1
        _clear_tool_defs_cache()
