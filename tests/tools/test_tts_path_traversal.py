...
def test_output_path_rejects_panergos_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    panergos_home = tmp_path / "panergos-home"
    panergos_home.mkdir()
    monkeypatch.setattr(file_safety, "_panergos_home_path", lambda: panergos_home)
    monkeypatch.setattr(file_safety, "_panergos_root_path", lambda: panergos_home)

    target = panergos_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    panergos_home = tmp_path / "panergos-home"
    token_dir = panergos_home / "mcp-tokens"
