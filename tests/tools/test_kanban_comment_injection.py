...
def worker_home(tmp_path, monkeypatch):
    ...
    for var in ("PANERGOS_KANBAN_DB", "PANERGOS_KANBAN_WORKSPACES_ROOT", "PANERGOS_KANBAN_HOME", "PANERGOS_KANBAN_BOARD"):
...
def test_seed_then_inject_new_comment(worker_home, monkeypatch):
    ...
    monkeypatch.setenv("PANERGOS_PROFILE", "worker-bot")
    agent = FakeAgent()
