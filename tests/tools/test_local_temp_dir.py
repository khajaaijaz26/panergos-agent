...
Panergos exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_panergos_cache_not_tmp(tmp_path, monkeypatch):
    """With no overrides at all, the default temp root is real storage under
    PANERGOS_HOME (cache/terminal), NOT tmpfs /tmp."""
    import panergos_constants  # noqa: F401 — resolves PANERGOS_HOME per call
    ...
    assert result == str(tmp_path / ".panergos" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "panergos_bg_dead1.log").exists()
