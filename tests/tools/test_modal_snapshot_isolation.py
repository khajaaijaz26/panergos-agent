...
def _restore_tool_modules():
    original_panergos_home = os.environ.get("PANERGOS_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "panergos_cli"
        or name.startswith("panergos_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_panergos_home is None:
            os.environ.pop("PANERGOS_HOME", None)
        else:
            os.environ["PANERGOS_HOME"] = original_panergos_home
        _reset_modules(("tools", "panergos_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    panergos_cli = types.ModuleType("panergos_cli")
    panergos_cli.__path__ = [str(REPO_ROOT / "panergos_cli")]  # type: ignore[attr-defined]
    sys.modules["panergos_cli"] = panergos_cli
    panergos_home = tmp_path / "panergos-home"
    os.environ["PANERGOS_HOME"] = str(panergos_home)
    sys.modules["panergos_cli.config"] = types.SimpleNamespace(
        get_panergos_home=lambda: panergos_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="panergos-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": panergos_home / "modal_snapshots.json",
