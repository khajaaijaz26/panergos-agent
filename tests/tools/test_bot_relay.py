    ...
    return [
        {
            ...
            "connection_label": "Hosted gateway",
            "title": "Moxie",
...
def test_cli_resolution_prefers_sibling_and_uses_path_fallback(tmp_path, monkeypatch):
    bin_dir = tmp_path / "bin"
    bin_dir.mkdir()
    suffix = ".exe" if bot_relay.sys.platform == "win32" else ""
    canonical = bin_dir / f"panergos{suffix}"
    path_fallback = bin_dir / f"path-panergos{suffix}"
    canonical.touch()
    monkeypatch.setattr(bot_relay.sys, "executable", str(bin_dir / f"python{suffix}"))
    monkeypatch.setattr(bot_relay.shutil, "which", lambda name: str(path_fallback))

    assert bot_relay._panergos_cli() == str(canonical)
    canonical.unlink()
    assert bot_relay._panergos_cli() == str(path_fallback)
...
def test_roster_roundtrip_and_validation(root):
    rows = _rows() + [
        ...
        "not-a-dict",
        {"profile": "default", "handle": "panergos", "connection_id": "cloud-1"},  # dupe
    ]
...
def test_resolve_remote_target_forms(root):
    ...
    assert bot_relay.resolve_remote_target("@panergos", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("panergos@cloud-1", roster)["profile"] == "default"
    ...
    assert bot_relay.resolve_remote_target("panergos@nope", roster) is None
...
def test_resolve_ambiguous_handle_across_connections(root):
    ...
    assert "panergos" in forms  # unique handle stays bare
...
def test_relay_route_queues_envelope_and_spawns_waiter(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "panergos", "connection_id": "cloud-1",
         "connection_label": "Hosted gateway", "title": "Moxie"},
    ])
    ...
    agent = _FakeAgent(home)
    out = json.loads(message_agent_tool(target="panergos", message="ping", agent=agent))
    assert out.get("status") == "sent"
    assert "Hosted gateway" in spawned["label"]
    ...
    assert pending[0]["message"].startswith("Message from 🤖 panergos (@panergos): ping")
...
def test_protocol_section_lists_remote_teammates(tmp_path):
    ...
    assert "`@panergos` — on Hosted gateway — Moxie" in section
...
def test_cleanup_bot_relay_artifacts_sweeps_stale_plaintext(tmp_path, monkeypatch):
    ...
    stale_env = bot_relay.enqueue_envelope(
        ...
        sender_profile="default", sender_handle="panergos",
    )
...
def test_cleanup_bot_relay_artifacts_missing_dir_is_zero(tmp_path, monkeypatch):
    monkeypatch.setenv("PANERGOS_HOME", str(tmp_path / "nope"))
...
def test_ttl_config_read_is_lazy_and_defensive(monkeypatch):
    ...
    def _boom(name, *a, **k):
        if name.startswith("panergos_cli"):
...
def test_message_agent_surfaces_runtime_offline_refusal(tmp_path, monkeypatch):
    ...
    bot_relay.write_remote_roster(home, [
        {"profile": "default", "handle": "panergos", "connection_id": "cloud-1",
         "connection_label": "Hosted gateway", "online": False},
    ])
    monkeypatch.setattr(
        "tools.bot_mode_dm._spawn_delivery",
        lambda *a, **k: json.dumps({"status": "sent"}),
...
# ── delivery turn author (PANERGOS_TURN_AUTHOR on the recipient turn) ──────────
...
def test_delivery_env_carries_only_the_given_author(monkeypatch):
    """The dispatcher's own PANERGOS_TURN_AUTHOR never reaches the child: dropped without an author, replaced with one."""
    from agent.turn_author import TURN_AUTHOR_ENV

    monkeypatch.setenv("PANERGOS_RELAY_TEST_MARKER", "kept")
    monkeypatch.setenv(TURN_AUTHOR_ENV, json.dumps({"id": "bot:previous", "name": "previous", "is_bot": True}))
    monkeypatch.setenv("PANERGOS_SESSION_KEY", "session-A")
    monkeypatch.setenv("PANERGOS_UI_SESSION_ID", "ui-A")
    monkeypatch.setenv("PANERGOS_SESSION_ID", "session-A")
    monkeypatch.setenv("PANERGOS_SESSION_PROFILE", "profile-A")
    # A session-* knob, not identity: stripping it would break the child's watcher tuning.
    monkeypatch.setenv("PANERGOS_SESSION_STALL_TIMEOUT", "97")
    ...
    assert env["PANERGOS_RELAY_TEST_MARKER"] == "kept"
    assert "PANERGOS_SESSION_KEY" not in env
    assert "PANERGOS_UI_SESSION_ID" not in env
    assert "PANERGOS_SESSION_ID" not in env
    assert "PANERGOS_SESSION_PROFILE" not in env
    assert env["PANERGOS_SESSION_STALL_TIMEOUT"] == "97"
