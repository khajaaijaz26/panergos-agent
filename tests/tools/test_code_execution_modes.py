...
from tools.code_execution_env import (
    ...
    _uses_panergos_python_environment,
)
...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        path stages in panergos_sandbox_*, the session kernel in
        panergos_kernel_* — either satisfies strict mode's isolation promise."""
        result = self._run("import os; print(os.getcwd())", mode="strict")
        self.assertEqual(result["status"], "success")
        cwd = result["output"].strip()
        self.assertTrue(
            "panergos_sandbox_" in cwd or "panergos_kernel_" in cwd,
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The panergos-agent venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_panergos_tools(self):
        """Regression: panergos_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix — without it, switching to session CWD
        breaks `from panergos_tools import terminal`.
        """
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            code = (
                "from panergos_tools import terminal\n"
                "r = terminal('echo x')\n"
    ...
    def test_strict_mode_can_still_import_panergos_tools(self):
...
# ---------------------------------------------------------------------------
# _python_environment_prefix / _uses_panergos_python_environment
# ---------------------------------------------------------------------------
...
class TestPythonEnvironmentPrefix(unittest.TestCase):
    ...
    def test_failure_is_not_cached(self):
        ...
        A sticky cached failure would silently drop the panergos root from
...
class TestUsesPanergosPythonEnvironment(unittest.TestCase):
    """Unit tests for _uses_panergos_python_environment."""
    ...
    def test_true_for_current_interpreter(self):
        ...
        self.assertTrue(_uses_panergos_python_environment(sys.executable))
    ...
    def test_true_for_current_interpreter_without_probe(self):
        ...
        must never drop the panergos root for the interpreter Panergos itself runs.
        """
        with patch("subprocess.run",
                   side_effect=subprocess.TimeoutExpired(cmd=[], timeout=5)) as mock_run:
            self.assertTrue(_uses_panergos_python_environment(sys.executable))
    ...
    def test_false_for_different_prefix(self):
        ...
        with patch("tools.code_execution_env._python_environment_prefix",
            ...
            self.assertFalse(_uses_panergos_python_environment("/other/python"))

    def test_false_when_prefix_is_empty(self):
        """If prefix cannot be determined (error path), treat as external."""
        with patch("tools.code_execution_env._python_environment_prefix",
                   return_value=""):
            self.assertFalse(_uses_panergos_python_environment("/bad/python"))

    def test_true_when_prefix_matches_sys_prefix(self):
        panergos_prefix = os.path.realpath(sys.prefix)
        with patch("tools.code_execution_env._python_environment_prefix",
                   return_value=panergos_prefix):
            self.assertTrue(_uses_panergos_python_environment("/same/env/python"))


# ---------------------------------------------------------------------------
# PYTHONPATH composition — panergos root included only for same-env interpreters
# ---------------------------------------------------------------------------

class TestPythonPathComposition(unittest.TestCase):
    """Verify panergos root inclusion in PYTHONPATH depends on env match.

    Patches ``_uses_panergos_python_environment`` directly so these tests are
    ...
    def _capture_pythonpath(self, same_env: bool) -> tuple:
        ...
        def _fake_popen(cmd, **kwargs):
            ...
            # [python, <staging_dir>/panergos_kernel_runner.py] (session
    ...
    def test_panergos_root_included_when_same_env(self):
        """When interpreter is in the Panergos env, panergos root is in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=True)
        parts = pythonpath.split(os.pathsep)
        self.assertIn(self._panergos_root(), parts,
                      "panergos root must be in PYTHONPATH for same-env interpreters")

    def test_panergos_root_excluded_when_external_env(self):
        """When interpreter is external, panergos root must NOT be in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=False)
        parts = pythonpath.split(os.pathsep)
        self.assertNotIn(self._panergos_root(), parts,
                         "panergos root must not leak into an external interpreter's PYTHONPATH")
