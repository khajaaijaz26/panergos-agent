# Panergos Plugin Catalog

Curated Panergos plugins. Each YAML file in this directory
(except `removed.yaml`) is one catalog entry, discoverable via
`panergos plugins catalog` / `panergos plugins search` and installable with
...
1. **Human-merged gate.** Entries are added *only* via a PR to the
   `panergos-agent` repository, reviewed and merged by a maintainer. There is
...
3. **Pin maturity.** The pinned release should be **at least 2 weeks old**
   ...
   compromised release before Panergos ships a pointer to it.
...
requires_panergos: ">=0.19"   # optional
docs_url: ""                # optional
