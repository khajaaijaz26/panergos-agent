...
_PANERGOS_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `panergos-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _PANERGOS_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_PANERGOS_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _PANERGOS_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `panergos tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``panergos tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _PANERGOS_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "project_memory": _ts("Incremental project index and durable coding handoff", ["project_memory"]),
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (PANERGOS_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _PANERGOS_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in panergos_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Panergos toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. panergos-acp is the
    ...
    "panergos-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors panergos-cli; `panergos tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "panergos-cron": _bundle("Default cron toolset - same core tools as Panergos; gated by `panergos tools`"),
    "panergos-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "panergos-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "panergos-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "panergos-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "panergos-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "panergos-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "panergos-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "panergos-email": _bundle("Email bot toolset - interact with Panergos via email (IMAP/SMTP)"),
    "panergos-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "panergos-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "panergos-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "panergos-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "panergos-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "panergos-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "panergos-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "panergos-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "panergos-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _PANERGOS_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "panergos-sms": _bundle("SMS bot toolset - interact with Panergos via SMS (Twilio)"),
    "panergos-webhook": _ts("Webhook toolset - receive and process external webhook events", _PANERGOS_WEBHOOK_SAFE_TOOLS),
    "panergos-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "panergos-telegram", "panergos-discord", "panergos-whatsapp", "panergos-slack",
            "panergos-signal", "panergos-bluebubbles", "panergos-homeassistant", "panergos-email",
            "panergos-sms", "panergos-mattermost", "panergos-matrix", "panergos-dingtalk",
            "panergos-feishu", "panergos-wecom", "panergos-wecom-callback", "panergos-weixin",
            "panergos-qqbot", "panergos-webhook", "panergos-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _PANERGOS_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only panergos-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_PANERGOS_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `panergos-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("panergos-"):
        return []
    platform_name = name[len("panergos-"):]
    try:
        ...
        return []
    tools = set(_PANERGOS_CORE_TOOLS)
    try:
