"""Profile-scoped reads of the ``PANERGOS_*`` tuning settings cron honours from ``.env``.

A standalone ``panergos -p X gateway run`` loads X's ``.env`` into ``os.environ``, so a bare
``os.getenv("PANERGOS_CRON_TIMEOUT")`` is X's value. Under ``gateway.multiplex_profiles`` the same
...
def cron_env_setting(name: str, default: str = "") -> str:
    ...
    if scope is None:
        scope = load_env_file(get_panergos_home() / ".env")
