...
A cron job that restarts/stops the gateway from inside the gateway (``panergos gateway restart``,
``launchctl kickstart ai.panergos.gateway``, ``systemctl restart panergos-gateway``) kills the process,
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive Panergos gateway ops. `start` is excluded: starting from inside a
    # gateway is benign and a job may legitimately start a sibling profile. The lookbehind keeps
    # `panergos` from being a path component or word tail (`/docs/panergos gateway restart-notes.md`)
    # while every real command position (text start, whitespace, `;`/`&`/`|`, `$(`, backtick,
    # U+FFFD) still matches.
    # See #77173.
    r"(?:(?<![/\w.\-])panergos\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops anchored on a panergos-gateway label so unrelated panergos services stay
    ...
    # -l ai.panergos.gateway-<suffix> -- <helper-script>` (or `launchctl bootstrap gui/<uid> <plist>`) creates
    ...
    # Branch C: systemctl ops on a panergos-gateway unit.
...
# split across continuation lines (e.g. `launchctl submit \` / `  -l ai.panergos.gateway-... \` / `  -- ...`,
...
# Branch A2: `panergos -p <profile> gateway restart|stop` (also `--profile <name>` /
...
# `panergos -p venus gateway restart` operates on a sibling profile's gateway and is a legitimate fleet
...
# `$label` next to the verb. These verbs act on an EXISTING job, so the panergos-gateway label anchor
...
_PANERGOS_GATEWAY_LABEL_RE = re.compile(r"(?i)\bpanergos[.\-]?gateway\b")
...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard: ``PANERGOS_PROFILE_NAME``/``PANERGOS_PROFILE`` env first, then
    ``panergos_cli.profiles.get_active_profile_name`` (from ``PANERGOS_HOME``); ``None`` if neither."""
    for env_name in ("PANERGOS_PROFILE_NAME", "PANERGOS_PROFILE"):
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _PANERGOS_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    ``panergos``/``gateway`` token) — this function is the single choke point
    ...
    # <<'EOF'`) are masked before scanning (#88336): a runbook line "a human can run: panergos gateway
    ...
    # Profile-flag form (#78028): `panergos -p <profile> gateway restart|stop` bypasses Branch A because the
    # selector sits between `panergos` and `gateway`. It is only the same foot-gun when the named profile IS
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<PANERGOS_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_panergos_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_panergos_home() falls back to Path.home(), which raises when neither PANERGOS_HOME nor
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return "panergos gateway restart"
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str] = None) -> None:
    ...
    if script:
        ...
        if resolved_script is not None and _on_cloud_path(resolved_script):
            raise GatewayLifecycleBlocked(
                ...
                "(e.g. ~/.panergos/scripts/) and recreate the job."
            )
    ...
    if python_script:
        ...
        # The direct command regex below still scans the full text, so a literal `panergos gateway restart`
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `panergos gateway restart` from a shell outside "
