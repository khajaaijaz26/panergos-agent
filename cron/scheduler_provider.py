...
def _existing_profile_homes(profile_homes: list) -> list:
    ...
    back on disk and in ``panergos profile list`` (#47368). Filtering on directory existence leaves a deleted
...
class CronScheduler(ABC):
    ...
    @abstractmethod
    def name(self) -> str:
        """Short identifier, e.g. ``builtin`` or a plugin name."""
...
def fire_overdue_jobs(
    ...
    from cron.jobs import (
        ONESHOT_GRACE_SECONDS, _ensure_aware, _panergos_now, is_job_runnable, load_jobs,
    )
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        # the process-global PANERGOS_HOME (the default profile) is ticked. Heartbeats and recovery are also
        # scoped per profile so `panergos cron status` reflects liveness for every profile independently.
        ...
        # Heartbeat before the first sleep so `panergos cron status` sees a live ticker immediately.
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # Persist the reason so `panergos cron status` (separate process) shows WHY.
