...
schedule via the `cronjob` tool; users via `panergos cron list|add|edit|pause|resume|run|remove` or
...
- File lock `~/.panergos/cron/.tick.lock` prevents duplicate ticks across processes.
...
- The cron ticker runs in the desktop-spawned backend when `PANERGOS_DESKTOP=1` — that env var means
...
Durable SQLite-backed board letting multiple profiles/workers collaborate. Users: `panergos kanban
...
- **CLI:** `panergos_cli/kanban.py` facade + 14 `kanban_*.py` siblings (`boards`, `db`, `db_connect`,
...
- **Toolset:** `tools/kanban_tools.py` — `kanban_show, kanban_complete, kanban_request_review,
  ...
  `kanban` (`panergos tools enable kanban --platform <p>`; default-off, in `CONFIGURABLE_TOOLSETS`) get
...
- **Dispatcher:** long-lived loop (default 60s) that reclaims stale claims, promotes ready tasks,
  ...
  (`kanban.dispatch_in_gateway: true`). Standalone: `plugins/kanban/systemd/panergos-kanban-dispatcher.service`.
...
Isolation: **board** is the hard boundary — workers get `PANERGOS_KANBAN_BOARD` pinned in their env and
...
`tests/cron/`, `tests/panergos_cli/test_kanban*.py`, `tests/tools/test_kanban*.py`. Schedule parsing
