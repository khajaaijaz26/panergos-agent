...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"Panergos — {job.get('name') or job.get('id', 'cron')}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from panergos_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    try:
        source_home = get_panergos_home().resolve()
    ...
    if panergos_bin:
        ...
        try:
            ...
            found = _ilu.find_spec("panergos_cli") is not None
        except Exception:
            found = False
        if not found:
            return "bot-chat delivery failed: Panergos CLI not resolvable"
        argv = [sys.executable, "-m", "panergos_cli.main"]
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution; this scheduler's PANERGOS_HOME must not shadow it.
        env.pop("PANERGOS_HOME", None)
    else:
        # Multiplex workers carry the profile in a ContextVar, not os.environ.
        env["PANERGOS_HOME"] = str(source_home)

    query_file = None
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="panergos-cron-botchat-", delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _deliver_result(
    ...
    # job's own attempt: a worker's script may dispatch another job in-process (`panergos cron run`),
    # and that nested delivery must not be keyed under the outer execution id.
    external_execution = os.environ.get("_PANERGOS_CRON_EXTERNAL_WORKER", "")
    ...
    # persisted as ``last_delivery_unverified`` so `panergos cron list` shows it.
    ...
    # at boot; standalone runs (`panergos cron run`) did not, silently dropping files. Idempotent.
