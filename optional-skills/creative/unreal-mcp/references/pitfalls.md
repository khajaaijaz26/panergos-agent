...
### 1. Start order: editor first, Panergos second

Panergos probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Panergos session.
...
restart the editor, restart the Panergos session.
...
server fails to bind (Output Log) or Panergos' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.panergos/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Panergos

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Panergos' connection
lives in `~/.panergos/config.yaml` via `panergos mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Panergos — just don't
mistake it for the Panergos setup step.
...
explicitly warns against overlapping calls. Panergos executes same-turn tool
...
### 9. Timeouts: Panergos gives up before Unreal does

Panergos' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Panergos runs elsewhere (SSH backend,
