...
Maps each role archetype to the Panergos skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public Panergos Agent
...
## Panergos skills relevant to video production

### Visual / rendering skills (`panergos-agent/skills/creative/`)
...
| `touchdesigner-mcp` (via the `touchdesigner` plugin: `panergos plugins install touchdesigner`) | Control a running TouchDesigner instance — real-time visuals, audio-reactive installation art, VJ | Renderer for real-time/audio-reactive content; installation art; live performance |
...
### Design / pre-production skills (`panergos-agent/skills/creative/`)
...
### Audio / media skills (`panergos-agent/skills/creative/` + `skills/media/`)
...
These are **not** Panergos skills but external CLIs / APIs that profiles invoke.
...
## Built-in Panergos tools for media review

These are native Panergos tools — not invoked via terminal but through their own
...
key is present in `${PANERGOS_HOME:-~/.panergos}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Panergos profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${PANERGOS_HOME:-~/.panergos}/.env` → environment variable.
...
When a new Panergos-public video skill ships:
