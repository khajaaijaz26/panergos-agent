...
panergos kanban watch --tenant <project-slug>

# Snapshot of the board
panergos kanban list --tenant <project-slug>
panergos kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
panergos kanban stats --tenant <project-slug>

# Visual dashboard (browser)
panergos dashboard

# Inspect a specific task (includes comments + events)
panergos kanban show <task-id>

# Follow a single task's event stream
panergos kanban tail <task-id>
```

Verify available subcommands with `panergos kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `panergos kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `panergos kanban block <id>` then `panergos kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `panergos kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `panergos kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `panergos profile list` (profile exists?), `panergos status` (gateway/dispatcher up?), and verify tenant. |
...
panergos kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
panergos kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(panergos kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
panergos kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
panergos kanban block <task-id>
panergos kanban archive <task-id>

# Diagnose what happened
panergos kanban show <task-id>      # task body, comments, recent events
panergos kanban tail <task-id>      # follow the live event stream
panergos kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `panergos kanban create` for the director
...
while true; do
    clear
    panergos kanban list --tenant <slug>
    echo "---"
    panergos kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `panergos kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `panergos profile list` and start
