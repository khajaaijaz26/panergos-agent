...
the actual `setup.sh` that creates the project workspace, configures Panergos
...
The setup uses Panergos' single-project workspace, profile configuration,
SOUL.md-per-profile, and `--workspace dir:<path>` conventions.
...
2. **Create profiles** — `panergos profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.panergos/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `panergos kanban create` assigned to the director
...
```bash
panergos profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.panergos/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.panergos/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Panergos
...
Each profile gets a `SOUL.md` at `~/.panergos/profiles/<name>/SOUL.md` that
...
```bash
panergos kanban create "Direct production of <video title>" \
    --assignee director \
...
the Panergos `.env` (`${PANERGOS_HOME:-$HOME/.panergos}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _panergos_env="${PANERGOS_HOME:-$HOME/.panergos}/.env"
    if grep -q "^${var}=" "$_panergos_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_panergos_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_panergos_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY panergos ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY panergos OPENROUTER_API_KEY || exit 1
# ...
```
