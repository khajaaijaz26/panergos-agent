...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Panergos uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Panergos has `image_generate`
...
- License: MIT (retained for this component)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Panergos Agent contributors
