...
The design is **Panergos-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
panergos skills install official/security/unbroker
```

Then start a new Panergos session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Panergos session:
...
- **PII handling.** Dossiers live under the Panergos home directory (`0600`, optionally
