...
All per-subject data lives under PDD_DATA_DIR (default: $PANERGOS_HOME/unbroker),
which is the same trust boundary Panergos uses for .env and OAuth tokens.
"""
...
def panergos_home() -> Path:
    return Path(os.environ.get("PANERGOS_HOME") or (Path.home() / ".panergos"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else panergos_home() / "unbroker"
