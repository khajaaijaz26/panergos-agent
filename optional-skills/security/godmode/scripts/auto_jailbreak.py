...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("PANERGOS_HOME", os.path.expanduser("~/.panergos")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("PANERGOS_HOME", Path.home() / ".panergos")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Panergos config paths
# ═══════════════════════════════════════════════════════════════════

PANERGOS_HOME = Path(os.getenv("PANERGOS_HOME", Path.home() / ".panergos"))
CONFIG_PATH = PANERGOS_HOME / "config.yaml"
PREFILL_PATH = PANERGOS_HOME / "prefill.json"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Panergos config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.panergos/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Panergos for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Panergos for changes to take effect.")
