...
| Panergos | No jailbreak needed | Apply the configured model's own safety policy |
...
8. **System prompt vs ephemeral system prompt** — The `agent.system_prompt` in config.yaml is appended AFTER Panergos' own system prompt. It doesn't replace the default prompt; it augments it. This means the jailbreak instructions coexist with Panergos' normal personality.
