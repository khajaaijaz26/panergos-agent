...
Usage:
  ...
  <server_name> matches the files in $PANERGOS_HOME/mcp-tokens/<server>.json etc.
...
def _panergos_home():
    # Prefer Panergos' own resolver (profile-safe); fall back to env then ~/.panergos.
    try:
        from panergos_constants import get_panergos_home
        return str(get_panergos_home())
    except Exception:
        return os.environ.get("PANERGOS_HOME") or os.path.expanduser("~/.panergos")


def _tokens_dir():
    return os.path.join(_panergos_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "panergos-diag", "version": "1.0"}},
        },
        headers={
