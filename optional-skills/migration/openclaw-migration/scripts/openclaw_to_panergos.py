#!/usr/bin/env python3
"""OpenClaw -> Panergos migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Panergos Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Panergos.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Panergos memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Panergos memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Panergos-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Panergos-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Panergos command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.panergos/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.panergos/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Panergos .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Panergos .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Panergos .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Panergos .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Panergos .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Panergos config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Panergos config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Panergos.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Panergos MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Panergos config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'panergos cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Panergos config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "full-providers": {
        ...
        "description": "Import custom model providers (baseUrl, apiType, headers) into Panergos custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Panergos config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Panergos config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Panergos config.yaml approvals section.",
    },
    "memory-backend": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `panergos config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`panergos config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.panergos/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Panergos config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Panergos in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Panergos`` and ``openclaw`` → ``panergos``
# in prose. Hidden-directory paths keep the compatible ``.panergos`` storage name.
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Panergos'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Panergos'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Panergos'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Panergos`` but maps lowercase prose to ``panergos``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Panergos.

    Preserves case so filesystem-path matches (lowercase) don't become
    capitalized directory names that don't exist.
    """
    text = re.sub(r"(?<=\.)openclaw\b", "panergos", text, flags=re.IGNORECASE)
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION Panergos memory store (memories/MEMORY.md, USER.md).
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Panergos Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-panergos/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Panergos config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-panergos/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Panergos session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Panergos config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Panergos
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Panergos-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Panergos-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: panergos config set",
                        )
                    continue
        ...
        for oc_key, panergos_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and panergos_key not in secret_additions:
                secret_additions[panergos_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, panergos_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and panergos_key not in secret_additions:
                        secret_additions[panergos_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        panergos_config = load_yaml_file(destination)
        current_model = panergos_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = panergos_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                panergos_config["model"] = {"default": model_str}
            dump_yaml_file(destination, panergos_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Panergos still uses "edge"
        ...
        panergos_config = load_yaml_file(destination)
        existing_tts = panergos_config.get("tts", {})
        ...
        if self.execute:
            ...
            panergos_config["tts"] = merged_tts
            dump_yaml_file(destination, panergos_config)
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Panergos destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Panergos-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Panergos-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        panergos_cfg_path = self.target_root / "config.yaml"
        panergos_cfg = load_yaml_file(panergos_cfg_path)
        existing_mcp = panergos_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Panergos config")
                continue

            panergos_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                panergos_srv["command"] = srv["command"]
                if srv.get("args"):
                    panergos_srv["args"] = srv["args"]
                if srv.get("env"):
                    panergos_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    panergos_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                panergos_srv["url"] = srv["url"]
                if srv.get("headers"):
                    panergos_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    panergos_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                panergos_srv["enabled"] = False
            if srv.get("timeout"):
                panergos_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                panergos_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                panergos_srv["tools"] = {}
                if tools_cfg.get("include"):
                    panergos_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    panergos_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                panergos_srv["sampling"] = {
                    k: v for k, v in {
            ...
            existing_mcp[name] = panergos_srv
            added += 1
        ...
        if added > 0 and self.execute:
            self.maybe_backup(panergos_cfg_path)
            panergos_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(panergos_cfg_path, panergos_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'panergos cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = panergos_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Panergos reasoning_effort
        ...
        if compaction:
            compression = panergos_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = panergos_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            panergos_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = panergos_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'panergos gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("PANERGOS_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if session.get("reset") or session.get("resetTriggers") or session.get("reset_triggers"):
            self.record("session-config", "session reset timers", None, "skipped",
                        "Panergos conversations do not reset on idle or daily timers")
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        panergos_cfg_path = self.target_root / "config.yaml"
        panergos_cfg = load_yaml_file(panergos_cfg_path)
        custom_providers = panergos_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(panergos_cfg_path)
            panergos_cfg["custom_providers"] = custom_providers
            dump_yaml_file(panergos_cfg_path, panergos_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Panergos config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            panergos_cfg_path = self.target_root / "config.yaml"
            panergos_cfg = load_yaml_file(panergos_cfg_path)
            discord_panergos = panergos_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_panergos["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_panergos["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                panergos_cfg["discord"] = discord_panergos
                dump_yaml_file(panergos_cfg_path, panergos_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        panergos_cfg_path = self.target_root / "config.yaml"
        panergos_cfg = load_yaml_file(panergos_cfg_path)
        browser_panergos = panergos_cfg.get("browser") or {}
        changed = False

        # Map fields that have Panergos equivalents
        if browser.get("cdpUrl"):
            browser_panergos["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_panergos["headless"] = browser["headless"]
            changed = True

        if changed:
            panergos_cfg["browser"] = browser_panergos
            if self.execute:
                self.maybe_backup(panergos_cfg_path)
                dump_yaml_file(panergos_cfg_path, panergos_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            panergos_mode = mode_map.get(mode, "manual")
            panergos_cfg.setdefault("approvals", {})["mode"] = panergos_mode
            if self.execute:
                self.maybe_backup(panergos_cfg_path)
                dump_yaml_file(panergos_cfg_path, panergos_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{panergos_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Panergos Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Panergos. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Panergos Config Not Overwritten)",
                "",
                "These items already existed in your Panergos config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Panergos agent discovers these",
            "directories, it may read/write to them instead of the Panergos state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `panergos claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `panergos claw cleanup` to fix it.",
            "",
            "## Panergos-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `panergos claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `panergos setup` to configure any remaining settings",
            "- Run `panergos mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `panergos cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `panergos cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `panergos gateway install` if you need the gateway service",
            "- Review `~/.panergos/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Panergos Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("PANERGOS_HOME") or str(Path.home() / ".panergos"), help="Panergos home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Panergos targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Panergos-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║  OpenClaw -> Panergos Migration  [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.panergos/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.panergos/config.yaml")
        print("    2. Run: panergos mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: panergos cron")
