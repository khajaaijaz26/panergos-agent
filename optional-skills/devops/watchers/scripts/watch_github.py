...
Usage (via cron with --no-agent):
    ...
    panergos cron create panergos-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$PANERGOS_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name panergos-issues --repo khajaaijaz26/panergos-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Panergos .env file
(``${PANERGOS_HOME:-~/.panergos}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Panergos-Watcher/1.0",
    }
