#!/usr/bin/env python3
"""Canvas LMS API CLI for Panergos Agent.
...
def _check_config():
    ...
    if missing:
        panergos_env = os.path.join(
            os.environ.get("PANERGOS_HOME", os.path.expanduser("~/.panergos")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {panergos_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Panergos Agent"
    )
