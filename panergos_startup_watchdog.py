...
timeout. Config is env-only (``PANERGOS_STARTUP_WATCHDOG=0``,
``PANERGOS_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "PANERGOS_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "PANERGOS_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (panergos_cli.main / panergos_cli.gateway /
...
def _process_panergos_home() -> Path:
    """PANERGOS_HOME for diagnostic files — stdlib-only replica of the panergos_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<PANERGOS_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_panergos_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``PANERGOS_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``PANERGOS_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
