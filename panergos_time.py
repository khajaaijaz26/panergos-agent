"""Timezone-aware clock for Panergos.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``PANERGOS_TIMEZONE`` env var, then ``timezone`` in ``~/.panergos/config.yaml``, else server-local
...
# PANERGOS_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _env_timezone() -> str:
    """``PANERGOS_TIMEZONE`` when it may speak for the active profile. Under the multiplexed
    ...
    if is_multiplex_active():
        return ""
    return os.getenv("PANERGOS_TIMEZONE", "").strip()
...
def _resolve_timezone_name() -> str:
    ...
    try:
        ...
        # parse is the stdlib-safe fallback for bootstrap consumers without panergos_cli importable.
        try:
