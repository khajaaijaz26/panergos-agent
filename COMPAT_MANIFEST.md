...
The September 2026 decomposition (PR #102117) split the large modules of Panergos Agent into focused
...
| when | CLI banner / `panergos doctor` / `panergos update` | Desktop | the plugin |
|---|---|---|---|
| before 2026-09-14 | yellow notice naming the plugin, the date, and `panergos plugins compat` | one-time modal (per set of affected plugins) | loads; each old-path resolution emits `PanergosPluginCompatWarning` once |
| from 2026-09-14 | red notice: plugin **DISABLED** | one-time modal | **not loaded**; `panergos plugins list` shows the reason |
...
**For plugin authors:** run `panergos plugins compat <path-to-your-plugin>` — it prints every `file:line`,
old path → new path, and exits 1 while anything remains. Import from the `new location` column.

**You will see a warning.** The first time a process resolves a name through one of these blocks, Panergos emits a
`PanergosPluginCompatWarning` (a `FutureWarning`) naming the old path, the new path, and the removal target — once per
name per process. Fix the import and it goes away. To silence during migration:
`python -W ignore::panergos_cli.plugin_compat.PanergosPluginCompatWarning` or `warnings.filterwarnings("ignore", category=PanergosPluginCompatWarning)`.
...
| moved-lazy | 1112 | same, resolved lazily via `__getattr__` to avoid an import cycle |
...
| restored-import | 16 | import restored because a restored definition depends on it |
...
| `copy` | import | `copy` |
| `get_panergos_home` | moved-lazy | `panergos_constants` |
...
| `read_panergos_oauth_credentials` | moved-lazy | `agent.anthropic_credentials` |
...
| `run_panergos_oauth_login_pure` | moved-lazy | `agent.anthropic_credentials` |
...
| `Dict` | import | `typing` |
| `List` | import | `typing` |
| `panergos_home_key` | moved-lazy | `panergos_constants` |
...
| `DEFAULT_BROWSER_CDP_URL` | moved-lazy | `panergos_cli.browser_connect` |
...
| `SlashCommandAutoSuggest` | moved-lazy | `panergos_cli.commands_completion` |
| `SlashCommandCompleter` | moved-lazy | `panergos_cli.commands_completion` |
...
| `base64` | import | `base64` |
| `build_welcome_banner` | moved-lazy | `panergos_cli.banner` |
| `concurrent` | import | `concurrent.futures` |
| `copy` | import | `copy` |
| `display_panergos_home` | moved-lazy | `panergos_constants` |
| `estimate_usage_cost` | moved-lazy | `agent.usage_pricing` |
...
| `init_skin_from_config` | moved-lazy | `panergos_cli.skin_engine` |
| `is_browser_debug_ready` | moved-lazy | `panergos_cli.browser_connect` |
| `is_table_divider` | moved-lazy | `agent.markdown_tables` |
| `looks_like_table_row` | moved-lazy | `agent.markdown_tables` |
| `manual_chrome_debug_command` | moved-lazy | `panergos_cli.browser_connect` |
| `print_config_warnings` | moved-lazy | `panergos_cli.config` |
| `prompt_for_secret` | moved-lazy | `panergos_cli.callbacks` |
| `rich_box` | import | `rich` |
| `set_friendly_tool_labels` | moved-lazy | `agent.display` |
| `set_tool_preview_max_len` | moved-lazy | `agent.display` |
| `setup_logging` | moved-lazy | `panergos_logging` |
...
| `clear_model_endpoint_credentials` | moved-lazy | `panergos_cli.config` |
| `extract_api_content_sidecar` | moved-lazy | `agent.turn_context` |
...
| `CODEX_OAUTH_USER_AGENT` | moved-lazy | `panergos_cli.auth_constants` |
| `CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS` | moved-lazy | `panergos_cli.auth_codex` |
| `DEFAULT_SPOTIFY_REDIRECT_URI` | moved-lazy | `panergos_cli.auth_constants` |
| `DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS` | moved-lazy | `panergos_cli.auth_constants` |
| `HTTPServer` | import | `http.server` |
| `MINIMAX_OAUTH_GRANT_TYPE` | moved-lazy | `panergos_cli.auth_constants` |
| `OAUTH_OVER_SSH_DOCS_URL` | moved-lazy | `panergos_cli.auth_constants` |
| `QWEN_OAUTH_CLIENT_ID` | moved-lazy | `panergos_cli.auth_constants` |
| `QWEN_OAUTH_TOKEN_URL` | moved-lazy | `panergos_cli.auth_constants` |
| `SINGLE_USE_OAUTH_SINGLETON_FILES` | moved-lazy | `panergos_cli.auth_oauth_grants` |
| `SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS` | moved-lazy | `panergos_cli.auth_constants` |
| `SPOTIFY_DASHBOARD_URL` | moved-lazy | `panergos_cli.auth_constants` |
| `TYPE_CHECKING` | import | `typing` |
| `XAI_OAUTH_DEVICE_CODE_URL` | moved-lazy | `panergos_cli.auth_constants` |
| `XAI_OAUTH_DISCOVERY_URL` | moved-lazy | `panergos_cli.auth_constants` |
| `XAI_OAUTH_ISSUER` | moved-lazy | `panergos_cli.auth_constants` |
| `base64` | import | `base64` |
| `hashlib` | import | `hashlib` |
...
### `panergos_cli.checkpoints`

| name | kind | new location |
|---|---|---|
| `cmd_list` | moved-lazy | `panergos_cli.plugins_cmd` |
| `datetime` | import | `datetime` |

### `panergos_cli.commands`
...
| `discord_skill_commands_by_category` | moved-lazy | `panergos_cli.commands_platforms` |
| `field` | import | `dataclasses` |
| `os` | import | `os` |
| `shutil` | import | `shutil` |
| `slack_app_manifest` | moved-lazy | `panergos_cli.commands_platforms` |
| `slack_native_slashes` | moved-lazy | `panergos_cli.commands_platforms` |
| `slack_subcommand_map` | moved-lazy | `panergos_cli.commands_platforms` |
| `subprocess` | import | `subprocess` |
| `telegram_bot_commands` | moved-lazy | `panergos_cli.commands_platforms` |
| `telegram_menu_commands` | moved-lazy | `panergos_cli.commands_platforms` |
| `telegram_menu_max_commands` | moved-lazy | `panergos_cli.commands_platforms` |
| `time` | import | `time` |
...
| name | kind | new location |
|---|---|---|
| `normalize_route_base_url` | moved-lazy | `panergos_cli.route_identity` |
...
### `panergos_cli.console_engine`

| name | kind | new location |
|---|---|---|
| `shlex` | import | `shlex` |

### `panergos_cli.curses_ui`

| name | kind | new location |
|---|---|---|
| `Protocol` | import | `typing` |

### `panergos_cli.dashboard_auth.audit`

| name | kind | new location |
|---|---|---|
| `os` | import | `os` |

### `panergos_cli.dashboard_auth.middleware`

| name | kind | new location |
|---|---|---|
| `DashboardAuthProvider` | moved-lazy | `panergos_cli.dashboard_auth.base` |

### `panergos_cli.dingtalk_auth`

| name | kind | new location |
|---|---|---|
| `logger` | moved-lazy | `panergos_cli.auth` |
| `logging` | import | `logging` |
...
| name | kind | new location |
|---|---|---|
| `FTS_STORAGE_VERSION` | moved-lazy | `panergos_state_common` |
| `OPENROUTER_MODELS_URL` | moved-lazy | `panergos_constants` |
| `Path` | import | `pathlib` |
| `STATE_DB_SIZE_WARN_BYTES` | moved-lazy | `panergos_cli.doctor_state` |
| `agent_browser_runnable` | moved-lazy | `panergos_constants` |
| `base_url_host_matches` | moved-lazy | `utils` |
| `check_certificates` | moved-lazy | `panergos_cli.doctor_platform` |
| `check_fail` | restored-def | `(deleted; BASE body restored)` |
| `check_macos_full_disk_access` | moved-lazy | `panergos_cli.doctor_platform` |
| `check_macos_tcc_anchor` | moved-lazy | `panergos_cli.doctor_platform` |
| `check_macos_tcc_grants` | moved-lazy | `panergos_cli.doctor_platform` |
| `check_ok` | restored-def | `(deleted; BASE body restored)` |
| `check_warn` | restored-def | `(deleted; BASE body restored)` |
| `collect_deprecated_config_keys` | moved-lazy | `panergos_cli.doctor_config` |
| `collect_deprecated_env_vars` | moved-lazy | `panergos_cli.doctor_config` |
| `collect_relay_plugin_cutover_findings` | moved-lazy | `panergos_cli.doctor_config` |
| `describe_vercel_auth` | moved-lazy | `panergos_cli.vercel_auth` |
| `detect_install_method` | moved-lazy | `panergos_cli.config` |
| `importlib` | import | `importlib.util` |
| `is_nix_install_method` | moved-lazy | `panergos_cli.config` |
| `managed_scope_check` | moved-lazy | `panergos_cli.doctor_config` |
| `recommended_update_command_for_method` | moved-lazy | `panergos_cli.config` |
| `report_deprecated_config_and_env` | moved-lazy | `panergos_cli.doctor_config` |
| `shutil` | import | `shutil` |
| `subprocess` | import | `subprocess` |

### `panergos_cli.doctor_live`
...
### `panergos_cli.gitlock`

| name | kind | new location |
|---|---|---|
| `is_ancestor_of_head` | restored-def | `(deleted; BASE body restored)` |

### `panergos_cli.heartbeat`

| name | kind | new location |
|---|---|---|
| `Dict` | import | `typing` |

### `panergos_cli.journey`
...
### `panergos_cli.kanban_db`

| name | kind | new location |
|---|---|---|
| `DEFAULT_BUSY_TIMEOUT_MS` | moved-lazy | `panergos_cli.kanban_db_connect` |
| `DEFAULT_LOG_BACKUP_COUNT` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `DEFAULT_LOG_ROTATE_BYTES` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `DEFAULT_SPAWN_FAILURE_LIMIT` | restored-def | `(deleted; BASE body restored)` |
| `DERIVED_MAX_IN_PROGRESS_CEILING` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `DERIVED_MAX_IN_PROGRESS_FLOOR` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `KANBAN_TERMINAL_TIMEOUT_GRACE_SECONDS` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `KanbanDbCorruptError` | moved-lazy | `panergos_cli.kanban_db_connect` |
| `MEMORY_GUARD_MB_PER_WORKER` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `Mapping` | import | `typing` |
| `RepairResult` | moved-lazy | `panergos_cli.kanban_db_connect` |
| `add_notify_sub` | moved-lazy | `panergos_cli.kanban_db_notify` |
| `advance_notify_cursor` | moved-lazy | `panergos_cli.kanban_db_notify` |
| `check_respawn_guard` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `claim_unseen_events_for_sub` | moved-lazy | `panergos_cli.kanban_db_notify` |
| `configured_max_in_progress` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `connect` | moved-lazy | `panergos_cli.kanban_db_connect` |
| `connect_closing` | moved-lazy | `panergos_cli.kanban_db_connect` |
| `count_notify_subs` | moved-lazy | `panergos_cli.kanban_db_notify` |
| `count_running_tasks` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `count_running_tasks_other_boards` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `derive_default_max_in_progress` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `detect_crashed_workers` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `detect_stale_running` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `dispatch_once` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `enforce_max_runtime` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `field` | import | `dataclasses` |
| `has_spawnable_ready` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `has_spawnable_review` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `hashlib` | import | `hashlib` |
| `heartbeat_worker` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `list_notify_subs` | moved-lazy | `panergos_cli.kanban_db_notify` |
| `parent_results` | restored-def | `(deleted; BASE body restored)` |
| `purge_stale_done_notify_subs` | moved-lazy | `panergos_cli.kanban_db_notify` |
| `random` | import | `random` |
| `reap_worker_zombies` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `reconcile_orphaned_running` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `remove_notify_sub` | moved-lazy | `panergos_cli.kanban_db_notify` |
| `repair_db` | moved-lazy | `panergos_cli.kanban_db_connect` |
| `resolve_max_in_progress` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `resolve_workspace` | moved-lazy | `panergos_cli.kanban_db_workspace` |
| `review_dispatch_enabled` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `rewind_notify_cursor` | moved-lazy | `panergos_cli.kanban_db_notify` |
| `run_daemon` | moved-lazy | `panergos_cli.kanban_db_dispatch` |
| `set_branch_name` | moved-lazy | `panergos_cli.kanban_db_workspace` |
| `set_workspace_path` | moved-lazy | `panergos_cli.kanban_db_workspace` |
| `shutil` | import | `shutil` |
| `threading` | import | `threading` |
| `unseen_events_for_sub` | moved-lazy | `panergos_cli.kanban_db_notify` |
| `worker_log_rotation_config` | moved-lazy | `panergos_cli.kanban_db_dispatch` |

### `panergos_cli.kanban_decompose`
...
### `panergos_cli.kanban_diagnostics`

| name | kind | new location |
|---|---|---|
| `DIAGNOSTIC_KINDS` | restored-def | `(deleted; BASE body restored)` |

### `panergos_cli.local_runtime.binaries`

| name | kind | new location |
|---|---|---|
| `get_panergos_home` | moved-lazy | `panergos_constants` |

### `panergos_cli.local_runtime.bootstrap`

| name | kind | new location |
|---|---|---|
| `get_panergos_home` | moved-lazy | `panergos_constants` |

### `panergos_cli.local_runtime.capabilities`
...
### `panergos_cli.local_runtime.catalog`
...
### `panergos_cli.local_runtime.hf_browse`
...
|---|---|---|
| `hashlib` | import | `hashlib` |
| `line_input` | moved-lazy | `panergos_cli.cli_output` |
| `shlex` | import | `shlex` |
| `stat` | import | `stat` |
| `tempfile` | import | `tempfile` |

### `panergos_cli.mcp_picker`

| name | kind | new location |
|---|---|---|
| `color` | moved-lazy | `panergos_cli.colors` |

### `panergos_cli.mcp_security`

| name | kind | new location |
|---|---|---|
| `is_mcp_server_entry_suspicious` | restored-def | `(deleted; BASE body restored)` |

### `panergos_cli.middleware`
...
### `panergos_cli.model_setup_flows`

| name | kind | new location |
|---|---|---|
| `BEDROCK_GEO_PREFIXES` | moved-lazy | `panergos_cli.model_setup_flows_bedrock` |
| `bedrock_model_routable_from_region` | moved-lazy | `panergos_cli.model_setup_flows_bedrock` |
| `bedrock_region_geo_prefix` | moved-lazy | `panergos_cli.model_setup_flows_bedrock` |
| `custom_provider_slug` | moved-lazy | `panergos_cli.providers` |
| `line_input` | moved-lazy | `panergos_cli.cli_output` |
| `subprocess` | import | `subprocess` |
| `urllib` | import | `urllib.parse` |

### `panergos_cli.model_switch`
...
| `list_picker_providers` | moved-lazy | `panergos_cli.model_switch_providers` |
| `prewarm_picker_cache_async` | moved-lazy | `panergos_cli.model_switch_providers` |
| `time` | import | `time` |
...
| name | kind | new location |
|---|---|---|
| `LMStudioLoadResult` | moved-lazy | `panergos_cli.models_local` |
| `NamedTuple` | import | `typing` |
| `PROVIDER_GROUPS` | moved-lazy | `panergos_cli.models_catalog_static` |
| `ProviderEntry` | moved-lazy | `panergos_cli.models_catalog_static` |
| `atomic_json_write` | moved-lazy | `utils` |
| `base_url_host_matches` | moved-lazy | `utils` |
| `compute_sale_discount` | moved-lazy | `panergos_cli.models_pricing` |
| `ensure_lmstudio_model_loaded` | moved-lazy | `panergos_cli.models_local` |
| `fetch_ai_gateway_pricing` | moved-lazy | `panergos_cli.models_pricing` |
| `fetch_lmstudio_models` | moved-lazy | `panergos_cli.models_local` |
| `fetch_models_with_pricing` | moved-lazy | `panergos_cli.models_pricing` |
| `fetch_ollama_local_models` | moved-lazy | `panergos_cli.models_local` |
| `get_close_matches` | import | `difflib` |
| `get_pricing_for_provider` | moved-lazy | `panergos_cli.models_pricing` |
| `group_providers` | moved-lazy | `panergos_cli.models_catalog_static` |
| `http` | import | `http.client` |
...
| `lmstudio_model_reasoning_options` | moved-lazy | `panergos_cli.models_local` |
| `ollama_model_supports_thinking` | moved-lazy | `panergos_cli.models_local` |
| `openrouter_model_reasoning_capabilities` | moved-lazy | `panergos_cli.models_reasoning_caps` |
| `parse_openrouter_reasoning_capabilities` | moved-lazy | `panergos_cli.models_reasoning_caps` |
| `peek_cached_pricing` | moved-lazy | `panergos_cli.models_pricing` |
| `pricing_cache_scope` | moved-lazy | `panergos_cli.models_pricing` |
| `probe_lmstudio_models` | moved-lazy | `panergos_cli.models_local` |
| `probe_ollama_local_models` | moved-lazy | `panergos_cli.models_local` |
| `provider_group_for_slug` | moved-lazy | `panergos_cli.models_catalog_static` |
| `refresh_reasoning_caps_async` | moved-lazy | `panergos_cli.models_reasoning_caps` |
| `should_use_ollama_native_catalog` | moved-lazy | `panergos_cli.models_local` |
| `url_origin` | moved-lazy | `panergos_cli.urllib_security` |
| `validate_requested_model` | moved-lazy | `panergos_cli.models_validate` |
| `warm_openrouter_reasoning_caps_async` | moved-lazy | `panergos_cli.models_reasoning_caps` |

### `panergos_cli.observability.relay_shared_metrics`

| name | kind | new location |
|---|---|---|
| `CLIENT_ACTIVE_MARK` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `MODEL_CALL_PROFILE_MODEL` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `SCHEMA_KEY` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `SCHEMA_VERSION` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `SKILL_LIFECYCLE_MARK` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `SKILL_LOAD_MARK` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `TOOL_APPROVAL_MARK` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `TOOL_CALL_SCOPE` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `model_call_fields` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `prepare_session_start` | restored-def | `(deleted; BASE body restored)` |
| `skill_lifecycle_fields` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `skill_load_fields` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `task_start_fields` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `task_terminal_fields` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `task_terminal_state` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `tool_approval_outcome` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `tool_category` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |
| `tool_terminal_fields` | moved-lazy | `panergos_cli.observability.shared_metrics_contract` |

### `panergos_cli.onepassword_secrets_cli`

| name | kind | new location |
|---|---|---|
| `Table` | import | `rich.table` |
| `masked_secret_prompt` | moved-lazy | `panergos_cli.secret_prompt` |
| `sys` | import | `sys` |

### `panergos_cli.platform_actions`

| name | kind | new location |
|---|---|---|
| `Optional` | import | `typing` |

### `panergos_cli.plugins`

| name | kind | new location |
|---|---|---|
| `CAPABILITY_REGISTRY` | moved-lazy | `panergos_cli.plugin_capabilities` |
| `ENTRY_POINT_CAPABILITIES_GROUP` | moved-lazy | `panergos_cli.plugins_discovery` |
| `Iterable` | import | `typing` |
| `LEGACY_RELAY_PLUGIN_KEYS` | moved-lazy | `panergos_cli.relay_plugin_cutover` |
| `MAX_SYSTEM_PROMPT_SECTIONS` | moved-lazy | `panergos_cli.plugins_dispatch` |
| `OBSERVER_SCHEMA_VERSION` | moved-lazy | `panergos_cli.middleware` |
| `Type` | import | `typing` |
| `VALID_CAPABILITY_IDS` | moved-lazy | `panergos_cli.plugin_capabilities` |
| `cfg_get` | moved-lazy | `panergos_cli.config` |
| `contextmanager` | import | `contextlib` |
| `contextvars` | import | `contextvars` |
| `copy` | import | `copy` |
| `fast_safe_load` | moved-lazy | `utils` |
| `format_system_prompt_section` | moved-lazy | `panergos_cli.plugins_dispatch` |
| `get_plugin_subscriptions` | restored-def | `(deleted; BASE body restored)` |
| `hashlib` | import | `hashlib` |
| `reset_panergos_home_override` | moved-lazy | `panergos_constants` |
| `set_panergos_home_override` | moved-lazy | `panergos_constants` |
| `time` | import | `time` |
| `unload_plugins` | restored-def | `(deleted; BASE body restored)` |
| `wraps` | import | `functools` |

### `panergos_cli.plugins_cmd`

| name | kind | new location |
|---|---|---|
| `importlib` | import | `importlib.metadata` |

### `panergos_cli.profile_describer`

| name | kind | new location |
|---|---|---|
| `json` | import | `json` |

### `panergos_cli.profile_distribution`

| name | kind | new location |
|---|---|---|
| `is_excluded_skill_path` | moved-lazy | `agent.skill_utils` |

### `panergos_cli.profiles`

| name | kind | new location |
|---|---|---|
| `has_bundled_skills_opt_out` | restored-def | `(deleted; BASE body restored)` |

### `panergos_cli.runtime_provider`

| name | kind | new location |
|---|---|---|
| `custom_provider_aliases` | moved-lazy | `panergos_cli.providers` |
| `custom_provider_slug` | moved-lazy | `panergos_cli.providers` |
| `os` | import | `os` |

### `panergos_cli.security_advisories`
...
| `Dict` | import | `typing` |
| `Optional` | import | `typing` |
| `get_optional_skills_dir` | moved-lazy | `panergos_constants` |
| `json` | import | `json` |
| `shutil` | import | `shutil` |

### `panergos_cli.slack_cli`

| name | kind | new location |
|---|---|---|
| `os` | import | `os` |

### `panergos_cli.sqlite_safe_read`
...
| name | kind | new location |
|---|---|---|
| `redact_key` | moved-lazy | `panergos_cli.config` |
| `subprocess` | import | `subprocess` |

### `panergos_cli.telegram_managed_bot`
...
| `is_truthy_value` | moved-lazy | `utils` |
| `save_env_value` | moved-lazy | `panergos_cli.config` |
| `shutil` | import | `shutil` |
| `subprocess` | import | `subprocess` |
| `sys` | import | `sys` |

### `panergos_cli.uninstall`

| name | kind | new location |
|---|---|---|
| `find_shell_configs` | restored-def | `(deleted; BASE body restored)` |

### `panergos_cli.update_cmd`
...
### `panergos_cli.web_routers.cron`

| name | kind | new location |
|---|---|---|
| `logging` | import | `logging` |

### `panergos_cli.web_routers.mcp`

| name | kind | new location |
|---|---|---|
| `logging` | import | `logging` |

### `panergos_cli.web_routers.sessions`
...
### `panergos_cli.web_routers.skills`

| name | kind | new location |
|---|---|---|
| `LateState` | moved-lazy | `panergos_cli.web_deps` |
| `logging` | import | `logging` |

### `panergos_cli.web_routers.tools`
...
### `panergos_cli.web_server`

| name | kind | new location |
|---|---|---|
| `AudioTranscriptionRequest` | moved-lazy | `panergos_cli.web_models` |
| `AutomationBlueprintInstantiate` | moved-lazy | `panergos_cli.web_models` |
| `BackupRequest` | moved-lazy | `panergos_cli.web_models` |
| `BulkDeleteSessions` | moved-lazy | `panergos_cli.web_models` |
| `CONFIG_SCHEMA` | moved-lazy | `panergos_cli.web_server_config` |
| `ChatImageUpload` | moved-lazy | `panergos_cli.web_models` |
| `ConfigUpdate` | moved-lazy | `panergos_cli.web_models` |
| `CredentialPoolAdd` | moved-lazy | `panergos_cli.web_models` |
| `CronJobCreate` | moved-lazy | `panergos_cli.web_models` |
| `CronJobUpdate` | moved-lazy | `panergos_cli.web_models` |
| `CuratorPause` | moved-lazy | `panergos_cli.web_models` |
| `CustomEndpointUpdate` | moved-lazy | `panergos_cli.web_models` |
| `DEFAULT_CONFIG` | moved-lazy | `panergos_cli.config` |
| `DebugShareRequest` | moved-lazy | `panergos_cli.web_models` |
| `EnvVarDelete` | moved-lazy | `panergos_cli.web_models` |
| `EnvVarReveal` | moved-lazy | `panergos_cli.web_models` |
| `EnvVarUpdate` | moved-lazy | `panergos_cli.web_models` |
| `FontSetBody` | moved-lazy | `panergos_cli.web_models` |
| `FsWriteText` | moved-lazy | `panergos_cli.web_models` |
| `GitBranchSwitchBody` | moved-lazy | `panergos_cli.web_models` |
| `GitCommitBody` | moved-lazy | `panergos_cli.web_models` |
| `GitFileBody` | moved-lazy | `panergos_cli.web_models` |
| `GitPathBody` | moved-lazy | `panergos_cli.web_models` |
| `GitWorktreeAddBody` | moved-lazy | `panergos_cli.web_models` |
| `GitWorktreeRemoveBody` | moved-lazy | `panergos_cli.web_models` |
| `HookCreate` | moved-lazy | `panergos_cli.web_models` |
| `HookDelete` | moved-lazy | `panergos_cli.web_models` |
| `ImportRequest` | moved-lazy | `panergos_cli.web_models` |
| `LearningNodeEdit` | moved-lazy | `panergos_cli.web_models` |
| `LearningNodeRef` | moved-lazy | `panergos_cli.web_models` |
| `List` | import | `typing` |
| `Literal` | import | `typing` |
| `MCPCatalogInstall` | moved-lazy | `panergos_cli.web_models` |
| `MCPEnabledToggle` | moved-lazy | `panergos_cli.web_models` |
| `MCPServerCreate` | moved-lazy | `panergos_cli.web_models` |
| `MCPServersReplace` | moved-lazy | `panergos_cli.web_models` |
| `ManagedDirectoryCreate` | moved-lazy | `panergos_cli.web_models` |
| `ManagedFileDelete` | moved-lazy | `panergos_cli.web_models` |
| `ManagedFileUpload` | moved-lazy | `panergos_cli.web_models` |
| `ManagedFilesPolicy` | moved-lazy | `panergos_cli.web_server_files` |
| `MemoryProviderConfigUpdate` | moved-lazy | `panergos_cli.web_models` |
| `MemoryProviderSelect` | moved-lazy | `panergos_cli.web_models` |
| `MemoryProviderSetupRequest` | moved-lazy | `panergos_cli.web_models` |
| `MemoryReset` | moved-lazy | `panergos_cli.web_models` |
| `MessagingPlatformUpdate` | moved-lazy | `panergos_cli.web_models` |
| `MoaConfigPayload` | moved-lazy | `panergos_cli.web_models` |
| `MoaModelSlot` | moved-lazy | `panergos_cli.web_models` |
| `MoaPresetPayload` | moved-lazy | `panergos_cli.web_models` |
| `ModelAssignment` | moved-lazy | `panergos_cli.web_models` |
| `OAuthSubmitBody` | moved-lazy | `panergos_cli.web_models` |
| `OPTIONAL_ENV_VARS` | moved-lazy | `panergos_cli.config` |
| `PairingApprove` | moved-lazy | `panergos_cli.web_models` |
| `PairingRevoke` | moved-lazy | `panergos_cli.web_models` |
| `ProfileActiveUpdate` | moved-lazy | `panergos_cli.web_models` |
| `ProfileCreate` | moved-lazy | `panergos_cli.web_models` |
| `ProfileDescribeAuto` | moved-lazy | `panergos_cli.web_models` |
| `ProfileDescriptionUpdate` | moved-lazy | `panergos_cli.web_models` |
| `ProfileModelUpdate` | moved-lazy | `panergos_cli.web_models` |
| `ProfileRename` | moved-lazy | `panergos_cli.web_models` |
| `ProfileSoulUpdate` | moved-lazy | `panergos_cli.web_models` |
| `ProviderConfigSchema` | moved-lazy | `plugins.memory.config_schema` |
| `ProviderField` | moved-lazy | `plugins.memory.config_schema` |
| `PtyBridge` | moved-lazy | `panergos_cli.pty_bridge` |
| `PtySessionRegistry` | moved-lazy | `panergos_cli.pty_session` |
| `PtyUnavailableError` | moved-lazy | `panergos_cli.pty_bridge` |
| `RawConfigUpdate` | moved-lazy | `panergos_cli.web_models` |
| `RegistryFull` | moved-lazy | `panergos_cli.pty_session` |
| `STORAGE_HONCHO_HOST_BLOCK` | moved-lazy | `plugins.memory.config_schema` |
| `SessionImport` | moved-lazy | `panergos_cli.web_models` |
| `SessionPrune` | moved-lazy | `panergos_cli.web_models` |
| `SessionRename` | moved-lazy | `panergos_cli.web_models` |
| `SkillContentUpdate` | moved-lazy | `panergos_cli.web_models` |
| `SkillCreate` | moved-lazy | `panergos_cli.web_models` |
| `SkillInstallRequest` | moved-lazy | `panergos_cli.web_models` |
| `SkillToggle` | moved-lazy | `panergos_cli.web_models` |
| `SkillUninstallRequest` | moved-lazy | `panergos_cli.web_models` |
| `SkillsUpdateRequest` | moved-lazy | `panergos_cli.web_models` |
| `TTSLeaseRequest` | moved-lazy | `panergos_cli.web_models` |
| `TTSSpeakRequest` | moved-lazy | `panergos_cli.web_models` |
| `TelegramOnboardingApply` | moved-lazy | `panergos_cli.web_models` |
| `TelegramOnboardingStart` | moved-lazy | `panergos_cli.web_models` |
| `TerminalBackendSelect` | moved-lazy | `panergos_cli.web_models` |
| `ThemeSetBody` | moved-lazy | `panergos_cli.web_models` |
| `ToolsetEnvUpdate` | moved-lazy | `panergos_cli.web_models` |
| `ToolsetModelSelect` | moved-lazy | `panergos_cli.web_models` |
| `ToolsetPostSetup` | moved-lazy | `panergos_cli.web_models` |
| `ToolsetProviderSelect` | moved-lazy | `panergos_cli.web_models` |
| `ToolsetToggle` | moved-lazy | `panergos_cli.web_models` |
| `WebhookCreate` | moved-lazy | `panergos_cli.web_models` |
| `WebhookEnabledToggle` | moved-lazy | `panergos_cli.web_models` |
| `WhatsAppOnboardingApply` | moved-lazy | `panergos_cli.web_models` |
| `WhatsAppOnboardingStart` | moved-lazy | `panergos_cli.web_models` |
| `activate_custom_endpoint` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `add_credential_pool_entry` | moved-lazy | `panergos_cli.web_routers.ops` |
| `add_mcp_server` | moved-lazy | `panergos_cli.web_routers.mcp` |
| `apply_telegram_onboarding` | moved-lazy | `panergos_cli.web_routers.messaging` |
| `apply_whatsapp_onboarding` | moved-lazy | `panergos_cli.web_routers.messaging` |
| `approve_pairing` | moved-lazy | `panergos_cli.web_routers.ops` |
| `atexit` | import | `atexit` |
| `auth_mcp_server` | moved-lazy | `panergos_cli.web_routers.mcp` |
| `base64` | import | `base64` |
| `binascii` | import | `binascii` |
| `build_cron_model_impact` | moved-lazy | `panergos_cli.config` |
| `bulk_delete_sessions_endpoint` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `cancel_oauth_session` | moved-lazy | `panergos_cli.web_routers.oauth` |
| `cancel_telegram_onboarding` | moved-lazy | `panergos_cli.web_routers.messaging` |
| `cancel_whatsapp_onboarding` | moved-lazy | `panergos_cli.web_routers.messaging` |
| `cfg_get` | moved-lazy | `panergos_cli.config` |
| `check_config_version` | moved-lazy | `panergos_cli.config` |
| `check_panergos_update` | moved-lazy | `panergos_cli.web_routers.actions` |
| `clear_model_endpoint_credentials` | moved-lazy | `panergos_cli.config` |
| `clear_pending_pairing` | moved-lazy | `panergos_cli.web_routers.ops` |
| `coerce_provider_id` | moved-lazy | `panergos_cli.config` |
| `concurrent` | import | `concurrent.futures` |
| `console_ws` | moved-lazy | `panergos_cli.web_routers.chat_ws` |
| `contextlib` | import | `contextlib` |
| `contextmanager` | import | `contextlib` |
| `count_empty_sessions_endpoint` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `create_cron_job` | moved-lazy | `panergos_cli.web_routers.cron` |
| `create_hook` | moved-lazy | `panergos_cli.web_routers.ops` |
| `create_managed_directory` | moved-lazy | `panergos_cli.web_routers.files` |
| `create_profile_endpoint` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `create_skill` | moved-lazy | `panergos_cli.web_routers.skills` |
| `create_webhook` | moved-lazy | `panergos_cli.web_routers.ops` |
| `cron_fire_webhook` | moved-lazy | `panergos_cli.web_routers.cron` |
| `custom_endpoint_key_env` | moved-lazy | `panergos_cli.config` |
| `dataclass` | import | `dataclasses` |
| `datetime` | import | `datetime` |
| `delete_agent_plugin` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `delete_cron_job` | moved-lazy | `panergos_cli.web_routers.cron` |
| `delete_custom_endpoint` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `delete_empty_sessions_endpoint` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `delete_hook` | moved-lazy | `panergos_cli.web_routers.ops` |
| `delete_learning_node` | moved-lazy | `panergos_cli.web_routers.status` |
| `delete_managed_file` | moved-lazy | `panergos_cli.web_routers.files` |
| `delete_profile_endpoint` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `delete_session_endpoint` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `delete_webhook` | moved-lazy | `panergos_cli.web_routers.ops` |
| `derive_gateway_busy` | moved-lazy | `gateway.status` |
| `derive_gateway_drainable` | moved-lazy | `gateway.status` |
| `describe_profile_auto_endpoint` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `detect_install_method` | moved-lazy | `panergos_cli.config` |
| `disconnect_oauth_provider` | moved-lazy | `panergos_cli.web_routers.oauth` |
| `download_dashboard_backup` | moved-lazy | `panergos_cli.web_routers.ops` |
| `download_managed_file` | moved-lazy | `panergos_cli.web_routers.files` |
| `enable_webhooks` | moved-lazy | `panergos_cli.web_routers.ops` |
| `env_var_enabled` | moved-lazy | `utils` |
| `events_ws` | moved-lazy | `panergos_cli.web_routers.chat_ws` |
| `export_session_endpoint` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `find_provider_entry` | moved-lazy | `panergos_cli.config` |
| `format_docker_update_message` | moved-lazy | `panergos_cli.config` |
| `fs_default_cwd` | moved-lazy | `panergos_cli.web_routers.files` |
| `fs_download` | moved-lazy | `panergos_cli.web_routers.files` |
| `fs_git_root` | moved-lazy | `panergos_cli.web_routers.files` |
| `fs_list` | moved-lazy | `panergos_cli.web_routers.files` |
| `fs_read_data_url` | moved-lazy | `panergos_cli.web_routers.files` |
| `fs_read_text` | moved-lazy | `panergos_cli.web_routers.files` |
| `fs_write_text` | moved-lazy | `panergos_cli.web_routers.files` |
| `functools` | import | `functools` |
| `gateway_drain` | moved-lazy | `panergos_cli.web_routers.actions` |
| `gateway_ws` | moved-lazy | `panergos_cli.web_routers.chat_ws` |
| `get_action_status` | moved-lazy | `panergos_cli.web_routers.actions` |
| `get_active_profile_endpoint` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `get_auxiliary_models` | moved-lazy | `panergos_cli.web_routers.models` |
| `get_client_voice_config` | moved-lazy | `panergos_cli.web_routers.audio` |
| `get_computer_use_status` | moved-lazy | `panergos_cli.web_routers.tools` |
| `get_config` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `get_config_path` | moved-lazy | `panergos_cli.config` |
| `get_config_raw` | moved-lazy | `panergos_cli.web_routers.analytics` |
| `get_cron_delivery_targets` | moved-lazy | `panergos_cli.web_routers.cron` |
| `get_cron_job` | moved-lazy | `panergos_cli.web_routers.cron` |
| `get_curator_status` | moved-lazy | `panergos_cli.web_routers.status` |
| `get_dashboard_font` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `get_dashboard_plugins` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `get_dashboard_themes` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `get_defaults` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `get_egress_status` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `get_elevenlabs_voices` | moved-lazy | `panergos_cli.web_routers.audio` |
| `get_env_path` | moved-lazy | `panergos_cli.config` |
| `get_env_vars` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `get_health` | moved-lazy | `panergos_cli.web_routers.status` |
| `get_panergos_home` | moved-lazy | `panergos_cli.config` |
| `get_learning_graph` | moved-lazy | `panergos_cli.web_routers.status` |
| `get_learning_node` | moved-lazy | `panergos_cli.web_routers.status` |
| `get_logs` | moved-lazy | `panergos_cli.web_routers.status` |
| `get_media` | moved-lazy | `panergos_cli.web_routers.files` |
| `get_memory_provider_config` | moved-lazy | `panergos_cli.web_routers.memory_providers` |
| `get_memory_status` | moved-lazy | `panergos_cli.web_routers.ops` |
| `get_messaging_platforms` | moved-lazy | `panergos_cli.web_routers.messaging` |
| `get_moa_models` | moved-lazy | `panergos_cli.web_routers.models` |
| `get_model_info` | moved-lazy | `panergos_cli.web_routers.models` |
| `get_model_options` | moved-lazy | `panergos_cli.web_routers.models` |
| `get_models_analytics` | moved-lazy | `panergos_cli.web_routers.analytics` |
| `get_plugins_hub` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `get_portal_status` | moved-lazy | `panergos_cli.web_routers.status` |
| `get_process_panergos_home` | moved-lazy | `panergos_cli.config` |
| `get_profile_setup_command` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `get_profile_soul` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `get_profiles_sessions` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `get_profiles_sessions_sidebar` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `get_provider_config_schema` | moved-lazy | `plugins.memory.config_schema` |
| `get_recommended_default_model` | moved-lazy | `panergos_cli.web_routers.models` |
...
| `get_schema` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `get_session_detail` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `get_session_latest_descendant` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `get_session_messages` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `get_session_stats` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `get_sessions` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `get_skill_content` | moved-lazy | `panergos_cli.web_routers.skills` |
| `get_skills` | moved-lazy | `panergos_cli.web_routers.skills` |
| `get_ssh_ownership` | moved-lazy | `panergos_cli.web_routers.status` |
| `get_status` | moved-lazy | `panergos_cli.web_routers.status` |
| `get_system_stats` | moved-lazy | `panergos_cli.web_routers.status` |
| `get_telegram_onboarding_status` | moved-lazy | `panergos_cli.web_routers.messaging` |
| `get_terminal_backends` | moved-lazy | `panergos_cli.web_routers.tools` |
| `get_toolset_config` | moved-lazy | `panergos_cli.web_routers.tools` |
| `get_toolset_models` | moved-lazy | `panergos_cli.web_routers.tools` |
| `get_toolsets` | moved-lazy | `panergos_cli.web_routers.tools` |
| `get_update_receipt` | moved-lazy | `panergos_cli.web_routers.actions` |
| `get_usage_analytics` | moved-lazy | `panergos_cli.web_routers.analytics` |
| `get_whatsapp_onboarding_status` | moved-lazy | `panergos_cli.web_routers.messaging` |
| `git_base_branches_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_branch_switch_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_branches_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_commit_context_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_commit_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_create_pr_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_file_diff_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_push_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_rev_parse_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_revert_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_review_diff_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_review_list_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_ship_info_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_stage_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_status_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_unstage_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_worktree_add_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_worktree_remove_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `git_worktrees_route` | moved-lazy | `panergos_cli.web_routers.git` |
| `grant_computer_use_permissions` | moved-lazy | `panergos_cli.web_routers.tools` |
| `hashlib` | import | `hashlib` |
| `import_sessions_endpoint` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `importlib` | import | `importlib.util` |
| `inspect` | import | `inspect` |
| `install_mcp_catalog_entry` | moved-lazy | `panergos_cli.web_routers.mcp` |
| `install_skill_hub` | moved-lazy | `panergos_cli.web_routers.skills` |
| `instantiate_blueprint` | moved-lazy | `panergos_cli.web_routers.cron` |
| `ipaddress` | import | `ipaddress` |
| `is_nix_install_method` | moved-lazy | `panergos_cli.config` |
| `json` | import | `json` |
| `list_checkpoints` | moved-lazy | `panergos_cli.web_routers.ops` |
| `list_credential_pool` | moved-lazy | `panergos_cli.web_routers.ops` |
| `list_cron_blueprints` | moved-lazy | `panergos_cli.web_routers.cron` |
| `list_cron_job_runs` | moved-lazy | `panergos_cli.web_routers.cron` |
| `list_cron_jobs` | moved-lazy | `panergos_cli.web_routers.cron` |
| `list_custom_endpoints` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `list_hooks` | moved-lazy | `panergos_cli.web_routers.ops` |
| `list_managed_files` | moved-lazy | `panergos_cli.web_routers.files` |
| `list_mcp_catalog` | moved-lazy | `panergos_cli.web_routers.mcp` |
| `list_mcp_servers` | moved-lazy | `panergos_cli.web_routers.mcp` |
| `list_oauth_providers` | moved-lazy | `panergos_cli.web_routers.oauth` |
| `list_pairing` | moved-lazy | `panergos_cli.web_routers.ops` |
| `list_profiles_endpoint` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `list_skills_hub_sources` | moved-lazy | `panergos_cli.web_routers.skills` |
| `list_webhooks` | moved-lazy | `panergos_cli.web_routers.ops` |
| `load_env` | moved-lazy | `panergos_cli.config` |
| `math` | import | `math` |
| `mcp_oauth_callback` | moved-lazy | `panergos_cli.web_routers.mcp` |
| `mcp_oauth_flow_status` | moved-lazy | `panergos_cli.web_routers.mcp` |
| `mimetypes` | import | `mimetypes` |
| `normalize_updated_at` | moved-lazy | `gateway.status` |
| `open_profile_terminal_endpoint` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `parse_active_agents` | moved-lazy | `gateway.status` |
| `pause_cron_job` | moved-lazy | `panergos_cli.web_routers.cron` |
| `poll_oauth_session` | moved-lazy | `panergos_cli.web_routers.oauth` |
| `post_agent_plugin_disable` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `post_agent_plugin_enable` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `post_agent_plugin_install` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `post_agent_plugin_update` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `post_plugin_visibility` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `preview_skill_hub` | moved-lazy | `panergos_cli.web_routers.skills` |
| `prune_checkpoints` | moved-lazy | `panergos_cli.web_routers.ops` |
| `prune_sessions_endpoint` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `pty_ws` | moved-lazy | `panergos_cli.web_routers.chat_ws` |
| `pub_ws` | moved-lazy | `panergos_cli.web_routers.chat_ws` |
| `put_plugin_providers` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `queue` | import | `queue` |
| `read_managed_file` | moved-lazy | `panergos_cli.web_routers.files` |
| `read_raw_config` | moved-lazy | `panergos_cli.config` |
| `read_runtime_status` | moved-lazy | `gateway.status` |
| `recommended_update_command_for_method` | moved-lazy | `panergos_cli.config` |
| `redact_key` | moved-lazy | `panergos_cli.config` |
| `remove_credential_pool_entry` | moved-lazy | `panergos_cli.web_routers.ops` |
| `remove_env_value` | moved-lazy | `panergos_cli.config` |
| `remove_env_var` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `remove_mcp_server` | moved-lazy | `panergos_cli.web_routers.mcp` |
| `rename_profile_endpoint` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `rename_session_endpoint` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `replace_mcp_servers` | moved-lazy | `panergos_cli.web_routers.mcp` |
| `rescan_dashboard_plugins` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `reset_memory` | moved-lazy | `panergos_cli.web_routers.ops` |
| `resolve_cron_model_drift_defaults` | moved-lazy | `panergos_cli.config` |
| `resolve_gateway_liveness` | moved-lazy | `gateway.status` |
| `restart_gateway` | moved-lazy | `panergos_cli.web_routers.actions` |
| `resume_cron_job` | moved-lazy | `panergos_cli.web_routers.cron` |
| `reveal_env_var` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `revoke_pairing` | moved-lazy | `panergos_cli.web_routers.ops` |
| `run_backup` | moved-lazy | `panergos_cli.web_routers.ops` |
| `run_config_migrate` | moved-lazy | `panergos_cli.web_routers.status` |
| `run_curator` | moved-lazy | `panergos_cli.web_routers.status` |
| `run_debug_share_endpoint` | moved-lazy | `panergos_cli.web_routers.status` |
| `run_doctor` | moved-lazy | `panergos_cli.doctor` |
| `run_dump` | moved-lazy | `panergos_cli.dump` |
| `run_import` | moved-lazy | `panergos_cli.web_routers.ops` |
| `run_import_upload` | moved-lazy | `panergos_cli.web_routers.ops` |
| `run_prompt_size` | moved-lazy | `panergos_cli.web_routers.status` |
| `run_security_audit` | moved-lazy | `panergos_cli.web_routers.ops` |
| `run_toolset_post_setup` | moved-lazy | `panergos_cli.web_routers.tools` |
| `save_config` | moved-lazy | `panergos_cli.config` |
| `save_env_value` | moved-lazy | `panergos_cli.config` |
| `save_toolset_env` | moved-lazy | `panergos_cli.web_routers.tools` |
| `scan_skill_hub` | moved-lazy | `panergos_cli.web_routers.skills` |
| `search_sessions` | moved-lazy | `panergos_cli.web_routers.sessions` |
| `search_skills_hub` | moved-lazy | `panergos_cli.web_routers.skills` |
| `select_terminal_backend` | moved-lazy | `panergos_cli.web_routers.tools` |
| `select_toolset_model` | moved-lazy | `panergos_cli.web_routers.tools` |
| `select_toolset_provider` | moved-lazy | `panergos_cli.web_routers.tools` |
| `serve_plugin_asset` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `set_active_profile_endpoint` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `set_curator_paused` | moved-lazy | `panergos_cli.web_routers.status` |
| `set_dashboard_font` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `set_dashboard_theme` | moved-lazy | `panergos_cli.web_routers.dashboard_ui` |
| `set_env_var` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `set_mcp_server_enabled` | moved-lazy | `panergos_cli.web_routers.mcp` |
| `set_memory_provider` | moved-lazy | `panergos_cli.web_routers.ops` |
| `set_moa_models` | moved-lazy | `panergos_cli.web_routers.models` |
| `set_model_assignment` | moved-lazy | `panergos_cli.web_routers.models` |
| `set_webhook_enabled` | moved-lazy | `panergos_cli.web_routers.ops` |
| `setup_memory_provider` | moved-lazy | `panergos_cli.web_routers.memory_providers` |
| `shlex` | import | `shlex` |
| `shutil` | import | `shutil` |
| `speak_stream_ws` | moved-lazy | `panergos_cli.web_routers.audio` |
| `speak_text` | moved-lazy | `panergos_cli.web_routers.audio` |
| `start_gateway` | moved-lazy | `panergos_cli.web_routers.ops` |
| `start_oauth_login` | moved-lazy | `panergos_cli.web_routers.oauth` |
| `start_telegram_onboarding` | moved-lazy | `panergos_cli.web_routers.messaging` |
| `start_whatsapp_onboarding` | moved-lazy | `panergos_cli.web_routers.messaging` |
| `stat` | import | `stat` |
| `stop_gateway` | moved-lazy | `panergos_cli.web_routers.ops` |
| `stream_managed_file` | moved-lazy | `panergos_cli.web_routers.files` |
| `submit_oauth_code` | moved-lazy | `panergos_cli.web_routers.oauth` |
| `tempfile` | import | `tempfile` |
| `test_mcp_server` | moved-lazy | `panergos_cli.web_routers.mcp` |
| `test_messaging_platform` | moved-lazy | `panergos_cli.web_routers.messaging` |
| `timezone` | import | `datetime` |
| `toggle_skill` | moved-lazy | `panergos_cli.web_routers.skills` |
| `toggle_toolset` | moved-lazy | `panergos_cli.web_routers.tools` |
| `transcribe_audio_upload` | moved-lazy | `panergos_cli.web_routers.audio` |
| `trigger_cron_job` | moved-lazy | `panergos_cli.web_routers.cron` |
| `tts_lease` | moved-lazy | `panergos_cli.web_routers.audio` |
| `uninstall_skill_hub` | moved-lazy | `panergos_cli.web_routers.skills` |
| `update_config` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `update_config_raw` | moved-lazy | `panergos_cli.web_routers.analytics` |
| `update_cron_job` | moved-lazy | `panergos_cli.web_routers.cron` |
| `update_panergos` | moved-lazy | `panergos_cli.web_routers.actions` |
| `update_learning_node` | moved-lazy | `panergos_cli.web_routers.status` |
| `update_memory_provider_config` | moved-lazy | `panergos_cli.web_routers.memory_providers` |
| `update_messaging_platform` | moved-lazy | `panergos_cli.web_routers.messaging` |
| `update_profile_description_endpoint` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `update_profile_model_endpoint` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `update_profile_soul` | moved-lazy | `panergos_cli.web_routers.profiles` |
| `update_skill_content` | moved-lazy | `panergos_cli.web_routers.skills` |
| `update_skills_hub` | moved-lazy | `panergos_cli.web_routers.skills` |
| `upload_chat_image` | moved-lazy | `panergos_cli.web_routers.files` |
| `upload_managed_file` | moved-lazy | `panergos_cli.web_routers.files` |
| `upload_managed_file_stream` | moved-lazy | `panergos_cli.web_routers.files` |
| `upsert_custom_endpoint` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `validate_custom_endpoint` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `validate_provider_credential` | moved-lazy | `panergos_cli.web_routers.config_env` |
| `windows_detach_flags` | moved-lazy | `panergos_cli._subprocess_compat` |
| `windows_hide_flags` | moved-lazy | `panergos_cli._subprocess_compat` |
| `write_platform_config_field` | moved-lazy | `panergos_cli.config` |
| `yaml` | import | `yaml` |
| `zipfile` | import | `zipfile` |

### `panergos_cli.webhook`
...
| name | kind | new location |
|---|---|---|
| `AUTO_VACUUM_MIN_FREELIST_RATIO` | moved-lazy | `panergos_state_common` |
| `ActivityProvenance` | moved-lazy | `agent.session_activity` |
| `CompressionSessionBusyError` | moved-lazy | `panergos_state_errors` |
| `CompressionSessionClosedError` | moved-lazy | `panergos_state_errors` |
| `DEFERRED_INDEX_SQL` | moved-lazy | `panergos_state_common` |
| `FTS_CJK_STALE_KEY` | moved-lazy | `panergos_state_common` |
| `FTS_CJK_TABLE_SQL` | moved-lazy | `panergos_state_fts` |
| `FTS_CJK_TRIGGER_SQL` | moved-lazy | `panergos_state_fts` |
| `FTS_REBUILD_DEFERRAL_KEY` | moved-lazy | `panergos_state_common` |
| `FTS_SQL` | moved-lazy | `panergos_state_common` |
| `FTS_STALE_KEY` | moved-lazy | `panergos_state_common` |
| `FTS_STORAGE_VERSION` | moved-lazy | `panergos_state_common` |
| `FTS_TRIGRAM_SQL` | moved-lazy | `panergos_state_common` |
| `LEGACY_FTS_SQL` | moved-lazy | `panergos_state_common` |
| `LEGACY_FTS_TRIGRAM_SQL` | moved-lazy | `panergos_state_common` |
| `MAX_FTS5_QUERY_CHARS` | moved-lazy | `panergos_state_common` |
...
| `PERSISTENCE_ERROR_CAUSES` | moved-lazy | `panergos_state_errors` |
| `SCHEMA_SQL` | moved-lazy | `panergos_state_common` |
| `SCHEMA_VERSION` | moved-lazy | `panergos_state_common` |
| `SESSION_STATUS_COMPLETE` | moved-lazy | `panergos_state_sessions` |
| `SESSION_STATUS_EMPTY` | moved-lazy | `panergos_state_sessions` |
| `SESSION_STATUS_ERROR` | moved-lazy | `panergos_state_sessions` |
| `SESSION_STATUS_INTERRUPTED` | moved-lazy | `panergos_state_sessions` |
| `SKILL_EXCERPT_JOINT` | moved-lazy | `agent.skill_commands` |
| `SKILL_SCAFFOLD_SQL_LIKE` | moved-lazy | `agent.skill_commands` |
| `SessionTurnLeaseLostError` | moved-lazy | `panergos_state_errors` |
| `Set` | import | `typing` |
| `WalUnsupportedError` | moved-lazy | `panergos_state_wal` |
| `apply_durability_barriers` | moved-lazy | `panergos_state_repair` |
| `classify_session_status` | moved-lazy | `panergos_state_sessions` |
| `collect_state_db_stats` | moved-lazy | `panergos_state_dbfile` |
| `contextlib` | import | `contextlib` |
| `count_db_holders` | moved-lazy | `panergos_state_dbfile` |
| `describe_skill_invocation` | moved-lazy | `agent.skill_commands` |
| `errno` | import | `errno` |
| `fts5_cjk_so_path` | moved-lazy | `panergos_state_fts` |
| `is_advisory_lock_contention` | moved-lazy | `panergos_state_common` |
| `is_automatic_end_reason` | moved-lazy | `panergos_state_common` |
| `is_disk_full_error` | moved-lazy | `panergos_state_errors` |
| `is_sqlite_wal_reset_vulnerable` | moved-lazy | `panergos_state_wal` |
| `is_transient_sqlite_error` | moved-lazy | `panergos_state_errors` |
| `iter_deleted_sqlite_sidecar_holders` | moved-lazy | `panergos_state_dbfile` |
| `release_or_close` | moved-lazy | `panergos_state_registry` |
| `report_startup_progress` | moved-lazy | `panergos_startup_watchdog` |
| `resolve_journal_mode` | moved-lazy | `panergos_state_wal` |
| `resolve_synchronous_level` | moved-lazy | `panergos_state_wal` |
| `sanitize_context` | moved-lazy | `agent.memory_manager` |
| `sqlite_source_id` | moved-lazy | `panergos_state_wal` |
| `struct` | import | `struct` |
| `weakref` | import | `weakref` |
| `workspace_key` | moved-lazy | `panergos_state_sessions` |

### `panergos_state_registry`
...
|---|---|---|
| `Any` | import | `typing` |
| `LoginStart` | moved-lazy | `panergos_cli.dashboard_auth` |
...
| name | kind | new location |
|---|---|---|
| `DashboardAuthProvider` | moved-lazy | `panergos_cli.dashboard_auth` |
| `InvalidCodeError` | moved-lazy | `panergos_cli.dashboard_auth` |
| `RefreshExpiredError` | moved-lazy | `panergos_cli.dashboard_auth` |
| `classify_jwks_lookup_error` | moved-lazy | `panergos_cli.dashboard_auth` |
| `hashlib` | import | `hashlib` |
| `os` | import | `os` |
| `secrets` | import | `secrets` |
...
| `reset_panergos_interactive_context` | moved-lazy | `tools.approval_context` |
...
| `set_panergos_interactive_context` | moved-lazy | `tools.approval_context` |
| `shlex` | import | `shlex` |
| `sys` | import | `sys` |
| `tempfile` | import | `tempfile` |
| `time` | import | `time` |
...
| `datetime` | import | `datetime` |
| `functools` | import | `functools` |
| `get_panergos_home_override` | moved-lazy | `panergos_constants` |
| `panergos_home_key` | moved-lazy | `panergos_constants` |
| `is_truthy_value` | moved-lazy | `utils` |
| `lightpanda_engine_status` | moved-lazy | `tools.browser_tool_lightpanda_fallback` |
| `node_tool_runnable` | moved-lazy | `panergos_constants` |
...
| `PANERGOS_INDEX_TTL` | moved-lazy | `tools.skills_hub_search` |
| `PANERGOS_INDEX_URL` | moved-lazy | `tools.skills_hub_search` |
| `PanergosIndexSource` | moved-lazy | `tools.skills_hub_official` |
...
| `dataclass` | import | `dataclasses` |
| `field` | import | `dataclasses` |
| `panergos_xai_user_agent` | moved-lazy | `tools.xai_http` |
| `platform` | import | `platform` |
| `queue` | import | `queue` |
| `re` | import | `re` |
...
### `panergos_cli.observability.relay_runtime`
