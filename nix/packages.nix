# nix/packages.nix — Panergos Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./panergos-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.panergosTui;
        web = full.panergosWeb;
        desktop = full.panergosDesktop;

        update-npm-lockfile = full.panergosNpmLib.updateNpmLockfile;
      };
    };
}
