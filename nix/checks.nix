...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      panergos-agent = self'.packages.default;
      panergosVenv = panergos-agent.panergosVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "panergos-check";
                homeDirectory = "/home/panergos-check";
                stateVersion = "24.11";
              };
            }
            { services.panergos-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.panergos-agent = programs;
              services.panergos-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.panergos-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.panergos-agent);
      ...
      configKeys = pkgs.runCommand "panergos-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${panergosVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "panergos-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "panergos-build-package" { } ''
          echo "PASS: package built at ${panergos-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "panergos-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.PANERGOS_TEST = "1";
                environmentFiles = [ "/run/secrets/panergos-env" ];
                panergosHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "panergos" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "panergos" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.panergosAgentSetup.data;

            failures =
              lib.optional (names != [
                "panergos-agent"
                "panergos-backend"
              ]) "expected panergos-agent + panergos-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/panergos gateway" (argvOf "panergos-agent")
              ) "gateway process does not run `panergos gateway`: ${argvOf "panergos-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/panergos serve" (argvOf "panergos-backend")
              ) "backend process does not run `panergos serve`: ${argvOf "panergos-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "panergos-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/panergos-check/.panergos" (envOf n)) names
              ) "gateway and backend must share one PANERGOS_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.PANERGOS_HOME or null != "/home/panergos-check/.panergos"
              ) "programs.panergos-agent.enable must export PANERGOS_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "panergos-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/panergos-check/.panergos/SOUL.md" activation
              ) "panergosHomeFiles must install into PANERGOS_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads PANERGOS_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "PANERGOS_MANAGED=home-manager" (envOf n)) names
              ) "processes must report PANERGOS_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "panergos-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "panergos-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/panergos-check";
            ...
            cases = [
              ...
              {
                name = "panergosHomeFiles needs no directory";
                ok = accepts { panergosHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "panergos-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one PANERGOS_HOME ───────────────
        # `programs.enable` exports PANERGOS_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.panergos
        # while the services use the PANERGOS_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/panergos-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                panergosHome = "/home/panergos-check/.panergos-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "panergos-desktop") cfg.home.packages;
            ...
            # second Panergos runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "panergos-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.panergos-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.panergos-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one panergos-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "PANERGOS_HOME" != "/home/panergos-check/.panergos-work"
              ) "the launcher must carry PANERGOS_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "PANERGOS_HOME")})"
              ++ lib.optional (
                setValue "PANERGOS_MANAGED" != "home-manager"
              ) "the launcher must report PANERGOS_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "PANERGOS_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "PANERGOS_DESKTOP_PANERGOS" != "${lib.head agentPackages}/bin/panergos"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "PANERGOS_DESKTOP_PANERGOS")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "PANERGOS_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "PANERGOS_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "PANERGOS_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "PANERGOS_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "PANERGOS_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "panergos-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares PANERGOS_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set PANERGOS_HOME" wrapper
              ) "the launcher must carry PANERGOS_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "PANERGOS_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "PANERGOS_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? panergos-backend || cfg.launchd.agents ? panergos-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "panergos-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `panergos`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.panergos-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.panergos-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "panergos-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "panergos-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "panergos" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."panergos-agent-setup".text;

            failures =
              lib.optional (names != [
                "panergos-agent"
                "panergos-backend"
              ]) "expected panergos-agent + panergos-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/panergos gateway" (execOf "panergos-agent")
              ) "gateway unit does not run `panergos gateway`: ${execOf "panergos-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/panergos dashboard" (execOf "panergos-backend")
              ) "backend unit does not run `panergos dashboard`: ${execOf "panergos-backend"}"
              ++ lib.optional (
                units.panergos-agent.environment.PANERGOS_HOME != units.panergos-backend.environment.PANERGOS_HOME
              ) "gateway and backend must share one PANERGOS_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/panergos/.panergos/SOUL.md" activation
              ) "panergosHomeFiles must install into PANERGOS_HOME";
          ...
          in
          pkgs.runCommand "panergos-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs panergos, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.panergos-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/panergos serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec panergos directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "panergos-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "panergos-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec panergos, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/panergos dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "panergos-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $PANERGOS_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                PANERGOS_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "panergos-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'PANERGOS_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--panergos-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "panergos-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "panergos-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${panergos-agent}/bin/panergos || (echo "FAIL: panergos binary missing"; exit 1)
          test -x ${panergos-agent}/bin/panergos-agent || (echo "FAIL: panergos-agent binary missing"; exit 1)
          test -x ${panergos-agent}/bin/panergos-acp || (echo "FAIL: panergos-acp binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${panergos-agent}/bin/panergos --version 2>&1 | grep -qi "panergos" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "panergos-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in panergos panergos-agent panergos-acp; do
            test -x ${panergos-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "panergos-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking panergos --help ==="
          ${panergos-agent}/bin/panergos --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${panergos-agent}/bin/panergos --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "panergos-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${panergos-agent}/share/panergos-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${panergos-agent}/share/panergos-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "PANERGOS_BUNDLED_SKILLS" ${panergos-agent}/bin/panergos || \
            (echo "FAIL: PANERGOS_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: PANERGOS_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${panergos-agent}/share/panergos-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${panergos-agent}/share/panergos-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "PANERGOS_OPTIONAL_SKILLS" ${panergos-agent}/bin/panergos || \
            (echo "FAIL: PANERGOS_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, PANERGOS_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "panergos-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${panergos-agent}/share/panergos-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${panergos-agent}/share/panergos-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "PANERGOS_BUNDLED_PLUGINS" ${panergos-agent}/bin/panergos || \
            (echo "FAIL: PANERGOS_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: PANERGOS_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "panergos-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${panergos-agent}/share/panergos-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${panergos-agent}/share/panergos-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${panergos-agent}/share/panergos-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "PANERGOS_BUNDLED_LOCALES" ${panergos-agent}/bin/panergos || \
            (echo "FAIL: PANERGOS_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: PANERGOS_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + PANERGOS_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && PANERGOS_BUNDLED_LOCALES=${panergos-agent}/share/panergos-agent/locales \
            ${panergosVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with PANERGOS_BUNDLED_LOCALES set"; exit 1)
        ...
        # PANERGOS_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "panergos-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${panergos-agent}/share/panergos-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${panergos-agent}/share/panergos-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "PANERGOS_OPTIONAL_MCPS" ${panergos-agent}/bin/panergos || \
            (echo "FAIL: PANERGOS_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: PANERGOS_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${panergos-agent}/bin/panergos mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: panergos mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "panergos-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${panergos-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${panergos-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "PANERGOS_TUI_DIR" ${panergos-agent}/bin/panergos || \
            (echo "FAIL: PANERGOS_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: PANERGOS_TUI_DIR set in wrapper"
        ...
        # Verify PANERGOS_NODE is set in wrapper and points to Node 26+
        # (Panergos pins its toolchain to Node 26 everywhere)
        panergos-node = pkgs.runCommand "panergos-node-version" { } ''
          set -e
          echo "=== Checking PANERGOS_NODE in wrapper ==="
          grep -q "PANERGOS_NODE" ${panergos-agent}/bin/panergos || \
            (echo "FAIL: PANERGOS_NODE not set in wrapper"; exit 1)
          echo "PASS: PANERGOS_NODE present in wrapper"

          PANERGOS_NODE=$(sed -n "s/^export PANERGOS_NODE='\(.*\)'/\1/p" ${panergos-agent}/bin/panergos)
          test -x "$PANERGOS_NODE" || (echo "FAIL: PANERGOS_NODE=$PANERGOS_NODE not executable"; exit 1)
          echo "PASS: PANERGOS_NODE executable at $PANERGOS_NODE"

          NODE_MAJOR=$("$PANERGOS_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Panergos requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All PANERGOS_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify PANERGOS_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "panergos-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(PANERGOS_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking PANERGOS_MANAGED guards ==="
          check_blocked "config set" ${panergos-agent}/bin/panergos config set model foo
          check_blocked "config edit" ${panergos-agent}/bin/panergos config edit
        ...
        extra-python-packages = let
          ...
          panergosWithExtra = panergos-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "panergos-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${panergosWithExtra}/bin/panergos || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${panergosWithExtra}/bin/panergos || \
          ...
          if grep -q "PYTHONPATH" ${panergos-agent}/bin/panergos; then
        ...
        extra-dependency-groups = let
          panergosWithGroups = panergos-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "panergos-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${panergosWithGroups}"
          echo "venv: ${panergosWithGroups.panergosVenv}"
        ...
        messaging-variant = pkgs.runCommand "panergos-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.panergosVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "panergos-config-roundtrip" {
          ...
          merge_and_load() {
            local panergos_home="$1"
            export PANERGOS_HOME="$panergos_home"
            ${configMergeScript} ${nixSettings} "$panergos_home/config.yaml"
            ${panergosVenv}/bin/python3 -c '
import json, sys
