# nix/homeManagerModules.nix — the Home Manager module for panergos-agent
#
# This module is the user-level equivalent of nixosModules.default. Panergos is
...
#
# `services.panergos-agent` is the same option set on both modules. All of the
...
#   changed   addToSystemPackages      -> programs.panergos-agent.enable and
#                                        home.sessionVariables
#   added     programs.panergos-agent    the CLI and the desktop application,
#                                      because Home Manager separates an
#                                      installation from a daemon
#   changed   stateDir (+ "/.panergos")  -> panergosHome, set directly
#
# To use the module:
#   imports = [ panergos-agent.homeManagerModules.default ];
#   programs.panergos-agent = {
#     enable = true;          # the Panergos CLI on your PATH
...
{ inputs, ... }:
{
  flake.homeManagerModules.default =
    ...
    let
      ...
      cfgPrograms = config.programs.panergos-agent;
      ...
      # not reach it, and the application would open ~/.panergos while the
      # services use panergosHome. Thus the launcher carries the value itself.
      #
      # PANERGOS_MANAGED rides along only when the services are enabled. That
      ...
      desktopEnvironment = {
        PANERGOS_HOME = cfg.panergosHome;
      }
      // lib.optionalAttrs cfg.enable {
        inherit (processEnvironment) PANERGOS_MANAGED;
      }
      // lib.optionalAttrs desktopUsesService {
        PANERGOS_DESKTOP_REMOTE_URL = "http://${cfg.backend.host}:${toString cfg.backend.port}";
      };
      ...
      # token the desktop resolver throws ("PANERGOS_DESKTOP_REMOTE_URL is set
      # but PANERGOS_DESKTOP_REMOTE_TOKEN is not"), so the two variables travel
      ...
      desktopRun = lib.optional desktopUsesService ''
        if [ -r ${lib.escapeShellArg cfg.backend.sessionTokenFile} ]; then
          PANERGOS_DESKTOP_REMOTE_TOKEN="$(tr -d '\r\n' < ${lib.escapeShellArg cfg.backend.sessionTokenFile})"
          export PANERGOS_DESKTOP_REMOTE_TOKEN
        else
          echo "panergos-desktop: cannot read the session token at ${cfg.backend.sessionTokenFile}." >&2
          echo "panergos-desktop: the application starts its own backend instead of the one of the service." >&2
        fi
      '';
    ...
    in
    {
      # ── programs.panergos-agent — the installation ───────────────────────
      # Home Manager separates "install this application for me" from "run
      # this daemon". Panergos needs both, and a person can want one without
      # the other: an application with no gateway, or a headless gateway on
      # a machine with no display.
      #
      # `services.panergos-agent` stays the authority for the state and the
      # configuration. This module reads panergosHome and the backend address
      # from it, and never the reverse.
      options.programs.panergos-agent = {
        enable = lib.mkEnableOption ''
          the Panergos Agent command line application.

          This adds the `panergos` command to home.packages, and exports PANERGOS_HOME with
          home.sessionVariables. An interactive shell then uses the same
          state as `services.panergos-agent`
        '';

        package = lib.mkOption {
          type = lib.types.package;
          default = effectivePackage;
          defaultText = lib.literalExpression "config.services.panergos-agent.package";
          description = ''
            The panergos-agent package to install.

            The default follows `services.panergos-agent.package`, and applies
        ...
        desktop = {
          enable = lib.mkEnableOption ''
            the Panergos Desktop application (Electron).

            This adds `panergos-desktop` to home.packages, with an XDG
            launcher entry on Linux. The launcher starts the same Panergos
            runtime that `package` gives, and reads the PANERGOS_HOME of
            `services.panergos-agent`. Thus the application, the interactive
            shell and the services share one state directory.

            The Electron application carries its own Panergos runtime with
            the usual distribution. This module gives it the Nix package
            instead, with PANERGOS_DESKTOP_PANERGOS. It installs no second copy
            of Panergos, and it downloads nothing on the first start
          '';

          package = lib.mkOption {
            type = lib.types.package;
            default = cfgPrograms.package.panergosDesktop;
            defaultText = lib.literalExpression "config.programs.panergos-agent.package.panergosDesktop";
            description = ''
              The panergos-desktop package to use.

              The default follows `package`, and thus also
              `services.panergos-agent.extraPythonPackages` and
              `extraDependencyGroups`, because the desktop application is a
              passthru of the agent package. A package that you set here
              carries its own Panergos runtime, and this module cannot make
      ...
      options.services.panergos-agent =
        ...
        // {
          panergosHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.panergos";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.panergos"'';
            description = ''
              The value of PANERGOS_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.panergos` to it.
              This module sets PANERGOS_HOME directly. Thus an existing
              ~/.panergos continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.panergos-work";
          };

          # `installPackage` moved to `programs.panergos-agent.enable`. The
          ...
          # quiet removal gives them a machine with no `panergos` and no
          ...
          installPackage = lib.mkOption {
            ...
            visible = false;
            description = ''
              Removed. Use `programs.panergos-agent.enable` instead.
            '';
          };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfgPrograms.enable {
          ...
          home.sessionVariables.PANERGOS_HOME = cfg.panergosHome;
        })

        # A launcher from the desktop menu reads no shell profile, so the
        # PANERGOS_HOME that `programs.enable` exports does not reach it. Home
        # Manager writes only systemd.user.sessionVariables into
        # environment.d, and this module does not put PANERGOS_HOME there,
          ...
          assertions = [
            {
              ...
              # with no `panergos` on the PATH and no message.
        ...
        (lib.mkIf cfg.enable (
          lib.mkMerge [
                ...
                ++ [
                  {
                    ...
                    message = "services.panergos-agent.backend.waitFor = \"interface\" works on Linux only. Use \"hostname\" on Darwin.";
                  }
                ];
            }
            ...
            # ── Activation: directories, config, secrets, documents ────────
            {
              ...
              home.activation.panergosAgentSetup =
                lib.hm.dag.entryAfter
                  ...
                  (
                    common.mkStateScript {
                      inherit pkgs cfg;
                      inherit (cfg) panergosHome workingDirectory;
                      run = "$DRY_RUN_CMD ";
            ...
            (lib.mkIf (isLinux && cfg.gateway.enable) {
              systemd.user.services.panergos-agent = mkUnit {
            ...
            (lib.mkIf (isLinux && cfg.backend.mode != "none") {
              systemd.user.services.panergos-backend = mkUnit {
            ...
            (lib.mkIf (isDarwin && cfg.gateway.enable) {
              launchd.agents.panergos-agent = mkAgent {
                argv = common.gatewayArgv cfg;
                logName = "panergos-agent";
              };
            })

            (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
              launchd.agents.panergos-backend = mkAgent {
                argv = common.backendArgv { inherit pkgs cfg; };
                logName = "panergos-backend";
              };
            })
          ]
        ))
