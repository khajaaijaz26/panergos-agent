# nix/web.nix — Panergos Web Dashboard (Vite/React) frontend build
{ panergosNpmLib, ... }:
panergosNpmLib.buildNpmPackage {
  dirs = [
    ...
    # @panergos/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../panergos_cli/web_dist for the
