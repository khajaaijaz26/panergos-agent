# nix/nixosModules.nix — the NixOS module for panergos-agent
#
...
#
# Container mode: Panergos runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $PANERGOS_HOME/.env
# and read by Panergos at startup — no container recreation needed for env changes.
#
# Tool resolution: the Panergos wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.panergos-agent;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      panergos-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      panergosHome = "${cfg.stateDir}/.panergos";
      ...
      # PANERGOS_HOME via addToSystemPackages, so they can save settings through the
      ...
      # panergos user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "panergos-container-entrypoint" ''
        set -eu

        PANERGOS_UID="''${PANERGOS_UID:?PANERGOS_UID must be set}"
        PANERGOS_GID="''${PANERGOS_GID:?PANERGOS_GID must be set}"

        # ── Group: ensure a group with GID=$PANERGOS_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$PANERGOS_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="panergos"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$PANERGOS_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$PANERGOS_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$PANERGOS_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$PANERGOS_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="panergos"
          TARGET_HOME="/home/panergos"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$PANERGOS_UID" -g "$PANERGOS_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$PANERGOS_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$PANERGOS_UID:$PANERGOS_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure PANERGOS_HOME is owned by the target user.
        ...
        if [ -n "''${PANERGOS_HOME:-}" ] && [ -d "$PANERGOS_HOME" ]; then
          find "$PANERGOS_HOME" \! -user "$PANERGOS_UID" -exec chown "$PANERGOS_UID:$PANERGOS_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/panergos-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/panergos-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/panergos ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/panergos
          chmod 0440 /etc/sudoers.d/panergos
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$PANERGOS_UID" --regid="$PANERGOS_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $PANERGOS_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "panergos-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        panergos_bin=${containerDataDir}/current-package/bin/panergos
      '';

      # Default: /var/lib/panergos/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the panergos group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit panergosHome; };

      unitPath = common.processPath { inherit pkgs cfg; };

    in
    {
      options.services.panergos-agent =
        common.sharedOptions {
          defaultPackage = panergos-agent;
          defaultPackageText = lib.literalExpression "panergos-agent.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/panergos";
              description = "State directory. Contains .panergos/ subdir (PANERGOS_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the Panergos CLI to environment.systemPackages and export
                PANERGOS_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.panergos symlink to the service
                  stateDir. These users are automatically added to the panergos group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.panergos-agent.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the Panergos CLI to system PATH and export PANERGOS_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.panergos/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.PANERGOS_HOME = panergosHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.panergos-agent";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.panergos-agent.workingDirectory;
                optionPath = "services.panergos-agent";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.panergos-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the panergos user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.panergos-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed panergos
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure panergos is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${panergosHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${panergosHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."panergos-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  mkdir -p ${panergosHome}
                  mkdir -p ${cfg.stateDir}/home
                  mkdir -p ${cfg.workingDirectory}
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${panergosHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${panergosHome} ${cfg.workingDirectory}
                  ...
                  find ${panergosHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${panergosHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${panergosHome}/$_subdir"
                    chmod 2770 "${panergosHome}/$_subdir"
                    find "${panergosHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg panergosHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${panergosHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${panergosHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${panergosHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.panergos";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${panergosHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "panergos-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.panergos -> stateDir/.panergos for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.panergos-agent = {
              description = "Panergos Agent Gateway";
              wantedBy = [ "multi-user.target" ];
              after = [ "network-online.target" ];
              wants = [ "network-online.target" ];

              # cfg.environment and cfg.environmentFiles are written to
              # $PANERGOS_HOME/.env by the activation script. load_panergos_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.panergos-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.panergos-agent = {
              description = "Panergos Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  PANERGOS_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  PANERGOS_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env PANERGOS_UID="$PANERGOS_UID" \
                    --env PANERGOS_GID="$PANERGOS_GID" \
                    --env PANERGOS_HOME=${containerDataDir}/.panergos \
                    --env PANERGOS_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/panergos gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
