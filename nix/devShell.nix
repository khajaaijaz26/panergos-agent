...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      panergosNpmLib = self'.packages.default.passthru.panergosNpmLib;
      ...
      panergosAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "panergos-cli" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../panergos} $out/bin/panergos
          '')
        ...
        shellHook = ''
          ${panergosAgentDevShellHook}
          ${panergosNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export PANERGOS_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Panergos Agent dev shell in $PANERGOS_PYTHON_SRC_ROOT"
          echo "Ready. Run 'panergos' or 'sandbox panergos' to start."
        '';
      };
    };
}
