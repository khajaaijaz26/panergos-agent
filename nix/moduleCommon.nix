...
#
# `services.panergos-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  panergosHome, programs.panergos-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "panergos-config-attrs";
    description = "Panergos YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $PANERGOS_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Panergos Agent";
      ...
      package = mkOption {
        ...
        description = "The panergos-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Panergos configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `panergos config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $PANERGOS_HOME/.env. Panergos reads that file at each start, with
          load_panergos_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."panergos/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Panergos refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Panergos reads SOUL.md and memories/ from PANERGOS_HOME, so put those
          files in `panergosHomeFiles`.
      ...
      panergosHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into PANERGOS_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Panergos reads SOUL.md and the memory files from PANERGOS_HOME and not
          from the working directory. Declare those files here, or Panergos
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the Panergos plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Panergos discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          panergos_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as panergos (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in panergos-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `panergos gateway`.";
      };
      ...
      # ── The backend: `panergos serve` or `panergos dashboard` ──────────
      # `panergos serve` and `panergos dashboard` are the same entry point,
      # panergos_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Panergos Desktop
      ...
      # controls an external gateway, with `panergos gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Panergos
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            PANERGOS_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Panergos Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."panergos/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `panergos` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.panergos-agent.installPackage was removed. Panergos now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.panergos-agent.enable = ${lib.boolToString (value != false)};  # the panergos CLI, and PANERGOS_HOME for your shells
        programs.panergos-agent.desktop.enable = true;  # the desktop application

      `services.panergos-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "panergos-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "panergos-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "panergos-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "panergos-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "panergos-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # PANERGOS_MANAGED variable of the service. The value tells the shell
    ...
    let
      ...
      homeDocumentTree = mkDocumentTree {
        inherit pkgs;
        documents = cfg.panergosHomeFiles;
      };
    ...
    in
    ''
      # Directories. The service units and Panergos make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${panergosHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Panergos
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${panergosHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${panergosHome}/config.yaml
            ${run}chmod ${modes.config} ${panergosHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "panergos-managed" managedSystem} ${panergosHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${panergosHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${panergosHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${panergosHome}/auth.json"
        else
          ''
            if [ ! -e ${panergosHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${panergosHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree panergosHome cfg.panergosHomeFiles}
      ...
      ${run}find ${panergosHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "panergos-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${panergosHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/panergos"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps panergos as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "panergos-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "panergos-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "panergos-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          PANERGOS_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export PANERGOS_DASHBOARD_SESSION_TOKEN

          if [ -z "$PANERGOS_DASHBOARD_SESSION_TOKEN" ]; then
            echo "panergos-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "panergos-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "panergos-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "panergos-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "panergos-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "panergos-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "panergos-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Panergos Agent web dashboard and desktop backend"
    else
      "Panergos Agent backend for Panergos Desktop";

  # The environment that each Panergos process needs, from either module.
  #
  # managedSystem gives the value of PANERGOS_MANAGED. The CLI reads that
  ...
  processEnvironment =
    ...
    }:
    {
      ...
      PANERGOS_MANAGED = managedSystem;
    };
    ...
    [
      {
        ...
        message = ''
          ...
          To give Panergos an identity and a memory, use
          ${optionPath}.panergosHomeFiles instead. Those files go to
          PANERGOS_HOME. Panergos reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of PANERGOS_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
