# nix/desktop.nix — Panergos Desktop (Electron) app build + wrapper
#
# `panergosAgent` is the fully-built `.#default` package — it ships the
# `panergos` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `PANERGOS_DESKTOP_PANERGOS` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Panergos CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # PANERGOS_HOME and PANERGOS_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "panergos-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "panergos-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = panergosNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "panergos-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe panergosNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/panergos-desktop $out/bin
    cp -r ${renderer}/* $out/share/panergos-desktop/
    ...
    substituteInPlace $out/share/panergos-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/panergos-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # PANERGOS_DESKTOP_PANERGOS to the absolute path of the nix-built `panergos`
    # binary so the desktop's resolver step 4 ("existing Panergos CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/panergos-desktop \
      --add-flags "$out/share/panergos-desktop" \
      --set PANERGOS_DESKTOP_PANERGOS "${lib.getExe panergosAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/panergos.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../panergos_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/panergos-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/panergos.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/panergos.desktop
    runHook postInstall
  '';
