...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Panergos derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            panergos-agent = prev.panergos-agent.overrideAttrs (_old: {
              PANERGOS_NIX_BUILD = "1";
            });
          })
        ]
      );
  ...
  # $PANERGOS_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$PANERGOS_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        panergos-agent = prev.panergos-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "panergos-agent-env" {
    panergos-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "panergos-agent-editable-env" {
    panergos-agent = dependency-groups;
  };
}
