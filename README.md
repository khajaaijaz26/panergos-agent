...
| --- | --- | --- |
| **Full terminal provider catalog** | `/model` now exposes all **55 canonical provider routes in this build** (plus installed extensions), keeps ready routes selectable, labels unconfigured routes as **Connect**, and never attempts to run an unconfigured provider. | Enter `/model`, then type any part of a provider name to filter the list. |
| **Searchable provider and model directory** | The browser and desktop show every built-in setup path plus safely routable [models.dev](https://models.dev/) OpenAI-compatible companies, their known model names, connection state, key name, and signup link. A manual URL/model path remains available for providers missing from the directory. | Open **Models → Connect a model** or **Settings → Providers → Any AI Provider**, search a company or model, then paste its API key. |
| **Simple provider connection** | API-key, account-sign-in, local-runtime, and custom OpenAI-compatible paths reuse the canonical setup flow; secrets stay out of `config.yaml` and are redacted after storage. | Run `panergos model`, or use the provider directory in the browser/desktop interface. |
| **Direct Groq and Mistral routes** | GroqCloud and Mistral AI are first-class provider rows with guided key links, credential validation, filtered model catalogs, terminal selection, and the same browser/Desktop connection flow. | Search **Groq** or **Mistral** under Models, then paste the provider API key. |
...
| **Visible or background browser** | Each user can choose silent headless work or a real, visible Chromium window they can watch and take over; signed-in profile use remains a separate consent switch. | Open **Settings → Browser** and toggle **Headed** and **Use real profile**, or set `browser.headed` and `browser.use_real_profile` from the terminal. |
...
In the browser dashboard, open **Models → Connect a model**, search by company, paste its API key, and select **Connect**. Panergos validates the key, discovers the provider's models, and selects a sensible default for new sessions; keys are redacted after saving and never copied into `config.yaml`. The prominent **Any other AI provider or model** panel searches the broader compatible directory by company, model ID, or key name and keeps a manual URL/model path for unlisted self-hosted or OpenAI-compatible servers.
