...
- ``plugin-catalog.json`` — ``{"entries": [raw YAML mappings], "removed": [...]}`` in the loader's own
  schema; installed Panergos clients fetch this for live catalog refresh
  (``panergos_cli.plugin_catalog.LIVE_CATALOG_URL``) so new entries and removals reach them without
...
def load_catalog_entries(catalog_dir: Path) -> list[dict]:
    ...
    for path in sorted(catalog_dir.glob("*.yaml")):
        ...
        entries.append({
            ...
            "requiresPanergos": str(raw.get("requires_panergos") or "").strip(),
            ...
            "installCommand": f"panergos plugins install {name}",
        })
