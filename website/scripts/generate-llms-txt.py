#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Panergos docs site.
...
LLM reads to learn what Panergos does.

Both publish at:
  https://khajaaijaz26.github.io/panergos-agent/docs/llms.txt
  https://khajaaijaz26.github.io/panergos-agent/docs/llms-full.txt
...
SITE_BASE = "https://khajaaijaz26.github.io/panergos-agent/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Panergos as a Python Library", None),
        ("guides/use-mcp-with-panergos", "Use MCP with Panergos", None),
        ("guides/use-voice-mode-with-panergos", "Use Voice Mode with Panergos", None),
        ("guides/use-soul-with-panergos", "Use SOUL.md with Panergos", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of skills bundled with Panergos"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Panergos Agent")
    lines.append("")
    lines.append(
        "> Panergos is an independent open-source autonomous agent for coding and "
        "general computer work. It combines persistent project memory, reusable skills, "
        "durable multi-agent missions, and a messaging gateway across terminal, desktop, "
        "web, IDE, and chat surfaces. It supports local models, major hosted providers, "
        "and OpenAI-compatible endpoints."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/khajaaijaz26/"
        "panergos-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/khajaaijaz26/panergos-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Panergos Agent — Full Documentation\n",
        (
            "This file is the entire Panergos Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Panergos, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://khajaaijaz26.github.io/panergos-agent/docs\n",
        "Short index: https://khajaaijaz26.github.io/panergos-agent/docs/llms.txt\n",
        "\n---\n\n",
    ]
