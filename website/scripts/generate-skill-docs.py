...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    """Rewrite source-relative links and normalize docs-site absolute links.
    ...
    base = (
        "https://github.com/khajaaijaz26/panergos-agent/blob/main/"
        f"{source_dir}/{meta['rel_path']}"
    )
    ...
    def sub_link(m: re.Match) -> str:
        ...
        if url.startswith("/docs/"):
            return f"[{text}]({url.removeprefix('/docs')})"
...
def render_skill_page(
    ...
    # Keep the identifier stable while presenting the product name in public docs.
    display_name = (
        "Panergos Agent"
        if name == "panergos-agent"
        else name.replace("-", " ").replace("_", " ").title()
    )

    panergos_meta = (fm.get("metadata") or {}).get("panergos") or {}
    tags = panergos_meta.get("tags") or []
    related = panergos_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `panergos skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def prune_stale_skill_pages(expected: set[Path]) -> list[Path]:
    """Remove generated pages whose source skill no longer exists."""
    removed: list[Path] = []
    for source_kind in ("bundled", "optional"):
        source_root = SKILLS_PAGES / source_kind
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Panergos Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Panergos ships with a large built-in skill library copied into `~/.panergos/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Panergos also syncs bundled skills on `panergos update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.panergos/skills/` tree, it is still shipped with Panergos; restore it with `panergos skills reset <name> --restore`.",
        "",
...
def main():
    ...
    removed = prune_stale_skill_pages(expected_pages)
    if removed:
        print(f"Removed {len(removed)} stale generated skill pages")
