    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                panergos_meta = metadata.get("panergos", {})
                if isinstance(panergos_meta, dict):
                    tags = panergos_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
...
def extract_unified_index_skills():
    ...
    for entry in data.get("skills", []):
        if not isinstance(entry, dict):
            continue
        if _LEGACY_IDENTITY.search(json.dumps(entry, ensure_ascii=False)):
            continue
...
def main(argv: list[str] | None = None):
    ...
    parser.add_argument(
        "--local-only",
        action="store_true",
        help="build from tracked local skills without the generated community index",
    )
    ...
    if args.local_only:
        external, index_meta = [], None
        external_source = "local-only validation"
    else:
        unified, index_meta = extract_unified_index_skills()
        if unified is None:
            raise SystemExit(
                f"Unified skills index not found at {UNIFIED_INDEX_PATH}; "
                "run scripts/build_skills_index.py to refresh it."
            )
        external = unified
