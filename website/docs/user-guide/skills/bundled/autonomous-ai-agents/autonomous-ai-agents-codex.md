...
| Related skills | [`claude-code`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`panergos-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-panergos-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Panergos terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Panergos itself, `model.provider: openai-codex` uses Panergos-managed Codex
OAuth from `~/.panergos/auth.json` after `panergos auth add openai-codex`. For the
...
## Panergos Gateway Caveat

When invoking the Codex CLI from a Panergos gateway/service context (for example,
