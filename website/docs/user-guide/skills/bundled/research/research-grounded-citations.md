...
Ledger location: `$PANERGOS_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `PANERGOS_CITATION_LEDGER`.

## How to Run

```bash
S=~/.panergos/skills/research/grounded-citations/scripts/sources.py
...
`panergos skills install official/social-media/reddit-reading` or
`panergos skills install official/research/rss-feeds` before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `PANERGOS_CITATION_LEDGER`) if their
