---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Panergos"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Panergos"
---
...
Import an OpenClaw setup (memories, skills) into Panergos.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `panergos skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Panergos`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Panergos Migration

Use this skill when a user wants to move their OpenClaw setup into Panergos Agent with minimal manual cleanup.
...
```bash
panergos claw migrate              # Full interactive migration
panergos claw migrate --dry-run    # Preview what would be migrated
panergos claw migrate --preset user-data   # Migrate without secrets
panergos claw migrate --overwrite  # Overwrite existing conflicts
panergos claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `panergos setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_panergos.py` to:

- import `SOUL.md` into the Panergos home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Panergos memory entries
- merge OpenClaw command approval patterns into Panergos `command_allowlist`
- migrate Panergos-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Panergos working-directory configuration
- copy OpenClaw skills into `~/.panergos/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Panergos workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.panergos/tts/`
- archive non-secret docs that do not have a direct Panergos destination
...
- `scripts/openclaw_to_panergos.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.panergos/skills/migration/openclaw-migration/scripts/openclaw_to_panergos.py`

Do not guess a shorter path like `~/.panergos/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.panergos/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Panergos-compatible secrets, currently:
...
Panergos CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Panergos. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Panergos workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Panergos skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.panergos/skills/migration/openclaw-migration/scripts/openclaw_to_panergos.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.panergos/skills/migration/openclaw-migration/scripts/openclaw_to_panergos.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.panergos/skills/migration/openclaw-migration/scripts/openclaw_to_panergos.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.panergos/skills/migration/openclaw-migration/scripts/openclaw_to_panergos.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.panergos/skills/migration/openclaw-migration/scripts/openclaw_to_panergos.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.panergos/skills/migration/openclaw-migration/scripts/openclaw_to_panergos.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Panergos unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Panergos targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Panergos destination. Unsupported auth blobs must still be reported as skipped.
...
- Panergos persona state imported
- Panergos memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.panergos/skills/openclaw-imports/`
