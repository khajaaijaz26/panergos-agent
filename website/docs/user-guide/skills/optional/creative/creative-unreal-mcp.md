...
| | |
|---|---|
| Source | Optional — install with `panergos skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Panergos MCP catalog. The
...
Two halves, in this order: the editor side must be up before Panergos connects.
...
### One-time, Panergos side

    panergos mcp install unreal-engine
...
`~/.panergos/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Panergos — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Panergos connects
...
2. Start the Panergos session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Panergos
they appear as:

| Panergos tool | Purpose |
|---|---|
...
then happens at `panergos mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Panergos runs batched calls
...
- **Start order matters.** Editor + server up first, then the Panergos
...
- **Timeouts on long operations.** Panergos' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.panergos/config.yaml` for
