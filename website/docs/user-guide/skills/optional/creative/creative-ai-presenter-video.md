...
| | |
|---|---|
| Source | Optional — install with `panergos skills install official/creative/ai-presenter-video` |
| Path | `optional-skills/creative/ai-presenter-video` |
| Version | `1.0.0` |
| Author | cclank (https://github.com/cclank/lanshu-create-ai-presenter-video), ported by Panergos Agent |
| License | MIT |
| Platforms | linux, macos |
| Tags | `video`, `presenter`, `avatar`, `lipsync`, `tts`, `captions`, `creative` |
| Related skills | [`hyperframes`](/user-guide/skills/optional/creative/creative-hyperframes), [`kanban-video-orchestrator`](/user-guide/skills/optional/creative/creative-kanban-video-orchestrator), [`comfyui`](/user-guide/skills/optional/creative/creative-comfyui) |
...
> kept substantively verbatim in `references/`; Panergos adaptations live in
> this hub file. Scripts are deterministic (no network, no credentials).

## Panergos adaptations (read first)

- **Skill dir resolution** — upstream hardcoded its own agent's skills path.
  In Panergos the loader expands `${PANERGOS_SKILL_DIR}` to this skill's installed
  directory, so every command below uses that token directly:

  ```bash
  SKILL_DIR="${PANERGOS_SKILL_DIR}"
  ```
