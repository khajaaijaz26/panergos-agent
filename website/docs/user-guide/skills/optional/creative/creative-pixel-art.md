...
| | |
|---|---|
| Source | Optional — install with `panergos skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.panergos/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.panergos/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Panergos installs package this)
