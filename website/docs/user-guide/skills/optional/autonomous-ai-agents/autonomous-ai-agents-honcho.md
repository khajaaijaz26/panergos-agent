---
title: "Honcho — Configure and troubleshoot Honcho memory for Panergos"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Panergos"
---
...
Configure and troubleshoot Honcho memory for Panergos.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `panergos skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Panergos

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Panergos profile its own peer identity while sharing a unified view of the user.
...
```bash
panergos honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Panergos profile's AI peer
...
Honcho models conversations as interactions between **peers**. Panergos creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Panergos instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Panergos run |
| `global` | Single session across all directories |

Manual override: `panergos honcho map my-project-name`
...
Each Panergos profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block panergos.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `panergos.coder` host block in `honcho.json`
...
```bash
panergos honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Panergos when Honcho memory is active.
...
Config file: `$PANERGOS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `panergos honcho setup`. Ensure `memory.provider: honcho` is in `~/.panergos/config.yaml`.

### Memory not persisting across sessions
Check `panergos honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `panergos profile create <name> --clone`. For existing profiles: `panergos honcho sync`.
...
| Command | Description |
|---------|-------------|
| `panergos honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `panergos honcho status` | Show resolved config, connection test, peer info for active profile |
| `panergos honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `panergos honcho disable` | Disable Honcho for the active profile |
| `panergos honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `panergos honcho peers` | Show peer identities across all profiles |
| `panergos honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `panergos honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `panergos honcho sessions` | List known directory-to-session-name mappings |
| `panergos honcho map <name>` | Map current working directory to a Honcho session name |
| `panergos honcho identity` | Seed AI peer identity or show both peer representations |
| `panergos honcho sync` | Create host blocks for all Panergos profiles that don't have one yet |
| `panergos honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Panergos + Honcho |
| `panergos memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `panergos memory status` | Show active memory provider and config |
| `panergos memory off` | Disable external memory provider |
