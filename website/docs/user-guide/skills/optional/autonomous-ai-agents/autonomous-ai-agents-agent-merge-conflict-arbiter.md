...
| | |
|---|---|
| Source | Optional — install with `panergos skills install official/autonomous-ai-agents/agent-merge-conflict-arbiter` |
...
- Both sides' intent sources: kanban completion summaries (`terminal` running
  `panergos kanban show <task-id>`), PR bodies, or at minimum each branch's
  commit messages.
...
- Collect each side's intent: `panergos kanban show <task-id>` for completion
