...
| | |
|---|---|
| Source | Optional — install with `panergos skills install official/mcp/fastmcp` |
...
| Related skills | [`panergos-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-panergos-agent), [`mcporter`](/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Panergos or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Panergos. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.panergos/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.panergos/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Panergos integration, either:

- configure the server in `~/.panergos/config.yaml` using the `native-mcp` skill, or
...
### Panergos cannot see the deployed server

The server-building part may be correct while the Panergos config is not. Load the `native-mcp` skill and configure the server in `~/.panergos/config.yaml`, then restart Panergos.
