...
| | |
|---|---|
| Source | Optional — install with `panergos skills install official/productivity/siyuan` |
...
3. Store it in `${PANERGOS_HOME:-~/.panergos}/.env`:
   ```
...
```yaml
# In ~/.panergos/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
