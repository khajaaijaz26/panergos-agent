...
| | |
|---|---|
| Source | Optional — install with `panergos skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${PANERGOS_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${PANERGOS_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${PANERGOS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client panergos
```
...
```bash
PUBLISH="${PANERGOS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client panergos
```
...
```bash
DRIVE="${PANERGOS_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `panergos`)    |
