...
| | |
|---|---|
| Source | Optional — install with `panergos skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Panergos Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${PANERGOS_HOME:-~/.panergos}/.env`:
...
```bash
CANVAS="python $PANERGOS_HOME/skills/productivity/canvas/scripts/canvas_api.py"
