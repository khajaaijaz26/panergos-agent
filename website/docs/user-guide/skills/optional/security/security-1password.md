...
| | |
|---|---|
| Source | Optional — install with `panergos skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Panergos Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Panergos terminal calls (desktop app flow only)
...
### Service Account (recommended for Panergos)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${PANERGOS_HOME:-~/.panergos}/.env` (the skill will prompt for this on first load).
...
## Panergos Execution Pattern (desktop app flow)

Panergos terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/panergos-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/panergos-op.sock"
