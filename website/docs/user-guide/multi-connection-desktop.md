...
# Connecting Desktop to Many Panergos Instances

Register every Panergos backend you own — the local runtime, remote gateways on
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Panergos gateway…"**) deep-links straight to
...
list of Panergos gateways. Its intro says it plainly: *"Manage this device and
every Panergos gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Panergos runtime managed by this app." | automatic |
| **Remote gateway** | "A Panergos gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Panergos install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
...
4. Fill the fields:
   ...
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `panergos serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated Panergos agents
...
connection is registered) dispatches `panergos update` to every eligible
...
panel, ⌘K **Update Panergos**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `panergos serve` is running on the remote host, the
...
- **"Update Panergos Desktop to chat with agents on other connections"** — the
