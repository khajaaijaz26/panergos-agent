...
Run multiple independent Panergos agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Panergos home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Panergos state.

:::caution Give every agent its own profile
Never point two agent processes at the same profile (the same Panergos home). Both write memory automatically, and each loads the other's writes into its system prompt at session start — so two writers on one home compound each other's state until it stops being anything you configured. Profiles exist exactly to prevent this; agents that need shared memory should use an [external memory provider](/user-guide/features/memory-providers) instead.
:::
...
```bash
panergos profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Panergos profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `panergos model --quick` inside the new profile, then configure only the tools that profile needs.
:::

### Blank profile

```bash
panergos profile create mybot
```
...
```bash
panergos profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `panergos profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
```bash
panergos profile create work-backup --clone-from coder --clone-all
```
...
parks the duplicate adapter (and `panergos gateway migrate --multiplex` refuses with one
duplicate-credential blocker per platform). Configure the new profile's own bots with
`panergos -p <name> setup` or the dashboard Messaging page.

```bash
panergos profile create twin --clone --clone-channels   # keep the source's bots anyway
```
...
without a clone flag. `panergos profile list` prints the same warning for any existing
...
- gateway-wide channel policy `GATEWAY_ALLOW_ALL_USERS` / `GATEWAY_ALLOWED_USERS`;
...
The alias works with every Panergos subcommand — it's just `panergos -p <name>` under the hood.
...
```bash
panergos -p coder chat
panergos --profile=coder doctor
panergos chat -p coder -q "hello"    # works in any position
```

### Sticky default (`panergos profile use`)

```bash
panergos profile use coder
panergos chat                   # now targets coder
panergos tools                  # configures coder's tools
panergos profile use default    # switch back
```

Sets a default so plain `panergos` commands target that profile. Like `kubectl config use-context`.
...
- **`panergos profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives Panergos its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Panergos was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.panergos/profiles/coder/.env

# Edit assistant's tokens
nano ~/.panergos/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates panergos-gateway-coder systemd/launchd service
assistant gateway install     # creates panergos-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `panergos profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `panergos -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.panergos/profiles/coder/SOUL.md
```
...
default for **future CLI/gateway runs** (same as `panergos profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`panergos update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
panergos profile list           # show all profiles with status
panergos profile show coder     # detailed info for one profile
panergos profile rename coder dev-bot   # rename (updates alias + service)
panergos profile export coder   # pack into coder.tar.gz (shareable; keys stripped)
panergos profile import coder.tar.gz   # install an archive as a new profile
```
...
renamed because `~/.panergos` is the installation root. Renaming it instead
sets a **display name**, which UI surfaces show in place of the bare ID:

```bash
panergos profile rename default Harumesu   # Unicode fine: 小助手
```

The display name appears in `panergos profile list`/`show`, the `/profile`
...
stored as `display_name` in `~/.panergos/profile.yaml`; remove that line to
...
```bash
panergos profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `panergos profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.panergos`). To remove everything, use `panergos uninstall`.
:::
...
```bash
# Bash
eval "$(panergos completion bash)"

# Zsh
eval "$(panergos completion zsh)"
```
...
Profiles use the `PANERGOS_HOME` environment variable. When you run `coder chat`, the wrapper script sets `PANERGOS_HOME=~/.panergos/profiles/coder` before launching panergos. Since 119+ files in the codebase resolve paths via `get_panergos_home()`, Panergos state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.

This is separate from terminal working directory. Tool execution starts from `terminal.cwd` (or the launch directory when `cwd: "."` on the local backend), not automatically from `PANERGOS_HOME`.
...
isolated by `PANERGOS_HOME`, not by changing `HOME`. Container backends still use
`{PANERGOS_HOME}/home` for persistent tool state, and host users who need strict
per-profile tool config can opt in with `terminal.home_mode: profile`.

This means two things that are easy to mix up:

- `PANERGOS_HOME` is the profile boundary. It controls Panergos config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other Panergos
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, Panergos keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode Panergos launches tool
subprocesses with `HOME={PANERGOS_HOME}/home`; you then need to initialize or link
...
Panergos also exposes `PANERGOS_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.panergos` itself. No migration needed — existing installs work identically.
...
panergos profile export coder
panergos profile import ./coder.tar.gz --name coder
```
