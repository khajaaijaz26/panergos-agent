...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Panergos Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Panergos Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
Run `panergos model --quick`, then start working with `panergos chat`.
:::

:::tip
Panergos also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `panergos --tui` — see the [TUI](tui.md) guide.
:::
...
panergos chat --query-file prompt.txt
panergos chat --query-file - < prompt.txt

# With a specific model
panergos chat --model "anthropic/claude-sonnet-4"

# With a specific provider
panergos chat --provider openrouter  # Force OpenRouter

# With specific toolsets
panergos chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
panergos -s panergos-agent-dev,github-auth
panergos chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
panergos --continue             # Resume the most recent CLI session (-c)
panergos --resume <session_id>  # Resume a specific session by ID (-r)
panergos --resume latest        # Resume the most recent session (same as -c)
panergos --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
panergos chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
panergos -w                         # Interactive mode in worktree
panergos -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`panergos -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
panergos worktree list              # audit: age, size, verdict, reason per tree
panergos worktree list --json       # machine-readable audit (trees, external trees, branches)
panergos worktree prune             # remove safe trees + delete merged branches
panergos worktree prune --dry-run   # show the plan without changing anything
panergos worktree prune --older-than 7   # only reap trees idle for 7+ days
panergos worktree prune --trees-only     # leave local branches alone
panergos worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running Panergos session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.panergos/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `panergos -w` for days — no longer accumulate merged scratch trees
...
The `panergos plugins` commands manage native Panergos plugins and portable Agent
...
panergos plugins disable <plugin-name>
panergos plugins update <plugin-name>
panergos plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Panergos currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Panergos CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Panergos CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `PANERGOS_YOLO_MODE` is on (either `panergos --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Panergos to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.panergos/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Panergos supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Panergos replies |
...
```bash
panergos -s panergos-agent-dev,github-auth
panergos chat -s github-pr-workflow -s github-auth
```

Panergos loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.panergos/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.panergos/config.yaml`:
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Panergos also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Panergos recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Panergos — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Panergos is working, Panergos prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Panergos to the background — just like any terminal process. The shell prints a confirmation:

```
Panergos Agent has been suspended. Run `fg` to bring Panergos Agent back.
```
...
Resume this session with:
  panergos --resume 20260225_143052_a1b2c3
...
```bash
panergos --continue                          # Resume the most recent CLI session
panergos -c                                  # Short form
panergos -c "my project"                     # Resume a named session (latest in lineage)
panergos --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
panergos --resume "refactoring auth"         # Resume by title
panergos --resume latest                     # Resume the most recent session (same as -c)
panergos --resume latest --in ./my-project   # Latest session for ./my-project's workspace
panergos -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `panergos sessions rename <id> <title>` from the command line. Use `panergos sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Panergos' SQLite state database under `~/.panergos/state.db`. The database keeps:
...
Panergos immediately confirms the task and gives you back the prompt:
