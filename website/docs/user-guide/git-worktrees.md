...
title: "Git Worktrees"
description: "Run multiple Panergos agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Panergos Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Panergos so each session has a clean, isolated working directory.

## Why Use Worktrees with Panergos?

Panergos treats the **current working directory** as the project root:

- CLI: the directory where you run `panergos` or `panergos chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.panergos/config.yaml`
...
Panergos creates `.worktrees/my-experiment/` inside the repo (branch
`panergos/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `panergos-<id>`
...
git worktree add ../repo-feature feature/panergos-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/panergos-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Panergos there:

```bash
cd ../repo-feature

# Start Panergos in the worktree
panergos
```
...
git worktree add ../repo-experiment-a feature/panergos-a
git worktree add ../repo-experiment-b feature/panergos-b
```
...
Each Panergos process:

- Works on its own branch (`feature/panergos-a` vs `feature/panergos-b`).
...
- Panergos checkpoint data under `~/.panergos/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Panergos experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/panergos-checkpoints-docs`, `feature/panergos-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Panergos from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `panergos -w` (Automatic Worktree Mode)

Panergos has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `panergos/panergos-<hash>`).
...
```bash
panergos -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `panergos -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Panergos session its own clean checkout.
