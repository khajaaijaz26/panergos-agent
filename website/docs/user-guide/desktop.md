---
sidebar_position: 3
title: "Panergos Desktop"
description: "The source-built Panergos desktop app for the Panergos runtime."
---

# Panergos Desktop

The Panergos desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Panergos Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `panergos` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Panergos has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`panergos`) and **[TUI](./tui.md)** (`panergos --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`panergos dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Panergos 0.1 does not publish prebuilt desktop installers. Follow the [Panergos source installation instructions](../getting-started/installation.md), then build and launch the desktop app from the managed checkout.

If you already have Panergos installed, run
...
- **The same conversation history** as every other Panergos surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a Panergos instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Panergos Desktop to a remote backend](./features/web-dashboard.md#connecting-panergos-desktop-to-a-remote-backend).

#### Repository discovery

Panergos Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional Panergos sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `panergos desktop --cwd <path>` (or the `PANERGOS_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask Panergos to open PR**. You can also create and switch branches from here.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `panergos model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Panergos management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md). The Skills tab lists your installed skills with enable/disable toggles, and below them the full built-in optional-skills catalog that ships with Panergos — each entry has a one-click **Install** button that flips the row into the installed list once it finishes.
...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#panergos-cron).
- **Profiles** — switch between [Panergos profiles](./profiles.md) (isolated config/skills/sessions).
- **Messaging** — set up gateway channels. Telegram has a **Quick setup** card: click **Create with QR**, scan the code (or open the link) in Telegram, and Panergos creates the bot, detects your user ID for the allowlist, saves the credentials, and restarts the gateway for you. Any credential save, clear, or enable toggle keeps a **Restart now** banner on the page until the gateway has actually restarted; if a restart fails, the banner stays so you can retry or restart manually.
...
roster where every [Panergos profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by Panergos cron). The roster lives in the left
...
each other's Bot Chats directly (`panergos -p <bot> chat`). The backend teaches
...
- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update Panergos, and more.
...
The [manual update process](https://khajaaijaz26.github.io/panergos-agent/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Panergos agent, your config, and your chats stay. (Same as `panergos uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `panergos uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `panergos uninstall --full`.)
...
You can do the same from the terminal — `panergos uninstall --gui` for the GUI alone, or `panergos uninstall` / `panergos uninstall --full` for the agent too.

:::note
Running `panergos uninstall --gui` from a **source checkout** (a `panergos desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `panergos desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `panergos desktop`

To launch via the CLI, simply run `panergos desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.

On Linux, launches refresh `$XDG_DATA_HOME/applications/panergos.desktop` (by default `~/.local/share/applications/panergos.desktop`) so Panergos appears in the application menu. To preserve a hand-edited entry, disable refreshes:

```bash
panergos config set desktop.manage_launcher_entry false
```

A missing entry is still created; the flag only stops `panergos desktop` from rewriting an entry that already exists.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `panergos update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `PANERGOS_DESKTOP_CWD`)           |
| `--panergos-root PATH` | Override the Panergos source root the app uses (sets `PANERGOS_DESKTOP_PANERGOS_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `panergos` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Panergos Agent runtime into `PANERGOS_HOME` (`~/.panergos`, or `%LOCALAPPDATA%\panergos` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `PANERGOS_DESKTOP_PANERGOS_ROOT`, then a completed managed install, then a probed `panergos` on `PATH` (unless `--ignore-existing` / `PANERGOS_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `PANERGOS_DESKTOP_PANERGOS` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `panergos serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `panergos --tui`. The desktop app is **self-contained**: it runs its own `panergos serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Panergos backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.
...
- **Remote gateway** — enter the URL of a `panergos serve` backend you run yourself and sign in. This is the mode the rest of this section walks through.
...
:::info The remote backend is a running `panergos serve` process
"Remote backend" means a **`panergos serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `panergos serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.panergos/.env` (the secrets file, mode 0600):
...
panergos serve --host 0.0.0.0 --port 9119
```

Keep that `panergos serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `panergos serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `PANERGOS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.panergos/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/panergos` work if you front it with a reverse proxy)
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `PANERGOS_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `PANERGOS_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateways settings panel.
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.panergos/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `PANERGOS_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Panergos Desktop to a remote backend](./features/web-dashboard.md#connecting-panergos-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Panergos Desktop](../reference/environment-variables.md#web-dashboard--panergos-desktop).
...
`$PANERGOS_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
Panergos: **one row per plugin**, with two switch columns.
...
- **Desktop column** — the half loaded into this app. It is app-level: the
  ...
  `~/.panergos/desktop-plugins/`; the desktop half of a unified agent+desktop
...
- A half the plugin does not ship shows a dash. A desktop half whose agent
  ...
  or git remote) for that profile only. Optional extras install from their own
  trusted repositories via **Install from Git**.
...
- **Open logs** — opens `PANERGOS_HOME/logs` in your file manager. On a remote
...
Boot logs land in `PANERGOS_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.panergos/panergos-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```

**Other native downloads (e.g. the `get-windows` prebuilt on Windows) that need a mirror:** put the npm keys in `$PANERGOS_HOME/npmrc` (`%LOCALAPPDATA%\panergos\npmrc` on Windows, `~/.panergos/npmrc` elsewhere) — for example `node_get_windows_binary_host_mirror=https://<mirror>/sindresorhus/get-windows/releases/download/`. Every `npm ci`/`npm run` the updater spawns (desktop, web and TUI builds) points `NPM_CONFIG_USERCONFIG` at that file when it exists, so the config survives `panergos update`; the repo-root `.npmrc` is git-tracked and gets autostashed on every update, and `~/.npmrc` may be missed because the desktop hand-off inherits the GUI's environment. An `NPM_CONFIG_USERCONFIG` you set yourself is never overridden.
...
(Desktop, then Downloads, then Documents, ...) as Panergos touches each folder.
A single **Full Disk Access** grant covers all of them, permanently — and
with Panergos' stable signing identities it survives every update:

1. System Settings → **Privacy & Security → Full Disk Access** (or run
   `open "x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles"`)
2. Enable your terminal app — and **Panergos.app** if you use Desktop.
3. Fully quit and relaunch them once.

`panergos doctor` reports whether the current terminal context already has the
grant, and `panergos setup` shows this tip on macOS when it doesn't.
...
Panergos. Grants are stable from then on.
...
certificate once and tell Panergos to use it. The one-shot command does
...
```bash
panergos desktop --setup-tcc-identity
```

Or do it manually:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `Panergos Local Signing`, Identity Type: *Self-Signed Root*,
...
4. `panergos config set desktop.macos_signing_identity "Panergos Local Signing"`

Use `--identity <name>` with the command to create/use a differently named
certificate (default: `Panergos Local Signing`). The command is idempotent —
...
- [TUI](./tui.md) — the modern terminal UI used by `panergos --tui` and the dashboard chat tab
