...
If you only run one Panergos agent, you don't need this page — see
...
[Connecting Desktop to Many Panergos Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Panergos agents that should all
...
(`ai.panergos.gateway-<name>.plist`) or systemd user service
(`panergos-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
panergos profile create coder
panergos profile create personal-bot
panergos profile create research
...
```bash
panergos config set gateway.multiplex_profiles true
panergos gateway restart
```

Equivalently, in the default profile's `~/.panergos/config.yaml`:
...
You do **not** run `panergos gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `panergos gateway run`, `start`,
...
loop. `panergos -p coder gateway stop` refuses the same way (exit 78) when coder has no
gateway of its own — there is nothing to stop but the multiplexer, which
`panergos gateway stop` on the default profile takes down for every served profile.
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.panergos/profiles/coder/.env`; the default listener key is rejected. Under
...
- Webhook routes without `profile` remain default-profile routes and are not
  ...
  way: `panergos webhook subscribe <name> --route-profile coder` writes
  `profile: coder` into the default gateway's `webhook_subscriptions.json` and
  prints the `/p/coder/webhooks/<name>` URL (`panergos webhook ls` shows the
...
A standalone `panergos -p coder gateway run` binds coder's Twilio, LINE, Teams,
...
```
$ panergos -p coder gateway status
✓ Gateway is running via the default-profile multiplexer
  Manage it from the default profile: panergos gateway status
...
`panergos gateway status` and `panergos status` on the default profile list the same
...
home). `panergos status` on the default profile reports the multiplexer and lists
the profiles it serves (`Serves: coder, research`); `panergos -p coder status`,
`panergos -p coder gateway status` and `panergos -p coder cron status` all report
...
`HINDSIGHT_BANK_ID`, `PANERGOS_HONCHO_HOST`) are read from the routed profile's
...
`PANERGOS_LANGUAGE` and `display.language`, and `hooks.outbound[].secret_env` are
...
`PANERGOS_HOME`, without the default profile's secrets in their environment, and
...
| Dashboard actions (`panergos -p <name> …` spawned by the Desktop/dashboard) | A scrubbed child env pinned to that profile's `PANERGOS_HOME` | The child loads its own `.env`; the dashboard profile's tokens and ports are not inherited |
| Cron `.env` tuning (`PANERGOS_CRON_TIMEOUT`, `PANERGOS_MODEL` fallback, `PANERGOS_CRON_MAX_PARALLEL`, prefill file), worker / Bot Chat child env | The profile's own `.env`; children never inherit the default profile's `.env` settings or bridged `TERMINAL_*` policy | Cron defaults / model refusal, exactly as a standalone `panergos -p <name> gateway run` |
...
archived or deleted instead — `panergos profile delete <name>`, or move the
...
multiplexer started as `panergos -p <name> gateway run` always ticks its own
profile's cron store as well.

The served set is **live**. A profile created while the multiplexer is running
(`panergos profile create`, the dashboard, Desktop or the TUI) is served at once:
...
default profile's `gateway_state.json` is updated, and `panergos -p <name> gateway
...
adapters the same way, and `panergos profile rename` unroutes the old name before
...
`~/.local/bin/panergos-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: panergos-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    panergos gateway "$action"
  else
    panergos -p "$profile" gateway "$action"
  fi
}
...
```bash
panergos-gateways start      # start every configured profile
panergos-gateways stop       # stop every configured profile
panergos-gateways restart    # restart all
panergos-gateways status     # status across all
panergos-gateways list       # delegates to `panergos gateway list`
```

:::tip
The `default` profile is targeted with `panergos gateway <action>` (no `-p`),
not `panergos -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `panergos -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.panergos.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/panergos-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.panergos.gateway.plist` /
`panergos-gateway.service`.
...
```bash
# Default profile
tail -f ~/.panergos/logs/gateway.log
tail -f ~/.panergos/logs/gateway.error.log

# Named profile
tail -f ~/.panergos/profiles/<name>/logs/gateway.log
tail -f ~/.panergos/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.panergos/logs/gateway.log ~/.panergos/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
panergos logs -f                  # follow default profile
panergos -p coder logs -f         # follow one profile
panergos logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
panergos profile list             # profiles + model + gateway state
panergos-gateways status          # full status across every profile
launchctl list | grep panergos    # macOS — PIDs and labels
systemctl --user list-units 'panergos-gateway-*'   # Linux — units
```
...
```
~/.panergos/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.panergos/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
panergos config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.panergos/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=panergos --why="gateways running" \
  sleep infinity &
...
`panergos-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.panergos/.env ~/.panergos/profiles/*/.env
```
...
```bash
panergos gateway migrate --multiplex --dry-run   # print the plan and any blockers; changes nothing
panergos gateway migrate --multiplex             # apply (asks for confirmation on a TTY; -y skips)
panergos gateway migrate --standalone            # roll back to per-profile gateways
```

### What `panergos update` does
...
service) and `gateway.multiplex_profiles` is off, `panergos update` runs the same
preflight:

- **Nothing blocks it** → the migration runs automatically (the same code path
  as `panergos gateway migrate --multiplex --yes`) and prints what it did. This
...
install that is already multiplexing is left alone. `panergos update` also does
nothing when no secondary profile runs its own gateway — it never flips modes
on an install where nothing was running.

### Boundaries `panergos update` never crosses on its own

The unattended hook only folds profiles that share **one UNIX user, one service
domain and one `profiles/` tree** — the shape `panergos profile create` produces.
...
| `PANERGOS_HOME` outside `<default home>/profiles/` | a unit pinning `PANERGOS_HOME=/opt/panergos/profiles/emma` |

In that case `panergos update` prints the boundary it found plus
`panergos gateway migrate --multiplex`, and changes nothing — no unit is removed
...
findings appear as **notices** in `panergos gateway migrate --multiplex --dry-run`
...
```bash
panergos config set gateway.auto_multiplex_migration false
```

`panergos update` then leaves per-profile gateways exactly as they are, with no
...
`panergos gateway migrate --multiplex` is an explicit request and still migrates
(and is the supported way to opt back in). Absent or `true` keeps the default
behaviour described above.

The explicit command is different: `panergos gateway migrate --multiplex` with
...
`panergos profile create --clone` leaves the source's bot tokens and allowlists
...
Older clones that still carry them are flagged by `panergos profile list`.
:::
...
1. Stops each secondary profile's standalone gateway and uninstalls its
   ...
   recorded in `~/.panergos/gateway_migration.json` for rollback.
...
above). `panergos profile create` confirms this when the live multiplexer picked the
profile up; it prints the `panergos gateway restart` reminder only when it could not
reach the multiplexer (for example, a gateway started from an older build).

### Rollback

```bash
panergos gateway migrate --standalone
```
...
with `panergos config set gateway.multiplex_profiles false && panergos gateway restart`
...
flag, stop the per-profile tasks, `panergos gateway restart`). The dashboard's
System page offers the same migration as a button when the preflight finds an
eligible install.

## Updating the code

`panergos update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `panergos gateway start` after a previous `panergos gateway stop`. The
...
```bash
ps -ef | grep "panergos_cli.*-p <profile>"
cat ~/.panergos/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.panergos.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.panergos.gateway-<profile>.plist

# Linux
systemctl --user restart panergos-gateway-<profile>.service
```

### Health check

```bash
panergos doctor                  # default profile
panergos -p <profile> doctor     # one profile
```
