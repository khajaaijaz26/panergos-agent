...
- **Plugin / integration authors** wanting to add a new lane shape (a CLI worker that wraps Codex / Claude Code / OpenCode, a containerised review worker, a non-Panergos service that pulls tasks via the API).

If you're writing the worker code itself — the agent that runs *inside* a lane — the kanban lifecycle and reference details are injected into the worker's system prompt automatically (the `KANBAN_GUIDANCE` block in [`agent/prompt_builder.py`](https://github.com/khajaaijaz26/panergos-agent/blob/main/agent/prompt_builder.py)).

## The hierarchy

```text
Panergos Kanban  =  canonical task lifecycle + audit trail
...
Panergos Kanban owns lifecycle truth — `ready` → `running` → `review` / `blocked` / `done` / `archived`. Worker lanes execute work but never own that truth; everything they do flows back through the kanban kernel via the `kanban_*` tools (or, for non-Panergos external workers, via the API). Reviewers gate the transition from "code change written" to "task done."
...
The dispatcher matches `task.assignee` against either a Panergos profile name (the default lane shape) or a registered non-spawnable identifier (the plugin lane shape — see [Adding an external CLI worker lane](#adding-an-external-cli-worker-lane) below). Tasks whose assignee doesn't resolve are left on `ready` with a `skipped_nonspawnable` event so a board operator can fix them; they are not silently dropped or executed by an arbitrary fallback.

### 2. A spawn mechanism

For Panergos profile lanes, the dispatcher's `_default_spawn` runs `panergos -p <assignee> chat -q <prompt>` (or the equivalent module form when the `panergos` shim isn't on `$PATH`) inside the task's pinned workspace, with these env vars set:

| Variable | Carries |
|---|---|
| `PANERGOS_KANBAN_TASK` | the task id the worker is operating on |
| `PANERGOS_KANBAN_DB` | absolute path to the per-board SQLite file |
| `PANERGOS_KANBAN_BOARD` | board slug |
| `PANERGOS_KANBAN_WORKSPACES_ROOT` | root of the board's workspace tree |
| `PANERGOS_KANBAN_WORKSPACE` | absolute path to *this* task's workspace |
| `PANERGOS_KANBAN_RUN_ID` | the current run's id (for the lifecycle gate) |
| `PANERGOS_KANBAN_CLAIM_LOCK` | the claim lock string (`<host>:<pid>:<uuid>`) |
| `PANERGOS_PROFILE` | the worker's own profile name (for `kanban_comment` author attribution) |
| `PANERGOS_TENANT` | tenant namespace, if the task has one |

For non-Panergos lanes (registered via a plugin), the plugin supplies its own `spawn_fn` callable that gets `task`, `workspace`, and `board` and returns an optional pid for crash detection.

### Descendant process scope

A task assignment belongs to the dispatcher worker, not to every program it starts.
Panergos subprocess helpers carry a non-owner fence into shells, execution kernels,
...
Panergos-tools MCP endpoint can likewise act for its supervising worker, while the
...
The dashboard renders run history with summaries, metadata blocks, and exit-status badges. CLI users can run `panergos kanban tail <task_id>` to follow live, or `panergos kanban runs <task_id>` for the historical attempt list.

## Existing lane shapes

### Panergos profile lane (default)

The shape every kanban worker takes today: the assignee is a profile name, the dispatcher spawns `panergos -p <profile>`, the worker gets the `KANBAN_GUIDANCE` system-prompt block injected automatically, and uses the `kanban_*` tools to terminate the run. No setup beyond defining the profile.
...
The historical issue for this is [#19931](https://github.com/khajaaijaz26/panergos-agent/issues/19931) and the closed-not-merged Codex-specific PR [#19924](https://github.com/khajaaijaz26/panergos-agent/pull/19924) — those describe the original architecture proposal but didn't land a runner.
...
- [`KANBAN_GUIDANCE`](https://github.com/khajaaijaz26/panergos-agent/blob/main/agent/prompt_builder.py) — the worker + orchestrator lifecycle injected into every kanban worker's system prompt.
