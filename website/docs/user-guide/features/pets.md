...
Panergos can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<PANERGOS_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Panergos renders the sprite at full fidelity when your
...
panergos pets list
panergos pets list cat

# Install a pet and make it active in one step
panergos pets install boba --select

# Preview / animate it in your terminal (Ctrl+C to stop)
panergos pets show

# Check your setup
panergos pets doctor
```

## `panergos pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `panergos pets list [query] [--limit N]` |
| List installed pets | `panergos pets list --installed` |
| Install a pet | `panergos pets install <slug> [--select] [--force]` |
| Set the active pet | `panergos pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `panergos pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `panergos pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `panergos pets off` |
| Remove an installed pet | `panergos pets remove <slug>` |
| Diagnose setup | `panergos pets doctor` |

`panergos pets show` flags:
...
Beyond installing pre-made pets from the gallery, Panergos can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `panergos pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Panergos state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends include **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**.

- Resolution order prefers the explicitly configured backend, then OpenAI and OpenRouter when their credentials are available.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `panergos tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `PANERGOS_PET_IMAGE_PROVIDER` env var (e.g. `PANERGOS_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Panergos is
...
Run `panergos pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Panergos uses its own
  profile-scoped `<PANERGOS_HOME>/pets/` instead — install through `panergos pets`.

## See also

- The [`panergos-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-panergos-agent.md)
