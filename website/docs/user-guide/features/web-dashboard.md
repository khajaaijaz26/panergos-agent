...
The default `panergos-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.panergos/panergos-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.panergos/panergos-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.
...
- **Disk warnings** — the volume holding `~/.panergos` is nearly full
...
- The server spawns `panergos --tui` behind a POSIX pseudo-terminal
...
- `ptyprocess` — installed by the `pty` extra (`cd ~/.panergos/panergos-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
| Theme | Character |
|-------|-----------|
| **Panergos Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Panergos Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
