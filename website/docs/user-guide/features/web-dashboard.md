...
# Panergos Web Dashboard

The web dashboard is a browser-based UI for managing your Panergos Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth depends on the deployment's identity provider. Configure the model with `panergos model --quick` before connecting the dashboard.
:::
...
```bash
# Custom port
panergos dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
panergos dashboard --host 0.0.0.0

# Start without opening browser
panergos dashboard --no-open
```
...
child with the selected profile's `PANERGOS_HOME`, so the conversation runs
...
processes (manage them via `panergos -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The current `panergos-agent` install includes FastAPI, Uvicorn, and the platform PTY helper used by the dashboard and embedded Chat tab: `ptyprocess` on POSIX or `pywinpty`/ConPTY on native Windows. For an older or deliberately minimal source environment, reinstall the compatible extras with:

```bash
cd ~/.panergos/panergos-agent && uv pip install -e ".[web,pty]"
```

The `web` and `pty` extras remain compatibility aliases for existing install commands. `cd ~/.panergos/panergos-agent && uv pip install -e ".[all]"` is the easiest source-install path if you also want messaging, voice, and other optional integrations.
...
- **Disk warnings** — the volume holding `~/.panergos` is nearly full. Pressure is
  *critical* below 256 MB free, or at least 95% used with less than 1 GB free;
  it is *elevated* below 512 MB free, or at least 85% used with less than 4 GB free.
...
- Node.js (same requirement as `panergos --tui`; the TUI bundle is built on first launch)
- A working platform PTY helper: `ptyprocess` on Linux/macOS/WSL2 or `pywinpty` on native Windows. Both are included by the current package; the compatible source-install command is `cd ~/.panergos/panergos-agent && uv pip install -e ".[web,pty]"` (`[all]` also covers it).

Close the browser tab and the PTY is reaped cleanly on the server. Re-opening spawns a fresh session.

To point [Panergos Desktop](#connecting-panergos-desktop-to-a-remote-backend) at a dashboard running on another machine instead of its own bundled backend, see the remote-backend section below.
...
```ini
[Service]
EnvironmentFile=%h/.panergos/.env
ExecStart=/path/to/venv/bin/python -m panergos_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.panergos/.env` containing:

```bash
PANERGOS_DASHBOARD_BASIC_AUTH_USERNAME=admin
PANERGOS_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
PANERGOS_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `panergos config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Panergos instances with their own config, skills, and sessions.
...
Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.panergos/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `panergos skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.


### MCP

Manage [MCP](./mcp) servers without the CLI. The same `mcp_servers`
block in `config.yaml` that `panergos mcp` reads from.
...
`panergos mcp catalog` / `panergos mcp install` use.
...
Connect Panergos to any messaging platform from the browser — full parity with
`panergos setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.panergos/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Panergos versions, CPU core count + utilization, memory, disk usage of the Panergos home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Panergos version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `panergos update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `panergos curator`.
...
This re-reads `~/.panergos/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`PANERGOS_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.panergos`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/panergos/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 24h cache (the check goes through the GitHub API, never `git fetch`) |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Panergos Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Two providers ship in the box:
...
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — connect any standards-compliant OpenID Connect provider using authorization code + PKCE.
...
|-------|-----------|----------|
| `panergos dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `panergos dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If no provider is configured, Panergos refuses a non-loopback bind. Configure
the bundled username/password provider or a self-hosted OIDC provider before
exposing the dashboard. There is no unauthenticated public-bind option; keep
the dashboard on `127.0.0.1` and tunnel in if you do not want an auth provider.
...
|---------|-----------|-------|
| `PANERGOS_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `PANERGOS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `PANERGOS_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `PANERGOS_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `PANERGOS_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.panergos/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.panergos/.env <<EOF
PANERGOS_DASHBOARD_BASIC_AUTH_USERNAME=admin
PANERGOS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
PANERGOS_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.panergos/.env
```
...
```bash
panergos dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/panergos/   # required
      client_id: panergos-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `PANERGOS_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `PANERGOS_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `PANERGOS_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-panergos.json` — it defines a `panergos` realm, a **public PKCE client** (`panergos-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "panergos-dashboard",
      "name": "Panergos Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-panergos.json:/opt/keycloak/data/import/realm-panergos.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/panergos/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/panergos`). The admin console is at
...
```bash
export PANERGOS_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/panergos"
export PANERGOS_DASHBOARD_OIDC_CLIENT_ID="panergos-dashboard"
export PANERGOS_DASHBOARD_PUBLIC_URL="http://localhost:9119"
panergos dashboard --host 0.0.0.0 --port 9119 --no-open
```

`PANERGOS_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → panergos-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `PANERGOS_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/panergos"
  trusted_proxies:
    - "172.20.0.5"
```
...
first; without one, Panergos fails closed at startup. This prevents the local SPA
...
Panergos always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
panergos dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `PANERGOS_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `PANERGOS_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
2. User chooses a configured OIDC provider.
3. Panergos creates a PKCE challenge and redirects to the provider's authorization endpoint.
4. The provider returns an authorization code to `/auth/callback`.
5. The provider plugin exchanges the code, validates the ID token against the discovered JWKS, and creates the dashboard session.
6. Panergos redirects to `/` or the original deep link from `next=`.

### Cookies set

| Name | Lifetime | Notes |
|------|----------|-------|
| `panergos_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `panergos_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip. SameSite=None + Secure over HTTPS (must survive the cross-site IDP redirect chain — Chromium drops SameSite=Lax cookies set on a 302 in a cross-site chain); SameSite=Lax on loopback HTTP |
| `panergos_session_rt` | Provider-defined | Optional refresh token, when the identity provider issues one |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$PANERGOS_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.

### Custom providers


```python
# ~/.panergos/plugins/dashboard-auth-myidp/__init__.py
from panergos_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
```bash
# Quick self-hosted OIDC path.
PANERGOS_DASHBOARD_OIDC_ISSUER=https://auth.example.com/application/o/panergos/ \
PANERGOS_DASHBOARD_OIDC_CLIENT_ID=panergos-dashboard \
  panergos dashboard --host 0.0.0.0
...
#       provider: self-hosted
#       self_hosted:
#         issuer: https://auth.example.com/application/o/panergos/
#         client_id: panergos-dashboard
#
# then just:
panergos dashboard --host 0.0.0.0
...
```bash
# 1. Set the dashboard login credentials in ~/.panergos/.env (secrets file, 0600).
cat >> ~/.panergos/.env <<'EOF'
PANERGOS_DASHBOARD_BASIC_AUTH_USERNAME=admin
PANERGOS_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
panergos dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `PANERGOS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.panergos/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.panergos/.env`, so the credentials are in the environment at boot.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/panergos` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `PANERGOS_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `PANERGOS_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `PANERGOS_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `PANERGOS_DASHBOARD_BASIC_AUTH_USERNAME` / `PANERGOS_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `PANERGOS_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `panergos_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `panergos update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `panergos dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Panergos Eclipse** (`default`) | Eclipse plum + jade/coral signals, system fonts, comfortable spacing |
| **Panergos Eclipse (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
| **Panergos Porcelain** (`porcelain`) | Warm porcelain + accessible deep jade, system fonts |
