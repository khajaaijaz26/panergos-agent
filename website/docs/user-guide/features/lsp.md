...
Panergos runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Panergos
...
1. Panergos captures a baseline of current diagnostics for the file.
...
…). Panergos auto-detects the binary on PATH or in
`<PANERGOS_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Panergos at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<PANERGOS_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`panergos lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Panergos installs both packages together when you
run `panergos lsp install typescript` or auto-install fires on first
use.

## CLI

```
panergos lsp status          # service state + per-server install status
panergos lsp list            # registry, optionally --installed-only
panergos lsp install <id>    # eagerly install one server
panergos lsp install-all     # try every server with a known recipe
panergos lsp restart         # tear down running clients
panergos lsp which <id>      # print resolved binary path
```

`panergos lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <PANERGOS_HOME>/lsp/bin
...
When `install_strategy: auto`, Panergos installs binaries into
`<PANERGOS_HOME>/lsp/bin/`. NPM packages land in
`<PANERGOS_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Panergos-owned and is
...
**single process** per Panergos process: the first Python project spawns
...
**`panergos lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<PANERGOS_HOME>/lsp/bin/`. Run
`panergos lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `panergos lsp status`**
...
When `panergos lsp status` shows a `Backend warnings` section, install
...
`~/.panergos/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.panergos/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `panergos lsp restart` to clear the set;
