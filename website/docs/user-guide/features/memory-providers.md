...
Panergos Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
panergos memory setup      # interactive picker + configuration
panergos memory status     # check what's active
panergos memory off        # disable external provider
```

You can also select the active memory provider via `panergos plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.panergos/config.yaml`:
...
When a memory provider is active, Panergos automatically:
...
**Setup Wizard:**
```bash
panergos memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `panergos honcho setup` command still works (it now redirects to `panergos memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$PANERGOS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$PANERGOS_HOME/honcho.json` > `~/.panergos/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/khajaaijaz26/panergos-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho documentation](https://docs.honcho.dev/).
...
:::tip Migrating from `panergos honcho`
If you previously used `panergos honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Panergos profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Panergos profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Panergos profile. Host key `panergos` → default; `panergos.<profile>` for others. |
...
```bash
panergos profile create coder --clone
```

`--clone` creates a `panergos.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
panergos honcho sync
```

Scans every Panergos profile, creates host blocks for any profile without one, inherits settings from the default `panergos` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `panergos memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/khajaaijaz26/panergos-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho documentation](https://docs.honcho.dev/).
...
panergos memory setup    # select "openviking"
# Or manually:
panergos config set memory.provider openviking
```

`panergos memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.panergos/.env`, and for named profiles use
`~/.panergos/profiles/<profile>/.env`.
...
Peer identity is optional. By default, Panergos sends no peer ID and writes
...
Set `memory.openviking.agent: panergos` to restore the old peer-scoped writes.
...
Panergos sends `User-Agent: openviking-memory-panergos/<version>` on OpenViking
...
**Setup (Platform):**
```bash
panergos memory setup    # select "mem0" → "Platform"
# Or manually:
panergos config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.panergos/.env
```

**Setup (OSS):**
```bash
panergos memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
panergos memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
panergos memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
panergos memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
panergos memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.panergos/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.panergos/.env
```
...
**Config:** `$PANERGOS_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.panergos/.env`.
...
| `user_id` | `panergos-user` | User identifier |
| `agent_id` | `panergos` | Agent identifier |
...
**Switching modes:** Re-run `panergos memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
panergos memory setup    # select "hindsight"
# Or manually:
panergos config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.panergos/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p panergos ui start`

**Config:** `$PANERGOS_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `panergos` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Panergos Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/khajaaijaz26/panergos-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
panergos memory setup    # select "holographic"
# Or manually:
panergos config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.panergos-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$PANERGOS_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
panergos memory setup    # select "retaindb"
# Or manually:
panergos config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.panergos/.env
```
...
# Then configure Panergos
panergos memory setup    # select "byterover"
# Or manually:
panergos config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$PANERGOS_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
panergos memory setup    # select "supermemory"
# Or manually:
panergos config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.panergos/.env
```
...
Before running `panergos memory setup`, set `base_url` in
`$PANERGOS_HOME/supermemory.json`:
...
Then run `panergos memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$PANERGOS_HOME/supermemory.json`
...
| `container_tag` | `panergos` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `panergos-{identity}` → `panergos-coder`) to isolate memories per Panergos profile
...
| **Requires** | `pip install panergos-memori` + `panergos-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install panergos-memori
panergos-memori install
panergos config set memory.provider memori
panergos memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `panergos-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$PANERGOS_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$PANERGOS_HOME/` so each profile has its own credentials
