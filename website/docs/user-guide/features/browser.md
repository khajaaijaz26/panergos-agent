...
Panergos Agent includes a full browser automation toolset with multiple backend options:
...
- **Lightpanda local engine** via [Lightpanda](https://lightpanda.io) — a headless browser built from scratch in Zig for machines; instant start up, 16x lower memory and 9x faster than Chrome. Works in Browser Use mode (Panergos spawns it, no Chromium or Node needed) and with the built-in tools (automatic Chrome fallback for actions it doesn't support yet)
...
When a cloud provider is configured, Panergos auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`panergos setup tools → Browser Automation`
...
When enabled, Panergos copies your default browser's **active** profile — the one
...
`~/.panergos/browser-profile/<browser>/`, then launches your **real browser
...
When you turn the toggle back off, Panergos deletes the snapshot store
(`~/.panergos/browser-profile/`) on the next browser use, so the copied
...
an exclusive (deny-all) lock, so Panergos cannot copy them while the browser is
...
stays locked, Panergos stops the launch and asks you to close the browser and retry.
...
Set `browser.real_profile_autoclose: true` to let Panergos **offer to close the
browser for you** when it's holding the profile. Even with this on, Panergos never
closes it automatically — when the profile is locked it always stops and the
agent asks you first; only on your approval does it run `panergos browser
...
(e.g. a background/tray instance relaunched), Panergos stays blocked and tells you
...
Or configure via `panergos tools` → Browser Automation → Camofox.

Camofox is selected like any other browser backend: pick **Camofox** in `panergos tools` → Browser Automation, which writes `browser.cloud_provider: camofox` to `config.yaml`. `CAMOFOX_URL` is only the server address — setting it no longer selects the backend by itself once a browser selection exists (never-configured setups still autodetect it).

#### Persistent browser sessions

By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.panergos/config.yaml`:
...
Then fully restart Panergos so the new config is picked up.

:::warning Nested path matters
Panergos reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — Panergos ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, Panergos silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active Panergos profile, so different Panergos profiles get different browser profiles (profile isolation).

##### What Panergos does not do
- It does not force persistence on the Camofox server. Panergos only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), Panergos cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start Panergos and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Panergos after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

Panergos derives the stable `userId` from the profile-scoped directory `~/.panergos/browser_auth/camofox/` (or the equivalent under `$PANERGOS_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Panergos profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure Panergos to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` Panergos uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, Panergos calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- Panergos skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- Panergos does **not** call `DELETE /sessions/<user_id>` — that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, Panergos issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, Panergos adopts the most recently created one in that group.
3. Otherwise, Panergos adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, Panergos falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error — Panergos does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want Panergos to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, Panergos generates a per-task `session_key` (`task_<id>`) — Panergos will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and Panergos can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while Panergos runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Panergos automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.
...
Lightpanda is a **local engine** (a browser source, like "Local Browser"), not a cloud provider. Install the binary and put it on your `PATH` (see the [Lightpanda installation guide](https://lightpanda.io/docs/run-locally/installation/one-liner)), then pick **Lightpanda** in `panergos tools` → Browser Automation, or set:
...
- **Browser Use mode (the default).** Panergos launches `lightpanda serve --host 127.0.0.1 --port <free>` itself — one process per `browser_exec` session name (or per task) — and points the Browser Use CLI at it. No Chromium, Playwright or Node.js is needed. The process is reaped after `browser.inactivity_timeout`, on exit, and by the orphan sweep if Panergos crashes. All of these processes share one on-disk HTTP cache at `$PANERGOS_HOME/cache/browser-use/lightpanda/http-cache`, so repeat visits skip re-downloading assets. Panergos passes the cache flag only when the installed Lightpanda supports it (0.3.x+); older binaries simply run without a cache. To clear it, stop your Lightpanda sessions first, then delete that directory. Lightpanda has no graphical renderer, so `capture_screenshot()` is unavailable and the tool description tells the model to work text-first; it also holds one page per session, so the model is told to call `new_tab()` once and `goto_url()` afterwards (tracked upstream in [lightpanda-io/browser#1962](https://github.com/lightpanda-io/browser/issues/1962)).
- **Built-in browser tools** (`/browser use off`). Panergos drives Lightpanda through `agent-browser --engine lightpanda` over CDP, the same way it drives local Chrome, with **automatic Chrome fallback**: Lightpanda handles the actions it supports (navigate, snapshot, click, type, scroll, back, press, eval) and Panergos transparently retries on Chrome for anything it doesn't. Screenshots and `browser_vision` are routed straight to Chrome.

**When the engine is ignored.** `browser.engine` is the lowest-precedence browser setting: a cloud provider (on never-configured setups, any `BROWSERBASE_API_KEY` / `BROWSER_USE_API_KEY` in `~/.panergos/.env` auto-selects one), Camofox, a `browser.cdp_url` / `/browser connect` override, or `browser.use_real_profile` all take precedence. Picking Lightpanda in `panergos tools` writes `cloud_provider: local` for you; `/browser status` and `panergos doctor` report when the engine is configured but shadowed, and by what.

### Local Chromium-family browser via CDP (`/browser connect`)

Instead of a cloud provider, you can attach Panergos browser tools to your own running Chrome, Brave, Chromium, or Edge instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start Panergos from the terminal (`panergos` or `panergos chat`) and issue `/browser connect` there.
:::
...
brave-browser \
  ...
  --user-data-dir=$HOME/.panergos/chrome-debug \
  --no-first-run \
...
Then launch the Panergos CLI and run `/browser connect`.
...
If Panergos runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects Panergos itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let Panergos talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through Panergos MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Panergos can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
# Browser Use mode (default) "lightpanda" makes Panergos spawn `lightpanda serve`;
...
# Extra Chromium launch flags (comma- or newline-separated). Panergos auto-injects
...
# Panergos doesn't add automatically; setting it disables the auto-injection.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `panergos config set toolsets '["panergos-cli", "browser"]'`.
:::
...
```
Navigate to https://github.com/khajaaijaz26/panergos-agent
```
...
```
Type "panergos agent" into the search field @e3
```
...
Screenshots are stored in `~/.panergos/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.panergos/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Panergos automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
...
- **Snapshot size** — large pages are truncated at `browser.snapshot_threshold` (default 15,000 characters, matching `web_extract`; no LLM summarization); the complete snapshot is saved to `~/.panergos/cache/web/` and the output points at it for `read_file` paging
