...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Panergos CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Panergos CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
Or set the default skin in `~/.panergos/config.yaml`:
...
Every built-in skin keeps the `Panergos Agent` name and the shared Panergos wordmark and Relay. A skin can change the palette and spinner character, but not the product identity.

| Skin | Description | Agent branding | Visual character |
|------|-------------|----------------|------------------|
| `default` | Panergos Eclipse — plum, coral, jade, and gold | `Panergos Agent` | Deep plum surfaces, coral accents, jade actions and success states, gold highlights, and the shared Panergos Relay. |
...
| `banner_hero` | string | Rich-markup hero art (replaces the default Panergos Relay art) | `""` |

## Custom skins

Create YAML files under `~/.panergos/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.panergos/skins/mytheme.yaml
...
- Built-in skins load from `panergos_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.panergos/skins/` take precedence over built-in skins with the same name.
