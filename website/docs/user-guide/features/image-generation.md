...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `panergos tools`.
...
Panergos Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `panergos tools` and persists in `config.yaml`.
...
Navigate to **🎨 Image Generation**, pick your backend (FAL.ai, OpenAI, xAI, Krea, or another installed provider), then the picker shows all supported models in a column-aligned table — arrow keys to navigate, Enter to select:
...
`image_gen.provider` is the single selection key. A vendor name (`fal`, `openai`, `xai`, `krea`, ...) routes directly using that provider's credentials. The runtime always follows this stored selection; for example, `provider: fal` without `FAL_KEY` fails with actionable guidance instead of silently rerouting. Change providers via `panergos tools`, not by adding or removing keys.

`max_parallel_requests` defaults to `4`. Panergos clamps it to at least one and
...
new models appear in the picker as soon as OpenRouter serves them; no Panergos
...
only for now; responses are saved to `$PANERGOS_HOME/cache/images/`.

## FAL: GPT Image 2.5

Select **GPT Image 2.5 Flare** or **GPT Image 2.5 Sunburst** under
`panergos tools` → Image Generation → FAL.ai. The model IDs are:
...
```bash
panergos config set image_gen.provider fal
panergos config set image_gen.model openai/gpt-image-2.5/flare/text-to-image
```
...
Both accept up to 16 source images. Panergos pins quality to `medium`, matching its
...
Select them through `panergos tools` → Image Generation → OpenAI, or set:

```bash
panergos config set image_gen.provider openai
panergos config set image_gen.openai.model gpt-image-2.5-flare
```
...
as a tool the chat model may call, and Panergos cannot force the call — the
...
3. **Submission** — the selected provider adapter submits the request using that provider's configured credentials.
...
- **Requires credentials** for the active backend (for example FAL `FAL_KEY`, `OPENAI_API_KEY`, xAI OAuth, or `KREA_API_KEY`)
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; Panergos materializes them to the local cache so delivery still works after expiry
