...
Panergos Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Panergos integration. It
...
**Fresh installs already have the driver.** The Panergos installer
...
- **`panergos tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
panergos computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `panergos computer-use
status` to verify the install.

Already have cua-driver? Panergos reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `panergos update`, and the first
`computer_use` call of a session, Panergos checks the local version and
...
selected with `PANERGOS_CUA_DRIVER_CMD` stays
under your control, so Panergos reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Panergos autodetection is a
planned cua-driver follow-up, so currently point Panergos at that directory or
...
interface. The built-in toolset provides Panergos actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `panergos computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Panergos host identity. |
...
```
panergos -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.panergos/config.yaml`.

## Permission modes and logged-in browser profiles

Panergos maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Panergos session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Panergos approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Panergos risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.panergos/cua-manifest.yaml
```
...
for the format). Panergos launches a private runtime with
...
identity instead of resetting with every Panergos build), and Panergos verifies
...
resetting or closing the Panergos session, cancellation cleanup, or process exit
closes that transport session. Panergos also stops private runtimes that it
launched for bounded or unrestricted access. One Panergos
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Panergos host identity.
...
## `panergos computer-use doctor` — your first triage stop

`panergos computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ panergos computer-use doctor
...
Panergos run declares a public cua-driver **session name** (something like
`panergos-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Panergos disables it automatically where it is a known failure
...
Panergos keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Panergos-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Panergos
autodetection is a planned cua-driver follow-up, so currently point Panergos at
...
These are **platform deep dives, not duplicates of the Panergos skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Panergos
...
image capture also saves a bounded, shareable copy under Panergos' image cache
...
Panergos applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.panergos/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Panergos applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Panergos agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Panergos agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Panergos warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Panergos from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. Panergos passes the
...
```
PANERGOS_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
PANERGOS_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Panergos disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Panergos sets
...
When it's on, `panergos computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Panergos at a binary you built from source instead
of the published release. Panergos resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`PANERGOS_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Panergos testing (see notes).
...
### Option B — point Panergos straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`PANERGOS_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
PANERGOS_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
PANERGOS_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Panergos is using your build

- `panergos computer-use status` prints the resolved binary path and
  version.
- `panergos computer-use doctor` confirms the binary is reachable and
...
- **Panergos spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Panergos YOLO, Panergos instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Panergos
  change needed either way.
- **Local builds skip the version check.** Panergos warns when the
...
**First action when anything's off: run `panergos computer-use doctor`.**
...
Run `panergos computer-use install` to fetch the cua-driver binary, or
run `panergos tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `panergos computer-use doctor` will
...
- **Panergos-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Panergos `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Panergos autodetection is a
  planned follow-up; currently point Panergos at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Panergos among them)
