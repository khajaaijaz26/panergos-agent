...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Panergos' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Panergos")

The wake word turns Panergos into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Panergos listens in the background for a spoken
trigger phrase. Say it, and Panergos starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Panergos backend (for example a
...
Panergos supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey panergos"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey panergos"** — a model for it ships with Panergos, so
...
```bash
cd ~/.panergos/panergos-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `panergos` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.panergos/config.yaml`,
...
wake_word:
  ...
  phrase: "hey panergos"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_panergos         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey panergos" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey panergos" utterances are missed.
...
default) to let Panergos choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Panergos installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Panergos surfaces, and `surface` picks which one
...
| `cli` | Only the classic `panergos` CLI. |
| `tui` | Only `panergos --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Panergos surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Panergos does not silently fail over to another open surface.
...
"Hey Panergos" works out of the box — the bundled openWakeWord model
(`model: hey_panergos`) is the default. To wake on something else, the easiest
...
command (`panergos -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.panergos/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("panergos" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Panergos" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey panergos"
  porcupine:
    keyword: ~/.panergos/wakewords/hey_panergos.ppn
```

Set your access key in `~/.panergos/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `panergos tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `panergos-agent[wake]`).
...
but the phrase never fires. Panergos detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Panergos
backend (it may appear as your terminal, `python`, or Panergos), then toggle the
...
```bash
panergos config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
panergos config set wake_word.input_device null
```
