...
When Panergos Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.panergos/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.panergos/config.yaml` (switchable per session
via `/personality`).
...
If you use Panergos' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.panergos/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Panergos Agent's deliverable
