...
Panergos supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.panergos/config.yaml`:
...
Or set the env var: `PANERGOS_KANBAN_DISPATCH_IN_GATEWAY=false`
