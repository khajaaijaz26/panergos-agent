...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Panergos Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Panergos Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Panergos](../../guides/use-voice-mode-with-panergos.md).

For hands-free session start — saying "hey panergos" (or any phrase) to open a fresh voice session on the CLI, TUI, or desktop app — see [Wake Word](/user-guide/features/wake-word).

## Prerequisites

Before using voice features, make sure you have:

1. **Panergos Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `panergos model` or set your preferred provider credentials in `~/.panergos/.env`
3. **A working base setup** — run `panergos` to verify the agent responds to text before enabling voice

:::tip
The `~/.panergos/` directory and default `config.yaml` are created automatically the first time you run `panergos`. You only need to create `~/.panergos/.env` manually for API keys.
:::
...
cd ~/.panergos/panergos-agent && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.panergos/panergos-agent && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.panergos/panergos-agent && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.panergos/panergos-agent && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`panergos chat`) and the **TUI** (`panergos --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.panergos/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
panergos                # Start the interactive CLI
```
...
1. Start the CLI with `panergos` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.panergos/config.yaml` (default: `ctrl+b`).
:::
...
```bash
panergos gateway        # Start the gateway (connects to configured platforms)
panergos gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@panergosbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.panergos/.env`:
```bash
...
```bash
panergos gateway        # Start with existing configuration
```
...
If you are running Panergos inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.panergos/.env`
...
- Review gateway logs: `tail -f ~/.panergos/logs/gateway.log`
