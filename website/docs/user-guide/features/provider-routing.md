...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Panergos Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.panergos/config.yaml`:
...
own pins. Edit `config.yaml` directly for these keys: model ids contain dots, which `panergos config set`
...
- **CLI mode** — configured in `~/.panergos/config.yaml`, loaded at startup
