...
description: "Customize Panergos Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Panergos Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `PANERGOS_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Panergos is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Panergos now seeds a default `SOUL.md` automatically in:

```text
~/.panergos/SOUL.md
```

More precisely, it uses the current instance's `PANERGOS_HOME`, so if you run Panergos with a custom home directory, it will use:

```text
$PANERGOS_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Panergos creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Panergos loads `SOUL.md` only from `PANERGOS_HOME`
- Panergos does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Panergos falls back to a built-in default identity
...
If Panergos loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `PANERGOS_HOME`, the personality belongs to the Panergos instance itself.

That also makes it easier to teach users:
- "Edit `~/.panergos/SOUL.md` to change Panergos' default personality."
...
- how Panergos should handle uncertainty, disagreement, or ambiguity
...
## What Panergos injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Panergos falls back to its built-in independent identity ("You are Panergos Agent, an independent open-source autonomous agent. Be direct..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Panergos ships with built-in personalities you can switch to with `/personality`.
...
These are convenient overlays, but your global `SOUL.md` still gives Panergos its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.panergos/config.yaml` under `agent.personalities` (a top-level `personalities:` block works too; `agent.personalities` wins if the same name appears in both).
...
1. Keep a thoughtful global `SOUL.md` in `~/.panergos/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-panergos)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Panergos speaks
- `display.skin` and `/skin` affect how Panergos looks in the terminal
