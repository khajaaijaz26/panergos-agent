...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Panergos automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
:::tip
Credential pools are mainly for API-key providers and multi-account rotation. Most single-account setups do not need one.
:::
...
If you already have an API key set in `.env`, Panergos auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
panergos auth add openrouter --api-key sk-or-v1-your-second-key

# ...or let a browser login mint one (OpenRouter OAuth PKCE; stored as a plain API key)
panergos auth add openrouter --type oauth

# Add a second Anthropic key
panergos auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
panergos auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  panergos_pkce          oauth   id=ab12cd34 priority=0 panergos_pkce ←
...
`panergos auth remove <provider> <target>` when a label is ambiguous, and `priority=` is
the order the pool tries credentials in under the `fill_first` strategy.

## Interactive Management

Run `panergos auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `panergos auth` | Interactive pool management wizard |
| `panergos auth list` | Show all pools and credentials |
| `panergos auth list <provider>` | Show a specific provider's pool |
| `panergos auth add <provider>` | Add a credential (prompts for type and key) |
| `panergos auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `panergos auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `panergos auth add <provider> --priority 0` | Add a credential and place it first in the `fill_first` order |
| `panergos auth priority <provider> <target> <n>` | Move a credential to priority `n` (0 = tried first); the rest are renumbered |
| `panergos auth remove <provider> <index>` | Remove credential by 1-based index |
| `panergos auth reset <provider>` | Clear all cooldowns/exhaustion status |
| `panergos auth reset <provider> <target>` | Clear the cooldown on one credential by index, id, or label |
| `panergos auth refresh <provider> [target]` | Refresh one OAuth credential's tokens and return it to rotation (proves the grant is alive; the next request re-checks quota) |
...
Configure via `panergos auth` → "Set rotation strategy" or in `config.yaml`:
...
| Strategy | Behavior |
|----------|----------|
| `fill_first` (default) | Use the first healthy key until it's exhausted, then move to the next; order is each credential's `priority` (`panergos auth priority` changes it) |
...
When you set up a custom endpoint via `panergos model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via panergos model:
panergos auth list
# Shows:
...
panergos auth add Together.ai --api-key sk-together-second-key
```
...
Panergos automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Panergos PKCE OAuth | `~/.panergos/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `panergos auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `panergos auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Panergos can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Panergos-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`panergos_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`panergos_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`agent/turn_api_error.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.panergos/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Panergos' credential store, so its token remains persistable.
