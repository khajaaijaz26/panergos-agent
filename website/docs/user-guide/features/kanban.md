...
description: "Durable SQLite-backed task board for coordinating multiple Panergos profiles"
---
...
Panergos Kanban is a durable task board, shared across all your Panergos profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.panergos/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.
...
The board has two front doors, both backed by the same `~/.panergos/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `panergos kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `panergos kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `PANERGOS_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
panergos kanban boards list

# Create a new board.
panergos kanban boards create atm10-server \
    --name "ATM10 Server" \
...
panergos kanban --board atm10-server list
panergos kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
panergos kanban boards switch atm10-server
panergos kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
panergos kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `PANERGOS_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.panergos/kanban/current` — the slug persisted by `panergos kanban
   boards switch`.
4. `default`.
...
`panergos dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<panergos-home>/kanban/attachments/<task_id>/` for the default board, or
  `<panergos-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `PANERGOS_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
panergos kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
panergos kanban watch

# 5. See the board (you)
panergos kanban list
panergos kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `panergos kanban show t_abcd`.
...
Override the config flag at runtime via `PANERGOS_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `panergos gateway
...
until one comes up — `panergos kanban create` warns about this at creation
time.

Running `panergos kanban daemon` as a separate process is **deprecated**;
...
panergos kanban create "nightly ops review" \
    --assignee ops \
...
```bash
panergos kanban complete t_abc t_def t_hij --result "batch wrap"
panergos kanban archive  t_abc t_def t_hij
panergos kanban unblock  t_abc t_def
panergos kanban block    t_abc "need input" --ids t_def t_hij
```
...
```bash
panergos -p planner tools enable kanban                      # CLI / TUI / Desktop chats
panergos -p planner tools enable kanban --platform telegram  # a gateway platform
```

Each platform has its own selection under `platform_toolsets.<platform>` in
`config.yaml`; the toolset is also a checkbox in `panergos tools` and the dashboard.
...
**Workers do not shell out to `panergos kanban`.** When the dispatcher spawns a worker it sets `PANERGOS_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `panergos kanban` CLI.
...
kanban_show()                                     # no args — uses PANERGOS_KANBAN_TASK
...
### Why tools instead of shelling to `panergos kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `panergos kanban complete` *inside* the container, where `panergos` isn't installed and `~/.panergos/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.panergos/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `panergos chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `PANERGOS_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/panergos_cli/test_kanban_db.py -q"],
...
2. `cd $PANERGOS_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
**Agent-side prevention:** Before the worker exits, Panergos injects up to two
...
guard is active only for dispatcher-spawned workers (`PANERGOS_KANBAN_TASK` is
set) and can be disabled with `PANERGOS_KANBAN_STOP_NUDGE=0`.
...
```bash
panergos kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

panergos kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `panergos skills list` to see what's available); there's no runtime install.
...
```bash
# At creation
panergos kanban create "hard refactor" --assignee coder \
    --model claude-opus-4.6 --provider anthropic

# Or later — takes effect on the next dispatch
panergos kanban set-model t_abcd claude-opus-4.6 --provider anthropic
panergos kanban set-model t_abcd none    # clear the override
```
...
Kanban's per-profile configs make the planner/worker cost split natural. Decomposing a project into well-scoped cards takes frontier-level judgment; executing a card that already carries a clear goal, context, and handoff evidence usually doesn't — and the workers are where the vast majority of tokens are spent, so the worker model is where the cost lives. Run your orchestrator/dispatcher profile on a frontier model and point worker profiles at inexpensive models. Each profile has its own `config.yaml` under `~/.panergos/profiles/<name>/`, and the dispatcher injects the profile-scoped `PANERGOS_HOME` when it spawns `panergos -p <assignee>`, so each worker reads its own profile's model settings:

```yaml
# ~/.panergos/config.yaml (orchestrator / dispatcher profile)
model:
  default: "your-frontier-model"

# ~/.panergos/profiles/coder/config.yaml (worker profile)
model:
  default: "your-inexpensive-model"

# ~/.panergos/profiles/researcher/config.yaml (another worker profile)
model:
  default: "your-inexpensive-model"
```

For the occasional quality-sensitive card, pin just that task back to a stronger model with the [per-task model override](#per-task-model-override) (`--model`/`--provider` at create time, `panergos kanban set-model` later, or the dashboard's model dropdown) — no profile edits needed.
...
```bash
panergos kanban create "Translate the docs site to French" \
...
```bash
panergos kanban init      # one-time: create kanban.db if not already present
panergos dashboard        # "Kanban" tab appears in the nav, after "Skills"
```
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `panergos kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.
...
Config knobs (all under `kanban:` in `~/.panergos/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `panergos profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.panergos/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
```
panergos kanban init                                     # create kanban.db + print daemon hint
panergos kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
panergos kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
panergos kanban show <id> [--json]
panergos kanban assign <id> <profile>                    # or 'none' to unassign
panergos kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
panergos kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
panergos kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
panergos kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
panergos kanban diagnostics [--json]                     # board health snapshot (alias: diag)
panergos kanban link <parent_id> <child_id>
panergos kanban unlink <parent_id> <child_id>
panergos kanban claim <id> [--ttl SECONDS]
panergos kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
panergos kanban complete <id>... [--result "..."]
panergos kanban block <id> "<reason>" [--ids <id>...]
panergos kanban unblock <id>...
panergos kanban archive <id>...

panergos kanban request-review <id> [--summary "..."] [--metadata JSON] [--reviewer PROFILE]
panergos kanban request-changes <id> "<required changes>"               # active reviewer -> implementer
panergos kanban reopen-review  <id>... [--reason "..."]                 # changes requested: 'review' -> ready/todo

panergos kanban tail <id>                                # follow a single task's event stream
panergos kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
panergos kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
panergos kanban runs <id> [--json]                       # attempt history (one row per run)
panergos kanban assignees [--json]                       # profiles on disk + per-assignee task counts
panergos kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
panergos kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `panergos gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
panergos kanban stats [--json]                           # per-status + per-assignee counts
panergos kanban log <id> [--tail BYTES]                  # worker log from ~/.panergos/kanban/logs/
panergos kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
        [--chat-type dm|group|channel|thread] [--delivery-mode notify|notify+wake|wake]
panergos kanban notify-list [<id>] [--json]
panergos kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
panergos kanban context <id>                             # what a worker sees
panergos kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
panergos kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
panergos kanban create "nightly backup audit" \
...
Every `panergos kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `panergos chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `panergos_cli.kanban.run_slash()` entry point that reuses the `panergos kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `panergos kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.panergos/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Dispatcher workers creating tasks through `kanban_create` or `panergos kanban create`
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`panergos kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `panergos kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |
...
_metadata_: `{"changed_files": ["panergos_cli/retry.py", "tests/test_retry.py"], "decisions": ["capped backoff at 60s", "jitter = full"]}`
```
...
panergos kanban create "Fix CI failure from t_impl: test_retry flakes on 3.11" \
    --assignee coder \
    --parent t_impl \
    --body "$(cat <<'EOF'
...
[`agent-merge-conflict-arbiter` optional skill](https://github.com/khajaaijaz26/panergos-agent/blob/main/optional-skills/autonomous-ai-agents/agent-merge-conflict-arbiter/SKILL.md)
...
```
hotspot: panergos_cli/kanban_db.py — third conflicting edit to the dispatch loop this wave
```
...
```bash
panergos kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7 \
    --chat-type group --delivery-mode notify+wake
panergos kanban notify-list
panergos kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
panergos kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
panergos kanban runs t_abcd
...
**Bulk close caveat.** `panergos kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`panergos kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `panergos kanban heartbeat $TASK` to signal liveness during long operations. |
...
`panergos kanban tail <id>` shows these for a single task. `panergos kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.panergos/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.
