...
fills the password through Panergos. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): Panergos generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `panergos vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, Panergos
...
in, Panergos picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). Panergos hands the master password to the manager's
...
Prefer not to use a detected manager? `panergos vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `panergos vault add`), bound to the checkout site,
...
- **CLI**: `panergos vault list`, `panergos vault add`, `panergos vault rm <handle>`,
  `panergos vault sources`.

Items live encrypted under `~/.panergos/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `panergos chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through Panergos: not in
