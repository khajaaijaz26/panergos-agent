...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Panergos' built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Panergos instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
panergos memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$PANERGOS_HOME/honcho.json` (profile-local). The setup wizard handles this for you.

### Self-Hosted Honcho with Authentication

When pointing Panergos at a self-hosted Honcho server, `panergos honcho setup` (and `panergos memory setup`) ask for a **local JWT / bearer token** after the base URL. Paste a JWT signed with the server's `AUTH_JWT_SECRET` (the Honcho compose env var) to enable authenticated access; leave it blank for servers running with `AUTH_USE_AUTH=false`. The local token is stored under the host block (`hosts.<host>.apiKey` in `honcho.json`), separate from any cloud `apiKey`, so you can flip the `Cloud or local?` prompt back to `cloud` later without losing either credential.
...
- `per-session` — each `panergos` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Panergos syncs them back at session init.
...
The `panergos honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `panergos memory setup honcho` (or run `panergos memory setup` and pick it from the list); the `panergos honcho` subcommand then appears on the next invocation.

```bash
panergos memory setup honcho    # Configure Honcho directly (works before activation)
panergos honcho status          # Connection status, config, and key settings
panergos honcho setup           # Redirects to `panergos memory setup` (post-activation alias)
panergos honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
panergos honcho peer            # Show or update peer names + dialectic reasoning level
panergos honcho mode            # Show or set recall mode (hybrid/context/tools)
panergos honcho tokens          # Show or set token budget for context and dialectic
panergos honcho identity        # Seed or show the AI peer's Honcho identity
panergos honcho sync            # Sync Honcho config to all existing profiles
panergos honcho peers           # Show peer identities across all profiles
panergos honcho sessions        # List known Honcho session mappings
panergos honcho map             # Map current directory to a Honcho session name
panergos honcho enable          # Enable Honcho for the active profile
panergos honcho disable         # Disable Honcho for the active profile
panergos honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `panergos honcho`

If you previously used the standalone `panergos honcho setup`:
...
No re-login or re-setup needed. Run `panergos memory setup` and select "honcho" — the wizard detects your existing config.
