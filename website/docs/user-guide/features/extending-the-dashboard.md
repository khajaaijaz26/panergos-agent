...
description: "Build themes and plugins for the Panergos web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Panergos web dashboard (`panergos dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.panergos/dashboard-themes/`; it appears in the theme switcher.
...
This page covers the **web dashboard** (`panergos dashboard`) plugin system — `window.__PANERGOS_PLUGIN_SDK__`, a `manifest.json`, and a pre-built JS bundle. The **native desktop app** (`panergos desktop`) has its own, unrelated SDK — `@panergos/plugin-sdk`, a single ESM file, no build step — documented at [Desktop Plugin SDK](/developer-guide/desktop-plugin-sdk). Only the backend `plugin_api.py` namespace (`/api/plugins/<name>`) is shared between them.
:::

:::note How the pieces compose
Themes and plugins are independent but synergistic. A theme can stand alone
(just a YAML file), and a plugin can stand alone (just a tab). Together they
can build a complete visual reskin with custom HUDs.
:::
...
- [Combining a theme and plugin](#combining-a-theme-and-plugin)
...
Themes are YAML files stored in `~/.panergos/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.panergos/dashboard-themes
```

```yaml
# ~/.panergos/dashboard-themes/neon.yaml
name: neon
label: Neon
...
The CSS is injected as a single scoped `<style data-panergos-theme-css>` tag on theme apply and cleaned up on theme switch. **Capped at 32 KiB per theme.**
...
|-------|---------|------------|--------|
| **Panergos Eclipse** (`default`) | Eclipse plum + jade/coral signals | System stack, 15px | 0.5rem radius, comfortable |
| **Panergos Eclipse (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
| **Panergos Porcelain** (`porcelain`) | Warm porcelain + accessible deep jade | System stack, 15px | 0.5rem radius, comfortable |
...
Themes that reference Google Fonts load the stylesheet on demand—the first time you switch to one, a `<link>` tag is injected into `<head>`. Panergos Eclipse and Porcelain use the system stack and need no font download.
...
```yaml
# ~/.panergos/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Panergos plugins in `~/.panergos/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.

Plugins don't bundle React or UI components. They use the **Plugin SDK** exposed on `window.__PANERGOS_PLUGIN_SDK__`. This keeps plugin bundles tiny (typically a few KB) and avoids version conflicts.
...
```bash
mkdir -p ~/.panergos/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.panergos/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.panergos/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
  ...
  window.__PANERGOS_PLUGINS__.register("my-plugin", MyPage);
})();
```
...
Everything a plugin needs is on `window.__PANERGOS_PLUGIN_SDK__`. Plugins should never import React directly.
...
// Panergos API client + raw fetcher
...
#### Calling built-in Panergos endpoints
...
```json
// ~/.panergos/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.panergos/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__PANERGOS_PLUGIN_SDK__;
  const { React } = SDK;
  ...
  window.__PANERGOS_PLUGINS__.register("session-notes", function () { return null; });

  // The real work.
  window.__PANERGOS_PLUGINS__.registerSlot("session-notes", "sessions:top", Banner);
})();
```
...
The snippet above is a complete slot-registration example and can be copied
into any dashboard plugin bundle.
...
```python
# ~/.panergos/plugins/my-plugin/dashboard/plugin_api.py
...
#### Accessing Panergos internals

Backend routes run inside the dashboard process, so they can import from the panergos-agent codebase directly:
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.panergos/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.panergos/plugins/<name>/dashboard/` | `project` — only when `PANERGOS_ENABLE_PROJECT_PLUGINS` is set |
...
…or restart `panergos dashboard`.

#### Plugin load lifecycle

1. Dashboard loads. `main.tsx` exposes the SDK on `window.__PANERGOS_PLUGIN_SDK__` and the registry on `window.__PANERGOS_PLUGINS__`.
...
4. The plugin's IIFE runs and calls `window.__PANERGOS_PLUGINS__.register(name, Component)` — and optionally `.registerSlot(name, slot, Component)` for each slot.
...
## Combining a theme and plugin

Pair a theme YAML with a slot-only plugin to produce a complete reskin without
forking the dashboard. A typical combination uses:
...
Install the two halves independently:

```bash
# Theme
cp my-dashboard-extension/theme/my-theme.yaml \
   ~/.panergos/dashboard-themes/

# Plugin
cp -r my-dashboard-extension/plugin ~/.panergos/plugins/my-dashboard-extension
```

Open the dashboard and select the theme from the switcher. Keep plugin slot
registration guarded so older dashboards without slot support continue to load.
...
|--------|------|----------|
| `window.__PANERGOS_PLUGIN_SDK__` | object | `registry.ts` — React, hooks, UI components, API client, utils. |
| `window.__PANERGOS_PLUGINS__.register(name, Component)` | function | Register a plugin's main component. |
| `window.__PANERGOS_PLUGINS__.registerSlot(name, slot, Component)` | function | Register into a named shell slot. |
...
Check that the file is in `~/.panergos/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.panergos/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.panergos/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
4. Open browser dev tools → Console — look for errors during the IIFE or `window.__PANERGOS_PLUGINS__ is undefined` (indicates the SDK didn't initialize, usually a React render crash earlier).
...
2. Restart `panergos dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
3. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
4. Tail `~/.panergos/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.panergos/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
