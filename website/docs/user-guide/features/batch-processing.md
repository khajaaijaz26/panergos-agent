...
Batch processing lets you run the Panergos agent across hundreds or thousands of prompts in parallel, generating structured trajectory data. This is primarily used for **training data generation** — producing ShareGPT-format trajectories with tool usage statistics that can be used for fine-tuning or evaluation.
...
:::tip Control cost at scale
Batch runs spin up many concurrent agent sessions. Use provider budgets, explicit concurrency limits, and a pinned model so cost and rate-limit behavior remain predictable.
:::
