...
Schedule tasks to run automatically with natural language or cron expressions. Panergos exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Panergos itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
**Which model does a cron job run on?** Resolution at fire time is: per-job pin → `cron.model` in `config.yaml` → the global default from `panergos model`.

- **Per-job pin** — set by *you* via the dashboard, `panergos cron create/edit --model … --provider …`, or by editing `~/.panergos/cron/jobs.json`. Once set, it sticks until you change it. The agent's `cronjob` tool cannot set or change per-job models — inference pins are user-owned.
- **`cron.model` / `cron.model_provider`** — a cron-fleet default: every unpinned job runs on this model, independent of your chat model. Set it once (`panergos config set cron.model <name>`) and switching your chat model with `panergos model` or `/model` never touches your cron fleet.
- **Global default** — only when neither of the above is set does a job follow `panergos model`. Panergos **snapshots** the provider and model at creation, and that snapshot is the job's effective pin: if you later switch the global default (`panergos model`, `/model`, `panergos config set model.default …`), the job **keeps running on the model and provider it was created under** and logs one INFO line per run noting the difference. A global model change never stops a scheduled job, and an unattended job never silently inherits a switch to a paid provider/model (#44585). To move a job to the new default, **resnap** it (`panergos cron resnap <job_id>`, or `--all` for every unpinned job) so it adopts the current default while staying unpinned, pin it (`panergos cron edit <job_id> --provider <provider> --model <model>`), or set `cron.model` to move the whole fleet at once. Jobs created before snapshots existed keep following the live global default.
...
For unattended runs, choose a provider with renewable credentials and verify it with `panergos doctor` before scheduling work.
:::

:::tip
**Per-job reasoning effort.** A job can pin its own thinking level, independent of the model pin: one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. When set, it overrides both the global `agent.reasoning_effort` and per-model `agent.reasoning_overrides` for that job's runs (`none` disables thinking). Set it via `panergos cron create/edit --reasoning-effort high`; pass an empty string on edit to clear the pin and follow config again. (It is deliberately not exposed on the agent's `cronjob` tool — model configuration stays a user decision.) Levels a model doesn't support are clamped or omitted by the provider at request time — pinning `xhigh` on a model that caps at `high` runs at `high`. The pin has no effect on `no_agent` jobs (there is no LLM call to tune). Use it to run heavy scheduled analyses at `high` while cheap recurring jobs run at `minimal`, without touching your global default.
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Panergos disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
panergos cron create "every 2h" "Check server status"
panergos cron create "every 1h" "Summarize new feed items" --skill blogwatcher
panergos cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask Panergos normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Panergos will use the unified `cronjob` tool internally.
...
Or: `panergos config set cron.preflight false`
...
```bash
panergos cron edit <job_id> --provider <provider> --model <model>   # one job
panergos config set cron.model <model>                               # every unpinned job
```

`panergos config set model.default …` and the Desktop model picker list the unpinned jobs that will
...
```bash
panergos cron resnap <job_id>   # one job
panergos cron resnap --all      # every unpinned agent job
```
...
panergos cron create "every 1d at 09:00" \
...
```bash
panergos cron edit <job_id> --schedule "every 4h"
panergos cron edit <job_id> --prompt "Use the revised task"
panergos cron edit <job_id> --skill blogwatcher --skill maps
panergos cron edit <job_id> --add-skill maps
panergos cron edit <job_id> --remove-skill blogwatcher
panergos cron edit <job_id> --clear-skills
```
...
```bash
panergos cron list
panergos cron pause <job_id_or_name>
panergos cron resume <job_id_or_name>
panergos cron run <job_id_or_name>
panergos cron remove <job_id_or_name>
panergos cron edit <job_id_or_name> [...flags]
panergos cron status
panergos cron tick
```
...
```bash
panergos cron create "every 1h" "Post the digest" --paused --paused-reason "Awaiting review"
panergos cron resume <job_id>
```
...
```bash
panergos gateway install     # Install as a user service
sudo panergos gateway install --system   # Linux: boot-time system service for servers
panergos gateway             # Or run in foreground
...
On each tick Panergos:

1. loads jobs from `~/.panergos/cron/jobs.json`
...
A file lock at `~/.panergos/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
Panergos records each claimed cron attempt in the profile-local
`~/.panergos/cron/executions.db` before executor or provider dispatch. Attempts
move through `claimed`, `running`, and one immutable terminal state:
`completed`, `failed`, or `unknown`. After restart, Panergos marks an abandoned
...
Inspect recent attempts with `panergos cron runs [job-id] --limit 20` (alias:
...
that the retained ledger records as completed, Panergos skips that replay and
...
in a row and suggesting you fix, pause (`panergos cron pause <job>`), or remove
it. Any successful run resets the streak, and `panergos cron list` shows the
...
```bash
panergos cron incidents                 # list incidents (newest activity first)
panergos cron incidents --state alerted # filter: detected | alerted | closed
panergos cron incidents ack <id>        # acknowledge — stop re-pinging
```
...
### Fleet health check: `panergos cron doctor`

`panergos cron doctor` is a read-only health check over every active job. It
...
- script missing, not a file, or resolving outside `PANERGOS_HOME/scripts`,
...
`panergos cron incidents` (durable failure records) and `panergos cron runs`
...
| `"local"` | Save to local files only (`~/.panergos/cron/output/`) | Default on CLI |
...
`last_delivery_error`. `panergos cron list` shows it in yellow as
`delivery_failed: <reason>`, `panergos cron doctor` reports it as a delivery
...
- `bot-chat:<profile>` targets another profile **on the same machine**. Names are validated against `panergos profile list` when the job is created; profiles on other gateways or machines can never be targeted, so same-named profiles across machines are unambiguous.
...
in `panergos cron list`:

```
⚠ Delivery UNVERIFIED: adapter acked slack:C0123456 without message_id/raw_response
```

and in `panergos cron doctor` as `last delivery unverified (...)`. The marker is
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.panergos/cron/output/`), but no message is sent to the delivery target.
...
Or set the `PANERGOS_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
Or set the `PANERGOS_CRON_MEDIA_SEND_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 300s default. A timed-out attachment is recorded in the job's run status as a partial delivery failure (the text still delivers).
...
`.sh` / `.bash` files run under `bash` from `PATH` when available, otherwise `/bin/bash` (important on Windows Git Bash). Anything else runs under the current Python interpreter (`sys.executable`). Scripts must resolve inside `$PANERGOS_HOME/scripts/` — relative names, absolute paths, and `~`-prefixed paths are accepted when the resolved target stays in that directory; paths that escape it are rejected. Subprocess env is sanitized (`_sanitize_subprocess_env`): provider API credentials and other Panergos-managed secrets are **not** inherited by cron scripts.

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Panergos directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Panergos will write the check script to `~/.panergos/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.panergos/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.panergos/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.panergos/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.panergos/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Panergos reads Job 1's most recent output from `~/.panergos/cron/output/{job1_id}/*.md`
...
From the CLI: `panergos cron create "every 6h" "Scan for news" --continuity`, and `panergos cron edit <job_id> --continuity` / `--no-continuity` to toggle it on an existing job. The same toggle appears in the dashboard's cron editor and the desktop Bot Mode routine dialog.
...
Panergos home. Its `## Error` section includes the chained traceback, with credential patterns
...
- `panergos cron list` — a red `⚠ Missed scheduled fire:` line under the job
- The dashboard job view

The stamp always reflects **current** auto-fire health: it is overwritten by newer misses and cleared automatically by the next successful run. If you see it, the job and its schedule are fine — the gateway side of the fire path needs attention (most commonly, restart the gateway through its supervisor so it loads the full profile environment: `panergos gateway restart`).
...
`panergos cron list` as `⚠ late` / `⚠ catch-up after missed fire`.
...
Or run `panergos config set cron.catch_up_missed false`. With this opt-out, a recurring
...
Runtimes that can't receive detached results (one-shot `panergos -z`, `panergos
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `panergos tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `panergos tools` cron-platform config wins; otherwise Panergos falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Panergos should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.panergos/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.panergos/scripts/.feed-changed.last"
...
```bash
#!/bin/bash
# ~/.panergos/scripts/flag-ready.sh
...
:::tip
Panergos' own `~/.panergos/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/khajaaijaz26/panergos-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.panergos/cron/jobs.json`. Output from job runs is saved to `~/.panergos/cron/output/{job_id}/{timestamp}.md`.

Job definitions are plain JSON on disk: they survive `panergos update`, gateway restarts, and machine reboots. A job that was mid-run during a restart is marked `unknown` in the execution ledger — it is not automatically retried, but the job's next scheduled tick fires normally. See [Execution history](#execution-history) for details.

:::tip
Ask the agent to manage jobs through the `cronjob` tool, `panergos cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `PANERGOS_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Panergos resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
