...
All skills live in **`~/.panergos/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Panergos at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `panergos update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.panergos` profile):

```bash
curl -fsSL https://raw.githubusercontent.com/khajaaijaz26/panergos-agent/main/scripts/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
panergos profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
panergos skills opt-out            # stop future seeding — nothing on disk is touched
panergos skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
panergos skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `panergos update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `panergos skills opt-in`) to re-enable.

:::note Safe by default
`panergos skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Panergos installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
(Plan mode works the same way but is a built-in command now: `/plan [request]` tells Panergos to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.panergos/plans/` relative to the active workspace/backend working directory.)

You can also interact with skills through natural conversation:

```bash
panergos chat --toolsets skills -q "What skills do you have?"
panergos chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Panergos-tool framing, no invented
commands).
...
/home/user/.panergos/cache/chart-q4-2025.png
...
When a missing value is encountered, Panergos asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `panergos setup` or `~/.panergos/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `panergos config migrate` prompts for unconfigured settings, and `panergos config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.panergos/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Panergos scans the
...
policy above), Panergos can run [NVIDIA SkillEvaluator](https://github.com/NVIDIA/SkillEvaluator)
...
If you maintain skills outside of Panergos — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Panergos to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.panergos/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.panergos/skills/` (or `skills.create_dir` when configured — see below). Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Panergos process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Panergos ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.panergos/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
By default the agent writes new skills to the profile-local `~/.panergos/skills/`. If you want agent-created skills to land somewhere else — a shared "brain" directory, a git-tracked repo, or a fleet-wide skills volume — set `create_dir` under the `skills` section:
...
Paths support `~` expansion and `${VAR}` substitution; relative paths resolve against your Panergos home. Setting `create_dir` to the local skills dir is the same as leaving it unset.


## Project-Local Skills

Repos can carry their own skills, active only for sessions started inside that project — the same pattern other agent harnesses use for repo-local configuration. When you launch Panergos inside a git checkout, it looks for skills in:

```text
<project-root>/.panergos/skills/    # Panergos-native location
...
```bash
panergos skills trust             # trust the current repo
panergos skills trust ~/myproject # or explicitly
panergos skills untrust           # revoke
```

Trusted roots are stored in `skills.trusted_project_dirs` in `~/.panergos/config.yaml`. Set `skills.project_discovery: false` to turn the feature off entirely (no scanning, no notices).

### Precedence

Project skills are the **highest-precedence tier**: `project → local (~/.panergos/skills/) → external_dirs`. A project skill named `deploy` overrides a same-named profile or bundled skill for sessions inside that repo — that's the point: vendored repo skills win on their home turf, without touching your global profile. Project skills are tagged `[project]` in the agent's skill index so provenance stays visible.

Like external dirs, project skill directories are treated as repo-owned: autonomous skill maintenance (the curator) never modifies them, and new agent-created skills always go to `~/.panergos/skills/`.

### Scan-time quarantine

Trust is a repo-level decision, but a repo's skill content changes with every `git pull`. To close that gap, every project skill is scanned with the same security scanner used for Skills Hub installs before it enters the index. A skill whose scan verdict is **dangerous** (prompt-injection directives, credential-exfiltration commands, hidden-text tricks) is quarantined: it does not appear in the skill index, `skills_list`, slash commands, and refuses to load by name with an explanatory error. Scans are content-hash cached under `~/.panergos/cache/project_skill_scans/` (never inside your repo) and re-run automatically when the skill's content changes.
...
Bundles live in **`~/.panergos/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `panergos bundles list`.
...
panergos bundles create research

# Overwrite an existing bundle
panergos bundles create backend-dev --skill ... --force

# Delete a bundle
panergos bundles delete backend-dev

# Re-scan ~/.panergos/skill-bundles/ and report changes
panergos bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.panergos/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.panergos/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.panergos/pending/skills/` and are
...
```bash
panergos skills browse                              # Browse all hub skills (official first)
panergos skills browse --source official            # Browse only official optional skills
panergos skills search kubernetes                   # Search all sources
panergos skills search react --source skills-sh     # Search the skills.sh directory
panergos skills search https://mintlify.com/docs --source well-known
panergos skills inspect openai/skills/k8s           # Preview before installing
panergos skills install openai/skills/k8s           # Install with security scan
panergos skills install official/security/1password
panergos skills install skills-sh/vercel-labs/json-render/json-render-react --force
panergos skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
panergos skills list --source hub                   # List hub-installed skills
panergos skills check                               # Check installed hub skills for upstream updates
panergos skills update                              # Reinstall hub skills with upstream changes when needed
panergos skills audit                               # Re-scan all hub skills for security
panergos skills uninstall k8s                       # Remove a hub skill
panergos skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
panergos skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
panergos skills publish skills/my-skill --to github --repo owner/repo
panergos skills snapshot export setup.json          # Export skill config
panergos skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Panergos. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `panergos skills search <query> --source skills-sh`. Panergos resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Panergos currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Panergos repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Panergos can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
panergos skills search https://mintlify.com/docs --source well-known
panergos skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
panergos skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Panergos can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
panergos skills tap add myorg/skills-repo
```
...
[Skills Hub](https://khajaaijaz26.github.io/panergos-agent/docs/) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Panergos-side changes required.
...
- Panergos source id: `clawhub`

#### 6. LobeHub (`lobehub`)

Panergos can search and convert agent entries from LobeHub's public catalog into installable Panergos skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Panergos source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

Panergos integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Panergos' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Panergos source id: `browse-sh`
- Trust level: `community`

```bash
panergos skills search airbnb --source browse-sh
panergos skills inspect browse-sh/airbnb.com/search-listings-ddgioa
panergos skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Panergos also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Panergos source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Panergos does not enumerate or copy unrelated files from the host.

```bash
panergos skills install https://sharethis.chat/SKILL.md
panergos skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
panergos skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `panergos skills update` re-fetches from the same URL automatically when you want to refresh.
...
`panergos skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Panergos | Always trusted |
...
```bash
panergos skills check          # Report which installed hub skills changed upstream
panergos skills update         # Reinstall only the skills with updates available
panergos skills update react   # Update one specific installed hub skill
panergos skills update react --force   # Overwrite a skill you've edited locally
```

This uses the stored source identifier plus the current upstream bundle content hash to detect drift.

Checks skip network requests for missing or non-directory installs (`orphaned`) and unsafe or unresolvable recorded paths (`invalid_install`). Missing-directory entries can be removed with `panergos skills uninstall <name>`; invalid paths require inspecting and repairing the active profile's `skills/.hub/lock.json` before retrying. No entries are removed automatically.
...
Skills you have edited locally (the on-disk content no longer matches the hash recorded at install time) are **skipped** by `panergos skills update` so your changes are never silently overwritten. Pass `--force` to replace them with the upstream version anyway.
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Panergos users add with `panergos skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.panergos.tags`, `version`, `author`, `platforms`, `metadata.panergos.config`).
...
Panergos discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/panergos-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Panergos user can subscribe and install:

```bash
panergos skills tap add my-org/panergos-skills
panergos skills search deploy
panergos skills install my-org/panergos-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.panergos/skills/.hub/taps.json`:
...
The `panergos skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `panergos skills tap list` shows the effective path per tap.
...
```bash
panergos skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.panergos/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`panergos skills reset`)

Panergos ships with a set of bundled skills in `skills/` inside the repo. On install and on every `panergos update`, a sync pass copies those into `~/.panergos/skills/` and records a manifest at `~/.panergos/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Panergos recomputes the hash of your local copy and compares it to the origin hash:
...
Generated runtime caches inside a skill (`__pycache__/`, `.pytest_cache/`, `.mypy_cache/`, `.ruff_cache/`, and a `.pyc` sitting next to its `.py`) are not part of the hash, so running a skill's helper script never marks it user-modified or hides it from `panergos skills list-modified` / `diff`.

The protection is good, but it has one sharp edge. If you edit a bundled skill and then later want to abandon your changes and go back to the bundled version by just copy-pasting from `~/.panergos/panergos-agent/skills/`, the manifest still holds the *old* origin hash from whenever the last successful sync ran. Your fresh copy-paste contents (current bundled hash) won't match that stale origin hash, so sync keeps flagging it as user-modified.

`panergos skills reset` is the escape hatch:
...
panergos skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `PANERGOS_HOME`, so `panergos -p coder skills reset <name>` only affects that profile.
:::
