...
It exists so that skills created via the [self-improvement loop](/user-guide/features/skills#agent-managed-skills-skill_manage-tool) don't pile up forever. Every time the agent solves a novel problem and saves a skill, that skill lands in `~/.panergos/skills/`. Without maintenance, you end up with dozens of narrow near-duplicates that pollute the catalog and waste tokens.

By default (`prune_builtins: true`) the curator can archive **unused bundled built-in skills** (shipped with the repo) after `archive_after_days` of non-use, alongside the agent-created skills it primarily manages. Hub-installed skills (from [agentskills.io](https://agentskills.io)) are always off-limits. Set `curator.prune_builtins: false` to restore the old agent-created-only behavior, where bundled skills are never touched. The curator also **never auto-deletes** — the worst outcome is archival into `~/.panergos/skills/.archive/`, which is recoverable.

Tracks [issue #7816](https://github.com/khajaaijaz26/panergos-agent/issues/7816).

## How it runs

The curator is triggered by an inactivity check, not a cron job. On CLI session start, during gateway housekeeping, and on the Desktop/`panergos serve` maintenance timer, Panergos checks whether:
...
Desktop and other `panergos serve` backends share the existing hourly maintenance timer (first poll after 90 seconds), independently of cron jobs. They measure inactivity from process startup and the most recent chat activity in the same profile, retaining that activity timestamp after a session closes or is reaped, and skip curator while a turn in that profile is running. A connected but inactive window does not block maintenance. This timer also polls personal and organization Skill Sync, subject to those features' own opt-in gates. A running messaging gateway for the same profile owns these chores instead.
...
On a brand-new install (or the first time a pre-curator install ticks after `panergos update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `panergos curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (14) become `stale`; skills unused for `archive_after_days` (30) are moved to `~/.panergos/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
**Easiest — `panergos model`:**

```bash
panergos model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`panergos model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
panergos curator status         # last run, counts, pinned list, LRU top 5
panergos curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
panergos curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
panergos curator run --background  # fire-and-forget: start the run in a background thread
panergos curator run --dry-run  # preview only — report without any mutations
panergos curator backup         # take a manual snapshot of ~/.panergos/skills/
panergos curator rollback       # restore from the newest snapshot
panergos curator rollback --list     # list available snapshots
panergos curator rollback --id <ts>  # restore a specific snapshot
panergos curator rollback -y         # skip the confirmation prompt
panergos curator pause          # stop runs until resumed
panergos curator resume
panergos curator pin <skill>    # never auto-transition this skill
panergos curator unpin <skill>
panergos curator adopt <skill>    # hand an unmanaged skill to the curator
panergos curator adopt --all-unmanaged   # hand over every unmanaged skill
panergos curator list-unmanaged   # itemize skills with no provenance marker
panergos curator restore <skill>  # move an archived skill back to active
panergos curator list-archived    # list skills currently in ~/.panergos/skills/.archive/
panergos curator archive <skill>  # manually archive a single skill now
panergos curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default: `archive_after_days`, 30)
panergos curator ledger           # list the per-mutation audit ledger (all actors)
panergos curator ledger --skill <name> --limit 50  # filter/paginate ledger entries
panergos curator rollback <entry-id>  # undo a single mutation from the ledger
panergos curator purge [--days N] [--dry-run]  # delete archived skills older than the TTL (explicit only)
```

## Backups and rollback

Before every real curator pass, Panergos takes a tar.gz snapshot of `~/.panergos/skills/` at `~/.panergos/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
panergos curator rollback        # restore newest snapshot (with confirmation)
panergos curator rollback -y     # skip the prompt
panergos curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, Panergos takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `panergos curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `panergos curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`panergos curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
Whole-run snapshots answer "undo everything the last curator pass did" — but sometimes you want to know *who changed what* and undo exactly one mutation. Every skill mutation — curator auto-transitions, agent `skill_manage` calls, and your own CLI archive/restore/purge — appends one entry to the append-only JSONL ledger at `~/.panergos/skills/.curator_ledger.jsonl`:
...
- **before/after** — per-file `{path, sha256}` manifests. File contents are stored content-addressed (deduped by hash) under `~/.panergos/.curator_backups/blobs/`, so a hundred entries touching the same unchanged file cost one blob.

```bash
panergos curator ledger                  # newest 20 entries
panergos curator ledger --skill my-skill --limit 50
panergos curator rollback <entry-id>     # restore that one mutation's before-state
```

Single-entry rollback restores exactly the files that mutation touched (and removes files it created) from the blob store — nothing else in the skills tree moves. Like whole-tree rollback, it takes a safety ledger entry of the current state first and **fails closed**: if the safety capture can't be written, nothing is changed. Because foreground deletes are ledgered too, `panergos curator rollback <entry-id>` can resurrect a hard-deleted skill.
...
Archived skills are kept forever by default. If you want `~/.panergos/skills/.archive/` bounded, set a TTL and purge explicitly — purging never runs automatically, and every purged skill is captured into the ledger (with blobs) first, so even a purge leaves an auditable, recoverable trail:
...
```bash
panergos curator purge --dry-run   # preview what would be deleted
panergos curator purge             # delete archives older than the TTL (with confirmation)
panergos curator purge --days 90   # one-off TTL override
```

## What "agent-created" means

The curator only manages skills explicitly marked as **agent-created** in
`~/.panergos/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.panergos/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.panergos/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed Panergos at an external skill
...
**To see which skills the curator actually manages**, run `panergos curator status`.
...
`panergos curator status` reports an **unmanaged** count alongside the managed
one:
...
unmanaged (no provenance marker): 112 total
  ...
  never auto-staled or archived — `panergos curator adopt <name>` hands one over
```
...
```bash
panergos curator list-unmanaged                    # itemize them, with reasons
panergos curator adopt <name> [<name> ...]         # hand specific skills over
panergos curator adopt --all-unmanaged --dry-run   # preview the full list
panergos curator adopt --all-unmanaged             # hand over everything (prompts)
panergos curator adopt --all-unmanaged --yes       # skip the prompt
```
...
`.usage.json`; read it as policy. `panergos curator adopt` changes the policy, and
...
**wrote** it — Panergos edits user-authored skills on your behalf constantly. An
...
- Archives are recoverable via `panergos curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `panergos curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `panergos curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
panergos curator pin <skill>
panergos curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.panergos/skills/.usage.json`, so it survives across sessions.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.panergos/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.panergos/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.panergos/logs/curator/`:

```
~/.panergos/logs/curator/
└── 20260429-111512/
...
```bash
panergos curator restore <skill-name>
```

This moves the skill back from `~/.panergos/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.panergos/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `panergos curator pause` — the pause persists across sessions; use `resume` to re-enable.
...
- [Issue #7816](https://github.com/khajaaijaz26/panergos-agent/issues/7816) — original proposal and design discussion
