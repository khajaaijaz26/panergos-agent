...
The `execute_code` tool lets the agent write Python scripts that call Panergos tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with Panergos over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from panergos_tools import ...`
2. Panergos generates a `panergos_tools.py` stub module with RPC functions
3. Panergos opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to Panergos
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from panergos_tools import web_search, web_extract
...
```python
from panergos_tools import search_files, read_file
import json
...
```python
from panergos_tools import search_files, read_file, patch
...
```python
from panergos_tools import terminal, read_file
import json
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.panergos/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to Panergos' own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (Panergos' own python) |
...
| **Stdout** | 50 KB | Shown head-and-tail inline; the full output is saved to `~/.panergos/cache/exec/` and the path is included in the result |
...
**Remote backends** (Docker, SSH, Modal) run a remote session kernel with the same contract. If the kernel cannot be spawned on the backend, Panergos falls back to running each call as a standalone script and says so in the result.

**Large output.** Stdout over 50 KB is shown head-and-tail inline, and the full text is saved under `~/.panergos/cache/exec/` with the path included in the result, so the agent can page through it with `read_file` instead of re-running the script.
...
### `PANERGOS_*` variables in the child

The child process receives only a small, fixed set of operational `PANERGOS_*`
...
(plus `PANERGOS_RPC_DIR` / `PANERGOS_RPC_SOCKET` / `TZ` / `HOME`, which Panergos
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `PANERGOS_`
through to the child. That broad prefix was removed for security hardening: it
could leak `PANERGOS_*`-named configuration that doesn't match a secret substring
(for example `PANERGOS_BASE_URL`, `PANERGOS_KANBAN_DB`, or a `PANERGOS_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `PANERGOS_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (configured provider credentials can never be
re-allowed this way):
...
**Diagnosing it.** When the child drops one or more non-allowlisted `PANERGOS_*`
variables, Panergos emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`panergos logs --level
DEBUG`, or check `~/.panergos/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted PANERGOS_* var(s)` if a script behaves
as though a `PANERGOS_*` variable is missing.

Panergos always writes the script and the auto-generated `panergos_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
**Rule of thumb:** Use `execute_code` when you need to call Panergos tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.

## Platform Support

Code execution is available on **Linux, macOS, and Windows**. On Linux and macOS the RPC channel uses a Unix domain socket; on Windows, where `AF_UNIX` is unreliable, Panergos automatically falls back to a loopback TCP socket for the sandbox RPC transport. Remote terminal backends (Docker/SSH/Modal/etc.) use a file-based RPC transport instead and additionally require Python 3 inside the backend.
