...
Type `@` followed by a reference to inject content directly into your message. Panergos expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Panergos env: `$PANERGOS_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$PANERGOS_HOME/skills/.hub/`
