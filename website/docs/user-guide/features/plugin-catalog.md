...
description: "Browse and install reviewed, SHA-pinned Panergos plugins from the curated catalog"
---

# Plugin Catalog

The plugin catalog is a curated, human-reviewed directory of Panergos plugins you
can install by name with a single command:

```bash
panergos plugins install <name>
```
...
[`plugin-catalog/`](https://github.com/khajaaijaz26/panergos-agent/tree/main/plugin-catalog)
directory of the Panergos repository, declaring:

| Field | Meaning |
|---|---|
| `name` | The catalog key you pass to `panergos plugins install` |
...
| `tier` | `official` (maintained by Panergos maintainers) or `community` |
| `maintainer` | Who owns the plugin |
| `capabilities` | Declared tools, hooks, middleware, and required env vars |
| `requires_panergos` | Minimum Panergos version, e.g. `>=0.19` (optional) |
...
The catalog name and the plugin's own manifest name can differ; `panergos
plugins install` prints the installed name, and `enable` takes that one.
Portable packages may also carry a stdio MCP server and related workflow
skills, all covered by the same pin and capability review.

### Updating a catalog install

`panergos plugins update <name>` never runs `git pull` for catalog installs —
...
enabled/disabled state is preserved. `panergos plugins list` shows catalog
...
(`https://khajaaijaz26.github.io/panergos-agent/docs/api/plugin-catalog.json`).
`search`/`install`/`update` fetch it at most every six hours and cache it under
`~/.panergos/cache/`, so new entries and removals reach installed clients without
updating Panergos. Offline, the copy shipped with your checkout is used. Removals
from the in-tree list and the live list are always both enforced.

### Custom git URLs are different

`panergos plugins install <git-url>` still works for any repository, but it
...
[plugin-catalog README](https://github.com/khajaaijaz26/panergos-agent/tree/main/plugin-catalog);
...
- [Plugins](plugins.md) — the plugin system itself: manifest format, enabling,
  configuration
- [Built-in Plugins](built-in-plugins.md) — plugins that ship with Panergos
- [Build a Panergos Plugin](/developer-guide/plugins) — write your own
