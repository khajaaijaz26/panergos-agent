...
Connect Panergos to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Panergos — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Panergos runs it in a
...
panergos photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Panergos Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.panergos/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.panergos/.env`
...
(device token, dashboard project id) lives in `~/.panergos/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Panergos
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Panergos replies with a pairing code. Approve it with:

```bash
panergos pairing approve photon <CODE>
```

Use `panergos pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.panergos/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.panergos/.env`):
...
By default Panergos responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Panergos` and
`@Panergos agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Panergos will reply.

## Status & troubleshooting

```bash
panergos photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Panergos uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run panergos photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `panergos photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `panergos photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `panergos photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Panergos sends images, voice
  ...
  media.
- **Native polls are supported.** Panergos sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Panergos, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for Panergos-sent messages
...
- **Message effects are supported.** Panergos sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `panergos send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<panergos-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Panergos is set
...
| `PHOTON_MENTION_PATTERNS` | Panergos wake words  | JSON list / comma / newline regex patterns for group mentions |
