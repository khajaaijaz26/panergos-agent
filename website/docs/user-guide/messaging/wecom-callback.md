...
Connect Panergos to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Panergos supports two WeCom integration modes:
...
> Run `panergos gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Panergos decrypts the message, queues it for the agent
...
(Use `panergos gateway start` only after `panergos gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Panergos and the token the
...
in `~/.panergos/.env` values around `=` will also break signature checks. After
fixing, restart `panergos gateway run`.
...
Check `panergos gateway run` logs for the bound host/port. If the adapter bound to
