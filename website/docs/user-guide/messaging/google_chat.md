...
sidebar_position: 12
title: "Google Chat"
description: "Set up Panergos Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Panergos Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Panergos
...
> Run `panergos gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Panergos can read (e.g.,
`~/.panergos/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a Panergos restart)
...
- Principal: `panergos-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Panergos calls
...
- **App name**: whatever you want users to see ("Panergos" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/panergos-chat-events`.
...
`ADDED_TO_SPACE` event that Panergos uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Panergos

Add the Google Chat section to `~/.panergos/.env`:
...
On Docker / hosted images `/opt/panergos/.venv` is read-only. That installer
routes through `tools.lazy_deps` into `PANERGOS_LAZY_INSTALL_TARGET`
...
Send "hola" in the test DM. The bot posts a "Panergos is thinking…" marker, then
...
`~/.panergos/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Panergos is thinking…").
...
Thread support: when a user replies inside a thread, Panergos detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Panergos session.
...
3. Download the JSON. Move it onto the host that runs Panergos.
4. Register the client with Panergos (run under the profile you want it scoped to):
...
panergos -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Panergos home (e.g.
`~/.panergos/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.panergos/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.panergos/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Panergos isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `panergos gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Panergos will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.panergos/google_chat_user_tokens/<sanitized_email>.json` (filesystem
