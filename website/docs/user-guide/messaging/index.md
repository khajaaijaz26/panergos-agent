...
description: "Chat with Panergos from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Panergos from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Panergos](/guides/use-voice-mode-with-panergos).

:::tip
Bots need a model provider and any tool providers their workflow uses. Configure and verify each before enabling unattended delivery.
:::
...
saved by `panergos gateway setup` can enable a credential-based platform without a
...
## Panergos connector quick-start

Panergos exposes one discoverable entry point over the existing gateway, platform-plugin,
and provider-account setup handlers:

```bash
panergos connect                         # List every discovered connector and readiness
panergos connect telegram                # Run Telegram's existing guided setup
panergos connect setup                   # Open the complete gateway setup wizard
panergos connect status                  # Add gateway/runtime health for every connector
panergos connect status whatsapp-cloud   # Inspect one connector (hyphen or underscore works)
panergos connect status --json           # Secret-free machine-readable status
panergos connect auth                    # Open the existing provider-account manager
panergos connect auth anthropic --type oauth
```

The list is built from the same catalog used by the dashboard, so bundled adapters and
enabled third-party platform plugins appear automatically. Status output includes only
credential field names that still need setup; it never prints credential values. A
connector marked configured is not necessarily online: `connect status` reports the live
gateway state separately. Provider authentication remains the existing credential-pool
flow, and model selection remains `panergos model`.
...
```mermaid
flowchart TB
    subgraph Gateway["Panergos Gateway"]
...
For group chats, hooks, and automation flows, Panergos supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Panergos keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Panergos does not hide failures just because the text resembles a silence token.

## Quick Setup

The shortest path is `panergos connect`; the gateway command is also available:

```bash
panergos connect             # List readiness and missing setup fields
panergos connect setup       # Interactive setup for all messaging platforms
panergos gateway setup       # Equivalent gateway wizard
```
...
```bash
panergos gateway              # Run in foreground
panergos gateway setup        # Configure messaging platforms interactively
panergos gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo panergos gateway install --system   # Linux only: install a boot-time system service
panergos gateway start        # Start the default service
panergos gateway stop         # Stop the default service
panergos gateway status       # Check default service status
panergos gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.panergos/config.yaml"
gateway:
...
```bash
panergos gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Panergos sends heartbeats
...
| `/update` | Update Panergos Agent to the latest version |
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.panergos/config.yaml`:
...
panergos pairing approve telegram XKGH5N7P

# Other pairing commands:
panergos pairing list          # View pending + approved users
panergos pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Panergos appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.panergos/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.panergos/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `PANERGOS_HOME`:

- `~/.panergos/status_phrases.yaml` or any `*.yaml` in `~/.panergos/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to PANERGOS_HOME
...
Off by default. When enabled, Panergos prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.panergos/config.yaml`:
...
```bash
PANERGOS_BACKGROUND_NOTIFICATIONS=result
```
...
panergos gateway start                 # Start the service
panergos gateway stop                  # Stop the service
panergos gateway status                # Check status
journalctl --user -u panergos-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo panergos gateway install --system
sudo panergos gateway start --system
sudo panergos gateway status --system
journalctl -u panergos-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Panergos installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit panergos-gateway` (or `sudo systemctl edit panergos-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `panergos update`. When `panergos update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart panergos-gateway` command (it never blocks on an interactive password prompt).
...
```bash
panergos gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `panergos update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo panergos update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/panergos-gateway`:

```
panergos ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed panergos-gateway*, /usr/bin/systemctl --no-ask-password start panergos-gateway*, /usr/bin/systemctl --no-ask-password restart panergos-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Panergos will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Panergos installations on the same machine (with different `PANERGOS_HOME` directories), each gets its own systemd service name. The default `~/.panergos` uses `panergos-gateway`; other installations use `panergos-gateway-<hash>`. The `panergos gateway` commands automatically target the correct service for your current `PANERGOS_HOME`.
:::

### macOS (launchd)

```bash
panergos gateway install               # Install as launchd agent
panergos gateway start                 # Start the service
panergos gateway stop                  # Stop the service
panergos gateway status                # Check status
tail -f ~/.panergos/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.panergos.gateway.plist`. It includes three environment variables:
...
- **PANERGOS_HOME** — scopes the gateway to your Panergos installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `panergos gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `PANERGOS_HOME` directory gets its own launchd label. The default `~/.panergos` uses `ai.panergos.gateway`; other installations use `ai.panergos.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `panergos-cli` | Full access |
| Telegram | `panergos-telegram` | Full tools including terminal |
| Discord | `panergos-discord` | Full tools including terminal |
| WhatsApp | `panergos-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `panergos-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `panergos-slack` | Full tools including terminal |
| Google Chat | `panergos-google_chat` | Full tools including terminal |
| Signal | `panergos-signal` | Full tools including terminal |
| SMS | `panergos-sms` | Full tools including terminal |
| Email | `panergos-email` | Full tools including terminal |
| Home Assistant | `panergos-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `panergos-mattermost` | Full tools including terminal |
| Matrix | `panergos-matrix` | Full tools including terminal |
| DingTalk | `panergos-dingtalk` | Full tools including terminal |
| Feishu/Lark | `panergos-feishu` | Full tools including terminal |
| WeCom | `panergos-wecom` | Full tools including terminal |
| WeCom Callback | `panergos-wecom-callback` | Full tools including terminal |
| Weixin | `panergos-weixin` | Full tools including terminal |
| BlueBubbles | `panergos-bluebubbles` | Full tools including terminal |
| QQBot | `panergos-qqbot` | Full tools including terminal |
| Yuanbao | `panergos-yuanbao` | Full tools including terminal |
| Microsoft Teams | `panergos-teams` | Full tools including terminal |
| API Server | `panergos-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `panergos-webhook` | Full tools including terminal |
| Raft | `panergos-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
`platforms.<name>.enabled: false` in `~/.panergos/config.yaml` is authoritative.
...
1. **Gateway log** (`~/.panergos/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
