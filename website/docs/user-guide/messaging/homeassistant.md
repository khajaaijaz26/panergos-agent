...
description: Control your smart home with Panergos Agent via Home Assistant integration.
...
Panergos Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Panergos Agent"
5. Copy the token
...
Panergos Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.panergos/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Panergos Agent".
...
The adapter reads credentials from `~/.panergos/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Panergos profile
...
running Panergos — `curl -H "Authorization: Bearer <token>" <url>/api/` should
