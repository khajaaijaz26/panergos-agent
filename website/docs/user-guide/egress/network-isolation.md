...
When running Panergos inside Docker, the default `network_mode: host` gives the
...
The Panergos [SECURITY.md](https://github.com/khajaaijaz26/panergos-agent/blob/main/SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ panergos-agent │   │ panergos-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   PANERGOS_UID=$(id -u) PANERGOS_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=panergos,panergos-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://panergos-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](https://github.com/khajaaijaz26/panergos-agent/blob/main/SECURITY.md) — Panergos trust model and vulnerability reporting
- [Docker](/user-guide/docker) — running Panergos in a container
- [Egress proxy](iron-proxy.md) — credential-injection firewall for the sandbox
- [docker-compose.yml](https://github.com/khajaaijaz26/panergos-agent/blob/main/docker-compose.yml) — default compose configuration
