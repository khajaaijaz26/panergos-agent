...
Panergos Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.panergos/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Panergos stores session history so it can resume conversations, but it does not
...
Panergos explicitly injects for that turn.
...
For example, if a user sends an image and asks Panergos to make a meme from it,
Panergos may inspect that image once with vision and run an image-processing
...
`panergos sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `panergos sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`panergos` or `panergos chat`) |
...
A bare `-c` is terminal-aware: each CLI session drops a small breadcrumb file under `~/.panergos/terminal-sessions/` keyed by the terminal it runs in (tty device, tmux pane, kitty window, wezterm pane, Zellij pane, Windows Terminal session, ...). When you run `panergos -c` again in the *same* terminal, Panergos resumes that terminal's own session — so two panes side by side each continue their own conversation instead of both grabbing the globally most-recent one. If there's no breadcrumb for the terminal (first use, deleted session, or a stale breadcrumb older than 30 days), `-c` falls back to the most-recent-session behavior. `-c "name"` and `--resume` are unaffected. Disable with `session.terminal_continue: false` in `config.yaml`.
...
panergos -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
panergos -c "my project"   # → resumes "my project #3"
```
...
panergos --resume 20250305_091523_a1b2c3d4
panergos -r 20250305_091523_a1b2c3d4

# Resume by title
panergos --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
panergos --resume latest

# Or with the chat subcommand
panergos chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `panergos sessions list`.
...
panergos --resume latest --in ./my-project

# Works with the TUI too
panergos --tui --resume latest --in ./my-project
```
...
```bash
panergos --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`panergos sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
panergos sessions list --workspace my-project
panergos sessions list --workspace ~/code/panergos-agent
```

### Conversation Recap on Resume

When you resume a session, Panergos displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Panergos session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.panergos/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `panergos -r "<title>"` from the shell) and pick up where the platform left off.
...
Panergos automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `panergos sessions list` or `panergos sessions browse`.
...
```bash
panergos sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Panergos creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`panergos -c "my project"`), it automatically picks the most recent session in the lineage.
...
Panergos provides a full set of session management commands via `panergos sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
panergos sessions list

# Filter by platform
panergos sessions list --source telegram

# Show more sessions
panergos sessions list --limit 50
```
...
`panergos sessions export` is one surface for every export format, selected with `--format`:
...
panergos sessions export backup.jsonl

# Export sessions from a specific platform
panergos sessions export telegram-history.jsonl --source telegram

# Export a single session
panergos sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Point at a directory (existing, or ending in /) and the file is named for you:
# ~/exports/panergos_session_20250305_091523_a1b2c3d4.jsonl
panergos sessions export ~/exports/ --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
panergos sessions export backup.jsonl --redact
```
...
panergos sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
panergos sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
panergos sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
panergos sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `panergos-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
panergos sessions export --format trace

# One session to a local trace file
panergos sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
panergos sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.panergos/session-exports`).

```bash
# Export one session to Markdown
panergos sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
panergos sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
panergos sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
panergos sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
panergos sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
panergos sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
panergos sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
panergos sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
panergos sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
panergos sessions pin 20250305_091523_a1b2c3d4
panergos sessions pin 20250305 20250306

# Remove the pin
panergos sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
panergos sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
panergos sessions pinned --json > pinned-sessions.json
```
...
panergos sessions prune

# Custom age threshold — bare numbers are days
panergos sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
panergos sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
panergos sessions prune --newer-than 5h

# Explicit window with absolute timestamps
panergos sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
panergos sessions prune --source telegram
panergos sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
panergos sessions prune --newer-than 5h --title "smoke test"   # title substring
panergos sessions prune --older-than 30 --max-messages 3        # tiny sessions
panergos sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
panergos sessions prune --model gpt-5 --older-than 1w           # by model (substring)
panergos sessions prune --provider openrouter --older-than 60   # by billing provider
panergos sessions prune --branch feature/old-experiment         # by git branch
panergos sessions prune --user 12345678 --chat-type group       # by messaging origin
panergos sessions prune --max-tokens 500 --older-than 7         # by token usage
panergos sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
panergos sessions prune --newer-than 5h --dry-run

# Skip confirmation
panergos sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `panergos sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `panergos sessions prune` keeps the 90-day cutoff. Every
...
`panergos sessions archive` takes the same filters as `prune` but soft-hides
...
panergos sessions archive --newer-than 5h

# Archive by title substring, preview first
panergos sessions archive --title "dry run" --dry-run
panergos sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `panergos sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`panergos sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
panergos sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`panergos insights`](/reference/cli-commands#panergos-insights).
...
`panergos sessions repair-routing` finds message-bearing session rows with no
...
panergos sessions repair-routing
...
panergos sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
panergos sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.panergos/state.db ~/.panergos/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into Panergos and
continue it here. Panergos reads Claude Code's session logs
...
panergos sessions import

# Limit to one tool, or point at a specific file
panergos sessions import --from claude
panergos sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl

# Import-and-resume in one step
panergos --resume @claude
panergos --resume @codex
```

`panergos sessions import` creates a new Panergos session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `panergos --resume <id>` command.
`--resume @claude` / `--resume @codex` show the same picker and drop you
straight into the imported conversation.

**Panergos Desktop** has the same importer in the command palette (**Import
session**). It lists the logs on the machine the
connected backend runs on — not the computer running the app — shows a
read-only preview, and **Continue in Panergos** copies the conversation into the
...
When Panergos cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Panergos uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Panergos in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.panergos/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.panergos/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.panergos/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.panergos/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.panergos/sessions/sessions.json` is a **legacy mirror** of it,
...
`panergos sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.panergos/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `panergos sessions list`, the cause is
`state.db` not receiving them — run `panergos sessions repair` and watch for a
...
`*.jsonl` files in `~/.panergos/sessions/`. They are no longer written or
read by Panergos. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Panergos process in the same `PANERGOS_HOME`

Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever (the pre-#54189 behavior), turn it off in `~/.panergos/config.yaml`:
...
`in its tip segment`). `panergos sessions export` still works for such sessions.
...
panergos sessions delete <session_id>
...
:::tip
Auto-prune is **on by default**: ended sessions that have been inactive for `sessions.retention_days` (default 90) are removed at startup, and active sessions are never touched (see [Automatic Cleanup](#automatic-cleanup) above). Session history powers `session_search` recall across past conversations, so if you want to keep every ended session forever, set `sessions.auto_prune: false` in `config.yaml`, or raise `retention_days`. With auto-prune off, `panergos sessions prune` remains available for one-off cleanup (observed failure mode without any pruning: a 384 MB `state.db` with ~1000 sessions slowing down FTS5 inserts and `/resume` listing).
:::
