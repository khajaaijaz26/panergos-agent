...
description: "Diagnose and fix common Panergos cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`panergos gateway` for foreground, or `panergos gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `panergos cron tick`.
...
```bash
date
panergos cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.panergos/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.panergos/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.panergos/.env` |
...
| `local` | Write access to `~/.panergos/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `panergos cron list` for updated `last_error` field (if available).
...
```bash
panergos skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `panergos skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `panergos skills list`.
...
2. `~/.panergos/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `panergos cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Panergos config directory). Verify:
```bash
ls ~/.panergos/scripts/your-script.py   # Must exist
panergos cron edit <job_id> --script ~/.panergos/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `panergos skills install <skill-name>`.
...
```bash
ps aux | grep panergos
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.panergos/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.panergos/cron/jobs.json
chmod 600 ~/.panergos/cron/jobs.json   # Your user should own it
```
...
```bash
panergos cron list                    # Show all jobs, states, next_run times
panergos cron run <job_id>            # Schedule for next tick (for testing)
panergos cron edit <job_id>           # Fix configuration issues
panergos logs                         # View recent Panergos logs
panergos skills list                  # Verify installed skills
```
...
1. Run the job with `panergos cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.panergos/logs/agent.log` for scheduler messages and `~/.panergos/logs/errors.log` for warnings
3. Open an issue at [khajaaijaz26/panergos-agent](https://github.com/khajaaijaz26/panergos-agent/issues) with:
