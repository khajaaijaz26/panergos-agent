...
This tutorial walks you through setting up a Telegram bot powered by Panergos Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Panergos Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.panergos/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Panergos itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Panergos Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_panergos_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Panergos Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.panergos/.env`:
...
```
[Gateway] Starting Panergos Gateway...
...
# View live logs
journalctl --user -u panergos-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `panergos gateway install` to update the plist.
:::
...
```bash
panergos gateway stop && panergos gateway start
```
...
panergos pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.panergos/.env`:
...
Control how much detail the bot shows when using tools. In `~/.panergos/config.yaml`:
...
Customize how the bot communicates by editing `~/.panergos/SOUL.md`:

For a full guide, see [Use SOUL.md with Panergos](/guides/use-soul-with-panergos).
...
```bash
# From the CLI
panergos cron list          # View all scheduled jobs
panergos cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u panergos-gateway` (Linux) or `~/.panergos/logs/gateway.log` (macOS) |
| Cron job output | `~/.panergos/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.panergos/cron/jobs.json` |
| Pairing data | `~/.panergos/pairing/` |
| Session history | `~/.panergos/sessions/` |
