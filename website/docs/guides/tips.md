...
description: "Practical advice to get the most out of Panergos Agent — prompt tips, CLI shortcuts, context files, memory, cost optimization, and security"
---

# Tips & Best Practices

A quick-wins collection of practical tips that make you immediately more effective with Panergos Agent. Each section targets a different aspect — scan the headers and jump to what's relevant.

:::tip Confused which model to pick?
Run `panergos model --quick` to detect ready local runtimes or connect a cloud endpoint.
:::
...
Forgot something from your last session? Run `panergos -c` to resume exactly where you left off, with full conversation history restored. You can also resume by title: `panergos -r "my research project"`.
...
Want Panergos to have a stable default voice? Edit `~/.panergos/SOUL.md` (or `$PANERGOS_HOME/SOUL.md` if you use a custom Panergos home). Panergos now seeds a starter SOUL automatically and uses that global file as the instance-wide personality source.

For a full walkthrough, see [Use SOUL.md with Panergos](/guides/use-soul-with-panergos).
...
Already have a `.cursorrules` or `.cursor/rules/*.mdc` file? Panergos reads those too. No need to duplicate your coding conventions — they're loaded automatically from the working directory.

### Discovery

Panergos loads the top-level `AGENTS.md` from the current working directory at session start. Subdirectory `AGENTS.md` files are discovered lazily during tool calls (via `subdirectory_hints.py`) and injected into tool results — they are not loaded upfront into the system prompt.
...
:::tip
Run `/usage` periodically to see your token consumption. Run `/insights` for a broader view of usage patterns over the last 30 days. To see what your *fixed* per-message cost is before any conversation — system prompt, skills index, memory, tool schemas — run [`panergos prompt-size`](/reference/cli-commands#panergos-prompt-size) (works offline).
:::
...
Name your sessions with `/title auth-refactor` or `/title research-llm-quantization`. Named sessions are easy to find with `panergos sessions list` and resume with `panergos -r "auth-refactor"`. Unnamed sessions pile up and become impossible to distinguish.

### DM Pairing for Team Access

Instead of manually collecting user IDs for allowlists, enable DM pairing. When a teammate DMs the bot, they get a one-time pairing code. You approve it with `panergos pairing approve telegram XKGH5N7P` — simple and secure.
...
# In your .env:
TERMINAL_ENV=docker
TERMINAL_DOCKER_IMAGE=panergos-sandbox:latest
```
...
Panergos checks every command against a curated list of dangerous patterns before execution. This includes recursive deletes, SQL drops, piping curl to shell, and more. Don't disable this in production — it exists for good reasons.
