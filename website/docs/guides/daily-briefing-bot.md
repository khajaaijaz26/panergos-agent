...
:::tip
This recipe uses web search, summarization, and optional TTS. Configure each provider through `panergos tools` before scheduling it.
:::
...
2. **Panergos spins up** a fresh agent session with your prompt
...
- **Gateway running** — the gateway daemon handles cron execution:
  ...
  sudo panergos gateway install --system   # Linux servers: boot-time system service
  # or
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.panergos/cron/output/` and you can read them anytime.
:::
...
Panergos will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Panergos Agent
```
...
Before creating cron jobs, ensure Panergos has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Panergos what you want:
...
Panergos will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Panergos to delegate each topic to a sub-agent:
...
Panergos will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
