...
```bash
panergos teams-pipeline validate
```
...
```bash
panergos teams-pipeline token-health
panergos teams-pipeline token-health --force-refresh
```
...
```bash
panergos teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
panergos teams-pipeline maintain-subscriptions
panergos teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Panergos cron (recommended if you already run the Panergos gateway)

Panergos ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.panergos/scripts/`. First create the script:

```bash
mkdir -p ~/.panergos/scripts
cat > ~/.panergos/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec panergos teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.panergos/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
panergos cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/panergos-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Panergos Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=panergos
EnvironmentFile=/etc/panergos/env
ExecStart=/usr/local/bin/panergos teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/panergos-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Panergos Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now panergos-teams-pipeline-maintain.timer
systemctl list-timers panergos-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/panergos teams-pipeline maintain-subscriptions >> /var/log/panergos/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.panergos/.env` at the top of a wrapper script that crontab calls.
...
```bash
panergos teams-pipeline subscriptions   # should show expirationDateTime advanced
panergos teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
panergos teams-pipeline list
panergos teams-pipeline list --status failed
panergos teams-pipeline show <job-id>
```

### Replay a stored job

```bash
panergos teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
panergos teams-pipeline fetch --meeting-id <meeting-id>
panergos teams-pipeline fetch --join-web-url "<join-url>"
panergos teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `panergos teams-pipeline maintain-subscriptions --dry-run`
- inspect `panergos teams-pipeline list --status failed`
...
- run `panergos teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `panergos teams-pipeline run`
...
- [ ] `panergos teams-pipeline validate` returns an OK snapshot
- [ ] `panergos teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Panergos cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
