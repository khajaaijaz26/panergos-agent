...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `panergos send`."
---

# Pipe Script Output to Messaging Platforms

`panergos send` is a small, scriptable CLI that pushes a message to any
messaging platform Panergos is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | panergos send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `panergos
...
panergos send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
panergos send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
panergos send --list

# Filter by platform
panergos send --list telegram
```
...
Any platform Panergos ships adapters for works as a target:
...
`panergos send` resolves the message body in this order:

1. **Positional argument** — `panergos send --to telegram "hi"`
2. **`--file PATH`** — `panergos send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | panergos send --to telegram`

When stdin is a TTY (no pipe), Panergos does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  panergos send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `panergos send` reuses your Panergos config, the same script works on
any host where Panergos is installed — no need to export bot tokens into
...
`panergos send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  panergos send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | panergos send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.panergos/bin/panergos send \
...
```bash
./train.py --epochs 200 && \
  panergos send --to telegram "training done" || \
  panergos send --to telegram "training failed (exit $?)"
```
...
panergos send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(panergos send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `panergos send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `panergos send` calls
the platform's REST endpoint directly using credentials from
`~/.panergos/.env` and `~/.panergos/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `panergos gateway start` and retry.
...
# Machine-readable
panergos send --list --json
```

The listing is built from `~/.panergos/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`panergos gateway
...
| Approach | Multi-platform | Reuses Panergos creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `panergos send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`panergos send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `panergos send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `panergos send` shares with cron delivery.
