...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Panergos Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`panergos claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Panergos. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::note
Coming from **Claude Code** or **OpenAI Codex CLI** instead? Use [`panergos import-agent`](../user-guide/import-from-other-agents.md).
:::

:::tip
If your OpenClaw setup was multi-provider, migrate one working model first with `panergos model --quick`, verify it, then add the remaining providers.
:::
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.panergos/` (by default a single restore-point archive is written before apply, under `~/.panergos/backups/pre-migration-*.zip`; restorable with `panergos import`). |
...
| What | OpenClaw source | Panergos destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.panergos/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.panergos/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.panergos/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.panergos/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Panergos destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.panergos/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.panergos/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.panergos/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.panergos/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Panergos skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Panergos destination | Notes |
|------|---------------------|-------------------|-------|
| Default model | `agents.defaults.model` | `config.yaml` → `model` | Can be a string or `{primary, fallbacks}` object |
| Custom providers | `models.providers.*` | `config.yaml` → `custom_providers` (auto-migrated to the canonical `providers:` dict on the next `panergos update` config migration) | Maps `baseUrl`, `apiType`/`api` — handles both short ("openai", "anthropic") and hyphenated ("openai-completions", "anthropic-messages", "google-generative-ai") values |
| Provider API keys | `models.providers.*.apiKey` | `~/.panergos/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Panergos config path | Mapping |
|------|---------------------|-------------------|---------|
...
Idle and daily reset timers are not imported: Panergos conversations persist until an explicit `/new` or `/reset`. Advanced session settings (identity links, thread bindings, maintenance, scope and send policy) remain archived for reference.

### MCP servers

| OpenClaw field | Panergos field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.panergos/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Panergos `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Panergos path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `PANERGOS_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct Panergos equivalent)

These are saved to `~/.panergos/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Panergos |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Panergos has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `panergos cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `panergos webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `panergos honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `panergos skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Panergos profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to Panergos manually via `panergos config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.panergos/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `panergos status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart panergos-gateway`

6. **Check session archives** — review archived advanced settings; idle and daily reset timers are intentionally not imported.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `panergos whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `panergos claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `panergos config set`.

### Skills not appearing after migration

Imported skills land in `~/.panergos/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `panergos config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
