...
This page covers all commands related to [Panergos profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `panergos profile` without a subcommand shows help.
...
## `panergos profile use`

```bash
panergos profile use <name>
```

Sets `<name>` as the active profile. All subsequent `panergos` commands (without `-p`) will use this profile.
...
panergos profile use default
```

## `panergos profile create`

```bash
panergos profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `panergos profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `panergos update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.panergos`), use `panergos skills opt-out` / `panergos skills opt-in`. |
...
panergos profile create work2 --clone-from work

# Clone everything from a specific profile
panergos profile create work2-backup --clone-from work --clone-all
```

## `panergos profile describe`

```bash
panergos profile describe [<name>] [options]
```
...
panergos profile describe researcher

# Set it explicitly
panergos profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
panergos profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
panergos profile describe --all --auto
```

## `panergos profile delete`

```bash
panergos profile delete <name> [options]
```
...
```bash
panergos profile delete mybot
panergos profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.panergos`) cannot be deleted — use `panergos uninstall` to remove everything.
:::

## `panergos profile show`

```bash
panergos profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Panergos home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ panergos profile show work
Profile: work
Path:    ~/.panergos/profiles/work
...
## `panergos profile alias`

```bash
panergos profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Panergos installation.
...
```bash
panergos profile alias work
# Creates/updates ~/.local/bin/work

panergos profile alias work --name mywork
# Creates ~/.local/bin/mywork

panergos profile alias work --remove
# Removes the wrapper script
```

## `panergos profile rename`

```bash
panergos profile rename <old-name> <new-name>
```
...
```bash
panergos profile rename mybot assistant
# ~/.panergos/profiles/mybot → ~/.panergos/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `panergos profile export`

```bash
panergos profile export <name> [options]
```
...
```bash
panergos profile export work
# Creates work.tar.gz in the current directory

panergos profile export work -o ./work-2026-03-29.tar.gz
```
...
## `panergos profile import`

```bash
panergos profile import <archive> [options]
```
...
```bash
panergos profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

panergos profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `panergos -p telemetry`). |
...
panergos profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
panergos profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
panergos profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
panergos profile install ./telemetry/
```

### `panergos profile update`

```bash
panergos profile update <name> [--force-config] [--yes]
```
...
### `panergos profile info`

```bash
panergos profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Panergos version, author, env var requirements, the source URL/path, and
...
`panergos profile list` also shows the distribution name and version in a
`Distribution` column, and `panergos profile show <name>` / `delete <name>`
...
panergos profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
panergos profile install https://github.com/your-org/internal-assistant.git
```
...
`panergos_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Panergos version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Panergos can clone from.
3. Tell recipients to run `panergos profile install <your-repo-url>`.
...
## `panergos -p` / `panergos --profile`

```bash
panergos -p <name> <command> [options]
panergos --profile <name> <command> [options]
```

Global flag to run any Panergos command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
panergos -p work chat -q "Check the server status"
panergos --profile dev gateway start
panergos -p personal skills list
panergos -p work config edit
```

## `panergos completion`

```bash
panergos completion <shell>
```
...
- `panergos profile <TAB>` — subcommands (list, use, create, etc.)
- `panergos profile use <TAB>` — profile names
- `panergos -p <TAB>` — profile names
