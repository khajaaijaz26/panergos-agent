...
description: "What every symbol in the Panergos terminal UI means — transcript markers, status-bar badges, overlay glyphs, and approval prompts."
---

# CLI Symbols Glossary

The Panergos terminal interfaces speak a compact visual language: dots, chevrons, braille spinners, and status glyphs. This page is the decoder ring. It covers the [TUI](../user-guide/tui.md) (where most of these render) and notes the pieces shared with the [Classic CLI](../user-guide/cli.md).
...
| `╲  ▶` → `╲━━▶` → `╱━━▶` | Default `relay` busy-indicator frames: work lanes converge into execution. |
...
| `♥` | Affection flash — Panergos noticed you being nice to it. |
