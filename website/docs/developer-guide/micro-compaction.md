...
has to be thrown away or summarized. Panergos has always done this in one batch:
...
Panergos folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Panergos' normal session flush is
...
With `micro_compact` unset or `false` Panergos behaves exactly as it always has:
...
Defaults to `$PANERGOS_HOME/logs/agent.log`. It reports passes, outcome mix, net
