...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_panergos_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`PANERGOS_HOME`, `PANERGOS_PROFILE`, proxy settings,
...
reload helper: `panergos_cli.env_loader.load_panergos_dotenv` skips the
...
## The PANERGOS_HOME override

`panergos_constants.py` holds a context-local override consulted by
`get_panergos_home()` before the `PANERGOS_HOME` env var. Everything that resolves
...
automatically. `get_process_panergos_home()` exists for the few machine-level
assets that must not follow the override. `panergos_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `panergos_cli/profiles.py`
...
DB handles are resolved at call time through the active PANERGOS_HOME override —
...
PANERGOS_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `panergos_home_key()`. |
...
| HTTP listener, process lock | One per process, owned by the default/active profile. Per-profile `runtime_status.json` is still written. |
...
- `agent/secret_scope.py`, `panergos_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `panergos_cli/profiles.py`
