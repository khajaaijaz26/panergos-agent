...
description: "How the iron-proxy egress firewall integrates with Panergos — module layout, lifecycle, security invariants, and extension points"
---

# Egress proxy internals

This page covers the architecture of the egress credential-injection firewall (`panergos egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy.md).
...
panergos_cli/proxy_cli.py               Wizard + slash command handlers.
                                       `panergos egress {install,setup,start,stop,
                                       status,disable,config}`.  Wires the
                                       core module into argparse.

panergos_cli/subcommands/egress.py:_dispatch_egress
                                       Top-level subparser dispatcher.
                                       dest='egress_command' (intentionally
                                       disjoint from the inbound OAuth
                                       `panergos proxy` subparser, which uses
                                       dest='proxy_command').

panergos_cli/config.py: proxy schema    The `proxy:` block in DEFAULT_CONFIG.
...
tools/environments/docker.py
  ...
  DockerEnvironment.__init__          Docker-side merge logic: collision
                                       ...
                                       _PANERGOS_EGRESS_NODE_OPTIONS_APPEND
...
tests/panergos_cli/test_iron_proxy_cli.py          CLI handler unit tests (~20).  Argparse
                                       wiring, fail-loud paths, BWS refresh
                                       wire-up, dest='egress_command'
                                       regression guard.

tests/agent/test_iron_proxy_e2e.py          Live E2E (gated on PANERGOS_RUN_E2E=1).
...
panergos egress install
  -> agent.proxy_sources.iron_proxy.install_iron_proxy(force=...)
       ...
       Stage into ~/.panergos/bin/.iron-proxy_XXXX, chmod 755, os.replace
         to ~/.panergos/bin/iron-proxy (atomic).
       _VERSION_CACHE.pop(target) so a forced reinstall re-probes
         --version on next call.

panergos egress setup [--from-bitwarden | --no-bitwarden] [--rotate-tokens]
  -> proxy_cli.cmd_setup
...
| Path | Mode | Test |
|---|---|---|
| `~/.panergos/proxy/` (dir) | `0o700` | `test_proxy_state_dir_is_0o700` |
...
`merge_mappings(existing, discovered, rotate=False)` MUST return prior tokens for providers that overlap.  Re-running `panergos egress setup` cannot silently 401 running sandboxes.  `--rotate-tokens` is the explicit opt-in.
...
`cmd_setup` MUST NOT downgrade `credential_source: bitwarden` to `env` on re-run without an explicit `--no-bitwarden` flag.  Running `panergos egress setup` (no flag) preserves whatever was previously configured.
...
If the provider uses SigV4 / SDK-minted OAuth / request signatures, a static header swap cannot cover it.  Add the env var to `_NON_BEARER_PROVIDERS` so the wizard and `panergos egress status` warn about it:
...
4. Sets the seven env vars (HTTPS_PROXY, NO_PROXY, REQUESTS_CA_BUNDLE, SSL_CERT_FILE, CURL_CA_BUNDLE, NODE_EXTRA_CA_CERTS, PANERGOS_EGRESS_PROXY) and the per-mapping `PANERGOS_PROXY_TOKEN_<NAME>` vars.
5. Distributes the CA cert into the sandbox at a path the runtime will trust (typically `/etc/ssl/certs/panergos-egress-ca.crt`).
...
- User-facing setup + troubleshooting: [Egress proxy](https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/egress/iron-proxy)
- Docker backend internals: [Docker](https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/docker)
- Bitwarden Secrets Manager integration: [`panergos secrets bitwarden`](https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/secrets/bitwarden)
- CLI command reference: [`panergos egress`](https://khajaaijaz26.github.io/panergos-agent/docs/reference/cli-commands#panergos-egress)
- Sandbox-injected environment variables: [Egress proxy (sandbox-injected)](https://khajaaijaz26.github.io/panergos-agent/docs/reference/environment-variables#egress-proxy-sandbox-injected)
