...
description: "How to build a memory provider plugin for Panergos Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Panergos Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `panergos plugins`.
:::

## Installation Layouts

Panergos discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Panergos. Closed to new providers — see [CONTRIBUTING](https://github.com/khajaaijaz26/panergos-agent/blob/main/CONTRIBUTING.md). |
| User | `$PANERGOS_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.panergos/plugins/<name>/` | Opt-in via `PANERGOS_ENABLE_PROJECT_PLUGINS=1`. |
| Package | `panergos_agent.memory_providers` entry point | `pip install`, nothing to copy. |
...
Panergos, in `$PANERGOS_HOME/plugins/<name>/` when installed by a user, or in
`./.panergos/plugins/<name>/` for a project-local one:
...
`panergos_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."panergos_agent.memory_providers"]
...
package layout. No copy under `$PANERGOS_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Panergos reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `panergos <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          panergos_home (str): Active PANERGOS_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `panergos memory setup` | **Yes** |
| `save_config(values, panergos_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
The gate binds to every compaction authority, not just the Panergos
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Panergos
...
`get_config_schema()` returns a list of field descriptors used by `panergos memory setup`:
...
Every field in `get_config_schema()` is prompted during `panergos memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$PANERGOS_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, panergos_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(panergos_home) / "my-provider.json"
...
**`sync_turn()` MUST be non-blocking.** If your backend has latency (API calls, LLM processing), run the work in a daemon thread — spawned with `agent.memory_provider.spawn_context_thread`, never a bare `threading.Thread`. Profile isolation (the active `PANERGOS_HOME`, the per-turn secret scope) lives in `contextvars`, and a plain thread starts with an empty context: under multiplexed profiles it would silently write into the *default* profile's store, and `get_secret()` fails closed there.
...
The same applies to prefetch and writer threads. Small JSON config sidecars (`$PANERGOS_HOME/<provider>.json`) are read with `utils.read_json_or_empty` and written with `utils.atomic_json_write`; anything under `config.yaml` goes through `panergos_cli.config.save_config(..., merge_existing=True)`.
...
the `messages` parameter; Panergos will continue calling them with the legacy
signature.
...
All storage paths **must** use the `panergos_home` kwarg from `initialize()`, not hardcoded `~/.panergos`:

```python
# CORRECT — profile-scoped
from panergos_constants import get_panergos_home
data_dir = get_panergos_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.panergos/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `panergos my-provider status`, `panergos my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `panergos <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `panergos --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: panergos my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the panergos my-provider argparse tree.
