...
description: "How to add a new inference provider to Panergos Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Panergos can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `panergos model` menu entries
...
1. `panergos_cli/auth.py` decides how credentials are found.
2. `panergos_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `panergos_cli/models.py` and `panergos_cli/main.py` make the provider show up in the CLI. (`panergos_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `panergos_cli/auth.py`
2. `panergos_cli/models.py`
3. `panergos_cli/runtime_provider.py`
4. `panergos_cli/main.py`
...
:::tip
`panergos_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `panergos setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$PANERGOS_HOME/plugins/model-providers/` both get picked up.
...
7. `panergos model` menu includes the provider
8. `panergos setup` wizard delegates to `main.py` automatically
...
User plugins at `$PANERGOS_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `panergos model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `panergos_cli/auth.py`
- `_PROVIDER_LABELS` in `panergos_cli/models_catalog_static.py` (re-exported by `panergos_cli/models.py`)
- `_PROVIDER_ALIASES` in both `panergos_cli/auth.py` and `panergos_cli/models_catalog_static.py`
- CLI `--provider` choices in `panergos_cli/main.py`
...
## Step 2: Add auth metadata in `panergos_cli/auth.py`
...
- What env vars should Panergos check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `panergos_cli/models.py`
...
## Step 4: Resolve runtime data in `panergos_cli/runtime_provider.py`
...
Be careful with API-key precedence. Panergos already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `panergos_cli/main.py`

A provider is not discoverable until it shows up in the interactive `panergos model` flow.

Update these in `panergos_cli/main.py`:
...
:::tip
`panergos_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `panergos model` and `panergos setup` automatically.
:::
...
When you add a native provider, double-check that Panergos is only sending fields that provider actually understands.
...
- `tests/panergos_cli/test_runtime_provider_resolution.py`
- `tests/panergos_cli/test_cli_provider_resolution.py`
- `tests/panergos_cli/test_model_switch_custom_providers.py` (and adjacent `tests/panergos_cli/test_model_switch_*.py`)
- `tests/panergos_cli/test_setup_model_provider.py`
...
python -m pytest tests/panergos_cli/test_runtime_provider_resolution.py tests/panergos_cli/test_cli_provider_resolution.py tests/panergos_cli/test_setup_model_provider.py tests/agent/test_provider_parity.py -q
```
...
```bash
source venv/bin/activate
python -m panergos_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m panergos_cli.main model
...
- [ ] `ProviderConfig` added in `panergos_cli/auth.py`
- [ ] aliases added in `panergos_cli/auth.py` and `panergos_cli/models.py`
- [ ] model catalog added in `panergos_cli/models.py`
- [ ] runtime branch added in `panergos_cli/runtime_provider.py`
- [ ] CLI wiring added in `panergos_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `panergos model` but not `panergos setup`
