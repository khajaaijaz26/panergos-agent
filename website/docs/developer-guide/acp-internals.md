...
The ACP adapter wraps Panergos' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
panergos acp / panergos-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.panergos/.env
  -> configure stderr logging
  -> construct PanergosACPAgent
...
- `allow_once` -> Panergos `once`
- `allow_always` -> Panergos `always`
- reject options -> Panergos `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Panergos tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["panergos-acp"])
...
Instead it reuses Panergos' runtime resolver:

- `acp_adapter/auth.py`
- `panergos_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Panergos provider/credentials. It also always advertises a terminal setup auth method (`panergos-setup`, args `--setup`) so first-run ACP clients can open Panergos' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.panergos/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `panergos-acp` toolset definition
- `panergos_cli/main.py` — `panergos acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `panergos-acp` script
