...
Plugins can launch and supervise fresh Panergos child sessions without importing
...
active agent turn fails closed with `No active Panergos parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Panergos can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Panergos' existing unsafe-tool block remains enforced.
