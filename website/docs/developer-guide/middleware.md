...
Panergos middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Panergos'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Panergos calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Panergos retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Panergos guardrails,
...
- `telemetry_schema_version`: currently `panergos.observer.v1`
- `middleware_schema_version`: currently `panergos.middleware.v1`
...
Panergos stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Panergos runs
them as a nested chain in registration order. Middleware failures are fail-open:
Panergos logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Panergos applies middleware in this order:
...
For each tool call, Panergos applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Panergos pre-execution path against the effective arguments:
...
For isolated local testing, use one `PANERGOS_HOME` for plugin enablement and the
agent run:

```bash
export PANERGOS_HOME=/tmp/panergos-middleware-test
mkdir -p "$PANERGOS_HOME"
panergos plugins enable <plugin-name>
panergos chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run panergos plugins enable <plugin-name>
uv run panergos chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["panergos_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/panergos-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Panergos expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Panergos treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Panergos preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Panergos does not convert downstream
