...
# Panergos Observer Hooks

Panergos observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Panergos' planner, model providers, memory, tool registry,
...
Panergos also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "panergos.observer.v1"
```

Hook callbacks are fail-open. Panergos catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Panergos only builds
...
The native NeMo Relay SDK integration maps Panergos session, turn, LLM, and tool
