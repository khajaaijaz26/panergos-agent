...
Panergos gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Panergos' native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `panergos.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `panergos.platform.up/degraded` with bounded `error_code` attributes |
...
`panergos.gateway.active_agents`, `panergos.gateway.background_work`, and
`panergos.gateway.background_delegations` are complementary. `active_agents`
...
```bash
panergos monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'panergos-agent[otlp]'`),
...
a plain `panergos gateway run`: the exporter lives in the gateway process, so
...
`panergos.gateway.up`, `panergos.platform.up`, and `panergos.platform.degraded`.
...
absent_over_time(panergos_gateway_up[5m])

# Locally owned bridge is explicitly down.
panergos_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
panergos_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
panergos_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
panergos_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(panergos_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `panergos.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
panergos.status = failed|unknown
panergos.delivery_outcome = failed|not_configured
panergos.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `panergos.job_key`;
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/panergos_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/panergos_otel_capture.jsonl --wait 8
...
`panergos.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
panergos monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `panergos monitoring` CLI intentionally exposes `status` only. This first
release covers only Panergos Agent-owned service-health and operational-diagnostic
signals. Product analytics, audit/quality reporting, and detailed execution
