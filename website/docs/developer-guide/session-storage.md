...
Panergos Agent uses a SQLite database (`~/.panergos/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source files: `panergos_state.py` (facade) plus the `panergos_state_*.py` siblings (schema, fts, search, compression, portability, gateway, ...)

### Desktop profile isolation and compaction generations

Each named profile stores its transcript in its own `$PANERGOS_HOME/state.db`,
including when one `panergos serve` process serves several profiles. In-session
...
splice boundary, Panergos excludes only that leading item when it exactly matches
...
```
~/.panergos/state.db (SQLite, WAL mode)
...
`panergos sessions recover` copies the row-bearing tables above into the
...
Abridged — see `SCHEMA_SQL` in `panergos_state_common.py` (applied by `panergos_state_schema.py`) for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `panergos_state_common.py` for the exact SQL.
...
Multiple Panergos processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.panergos/state.db
...
Default path: `~/.panergos/state.db`

This is derived from `panergos_constants.get_panergos_home()` which resolves to
`~/.panergos/` by default, or the value of `PANERGOS_HOME` environment variable.
