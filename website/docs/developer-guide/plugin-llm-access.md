...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Panergos already has applies. The plugin never sees a
...
For image input, pass the same content blocks accepted by the provider runtime.
The async surface exposes `acomplete()` and `acomplete_structured()` and can be
combined with `asyncio.gather()` when calls are independent.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Panergos provider id (e.g. "openrouter")
...
using Panergos built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Panergos provider
        # (must be one Panergos already knows about — same names as
        # `panergos model` and config.yaml model.provider).
        ...
        # plugin sends — Panergos does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.panergos/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Panergos' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Panergos subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/khajaaijaz26/panergos-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/khajaaijaz26/panergos-agent/blob/main/tests/agent/test_plugin_llm.py)
