...
title: "Build a Panergos Plugin"
description: "Step-by-step guide to building a complete Panergos plugin with tools, hooks, data files, and skills"
---

# Build a Panergos Plugin

This guide walks through building a complete Panergos plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Panergos has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `PANERGOS_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.panergos/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `panergos skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `khajaaijaz26/panergos-agent`. Users install them into `~/.panergos/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Share it in [Panergos Discussions](https://github.com/khajaaijaz26/panergos-agent/discussions). See [CONTRIBUTING.md](https://github.com/khajaaijaz26/panergos-agent/blob/main/CONTRIBUTING.md) for the policy.
:::

## Portable Agent Plugins v1 packages

Panergos can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components Panergos already owns. It does not replace native `plugin.yaml` plus
...
```bash
panergos plugins install owner/repository --no-enable
panergos plugins list
panergos plugins enable <plugin-name>
```
...
Panergos validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Panergos.
...
Portable `streamable-http` entries are routed through Panergos' existing native
...
same full-trust posture as other installed Panergos plugins.
...
records it as Published. Panergos keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Panergos does not expose a
...
continue to work after a normal Panergos upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Panergos inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Panergos releases can therefore load a plugin whose manifest contains
...
Panergos enforces this contract with frozen external-plugin fixtures discovered
from an isolated `PANERGOS_HOME`. Those tests load and invoke the plugin through
...
Panergos' internals were split into `<stem>_<topic>` sibling modules in Sep 2026 (PR #102117). **Internal
...
- **Check your plugin:** `panergos plugins compat /path/to/your/plugin` lists every `file:line` with the
  old path and the new one, and exits 1 while any remain. `COMPAT_MANIFEST.md` in the repo is the full map.
- **What users see:** a notice under the CLI banner, in `panergos doctor` and after `panergos update`, and a
  one-time Desktop dialog naming the plugin. Each resolution through an old path also emits a
  `PanergosPluginCompatWarning` once per process.
- **From 2026-09-14:** plugins that still import old paths are **not loaded** (the reason shows in
  `panergos plugins list`). Users can force-load with `plugins.allow_deprecated_imports: true` until the
...
```bash
mkdir -p ~/.panergos/plugins/calculator
cd ~/.panergos/plugins/calculator
```

### Validate with Plugin Doctor

`panergos plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Panergos itself. It reports invalid hook names, callbacks that do
...
```bash
panergos plugins doctor . --ci
```

Doctor uses a temporary `PANERGOS_HOME`, restores plugin registration state after
...
This tells Panergos: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `panergos_cli/plugin_capabilities.py` for the
...
`panergos_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`panergos_agent.plugins` entry point:

```toml
[project.entry-points."panergos_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."panergos_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Panergos reads these from installed metadata without importing your code, so
`panergos plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Panergos understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — Panergos validates them, and `panergos plugins install` / `panergos plugins doctor` surface missing ones with a `pip install` hint, but Panergos **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into Panergos' shared venv is a conflict and supply-chain
...
[#64165](https://github.com/khajaaijaz26/panergos-agent/issues/64165) and
[#15220](https://github.com/khajaaijaz26/panergos-agent/issues/15220). Plugin
...
4. **Accept `**kwargs`:** Panergos may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `panergos my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Panergos continues fine
...
Use plugin-relative config keys for user-visible behavior. Panergos resolves them
...
`<PANERGOS_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `PANERGOS_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
PANERGOS_PLUGINS_DEBUG=1 panergos plugins list
```
...
The same logs are always written to `~/.panergos/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
panergos logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `panergos plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.panergos/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `panergos plugins update` / `remove` git-pull or delete it — your
...
# <panergos home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.panergos/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.panergos/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `panergos plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Panergos into installing arbitrary packages — only specs Panergos itself ships are eligible |
...
auditable in `~/.panergos/logs/agent.log`. Plugins load after built-in
...
does not ship with Panergos core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `panergos plugins list`
...
loader, the plugin is disabled and Panergos continues. The gate exists
...
You normally never edit this key by hand. `panergos plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Panergos injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: panergos-agent"}
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$PANERGOS_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $PANERGOS_HOME/hook_outputs
```
...
- **The system prompt is Panergos' territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `panergos_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Panergos sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`panergos.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Panergos still loads on an older one.

### Register CLI commands

Plugins can add their own `panergos <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for panergos my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: panergos my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for panergos my-plugin."""
...
After registration, users can run `panergos my-plugin status`, `panergos my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `panergos name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `panergos chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `PANERGOS_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `panergos <subcommand>` (e.g. `panergos kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "panergos kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Panergos from a hook.
...
Panergos has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.panergos/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `panergos gateway status` without SDK instantiation.
...
Panergos also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Panergos without any Python plugin. Declare them in `~/.panergos/config.yaml`:
...
Panergos connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.panergos/hooks/<name>/`:

```yaml
# ~/.panergos/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.panergos/hooks/long-task-alert/handler.py
...
panergos skills search my-workflow --source myorg/skills-repo
panergos skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install panergos-plugin-calculator
# Plugin auto-discovered on next Panergos startup
```
...
```python
# Wrong — will break if Panergos passes extra context
def handler(args):
    ...
