...
description: "How Panergos stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `panergos_cli/cron.py` | CLI `panergos cron` subcommands |
...
Jobs are stored in `~/.panergos/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
renderer (`panergos cron list`/`doctor`, the `cronjob` tool, the web dashboard
...
The same store fields drive every topology: a standalone `panergos -p X gateway
...
- **a named provider** → discovered from `plugins/cron_providers/<name>/` or
  `$PANERGOS_HOME/plugins/<name>/`.
...
In CLI mode, cron jobs only fire when `panergos cron` commands are run or during active CLI sessions.
...
```python
# ~/.panergos/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.panergos/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `panergos cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `panergos cron` CLI provides direct job management:

```bash
panergos cron list                    # Show all jobs
panergos cron create                  # Interactive job creation (alias: add)
panergos cron edit <job_id>           # Edit job configuration
panergos cron pause <job_id>          # Pause a running job
panergos cron resume <job_id>         # Resume a paused job
panergos cron run <job_id>            # Trigger immediate execution
panergos cron remove <job_id>         # Delete a job
```
