...
title: "Desktop Plugin SDK (@panergos/plugin-sdk)"
description: "Extend the native Panergos Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Panergos Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `PanergosPlugin`.
It imports one module — `@panergos/plugin-sdk` — and gets everything: the app's
...
`$PANERGOS_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Panergos. This page is the **native
desktop app** (`panergos desktop`) SDK — the `@panergos/plugin-sdk` module and
`$PANERGOS_HOME/desktop-plugins/`. The **web dashboard** (`panergos dashboard`) has
its own, unrelated plugin system on `window.__PANERGOS_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Panergos Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$PANERGOS_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$PANERGOS_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `PanergosPlugin` contract, appear in **Capabilities → Plugins**,
...
toggle and contributes nothing while disabled. The quick start below is the
canonical reference implementation.

## Quick start — your first plugin

Create `$PANERGOS_HOME/desktop-plugins/hello/plugin.js` (that's `~/.panergos/...`
...
```javascript
// ~/.panergos/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@panergos/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Panergos' }),
      jsx('div', {
...
The only importable specifiers are `@panergos/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `PanergosPlugin`:

```ts
interface PanergosPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `panergos.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@panergos/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@panergos/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@panergos/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@panergos/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@panergos/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@panergos/plugin-sdk'
...
`setAccentOverride(hex)` and clear it in `ctx.onDispose`; this keeps the override
scoped to the plugin lifecycle.
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@panergos/plugin-sdk'
...
`window.panergos.send('get-price eth')` (or a declarative
`<button data-panergos-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@panergos/plugin-sdk'
...
for requests; `targetProfile` is the backend Panergos profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Panergos
...
ctx.os.notify({
  ...
  // Body click → focus Panergos + navigate. Same vocabulary as OS deep links:
  activate: 'panergos://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'panergos://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `panergos://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `panergos://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@panergos/plugin-sdk'
...
```javascript
import { queryClient } from '@panergos/plugin-sdk'
...
copies that half into `$PANERGOS_HOME/desktop-plugins/<id>/` beside a
`.panergos-package.json` marker, and the renderer loads it through the exact same
pipeline as the standalone disk door (hot reload included):

```
~/.panergos/plugins/<id>/           # ONE installable folder
...
is refreshed when the source `plugin.js` changes (`panergos plugins update`, or
...
a package into `~/.panergos/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.panergos/plugins` is not reachable as a filesystem —
...
the `panergos://` scheme — a plain anchor on your website or README:
...
`panergos-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Panergos plugin and declare it in a
`manifest.json`:
...
panergos-agent codebase directly (`panergos_state`, `panergos_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.panergos/`)
...
(`panergos.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `PanergosPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@panergos/plugin-sdk` alias.
2. It's still lint-fenced to `@panergos/plugin-sdk` + `react` only — no `@/…` app
   internals.

Bundled plugins stay deliberately small; use the disk-plugin quick start for
reference implementations and experimentation.
...
- **Only three specifiers resolve:** `@panergos/plugin-sdk`, `react`,
...
| Plugin contract | `PanergosPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `PanergosOpenTarget`, `Contribution` |
...
### Agents: the `panergos-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`panergos-desktop-plugins`** skill — it carries the same contract as this page in
...
`$PANERGOS_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `panergos logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@panergos/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.panergos/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.panergos/logs/errors.log` for
