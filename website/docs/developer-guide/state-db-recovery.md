...
description: "How Panergos recovers state.db when the FTS index or the file itself is corrupt"
---
...
`panergos doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `panergos doctor --fix` while the gateway
is still up. Next steps:

```bash
panergos gateway stop
PANERGOS_HOME="$HOME/.panergos" panergos sessions recover --source "$HOME/.panergos/state.db" --inspect-only
# if recoverable:
PANERGOS_HOME="$HOME/.panergos" panergos sessions recover --source "$HOME/.panergos/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
panergos gateway stop
PANERGOS_HOME="$HOME/.panergos" panergos sessions repair --check-only
PANERGOS_HOME="$HOME/.panergos" panergos sessions repair
```
...
```bash
PANERGOS_HOME="$HOME/.panergos" panergos sessions repair --check-only
sqlite3 "$HOME/.panergos/state.db" \
