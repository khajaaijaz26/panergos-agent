...
sidebar_position: 1
title: "Architecture"
description: "Panergos Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Panergos Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ panergos_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # PanergosCLI facade — mixins in panergos_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── panergos_state.py           # SQLite session/state database facade (+ panergos_state_*.py siblings)
├── panergos_constants.py       # PANERGOS_HOME, profile-aware paths
...
│
├── panergos_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `panergos` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # panergos skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # panergos tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # panergos gateway start/stop
│
...
```text
User input → PanergosCLI.process_input()
...
Three discovery sources: `~/.panergos/plugins/` (user), `.panergos/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `panergos plugins` or `config.yaml`.
...
Exposes Panergos as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`panergos -p <name>`) gets its own PANERGOS_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
