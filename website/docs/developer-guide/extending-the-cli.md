...
description: "Build wrapper CLIs that extend the Panergos TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Panergos exposes protected extension hooks on `PanergosCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the `run()` method or the TUI construction in `panergos_cli/cli_tui_mixin.py` (where these hooks are defined; `PanergosCLI` in `cli.py` mixes it in). This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends Panergos."""
...
Called after Panergos registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
