...
sidebar_position: 15
title: "CLI Internals"
description: "How panergos_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `panergos_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `panergos_cli/AGENTS.md`; user-facing behaviour
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `panergos_cli.update_cmd._panergos_holder_subcommand` (top-level subcommand of any Panergos argv). Never
...
Panergos supports profiles — fully isolated instances, each with its own `PANERGOS_HOME` (config, API
keys, memory, sessions, skills, gateway). `_apply_profile_override()` in `panergos_cli/main.py` sets
`PANERGOS_HOME` before any module imports, so every `get_panergos_home()` reference scopes to the active
profile. Profile operations are HOME-anchored (`_get_profiles_root()` returns
`Path.home() / ".panergos" / "profiles"`, not `get_panergos_home() / "profiles"`) so
`panergos -p coder profile list` sees all profiles regardless of which one is active — intentional.
