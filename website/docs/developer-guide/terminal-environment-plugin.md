...
Panergos runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.panergos/plugins/`, registering a backend the user selects
...
| `panergos setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `panergos status` / `panergos doctor` | `doctor_checks()` |
...
```python title="~/.panergos/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.panergos"    # where synced cache files land, or None
...
```yaml title="~/.panergos/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
panergos plugins enable acmebox
panergos config set terminal.backend acmebox
```
