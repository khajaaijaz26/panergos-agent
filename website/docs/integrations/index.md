...
Panergos Agent connects to external systems for AI inference, tool servers, IDE workflows, programmatic access, and more. These integrations extend what Panergos can do and where it can run.

:::tip Start here
If you only have time to set up one integration, run [`panergos model --quick`](/getting-started/model-quickstart) and choose a ready local runtime or cloud endpoint.
:::

## AI Providers & Routing

Panergos supports multiple AI inference providers out of the box. Use `panergos model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](/integrations/providers)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. Panergos auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[MCP Servers](/user-guide/features/mcp)** — Connect Panergos to external tool servers via Model Context Protocol. Access tools from GitHub, databases, file systems, browser stacks, internal APIs, and more without writing native Panergos tools. Supports both stdio and SSE transports, per-server tool filtering, and capability-aware resource/prompt registration.

## Web Search Backends

The `web_search` and `web_extract` tools support eight backend providers, configured via `config.yaml` or `panergos tools`:
...
Panergos includes full browser automation with multiple backend options for navigating websites, filling forms, and extracting information:
...
- **[IDE Integration (ACP)](/user-guide/features/acp)** — Use Panergos Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains. Panergos runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](/user-guide/features/api-server)** — Expose Panergos as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use Panergos as a backend with its full toolset.
...
Panergos runs as a gateway bot on 27+ messaging platforms, all configured through the same `gateway` subsystem:
...
| **Slack** | [api.slack.com/apps?new_app=1](https://api.slack.com/apps?new_app=1) | The **Create New App** dialog — pick *From an app manifest* and paste the manifest `panergos slack manifest --agent-view` generates |
...
- **[Buzz](/integrations/buzz)** — Block's Nostr-based human+agent workspace. Three integration paths: Buzz Desktop spawns Panergos as a managed ACP runtime, the `buzz-acp` relay bridge hosts a Panergos identity server-side, or the native gateway platform joins Buzz channels with full Panergos memory/skills/approvals/cron. The overview page compares all three.
...
- **[Plugin System](/user-guide/features/plugins)** — Extend Panergos with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.panergos/plugins/`, project-local `.panergos/plugins/`, and pip-installed entry points.
- **[Build a Plugin](/developer-guide/plugins)** — Step-by-step guide for creating Panergos plugins with tools, hooks, and CLI commands.
