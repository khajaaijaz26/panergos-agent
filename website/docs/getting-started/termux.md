...
description: "Run Panergos Agent directly on an Android phone with Termux"
---

# Panergos on Android with Termux
...
Panergos Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Panergos from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Panergos now ships a Termux-aware installer path:
...
- links `panergos` into `$PREFIX/bin` so it stays on your Termux PATH
...
Panergos requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `panergos` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/panergos" "$PREFIX/bin/panergos"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `panergos` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.panergos/.env`.
...
### `panergos doctor` says ripgrep or Node is missing
