...
sidebar_position: 10
title: "皮肤与主题"
description: "使用内置和用户自定义皮肤定制 Panergos CLI 的外观"
---

# 皮肤与主题

皮肤控制 Panergos CLI 的**视觉呈现**：横幅颜色、spinner（加载动画）面孔与动词、响应框标签、品牌文本以及工具活动前缀。
...
或在 `~/.panergos/config.yaml` 中设置默认皮肤：
...
在 `~/.panergos/skins/` 下创建 YAML 文件。用户皮肤会从内置 `default` 皮肤继承缺失的值，因此只需指定要更改的键。
...
- 内置皮肤从 `panergos_cli/skin_engine.py` 加载。
- 未知皮肤自动回退到 `default`。
- `/skin` 立即更新当前会话的活动 CLI 主题。
- `~/.panergos/skins/` 中的用户皮肤优先于同名内置皮肤。
