...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.panergos/.env ] && grep -q "^GITHUB_TOKEN=" ~/.panergos/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.panergos/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${PANERGOS_HOME:-$HOME/.panergos}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Panergos Agent\",
    \"comments\": [
...
```bash
source "${PANERGOS_HOME:-$HOME/.panergos}/skills/github/github-auth/scripts/gh-env.sh"
...
```bash
# 若无问题——批准
gh pr review $PR_NUMBER --approve --body "Reviewed by Panergos Agent. Code looks clean — good test coverage, no security concerns."
