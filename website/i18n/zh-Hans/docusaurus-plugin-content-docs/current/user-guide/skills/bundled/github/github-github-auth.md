...
- 点击"New SSH key"
- 粘贴公钥内容
- 填写标题，如"panergos-agent-&lt;machine-name>"
...
uv run python3 "${PANERGOS_HOME:-$HOME/.panergos}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif [ -f ~/.panergos/.env ] && grep -q "^GITHUB_TOKEN=" ~/.panergos/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.panergos/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python3 "${PANERGOS_HOME:-$HOME/.panergos}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
