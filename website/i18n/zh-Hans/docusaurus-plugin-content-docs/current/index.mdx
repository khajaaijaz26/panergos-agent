...
Panergos 保留了 Panergos 的 CLI、桌面端、网关、提供商、工具、技能、记忆、插件、MCP、调度和委派运行时，并新增用于重启安全协调的 Durable Missions。当前 0.1 版本仅提供源码安装，不发布预构建桌面安装程序。

| 能力 | 来源 | 状态 |
|---|---|---|
| CLI、TUI、桌面端、Web 控制台及 IDE/ACP 界面 | 继承自固定版本的 Panergos | 源码发行版中可用 |
| 提供商、工具、记忆、技能、插件、MCP、定时任务、网关及委派 | 继承自固定版本的 Panergos | 可用；兼容性由分支测试覆盖 |
| 持久任务状态、基于 Kanban 的任务图、点对点消息及审计事件 | Panergos 新增 | 0.1 中为实验性功能 |
| Capability Forge、Policy Ledger 及对比 Evidence Gates | Panergos 路线图 | 规划中，并非当前发行声明 |
