...
> = {
  ...
  optional: {
    label: "Optional",
    color: "#f7c453",
    bg: "rgba(247, 196, 83, 0.08)",
    border: "rgba(247, 196, 83, 0.2)",
    icon: "\u{2B50}",
  },
  Anthropic: {
    label: "Anthropic",
        ...
        {expanded && (
          <div className={styles.cardDetail}>
            ...
            <div className={styles.installHint}>
              <code>{skill.installCmd || `panergos skills install ${skill.name}`}</code>
              <CopyButton
                text={skill.installCmd || `panergos skills install ${skill.name}`}
              />
            </div>
            {onPick ? (
              <button
...
export default function SkillsDashboard() {
  ...
  // app (Panergos desktop's Bot Mode agent editor) as a skill PICKER. Site
  // chrome is hidden via a CSS class and every card gains an
  // "+ Add to this Agent" button that posts
  //   { type: 'panergos-skill-pick', name, identifier, installCmd, source }
  ...
  const pickSkill = useCallback(
    (skill: Skill) => {
      ...
      window.parent.postMessage(
        {
          type: "panergos-skill-pick",
          name: skill.name,
          identifier: skill.identifier || skill.name,
          installCmd: skill.installCmd || `panergos skills install ${skill.name}`,
          source: skill.source,
        },
        "*"
      );
