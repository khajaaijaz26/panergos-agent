...
const CATALOG_README_URL =
  "https://github.com/khajaaijaz26/panergos-agent/tree/main/plugin-catalog";
...
> = {
  official: {
    ...
    bg: "rgba(var(--panergos-accent-rgb), 0.08)",
    border: "rgba(var(--panergos-accent-rgb), 0.25)",
    icon: "\u{2713}",
  },
  community: {
    label: "Community",
      ...
      <div className={styles.cardInner}>
        ...
        {expanded && (
          <div className={styles.cardDetail}>
            ...
            {plugin.requiresPanergos && (
              <div className={styles.metaRow}>
                <span className={styles.metaLabel}>Requires</span>
                <span className={styles.metaValue}>
                  <code>panergos {plugin.requiresPanergos}</code>
                </span>
              </div>
            )}
            <div className={styles.metaRow}>
...
export default function PluginCatalogPage() {
  const pluginsUrl = useBaseUrl("/api/plugins.json");
  const metaUrl = useBaseUrl("/api/plugins-meta.json");
  // Picker embed mode (?embed=picker): the page is iframed by a host app
  // (Panergos desktop's Capabilities > Plugins tab) as a one-click catalog
  // picker. Site chrome is hidden via CSS and every card gains an
  // "+ Add to this Agent" button that posts
  //   { type: 'panergos-plugin-pick', name, repo, sha, subdir, tier,
  //     installCmd }
  ...
  const pickPlugin = useCallback((plugin: CatalogPlugin) => {
    ...
    window.parent.postMessage(
      {
        type: "panergos-plugin-pick",
        name: plugin.name,
        repo: plugin.repo,
        sha: plugin.sha,
        subdir: plugin.subdir || "",
        tier: plugin.tier,
        installCmd: plugin.installCommand || `panergos plugins install ${plugin.name}`,
      },
      "*"
    );
  }, []);
  ...
  return (
    <Layout
      ...
      description="Browse reviewed, SHA-pinned plugins for Panergos Agent"
    >
      <div className={`${styles.page} ${pickerMode ? styles.pickerMode : ""}`}>
        <header className={styles.hero}>
          <div className={styles.heroGlow} />
          <div className={styles.heroContent}>
            <p className={styles.heroEyebrow}>Panergos Agent</p>
          ...
          {!data && !loadError ? (
            ...
            <div className={styles.empty}>
              ...
              <p className={styles.emptyDesc}>
                The plugin catalog is a curated, human-reviewed list of Panergos
