...
Panergos Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``PANERGOS_NIX_BUILD=1`` on the
Panergos package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("PANERGOS_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for Panergos Agent is not supported.\n"
    "Panergos is distributed from its public source checkout.\n"
    "See: https://github.com/khajaaijaz26/panergos-agent#installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the inherited Nix derivation sets PANERGOS_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``panergos_state``, ``toolsets``...)
