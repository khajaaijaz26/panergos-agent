...
class SessionSessionsMixin:
    ...
    def _own_profile_name(self) -> Optional[str]:
        ...
        try:
            ...
            root = get_default_panergos_root().resolve()
...
""" + _UPSERT_KEEP_EXISTING_SQL,
    ...
    def _project_compression_tips(self, sessions: List[Dict[str, Any]], compact_rows: bool) -> List[Dict[str, Any]]:
        ...
        for s in sessions:
            ...
            if merged.get("title") is None:
                ...
                # between leaves it on the ended root, and exact-title lookups (`panergos peer dm` ->
    ...
    def search_sessions(
        ...
        so ``panergos -c``/``--resume`` picks its last session."""
        where_clauses = []
        params: list = []
        if source:
