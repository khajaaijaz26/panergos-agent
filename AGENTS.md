# Panergos Agent - Development Guide

Instructions for AI coding assistants and developers working on the Panergos Agent codebase.
...
Panergos is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: Panergos ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`panergos tools`, `panergos setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `PANERGOS_HOME`. Mocks hide integration bugs.
...
- **New `PANERGOS_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `panergos tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.panergos/plugins/` or pip
  entry point), and publish it through an independent community channel. This is a
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `panergos <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`panergos webhook`, `panergos cron`, `panergos tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.panergos/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, one behind URL + token, or a hosted backend, and
only the first two carry `PANERGOS_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the Panergos desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_PANERGOS_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `PANERGOS_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`panergos --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.panergos/panergos-agent/venv`
...
├── toolsets.py           # TOOLSETS dict, _PANERGOS_CORE_TOOLS
├── cli.py                # PanergosCLI (REPL, slash dispatch) + panergos_cli/cli_*_mixin.py
├── panergos_state.py       # SessionDB facade; panergos_state_*.py siblings
├── panergos_constants.py   # get_panergos_home(), display_panergos_home() — profile-aware paths
├── panergos_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── panergos_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
├── ui-tui/               # Ink (React) terminal UI — `panergos --tui`
...
**User state:** `~/.panergos/config.yaml` (settings), `~/.panergos/.env` (secrets only),
`~/.panergos/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_panergos_home()`. Browse logs with `panergos logs [--follow] [--level] [--session]`.
...
families: `panergos_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`panergos_cli/kanban.py` (14), `panergos_cli/web_server.py` (13 + 24 routers), `panergos_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `panergos_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::panergos_cli.plugin_compat.PanergosPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `panergos_cli.update_cmd._panergos_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `panergos_cli/AGENTS.md`.
- **Never hardcode `~/.panergos`.** `get_panergos_home()` for code paths, `display_panergos_home()`
  for user-facing text (both from `panergos_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Module-level constants are fine — they cache after `_apply_profile_override()`
  sets `PANERGOS_HOME`. Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.panergos/profiles`) so `panergos -p x profile list`
  sees all profiles — intentional, not a bug.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `panergos webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `PANERGOS_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `PANERGOS_HOME` → temp dir, and per-file subprocess
...
- **Flake policy:** a failing FILE is retried once in a fresh subprocess (`--file-retries`;
  `PANERGOS_TEST_FILE_RETRIES=0` disables). Pass-on-retry is green but printed under `⚠ FLAKY`
...
- **Placement mirrors the source tree.** A test lives in `tests/<top-level source dir>/` (`tests/panergos_cli/`,
  `tests/agent/`, `tests/panergos_state/`, `tests/gateway/relay/`, ...); installer/updater script tests
  under `tests/scripts/{install,desktop_update}/`. Only tests of root-level modules (`batch_runner`,
  `utils`, `panergos_constants`, packaging) sit directly in `tests/`. No issue numbers in filenames —
...
- **Tests must not write to `~/.panergos/`.** The autouse `_isolate_panergos_home` fixture in
  `tests/conftest.py` redirects `PANERGOS_HOME`; never hardcode `~/.panergos/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_panergos_home()` stay
  in the temp dir (pattern: `tests/panergos_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      home = tmp_path / ".panergos"; home.mkdir()
      monkeypatch.setattr(Path, "home", lambda: tmp_path)
      monkeypatch.setenv("PANERGOS_HOME", str(home))
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `PANERGOS_HOME` — code reads the
  env var, not `Path.home()/.panergos`.
...
`tests/panergos_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `panergos_cli/`, `main.py` | `panergos_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `panergos update` pipeline, profiles / multiplex |
...
| `plugins/`, `panergos_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `panergos_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
| `cron/`, kanban (`panergos_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
...
`panergos-agent-dev` skill, not here.
