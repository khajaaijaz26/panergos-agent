#!/usr/bin/env python3
"""Google Workspace API CLI for Panergos Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Panergos-facing JSON contract and falls back to the Python client
...
from _panergos_home import get_panergos_home
...
def _gws_binary() -> str | None:
    override = os.getenv("PANERGOS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Panergos Agent")
