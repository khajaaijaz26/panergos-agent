...
Ledger path resolution (first wins):
  --ledger PATH
  $PANERGOS_CITATION_LEDGER
  $PANERGOS_HOME/cache/citations/ledger.json
"""
...
from _panergos_home import get_panergos_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("PANERGOS_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_panergos_home() / "cache" / "citations" / "ledger.json"
