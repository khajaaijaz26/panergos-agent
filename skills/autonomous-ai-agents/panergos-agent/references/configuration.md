...
Edit with `panergos config edit` or `panergos config set section.key value`.
Full reference: https://khajaaijaz26.github.io/panergos-agent/docs/user-guide/configuration
...
`panergos config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `panergos tools` (interactive) or `panergos tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_PANERGOS_CORE_TOOLS` is the default bundle most platforms inherit).
