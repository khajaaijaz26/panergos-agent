...
For occasional contributors and PR authors. Full developer docs: https://khajaaijaz26.github.io/panergos-agent/docs/developer-guide/
...
Config: `~/.panergos/config.yaml` (settings), `~/.panergos/.env` (API keys) — both under `$PANERGOS_HOME` when it is set.
...
`_PANERGOS_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_panergos_home()` for paths,
never hardcode `~/.panergos`. For custom/local-only tools, write a plugin in
`~/.panergos/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `panergos_cli/commands.py`
...
- Tests auto-redirect `PANERGOS_HOME` to temp dirs — never touch real `~/.panergos/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/panergos_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_panergos_home()` from `panergos_constants` for all paths (profile-safe)
