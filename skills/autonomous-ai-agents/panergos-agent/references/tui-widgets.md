...
Author widget apps for the Panergos TUI (`panergos --tui`): glanceable ambient
...
- The TUI must be in use (`panergos --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.panergos/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.panergos/logs/tui_gateway_crash.log` if a widget never appears.
