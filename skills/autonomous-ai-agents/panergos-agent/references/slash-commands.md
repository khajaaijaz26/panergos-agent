...
Registry of record: `panergos_cli/commands.py` (`COMMAND_REGISTRY`) — every
...
/snapshot [sub]          Create/restore Panergos config+state snapshots (CLI)
...
/update                  Update Panergos to latest
/version (/v)            Show version
/debug [local]           Generate a local debug report
```
