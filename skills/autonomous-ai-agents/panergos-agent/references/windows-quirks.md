...
Panergos runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`panergos config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Panergos's env scrubber dropping
...
Panergos-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Panergos tool and most Windows APIs. Prefer forward slashes in code
