...
Write plugins for the Panergos desktop app: statusbar items, layout panes,
...
system (`~/.panergos/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$PANERGOS_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$PANERGOS_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.panergos-package.json` marker) — the ONLY root the renderer loads from — so
...
- The Panergos desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$PANERGOS_HOME/desktop-plugins/` (usually
  `~/.panergos/desktop-plugins/`).

## How to Run

1. Create `$PANERGOS_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.panergos/...` by default, or `~/.panergos/profiles/<profile>/...` under a
...
The ONLY import surface is `@panergos/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Panergos
  ...
  (`panergos://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.panergos/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@panergos/plugin-sdk`, `react`, and
