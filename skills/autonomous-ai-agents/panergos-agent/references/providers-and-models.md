...
Set via `panergos model` (picker) or `panergos setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://khajaaijaz26.github.io/panergos-agent/docs/integrations/providers
...
| openai-codex | OAuth | `panergos auth add openai-codex` |
| qwen-oauth | OAuth | `panergos auth add qwen-oauth` |
| minimax-oauth | OAuth | `panergos auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`panergos auth`).
Fallback chain when the primary fails: `panergos fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`panergos_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   panergos config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
