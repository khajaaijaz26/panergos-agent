# Themes / Skins — Author a Panergos Color Theme

Author a Panergos **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`panergos_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.panergos/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Panergos itself.
...
- Write access to the Panergos home dir — `~/.panergos` by default, or `$PANERGOS_HOME`
  / the active profile's dir. Skins live in `$PANERGOS_HOME/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `panergos config set`).
...
3. `write_file` it to `$PANERGOS_HOME/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `panergos config set display.skin default` (or they can `/skin default`).
...
```
panergos skin set <key> <hex>      # e.g. panergos skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `panergos skin set background "#08201f"` changes
only the background; `panergos skin use <name>` / `panergos skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.panergos`** when a profile is active — resolve the real home
  from `$PANERGOS_HOME` first, falling back to `~/.panergos`.
...
- **Never hand-edit `config.yaml` to activate.** Use `panergos config set
...
- **You apply it, not the user.** `panergos config set display.skin <name>` is
...
- `read_file` the written `$PANERGOS_HOME/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `panergos config get display.skin` and confirm it reports `<name>`.
