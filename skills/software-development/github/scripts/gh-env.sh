#!/usr/bin/env bash
# GitHub environment detection helper for Panergos Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _panergos_env="${PANERGOS_HOME:-$HOME/.panergos}/.env"; [ -f "$_panergos_env" ] && grep -q "^GITHUB_TOKEN=" "$_panergos_env" 2>/dev/null; then
