...
- Give it a name like "panergos-agent"
- Select scopes:
...
- Give it a title like "panergos-agent-<machine-name>"
...
uv run python "${PANERGOS_HOME:-$HOME/.panergos}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _panergos_env="${PANERGOS_HOME:-$HOME/.panergos}/.env"; [ -f "$_panergos_env" ] && grep -q "^GITHUB_TOKEN=" "$_panergos_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_panergos_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python "${PANERGOS_HOME:-$HOME/.panergos}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
