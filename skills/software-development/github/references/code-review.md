...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _panergos_env="${PANERGOS_HOME:-$HOME/.panergos}/.env"; [ -f "$_panergos_env" ] && grep -q "^GITHUB_TOKEN=" "$_panergos_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_panergos_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python "${PANERGOS_HOME:-$HOME/.panergos}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
