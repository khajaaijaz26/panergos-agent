...
- **`optional-skills/`** — heavier/niche skills shipped but NOT active; installed via
  `panergos skills install official/<category>/<skill>` (adapter `tools/skills_hub_official.py`
...
`[linux, macos]`, ...), `metadata.panergos.tags`, `metadata.panergos.category`,
`metadata.panergos.related_skills`, `metadata.panergos.config` (config.yaml settings the skill needs —
stored under `skills.config.<key>`, prompted during setup, injected at load). Top-level `tags:` /
`category:` are accepted and mirrored from `metadata.panergos.*` by the loader.
...
2. **Prose references native Panergos tools or the MCP servers the skill expects, in backticks**
...
4. **`author` credits the human first.** External contributor's real name + GitHub handle first,
   "Panergos Agent" second. A commit authored as "Panergos Agent" (they drafted with Panergos) is replaced
...
in the `panergos-agent-dev` skill.

## Curator (skill lifecycle)

Background maintenance that tracks usage on agent-created skills and auto-archives stale ones;
archives go to `~/.panergos/skills/.archive/` and are restorable. Core `agent/curator.py` (review
loop, auto-transitions, LLM review prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots);
CLI `panergos_cli/curator.py` → `panergos curator status|run|pause|resume|pin|unpin|archive|restore|
prune|backup|rollback`; telemetry `tools/skill_usage.py` owns `~/.panergos/skills/.usage.json`
